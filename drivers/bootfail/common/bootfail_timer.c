...
static DEFINE_MUTEX(boot_timer_mutex);
				...
				struct boot_timer_info *ptimer_info)
{
	...
	if (atimer->action_timer_timing_count >=
		...
		panic("boot_timer: %s expired! boot fail", atimer->action_name);
#else
...
static void check_boot_time(struct boot_timer_info *ptimer_info)
{
	...
	if (ptimer_info->boot_timer_count_enable) {
		unsigned long jif_time =
			jiffies - ptimer_info->short_timer_last_start_time;
...
static void wakeup_boot_timer_count_task(struct timer_list *bf_timer)
{
	...
	ptimer_info =
		container_of(bf_timer, struct boot_timer_info, boot_timer);
	complete(&ptimer_info->boot_complete);
}
#endif
...
static __ref int boot_timer_count_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ptimer_info->boot_timer_count_task_should_run) {
			...
			mod_timer(&ptimer_info->boot_timer,
				  jiffies + delay_time);
		} else {
			print_info(
				"boot_timer: time thread will stop now:[boot_short_timer:%d]!\n",
...
static void __free_action_timer_list(struct list_head *atlist)
{
	...
	list_for_each_safe(pos, next, atlist) {
		...
		print_info(
			"WARNING:[%s:%d]%s act_timer in list(activetime:%d ms, timeout:%d ms)\n",
			atimer->action_comm, atimer->action_pid,
			atimer->action_name, atimer->action_timer_timing_count,
			atimer->action_timer_timeout_value);
...
int action_timer_stop(char *act_name)
{
	...
	if (atimer != NULL) {
		print_info(
			"have find out \"%s\" timer,so stop it!(activetime:%u ms, timeout:%u ms)\n",
			act_name, atimer->action_timer_timing_count,
			atimer->action_timer_timeout_value);
...
int action_timer_pause(char *act_name)
{
	...
	if (atimer->action_pid != task_pid_tmp)
		print_err("WARNING: pid mismatch pause[%s:%d]: start[%s:%d]!!",
			  task_comm_tmp, task_pid_tmp, atimer->action_comm,
			  atimer->action_pid);
...
int start_boot_timer(void)
{
	...
	ptimer_info->boot_time_task = kthread_create(boot_timer_count_kthread,
