...
static long boot_detector_ioctl(struct file *file, unsigned int cmd,
				unsigned long arg);
static int boot_detector_open(struct inode *inode, struct file *file);
static ssize_t boot_detector_write(struct file *file, const char *data,
				   size_t len, loff_t *ppos);
static ssize_t boot_detector_read(struct file *file, char __user *buf,
...
static long full_rw_file(struct file *fp, char *buf, size_t buf_size, bool read)
{
	...
	while (bytes_total_to_rw > 0) {
		bytes_this_time =
			read ? kernel_read(fp, ptemp, bytes_total_to_rw,
					   &(fp->f_pos)) :
			       kernel_write(fp, ptemp, bytes_total_to_rw,
					    &(fp->f_pos));
...
#endif
static int rw_part(const char *dev_path, unsigned long long offset, char *buf,
		   unsigned long long buf_size, bool read)
{
#ifndef CONFIG_BOOT_DETECTOR_GKI
	int ret = -1;
	long bytes_total;
...
static long full_rw_file(int fd, char *buf, size_t buf_size, bool read)
{
	...
	while (bytes_total_to_rw > 0) {
		bytes_this_time =
			read ? bf_sys_read(fd, ptemp, bytes_total_to_rw) :
...
int read_part(const char *dev_path, unsigned long long offset, char *buf,
...
int write_part(const char *dev_path, unsigned long long offset, const char *buf,
	       unsigned long long buf_size)
{
	if (dev_path == NULL || buf == NULL)
		return -1;
...
static long ioctl_stage(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GET_BOOT_STAGE:
		...
		if (copy_to_user((enum boot_stage *)(uintptr_t)arg, &stage,
				 sizeof(stage)) != 0) {
...
{
	return (cmd == GET_TIMER_STATUS || cmd == RESUME_TIMER ||
		cmd == SUSPEND_TIMER || cmd == GET_TIMER_TIMEOUT_VALUE ||
		cmd == SET_TIMER_TIMEOUT_VALUE);
}

static long ioctl_boot_timer(struct file *file, unsigned int cmd,
			     unsigned long arg)
{
	long ret = 0;
	uintptr_t args = arg;
...
static long ioctl_action_timer(struct file *file, unsigned int cmd,
			       unsigned long arg)
{
	long ret = 0;
...
static int process_upper_bootfail(unsigned long arg)
{
	...
	if (copy_from_user(&pparam,
			   ((struct bootfail_proc_param *)(uintptr_t)arg),
			   sizeof(pparam)) != 0) {
		print_err("copy_from_user failed!\n");
		return -EFAULT;
	}
...
static int process_bootfail(struct bootfail_proc_param *pparam)
{
	...
	print_info("bootfail_errno: 0x%x, detail_info: %s, bootup_time: %ds\n",
		   param->pparam.binfo.bootfail_errno,
		   param->pparam.detail_info, param->pparam.binfo.bootup_time);
	sema_init(&param->sem, 0);
	tsk = kthread_run(__process_upper_bootfail, (void *)param,
			  "proc_upper_bf");
	if (IS_ERR(tsk)) {
	...
	down(&param->sem);
	if (param->pparam.binfo.suggest_recovery_method == METHOD_NO_SUGGESTION)
...
static long get_key_struct_size(unsigned long arg)
{
	...
	if (copy_to_user((void *)(uintptr_t)arg, &size, sizeof(size)) != 0) {
...
static long ioctl_others(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
	int enable_flag;
...
static long ioctl_dynamic_brd(struct file *file, unsigned int cmd,
			      unsigned long arg)
{
	long ret = 0;
...
{
	return (cmd == IOC_CHECK_BOOTDISK_WP || cmd == IOC_ENABLE_MONITOR ||
...
static long ioctl_storage_rofa(struct file *file, unsigned int cmd,
			       unsigned long arg)
{
	long ret = 0;
...
int read_from_phys_mem(unsigned long dst, unsigned long dst_max,
...
int write_to_phys_mem(unsigned long dst, unsigned long dst_max, void *src,
		      unsigned long src_len)
{
	...
	/*lint -e446*/
	paddr = (char *)ioremap_uc((phys_addr_t)dst, bytes_to_write);
	/*lint +e446*/
	if (paddr == NULL)
		return -1;
...
{
	return blocking_notifier_chain_unregister(&long_press_notifier_list,
						  nb);
}
...
static size_t get_memory_available(char *buf, size_t size)
{
	...
	ret = snprintf_s(
		...
		get_sys_rtc_time(), stage, PAGE_TO_KB(mem_available),
...
static size_t get_process_stat(char *buf, size_t size)
{
	...
	for_each_process(p) { // stat of all processes
		...
		ret = snprintf_s(buf + len, size - len, size - len - 1,
				 ...
				 PAGE_TO_KB(get_mm_rss(task->mm)), task->comm);
		if (ret < 0) {
...
static void bootfail_longpress_dump(enum boot_stage stage)
{
	...
	pparam.binfo.suggest_recovery_method = METHOD_DO_NOTHING;
	strncpy_s(pparam.detail_info, sizeof(pparam.detail_info), "long_press",
		  strlen("long_press"));
...
static void formate_metadata(struct long_press_metadata *pmetadata)
{
	...
	memset_s(pmetadata->log_headers, sizeof(pmetadata->log_headers), 0,
		 sizeof(pmetadata->log_headers));
}
...
static bool update_metadata(struct long_press_metadata *pmetadata)
{
	...
	bootfail_sha256(pmetadata->sha256, SHA256_DATA_LEN,
...
static u32 get_next_offset(struct long_press_metadata *pmetadata, u32 log_size)
{
	...
	read_part(LONG_PRESS_DEV_PATH, 0, (char *)pmetadata,
		  sizeof(struct long_press_metadata));
	...
	if (cursor == 0 || tail >= LONG_PRESS_DUMP_MAX_COUNT ||
	    (offset < sizeof(struct long_press_metadata) +
			      FULLDUMP_HEADER_MAXSIZE ||
	     offset > LONG_PRESS_PARTITION_SIZE) ||
	    (offset + size + log_size >=
	     LONG_PRESS_PARTITION_SIZE)) { // if cut, reset index
		pmetadata->log_headers[0].offset =
			sizeof(struct long_press_metadata) +
			FULLDUMP_HEADER_MAXSIZE;
