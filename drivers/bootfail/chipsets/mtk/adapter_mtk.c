...
static int mtk_read_from_phys_mem(unsigned long dst, unsigned long dst_max,
				  void *phys_mem_addr, unsigned long data_len)
{
	...
	if (phys_mem_addr == NULL || dst == 0 || dst_max == 0 ||
		...
		print_invalid_params(
			"bootfail: dst: %u, dst_max: %u, data_len: %u\n", dst,
			dst_max, data_len);
		return -1;
	}
...
static int mtk_write_to_phys_mem(unsigned long dst, unsigned long dst_max,
				 void *src, unsigned long src_len)
{
	...
	if (src == NULL || dst == 0 || dst_max == 0 || src_len == 0) {
		print_invalid_params(
			"bootfail: dst: %u, dst_max: %u, src_len: %u\n", dst,
			dst_max, src_len);
		return -1;
	}
...
#else
	errno_t ret =
		memcpy_s(pbuf, buf_size, klog_addr, min(klog_size, buf_size));
	if (ret != EOK)
...
static void capture_pl_lk_log(char *pbuf, unsigned int buf_size)
{
	...
	pl_lk_log_size = plog_hdr->dram_curlog_header.sz_pl +
			 plog_hdr->dram_curlog_header.sz_lk;
	...
	ret = memcpy_s(pbuf, buf_size, pl_lk_log_addr,
		       min(pl_lk_log_size, buf_size));

	if (ret != EOK)
		print_err("memcpy_s failed, ret: %d\n", ret);
}

__weak long ksys_readlink(const char __user *path, char __user *buf,
			  int buf_size)
{
...
static void get_log_ops_info(struct adapter *padp)
{
	...
	padp->kernel_log_ops.log_size = BF_SIZE_2M;
