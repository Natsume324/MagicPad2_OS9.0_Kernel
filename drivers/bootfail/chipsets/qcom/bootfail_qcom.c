...
{
	if (p != NULL) {
		g_is_poweroff_charge =
			(strncmp(p, "charger", strlen("charger")) == 0) ? true :
									  false;
	}
...
long simulate_storge_rdonly(unsigned long arg)
{
	...
	if (copy_from_user(&ufs_type, (int *)(uintptr_t)arg, sizeof(int)) !=
	    0) {
 ...
 * @version 1.0
 */
static int process_panic_event(struct notifier_block *self, unsigned long val,
			       void *data)
{
	...
	if (check_recovery_mode() && mem_base != NULL) {
		...
		writel(is_erecovery_mode() ? PANIC_IN_ERECOVERY :
					     PANIC_IN_RECOVERY,
		       (char *)mem_base);
	} else if (mem_base != NULL && mem_size >= BF_SIZE_1K) {
		print_err("dump stack trace for panic @ %p!\n", mem_base);
		dump_stacktrace((char *)mem_base, BF_SIZE_1K - BF_BOPD_RESERVE);
