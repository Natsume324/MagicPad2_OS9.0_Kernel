...
*/
static const unsigned long boot_stage_offset =
	offsetof(struct bootfail_proc_param, binfo) +
	offsetof(struct bootfail_basic_info, stage);
...
static int qcom_read_from_phys_mem(unsigned long dst, unsigned long dst_max,
				   void *phys_mem_addr, unsigned long data_len)
{
	...
	if (phys_mem_addr == NULL || dst == 0 || dst_max == 0 ||
	    data_len == 0) {
		print_invalid_params(
			"bootfail: dst: %lu, dst_max: %lu, data_len: %lu\n",
...
static int qcom_write_to_phys_mem(unsigned long dst, unsigned long dst_max,
				  void *src, unsigned long src_len)
{
	...
	if (src == NULL || dst == 0 || dst_max == 0 || src_len == 0) {
		print_invalid_params(
			"bootfail: dst: %lu, dst_max: %lu, src_len: %lu\n", dst,
			dst_max, src_len);
		return -1;
	}
...
int bf_rmem_init(void)
{
	...
	bootfail_node = of_find_compatible_node(NULL, NULL, "bootfail_mem");
	if (bootfail_node == 0) {
	...
	print_err("g_bf_rmem_addr:%p, size:%lx\n", g_bf_rmem_addr,
		  g_bf_rmem_size);
	return BF_OK;
rmem_init_err:
	g_bf_rmem_addr = NULL;
...
static void capture_bl_log(char *pbuf, unsigned int buf_size)
{
	...
	if (bl_addr == NULL || bl_log_size > BL_LOG_MAX_LEN ||
...
static void get_log_ops_info(struct adapter *padp)
{
	errno_t ret;
	/* set bl log info */
	ret = strncpy_s(padp->bl_log_ops.log_name, BF_BL_LOG_NAME_SIZE,
			BL_LOG_NAME,
	...
	/* set kernel log info */
	ret = strncpy_s(padp->kernel_log_ops.log_name, BF_KERNEL_LOG_NAME_SIZE,
			KERNEL_LOG_NAME,
...
static void qcom_shutdown(const char *detail, unsigned int len)
{
	print_err("unsupported\n");
}

static void qcom_reboot(const char *detail, unsigned int len)
{
#if IS_ENABLED(CONFIG_RAINBOW_REASON)
	rb_sreason_set("bootfail"); // set sreason, do not change.
#endif
	// 8450 & 8550 not support warm reset, so use panic alternate
	if (detail == NULL || len < 1 || detail[len - 1] != 0 ||
	    strlen(detail) >= len) {
		panic("bootfail");
		return;
	}
...
static int qcom_set_boot_stage(int stage)
{
	...
	print_err("set boot stage:%x, offset=%lx\n", *(int *)addr,
		  boot_stage_offset);
	return BF_OK;
}
...
static int qcom_get_boot_stage(int *stage)
{
	...
	addr += BF_SIZE_1K + boot_stage_offset;
	qcom_read_from_phys_mem((unsigned long)(stage), sizeof(int),
				(void *)addr, sizeof(int));
...
static void save_long_press_meta_log(struct every_number_info *pinfo)
{
	...
	pinfo->last_kmsg_start_addr =
		pinfo->fastbootlog_start_addr + pinfo->fastbootlog_size;
	pinfo->last_kmsg_size = get_kmsg_buf_size();
	pinfo->last_applog_start_addr =
		pinfo->last_kmsg_start_addr + pinfo->last_kmsg_size;
