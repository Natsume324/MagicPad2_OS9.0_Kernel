...
#define RECORD_SIZE (RECORD_HDR_SIZE + RECORD_COUNT * sizeof(struct rrecord))
...
#define is_download(meth)                                              \
	(((meth) == BL1_DOWNLOAD) || ((meth) == BL2_DOWNLOAD) ||       \
	 ((meth) == KERNEL_DOWNLOAD) || ((meth) == NATIVE_DOWNLOAD) || \
	 ((meth) == FRAMEWORK_DOWNLOAD))
#define is_install_succ(status) ((status) == INSTALL_PACKAGE_SUCC)
#define is_download_succ(method, status) \
	(is_download(method) && is_install_succ(status))
...
static struct fixed_method_map fixed_method[] = {
	{
		{BL1_CORE_DEV_FAULT, BL1_NON_CORE_DEV_FAULT, BL1_IMG_FAULT},
		{/* 1 <= bootfail times < 2 */
		 {1, 2, BL1_REBOOT},
		 /* 2 <= bootfail times < 3 */
		 {2, 3, BL1_PREVENTION},
	...
	{
		{BL2_CORE_DEV_FAULT, BL2_NON_CORE_DEV_FAULT, BL2_IMG_FAULT},
		{/* 1 <= bootfail times < 2 */
		 {1, 2, BL2_REBOOT},
		 /* 2 <= bootfail times < 3 */
		 {2, 3, BL2_PREVENTION},
	...
	{
		{KERNEL_CORE_DEV_FAULT, KERNEL_NON_CORE_DEV_FAULT,
		 KERNEL_IMG_FAULT},
		{/* 1 <= bootfail times < 2 */
		 {1, 2, KERNEL_REBOOT},
		 /* 2 <= bootfail times < 3 */
	...
	{
		{NATIVE_CORE_DEV_FAULT, NATIVE_NON_CORE_DEV_FAULT,
	...
	{
		{NATIVE_DATA_DAMAGED, NATIVE_DATA_DAMAGED, NATIVE_DATA_DAMAGED},
		{/* 1 <= bootfail times < 2 */
		 {1, 2, NATIVE_REBOOT},
		 /* 2 <= bootfail times < 3 */
		 {2, 3, NATIVE_DOWNLOAD},
	...
	{
		{FRAMEWORK_CORE_DEV_FAULT, FRAMEWORK_NON_CORE_DEV_FAULT,
	...
	{
		{HARDWARE_STORAGE_FAULT, HARDWARE_STORAGE_FAULT,
...
static struct errno_to_bi errno_to_bi_tbl[] = {
	...
	{{NATIVE_PART_MOUNT_FAIL, NATIVE_PART_MOUNT_FAIL}, BI_DEG_BOOT_FAIL},
	{{NATIVE_SERVICE_CRASH, NATIVE_SERVICE_CRASH}, BI_KEY_SVC_START_FAIL},
	{{NATIVE_DATA_PART_MOUNT_FAIL, NATIVE_DATA_PART_MOUNT_FAIL},
	 BI_DATA_MNT_FAIL},
...
static void validate_rrecord(struct recovery_run_param *pparam)
{
	...
	struct rrecord_header *pbackup_hdr =
		(struct rrecord_header *)&prrecord->backup_hdr;
...
static enum bootfail_errorcode verify_rrecord(struct recovery_run_param *pparam)
{
	...
	pparam->padp->sec_funcs.memset_s(sha256, (unsigned long)sizeof(sha256),
...
{
	return (method == KERNEL_BOPD || method == NATIVE_BOPD ||
...
static enum bootfail_errorcode run_erecovery(struct recovery_run_param *pparam)
{
	...
	pparam->padp->sec_funcs.memset_s(&hdr, (unsigned long)sizeof(hdr), 0,
					 (unsigned long)sizeof(hdr));
	/*lint -e666*/
	pparam->padp->sec_funcs.strncpy_s(
		hdr.misc_msg.command,
		(unsigned long)sizeof(hdr.misc_msg.command), BOOTFAIL_RECOVERY,
		(unsigned long)min(sizeof(hdr.misc_msg.command) - 1,
				   pparam->padp->sec_funcs.bootfail_strlen(
					   BOOTFAIL_RECOVERY)));
	/*lint +e666*/
	...
	return pparam->padp->bfi_part.ops.write(pparam->padp->bfi_part.dev_path,
						0, (char *)&hdr, sizeof(hdr));
}
...
static enum bootfail_errorcode run_bopd(struct recovery_run_param *pparam)
{
	...
	pparam->padp->sec_funcs.strncpy_s(
		...
		(unsigned long)min(
			...
			pparam->padp->sec_funcs.bootfail_strlen(BOOT_BOPD)));
	/*lint +e666*/
	hdr.misc_msg.bootfail_errno = pparam->pparam->binfo.bootfail_errno;
...
{
	return (method == BL1_PREVENTION || method == BL2_PREVENTION ||
		method == KERNEL_PREVENTION || method == NATIVE_PREVENTION ||
		method == FRAMEWORK_PREVENTION);
}

static int is_erecovery(enum recovery_method method)
{
	return (method == BL1_DOWNLOAD || method == BL2_DOWNLOAD ||
		method == KERNEL_FACTORY_REST || method == KERNEL_DOWNLOAD ||
		method == NATIVE_FACTORY_REST || method == NATIVE_DOWNLOAD ||
...
{
	return (method == KERNEL_BOPD || method == KERNEL_UFS_RDONLY_BOPD ||
...
{
	return (method == NATIVE_BOPD || method == NATIVE_UFS_RDONLY_BOPD ||
		method == NATIVE_DEGRADE_BOPD ||
		method == NATIVE_BOPD_AFTER_DOWNLOAD ||
		method == NATIVE_DEGRADE_BOPD_AFTER_DOWNLOAD ||
		method == NATIVE_UFS_RDONLY_DEGRADE_BOPD);
...
{
	return (method == FRAMEWORK_SAFE_MODE || method == NATIVE_SAFE_MODE ||
...
static enum bootfail_errorcode save_rrecord(struct recovery_run_param *pparam)
{
	...
	struct rrecord_header *pbackup_hdr =
		(struct rrecord_header *)((char *)prrecord + RECORD2_OFFSET);
	struct rrecord *precord =
		(struct rrecord *)((char *)phdr + RECORD_HDR_SIZE +
				   phdr->write_idx * sizeof(*precord));
...
static void parse_kernel_bootfail_further(struct select_param *pparam)
{
	...
	for (i = 0; i < (int)ARRAY_SIZE(prrecord->rrecord); i++) {
		struct rrecord *precord =
			(struct rrecord *)(&prrecord->rrecord[i]);
...
static void parse_bootloader_bootfail_further(struct select_param *pparam)
{
	...
	for (i = 0; i < pparam->record_info.count; i++) {
		struct rrecord *precord =
			(struct rrecord *)(pparam->record_info.pstart +
					   i * sizeof(struct rrecord));
...
static void parse_bootloader_bootfail(struct select_param *pparam)
{
	...
	if (phdr->write_idx >= bootfail_count) {
		pparam->record_info.pstart =
			(char *)prrecord + RECORD_HDR_SIZE +
			(phdr->write_idx - bootfail_count) *
		...
		pparam->record_info.pstart =
			(char *)prrecord + RECORD_SIZE -
			count_of_end * sizeof(struct rrecord);
...
static void select_upper_method_with_bopd(struct select_param *pparam)
{
...
static void select_bottom_method_with_bopd(struct select_param *pparam)
{
...
select_method_with_bopd(struct select_param *pparam)
{
	if (pparam->is_download_succ != 0) {
	...
	pparam->rparam->padp->print(
		"func: select_method_with_bopd, "
...
static enum bootfail_errorcode
select_method_without_bopd(struct select_param *pparam)
{
	if (pparam->is_download_succ != 0) {
		pparam->rparam->padp->print(
			"func: "
...
static enum bootfail_errorcode select_method(struct recovery_run_param *pparam)
{
	...
	pparam->padp->sec_funcs.memset_s(&sparam, (unsigned long)sizeof(sparam),
					 0, (unsigned long)sizeof(sparam));
...
static enum bootfail_errorcode read_rrecord(struct recovery_run_param *pparam)
{
	...
	if (pparam->padp->bfi_part.ops.read(
		    pparam->padp->bfi_part.dev_path, pparam->rrecord_offset,
		    (char *)prrecord, RECORD_SIZE_TOTAL) != 0) {
...
static enum bootfail_errorcode
select_method_for_bootloader(struct recovery_run_param *pparam)
{
...
static enum bootfail_errorcode
select_method_for_kernel(struct recovery_run_param *pparam)
{
...
static enum bootfail_errorcode
select_and_run_recovery(struct recovery_run_param *pparam)
{
...
enum bootfail_errorcode do_recovery(struct recovery_run_param *pparam)
{
	...
	if (pparam->padp->is_ab_update != NULL &&
	    pparam->padp->is_ab_update() == 0) {
		pparam->pparam->binfo.suggest_recovery_method =
			METHOD_DO_NOTHING;
	}
#endif
