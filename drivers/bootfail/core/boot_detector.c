...
/* ----local macroes ---- */
#define BFI_PART_MIN_SIZE                                      \
	(BFI_PART_HDR_SIZE + BF_INFO_SIZE + BF_META_LOG_SIZE + \
	 sizeof(struct bootfail_log_header) + sizeof(struct recovery_info))
...
static enum bootfail_errorcode
__boot_fail_error(struct bootfail_proc_param *pparam);
...
static int is_valid_log_meta_info(struct bootfail_meta_log *pinfo)
{
	...
	run_param.adp.sec_funcs.memset_s(sha256, (unsigned long)sizeof(sha256),
					 0, (unsigned long)sizeof(sha256));
	run_param.adp.sha256(sha256, sizeof(sha256),
			     ((unsigned char *)pinfo + sizeof(pinfo->sha256)),
			     BF_META_LOG_SIZE - sizeof(pinfo->sha256));
	return (run_param.adp.sec_funcs.memcmp(sha256, pinfo->sha256,
					       sizeof(sha256)) == 0) ?
		       1 :
		       0;
}
...
static void update_log_meta_info(struct bootfail_meta_log *pinfo)
{
	run_param.adp.sha256(pinfo->sha256, sizeof(pinfo->sha256),
			     ((unsigned char *)pinfo + sizeof(pinfo->sha256)),
			     BF_META_LOG_SIZE - sizeof(pinfo->sha256));
	(void)run_param.adp.bfi_part.ops.write(run_param.adp.bfi_part.dev_path,
					       BF_META_LOG_OFFSET,
					       (char *)pinfo, BF_META_LOG_SIZE);
}

static void calc_log_max_count(struct bootfail_meta_log *pinfo)
{
	pinfo->log_max_count =
		(run_param.adp.bfi_part.part_size - BFI_PART_HDR_SIZE -
...
{
	pinfo->rrecord_offset =
		(unsigned int)(BFI_PART_HDR_SIZE + BF_INFO_SIZE +
			       BF_META_LOG_SIZE +
			       pinfo->log_max_count *
				       (sizeof(struct bootfail_log_header) +
					run_param.adp.bl_log_ops.log_size +
...
static void init_bootfail_part(struct bootfail_meta_log *pinfo)
{
	...
	for (i = 0; i < pinfo->log_max_count; i++) {
		run_param.adp.sec_funcs.strncpy_s(
			...
			(unsigned long)run_param.adp.sec_funcs.bootfail_strlen(
				run_param.adp.bl_log_ops.log_name));
		      ...
		      struct bootfail_log *plog)
{
	...
	if (pinfo->write_idx < pinfo->log_max_count) {
		...
		/* update meta log */
		pinfo->write_idx =
			(pinfo->write_idx + 1) % pinfo->log_max_count;
		pinfo->log_count++;
...
static void post_proc_kernel_bootfail(struct recovery_run_param *rparam,
				      struct bootfail_proc_param *pparam,
				      int rw_storage_succ)
{
	...
	if (rw_storage_succ == 0 ||
	    run_param.adp.bfi_part.ops.write(run_param.adp.bfi_part.dev_path,
					     BF_INFO_OFFSET, (char *)pparam,
					     sizeof(*pparam)) != 0) {
		run_param.adp.pyhs_mem_info.ops.write(
			run_param.adp.pyhs_mem_info.base +
				BOOTFAIL_INFO_OFFSET_IN_PHYMEM,
			run_param.adp.pyhs_mem_info.size, (char *)pparam,
			sizeof(*pparam));
		run_param.adp.print("bootfail: write to phys mem errno=%d\n",
				    pparam->binfo.bootfail_errno);
	}
	msleep(200);
	run_param.adp.sys_ctl.reboot(pparam->detail_info,
				     BOOTFAIL_DETAIL_INFO_LEN);
}

static void post_proc_bootloader_bootfail(struct recovery_run_param *rparam,
					  struct bootfail_proc_param *pparam)
{
	if (rparam->run_prevention != NULL)
		rparam->run_prevention(rparam);
	...
	if (pparam->binfo.post_action == PA_REBOOT &&
		...
		run_param.adp.sys_ctl.shutdown(pparam->detail_info,
					       BOOTFAIL_DETAIL_INFO_LEN);
	}
}

static void post_proc_bootfail(struct recovery_run_param *rparam,
			       struct bootfail_proc_param *pparam,
			       int rw_storage_succ)
{
	if (run_param.adp.is_bootloader() != 0)
		...
		post_proc_kernel_bootfail(rparam, pparam, rw_storage_succ);
}

static enum bootfail_errorcode
alloc_mem_for_bootfail_log(struct bootfail_log **plog)
{
	unsigned int log_size;

	log_size = sizeof(**plog) + run_param.adp.bl_log_ops.log_size +
		   run_param.adp.kernel_log_ops.log_size;
	*plog = run_param.adp.mem_ops.malloc(log_size);
	if (*plog == NULL) {
		run_param.adp.print(
			"func: alloc_mem_for_bootfail_log,"
			"error: %d(BF_ALLOC_MEM_FAIL), malloc size: 0x%lx\n",
			BF_ALLOC_MEM_FAIL, (unsigned long)log_size);
		return BF_ALLOC_MEM_FAIL;
	}
	run_param.adp.sec_funcs.memset_s(*plog, (unsigned long)log_size, 0,
			       ...
			       struct bootfail_log_header *phdr)
{
	...
	run_param.adp.sec_funcs.memcpy_s((char *)&phdr->binfo,
					 ...
					 (char *)&pparam->binfo,
					 (unsigned long)sizeof(phdr->binfo));
	run_param.adp.sec_funcs.memcpy_s(
		phdr->detail_info, (unsigned long)sizeof(phdr->detail_info),
		pparam->detail_info, (unsigned long)sizeof(phdr->detail_info));
	run_param.adp.sec_funcs.memcpy_s(phdr->log_path,
					 (unsigned long)sizeof(phdr->log_path),
					 pparam->log_path,
					 (unsigned long)sizeof(phdr->log_path));
	run_param.adp.sec_funcs.memcpy_s(
		phdr->part_name, (unsigned long)sizeof(phdr->part_name),
		pparam->part_name, (unsigned long)sizeof(phdr->part_name));
	run_param.adp.sec_funcs.memcpy_s(
		phdr->hardware_detail,
...
static enum bootfail_errorcode proc_bootfail(struct bootfail_proc_param *pparam,
					     struct bootfail_meta_log *pinfo,
					     int rw_storage_succ)
{
	...
	run_param.adp.sec_funcs.memset_s((void *)phdr,
					 (unsigned long)sizeof(*phdr), 0,
					 (unsigned long)sizeof(*phdr));
	copy_bootfail_info(pparam, phdr);
	run_param.adp.sec_funcs.memset_s((char *)&rparam,
					 (unsigned long)sizeof(rparam), 0,
					 (unsigned long)sizeof(rparam));
	rparam.rrecord_offset =
		rw_storage_succ ? (unsigned long long)pinfo->rrecord_offset :
				  (INVALID_RRECORD_OFFSET);
	rparam.pparam = pparam;
	rparam.padp = &run_param.adp;
	(void)do_recovery(&rparam);
	phdr->binfo.recovery_method = rparam.method;
	pparam->binfo.is_perceptible =
...
static int is_valid_magic_number(unsigned int magic_num)
{
	return (magic_num == BF_INJECT_MAGIC_NUM ||
		magic_num == BF_SW_MAGIC_NUM || magic_num == BF_HW_MAGIC_NUM) ?
		       1 :
		       0;
}

static int is_valid_bootfail_injected(struct bootfail_proc_param *pparam)
{
	if (pparam->magic == BF_INJECT_MAGIC_NUM)
...
static void update_bootfail_proc_param(struct bootfail_proc_param *pparam)
{
	if (pparam->magic == 0)
		run_param.adp.sec_funcs.memset_s(
			pparam, (unsigned long)sizeof(*pparam), 0,
			(unsigned long)sizeof(*pparam));

	if (run_param.bootfail_saved_in_mem != 0) {
		(void)run_param.adp.pyhs_mem_info.ops.write(
			run_param.adp.pyhs_mem_info.base,
			run_param.adp.pyhs_mem_info.size, pparam,
			sizeof(*pparam));
	} else {
		(void)run_param.adp.bfi_part.ops.write(
...
static enum bootfail_errorcode
proc_last_bootfail(struct bootfail_proc_param *pparam)
{
	...
	pparam_local->bootfail_count = (pparam->magic == BF_INJECT_MAGIC_NUM) ?
					       (pparam->bootfail_count - 1) :
					       (0);
	pparam_local->magic =
		(pparam_local->bootfail_count <= 0) ? (0) : (pparam->magic);
...
static void read_bootfail_from_pyhs_mem(void)
{
	...
	run_param.adp.sec_funcs.memset_s(pparam, (unsigned long)sizeof(*pparam),
					 0, (unsigned long)sizeof(*pparam));
	run_param.adp.pyhs_mem_info.ops.read(
		(unsigned long)(pparam), sizeof(*pparam),
		(void *)run_param.adp.pyhs_mem_info.base,
		run_param.adp.pyhs_mem_info.size);
	pparam->magic = BF_SW_MAGIC_NUM;
	/*lint -e666*/
	run_param.adp.sec_funcs.strncpy_s(
		pparam->detail_info, (unsigned long)sizeof(pparam->detail_info),
		DEFAULT_DETAIL_INFO,
		(unsigned long)min(run_param.adp.sec_funcs.bootfail_strlen(
					   DEFAULT_DETAIL_INFO),
				   sizeof(pparam->detail_info) - sizeof(char)));
	/*lint +e666*/
	...
	run_param.adp.sec_funcs.memcpy_s(
		&run_param.pparam, (unsigned long)sizeof(run_param.pparam),
...
static enum bootfail_errorcode
proc_upper_layer_bootfail(const struct bootfail_proc_param *phelper_pparam)
{
	enum bootfail_errorcode errcode;
...
enum bootfail_errorcode get_boot_stage(enum boot_stage *pstage)
{
	...
	if (run_param.is_init_succ == 0) {
		if (run_param.adp.print != NULL)
			run_param.adp.print("func: get_boot_stage, "
...
static enum bootfail_errorcode
check_condition_for_boot_fail_err(struct bootfail_proc_param *pparam)
{
	if (pparam == NULL) {
		run_param.adp.print("func: check_condition_for_boot_fail_err, "
				    "error: %d(BF_INVALID_PARM)\n",
				    BF_INVALID_PARM);
		return BF_INVALID_PARM;
	}
...
static int is_valid_log_name(const struct adapter *padp)
{
	if ((padp->bl_log_ops.log_size > 0) &&
	    (padp->sec_funcs.bootfail_strlen(padp->bl_log_ops.log_name) == 0))
		return 0;

	if ((padp->kernel_log_ops.log_size > 0) &&
	    (padp->sec_funcs.bootfail_strlen(padp->kernel_log_ops.log_name) ==
	     0))
		return 0;
...
static enum bootfail_errorcode
check_param_for_raw_part(const struct adapter *padp)
{
	...
	if ((BFI_PART_HDR_SIZE + BF_INFO_SIZE + BF_META_LOG_SIZE +
	     sizeof(struct bootfail_log_header) + padp->bl_log_ops.log_size +
	     padp->kernel_log_ops.log_size + sizeof(struct recovery_info)) >
	    padp->bfi_part.part_size)
		return BF_INVALID_DEV_PATH_OR_SIZE;
...
static enum bootfail_errorcode
check_raw_part_ops_ptrs(const struct adapter *padp)
{
	if (padp->bfi_part.ops.read == NULL || padp->bfi_part.ops.write == NULL)
...
static enum bootfail_errorcode
check_pyhs_mem_ops_ptrs(const struct adapter *padp)
{
...
static enum bootfail_errorcode check_sec_funs_ptrs(const struct adapter *padp)
{
	if (padp->sec_funcs.memcpy_s == NULL ||
	    padp->sec_funcs.memset_s == NULL ||
	    padp->sec_funcs.strncpy_s == NULL ||
	    padp->sec_funcs.memcmp == NULL ||
	    padp->sec_funcs.bootfail_strlen == NULL)
		return BF_INVALID_FUNC_PTR;

	return BF_OK;
}

static enum bootfail_errorcode check_mem_ops_ptrs(const struct adapter *padp)
{
	if (padp->mem_ops.malloc == NULL || padp->mem_ops.free == NULL)
		return BF_INVALID_FUNC_PTR;

	return BF_OK;
}

static enum bootfail_errorcode check_sys_ctl_ptrs(const struct adapter *padp)
{
	if (padp->sys_ctl.reboot == NULL || padp->sys_ctl.shutdown == NULL)
		return BF_INVALID_FUNC_PTR;

	return BF_OK;
}

static enum bootfail_errorcode check_stage_ops_ptrs(const struct adapter *padp)
{
...
static enum bootfail_errorcode check_verify_ops_ptrs(const struct adapter *padp)
{
	if (padp->sha256 == NULL)
		return BF_INVALID_FUNC_PTR;
...
static enum bootfail_errorcode
check_feature_config_ops_ptrs(const struct adapter *padp)
{
...
static enum bootfail_errorcode check_mutex_ops_ptrs(const struct adapter *padp)
{
	if (padp->mutex_ops.lock == NULL || padp->mutex_ops.unlock == NULL)
		return BF_INVALID_FUNC_PTR;

	return BF_OK;
}

static enum bootfail_errorcode check_func_ptrs(const struct adapter *padp)
{
	if (check_raw_part_ops_ptrs(padp) != BF_OK ||
	    ...
	    padp->save_misc_msg == NULL || padp->print == NULL)
		return BF_INVALID_FUNC_PTR;
...
static enum bootfail_errorcode
check_param_for_bootloader(const struct adapter *padp)
{
...
static enum bootfail_errorcode
check_param_for_pyhs_mem(const struct adapter *padp)
{
...
static enum bootfail_errorcode
check_param_for_log_name(const struct adapter *padp)
{
...
static enum bootfail_errorcode check_params(const struct adapter *padp)
{
	enum bootfail_errorcode errorcode;
...
enum bootfail_errorcode boot_detector_init(struct adapter *padp)
{
	...
	padp->sec_funcs.memcpy_s(&run_param.adp,
				 (unsigned long)sizeof(run_param.adp), padp,
				 (unsigned long)sizeof(*padp));
