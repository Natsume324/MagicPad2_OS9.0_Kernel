...
static int hiview_hievent_convert_string(struct hiview_hievent *event,
					 char **buf_ptr);

static struct hiview_hievent_payload *hiview_hievent_payload_create(void);

static void
hiview_hievent_payload_destroy(struct hiview_hievent_payload *payload);

static struct hiview_hievent_payload *
hiview_hievent_get_payload(struct hiview_hievent_payload *head,
			   const char *key);

static void hiview_hievent_add_payload(struct hiview_hievent *event,
...
static struct hiview_hievent_payload *
hiview_hievent_get_payload(struct hiview_hievent_payload *head, const char *key)
{
...
int hiview_hievent_put_integral(struct hiview_hievent *event, const char *key,
				long long value)
{
	struct hiview_hievent_payload *payload = NULL;
	...
	if (!payload->value)
		return -ENOMEM;
	if (memset_s(payload->value, INT_TYPE_MAX_LEN, 0, INT_TYPE_MAX_LEN) !=
	    EOK)
		return -ENOMEM;
	if (snprintf_s(payload->value, INT_TYPE_MAX_LEN, INT_TYPE_MAX_LEN - 1,
		       "%lld", value) < 0)
		return -ENOMEM;
...
int hiview_hievent_put_string(struct hiview_hievent *event, const char *key,
			      const char *value)
{
	struct hiview_hievent_payload *payload = NULL;
	int len;
				 ...
				 const char *value_array[], int tag_num)
{
	...
	for (tempi = 0; tempi < tag_num; tempi++) {
		...
		tmp_len =
			snprintf_s(tmp, len + 1, len, "%s", value_array[tempi]);
		BUF_POINTER_FORWARD;
	}
					 ...
					 char **buf_ptr)
{
	...
	if (event->time) {
		tmp_len =
			snprintf_s(tmp, len + 1, len, " -t %lld", event->time);
		BUF_POINTER_FORWARD;
	} else {
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0)
		do_gettimeofday(&tv);
		event->time = tv.tv_sec * TIME_CONVERSION_UNIT +
			      tv.tv_usec / TIME_CONVERSION_UNIT;
#else
		ktime_get_real_ts64(&tv);
		event->time = tv.tv_sec * TIME_CONVERSION_UNIT +
			      tv.tv_nsec / TIME_NS_TO_MS;
#endif
		tmp_len =
			snprintf_s(tmp, len + 1, len, " -t %lld", event->time);
		hilog_debug(
			"%s eventid = %d event->time is 0, set event->time = %lld",
			__func__, event->event_id, event->time);
		BUF_POINTER_FORWARD;
	}
...
static int hiview_hievent_write_log_exception(char *str, const int str_len)
{
	...
	if (left_buf_len > EVENT_INFO_PACK_BUF_LEN) {
		...
		logbuf_to_exception(0, 0, IDAP_LOGTYPE_CMD, 0, strptr,
				    left_buf_len);
#else
