...
static inline struct hiview_fs_work_t *
create_work_item(struct common_audit_data *cad)
{
...
#ifdef CONFIG_HONOR_HIVIEW_SELINUX_PERFORMANCE
			pr_devel(
				"%s larger than excepted, droped:%d, queued:%d!\n",
				__func__, droped_worker,
				atomic_read(&s_c_queued));
#endif
			return 1;
		}
	}

	if (atomic_read(&s_c_queued) - atomic_read(&s_c_finished) <
	    MAX_WORKER) {
