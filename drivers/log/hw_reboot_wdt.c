...
static void insert_pid_list(int pid)
{
	...
	for (i = 0; i < LIST_MAX; i++) {
		if (gs_pid_list[i].pid == 0 && gs_pid_list[i].name[0] == '\0') {
			gs_pid_list[i].pid = pid;
			memcpy_s(gs_pid_list[i].name, NAME_MAX, t->comm,
				 strlen(t->comm));
...
static ssize_t recv_pid(struct file *file, const char __user *buf, size_t count,
			loff_t *ppos)
{
	char mybuf[LIST_MAX];
	unsigned int pid = 0;
...
int reboot_wdt_module_init(void)
{
	...
	proc_file =
		proc_create("reboot_watchdog", 0200, NULL, &config_node_fops);
