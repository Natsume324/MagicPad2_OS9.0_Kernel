...
#define LOGGER_LOG_PICTURE_DEL_RECORD_BUF_SIZE (32 * 1024)
					     ...
					     struct logger_entry *scratch)
{
	...
	if (len != sizeof(*scratch)) {
		err = memcpy_s(((void *)scratch), sizeof(struct logger_entry),
			       log->buffer + off, len);
		if (unlikely(err != EOK)) {
			pr_err_ratelimited("memcpy scratch failed\n");
		}
		err = memcpy_s(((void *)scratch) + len,
			       sizeof(struct logger_entry) - len, log->buffer,
			       sizeof(*scratch) - len);
		if (unlikely(err != EOK)) {
			pr_err_ratelimited("memcpy scratch failed\n");
		}
...
static void do_write_log(struct logger_log *log, const void *buf, size_t count)
{
	...
	err = memcpy_s(log->buffer + log->w_off, log->size - log->w_off, buf,
		       len);
	if (unlikely(err != EOK)) {
		pr_err_ratelimited("memcpy log buffer failed\n");
	}

	if (count != len) {
		err = memcpy_s(log->buffer, log->size, buf + len, count - len);
		if (unlikely(err != EOK)) {
			pr_err_ratelimited("memcpy log buffer failed\n");
		}
	}
...
static long zrhung_ioctl_notify(struct file *file, unsigned int cmd,
				unsigned long arg)
{
	...
	ioctl_entry.arg = arg;
	ioctl_entry.ret = &ret;
	blocking_notifier_call_chain(&logger_notifier_list,
				     (unsigned long)&ioctl_entry, NULL);
...
static long logger_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LOGGER_RESET_RDDEL:
		...
		break;
	case FIONREAD:
		if (!strncmp(log->misc.name, LOGGER_LOG_JANK,
			     strlen(LOGGER_LOG_JANK)) ||
		    !strncmp(log->misc.name, LOGGER_LOG_DUBAI,
			     strlen(LOGGER_LOG_DUBAI)))
			ret = -ENOTTY;
		break;
	default:
		break;
...
static int __init logger_init(void)
{
	...
	if (unlikely(ret))
		goto out;
#endif
	ret = create_log(LOGGER_LOG_PICTURE_DEL_RECORD,
			 LOGGER_LOG_PICTURE_DEL_RECORD_BUF_SIZE);
	if (unlikely(ret))
		goto out;
	hwlog_wq_init();
			    ...
			    ssize_t vec_size)
{
	...
	if (unlikely(iov[0].iov_len != sizeof(int)))
		return -EINVAL;
	err = memcpy_s(&code, sizeof(int), iov[0].iov_base, sizeof(int));
	if (err != EOK) {
		return -EINVAL;
	}
	...
	init_header(&header);
	header.len =
		min(calc_iovc_ki_left(iov, vec_size), LOGGER_ENTRY_MAX_PAYLOAD);
	if (unlikely(!header.len)) {
		return 0;
	}
...
ssize_t log_file_operation(struct iovec *vec, ssize_t vec_size, int32_t pid,
			   int32_t tid)
{
	int code;
	ssize_t ret = 0;
	...
	ktime_get_coarse_real_ts64(&now);
	log = get_log_from_name(LOGGER_LOG_PICTURE_DEL_RECORD);
	if (unlikely(log == NULL || vec_size < 1))
		return -EINVAL;
	if (unlikely(iov[0].iov_len != sizeof(int)))
		return -EINVAL;
