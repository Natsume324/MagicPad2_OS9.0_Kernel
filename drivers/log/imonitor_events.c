...
#define check_v1_api(obj)                                                   \
	do {                                                                \
		if ((obj)->api_version == 0) {                              \
			(obj)->api_version = 1;                             \
		} else if ((obj)->api_version ==                            \
			   2) { /* 2 means api version */                   \
			hilog_err("cannot use v1 api %s after v2 api used", \
				  __func__);                                \
...
#define check_v2_api(obj)                                                   \
	do {                                                                \
		if ((obj)->api_version == 0) {                              \
			...
			hilog_err("cannot use v2 api %s after v1 api used", \
				  __func__);                                \
			return INVALID_VERSION;                             \
		}                                                           \
	} while (0)
...
int imonitor_set_param_integer(struct imonitor_eventobj *event_obj,
			       unsigned short param_id, long value);
int imonitor_set_param_string(struct imonitor_eventobj *event_obj,
			      unsigned short param_id, const char *value);
#endif

int imonitor_unset_param(struct imonitor_eventobj *event_obj,
			 unsigned short param_id);

#ifndef IMONITOR_NEW_API_V2
int imonitor_set_param_integer_v2(struct imonitor_eventobj *event_obj,
				  const char *param, long value);
int imonitor_set_param_string_v2(struct imonitor_eventobj *event_obj,
				 const char *param, const char *value);
int imonitor_unset_param_v2(struct imonitor_eventobj *event_obj,
			    const char *param);
#endif

static int imonitor_convert_string(struct imonitor_eventobj *event_obj,
				   char **buf_ptr);
...
static struct imonitor_param *get_imonitor_param(struct imonitor_param *head,
						 int key, const char *key_s);
static void del_imonitor_param(struct imonitor_eventobj *event_obj, int key,
			       const char *key_s);
static void add_imonitor_param(struct imonitor_eventobj *event_obj,
...
struct imonitor_eventobj *imonitor_create_eventobj(unsigned int event_id)
{
	...
	for (i = 0; i < imonitor_modules_count; i++) {
		if ((event_id >= imonitor_modules_table[i].min_eventid) &&
		    (event_id <= imonitor_modules_table[i].max_eventid)) {
	...
	if (memset_s(event_obj, sizeof(*event_obj), 0, sizeof(*event_obj)) !=
	    EOK) {
		vfree(event_obj);
		return NULL;
	}
...
int imonitor_set_param(struct imonitor_eventobj *event_obj,
		       unsigned short param_id, long value)
{
	unsigned short type;
	...
	case BIT_TYPE:
		if (((char)value) > 1) {
			hilog_err("Invalid value for bit type, value: %d",
				  (int)value);
			return -EINVAL;
		}
		fallthrough;
	...
	case DATETIME_TYPE:
		if (value == 0) {
			...
			imonitor_set_param_string(event_obj, param_id,
						  (char *)value);
		}
		break;
	case CLASS_TYPE:
			       ...
			       unsigned short param_id, long value)
{
	...
	if (!param->value)
		return -ENOMEM;
	if (memset_s(param->value, INT_TYPE_MAX_LEN, 0, INT_TYPE_MAX_LEN) !=
	    EOK)
		return -ENOMEM;
	if (snprintf_s(param->value, INT_TYPE_MAX_LEN, INT_TYPE_MAX_LEN - 1,
		       "%d", (int)value) < 0)
		return -ENOMEM;
				  ...
				  const char *param, long value)
{
	...
	if (!i_param->value)
		return -ENOMEM;
	if (memset_s(i_param->value, INT_TYPE_MAX_LEN, 0, INT_TYPE_MAX_LEN) !=
	    EOK)
		return -ENOMEM;
	if (snprintf_s(i_param->value, INT_TYPE_MAX_LEN, INT_TYPE_MAX_LEN - 1,
		       "%d", (int)value) < 0)
		return -ENOMEM;
...
int imonitor_add_dynamic_path(struct imonitor_eventobj *event_obj,
			      const char *path)
{
	if (!event_obj) {
		hilog_err("Bad param %s", __func__);
...
int imonitor_add_and_del_dynamic_path(struct imonitor_eventobj *event_obj,
				      const char *path)
{
	if (!event_obj) {
		hilog_err("Bad param %s", __func__);
...
static char *make_regular(char *value)
{
	...
	if (!regular)
		return NULL;
	if (memset_s(regular, len + count * 2 + 1, 0, len + count * 2 + 1) !=
	    EOK) {
		vfree(regular);
		return NULL;
	}
				   ...
				   char **buf_ptr)
{
	...
	/* fill event_id */
	tmp_len = snprintf_s(tmp, len + 1, len, "eventid %d",
			     event_obj->event_id);
	BUF_POINTER_FORWARD;
	...
	for (i = 0; i < MAX_PATH_NUMBER; i++) {
		if (!event_obj->dynamic_path[i])
			break;
		tmp_len = snprintf_s(tmp, len + 1, len, " -i %s",
				     event_obj->dynamic_path[i]);
		BUF_POINTER_FORWARD;
	}
	...
	for (i = 0; i < MAX_PATH_NUMBER; i++) {
		if (!event_obj->dynamic_path_delete[i])
			break;
		tmp_len = snprintf_s(tmp, len + 1, len, " -d %s",
				     event_obj->dynamic_path_delete[i]);
		BUF_POINTER_FORWARD;
	}

	/* fill time */
	if (event_obj->time) {
		tmp_len = snprintf_s(tmp, len + 1, len, " -t %lld",
				     event_obj->time);
		BUF_POINTER_FORWARD;
	}
...
static int imonitor_write_log_exception(char *str, const int str_len)
{
	...
	while (left_buf_len > 0) {
		if (left_buf_len > EVENT_INFO_PACK_BUF_LEN) {
			...
			logbuf_to_exception(0, 0, IDAP_LOGTYPE_CMD, 1, str_ptr,
					    EVENT_INFO_PACK_BUF_LEN);
#else
			send_hievent(1, str_ptr,
				     EVENT_INFO_PACK_BUF_LEN); // 1:not tail
#endif
...
#ifdef CONFIG_LOGGER_LEGACY
			logbuf_to_exception(0, 0, IDAP_LOGTYPE_CMD, 0, str_ptr,
					    left_buf_len);
#else
