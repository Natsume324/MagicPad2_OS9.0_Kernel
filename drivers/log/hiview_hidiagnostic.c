...
static void
hiview_hidiagnostic_info_node_destroy(struct diagnostic_info *diag_info);

static struct diagnostic_info *
hiview_hidiagnostic_get_info_node(struct diagnostic_info *head,
				  const char *key);

static void
hiview_hidiagnostic_add_info_node(struct hiview_hidiagnostic *hidiagnostic,
				  struct diagnostic_info *diag_info);
...
static struct diagnostic_info *
hiview_hidiagnostic_get_info_node(struct diagnostic_info *head, const char *key)
{
...
struct hiview_hidiagnostic *hiview_hidiagnostic_create(void)
{
	...
	if (memset_s(hidiagnostic, sizeof(*hidiagnostic), 0,
		     sizeof(*hidiagnostic)) != EOK) {
		vfree(hidiagnostic);
		return NULL;
	}
 ...
 */
int hiview_hidiagnositc_for_eventid(struct hiview_hidiagnostic *hidiagnostic,
				    int event_id)
{
	if (!hidiagnostic) {
 ...
 */
int hiview_hidiagnositc_for_eventid_array(
	struct hiview_hidiagnostic *hidiagnostic, int event_id[], int id_num)
{
	int tempi;

	if ((!hidiagnostic) || (!event_id)) {
		hilog_err("Bad input hidiagnostic or event_id for %s",
			  __func__);
		return -EINVAL;
	}
	...
	for (tempi = 0; tempi < id_num; tempi++) {
		if (hiview_hidiagnostic_is_ue_event(event_id[tempi])) {
			hilog_err("%s do not suport ue event %d", __func__,
				  event_id[tempi]);
 ...
 */
int hiview_hidiagnositc_set_tag(struct hiview_hidiagnostic *hidiagnostic,
				const char *tag)
{
	int tag_len = 0;
	...
	if (!hidiagnostic->tag[0]) {
		hilog_err("Failed to allocate memory for tag[0] in %s",
			  __func__);
		return -ENOMEM;
	}
 ...
 */
int hiview_hidiagnositc_set_tag_array(struct hiview_hidiagnostic *hidiagnostic,
				      const char *tag[], int tag_num)
{
	int tempi;
	int tag_len = 0;
	...
	for (tempi = 0; tempi < tag_num; tempi++) {
		...
		if (tag_len > MAX_TAG_VALUE_LEN) {
			hilog_err("Tag %s over max len: %d", tag[tempi],
				  MAX_TAG_VALUE_LEN);
			while (--tempi >= 0) {
		...
		if (!hidiagnostic->tag[tempi]) {
			hilog_err("Failed to allocate memory for tag[%d] in %s",
				  tempi, __func__);
			while (--tempi >= 0) {
				kfree(hidiagnostic->tag[tempi]);
				hidiagnostic->tag[tempi] = NULL;
 ...
 */
int hiview_hidiagnositc_add_info(struct hiview_hidiagnostic *hidiagnostic,
				 const char *key, const char *value)
{
	...
	diagnostic_info =
		hiview_hidiagnostic_get_info_node(hidiagnostic->head, key);
	if (!diagnostic_info) {
		...
		hiview_hidiagnostic_add_info_node(hidiagnostic,
						  diagnostic_info);
	}
 ...
 */
int hiview_hidiagnositc_set_privacy(struct hiview_hidiagnostic *hidiagnostic,
				    enum privacy_flag flag)
{
	if (!hidiagnostic) {
		hilog_err("Bad input hidiagnostic for %s", __func__);
	...
	if (flag == UN_SET_DEST) {
		hilog_err("%s privacy can not be set for UN_SET_DEST",
			  __func__);
		return -EINVAL;
	}
 ...
 */
int hiview_hidiagnositc_mark_related_end(
	struct hiview_hidiagnostic *hidiagnostic, bool mark)
{
	if (!hidiagnostic) {
		hilog_err("Bad input hidiagnostic for %s", __func__);
		return -EINVAL;
...
int hiview_hidiagnositc_report(struct hiview_hidiagnostic *hidiagnostic)
{
	...
	if (hidiagnostic->related_id_num == 0) {
		hilog_err(
			"Event ID is not allowed to be empty, failed to report in %s",
			__func__);
		return -EINVAL;
	}
	if (hidiagnostic->tag_num == 0) {
		hilog_err(
			"Tag is not allowed to be empty, failed to report in %s",
			__func__);
		return -EINVAL;
	}
	if (hidiagnostic->head == NULL) {
		hilog_err(
			"Info is not allowed to be empty, failed to report in %s",
			__func__);
		return -EINVAL;
	}
	if (hidiagnostic->flag == UN_SET_DEST) {
		hilog_err("Privacy level is not set, failed to report in %s",
			  __func__);
		return -EINVAL;
	}
	...
	for (tempi = 0; tempi < hidiagnostic->related_id_num; tempi++) {
		...
		tmp_len = snprintf_s(tmp, len + 1, len, "%lld",
				     hidiagnostic->related_id[tempi]);
		BUF_POINTER_FORWARD;
	}
	...
	ret = hiview_hievent_set_tag_array(event, hidiagnostic->tag,
					   hidiagnostic->tag_num);

	diag_info = hidiagnostic->head;
	while (diag_info) {
		hiview_hievent_put_string(event, diag_info->key,
					  diag_info->value);
		diag_info = diag_info->next;
	}
