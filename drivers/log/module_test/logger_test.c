...
static bool test_hievent(u32 len)
{
	...
	err = memset_s(msg, len, (int)'A', len - 1);
	if (err != EOK) {
		log_err("memset msg fail\n");
		return false;
	}
	tmp = msg + len - 1;
	*tmp = '\0';
...
static u64 latency_detect(u64 start)
{
	...
	if (latency > LATENCY_LIMIT_200ms) {
		log_err("LOGGER_FAILED: spend %lld (ms)\n", latency);
...
{
	u32 size = get_random_u32() % pack_maxsize;
...
static int logger_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if ((get_random_u32() % kthread_count) == 0) {
			test_jank();
			test_dubai();
		}
...
static void launch_logger_test_task(void)
{
	...
	for (i = 0; i < kthread_count; i++) {
		...
		if (ret != EOK) {
			log_err("memset_s name failed\n");
			break;
		}
		ret = snprintf_s(name, sizeof(name), sizeof(name) - 1,
				 "logger_test%d", i);
		if (ret < 0) {
			log_err("snprintf_s name failed\n");
			break;
		}
		...
		if (!tsk_tester)
			break;
		errno_t ret = memset_s(tsk_tester, sizeof(struct task_test), 0,
				       sizeof(struct task_test));
		if (ret != EOK) {
			log_err("memset_s tsk_tester failed\n");
			break;
		}
...
static void __exit logger_test_exit(void)
{
	...
	list_for_each_entry_safe(tsk_tester, tsk_tester_next, &task_list,
				 list) {
		if (!tsk_tester)
			continue;

		log_info("%s latency_max:%lld(us)\n", tsk_tester->ptask->comm,
	...
	log_info("total:%lld, over 200ms:%lld, over 10ms:%lld", total_test,
		 total_over_200ms, total_over_10ms);
}
