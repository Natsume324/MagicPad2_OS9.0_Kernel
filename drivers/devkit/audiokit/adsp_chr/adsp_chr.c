...
static long adsp_chr_ioctl(struct file *file, unsigned int cmd,
			   unsigned long arg)
{
	int ret;
	char *wakeInfo = NULL;
...
static int adsp_chr_notifier_call(unsigned long event, char *str)
{
	...
	kobject_uevent_env(&g_adsp_chr_miscdev.this_device->kobj, KOBJ_CHANGE,
			   envp_ext);
	mutex_unlock(&g_adsp_chr_pdata->notifier_lock);
...
static int adsp_chr_probe(struct platform_device *pdev)
{
	...
	g_adsp_chr_pdata =
		devm_kzalloc(dev, sizeof(*g_adsp_chr_pdata), GFP_KERNEL);
	if (g_adsp_chr_pdata == NULL) {
