...
self_dev_regulator_reg_ops_register(struct self_dev_regulator_reg_ops *reg_ops)
{
	return 0;
}
#endif
...
static int (*smartpakit_ctrl_init_chips)(struct smartpakit_priv *pakit_priv,
					 ...
					 int compat_mode, unsigned int id);
...
int smartpakit_register_i2c_device(struct smartpakit_i2c_priv *i2c_priv)
{
	...
	str_length = (strlen(i2c_priv->chip_model) < SMARTPAKIT_NAME_MAX) ?
			     strlen(i2c_priv->chip_model) :
			     (SMARTPAKIT_NAME_MAX - 1);
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	for (i = 0; i < pa_num_need_ops; i++) {
		...
		ret += pakit_priv->ioctl_ops->write_regs(
			i2c_priv_p, reg_num_need_ops, param_node);
	}
	return ret;
}
...
int smartpakit_resume_chips(struct smartpakit_priv *pakit_priv)
{
	...
	for (i = 0; i < SMARTPAKIT_PA_ID_MAX; i++) {
		...
		ret = pakit_priv->ioctl_ops->write_regs(
			i2c_priv_p, sequence->param_num, sequence->node);
		if (ret < 0) {
...
// clang-format off
static int smartpakit_get_param_header_from_user(
...
static int smartpakit_fill_pa_ctl_items(
	struct smartpakit_pa_ctl_sequence *sequence, unsigned int ctl_mask)
// clang-format on
{
	...
	struct smartpakit_param_node *node)
{
	...
	hilog_info("%s: ctl_mask=0x%08x, ctl_num=%d\n", __func__,
		   param->pa_ctl_mask, sequence->pa_ctl_num);
	for (i = 0; i < SMARTPAKIT_PA_ID_MAX; i++)
		hilog_info("%s: pa_ctl_index:%d %u\n", __func__, i,
			   sequence->pa_ctl_index[i]);
#endif
	return 0;
}
				       ...
				       unsigned int node_num)
{
	...
	for (i = 0; i < print_node_num; i++)
		hilog_info("%s: reg_%d=0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n",
			   __func__, i, node[i].index, node[i].mask,
			   node[i].value, node[i].delay, node[i].reserved);
}
	...
	struct smartpakit_pa_ctl_sequence *sequence)
{
	...
	if (sequence->pa_ctl_mask == 0) {
		...
		/* 1: one pa */
		if (smartpakit_priv_data->pa_num >= 1)
			sequence->pa_ctl_index_max =
				smartpakit_priv_data->pa_num - 1;
		else
			sequence->pa_ctl_index_max = 0;
	}
	...
	sequence->param_num_of_one_pa =
		sequence->param_num / sequence->pa_ctl_num;
	return 0;
}
...
static int smartpakit_get_current_i2c_client(struct smartpakit_priv *pakit_priv,
					     unsigned short addr)
{
...
static ssize_t smartpakit_ctrl_write(struct file *file, const char __user *buf,
					     ...
					     struct smartpakit_priv *pakit_priv)
{
	...
	case SMARTPAKIT_ALGO_IN_WITH_DSP:
		...
		if (i2c_priv != NULL) {
			info->chip_vendor = i2c_priv->chip_vendor;
			smartpakit_get_model_from_config_str(
				info->chip_model, i2c_priv->chip_model,
				SMARTPAKIT_NAME_MAX);
			ret = 0;
		}
		break;
					   ...
					   struct smartpakit_info *info)
{
	...
	if (pakit_priv->cust != NULL) {
		...
		cust_len = (cust_len < SMARTPAKIT_NAME_MAX) ?
				   cust_len :
				   (SMARTPAKIT_NAME_MAX - 1);
	...
	if (pakit_priv->product_name != NULL) {
		...
		product_name_len = (product_name_len < SMARTPAKIT_NAME_MAX) ?
					   product_name_len :
					   (SMARTPAKIT_NAME_MAX - 1);
		strncpy(info->product_name, pakit_priv->product_name,
			product_name_len);
	}
...
static int smartpakit_get_self_dev_smartpa_cfg(
	struct smartpakit_priv *pakit_priv, struct smartpakit_info *info)
{
	struct smartpakit_i2c_priv *i2c_priv_p = NULL;
	unsigned int i;
	int ret = 0;
	...
	for (i = 0; i < pakit_priv->pa_num; i++) {
		...
		if (i2c_priv_p == NULL)
			continue;
		ret += pakit_priv->dsm_info_ops->smartpa_info(i2c_priv_p,
							      &(info->otp[i]));
	}
			       ...
			       void __user *arg)
{
	...
	hilog_info("%s: pa_num=%u, algo_delay_time=%u, chip_vendor=%u\n",
		   __func__, info.pa_num, info.algo_delay_time,
		   info.chip_vendor);
#ifdef CONFIG_HONOR_DEBUG
	hilog_info("%s: chip_model = %s, special_name_config = %s\n", __func__,
		   info.chip_model, info.special_name_config);
#endif
...
static int smartpakit_get_single_irq_filter_cfgs(
	struct smartpakit_param_node *node, unsigned int node_num, int chip_id,
...
static void smartpakit_get_irq_filter_cfgs(struct smartpakit_priv *pakit_priv)
{
	...
	for (j = 0; j < sequence->pa_ctl_num; j++) {
		...
		ret = smartpakit_get_single_irq_filter_cfgs(
			node, param_num_of_one_pa, chip_id,
...
static int smartpakit_get_reg_node_seq_for_multi_chips(
	struct smartpakit_pa_ctl_sequence *sequence, const void __user *arg,
	int compat_mode)
{
	int ret;
...
static void smartpakit_i2c_error_reset_and_resume_chip(
	struct smartpakit_priv *pakit_priv)
{
				    ...
				    void __user *arg, int compat_mode)
{
	...
	case I2C_SLAVE_FORCE:
		ret = smartpakit_get_current_i2c_client(
			pakit_priv, (unsigned short)(uintptr_t)arg);
		break;
	case SMARTPAKIT_R_PRIL:
	case SMARTPAKIT_R_PRIR:
	case SMARTPAKIT_R_SECL:
	...
	case SMARTPAKIT_INIT:
		...
		ret = smartpakit_ctrl_init_chips(pakit_priv, arg, compat_mode,
						 SMARTPAKIT_PA_ID_ALL);
		break;
	case SMARTPAKIT_W_ALL:
	case SMARTPAKIT_W_PRIL:
	case SMARTPAKIT_W_PRIR:
	...
	case SMARTPAKIT_W_SECR:
		...
		ret = smartpakit_ctrl_write_chips(pakit_priv, arg, compat_mode,
						  id);
		break;
				     ...
				     int *num)
{
	...
	if (of_property_read_bool(node, propname))
		count = of_property_count_elems_of_size(node, propname,
							(int)sizeof(u32));
...
int smartpakit_get_prop_of_u32_type(struct device_node *node,
				    const char *key_str, unsigned int *type,
				    bool is_requisite)
{
	int ret = 0;
					    ...
					    struct smartpakit_priv *pakit_priv)
{
	...
	ret = smartpakit_save_switch_ctl_default_value(
		node, pakit_priv->switch_ctl, gpio_num);
	if (ret < 0) {
					   ...
					   struct smartpakit_priv *pakit_priv)
{
	...
	support_irq_pinctrl =
		of_property_read_bool(dev->of_node, smartpakit_irq_pinctrl);
				    ...
				    struct smartpakit_priv *pakit_priv)
{
	...
	ret = smartpakit_get_prop_of_u32_type(pdev->dev.of_node,
					      soc_platform_str,
					      &pakit_priv->soc_platform, true);
	ret += smartpakit_get_prop_of_u32_type(pdev->dev.of_node, algo_in_str,
					       &pakit_priv->algo_in, true);
	ret += smartpakit_get_prop_of_u32_type(pdev->dev.of_node,
					       algo_delay_str,
					       &pakit_priv->algo_delay_time,
					       false);
	ret += smartpakit_get_prop_of_u32_type(pdev->dev.of_node,
					       chip_vendor_str,
					       &pakit_priv->chip_vendor, false);
	ret += smartpakit_get_prop_of_u32_type(pdev->dev.of_node,
					       param_version_str,
					       &pakit_priv->param_version,
					       false);

	ret += smartpakit_get_prop_of_str_type(
		pdev->dev.of_node, chip_model_str, &pakit_priv->chip_model);
	ret += smartpakit_get_prop_of_str_type(pdev->dev.of_node, cust_str,
					       &pakit_priv->cust);
	ret += smartpakit_get_prop_of_str_type(
		pdev->dev.of_node, special_cfg_str,
		&pakit_priv->special_name_config);
	ret += smartpakit_get_prop_of_str_type(
		pdev->dev.of_node, product_name_str, &pakit_priv->product_name);
