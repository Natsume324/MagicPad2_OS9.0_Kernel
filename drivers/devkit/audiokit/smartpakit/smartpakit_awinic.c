...
int aw_smartpa_set_correction_v1(struct regmap *regmap, unsigned int reg_addr,
				 unsigned int isn_addr, unsigned int vsn_addr)
{
	int ret;
	int i_trim = 0;
	int v_trim = 0;
	...
	ret = aw_smartpakit_regmap_write(regmap, reg_addr,
 ...
 *      for example: 0x03ff (0000 0011 1111 1111)      *
 *                                                     */
static int aw_smartpa_get_param_from_mask(unsigned short mask,
					  unsigned int *shift,
...
static int aw_smartpa_get_icalk(struct regmap *regmap,
				struct aw_vcalb_desc *desc, int efuse_check,
				short *icalk)
{
	int ret;
...
static int aw_smartpa_get_vcalk(struct regmap *regmap,
				struct aw_vcalb_desc *desc, int efuse_check,
				short *vcalk)
{
	int ret;
...
static int aw_smartpa_param_init(struct aw_vcalb_desc *desc,
				 unsigned int value_factor,
				 unsigned int xml_mask,
				 unsigned int vcal_factor)
{
	int ret;
	...
	ret = aw_smartpa_get_param_from_mask(mask, &shift, &sign_mask,
					     &neg_mask);
	if (ret < 0) {
				 ...
				 unsigned int vcal_factor)
{
	...
	if (aw_smartpa_param_init(&desc, value_factor, xml_mask, vcal_factor) <
	    0)
		return -EINVAL;
	...
	hilog_info("%s: icalk=%d, vcalk=%d, vcalb=%d, reg_val=0x%04x", __func__,
