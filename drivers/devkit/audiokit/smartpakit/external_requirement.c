...
static struct smartpakit_i2c_priv *
get_i2c_priv_by_addr(struct smartpakit_priv *kit_priv, int i2c_addr)
{
...
static int smartpakit_self_dev_regulator_reg_read(int i2c_addr, int reg_addr,
						  int *value)
{
	struct smartpakit_i2c_priv *i2c_priv = NULL;
	...
	if ((kit_priv == NULL) || (kit_priv->i2c_num <= 0)) {
		hilog_err(
			"smartpakit_get_misc_priv: Invalid smartpakit_priv param");
		return -EINVAL;
	}
	...
	if ((i2c_priv == NULL) || (value == NULL)) {
		hilog_err(
			"get_i2c_priv_by_addr: invalid input arg, i2c_addr0x%x\n",
			i2c_addr);
		ret = -EINVAL;
		goto err_out;
	}
						   ...
						   int mask, int value)
{
	...
	ret = kit_priv->ioctl_ops->write_regs(i2c_priv, EXTERN_NODE_NUM,
					      &reg_info);
