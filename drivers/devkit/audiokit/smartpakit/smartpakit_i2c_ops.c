...
static bool smartpakit_i2c_probe_skip[SMARTPAKIT_PA_ID_MAX] = {
	0, 0, 0, 0,
	0, 0, 0, 0
};
// clang-format on
...
static int smartpakit_regmap_xorw(struct regmap *regmap, unsigned int reg_addr,
					   ...
					   unsigned int value)
{
	...
	if ((mask ^ cfg->value_mask) == 0)
		...
		ret = smartpakit_regmap_update_bits(cfg->regmap, reg_addr, mask,
						    value);
				      ...
				      struct i2c_err_info *info)
{
	...
	smartpakit_dsm_report_by_i2c_error(i2c_priv->chip_model,
					   (int)i2c_priv->chip_id, flag, errno,
					   info);
}
					     ...
					     struct list_head *head)
{
	...
	list_for_each_entry(dev_info, head, list) {
		if (dev_info->chip_id != id)
			continue;
		smartpakit_append_dsm_report(
			report, "%s_%x,", dev_info->chip_model, dev_info->addr);
	}
}
...
#ifdef CONFIG_SND_SOC_SELF_DEV_PA
static void smartpakit_switch_self_dev_smartpa_gpio(const char *model,
						    int state,
						    unsigned int gpio,
						    int i2c_state)
{
	...
	const char *self_dev_pa_type =
		"04-01"; // self_dev_martpa chip_model type
...
static void smartpakit_i2c_hw_reset_gpio_ctl(
	struct smartpakit_i2c_priv *i2c_priv,
	struct smartpakit_gpio_sequence *sequence)
{
	...
	for (j = 0; j < num; j++) {
		hilog_info("%s: gpio %d set %u, delay %u\n", __func__,
			   i2c_priv->hw_reset->gpio, node[j].state,
			   node[j].delay);
		gpio_direction_output((unsigned int)i2c_priv->hw_reset->gpio,
				      (int)node[j].state);
#ifdef CONFIG_SND_SOC_SELF_DEV_PA
		if (i2c_priv->irq_handler != NULL)
			smartpakit_switch_self_dev_smartpa_gpio(
				i2c_priv->chip_model, (int)node[j].state,
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	i2c_priv->irq_debounce_jiffies =
		jiffies + msecs_to_jiffies(i2c_priv->reset_debounce_wait_time);
	schedule_delayed_work(
		&i2c_priv->irq_debounce_work,
	...
	struct list_head *dump_reg_list)
{
	...
	for (i = 0; i < (int)sequence->num; i++) {
		...
		if (ctl_type == SMARTPAKIT_REG_CTL_TYPE_W) {
			...
			ret += kit_i2c_dts_regs_read(
				i2c_priv, &sequence->regs[i], dump_reg_list);
		}
	}
...
int smartpakit_i2c_get_self_dev_smartpa_info(
	struct smartpakit_i2c_priv *i2c_priv,
	struct self_dev_smartpa_coeff *cfg)
{
	...
	hilog_debug("%s: pa %u, operate self_dev_smartpa otp regs\n", __func__,
		    i2c_priv->chip_id);
	ret = smartpakit_i2c_dts_reg_ops(
		i2c_priv, i2c_priv->self_dev_smartpa_otp_sequence,
		&otp_req_list);
	if (ret < 0) {
...
static bool smartpakit_is_need_write_regs(struct smartpakit_i2c_priv *i2c_priv,
					  unsigned int num,
					  struct smartpakit_param_node *regs)
{
	struct smartpakit_priv *pakit_priv = NULL;
...
static int smartpakit_i2c_reg_node_ops(struct smartpakit_regmap_cfg *cfg,
				       struct smartpakit_param_node *reg,
				       int index, unsigned int node_num)
{
	int ret = 0;
	int value = 0;
	...
	case SMARTPAKIT_PARAM_NODE_TYPE_REG_RXORW:
		...
		ret = smartpakit_regmap_xorw(cfg->regmap, reg->index, reg->mask,
					     reg->value);
		break;
	...
	for (i = 0; i < (int)num; i++) {
		...
		if (ret < 0) {
			hilog_err("%s: node %d, reg 0x%x w/r %x err, ret %d\n",
				  __func__, i, regs[i].index, regs[i].value,
				  ret);
...
static int smartpakit_i2c_ctrl_write(struct i2c_client *i2c, char *rx_data,
				     int length)
{
	int ret;
	struct i2c_msg msg[] = {
...
static bool smartpakit_is_rw_sequence_exited(
	struct smartpakit_i2c_priv *i2c_priv)
{
	if ((i2c_priv == NULL) || (i2c_priv->irq_handler == NULL) ||
...
static bool is_irq_filter_config_hited(struct list_head *dump_reg_list,
				       struct smartpakit_reg_info *cfg_info)
{
	...
	bool need_reset)
{
	...
	for (i = 0; i < (int)pakit_priv->pa_num; i++) {
		...
		smartpakit_append_dsm_report(
			report, "pa %s_%d:", i2c_priv_p->chip_model,
			i2c_priv_p->chip_id);
...
void smartpakit_i2c_handler_irq(struct work_struct *work)
{
	...
	i2c_priv =
		container_of(work, struct smartpakit_i2c_priv, irq_handle_work);
...
static bool smartpakit_i2c_writeable_reg(struct device *dev, unsigned int reg)
{
	...
	if (cfg->num_writeable > 0)
		return smartpakit_i2c_is_reg_in_special_range(
			reg, cfg->num_writeable, cfg->reg_writeable);
	if (cfg->num_unwriteable > 0)
		return !smartpakit_i2c_is_reg_in_special_range(
			reg, cfg->num_unwriteable, cfg->reg_unwriteable);
...
static bool smartpakit_i2c_readable_reg(struct device *dev, unsigned int reg)
{
	...
	if (cfg->num_readable > 0)
		return smartpakit_i2c_is_reg_in_special_range(
			reg, cfg->num_readable, cfg->reg_readable);
	if (cfg->num_unreadable > 0)
		return !smartpakit_i2c_is_reg_in_special_range(
			reg, cfg->num_unreadable, cfg->reg_unreadable);
...
static bool smartpakit_i2c_volatile_reg(struct device *dev, unsigned int reg)
{
	...
	if (cfg->num_volatile > 0)
		return smartpakit_i2c_is_reg_in_special_range(
			reg, cfg->num_volatile, cfg->reg_volatile);
	if (cfg->num_unvolatile > 0)
		return !smartpakit_i2c_is_reg_in_special_range(
			reg, cfg->num_unvolatile, cfg->reg_unvolatile);

	return true;
}

static void smartpakit_i2c_free_hw_reset(struct smartpakit_gpio_reset *hw_reset)
{
	if (hw_reset == NULL)
		return;
...
static int smartpakit_i2c_parse_hw_reset_gpio_ctl_seq(
	struct device_node *node, struct smartpakit_gpio_sequence *sequence)
{
	...
	struct smartpakit_gpio_reset **reset)
{
	...
	reset_info->not_need_shutdown =
		of_property_read_bool(node, not_need_shutdown_str);
...
static int smartpakit_i2c_parse_reg_ctl(
	struct smartpakit_reg_ctl_sequence **reg_ctl,
	struct device_node *node, const char *ctl_str)
{
	...
	struct smartpakit_gpio_irq **irq)
{
	...
	irq_info->need_filter_irq =
		of_property_read_bool(node, need_filter_irq_str);
#ifdef CONFIG_HONOR_DEBUG
	...
	ret = snprintf(irq_info->gpio_name, (unsigned long)SMARTPAKIT_NAME_MAX,
		       "%s_gpio_irq_%d", i2c_priv->chip_model,
		       i2c_priv->chip_id);
	...
	if (of_property_read_bool(node, rw_sequence_str)) {
		ret = smartpakit_i2c_parse_reg_ctl(&irq_info->rw_sequence, node,
						   rw_sequence_str);
		if (ret < 0) {
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	ret = devm_request_threaded_irq(
		&i2c->dev, (unsigned int)irq_handler->irq, NULL,
	...
	struct smartpakit_i2c_priv *i2c_priv)
{
	...
	if (!of_property_read_bool(i2c->dev.of_node,
				   self_dev_smartpa_otp_regs_str)) {
		hilog_debug("%s: self_dev_smartpa_otp not existed, skip\n",
			    __func__);
		return 0;
	}
	ret = smartpakit_i2c_parse_reg_ctl(
		&i2c_priv->self_dev_smartpa_otp_sequence, i2c->dev.of_node,
		self_dev_smartpa_otp_regs_str);
	if (ret < 0) {
	...
	ret = smartpakit_get_prop_of_u32_type(i2c->dev.of_node,
					      self_dev_smartpa_i2c_state_str,
	...
	struct smartpakit_regmap_cfg *cfg_info)
{
	...
	return smartpakit_get_prop_of_u32_array(node, reg_defaults_str,
						(u32 **)&cfg_info->reg_defaults,
						&cfg_info->num_defaults);
}
					  ...
					  struct smartpakit_regmap_cfg **cfg)
{
	...
	cfg_info->value_mask =
		smartpakit_i2c_get_reg_value_mask(cfg_info->cfg.val_bits);
					...
					struct smartpakit_i2c_priv *i2c_priv)
{
	...
	ret += smartpakit_get_prop_of_u32_type(
		i2c->dev.of_node, wait_time_str,
...
static void smartpakit_i2c_reset_priv_data(struct smartpakit_i2c_priv *i2c_priv)
{
	if (i2c_priv == NULL)
		return;
...
static void smartpakit_i2c_shutdown(struct i2c_client *i2c)
{
	...
	for (i = (int)(sequence->num - 1); i >= 0; i--) {
		gpio_direction_output(gpio, (int)sequence->node[i].state);
