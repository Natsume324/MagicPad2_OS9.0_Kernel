...
int boxid_read(int id)
{
	...
	if (device_boxid.check_mode == DEVICEBOX_ID_MODE_USE_HKADC &&
		...
		vendor = status0 < 0 ? BOX_NAME_DEFAULT :
				       info[id].box_map[0][status0];
	}
		...
		for (j = 1; j < group->num; j++) {
			...
			if (value1 != value2) {
				pr_err("%s:group[%d],[%d:%d],[%d:%d] is diff\n",
				       __func__, i, boxid1, value1, boxid2,
				       value2);
				value = BOX_CONSISTENT_TEST_FAILED;
				return put_user(value, p_user);
			}
...
static long boxid_do_ioctl(struct file *file, unsigned int cmd, void __user *p,
			   int compat_mode)
{
...
static int get_boxid_info(struct device_node *node)
{
	...
	for (i = 0; i < device_boxid.box_num; i++) {
		if (info[i].box_enable == BOX_ID_ENABLED) {
			if (!of_property_read_u32_index(node, "gpio_num", i,
							&temp))
			      ...
			      int id)
{
	...
	if (info[id].gpio_num == USE_ONE_GPIO ||
		...
		for (i = 0; i < (VENDOR * VENDOR); i++) {
			if (!of_property_read_u32_index(node, propname, i,
							&temp)) {
				index1 = i / VENDOR;
				index2 = i % VENDOR;
...
static void update_spkbox_voltage_status(int voltage)
{
	...
	pr_info("hkadc voltage = %d, spkbox_voltage_status = %d\n", voltage,
		info[SPEAKER_ID].voltage_status);
}
...
int boxid_get_prop_of_u32_array(struct device_node *node, const char *propname,
				u32 **value, unsigned int *num)
{
	u32 *array = NULL;
	int ret;
	int count = 0;
				       ...
				       int actual_id)
{
	...
	actual_info = (struct box_info_st *)&(device_boxid.box_info[actual_id]);
					   ...
					   struct boxid_share_info *share_info)
{
	...
	for (i = 0; i < share_info->actual_boxid_num; i++) {
		...
		snprintf(group_name, (unsigned long)NAME_LEN_MAX,
			 "actual_boxid%d_group", i);

		ret = boxid_get_prop_of_u32_array(
			node, group_name, (u32 **)&(box_group->share_spkid),
			&box_group->num);
					...
					struct boxid_share_info *share_info)
{
	...
	if ((share_info->actual_boxid_num == 0 ||
	     share_info->actual_boxid_num > BOX_NUM_MAX) ||
	    (ret < 0)) {
...
static void boxid_free_share_info(struct boxid_share_info *share_info)
{
	unsigned int i;
	struct actual_box_group *box_group = NULL;
...
static int boxid_parse_dt_share_info(struct platform_device *pdev)
{
	...
	share_info->share_support =
		of_property_read_bool(node, support_share_str);
...
static void boxid_free_consistent_test_info(struct box_consistent_test *test)
{
	unsigned int i;
				      ...
				      struct device_node *node)
{
	...
	for (i = 0; i < test->group_num; i++) {
		ret = boxid_get_prop_of_u32_array(
			node, test_groups, (u32 **)&(test->groups[i].boxid),
			&(test->groups[i].num));
...
static void box_parse_dt_consistent_test_info(struct platform_device *pdev)
{
	...
	test->test_support =
		of_property_read_bool(node, consistent_test_support);
	if (!test->test_support) {
