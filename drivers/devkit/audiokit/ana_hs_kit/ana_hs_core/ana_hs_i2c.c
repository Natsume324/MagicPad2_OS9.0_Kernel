...
static int ana_hs_regmap_read(struct regmap *regmap, unsigned int reg_addr,
			      unsigned int *value)
{
	int ret = 0;
	int i;
...
static int ana_hs_regmap_write(struct regmap *regmap, unsigned int reg_addr,
			       unsigned int value)
{
	int ret = 0;
	int i;
...
static int
ana_hs_i2c_parse_special_regs_range(struct device_node *node,
				    struct ana_hs_regmap_cfg *cfg_info)
{
				       ...
				       struct ana_hs_regmap_cfg **cfg)
{
	...
	cfg_info->value_mask =
		ana_hs_i2c_get_reg_value_mask(cfg_info->cfg.val_bits);
...
static bool ana_hs_i2c_is_reg_in_special_range(unsigned int reg_addr, int num,
					       unsigned int *reg)
{
	int i;
...
static struct ana_hs_regmap_cfg *ana_hs_i2c_get_regmap_cfg(struct device *dev)
{
...
static bool ana_hs_i2c_writeable_reg(struct device *dev, unsigned int reg)
{
	...
	if (cfg->num_writeable > 0)
		return ana_hs_i2c_is_reg_in_special_range(
			       ...
			       struct ana_hs_i2c_data *i2c_priv)
{
	...
	i2c_priv->handler->irq =
		gpio_to_irq((unsigned int)i2c_priv->handler->gpio);
	hilog_info("%s i2c_priv->handler->irq is %d\n", __func__,
		   i2c_priv->handler->irq);

	ret = request_threaded_irq(i2c_priv->handler->irq, NULL, handler_func,
				   i2c_priv->handler->irq_flag | IRQF_ONESHOT |
					   IRQF_NO_SUSPEND,
				   "ana_hs_irq", NULL);
	if (ret < 0) {
...
static int ana_hs_i2c_suspend(struct device *dev)
{
	...
	if ((data->regmap_cfg != NULL) && (data->regmap_cfg->regmap != NULL) &&
