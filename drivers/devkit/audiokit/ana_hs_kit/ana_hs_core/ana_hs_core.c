...
int ana_hs_bus_read(unsigned int addr, unsigned int *value)
{
	...
	if (g_data == NULL || g_data->ops == NULL || g_data->bus_data == NULL ||
	    value == NULL) {
	...
	if (g_data->bus_type == ANA_HS_I2C) {
		...
		ret = g_data->ops->read_regs(data->regmap_cfg->regmap, addr,
					     value);
	}
...
int ana_hs_bus_write(unsigned int addr, unsigned int value)
{
	...
	if (g_data->bus_type == ANA_HS_I2C) {
		...
		ret = g_data->ops->write_regs(data->regmap_cfg->regmap, addr,
					      value);
	}
...
static int ana_hs_bus_update_bits(unsigned int addr, unsigned int mask,
				  unsigned int value)
{
	int ret = -1;
	...
	if (g_data->bus_type == ANA_HS_I2C) {
		...
		value = value & data->regmap_cfg->value_mask;
		ret = g_data->ops->update_bits(data->regmap_cfg->regmap, addr,
					       mask, value);
	}
...
static bool read_property_setting(struct device *dev, const char *property_name,
...
int ana_hs_get_u32_array(struct device_node *node, const char *propname,
			 u32 **value, int *num)
{
	u32 *array = NULL;
	int ret;
			       ...
			       struct device_node *node, const char *seq_str)
{
	...
	ret = ana_hs_get_u32_array(node, seq_str, (u32 **)&data_temp->nodes,
				   &count);
	if ((count <= 0) || (ret < 0)) {
...
static int ana_hs_load_gpio_data(struct device *dev)
{
	...
	for (i = 0; i < g_data->gpio_num; i++) {
		g_data->gpio_list[i] =
			of_get_named_gpio(dev->of_node, "gpios", i);
		if (g_data->gpio_list[i] < 0) {
...
static bool ana_hs_prepare_regulator(void)
{
	...
	g_data->phy_ldo = devm_regulator_get(g_data->dev, "ana_hs_switch");
	...
	ret = regulator_set_voltage(g_data->phy_ldo, g_data->phy_ldo_vol,
				    g_data->phy_ldo_vol);
	if (ret != 0) {
...
static void plugin_work(struct work_struct *work)
{
	 ...
	 */
	g_data->codec_ops->ops.hs_high_resistence_enable(g_data->private_data,
							 true);
	...
	g_data->codec_ops->ops.plug_in_detect(g_data->private_data);
...
static void plugout_work(struct work_struct *work)
{
	...
	if (g_data->ana_hs_in == ANA_HS_PLUG_IN) {
		...
		g_data->codec_ops->ops.plug_out_detect(g_data->private_data);
		mutex_lock(&g_data->mutex);
...
static int ana_hs_create_workqueue(void)
{
	...
	/* create workqueue */
	g_data->plugin_delay_wq =
		create_singlethread_workqueue("ana_hs_plugin_delay_wq");
	if (!(g_data->plugin_delay_wq)) {
	...
	g_data->plugout_delay_wq =
		create_singlethread_workqueue("ana_hs_plugout_delay_wq");
	if (!(g_data->plugout_delay_wq)) {
...
static int ana_hs_core_codec_dev_register(struct ana_hs_codec_dev *dev,
					  void *codec_data)
{
...
static void ana_hs_core_plug_handle(int hs_state)
{
	...
	__pm_wakeup_event(g_data->wake_lock, WAKE_LOCK_TIMEOUT);

	switch (hs_state) {
	case ANA_HS_PLUG_IN:
		queue_delayed_work(
			g_data->plugin_delay_wq, &g_data->plugin_delay_work,
...
static long ana_hs_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_ANA_HS_GET_MIC_SWITCH_STATE:
		if (g_data->bus_type == ANA_HS_GPIO) {
			...
			gpio_mic_sel_val =
				hs_gpio_get_value(g_data->gpio_list[index]);
	...
	case IOCTL_ANA_HS_GET_CONNECT_LINEIN_R_STATE:
		hilog_info("%s connect_linein_r = %d\n", __func__,
			   g_data->connect_linein_r);
		ret = put_user((__u32)(g_data->connect_linein_r), p_user);
		break;
...
static void parse_sequence_data(struct device_node *node)
{
	...
	ret = ana_hs_parse_sequence_data(&g_data->init_seq, node, "init_seq");
	if (ret < 0)
		hilog_err("%s: parse dts init_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->plug_in_seq, node,
					 "plug_in_seq");
	if (ret < 0)
		hilog_err("%s: parse dts plug_in_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->plug_out_seq, node,
					 "plug_out_seq");
	if (ret < 0)
		hilog_err("%s: parse dts plug_out_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->mic_gnd_sequence, node,
					 "mic_gnd_sequence");
	if (ret < 0)
		hilog_err("%s: parse dts mic_gnd_sequence fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->dp_charger_in_seq, node,
					 "dp_charger_in_seq");
	if (ret < 0)
		hilog_err("%s: parse dts dp_charger_in_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->dp_cross_seq, node,
					 "dp_cross_seq");
	if (ret < 0)
		hilog_err("%s: parse dts dp_cross_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->dp_charger_out_seq, node,
					 "dp_charger_out_seq");
	if (ret < 0)
		hilog_err("%s: parse dts dp_charger_out_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->fb_connect_seq, node,
					 "fb_connect_seq");
	if (ret < 0)
		hilog_err("%s: parse dts fb_connect_seq fail\n", __func__);
	ret = ana_hs_parse_sequence_data(&g_data->fb_disconnect_seq, node,
					 "fb_disconnect_seq");
	if (ret < 0)
...
static void ana_hs_get_prop(struct device *dev)
{
	...
	g_data->connect_linein_r =
		read_property_setting(dev, "connect_linein_r", true);
	g_data->support_cc = read_property_setting(dev, "support_cc", false);
	g_data->using_superswitch =
		read_property_setting(dev, "using_superswitch", false);
	g_data->switch_antenna =
		read_property_setting(dev, "switch_antenna", false);
}
...
static int ana_hs_get_regulator(struct device *dev)
{
	...
	g_data->phy_ldo = devm_regulator_get(dev, "ana_hs_switch");
	if (IS_ERR(g_data->phy_ldo)) {
		hilog_err("phy_ldo regulator dts read failed\n");
		g_data->phy_ldo = NULL;
		return -1;
