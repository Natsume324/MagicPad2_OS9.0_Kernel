...
struct ana_hs_bus_ctl_ops {
	int (*read_regs)(struct regmap *regmap, unsigned int reg_addr,
			 unsigned int *value);
	int (*write_regs)(struct regmap *regmap, unsigned int reg_addr,
			  unsigned int value);
...
static inline int ana_hs_register_bus_ops(struct ana_hs_bus_ctl_ops *ops,
					  void *data)
{
	return 0;
}
