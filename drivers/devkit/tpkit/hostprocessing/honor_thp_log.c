...
static int thp_write_dif_data(void *buff)
{
	...
	if (num_col * num_row * sizeof(short) > THP_LOG_DIF_DATA_LENGTH_MAX) {
		thp_log_err("%s col or row is too large\n", __func__);
		return -1;
	}
...
int thp_log_save(const char *head)
{
	...
	if (g_thplog_save.data == NULL)
		g_thplog_save.data = (struct pkt_thplog_report_data_t *)kzalloc(
			sizeof(struct pkt_thplog_report_data_t), GFP_ATOMIC);
	if (g_raw_data_buff == NULL)
		g_raw_data_buff = (char *)kzalloc(
			(size_t)THP_LOG_RAW_DATA_LENGTH_MAX, GFP_ATOMIC);
	if (g_dif_data_buff == NULL)
		g_dif_data_buff = (char *)kzalloc(
			(size_t)THP_LOG_DIF_DATA_LENGTH_MAX, GFP_ATOMIC);
	if ((g_thplog_save.data == NULL) || (g_raw_data_buff == NULL) ||
