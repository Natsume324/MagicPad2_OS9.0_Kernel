...
static int thp_pen_put_one_cmd(struct thp_core_data *cd,
...
static int thp_pen_power_control_notify(struct thp_core_data *cd,
...
static struct thp_queue *thp_pen_queue_init(struct thp_core_data *cd)
{
	...
	cd->queue_data_buf =
		vzalloc(cd->thp_queue_buf_len * THP_PEN_LIST_MAX_FRAMES);
...
static bool thp_pen_queue_is_empty(struct thp_core_data *cd,
...
static bool thp_pen_queue_is_full(struct thp_core_data *cd,
				  struct thp_queue *queue)
{
	if (queue == NULL) {
		thp_log_err(cd, "%s queue is NULL\n", __func__);
...
static bool thp_pen_queue_dequeue(struct thp_core_data *cd,
				  struct thp_queue *queue)
{
	if (queue == NULL) {
		thp_log_err(cd, "%s queue is NULL\n", __func__);
...
static bool thp_pen_queue_enqueue(struct thp_core_data *cd,
				  struct thp_queue *queue, u8 *read_buf,
				  unsigned int len)
{
...
static void thp_pen_queue_clear_all(struct thp_core_data *cd,
				    struct thp_queue *queue)
{
	int i;
...
static void thp_pen_queue_free(struct thp_core_data *cd,
			       struct thp_queue *queue)
{
	if (queue == NULL) {
		thp_log_err(cd, "%s queue is NULL\n", __func__);
...
static int thp_pen_set_spi3_max_speed(struct thp_core_data *cd,
				      unsigned int speed)
{
	int rc;
...
static int thp_pen_spi3_transfer(struct thp_core_data *cd, char *tx_buf,
...
int thp_pen_single_power_control_notify(enum lcd_kit_ts_pm_type pm_type,
					int timeout)
{
...
static long thp_pen_ioctl_spi_sync(struct thp_core_data *cd,
				   const void __user *data,
				   unsigned int lock_status)
{
	...
	if (lock_status == NEED_LOCK) {
		...
		rc = thp_pen_spi3_transfer(cd, tx_buf, rx_buf, sync_data.size,
					   lock_status);
	}
	if (rc) {
...
static long thp_pen_ioctl_finish_notify(struct thp_core_data *cd,
					unsigned long arg)
{
...
static long thp_pen_ioctl_get_frame_count(struct thp_core_data *cd,
					  unsigned long arg)
{
...
static long
thp_pen_get_frame_from_thp_queue(struct thp_core_data *cd,
...
static long thp_pen_ioctl_get_frame(struct thp_core_data *cd, unsigned long arg,
				    unsigned int f_flag)
{
	struct thp_ioctl_get_frame_data data;
...
static long thp_pen_ioctl_set_timeout(struct thp_core_data *cd,
				      unsigned long arg)
{
			  ...
			  unsigned long arg)
{
	...
	case THP_IOCTL_CMD_MUILTIPLE_SPI_XFRE_SYNC:
		ret = thp_pen_ioctl_multiple_spi_xfer_sync(
			cd, (void __user *)arg, NEED_LOCK);
		break;
	case THP_IOCTL_CMD_HW_LOCK:
		ret = thp_ioctl_hw_lock_status(cd, arg);
		break;
	case THP_IOCTL_CMD_SPI_SYNC_NO_LOCK:
		ret = thp_pen_ioctl_spi_sync(cd, (void __user *)arg,
					     DONOT_NEED_LOCK);
		break;
	case THP_IOCTL_CMD_MUILTIPLE_SPI_XFRE_SYNC_NO_LOCK:
		ret = thp_pen_ioctl_multiple_spi_xfer_sync(
			cd, (void __user *)arg, DONOT_NEED_LOCK);
		break;
...
static void thp_pen_copy_frame(struct thp_core_data *cd)
{
	...
	if (cd->frame_count >= THP_PEN_LIST_MAX_FRAMES) {
		if (cd->frame_count != pre_frame_count)
			thp_log_err(cd,
				    "pen frame buf full start,frame_count:%d\n",
				    cd->frame_count);
...
static int thp_pen_mtk_pinctrl_get_init(struct thp_device *tdev)
{
	...
	cd->mtk_pinctrl.spi_status =
		pinctrl_lookup_state(cd->pctrl, PINCTRL_STATE_SPI_STATUS);
	...
	ret = pinctrl_select_state(cd->pctrl, cd->mtk_pinctrl.spi_status);
	if (ret < 0)
...
static int thp_pen_setup_irq(struct thp_core_data *cd)
{
	...
	if (cd->multi_panel_index != SINGLE_TOUCH_PANEL) {
		...
		rc = request_threaded_irq(irq, NULL, thp_pen_irq_thread,
...
static int thp_pen_parse_test_config(struct thp_core_data *cd,
...
static void thp_pen_chip_detect(struct thp_core_data *cd,
...
int thp_pen_register_dev(struct thp_core_data *cd, struct thp_device *dev)
{
	...
	cd->thp_unregister_ic_num--;
	thp_log_info(cd, "%s:thp_pen_unregister_ic_num :%d", __func__,
		     cd->thp_unregister_ic_num);
...
static void thp_pen_parse_extra_feature_config(struct device_node *thp_node,
					       struct thp_core_data *cd)
{
	int rc;
	unsigned int value = 0;
...
static int thp_pen_cmd_sync_allocate(struct thp_core_data *cd,
				     struct thp_cmd_node *cmd, int timeout)
{
...
static int thp_pen_proc_command(struct thp_core_data *cd,
				struct thp_cmd_node *cmd)
{
	int error = NO_ERR;
	struct thp_cmd_sync *sync = NULL;
...
static int thp_pen_thread_init(struct thp_core_data *cd)
{
	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL)
		cd->thp_task =
			kthread_create(thp_pen_thread, cd, "ts_thread:%d", 0);
	else
		cd->thp_task = kthread_create(thp_pen_thread, cd,
					      "ts_thread:%d",
					      cd->multi_panel_index);
...
static int thp_pen_probe(struct spi_device *sdev)
{
	...
	if (thp_pen_core->wait_afe_screen_on_support) {
		...
		atomic_set(&(thp_pen_core->afe_screen_on_waitq_flag),
			   WAITQ_WAKEUP);
		thp_log_info(thp_pen_core,
			     "%s, init afe_screen_on_waitq done\n", __func__);
	}
...
static void thp_pen_remove(struct spi_device *sdev)
{
...
static const struct spi_device_id g_thp_pen_device_id[] = {{THP_PEN_DEVICE_NAME,
							    0},
							   {}};
