...
static int touch_driver_83112_get_frame(struct thp_device *tdev, char *buf,
					unsigned int len);
...
static int touch_driver_spi_read(struct thp_device *tdev, uint8_t *command,
				 uint8_t command_len, uint8_t *data,
				 unsigned int length, uint8_t to_retry)
{
	struct spi_message message;
	...
	memset(g_transfer, 0, XFER_CNT * sizeof(struct spi_transfer));
...
static int touch_driver_spi_write(struct thp_device *tdev, uint8_t *buf,
				  unsigned int length)
{
	int ret;
	struct spi_message message;
...
static int touch_driver_83102_bus_read(struct thp_device *tdev, uint8_t command,
				       ...
				       uint8_t toretry)
{
	...
	return touch_driver_spi_read(tdev, &spi_format_buf[DATA_0],
				     SPI_FORMAT_ARRAY_SIZE, data, length,
				     toretry);
}
				 ...
				 uint8_t to_retry)
{
	...
	if ((himax_p->himax_ic_83102_flag) || (himax_p->himax_ic_83121_flag) ||
		...
		ret = touch_driver_83102_bus_read(tdev, command, data, length,
						  to_retry);
		return ret;
	}
	if (!spi_format_buf) {
...
{
	uint8_t spi_format_buf[HIMAX_NORMAL_DATA_LEN + HIMAX_NORMAL_ADDR_LEN +
			       DATA_2];
	int i;
				  ...
				  uint8_t to_retry)
{
	...
	if ((himax_p->himax_ic_83102_flag) || (himax_p->himax_ic_83121_flag) ||
	    (himax_p->himax_ic_83122_flag)) {
		ret = touch_driver_83102_bus_write(tdev, command, data, length,
						   to_retry);
		return ret;
	}
	if (!spi_format_buf) {
...
static void touch_driver_register_write(struct thp_device *tdev,
					uint8_t *write_addr, uint8_t len,
					uint8_t *write_data)
{
					   ...
					   unsigned int *gesture_wakeup_value)
{
	...
	cd = thp_get_core_data();
	if (!cd) {
		thp_log_err(tdev->thp_core, "%s: cd null ptr\n", __func__);
		return -EINVAL;
	}
	if (!cd->vendor_name) {
		thp_log_err(tdev->thp_core, "%s: vendor_name null ptr\n",
			    __func__);
		return -EINVAL;
	}
	if (!strncmp(cd->vendor_name, VENDOR_NAME_BOE,
		     THP_PROJECTID_VENDOR_NAME_LEN)) {
		tx_num = HX_BOE_TX_NUM;
		rx_num = HX_BOE_RX_NUM;
	} else if (!strncmp(cd->vendor_name, VENDOR_NAME_INX,
			    THP_PROJECTID_VENDOR_NAME_LEN)) {
	...
	length--;
	thp_log_info(tdev->thp_core, "data[%d] = 0x%X, data[%d] = 0x%X\n",
		     length, data[length], length - 1, data[length - 1]);
				       ...
				       unsigned int *gesture_wakeup_value)
{
	...
	for (i = 0; i < GESTURE_EVENT_RETRY_TIME; i++) {
		retval = touch_driver_83112_get_frame(tdev, frame_read_buf,
						      cd->frame_size);
		if (retval) {
			thp_log_err(tdev->thp_core,
	...
	retval = touch_driver_parse_gesture_data(tdev, frame_read_buf,
...
int touch_driver_init(struct thp_device *tdev)
{
	...
	struct device_node *hx83112_node =
		of_get_child_by_name(cd->thp_node, THP_HX83112_DEV_NODE_NAME);
	...
	if (rc) {
		...
		thp_log_info(tdev->thp_core,
			     "%s:hx_get_frame_optimized_method_flag %d\n",
			     __func__, value);
	}

	rc = of_property_read_u32(hx83112_node, "himax_ic_83102_flag", &value);
	if (rc) {
	...
	rc = of_property_read_u32(hx83112_node, "himax_ic_83121_flag", &value);
	if (rc) {
	...
	rc = of_property_read_u32(hx83112_node, "himax_ic_83122_flag", &value);
	if (rc) {
	...
	rc = of_property_read_u32(hx83112_node, "himax_ic_hispeed_support",
				  &value);
	if (rc) {
	...
	rc = of_property_read_u32(hx83112_node, "himax_gesture_need_lcd_rst",
				  &value);
	if (rc) {
	...
	rc = thp_parse_timing_config(tdev->thp_core, hx83112_node,
				     &tdev->timing_config);
	if (rc) {
	...
	if (ret) {
		thp_log_err(tdev->thp_core,
			    "%s: Failed to configure regulator\n", __func__);
		return ret;
	}
#else
...
static int touch_driver_sense_off(struct thp_device *tdev)
{
	...
	touch_driver_register_write(tdev, tmp_addr, HIMAX_NORMAL_ADDR_LEN,
				    tmp_data);
	...
	do {
		...
		if ((touch_driver_bus_write(tdev, HIMAX_CONTI_BURST_ADDR,
					    tmp_data, HIMAX_REG_DATA_LEN,
					    HIMAX_BUS_RETRY_TIMES)) < 0)
			return -ENOMEM;
		if ((himax_p->himax_ic_83121_flag) ||
		    (himax_p->himax_ic_83122_flag))
			himax_assign_data(HIMAX_121A_AUTO_PLUS_4_DIS, tmp_data);
		else
			himax_assign_data(HIMAX_AUTO_PLUS_4_DIS, tmp_data);
		if ((touch_driver_bus_write(tdev, HIMAX_AUTO_PLUS_4_ADDR,
					    tmp_data, HIMAX_REG_DATA_LEN,
					    HIMAX_BUS_RETRY_TIMES)) < 0)
			return -ENOMEM;

		himax_assign_data(HIMAX_CHK_FW_STATUS, tmp_addr);
		ret = touch_driver_register_read(
			tdev, tmp_addr, HIMAX_NORMAL_DATA_LEN, tmp_data);
		if (ret)
			return ret;
		thp_log_info(tdev->thp_core,
			     "%s: Check save mode data[0] = %02X\n", __func__,
			     tmp_data[0]);

		if (tmp_data[0] == HIMAX_FW_SSOFF) {
			if (himax_p->himax_ic_83102_flag) {
				himax_assign_data(HIMAX_102E_TCON_RST_ADDR,
						  tmp_addr);
				himax_assign_data(HIMAX_102E_TCON_RST_ON_CMD,
						  tmp_data);
			} else {
				himax_assign_data(HIMAX_102D_TCON_RST_ADDR,
						  tmp_addr);
				himax_assign_data(HIMAX_102D_TCON_RST_ON_CMD,
						  tmp_data);
			}
			touch_driver_register_write(tdev, tmp_addr,
						    HIMAX_NORMAL_ADDR_LEN,
						    tmp_data);
		...
		if ((touch_driver_bus_write(tdev, HIMAX_SSOFF_ADDR_FIRST,
					    tmp_data, HIMAX_REG_DATA_LEN,
					    HIMAX_BUS_RETRY_TIMES)) < 0)
			return -ENOMEM;

		himax_assign_data(HIMAX_SSOFF_CMD_SECOND, tmp_data);
		if ((touch_driver_bus_write(tdev, HIMAX_SSOFF_ADDR_SECOND,
					    tmp_data, HIMAX_REG_DATA_LEN,
					    HIMAX_BUS_RETRY_TIMES)) < 0)
			return -ENOMEM;
...
static int touch_driver_ddreg_read(struct thp_device *tdev, uint8_t cmd,
				   int bank_idx, int start_para, int read_size,
				   uint8_t *out)
{
	int i;
	int ret = NO_ERR;
	...
	dd_reg_reformat = HX_DDREG_FORMAT + (cmd << HX_DDREG_REG_IDX) +
	...
	thp_log_info(tdev->thp_core,
		     "%s:bank_idx=%d, start_para=%d, read_size=%d\n", __func__,
		     bank_idx, start_para, read_size);

	for (i = 0; i < read_size; i++) {
		himax_assign_data(dd_reg_reformat + i, tmp_addr);
		ret = touch_driver_register_read(
			tdev, tmp_addr, HIMAX_NORMAL_DATA_LEN, tmp_data);
		if (ret)
			return ret;
		thp_log_info(tdev->thp_core,
			     "%s:use %d;[3]=0x%X,[2]=0x%X,[1]=0x%X,[0]=0x%X\n",
			     __func__,
			     ((start_para + i) % HIMAX_NORMAL_DATA_LEN),
...
static void touch_driver_rw_reg_reformat_com(uint32_t addr, uint32_t data,
					     uint8_t *addr_arr,
					     uint8_t *data_arr)
{
...
static int touch_driver_9l_read_ddreg_en(struct thp_device *tdev, int en)
{
	...
	ret = touch_driver_register_read(tdev, tmp_addr, HIMAX_NORMAL_DATA_LEN,
					 tmp_data);
	if (ret)
		return ret;
	if (tmp_data[DATA_0] != ENTER_SAVE_MODE) {
		thp_log_info(tdev->thp_core,
			     "%s: Doesn't enter save mode, need sense off!\n",
			     __func__);
		...
		thp_log_info(tdev->thp_core,
			     "%s: Already enter into save mode!\n", __func__);
	}
	if (en) {
		thp_log_info(tdev->thp_core,
			     "%s: Ready to enable to read ddreg!\n", __func__);
		touch_driver_rw_reg_reformat_com(ADDR_DD_OSC_HX83102,
						 DATA_DD_OSC_HX83102_EN,
						 &tmp_addr[DATA_0],
						 &tmp_data[DATA_0]);
		touch_driver_register_write(tdev, tmp_addr,
		...
		thp_log_info(tdev->thp_core,
			     "%s: Ready to disable to read ddreg!\n", __func__);
		touch_driver_rw_reg_reformat_com(ADDR_DD_PW_HX83102, DATA_INIT,
						 &tmp_addr[DATA_0],
						 &tmp_data[DATA_0]);
		touch_driver_register_write(tdev, tmp_addr,
					    HIMAX_NORMAL_DATA_LEN, tmp_data);
		msleep(REG_WRITE_INTERVAL);
		touch_driver_rw_reg_reformat_com(ADDR_DD_OSC_HX83102, DATA_INIT,
						 &tmp_addr[DATA_0],
						 &tmp_data[DATA_0]);
		touch_driver_register_write(tdev, tmp_addr,
					    HIMAX_NORMAL_DATA_LEN, tmp_data);
				      ...
				      uint8_t auto_add_4_byte)
{
	...
	if (touch_driver_bus_write(tdev, ADDR_AHB, tmp_data, sizeof(tmp_data),
				   HIMAX_BUS_RETRY_TIMES) < DATA_0) {
		thp_log_err(tdev->thp_core, "%s: spi access fail!\n", __func__);
		return;
	}
					  ...
					  int reload_length)
{
	...
	touch_driver_register_write(tdev, tmp_addr, HIMAX_NORMAL_DATA_LEN,
				    start_addr);

	tmp_data[DATA_3] = CRC_DATA_THREE;
	tmp_data[DATA_2] = CRC_DATA_TWO;
	tmp_data[DATA_1] = length >> ONE_BYTE_LEN;
	...
	do {
		...
		ret = touch_driver_register_read(tdev, tmp_addr, FOUR_BYTE_CMD,
						 tmp_data);
		if (ret)
			return ret;
		thp_log_info(tdev->thp_core,
			     "%s: >>>>>>1.tmp_data[3]=%X,tmp_data[2]=%X\n",
			     __func__, tmp_data[DATA_3], tmp_data[DATA_2]);
		thp_log_info(tdev->thp_core,
			     "%s: >>>>>>2.tmp_data[1]=%X,tmp_data[0]=%X\n",
		...
		if ((tmp_data[DATA_0 + mod] & LOW_BIT) == LOW_BIT) {
			...
			ret = touch_driver_register_read(
				tdev, tmp_addr, FOUR_BYTE_CMD, tmp_data);
			if (ret)
				return ret;
			thp_log_info(
				tdev->thp_core,
				"%s: >>>>>>3.tmp_data[3]=%X, tmp_data[2]=%X\n",
				__func__, tmp_data[DATA_3], tmp_data[DATA_2]);
			thp_log_info(
				tdev->thp_core,
				"%s: >>>>>>4.tmp_data[1]=%X, tmp_data[0]=%X\n",
				__func__, tmp_data[DATA_1], tmp_data[DATA_0]);
...
static int touch_driver_projectid(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (type == STR_TYPE) {
		g_honor_project_id = kcalloc(
			(len + 1), sizeof(*g_honor_project_id), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_honor_project_id =
			kcalloc(len, sizeof(*g_honor_project_id), GFP_KERNEL);
	} else {
		thp_log_err(cd, "%s: project id UNKNOWN type %d\n", __func__,
			    type);
	}
	if (g_honor_project_id) {
		memcpy(g_honor_project_id, buffer + MOVE_8BIT, len);
...
static int touch_driver_cgcolor(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (type == STR_TYPE) {
		g_honor_cg_color = kcalloc((len + 1), sizeof(*g_honor_cg_color),
					   GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_honor_cg_color =
			kcalloc(len, sizeof(*g_honor_cg_color), GFP_KERNEL);
	} else {
		thp_log_err(cd, "%s: CG color UNKNOWN type %d\n", __func__,
			    type);
	}
	if (g_honor_cg_color) {
		memcpy(g_honor_cg_color, buffer + MOVE_8BIT, len);
...
static int touch_driver_sensorid(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (type == STR_TYPE) {
		g_honor_sensor_id = kcalloc(
			(len + 1), sizeof(*g_honor_sensor_id), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_honor_sensor_id =
			kcalloc(len, sizeof(*g_honor_sensor_id), GFP_KERNEL);
	} else {
		thp_log_err(cd, "%s: sensor id UNKNOWN type %d\n", __func__,
			    type);
	}
	if (g_honor_sensor_id) {
		memcpy(g_honor_sensor_id, buffer + MOVE_8BIT, len);
...
static int touch_driver_read_panel_info(struct thp_device *tdev)
{
	...
	if (himax_p->himax_ic_hispeed_support) {
		himax_assign_data(HIMAX_HISPEED_SUPPORT_REG, tmp_addr);
		himax_assign_data(HIMAX_HISPEED_SUPPORT_VAL, tmp_data);
		touch_driver_register_write(tdev, tmp_addr,
	...
	for (i = DATA_0; i < INFO_SECTION_NUM; i++) {
		...
		if (touch_driver_hw_check_crc(tdev, tmp_addr,
					      INFO_PAGE_LENGTH) != DATA_0) {
			thp_log_err(
				tdev->thp_core,
				"%s:section %d CRC FAIL tmp_addr = 0x%02X, DATA_0 = 0x%02X\n",
		...
		for (temp_addr = saddr; temp_addr < saddr + INFO_PAGE_LENGTH;
			...
			ret = touch_driver_register_read(tdev, tmp_addr, DATA_4,
							 tmp_buf);
			if (ret)
				goto err;
		...
		if ((title[DATA_0] == DATA_0) && (title[DATA_1] == DATA_1) &&
			...
			if (ret) {
				thp_log_err(tdev->thp_core,
					    "%s: projectid cgcolor fail\n",
					    __func__);
				goto err;
			}
		} else if ((title[DATA_0] == 1) && (title[DATA_1] == 1) &&
			   (title[DATA_2] == 1)) {
			/* sensor id */
			ret = touch_driver_sensorid(type, len, buffer);
			if (ret) {
				thp_log_err(tdev->thp_core,
					    "%s: sensorid read fail\n",
					    __func__);
				goto err;
			}
		} else {
			thp_log_err(tdev->thp_core,
				    "%s: UNKNOWN title %X%X%X\n", __func__,
				    title[DATA_0], title[DATA_1],
				    title[DATA_2]);
			goto err;
		}
	}
...
static int touch_driver_9l_ic_check(struct thp_device *tdev)
{
	...
	touch_driver_ddreg_read(tdev, ADDR_DD_ICID_DETAIL_HX83102, DATA_0,
				DATA_1, DATA_8, dd_icid_detail);
	thp_log_info(tdev->thp_core,
		     "Now [8]=0x%X,[7]=0x%X,[6]=0x%X,[5]=0x%X\n",
		     dd_icid_detail[DATA_7], dd_icid_detail[DATA_6],
		     dd_icid_detail[DATA_5], dd_icid_detail[DATA_4]);
	thp_log_info(tdev->thp_core,
		     "Now [4]=0x%X,[3]=0x%X,[2]=0x%X,[1]=0x%X\n",
		     dd_icid_detail[DATA_3], dd_icid_detail[DATA_2],
		     dd_icid_detail[DATA_1], dd_icid_detail[DATA_0]);
	touch_driver_ddreg_read(tdev, ADDR_DD_PEN_EN_HX83102, DATA_1, DATA_1,
				DATA_1, dd_pen_func_check);
...
static int touch_driver_9l_communication_check(struct thp_device *tdev)
{
	...
	for (i = 0; i < COMM_TEST_RW_RETRY_TIME; i++) {
		...
		thp_log_err(tdev->thp_core,
			    "%s:Read driver ID register Fail:\n", __func__);
	}
...
static int touch_driver_121_communication_check(struct thp_device *tdev)
{
	...
	for (i = 0; i < COMM_TEST_RW_RETRY_TIME; i++) {
		...
		ret = touch_driver_register_read(
			tdev, tmp_addr, HIMAX_NORMAL_DATA_LEN, tmp_data);
		thp_log_info(tdev->thp_core, "%s:Read driver IC ID\n",
			     __func__);
		for (j = DATA_0; j < ARRAY_SIZE(himax_id_match_table); j++) {
			vendor_assign_data(himax_id_match_table[j], ic_name,
					   HIMAX_NORMAL_DATA_LEN);
...
static int touch_driver_get_dsram_data(struct thp_device *tdev, char *info_data,
				       unsigned int len)
{
	unsigned char tmp_addr[HIMAX_NORMAL_ADDR_LEN] = {0};
	unsigned int read_size = len;
...
static int touch_driver_get_event_stack(struct thp_device *tdev, char *buf,
					unsigned int len)
{
	unsigned int read_size = len;
	...
	if (touch_driver_bus_read(tdev, HIMAX_EVENT_STACK_CMD_ADDR, buf,
				  read_size, HIMAX_BUS_RETRY_TIMES) < 0) {
		thp_log_err(tdev->thp_core, "%s: spi access fail!\n", __func__);
		thp_bus_unlock(tdev->thp_core);
		return -EINVAL;
	}
...
int touch_driver_83102_get_frame(struct thp_device *tdev, char *buf,
				 unsigned int len)
{
	struct himax_thp_private_data *himax_p = NULL;
...
int touch_driver_get_project_id(struct thp_device *tdev, char *buf,
				unsigned int len)
{
	...
	memcpy(buf, g_honor_project_id, HIMAX_ACTUAL_PROJECTID_LEN);
	...
	strncpy(temp_buf,
		buf + THP_PROJECTID_PRODUCT_NAME_LEN +
	...
	if (!strncmp(temp_buf, VENDOR_ID_BOE, THP_PROJECTID_VENDOR_NAME_LEN)) {
		cd->vendor_name = VENDOR_NAME_BOE;
	} else if (!strncmp(temp_buf, VENDOR_ID_BOE_1,
			    THP_PROJECTID_VENDOR_NAME_LEN)) {
		cd->vendor_name = VENDOR_NAME_BOE;
	} else if (!strncmp(temp_buf, VENDOR_ID_INX,
			    THP_PROJECTID_VENDOR_NAME_LEN)) {
		cd->vendor_name = VENDOR_NAME_INX;
	} else if (!strncmp(temp_buf, VENDOR_ID_CSOT,
			    THP_PROJECTID_VENDOR_NAME_LEN)) {
...
static void touch_driver_clear_all_interrupt(struct thp_device *tdev)
{
	uint8_t tmp_addr[HIMAX_NORMAL_ADDR_LEN] = {0};
	uint8_t tmp_data[HIMAX_NORMAL_DATA_LEN] = {0};
	int ret;
	...
	if (ret) {
		thp_log_err(tdev->thp_core,
			    "%s: read clear all interrupt register fail\n",
			    __func__);
		thp_bus_unlock(tdev->thp_core);
		return;
	}
	...
	thp_log_info(
		...
		__func__, HIMAX_CMD_CLEAR_INTERRUPT_ADDR, tmp_data[3],
...
static void touch_driver_enter_gesture_mode(struct thp_device *tdev)
{
	...
	if (ret) {
		thp_log_err(tdev->thp_core,
			    "%s: read gesture wakeup register fail\n",
			    __func__);
		thp_bus_unlock(tdev->thp_core);
		return;
	}
	...
	thp_log_info(
		tdev->thp_core,
		"%s: read 0x%X:data[3] = 0x%X, data[2] = 0x%X, data[1] = 0x%X, data[0] = 0x%X\n",
		__func__, HIMAX_GESTURE_WAKEUP_ADDR, tmp_data[3], tmp_data[2],
...
int touch_driver_83112_resume(struct thp_device *tdev)
{
	...
	gpio_set_value(tdev->gpios->rst_gpio, THP_RESET_HIGH);
	thp_time_delay(tdev->thp_core,
		       tdev->timing_config.resume_reset_after_delay_ms);
#endif
	himax_p = tdev->private_data;
