...
#define SPI_APP_BUF_SIZE_WRITE \
	(SPI_APP_HEADER_LEN + SPI_APP_DATA_MAX_LEN + WRITE_DUMMY_BYTE)
#define SPI_APP_BUF_SIZE_READ                                            \
	(NUMBER_OF_HEADER * SPI_APP_HEADER_LEN + T117_BYTES_READ_LIMIT + \
	 FRAME_READ_DUMMY_BYTE)
/* for tp ic mxt3662 */
...
static int touch_driver_wakeup_gesture_en_switch(struct thp_device *tdev,
						 u8 switch_value);
...
static void spi_prepare_header(u8 *header, u8 opcode, u16 start_register,
			       u16 count)
{
	int index = 0;
...
static int bootloader_one_byte_transfer(struct thp_device *tdev,
...
static int touch_driver_check_bootloader_status(struct thp_device *tdev,
						u8 const *status_byte,
						int status_byte_size)
{
	int i;
...
static int touch_driver_check_is_bootloader_for_3662(struct thp_device *tdev)
{
	...
	do {
		thp_log_info("%s:call bootloader_one_byte_transfer\n",
			     __func__);
		memcpy(tdev->tx_buff, op_header, sizeof(op_header));
		ret = bootloader_one_byte_transfer(
			tdev, tdev->tx_buff, tdev->rx_buff,
		...
		thp_log_info("%s:status_byte %*ph\n", __func__,
			     BOOTLOADER_READ_LEN_FOR_MXT3662, status_byte);
		ret = touch_driver_check_bootloader_status(
			tdev, status_byte, BOOTLOADER_READ_LEN_FOR_MXT3662);
		if (ret == 0) {
				 ...
				 u16 len, u8 *val)
{
	...
	do {
		...
		memset(tx_buf, 0xFF,
		       (NUMBER_OF_HEADER * SPI_APP_HEADER_LEN + dummy_byte));
...
static int touch_driver_read_blks(struct thp_device *tdev,
				  struct spi_device *client, u16 start,
				  u16 count, u8 *buf, u16 override_limit)
{
	u16 offset = 0;
	int ret_val;
	u16 size;
				   ...
				   u16 count, u8 *buf)
{
	...
	while (offset < count) {
		...
		ret_val = touch_driver_write_reg(tdev, client, start + offset,
						 size, buf + offset);
		if (ret_val)
			break;
		offset += size;
				       ...
				       unsigned int len)
{
	...
	do {
		ret = touch_driver_write_blks(
			tdev, tdev->thp_core->sdev,
...
static int touch_driver_update_obj_addr(struct thp_device *tdev)
{
	...
	ret_val = touch_driver_read_blks(tdev, tdev->thp_core->sdev,
					 MXT_OBJECT_START, curr_size, buff,
...
static int set_bootloader_to_normal_mode(struct thp_device *tdev)
{
	u8 status_byte = 0;
	int ret;
	/* app mode cmd */
		...
		if (tdev->thp_core->support_vendor_ic_type == MXT3662) {
			...
			if (ret_val)
				thp_log_err(
					...
					__func__);
			ret_val = touch_driver_bootloader_read(
				tdev, tdev->thp_core->sdev, &status_byte,
				1); /* 1: status_byte len */
			if (ret_val)
				thp_log_info("%s:bootloader read failed\n",
					     __func__);
					 ...
					 struct mxt_info *mxtinfo)
{
	...
	thp_log_info("%s call\n", __func__);
	do {
		ret_val = touch_driver_read_blks(
			tdev, tdev->thp_core->sdev, CHIP_IDENTIFIED_START_ADDR,
...
static int touch_driver_read_obj_addr(struct thp_device *tdev, size_t curr_size,
				      const struct mxt_info *mxtinfo)
{
	int ret_val;
	int i;
	u8 *buff = NULL;
	...
	curr_size +=
		mxtinfo->object_num * sizeof(*object) + MXT_INFO_CHECKSUM_SIZE;
	buff = kzalloc(curr_size, GFP_KERNEL);
	if (buff == NULL) {
...
static int touch_driver_get_frame_for_3662(struct thp_device *tdev, char *buf)
{
	int ret_val;
	...
	ret_val = touch_driver_read_blks(
		...
		(buf + T117_HEADER_DATA_READ_LEN), T117_BYTES_READ_LIMIT);
	if (ret_val)
...
static int touch_driver_set_screen_off_mode(struct thp_device *tdev)
{
	...
	ret_val = touch_driver_write_blks(tdev, tdev->thp_core->sdev,
					  t7_address, sizeof(t7_idle_scan_rate),
					  &t7_idle_scan_rate);
	if (ret_val != 0)
					      ...
					      uint32_t *dozing_time)
{
	...
	ret_val =
		touch_driver_read_blks(tdev, tdev->thp_core->sdev,
...
static int
touch_driver_set_fw_update_mode(struct thp_device *tdev,
...
static void touch_driver_check_screen_off_mode(struct thp_device *tdev)
{
	...
	if (!ret_val) {
		if ((buffer & SSL_SCREEN_OFF_MODE) != SSL_SCREEN_OFF_MODE) {
			...
			ret_val =
				touch_driver_wakeup_gesture_en_switch(tdev, 1);
			if (ret_val != 0)
				thp_log_err(
					"Failed to send wakeup gesture enable command\n");
...
static int touch_driver_spi_transfer_one_byte_bootloader(
	struct thp_device *tdev, const char *const tx_buf, char *const rx_buf,
	const unsigned int buf_len)
{
