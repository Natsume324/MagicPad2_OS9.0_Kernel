...
static int touch_driver_spi_read_write(struct spi_device *client, void *tx_buf,
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	struct device_node *nova_node =
		of_get_child_by_name(cd->thp_node, THP_NOVA_DEV_NODE_NAME);
				  ...
				  unsigned int len)
{
	...
	ret = touch_driver_spi_read_write(tdev->thp_core->sdev, w_buf, buf,
					  len);
	if (ret)
		thp_log_err("%s, spi transfer failed\n", __func__);
					    ...
					    uint8_t *r_buf, uint8_t *w_buf)
{
	...
	thp_log_err("found invalid gesture id: %02X-%02X-%02X-%02X-%02X-%02X\n",
		    r_buf[6], r_buf[7], r_buf[8], r_buf[9], r_buf[10],
		    r_buf[11]);
	return -EINVAL;
}
...
static int gesture_data_read_write(struct thp_device *tdev)
{
	...
	retval = touch_driver_spi_read_write(tdev->thp_core->sdev, w_buf, r_buf,
					     GESTURE_DATA_LEN);
	if (retval < 0) {
				      ...
				      unsigned int stamp_addr)
{
	...
	rw_len = SEND_CMD_LEN;
	ret = touch_driver_spi_read_write(tdev->thp_core->sdev, w_buf, NULL,
					  rw_len);
	if (ret) {
	...
	rw_len = SEND_CMD_LEN;
	ret = touch_driver_spi_read_write(tdev->thp_core->sdev, w_buf, r_buf,
					  rw_len);
	if (ret) {
	...
	thp_log_info("%s current stamp is %x %x [%x]\n", __func__, r_buf[0],
		     r_buf[1], r_buf[2]); /* byte2 is valid value */
...
static int touch_driver_send_fw_cmd(struct thp_device *tdev, uint8_t cmd)
{
	...
	w_buf[4] = 0x00;
	w_buf[5] = stamp_val - 1;
	w_buf[6] = (uint8_t)(~(w_buf[1] + w_buf[2] + w_buf[3] + w_buf[4] +
			       w_buf[5]) +
			     1);
	ret = touch_driver_spi_read_write(tdev->thp_core->sdev, w_buf, NULL,
					  D_SEND_CMD_LEN);
...
static void touch_driver_enter_gesture_mode(struct thp_device *tdev)
{
	...
	retval = touch_driver_send_fw_cmd(tdev, NVT_CMD_ENTER_GESTURE_MODE);
	if (retval < 0)
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->support_gesture_mode &&
		...
		if (pt_test_mode) { /* PT test spec */
			...
			(void)touch_driver_send_fw_cmd(
				tdev, NVT_CMD_ENTER_CUSTOMIZED_DP_DSTB);
#ifndef CONFIG_HONOR_THP_MTK
