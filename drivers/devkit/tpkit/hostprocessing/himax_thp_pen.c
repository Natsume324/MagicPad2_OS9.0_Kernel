...
#endif
static int pen_driver_83112_get_frame(struct thp_device *tdev, char *buf,
				      unsigned int len);
enum Temp_Data {
	PEN_DATA_0 = 0,
	PEN_DATA_1,
...
static void pen_vendor_assign_data(uint32_t cmd, uint8_t *tmp_value,
				   uint32_t length)
{
...
static int pen_driver_spi_read(struct thp_device *tdev, uint8_t *command,
...
static int pen_driver_spi_write(struct thp_device *tdev, uint8_t *buf,
				unsigned int length)
{
	int ret;
	struct spi_message message;
	...
	g_transfer[0].bits_per_word = SPI_PEN_BITS_PER_WORD_READ;
...
static int pen_driver_83102_bus_read(struct thp_device *tdev, uint8_t command,
				     ...
				     uint8_t toretry)
{
	...
	return pen_driver_spi_read(tdev, &spi_format_buf[PEN_DATA_0],
				   SPI_PEN_FORMAT_ARRAY_SIZE, data, length,
				   toretry);
}
			       ...
			       uint8_t to_retry)
{
	...
	if ((himax_p->himax_ic_83102_flag) || (himax_p->himax_ic_83121_flag) ||
		...
		ret = pen_driver_83102_bus_read(tdev, command, data, length,
						to_retry);
		return ret;
	}
	if (!spi_format_buf) {
...
static int pen_driver_83102_bus_write(struct thp_device *tdev, uint8_t command,
				      uint8_t *data, unsigned int length,
				      uint8_t toretry)
{
	uint8_t spi_format_buf[HIMAX_NORMAL_PEN_DATA_LEN +
			       HIMAX_NORMAL_PEN_ADDR_LEN + PEN_DATA_2];
	int i;
				...
				uint8_t to_retry)
{
	...
	if ((himax_p->himax_ic_83102_flag) || (himax_p->himax_ic_83121_flag) ||
	    (himax_p->himax_ic_83122_flag)) {
		ret = pen_driver_83102_bus_write(tdev, command, data, length,
						 to_retry);
		return ret;
	}
	if (!spi_format_buf) {
...
static int pen_driver_register_read(struct thp_device *tdev, uint8_t *read_addr,
				    ...
				    uint8_t *read_data)
{
	...
	address = (read_addr[3] << MOVE_24BIT_PEN) +
		  (read_addr[2] << MOVE_16BIT_PEN) +
					 ...
					 unsigned int *gesture_wakeup_value)
{
	...
	if (!strncmp(cd->vendor_name, VENDOR_NAME_BOE_PEN,
		     THP_PROJECTID_PEN_VENDOR_NAME_LEN)) {
		tx_num = HX_BOE_TX_NUM_PEN;
		rx_num = HX_BOE_RX_NUM_PEN;
	} else if (!strncmp(cd->vendor_name, VENDOR_NAME_INX_PEN,
			    THP_PROJECTID_PEN_VENDOR_NAME_LEN)) {
				     ...
				     unsigned int *gesture_wakeup_value)
{
	...
	for (i = 0; i < GESTURE_PEN_EVENT_RETRY_TIME; i++) {
		retval = pen_driver_83112_get_frame(tdev, frame_read_buf,
						    THP_MAX_FRAME_SIZE);
		if (retval) {
			thp_log_err(tdev->thp_core,
	...
	retval = pen_driver_parse_gesture_data(tdev, frame_read_buf,
...
static int pen_driver_init(struct thp_device *tdev)
{
	...
	struct device_node *hx83112_node = of_get_child_by_name(
		cd->thp_node, THP_PEN_HX83122_DEV_NODE_NAME);
	...
	rc = thp_pen_parse_timing_config(tdev->thp_core, hx83112_node,
					 &tdev->timing_config);
	if (rc) {
...
static int pen_driver_121_communication_check(struct thp_device *tdev)
{
	...
	for (i = 0; i < COMM_PEN_TEST_RW_RETRY_TIME; i++) {
		...
		ret = pen_driver_register_read(
			tdev, tmp_addr, HIMAX_NORMAL_PEN_DATA_LEN, tmp_data);
		thp_log_info(tdev->thp_core, "%s:Read driver IC ID\n",
			     __func__);
		for (j = PEN_DATA_0; j < ARRAY_SIZE(himax_pen_id_match_table);
		     j++) {
			...
			if ((tmp_data[PEN_DATA_3] == HX_PEN_83122A_ID_PART_1) &&
				...
				break;
			} else if ((tmp_data[PEN_DATA_3] ==
				    HX_PEN_83121A_ID_PART_1) &&
				   (tmp_data[PEN_DATA_2] ==
				    HX_PEN_83121A_ID_PART_2) &&
				   (tmp_data[PEN_DATA_1] ==
				    HX_PEN_83121A_ID_PART_3)) {
				break;
			} else if ((tmp_data[PEN_DATA_3] ==
				    ic_name[PEN_DATA_3]) &&
				   (tmp_data[PEN_DATA_2] ==
...
static int pen_driver_get_dsram_data(struct thp_device *tdev, char *info_data,
				     unsigned int len)
{
	unsigned char tmp_addr[HIMAX_NORMAL_PEN_ADDR_LEN] = {0};
	unsigned int read_size = len;
...
static int pen_driver_get_event_stack(struct thp_device *tdev, char *buf,
				      unsigned int len)
{
	unsigned int read_size = len;
	...
	if (pen_driver_bus_read(tdev, HIMAX_PEN_EVENT_STACK_CMD_ADDR, buf,
				read_size, HIMAX_PEN_BUS_RETRY_TIMES) < 0) {
		thp_log_err(tdev->thp_core, "%s: spi access fail!\n", __func__);
		thp_bus_unlock(tdev->thp_core);
		return -EINVAL;
	}
...
static int pen_driver_83102_get_frame(struct thp_device *tdev, char *buf,
				      unsigned int len)
{
	struct himax_thp_pen_private_data *himax_p = NULL;
