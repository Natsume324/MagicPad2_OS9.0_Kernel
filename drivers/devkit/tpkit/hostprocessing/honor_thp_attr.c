...
#define AOD_OFF_RESUME_SUSPEN_STATE 0
#define AOD_ON_SUSPEN_STATE 1
#define AOD_ON_RESUME_STATE 2

u8 g_thp_log_cfg;
EXPORT_SYMBOL(g_thp_log_cfg);
extern int thp_init_chip_info(struct thp_core_data *cd);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if ((cd->support_get_project_id_late) &&
	    !strcmp(cd->project_id, "\0")) {
		thp_log_info(cd, "%s: read project_id late\n", __func__);
		thp_init_chip_info(cd);
	}
					...
					const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%4d %4d %4d %4d %4d", &window_enable, &x0, &y0, &x1,
		     &y1);
	if (ret <= 0) {
		thp_log_err(cd, "%s: illegal input\n", __func__);
		return -EBUSY;
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE - 1, "%d %d %d %d %d\n",
			thp_get_status(cd, THP_STATUS_HOLSTER), window->x0,
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	if ((dev == NULL) || (attr == NULL) || (buf == NULL) || (cd == NULL)) {
					...
					const char *buf, size_t count)
{
	...
	case STYLUS_WAKEUP_TESTMODE_IN:
		...
		if (cd->send_bt_status_to_fw) {
			...
			if ((cur_status & STYLUS3_CONNECT) != STYLUS3_CONNECT) {
				...
				atomic_set(&cd->last_stylus3_status,
					   STYLUS3_CONNECT);
				if (cd->thp_dev->ops->bt_handler)
					if (cd->thp_dev->ops->bt_handler(
						...
						thp_log_err(
							cd,
							"ioctl mmi in send bt status fail\n");
				atomic_set(&cd->last_stylus3_status,
					   cur_status);
			}
		}
			...
			if ((cur_status & STYLUS3_CONNECT) != STYLUS3_CONNECT) {
				...
				if (cd->thp_dev->ops->bt_handler)
					if (cd->thp_dev->ops->bt_handler(
						...
						thp_log_err(
							cd,
							"ioctl mmi out send bt status fail\n");
				atomic_set(&cd->last_stylus3_status,
					   cur_status);
			}
		}
	...
	case STYLUS_GESTURE_WAKEUP_ENABLE:
		if (cd->support_pen_wakeup_gesture) {
			...
			if (cd->thp_dev->ops->bt_handler)
				if (cd->thp_dev->ops->bt_handler(cd->thp_dev,
								 false))
					thp_log_err(
						cd,
						"send bt status to fw fail\n");
		}
		break;
					     ...
					     const char *buf, size_t size)
{
	...
	if (info->easy_wakeup_gesture == false) {
		...
		thp_lcd_status_notifier(cd, TS_DOUBLE_CLICK_SWITCH_OFF);
		thp_log_info(cd, "poweroff mode\n");
	} else {
		cd->sleep_mode = TS_GESTURE_MODE;
		thp_lcd_status_notifier(cd, TS_DOUBLE_CLICK_SWITCH_ON);
						...
						char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE - 1, "%u,%u\n",
			cd->udfp_status.udfp_enable,
			cd->udfp_status.udfp_switch);
}
					  ...
					  const char *buf, size_t count)
{
	...
	if ((cd->suspended || cd->support_ring_feature) &&
		...
		ret = send_thp_driver_status_cmd(ring_status, 0,
	...
	case SUSPEND_NEED_CAP_TEST:
		if (thp_is_factory()) {
			...
			if (data == NEED_SCREEN_OFF) {
				...
				msleep(100);
				honorfb_set_backlight(
					honorfd,
					cd->factory_mode_current_backlight,
					false);
				thp_log_info(
					cd, "%s lcd power on current_bl = %d\n",
					__func__,
					cd->factory_mode_current_backlight);
				cd->always_poweron_in_screenoff = data;
			}
...
/*
 * AFT in the following function is an abbreviation for anti false touch
 */
static ssize_t set_phone_call_aft_event_store(struct device *dev,
					      struct device_attribute *attr,
					      const char *buf, size_t count)
{
	...
	value = buf[0] - '0';
	cd->phone_call_aft_event_status = value;
	thp_log_info(cd, "%s: phone_call_aft_event_status = %d\n", __func__,
		     cd->phone_call_aft_event_status);
	sysfs_notify(&dev->kobj, NULL, "set_phone_call_aft_event");
	return count;
}

static ssize_t set_phone_call_aft_event_show(struct device *dev,
					     struct device_attribute *attr,
					     char *buf)
{
	struct thp_core_data *cd = NULL;
					   ...
					   const char *buf, size_t count)
{
	...
	thp_log_info(cd, "%s: update aod_touch_enable: %u\n", __func__,
		     aod_touch_enable);
	return count;
}
					      ...
					      const char *buf, size_t count)
{
	...
	thp_screen_switch(screen_status);
					 ...
					 const char *buf, size_t count)
{
	...
	notify_tp_lowpower_status(cd, status);
	return count;
}
						...
						const char *buf, size_t count)
{
	...
	if (status == NOTIFY_TP_STANDBY_IN || status == NOTIFY_TP_STANDBY_OUT) {
		if (cd->is_fold_device && cd_sub)
			cd_sub->in_standby_state = 1;
		cd->in_standby_state = 1;
	}
	...
	if (cd->is_fold_device) {
		...
		cd->aod_notify_tp_power_status = status;
		switch (status) {
		case AOD_OFF_RESUME_SUSPEN_STATE:
			thp_lcd_status_notifier(cd, TS_AOD_OFF_RESUME_SUSPEN);
			break;
		case AOD_ON_SUSPEN_STATE:
			thp_lcd_status_notifier(cd, TS_AOD_ON_SUSPEN);
			break;
		case AOD_ON_RESUME_STATE:
			thp_lcd_status_notifier(cd, TS_AOD_ON_RESUME);
			break;
		default:
			thp_log_err(cd, "not support type\n");
			break;
		}
		aod_notify_main_panel_power(cd, status);
	}
...
static ssize_t set_standby_enable_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct thp_core_data *cd = NULL;
	...
	cd->standby_enable = status;
	thp_log_info(cd, "%s: notify standby status %d\n", __func__, status);
	return count;
}
static ssize_t set_standby_enable_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	struct thp_core_data *cd = NULL;
...
static ssize_t
aod_notify_tp_fullaod_doze_status_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct thp_core_data *cd = NULL;
	...
	thp_log_info(cd, "%s: status = %d,panel_index = %d\n", __func__, status,
		     panel_index);

	if (cd->is_fold_device) {
		if (panel_index == MAIN_TOUCH_PANEL) {
			aod_notify_fullaod_doze_status(cd, status);
		} else if (panel_index == SUB_TOUCH_PANEL) {
			aod_notify_fullaod_doze_status(cd_sub, status);
		} else {
			thp_log_err(cd, "panel index is invaild\n");
		}
	} else {
...
static ssize_t fullaod_gesture_mode_support_store(struct device *dev,
						  struct device_attribute *attr,
						  const char *buf, size_t count)
{
	struct thp_core_data *cd = NULL;
	...
	cd->fullaod_support_gesture = status;
	return count;
}
static ssize_t tp_touchpad_config_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct thp_core_data *cd = NULL;
	...
	if (cd->touchpad_supported == 0) {
		thp_log_err(cd, "%s: touchpad not support\n", __func__);
		return -EINVAL;
	}
	ret = sscanf(buf, "%4d %4d %4d %4d %4d", &touchpad_enable, &x0, &y0,
		     &x1, &y1);
	if (ret <= 0) {
		thp_log_err(cd, "%s: illegal input\n", __func__);
		return -EINVAL;
	}
	if ((cd->touchpad.touchpad_enable == touchpad_enable) &&
	    (cd->touchpad.x0 == x0) && (cd->touchpad.y0 == y0) &&
	    (cd->touchpad.x1 == x1) && (cd->touchpad.y1 == y1)) {
		thp_log_err(cd, "%s: repeat cmd, ignore\n", __func__);
		return count;
	}

	cd->touchpad.touchpad_enable = touchpad_enable;
	cd->touchpad.x0 = x0;
	cd->touchpad.y0 = y0;
	cd->touchpad.x1 = x1;
	cd->touchpad.y1 = y1;
	thp_set_status(cd, THP_STATUS_TOUCHPAD_UPDATE,
		       !thp_get_status(cd, THP_STATUS_TOUCHPAD_UPDATE));
	thp_log_info(cd, "%s: update touchpad config %d %d %d %d %d\n",
		     __func__, touchpad_enable, x0, y0, x1, y1);

	return count;
}

static ssize_t tp_touchpad_config_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	struct thp_core_data *cd = NULL;
	...
	return snprintf(buf, PAGE_SIZE - 1, "%d %d %d %d %d\n",
			cd->touchpad.touchpad_enable, cd->touchpad.x0,
...
static DEVICE_ATTR(touch_chip_info, 0640, thp_chip_info_show, NULL);
...
static DEVICE_ATTR(charger_state, 0660, thp_host_charger_state_show,
		   thp_host_charger_state_store);
static DEVICE_ATTR(roi_data, 0660, thp_roi_data_show, thp_roi_data_store);
static DEVICE_ATTR(roi_data_internal, 0400, thp_roi_data_debug_show, NULL);
static DEVICE_ATTR(roi_enable, 0660, thp_roi_enable_show, thp_roi_enable_store);
static DEVICE_ATTR(touch_sensitivity, 0660, thp_holster_enable_show,
		   thp_holster_enable_store);
static DEVICE_ATTR(touch_glove, 0660, thp_glove_enable_show,
		   thp_glove_enable_store);
static DEVICE_ATTR(touch_window, 0660, thp_holster_window_show,
		   thp_holster_window_store);
static DEVICE_ATTR(touch_switch, 0660, thp_touch_switch_show,
		   thp_touch_switch_store);
static DEVICE_ATTR(udfp_enable, 0660, thp_udfp_enable_show,
		   thp_udfp_enable_store);
static DEVICE_ATTR(stylus_wakeup_ctrl, 0660, stylus_wakeup_ctrl_show,
		   stylus_wakeup_ctrl_store);
static DEVICE_ATTR(supported_func_indicater, 0400,
		   thp_supported_func_indicater_show, NULL);
static DEVICE_ATTR(tui_wake_up_enable, 0660, thp_tui_wake_up_enable_show,
		   thp_tui_wake_up_enable_store);
static DEVICE_ATTR(easy_wakeup_gesture, 0600, thp_easy_wakeup_gesture_show,
		   thp_easy_wakeup_gesture_store);
static DEVICE_ATTR(wakeup_gesture_enable, 0600, thp_wakeup_gesture_enable_show,
...
static DEVICE_ATTR(easy_wakeup_position, 0400, thp_easy_wakeup_position_show,
		   NULL);
static DEVICE_ATTR(touch_oem_info, 0640, thp_oem_info_show, NULL);
static DEVICE_ATTR(fingerprint_switch_ctrl, 0660,
		   thp_fingerprint_switch_ctrl_show,
		   thp_fingerprint_switch_ctrl_store);
static DEVICE_ATTR(ring_switch_ctrl, 0660, thp_ring_switch_ctrl_show,
		   thp_ring_switch_ctrl_store);
static DEVICE_ATTR(power_switch_ctrl, 0660, thp_power_switch_ctrl_show,
		   thp_power_switch_ctrl_store);
static DEVICE_ATTR(tp_dfx_info, 0660, NULL, thp_dfx_info_store);
static DEVICE_ATTR(aod_touch_switch_ctrl, 0660, aod_touch_switch_ctrl_show,
		   aod_touch_switch_ctrl_store);
static DEVICE_ATTR(ring_setting_config_ctrl, 0660,
		   thp_ring_setting_config_ctrl_show,
		   thp_ring_setting_config_ctrl_store);
static DEVICE_ATTR(factory_extra_cmd, 0660, NULL, thp_factory_extra_cmd_store);
static DEVICE_ATTR(screen_status_switch, 0660, NULL,
		   thp_screen_status_switch_store);
static DEVICE_ATTR(set_phone_call_aft_event, 0660,
		   set_phone_call_aft_event_show,
		   set_phone_call_aft_event_store);
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
...
static DEVICE_ATTR(aod_notify_tp_power_ctrl, 0660,
		   aod_notify_tp_power_status_show,
		   aod_notify_tp_power_status_store);
static DEVICE_ATTR(set_standby_enable, 0660, set_standby_enable_show,
		   set_standby_enable_store);
static DEVICE_ATTR(aod_notify_tp_fullaod_doze_status, 0220, NULL,
		   aod_notify_tp_fullaod_doze_status_store);
static DEVICE_ATTR(fullaod_support_gesture_status, 0220, NULL,
		   fullaod_gesture_mode_support_store);
static DEVICE_ATTR(tp_touchpad_window_enable, 0660, tp_touchpad_config_show,
		   tp_touchpad_config_store);
#endif
static DEVICE_ATTR(aod_notify_tp_talk_mode, 0660, NULL,
		   aod_notify_tp_talk_mode_store);
...
static struct attribute *thp_ts_attributes[] = {
	...
	&dev_attr_set_phone_call_aft_event.attr,
#if (IS_ENABLED(CONFIG_HONOR_THP_QCOM) || IS_ENABLED(CONFIG_HONOR_THP_MTK))
	&dev_attr_aod_event_report_ctrl.attr,
	&dev_attr_tp_ud_lowpower_ctrl.attr,
	&dev_attr_aod_notify_tp_power_ctrl.attr,
	&dev_attr_set_standby_enable.attr,
	&dev_attr_aod_notify_tp_fullaod_doze_status.attr,
	&dev_attr_fullaod_support_gesture_status.attr,
	&dev_attr_tp_touchpad_window_enable.attr,
#endif
...
int thp_init_sysfs(struct thp_core_data *cd)
{
	...
	rc = thp_vm_init_sysfs(cd, &cd->thp_platform_dev->dev.kobj);
	if (rc) {
		thp_log_err(cd, "%s:can't init vm's sysfs\n", __func__);
		goto err_create_group;
	}
