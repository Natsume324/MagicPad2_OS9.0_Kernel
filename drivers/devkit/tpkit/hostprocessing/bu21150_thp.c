...
lcd_honor_thp_register(struct tp_thp_device_ops *tp_thp_device_ops);
...
static int touch_driver_read_register_(struct thp_device *tdev, u32 addr,
				       u16 size, u8 *data)
{
...
static int touch_driver_read_register(struct thp_device *tdev, u32 addr,
				      u16 size, u8 *data)
{
...
static int touch_driver_write_register(struct thp_device *tdev, u32 addr,
				       u16 size, u8 *data)
{
	struct spi_transfer t;
	struct spi_message msg;
...
static int touch_driver_otp_read(char *otp_data)
{
	...
	for (i = 0; i < OTP_SIZE; i++) {
		...
		touch_driver_read_register(tdev,
					   (OTP_ADDR_REG + i * MULTI_FACTOR),
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	struct device_node *bu21150_node =
		of_get_child_by_name(cd->thp_node, THP_BU21150_DEV_NODE_NAME);
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	rc = touch_driver_write_register(tdev, SENS_START_REG,
					 (u16)sizeof(sens_stop_cmd),
					 sens_stop_cmd);
	if (rc)
		thp_log_err("%s:err to write SENS_START_REG\n", __func__);

	rc = touch_driver_write_register(
		tdev, INT_RUN_ENB_REG, (u16)sizeof(int_stop_cmd), int_stop_cmd);
	if (rc)
