...
static int spi_driver_write_read(struct spi_device *client, uint8_t *cmd,
				 int cmd_length, uint8_t *read_data,
				 int read_count);
...
static int write_u8_ux(struct spi_device *client, uint8_t op_code,
		       uint32_t address, uint8_t *write_buff,
		       uint32_t write_cnt);
...
static int poll_for_event(struct spi_device *client, uint8_t *event_to_search,
			  int event_bytes, uint8_t *read_data,
			  int time_to_wait);
		      ...
		      uint32_t address, uint8_t *read_buff, uint32_t read_cnt)
{
	...
	memset(temp_read_buff, 0,
	       ADDRESS_SAFE_SIZE + read_chunk * sizeof(uint8_t));
	...
	while (read_cnt > 0) {
		...
		*(int *)(write_buff + 1) = htonl(address_now);
		...
		err = spi_driver_write_read(client, write_buff,
					    ADDRESS_SIZE + ADDRESS_SAFE_SIZE,
					    temp_read_buff,
		...
		memcpy(read_buff + read_already,
		       temp_read_buff + ADDRESS_SAFE_SIZE, to_read_now);
		       ...
		       uint32_t write_cnt)
{
	...
	while (write_cnt > 0) {
		...
		*(int *)(temp_buff + 1) = htonl(address_now);
		...
		err = spi_driver_write(client, temp_buff,
				       to_write_now + ADDRESS_SAFE_SIZE +
					       ADDRESS_SIZE);
		if (err != 0) {
			thp_log_err("%s: spi transfer error\n", __func__);
			return err;
			    ...
			    struct thp_udfp_data *udfp_data, unsigned int len)
{
	...
	udfp_data->tpud_data.tp_x =
		read_buff[4] + ((read_buff[5] & KEEP_LOW_BIT) << 8);
	...
	thp_log_info("%s:version:%x, udfp:%d, x:%d, y:%d, aod:%d, key:%d\n",
		     ...
		     udfp_data->tpud_data.tp_y, udfp_data->aod_event,
		     udfp_data->key_event);
	return NO_ERR;
}
...
static void esd_abnormal_handle(struct thp_device *tdev)
{
	...
	ret = poll_for_event(tdev->sdev, &fw_ready_search,
			     sizeof(fw_ready_search), read_data,
			     CHECK_WAIT_TIME);
	mutex_unlock(&poll_event_mutex);
	if (ret) {
...
static int touch_driver_resume(struct thp_device *tdev)
{
	...
	ret = alix_write(tdev->thp_core->sdev, ALIX_INTERRUPT_ADDRESS, &cmd,
			 sizeof(cmd));
	if (ret != 0)
		thp_log_err("%s: error clearing the interrupt\n", __func__);
	...
	if (is_pt_test_mode(tdev)) {
		...
		if (tdev->thp_core->gesture_from_sensorhub) {
			...
			ret = send_thp_driver_status_cmd(
				POWER_ON, tdev->thp_core->multi_panel_index,
...
static int touch_driver_suspend_ap(struct thp_device *tdev)
{
	...
	uint8_t cmd_single_tap[11] = {0xC0, 0x06, 0x01, 0x00, 0x00, 0x00,
	...
	if ((tdev->thp_core->easy_wakeup_info.aod_mode == TS_TOUCH_AOD_OPEN) &&
		...
		ret = fw_write(sdev, cmd_single_tap, sizeof(cmd_single_tap));
		if (ret)
...
#ifdef CONFIG_HONOR_SHB_THP
			ret = send_thp_driver_status_cmd(
				POWER_OFF, tdev->thp_core->multi_panel_index,
				ST_CMD_TYPE_MULTI_TP_UD_STATUS);
#endif
				 ...
				 struct thp_aod_window window)
{
	...
	thp_log_info("%s status %u, x0,y0,x1,y1: (%u, %u), (%u, %u)\n",
		     __func__, status, window.x0, window.y0, window.x1,
		     window.y1);
...
static int thp_communication_config(struct spi_device *sdev)
{
	...
	if (cd->support_vendor_ic_type == ST_BA80Y) {
		...
		err = alix_write(sdev, ADDR_GPIO_REG2_FOR_BA80Y, &cmd,
				 sizeof(cmd));
	} else {
		cmd = VAL_GPIO_REG2;
...
static int touch_driver_check_chip_id(struct spi_device *sdev)
{
	...
	ret = alix_read(sdev, ALIX_HW_ID_ADDRESS, chip_id, ST_CHIP_ID_LEN);
	if (ret != 0) {
	...
	thp_log_info("%s:chip_id:0x%02X%02X%d,fw version:0x%02X%02X\n",
		     ...
		     chip_id[4]);
	if (cd->support_vendor_ic_type == STU56A) {
		if ((chip_id[0] != ALIX_HW_ID_0) ||
		    (chip_id[1] != ALIX_HW_ID_1_STU56A)) {
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	stmicro_node =
		of_get_child_by_name(cd->thp_node, THP_STMICRO_DEV_NODE_NAME);
	if (!stmicro_node) {
				   ...
				   uint32_t count)
{
	...
	if (tdev->thp_core->support_vendor_ic_type == STU56A)
		...
		err = alix_read(tdev->thp_core->sdev, PROJECT_ID_ADDRESS, data,
				count);
	if (err != 0) {
