...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	struct device_node *ilitek_node =
		of_get_child_by_name(cd->thp_node, THP_ILITEK_DEV_NODE_NAME);
...
int touch_driver_ice_mode_write(struct spi_device *sdev, u32 addr, u32 data,
				int len)
{
	int ret;
	int i;
				      ...
				      bool mcu_enable)
{
	...
	if (ice_enable) {
		...
		ret = touch_driver_spi_write(tdev->thp_core->sdev, cmd,
					     sizeof(cmd));
		if (ret < 0)
