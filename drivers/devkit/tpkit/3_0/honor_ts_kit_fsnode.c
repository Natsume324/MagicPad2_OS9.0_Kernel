				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if (g_ts_kit_platform_data.get_info_flag) {
		...
		error = snprintf(buf, CHIP_INFO_LENGTH * TS_CHIP_INFO_SIZE,
				 "%s-%s-%s\n", info->ic_vendor,
				     ...
				     const char *buf, size_t count)
{
	...
	error = sscanf(buf, "%4d %4d %4d %4d %4d", &window_enable, &x0, &y0,
		       &x1, &y1);
	if (error <= 0) {
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	error = snprintf(buf, MAX_STR_LEN, "%d %d %d %d %d\n",
			 ...
			 info->top_left_y0, info->bottom_right_x1,
				 ...
				 size_t count)
{
	...
	error = sscanf(buf, "%d 0x%4x %79s %d %d", &flag, &addr, value, &num,
		       &bit);
	if (error <= 0) {
		TS_LOG_ERR(
	...
	for (i = 0;
	     (TS_OFFSET_4BIT * i + TS_OFFSET_3BIT) < TS_MAX_REG_VALUE_NUM;
	     i++) {
					    ...
					    const char *buf, size_t size)
{
	...
	if (value == 1) {
		if (pdata->chip_data->ops->chip_wrong_touch != NULL) {
					    ...
					    char *buf)
{
	...
	for (i = 0; i < MAX_LOTUS_NUM; i++) {
		snprintf(temp, (sizeof(u32) * EASYWAKE_POSITION_SIZE + 1),
			 "%08x", pchip->easy_wakeup_info.easywake_position[i]);
					  ...
					  char *buf)
{
	...
	if (info->debug_type == READ_FORCE_DATA) {
		...
		for (index = 1; index < size; index++) {
			len = snprintf(buf + count, FORCEKEY_DATA_LEN, "%6d",
				       info->buff[index]);
			count = count + len;
		...
		for (index = 0;
		     (row_size * index + ROI_DATA_STEP) < (info->used_size);
		     index++) {
					      ...
					      char *buf)
{
	...
	return snprintf(
		...
		feature_all, local_param->lcd_width, local_param->lcd_height,
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	for (; i < ROI_DATA_READ_LENGTH; i += ROI_DATA_STEP, j++) {
		...
		cnt = snprintf(buf, PAGE_SIZE - count - 1, "%4d\t",
			       roi_data_16[j]);
		buf += cnt;
		count += cnt;
		...
		if (info->data[1] != 0) {
			for (i = 0; i < info->length; ++i) {
				for (index = 0; index < INFO_DATA_TEMP_LENGTH;
					...
					snprintf(
						str_tmp, sizeof(str_tmp), "%d,",
						info->data
							[index +
							 i * INFO_DATA_TEMP_LENGTH]);
					strncat(str_oem, str_tmp,
						strlen(str_tmp));
				}
			}
		...
		if (ts_is_factory()) {
			...
			if (data == NEED_SCREEN_OFF) {
				factory_extra_cmd->always_poweron_in_screenoff =
					data;
				factory_extra_cmd
					->factory_mode_current_backlight =
					honorfd->bl_level;
				TS_LOG_INFO(
...
static DEVICE_ATTR(touch_chip_info, 0640, ts_chip_info_show,
		   ts_chip_info_store);
...
static DEVICE_ATTR(touch_glove, 0600, ts_glove_mode_show, ts_glove_mode_store);
...
static DEVICE_ATTR(loglevel, 0600, ts_loglevel_show, ts_loglevel_store);
...
static DEVICE_ATTR(easy_wakeup_gesture, 0600, ts_easy_wakeup_gesture_show,
		   ts_easy_wakeup_gesture_store);
static DEVICE_ATTR(aod_touch_switch_ctrl, 0600, ts_aod_switch_ctrl_show,
		   ts_aod_switch_ctrl_store);
static DEVICE_ATTR(wakeup_gesture_enable, 0600, ts_wakeup_gesture_enable_show,
...
static DEVICE_ATTR(touch_rawdata_debug, 0660, ts_rawdata_debug_test_show,
		   ts_rawdata_debug_test_store);
static DEVICE_ATTR(anti_false_touch_param, 0660, ts_anti_false_touch_param_show,
		   ts_anti_false_touch_param_store);
static DEVICE_ATTR(touch_wideth, 0660, ts_touch_wideth_show,
		   ts_touch_wideth_store);
...
static DEVICE_ATTR(touch_switch, 0660, ts_touch_switch_show,
		   ts_touch_switch_store);
...
static DEVICE_ATTR(factory_extra_cmd, 0660, NULL, ts_factory_extra_cmd_store);
