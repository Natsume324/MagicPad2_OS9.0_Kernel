...
TRACE_EVENT(
	touch, TP_PROTO(int task, int phase, const char *string),
	TP_ARGS(task, phase, string),
	TP_STRUCT__entry(__field(int, task) __field(int, phase)
				 __string(str, string)),

	TP_fast_assign(__entry->task = task; __entry->phase = phase;
		       __assign_str(str, string);),

	TP_printk("touch_trace: %s %s %s",
		  __print_symbolic(
			  __entry->task, {TOUCH_TRACE_IRQ_TOP, "irq top"},
		  ...
		  __print_symbolic(__entry->phase, {TOUCH_TRACE_FUNC_IN, "in"},
