...
#define BOOT_UPDATE_FIRMWARE_FLAG_FILENAME \
	"/system/etc/tp_test_parameters/boot_update_firmware.flag"
...
static int check_firmware_version(const struct firmware *fw)
{
	...
	if (fw_update_boot_sd_flag == FW_UPDATE_BOOT) {
		if ((g_himax_nc_ts_data->vendor_fw_ver <
		     ((fw->data[nc_fw_ver_maj_flash_addr] << 8) |
		      fw->data[nc_fw_ver_min_flash_addr])) ||
		    (g_himax_nc_ts_data->vendor_config_ver <
		     ((fw->data[nc_cfg_ver_maj_flash_addr] << 8) |
...
static int check_need_firmware_upgrade(const struct firmware *fw)
{
	...
	if (check_firmware_version(fw)) {
		...
		if (retval == 0) {
			TS_LOG_INFO(
				"now is factory test mode, not update firmware\n");
			return 0;
		}
		return FW_NEED_TO_UPDATE;
	}
	...
	if ((check_need_firmware_upgrade(fw)) ||
		...
		if (retval == 0) {
			...
			g_himax_nc_ts_data->vendor_config_ver =
				...
				fw->data[nc_cfg_ver_min_flash_addr];
			TS_LOG_INFO(
				"firmware_update: upgraded IMAGE FW_VER = %x.\n",
				(fw->data[nc_fw_ver_maj_flash_addr]
				 << LEFT_MOV_8BIT) |
					fw->data[nc_fw_ver_min_flash_addr]);
			TS_LOG_INFO(
				"firmware_update: upgraded IMAGE CFG_VER = %x.\n",
				(fw->data[nc_cfg_ver_maj_flash_addr]
				 << LEFT_MOV_8BIT) |
					fw->data[nc_cfg_ver_min_flash_addr]);
		}
		__pm_relax(&g_himax_nc_ts_data->tskit_himax_data
				    ->ts_platform_data->ts_wake_lock);
				    ...
				    int *file_size)
{
	...
	if (file_len == 0) {
		TS_LOG_ERR(
			"file_open_firmware: file size error,file_len = %d\n",
			file_len);
		retval = -EINVAL;
		goto exit;
	}
	...
	if (filp->f_op->read(filp, databuf, file_len, &filp->f_pos) !=
	    file_len) {
...
static int himax_check_update_firmware_flag(void)
{
	...
	if (retval != 0) {
		TS_LOG_ERR(
			"check_update_firmware_flag, file_open error, code = %d\n",
			retval);
		goto exit;
	}
	...
	if (update_flag == 1) {
		retval = 0;
		TS_LOG_INFO(
			"check_update_firmware_flag: check success, flag=%d\n",
			update_flag);
		goto exit;
	} else {
		retval = -1;
		TS_LOG_INFO(
			"check_update_firmware_flag: check failed, flag=%d\n",
			update_flag);
		goto exit;
	}
exit:
...
static void himax_get_fw_name(char *file_name)
{
	...
	strncat(file_name, himax_nc_project_id,
		strlen(himax_nc_project_id) + 1);
	strncat(file_name, &joint_chr, 1);
	strncat(file_name,
		g_himax_nc_ts_data->tskit_himax_data->ts_platform_data
			->chip_data->module_name,
		strlen(g_himax_nc_ts_data->tskit_himax_data->ts_platform_data
			       ->chip_data->module_name) +
			1);
...
int himax_nc_fw_update_boot(char *file_name)
{
	...
	if (err < 0) {
		TS_LOG_ERR(
			"fw_update_boot: %d: Fail request firmware %s, retval = %d\n",
...
int himax_nc_fw_update_sd(void)
{
	...
	retval = request_firmware(&fw_entry, fir_name,
				  &g_himax_nc_ts_data->tskit_himax_data
					   ->ts_platform_data->client->dev);
	if (retval < 0) {
		TS_LOG_ERR(
			"fw_update_sd: %d: Fail request firmware %s, retval = %d\n",
			__LINE__, firmware_name, retval);
		retval =
			0; // honor request :request_firmware fail, return NO ERROR
		return retval;
	}
	fw_update_boot_sd_flag = FW_UPDATE_SD;
	firmware_update(fw_entry);
