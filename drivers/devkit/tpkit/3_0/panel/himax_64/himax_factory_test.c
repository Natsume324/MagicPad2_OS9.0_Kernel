...
#define HIMAX_PROC_FACTORY_TEST_FILE \
	"ts/himax_threshold.csv" // "touchscreen/tp_capacitance_data"
...
int himax_nc_chip_self_test(void)
{
	...
	himax_nc_flash_write_burst_length(tmp_addr, tmp_data,
					  2 * FOUR_BYTE_CMD);
	// 0x10007294 -> 0x0000190 SET IIR_MAX FRAMES
	himax_rw_reg_reformat_com(ADDR_NFRAME_SEL, DATA_SET_IIR_FRM, tmp_addr,
				  tmp_data);
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
	// Disable IDLE Mode
	himax_nc_idle_mode(ON);
	himax_rw_reg_reformat_com(ADDR_SWITCH_FLASH_RLD, DATA_DISABLE_FLASH_RLD,
				  tmp_addr, tmp_data);
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
	...
	for (i = 0; i < 10; i++) {
		if (ic_nc_type == HX_83112A_SERIES_PWON) {
			// 0x10007F18 -> 0x00006AA6
			himax_rw_reg_reformat(ADDR_HAND_SHAKING_HX83112,
					      tmp_addr);
		} else if (ic_nc_type == HX_83102B_SERIES_PWON) {
			// 0x100007F8 -> 0x00006AA6
			himax_rw_reg_reformat(ADDR_HAND_SHAKING_HX83102,
					      tmp_addr);
		} else if (ic_nc_type == HX_83102E_SERIES_PWON) {
		...
		TS_LOG_INFO(
			"chip_self_test: tmp_data[0]=0x%02X,tmp_data[1]=0x%02X\n",
			tmp_data[0], tmp_data[1]);
		TS_LOG_INFO(
			"chip_self_test: tmp_data[2]=0x%02X,tmp_data[3]=0x%02X,cnt=%d\n",
			tmp_data[2], tmp_data[3], i);
		msleep(HX_SLEEP_10MS);
		if (tmp_data[1] == 0xA6 && tmp_data[0] == 0x6A) {
			TS_LOG_INFO(
				"chip_self_test: Data ready goto moving data\n");
			break;
		}
	}
	...
	TS_LOG_INFO(
		"chip_self_test: check test result, test_result_id=%x, test_result=%x\n",
		test_result_id, self_test_info[0]);
...
static int himax_alloc_rawmem(void)
{
	...
	if ((ic_nc_type == HX_83112A_SERIES_PWON) ||
		...
		mutual_open =
			kzalloc(mutual_num * sizeof(uint32_t), GFP_KERNEL);
		if (mutual_open == NULL) {
...
static void himax_print_rawdata(int mode)
{
	...
	case RTX_DELTA_CMD:
		...
		for (index1 = 0; index1 < self_num; index1++) {
			...
			strncat((char *)info_test->rx_delta_buf, buf,
				sizeof(buf));
	...
	case RTX_DELTA_CMD1:
		...
		self_num = (x_channel - DATA_1) * y_channel;
		for (index1 = 0; index1 < self_num; index1++)
			info_test->rx_delta_buf[index1] = rx_delta[index1];

		for (index1 = 0; index1 < mutual_num; index1++)
			info_test->tx_delta_buf[index1] = tx_delta[index1];
		break;
	case OPEN_CMD:
		TS_LOG_INFO("print_rawdata: %s\n", "OPEN_CMD");
		mutual_num = x_channel * y_channel;
		TS_LOG_INFO(
			"print_rawdata: x_ch = %d, y_ch = %d, mu_num = %d\n",
			x_channel, y_channel, mutual_num);
		for (index1 = 0; index1 < mutual_num; index1++)
			info_test->buff[current_index++] = mutual_open[index1];
		break;
	case SHORT_CMD:
		TS_LOG_INFO("print_rawdata: %s\n", "SHORT_CMD");
		mutual_num = x_channel * y_channel;
...
static int himax_interface_on_test(int step)
{
	...
	do {
		...
		if (i2c_himax_nc_write(ADDR_EN_BURST_MODE, tmp_data,
				       ...
				       DEFAULT_RETRY_CNT) < 0) {
			TS_LOG_ERR(
				"[SW_ISSUE] interface_on_test: i2c access fail\n");
			goto err_i2c;
		}
		// ===========================================
	...
	if (cnt > 0)
		TS_LOG_DEBUG(
			"interface_on_test: Polling burst mode: %d times\n",
			cnt);
...
err_i2c:
	TS_LOG_ERR(
		"[SW_ISSUE] interface_on_test: Exit because there is I2C fail\n");
	himax_nc_int_enable(
		g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->irq_id,
		IRQ_ENABLE);
	himax_free_rawmem();
...
int himax_raw_data_test(int step) // for Rawdara
{
	...
	if (ic_nc_type == HX_83102E_SERIES_PWON) {
		...
		if (!info_data_hx83102e) {
			TS_LOG_ERR(
				"raw_data_test: fail to alloc info data mem\n");
			return HX_ERR;
		}
	}
	// check if devided by zero
	...
	for (i = 0; i < tx; i++) {
		for (j = 0; j < rx; j++) {
			if (ic_nc_type == HX_83102B_SERIES_PWON)
				...
				new_data = (short)((info_data_hx83102e[index +
								       DATA_1]
						    << DATA_8) |
	...
	for (i = 0; i < rx * tx; i++) {
		if (g_himax_nc_ts_data->p2p_test_sel) {
			cap_rawdata_limit[UP_LIMIT] =
				p2p_cap_rawdata_limit_up[i];
			cap_rawdata_limit[DOWN_LIMIT] =
				p2p_cap_rawdata_limit_dw[i];
		}
		if (mutual_bank[i] < cap_rawdata_limit[DOWN_LIMIT] ||
		    mutual_bank[i] > cap_rawdata_limit[UP_LIMIT]) {
			TS_LOG_INFO(
				"[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, CAP_RAWDATA: %6d\n",
				(i % rx), (i / rx), mutual_bank[i]);
			TS_LOG_INFO(
				"[PANEL_ISSUE] UP_LIMIT:%6d, DOWN_LIMIT: %6d\n",
				cap_rawdata_limit[UP_LIMIT],
				cap_rawdata_limit[DOWN_LIMIT]);
			result = HX_ERR;
		}
...
static int himax_self_delta_test(int step)
{
	...
	for (index1 = 0; index1 < tx_delta_num; index1++) {
		...
		if (g_himax_nc_ts_data->p2p_test_sel)
			trx_delta_limit[UP_LIMIT] =
				p2p_tx_delta_limit_up[index1];
		if (tx_delta[index1] > trx_delta_limit[UP_LIMIT]) {
			TS_LOG_INFO(
				"[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, TX_DELTA: %6d\n",
	...
	for (index1 = 1; index2 < rx_delta_num; index1++) {
		if (index1 % (rx) == 0)
			continue;
		rx_delta[index2] =
			hx_abs(mutual_bank[index1] - mutual_bank[index1 - 1]);
		if (g_himax_nc_ts_data->p2p_test_sel)
			trx_delta_limit[UP_LIMIT] =
				p2p_rx_delta_limit_up[index2];
		if (rx_delta[index2] > trx_delta_limit[UP_LIMIT]) {
			TS_LOG_INFO(
				"[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, RX_DELTA: %6d\n",
				(index1 % (rx - 1)), (index1 / (rx - 1)),
				rx_delta[index2]);
			TS_LOG_INFO("[PANEL_ISSUE] UP_LIMIT:%6d\n",
				    trx_delta_limit[UP_LIMIT]);
			result = HX_ERR;
...
static int himax_iir_test(int step) // for Noise Delta
{
	...
	for (i = 0; i < rx * tx; i++) {
		...
		if (mutual_iir[i] > noise_limit[UP_LIMIT]) {
			TS_LOG_INFO(
				"[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, NOISE : %6d\n",
...
int himax_switch_mode_inspection(int mode)
{
	...
	// Stop Handshaking
	himax_rw_reg_reformat_com(ADDR_STP_HNDSHKG, DATA_INIT, tmp_addr,
				  tmp_data);
...
uint32_t himax_get_rawdata_func(uint32_t *raw, uint32_t datalen)
{
	...
	while (retry < 10) {
		...
		for (i = 2; i < datalen * 2 + 4; i = i + 2)
			checksum_cal += tmp_rawdata[i + 1] * MAX_READ_LENGTH +
					tmp_rawdata[i];
...
uint32_t himax_check_mode(uint8_t checktype)
{
	...
	himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
	TS_LOG_INFO(
		"check_mode: himax_wait_sorting_mode, tmp_data[0]=%x,tmp_data[1]=%x\n",
		tmp_data[0], tmp_data[1]);
...
uint32_t himax_wait_sorting_mode(uint8_t checktype)
{
	...
	do {
		...
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		TS_LOG_INFO(
			"wait_sorting_mode: himax_wait_sorting_mode, tmp_data[0]=%x,tmp_data[1]=%x\n",
			tmp_data[0], tmp_data[1]);
		if (wait_pwd[0] == tmp_data[0] && wait_pwd[1] == tmp_data[1]) {
			return NO_ERR;
		}
		...
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		TS_LOG_INFO(
			"wait_sorting_mode: 0x900000A8, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n",
			tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

		himax_rw_reg_reformat(ADDR_HW_STST_CHK, tmp_addr);
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		TS_LOG_INFO(
			"wait_sorting_mode: 0x900000E4, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n",
			tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

		himax_rw_reg_reformat(ADDR_READ_REG_FW_STATUS, tmp_addr);
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		TS_LOG_INFO(
			"wait_sorting_mode: 0x900000F8, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n",
			tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
...
uint32_t mp_test_func(uint8_t checktype, uint32_t datalen)
{
	...
	case HIMAX_INSPECTION_OPEN:
		...
		for (i = 0; i < (hx_nc_tx_num * hx_nc_rx_num); i++) {
			if (g_himax_nc_ts_data->p2p_test_sel) {
				open_short_limit[UP_LIMIT] =
					p2p_open_limit_up[i];
				open_short_limit[DOWN_LIMIT] =
					p2p_open_limit_dw[i];
			}
			if (raw[i] > open_short_limit[UP_LIMIT] ||
			    raw[i] < open_short_limit[DOWN_LIMIT]) {
				TS_LOG_ERR("%s: open test FAIL\n", __func__);
				TS_LOG_INFO(
					"[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, CAP_RAWDATA: %6d\n",
					(i % hx_nc_rx_num), (i / hx_nc_rx_num),
					raw[i]);
				TS_LOG_INFO(
					"[PANEL_ISSUE] UP_LIMIT:%6d, DOWN_LIMIT: %6d\n",
					open_short_limit[UP_LIMIT],
	...
	case HIMAX_INSPECTION_MICRO_OPEN:
		...
		for (i = 0; i < (hx_nc_tx_num * hx_nc_rx_num); i++) {
			if (g_himax_nc_ts_data->p2p_test_sel) {
				open_short_limit[UP_LIMIT + 2] =
					p2p_micro_open_limit_up[i];
				open_short_limit[DOWN_LIMIT + 2] =
					p2p_micro_open_limit_dw[i];
			}
	...
	case HIMAX_INSPECTION_SHORT:
		...
		for (i = 0; i < (hx_nc_tx_num * hx_nc_rx_num); i++) {
			if (g_himax_nc_ts_data->p2p_test_sel) {
				open_short_limit[UP_LIMIT + 4] =
					p2p_short_limit_up[i];
				open_short_limit[DOWN_LIMIT + 4] =
					p2p_short_limit_dw[i];
			}
			if (raw[i] > open_short_limit[UP_LIMIT + 4] ||
			    raw[i] < open_short_limit[DOWN_LIMIT + 4]) {
				TS_LOG_INFO(
					"[PANEL_ISSUE] POS_RX:%3d, POS_TX:%3d, CAP_RAWDATA: %6d\n",
					(i % hx_nc_rx_num), (i / hx_nc_rx_num),
					raw[i]);
...
int himax_open_test(int step)
{
	...
	retval += mp_test_func(HIMAX_INSPECTION_OPEN,
			       (hx_nc_tx_num * hx_nc_rx_num) + hx_nc_tx_num +
				       hx_nc_rx_num);
...
int himax_short_test(int step)
{
	...
	retval += mp_test_func(HIMAX_INSPECTION_SHORT,
			       (hx_nc_tx_num * hx_nc_rx_num) + hx_nc_tx_num +
				       hx_nc_rx_num);
					    ...
					    struct get_csv_data *data)
{
	...
	snprintf(file_path, sizeof(file_path), "/odm/etc/firmware/ts/%s",
		 file_name);
	TS_LOG_INFO(
		"threshold file name:%s, rows_size=%d, columns_size=%d, target_name = %s\n",
		file_path, rows, columns, target_name);
	result = ts_kit_parse_csvfile(file_path, target_name, data->csv_data,
				      rows, columns);
	if (result == HX_OK) {
		ret = HX_OK;
...
int himax_parse_threshold_csvfile(void)
{
	...
	rawdata_limit = kzalloc(
		8 * sizeof(int32_t) + sizeof(struct get_csv_data), GFP_KERNEL);
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				2, 1, CSV_CAP_RADATA_LIMIT, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_threshold_csvfile:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	} else {
		memcpy(cap_rawdata_limit, rawdata_limit->csv_data,
		       2 * sizeof(u32));
	}
	TS_LOG_INFO(
		"parse_threshold_csvfile:cap_rawdata_limit[%d] = %d, cap_rawdata_limit[%d] = %d \n",
		DOWN_LIMIT, cap_rawdata_limit[0], UP_LIMIT,
		cap_rawdata_limit[1]);

	if (HX_ERROR == himax_get_threshold_from_csvfile(
				2, 1, CSV_TRX_DELTA_LIMIT, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_threshold_csvfile:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	} else {
		memcpy(trx_delta_limit, rawdata_limit->csv_data,
		       2 * sizeof(u32));
	}
	TS_LOG_INFO(
		"parse_threshold_csvfile: trx_delta_limit[%d] = %d, trx_delta_limit[%d] = %d\n",
		DOWN_LIMIT, trx_delta_limit[0], UP_LIMIT, trx_delta_limit[1]);

	if (HX_ERROR == himax_get_threshold_from_csvfile(2, 1, CSV_NOISE_LIMIT,
							 rawdata_limit)) {
		TS_LOG_ERR(
			"parse_threshold_csvfile: get threshold from csvfile failed\n");
		retval = -1;
	...
	TS_LOG_INFO(
		"parse_threshold_csvfile: noise_limit[%d] = %d, noise_limit[%d] = %d \n",
		DOWN_LIMIT, noise_limit[0], UP_LIMIT, noise_limit[1]);

	rawdata_limit->size = CSV_OPEN_SHORT_LIMIT_MAX_LEN;
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				6, 1, CSV_OPEN_SHORT_LIMIT, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_threshold_csvfile: get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	} else {
		memcpy(open_short_limit, rawdata_limit->csv_data,
		       6 * sizeof(u32));
	}

	TS_LOG_DEBUG(
		"parse_threshold_csvfile: open_short_limit_aa_down[%d] = %d, open_short_limit_aa_up[%d] = %d\n",
		DOWN_LIMIT, open_short_limit[DOWN_LIMIT], UP_LIMIT,
		open_short_limit[UP_LIMIT]);
	TS_LOG_DEBUG(
		"parse_threshold_csvfile: open_short_limit_key_down[%d] = %d, open_short_limit_key_up[%d] = %d\n",
		DOWN_LIMIT + 2, open_short_limit[DOWN_LIMIT + 2], UP_LIMIT + 2,
		open_short_limit[UP_LIMIT + 2]);
	TS_LOG_DEBUG(
		"parse_threshold_csvfile: open_short_limit_avg_down[%d] = %d, open_short_limit_avg_up[%d] = %d\n",
		DOWN_LIMIT + 4, open_short_limit[DOWN_LIMIT + 4], UP_LIMIT + 4,
		open_short_limit[UP_LIMIT + 4]);
...
int himax_parse_threshold_csvfile_p2p(void)
{
	...
	rawdata_limit = kzalloc(rawdata_limit_col * rawdata_limit_row *
						sizeof(int32_t) +
					sizeof(struct get_csv_data),
				GFP_KERNEL);
	if (rawdata_limit == NULL) {
		TS_LOG_ERR(
			"parse_csvfile_p2p: rawdata_limit alloc memory failed\n");
		return HX_ERR;
	}
	rawdata_limit->size = rawdata_limit_col * rawdata_limit_row;
	/*	1	*/
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_CAP_RADATA_LIMIT_UP, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	} else {
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_CAP_RADATA_LIMIT_DW, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, (rawdata_limit_row - 1),
				CSV_TX_DELTA_LIMIT_UP, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				(rawdata_limit_col - 1), rawdata_limit_row,
				CSV_RX_DELTA_LIMIT_UP, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_OPEN_LIMIT_UP, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_OPEN_LIMIT_DW, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_MICRO_OPEN_LIMIT_UP, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_MICRO_OPEN_LIMIT_DW, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_SHORT_LIMIT_UP, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
	...
	if (HX_ERROR == himax_get_threshold_from_csvfile(
				rawdata_limit_col, rawdata_limit_row,
				CSV_SHORT_LIMIT_DW, rawdata_limit)) {
		TS_LOG_ERR(
			"parse_csvfile_p2p:get threshold from csvfile failed\n");
		retval = -1;
		goto exit;
...
int himax_parse_threshold_dts(void)
{
	...
	TS_LOG_INFO(
		"parse_threshold_dts:cap_rawdata_limit[%d] = %d, cap_rawdata_limit[%d] = %d \n",
		DOWN_LIMIT, cap_rawdata_limit[0], UP_LIMIT,
		cap_rawdata_limit[1]);
	...
	TS_LOG_INFO(
		"parse_threshold_dts:trx_delta_limit[%d] = %d, trx_delta_limit[%d] = %d \n",
		DOWN_LIMIT, trx_delta_limit[0], UP_LIMIT, trx_delta_limit[1]);

	retval = of_property_read_u32_array(device, NOISE_LIMIT,
					    &noise_limit[0], 2);
	if (retval < 0) {
		TS_LOG_ERR("Failed to read limits from dt:retval = %d \n",
			   retval);
	...
	TS_LOG_INFO(
		"parse_threshold_dts:noise_limit[%d] = %d, noise_limit[%d] = %d \n",
		DOWN_LIMIT, noise_limit[0], UP_LIMIT, noise_limit[1]);

	retval = of_property_read_u32_array(device, OPEN_SHORT_LIMIT,
					    &open_short_limit[0], 6);
	if (retval < 0) {
		TS_LOG_ERR("Failed to read limits from dt:retval = %d\n",
			   retval);
		return retval;
	...
	TS_LOG_INFO(
		"parse_threshold_dts:open_short_limit_aa_down[%d] = %d, open_short_limit_aa_up[%d] = %d\n",
		DOWN_LIMIT, open_short_limit[DOWN_LIMIT], UP_LIMIT,
		open_short_limit[UP_LIMIT]);
	TS_LOG_INFO(
		"parse_threshold_dts:open_short_limit_key_down[%d] = %d, open_short_limit_key_up[%d] = %d\n",
		DOWN_LIMIT + 2, open_short_limit[DOWN_LIMIT + 2], UP_LIMIT + 2,
		open_short_limit[UP_LIMIT + 2]);
	TS_LOG_INFO(
		"parse_threshold_dts:open_short_limit_avg_down[%d] = %d, open_short_limit_avg_up[%d] = %d\n",
		DOWN_LIMIT + 4, open_short_limit[DOWN_LIMIT + 4], UP_LIMIT + 4,
		open_short_limit[UP_LIMIT + 4]);
	return retval;
}
...
static int himax_p2p_test_init(void)
{
	...
	if (p2p_test_sel) {
		...
		p2p_cap_rawdata_limit_up = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_cap_rawdata_limit_dw = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_tx_delta_limit_up =
			kzalloc(rawdata_limit_col * (rawdata_limit_row - 1) *
					sizeof(u32),
				GFP_KERNEL);
		p2p_rx_delta_limit_up =
			kzalloc((rawdata_limit_col - 1) * rawdata_limit_row *
					sizeof(u32),
				GFP_KERNEL);
		p2p_open_limit_up = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_open_limit_dw = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_micro_open_limit_up = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_micro_open_limit_dw = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_short_limit_up = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_short_limit_dw = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);
		p2p_noise_limit = kzalloc(
			rawdata_limit_col * rawdata_limit_row * sizeof(u32),
			GFP_KERNEL);

		if (!p2p_cap_rawdata_limit_up || !p2p_cap_rawdata_limit_dw ||
		    !p2p_tx_delta_limit_up || !p2p_rx_delta_limit_up ||
		    !p2p_open_limit_up || !p2p_open_limit_dw ||
		    !p2p_micro_open_limit_up || !p2p_micro_open_limit_dw ||
		    !p2p_short_limit_up || !p2p_short_limit_dw ||
		    !p2p_noise_limit) {
			TS_LOG_ERR(
				"p2p_test_init: alloc mem fail! p2p_test_sel = %d \n",
				p2p_test_sel);
			return HX_ERROR;
		}
	} else {
			   ...
			   struct ts_rawdata_info *info_top)
{
	...
	fac_dc_dump_buffer =
		kzalloc(mutual_num * 2 * sizeof(uint16_t), GFP_KERNEL);
	if (!fac_dc_dump_buffer) {
		TS_LOG_ERR("[SW_ISSUE] device, fac_dc_dump_buffer is NULL\n");
		goto err_alloc_p2p_test;
	}
	fac_iir_dump_buffer =
		kzalloc(mutual_num * 2 * sizeof(int16_t), GFP_KERNEL);
	...
	if (ts->test_capacitance_via_csvfile == true) {
		...
		if (retval < 0)
			TS_LOG_ERR(
				"[SW_ISSUE] factory_start: Parse csvfile limit Fail\n");
	} else {
		retval = himax_parse_threshold_dts();
		if (retval < 0)
			TS_LOG_ERR(
				"[SW_ISSUE] factory_start: Parse dts limit Fail\n");
	}

	if (atomic_read(&hmx_nc_mmi_test_status)) {
		TS_LOG_ERR(
			"[SW_ISSUE] factory_start: factory test already has been called\n");
	...
	__pm_stay_awake(&g_himax_nc_ts_data->tskit_himax_data->ts_platform_data
				 ->ts_wake_lock);

	retval = himax_alloc_rawmem();
	if (retval != 0) {
		TS_LOG_ERR(
			"[SW_ISSUE] factory_start factory test alloc_Rawmem failed\n");
		goto err_alloc_rawmem;
	}

	/* step0: himax self test */
	hx_result_status[TEST0] = himax_interface_on_test(
		TEST0); // 0 :i2c test pass  -1: i2c test fail
	/* step1: cap rawdata */
	...
	/* step4: Open/Short */
	if (ic_nc_type == HX_83102B_SERIES_PWON) {
		hx_result_status[TEST4] =
			himax_basec_test(TEST4); // for short/open
	} else if (ic_nc_type == HX_83102E_SERIES_PWON) {
	...
	strncat(buf_test_result,
		...
		strlen(ts->tskit_himax_data->ts_platform_data->chip_data
			       ->chip_name) +
			1);
	strncat(buf_test_result, "-", strlen("-") + 1);
	strncat(buf_test_result,
		ts->tskit_himax_data->ts_platform_data->product_name,
		strlen(ts->tskit_himax_data->ts_platform_data->product_name) +
			1);
	strncat(buf_test_result, ";", strlen(";") + 1);

	strncat(info_test->result, buf_test_result,
		strlen(buf_test_result) + 1);
	info_test->buff[0] = rx;
	info_test->buff[1] = tx;
...
err_alloc_rawmem:
	__pm_relax(&g_himax_nc_ts_data->tskit_himax_data->ts_platform_data
			    ->ts_wake_lock);
