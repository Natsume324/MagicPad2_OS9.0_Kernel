			       ...
			       size_t count)
{
	...
	if (buf[0] == '1') {
		...
		if (error < 0)
			TS_LOG_ERR(
				"reset_set: Set reset pin low state error:%d\n",
				error);
		msleep(RESET_LOW_TIME);
		error = pinctrl_select_state(
			g_himax_nc_ts_data->pctrl,
		...
		if (error < 0)
			TS_LOG_ERR(
				"reset_set: Set reset pin high state error:%d\n",
				error);
		msleep(RESET_HIGH_TIME);
#endif
	}
...
{
	diag_mutual =
		kzalloc(x_channel * y_channel * sizeof(int16_t), GFP_KERNEL);
	if (diag_mutual == NULL)
	...
	if ((g_diag_command >= 1 && g_diag_command <= 3) ||
		...
		for (loop_i = 0; loop_i < mutual_num; loop_i++) {
			count += snprintf(buf + count,
					  HX_MAX_PRBUF_SIZE - count, "%6d",
					  diag_mutual[loop_i]);
			if ((loop_i % width) == (width - 1)) {
				flg = width + loop_i / width;
				count += snprintf(buf + count,
						  HX_MAX_PRBUF_SIZE - count,
						  " %6d\n", diag_self[flg]);
			}
		}
		...
		for (loop_i = 0; loop_i < hx_nc_bt_num; loop_i++)
			count += snprintf(
				buf, HX_MAX_PRBUF_SIZE - count, "%6d",
		...
		for (loop_i = 0; loop_i < 128; loop_i++) {
			if ((loop_i % 16) == 0)
				count += snprintf(buf,
						  HX_MAX_PRBUF_SIZE - count,
						  "LineStart:");
			count += snprintf(buf, HX_MAX_PRBUF_SIZE - count, "%4x",
					  hx_nc_diag_coor[loop_i]);
			if ((loop_i % 16) == 15)
				count += snprintf(
					buf, HX_MAX_PRBUF_SIZE - count, "\n");
		}
		...
		for (loop_i = 0, index = 0; loop_i < hx_nc_tx_num; loop_i++) {
			for (loop_j = 0; loop_j < hx_nc_rx_num; loop_j++) {
				if (ic_nc_type == HX_83102B_SERIES_PWON)
					new_data =
						(short)((info_data_hx102b[index +
									  1]
							 << 8) |
							info_data_hx102b[index]);
				else if (ic_nc_type == HX_83112A_SERIES_PWON)
					new_data =
						(short)((info_data_hx112a[index +
									  1]
							 << 8) |
							info_data_hx112a[index]);
				if (dsram_type == 1)
					diag_mutual[loop_i * hx_nc_rx_num +
						    loop_j] = new_data;
				index += 2;
			}
		}
			       ...
			       size_t len)
{
	...
	if (g_diag_command > 0 && rawdata_type == 0) {
		...
		TS_LOG_INFO(
			"g_diag_command=0x%x,storage_type=%d,rawdata_type=%d\n",
	...
	if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
		...
		if (dsram_flag) {
			...
			dsram_flag = false;
			// 2. Enable ISR
			himax_nc_int_enable(g_himax_nc_ts_data->tskit_himax_data
		...
		if (g_switch_mode == 2) {
			if (rawdata_type == 1) {
				...
				command[0] = 0x00;
				TS_LOG_ERR(
					"diag_dump: Now Sorting Mode does not support this command=%d\n",
					g_diag_command);
			}
			himax_nc_diag_register_set(command[0]);
		}
		...
		if (g_diag_command != 0x00) {
			TS_LOG_ERR(
				"g_diag_command error!diag_command=0x%x so reset\n",
				g_diag_command);
			command[0] = 0x00;
...
static DEVICE_ATTR(diag, (S_IWUSR | S_IRUGO), himax_diag_show,
		   himax_diag_dump); //Debug_diag_done
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	for (loop_i = 0; loop_i < REG_COMMON_LEN; loop_i++) {
		ret += snprintf(buf + ret, HX_MAX_PRBUF_SIZE - ret, "0x%2.2X ",
				data[loop_i]);
		...
		if (buff[0] == 'r') {
			...
			if (flg_cnt < 3) {
				...
				himax_register_write(register_command,
						     byte_length, w_data);
				TS_LOG_INFO(
					"register_store: buff[0] == 'w' && flg_cnt < 3\n");
			} else {
					...
					if (!kstrtoul(buf_tmp, 16, &result)) {
						if (loop_i == 0) {
							register_command[loop_i] =
								(uint8_t)(result);
							TS_LOG_INFO(
								"register_store: register_command = %X\n",
								register_command
									[0]);
						} else {
							w_data[loop_i - 1] =
...
static DEVICE_ATTR(register, (S_IWUSR | S_IRUGO), himax_register_show,
		   himax_register_store); // Debug_register_done
				...
				size_t count)
{
	...
	if (buf[0] == 'v') {
		...
		TS_LOG_INFO(
			"debug_dump: upgrade start,len %d: %02X, %02X, %02X, %02X\n",
		...
		if (result > 0) {
			...
			;
			if (hx_nc_fts_ctpm_fw_upgrade_with_fs(
				    upgrade_fw, result, true) == 0) {
				TS_LOG_ERR(
					"debug_dump: TP upgrade error, line: %d\n",
					__LINE__);
				fw_update_complete = false;
			} else {
				TS_LOG_INFO(
					"debug_dump: TP upgrade OK, line: %d\n",
					__LINE__);
				himax_nc_reload_disable(0);
				fw_update_complete = true;
				g_himax_nc_ts_data->vendor_fw_ver =
					((upgrade_fw[nc_fw_ver_maj_flash_addr]
					  << 8) |
					 upgrade_fw[nc_fw_ver_min_flash_addr]);
				g_himax_nc_ts_data->vendor_config_ver =
					(upgrade_fw[nc_cfg_ver_maj_flash_addr]
					 << 8) |
...
static DEVICE_ATTR(debug, (S_IWUSR | S_IRUGO), himax_debug_show,
		   himax_debug_dump); // Debug_debug_done
#endif
	...
	for (i = 0; i < flash_size; i++) {
		if (i % cloum == 0) {
			if (flash_buffer[i] < COLUMNS_LEN)
				printk("hx_flash_dump: 0x0%X,",
				       flash_buffer[i]);
			else
				printk("hx_flash_dump: 0x%2X,",
				       flash_buffer[i]);
		} else {
			if (flash_buffer[i] < COLUMNS_LEN)
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (local_flash_command == 3 && local_flash_complete) {
		...
		for (loop_i = 0; loop_i < 128; loop_i++) {
			retval += snprintf(buf + retval,
					   HX_MAX_PRBUF_SIZE - retval, "x%2.2x",
					   flash_buffer[loop_i]);
			if ((loop_i % 16) == 15)
				retval += snprintf(buf + retval,
						   HX_MAX_PRBUF_SIZE - retval,
	...
	retval +=
		snprintf(buf + retval, HX_MAX_PRBUF_SIZE - retval, "FlashEnd");
	retval += snprintf(buf + retval, HX_MAX_PRBUF_SIZE - retval, "\n");
	return retval;
}
...
static DEVICE_ATTR(flash_dump, (S_IWUSR | S_IRUGO), himax_flash_show,
		   himax_flash_store); // Debug_flash_dump_done
#endif
				      ...
				      const char *buf, size_t count)
{
	...
	if (ts->debug_log_level & BIT(3)) {
		if (ts->pdata->screen_width > 0 &&
		    ts->pdata->screen_height > 0 &&
		    (ts->pdata->abs_x_max - ts->pdata->abs_x_min) > 0 &&
		    (ts->pdata->abs_y_max - ts->pdata->abs_y_min) > 0) {
			ts->width_factor =
				(ts->pdata->screen_width << SHIFTBITS) /
				(ts->pdata->abs_x_max - ts->pdata->abs_x_min);
			ts->height_factor =
				(ts->pdata->screen_height << SHIFTBITS) /
			...
			TS_LOG_INFO(
				"Enable finger debug with raw position mode!\n");
		}
	} else {
		ts->use_screen_res = 0;
		ts->width_factor = 0;
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	retval += snprintf(buf, HX_MAX_PRBUF_SIZE - retval,
			   ...
			   ts_data->tskit_himax_data->ts_platform_data
				   ->chip_data->chip_name,
...
static DEVICE_ATTR(vendor, (S_IRUGO), touch_vendor_show,
		   NULL); // Debug_vendor_done
...
{
	size_t retval = 0;
	retval += snprintf(buf + retval, HX_MAX_PRBUF_SIZE - retval, "%d ",
			   hx_nc_irq_enable_count);
	retval += snprintf(buf + retval, HX_MAX_PRBUF_SIZE - retval, "\n");
	return retval;
}
				      ...
				      const char *buf, size_t count)
{
	...
	if (value) {
		himax_nc_int_enable(
			ts->tskit_himax_data->ts_platform_data->irq_id, 1);
		ts->irq_enabled = 1;
		hx_nc_irq_enable_count = 1;
	} else {
		himax_nc_int_enable(
			ts->tskit_himax_data->ts_platform_data->irq_id, 0);
		ts->irq_enabled = 0;
...
static DEVICE_ATTR(sns_onoff, 0644, himax_sns_onoff_show,
		   himax_sns_onoff_store);
...
static void himax_storage0_8_diag0_8_func(int rawdata_type)
{
	...
	default:
		command[DATA_0] = RAWDATA_TPYE_NONE;
		TS_LOG_ERR(
			"storage0_8_diag0_8_func: Sram no support this type\n");
		break;
	}
	himax_nc_diag_register_set(command[DATA_0]);
	// 3. Set DSRAM flag
...
static void himax_storage8_diag_func(int rawdata_type)
{
	...
	if (g_switch_mode == SORTING_MODE) {
		if (rawdata_type == USER_RAWDATA_CMD1) {
			...
			command[DATA_0] = RAWDATA_TPYE_NONE;
			TS_LOG_ERR(
				"storage8_diag_func: Sorting Mode no support command=%d\n",
				g_diag_command);
		}
		himax_nc_diag_register_set(command[DATA_0]);
	}
...
static void himax_diag_other_func(void)
{
	...
	if (g_diag_command != RAWDATA_DIAG_CMD_NONE) {
		TS_LOG_ERR(
			"diag_other_func: diag_command=0x%x error so reset\n",
			g_diag_command);
		command[DATA_0] = RAWDATA_TPYE_NONE;
...
static ssize_t himax_diag_seq_write(struct file *filp, const char __user *buff,
				    size_t len, loff_t *data)
{
	...
	if (buf[DATA_1] == USER_SEPARATION_CHAR)
		...
		g_diag_command = (buf[DATA_0] - '0') * BUFF_NUM_TEN +
				 (buf[DATA_1] - '0');
	storage_type = himax_determin_diag_storage(g_diag_command);
	rawdata_type = himax_determin_diag_rawdata(g_diag_command);
	if ((g_diag_command > RAWDATA_DIAG_CMD_NONE) &&
				   ...
				   int dsram_type, uint16_t mutual_num)
{
	...
	for (loop_i = DATA_0, index = DATA_0; loop_i < hx_nc_tx_num; loop_i++) {
...
static int himax_diag_seq_show(struct seq_file *s, void *v)
{
	...
	} else if ((dsram_type > DSRAM_TYPE0) && (dsram_type <= DSRAM_TYPE8)) {
		TS_LOG_INFO("DSRAM\n");
...
static ssize_t himax_attr_show(struct kobject *kobj, struct attribute *attr,
			       char *buf)
{
...
static ssize_t himax_attr_store(struct kobject *kobj, struct attribute *attr,
...
int himax_nc_touch_sysfs_init(void)
{
	...
	diag_seq_dentry = debugfs_create_file("diag", 0644, himax_debugfs_dir,
					      NULL, &hx_diag_seq_fop);
	...
	if (!android_touch_kobj) {
		TS_LOG_ERR(
			"touch_sysfs_init: alloc android_touch_kobj failed\n");
		ret = -ENOMEM;
		return ret;
	}

	ret = kobject_init_and_add(android_touch_kobj, &himax_type, NULL,
				   "himax_debug");
	if (ret) {
		TS_LOG_ERR("touch_sysfs_init: subsystem_register failed\n");
		ret = -ENOMEM;
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: create_file dev_attr_debug_level failed\n");
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: sysfs_create_file dev_attr_vendor failed\n");
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: sysfs_create_file dev_attr_reset failed\n");
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: sysfs_create_file dev_attr_attn failed\n");
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: sysfs_create_file dev_attr_enabled failed\n");
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: create_file dev_attr_register failed\n");
		return ret;
	}
#endif
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: sysfs_create_file dev_attr_diag failed\n");
		return ret;
	}
#endif
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: create_file dev_attr_debug failed\n");
		return ret;
	}
#endif
	...
	if (ret) {
		TS_LOG_ERR(
			"touch_sysfs_init: sysfs_create_file dev_attr_flash_dump failed\n");
		return ret;
	}
#endif
