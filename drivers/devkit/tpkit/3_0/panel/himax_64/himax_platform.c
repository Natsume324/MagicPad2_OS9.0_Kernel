...
int i2c_himax_nc_read(uint8_t command, uint8_t *data, uint16_t length,
		      uint16_t limit_len, uint8_t toretry)
{
	...
	if (length > limit_len) {
		TS_LOG_ERR(
			"i2c_nc_read: i2c_read_block size error %d, over array size%d\n",
			length, limit_len);
		return retval;
	}
	...
	if (retval < 0) {
		mdelay(HX_SLEEP_5MS);
		TS_LOG_ERR(
			"i2c_nc_read: i2c_read_block error %d, retry over %d\n",
			retval, toretry);
	}
	return retval;
}

int i2c_himax_nc_write(uint8_t command, uint8_t *data, uint16_t length,
		       uint16_t limit_len, uint8_t toretry)
{
	int retval = HX_ERR;
	...
	if ((length > limit_len) || (length >= HX_I2C_MAX_SIZE)) {
		TS_LOG_ERR(
			"i2c_nc_write: i2c_write_block size error %d, over array size%d\n",
			length, limit_len);
		return retval;
	}
	bops = g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->bops;
	...
	if (retval < 0) {
		mdelay(HX_SLEEP_5MS);
		TS_LOG_ERR(
			"i2c_nc_write: i2c_write_block error %d, retry over %d\n",
			retval, toretry);
	}
	return retval;
}
...
void himax_burst_enable(uint8_t auto_add_4_byte)
{
	...
	if (i2c_himax_nc_write(ADDR_EN_BURST_MODE, tmp_data, ONE_BYTE_CMD,
			     ...
			     size_t cnta, size_t cntb)
{
	...
	if (i2c_himax_nc_write(ADDR_FLASH_BURNED, data_byte, 2 * FOUR_BYTE_CMD,
...
void himax_nc_flash_write_burst_length(uint8_t *reg_byte, uint8_t *write_data,
				       int length)
{
	int i;
	int j;
	...
	if (i2c_himax_nc_write(ADDR_FLASH_BURNED, data_byte, length + 4,
			       sizeof(data_byte), DEFAULT_RETRY_CNT) < 0) {
			    ...
			    uint8_t *read_data)
{
	...
	if (i2c_himax_nc_write(ADDR_FLASH_BURNED, tmp_data, FOUR_BYTE_CMD,
			       sizeof(tmp_data), DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("nc_register_read: i2c access fail\n");
		return;
	}
	tmp_data[0] = DATA_READ_ACCESS;
	if (i2c_himax_nc_write(ADDR_READ_ACCESS, tmp_data, ONE_BYTE_CMD,
			       sizeof(tmp_data), DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("nc_register_read: i2c access fail\n");
		return;
	}

	if (i2c_himax_nc_read(DUMMY_REGISTER, read_data, read_length,
			      MAX_READ_LENGTH, DEFAULT_RETRY_CNT) < 0) {
		TS_LOG_ERR("nc_register_read: i2c access fail\n");
		return;
	}
	if (read_length > FOUR_BYTE_CMD)
			  ...
			  uint8_t *write_data)
{
	...
	for (i = address; i < address + write_length; i++) {
		...
		himax_nc_flash_write_burst_length(write_addr, write_data,
						  write_length);
	}
}

int himax_nc_write_read_reg(uint8_t *tmp_addr, uint8_t *tmp_data, uint8_t hb,
			    uint8_t lb)
{
	int cnt = 0;

	do {
		himax_flash_write_burst(tmp_addr, tmp_data, FOUR_BYTE_CMD,
					FOUR_BYTE_CMD);
