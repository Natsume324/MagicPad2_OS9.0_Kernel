...
extern int i2c_himax_nc_read(uint8_t command, uint8_t *data, uint16_t length,
			     uint16_t limit_len, uint8_t toRetry);
extern int i2c_himax_nc_write(uint8_t command, uint8_t *data, uint16_t length,
			      uint16_t limit_len, uint8_t toRetry);
extern void himax_nc_rst_gpio_set(int pinnum, uint8_t value);
extern void himax_nc_register_read(uint8_t *read_addr, int read_length,
				   uint8_t *read_data);
extern void himax_flash_write_burst(uint8_t *reg_byte, uint8_t *write_data,
				    size_t cnta, size_t cntb);
...
extern void himax_register_write(uint8_t *write_addr, int write_length,
				 uint8_t *write_data);
...
static uint32_t himax_hw_check_crc(uint8_t *start_addr, int reload_length,
				   size_t count);
static uint32_t touch_driver_check_crc(uint8_t *start_addr, int reload_length);
			      ...
			      uint8_t *addr_buf, uint8_t *data_buf)
{
	...
	TS_LOG_DEBUG(
		"rw_reg_reformat_com: addr_buf[3~1] = %2X  %2X  %2X  %2X \n",
		addr_buf[3], addr_buf[2], addr_buf[1], addr_buf[0]);
	TS_LOG_DEBUG(
		"rw_reg_reformat_com: data_buf[3~1] = %2X  %2X  %2X  %2X \n",
...
static int himax_knuckle(int hx_touch_info_size, uint8_t *buf)
{
	...
	if (buf[hx_touch_info_size + IDX_PKG_NUM] == 0x01) {
		...
		for (i = 0; i < SIZE_2ND_PKG; i++)
			tmp_roi_data[SIZE_1ST_PKG + i] =
				buf[hx_touch_info_size + SIZE_HX_HEADER +
				    SIZE_HX_HEADER + i];
	} else {
...
static int
hmx_wakeup_gesture_enable_switch(struct ts_wakeup_gesture_enable_info *info)
{
	return NO_ERR;
}
static int himax_get_glove_switch(u8 *glove_switch)
{
	...
	if (tmp_data[3] == 0xA5 && tmp_data[2] == 0x5A && tmp_data[1] == 0xA5 &&
	    tmp_data[0] == 0x5A)
...
static int himax_set_glove_switch(u8 glove_switch)
{
	...
	do { // Enable:0x10007F10 = 0xA55AA55A
		if (glove_switch) {
			...
			himax_rw_reg_reformat_com(ADDR_GLOVE_EN,
						  (int)glove_switch, tmp_addr,
						  tmp_data);
			himax_flash_write_burst(tmp_addr, tmp_data,
						sizeof(tmp_addr),
						sizeof(tmp_data));
...
static int himax_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_READ:
		...
		if (ret) {
			TS_LOG_ERR(
				"glove_switch: get glove switch fail,ret=%d\n",
				ret);
			return ret;
		} else {
	...
	case TS_ACTION_WRITE:
		...
		if (ret) {
			TS_LOG_ERR(
				"glove_switch: set glove switch fail, ret=%d\n",
				ret);
			return ret;
		}
		break;
...
static int himax_get_capacitance_test_type(struct ts_test_type_info *info)
{
	...
	chip_data = g_himax_nc_ts_data->tskit_himax_data->ts_platform_data
			    ->chip_data;
	...
	memcpy(info->tp_test_type, chip_data->tp_test_type,
...
int himax_nc_input_config(struct input_dev *input_dev)
{
	...
	TS_LOG_INFO(
		"input_set_abs_params: min_x %d, max_x %d, min_y %d, max_y %d\n",
...
static void calc_data_size(uint8_t finger_num)
{
	...
	ts_data->coord_data_size =
		HX_COORD_BYTE_NUM * finger_num; // 1 coord 4 bytes.
	ts_data->area_data_size = ((finger_num / HX_COORD_BYTE_NUM) +
				   (finger_num % HX_COORD_BYTE_NUM ? 1 : 0)) *
				  HX_COORD_BYTE_NUM; // 1 area 4 finger ?
	...
	ts_data->raw_data_nframes =
		((uint32_t)ts_data->x_channel * ts_data->y_channel +
		 ts_data->x_channel + ts_data->y_channel) /
			...
			0;
	TS_LOG_INFO(
		"%s: coord_data_size: %d, area_data_size:%d, raw_data_frame_size:%d, raw_data_nframes:%d",
...
static void calculate_point_number(void)
{
	...
	if ((hx_nc_max_pt % 4) == 0)
		...
		hx_touch_info_point_cnt +=
			((hx_nc_max_pt / HX_COORD_BYTE_NUM) + 1) *
			HX_COORD_BYTE_NUM;
}
...
static void himax_touch_information(void)
{
	...
	if ((ic_nc_type == HX_83102B_SERIES_PWON) ||
		...
		while (reload_status == 0) {
			...
			if ((data1[1] == 0x3A && data1[0] == 0xA3) ||
				...
				TS_LOG_INFO(
					"reload fail ,delay 10ms retry=%d\n",
					retry);
			}
		}
		TS_LOG_INFO(
			"touch_information: data[0]=0x%2.2X,data[1]=0x%2.2X,data[2]=0x%2.2X,data[3]=0x%2.2X\n",
		...
		if (ic_nc_type == HX_83102E_SERIES_PWON) {
			...
			himax_nc_register_read(cmd, DATA_2 * FOUR_BYTE_CMD,
					       data);
		...
		if (ic_nc_type == HX_83102E_SERIES_PWON) {
			...
			himax_nc_register_read(cmd, FOUR_BYTE_CMD * DATA_3,
					       data);
		} else {
			himax_rw_reg_reformat(ADDR_XY_RVRS, cmd);
			himax_nc_register_read(cmd, FOUR_BYTE_CMD, data);
		...
		if (ic_nc_type == HX_83102E_SERIES_PWON) {
			hx_nc_y_res = data[mod + DATA_2] * SHIFT_NUM +
				      data[mod + DATA_3];
			hx_nc_x_res = data[mod + DATA_4] * SHIFT_NUM +
				      data[mod + DATA_5];
		} else {
	...
	TS_LOG_INFO(
		"touch_information: HX_RX_NUM=%d,HX_TX_NUM=%d,HX_MAX_PT=%d\n",
		hx_nc_rx_num, hx_nc_tx_num, hx_nc_max_pt);
	TS_LOG_INFO(
		"touch_information: HX_XY_REVERSE=%d,HX_Y_RES=%d,HX_X_RES=%d\n",
...
int himax_nc_hw_reset(bool loadconfig, bool int_off)
{
	...
	if (info->charger_info.charger_supported) {
		...
		if (retval < 0)
			TS_LOG_ERR(
				"nc_hw_reset: Failed set charger switch:%d, err:%d\n",
...
static bool himax_ic_package_check(void)
{
	...
	for (i = 0; i < retry; i++) {
		...
		if ((tmp_data[DATA_3] == HX_83102_ID_PART_1) &&
			...
			TS_LOG_INFO(
				"ic_package_check: IC package HX83102 series\n");
			...
			ret_data = IC_PACK_CHECK_SUCC;
			TS_LOG_INFO(
				"ic_package_check: IC package HX83112 series\n");
	...
	himax_rw_reg_reformat_com(ADDR_SET_DDREG_REQ, DATA_INIT, &tmp_addr[0],
				  &tmp_data[0]);
...
void himax_nc_read_tp_info(void)
{
	...
	TS_LOG_INFO("CID_VER: %X \n",
		    ((g_himax_nc_ts_data->vendor_cid_maj_ver << 8) |
...
void touch_driver_83102_read_tp_info(void)
{
	...
	himax_nc_register_read(cmd, FOUR_BYTE_CMD, data);
	g_himax_nc_ts_data->vendor_fw_ver =
		(data[DATA_0 + mod] << LEFT_MOV_8BIT) | data[DATA_1 + mod];
	...
	himax_nc_register_read(cmd, FOUR_BYTE_CMD, data);
	g_himax_nc_ts_data->vendor_config_ver =
		(data[mod + DATA_0] << LEFT_MOV_8BIT) | data[mod + DATA_1];
	TS_LOG_INFO("CFG_VER:%X\n", g_himax_nc_ts_data->vendor_config_ver);
...
static void himax_get_rawdata_from_event(int rawdatalen, int hx_touch_info_size,
					 int mul_num, int sel_num, int index,
					 uint8_t *buff)
{
	int i;
	int temp1;
					      ...
					      int rawdatalen, uint8_t *buf)
{
	...
	if ((buf == NULL) ||
	    (hx_touch_info_size > HX_RECEIVE_BUF_MAX_SIZE - 3)) {
		return HX_ERROR;
	}

	for (i = 0, check_sum_cal = 0;
	     i < (HX_RECEIVE_BUF_MAX_SIZE - hx_touch_info_size); i = i + 2)
	...
	if (buf[hx_touch_info_size] == 0x3A &&
		...
		if (hx_nc_get_diag_command() == HX_ROI_EN_PSD) {
			...
			himax_get_rawdata_from_event(rawdatalen,
						     hx_touch_info_size,
						     mul_num, self_num, index,
						     buf);
#ifdef ROI
		}
#endif
		...
		TS_LOG_INFO(
			"get_rawdata_from_event: header format is wrong!\n");
		retval = HX_ERROR;
		return retval;
	}
...
static int himax_check_report_data_for_esd(int hx_touch_info_size, uint8_t *buf)
{
	...
	if (buf == NULL) {
		TS_LOG_ERR(
			"himax_check_report_data_for_esd buf pointer NULL!\n");
		return HX_ERROR;
	}
	for (i = 0; i < hx_touch_info_size; i++) {
		if (buf[i] ==
		    ESD_EVENT_ALL_ZERO) { // case 2 ESD recovery flow-Disable
			retval =
				ESD_ALL_ZERO_BAK_VALUE; // if hand shanking fail,firmware error
		} else if (buf[i] ==
			   ESD_EVENT_ALL_ED) { /* case 1 ESD recovery flow */
				    ...
				    uint8_t *buf)
{
	...
	case 1:
		TS_LOG_INFO("Finger %d=> W:%d, Z:%d, F:%d, N:%d\n",
			    hx_touching.loop_i + 1, hx_touching.w,
			    hx_touching.w, hx_touching.loop_i + 1,
			    en_noisefilter);
		break;
	case 2:
		TS_LOG_INFO("Finger %d=> X:%d, Y:%d W:%d, Z:%d, F:%d\n",
			    hx_touching.loop_i + 1, hx_touching.x,
			    hx_touching.y, hx_touching.w, hx_touching.w,
			    hx_touching.loop_i + 1);
		break;
	case 3:
		if (status == 0) { // reporting down
			if ((hx_touching.old_finger >> hx_touching.loop_i) ==
			    0) {
				if (g_himax_nc_ts_data->use_screen_res) {
					TS_LOG_INFO(
						"Screen:F:%02d Down, W:%d, N:%d\n",
						hx_touching.loop_i + 1,
						hx_touching.w, en_noisefilter);
				} else {
					TS_LOG_INFO(
						"Raw:F:%02d Down, W:%d, N:%d\n",
						hx_touching.loop_i + 1,
						hx_touching.w, en_noisefilter);
				}
			}
			...
			if ((hx_touching.old_finger >> hx_touching.loop_i) ==
				...
				if (g_himax_nc_ts_data->use_screen_res) {
					TS_LOG_INFO(
						"Screen:F:%02d Up, X:%d, Y:%d, N:%d\n",
						hx_touching.loop_i + 1,
						(g_himax_nc_ts_data->pre_finger_data
			...
			for (hx_touching.loop_i = 0;
				...
				if (((g_himax_nc_ts_data->pre_finger_mask >>
					...
					if (g_himax_nc_ts_data->use_screen_res) {
						TS_LOG_INFO(
							"status:%X, Screen:F:%02d Up, X:%d, Y:%d, N:%d\n",
							0,
							hx_touching.loop_i + 1,
							(g_himax_nc_ts_data->pre_finger_data
								 [hx_touching
						...
						TS_LOG_INFO(
							"status:%X, Raw:F:%02d Up, X:%d, Y:%d, N:%d\n",
							0,
							hx_touching.loop_i + 1,
							g_himax_nc_ts_data->pre_finger_data
								[hx_touching
			       ...
			       uint8_t *buf)
{
	...
	if (hx_point_num != 0) {
		...
		for (hx_touching.loop_i = 0;
		     hx_touching.loop_i < g_himax_nc_ts_data->nfinger_support;
		     hx_touching.loop_i++) {
			base = hx_touching.loop_i *
			       4; // every finger coordinate need 4 bytes.
			m = base + 1;
			m1 = base + 2;
			m2 = base + 3;
			...
			if (hx_touching.x >= 0 &&
				...
				if ((g_himax_nc_ts_data->debug_log_level &
				     BIT(3)) >
				    0) // debug 3: print finger coordinate information
					himax_debug_level_print(
						3, 0, hx_touch_info_size,
						hx_touching,
						buf); // status = report down

				if (g_himax_nc_ts_data->debug_log_level &
				    BIT(1)) {
					TS_LOG_INFO(
						"parse_coords: raw data:\n");
					himax_debug_level_print(
						2, 0, hx_touch_info_size,
						hx_touching, buf);
				}
				info->fingers[hx_touching.loop_i].status =
					TS_FINGER_PRESS;
				info->fingers[hx_touching.loop_i].x =
					hx_touching.x;
				info->fingers[hx_touching.loop_i].y =
					hx_touching.y;
				info->fingers[hx_touching.loop_i].major = 255;
				info->fingers[hx_touching.loop_i].minor = 255;
				info->fingers[hx_touching.loop_i].pressure =
					hx_touching.w;

				if (!g_himax_nc_ts_data->first_pressed)
					g_himax_nc_ts_data->first_pressed =
						1; // first report

				g_himax_nc_ts_data
					->pre_finger_data[hx_touching.loop_i]
							 [0] = hx_touching.x;
				g_himax_nc_ts_data
					->pre_finger_data[hx_touching.loop_i]
							 [1] = hx_touching.y;
				...
				if ((g_himax_nc_ts_data->debug_log_level &
				     BIT(3)) > 0)
					himax_debug_level_print(
						3, 1, hx_touch_info_size,
						hx_touching,
						buf); // status= report up
			}
		}
	} else {
		for (hx_touching.loop_i = 0;
...
static void himax_set_smwp_enable(uint8_t smwp_enable)
{
	...
	do { // Enable:0x10007F10 = 0xA55AA55A
		if (smwp_enable) {
			himax_rw_reg_reformat_com(ADDR_SMWP_EN, DATA_SMWP_EN,
						  tmp_addr, tmp_data);
			himax_flash_write_burst(tmp_addr, tmp_data,
						sizeof(tmp_addr),
						sizeof(tmp_data));
...
static int
easy_wakeup_gesture_report_coordinate(unsigned int reprot_gesture_point_num,
	...
	if (reprot_gesture_point_num != 0) {
		...
		if (reprot_gesture_point_num == 2) {
			...
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[0] = (gest_start_x << 16) |
							gest_start_y;
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[1] = (gest_end_x << 16) |
							gest_end_y;
			return retval;
		} else {
			/* 1.begin */
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[0] = (gest_start_x << 16) |
							gest_start_y;
			TS_LOG_INFO(
				"begin = 0x%08x,  begin_x= %d , begin_y= %d \n",
				g_himax_nc_ts_data->tskit_himax_data
			...
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[1] = (gest_end_x << 16) |
							gest_end_y;
			TS_LOG_INFO(
				"top = 0x%08x,  end_x= %d , end_y= %d \n",
				g_himax_nc_ts_data->tskit_himax_data
					->easy_wakeup_info.easywake_position[1],
				gest_end_x, gest_end_y);
			/* 3.top */
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[2] =
				(gest_most_top_x << 16) | gest_most_top_y;
			TS_LOG_INFO(
				"top = 0x%08x,  top_x= %d , top_y= %d \n",
				g_himax_nc_ts_data->tskit_himax_data
					->easy_wakeup_info.easywake_position[2],
				gest_most_top_x, gest_most_top_y);
			/* 4.leftmost */
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[3] =
				(gest_most_left_x << 16) | gest_most_left_y;
			TS_LOG_INFO(
				"leftmost = 0x%08x,  left_x= %d , left_y= %d \n",
				g_himax_nc_ts_data->tskit_himax_data
					->easy_wakeup_info.easywake_position[3],
				gest_most_left_x, gest_most_left_y);
			/* 5.bottom */
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[4] =
				(gest_most_bottom_x << 16) | gest_most_bottom_y;
			TS_LOG_INFO(
				"bottom = 0x%08x,  bottom_x= %d , bottom_y= %d \n",
				g_himax_nc_ts_data->tskit_himax_data
					->easy_wakeup_info.easywake_position[4],
				gest_most_bottom_x, gest_most_bottom_x);
			/* 6.rightmost */
			g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
				.easywake_position[5] =
				(gest_most_right_x << 16) | gest_most_right_y;
			TS_LOG_INFO(
				"rightmost = 0x%08x,  right_x= %d , right_y= %d \n",
				g_himax_nc_ts_data->tskit_himax_data
					->easy_wakeup_info.easywake_position[5],
				gest_most_right_x, gest_most_right_y);
		}
	}
	return retval;
}
static int
hmx_check_key_gesture_report(struct ts_fingers *info,
				  ...
				  size_t count)
{
	...
	for (i = 0; i < 4; i++) {
		if (check_fc == 0) {
			if ((buf[0] != 0x00) &&
				...
				check_fc = 0;
				TS_LOG_INFO(
					"ID START at %x,value=%x skip the event\n",
					i, buf[i]);
				break;
			}
		} else {
			if (buf[i] != gesture_flag) {
				check_fc = 0;
				TS_LOG_INFO(
					"ID NOT the same %x != %x So STOP parse event\n",
					buf[i], gesture_flag);
				break;
			}
		}
	...
	retval = hmx_check_key_gesture_report(info, gesture_report_info,
					      gesture_flag, buf);
...
static bool himax_read_event_stack(uint8_t *buf, uint8_t length)
{
	...
	if (i2c_himax_nc_write(ADDR_BURST_READ, cmd, ONE_BYTE_CMD, sizeof(cmd),
			       DEFAULT_RETRY_CNT) < 0) {
				 ...
				 struct ts_cmd_node *out_cmd)
{
	...
	if (hx_touch_info_size > HX_RECEIVE_BUF_MAX_SIZE) {
		TS_LOG_ERR(
			"irq_bottom_half: hx_touch_info_size larger than HX_RECEIVE_BUF_MAX_SIZE\n");
	...
	if (atomic_read(&g_himax_nc_ts_data->suspend_mode) &&
	    g_himax_nc_ts_data->tskit_himax_data->ts_platform_data->chip_data
		    ->easy_wakeup_info.sleep_mode) {
		/* increase wake_lock time to avoid system suspend. */
		__pm_wakeup_event(&g_himax_nc_ts_data->tskit_himax_data
		...
		if (retval < 0)
			TS_LOG_ERR(
				"irq_bottom_half: can't read data from chip!\n");
	...
	if (himax_read_event_stack(buf, hx_touch_info_size) < 0) {
...
#ifdef HX_ESD_WORKAROUND
		check_sum_cal = himax_check_report_data_for_esd(
			hx_touch_info_size, buf);
#ifdef HX_TP_SYS_DIAG
		diag_cmd = hx_nc_get_diag_command();
		...
		if (check_sum_cal != 0 && esd_reset_activate == 0 &&
			...
			if (check_sum_cal == ESD_ALL_ZERO_BAK_VALUE) {
				TS_LOG_INFO(
					"[Bottom Half]: ESD event checked - ALL Zero\n");
				g_zero_event_count++;
				...
				TS_LOG_INFO(
					"[Bottom Half]: ESD event checked - ALL 0xED\n");
				esd_hw_rest();
				goto err_no_reset_out;
			}
		} else if (esd_reset_activate) {
			esd_reset_activate =
				0; /* drop 1st interrupts after chip reset */
			TS_LOG_INFO(
				"[Bottom Half]irq_bottom_half: Back from reset, ready to serve\n");
			return retval;
		} else if (hw_nc_reset_activate) {
#else
		if (hw_nc_reset_activate) {
#endif
			hw_nc_reset_activate =
				0; /* drop 1st interrupts after chip reset */
			TS_LOG_INFO(
				"[Bottom Half]irq_bottom_half: HW_RST Back from reset, ready to serve.\n");
			return retval;
		}
		check_sum_cal = himax_checksum_cal(hx_touch_info_size,
						   hx_touching, buf);
		if (check_sum_cal != 0x00) {
			TS_LOG_INFO(
				"[Bottom Half] checksum fail : check_sum_cal: 0x%02X\n",
				check_sum_cal);
			icount++;
			TS_LOG_ERR("irq_bottom_half: error count is %d\n",
				   icount);
	...
	if (diag_cmd >= 1 && diag_cmd <= 6) {
		if (himax_start_get_rawdata_from_event(
			    hx_touch_info_size, rawdatalen, buf) == HX_ERROR)
...
bypass_checksum_failed_packet:
	...
	himax_parse_coords(hx_touch_info_size, hx_real_point_num, info,
			   hx_touching, buf);
				    ...
				    struct himax_i2c_platform_data *pdata)
{
	...
	if (ts->power_support) {
		retval = of_property_read_u32(dt, "himax,power_type_sel",
					      &read_val);
		if (retval) {
		...
		if (read_val == 0) {
			pdata->gpio_3v3_en = of_get_named_gpio(
				dt, "himax,vdd_ana-supply", 0);
			if (!gpio_is_valid(pdata->gpio_3v3_en))
				TS_LOG_INFO("DT:gpio_3v3_en is not valid\n");

			pdata->gpio_1v8_en = of_get_named_gpio(
				dt, "himax,vcc_i2c-supply", 0);
	...
	if (of_property_read_u32_array(dt, "himax,id-addr", hx_id_addr,
				       length) == NO_ERR) {
		TS_LOG_INFO(
			"DT-parse_specific_dts:id-addr = %2X, %2X, %2X, %2X\n",
			hx_id_addr[0], hx_id_addr[1], hx_id_addr[2],
			hx_id_addr[3]);
	} else {
	...
	if (prop) {
		length = prop->length / ((int)sizeof(uint32_t));
		TS_LOG_DEBUG("parse_specific_dts: flash-addr length = %d",
			     length);
	}

	if (of_property_read_u32_array(dt, "himax,flash-addr", hx_flash_addr,
				       length) == NO_ERR) {
		for (i = 0; i < FLASH_ADDR_LEN; i += 7)
			TS_LOG_INFO(
				"DT-parse_specific_dts: flash-addr=%d,%d,%d,%d,%d,%d,%d\n",
				hx_flash_addr[i], hx_flash_addr[i + 1],
				hx_flash_addr[i + 2], hx_flash_addr[i + 3],
				hx_flash_addr[i + 4], hx_flash_addr[i + 5],
				hx_flash_addr[i + 6]);
	} else {
	...
	if (retval) {
		ts->p2p_test_sel = 0;
		TS_LOG_INFO(
			"get device p2p_test_sel not exit,use default value\n");
	} else {
				   ...
				   struct himax_i2c_platform_data *pdata)
{
	...
	if (prop) {
		...
		if (coords_size != HX_COORDS_MAX_SIZE)
			TS_LOG_DEBUG(
				"parse_project_dts: Invalid panel coords size %d",
				coords_size);
	}
	...
	if (of_property_read_u32_array(dt, "himax,panel-coords", coords,
		...
		TS_LOG_INFO(
			"DT-parse_project_dts:panel-coords = %d, %d, %d, %d\n",
	...
	if (prop) {
		...
		if (coords_size != HX_COORDS_MAX_SIZE)
			TS_LOG_DEBUG(
				"parse_project_dts:Invalid display coords size %d",
				coords_size);
	}

	retval = of_property_read_u32_array(dt, "himax,display-coords", coords,
					    coords_size);
	if (retval) {
		TS_LOG_DEBUG(
			"parse_project_dts: Fail to read display-coords %d\n",
			retval);
		return retval;
	}
	...
	if (retval) {
		...
		TS_LOG_INFO(
			"Not define chip rawdata limit time in Dts, use default\n");
	}
	...
	if (retval) {
		TS_LOG_INFO(
			"get device trx_delta_test_support null, use default\n");
	...
	retval = of_property_read_u32(dt, TEST_CAPACITANCE_VIA_CSVFILE,
				      &read_val);
	if (retval) {
			   ...
			   struct ts_kit_device_data *chip_data)
{
	...
	retval = of_property_read_u32(
		device, "hx_ic_rawdata_proc_printf",
	...
	if (retval) {
		chip_data->ts_platform_data->feature_info.glove_info
			.glove_supported = NOT_SUPPORT;
	...
	retval = of_property_read_u32(device, "himax,gesture_supported",
				      &read_val);
	if (retval) {
		read_val = NOT_SUPPORT;
		TS_LOG_INFO("NOT support to parse the gesture dts!\n");
	}
	chip_data->ts_platform_data->feature_info.wakeup_gesture_enable_info
		.switch_value = (uint8_t)read_val;
	TS_LOG_INFO("parse_dts: gesture_supported = %d\n",
		    chip_data->ts_platform_data->feature_info
			    .wakeup_gesture_enable_info.switch_value);

	retval = of_property_read_u32(device, "himax,gesture_need_lcd_rst",
				      &read_val);
	if (retval) {
		read_val = NOT_SUPPORT;
	...
	if (of_property_read_u32_array(device, "himax,panel-coords", coords,
				       coords_size) == NO_ERR) {
		ts->pdata->abs_x_min = coords[0],
		ts->pdata->abs_x_max = coords[1];
		ts->pdata->abs_y_min = coords[2],
		ts->pdata->abs_y_max = coords[3];
	...
	if (prop) {
		...
		if (coords_size != HX_COORDS_MAX_SIZE)
			TS_LOG_DEBUG(
				"parse_dts: Invalid display coords size %d",
				coords_size);
	}
	...
	/* get tp color flag */
	retval =
		of_property_read_u32(device, "support_get_tp_color", &read_val);
	if (retval) {
	...
	/* get project id flag */
	retval = of_property_read_u32(device, "support_read_projectid",
				      &read_val);
	if (retval) {
...
static int himax_pinctrl_init(void)
{
...
#else
	g_himax_nc_ts_data->pinctrl_state_reset_high = pinctrl_lookup_state(
		g_himax_nc_ts_data->pctrl, PINCTRL_STATE_RESET_HIGH);
	...
	g_himax_nc_ts_data->pinctrl_state_reset_low = pinctrl_lookup_state(
		g_himax_nc_ts_data->pctrl, PINCTRL_STATE_RESET_LOW);
	...
	g_himax_nc_ts_data->pinctrl_state_as_int = pinctrl_lookup_state(
		g_himax_nc_ts_data->pctrl, PINCTRL_STATE_AS_INT);
	...
	g_himax_nc_ts_data->pinctrl_state_int_high = pinctrl_lookup_state(
		g_himax_nc_ts_data->pctrl, PINCTRL_STATE_INT_HIGH);
	...
	g_himax_nc_ts_data->pinctrl_state_int_low = pinctrl_lookup_state(
		g_himax_nc_ts_data->pctrl, PINCTRL_STATE_INT_LOW);
...
static void himax_pinctrl_select_normal(void)
{
	...
	if (pdata->pctrl && pdata->pins_default) {
		...
		if (ret < 0)
			TS_LOG_ERR(
				"pinctrl_release: Set normal pin state error:%d\n",
				ret);
	}
#endif
}
...
static void himax_pinctrl_select_suspend(void)
{
	...
	if (pdata->pctrl && pdata->pins_idle) {
		...
		if (ret < 0)
			TS_LOG_ERR(
				"pinctrl_select_suspend: Set suspend pin state error:%d\n",
				ret);
	}
#endif
}
...
static int himax_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	if (ts->ts_dev->dev.of_node) { /* DeviceTree Init Platform_data */
		...
		if (err < 0) {
			TS_LOG_INFO(
				"chip_detect: himax_parse_specific_dts failed\n");
	...
	if (err) {
		TS_LOG_ERR(
			"chip_detect: gpio direction output to 1 fail, err=%d\n",
			err);
		return err;
	}
#else
	...
	if (ic_nc_type == HX_83102E_SERIES_PWON) {
		...
		if (ts->support_get_tp_color || ts->support_read_projectid) {
	...
	if (ts->ts_dev->dev.of_node) { /* DeviceTree Init Platform_data */
		...
		if (err < 0) {
			TS_LOG_INFO(
				"chip_detect: himax_parse_project_dts failed\n");
...
static int himax_init_chip(void)
{
	...
	if (himax_nc_loadsensorconfig() < 0) {
		TS_LOG_ERR(
			"init_chip: Load Sesnsor config failed, unload driver\n");
		err = -ENOMEM;
	...
	if (hx_nc_get_mutual_new_buffer() == NULL) {
		TS_LOG_ERR(
			"init_chip: New mutual buffer allocate fail failed\n");
		goto err_getMutualNewBuffer_failed;
	}
	hx_nc_set_mutual_old_buffer();
	if (hx_nc_get_mutual_old_buffer() == NULL) {
		TS_LOG_ERR(
			"init_chip: Old mutual buffer allocate fail failed\n");
...
static int himax_core_suspend(void)
{
	...
	if (atomic_read(&hmx_nc_mmi_test_status)) {
		TS_LOG_INFO(
			"core_suspend: tp fw is hmx_nc_mmi_test_status,return\n");
		return NO_ERR;
	}
	if (ts->suspended) {
	...
	if (hx_nc_get_flash_dump_going()) {
		TS_LOG_INFO(
			"core_suspend: Flash dump is going, reject suspend\n");
	...
	if (true == ts->tskit_himax_data->ts_platform_data->feature_info
			    .wakeup_gesture_enable_info.switch_value) {
		if (ts->tskit_himax_data->ts_platform_data->chip_data
			    ->easy_wakeup_info.sleep_mode) {
			...
			if (retval < 0) {
				TS_LOG_ERR(
					"core_suspend: himax_enter_sleep_mode fail\n");
				return retval;
			}
		}
	}
	...
	if (retval < 0) {
		TS_LOG_ERR(
			"core_suspend: set pinctrl reset low failed,retval=%d\n",
			retval);
		return retval;
	}
#endif
...
static int himax_core_resume(void)
{
	...
	if (atomic_read(&hmx_nc_mmi_test_status)) {
		TS_LOG_INFO(
			"core_resume: tp fw hmx_nc_mmi_test_status, return\n");
		return NO_ERR;
	}
	...
	if (ts->tskit_himax_data->ts_platform_data->chip_data->easy_wakeup_info
		    .sleep_mode) {
		mutex_lock(&wrong_touch_lock);
		g_himax_nc_ts_data->tskit_himax_data->easy_wakeup_info
			.off_motion_on = false;
	...
	if (retval < 0) {
		TS_LOG_ERR(
			"core_resume: set pinctrl reset high failed,retval=%d",
			retval);
		return retval;
	}
#endif
...
static int himax_after_resume(void *feature_info)
{
	...
	if (info->charger_info.charger_supported) {
		...
		if (retval < 0) {
			TS_LOG_ERR(
				"after_resume: Failed set charger switch:%d, err:%d\n",
				info->charger_info.charger_switch, retval);
			return retval;
		}
	}
...
static int himax_power_off(void)
{
	...
	if (g_himax_nc_ts_data->pdata->gpio_3v3_en >= 0) {
		err = gpio_direction_output(
			g_himax_nc_ts_data->pdata->gpio_3v3_en, 0);
		if (err) {
	...
	if (g_himax_nc_ts_data->pdata->gpio_1v8_en >= 0) {
		err = gpio_direction_output(
			g_himax_nc_ts_data->pdata->gpio_1v8_en, 0);
		if (err) {
			TS_LOG_ERR("unable to set direction for gpio [%d]\n",
...
static int himax_chip_get_info(struct ts_chip_info_param *info)
{
	...
	snprintf(info->mod_vendor, sizeof(info->mod_vendor), "%s",
		 g_himax_nc_ts_data->tskit_himax_data->ts_platform_data
			 ->chip_data->module_name);
	snprintf(info->fw_vendor, sizeof(info->fw_vendor), "%x.%x.%x.%x.%x",
		 g_himax_nc_ts_data->vendor_panel_ver,
		 g_himax_nc_ts_data->vendor_fw_ver,
		 g_himax_nc_ts_data->vendor_config_ver,
...
bool himax_nc_sense_off(void)
{
	...
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
...
void himax_nc_sense_on(uint8_t flashmode)
{
	...
	if (retry >= 5) {
		...
		himax_rw_reg_reformat_com(ADDR_SENSE_ON, DATA_INIT, tmp_addr,
					  tmp_data);
		himax_register_write(tmp_addr, FOUR_BYTE_CMD, tmp_data);
	}
}
...
void touch_driver_sense_on(uint8_t flashmode)
{
	...
	TS_LOG_INFO("Enter %s\n", __func__);
	himax_nc_interface_on();
	himax_rw_reg_reformat_com(HX_FW_ADDR_CTRL_FW, DATA_INIT, tmp_addr,
				  tmp_data);
	himax_register_write(tmp_addr, FOUR_BYTE_CMD, tmp_data);
	msleep(HX_SLEEP_10MS);
	...
	if (!flashmode) {
		...
		do {
			...
			TS_LOG_INFO("%s:Read status from IC = %X, %X\n",
				    __func__, tmp_data[DATA_0],
				    tmp_data[DATA_1]);

		} while ((tmp_data[DATA_1] != HEX_NUM1 ||
			  tmp_data[DATA_0] != HEX_NUM0) &&
			 retry++ < DATA_5);
...
int himax_read_project_id(void)
{
	...
	flash_tmp_buffer =
		kzalloc(flash_page_len * sizeof(uint8_t), GFP_KERNEL);
	if (!flash_tmp_buffer)
		return HX_ERR;
	himax_nc_sense_off();
	...
	if (cnt > 0)
		TS_LOG_INFO("read_project_id: Polling auto+4 mode:%d times",
			    cnt);

	for (temp_addr = project_id_addr;
	     temp_addr < project_id_addr + flash_page_len;
		...
		memcpy(&flash_tmp_buffer[temp_addr - project_id_addr], buffer,
		       NOR_READ_LENGTH);
	}
	...
	if ((flash_tmp_buffer[8] != 0x00) && (flash_tmp_buffer[9] != 0x00) &&
		...
		strncpy(himax_nc_project_id, flash_tmp_buffer + 8,
			HX_PROJECT_ID_LEN); /* get project id from flash */
...
static int touch_driver_projectid_read(uint8_t *buffer)
{
	...
	len = (buffer[DATA_3] << LEFT_MOV_24BIT) |
	      ...
	      (buffer[DATA_1] << LEFT_MOV_8BIT) | buffer[DATA_0];
	...
	if (type == DATA_0) {
		honor_project_id =
			kzalloc((len + DATA_1) * sizeof(uint8_t), GFP_KERNEL);
		if (!honor_project_id) {
			TS_LOG_ERR("%s: Memory allocate FAIL!\n", __func__);
			return -ENOMEM;
		}
	} else if (type == DATA_1) {
		honor_project_id = kzalloc(len * sizeof(uint8_t), GFP_KERNEL);
		if (!honor_project_id) {
			TS_LOG_ERR("%s: Memory allocate FAIL!\n", __func__);
			return -ENOMEM;
		}
...
static int touch_driver_tpcolor_read(uint8_t *buffer)
{
	...
	if (type == DATA_0) {
		...
		honor_cg_color = kzalloc(len * sizeof(uint8_t), GFP_KERNEL);
		if (!honor_cg_color) {
			TS_LOG_ERR("%s: Memory allocate FAIL!\n", __func__);
			return -ENOMEM;
		}
	...
	if (type == DATA_0) {
		...
		for (j = 0; j < len; j++)
			TS_LOG_INFO("0x%02X", *(honor_cg_color + j));
	}
	TS_LOG_INFO("\n");
...
int touch_driver_read_panel_info(void)
{
	...
	buffer = kzalloc(INFO_PAGE_LENGTH * sizeof(uint8_t), GFP_KERNEL);
	if (!buffer) {
		TS_LOG_ERR("%s: Memory allocate FAIL!\n", __func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < INFO_SECTION_NUM; i++) {
		...
		if (touch_driver_check_crc(tmp_addr, INFO_PAGE_LENGTH) != 0) {
		...
		for (temp_addr = saddr; temp_addr < saddr + INFO_PAGE_LENGTH;
		     temp_addr += NOR_READ_LENGTH) {
			tmp_addr[DATA_0] = temp_addr % HEX_ONE_HUNDRED;
			tmp_addr[DATA_1] =
				(temp_addr >> RIGHT_MOV_8BIT) % HEX_ONE_HUNDRED;
			tmp_addr[DATA_2] = (temp_addr >> RIGHT_MOV_16BIT) %
					   HEX_ONE_HUNDRED;
			tmp_addr[DATA_3] = temp_addr / HEX_ONE_MILLION;
			himax_nc_register_read(tmp_addr, NOR_READ_LENGTH,
					       tmp_buf);
			memcpy(&buffer[temp_addr - saddr], tmp_buf,
			       NOR_READ_LENGTH);
		}
		...
		} else if (title[DATA_0] == DATA_0 && title[DATA_1] == DATA_0 &&
			   title[DATA_2] == DATA_1) {
			touch_driver_tpcolor_read(buffer);
		} else {
			TS_LOG_ERR("%s: UNKNOWN title %X%X%X\n", __func__,
				   title[DATA_0], title[DATA_1], title[DATA_2]);
		}
	}
	if (buffer != NULL)
		kfree(buffer);
...
{
	memcpy(ts->color_id, honor_cg_color, sizeof(ts->color_id));
...
void himax_nc_interface_on(void)
{
	...
	if (i2c_himax_nc_read(DUMMY_REGISTER, tmp_data, FOUR_BYTE_CMD,
			      sizeof(tmp_data), DEFAULT_RETRY_CNT) <
	    0) { // Read a dummy register to wake up I2C.
	...
	do {
		...
		if (tmp_data[0] == DATA_EN_BURST_MODE &&
		    tmp_buf[0] == DATA_AHB) {
			break;
		}
		msleep(HX_SLEEP_1MS);
	...
	if (cnt > 0)
		TS_LOG_INFO("nc_interface_on: Polling burst mode: %d times",
			    cnt);
}
...
static void himax_init_psl(void) // power saving level
{
	...
	// ==============================================================
	himax_rw_reg_reformat_com(ADDR_SCU_POWER_STATE, DATA_INIT, tmp_addr,
				  tmp_data);
	himax_register_write(tmp_addr, FOUR_BYTE_CMD, tmp_data);
...
static bool wait_wip(int Timing)
{
	...
	himax_rw_reg_reformat_com(ADDR_SPI_FORMAT, DATA_SPI_FORMAT, tmp_addr,
				  tmp_data);
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
	...
	do {
		...
		// =====================================
		himax_rw_reg_reformat_com(ADDR_SPI_WREN, DATA_SPI_CMD, tmp_addr,
					  tmp_data);
		himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
					sizeof(tmp_data));

		himax_rw_reg_reformat_com(ADDR_SPI_RESD_STATUS,
					  DATA_SPI_RESD_STATUS, tmp_addr,
					  in_buffer);
...
static void himax_chip_erase(void)
{
	...
	// =====================================
	himax_rw_reg_reformat_com(ADDR_SPI_CONTROL, DATA_WRITE_EN, tmp_addr,
				  tmp_data);
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
	himax_rw_reg_reformat_com(ADDR_SPI_WREN, DATA_ERASE_PRE, tmp_addr,
				  tmp_data);
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
	// =====================================
	...
	// =====================================
	himax_rw_reg_reformat_com(ADDR_SPI_WREN, DATA_ERASE, tmp_addr,
				  tmp_data);
	tmp_addr[3] = 0x80;
	tmp_addr[2] = 0x00;
	tmp_addr[1] = 0x00;
...
static void himax_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size;
		...
		if (i2c_himax_nc_write(ADDR_FLASH_BURNED, buring_data,
				       5 * FOUR_BYTE_CMD, sizeof(buring_data),
				       DEFAULT_RETRY_CNT) < 0) {
		...
		for (j = 0; j < 5; j++) {
			...
			if (i2c_himax_nc_write(ADDR_FLASH_BURNED, buring_data,
					       ...
					       DEFAULT_RETRY_CNT) < 0) {
				TS_LOG_ERR(
					"flash_programming: i2c access fail\n");
				return;
			}
		}
		...
		if (!wait_wip(1))
			TS_LOG_ERR(
				"flash_programming: 83102_Flash_Programming Fail\n");
	}
...
#define ADDR_CRC 0x80050020
	...
	himax_flash_write_burst(tmp_addr, start_addr, sizeof(tmp_addr),
				sizeof(tmp_data));

	tmp_len = (tmp_len << 16) + (length & 0x0000FFFF);
	himax_rw_reg_reformat_com(ADDR_CRC_DATAMAXLEN_SET, tmp_len, tmp_addr,
				  tmp_data);
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
	...
	do {
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		TS_LOG_INFO(
			"hw_check_crc: waiting for hw ready  tmp_data[3]=%X, tmp_data[2]=%X, tmp_data[1]=%X, tmp_data[0]=%X\n",
		...
		if ((tmp_data[0] & 0x01) != 0x01) {
			...
			himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD,
					       tmp_data);
			TS_LOG_INFO(
				"hw_check_crc: tmp_data[3]=%X, tmp_data[2]=%X, tmp_data[1]=%X, tmp_data[0]=%X\n",
	...
	do {
		...
		if ((tmp_data[DATA_0 + mod] & HEX_NUM1) != HEX_NUM1) {
			...
			himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD,
					       tmp_data);
			TS_LOG_INFO("%s: tmp_data[3]=%X, tmp_data[2]=%X\n",
				    __func__, tmp_data[DATA_3],
				    tmp_data[DATA_2]);
			TS_LOG_INFO("%s: tmp_data[1]=%X, tmp_data[0]=%X\n",
				    __func__, tmp_data[DATA_1],
				    tmp_data[DATA_0]);
...
uint8_t himax_nc_calculate_checksum(bool change_iref)
{
	...
	if (ic_nc_type == HX_83102E_SERIES_PWON) {
		...
		crc_result = touch_driver_check_crc(tmp_data, FW_SIZE_128K);
...
int hx_nc_fts_ctpm_fw_upgrade_with_fs(unsigned char *fw, int len,
				      bool change_iref)
{
	uint8_t tmp_addr[4] = {0};
	uint8_t tmp_data[4] = {0};
	...
	himax_rw_reg_reformat_com(ADDR_AHBI2C_SYSRST, DATA_AHBI2C_SYSRST,
				  tmp_addr, tmp_data);
	himax_register_write(tmp_addr, FOUR_BYTE_CMD, tmp_data);
	...
	if (ic_nc_type == HX_83102E_SERIES_PWON) {
		...
		himax_rw_reg_reformat_com(ADDR_DIAG_REG_SET_HX83102E, DATA_INIT,
					  tmp_addr, tmp_data);
	} else {
...
void himax_nc_flash_dump_func(uint8_t local_flash_command, int flash_size,
			      uint8_t *flash_buffer)
{
	...
	for (page_prog_start = 0; page_prog_start < flash_size;
		...
		himax_rw_reg_reformat_com(ADDR_SPI_CONTROL,
					  DATA_SET_PAGE_256_READ, tmp_addr,
					  tmp_data);
		himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
					sizeof(tmp_data));
		himax_rw_reg_reformat_com(ADDR_SET_READ_START,
		...
		himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
					sizeof(tmp_data));
		himax_rw_reg_reformat_com(ADDR_SPI_WREN, DATA_WRITE_CMD_X3B,
					  tmp_addr, tmp_data);
		himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
					sizeof(tmp_data));
		...
		i2c_himax_nc_write(ADDR_FLASH_BURNED, out_buffer, FOUR_BYTE_CMD,
				   sizeof(out_buffer), DEFAULT_RETRY_CNT);

		// ==================================
		// Read access : 0x0C ==> 0x00
		// ==================================
		out_buffer[0] = DATA_READ_ACCESS;
		i2c_himax_nc_write(ADDR_READ_ACCESS, out_buffer, ONE_BYTE_CMD,
				   sizeof(out_buffer), DEFAULT_RETRY_CNT);

		// ==================================
		// Read 128 bytes two times
		// ==================================
		i2c_himax_nc_read(DUMMY_REGISTER, in_buffer, NOR_READ_LENGTH,
				  sizeof(in_buffer), DEFAULT_RETRY_CNT);
		for (i = 0; i < 128; i++)
...
void himax_nc_get_dsram_data(uint8_t *info_data, size_t count)
{
	...
	uint8_t temp_info_data_hx102b[(MUTUL_NUM_HX83102 + SELF_NUM_HX83102) * 2 +
				      4 + 8] = {0}; // max mkey size = 8
	uint8_t temp_info_data_hx112a[(MUTUL_NUM_HX83112 + SELF_NUM_HX83112) * 2 +
				      4 + 8] = {0};
	...
	if (ic_nc_type == HX_83102E_SERIES_PWON) {
		temp_info_data =
			kcalloc(total_size, sizeof(uint8_t), GFP_KERNEL);
		if (!temp_info_data) {
	...
	himax_rw_reg_reformat_com(ADDR_DSRAM_START, DATA_DSRAM_START, tmp_addr,
				  tmp_data);
	...
	if (dsram_flag == false) {
		...
		himax_rw_reg_reformat_com(ADDR_DSRAM_START, DATA_INIT, tmp_addr,
					  tmp_data);
		himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
					sizeof(tmp_data));
	} else {
		...
		himax_rw_reg_reformat_com(ADDR_DSRAM_START,
					  DATA_RETURN_SRAM_EVENT, tmp_addr,
					  tmp_data);
		himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
					sizeof(tmp_data));
	}
	...
	for (i = 2; i < total_size; i = i + 2) { /* 2:PASSWORD NOT included */
		if (ic_nc_type == HX_83102B_SERIES_PWON)
			...
			check_sum_cal +=
				(temp_info_data[i + DATA_1] * SHIFT_NUM +
				 temp_info_data[i]);
	}
	...
	if (check_sum_cal % 0x10000 != 0) {
		...
		if (ic_nc_type == HX_83102E_SERIES_PWON) {
			if (g_himax_nc_ts_data->debug_log_level & BIT(DATA_5)) {
				TS_LOG_DEBUG(
					"get_dsram_data: skip checksum fail\n");
...
void himax_nc_diag_register_set(uint8_t diag_command)
{
	...
	if (ic_nc_type == HX_83102E_SERIES_PWON) {
		...
		himax_rw_reg_reformat_com(diag_reg_addr, (int)diag_command,
					  tmp_addr, tmp_data);
	} else {
		himax_rw_reg_reformat_com(ADDR_DIAG_REG_SET, (int)diag_command,
					  tmp_addr, tmp_data);
	}
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
	himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
	TS_LOG_INFO(
		"diag_register_set: tmp_data[3]=0x%02X,tmp_data[2]=0x%02X,tmp_data[1]=0x%02X,tmp_data[0]=0x%02X!\n",
		tmp_data[3], tmp_data[2], tmp_data[1], tmp_data[0]);
}
void himax_nc_reload_disable(int on)
{
	...
	if (on) {
		himax_rw_reg_reformat_com(ADDR_SWITCH_FLASH_RLD,
					  DATA_DISABLE_FLASH_RLD, tmp_addr,
					  tmp_data);
	} else {
		himax_rw_reg_reformat_com(ADDR_SWITCH_FLASH_RLD, DATA_INIT,
					  tmp_addr, tmp_data);
	}
	himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
				sizeof(tmp_data));
...
void himax_nc_idle_mode(int disable)
{
	...
	if (disable) {
		...
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		TS_LOG_INFO(
			"nc_idle_mode: After turn ON/OFF IDLE Mode [0]=0x%02X,[1]=0x%02X\n",
			tmp_data[0], tmp_data[1]);
		TS_LOG_INFO(
			"nc_idle_mode: After turn ON/OFF IDLE Mode [2]=0x%02X,[3]=0x%02X\n",
...
int himax_nc_switch_mode(int mode)
{
	...
	if (mode) {
		...
		himax_rw_reg_reformat_com(
			ADDR_NFRAME_SEL, DATA_SET_IIR_FRM, tmp_addr,
			tmp_data); /* 0x0A normal mode 10 frame */
		/* N Frame Sorting */
		himax_flash_write_burst(tmp_addr, tmp_data, sizeof(tmp_addr),
					sizeof(tmp_data));
		himax_nc_idle_mode(ON);
	...
	while (retry != 0) {
		...
		if ((ic_nc_type == HX_83112A_SERIES_PWON) ||
		    (ic_nc_type == HX_83102E_SERIES_PWON))
			himax_rw_reg_reformat(ADDR_MODE_SWITCH_HX83112,
					      tmp_addr);
		else if (ic_nc_type == HX_83102B_SERIES_PWON)
			himax_rw_reg_reformat(ADDR_MODE_SWITCH_HX83102,
					      tmp_addr);
		himax_nc_register_read(tmp_addr, FOUR_BYTE_CMD, tmp_data);
		msleep(HX_SLEEP_100MS);
		TS_LOG_INFO(
			"mode_read_cmd(0)=0x%2.2X,mode_read_cmd(1)=0x%2.2X\n",
			tmp_data[0], tmp_data[1]);
...
static int touch_driver_rawdata_proc_printf(struct seq_file *m,
					    ...
					    int range_size, int row_size)
{
	...
	if (g_ts_kit_platform_data.chip_data->trx_delta_test_support) {
		...
		for (index = 0; index < range_size - DATA_1; index++) {
			for (index1 = 0; index1 < row_size; index1++)
				seq_printf(m, "%4d,",
					   info->tx_delta_buf[index * row_size +
							      index1]);
			seq_puts(m, "\n");
		}
		...
		for (index = 0; index < range_size; index++) {
			for (index1 = 0; index1 < row_size - DATA_1; index1++)
				seq_printf(
					m, "%4d,",
					info->rx_delta_buf[index * (row_size -
								    DATA_1) +
							   index1]);
			seq_puts(m, "\n");
		}
...
static int __init himax_module_init(void)
{
	...
	if (!g_himax_nc_ts_data) {
		TS_LOG_ERR(
			"Failed to alloc mem for struct g_himax_nc_ts_data\n");
		err = -ENOMEM;
	...
	if (!root) {
		TS_LOG_ERR(
			"[HXTP]honor_ts, find_compatible_node honor,ts_kit error\n");
		err = -EINVAL;
		goto out;
	}
	/* find the chip node */
