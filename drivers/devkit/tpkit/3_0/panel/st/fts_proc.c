...
static void *fts_seq_start(struct seq_file *s, loff_t *pos)
{
	...
	if ((driver_test_buff == NULL) && (*pos == 0)) {
		...
		driver_test_buff =
			kzalloc(DRIER_TEST_BUFF_MAX * sizeof(u8), GFP_KERNEL);
 ...
 */
static const struct seq_operations fts_seq_ops = {.start = fts_seq_start,
						  .next = fts_seq_next,
						  .stop = fts_seq_stop,
						  .show = fts_seq_show};
				     ...
				     size_t count, loff_t *pos)
{
	...
	p = pbuf;
	if ((count > (MESSAGE_MIN_HEADER_SIZE - 1)) &&
	    (p[0] == MESSAGE_START_BYTE)) {
		...
		if ((count < mess.msg_size) ||
			...
			TS_LOG_ERR(
				"%s msg_size = %d != %d, last_byte = %02X != %02X error %08X\n",
		...
		case ACTION_WRITE_READ_U8UX:
			cmd[0] = func_to_test[0] = CMD_WRITE_READ_U8UX_BYTE;
			break;
		...
		case ACTION_WRITE_U8UX_THEN_WRITE_READ_U8UX:
			...
			break;
		case ACTION_GET_FW:
			cmd[0] = func_to_test[0] = CMD_GET_FW_FILE_BYTE;
			break;
		case ACTION_GET_LIMIT:
			cmd[0] = func_to_test[0] = CMD_GET_LIMITS_FILE_BYTE;
			break;
		default:
			TS_LOG_ERR("%s invalid action = %d error %08X\n",
				   __func__, mess.action, ERROR_OP_NOT_ALLOW);
			res = ERROR_OP_NOT_ALLOW;
			goto end;
		}
	...
	case CMD_WRITE_READ_BYTE:
		...
		read_data = kzalloc((byte_to_read + mess.dummy) * sizeof(u8),
				    GFP_KERNEL);
		if (read_data == NULL) {
			res = ERROR_ALLOC;
	...
	case CMD_WRITE_THEN_WRITE_READ_BYTE:
		 ...
		 */
		res = fts_write_then_write_read(&cmd[3], cmd[1],
	...
	case CMD_WRITE_READ_U8UX_BYTE:
		...
		if (read_data == NULL) {
			...
			res = fts_writeReadU8UX(cmd[1], cmd[2], addr, read_data,
						byte_to_read,
						cmd[number_param - 1]);
			size += (byte_to_read * sizeof(u8));
		}
	...
	index += snprintf(&driver_test_buff[index], BUFF_LENGTH, "%04X",
			  (u16)mess.counter);
	index += snprintf(&driver_test_buff[index], BUFF_LENGTH, "%04X",
			  (u16)mess.action);
	index += snprintf(&driver_test_buff[index], BUFF_LENGTH, "%02X%02X",
