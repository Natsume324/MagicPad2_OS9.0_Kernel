...
LOAD_FRC:
	...
	memset(frame->node_data, 0, frame->node_data_size * sizeof(short));
...
int getSSFrame3(SSFrameType type, SelfSenseFrame *frame)
{
	...
	memset(frame->force_data, 0, frame->header.force_node * sizeof(short));
		       ...
		       u64 *address)
{
	...
	if (data[0] != HEADER_SIGNATURE) {
		TS_LOG_ERR(
			"%s: The Header Signature was wrong! %02X != %02X ERROR %08X\n",
...
int getSyncFrame(u8 type, MutualSenseFrame *msFrame, SelfSenseFrame *ssFrame)
{
	...
	if (res < OK) {
		TS_LOG_ERR(
			"%s: error while reading Sync Frame header... ERROR %08X\n",
			__func__, res | ERROR_GET_FRAME_DATA);
		return res | ERROR_GET_FRAME_DATA;
	}
	...
	if (msFrame->node_data == NULL) {
		TS_LOG_ERR(
			"%s: impossible allocate memory for MS frame... ERROR %08X\n",
			__func__, ERROR_ALLOC | ERROR_GET_FRAME);
		return ERROR_ALLOC | ERROR_GET_FRAME;
	}
	...
	if (ssFrame->force_data == NULL) {
		TS_LOG_ERR(
			"%s: impossible allocate memory for SS force frame...ERROR %08X\n",
			__func__, ERROR_ALLOC | ERROR_GET_FRAME);
	...
	if (res < OK) {
		TS_LOG_ERR(
			"%s: error while getting SS force data...ERROR %08X\n",
			__func__, res);
		res |= ERROR_GET_FRAME_DATA | ERROR_GET_FRAME;
		goto ERROR;
	}
	...
	if (ssFrame->sense_data == NULL) {
		TS_LOG_ERR(
			"%s: impossible allocate memory for SS sense frame...ERROR %08X\n",
			__func__, ERROR_ALLOC | ERROR_GET_FRAME);
		res = ERROR_ALLOC | ERROR_GET_FRAME;
		goto ERROR;
	}
	...
	if (res < OK) {
		TS_LOG_ERR(
			"%s: error while getting SS sense data...ERROR %08X\n",
			__func__, res);
		res |= ERROR_GET_FRAME_DATA | ERROR_GET_FRAME;
		goto ERROR;
	}
