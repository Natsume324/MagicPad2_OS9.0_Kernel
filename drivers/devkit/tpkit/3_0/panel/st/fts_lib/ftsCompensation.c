...
int readSelfSenseNodeData(u64 address, SelfSenseData *node)
{
	...
	if (node->ix2_fm == NULL) {
		TS_LOG_ERR(
			"%s: can not allocate memory for ix2_fm... ERROR %08X",
			__func__, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
	...
	if (node->cx2_fm == NULL) {
		TS_LOG_ERR(
			"%s: can not allocate memory for cx2_fm ... ERROR %08X",
			__func__, ERROR_ALLOC);
		ret = ERROR_ALLOC;
		goto ERR;
	}
...
int readSelfSenseCompensationData(u8 type, SelfSenseData *data)
{
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"%s: error while reading data header... ERROR %08X\n",
...
int readTotMutualSenseCompensationData(u8 type, TotMutualSenseData *data)
{
	...
	if (!(type == LOAD_PANEL_CX_TOT_MS_TOUCH ||
		...
		TS_LOG_ERR(
			"%s: Choose a TOT MS type of compensation data ERROR %08X\n",
			__func__, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
