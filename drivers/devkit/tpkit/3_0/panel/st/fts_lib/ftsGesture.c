...
int updateGestureMask(u8 *mask, int size, int en)
{
	...
	if (en == FEAT_ENABLE) {
		...
		TS_LOG_ERR(
			...
			__func__, en, FEAT_DISABLE, FEAT_ENABLE,
			ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
}
...
int isAnyGestureActive(void)
{
	...
	if (gesture_mask[res] != 0) {
		TS_LOG_INFO(
			"%s: Active Gestures Found! gesture_mask[%d] = %02X !\n",
...
int readGestureCoords(u8 *event, struct fts_ts_info *info)
{
	...
	if (event[0] == EVT_ID_USER_REPORT &&
		...
		gesture_coords_reported =
			event[5]; /* number of pairs coords reported */
		...
		if (res < OK) {
			TS_LOG_ERR(
				"%s: Cannot read the coordinates! ERROR %08X\n",
				__func__, res);
		...
		TS_LOG_ERR(
			"%s: The event passsed as argument is invalid! ERROR %08X\n",
			__func__, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
}
