...
int fts_read(u8 *outBuf, int byteToRead)
{
	...
	while (retry < I2C_RETRY && ret < OK) {
		ret = i2c_transfer(((struct i2c_client *)client)->adapter,
				   I2CMsg, 1);
		retry++;
		if (ret < OK)
...
int fts_writeRead(u8 *cmd, int cmdLength, u8 *outBuf, int byteToRead)
{
	...
	while (retry < I2C_RETRY && ret < OK) {
		ret = info->chip_data->ts_platform_data->bops->bus_read(
			cmd, cmdLength, outBuf, byteToRead);
		retry++;
		if (ret < OK)
			msleep(I2C_WAIT_BEFORE_RETRY);
	}
...
int fts_write(u8 *cmd, int cmdLength)
{
	...
	while (retry < I2C_RETRY && ret < OK) {
		ret = info->chip_data->ts_platform_data->bops->bus_write(
			cmd, cmdLength);
		retry++;
		if (ret < OK)
			msleep(I2C_WAIT_BEFORE_RETRY);
 ...
 * @return OK if success or an error code which specify the type of error
 */
int fts_write_then_write_read(u8 *write_cmd, int write_cmd_length, u8 *read_cmd,
			      int read_cmd_length, u8 *out_buf,
			      int byte_to_read)
{
	struct fts_ts_info *info = fts_get_info();
	...
	if (info->chip_data->ts_platform_data->bops->bus_write(
		    write_cmd, write_cmd_length))
		TS_LOG_ERR("%s:write fail\n", __func__);
	return info->chip_data->ts_platform_data->bops->bus_read(
		read_cmd, read_cmd_length, out_buf, byte_to_read);
}
...
int fts_chunk_write(u8 cmd_first, AddrSize addr_size_first, u8 cmd_second,
		    AddrSize addr_size_second, u64 address, u8 *data,
		    int data_size)
{
	int to_write;
	int i;
	...
	while (remaining > 0) {
		...
		for (i = 0; i < addr_size_first; i++)
			final_cmd_first[i + 1] =
				(u8)((address >> ((addr_size_first +
						   addr_size_second - 1 - i) *
						  ADDR_SIZE)) &
				     0xFF);
		final_cmd_second[0] = cmd_second;
		for (i = addr_size_first;
		     i < addr_size_first + addr_size_second; i++)
		...
		memcpy(&final_cmd_second[addr_size_second + 1], data, to_write);
		...
		if (fts_write(final_cmd_second,
			      1 + addr_size_second + to_write) < OK) {
 ...
 * max 8 bytes
 */
int fts_chunk_write_read(u8 cmd_first, AddrSize addr_size_first, u8 cmd_second,
			 AddrSize addr_size_second, u64 address, u8 *out_buf,
			 int byte_to_read, int has_dummy_byte)
{
	int to_read;
	int i;
	/*
