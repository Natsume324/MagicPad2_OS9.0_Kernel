...
int checkLimitsGapOffsets(short *data, int row, int column, int threshold,
			  int row_start, int column_start, int row_end,
			  int column_end)
{
	int i = 0;
	int j = 0;
...
static void st_fill_rawdata_buf(struct ts_rawdata_info *info,
				short *source_data, int rows, int columns,
				u16 datatype)
{
	int i = 0;
	int j = 0;
	...
	TS_LOG_DEBUG(
		"%s:info used size=%d, hybird used size=%d data type=%d, rows=%d, columns=%d\n",
		__func__, info->used_size, info->hybrid_buff_used_size,
...
static void get_average_max_min(short *data, int rows, int columns,
				char *result)
{
	int i = 0;
	short ms_raw_max = 0;
	...
	snprintf(result, ST_NP_TEST_RES_BUF_LEN - 1, "[%d,%d,%d]",
		 ms_raw_average, ms_raw_max, ms_raw_min);
}
				...
				TestResult *result)
{
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					MS_RAW_ITO_MAX, &thresholds_max, &rows,
					&columns);
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					MS_RAW_ITO_MIN, &thresholds_min, &rows,
					&columns);
	if ((ret < OK) || (rows != ms_raw_frame.header.force_node) ||
	    (columns != ms_raw_frame.header.sense_node)) {
		TS_LOG_ERR("%s: Test Limits failed %08X\n", __func__,
			...
			struct ts_rawdata_info *info)
{
	...
	if (todo->MutualRawAdjITO == 1) {
		...
		if (res < OK ||
			...
			TS_LOG_ERR(
				"%s: parseProductionTestLimits MS_RAW_ITO_ADJH failed... ERROR %08X\n",
		...
		if (res != OK) {
			...
			TS_LOG_INFO(
				...
				__func__);
			temp1 = array1dTo2d_short(msRawFrame.node_data,
		...
		if (res < OK || (trows != msRawFrame.header.force_node - 1 ||
			...
			TS_LOG_ERR(
				"%s: parseProductionTestLimits MS_RAW_ITO_ADJV failed... ERROR %08X\n",
				__func__, ERROR_PROD_TEST_ITO);
			goto ERROR;
		}
		...
		if (res != OK) {
			TS_LOG_ERR(
				"%s: checkLimitsAdj MS RAW ITO ADJV failed... ERROR COUNT = %d\n",
				__func__, res);
			TS_LOG_INFO(
			 ...
			 struct ts_rawdata_info *info, TestResult *result)
{
	...
	ret = production_test_data(pathThresholds, stop_on_fail, todo, info,
				   result);
	if (ret < OK) {
		TS_LOG_INFO(
			   ...
			   struct ts_rawdata_info *info, TestResult *result)
{
	...
	if (todo->MutualRaw == 1 || todo->MutualRawGap == 1 ||
		...
		if (info && result) {
			ret = parseProductionTestLimits(
				path_limits, &limit_file, MS_STRENGTH_MIN_MAX,
				&thresholds, &trows, &tcolumns);
			if (ret < OK || (trows != 1 || tcolumns != 2)) {
				TS_LOG_ERR(
		...
		if ((todo->MutualRawAdj == 1) || (todo->MutualRawAdjGap == 1)) {
			...
			if (todo->MutualRawAdjGap) {
				...
				ret = parseProductionTestLimits(
					...
					MS_RAW_ADJH_GAP, &thresholds, &trows,
					&tcolumns);
				if (ret < OK || (trows != 1 || tcolumns != 1)) {
					TS_LOG_ERR(
						"%s production_test_data: parseProductionTestLimits MS_RAW_ADJH failed... ERROR %08X\n",
			...
			if (todo->MutualRawAdjGap) {
				...
				ret = parseProductionTestLimits(
					path_limits, &limit_file,
					MS_RAW_ADJV_GAP, &thresholds, &trows,
					&tcolumns);
				if (ret < OK || (trows != 1 || tcolumns != 1)) {
					TS_LOG_ERR(
	...
	if (info && result) {
		print_data_s16(msRawFrame.node_data,
			       msRawFrame.header.force_node,
			       msRawFrame.header.sense_node);
		print_data_s16(msStrengthFrame.node_data,
			       msStrengthFrame.header.force_node,
			       msStrengthFrame.header.sense_node);
		...
		st_fill_rawdata_buf(info, msRawFrame.node_data,
				    ...
				    msRawFrame.header.sense_node,
				    MUTUALRAWTYPE);
		st_fill_rawdata_buf(info, msStrengthFrame.node_data,
				    msStrengthFrame.header.force_node,
				    msStrengthFrame.header.sense_node,
				    STRENGTHTYPE);

		get_average_max_min(msRawFrame.node_data,
				    msRawFrame.header.force_node,
				    msRawFrame.header.sense_node,
				    result->mutal_raw_res_buf);
		get_average_max_min(msStrengthFrame.node_data,
				    msStrengthFrame.header.force_node,
				    msStrengthFrame.header.sense_node,
				    result->mutal_noise_res_buf);
	}
ERROR:
	TS_LOG_INFO("%s\n", __func__);
	...
	if (count_fail == 0) {
		...
		temp1 = array1dTo2d_short(msRawFrame.node_data,
					  ...
					  msRawFrame.header.sense_node);
		if (temp1) {
			print_frame_short("MS Raw frame =", temp1,
					  msRawFrame.header.force_node,
					  msRawFrame.header.sense_node);
		}
					       ...
					       MutualSenseData ms_temp_data)
{
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					MS_CX2_ADJV_MAP_MAX, &thresholds_max,
					&rows, &columns);
	/* check  rows - 1 times */
						  ...
						  MutualSenseData ms_temp_data)
{
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					MS_CX2_ADJH_MAP_MAX, &thresholds_max,
					&rows, &columns);
	/* check  columns - 1 times */
	...
	ret = checkLimitsMapAdj(adj_horizontal, ms_temp_data.header.force_node,
				ms_temp_data.header.sense_node - 1,
				thresholds_max);
	if (ret != OK)
			  ...
			  struct ts_rawdata_info *info, TestResult *result)
{
	...
	temp_u16_array = kzalloc(sizeof(u16) * columns * rows, GFP_KERNEL);
	...
	if (todo->SelfForceRaw == 1) {
		...
		if (todo->SelfForceRaw == 1) {
			...
			if (ret != OK) {
				...
				temp_short = array1dTo2d_short(
					ssRawFrame.force_data, rows * columns,
					columns);
				if (temp_short) {
					print_frame_short(
						"SS Raw force frame =",
						temp_short, rows, columns);
				}
	...
	if (todo->SelfSenseRaw == 1) {
		...
		if (todo->SelfSenseRaw == 1) {
			...
			if (ret != OK) {
				...
				temp_short = array1dTo2d_short(
					ssRawFrame.sense_data, rows * columns,
					columns);
				if (temp_short) {
					print_frame_short(
						"SS Raw sense frame =",
		...
		if (result && info) {
			ret = parseProductionTestLimits(
				path_limits, &limit_file,
				SS_STRENGTH_FORCE_MIN_MAX, &thresholds, &trows,
				&tcolumns);
			if (ret < 0 || (trows != 1 || tcolumns != 2)) {
				TS_LOG_ERR(
					"%s production_test_data: parseProductionTestLimits SS_STRENGTH_FORCE_MIN_MAX failed... ERROR %08X\n",
					__func__, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			ret = checkLimitsMinMax(
				ssStrengthFrame.force_data,
				ssStrengthFrame.header.force_node, 1,
				thresholds[0], thresholds[1]);
			if (ret != OK) {
				TS_LOG_ERR(
					"%s production_test_data:SS_STRENGTH_FORCE_MIN_MAX failed... ERROR COUNT = %d\n",
					__func__, ret);
				TS_LOG_INFO(
					"%s SS SS_STRENGTH_FORCE_MIN_MAX TEST:.................FAIL\n\n",
					__func__);
				if (result != NULL)
				...
				TS_LOG_INFO(
					"%s SS_STRENGTH_FORCE_MIN_MAX TEST:.................OK\n\n",
					__func__);
			}
			if (thresholds == NULL) {
				kfree(thresholds);
			...
			ret = parseProductionTestLimits(
				path_limits, &limit_file,
				SS_STRENGTH_SENSE_MIN_MAX, &thresholds, &trows,
				&tcolumns);
			if (ret < 0 || (trows != 1 || tcolumns != 2)) {
				TS_LOG_ERR(
					"%s production_test_data: parseProductionTestLimits SS_STRENGTH_SENSE_MIN_MAX failed... ERROR %08X\n",
					__func__, ERROR_PROD_TEST_DATA);
				ret |= ERROR_PROD_TEST_DATA;
				goto ERROR_LIMITS;
			}
			...
			ret = checkLimitsMinMax(
				ssStrengthFrame.sense_data, 1,
				ssStrengthFrame.header.sense_node,
				thresholds[0], thresholds[1]);
			if (ret != OK) {
				TS_LOG_ERR(
					"%s production_test_data:SS_STRENGTH_SENSE_MIN_MAX failed... ERROR COUNT = %d\n",
					__func__, ret);
				TS_LOG_INFO(
					"%s SS SS_STRENGTH_SENSE_MIN_MAX TEST:.................FAIL\n\n",
					__func__);
				if (result != NULL)
				...
				TS_LOG_INFO(
					"%s SS_STRENGTH_SENSE_MIN_MAX TEST:.................OK\n\n",
					__func__);
			}
			kfree(thresholds);
			thresholds = NULL;
			TS_LOG_INFO("%s.\n\n", __func__);
			print_data_s16(ssStrengthFrame.sense_data, 1,
				       ssStrengthFrame.header.sense_node);
			st_fill_rawdata_buf(info, ssStrengthFrame.sense_data, 1,
					    ssStrengthFrame.header.sense_node,
					    SSSENSEPRXTYPE);

			print_data_s16(ssStrengthFrame.force_data,
				       ssStrengthFrame.header.force_node, 1);
			st_fill_rawdata_buf(info, ssStrengthFrame.force_data,
					    ssStrengthFrame.header.force_node,
					    1, SSFORCEPRXTYPE);

			print_data_s16(ssRawFrame.sense_data, 1,
				       ssRawFrame.header.sense_node);
			st_fill_rawdata_buf(info, ssRawFrame.sense_data, 1,
					    ssRawFrame.header.sense_node,
					    SELFSENSERAWTYPE);

			print_data_s16(ssRawFrame.force_data,
				       ssRawFrame.header.force_node, 1);
			st_fill_rawdata_buf(info, ssRawFrame.force_data,
					    ssRawFrame.header.force_node, 1,
					    SELFFORCERAWTYPE);
		}
			      ...
			      TestToDo *todo)
{
	...
	if ((todo->SelfForceRawLP == 1) && ssRawFrame.header.force_node != 0) {
		columns = 1; /* there are no data for the sense channels
				  *  because is a force frame */
		rows = ssRawFrame.header.force_node;
					     ...
					     TestResult *result)
{
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					SS_TX_IX_FORCE_MIN, &thresholds_min,
					&rows, &columns);
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					SS_TX_IX_FORCE_MAX, &thresholds_max,
					&rows, &columns);
	if ((ret < 0) || (rows != LIMITS_ROWS_SIZE) ||
	    (columns != ssCompData.header.force_node)) {
		TS_LOG_ERR("%s : parse Test Limits failed %08X\n", __func__,
	...
	ret = check_limits_map_u8(ssCompData.ix2_fm, rows,
				  ssCompData.header.force_node, thresholds_min,
				  thresholds_max);
	if (ret != OK) {
					  ...
					  TestResult *result)
{
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					SS_RX_IX_SENSE_MIN, &thresholds_min,
					&rows, &columns);
	if ((ret < 0) || (rows != LIMITS_ROWS_SIZE) ||
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					SS_RX_IX_SENSE_MAX, &thresholds_max,
					&rows, &columns);
	if ((ret < 0) || (rows != LIMITS_ROWS_SIZE) ||
	    (columns != ssCompData.header.sense_node)) {
		TS_LOG_ERR("%s : parse Test Limits failed %08X\n", __func__,
				       ...
				       TestResult *result)
{
	...
	info->self_buff_size =
		ssCompData.header.sense_node * ssCompData.header.force_node;
	...
	ret = parseProductionTestLimits(path_limits, &limit_file,
					SS_IX1_SENSE_FORCE_MIN_MAX, &thresholds,
					&rows, &columns);
...
int production_test_ss_ix_cx(char *path_limits, int stop_on_fail,
			     TestToDo *todo, struct ts_rawdata_info *info,
			     TestResult *result)
{
	int ret = 0;
	int count_fail = 0;
	...
	if (fts_info->chip_data->support_self_calibrate_test) {
		...
		ret = self_calibrate_min_max_test(path_limits, ssCompData, info,
						  result);
		if (ret != OK) {
			result->self_calibrate_min_max = false;
...
ERROR:
	...
	if (count_fail == 0) {
		...
		temp_i8 = array1dTo2d_i8(ssCompData.cx2_fm,
					 ssCompData.header.force_node, 1);
		if (temp_i8) {
			print_frame_i8("SS Init Data Cx2_fm = ", temp_i8,
				       ssCompData.header.force_node, 1);
		}
		temp_u8 = array1dTo2d_u8(ssCompData.ix2_sn,
					 ...
					 ssCompData.header.sense_node);
		if (temp_u8) {
			print_frame_u8("SS Init Data Ix2_sn = ", temp_u8, 1,
				       ssCompData.header.sense_node);
		}
		...
		temp_u16 = array1dTo2d_u16(totCompData.ix_fm,
					   totCompData.header.force_node, 1);
		if (temp_u16) {
		...
		temp_short = array1dTo2d_short(
			totCompData.cx_fm, totCompData.header.force_node, 1);
		if (temp_short) {
			print_frame_short(
				"TOT SS Init Data Cx_fm = ", temp_short,
				totCompData.header.force_node, 1);
		}
		temp_u16 = array1dTo2d_u16(totCompData.ix_sn,
					   totCompData.header.sense_node,
					   totCompData.header.sense_node);
		if (temp_u16) {
			print_frame_u16("TOT SS Init Data Ix_sn = ", temp_u16,
		...
		if (temp_short) {
			print_frame_short(
				"TOT SS Init Data Cx_sn = ", temp_short, 1,
				totCompData.header.sense_node);
		}
		TS_LOG_INFO(
				...
				TestToDo *todo)
{
	...
	if (todo->SelfForceIxTotalLP == 1) {
		...
		if (todo->SelfForceIxTotalLP == 1) {
			ret = parseProductionTestLimits(
				path_limits, &limit_file,
				SS_TOTAL_IX_LP_FORCE_MAP_MIN, &thresholds_min,
				&trows, &tcolumns);
			/* load the min thresholds */
			if (ret < 0 ||
			    (trows != totCompData.header.force_node ||
			...
			ret = parseProductionTestLimits(
				path_limits, &limit_file,
				SS_TOTAL_IX_LP_FORCE_MAP_MAX, &thresholds_max,
				&trows, &tcolumns);
			/* load the max thresholds */
			if (ret < 0 ||
			    (trows != totCompData.header.force_node ||
	...
	if (todo->SelfSenseIxTotalLP == 1) {
		...
		if (todo->SelfSenseIxTotalLP == 1) {
			ret = parseProductionTestLimits(
				path_limits, &limit_file,
				SS_TOTAL_IX_LP_SENSE_MAP_MIN, &thresholds_min,
				&trows, &tcolumns);
			/* load the min thresholds */
			if (ret < 0 ||
			    (trows != 1 ||
			...
			ret = parseProductionTestLimits(
				path_limits, &limit_file,
				SS_TOTAL_IX_LP_SENSE_MAP_MAX, &thresholds_max,
				&trows, &tcolumns);
			/* load the max thresholds */
			if (ret < 0 ||
			    (trows != 1 ||
...
ERROR:
	...
	if (count_fail == 0) {
		...
		if (temp_u16) {
			print_frame_u16(
				"TOT SS LP Init Data Ix_fm = ", temp_u16,
				totCompData.header.force_node, 1);
		}
		temp_short = array1dTo2d_short(
			totCompData.cx_fm, totCompData.header.force_node, 1);
		if (temp_short) {
			print_frame_short(
				"TOT SS LP Init Data Cx_fm = ", temp_short,
				totCompData.header.force_node, 1);
		}
		temp_u16 = array1dTo2d_u16(totCompData.ix_sn,
					   totCompData.header.sense_node,
		...
		if (temp_u16) {
			print_frame_u16(
				"TOT SS LP Init Data Ix_sn = ", temp_u16, 1,
				totCompData.header.sense_node);
		}
		temp_short = array1dTo2d_short(totCompData.cx_sn,
					       totCompData.header.sense_node,
		...
		if (temp_short) {
			print_frame_short(
				"TOT SS LP Init Data Cx_sn = ", temp_short, 1,
				totCompData.header.sense_node);
		}
		TS_LOG_INFO(
			 ...
			 struct ts_rawdata_info *info, TestResult *result)
{
	...
	ret = production_test_ms_raw(path_limits, stop_on_fail, todo, info,
				     result);
	res |= ret;
	if (ret < 0) {
		TS_LOG_ERR(
	...
	ret = production_test_ms_cx(path_limits, stop_on_fail, todo, info,
				    result);
	res |= ret;
	if (ret < 0) {
		TS_LOG_ERR(
	...
	ret = production_test_ss_ix_cx(path_limits, stop_on_fail, todo, info,
				       result);
	res |= ret;
	if (ret < 0) {
		TS_LOG_ERR(
...
int st_get_rawdata_aftertest(struct ts_rawdata_info *info, u8 signature)
{
	...
	ret = production_test_main(file_path, 0, NO_INIT, &tests, MP_FLAG_BOOT,
				   info, TestRes);
	...
	strncat(info->result, TestRes->mutal_raw_res_buf,
		ST_NP_TEST_RES_BUF_LEN);
	strncat(info->result, TestRes->mutal_noise_res_buf,
		ST_NP_TEST_RES_BUF_LEN);
	strncat(info->result, TestRes->mutal_cal_res_buf,
		ST_NP_TEST_RES_BUF_LEN);
	...
	snprintf(fw_version_info, sizeof(fw_version_info) - 1, "-%x_%x;",
