...
int dumpErrorInfo(void)
{
	...
	if (ret < OK) {
		...
		if (sign != ERROR_DUMP_SIGNATURE)
			TS_LOG_ERR(
				" %s: Wrong Error Signature! Data may be invalid!\n",
				__func__);
		else
...
int errorHandler(u8 *event, int size)
{
	...
	if (info != NULL && event != NULL && (size > 1) &&
		...
		case EVT_TYPE_ERROR_ITO_FORCE_P2P:
			TS_LOG_ERR("%s errorHandler: Force Pin to Pin Short!\n",
				   __func__);
			break;
...
int addErrorIntoList(u8 *event, int size)
{
	...
	if (!event ||
	    ((FIFO_DEPTH - errors.last_index) * FIFO_EVENT_SIZE <= size)) {
...
int pollForErrorType(u8 *list, int size)
{
	...
	if (find == 1) {
		...
		TS_LOG_ERR(
			" %s: Error Type Not Found into ErrorList! ERROR %08X\n",
			__func__, ERROR_TIMEOUT);
		return ERROR_TIMEOUT;
	}
}
