		 ...
		 int time_to_wait)
{
	...
	while (find != 1 && retry < time_to_count &&
		...
		if (readData[0] == EVT_ID_ERROR) {
			temp1 = printHex("ERROR EVENT = ", readData,
			...
			if (readData[0] != EVT_ID_NOEVENT) {
				temp1 = printHex("READ EVENT = ", readData,
						 FIFO_EVENT_SIZE, temp);
				if (temp1) {
					TS_LOG_ERR("%s %s\n", __func__, temp1);
				}
	...
	if ((retry >= time_to_count) && find != 1) {
		...
		temp1 = printHex("FOUND EVENT = ", readData, FIFO_EVENT_SIZE,
				 temp);
		if (temp1) {
			TS_LOG_ERR("%s %s\n", __func__, temp1);
		}
...
int readSysInfo(int request)
{
	...
	if (ret < OK) {
		TS_LOG_ERR(
			"%s: error while reading the system data ERROR %08X\n",
			__func__, ret);
		goto FAIL;
	}
	...
	if (data[0] != HEADER_SIGNATURE) {
		TS_LOG_ERR(
			"%s : The Header Signature is wrong!  sign: %02X != %02X ERROR %08X\n",
	...
	if (data[1] != LOAD_SYS_INFO) {
		TS_LOG_ERR(
			"%s: The Data ID is wrong!  ids: %02X != %02X ERROR %08X\n",
	...
	temp1 = printHex("Die Info =  ", systemInfo.u8_dieInfo, DIE_INFO_SIZE,
			 temp);
	if (temp1) {
		TS_LOG_ERR("%s %s\n", __func__, temp1);
	}
	...
	temp1 = printHex("Release Info =  ", systemInfo.u8_releaseInfo,
...
int readConfig(u16 offset, u8 *outBuf, int len)
{
	...
	TS_LOG_INFO("%s: Starting to read config memory at %08llX\n", __func__,
		    final_offset);
	ret = fts_writeReadU8UX(FTS_CMD_CONFIG_R, BITS_16, final_offset, outBuf,
				len, DUMMY_CONFIG);
	if (ret < OK) {
		TS_LOG_ERR(
			" %s: Impossible to read Config Memory... ERROR %08X!\n",
			__func__, ret);
		return ret;
	}
...
int writeConfig(u16 offset, u8 *data, int len)
{
	...
	if (ret < OK) {
		TS_LOG_ERR(
			"%s: Impossible to write Config Memory... ERROR %08X!\n",
			__func__, ret);
		return ret;
	}
	...
	if (res >= OK) {
		...
		if (res < OK) {
			...
			if (res < OK) {
				...
				TS_LOG_ERR(
					"%s: Cx CRC Error found! CRC ERROR = %02X\n",
					__func__, res);
				return CRC_CX;
			}
		} else {
			TS_LOG_ERR(
				"%s: Config CRC Error found! CRC ERROR = %02X\n",
				__func__, res);
			return CRC_CONFIG;
		}
	} else {
		TS_LOG_ERR(
			"%s: Error while executing system reset! ERROR %08X\n",
			__func__, res);
		return res;
	}
...
int requestSyncFrame(u8 type)
{
	...
	while (retry2 < RETRY_MAX_REQU_DATA) {
		...
		if (ret < OK) {
			TS_LOG_INFO(
				"%s: Error while reading count! ERROR %08X\n",
		...
		if (readData[0] != HEADER_SIGNATURE) {
			TS_LOG_ERR(
				"%s: Invalid Signature while reading count! ERROR %08X\n",
				__func__, ret | ERROR_REQU_DATA);
		}
		...
		if (ret >= OK) {
			...
			while (count == new_count && retry < time_to_count) {
				...
				if ((ret >= OK) &&
					...
					TS_LOG_INFO(
						"%s: invalid Signature or can not read count... ERROR %08X\n",
						__func__, ret);
				retry++;
				msleep(TIMEOUT_RESOLUTION);
			}
			...
			if (count == new_count) {
				TS_LOG_ERR(
					"%s: New count not received! ERROR %08X\n",
					__func__,
					ERROR_TIMEOUT | ERROR_REQU_DATA);
				ret = ERROR_TIMEOUT | ERROR_REQU_DATA;
			} else {
				TS_LOG_INFO(
					"%s: New count found! count = %d! Frame ready!\n",
					__func__, new_count);
				return OK;
			}
		}
...
int saveMpFlag(u8 mpflag)
{
	...
	if (ret < OK) {
		TS_LOG_ERR(
			"%s: Error while writing MP flag on ram... ERROR %08X\n",
			__func__, ret);
		return ret;
	}
	...
	if (ret < OK) {
		TS_LOG_ERR(
			"%s: Error while saving MP flag on flash... ERROR %08X\n",
			__func__, ret);
		return ret;
	}
...
int calculateCRC8(u8 *u8_srcBuff, int size, u8 *crc)
{
	...
	if (size != 0 && u8_srcBuff != NULL) {
		...
		for (i = 0; i < size;
		     i++) { //Bring the next byte into the remainder.
			u8_remainder ^= u8_srcBuff
				[i]; //Perform modulo-2 division, a bit at a time.
			for (bit = 8; bit > 0;
			     --bit) { //Try to divide the current data bit.
		...
		*crc = u8_remainder;
		...
		TS_LOG_ERR(
			"%s: Arguments passed not valid! Data pointer = NULL or size = 0 (%d) ERROR %08X\n",
			__func__, size, ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}
}
...
int writeLockDownInfo(u8 *data, int size, u8 lock_id)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret < OK) {
			TS_LOG_ERR(
				"%s : Unable to compute data CRC.. ERROR %08X\n",
				__func__, ret);
		...
		if (ret < OK) {
			TS_LOG_ERR(
				"%s  Unable to write Lockdown data prepare at %d iteration.. ERROR %08X\n",
				__func__, i, ret);
			ret = (ret | ERROR_LOCKDOWN_CODE);
			continue;
		}
		...
		if (ret < OK) {
			TS_LOG_ERR(
				"%s: Unable to compute head CRC.. ERROR %08X\n",
				__func__, ret);
			ret = (ret | ERROR_LOCKDOWN_CODE);
			continue;
		}
		...
		if (ret < OK) {
			TS_LOG_ERR(
				"%sUnable to write Lockdown  head at %d iteration.. ERROR %08X\n",
				__func__, i, ret);
			ret = (ret | ERROR_LOCKDOWN_CODE);
			continue;
		}
		...
		if (ret < OK) {
			TS_LOG_ERR(
				"%s: Unable to write Lockdown  head at %d iteration.. ERROR %08X\n",
				__func__, i, ret);
			ret = (ret | ERROR_LOCKDOWN_CODE);
			continue;
		}
		...
		if (ret < OK) {
			...
			ret = pollForErrorType(error_to_search,
					       sizeof(error_to_search));
			if (ret < OK) {
				TS_LOG_ERR("%s : No Error Found! \n", __func__);
				ret = OK;
			} else {
				TS_LOG_ERR(
					"%s: have error when write lockdown ERROR = %02X\n",
					__func__, ret);
...
static int lock_down_info_check(u8 *data, u8 lock_id, int data_length)
{
	...
	for (i = index - 1; i >= 0; i--) {
		...
		if (lockindex >
		    (LOCKDOWN_LENGTH - FTS_DATA_LEN_SIZE - data_len))
			continue;
...
static int lock_down_info_init(u8 valueoriginal, u8 **data)
{
	...
	*data = kzalloc((LOCKDOWN_LENGTH * sizeof(u8)), GFP_KERNEL);
	...
	res = fts_writeReadU8UX(FTS_CMD_HW_REG_R, ADDR_SIZE_HW_REG,
				ADDR_INFO_LOCKDOWN, *data, LOCKDOWN_LENGTH,
				DUMMY_HW_REG);
	if (res < OK)
