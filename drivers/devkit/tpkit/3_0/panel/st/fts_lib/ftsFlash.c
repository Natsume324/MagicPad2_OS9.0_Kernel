...
int getFWdata(const char *pathToFile, u8 **data, int *size)
{
	...
	default:
		...
		if (fts_info != NULL && fts_info->i2c_cmd_dev != NULL) {
			res = request_firmware(&fw, path,
					       fts_info->i2c_cmd_dev);
			if (res == 0) {
				*size = fw->size;
...
int wait_for_flash_ready(u8 type)
{
	// clang-format off
	u8 cmd[5] = {
		FTS_CMD_HW_REG_R, 0x20, 0x00, 0x00, type
	};
	// clang-format on
	u8 readData[2] = {0};
	int i = 0, res = -1;
...
int parseBinFile(u8 *fw_data, int fw_size, Firmware *fwData, int keep_cx)
{
	...
	if ((fw_size < FW_HEADER_SIZE + FW_BYTES_ALLIGN) || !fw_data ||
	    !fwData) {
		TS_LOG_ERR(
			"%s parseBinFile: Read only %d instead of %d... ERROR %08X\n",
			__func__, fw_size, FW_HEADER_SIZE + FW_BYTES_ALLIGN,
			ERROR_FILE_PARSE);
		...
		if (fw_data[index] != DCHIP_ID_0 ||
			...
			TS_LOG_ERR(
				...
				__func__, fw_data[index], DCHIP_ID_0,
				fw_data[index + 1], DCHIP_ID_1,
				ERROR_FILE_PARSE);
			res = ERROR_FILE_PARSE;
			goto END;
...
int flash_enable_uvlo_autopowerdown(void)
{
	// clang-format off
	u8 cmd[6] = {
		FTS_CMD_HW_REG_W, 0x20, 0x00, 0x00,
		FLASH_UVLO_ENABLE_CODE0, FLASH_UVLO_ENABLE_CODE1
	};
...
int flash_erase_page_by_page(ErasePage keep_cx)
{
	...
	u8 cmd2[9] = {
		FTS_CMD_HW_REG_W, 0x20, 0x00, 0x01, 0x28,
		0xFF, 0xFF, 0xFF, 0xFF
	};
	// clang-format on
	u8 mask[4] = {0};
...
int start_flash_dma(void)
{
	...
	u8 cmd[ST_FLASH_DMA_LEN] = {
		FLASH_CMD_WRITE_REGISTER, 0x20, 0x00, 0x00, 0x6B,
		0x00, 0x40, 0x42, 0x0F, 0x00, 0x00, FLASH_DMA_CODE1
	};
	// clang-format on
...
int fillFlash(u32 address, u8 *data, int size)
{
	...
	while (remaining > 0) {
		...
		while (byteBlock < FLASH_CHUNK && remaining > 0) {
			...
			buff[index++] =
				(u8)((addr & FLASH_ADDR_1BYTE_8BIT) >> 24);
			buff[index++] =
				(u8)((addr & FLASH_ADDR_2BYTE_8BIT) >> 16);
			buff[index++] =
				(u8)((addr & FLASH_ADDR_3BYTE_8BIT) >> 8);
			buff[index++] = (u8)(addr & FLASH_ADDR_4BYTE_8BIT);
		...
		TS_LOG_INFO(
			...
			__func__, buff2[0], buff2[8], buff2[7], buff2[10],
			buff2[9]);
...
int flash_burn(Firmware fw, int force_burn, int keep_cx)
{
	int res = 0;
	if (!force_burn) {
		if (fw.fw_ver != systemInfo.u16_fwVer ||
		    fw.config_id != systemInfo.u16_cfgProjectId)
			goto start;
		TS_LOG_ERR(
