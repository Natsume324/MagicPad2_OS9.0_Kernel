...
static void st_set_ext_event_to_fingers(struct fts_ts_info *info, int wx,
					int wy, int xer, int yer, int ewx,
					int ewy, unsigned char touchId,
					unsigned char touchcount)
{
	...
	TS_LOG_DEBUG(
		"%s: wx:%d, wy:%d, xer:%d, yer:%d,ewx:%d, ewy:%d,touchId:%u, touchcount:%u\n",
...
static void fts_ext_pointer_event_handler(struct fts_ts_info *info,
					  unsigned char *event)
{
	...
	st_set_ext_event_to_fingers(info, wx, wy, xer, yer, ewx, ewy, touchId,
				    touchcount);
}

static void st_set_event_to_fingers(struct fts_ts_info *info, int x, int y,
				    int major, int minor, int pressure,
				    int status, unsigned char touchId,
				    unsigned char touchcount)
{
	struct ts_fingers *f_info = NULL;
	u32 max_fingers = fts_info->chip_data->ts_platform_data->max_fingers;
	...
	if (NOT_HANDLE_EVENT == f_info->fingers[touchId].pressure) {
		TS_LOG_DEBUG(
			"Not handle pointer event, has handled leave event\n");
	}
	...
	TS_LOG_DEBUG(
		"%s:cur_finger_number %d,touchcount %d,major %d,minor %d\n",
	...
	if ((event[0] == EVT_ID_USER_REPORT) &&
		...
		case GEST_ID_DOUBLE_TAP:
			...
			if (info->chip_data->easy_wakeup_info.off_motion_on ==
			    true) {
				info->chip_data->easy_wakeup_info.off_motion_on =
					false;
					    ...
					    unsigned char *event)
{
	 ...
	 */
	st_set_event_to_fingers(info, x, y, 1, 1, 1, status1, touchId,
				touchcount);
	if (st_roi_switch_on)
					    ...
					    unsigned char *event)
{
	...
	st_set_event_to_fingers(info, 0, 0, 0, 0, 0, status1, touchId,
				touchcount);
	return;
}
				     ...
				     unsigned char *event)
{
	...
	case EVT_TYPE_STATUS_ECHO:
		TS_LOG_ERR(
			"%s Echo event of command = %02X %02X %02X %02X %02X %02X\n",
	...
	case EVT_TYPE_STATUS_FORCE_CAL:
		...
		case EVT_FORCE_CAL_CONTINUOUS_DROP:
			TS_LOG_ERR(
				"%s Continuous frame drop Force cal = %02X %02X %02X %02X %02X %02X\n",
		...
		case EVT_FORCE_CAL_MUTUAL_NEGATIVE:
			TS_LOG_ERR(
				"%s Mutual negative detect Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_MUTUAL_CALIB:
			TS_LOG_ERR(
				"%s Mutual calib deviation Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_SS_NEGATIVE:
			TS_LOG_ERR(
				"%s SS negative detect Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_SS_NEGATIVE_LOW:
			TS_LOG_ERR(
				"%s SS negative detect Force cal in Low Power mode = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_SS_NEGATIVE_IDLE:
			TS_LOG_ERR(
				" %s SS negative detect Force cal in Idle mode = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_SS_INVALID_MUTUAL:
			TS_LOG_ERR(
				"%s SS invalid Mutual Strength soft Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_SS_INVALID_SELF_STRENGTH:
			TS_LOG_ERR(
				"%s SS invalid Self Strength soft Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_SS_INVALID_SELF_ISLAND:
			TS_LOG_ERR(
				"%s :SS invalid Self Island soft Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_MS_INVALID_MUTUAL:
			TS_LOG_ERR(
				"%s MS invalid Mutual Strength soft Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FORCE_CAL_MS_INVALID_SELF:
			TS_LOG_ERR(
				"%s MS invalid Self Strength soft Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		default:
			TS_LOG_ERR(
				"%s Force cal = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
		}
		break;
	...
	case EVT_TYPE_STATUS_FRAME_DROP:
		...
		case EVT_FRAME_DROP_NOISY:
			TS_LOG_ERR(
				"%s Frame drop noisy frame = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FRAME_DROP_BAD:
			TS_LOG_ERR(
				"%s Frame drop bad R = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		case EVT_FRAME_DROP_INVALID:
			TS_LOG_ERR(
				"%s Frame drop invalid processing state = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
			break;
		...
		default:
			TS_LOG_ERR(
				"%s Frame drop = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
		}
		break;
	...
	case EVT_TYPE_STATUS_SS_RAW_SAT:
		if (event[2] == 1) {
			TS_LOG_ERR(
				"%s SS Raw Saturated = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
		} else {
			TS_LOG_ERR(
				"%s SS Raw No more Saturated = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
		}
		break;
	...
	case EVT_TYPE_STATUS_WATER:
		if (event[2] == 1)
			TS_LOG_ERR(
				"%s Enter Water mode = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
		else
			TS_LOG_ERR(
				"%s Exit Water mode = %02X %02X %02X %02X %02X %02X\n",
				__func__, event[2], event[3], event[4],
				event[5], event[6], event[7]);
		break;
	...
	default:
		TS_LOG_ERR(
			"%s Received unhandled status event = %02X %02X %02X %02X %02X %02X %02X %02X\n",
 ...
 */
static void fts_event_handler(struct fts_ts_info *info, int *touch_event_count)
{
	int error = 0, count = 0;
	...
	if (error != OK) {
		TS_LOG_ERR(
			"%s:Error (%d) while reading from FIFO in fts_event_handler",
			__func__, error);
	} else {
...
static int fts_fw_update_auto(struct fts_ts_info *info, int force)
{
	...
	if (ret > OK) {
		...
		crc_status = 0;
		TS_LOG_ERR(
			"%s: NO CRC Error or Impossible to read CRC register!\n",
			__func__);
	}
	if (force == 1)
		crc_status = CRC_CODE;
	...
	if ((retval & 0xFF000000) == ERROR_FLASH_PROCEDURE) {
		TS_LOG_ERR(
			" %s: firmware update failed and retry! ERROR %08X\n",
			__func__, retval);
		...
		if ((retval1 & 0xFF000000) == ERROR_FLASH_PROCEDURE) {
			TS_LOG_ERR(
				"%s: firmware update failed again!  ERROR %08X\n",
				__func__, retval1);
		}
	}
...
static int st_resume(void)
{
	...
	case TS_POWER_OFF_MODE:
		if (!tskit_pt_station_flag) {
			gpio_set_value(
				st_info->chip_data->ts_platform_data->reset_gpio,
				0);
...
static void fts_gesture_mode_in(void)
{
	...
	if (res >= OK) {
		fromIDtoMask(FEAT_SEL_GESTURE, (u8 *)&info->mode,
			     sizeof(info->mode));
		MODE_LOW_POWER(info->mode, 0);
		TS_LOG_INFO("%s: enterGestureMode suscessful\n", __func__);
	} else {
		TS_LOG_ERR(
			"%s: enterGestureMode failed! ERROR %08X recovery in senseOff...\n",
			__func__, res);
	}
	return;
}
static int st_suspend(void)
{
	...
	case TS_POWER_OFF_MODE:
		...
		if (!tskit_pt_station_flag) {
			...
			retval |= fts_mode_handler(st_info,
						   0); /*goto sleep mode*/
		}
		break;
...
static void st_hw_reset(struct ts_kit_device_data *chip_data)
{
	...
	if (chip_data->ts_platform_data->reset_gpio > 0) {
		gpio_direction_output(chip_data->ts_platform_data->reset_gpio,
				      0);
...
int st_get_debug_data(struct ts_diff_data_info *info,
...
void st_ts_work_after_input_kit(void)
{
	...
	if (st_roi_switch_on) {
		...
		ret = fts_writeReadU8UX(FTS_CMD_FRAMEBUFFER_R, BITS_16,
					matrix_offset, matrix_data,
					MATRIX_DATA_NUM, DUMMY_FRAMEBUFFER);
		if (ret < OK)
...
static int st_chip_get_info(struct ts_chip_info_param *info)
{
	...
	snprintf(info->mod_vendor, sizeof(info->mod_vendor),
		 ts_info->project_id);
	snprintf(info->fw_vendor, sizeof(info->fw_vendor), "%x.%x",
		 systemInfo.u16_fwVer, systemInfo.u16_cfgProjectId);
...
static int st_irq_bottom_half(struct ts_cmd_node *in_cmd,
			      struct ts_cmd_node *out_cmd)
{
	/*
...
static void cap_test_parse_dts(struct device_node *device)
{
	...
	retval = of_property_read_u32(
		device, "support_self_calibrate_test",
...
static int st_parse_dts(struct device_node *device,
			struct ts_kit_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, ST_IRQ_CFG,
	...
	retval = of_property_read_u32(device, "i2c_address",
				      &chip_data->slave_addr);
	if (retval) {
	...
	retval = of_property_read_u32(
		device, "support_notice_aft_gesture_mode",
	...
	retval =
		of_property_read_u32(device, "support_extra_key_event_input",
	...
	TS_LOG_INFO(
		"irq_config=%d, algo_id=%d, x_max=%d, y_max=%d, x_mt=%d,y_mt=%d\n",
		chip_data->irq_config, chip_data->algo_id, chip_data->x_max,
...
static int st_init_chip(void)
{
	...
	info->i2c_cmd_dev =
		device_create(fts_cmd_class, NULL, DCHIP_ID_0, info, "fts_i2c");
...
static int st_get_testdata(struct ts_rawdata_info *info,
			   struct ts_cmd_node *out_cmd)
{
	int retval = 0;
	struct fts_ts_info *fts_info = fts_get_info();
	...
	retval |= fts_mode_handler(fts_info,
				   0); //enable the features and the sensing
	msleep(500);
...
static void st_set_fold_status(struct ts_fold_info info, int calibration)
{
	...
	if (!calibration) {
		...
		TS_LOG_INFO("%s:%x, %x, %x, %x\n", __func__, fold_status_cmd[0],
			    fold_status_cmd[1], fold_status_cmd[2],
			    fold_status_cmd[3]);
...
static int fts_set_charger_status(u8 charger_switch)
{
	...
	TS_LOG_INFO("%s:%x, %x, %x\n", __func__, charger_cmd[0], charger_cmd[1],
		    charger_cmd[2]);
