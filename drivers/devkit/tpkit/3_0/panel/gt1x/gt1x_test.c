...
#define SHORT_TO_GND_RESISTER(sig) \
	(div_s64(5266285, (sig) & (~0x8000)) - 40 * 100) /* 52662.85/code-40 */
#define SHORT_TO_VDD_RESISTER(sig, value)          \
	(div_s64((s64)(36864 * ((value)-9)) * 100, \
		 (((sig) & (~0x8000)) * 7)) -      \
	 40 * 100)
...
static int gt1x_init_testlimits(struct gt1x_ts_test *ts_test)
{
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_UNIFIED_LIMIT, data_buf, 1,
				   3);
	if (ret) {
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_NOISE_LIMIT, data_buf, 1,
				   1);
	if (ret) {
		TS_LOG_INFO("%s: Failed get %s \n", __func__,
			    CSV_TP_NOISE_LIMIT);
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SHORT_THRESHOLD, data_buf,
				   1, 7);
	if (ret) {
		TS_LOG_INFO("%s: Failed get %s \n", __func__,
...
static int gt1x_cache_origconfig(struct gt1x_ts_test *ts_test)
{
	...
	if (ts_test->ts->ops.i2c_read) {
		...
		if (ts_test->orig_config.data[EXTERN_CFG_OFFSET] & 0x40) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"Failed to read external config data\n");
				return ret;
			}
		...
		strncpy(ts_test->orig_config.name, "original_config",
			MAX_STR_LEN);
...
static int gt1x_tptest_prepare(struct gt1x_ts_test *ts_test)
{
	...
	ret = gt1x_get_channel_num(&ts_test->test_params.sen_num,
	...
	if (ts_test->ts->ops.send_cfg) {
		...
		ret = ts_test->ts->ops.send_cfg(
			&ts_test->ts->noise_test_config);
		if (ret) {
			ts_test->ts->noise_test_config.data[6] &= 0x7f;
			TS_LOG_ERR(
				"Failed to send noise test config config:%d\n",
				ret);
			return ret;
		}
		ts_test->ts->noise_test_config.data[6] &= 0x7f;
...
static int gt1x_cache_rawdata(struct gt1x_ts_test *ts_test)
{
	...
	for (j = 0; j < GT1X_RETRY_NUM - 1; j++) {
		ret = ts_test->ts->ops.send_cmd(GTP_CMD_RAWDATA, 0x00,
						GT1X_NEED_SLEEP);
		if (ret) {
		...
		for (retry = 0; retry < GT1X_RETRY_NUM; retry++) {
			ret = ts_test->ts->ops.i2c_read(GTP_READ_COOR_ADDR,
							&buf[0], 1);
			...
			TS_LOG_INFO(
				"Rawdata in not ready:ret=%d, buf[0]=0x%x\n",
				ret, buf[0]);
			msleep(15);
		}
		...
		if (retry < GT1X_RETRY_NUM) {
			...
			TS_LOG_ERR(
				"%s : Read rawdata timeout, retry[%d] send command\n",
				__func__, j);
			ret = -EFAULT;
		}
	}
...
static void gt1x_test_noisedata(struct gt1x_ts_test *ts_test)
{
	...
	data_size = ts_test->test_params.sen_num * ts_test->test_params.drv_num;
	...
	for (test_cnt = 0; test_cnt < RAWDATA_TEST_TIMES; test_cnt++) {
		for (i = 0; i < GT1X_RETRY_NUM; i++) {
			...
			if ((ret == 0) && (buf[0] & 0x80))
				...
				TS_LOG_ERR(
					"%s: failed read noise data status\n",
					__func__);
				goto soft_err_out;
			}
		...
		if (i >= GT1X_RETRY_NUM) {
			fail_cnt++;
			TS_LOG_INFO(
				"%s: wait for noise data timeout, retry[%d] send command\n",
				__func__, fail_cnt);
			ret = ts_test->ts->ops.send_cmd(GTP_CMD_RAWDATA, 0x00,
							GT1X_NEED_SLEEP);
			if (ret) {
				TS_LOG_ERR(
					"%s: Failed send rawdata command, fail_cnt[%d]\n",
					__func__, fail_cnt);
				goto soft_err_out;
			}
			continue;
		...
		/* read noise data */
		ret = ts_test->ts->ops.i2c_read(noisedata_addr, data_buf,
						data_size);
		if (!ret) {
			/* check noise data */
			find_bad_node = 0;
			...
			if (find_bad_node) {
				TS_LOG_INFO(
					"%s:noise test find bad node, test times=%d\n",
					__func__, test_cnt);
				fail_cnt++;
			}
		} else {
		...
		/* clear data ready flag */
		buf[0] = 0x00;
		ret = ts_test->ts->ops.i2c_write(GTP_READ_COOR_ADDR, &buf[0],
						 1);
		if (ret) {
			TS_LOG_ERR(
				"%s:failed clear noise data ready register\n",
				__func__);
			goto soft_err_out;
		}
	}
...
static void gt1x_capacitance_test(struct gt1x_ts_test *ts_test)
{
	...
	ret = gt1x_rawcapacitance_test(&ts_test->rawdata,
				       &ts_test->test_params);
	if (!ret) {
	...
	ret = gt1x_deltacapacitance_test(&ts_test->rawdata,
					 &ts_test->test_params);
	if (!ret) {
...
static void gt1x_put_test_result(struct ts_rawdata_info *info,
...
gt1x_put_test_failed_prepare_newformat(struct ts_rawdata_info_new *info)
{
	...
	ret = gt1x_strncat(info->i2cerrinfo, "software reason",
...
int gt1x_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	if (!gts_test) {
		...
		memset(&(gts_test->noisedata), 0,
		       sizeof(struct ts_test_rawdata));
	}
	...
	if (ret) {
		TS_LOG_ERR("%s: Failed parse test peremeters, exit test\n",
			   __func__);
		if (gt1x_ts->dev_data->ts_platform_data->chip_data
 ...
 */
static void gt1x_data_statistics(u16 *data, size_t data_size, char *result,
				 size_t res_size)
{
	u16 i = 0;
	u16 avg = 0;
	...
	if (!data || !result) {
		TS_LOG_ERR(
			"parameters error please check *data and *result value\n");
		return;
	}

	if (data_size <= 0 || res_size <= 0) {
		TS_LOG_ERR(
			"input parameter is illegva:data_size=%ld, res_size=%ld\n",
			data_size, res_size);
		return;
	}
...
static void gt1x_put_test_result_newformat(struct ts_rawdata_info_new *info,
					   struct gt1x_ts_test *ts_test)
{
	int i = 0;
	...
	if (ts_test->rawdata.size) {
		pts_node->values = kzalloc(ts_test->rawdata.size * sizeof(int),
					   GFP_KERNEL);
		if (!pts_node->values) {
		...
		gt1x_data_statistics(&ts_test->rawdata.data[0],
	...
	if (ts_test->noisedata.size) {
		pts_node->values = kzalloc(
			ts_test->noisedata.size * sizeof(int), GFP_KERNEL);
		if (!pts_node->values) {
			TS_LOG_ERR("malloc failed  for values\n");
			kfree(pts_node);
			pts_node = NULL;
		...
		/* calculate rawdata min avg max vale*/
		gt1x_data_statistics(&ts_test->noisedata.data[0],
	...
	gt1x_strncat(info->deviceinfo, ts_test->ts->hw_info.product_id,
		     sizeof(info->deviceinfo) - strlen(info->deviceinfo));
	gt1x_strncat(info->deviceinfo, "-",
		     sizeof(info->deviceinfo) - strlen(info->deviceinfo));
	gt1x_strncatint(info->deviceinfo,
			ts_test->ts->dev_data->ts_platform_data->panel_id,
			"%d;",
			sizeof(info->deviceinfo) - strlen(info->deviceinfo));
				 ...
				 struct gt1x_ts_test *ts_test)
{
	...
	if (gt1x_ts->dev_data->ts_platform_data->chip_data
		    ->rawdata_newformatflag == TS_RAWDATA_NEWFORMAT) {
		gt1x_put_test_result_newformat(
			(struct ts_rawdata_info_new *)info, ts_test);
	} else {
...
static int gt1x_short_test_prepare(struct gt1x_ts_test *ts_test)
{
	...
	while (--retry) {
		...
		ret = ts_test->ts->ops.send_cmd(
			0x42, 0x0, GT1X_NEED_SLEEP); /*bagan test command*/
		if (ret) {
	...
	ret = ts_test->ts->ops.i2c_write(SHORT_THRESHOLD_REG, data,
	...
	chksum = 0 - chksum;
	ret = ts_test->ts->ops.i2c_write(DRV_CONFIG_REG, &data[MAX_DRV_NUM],
					 MAX_DRV_NUM);
	if (ret) {
		TS_LOG_ERR("Failed writ tp driver config\n");
		return ret;
	}
	ret = ts_test->ts->ops.i2c_write(SENSE_CONFIG_REG, &data[0],
					 MAX_SEN_NUM);
	if (ret) {
	...
	ret = ts_test->ts->ops.i2c_write(DRVSEN_CHECKSUM_REG, &chksum,
					 1); /* DRVSEN_CHECKSUM_REG   0x884E */
	if (ret) {
	...
	data[0] = 0x01;
	ret = ts_test->ts->ops.i2c_write(CONFIG_REFRESH_REG, data,
					 1); /* CONFIG_REFRESH_REG   0x813E  */
	if (ret) {
	...
	/* clr 5095, runing dsp */
	data[0] = 0x00;
	ret = ts_test->ts->ops.i2c_write(SHORT_STATUS_REG, data,
					...
					u16 adc_signal, u8 pos)
{
	...
	if (r < r_th) {
		if ((adc_signal & (0x8000)) == 0) {
			if (pos < MAX_DRV_NUM)
				TS_LOG_ERR(
					"Tx%d shortcircut to GND,R=%ldK,R_Threshold=%dK\n",
					pos, r, r_th);
			else
				TS_LOG_ERR(
					"Rx%d shortcircut to GND,R=%ldK,R_Threshold=%dK\n",
					pos - MAX_DRV_NUM, r, r_th);
		} else {
			if (pos < MAX_DRV_NUM)
				TS_LOG_ERR(
					"Tx%d shortcircut to VDD,R=%ldK,R_Threshold=%dK\n",
					pos, r, r_th);
			else
				TS_LOG_ERR(
					"Rx%d shortcircut to VDD,R=%ldK,R_Threshold=%dK\n",
					pos - MAX_DRV_NUM, r, r_th);
		}
		return RESULT_ERR;
	}
...
static int gt1x_shortcircut_analysis(struct gt1x_ts_test *ts_test)
{
	...
	ret = ts_test->ts->ops.i2c_read(TEST_RESTLT_REG, &short_flag,
					1); /* TEST_RESTLT_REG  0x8801 */
	if (ret < 0) {
	...
	if (short_flag & 0x08) {
		...
		for (i = 0; i < size; i += 2) {
			...
			ret = gt1x_check_resistance_to_gnd(
				&ts_test->test_params, adc_signal,
				i >> 1); /* i >> 1 = i / 2 */
			if (ret) {
	...
	/* read self-capdata+ */
	size = (MAX_DRV_NUM + MAX_SEN_NUM) * 2;
	ret = ts_test->ts->ops.i2c_read(DRV_SELF_CODE_REG, data_buf,
					size); /* DRV_SELF_CODE_REG   0xA2A0 */
	if (ret) {
		TS_LOG_ERR("Failed read selfcap rawdata\n");
		goto shortcircut_analysis_error;
	}
	for (i = 0; i < MAX_DRV_NUM + MAX_SEN_NUM; i++)
		self_capdata[i] = be16_to_cpup((__be16 *)&data_buf[i * 2]) &
				  0x7fff;
	/* read self-capdata- */

	/* read tx tx short number */
	ret = ts_test->ts->ops.i2c_read(TX_SHORT_NUM, &short_status[0],
					3); /* TX_SHORT_NUM   0x8802 */
	if (ret) {
	...
	*/
	...
	for (i = 0; i < short_status[1]; i++) {
		...
		short_pin_num =
			be16_to_cpup((__be16 *)&data_buf[0]) + MAX_DRV_NUM;
		if (short_pin_num > (MAX_DRV_NUM + MAX_SEN_NUM - 1))
			continue;
	...
	for (i = 0; i < short_status[2]; i++) {
		size = SHORT_CAL_SIZE(
			MAX_SEN_NUM); //size = 4 + MAX_SEN_NUM * 2 + 2;
		ret = ts_test->ts->ops.i2c_read(data_addr, data_buf, size);
		if (ret) {
...
static void gt1x_shortcircut_test(struct gt1x_ts_test *ts_test)
{
	...
	for (i = 0; i < GT1X_RETRY_FIVE; i++) {
		...
		ret = ts_test->ts->ops.i2c_read(
			TEST_SHORT_STATUS_REG, data,
			1); /* Test_SHORT_STATUS_REG   0x8800 */
		if (ret)
