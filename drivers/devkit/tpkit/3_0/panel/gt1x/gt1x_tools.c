...
int gt1x_init_tool_node(void)
{
	...
	gt1x_tool_proc_entry =
		proc_create(procname, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH,
			    NULL, &gt1x_tool_fops);
...
static char comfirm(void)
{
	...
	memcpy(buf, cmd_head.flag_addr,
	       (cmd_head.addr_len > sizeof(buf)) ? sizeof(buf) :
						   cmd_head.addr_len);
			       ...
			       size_t len, loff_t *data)
{
	...
	if (WRITE_REG_MODE == cmd_head.wr) {
		...
		data_len = cmd_head.data_len > (sizeof(IC_TYPE) - 1) ?
				   (sizeof(IC_TYPE) - 1) :
				   cmd_head.data_len;
		...
		data_len = cmd_head.data_len > DATA_LENGTH ? DATA_LENGTH :
							     cmd_head.data_len;
		ret = copy_from_user(&cmd_head.data[GTP_ADDR_LENGTH],
				     &buff[CMD_HEAD_LENGTH], data_len);
		if (ret) {
			TS_LOG_ERR("copy_from_user failed.\n");
			return RESULT_ERR;
		}
		...
		if (copy_from_user(cmd_head.data, &buff[CMD_HEAD_LENGTH],
				   data_len)) {
			TS_LOG_ERR("%s:copy to user error\n", __func__);
		}
		memset(gt1x_ts->firmware_name, 0, GT1X_FW_NAME_LEN);
		snprintf(gt1x_ts->firmware_name, GT1X_FW_NAME_LEN, "%s",
			 cmd_head.data);
		TS_LOG_INFO("%s: fw_name=%s\n", __func__,
			    gt1x_ts->firmware_name);
			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (cmd_head.wr % 2) {
		...
		*ppos += 4;
		...
		if (copy_to_user(buffer, GTP_DRIVER_VERSION,
				 sizeof(GTP_DRIVER_VERSION))) {
			TS_LOG_ERR("%s:copy to user error\n", __func__);
		}
		*ppos += tmp_len + 1;
		return (tmp_len + 1);
