...
struct fw_update_info update_info = {.status = UPDATE_STATUS_IDLE,
				     .progress = 0,
				     .max_progress = 9,
				     .force_update = 0};
...
int gt1x_update_firmware(void)
{
	...
	subsystem_info =
		(u8 *)(&update_info.fw
				->data[update_info.firmware_info->subsystem[0]
					       .offset]);
	...
	ret = gt1x_run_ss51_isp(subsystem_info,
	...
	for (i = 1; i < update_info.firmware_info->subsystem_count; i++) {
		...
		ret = gt1x_burn_subsystem(
			&(update_info.firmware_info->subsystem[i]));
		if (ret) {
	...
	subsystem_info =
		&update_info.fw
			 ->data[update_info.firmware_info->subsystem[0].offset];
	if (subsystem_info == NULL) {
		TS_LOG_ERR("%s:get isp fail\n", __func__);
		ret = ERROR_FW;
		goto update_exit;
...
static int gt1x_update_prepare(void)
{
	...
	if (ret < 0) {
		TS_LOG_ERR("%s:Request firmware failed - %s (%d)\n", __func__,
			   gt1x_ts->firmware_name, ret);
		return ERROR_FW;
	}
	TS_LOG_INFO("Firmware size: %zd\n", update_info.fw->size);

	for (retry = 0; retry < GT1X_RETRY_NUM; retry++) {
		update_info.firmware_info = (struct fw_info *)kzalloc(
			sizeof(struct fw_info), GFP_KERNEL);
	...
	for (retry = 0; retry < GT1X_RETRY_NUM; retry++) {
		update_info.buffer =
			(u8 *)kzalloc(1024 * 4, GFP_KERNEL); /* every burn 4k*/
...
static int gt1x_check_firmware(void)
{
	...
	if (getU32(update_info.fw->data) + FW_START_LOCAL !=
	    update_info.fw->size) {
		TS_LOG_ERR(
	...
	// parse firmware
	memcpy((u8 *)update_info.firmware_info, &update_info.fw->data[0],
	       FW_INFO_LOCAL);
	update_info.firmware_info->pid[PATCH_MAIN_VERSION] = 0;

	subsystem_info =
		(u8 *)(&update_info.fw
				->data[FW_HEAD_OFFSET_SUBSYSTEM_INFO_BASE]);
	...
	for (i = 0; i < firmware_tmpinfo->subsystem_count; i++) {
		...
		firmware_tmpinfo->subsystem[i].length = getU16(
			&subsystem_info[i * FW_HEAD_SUBSYSTEM_INFO_SIZE + 1]);
		firmware_tmpinfo->subsystem[i].address =
			getU16(&subsystem_info[i * FW_HEAD_SUBSYSTEM_INFO_SIZE +
					       3]) *
			256;
	...
	TS_LOG_INFO("Firmware patch: %02X%02X%02X\n",
		    update_info.firmware_info->version[0],
		    update_info.firmware_info->version[1],
...
static int gt1x_update_judge(void)
{
	...
	fw_ver_info.mask_id =
		(update_info.firmware_info->target_mask_version[0] << 16) |
	...
	fw_ver_info.patch_id = (update_info.firmware_info->version[0] << 16) |
			       ...
			       (update_info.firmware_info->version[2]);
	memcpy(fw_ver_info.product_id, update_info.firmware_info->pid,
	       GT1X_PRODUCT_ID_LEN);
	fw_ver_info.product_id[PID_RESERVE_BIT] = 0;

	/* check fw status reg */
	do {
		ret = gt1x_i2c_read_dbl_check(GTP_REG_FW_CHK_MAINSYS, reg_val,
					      1);
...
_reset:
	...
	if (memcmp(fw_ver_info.product_id, ver_info.product_id,
		   GT1X_PRODUCT_ID_LEN)) {
	...
	if ((fw_ver_info.patch_id & 0xFF0000) !=
	    (ver_info.patch_id & 0xFF0000)) {
...
static int gt1x_write_block_data(u32 addr, u8 *buffer, int len)
{
	...
	for (i = 0; i < package_count; i++) {
		...
		if ((i + 1 == package_count) &&
		    (len % MTK_IIC_MAX_TRANSFER_SIZE != 0)) {
		...
		ret = gt1x_i2c_write(start_write_addr, &buffer[copy_offset],
				     data_size);
		if (ret) {
...
static int gt1x_check_subsystem_in_flash(struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0) {
		...
		fw = (u8 *)(&update_info.fw
				     ->data[subsystem->offset + checked_len]);
...
static int gt1x_error_erase(void)
{
	...
	fw = (u8 *)(&update_info.fw->data[update_info.firmware_info->subsystem[0]
						  .offset]);
	if (fw == NULL) {
		TS_LOG_ERR("get isp fail\n");
		return ERROR_FW;
	}
	ret = gt1x_run_ss51_isp(fw,
				update_info.firmware_info->subsystem[0].length);
	if (ret) {
