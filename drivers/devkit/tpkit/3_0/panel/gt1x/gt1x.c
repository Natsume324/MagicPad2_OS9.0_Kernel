...
int gt1x_i2c_read(u16 addr, u8 *buffer, s32 len)
{
	...
	ret = dev_data->ts_platform_data->bops->bus_read((u8 *)&addr, 2, buffer,
							 len);
	if (ret < 0)
...
int gt1x_i2c_read_dbl_check(u16 addr, u8 *buffer, s32 len)
{
	...
	if (len > 16) {
		TS_LOG_ERR(
			"%s:i2c_read_dbl_check length %d is too long, exceed %zu\n",
...
static int gt1x_send_cfg(struct gt1x_ts_config *cfg_ptr)
{
	...
	if (config[EXTERN_CFG_OFFSET] & 0x40) {
		...
		if (!checksum) {
			TS_LOG_ERR(
				"%s:Invalid ext config,all of the bytes is zero\n",
				__func__);
	...
	while (retry++ < GT1X_RETRY_NUM) {
		...
		if (!ret) {
			if (config[6] &
			    0x80) /* judge weather store the config to Flash */
				msleep(SEND_CFG_RAM);
			else
...
static inline int gt1x_init_watchdog(void)
{
	...
	for (i = 0; i < GT1X_RETRY_NUM; i++) {
		ret = gt1x_i2c_write(GTP_REG_CMD + INDEX_1, &value[INDEX_1],
				     (I2C_WD_DATA_LENGTH - I2C_ENABLE_BIT));
		if (ret < NO_ERR) {
			TS_LOG_ERR(
				"%s:Init watchdog error ,i2c communicate error , retry  = [%d] \n",
				__func__, i);
			msleep(I2C_RETRY_SLEEP_TIME);
		} else {
			TS_LOG_INFO(
				"%s, Init watchdog success ,i2c communicate check success\n",
				__func__);
			return gt1x_i2c_write(GTP_REG_CMD, &value[INDEX_0],
					      I2C_ENABLE_BIT);
		}
	}
			       ...
			       enum gt1x_ts_feature fea, int on)
{
	...
	struct ts_feature_info *info =
		&ts->dev_data->ts_platform_data->feature_info;
...
static int gt1x_feature_resume(struct gt1x_ts_data *ts)
{
	...
	TS_LOG_INFO(
		"holster_supported:%d,holster_switch_staus:%d,glove_supported:%d,"
	...
	if (info->charger_info.charger_supported &&
	    info->charger_info.charger_switch) {
		ret = gt1x_send_cmd(GTP_CMD_CHARGER_ON, 0x00,
				    GT1X_NOT_NEED_SLEEP);
		if (ret)
...
static int gt1x_get_standard_matrix(struct roi_matrix *src_mat,
...
static int gt1x_cache_roidata_device(struct gt1x_ts_roi *roi)
{
	...
	/* read ROI rawdata */
	ret = gt1x_i2c_read(
		(ts->gt1x_roi_data_add + ROI_HEAD_LEN), (char *)src_mat.data,
...
static int gt1x_cache_roidata_fw(struct gt1x_ts_roi *roi)
{
	...
	if (unlikely((u8)checksum != 0)) { /* cast to 8bit checksum,*/
		TS_LOG_ERR(
			"%s:roi status checksum error{%02x %02x %02x %02x}\n",
	...
	ret = gt1x_i2c_read(ROI_DATA_REG, (u8 *)(roi->rawdata) + ROI_HEAD_LEN,
			    roi_bytes);
	if (unlikely(ret < 0)) {
		mutex_unlock(&roi->mutex);
		TS_LOG_ERR("%s:i2c_read failed!\n", __func__);
...
static int gt1x_request_evt_handler(struct gt1x_ts_data *ts)
{
	...
	case GTP_RQST_IDLE:
		TS_LOG_INFO(
			"[%s] Warning : An empty request, without any processing!!\n",
			__func__);
		break;
	default:
		TS_LOG_INFO(
			"[%s] Warning : An undefined request[0x%02x] , without any processing!!\n",
			__func__, rqst_data);
		break;
	}
				  ...
				  struct ts_fingers *info)
{
	...
	if ((ts->gt1x_edge_add != 0) && (touch_num > 0)) {
		ret = ts->ops.i2c_read(ts->gt1x_edge_add, touch_ewxy,
				       GT1X_EDGE_DATA_SIZE);
		if (ret) {
			goto exit;
		}
	}
	if ((touch_num > 0) && (0 != ts->gt1x_support_wxy) &&
	    (0 != ts->gt1x_wxy_data_add)) {
		ret = ts->ops.i2c_read(ts->gt1x_wxy_data_add, touch_wxy,
				       GT1X_EDGE_DATA_SIZE);
		if (ret) {
			goto exit;
		}
	...
	for (i = 0; i < touch_num; i++) {
		...
		if (ts->gt1x_edge_support_xyer) {
			w = coor_data[i * GT1X_EDGE_POINT_DATA_SIZE +
				      GT1X_ADDR_EDGE_W];
			xer = coor_data[i * GT1X_EDGE_POINT_DATA_SIZE +
					GT1X_ADDR_EDGE_XER];
			yer = coor_data[i * GT1X_EDGE_POINT_DATA_SIZE +
					GT1X_ADDR_EDGE_YER];
		...
		cur_index |= 1 << id;
		TS_LOG_DEBUG(
			"%s:[%d](ewx = %d, ewy =  %d, xer=%d, yer=%d, wx=%d, wy=%d)\n",
			__func__, id, info->fingers[id].ewx,
			info->fingers[id].ewy, xer, yer, touch_wxy[2 * id],
			touch_wxy[2 * id + 1]);
	}
...
static void gt1x_set_gesture_key(struct ts_fingers *info,
...
static void gt1x_double_tap_event(struct gt1x_ts_data *ts,
				  struct ts_fingers *info,
				  struct ts_cmd_node *out_cmd, u8 num)
{
	...
	ret = gt1x_i2c_read(reg_add, buf,
			    GT1X_DOUBLE_TAB_POINT_NUM *
				    GT1X_GESTURE_ONE_POINT_BYTES);
	if (ret < 0) {
	...
	for (i = 0; i < GT1X_DOUBLE_TAB_POINT_NUM; i++) {
		...
		*/
		...
		ts->dev_data->ts_platform_data->chip_data->easy_wakeup_info
				    ...
				    struct ts_cmd_node *out_cmd)
{
	...
	/** package: -head 4B + track points + extra info- 
		...
		*/
	ret = gt1x_i2c_read(GTP_REG_WAKEUP_GESTURE, buf,
			    GT1X_GESTURE_PACKAGE_LEN);
	if (ret < 0) {
	...
	TS_LOG_DEBUG("0x%x = 0x%02X,0x%02X,0x%02X,0x%02X\n",
		     GTP_REG_WAKEUP_GESTURE, buf[0], buf[1], buf[2], buf[3]);
	gesture_type = buf[0];
	points_num = buf[1];
	...
	case GT1X_DOUBLE_TAB_TYPE: //double tap
		if (ts->dev_data->easy_wakeup_info.easy_wakeup_gesture &
		    GT1X_DOUBLE_TAB_FLAG_BIT) {
...
static int gt1x_pinctrl_init(void)
{
	...
	ts->pinctrl_state_reset_high =
		pinctrl_lookup_state(ts->pinctrl, PINCTRL_STATE_RESET_HIGH);
	...
	ts->pinctrl_state_reset_low =
		pinctrl_lookup_state(ts->pinctrl, PINCTRL_STATE_RESET_LOW);
	...
	ts->pinctrl_state_as_int =
		pinctrl_lookup_state(ts->pinctrl, PINCTRL_STATE_AS_INT);
	...
	ts->pinctrl_state_int_high =
		pinctrl_lookup_state(ts->pinctrl, PINCTRL_STATE_INT_HIGH);
	...
	ts->pinctrl_state_int_low =
		pinctrl_lookup_state(ts->pinctrl, PINCTRL_STATE_INT_LOW);
...
static unsigned int gt1x_get_chip_id(u8 *chip_name)
{
	...
	if (!memcmp(chip_name, GT1X_1151Q_CHIP_ID,
		    GT1X_PRODUCT_ID_LEN)) { /* GT1151Q */
		chip_id = GT_1151Q_ID;
	} else {
...
static void gt1x_creat_project_id(void)
{
	...
	if (ts->create_project_id_supported && !ts->fw_damage_flag) {
		...
		snprintf(ts->project_id, MAX_STR_LEN, "%s%02d%02d0",
			 ts->dev_data->ts_platform_data->product_name, chip_id,
			 ts->dev_data->ts_platform_data->panel_id);
	} else {
		TS_LOG_INFO(
			"%s: no support create_project_id, use default project_id\n",
			__func__);
	}
...
int gt1x_read_version(struct gt1x_hw_info *hw_info)
{
	...
	while (retry--) {
		...
		if (!ret) {
			...
			if (checksum ==
				...
				if (checksum == (u8)(buf[11] * 2) &&
				    buf[10] !=
					    0xFF) /* checksum calculated by boot code */
					break;
			}
		}
	...
	TS_LOG_INFO("%s :IC Version:GT%s_%06X(FW)_%04X(Boot)_%02X(SensorID)\n",
		    __func__, hw_info->product_id, patch_id, mask_id >> 8,
		    sensor_id);

	gt1x_ts->sensor_id_valid = true;

	snprintf(ts->dev_data->version_name, MAX_STR_LEN - 1, "0x%04X",
		 patch_id);
...
static int gt1x_parse_dts(void)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"%s:fw_only_depend_on_lcd read failed, Use default value: 0\n",
			__func__);
		value = 0;
	}
	ts->fw_only_depend_on_lcd = value;
	TS_LOG_INFO("%s: fw_only_depend_on_lcd = %d  \n", __func__,
		    ts->fw_only_depend_on_lcd);
	...
	if (ts->gt1x_support_wxy) {
		...
		if (ret) {
			TS_LOG_ERR(
				"%s:gt1x_wxy_data_add read failed, Use default value: 0\n",
				__func__);
			value = 0;
		}
		ts->gt1x_wxy_data_add = value;
		TS_LOG_INFO("%s:gt1x_wxy_data_add value: [%x]\n", __func__,
			    ts->gt1x_wxy_data_add);
	}
	...
	TS_LOG_INFO("%s:create_project_id_supported value: [%d]\n", __func__,
		    ts->create_project_id_supported);
	...
	if (ret) {
		value = 0;
		TS_LOG_INFO(
			"%s: only_open_once_captest_threshold use default value\n",
			__func__);
...
static int gt1x_parse_specific_dts(void)
{
	...
	snprintf(project_id, sizeof(project_id), "gt1x-sensorid-%u",
	...
	TS_LOG_INFO(
		"%s: x_max_mt=%d,y_max_mt=%d,panel_id=%d,vendor_name=%s,project_id=%s\n",
		__func__, ts->max_x, ts->max_y,
		ts->dev_data->ts_platform_data->panel_id,
...
static int gt1x_parse_cfg_data(const struct firmware *cfg_bin, char *cfg_type,
			       u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (getU32(cfg_bin->data) + BIN_CFG_START_LOCAL != cfg_bin->size) {
		TS_LOG_ERR(
			"%s:Bad firmware!(file length: %d, header define: %d)\n",
	...
	if (!strncmp(cfg_type, GT1X_TEST_CONFIG, strlen(GT1X_TEST_CONFIG)))
		config_status = 0;
	else if (!strncmp(cfg_type, GT1X_NORMAL_CONFIG,
			  strlen(GT1X_NORMAL_CONFIG)))
		config_status = 1;
	else if (!strncmp(cfg_type, GT1X_NORMAL_NOISE_CONFIG,
			  strlen(GT1X_NORMAL_NOISE_CONFIG)))
		config_status = 2;
	else if (!strncmp(cfg_type, GT1X_GLOVE_CONFIG,
			  strlen(GT1X_GLOVE_CONFIG)))
		config_status = 3;
	else if (!strncmp(cfg_type, GT1X_GLOVE_NOISE_CONFIG,
			  strlen(GT1X_GLOVE_NOISE_CONFIG)))
		config_status = 4;
	else if (!strncmp(cfg_type, GT1X_HOLSTER_CONFIG,
			  strlen(GT1X_HOLSTER_CONFIG)))
		config_status = 5;
	else if (!strncmp(cfg_type, GT1X_HOLSTER_NOISE_CONFIG,
			  strlen(GT1X_HOLSTER_NOISE_CONFIG)))
		config_status = 6;
	else if (!strncmp(cfg_type, GT1X_NOISE_TEST_CONFIG,
			  strlen(GT1X_NOISE_TEST_CONFIG)))
		config_status = 7;
	else if (!strncmp(cfg_type, GT1X_GAME_SCENE_CONFIG,
			  strlen(GT1X_GAME_SCENE_CONFIG)))
		config_status = 8;
	else {
	...
	for (i = 0; i < bin_group_num * bin_cfg_num; i++) {
		if (sid ==
		    (cfg_bin->data[CFG_HEAD_BYTES +
				   i * CFG_INFO_BLOCK_BYTES])) //find cfg's sid in cfg.bin
		{
			...
			if (config_status ==
				...
				one_cfg_count = getU16(
					&cfg_bin->data[CFG_HEAD_BYTES + 2 +
						       i * CFG_INFO_BLOCK_BYTES]);
				if (one_cfg_count == CFG_SIZE_MAX ||
				    one_cfg_count == CFG_SIZE_MIN) {
					memcpy(cfg,
					       &cfg_bin->data
							[CFG_HEAD_BYTES +
							 bin_group_num *
 ...
 */
static int gt1x_get_cfg_data(const struct firmware *cfg_bin, char *config_name,
			     struct gt1x_ts_config *config)
{
	struct gt1x_ts_data *ts = gt1x_ts;
	...
	/* parse config data */
	ret = gt1x_parse_cfg_data(cfg_bin, config_name, cfg_data, &cfg_len,
				  ts->hw_info.sensor_id);
	if (ret < 0) {
...
static int gt1x_get_cfg_parms(struct gt1x_ts_data *ts, const char *filename)
{
	...
	ret = gt1x_get_cfg_data(cfg_bin, GT1X_NORMAL_CONFIG,
				&ts->normal_config);
	if (ret < 0) {
	...
	ret = gt1x_get_cfg_data(cfg_bin, GT1X_NORMAL_NOISE_CONFIG,
				&ts->normal_noise_config);
	if (ret < 0) {
	...
	ret = gt1x_get_cfg_data(cfg_bin, GT1X_NOISE_TEST_CONFIG,
				&ts->noise_test_config);
	if (ret < 0) {
		memcpy(&(ts->noise_test_config), &(ts->normal_config),
		       sizeof(ts->noise_test_config));
		TS_LOG_ERR(
	...
	if (ts->dev_data->ts_platform_data->feature_info.glove_info
		    .glove_supported) {
		ret = gt1x_get_cfg_data(cfg_bin, GT1X_GLOVE_CONFIG,
					&ts->glove_config);
		if (ret < 0) {
		...
		ret = gt1x_get_cfg_data(cfg_bin, GT1X_GLOVE_NOISE_CONFIG,
					&ts->glove_noise_config);
		if (ret < 0) {
			TS_LOG_ERR(
	...
	if (ts->dev_data->ts_platform_data->feature_info.holster_info
		    .holster_supported) {
		ret = gt1x_get_cfg_data(cfg_bin, GT1X_HOLSTER_CONFIG,
					&ts->holster_config);
		if (ret < 0) {
			TS_LOG_ERR(
		...
		ret = gt1x_get_cfg_data(cfg_bin, GT1X_HOLSTER_NOISE_CONFIG,
					&ts->holster_noise_config);
		if (ret < 0) {
			TS_LOG_ERR(
	...
	if (TS_SWITCH_TYPE_GAME ==
	    (ts->dev_data->touch_switch_flag & TS_SWITCH_TYPE_GAME)) {
		ret = gt1x_get_cfg_data(cfg_bin, GT1X_GAME_SCENE_CONFIG,
					&ts->game_scene_config);
		if (ret < 0) {
...
static int gt1x_init_configs(struct gt1x_ts_data *ts)
{
	...
	if (ts->fw_only_depend_on_lcd) { //TP fw only depend on LCD module
		...
		if (ret) {
			...
			if (!ret) {
				strncat(filename, "_",
					GT1X_FW_NAME_LEN - strlen(filename));
				strncat(filename, ts->lcd_module_name,
					GT1X_FW_NAME_LEN - strlen(filename));
			}
		}
	}
...
static int gt1x_chip_detect(struct ts_kit_platform_data *pdata)
{
	...
	if (!pdata) {
		...
		TS_LOG_ERR(
			"%s device, ts_kit_platform_data data->ts_dev is NULL \n",
			__func__);
		ret = -ENOMEM;
		goto exit;
	}
...
static int gt1x_chip_init(void)
{
	...
	if (ret ||
	    reg_val[0] == FW_IS_RUNING) { /*Check the firmware running status*/
		TS_LOG_ERR("%s : Check main system not pass[0x%2X]\n", __func__,
			   reg_val[0]);
	}
...
static int gt1x_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
			     gt1x_ts->dev_data->x_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
			     gt1x_ts->dev_data->y_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, GT1X_ABS_MAX_VALUE,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0,
			     GT1X_ABS_MAX_VALUE, 0, 0);
...
static int
gt1x_wakeup_gesture_switch(struct ts_wakeup_gesture_enable_info *info)
{
	TS_LOG_INFO("%s:done\n", __func__);
	return NO_ERR;
}
...
static void gt1x_irq_output(int level)
{
	...
	if (level)
		error = pinctrl_select_state(gt1x_ts->pinctrl,
					     gt1x_ts->pinctrl_state_int_high);
	else
		error = pinctrl_select_state(gt1x_ts->pinctrl,
					     gt1x_ts->pinctrl_state_int_low);
...
static int gt1x_reset_output(int level)
{
	...
	if (level)
		error = pinctrl_select_state(gt1x_ts->pinctrl,
					     gt1x_ts->pinctrl_state_reset_high);
	else
		error = pinctrl_select_state(gt1x_ts->pinctrl,
					     gt1x_ts->pinctrl_state_reset_low);
#else
...
static void gt1x_select_addrs(void)
{
	...
	ret = pinctrl_select_state(gt1x_ts->pinctrl,
				   gt1x_ts->pinctrl_state_as_int);
	if (ret < 0) {
...
static int gt1x_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_WRITE:
		if (info->glove_switch) {
			...
			ret = gt1x_feature_switch(gt1x_ts, TS_FEATURE_GLOVE,
						  SWITCH_ON);
			if (!ret)
				glove_en = true;
		} else {
...
static int gt1x_charger_switch(struct ts_charger_info *info)
{
	...
	case TS_ACTION_WRITE:
		if (info->charger_switch) {
			...
			ret = gt1x_send_cmd(GTP_CMD_CHARGER_OFF, 0x00,
					    GT1X_NOT_NEED_SLEEP);
...
static int gt1x_holster_switch(struct ts_holster_info *info)
{
	...
	case TS_ACTION_WRITE:
		if (info->holster_switch)
			ret = gt1x_feature_switch(gt1x_ts, TS_FEATURE_HOLSTER,
						  SWITCH_ON);
		else
			ret = gt1x_feature_switch(gt1x_ts, TS_FEATURE_HOLSTER,
						  SWITCH_OFF);
...
static int gt1x_check_hw_status(void)
{
	...
	if (likely(!ret && i < 3)) {
		...
		ret = gt1x_i2c_write(GTP_REG_CMD, &feed_wd[0], sizeof(feed_wd));
	} else {
		TS_LOG_ERR(
			"%s: HW status:Error,reg{8040h}=%02X,reg{8043h}=%02X\n",
...
{
	if (gt1x_ts->dev_data->ts_platform_data->feature_info.roi_info
		    .roi_supported) {
		...
		TS_LOG_DEBUG(
			"%s: pre[%d]  cur[%d]  &[%d] gt1x_roi_fw_supported[%d] \n",
			__func__, pre_index, cur_index, (cur_index & pre_index),
			gt1x_ts->gt1x_roi_fw_supported);
...
static int gt1x_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
	int ret = 0;
	...
	switch (info->op_action) {
	case TS_ACTION_READ:
		memcpy(info->tp_test_type, gt1x_ts->dev_data->tp_test_type,
...
static int gt1x_get_debug_data_(u16 *debug_data, u32 sen_num, u32 drv_num,
				int data_type)
{
	int i = 0;
	int ret = 0;
	...
	if (READ_RAW_DATA == data_type) { /*  get rawdata */
		data_len = (sen_num * drv_num) *
			   sizeof(u16); /*  raw data is u16 type */
		if (IC_TYPE_9P == gt1x_ts->ic_type)
	...
	if (wait_cnt < GT1X_RETRY_NUM) {
		ret = gt1x_i2c_read(data_start_addr, ptr, data_len);
		if (ret) {
			...
			struct ts_cmd_node *out_cmd)
{
	...
	case READ_DIFF_DATA:
		...
		ret = gt1x_get_debug_data_(debug_data, sen_num, drv_num,
					   READ_DIFF_DATA);
		break;
	case READ_RAW_DATA:
...
static void gt1x_set_doze_mode(unsigned int soper, unsigned char param)
{
	...
	if (TS_SWITCH_TYPE_DOZE !=
	    (ts->dev_data->touch_switch_flag & TS_SWITCH_TYPE_DOZE)) {
	...
	case TS_SWITCH_DOZE_DISABLE:
		...
		if (error) {
			TS_LOG_ERR(
				"%s: Failed send doze disable cmd: error%d\n",
				__func__, error);
		}
		break;
	default:
...
static void gt1x_set_game_mode(unsigned int soper)
{
	...
	if ((NULL == ts) || (NULL == ts->dev_data) ||
	    (NULL == ts->dev_data->ts_platform_data)) {
		TS_LOG_ERR("%s, error chip data\n", __func__);
		goto out;
	}
	...
	case TS_SWITCH_GAME_DISABLE:
		...
		TS_LOG_INFO(
			"%s: exit game_mode to [holster_switch:%d][glove_switch:%d]\n",
