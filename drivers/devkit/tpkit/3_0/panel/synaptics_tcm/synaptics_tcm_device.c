...
static void device_capture_touch_report(unsigned int count)
{
	...
	case REPORT_TOUCH:
		...
		retval = tskit_driver_alloc_mem(tcm_hcd, &device_hcd->report,
						remaining_size);
		if (retval < 0) {
			TS_LOG_ERR(
...
static int device_capture_touch_report_config(unsigned int count)
{
	...
	retval = tskit_driver_alloc_mem(tcm_hcd, &tcm_hcd->config, size);
	if (retval < 0) {
		TS_LOG_ERR(
...
#endif
{
	...
	case DEVICE_IOC_IRQ:
		...
		if (arg == 0) {
			if (TS_BUS_SPI ==
			    tcm_hcd->syna_tcm_chip_data->ts_platform_data->bops
				    ->btype) {
			   ...
			   loff_t *f_pos)
{
	...
	if (device_hcd->raw_mode) {
		retval = tskit_driver_alloc_mem(tcm_hcd, &device_hcd->resp,
						count);
		if (retval < 0) {
			TS_LOG_ERR(
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	retval = tskit_driver_alloc_mem(tcm_hcd, &device_hcd->out,
