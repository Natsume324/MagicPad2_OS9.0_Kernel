...
#define reflash_write(p_name)                                           \
	...
	{                                                               \
		...
		if (retval < 0) {                                       \
			TS_LOG_ERR("Failed to write to flash\n");       \
			return retval;                                  \
		}                                                       \
                                                                        \
		return 0;                                               \
...
#define reflash_erase(p_name)                                           \
	...
	{                                                               \
		...
		page_count = ceil_div(size, reflash_hcd->page_size);    \
                                                                        \
		TS_LOG_ERR("Page start = %d\n", page_start);            \
                                                                        \
		TS_LOG_ERR("Page count = %d\n", page_count);            \
                                                                        \
		retval = reflash_erase_flash(page_start, page_count);   \
		if (retval < 0) {                                       \
			TS_LOG_ERR("Failed to erase flash pages\n");    \
			return retval;                                  \
		}                                                       \
                                                                        \
		return 0;                                               \
...
#define reflash_update(p_name)                                                 \
	...
	{                                                                      \
		...
		if (retval < 0) {                                              \
			TS_LOG_ERR("Failed " #p_name " partition check\n");    \
			reset = true;                                          \
			goto reset;                                            \
		}                                                              \
                                                                               \
		...
		if (retval < 0) {                                              \
			TS_LOG_ERR("Failed to erase " #p_name " partition\n"); \
			reset = true;                                          \
			goto reset;                                            \
		}                                                              \
                                                                               \
		TS_LOG_ERR("Partition erased (" #p_name ")\n");                \
                                                                               \
		retval = reflash_write_##p_name();                             \
		if (retval < 0) {                                              \
			TS_LOG_ERR("Failed to write " #p_name " partition\n"); \
			reset = true;                                          \
			goto reset;                                            \
		}                                                              \
                                                                               \
		TS_LOG_ERR("Partition written (" #p_name ")\n");               \
                                                                               \
		retval = 0;                                                    \
                                                                               \
reset:                                                                         \
...
static int reflash_set_up_flash_access(void)
{
	...
	TS_LOG_ERR("response[0] %x, %x, %x, %x, %x, %x, %x\n", tmp_response[0],
		   tmp_response[1], tmp_response[2], tmp_response[3],
		   tmp_response[4], tmp_response[5], tmp_response[6]);
	if (retval < 0 || (tmp_response[1] != STATUS_OK &&
			   tmp_response[1] != REPORT_IDENTIFY)) {
		TS_LOG_ERR("in set up reflash 1\n");
		return -EINVAL;
	}

	count = MIN(sizeof(tcm_hcd->id_info), TMP_RESPONSE_LEN);
	if ((count > TMP_RESPONSE_LEN) || (count > sizeof(tcm_hcd->id_info))) {
	...
	if (tcm_hcd->id_info.mode == MODE_APPLICATION) {
		TS_LOG_ERR("need to enter mode bootloader.....\n");
		retval = reflash_raw_write(tcm_hcd, CMD_RUN_BOOTLOADER_FIRMWARE,
					   NULL, 0);
		if (retval < 0) {
		...
		msleep(50);
		retval = tskit_driver_read(tcm_hcd, tmp_response,
					   TMP_RESPONSE_LEN);
		if (retval < 0) {
		...
		TS_LOG_ERR("response[0] %x, %x, %x, %x, %x, %x, %x\n",
			   tmp_response[0], tmp_response[1], tmp_response[2],
			   tmp_response[3], tmp_response[4], tmp_response[5],
			   tmp_response[6]);
	...
	retval = reflash_raw_write(tcm_hcd, CMD_GET_BOOT_INFO, NULL, 0);
	if (retval < 0) {
	...
	retval = tskit_driver_read(tcm_hcd, tmp_response, TMP_RESPONSE_LEN);
	if (retval < 0) {
		TS_LOG_ERR("in set up reflash 3\n");
		return -EINVAL;
	}
	...
	TS_LOG_ERR("Write block size = %d\n", reflash_hcd->write_block_size);

	TS_LOG_ERR("Page size = %d\n", reflash_hcd->page_size);

	TS_LOG_ERR("Max write payload size = %d\n",
...
static enum update_area reflash_compare_id_info(void)
{
	...
	if (reflash_hcd->image_info.app_config.size < sizeof(*header)) {
		TS_LOG_ERR("Invalid application config in image file\n");
		goto exit;
	}
...
exit:
	if (update_area == NONE) {
		TS_LOG_ERR("No need to do reflash\n");
	} else {
		TS_LOG_ERR("Updating %s\n", update_area == FIRMWARE_CONFIG ?
						    "firmware and config" :
						    "config only");
	}
...
static int reflash_check_app_config(void)
{
	...
	if (image_addr != device_addr) {
		TS_LOG_ERR("Flash address mismatch\n");
		return -EINVAL;
	}
...
static int reflash_check_disp_config(void)
{
	...
	if (image_size != device_size) {
		TS_LOG_ERR("Config size mismatch\n");
		return -EINVAL;
	}
			       ...
			       unsigned int datalen)
{
	...
	if (!response) {
		TS_LOG_ERR("Failed to allocate memory for response\n");
		return -ENOMEM;
	}
	...
	while (remaining_length) {
		...
		retval = tskit_driver_alloc_mem(tcm_hcd, &reflash_hcd->out,
						xfer_length + 2);
		if (retval < 0) {
			TS_LOG_ERR(
		...
		TS_LOG_DEBUG(
			"xfer_length:%d, remaining_length:%d, w_length:%d, wr_chunk_size:%d\n",
			xfer_length, remaining_length, w_length,
			tcm_hcd->wr_chunk_size);
		flash_offset = address + offset;
		if (reflash_hcd->write_block_size != 0)
			block_address =
				flash_offset / reflash_hcd->write_block_size;
		...
		retval = reflash_raw_write(tcm_hcd, CMD_WRITE_FLASH,
					   reflash_hcd->out.buf,
					   xfer_length + 2);
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		if (idx == 0) {
			...
			memcpy(&tcm_hcd->out.buf[1],
			       &payload[idx * chunk_space - 2], xfer_length);
		}
...
static int reflash_erase_flash(unsigned int page_start, unsigned int page_count)
{
	...
	TS_LOG_ERR("start to write erase command %d %d\n", page_start,
		   page_count);
	retval = reflash_raw_write(tcm_hcd, CMD_ERASE_FLASH, out_buf,
				   size_erase_cmd);
	...
	retval = tskit_driver_read(tcm_hcd, response, ERASE_FLASH_RESPONSE_LEN);
	if (retval < 0) {
		TS_LOG_ERR("Failed to read erase response\n");
		UNLOCK_BUFFER(reflash_hcd->resp);
		return retval;
	}
	UNLOCK_BUFFER(reflash_hcd->resp);
							...
							unsigned int len)
{
	...
	case FIRMWARE_CONFIG:
		...
		if (retval < 0) {
			TS_LOG_ERR("Failed to reflash application firmware\n");
			goto exit;
		}
	...
	case CONFIG_ONLY:
		...
		if (retval < 0) {
			TS_LOG_ERR("Failed to reflash application config\n");
			goto exit;
		}
		break;
	case NONE:
...
int reflash_after_fw_update_do_reset(void)
{
	...
	if (tcm_hcd->id_info.mode != MODE_APPLICATION) {
		...
		retval = reflash_raw_write(
			tcm_hcd, CMD_RUN_APPLICATION_FIRMWARE, NULL, 0);
		if (retval < 0) {
			TS_LOG_ERR("Failed to enter bootloader mode 1\n");
			return retval;
		}
			      ...
			      unsigned int datalen)
{
	...
	retval = tskit_driver_alloc_mem(tcm_hcd, &reflash_hcd->out, 6);
	if (retval < 0) {
		TS_LOG_ERR(
			"Failed to allocate memory for reflash_hcd->out.buf\n");
		UNLOCK_BUFFER(reflash_hcd->out);
	...
	retval = reflash_raw_write(tcm_hcd, CMD_READ_FLASH,
				   reflash_hcd->out.buf, 6);
	if (retval < 0) {
		TS_LOG_ERR("Failed to write command %s\n", STR(CMD_READ_FLASH));
	...
	msleep(50);
	retval = tskit_driver_read(tcm_hcd, flash_read_data,
				   FLASH_READ_DATA_LEN);
	if (retval < 0) {
	...
	memcpy(data, &flash_read_data[MESSAGE_HEADER_SIZE], datalen);
...
int reflash_parse_boot_config(unsigned char *boot_config_data, int data_length,
			      unsigned char *out_info, int length)
{
	int i = 0;
...
int reflash_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (!reflash_hcd) {
		TS_LOG_ERR("Failed to allocate memory for reflash_hcd\n");
		return -ENOMEM;
	}
	tmp_response = kzalloc(TMP_RESPONSE_LEN, GFP_KERNEL);
	if (!tmp_response) {
		TS_LOG_ERR("Failed to allocate memory for tmp_response\n");
		return -ENOMEM;
	}
	flash_read_data = kzalloc(FLASH_READ_DATA_LEN, GFP_KERNEL);
	if (!flash_read_data) {
		TS_LOG_ERR("Failed to allocate memory for flash_read_data\n");
		return -ENOMEM;
	}
	boot_config_data = kzalloc(BOOT_CONFIG_DATA_LEN, GFP_KERNEL);
	if (!boot_config_data) {
		TS_LOG_ERR("Failed to allocate memory for boot_config_data\n");
		return -ENOMEM;
	}
	...
	if (tcm_hcd->htd == false) {
		...
		reflash_read_boot_config(boot_config_data,
					 BOOT_CONFIG_DATA_LEN);
		reflash_parse_boot_config(
			boot_config_data, BOOT_CONFIG_DATA_LEN,
