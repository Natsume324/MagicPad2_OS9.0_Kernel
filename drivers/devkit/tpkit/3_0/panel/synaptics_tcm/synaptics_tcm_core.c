...
// clang-format off
static int syna_report_priority[BIT_MAX] = {
	0, 3, 8, 7, 12, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF, 0xFF, 0xFF,
	0xFF, 0xFF
...
static int syna_report_priority_limite[BIT_MAX] = {
...
static int
tskit_driver_chip_get_capacitance_test_type(struct ts_test_type_info *info);
...
static int tskit_driver_lowpower_ctrl(unsigned int state)
{
	...
	delta_time =
		((lowpower_receive.tv_sec - kit_lowpower_switch_time.tv_sec) *
		 1000000) +
		lowpower_receive.tv_usec - kit_lowpower_switch_time.tv_usec;
	delta_time /= 1000;
	if (delta_time < 10) {
...
static int tskit_driver_gesture_mode_config(bool en, unsigned short mode)
{
	...
	resp_buf = kzalloc(RESP_BUF_LEN * sizeof(*resp_buf),
			   GFP_KERNEL); /* response length is 10 */
	if (!resp_buf) {
	...
	do {
		...
		retval = tskit_driver_read(tcm_hcd, resp_buf, RESP_BUF_LEN);
...
static int tskit_driver_chip_reset(void)
{
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		TS_LOG_INFO("%s reset\n", __func__);
		gpio_direction_output(tcm_hcd->syna_tcm_chip_data
...
static void tskit_driver_report_dmd_state_report(void)
{
	...
	TS_LOG_INFO("%s, input value is %lu  buf[0] = %d buf[1] = %d", __func__,
		    abnormal_status, buf[0], buf[1]);

	for (i = 0; i < BIT_MAX; i++) {
		if (0xFF != syna_report_priority[i] &&
		    BIT15_RESERVED >=
			    syna_report_priority[i]) { // check same value
			count++;
			if (test_bit(syna_report_priority[i],
				     (unsigned long *)&abnormal_status) &&
			    syna_report_priority_limite[i]) {
				if (BIT6_CHARGER_NOISE_HOP ==
					    syna_report_priority[i] &&
				    CHARGE_REPORT_DONE !=
					    syna_dmd_charge_info
			...
			TS_LOG_INFO(
				"tp_status_dmd_bit_status  after [%d] = report_priority = %d status %d count = %d , limite %d",
				i, syna_report_priority[i],
				tp_status_dmd_bit_status[i].bit_status,
				tp_status_dmd_bit_status[i].bit_count,
				syna_report_priority_limite[i]);
		} else {
			break;
...
static int tskit_driver_hdl_get_state(void)
{
	...
	if (!temp_buf) {
		TS_LOG_ERR("Failed to allocate memory for temp_buf\n");
		return -ENOMEM;
	}

	retval = tskit_driver_write_hdl_message(
		tcm_hcd, CMD_GET_TP_ABNORMAL_STATE, NULL, 0,
		&tcm_hcd->config.buf, &tcm_hcd->config.buf_size,
	...
	if (tcm_hcd->in_before_suspend) {
		while (retry) {
			mdelay(1);
			retval = tskit_driver_read(tcm_hcd, temp_buf,
						   ABNORMAL_STATE_LEN);
			if (temp_buf[1] != STATUS_OK)
				TS_LOG_INFO(
					"temp_buf: 0x%02x 0x%02x 0x%02x 0x%02x ",
...
static int tskit_driver_get_project_id(char *project_id)
{
	...
	if (bdata->default_project_id) {
		if (strncmp(project_id, bdata->default_project_id,
			    PRODUCT_ID_FW_LEN - 1)) {
			TS_LOG_INFO(
				"%s: Get project id from lcd error, use default: %s\n",
				__func__, bdata->default_project_id);
			memcpy(project_id, bdata->default_project_id,
			       projectid_lenth);
			return 0;
		}
	}
...
static int tskit_driver_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if (TS_BUS_SPI ==
		...
		len = MIN(sizeof(buf_proj_id) - 1,
			  strlen(tcm_hcd->tcm_mod_info.project_id_string));
		memcpy(buf_proj_id, tcm_hcd->tcm_mod_info.project_id_string,
		       len);
	}

	snprintf(buf_fw_ver, CHIP_INFO_LENGTH - 1, "PR%d",
		 tcm_hcd->packrat_number);
	TS_LOG_INFO("buf_fw_ver = %s", buf_fw_ver);

	if ((!tcm_hcd->syna_tcm_chip_data->ts_platform_data->hide_plain_id) ||
	    ts_is_factory()) {
		len = MIN(sizeof(info->ic_vendor) - 1,
			  sizeof(SYNAPTICS_CHIP_INFO));
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		 ...
		 * 2,3 : config id flag
		 */
		snprintf(config_id, SYNC_TCM_CONFIG_ID_ARRY_SIZE, "_%02x%02x",
				      ...
				      unsigned int length)
{
	...
	struct i2c_client *i2c =
		tcm_hcd->syna_tcm_chip_data->ts_platform_data->client;
...
static int tskit_driver_spi_rmi_read_stransfer(struct syna_tcm_hcd *tcm_hcd,
					       unsigned short addr,
					       unsigned char *data,
					       unsigned int length)
{
	...
	struct spi_device *spi =
		tcm_hcd->syna_tcm_chip_data->ts_platform_data->spi;
				     ...
				     unsigned int length)
{
	...
	retval = tskit_driver_spi_rmi_read_stransfer(tcm_hcd, addr, data,
						     length);
	if (retval < 0) {
...
static int tskit_driver_spi_rmi_write_transfer(struct syna_tcm_hcd *tcm_hcd,
					       unsigned short addr,
					       unsigned char *data,
					       unsigned int length)
{
				      ...
				      unsigned int length)
{
	...
	retval = tskit_driver_spi_rmi_write_transfer(tcm_hcd, addr, data,
						     (length + 2));
	if (retval < 0) {
...
static int tskit_driver_spi_read_transfer(struct syna_tcm_hcd *tcm_hcd,
					  unsigned char *data,
					  unsigned int length)
{
	int retval = NO_ERR;
...
static int tskit_driver_spi_write_transfer(struct syna_tcm_hcd *tcm_hcd,
					   unsigned char *data,
					   unsigned int length)
{
	int retval = NO_ERR;
...
static int tskit_driver_spi_write(struct syna_tcm_hcd *tcm_hcd,
				  unsigned char *data, unsigned int length)
{
	int retval = NO_ERR;
...
static void tskit_driver_dispatch_response(struct syna_tcm_hcd *tcm_hcd)
{
	...
	retval = tskit_driver_alloc_mem(tcm_hcd, &tcm_hcd->resp,
...
static void
tskit_driver_dispatch_response_for_one_package(struct syna_tcm_hcd *tcm_hcd,
					       char *in_buf)
{
	int retval = NO_ERR;
	...
	if (tcm_hcd->status_report_code == REPORT_IDENTIFY) {
		...
		if (TS_BUS_SPI ==
			...
			tcm_hcd->wr_chunk_size =
				MIN(max_write_size, WR_CHUNK_SIZE_SPI);
		} else {
			if (chip_data->ts_platform_data->i2c_driver_type ==
			    I3C_MASTER_I2C_DRIVER)
				tcm_hcd->wr_chunk_size =
					MIN(max_write_size, WR_CHUNK_SIZE_I3C);
			else
				tcm_hcd->wr_chunk_size =
					MIN(max_write_size, WR_CHUNK_SIZE_I2C);
		}
...
static int tskit_driver_continued_read(struct syna_tcm_hcd *tcm_hcd)
{
	...
	LOCK_BUFFER(tcm_hcd->in);
	retval = tskit_driver_realloc_mem(tcm_hcd, &tcm_hcd->in,
					  total_length + 1);
	if (retval < 0) {
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval = tskit_driver_alloc_mem(tcm_hcd, &tcm_hcd->temp,
						xfer_length + 2);
		if (retval < 0) {
			TS_LOG_ERR(
		...
		UNLOCK_BUFFER(tcm_hcd->temp);
		retval = tskit_driver_read(tcm_hcd, tcm_hcd->temp.buf,
					   xfer_length + 2);
		LOCK_BUFFER(tcm_hcd->temp);
		if (retval < 0) {
		...
		if ((xfer_length > tcm_hcd->in.buf_size - offset) ||
		    (xfer_length > tcm_hcd->temp.buf_size - MSG_STATUS_SIZE)) {
			TS_LOG_ERR("Failed to copy payload\n");
			UNLOCK_BUFFER(tcm_hcd->temp);
			UNLOCK_BUFFER(tcm_hcd->in);
			return -EINVAL;
		...
		memcpy(&tcm_hcd->in.buf[offset],
		       &tcm_hcd->temp.buf[MSG_STATUS_SIZE], xfer_length);
...
int tskit_driver_i2c_read(struct syna_tcm_hcd *tcm_hcd, unsigned char *in_buf,
			  unsigned int length)
{
	int retval = NO_ERR;
	unsigned char code = 0;
	...
	if (length < 2) {
		retval = tskit_driver_i2c_read_data(tcm_hcd, in_buf, length);
		if (retval < 0) {
			TS_LOG_ERR("Failed to read from device\n");
			return retval;
		}
		TS_LOG_ERR("small length information\n");
		return length;
	}
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval = tskit_driver_i2c_read_data(tcm_hcd, tcm_hcd->temp.buf,
						    xfer_length + 2);
		if (retval < 0) {
			TS_LOG_ERR("Failed to read from device\n");
			UNLOCK_BUFFER(tcm_hcd->temp);
		...
		TS_LOG_DEBUG("temp buf:%x, %x, %x, %x, %x, %x\n",
			     tcm_hcd->temp.buf[0], tcm_hcd->temp.buf[1],
			     tcm_hcd->temp.buf[2], tcm_hcd->temp.buf[3],
			     tcm_hcd->temp.buf[4], tcm_hcd->temp.buf[5]);
		...
		if (idx == 0) {
			...
			memcpy(&in_buf[0], &tcm_hcd->temp.buf[0],
			       xfer_length + MSG_STATUS_SIZE);
			//remaiming length cust
			TS_LOG_DEBUG("remaining length is  %d\n",
				     remaining_length);
			if ((length > ((tcm_hcd->temp.buf[3] << 8 |
					tcm_hcd->temp.buf[2]) +
				       5)) &&
			    (code != STATUS_CONTINUED_READ)) {
				/* 5:header length, 2:minus header marker byte and header code byte */
				remaining_length = (tcm_hcd->temp.buf[3] << 8 |
						    tcm_hcd->temp.buf[2]) +
						   5 - 2;
			}
				  ...
				  unsigned int length)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval = tskit_driver_alloc_mem(tcm_hcd, &tcm_hcd->out,
						xfer_length + 1);
		if (retval < 0) {
			TS_LOG_ERR(
		...
		if (xfer_length) {
			...
			memcpy(&tcm_hcd->out.buf[1], &data[idx * chunk_space],
			       xfer_length);
		}

		retval = tskit_driver_write(tcm_hcd, tcm_hcd->out.buf,
					    xfer_length + 1);
		if (retval < 0) {
...
retry:
	...
	retval = tskit_driver_read(tcm_hcd, tcm_hcd->in.buf,
				   tcm_hcd->read_length);
	if (retval < 0) {
		TS_LOG_ERR("Failed to read from device\n");
		UNLOCK_BUFFER(tcm_hcd->in);
...
static int tskit_driver_write_message(
	...
	unsigned char *response_code, unsigned int polling_delay_ms)
{
	...
	if (resp_buf == NULL) {
		retval = tskit_driver_raw_write(tcm_hcd, command, payload,
						length);
		goto exit;
	}
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type != S3909) {
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		if (idx == 0) {
			...
			if (xfer_length > 2) {
				...
				memcpy(&tcm_hcd->out.buf[3], payload,
				       xfer_length - MSG_STATUS_SIZE);
			}
		} else {
			tcm_hcd->out.buf[0] = CMD_CONTINUE_WRITE;
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		...
		do {
			...
			if (retval < 0) {
				TS_LOG_ERR(
					"Failed to read in the response command 0x%02x\n",
					tcm_hcd->command);
				goto exit;
			}
...
static void tskit_driver_pinctrl_select_lowpower(void)
{
	...
	if (tcm_hcd->pctrl) {
		retval = pinctrl_select_state(tcm_hcd->pctrl,
					      tcm_hcd->pins_idle);
		if (retval < 0) {
...
static void tskit_driver_power_on_gpio_set(void)
{
	...
	retval = gpio_direction_output(
		tcm_hcd->syna_tcm_chip_data->ts_platform_data->reset_gpio, 1);
	if (retval) {
...
{
	gpio_direction_output(
		tcm_hcd->syna_tcm_chip_data->ts_platform_data->reset_gpio, 0);
	mdelay(1);
	return;
}
...
static int tskit_driver_gpio_request(void)
{
	...
	if ((1 == tcm_hcd->syna_tcm_chip_data->vci_gpio_type) &&
		...
		if (tcm_hcd->syna_tcm_chip_data->vci_gpio_ctrl ==
			...
			if (retval) {
				TS_LOG_INFO(
					"unable to request vci_gpio_ctrl firset:%d\n",
			...
			if (retval) {
				TS_LOG_INFO(
					"unable to request vci_gpio_ctrl2:%d\n",
					tcm_hcd->syna_tcm_chip_data
						->vci_gpio_ctrl);
				goto ts_vci_out;
			}
			...
			if (retval) {
				TS_LOG_INFO(
					"unable to request vddio_gpio_ctrl:%d\n",
					tcm_hcd->syna_tcm_chip_data
						->vddio_gpio_ctrl);
				goto ts_vddio_out;
			}
...
get_app_info:
	...
	if ((count > sizeof(tcm_hcd->app_info)) || (count > resp_buf_size)) {
		TS_LOG_ERR("Failed to copy application info\n");
		retval = -EINVAL;
		goto exit;
	}
	memcpy((unsigned char *)&tcm_hcd->app_info, resp_buf,
...
static int tskit_driver_get_boot_info(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if ((count > sizeof(tcm_hcd->boot_info)) || (count > resp_buf_size)) {
		TS_LOG_ERR("Failed to copy boot info\n");
		retval = -EINVAL;
		goto exit;
	}
	memcpy((unsigned char *)&tcm_hcd->boot_info, resp_buf,
...
static int tskit_driver_identify(struct syna_tcm_hcd *tcm_hcd, bool id)
{
	...
	if ((count > sizeof(tcm_hcd->id_info) || (count > resp_buf_size))) {
		TS_LOG_ERR("Failed to copy identification info\n");
		retval = -EINVAL;
		goto exit;
	}
	memcpy((unsigned char *)&tcm_hcd->id_info, resp_buf,
					   ...
					   unsigned short *value)
{
	...
	retval = tskit_driver_write_hdl_message(tcm_hcd, CMD_GET_DYNAMIC_CONFIG,
						&out_buf, sizeof(out_buf),
						(unsigned char **)resp_buf,
	...
	while (retry) {
		...
		if (resp_buf[1] != STATUS_OK)
			TS_LOG_INFO("resp_buf: 0x%02x 0x%02x 0x%02x 0x%02x ",
					   ...
					   unsigned short value)
{
	...
	while (retry) {
		retval = tskit_driver_write_hdl_message(
			tcm_hcd, CMD_SET_DYNAMIC_CONFIG, out_buf,
		...
		if (tcm_hcd->bdata->support_vendor_ic_type == S3909) {
			...
			do {
				mdelay(5);
				retval = tskit_driver_read(tcm_hcd, resp_buf,
							   RESP_BUF_LEN);
				if ((retval >= 0) &&
	...
	if (resp_buf[1] != STATUS_OK)
		TS_LOG_ERR(
			"Failed to write dynamic 0x%02x payload 0x%02x 0x%02x\n",
					   ...
					   unsigned short value)
{
	...
	while (retry) {
		retval = tskit_driver_write_hdl_message(
			s_tcm_hcd, CMD_AP_SET_DYNAMIC_CMD, out_buf,
			sizeof(out_buf), (unsigned char **)resp_buf,
			&resp_buf_size, &resp_length, NULL, 0);
		if (retval < 0) {
			TS_LOG_ERR("Failed to write command %s\n",
		...
		/* sequential */
		udelay(500);
		retval = tskit_driver_read(s_tcm_hcd, resp_buf, RESP_BUF_LEN);
		if ((retval < 0) || (resp_buf[0] != MESSAGE_MARKER)) {
			TS_LOG_ERR("Failed to read response %s\n",
				   STR(CMD_AP_SET_DYNAMIC_CMD));
			goto exit;
...
static int tskit_driver_sleep(struct syna_tcm_hcd *tcm_hcd, bool en)
{
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type != S3909)
		retval = tskit_driver_write_hdl_message(
			tcm_hcd, command, NULL, 0, (unsigned char **)resp_buf,
			&resp_buf_size, &resp_length, NULL, 0);
	else
		retval = tcm_hcd->write_message(tcm_hcd, command, NULL, 0,
						&resp_buf_p, &resp_buf_size,
						&resp_length, NULL, 0);
...
static int tskit_driver_reset(struct syna_tcm_hcd *tcm_hcd, bool hw,
			      bool update_wd)
{
	int retval = NO_ERR;
	unsigned char *resp_buf = NULL;
	...
	if (hw) {
		...
		gpio_set_value(tcm_hcd->syna_tcm_chip_data->ts_platform_data
				       ->reset_gpio,
...
static int tskit_driver_comm_check(void)
{
	...
	if (TS_BUS_SPI ==
		...
		identify_info = kzalloc(
			IDENTIFY_INFO_LEN * sizeof(*identify_info), GFP_KERNEL);
		if (!identify_info) {
			TS_LOG_ERR(
				"Failed to allocate memory for identify_info\n");
			return -ENOMEM;
		}
		retval = tskit_driver_rmi_read(tcm_hcd, PDT_START_ADDR, uboot,
					       UBOOT_LEN);
		TS_LOG_ERR(
			"detect uboot[0] = %x.uboot[1] = %xuboot[2] = %x.uboot[3] = %xuboot[4] = %x.uboot[5] = %x\n",
			uboot[0], uboot[1], uboot[2], uboot[3], uboot[4],
			uboot[5]);
		...
		if (uboot[5] == UBL_FN_NUMBER) {
			...
			retval = tskit_driver_read(tcm_hcd, identify_info,
						   IDENTIFY_INFO_LEN);
			if ((retval < 0) ||
			    (identify_info[0] != MESSAGE_MARKER)) {
		...
		retval = tskit_driver_read(tcm_hcd, &marker, 1);
...
retry:
	if (tcm_hcd->bdata->support_vendor_ic_type == S3909)
		retval =
			tskit_driver_read(tcm_hcd, buffer, MESSAGE_HEADER_SIZE);
	else
		retval = tskit_driver_read(tcm_hcd, buffer, FIXED_READ_LENGTH);
	if (retval < 0) {
		TS_LOG_ERR("Failed to read from device\n");
		if (retry) {
			usleep_range(READ_RETRY_US_MIN, READ_RETRY_US_MAX);
	...
	if (header->marker != MESSAGE_MARKER) {
		if (error_log_count < 5) {
			error_log_count++;
			TS_LOG_ERR(
				"Incorrect header marker (0x%02x),error_log_count=%d\n",
	...
	if (tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		if (tcm_hcd->payload_length > 0) {
			if (tcm_hcd->payload_length >=
			    FIXED_READ_LENGTH - MESSAGE_HEADER_SIZE - 1) {
				retval = -1;
				TS_LOG_ERR("Payload length = %d is too big\n",
					   tcm_hcd->payload_length);
				goto exit;
			}
			retval = tskit_driver_read(
				tcm_hcd, &buffer[SECOND_PACKAGE_HEAD_SIZE],
				tcm_hcd->payload_length +
					SECOND_PACKAGE_HEAD_SIZE + 1);
			if (retval < 0) {
				TS_LOG_ERR(
					"Failed to read payload from device\n");
				goto exit;
			}
	...
	if (tcm_hcd->status_report_code <= STATUS_ERROR ||
		...
		case STATUS_IDLE:
		case STATUS_BUSY:
			memset(buffer + MESSAGE_HEADER_SIZE, 0x00,
			       (FIXED_READ_LENGTH - MESSAGE_HEADER_SIZE));
			tcm_hcd->payload_length = 0;
			retval = 0;
			goto exit;
...
static int tskit_driver_suspend_for_gesture_mode(void)
{
	...
	retval = send_thp_driver_status_cmd(
		POWER_OFF, g_ts_kit_platform_data.multi_panel_index,
	...
	if (g_ts_kit_platform_data.support_ap_shb_i2c_switch)
		gpio_direction_output(g_ts_kit_platform_data.i2c_switch_gpio,
				      1);
	return retval;
}
...
static int tskit_driver_suspend(void)
{
	...
	if (g_ts_kit_platform_data.udfp_enable_flag) {
		...
		retval = tskit_driver_ap_set_dynamic_cmd(
			tcm_hcd, DC_AP_SET_ENABLE_LOZE, 1);
...
#ifdef CONFIG_HONOR_SHB_THP
	retval = send_thp_driver_status_cmd(
		POWER_ON, g_ts_kit_platform_data.multi_panel_index,
		ST_CMD_TYPE_MULTI_TP_UD_STATUS);
	if (retval < 0)
...
static int tskit_driver_after_resume(void *feature_info)
{
	...
	read_buffer =
		kzalloc(FIXED_READ_LENGTH * sizeof(*read_buffer), GFP_KERNEL);
	if (!read_buffer) {
	...
	if (g_ts_kit_platform_data.udfp_enable_flag) {
		...
		retval = tskit_driver_read(tcm_hcd, tmp_buffer, TMP_BUFFER_LEN);
		if (retval < 0)
...
static int tskit_driver_set_info_flag(struct ts_kit_platform_data *info)
{
	tcm_hcd->syna_tcm_chip_data->ts_platform_data->get_info_flag =
		info->get_info_flag;
	return NO_ERR;
}
...
static int tskit_driver_fw_update_boot(char *file_name, bool force)
{
	...
	if (TS_BUS_SPI ==
		...
		if (tcm_hcd->syna_tcm_chip_data->projectid_len) {
			projectid_lenth =
				tcm_hcd->syna_tcm_chip_data->projectid_len;
		} else {
			projectid_lenth = PROJECT_ID_FW_LEN;
		}
...
retry:
			...
			if (retval) {
				...
				TS_LOG_INFO(
					"downloaded firmware successfully\n");
					...
					struct ts_cmd_node *out_cmd)
{
	...
	if (retval < 0) {
		TS_LOG_ERR(
			"Failed to tskit_driver_read_one_package, try to read F$35\n");
	}
	if (tcm_hcd->status_report_code <= REPORT_IDENTIFY) {
		out_cmd->command = TS_INVAILD_CMD;
	...
	if (NEED_REPORT == tcm_hcd->esd_report_status &&
	    tcm_hcd->use_esd_report) {
...
static int tskit_driver_input_config(struct input_dev *input_dev)
{
	...
	if (bdata->support_vendor_ic_type == S3909) {
		input_set_abs_params(input_dev, ABS_X, 0, bdata->x_max - 1, 0,
				     0);
		input_set_abs_params(input_dev, ABS_Y, 0, bdata->y_max - 1, 0,
				     0);
		input_set_abs_params(input_dev, ABS_PRESSURE, 0, 255, 0, 0);
		input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, 15, 0,
				     0);
...
static void tskit_driver_parse_basic_dts(struct device_node *np,
					 struct ts_kit_device_data *chip_data)
{
	int retval = NO_ERR;
	u32 value = 0;
	...
	retval = of_property_read_u32(np, "max_finger_objects",
				      &bdata->max_finger_objects);
	if (retval) {
		TS_LOG_ERR("get device y_max failed\n");
		bdata->max_finger_objects = 0;
	}

	retval = of_property_read_string(np, "default_project_id",
...
static void tskit_driver_parse_power_dts(struct device_node *np,
					 struct ts_kit_device_data *chip_data)
{
	int retval = NO_ERR;
	u32 value = 0;
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			TS_LOG_ERR(
				"Unable to read ubl-byte-delay-us property\n");
			return;
		} else {
...
static void tskit_driver_parse_feature_dts(struct device_node *np,
					   struct ts_kit_device_data *chip_data)
{
	int retval = NO_ERR;
	...
	retval = of_property_read_u32(np, "tp_status_report_support",
				      &tcm_hcd->tp_status_report_support);
	if (retval) {
		tcm_hcd->tp_status_report_support = 0;
	}
	TS_LOG_INFO("tp_status_report_support is %d",
		    tcm_hcd->tp_status_report_support);
	if (tcm_hcd->tp_status_report_support) {
		retval = of_property_read_u32_array(np, "report_priority",
						    &syna_report_priority[0],
						    BIT_MAX);
		if (retval) {
			TS_LOG_INFO(
				"device get report_priority failed, use default value\n");
		} else {
			int i = 0;
			for (i = 0; i < BIT_MAX; i++) {
		...
		retval = of_property_read_u32_array(
			np, "report_priority_limite",
			&syna_report_priority_limite[0], BIT_MAX);
		if (retval) {
			TS_LOG_INFO(
	...
	retval = of_property_read_u32(np, "aft_wxy_enable",
				      &tcm_hcd->aft_wxy_enable);
	if (retval) {
	...
	retval = of_property_read_u32(np, "use_esd_report",
				      &tcm_hcd->use_esd_report);
	if (retval) {
	...
	retval = of_property_read_u32(np, "use_dma_download_firmware",
				      &tcm_hcd->use_dma_download_firmware);
	if (retval) {
	...
	if (tcm_hcd->use_dma_download_firmware) {
		retval = of_property_read_u32(
			np, "downmload_firmware_frequency",
			&tcm_hcd->downmload_firmware_frequency);
		if (retval) {
			tcm_hcd->downmload_firmware_frequency =
				SPI_DEFLAUT_SPEED;
		}
	...
	TS_LOG_INFO(
		"use_dma_download_firmware = %d downmload_firmware_frequency = %d\n",
		tcm_hcd->use_dma_download_firmware,
		tcm_hcd->downmload_firmware_frequency);
	...
	TS_LOG_INFO("retval = %d, spec_delay_after_lcd = %u\n", retval,
		    tcm_hcd->syna_spec_delay_after_lcd);
	retval = of_property_read_u32(
		np, "support_notice_aft_gesture_mode",
		&(tcm_hcd->syna_tcm_chip_data->support_notice_aft_gesture_mode));
	if (retval)
		tcm_hcd->syna_tcm_chip_data->support_notice_aft_gesture_mode =
			0;
	TS_LOG_INFO(
		"support_notice_aft_gesture_mode = %u\n",
		tcm_hcd->syna_tcm_chip_data->support_notice_aft_gesture_mode);
...
static int tskit_driver_init_chip(void)
{
	...
	if (TS_BUS_SPI ==
		...
		strncpy(tcm_hcd->syna_tcm_chip_data->chip_name,
		...
		strncpy(tcm_hcd->syna_tcm_chip_data->module_name, buf_proj_id,
			projectid_lenth);
		memcpy(tcm_hcd->tcm_mod_info.project_id_string, buf_proj_id,
		       projectid_lenth);
	...
	tee_tui_data.tui_special_feature_support =
		tcm_hcd->syna_tcm_chip_data->ts_platform_data->chip_data
			->tui_special_feature_support;
	if ((tee_tui_data.tui_special_feature_support & TP_TUI_NEW_IRQ_MASK) ==
	    TP_TUI_NEW_IRQ_SUPPORT) {
		tee_tui_data.tui_irq_num =
			chip_data->ts_platform_data->irq_gpio;
...
static int
touch_driver_sub_board_connect_check(struct ts_kit_platform_data *data)
{
	int gpio_value;
	int retval;
	int i;
...
static int tskit_driver_chip_detect(struct ts_kit_platform_data *data)
{
	...
	LOCK_BUFFER(tcm_hcd->in);
	retval = tskit_driver_alloc_mem(tcm_hcd, &tcm_hcd->in,
...
static int tskit_driver_roi_switch(struct ts_roi_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		retval = tskit_driver_set_dynamic_config(
			tcm_hcd, info->roi_control_addr, buf);
		if (retval < 0) {
		...
		if (0 == tcm_hcd->roi_enable_status) {
			memset(tcm_hcd->syna_tcm_roi_data, 0,
			       sizeof(tcm_hcd->syna_tcm_roi_data));
		}
		break;
	case TS_ACTION_READ:
...
static int tskit_driver_charger_switch(struct ts_charger_info *info)
{
	...
	switch (info->op_action) {
	case TS_ACTION_READ:
		retval = tskit_driver_get_dynamic_config(
			tcm_hcd, DC_CHARGER_CONNECTED, &buf);
		if (retval < 0) {
	...
	case TS_ACTION_WRITE:
		...
		if (tcm_hcd->tp_status_report_support &&
		    ...
		    !tcm_hcd->in_suspend_charge) {
			TS_LOG_INFO("change charge to no");
			syna_dmd_charge_info.charge_CHARGER_NOISE_HOP =
				CHARGE_REPORT_NOT_REPORT;
			syna_dmd_charge_info.charge_CHARGER_NOISE_EX =
				CHARGE_REPORT_NOT_REPORT;
		}
		break;
	default:
...
static int tskit_driver_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_READ:
		retval = tskit_driver_get_dynamic_config(
			tcm_hcd, info->glove_switch_addr, &buf);
		if (retval < 0) {
...
static int tskit_driver_check_scene_type(unsigned int stype)
{
	...
	for (i = 0; i < tcm_hcd->syna_tcm_chip_data->scene_type_num; i++) {
		if (stype ==
		    tcm_hcd->syna_tcm_chip_data->scene_support_array[i])
			error = NO_ERR;
	}
	return error;
}
...
static void tskit_driver_scene_switch(unsigned char stype, unsigned char soper)
{
	...
	case TS_SWITCH_SCENE_ENTER:
		buf = stype;
		retval = tskit_driver_set_dynamic_config(
			tcm_hcd, tcm_hcd->syna_tcm_chip_data->touch_scene_reg,
			(unsigned short)buf);
		if (retval < 0)
			TS_LOG_ERR("%s, set_roi_switch failed\n", __func__);
...
static int tskit_driver_chip_get_touch_switch_data(unsigned int *stype,
						   unsigned int *soper,
						   unsigned int *para)
{
...
static void tskit_driver_chip_touch_switch(void)
{
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		if (stype == TS_SWITCH_SCENE_5) { /* game mode */
			if (soper == TS_SWITCH_GAME_ENABLE) {
				error = tskit_driver_set_dynamic_config(
					tcm_hcd, DC_GAME_MODE_CTRL,
					TS_GAME_TYPE);
				if (error < 0)
	...
	if (tcm_hcd->syna_tcm_chip_data->scene_type_mode == SCENE_MODE_ITOUCH) {
		/* itouch */
...
static int tskit_driver_status_resume(void)
{
	...
	struct ts_feature_info *info =
		&tcm_hcd->syna_tcm_chip_data->ts_platform_data->feature_info;
