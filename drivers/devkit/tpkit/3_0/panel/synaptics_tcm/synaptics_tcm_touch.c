						 ...
						 unsigned char *temp_data)
{
	...
	left_bits = bits;
	while (left_bits) {
		bits = (left_bits >= SYN_BYTE_COUNT) ? SYN_BYTE_COUNT :
						       left_bits;
...
static int touch_parse_report(void)
{
	...
	while (idx < config_size) {
		...
		case TOUCH_GESTURE_DATA:
			...
			retval = touch_get_report_data_morethan_32bits(
				offset, bits, sizeof(struct gesture_data),
				temp_data);
			if (retval < 0) {
				TS_LOG_ERR(
					"Failed to get number of gesture data\n");
				return retval;
			}
			offset += bits;
			TS_LOG_DEBUG(
				"Gesture Data: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
		...
		case TOUCH_ROI_DATA:
			...
			if (offset >=
			    (FIXED_READ_LENGTH - MESSAGE_HEADER_SIZE - 1) * 8) {
				TS_LOG_ERR("offset: %d is too big\n", offset);
				return -EINVAL;
			}
			touch_report_roi_data(&touch_report[offset / 8],
					      bits / 8);
			offset += bits;
			break;
		...
		case TOUCH_BASELINE_STATUS:
			bits = config_data[idx++];
			temp_data =
				(unsigned char *)&touch_data->baseline_status;
			retval = touch_get_report_data_morethan_32bits(
				offset, bits, sizeof(struct baseline_status),
				temp_data);
			if (retval < 0) {
				TS_LOG_ERR(
					"Failed to get number of baseline status\n");
				return retval;
			}
			offset += bits;
			break;
		case TOUCH_SIGNAL_INFO:
			...
			retval = touch_get_report_data_morethan_32bits(
				offset, bits, sizeof(struct signal_info),
				temp_data);
			if (retval < 0) {
				TS_LOG_ERR(
					"Failed to get number of signal info\n");
				return retval;
			}
			offset += bits;
			break;
...
void touch_report(struct ts_fingers *info)
{
	...
	if ((!touch_hcd) || (!touch_hcd->tcm_hcd) ||
	    (!touch_hcd->tcm_hcd->bdata)) {
	...
	if (tcm_hcd->in_suspend) {
		if (touch_data->gesture_id)
			TS_LOG_INFO(
				"in suspend,touch report in gesture_id = %d\n",
				touch_data->gesture_id);
	...
	if (SYNA_FW_SUPPORT_ESD == tcm_hcd->device_status_check) {
		touch_report_fw =
			(unsigned char)(tcm_hcd->device_status &
					0xff); /* If the INT is TOUCH, the value is 0x01, otherwise is 0x00. */
		esd_report =
			(unsigned char)(tcm_hcd->device_status >>
					8); /* If the INT is ESD, the value is 0x01, otherwise is 0x00.  */
	...
	/************************************************************************************************/
	if ((NEED_REPORT == touch_report_fw &&
	     SYNA_FW_SUPPORT_ESD == tcm_hcd->device_status_check) ||
		...
		for (idx = 0; idx < touch_hcd->max_objects; idx++) {
			...
			case GLOVED_FINGER:
				...
				if (bdata->x_flip)
					x = touch_hcd->tcm_hcd->bdata->x_max_mt -
					    x;
				if (bdata->y_flip)
					y = touch_hcd->tcm_hcd->bdata->y_max_mt -
					    y;
				if (tcm_hcd->bdata->support_vendor_ic_type ==
				    S3909) {
				...
				info->fingers[idx].yer =
					(object_data[idx].grip_data >> 24) &
					0xFF;
				info->fingers[idx].xer =
					(object_data[idx].grip_data >> 16) &
					0xFF;
				info->fingers[idx].ewy =
					(object_data[idx].grip_data >> 8) &
					0xFF;
				info->fingers[idx].ewx =
					(object_data[idx].grip_data >> 0) &
					0xFF;
				TS_LOG_DEBUG("Finger %d: ewx= %d, ewy= %d\n",
					     idx, info->fingers[idx].ewx,
					     info->fingers[idx].ewy);
				TS_LOG_DEBUG("Finger %d: xer= %d, yer= %d\n",
					     idx, info->fingers[idx].xer,
					     info->fingers[idx].yer);
				touch_count++;
				break;
			default:
		...
		memcpy(&info->fingers_debug_info.baseline_status,
		       &touch_data->baseline_status,
		       sizeof(struct baseline_status));
	...
	if (!last_touch_count && !touch_count && NEED_REPORT == esd_report &&
	    SYNA_FW_SUPPORT_ESD == tcm_hcd->device_status_check) {
...
static int touch_get_input_params(void)
{
	...
	retval = tskit_driver_alloc_mem(tcm_hcd, &tcm_hcd->config,
	...
	retval = tskit_driver_write_hdl_message(
		tcm_hcd, CMD_GET_TOUCH_REPORT_CONFIG, NULL, 0,
	...
	udelay(1000);
	retval = tskit_driver_read(tcm_hcd, report_config,
	...
	TS_LOG_INFO(
		"report_config = 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
	...
	tcm_hcd->config.data_length = report_config[2] |
				      (report_config[3] << 8);
...
static int tskit_driver_get_app_info(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (!app_info) {
		TS_LOG_ERR("Failed to allocate memory for app_info\n");
		return -ENOMEM;
	}
get_app_info:
	msleep(APP_STATUS_POLL_MS);
	retval = tskit_driver_write_hdl_message(tcm_hcd,
						CMD_GET_APPLICATION_INFO, NULL,
	...
	retval = tskit_driver_read(tcm_hcd, app_info, APP_INFO_LEN);
	if (retval < 0) {
		TS_LOG_ERR("Failed to read app_info\n");
		return retval;
	}
	TS_LOG_INFO("app_info = 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
		    app_info[0], app_info[1], app_info[2], app_info[3],
...
static int tskit_driver_get_id_info(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (!id_report) {
		TS_LOG_ERR("Failed to allocate memory for id_report\n");
		return -ENOMEM;
	}
	retval = tskit_driver_write_hdl_message(tcm_hcd, CMD_IDENTIFY, NULL, 0,
	...
	retval = tskit_driver_read(tcm_hcd, id_report, ID_REPORT_LEN);
	if (retval < 0) {
		TS_LOG_ERR("Failed to read id report\n");
		return retval;
	}
	if (TS_BUS_SPI ==
		...
		if (id_report[0] != MESSAGE_MARKER ||
		    (id_report[1] != STATUS_OK &&
		     id_report[1] != REPORT_IDENTIFY)) {
			TS_LOG_ERR("id_report:%x  %x %x %x %x\n", id_report[0],
				   id_report[1], id_report[2], id_report[3],
				   id_report[4]);
			return -EINVAL;
		}
	}
...
int tskit_driver_enable_touch(struct syna_tcm_hcd *tcm_hcd, bool en)
{
	...
	retry = 3;
	retval = tskit_driver_write_hdl_message(tcm_hcd, cmd, &payload, 1,
						&resp_buf, &resp_buf_size,
						&resp_length, NULL, 0);
	...
	while (retry) {
		...
		retval = tskit_driver_read(tcm_hcd, response_buf,
		...
		if (tcm_hcd->bdata &&
			...
			if (response_buf[1] == REPORT_TOUCH) {
				...
				if ((retval < 0) ||
					...
					TS_LOG_ERR(
						"Failed to response_temp_buf data\n");
					goto exit;
				}
				continue;
			}
	...
	while (retry) {
		udelay(1000);
		retval =
			tskit_driver_read(tcm_hcd, resp_buf, FIXED_READ_LENGTH);
		if (retval < 0) {
			TS_LOG_ERR("Failed to read id report\n");
			goto exit;
		}
