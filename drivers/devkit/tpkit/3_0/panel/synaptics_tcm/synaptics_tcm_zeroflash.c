...
int tskit_driver_write_hdl_message(
...
static int zeroflash_check_uboot(void)
{
	...
	retval = tskit_driver_rmi_read(tcm_hcd, PDT_END_ADDR, &fn_number,
				       sizeof(fn_number));
	if (retval < 0) {
	...
	retval = tskit_driver_rmi_read(tcm_hcd, PDT_START_ADDR,
...
static int zeroflash_parse_fw_image(void)
{
	...
	for (idx = 0; idx < num_of_areas; idx++) {
		...
		if (0 == strncmp((char *)descriptor->id_string, BOOT_CONFIG_ID,
			...
			TS_LOG_INFO(
				"Application config size = %d  image_info->packrat_number = %d\n",
...
int zeroflash_get_fw_image(char *file_name)
{
	...
	retval = request_firmware(&zeroflash_hcd->fw_entry, fw_name,
...
static int zeroflash_download_disp_config(void)
{
	...
	retval = tskit_driver_alloc_mem(tcm_hcd, &zeroflash_hcd->out,
	...
	zeroflash_hcd->out.buf[1] =
		HDL_DISPLAY_CONFIG_TO_PMEM; // 4320 ramless, write to PMEM //HDL_DISPLAY_CONFIG_TO_RAM;  HDL_DISPLAY_CONFIG_TO_PMEM
	if (image_info->disp_config.size > zeroflash_hcd->out.buf_size - 2) {
		TS_LOG_ERR("Failed to copy display config data\n");
		retval = -EINVAL;
		goto unlock_out;
	}
	memcpy(&zeroflash_hcd->out.buf[2], image_info->disp_config.data,
...
static int zeroflash_download_app_config(void)
{
	...
	memcpy(&zeroflash_hcd->out.buf[2], image_info->app_config.data,
...
static int zeroflash_download_app_fw(void)
{
	...
	memcpy(zeroflash_hcd->out.buf, image_info->app_firmware.data,
	       image_info->app_firmware.size);
...
int check_report_status(void)
{
	...
	while (retry) {
		retval = tskit_driver_read(zeroflash_hcd->tcm_hcd,
					   status_report, STATUS_REPORT_LEN);
		if (retval < 0) {
			TS_LOG_ERR("Failed to read status report\n");
			return retval;
		}
		TS_LOG_INFO(
			"status_report = 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
			status_report[0], status_report[1], status_report[2],
			status_report[3], status_report[4], status_report[5]);
		if (status_report[1] != REPORT_STATUS &&
		    status_report[1] != STATUS_OK) {
			retry--;
		} else {
			break;
		}
	}
	if ((!retry) && (status_report[1] != REPORT_STATUS &&
			 status_report[1] != STATUS_OK)) {
...
int zeroflash_download(char *file_name, struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->use_dma_download_firmware) {
		...
		tcm_hcd->spi_comnunicate_frequency =
