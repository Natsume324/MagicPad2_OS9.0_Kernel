...
static int
touch_driver_cap_test_for_newformat(struct ts_rawdata_info_new *info);
static int touch_driver_delta_test(struct ts_rawdata_info_new *info,
				   int *rawdata);
...
static int tskit_driver_testing_continued_read(struct syna_tcm_hcd *tcm_hcd,
					       unsigned char *buf,
					       ...
					       unsigned int payload_length)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval =
			tskit_driver_read(tcm_hcd, read_temp_buf,
...
static inline int touch_driver_testing_read(struct syna_tcm_hcd *tcm_hcd,
		...
		if ((data_buf[0] == MESSAGE_MARKER) &&
			...
			if (payload_length > 0) {
				retval = tskit_driver_testing_continued_read(
					tcm_hcd, data_buf, buf_length,
					payload_length);
				if (retval < 0) {
					TS_LOG_ERR(
						"Failed to read the data payload\n");
					goto exit;
				}
			}
			break;
...
static void touch_driver_read_csvfile(unsigned int rows, unsigned int cols,
				      char *file_path)
{
	int ret;
	...
	ret = ts_kit_parse_csvfile(file_path,
				   ...
				   threshold->self_cap_raw_data_col_min_limits,
				   rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			    CSV_SELF_CAP_RAW_DATA_COL_MIN_ARRAY);
	ret = ts_kit_parse_csvfile(file_path,
				   ...
				   threshold->self_cap_raw_data_col_max_limits,
				   rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			    CSV_SELF_CAP_RAW_DATA_COL_MAX_ARRAY);
	ret = ts_kit_parse_csvfile(file_path,
				   ...
				   threshold->self_cap_raw_data_row_min_limits,
				   rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			    CSV_SELF_CAP_RAW_DATA_ROW_MIN_ARRAY);
	ret = ts_kit_parse_csvfile(file_path,
				   ...
				   threshold->self_cap_raw_data_row_max_limits,
				   rows, cols);
	if (ret)
		TS_LOG_INFO("%s: Failed get %s\n", __func__,
			    CSV_SELF_CAP_RAW_DATA_ROW_MAX_ARRAY);
	...
	if (ret) {
		TS_LOG_INFO("%s: Failed get %s \n", __func__,
			    CSV_RAW_DATA_MAX_ARRAY);
	}

	ret = ts_kit_parse_csvfile(file_path, CSV_OPEN_SHORT_MIN_ARRAY,
				   threshold->open_short_min_limits, rows,
				   cols);
	if (ret) {
		TS_LOG_INFO("%s: Failed get %s \n", __func__,
			    CSV_OPEN_SHORT_MIN_ARRAY);
	}

	ret = ts_kit_parse_csvfile(file_path, CSV_OPEN_SHORT_MAX_ARRAY,
				   threshold->open_short_max_limits, rows,
				   cols);
	if (ret) {
		TS_LOG_INFO("%s: Failed get %s \n", __func__,
			    CSV_OPEN_SHORT_MAX_ARRAY);
	}
...
static void touch_driver_get_thr_from_csvfile(void)
{
	...
	factory_extra_cmd = &tcm_hcd->syna_tcm_chip_data->ts_platform_data
				     ->factory_extra_cmd;
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type == S3909) {
		...
		snprintf(file_name, (MAX_STR_LEN * 4), "%s_%s_%s_limits.csv",
			 ts_platform_data->product_name, SYNAPTICS_VENDER_NAME,
			 tcm_hcd->tcm_mod_info.project_id_string);
	}
				      ...
				      unsigned int data_size)
{
	...
	case TOUCH_DATA_XY_SWAP: /* clockwise 90D */
		for (i = 0; i < rows; i++) {
			for (j = 0; j < cols; j++) {
				ptr[(i * cols + j) * 2 + 0] =
					temp_buf[(rows * j + i) * 2 + 0];
				ptr[(i * cols + j) * 2 + 1] =
					temp_buf[(rows * j + i) * 2 + 1];
			}
		}
		break;
	default:
	...
	case TOUCH_DATA_XY_SWAP:
		for (i = 0; i < cols; i++) {
			for (j = 0; j < bytes_size; j++)
				ptr[i * bytes_size + j] =
					temp_buf[(rows + i) * bytes_size + j];
		}
		for (i = 0; i < rows; i++) {
			for (j = 0; j < bytes_size; j++)
				ptr[(cols + i) * bytes_size + j] =
					temp_buf[i * bytes_size + j];
		}
		break;
	default:
		TS_LOG_ERR("Flip Type %d, not support\n", flip_type);
...
static void tskit_driver_get_frame_size_words(unsigned int *size,
					      bool image_only)
{
	unsigned int rows = 0;
	unsigned int cols = 0;
...
static void tskit_driver_put_device_info(struct ts_rawdata_info_new *info)
{
	...
	if (tcm_hcd->bdata && tcm_hcd->bdata->support_vendor_ic_type != S3909) {
		...
		strncat(info->deviceinfo,
			tcm_hcd->tcm_mod_info.project_id_string, len);
	} else {
...
static int tskit_driver_open_short_test(struct ts_rawdata_info_new *info)
{
	...
	pts_node->values =
		kzalloc(open_short_data_size * sizeof(int), GFP_KERNEL);
	if (!pts_node->values) {
	...
	temp_buf =
		kzalloc(((rows * cols + MESSAGE_HEADER_SIZE) * 2), GFP_KERNEL);
	if (!temp_buf) {
		TS_LOG_ERR("%s Failed to allocate memory for temp_buf\n",
			   __func__);
		testresult = CAP_TEST_FAIL_CHAR;
	...
	retval = tskit_driver_write_hdl_message(
		tcm_hcd, CMD_PRODUCTION_TEST, test_hcd->out.buf, 1,
	...
	idx = 0;
	open_short_data_max = open_short_data_min =
		le2_to_uint(&temp_buf[MESSAGE_HEADER_SIZE]);
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			data = (signed short)le2_to_uint(
				&temp_buf[idx * 2 + MESSAGE_HEADER_SIZE]);
			pts_node->values[idx] = (int)data;
			open_short_data_max = data > open_short_data_max ?
						      data :
						      open_short_data_max;
			open_short_data_min = data < open_short_data_min ?
						      data :
						      open_short_data_min;
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			...
			if (data > threshold->open_short_max_limits[idx] ||
				...
				TS_LOG_ERR(
					"%s overlow_data = %d, row = %d, col = %d , limits [%d, %d]\n",
					__func__, data, row, col,
					threshold->open_short_min_limits[idx],
					threshold->open_short_max_limits[idx]);
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			data = (signed short)le2_to_uint(
				&temp_buf[idx * 2 + MESSAGE_HEADER_SIZE]);
			pts_node->values[idx] = (int)data;
			noise_data_max = data > noise_data_max ? data :
								 noise_data_max;
			noise_data_min = data < noise_data_min ? data :
								 noise_data_min;
			noise_data_avg += data;
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			...
			if (data > threshold->lcd_noise_max_limits[idx]) {
				TS_LOG_ERR(
					"%s overlow_data = %d, limits = %d, row = %d, col = %d\n",
					__func__, data,
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			data = (short)le2_to_uint(
				&temp_buf[idx * 2 + MESSAGE_HEADER_SIZE]);
			pts_node->values[idx] = (int)data;
			raw_data_max = data > raw_data_max ? data :
							     raw_data_max;
			raw_data_min = data < raw_data_min ? data :
							     raw_data_min;
			raw_data_avg += data;
			idx++;
		}
...
static int tskit_driver_delta_test(struct ts_rawdata_info_new *info,
				   int *rawdata)
{
	int retval = 0;
	unsigned int rows = 0;
	...
	for (i = 0, j = 0; i < rows * cols; i++) {
		if (i < cols) {
			...
			pts_node->values[j] = tskit_driver_abs(
				rawdata[i] - rawdata[i - cols]);
...
static int tskit_driver_abs_raw_cap_test(struct ts_rawdata_info_new *info)
{
	...
	temp_buf = kzalloc(((rows + cols) * 4 + MESSAGE_HEADER_SIZE + 1),
			   GFP_KERNEL);
	if (!temp_buf) {
		TS_LOG_ERR("%s Failed to allocate memory for temp_buf\n",
			   __func__);
	...
	if ((temp_buf[1] != STATUS_OK) ||
		...
		TS_LOG_ERR(
			"not correct status or data_length status:%x  length:%x\n",
			temp_buf[1], data_length);
		retval = -EINVAL;
		goto exit;
	}
	/* flip the abs data */
	touch_driver_flip_abs_data(&temp_buf[MESSAGE_HEADER_SIZE], data_length,
				   4);
	...
	for (col = 0; col < cols; col++) {
		temp_buf_col_int_p[col] =
			(unsigned int)(temp_buf[idx] & 0xff) |
		...
		if (temp_buf_col_int_p[col] <
			...
			retval = -EINVAL;
			TS_LOG_ERR(
				"fail at col=%-2d data = %d (0x%2x 0x%2x 0x%2x 0x%2x), limit = [%d,%d]\n",
				col, temp_buf_col_int_p[col], temp_buf[idx],
				temp_buf[idx + 1], temp_buf[idx + 2],
				temp_buf[idx + 3],
				threshold->raw_data_col_min_limits[col],
				threshold->raw_data_col_max_limits[col]);
		}
	...
	for (row = 0; row < rows; row++) {
		...
		if (temp_buf_row_int_p[row] <
			...
			retval = -EINVAL;
			TS_LOG_ERR(
				"fail at row=%-2d data = %d (0x%2x 0x%2x 0x%2x 0x%2x), limit = [%d, %d]\n",
				row, temp_buf_row_int_p[row], temp_buf[idx],
				temp_buf[idx + 1], temp_buf[idx + 2],
				temp_buf[idx + 3],
				threshold->raw_data_row_min_limits[row],
				threshold->raw_data_row_max_limits[row]);
	...
	memcpy(&(pts_node->values[rows]), temp_buf_col_int_p,
	       cols * sizeof(int));
			  ...
			  struct ts_cmd_node *out_cmd)
{
	...
	if (tcm_hcd->htd == true) {
		...
		if (tcm_hcd->syna_tcm_chip_data->ts_platform_data->bops->btype ==
		    TS_BUS_SPI) {
...
static void judge_pin(unsigned char *trx, int py_pin, bool *flag, int *pt_flag,
		      short rx, short *rxp)
{
	int i = 0;
	...
	if ((py_p == 0) || (py_p == 1) || (py_p == SYNA_PIN_32) ||
	    (py_p == SYNA_PIN_33)) {
...
static bool testing_get_pins_assigned(unsigned short pin, short *tx_pins,
				      short tx_assigned, short *rx_pins,
				      short rx_assigned, short *g_pins,
				      short guard_assigned)
{
	int i;
...
static void testing_pins_mapping_trgm(unsigned char *cfg_data,
				      unsigned int cfg_data_len, int nu_tx_g,
				      int nu_rx_g, int j)
{
	int i;
	int idx = 0;
				     ...
				     unsigned int cfg_data_len)
{
	...
	testing_pins_mapping_trgm(cfg_data, cfg_data_len, nu_tx_g, nu_rx_g, j);
}
				...
				unsigned int cfg_data_len)
{
	...
	if (cfg_data_len > (of_tx + len_tx)) {
		...
		for (i = 0; i < (len_tx / 2); i++) {
			test_hcd->tx_pins[i] =
				(short)cfg_data[of_tx + idx] |
				(short)(cfg_data[of_tx + idx + 1] << 8);
			idx += 2;
			TS_LOG_INFO("i = %d, tx = %2d", i,
				    test_hcd->tx_pins[i]);
		}
	}
	...
	if (cfg_data_len > of_rx + len_rx) {
		...
		for (i = 0; i < (len_rx / 2); i++) {
			test_hcd->rx_pins[i] =
				(short)cfg_data[of_rx + idx] |
				(short)(cfg_data[of_rx + idx + 1] << 8);
			idx += 2;
			TS_LOG_INFO("i = %d, rx = %2d", i,
				    test_hcd->rx_pins[i]);
		}
	}
...
static int testing_get_static_config(unsigned char *buf, unsigned int buf_len)
{
	...
	retval = tskit_driver_write_hdl_message(
		tcm_hcd, CMD_GET_STATIC_CONFIG, NULL, 0, &test_hcd->resp.buf,
	...
	if ((g_temp_buf[0] != MESSAGE_MARKER) || (g_temp_buf[1] != STATUS_OK)) {
		TS_LOG_ERR("message error %x, %x %x %x\n", g_temp_buf[0],
			   g_temp_buf[1], g_temp_buf[2], g_temp_buf[3]);
		retval = -EINVAL;
		goto exit;
	}
...
static int tskit_driver_trx_trx_short_test(struct ts_rawdata_info_new *info)
{
	...
	/* get pins mapping */
	if ((test_hcd->tx_assigned <= 0) || (test_hcd->rx_assigned <= 0) ||
	    (test_hcd->guard_assigned <= 0)) {
		if (satic_cfg_buf) {
			...
			TS_LOG_ERR("tx = %d, rx = %d, guard = %d",
				   test_hcd->tx_assigned, test_hcd->rx_assigned,
	...
	memcpy(pt1_data, &g_temp_buf[MESSAGE_HEADER_SIZE], pt1_data_size);
	...
	for (i = 0; i < pt1_data_size; i++) {
		...
		for (j = 0; j < 8; j++) {
			py_p = (i * 8 + j);
			do_pin_test = testing_get_pins_assigned(
				py_p, tx_pins, tx_assigned, rx_pins,
				rx_assigned, g_pins, guard_assigned);
			if (do_pin_test) {
				if (CHECK_BIT(pt1_data[i], j) == 0)
					TS_LOG_DEBUG("pin-%2d : pass\n", py_p);
				else /* check pin-0, 1, 32, 33 */
					judge_pin(g_trx_result, py_p, &is_rx,
						  &failure_cnt_pt1, rx_assigned,
						  rx_pins);
			} /* end if(do_pin_test) */
		}
	}
...
static int touch_driver_self_cap_raw_data_test(struct ts_rawdata_info_new *info,
					       unsigned char *self_raw_data)
{
	int retval;
	unsigned int idx;
	unsigned int row;
	...
	if (tcm_hcd->id_info.mode != MODE_APPLICATION ||
	    tcm_hcd->app_status != APP_STATUS_OK) {
		TS_LOG_ERR(
			"%s Application firmware not running mode = %d, app_status = %d\n",
			__func__, tcm_hcd->id_info.mode, tcm_hcd->app_status);
		return -ENODEV;
	}
	...
	for (col = 0; col < cols; col++) {
		...
		if (col == 0) {
			...
			if (cap_raw_data_max < temp_buf_col_int_p[col])
				cap_raw_data_max = temp_buf_col_int_p[col];
			cap_raw_data_avg =
				(temp_buf_col_int_p[col] + cap_raw_data_avg);
		}
		if (temp_buf_col_int_p[col] >
			    threshold->self_cap_raw_data_col_max_limits[col] ||
		    temp_buf_col_int_p[col] <
			...
			retval = -EINVAL;
			TS_LOG_ERR(
				"%s fail at col=%-2d data = %d 0x%2x 0x%2x, limit min = [%d] max = [%d]\n",
				__func__, col, temp_buf_col_int_p[col],
	...
	for (row = 0; row < rows; row++) {
		...
		if (temp_buf_row_int_p[row] >
			...
			retval = -EINVAL;
			TS_LOG_ERR(
				"%s fail at row=%-2d data = %d 0x%2x 0x%2x, limit min = [%d] max = [%d]\n",
				__func__, row, temp_buf_row_int_p[row],
				temp_buf[idx], temp_buf[idx + 1],
				threshold->self_cap_raw_data_row_min_limits[row],
				threshold->self_cap_raw_data_row_max_limits[row]);
	...
	strncpy(pts_node->test_name, "Self_Cap_Rawdata",
		TS_RAWDATA_TEST_NAME_LEN - 1);
	snprintf(pts_node->statistics_data, TS_RAWDATA_STATISTICS_DATA_LEN,
		 "[%d,%d,%d]", cap_raw_data_min, cap_raw_data_max,
		 cap_raw_data_avg);
	if (testresult == CAP_TEST_FAIL_CHAR)
		strncpy(pts_node->tptestfailedreason, failedreason,
			TS_RAWDATA_FAILED_REASON_LEN - 1);
...
static int touch_driver_cap_raw_data_test(struct ts_rawdata_info_new *info)
{
	int retval;
	...
	retval = tskit_driver_write_hdl_message(
		tcm_hcd, CMD_ENABLE_REPORT, test_hcd->out.buf, 1,
		&test_hcd->resp.buf, &test_hcd->resp.buf_size,
		&test_hcd->resp.data_length, NULL, 0);
	if (retval < 0) {
		TS_LOG_ERR("Failed to write command %s\n",
	...
	test_hcd->out.buf[0] = REPORT_199;
	retval = tskit_driver_write_hdl_message(
		tcm_hcd, CMD_DISABLE_REPORT, test_hcd->out.buf, 1,
		&test_hcd->resp.buf, &test_hcd->resp.buf_size,
		&test_hcd->resp.data_length, NULL, 0);
	if (retval < 0) {
		TS_LOG_ERR("Failed to write command %s\n",
	...
	if ((retval < 0) || (resp_buf[1] != STATUS_OK)) {
		if (resp_buf[1] == REPORT_199) {
			for (i = 0; i < 10;
			     i++) { /* 10: testing read retry 10 times max */
				retval = touch_driver_testing_read(
					tcm_hcd, resp_buf, MAX_FRAME_BUFFER);
				if (retval == 0 || resp_buf[1] == STATUS_OK)
					break;
			}
		} else {
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			data = (short)le2_to_uint(
				&temp_buf[idx * 2 + MESSAGE_HEADER_SIZE]);
			pts_node->values[idx] = (int)data;
			cap_raw_data_max = data > cap_raw_data_max ?
						   data :
						   cap_raw_data_max;
			cap_raw_data_min = data < cap_raw_data_min ?
						   data :
						   cap_raw_data_min;
...
exit:
	...
	if (retval >= 0) {
		retval_selfcaptest = touch_driver_self_cap_raw_data_test(
			info, &temp_buf[idx * 2 + MESSAGE_HEADER_SIZE]);
		if (retval_selfcaptest)
			TS_LOG_INFO(
				"touch_driver_self_cap_raw_data_test failed = %d\n",
				retval_selfcaptest);
...
static int touch_driver_full_raw_cap_test(struct ts_rawdata_info_new *info)
{
	int retval;
	unsigned int idx;
	signed short data;
...
static int
touch_driver_noise_different_gear_test(struct ts_rawdata_info_new *info)
{
	int retval;
	unsigned int idx;
	...
	retval = test_hcd->tcm_hcd->set_dynamic_config(
		test_hcd->tcm_hcd, DC_INHIBIT_FREQUENCY_SHIFT, 1);
	if (retval < 0)
		TS_LOG_ERR("%s failed to set dc\n", __func__);

	for (gear_index = 0; gear_index < SYNC_TCM_CAP_TEST_GEAR_COUNT;
	     gear_index++) {
		if (gear_index == 0)
			gear = 0;
		...
		/* flip the 2D data */
		touch_driver_flip_2D_data(&temp_buf[MESSAGE_HEADER_SIZE],
					  data_length);

		idx = 0;
		value_index = gear_index * rows * cols;
		if (gear_index == 0)
			noise_data_max = noise_data_min = (int)le2_to_uint(
			...
			for (col = 0; col < cols; col++) {
				...
				if (data >
					...
					TS_LOG_ERR(
						"%s overlow_data = %d, limits = %d, row = %d, col = %d\n",
						__func__, data,
						threshold->lcd_noise_max_limits
							[idx],
						row, col);
					test_hcd->result = false;
					testresult = CAP_TEST_FAIL_CHAR;
...
exit:
	...
	/* set the gear to 0 */
	retval = test_hcd->tcm_hcd->set_dynamic_config(
		test_hcd->tcm_hcd, DC_REQUESTED_FREQUENCY, 0);
	if (retval < 0)
		TS_LOG_ERR("%s failed to switch the frequency\n", __func__);
	/* set the frequency shift to 0 */
	retval = test_hcd->tcm_hcd->set_dynamic_config(
		test_hcd->tcm_hcd, DC_INHIBIT_FREQUENCY_SHIFT, 0);
	if (retval < 0)
...
static unsigned int touch_driver_thp_check_short(void *pdata, unsigned int rows,
						 unsigned int cols)
{
	unsigned short i;
	unsigned short j;
...
static int touch_driver_predecode_test(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (retval < 0) {
			TS_LOG_ERR(
				"Failed to write command %s with TEST_PREDECODE_RAW\n",
				STR(CMD_PRODUCTION_TEST));
			goto exit;
		}
...
static int touch_driver_trx_trx_short_test(struct ts_rawdata_info_new *info)
{
	int retval = NO_ERR;
	int i;
	int j;
	...
	retval = touch_driver_testing_read(
		tcm_hcd, g_temp_buf, (G_TEMP_BUF_LEN * sizeof(*g_temp_buf)));
	if (retval < 0 || (g_temp_buf[1] != STATUS_OK)) {
		TS_LOG_ERR("status = 0x%02x", g_temp_buf[1]);
...
static int touch_driver_abs_raw_cap_test(struct ts_rawdata_info_new *info)
{
	int retval;
	unsigned int idx;
	unsigned int data_length;
...
static int touch_driver_abs_noise_cap_test(struct ts_rawdata_info_new *info)
{
	int retval;
	unsigned int idx;
	unsigned int data_length;
	...
	if ((temp_buf[1] != STATUS_OK) ||
		...
		TS_LOG_ERR(
			"%s not correct status or data_length status:%x  length:%x\n",
	...
	for (col = 0; col < cols; col++) {
		...
		if (col == 0) {
			...
			if (noise_data_max < temp_buf_col_int_p[col])
				noise_data_max = temp_buf_col_int_p[col];
			noise_data_avg =
				(temp_buf_col_int_p[col] + noise_data_avg);
		}
		if (temp_buf_col_int_p[col] >
		    threshold->abs_noise_col_max_limits[col]) {
			test_hcd->result = false;
			...
			retval = -EINVAL;
			TS_LOG_ERR(
				"%s fail at col=%-2d data = %d 0x%2x 0x%2x, limit = [%d]\n",
				__func__, col, temp_buf_col_int_p[col],
				temp_buf[idx], temp_buf[idx + 1],
				threshold->abs_noise_col_max_limits[col]);
	...
	for (row = 0; row < rows; row++) {
		...
		if (temp_buf_row_int_p[row] >
			...
			retval = -EINVAL;
			TS_LOG_ERR(
				"%s fail at row=%-2d data = %d 0x%2x 0x%2x, limit = [%d]\n",
				__func__, row, temp_buf_row_int_p[row],
				temp_buf[idx], temp_buf[idx + 1],
				threshold->abs_noise_row_max_limits[row]);
...
static int touch_driver_i2c_communication_test(struct ts_rawdata_info_new *info)
{
	struct syna_tcm_hcd *tcm_hcd = test_hcd->tcm_hcd;

	if (tcm_hcd->id_info.mode != MODE_APPLICATION ||
	    tcm_hcd->app_status != APP_STATUS_OK) {
		snprintf(info->i2cinfo, TS_RAWDATA_RESULT_CODE_LEN, "%d%c",
			 RAW_DATA_TYPE_IC, CAP_TEST_FAIL_CHAR);
		strncat(info->i2cerrinfo, "software reason ",
			sizeof(info->i2cerrinfo));
		return -ENODEV;
	}
	snprintf(info->i2cinfo, TS_RAWDATA_RESULT_CODE_LEN, "%d%c",
		 RAW_DATA_TYPE_IC, CAP_TEST_PASS_CHAR);
	return 0;
}

static void touch_driver_cap_test_order(struct ts_rawdata_info_new *info)
{
	int retval;
...
static int touch_driver_cap_test_for_newformat(struct ts_rawdata_info_new *info)
{
	int retval;
	unsigned int rows;
	unsigned int cols;
...
int tskit_driver_cap_test_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (!read_temp_buf) {
		read_temp_buf = kzalloc(
			READ_TEMP_BUF_SIZE * sizeof(unsigned char), GFP_KERNEL);
		if (!read_temp_buf) {
			TS_LOG_ERR(
				"Failed to allocate memory for read_temp_buf\n");
			return -ENOMEM;
		}
	}
	if (!test_params) {
