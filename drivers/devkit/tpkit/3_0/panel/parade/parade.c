...
#define PutStringToReasonBuf(x) \
	memcpy(g_tp_test_failed_reason, x, (strlen(x) + 1))
...
static int
parade_hid_send_output_and_wait_(struct parade_core_data *cd,
...
static int _parade_fw_update_from_sdcard(u8 *file_path, bool update_flag);
...
static int
parade_wakeup_gesture_switch(struct ts_wakeup_gesture_enable_info *info);
...
static char g_tp_test_failed_reason[PARADE_MMI_TEST_FAILED_REASON_LEN] = {
	"-unknown"};
static char g_buf_parademmitest_result[PARADE_MMI_RESULT_BUF_LEN] = {
	0}; /* store mmi test result */
...
#define HID_OUTPUT_APP_COMMAND(command) \
	.cmd_type = HID_OUTPUT_CMD_APP, .command_code = command

#define HID_OUTPUT_BL_COMMAND(command) \
	.cmd_type = HID_OUTPUT_CMD_BL, .command_code = command
		      ...
		      const char *data_name)
{
	...
	*p = 0;
	for (i = 0; i < buf_len; i++) {
		 ...
		 */
		if ((cd->touch_report_with_grip == true) &&
		    (i > (TOUCH_INPUT_HEADER_SIZE - 1)) &&
		    (((i - TOUCH_INPUT_HEADER_SIZE + 1) %
			      TOUCH_REPORT_SIZE_EXT ==
		      0) ||
		     ((i - TOUCH_INPUT_HEADER_SIZE + 2) %
			      TOUCH_REPORT_SIZE_EXT ==
		      0))) {
			continue;
		}
...
static void pt_pr_buf(u8 *dptr, int size, const char *data_name)
{
	u8 *pr_buf = NULL;
	int i = 0;
	int k = 0;
...
static int
parade_oem_hid_send_output_and_wait(struct parade_oem_data *parade_oem_data,
				    struct parade_hid_output *hid_output)
{
	int ret = 0;
 ...
 */
static int
parade_oem_write_nv_any_data_(struct parade_oem_data *parade_oem_data, u8 buf[],
			      int offset, int length)
{
	u8 *response = NULL;
	...
	length = (length + cmd_offset + PT_SECURITY_KEY_LENGTH + 2) >
				 PT_MAX_INPUT ?
			 (PT_MAX_INPUT - cmd_offset - PT_SECURITY_KEY_LENGTH -
			  2) :
			 length; // 2, crc bytes
	memcpy(&write_buf[cmd_offset], buf, length);

	cmd_offset += length;
	memcpy(&write_buf[cmd_offset], g_pt_security_key,
	       PT_SECURITY_KEY_LENGTH);
 ...
 */
static int
parade_oem_write_nv_one_block_(struct parade_oem_data *parade_oem_data,
			       u8 buf[], int block_num)
{
	int rc = 0;

	rc = parade_oem_write_nv_any_data_(
		parade_oem_data, buf, PARADE_OEM_FLASH_ROW_SIZE_MAX * block_num,
		PARADE_OEM_FLASH_ROW_SIZE_MAX);
	return rc;
}
				    ...
				    u8 buf[], int offset, int length)
{
	...
	for (i = PARADE_OEM_FLASH_ROW_SIZE_MAX * 2;
		...
		if (parade_oem_data->stored_flash_data[i] != buf[i]) {
			rc = parade_oem_write_nv_one_block_(parade_oem_data,
							    buf, 2);
			if (rc != 0) {
				TS_LOG_ERR(
					"%s write nv data failed, with block 2\n",
					__func__);
				rc = -1;
				goto exit;
			}
	...
	for (row_index = start_row; row_index < end_row; row_index++) {
		...
		if ((uLastRowChanged == 0) && (uThisRowChanged == 1)) {
			...
			rc = parade_oem_write_nv_any_data_(
				parade_oem_data, &buf[offset_start],
				offset_start, offset_end - offset_start);
			if (rc != 0) {
				TS_LOG_ERR(
					"%s write nv data failed, with block row index:%d\n",
					__func__, row_index);
				rc = -1;
				goto exit;
			}
			...
			if (buf[(row_index - start_row) *
			    ...
			    parade_oem_data->stored_flash_data
				    [row_index * PARADE_OEM_FLASH_ONE_ROW_SIZE +
				     column_index]) {
				TS_LOG_ERR(
					"%s error:read data and write data not consistent\n",
					__func__);
				rc = -1;
				goto exit;
			}
				   ...
				   u8 buf[], int offset, int length)
{
	...
	while (length > 0) {
		...
		if (length > PARADE_OEM_FLASH_ROW_SIZE_MAX) {
			cmd_buf[cmd_len++] =
				LOW_BYTE(PARADE_OEM_FLASH_ROW_SIZE_MAX);
			cmd_buf[cmd_len++] =
				HI_BYTE(PARADE_OEM_FLASH_ROW_SIZE_MAX);
		} else {
		...
		status =
			response[HID_OUTPUT_RESPONSE_READ_NVDATA_STATUS_OFFSET];
		if (status) {
			TS_LOG_ERR("%s read failed, offset=%d", __func__,
				   offset);
			goto exit;
		}
		...
		actual_read_len =
			(response[HID_OUTPUT_RESPONSE_READ_NVDATA_ACTUAL_LEN_HIGH_OFFSET]
			 << 8) |
			response[HID_OUTPUT_RESPONSE_READ_NVDATA_ACTUAL_LEN_LOW_OFFSET];
		memcpy(buf,
		       &response[HID_OUTPUT_RESPONSE_READ_NVDATA_DATA_OFFSET],
		       actual_read_len);
		if (actual_read_len % PARADE_OEM_FLASH_ONE_ROW_SIZE != 0) {
			TS_LOG_ERR(
				"%s actual read length not aligned with 16 bytes",
				__func__);
			goto exit;
		}
...
static int parade_get_nv_info(struct parade_oem_data *parade_oem_data)
{
	...
	info->flash_size =
		(response[HID_OUTPUT_RESPONSE_GET_NVINFO_FLASHSIZE_HIGH_OFFSET]
		 << 8) |
	...
	row_setting =
		response[HID_OUTPUT_RESPONSE_GET_NVINFO_ROW_SETTING_OFFSET];
...
static int parade_oem_init(struct parade_oem_data *parade_oem_data)
{
	...
	parade_oem_data->ops.hid_send_output_and_wait =
		parade_oem_hid_send_output_and_wait;
...
static void parade_print_NVstructure(struct ts_oem_info_param *info)
{
	...
	if (flash_size <= 0 ||
	    flash_size >=
		    (TS_CHIP_BUFF_MAX_SIZE - PARADE_OEMINFO_LEN_PER_LINE + 1)) {
...
static int parade_reconstruct_barcode(struct ts_oem_info_param *info)
{
	...
	type = info->buff[offset1 * PARADE_OEMINFO_LEN_PER_LINE +
			  TS_NV_STRUCTURE_DATA_TYPE_OFFSET];
	len = info->buff[offset1 * PARADE_OEMINFO_LEN_PER_LINE +
			 TS_NV_STRUCTURE_DATA_LEN_OFFSET];
	...
	if ((type == 0x00 && len == 0x00) || (type == 0xFF && len == 0xFF)) {
		copy_len = g_tp_type_cmd[TS_NV_STRUCTURE_DATA_LEN_OFFSET] *
			   PARADE_OEMINFO_LEN_PER_LINE;
		...
		TS_LOG_INFO(
			"%s Will write the data to info_buff, offset is %d \n",
			__func__, offset1);
		return retval;
	}

	type = info->buff[offset2 * PARADE_OEMINFO_LEN_PER_LINE +
			  TS_NV_STRUCTURE_DATA_TYPE_OFFSET];
	len = info->buff[offset2 * PARADE_OEMINFO_LEN_PER_LINE +
			 TS_NV_STRUCTURE_DATA_LEN_OFFSET];
	// check whether data is erased. some memory is erased to 0x00, others is erased to 0xFF.
	if ((type == 0x00 && len == 0x00) || (type == 0xFF && len == 0xFF)) {
		copy_len = g_tp_type_cmd[TS_NV_STRUCTURE_DATA_LEN_OFFSET] *
...
static int parade_reconstruct_brightness(struct ts_oem_info_param *info)
{
	...
	TS_LOG_INFO(
		"%s brightness data is full, could not write into the data\n",
		__func__);
	g_tp_result_info[TS_NV_STRUCTURE_RESULT_INFO] = TS_CHIP_NO_FLASH_ERROR;
	retval = -EINVAL;
	return retval;
...
static int
parade_reconstruct_brightness_whitepoint(struct ts_oem_info_param *info)
{
	int retval = NO_ERR;
...
static int parade_reconstruct_repair_recode(struct ts_oem_info_param *info)
{
	...
	for (; offset <= TS_NV_STRUCTURE_REPAIR_OFFSET5; ++offset) {
		type = info->buff[offset * PARADE_OEMINFO_LEN_PER_LINE +
				  TS_NV_STRUCTURE_DATA_TYPE_OFFSET];
		len = info->buff[offset * PARADE_OEMINFO_LEN_PER_LINE +
				 TS_NV_STRUCTURE_DATA_LEN_OFFSET];

		if ((type == 0x00 && len == 0x00) ||
		    (type == 0xFF && len == 0xFF)) {
			memcpy(&(info->buff[offset *
					    PARADE_OEMINFO_LEN_PER_LINE]),
			       g_tp_type_cmd,
			       g_tp_type_cmd[TS_NV_STRUCTURE_DATA_LEN_OFFSET] *
				       PARADE_OEMINFO_LEN_PER_LINE);
			TS_LOG_INFO(
				"%s Will write the data to info_buff, offset is %d\n",
				__func__, offset);
			...
			TS_LOG_INFO(
				"%s repaire recode is full, could not write into the data\n",
				__func__);
			g_tp_result_info[TS_NV_STRUCTURE_RESULT_INFO] =
				TS_CHIP_NO_FLASH_ERROR;
			retval = -EINVAL;
		}
	}
	return retval;
...
static int parade_reconstruct_NVstructure(struct ts_oem_info_param *info)
{
	...
	case TS_NV_STRUCTURE_BRI_WHITE:
		...
		if (retval != 0) {
			TS_LOG_ERR(
				"%s, parade_reconstruct_brightness_whitepoint failed\n",
				__func__);
		}
		break;
	case TS_NV_STRUCTURE_REPAIR:
		...
		if (retval != 0) {
			TS_LOG_ERR(
				"%s, parade_reconstruct_repair_recode failed\n",
				__func__);
		}
		break;
	default:
...
static int parade_get_NVstructure_index(struct ts_oem_info_param *info, u8 type)
{
	...
	for (index = 1; index < (TS_NV_STRUCTURE_REPAIR_OFFSET5 + 1); ++index) {
		...
		tmp_len = info->buff[index * PARADE_OEMINFO_LEN_PER_LINE +
				     TS_NV_STRUCTURE_DATA_LEN_OFFSET];
		if (tmp_type == type &&
		    ts_oemdata_type_check_legal(tmp_type, tmp_len)) {
			latest_index = index;
			count += 1;
		}
	}
	...
	if (type == TS_NV_STRUCTURE_REPAIR) {
		...
		info->length =
			info->buff[latest_index * PARADE_OEMINFO_LEN_PER_LINE +
				   TS_NV_STRUCTURE_DATA_LEN_OFFSET];
	}
...
static int parade_get_oem_info(struct ts_oem_info_param *info)
{
	...
	if (latest_index > 0) {
		TS_LOG_INFO("%s type data find. len = %d\n", __func__,
			    info->length);
		if (info->length * PARADE_OEMINFO_LEN_PER_LINE >=
			    TS_CHIP_TYPE_MAX_SIZE ||
		    (info->length + latest_index) *
				    PARADE_OEMINFO_LEN_PER_LINE >
			    TS_CHIP_BUFF_MAX_SIZE) {
				      ...
				      int tx_diff_threshold)
{
	...
	for (i = 0; i < rx_num; i++) {
		if (ABS(cmcp_info->diff_self_data[i]) > rx_diff_threshold) {
			...
			TS_LOG_ERR(
				"%s: Rx self noise data-%d=%d breached threshold\n",
				__func__, (i + PT_SEE_IT_NEXT_ONE),
	...
	for (i = rx_num; i < (rx_num + tx_num); i++) {
		if (ABS(cmcp_info->diff_self_data[i]) > tx_diff_threshold) {
			...
			TS_LOG_ERR(
				"%s: Tx self noise data-%d=%d breached threshold\n",
				__func__, (i + PT_SEE_IT_NEXT_ONE - rx_num),
				cmcp_info->diff_self_data[i]);
			return;
		}
	}
...
static int parade_get_diff_data(void)
{
	...
	int iLoop =
		2; /* to capture Mutual diff and Self diff need two rounds */
	...
	while (iAveTimes > 0) {
		...
		while (iLoop > 0) {
			...
			if (iLoop == 1) {
				...
				elem = si->sensing_conf_data.electrodes_x *
			...
			if (elem > max_read_len) {
				rc = parade_ret_scan_data_cmd_(
					dev, elem_offset, max_read_len,
					dad->heatmap.data_type, NULL, &config,
					&actual_read_len, buf_offset);
			} else {
				rc = parade_ret_scan_data_cmd_(
					dev, elem_offset, elem,
					dad->heatmap.data_type, NULL, &config,
					&actual_read_len, buf_offset);
			}
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"%s parade_ret_scan_data_cmd_ failed\n",
					__func__);
				goto release_mutex;
			}
...
static int parade_get_project_id(struct ts_oem_info_param *info)
{
	...
	if (info->buff[0] !=
	    TS_CHIP_PROJECTID_TYPE) { /* product id is locates at first 10 bytes */
		memcpy(tskit_parade_data->project_id, info->buff,
		       TS_CHIP_PROJECTID_LENGTH);
	} else {
...
static int parade_get_brightness_info(void)
{
	...
	if ((tskit_parade_data->need_provide_projectID_for_sensor) &&
	    (tskit_parade_data->create_project_id_flag ==
	     1)) { // To generate projectiD for shutdown charging
	...
	if (atomic_read(&tskit_parade_data->parade_chip_data->ts_platform_data
				 ->state) == TS_UNINIT) {
	...
	info = (struct ts_oem_info_param *)kzalloc(
		sizeof(struct ts_oem_info_param), GFP_KERNEL);
	if (!info) {
	...
	if (tskit_parade_data->mode == PT_MODE_BOOTLOADER) {
		TS_LOG_ERR(
			"%s:In bootloader mode, such function does not support\n",
			__func__);
		rc = -EINVAL;
		goto out;
	}
	...
	if (info->data[TS_NV_STRUCTURE_DATA_TYPE_OFFSET] ==
	    TS_NV_STRUCTURE_DATA_UNDEFINED) {
...
static void parade_reset_pin_low(void)
{
	...
	gpio_set_value(tskit_parade_data->parade_chip_data->ts_platform_data
			       ->reset_gpio,
		       0);
}
					    ...
					    int size)
{
	...
	if (action) {
		TS_LOG_DEBUG(
			"[parade debug] i2c action write addr:0x%x, size:%d\n",
			addr, size);
	} else {
		TS_LOG_DEBUG(
			"[parade debug] i2c action read addr:0x%x, size:%d\n",
			addr, size);
	}
...
static int parade_i2c_read(u8 *reg_addr, u16 reg_len, u8 *buf, u16 len)
{
	...
	if (IS_NULL_PTR(reg_addr) || IS_NULL_PTR(buf) ||
	    IS_NULL_PTR(tskit_parade_data) ||
	    IS_NULL_PTR(tskit_parade_data->parade_chip_data) ||
	    IS_NULL_PTR(
		    tskit_parade_data->parade_chip_data->ts_platform_data) ||
	...
	if (parade_is_power_on) {
		rc = tskit_parade_data->parade_chip_data->ts_platform_data->bops
			     ->bus_read(reg_addr, reg_len, buf, len);
		if (rc < 0) {
...
static int i2c_communicate_check(void)
{
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		rc = parade_i2c_read(&reg_addr, sizeof(reg_addr), &mode,
				     sizeof(mode));
		if (rc < 0) {
			TS_LOG_ERR(
				"%s, Failed to read register 0x00, i = %d, rc = %d\n",
				__func__, i, rc);
			msleep(50); // ic need
		} else {
...
static int parade_check_and_deassert_int(struct parade_core_data *cd)
{
	...
	do {
		...
		if (size == PT_PIP_DEFAULT_EMPTY_BUF ||
		    size == PT_RESET_SENTINEL_BUF ||
				      ...
				      struct parade_hid_desc *desc)
{
	...
	if (IS_NULL_PTR(cd) || IS_NULL_PTR(cd->core_pdata) ||
	    IS_NULL_PTR(desc)) {
		return -EFAULT;
	}
...
static int parade_init_report_descriptor_(struct parade_core_data *cd)
{
	...
	si->desc.tch_record_size = cd->touch_report_with_grip ?
					   TOUCH_REPORT_SIZE_EXT :
					   TOUCH_REPORT_SIZE;
...
#define HID_BL_OUTPUT_INIT_LEN \
	11 /* LEN(2) + ID + RSVD + SOP + DATA_LEN(2) + CMD + CRC(2) + EOP */
				   ...
				   struct parade_hid_output *hid_output)
{
	...
	if (hid_output->write_length && hid_output->write_buf) {
		if (((cmd_allocated == true) &&
		     (hid_output->write_length + cmd_offset) >
...
static int
parade_hid_send_output_user_and_wait_(struct parade_core_data *cd,
				      struct parade_hid_output *hid_output)
{
	int rc = 0;
	int t = 0;
	...
	if (IS_TMO(t)) {
#if defined(CONFIG_HONOR_DSM)
		ts_dmd_report(DSM_TP_IC_TIMEOUT_ERROR_NO,
			      "parade output cmd execution timed out.\n");
#endif
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&dad->sysfs_lock);
	index = scnprintf(buf, PARADE_MAX_PRBUF_SIZE, "Status %d\n",
			  dad->status);
	if (dad->status == 0) {
		goto error;
	}
				 ...
				 size_t ic_buf_size)
{
	...
	TS_LOG_DEBUG("%s: pbuf=%pK buf=%pK size=%zu %s=%zu buf=%s\n", __func__,
		     pbuf, buf, buf_size, "scan buf size", PARADE_INPUT_ELEM_SZ,
		     buf);
...
static int
parade_hid_output_validate_bl_response(struct parade_core_data *cd,
				       struct parade_hid_output *hid_output)
{
	u16 size = 0;
	u16 crc = 0;
	...
	if (cd->response_buf[HID_OUTPUT_BL_RESPONSE_SOP_OFFSET] !=
	    HID_OUTPUT_BL_SOP) {
	...
	if (cd->response_buf[size - PARADE_BOOTLOADER_COMMAND_RESPONSE_LOW_CRC] !=
		    LOW_BYTE(crc) ||
...
static int
parade_hid_output_validate_app_response(struct parade_core_data *cd,
					struct parade_hid_output *hid_output)
{
	int command_code = 0;
	u16 size = 0;
...
static int
parade_hid_output_validate_response(struct parade_core_data *cd,
				    struct parade_hid_output *hid_output)
{
	int ret = 0;
	if (IS_NULL_PTR(cd) || IS_NULL_PTR(hid_output)) {
	...
	if (hid_output->cmd_type == HID_OUTPUT_CMD_BL) {
		...
		if (ret) {
			TS_LOG_ERR(
				"%s: parade_hid_output_validate_bl_response failed\n",
				__func__);
		}
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR(
			"%s, parade_hid_output_validate_app_response failed\n",
			__func__);
		return ret;
	}
	return NO_ERR;
...
static void parade_si_get_ttdata(struct parade_core_data *cd)
{
	...
	if (!cd || !tskit_parade_data || !tskit_parade_data->parade_chip_data) {
		TS_LOG_ERR("%s, param is null\n", __func__);
		return;
	}
	ttdata = &cd->sysinfo.ttdata;
	ttdata_dev = (struct parade_ttdata_dev *)&cd
			     ->response_buf[HID_SYSINFO_TTDATA_OFFSET];
	...
	memset(tskit_parade_data->parade_chip_data->version_name, 0,
	       MAX_STR_LEN);
	snprintf(tskit_parade_data->parade_chip_data->version_name,
		 MAX_STR_LEN - 1, "0x%04X", ttdata->fw_ver_conf);
}
...
static void parade_si_get_sensing_conf_data(struct parade_core_data *cd)
{
	...
	scd_dev = (struct parade_sensing_conf_data_dev *)&cd
...
static int parade_si_setup(struct parade_core_data *cd)
{
	...
	if (!si->xy_data)
		si->xy_data =
			kzalloc(max_tch * si->desc.tch_record_size, GFP_KERNEL);
	if (!si->xy_data) {
		return -ENOMEM;
	}
...
static int parade_si_get_btn_data(struct parade_core_data *cd)
{
	...
	si = &cd->sysinfo;
	btns = cd->response_buf[HID_SYSINFO_BTN_OFFSET] & HID_SYSINFO_BTN_MASK;
	...
	if (num_btns) {
		...
		if (cd->core_pdata->sett[PT_IC_GRPNUM_BTN_KEYS] == NULL) {
			num_defined_keys = 0;
		} else if (cd->core_pdata->sett[PT_IC_GRPNUM_BTN_KEYS]->data ==
			   NULL) {
			num_defined_keys = 0;
		} else
			num_defined_keys =
				cd->core_pdata->sett[PT_IC_GRPNUM_BTN_KEYS]
					->size;

		for (btn = 0; btn < num_btns && btn < num_defined_keys; btn++) {
			key_table = (u16 *)cd->core_pdata
					    ->sett[PT_IC_GRPNUM_BTN_KEYS]
					    ->data;
...
static int parade_before_suspend(void)
{
	...
	if (tskit_parade_data->show_before_suspend_flag ==
	    PT_SHOW_BEFORE_SUSPEND) {
	...
	if (g_ts_kit_platform_data.chip_data->isbootupdate_finish == false &&
		...
		enable_irq(tskit_parade_data->parade_chip_data->ts_platform_data
				   ->irq_id);
	}
				     ...
				     u8 power_state)
{
	 ...
	 */
	if ((cd->response_buf[HID_OUTPUT_RESPONSE_REPORT_OFFSET] !=
	     HID_RESPONSE_REPORT_ID) ||
	    ((cd->response_buf[HID_OUTPUT_RESPONSE_SET_POWER_STATE_OFFSET] &
	      0x3) != power_state) ||
	    ((cd->response_buf[HID_OUTPUT_RESPONSE_SET_POWER_OPCODE_OFFSET] &
	      0xF) != HID_CMD_SET_POWER)) {
		rc = -EINVAL;
	}
...
#if defined(CONFIG_HONOR_DSM)
		ts_dmd_report(
			DSM_TP_WAKEUP_ERROR_NO,
			"wakeup parade ts failed.\nfailed status is %d.\n",
			TS_WAKEUP_FAIL);
#endif
	}
	msleep(20); // ic need
...
static int parade_chip_wrong_touch(void)
{
	...
	tskit_parade_data->parade_chip_data->easy_wakeup_info.off_motion_on =
		true;
...
static void parade_enter_easy_wakeup(struct work_struct *work)
{
	...
	if (false == g_ts_kit_platform_data.chip_data->isbootupdate_finish) {
		TS_LOG_INFO(
			"%s MMI/fwupgrade is running, no need easywakeup.\n",
			__func__);
		parade_finish_cmd();
		return;
	}
	...
	if (rc || cd->response_buf[PT_APP_RETURN_RESPONSE_STATUS_BIT] == 0) {
		TS_LOG_ERR(
			"%s: Get in easy wakeup mode fail, rc = %d, status = %u",
			__func__, rc,
			cd->response_buf[PT_APP_RETURN_RESPONSE_STATUS_BIT]);
		parade_finish_cmd();
		return;
	}
...
static void parade_deep_sleep_work_fn(struct work_struct *work)
{
	...
	if (g_ts_kit_platform_data.chip_data->isbootupdate_finish == false) {
		TS_LOG_INFO(
			"%s MMI/fwupgrade is running, no need suspend/resume.\n",
			__func__);
		return;
	}
	if (g_sleep_resume_flag == POWER_DEEP_SLEEP) {
		...
		if (cd->parade_chip_data->easy_wakeup_info.sleep_mode !=
		    TS_GESTURE_MODE) {
		...
		if (rc) {
			TS_LOG_ERR(
				"%s: parade_core_wake_device_from_deep_sleep_\n",
				__func__);
		}
		parade_finish_cmd();
...
static int parade_core_suspend(void)
{
	...
	if (!tskit_parade_data || !tskit_parade_data->core_pdata ||
	...
	case TS_POWER_OFF_MODE:
		if (sleep_in_mode == 0) { // Deep Sleep
			...
			TS_LOG_INFO(
				"%s: power off device, current control by LCD\n",
				__func__);
			if (g_ts_kit_platform_data.chip_data
				    ->isbootupdate_finish) {
				/* Disable IRQ to avoid the glitch changes of LCD */
				disable_irq(tskit_parade_data->parade_chip_data
						    ->ts_platform_data->irq_id);
			}
		}
		break;
	case TS_GESTURE_MODE: /* gesture easy_wake_up mode. */
		if (cd->core_pdata->wakeup_gesture_enable == 0) {
			TS_LOG_ERR(
				"%s: disable easy_wakeup, enter deep sleep!!\n",
				__func__);
			g_sleep_resume_flag = POWER_DEEP_SLEEP;
			schedule_work(&parade_deep_sleep_work);
		} else {
			...
			mutex_lock(&cd->system_lock);
			tskit_parade_data->parade_chip_data->easy_wakeup_info
				.off_motion_on = true;
			mutex_unlock(&cd->system_lock);
	...
	if (sleep_in_mode &&
	    g_ts_kit_platform_data.chip_data->isbootupdate_finish != 0) {
...
static int parade_core_resume(void)
{
	...
	case TS_POWER_OFF_MODE:
		if (sleep_in_mode == 0) { // Deep Sleep
			...
			TS_LOG_INFO(
				"%s: power on device by LCD, start i2c seq\n",
				__func__);
			rc = parade_hw_reset();
			if (rc) {
				TS_LOG_ERR("%s: parade_hw_reset failed\n",
			...
			msleep(PARADE_DELAY_20MS);
			enable_irq(tskit_parade_data->parade_chip_data
					   ->ts_platform_data->irq_id);
		}
		break;
	case TS_GESTURE_MODE: /* gesture easy_wake_up mode. */
...
static int parade_after_resume(void *feature_info)
{
	...
	if (tskit_parade_data->no_need_after_resume_flag ==
	    PARADE_NO_NEED_DO_AFTER_RESUME) {
		TS_LOG_INFO("%s No need doing parade_after_resume\n", __func__);
		return retval;
	}
	if (si->ready) {
		tskit_parade_data->is_need_check_mode_before_status_resume =
			false;
		schedule_delayed_work(
			&parade_status_resume_work,
			msecs_to_jiffies(PARADE_AFTER_RESUME_TIMEOUT));
	} else {
		TS_LOG_ERR("%s sysinfo is not ready, may not success start up",
			   __func__);
	}
	return retval;
}
...
static void parade_status_resume_work_fn(struct work_struct *work)
{
	...
	cd = tskit_parade_data;
	info = &tskit_parade_data->parade_chip_data->ts_platform_data
			->feature_info;

	mutex_lock(&tskit_parade_data->parade_chip_data->device_call_lock);
	if (tskit_parade_data->is_need_check_mode_before_status_resume ==
	    true) {
	...
	if (info->holster_info.holster_supported) {
		retval = parade_set_holster_switch(
				    ...
				    int *parameter_out, int timeout)
{
	...
	case 1: /* 1-byte parameter,little endian */
		*parameter_out =
			cd->response_buf
				[HID_OUTPUT_RESPONSE_GET_PARAM_DATA_OFFSET];
		break;
...
static int parade_set_power_on_flag(struct parade_core_data *cd)
{
	...
	rc = parade_set_ram_parameter(cd, PT_RAM_ID_POWER,
				      RAM_PARA_POWER_ON_FLAG,
...
static int parade_set_glove_switch(u8 glove_switch)
{
	...
	rc = parade_set_ram_parameter(tskit_parade_data, PT_RAM_ID_TOUCH_MODE,
				      data, PT_HID_OUTPUT_SET_RAM_NORMAL_PARAM);
	return rc;
}
...
static int parade_set_holster_switch(u8 in_switch)
{
	...
	rc = parade_set_ram_parameter(tskit_parade_data, PT_RAM_ID_HOLSTER,
				      data, PT_HID_OUTPUT_SET_RAM_NORMAL_PARAM);
	return rc;
}
				       ...
				       struct ts_cmd_node *out_cmd)
{
	...
	if (rc) {
		TS_LOG_ERR(
			"%s parade_hid_send_output_and_wait_  for Mutual Cap failed\n",
			__func__);
		goto exit;
	}
	status = tskit_parade_data->response_buf
			 [HID_OUTPUT_RESPONSE_GET_DATA_STRUCTURE_STATUS_OFFSET];
	if (status) {
	...
	actual_read_len =
		(tskit_parade_data->response_buf
			 [HID_OUTPUT_RESPONSE_GET_DATA_STRUCTURE_DATALEN_HIGH_OFFSET]
		 << 8) |
		tskit_parade_data->response_buf
			[HID_OUTPUT_RESPONSE_GET_DATA_STRUCTURE_DATALEN_LOW_OFFSET];
	if ((actual_read_len + PARADE_RETRIEVE_DATA_STRUCTURE_RESPONSE_DATA >=
	     PT_MAX_INPUT) ||
	...
	memcpy(info->data,
	       &tskit_parade_data->response_buf
			[HID_OUTPUT_RESPONSE_GET_DATA_STRUCTURE_DATA_OFFSET],
	       actual_read_len);
	info->used_size = actual_read_len;
	info->tx_num =
		tskit_parade_data->sysinfo.sensing_conf_data.electrodes_x;
	info->rx_num =
		tskit_parade_data->sysinfo.sensing_conf_data.electrodes_y;
...
{
	TS_LOG_INFO("%s enter", __func__);
	tskit_parade_data->parade_chip_data->ts_platform_data->feature_info
		.roi_info.roi_switch = tskit_parade_data->roi_switch;
	return NO_ERR;
}
...
static int parade_roi_switch(struct ts_roi_info *info)
{
	...
	if (tskit_parade_data->show_before_suspend_flag ==
	    PT_SHOW_BEFORE_SUSPEND) {
		if (tskit_parade_data->avoid_roi_switch_flag ==
		    PT_AVOID_ROI_SWITCH) {
...
static void parade_chip_touch_switch(void)
{
...
#ifdef PARADE_DOZE_ENABLE
	if ((tskit_parade_data->parade_chip_data->touch_switch_flag &
	     TS_SWITCH_TYPE_DOZE) != TS_SWITCH_TYPE_DOZE) {
	...
	/* get switch operate */
#ifdef PARADE_DOZE_ENABLE
	memcpy(in_data, tskit_parade_data->parade_chip_data->touch_switch_info,
	       MAX_STR_LEN);
#else
...
static int parade_calibrate_child(void)
{
	...
	status = tskit_parade_data
			 ->response_buf[HID_OUTPUT_RESPONSE_CAL_STATUS_OFFSET];
	if (status) {
	...
	if (rc) {
		TS_LOG_ERR(
			"%s parade_hid_send_output_and_wait_ for Self Cap failed\n",
			__func__);
		goto exit;
	}
	status = tskit_parade_data
...
static int parade_hid_output_get_sysinfo_(struct parade_core_data *cd)
{
	...
	if (rc) {
		if (-EPROTO ==
		    rc) { /* Patch: Wait for the first chip initialization, just in case the chip has no info */
			...
			if (rc) {
				TS_LOG_ERR(
					"%s failed, with add 10s delay retry, rc:%d\n",
					__func__, rc);
				return rc;
			}
		} else {
					      ...
					      u8 ebid, u8 *read_buf, u16 *crc)
{
	...
	status = cd->response_buf
			 [HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_STATUS_OFFSET];
	if (status) {
		return -EINVAL;
	}

	read_ebid =
		cd->response_buf[HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_EBID_OFFSET];
	if ((read_ebid != ebid) ||
	    (cd->response_buf[HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_RSVD_OFFSET] !=
	     0)) {
		return -EPROTO;
	}

	read_length = get_unaligned_le16(
		&cd->response_buf
			 [HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_LEN_OFFSET]);
	if (length < read_length) {
		length = read_length;
	}

	length = (length > (PT_MAX_INPUT -
			    HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_DATA_OFFSET) ?
			  (PT_MAX_INPUT -
			   HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_DATA_OFFSET) :
			  length);
	memcpy(read_buf,
	       &cd->response_buf[HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_DATA_OFFSET],
	       length);
	*crc = get_unaligned_le16(
		&cd->response_buf
			 [read_length +
			  HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_CRC_OFFSET]);
					      ...
					      u8 *color_info)
{
	...
	rc = parade_hid_output_read_conf_block_(cd, PT_TPCOLOR_ROW,
						PT_TPCOLOR_READ_SIZE,
						PT_MDATA_EBID, read_buf, &crc);
	if (rc) {
		return rc;
	}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (rc < 0) {
		return scnprintf(
			buf, PAGE_SIZE,
			"%s:Get Run Time Sysinfo failed, Error Code: %d",
			__func__, rc);
	} else {
		return scnprintf(
			buf, PAGE_SIZE,
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
			 "%s: %d.%d\n"
			 "%s: %s\n",
			 "Driver Version", PARADE_DRV_VER_MAJOR,
			 PARADE_DRV_VER_MINOR, "Last Change Date",
			 PARADE_DRV_LAST_CHANGE_DATE);
}
				     ...
				     u8 *return_buf)
{
	...
	status = cd->response_buf
			 [HID_OUTPUT_RESPONSE_GET_PANEL_SCAN_STATUS_OFFSET];
	if (status) {
		return -EINVAL;
	}

	read_data_id =
		cd->response_buf[HID_OUTPUT_RESPONSE_GET_PANEL_SCAN_ID_OFFSET];
	...
	*actual_read_len = get_unaligned_le16(
		&cd->response_buf
			 [HID_OUTPUT_RESPONSE_GET_PANEL_SCAN_DATALEN_OFFSET]);
	*config =
		cd->response_buf
			[HID_OUTPUT_RESPONSE_GET_PANEL_SCAN_ELEMENT_SIZE_OFFSET];
	...
	if (return_buf &&
	    (data_size <=
	     (PT_MAX_INPUT - HID_OUTPUT_RESPONSE_GET_PANEL_SCAN_DATA_OFFSET))) {
		memcpy(return_buf,
		       &cd->response_buf
				[HID_OUTPUT_RESPONSE_GET_PANEL_SCAN_DATA_OFFSET],
		       data_size);
	}
						...
						loff_t *ppos)
{
	...
	if (elem > PT_MAX_ELEN) {
		rc = parade_ret_scan_data_cmd_(dev, elem_offset, PT_MAX_ELEN,
	...
	while (elem > 0) {
		...
		if (elem > PT_MAX_ELEN) {
			rc = parade_ret_scan_data_cmd_(
				dev, elem_offset, PT_MAX_ELEN,
				dad->heatmap.data_type, NULL, &config,
				&actual_read_len, buf_offset);
		} else {
			rc = parade_ret_scan_data_cmd_(
				dev, elem_offset, elem, dad->heatmap.data_type,
	...
	for (i = 0; i < length; i++) {
		print_idx += scnprintf(buf_out + print_idx,
				       TTHE_TUNER_MAX_BUF - print_idx, "%02X ",
				       dad->ic_buf[i]);
	}
	print_idx += scnprintf(buf_out + print_idx,
			       TTHE_TUNER_MAX_BUF - print_idx, ":(%d bytes)\n",
			       length);
			   ...
			   struct parade_device_access_data *dad)
{
	...
	dad->tthe_get_panel_data_debugfs =
		debugfs_create_file(PARADE_TTHE_TUNER_GET_PANEL_DATA_FILE_NAME,
...
static int parade_watchdog_work_check(void)
{
	...
	rc = parade_check_cmd_status();
	if (rc) {
		TS_LOG_ERR("%s: failed to check cmd status r=%d\n", __func__,
			   rc);
		return rc;
	}
...
reset:
	...
	if (cd->mode == PT_MODE_BOOTLOADER) {
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s: Error on launch app r=%d,FW May be Corrupted\n",
				__func__, rc);
...
static int parade_pinctrl_get_init(void)
{
	...
	tskit_parade_data->pctrl =
		devm_pinctrl_get(&tskit_parade_data->parade_dev->dev);
	...
	tskit_parade_data->pins_idle =
		pinctrl_lookup_state(tskit_parade_data->pctrl, "idle");
...
static int parade_get_lcd_panel_info(void)
{
	...
	strncpy(tskit_parade_data->lcd_panel_info, lcd_type,
...
static void get_current_distinguishability(void)
{
	char *cur = NULL;
	int ret = 0;
	tskit_parade_data->current_distinguishability =
		false; // false:default distinguishability
	...
	tskit_parade_data->current_distinguishability =
		true; // true :second distinguishability
	return;
}
static int get_lcd_module_name(void)
{
	...
	int i = 0;
	strncpy(temp, tskit_parade_data->lcd_panel_info,
		LCD_PANEL_INFO_MAX_LEN - 1);
...
static void parse_report_rate_check_support(struct device_node *core_node)
{
	...
	if (value <= PT_REPORT_RATE_UNSUP) {
		tskit_parade_data->report_rate_check_supported =
			PT_REPORT_RATE_UNSUP;
	} else {
		tskit_parade_data->report_rate_check_supported =
			PT_REPORT_RATE_SUP;
	}
	TS_LOG_INFO(
		"%s, tskit_parade_data->report_rate_check_supported = %d.\n",
		__func__, value);
}
...
void parse_check_irq_state(struct device_node *core_node)
{
	...
	if (value <= PT_CHECK_IRQ_STATE_FALSE) {
		tskit_parade_data->core_pdata->check_irq_state =
			PT_CHECK_IRQ_STATE_FALSE;
	} else {
		tskit_parade_data->core_pdata->check_irq_state =
			PT_CHECK_IRQ_STATE_TRUE;
	}
...
{
	tskit_parade_data->no_need_after_resume_flag = parade_get_dts_value(
		core_node, "parade,no_need_doing_after_resume");
...
{
	tskit_parade_data->calibrate_fw_update_and_cmcp = parade_get_dts_value(
		core_node, "parade,calibrate_fw_update_and_cmcp");
...
{
	tskit_parade_data->show_before_suspend_flag = parade_get_dts_value(
		core_node, "parade,show_before_suspend_flag");
...
static void parse_need_set_rst_after_iovcc_flag(struct device_node *core_node)
{
	...
	value = parade_get_dts_value(core_node,
				     "need_set_rst_after_iovcc_flag");
	if (value < 0) {
		TS_LOG_INFO(
			"%s, get device need_set_rst_after_iovcc_flag failed\n ",
			__func__);
...
static int hw_get_dts_value(struct device_node *core_node, char *name)
{
	...
	if (retval < 0) {
		TS_LOG_INFO("%s, get device %s failed\n", __func__, name);
		retval = 0; // default
	}
					 ...
					 struct ts_kit_device_data *chip_data)
{
	...
	value = parade_get_dts_value(core_node,
				     "need_provide_projectID_for_sensor");
	if (value < 0) {
		TS_LOG_INFO(
			"%s, get device need_provide_projectID_for_sensor failed\n",
			__func__);
		value = 0; // default
	}
...
static void
parse_need_check_report_descriptor_flag(struct device_node *core_node,
					struct ts_kit_device_data *chip_data)
{
	int value = 0;
	...
	value = parade_get_dts_value(core_node,
				     "need_check_report_descriptor_flag");
	if (value < 0) {
		TS_LOG_INFO(
			"%s, get device need_check_report_descriptor_flag failed\n",
			__func__);
		value = 0; // default
	}
...
static void
parade_parse_fw_need_depend_on_lcd(struct device_node *core_node,
				   struct ts_kit_device_data *chip_data)
{
	int value = 0;
...
static void
parade_need_delay_after_power_off(struct device_node *core_node,
				  struct ts_kit_device_data *chip_data)
{
	int value = 0;
	...
	value = parade_get_dts_value(core_node,
				     "parade,need_delay_after_power_off");
	if (value < 0) {
		TS_LOG_INFO(
			"%s, get device need_delay_after_power_off failed\n",
			__func__);
		value = 0; // default
	}
...
static int parade_parse_abs_dts(struct device_node *device)
{
	...
	tskit_parade_data->md.t_max =
		parade_get_dts_value(core_node, "parade,abs_max_t");
	...
	tskit_parade_data->md.t_min =
		parade_get_dts_value(core_node, "parade,abs_min_t");
	...
	tskit_parade_data->md.or_min =
		parade_get_dts_value(core_node, "parade,abs_min_or");
	...
	tskit_parade_data->md.or_max =
		parade_get_dts_value(core_node, "parade,abs_max_or");
	...
	tskit_parade_data->md.p_min =
		parade_get_dts_value(core_node, "parade,abs_min_p");
	...
	tskit_parade_data->md.p_max =
		parade_get_dts_value(core_node, "parade,abs_max_p");
	...
	tskit_parade_data->md.major_min =
		parade_get_dts_value(core_node, "parade,abs_min_mj");
	...
	tskit_parade_data->md.major_max =
		parade_get_dts_value(core_node, "parade,abs_max_mj");
	...
	tskit_parade_data->md.minor_min =
		parade_get_dts_value(core_node, "parade,abs_min_mn");
	...
	tskit_parade_data->md.minor_max =
		parade_get_dts_value(core_node, "parade,abs_max_mn");
	...
	tskit_parade_data->md.x_min =
		parade_get_dts_value(core_node, "parade,abs_min_x");
	...
	/* read abs_max_x */
	tskit_parade_data->md.x_max =
		parade_get_dts_value(core_node, "parade,abs_max_x");
	...
	tskit_parade_data->md.y_min =
		parade_get_dts_value(core_node, "parade,abs_min_y");
	...
	/* read abs_max_y */
	tskit_parade_data->md.y_max =
		parade_get_dts_value(core_node, "parade,abs_max_y");
			    ...
			    struct ts_kit_device_data *chip_data)
{
	...
	value = parade_get_dts_value(core_node,
				     "parade,easy_wakeup_supported_gestures");
	...
	tskit_parade_data->parade_chip_data->ts_platform_data->feature_info
		.wakeup_gesture_enable_info.switch_value =
		core_pdata->wakeup_gesture_enable;
	...
	value = parade_get_dts_value(
		core_node, "parade,need_surport_second_distinguishability");
	if (value == SUPPORT_DIFF_DISTINGUISHABILITY) {
		rc = of_property_read_string(
			core_node, "parade,sign_of_second_distinguishability",
			(const char **)&(
				tskit_parade_data
					->sign_of_second_distinguishability));
		if (rc) {
			TS_LOG_ERR(
				"%s, parade,sign_of_second_distinguishability read failed:%d\n",
				__func__, rc);
			goto fail_free;
		}
		TS_LOG_INFO(
			"%s,sign_of_second_distinguishability: %s. \n",
			__func__,
			tskit_parade_data->sign_of_second_distinguishability);
		get_current_distinguishability();
		if (tskit_parade_data->current_distinguishability) {
			tskit_parade_data->md.x_max = parade_get_dts_value(
				core_node,
				"parade,abs_max_x_of_second_distinguishability");
			if (tskit_parade_data->md.x_max < 0) {
				TS_LOG_ERR(
					"%s, tskit_parade_data->md.x_max is illegal\n",
					__func__);
				goto fail_free;
			}
			tskit_parade_data->md.y_max = parade_get_dts_value(
				core_node,
				"parade,abs_max_y_of_second_distinguishability");
			if (tskit_parade_data->md.y_max < 0) {
				TS_LOG_ERR(
					"%s, tskit_parade_data->md.y_max is illegal\n",
					__func__);
				goto fail_free;
			}
		}
	...
	TS_LOG_INFO(
		"%s,tskit_parade_data->md.x_max = %d.tskit_parade_data->md.y_max = %d.\n",
	...
	value = 0;
	value = parade_get_dts_value(core_node,
				     "parade,need_wait_after_cm_test");
	if (value <= 0 || value > 1) { // 1,max value of bool param
	...
	if (value < 0 || value > 0xFFFF) {
		tskit_parade_data->pt_hid_output_timeout =
			PT_HID_OUTPUT_TIMEOUT;
	} else {
	...
	tskit_parade_data->sleep_in_mode =
		parade_get_dts_value(core_node, "parade,sleep_in_mode");
	if (tskit_parade_data->sleep_in_mode < 0) {
		goto fail_free;
	}
	g_ts_kit_platform_data.chip_data->sleep_in_mode =
		tskit_parade_data->sleep_in_mode;
	TS_LOG_INFO("%s, chip sleep in mode %d \n", __func__,
		    tskit_parade_data->sleep_in_mode);
	/* power_self_ctrl */
	tskit_parade_data->self_ctrl_power =
		parade_get_dts_value(core_node, "power_self_ctrl");
	...
	if (value < 0) {
		TS_LOG_INFO(
			"%s, get device support_get_tp_color failed, will use default value: 0 \n ",
			__func__);
		value = 0; // default 0: no need know tp color
	}
	if (value > 1) {
		TS_LOG_INFO(
			"%s, get device support_get_tp_color=%d, dts value is invalid\n",
			__func__, value);
		value = 0;
	}
	...
	if (retval < 0) {
		TS_LOG_INFO("%s, get device create_project_id_flag failed\n",
			    __func__);
		retval = 0; // default
	}
	...
	/* project id */
	rc = of_property_read_string(core_node, "project_id",
				     (const char **)&tmp_buff);
	if (rc) {
	...
	/* chip name */
	rc = of_property_read_string(core_node, "chip_name",
				     (const char **)&tmp_buff);
	if (rc) {
	...
	TS_LOG_INFO("%s, chip_name %s\n", __func__,
		    tskit_parade_data->chip_name);
	/* module vendor */
	rc = of_property_read_string(core_node, "module_vendor",
				     (const char **)&tmp_buff);
	if (rc) {
...
static int parade_read_input(struct parade_core_data *cd)
{
	...
	rc = parade_i2c_read(&reg_addr, sizeof(reg_addr), cd->input_buf,
			     2); // 2, read 2 byte
	if (rc < 0) {
		TS_LOG_ERR("%s, fail read  rc=%d\n", __func__, rc);
		return rc;
...
/* lint -restore */
static void parade_get_touch_axis(struct parade_mt_data *md, int *axis,
				  int size, int max, u8 *data, int bofs)
{
	int nbyte = 0;
	int next = 0;
				 ...
				 struct parade_touch *touch, u8 *xy_mode)
{
	...
	if (touch->hdr[PT_TCH_NOISE] > 1) {
		TS_LOG_INFO(
			...
			__func__, touch->hdr[PT_TCH_TIME],
			touch->hdr[PT_TCH_NUM], touch->hdr[PT_TCH_LO],
			touch->hdr[PT_TCH_NOISE], touch->hdr[PT_TCH_COUNTER]);
	} else {
		TS_LOG_DEBUG(
			"%s: time=%X tch_num=%d lo=%d noise=%d counter=%d\n",
			__func__, touch->hdr[PT_TCH_TIME],
				    ...
				    struct parade_touch *touch, u8 *xy_data)
{
	...
	for (abs = PT_TCH_X; abs < abs_max; abs++) {
		...
		TS_LOG_DEBUG("%s: get %s=%04X(%d)\n", __func__,
			     parade_tch_abs_string[abs], touch->abs[abs],
			     touch->abs[abs]);
	}
}
				    ...
				    struct parade_touch *touch)
{
	...
	TS_LOG_DEBUG(
		...
		tskit_parade_data->mt_pdata->flags & PT_MT_FLAG_INV_X ? "true" :
									"false",
		tskit_parade_data->mt_pdata->flags & PT_MT_FLAG_INV_Y ? "true" :
									"false",
...
static void parade_get_mt_touches(struct parade_mt_data *md,
				  struct parade_touch *tch, int num_cur_tch,
				  struct ts_fingers *info)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		TS_LOG_DEBUG(
			...
			tch->abs[PT_TCH_P], tch->abs[PT_TCH_MAJ],
			tch->abs[PT_TCH_MIN], tch->abs[PT_TCH_OR],
			tch->abs[PT_TCH_E], tch->abs[PT_TCH_O],
			tch->abs[PT_TCH_TIP]);
	}
			    ...
			    struct ts_cmd_node *out_cmd)
{
	...
	parade_get_touch_hdr(
		md, &tch,
		si->xy_mode + TOUCH_DATA_START_INDEX); /* scan time offset */
...
static void parade_core_get_touch_axis(struct parade_core_data *cd, int *axis,
				       int size, int max, u8 *data, int bofs)
{
	int nbyte = 0;
	int next = 0;
			     ...
			     struct parade_sysinfo *si)
{
	...
	if (tskit_parade_data->roi_switch != 0) {
		...
		memcpy(&(g_roi_data[4]), &cd->input_buf[6],
		       PARADE_ROI_DATA_SEND_LENGTH);
	}
#endif
	return 0;
}
			   ...
			   struct parade_sysinfo *si)
{
	...
	parade_core_get_touch_axis(
		cd, &num_cur_tch, tch->size, tch->max,
		si->xy_mode + TOUCH_DATA_START_INDEX + tch->ofs, tch->bofs);
...
static int parade_set_gesture_key(struct ts_fingers *info,
				  struct ts_cmd_node *out_cmd)
{
	if (tskit_parade_data->parade_chip_data->easy_wakeup_info
		    .off_motion_on == true) {
		tskit_parade_data->parade_chip_data->easy_wakeup_info
			.off_motion_on = false;
	}
				    ...
				    struct ts_cmd_node *out_cmd)
{
	...
	memset(g_ts_kit_platform_data.chip_data->easy_wakeup_info
		       .easywake_position,
	       0, MAX_POSITION_NUMS);
	...
	for (i = 0; i < PT_DOUBULE_TAB_POINT_NUM; i++) {
		g_ts_kit_platform_data.chip_data->easy_wakeup_info
			.easywake_position[i] =
			cd->double_tab_info->postion[i].x << 16 |
			cd->double_tab_info->postion[i].y;
	}
	...
	case PT_CORE_EWG_TAP_TAP:
		...
		if (size == CY_WAKEUP_DOUBLE_TAB_BUF_SIZE &&
		    cd->parade_chip_data->easy_wakeup_info.easy_wakeup_gesture &
			    PT_DOUBLE_CLICK_WAKEUP) {
			parade_double_tap_event(cd, info, out_cmd);
			return;
		} else {
			TS_LOG_ERR(
				"%s: invalid size or not support double_tab event!\n",
				__func__);
		}
		break;
	default:
			     ...
			     struct ts_cmd_node *out_cmd)
{
	...
	if (report_id == si->desc.tch_report_id) {
		...
		if (g_ts_kit_log_cfg) {
			...
			if (rc) {
				TS_LOG_ERR(
					"%s: parade_debug_xy_worker failed\n",
					__func__);
			}
		}
	} else if (report_id == si->desc.btn_report_id) {
...
static bool IS_INT_Glitch(struct parade_core_data *cd)
{
	...
	if (cd->hid_cmd_state == HID_OUTPUT_BL_LAUNCH_APP + 1 &&
		 ...
		 */
		udelay(20); // ic need
		if (cd->core_pdata->irq_stat(cd->parade_chip_data) !=
				  ...
				  struct ts_cmd_node *out_cmd)
{
	...
	out_cmd->command = TS_INVAILD_CMD;
	if (atomic_read(&g_parade_reset_pin_status) !=
	    0) { /* Do not read i2c during reset is low stage */
		rc = parade_read_input(cd);
		if (!rc) {
...
static int parade_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_ORIENTATION,
			     tskit_parade_data->md.minor_min,
			     tskit_parade_data->md.minor_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, ABS_MT_WIDTH_MAX,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MINOR, 0, ABS_MT_WIDTH_MAX,
			     0, 0);
...
static u8 *pt_get_row_(struct parade_core_data *cd, u8 *row_buf, u8 *image_buf,
		       int size)
{
...
static int parade_initiate_bl_(struct parade_core_data *cd, u16 key_size,
			...
			struct pt_hex_image *row_image)
{
	...
	rc = parade_initiate_bl_(cd, ARRAY_SIZE(g_pt_security_key),
				 (u8 *)g_pt_security_key, row_image->row_size,
				 row_image->row_data);
	return rc;
}

static int parade_bl_program_and_verify_(struct parade_core_data *cd,
...
static int pt_ldr_prog_row_(struct parade_core_data *cd,
			    struct pt_hex_image *row_image)
{
	u16 length = row_image->row_size +
		     PT_PROG_ROW_HEAD_LEN; /* row_data + array_id + row_id(2) */
	...
	memcpy(data + PT_PROG_ROW_HEAD_LEN, row_image->row_data,
	       row_image->row_size);
...
static int pt_ldr_verify_chksum_(struct parade_core_data *cd)
{
	...
	result =
		cd->response_buf[HID_OUTPUT_BL_RESPONSE_CHECKSUM_RESULT_OFFSET];
...
static int parade_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
	...
	while (p < last_row) {
		...
		/* Parse row */
		row_image->row_size =
			get_unaligned_be16(&row_buf[CY_ROW_SIZE_OFFSET]);
		if (row_image->row_size > ARRAY_SIZE(row_image->row_data)) {
			TS_LOG_ERR(
				"%s: check row data buffer overflow,not upgrade,read row=%d\n",
				__func__, row_count);
			rc = -EOVERFLOW;
			goto _parade_load_app_exit;
		}
...
static int parade_fw_update(const struct firmware *fw)
{
	...
	if (!fw) {
		rc = -1;
#if defined(CONFIG_HONOR_DSM)
		tskit_parade_data->parade_chip_data->ts_platform_data->dsm_info
			.constraints_update_status = FWU_REQUEST_FW_FAIL;
#endif
...
#if defined(CONFIG_HONOR_DSM)
		tskit_parade_data->parade_chip_data->ts_platform_data->dsm_info
			.constraints_update_status = FWU_FW_CONT_ERROR;
#endif
		rc = -1;
...
static int parade_create_project_id(void)
{
	...
	if (tskit_parade_data->sysinfo.ready && g_already_get_flag == false) {
		...
		for (i = 0; (i < PROJECT_ID_PRODUCT_NAME_LEN) &&
			    g_ts_kit_platform_data.product_name[i];
		     i++)
			tmp_buff[i] =
				toupper(g_ts_kit_platform_data.product_name[i]);

		/* create project id */
		snprintf(tskit_parade_data->project_id, MAX_STR_LEN,
			 "%s%02d%02d0", tmp_buff, chip_id,
			 tskit_parade_data->panel_id);

		/* get module name */
		switch (tskit_parade_data->panel_id) {
		case MODULE_OFILM_ID:
			strncpy(tskit_parade_data->module_vendor, "ofilm",
				MAX_STR_LEN);
			break;
		case MODULE_TOPTOUCH_ID:
			strncpy(tskit_parade_data->module_vendor, "toptouch",
				MAX_STR_LEN);
			break;
		case MODULE_LENSONE_ID:
			strncpy(tskit_parade_data->module_vendor, "lensone",
				MAX_STR_LEN);
			break;
		case MODULE_JUNDA_ID:
			strncpy(tskit_parade_data->module_vendor, "junda",
				MAX_STR_LEN);
			break;
		case MODULE_MUTTO_ID:
			strncpy(tskit_parade_data->module_vendor, "mutto",
				MAX_STR_LEN);
			break;
		case MODULE_EELY_ID:
			strncpy(tskit_parade_data->module_vendor, "eely",
				MAX_STR_LEN);
			break;
		case MODULE_TRULY_ID:
			strncpy(tskit_parade_data->module_vendor, "truly",
				MAX_STR_LEN);
			break;
		default:
			retval = -EINVAL;
		...
		memset(tskit_parade_data->parade_chip_data->module_name, 0,
		       MAX_STR_LEN);
		snprintf(tskit_parade_data->parade_chip_data->module_name,
			 MAX_STR_LEN - 1, "%s",
			 tskit_parade_data->module_vendor);
		...
		if (tskit_parade_data->need_provide_projectID_for_sensor) {
			memcpy(tskit_parade_data->parade_chip_data->project_id,
			       tskit_parade_data->project_id,
			       MAX_STR_LEN -
				       1); // proviod TP's project id for other module.
		}
...
static void parade_get_fw_name(char *file_name)
{
	...
	tskit_parade_data->fw_and_config_name =
		file_name; // Point to the host structure
	if (tskit_parade_data
		    ->fw_only_depend_on_lcd) { // TP fw only depend on LCD module
		...
		strncat(file_name, tskit_parade_data->module_vendor,
			MAX_STR_LEN);
	}
...
static int parade_fw_update_boot(char *file_name)
{
	...
	if (!file_name || !tskit_parade_data ||
	    !g_ts_kit_platform_data.chip_data ||
	    !tskit_parade_data->parade_chip_data ||
	    !tskit_parade_data->parade_chip_data->ts_platform_data) {
		TS_LOG_ERR("%s, param is null\n", __func__);
	...
	if (tskit_parade_data->need_upgrade_again) {
		strncpy(tskit_parade_data->temporary_firmware_name, file_name,
			TEMP_MAX_STR_LEN - 1);
		tskit_parade_data
			->temporary_firmware_name[TEMP_MAX_STR_LEN - 1] = '\0';
	}
upgrade_firmware_again:
	...
	if (tskit_parade_data->need_upgrade_again &&
		...
		TS_LOG_INFO(
			"%s, firmware is restore,no need parade_loader_probe\n",
			__func__);
	} else {
	...
	if (rc == 0) {
		TS_LOG_INFO(
			"%s: FW upgrade done, Restart up after fw upprade\n",
			__func__);
		...
		if (rc) {
			...
			TS_LOG_INFO(
				"%s: Start up successfully after FW upgrade\n",
				__func__);
...
static int parade_check_firmware_version(u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	if (!tskit_parade_data->sysinfo.ready) {
		TS_LOG_INFO(
			"%s: sysinfo is not ready, fw may be corrupted, FW Upgrade\n",
			__func__);
		...
		if (tskit_parade_data->need_upgrade_again) {
			...
			TS_LOG_INFO(
				"%s: Firmware is broken ,is_firmware_broken is %d\n",
				__func__,
				tskit_parade_data->is_firmware_broken);
		}
		return 1;
	...
	if (fw_ver_new != fw_ver_img) {
		if (!tskit_parade_data->check_fw_right_flag) {
			TS_LOG_INFO(
				"%s: built in fw does not match ic firmware, will upgrade\n",
				__func__);
			return 1;
		} else {
			if (tskit_parade_data->check_fw_right_flag !=
			    fw_ver_new) {
				TS_LOG_ERR(
					"%s: check_fw_right_flag is not valid,will not upgrade\n",
					__func__);
#if defined(CONFIG_HONOR_DSM)
				if (!dsm_client_ocuppy(ts_dclient)) {
					dsm_client_record(
						ts_dclient,
						"parade ts check_fw_right_flag error\n");
					dsm_client_notify(
						ts_dclient,
...
static DEVICE_ATTR(manual_upgrade,
		   S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH | S_IWGRP | S_IRGRP,
		   pt_manual_upgrade_show, pt_manual_upgrade_store);

static struct attribute *parade_attributes[] = {&dev_attr_manual_upgrade.attr,
						NULL};
					...
					u8 param_id, u32 value, u8 size)
{
	...
	if (param_id !=
		    cd->response_buf[HID_OUTPUT_RESPONSE_SET_PARAM_ID_OFFSET] ||
...
static int parade_hid_output_run_selftest_(struct parade_core_data *cd,
					   ...
					   u8 *results_available)
{
	...
	if (status) {
		*status =
			cd->response_buf
				[HID_OUTPUT_RESPONSE_RUN_SELFTEST_STATUS_OFFSET];
	}
	if (summary_result) {
		*summary_result =
			cd->response_buf
				[HID_OUTPUT_RESPONSE_RUN_SELFTEST_SUMMARY_OFFSET];
	}
	if (results_available) {
		*results_available =
			cd->response_buf
				[HID_OUTPUT_RESPONSE_RUN_SELFTEST_RESULT_OFFSET];
	}
...
again:
	write_buf[HID_OUTPUT_GET_SELFTEST_READ_OFFSET_LOW] =
		LOW_BYTE(read_offset);
	write_buf[HID_OUTPUT_GET_SELFTEST_READ_OFFSET_HIGH] =
		HI_BYTE(read_offset);
	write_buf[HID_OUTPUT_GET_SELFTEST_READ_LENGTH_LOW] =
		LOW_BYTE(read_length);
	write_buf[HID_OUTPUT_GET_SELFTEST_READ_LENGTH_HIGH] =
		HI_BYTE(read_length);
	...
	read_test_id =
		cd->response_buf[HID_OUTPUT_RESPONSE_GET_SELFTEST_ID_OFFSET];
	if (read_test_id != test_id) {
		return -EPROTO;
	}

	read_len = get_unaligned_le16(
		&cd->response_buf
			 [HID_OUTPUT_RESPONSE_GET_SELFTEST_DATALEN_OFFSET]);
	read_len = read_len > (PT_MAX_INPUT -
			       HID_OUTPUT_RESPONSE_GET_SELFTEST_DATA_OFFSET) ?
			   (PT_MAX_INPUT -
			    HID_OUTPUT_RESPONSE_GET_SELFTEST_DATA_OFFSET) :
			   read_len;
	if (read_len && data) {
...
set_status:
	if (status) {
		*status =
			cd->response_buf
				[HID_OUTPUT_RESPONSE_GET_SELFTEST_STATUS_OFFSET];
	}

	return rc;
}

static ssize_t parade_run_and_get_selftest_result_noprint(
	struct parade_core_data *cd, char *buf, size_t buf_len, u8 test_id,
	u16 read_length, bool get_result_on_pass)
{
	...
	rc = parade_hid_output_run_selftest_(cd, test_id, 0, &cmd_status,
					     &summary_result, NULL);
	if (rc < 0) {
	...
	rc = parade_hid_output_get_selftest_result_(cd, 0, read_length, test_id,
						    &cmd_status, &act_length,
						    &cd->dad.ic_buf[6]);
	if (rc < 0) {
...
#define CMCP_DEBUG 0
	...
	if (config->cm_enabled && config->cp_enabled) {
		...
		tx_num = (field_array[PER_ELEMENT_MIN_MAX_TABLE_SENSOR]
				  .data_num >>
			  1) /
			 field_array[PER_ELEMENT_MIN_MAX_TABLE_SENSOR].line_num;
		...
		for (index = 0;
		     ...
		     index++) {
			TS_LOG_DEBUG("%d\n",
				     config->cm_max_table_gradient_cols_percent
					     [index]);
		}
		for (index = 0;
		     ...
		     index++) {
			TS_LOG_DEBUG("%d\n",
				     config->cm_max_table_gradient_rows_percent
					     [index]);
		}
		...
		for (index = 0; index < config->cp_min_max_table_rx_size;
		     index++) {
			TS_LOG_DEBUG("%d\n",
				     config->cp_min_max_table_rx[index]);
		}
		for (index = 0; index < config->cp_min_max_table_tx_size;
		     index++) {
			TS_LOG_DEBUG("%d\n",
				      ...
				      struct configuration *configs)
{
	...
	struct test_case_field test_case_field_array[MAX_CASE_NUM] = {
		{"CM TEST INPUTS", strlen("CM TEST INPUTS"), TEST_CASE_TYPE_NO,
		 NULL, 0, 0, 0},
		{"CM_EXCLUDING_COL_EDGE", strlen("CM_EXCLUDING_COL_EDGE"),
		 TEST_CASE_TYPE_ONE, &configs->cm_excluding_col_edge, 0, 0, 0},
		{"CM_EXCLUDING_ROW_EDGE", strlen("CM_EXCLUDING_ROW_EDGE"),
		 TEST_CASE_TYPE_ONE, &configs->cm_excluding_row_edge, 0, 0, 0},
		{"CM_GRADIENT_CHECK_COL", strlen("CM_GRADIENT_CHECK_COL"),
		 TEST_CASE_TYPE_MUL,
		 &configs->cm_max_table_gradient_cols_percent[0], 0, 0, 0},
		{"CM_GRADIENT_CHECK_ROW", strlen("CM_GRADIENT_CHECK_ROW"),
		 TEST_CASE_TYPE_MUL,
		 &configs->cm_max_table_gradient_rows_percent[0], 0, 0, 0},
		{"DIFF_NOISE_THRESHOLD", strlen("DIFF_NOISE_THRESHOLD"),
		 TEST_CASE_TYPE_ONE, &configs->diff_noise_threshold, 0, 0, 0},
		{"DIFF_SELF_RX_NOISE_THRESHOLD", PT_RAW_RX_NUM,
		 TEST_CASE_TYPE_ONE, &configs->diff_self_rx_noise_threshold,
		 PT_CASE_EXIST, PT_DATA_NUM, PT_LINE_NUM},
		{"DIFF_SELF_TX_NOISE_THRESHOLD", PT_RAW_TX_NUM,
		 TEST_CASE_TYPE_ONE, &configs->diff_self_tx_noise_threshold,
		 PT_CASE_EXIST, PT_DATA_NUM, PT_LINE_NUM},
		{"CM_RANGE_LIMIT_ROW", strlen("CM_RANGE_LIMIT_ROW"),
		 CmRangeLimitType, CmRangeLimitRowBuf, 0, 0, 0},
		{"CM_RANGE_LIMIT_COL", strlen("CM_RANGE_LIMIT_COL"),
		 CmRangeLimitType, CmRangeLimitColBuf, 0, 0, 0},
		{"CM_MIN_LIMIT_CAL", strlen("CM_MIN_LIMIT_CAL"),
		 TEST_CASE_TYPE_ONE, &configs->cm_min_limit_cal, 0, 0, 0},
		{"CM_MAX_LIMIT_CAL", strlen("CM_MAX_LIMIT_CAL"),
		 TEST_CASE_TYPE_ONE, &configs->cm_max_limit_cal, 0, 0, 0},
		{"CM_MAX_DELTA_SENSOR_PERCENT",
		 strlen("CM_MAX_DELTA_SENSOR_PERCENT"), TEST_CASE_TYPE_ONE,
		 &configs->cm_max_delta_sensor_percent, 0, 0, 0},
		{"CM_MAX_DELTA_BUTTON_PERCENT",
		 strlen("CM_MAX_DELTA_BUTTON_PERCENT"), TEST_CASE_TYPE_ONE,
		 &configs->cm_max_delta_button_percent, 0, 0, 0},
		{"PER_ELEMENT_MIN_MAX_TABLE_BUTTON",
		 strlen("PER_ELEMENT_MIN_MAX_TABLE_BUTTON"), TEST_CASE_TYPE_MUL,
		 &configs->cm_min_max_table_button[0], 0, 0, 0},
		{"PER_ELEMENT_MIN_MAX_TABLE_SENSOR",
		 strlen("PER_ELEMENT_MIN_MAX_TABLE_SENSOR"),
		 TEST_CASE_TYPE_MUL_LINES, &configs->cm_min_max_table_sensor[0],
		 0, 0, 0},
		{"CP TEST INPUTS", strlen("CP TEST INPUTS"), TEST_CASE_TYPE_NO,
		 NULL, 0, 0, 0},
		{"CP_PER_ELEMENT_MIN_MAX_BUTTON",
		 strlen("CP_PER_ELEMENT_MIN_MAX_BUTTON"), TEST_CASE_TYPE_MUL,
		 &configs->cp_min_max_table_button[0], 0, 0, 0},
		{"CP_MAX_DELTA_SENSOR_RX_PERCENT",
		 strlen("CP_MAX_DELTA_SENSOR_RX_PERCENT"), TEST_CASE_TYPE_ONE,
		 &configs->cp_max_delta_sensor_rx_percent, 0, 0, 0},
		{"CP_MAX_DELTA_SENSOR_TX_PERCENT",
		 strlen("CP_MAX_DELTA_SENSOR_TX_PERCENT"), TEST_CASE_TYPE_ONE,
		 &configs->cp_max_delta_sensor_tx_percent, 0, 0, 0},
		{"CP_MAX_DELTA_BUTTON_PERCENT",
		 strlen("CP_MAX_DELTA_BUTTON_PERCENT"), TEST_CASE_TYPE_ONE,
		...
		{"PER_ELEMENT_MIN_MAX_RX", strlen("PER_ELEMENT_MIN_MAX_RX"),
		 TEST_CASE_TYPE_MUL, &configs->cp_min_max_table_rx[0], 0, 0, 0},
		{"PER_ELEMENT_MIN_MAX_TX", strlen("PER_ELEMENT_MIN_MAX_TX"),
		 TEST_CASE_TYPE_MUL, &configs->cp_min_max_table_tx[0], 0, 0, 0},
	};
...
static int cmcp_return_offset_of_new_case(const char *bufPtr, u32 first_time,
					  const char *pFileEnd)
{
	static int offset = 0;
...
static int cmcp_get_case_info_from_threshold_file(
	const char *buf, struct test_case_search *search_array, u32 file_size)
{
	int case_num;
	int buffer_offset = 0;
	int name_count;
	...
	TS_LOG_INFO(
		"%s: Search cmcp threshold file, file start p=%pK,end p=%pK\n",
	...
	for (case_num = 0; case_num < MAX_CASE_NUM; case_num++) {
		buffer_offset = cmcp_return_offset_of_new_case(
			&buf[buffer_offset], first_search, pFileEnd);
...
static void cmcp_get_configuration_info(const char *buf,
					...
					u32 case_count,
					struct test_case_field *field_array,
					struct configuration *config)
{
	u32 count = 0;
	u32 sub_count = 0;
...
static ssize_t parade_parse_cmcp_threshold_file_common(const char *buf,
						       u32 file_size)
{
	...
	case_count = cmcp_get_case_info_from_threshold_file(
	...
	/* Search cases */
	cmcp_get_configuration_info(buf, dad->test_search_array, case_count,
				    dad->test_field_array, dad->configs);
...
static int parade_cmcp_parse_threshold_file(void)
{
	...
	if (tskit_parade_data
		    ->fw_need_depend_on_lcd) { // limit depen on lcd module name
		snprintf(tmp_filename, sizeof(tmp_filename) - 1,
			 "ts/%s_%s_%s_%s_%s_limits.csv",
				    ...
				    int test_item)
{
	...
	if (!cmcp_info || !cmcp_info->cp_tx_data_panel ||
	    !cmcp_info->cp_rx_data_panel || !configuration || !result ||
	    !pass) {
		TS_LOG_ERR("%s, param is null\n", __func__);
		return -EFAULT;
	}
	result->cp_test_pass = true;
	configuration_rx_num =
		cmcp_info->rx_num; // Fix bug just incase the threshold file parse fail
				...
				if (cmcp_info->cp_tx_data_panel[i] != 0) {
					cp_sensor_tx_delta = ABS(
						...
						cmcp_info->cp_tx_data_panel
							[i]); /* multiple 1000 to increate accuracy */
				}
		...
		for (i = 0; i < configuration_rx_num; i++) {
			...
			if ((cp_sensor_rx_data[i] <= cp_rx_min) ||
				...
				TS_LOG_ERR("%s: Cp Rx[%d]:%d (%d,%d)\n",
					   ...
					   (int)cp_sensor_rx_data[i], cp_rx_min,
					   cp_rx_max);
...
#define LIMIT_INDEX(x) \
	((tskit_parade_data->cm_delta_lattice_flag) ? (x - 1) : (x))

#define LIMIT_ROW(i, j)                                                        \
	((tskit_parade_data->cm_delta_lattice_flag) ?                          \
		 (configuration->cm_range_limit_row_lattice[(j - 1) * tx_num + \
							    i]) :              \
		 (configuration->cm_range_limit_row))

#define LIMIT_COL(i, j)                                                        \
	((tskit_parade_data->cm_delta_lattice_flag) ?                          \
		 (configuration->cm_range_limit_col_lattice[j * (tx_num - 1) + \
							    i - 1]) :          \
				    ...
				    int test_item)
{
	...
	if (test_item) {
		TS_LOG_DEBUG(
			"%s: Check each sensor Cm data for min max value\n ",
			__func__);
		...
		for (i = 0; i < sensor_num; i++) {
			...
			int32_t cm_sensor_min =
				configuration->cm_min_max_table_sensor
					[(row * tx_num + col) * 2];
			...
			if ((cm_sensor_data[i] < cm_sensor_min) ||
				...
				TS_LOG_ERR("%s: Sensor[%d,%d]:%d (%d,%d)\n",
					   "Cm sensor min/max test", row, col,
					   cm_sensor_data[i], cm_sensor_min,
					   cm_sensor_max);
		...
		for (i = 0;
			...
			if ((gd_sensor_col + i)->gradient_val >
			    10 * configuration
					    ->cm_max_table_gradient_cols_percent
						    [i]) {
				TS_LOG_ERR(
		...
		for (j = 0;
			...
			if ((gd_sensor_row + j)->gradient_val >
			    10 * configuration
					    ->cm_max_table_gradient_rows_percent
						    [j]) {
				TS_LOG_ERR(
		...
		for (i = 0; i < tx_num; i++) {
			for (j = 1; j < rx_num; j++) {
				...
				cm_sensor_row_delta[i * LIMIT_INDEX(rx_num) +
						    j - 1] = cm_sensor_row_diff;
				if (cm_sensor_row_diff > LIMIT_ROW(i, j)) {
					TS_LOG_ERR(
						"%s: Sensor[%d,%d]:%d (%d)\n",
		...
		for (i = 1; i < tx_num; i++) {
			for (j = 0; j < rx_num; j++) {
				int32_t cm_sensor_col_diff = ABS(
					(int)cm_sensor_data[i * rx_num + j] -
					(int)cm_sensor_data[(i - 1) * rx_num +
							    j]);
				cm_sensor_column_delta[(i - 1) * rx_num + j] =
					cm_sensor_col_diff;
				if (cm_sensor_col_diff > LIMIT_COL(i, j)) {
					TS_LOG_ERR(
						"%s: Sensor[%d,%d]:%d (%d)\n",
	...
	if (!exclude_col_edge) {
		for (i = 0; i < col_num; i++) {
			if (!exclude_row_edge) {
				...
				cm_max_cur = (p + i)->cm_max_exclude_edge;
				if (i <
				    (col_num - 1)) /* i=col_num no next node */
					cm_ave_next =
						(p + i + 1)->cm_ave_exclude_edge;
				if (i > 0) /* i=0 no pre node */
					cm_ave_prev =
...
static void calculate_gd_info(struct gd_sensor *gd_sensor_col,
			      struct gd_sensor *gd_sensor_row, int tx_num,
			      int rx_num, int32_t *cm_sensor_data,
				...
				struct cmcp_data *cmcp_info)
{
	...
	rc = parade_run_and_get_selftest_result_noprint(
		cd, tmp_buf, sizeof(tmp_buf), PT_ST_ID_AUTOSHORTS,
		PIP_CMD_MAX_LENGTH, false);
	if (rc) {
	...
	/* Get cm_panel data */
	rc = parade_run_and_get_selftest_result_noprint(
		cd, tmp_buf, sizeof(tmp_buf), PT_ST_ID_CM_PANEL,
		PIP_CMD_MAX_LENGTH, true);
	if (rc) {
	...
	if (cm_data_panel != NULL) {
		for (i = 0; i < tx_num * rx_num; i++) {
			...
			cm_data_panel[i] =
				10 *
				(cd->dad.ic_buf[CM_PANEL_DATA_OFFSET + i * 2] +
				 (cd->dad.ic_buf[CM_PANEL_DATA_OFFSET + i * 2 +
						 1]
				  << 8));
	...
	for (i = 0; i < tx_num; i++) {
		TS_LOG_DEBUG("i=%d max=%d,min=%d,ave=%d, gradient=%d\n", i,
			     gd_sensor_col[i].cm_max, gd_sensor_col[i].cm_min,
	...
	for (i = 0; i < rx_num; i++) {
		TS_LOG_DEBUG("i=%d max=%d,min=%d,ave=%d, gradient=%d\n", i,
			     gd_sensor_row[i].cm_max, gd_sensor_row[i].cm_min,
	...
	/* Get cp data */
	rc = parade_run_and_get_selftest_result_noprint(
		cd, tmp_buf, sizeof(tmp_buf), PT_ST_ID_CP_PANEL,
		PIP_CMD_MAX_LENGTH, true);
	if (rc) {
	...
	if (cp_tx_data_panel != NULL) {
		for (i = 0; i < tx_num; i++) {
			...
			cp_tx_data_panel[i] =
				10 *
				(cd->dad.ic_buf[CP_PANEL_DATA_OFFSET + i * 2] +
				 (cd->dad.ic_buf[CP_PANEL_DATA_OFFSET + i * 2 +
						 1]
				  << 8));
	...
	if (cp_tx_cal_data_panel != NULL) {
		for (i = 0; i < tx_num; i++) {
			...
			cp_tx_cal_data_panel[i] =
				10 * (cd->dad.ic_buf[CP_PANEL_DATA_OFFSET +
						     tx_num * 2 + i * 2] +
				      (cd->dad.ic_buf[CP_PANEL_DATA_OFFSET +
						      tx_num * 2 + i * 2 + 1]
				       << 8));
	...
	if (cp_tx_ave_data_panel != 0 && cp_tx_cal_data_panel != NULL) {
		cmcp_info->cp_sensor_tx_delta =
			ABS((cp_tx_cal_data_panel[0] - cp_tx_ave_data_panel) *
			    1000 / cp_tx_ave_data_panel);
	} else {
		TS_LOG_ERR(
			"%s: cp_tx_ave_data_panel is zero, please check it\n",
			__func__);
	}
	...
	if (cp_rx_ave_data_panel != 0 && cp_rx_cal_data_panel != NULL) {
		cmcp_info->cp_sensor_rx_delta =
			ABS((cp_rx_cal_data_panel[0] - cp_rx_ave_data_panel) *
			    1000 / cp_rx_ave_data_panel);
	} else {
		TS_LOG_ERR(
			"%s: cp_rx_ave_data_panel is zero, please check it\n",
			__func__);
	}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	rc = parade_hid_output_calibrate_idacs_(cd, PT_CAL_IDAC_MUTUAL,
						&status);
	if (rc < 0) {
		TS_LOG_ERR("%s: Error on calibrate idacs for mutual r=%d\n",
			   __func__, rc);
	}
	rc = parade_hid_output_calibrate_idacs_(cd, PT_CAL_IDAC_BUTTON,
						&status);
	if (rc < 0) {
	...
	if (final_pass) {
		index += snprintf(buf + index, PT_MAX_PRBUF_SIZE,
				  "cm test pass!\n");
	} else {
		index += snprintf(buf + index, PT_MAX_PRBUF_SIZE,
				  "cm test fail!\n");
	...
	if (final_pass) {
		index += scnprintf(buf + index, PT_MAX_PRBUF_SIZE,
				   "cp test pass!\n");
	} else {
		index += scnprintf(buf + index, PT_MAX_PRBUF_SIZE,
				   "cp test fail!\n");
...
static int parade_device_access_probe(struct parade_core_data *cd)
{
	...
	test_case_field_array = kzalloc(
		sizeof(*test_case_field_array) * MAX_CASE_NUM, GFP_KERNEL);
	if (!test_case_field_array) {
		rc = -ENOMEM;
		goto test_case_field_array_failed;
	}

	test_case_search_array = kzalloc(
		sizeof(*test_case_search_array) * MAX_CASE_NUM, GFP_KERNEL);
	if (!test_case_search_array) {
		rc = -ENOMEM;
		goto test_case_search_array_failed;
	}

	cmcp_info->gd_sensor_col = (struct gd_sensor *)kzalloc(
		tx_num * sizeof(struct gd_sensor), GFP_KERNEL);
	if (cmcp_info->gd_sensor_col == NULL) {
		goto gd_sensor_col_failed;
	}

	cmcp_info->gd_sensor_row = (struct gd_sensor *)kzalloc(
		rx_num * sizeof(struct gd_sensor), GFP_KERNEL);
...
static int parade_chip_get_info(struct ts_chip_info_param *info)
{
	...
	sprintf(info->fw_vendor, "%02X.%02X.%08X.%04X", ttdata->fw_ver_major,
		ttdata->fw_ver_minor, ttdata->revctrl, ttdata->fw_ver_conf);
				       ...
				       struct parade_core_data *pcd)
{
	...
	if (mmiresult->cm_sensor_gd_col_pass &&
	    mmiresult->cm_sensor_gd_row_pass &&
	    mmiresult->cm_sensor_row_delta_pass &&
	    mmiresult->cm_sensor_col_delta_pass) {
	...
	if (pcd->self_noise_supported == PT_SELF_NOISE_SUP) {
		if (mmiresult->diff_self_rx_test_pass &&
		    mmiresult->diff_self_tx_test_pass) {
			      ...
			      struct ts_cmd_node *out_cmd)
{
	...
	if (!info || !cd || !g_ts_kit_platform_data.chip_data ||
	    ...
	    !tskit_parade_data->dad.result || !tskit_parade_data->dad.configs) {
		rc = -EFAULT;
		TS_LOG_ERR("%s, param is null\n", __func__);
		goto exit_error;
	}
	...
	g_ts_kit_platform_data.chip_data->isbootupdate_finish = false;
	memset(g_buf_parademmitest_result, 0,
	       sizeof(g_buf_parademmitest_result));
	memset(g_tp_test_failed_reason, 0, sizeof(g_tp_test_failed_reason));
	/* i2c test */
	rc = parade_i2c_read(&reg_addr, sizeof(reg_addr), buf,
			     2); // read 2 bytes
	if (rc < 0) {
	...
	if (rc) {
		TS_LOG_ERR(
			"%s: get diff parade_hid_output_suspend_scanning_ failed\n",
			__func__);
	...
	if (self_test_id_supported) {
		TS_LOG_ERR(
			"%s: Error on parade_gtp_init_work_fnet_cmcp_info failed r=%d\n",
	...
	if (rc) {
		TS_LOG_ERR(
			"%s: init baseline parade_hid_output_suspend_scanning_ failed\n",
			__func__);
		strncat(g_buf_parademmitest_result, "3F-", strlen("1F-"));
	...
	rc = parade_hid_output_initialize_baselines_(
		PT_IB_SM_MUTCAP | PT_IB_SM_SELFCAP, &status);
	if (rc) {
	...
	rc = validate_diff_result(
		configuration->diff_noise_threshold); // Hardcode here
	if (rc) {
		TS_LOG_ERR("%s: validate_diff_result failed\n", __func__);
	}
	if (cd->self_noise_supported == PT_SELF_NOISE_SUP) {
		validate_diff_self_result(
			configuration->diff_self_rx_noise_threshold,
	...
	/* mc diff */
	for (index = 0; index < sensor_num; index++) {
		info->buff[result_buf_index++] =
			cmcp_info->diff_sensor_data[index];
	}
	/* sc diff */
	for (index = 0; index < (cmcp_info->rx_num + cmcp_info->tx_num);
	     index++) {
		info->buff[result_buf_index++] =
			cmcp_info->diff_self_data[index];
	}
	/* Cm Gradient RX */
	for (index = 0; index < cmcp_info->rx_num; index++) {
		info->buff[result_buf_index++] =
			cmcp_info->gd_sensor_row[index].gradient_val;
	}
	/* Cm Gradient TX */
	for (index = 0; index < cmcp_info->tx_num; index++) {
		info->buff[result_buf_index++] =
			cmcp_info->gd_sensor_col[index].gradient_val;
	}
	/* Cp RX */
	for (index = 0; index < cmcp_info->rx_num; index++) {
		info->buff[result_buf_index++] =
	...
	for (index = 0; index < cmcp_info->rx_num; index++) {
		if (cmcp_info->cp_rx_data_panel[index] != 0) {
			info->buff[result_buf_index++] =
				ABS((cmcp_info->cp_rx_cal_data_panel[index] -
				     cmcp_info->cp_rx_data_panel[index]) *
				    100 / cmcp_info->cp_rx_data_panel[index]);
		}
	}
	/* Cp Delta TX */
	for (index = 0; index < cmcp_info->tx_num; index++) {
		if (cmcp_info->cp_tx_data_panel[index] != 0) {
			info->buff[result_buf_index++] =
				ABS((cmcp_info->cp_tx_cal_data_panel[index] -
				     cmcp_info->cp_tx_data_panel[index]) *
				    100 / cmcp_info->cp_tx_data_panel[index]);
		}
	}
...
exit:
	if (strlen(g_buf_parademmitest_result) != 0) {
		if (strlen(g_tp_test_failed_reason) != 0) {
			strncat(g_buf_parademmitest_result,
				g_tp_test_failed_reason,
...
static int parade_power_on(void)
{
	...
	if (tskit_parade_data->self_ctrl_power == 1) { /* tp self_ctrl_power */
		...
		rc = ts_kit_power_supply_ctrl(TS_KIT_VCC, TS_KIT_POWER_ON,
					      0); /* power on vcc */
		if (rc) {
		...
		rc = ts_kit_power_supply_ctrl(TS_KIT_IOVDD, TS_KIT_POWER_ON,
					      0); /* power on iovdd */
		if (rc) {
...
static int parade_power_off(void)
{
	...
	if (tskit_parade_data->self_ctrl_power == 1) {
		...
		if (g_ts_kit_platform_data.ts_kit_powers[TS_KIT_IOVDD].type ==
			    TS_KIT_POWER_LDO ||
		    g_ts_kit_platform_data.ts_kit_powers[TS_KIT_VCC].type ==
			    TS_KIT_POWER_LDO) {
...
static void parade_power_off_gpio_set(void)
{
	...
	rc = gpio_direction_output(tskit_parade_data->parade_chip_data
	...
	rc = gpio_direction_input(tskit_parade_data->parade_chip_data
					  ->ts_platform_data->reset_gpio);
	if (rc) {
...
static void parade_power_on_gpio_set(void)
{
	...
	rc = gpio_direction_input(
		tskit_parade_data->parade_chip_data->ts_platform_data->irq_gpio);
	if (rc) {
...
static int parade_file_open_firmware(u8 *file_path, u8 *databuf, int *file_size)
{
	...
	if (file_len > PARADE_FIRMWARE_SIZE) {
		...
		retval = -EINVAL;
		goto exit;
	} else if (vfs_read(filp, databuf, file_len, &filp->f_pos) !=
		   file_len) {
...
static int _parade_fw_update_from_sdcard(u8 *file_path, bool update_flag)
{
	...
	if (header_size >= (fw_size + 1)) {
		TS_LOG_ERR(
			"%s: Firmware format is invalid,header_size = %d,fw_size = %d.\n",
	...
	retval = pt_upgrade_firmware(dev, (const u8 *)&fw_data[header_size + 1],
...
static int parade_hid_output_initialize_baselines_(u8 test_id, u8 *status)
{
	...
	*status =
		cd->response_buf[HID_OUTPUT_RESPONSE_INIT_BASELINE_STATUS_OFFSET];
	if (*status) {
		return -EINVAL;
	}
	return rc;
...
void ts_kit_check_bootup_upgrade(void)
{
	...
	TS_LOG_INFO("%s enter, with max timeout: %d s, current bootupgrade "
		    ...
		    istatus); // printf timeout time in s
	t = wait_event_timeout(
...
static int parade_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	tskit_parade_data->parade_dev->dev.of_node =
		tskit_parade_data->parade_chip_data->cnode;
	...
	if (cd->core_pdata->wakeup_gesture_enable) {
		cd->double_tab_info = (struct parade_double_tap_info *)kzalloc(
			sizeof(struct parade_double_tap_info), GFP_KERNEL);
		if (!cd->double_tab_info) {
			TS_LOG_ERR(
				"%s : Failed to kzalloc for parade_double_tap_info.\n",
				__func__);
			rc = -ENOMEM;
			goto alloc_failed;
		}
	...
	if (tskit_parade_data->need_set_rst_after_iovcc_flag == false) {
		...
		TS_LOG_INFO(
			"%s, set rst & int after iovcc, set reset input when power on\n",
			__func__);
	}
	...
	tskit_parade_data->tthe_debugfs =
		debugfs_create_file(PARADE_TTHE_TUNER_FILE_NAME, 0644, NULL,
				    tskit_parade_data, &tthe_debugfs_fops);
#endif
...
static int __init parade_module_init(void)
{
	...
	tskit_parade_data =
		kzalloc(sizeof(struct parade_core_data), GFP_KERNEL);
	if (!tskit_parade_data) {
		TS_LOG_ERR(
			"Failed to alloc mem for struct tskit_parade_data\n");
		error = -ENOMEM;
		return error;
	}
	tskit_parade_data->parade_chip_data =
	...
	if (!tskit_parade_data->parade_chip_data) {
		TS_LOG_ERR(
			"Failed to alloc mem for struct synaptics_chip_data\n");
		error = -ENOMEM;
		goto out;
	}
	...
	if (!root) {
		TS_LOG_ERR(
			"honor_ts, find_compatible_node honor,ts_kit error\n");
		error = -EINVAL;
		goto out;
	}
...
out:
	if (tskit_parade_data->parade_chip_data) {
		kfree(tskit_parade_data->parade_chip_data);
		tskit_parade_data->parade_chip_data =
			NULL; /* Fix the DTS parse error cause panic bug */
	}
	kfree(tskit_parade_data);
	tskit_parade_data = NULL;
	return error;
