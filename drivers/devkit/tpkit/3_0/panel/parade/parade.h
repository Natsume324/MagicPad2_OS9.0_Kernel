...
#define PARADE_ROI_DATA_SEND_LENGTH \
	(PARADE_ROI_DATA_READ_LENGTH - PARADE_ROI_HEAD_DATA_LENGTH)
#endif
/* Doze related macro */
...
#define HID_OUTPUT_RESPONSE_READ_CONF_BLOCK_CRC_OFFSET \
	10 /* Based on data length */
...
static const char *const parade_tch_hdr_string[] = {
	[PT_TCH_TIME] = "SCAN TIME",	     [PT_TCH_NUM] = "NUMBER OF RECORDS",
	[PT_TCH_LO] = "LARGE OBJECT",	     [PT_TCH_NOISE] = "NOISE EFFECT",
	[PT_TCH_COUNTER] = "REPORT_COUNTER", [PT_TCH_NUM_HDR] = "INVALID",
};
...
struct parade_oem_ops {
	...
	int (*write_nv_data)(struct parade_oem_data *parade_oem_data, u8 buf[],
			     int offset, int length);
	int (*read_nv_data)(struct parade_oem_data *parade_oem_data, u8 buf[],
			    int offset, int length);
	...
	int (*hid_send_output_and_wait)(struct parade_oem_data *parade_oem_data,
					struct parade_hid_output *hid_output);
};

enum pt_selftest_result { PT_SELFTEST_STATUS_SUCCESS, PT_SELFTEST_STATUS_FAIL };
