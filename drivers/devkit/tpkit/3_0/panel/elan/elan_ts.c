...
static int elan_ktf_irq_bottom_half(struct ts_cmd_node *in_cmd,
...
static int elan_ktf_parse_dts(struct device_node *device,
...
static int elan_ktf_get_rawdata(struct ts_rawdata_info *info,
				struct ts_cmd_node *out_cmd);
...
static int rawdata_proc_elan_printf(struct seq_file *m,
				    struct ts_rawdata_info *info,
				    int range_size, int row_size);
static void elan_chip_touch_switch(void);
static int elan_ktf_ts_calibrate(void);
static atomic_t g_last_pen_inrange_status =
	ATOMIC_INIT(TS_PEN_OUT_RANGE); // remember the last pen inrange status
...
static void elan_scene_switch(unsigned int scene, unsigned int oper)
{
	int error;
	// clang-format off
	u8 enter_scene_cmd[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x54,
		0xcf, 0x00, (u8)scene
	};
	u8 exit_scene_cmd[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x54,
		0xcf, 0x00, 0x00
	};
	...
	case TS_SWITCH_SCENE_ENTER:
		...
		if (error) {
			TS_LOG_ERR(
				"%s: Switch to scene %d mode Failed: error:%d\n",
				__func__, scene, error);
		}
		break;
	case TS_SWITCH_SCENE_EXIT:
		TS_LOG_INFO("%s, enter normal scene\n", __func__);
		error = elan_i2c_write(exit_scene_cmd, sizeof(exit_scene_cmd));
		if (error) {
...
static int tp_module_test_init(struct ts_rawdata_info *info)
{
	...
	if (g_elan_ts->rx_num <= 1 ||
	    g_elan_ts->tx_num <= 1) { // avoid rx-1 or tx -1
...
test_exit:
	strncat(info->result, adc_mean_low_boundary_test_result,
		strlen(adc_mean_low_boundary_test_result));
	strncat(info->result, txrx_open_test_result,
		strlen(txrx_open_test_result));
	strncat(info->result, noise_test_result, strlen(noise_test_result));
	strncat(info->result, txrx_short_test_result,
		strlen(txrx_short_test_result));
	strncat(info->result, ";", sizeof(";"));
	strncat(info->result, g_elan_ts->elan_chip_data->chip_name,
		sizeof(g_elan_ts->elan_chip_data->chip_name));
	strncat(info->result, "_", sizeof("_"));
	strncat(info->result, g_elan_ts->project_id,
				    ...
				    int range_size, int row_size)
{
	...
	for (index = 0; row_size * index + 2 < (2 + row_size * range_size * 3);
	     index++) {
		if (index == 0) {
			seq_printf(m,
				   "noisedata begin\n"); /* print the title */
		...
		for (index1 = 0; index1 < row_size; index1++) {
			seq_printf(m, "%d,",
				   info->buff[2 + row_size * index +
					      index1]); /* print oneline */
		}
		/* index1 = 0; */
		seq_printf(m, "\n ");
	...
	if (info->used_size > (2 + row_size * range_size * 3)) {
		...
		for (index1 = 0; index1 < (row_size + range_size) * 2;
		     index1++) {
			seq_printf(m, "%d,",
				   info->buff[2 + row_size * range_size * 3 +
					      index1]);
...
static int elan_ktf_get_info(struct ts_chip_info_param *info)
{
	...
	snprintf(info->fw_vendor, sizeof(info->fw_vendor), "0x%04x",
		 ts->fw_ver);
...
static int elan_config_gpio(void)
{
	...
	if (gpio_is_valid(g_elan_ts->int_gpio)) {
		...
		if (err) {
			TS_LOG_ERR(
				"[elan]:unable to set direction for gpio [%d]\n",
				g_elan_ts->int_gpio);
			return -EINVAL;
		}
	} else {
...
static inline void elan_ktf_finger_parse_xy(uint8_t *data)
{
	...
	g_elan_ts->finger_x =
		((data[FINGERX_POINT_HBYTE] << 8) | data[FINGERX_POINT_LBYTE]);
	g_elan_ts->finger_y =
		((data[FINGERY_POINT_HBYTE] << 8) | data[FINGERY_POINT_LBYTE]);
...
{
	g_elan_ts->pen_x =
		((data[PENX_POINT_HBYTE] << 8) | data[PENX_POINT_LBYTE]);
	g_elan_ts->pen_y =
		((data[PENY_POINT_HBYTE] << 8) | data[PENY_POINT_LBYTE]);
	g_elan_ts->pen_pressure =
		((data[PEN_PRESS_HBYTE] << 8) | data[PEN_PRESS_LBYTE]);
...
int elan_i2c_read(u8 *reg_addr, u16 reg_len, u8 *buf, u16 len)
{
	...
	rc = g_elan_ts->elan_chip_client->bops->bus_read(reg_addr, reg_len, buf,
							 len);
	if (rc) {
...
static int i2c_communicate_check(void)
{
	...
	u8 cmd_id[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		...
		if (rc) {
			TS_LOG_ERR(
				"[elan]:%s, Failed to read tp info,i = %d, rc = %d\n",
				__func__, i, rc);
			msleep(50); // IC need
		} else {
			TS_LOG_INFO(
				"[elan]:%s, i2c communicate check success,buf[0]=%x\n",
...
static int elan_ktf_ts_recv_data(u8 *pbuf)
{
	...
	if (buf[REPORT_ID_BYTE] == FINGER_REPORT_ID) {
		...
		recv_count_max =
			(fingernum / 2) +
			((fingernum % 2) !=
			 0); // 2, one recv_package include 2 fingers touch mesg
		for (recv_count = 1; recv_count < recv_count_max;
		     recv_count++) {
			...
			memcpy(buf + (REPORT_DATA_LEN - 1) * recv_count -
				       POINT_HEAD_LEN * (recv_count - 1),
			       data_buf + POINT_HEAD_LEN,
			       REPORT_DATA_LEN -
				       POINT_HEAD_LEN); // not Out of bounds
		}
	}
	return NO_ERR;
}

static void elan_mt_process_touch(uint16_t *x, uint16_t *y, int x_resolution,
				  int y_resolution)
{
...
static void parse_pen_point(struct ts_pens *pointinfo, u8 *pbuf,
			    struct ts_cmd_node *out_cmd)
{
	...
	if (pen_down) {
		...
		TS_LOG_DEBUG(
			"[elan]:report pen pressure = %d, pen_inrange_status = %d\n",
...
static int check_fw_status(void)
{
	int ret = 0;
	// clang-format off
	u8 checkstatus[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23,
		0x00, 0x03, 0x18
	};
	u8 check_rek_count[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x53,
		0xd0, 0x00, 0x01
	};
	...
	ret = elan_i2c_read(checkstatus, sizeof(checkstatus), buff,
			    sizeof(buff));
	if (ret) {
		...
		TS_LOG_INFO("[elan]:Tp mode check:%x,%x,%x,%x,%x\n", buff[0],
			    ...
			    buff[4]); // 0-3 hid head 4 th fw status buff
	}
	...
	if (buff[TP_NORMAL_DATA_BYTE] == TP_NORMAL_DATA) {
		...
		TS_LOG_INFO(
			"[elan]:Tp rek_count check:%x,%x,%x,%x,%x,%x,%x,%x\n",
			buff[0], buff[1], buff[2], buff[3], buff[4], buff[5],
			buff[ELAN_CALIBRATE_STATUS_HBYTE],
...
static int enter_iap_mode(void)
{
	int ret = 0;
	// clang-format off
	u8 flash_key[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x54, 0xc0, 0xe1, 0x5a
	};
	u8 isp_cmd[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x54, 0x00, 0x12, 0x34
	};
	u8 check_addr[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x01, 0x10
	};
	// clang-format on
	...
	if (ret) {
		...
		TS_LOG_INFO("[elan]:addr data:%x,%x,%x,%x,%x,%x,%x\n", buff[0],
			    ...
			    buff[6]); // 0-3th hid head 4-6th fw status buff
	}
	return NO_ERR;
}
...
static int get_ack_data(void)
{
	...
	if (res) {
		...
		TS_LOG_INFO("[elan]:ack data:%x,%x,%x,%x,%x,%x\n", buff[0],
			    ...
			    buff[5]); // 0-3 hid head data 4&5th ic ack
	}
...
{
	int len = 0;
	u8 finish_cmd[ELAN_SEND_DATA_LEN] = {0x04, 0x00, 0x23,
					     0x00, 0x03, 0x1A};
...
static int elan_ktf_ts_calibrate(void)
{
	...
	u8 cal_cmd[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x54, 0x29, 0x00, 0x01
	};
	// clang-format on
	ret = elan_i2c_write(flash_key, sizeof(flash_key));
	if (ret) {
...
static int hid_write_page(int pagenum, const u8 *fwdata)
{
	...
	// clang-format off
	u8 write_buf[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x21, 0x00, 0x00, 0x28
	};
	u8 cmd_iap_write[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23,
		0x00, 0x03, 0x22
	};
	// clang-format on
	...
	for (ipage = 1; ipage <= pagenum; ipage += WRITE_PAGES) {
		...
		if (ipage == lastpages) {
			write_times =
				(int)((pagenum - lastpages + 1) * FW_PAGE_SIZE /
				      WRITE_DATA_VALID_LEN) +
				((pagenum - lastpages + 1) * FW_PAGE_SIZE %
					 WRITE_DATA_VALID_LEN !=
				 0);
			lastdatalen = (pagenum - lastpages + 1) * FW_PAGE_SIZE %
				      WRITE_DATA_VALID_LEN;
		} else {
			write_times = (int)(WRITE_PAGES * FW_PAGE_SIZE /
					    WRITE_DATA_VALID_LEN) +
				      (WRITE_PAGES * FW_PAGE_SIZE %
					       WRITE_DATA_VALID_LEN !=
				       0);
			lastdatalen = WRITE_PAGES * FW_PAGE_SIZE %
				      WRITE_DATA_VALID_LEN;
		}
		...
		for (byte_count = 1; byte_count <= write_times; byte_count++) {
			...
			if (byte_count != write_times) {
				write_buf[WRITE_DATA_VALID_LEN_BYTE] =
					WRITE_DATA_VALID_LEN;
				offset += WRITE_DATA_VALID_LEN;
				curIndex = curIndex + WRITE_DATA_VALID_LEN;
				memcpy(write_buf + 9, szBuff,
				       WRITE_DATA_VALID_LEN); // 0-8 is write head
			} else {
				write_buf[WRITE_DATA_VALID_LEN_BYTE] =
					lastdatalen;
				curIndex = curIndex + lastdatalen;
				memcpy(write_buf + 9, szBuff,
				       lastdatalen); // 0-8 is write head
			}
			res = elan_i2c_write(write_buf, ELAN_SEND_DATA_LEN);
			if (res) {
				TS_LOG_ERR(
					"[elan]:updata fw write page fail!res=%d\n",
					res);
				return -EINVAL;
			}
		}
...
retry_updata:
	...
	res = elan_read_tx_rx();
	if (res == NO_ERR && g_elan_ts->rx_num - 1 > 0 &&
	    g_elan_ts->tx_num - 1 > 0) {
		g_elan_ts->finger_y_resolution =
			(g_elan_ts->rx_num - 1) * FINGER_OSR;
		g_elan_ts->finger_x_resolution =
			(g_elan_ts->tx_num - 1) * FINGER_OSR;
...
static int elan_get_lcd_panel_info(void)
{
	...
	strncpy(g_elan_ts->lcd_panel_info, lcd_type,
...
static int elan_ktf_fw_update_sd(void)
{
	...
	if (err < 0) {
		TS_LOG_ERR(
			"[elan]:%s %d: Fail request firmware %s, retval = %d\n",
...
static int elan_ktf_fw_update_boot(char *file_name)
{
	...
	strncat(file_name, g_elan_ts->project_id,
		sizeof(g_elan_ts->project_id));
	strncat(file_name, "_", strlen("_"));
	strncat(file_name, g_elan_ts->lcd_module_name,
		strlen(g_elan_ts->lcd_module_name));
	strncat(file_name, FW_SUFFIX, strlen(FW_SUFFIX));
	snprintf(file_path, strlen(file_name) + strlen("ts/") + 1, "ts/%s",
		 file_name);
	...
	New_Fw_Ver = fw_data[FWVER_HIGH_BYTE_IN_EKT] << 8 |
		     fw_data[FWVER_LOW_BYTE_IN_EKT];
...
static int elan_ktf_parse_dts_power(struct device_node *device,
				    struct ts_kit_device_data *chip_data)
{
	int slave_addr = 0;
	int ret = NO_ERR;
	if ((!device) || (!chip_data) || (!g_elan_ts) ||
	    (!g_elan_ts->elan_chip_data) ||
...
static void elan_power_off(void)
{
	...
	rc = ts_kit_power_supply_ctrl(TS_KIT_VCC, TS_KIT_POWER_OFF,
				      0); /* power off vcc, delay 0ms */
	if (rc) {
		TS_LOG_ERR("%s, power on vcc fail, %d\n", __func__, rc);
	}
	rc = ts_kit_power_supply_ctrl(TS_KIT_IOVDD, TS_KIT_POWER_OFF,
				      0); /* power off iovdd, delay 0ms */
	if (rc) {
		TS_LOG_ERR("%s, power on iovdd fail, %d\n", __func__, rc);
	}
...
static int elan_ktf_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	if (((!platform_data) || (!platform_data->ts_dev)) || (!g_elan_ts) ||
	    (!g_elan_ts->elan_chip_data)) {
		TS_LOG_ERR(
			"[elan]:%s device, ts_kit_platform_data *data or data->ts_dev is NULL \n",
			__func__);
		ret = -ENOMEM;
		goto exit;
	}
	...
	ret = elan_ktf_parse_dts_power(g_elan_ts->elan_chip_data->cnode,
				       g_elan_ts->elan_chip_data);
	if (ret) {
	...
	ret = elan_ktf_parse_dts(g_elan_ts->elan_chip_data->cnode,
				 g_elan_ts->elan_chip_data);
	if (ret) {
...
static int elan_project_color(void)
{
	...
	// clang-format off
	u8 reg_cmd[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x00, 0x06, 0x96, 0x80, 0x80,
		0x00, 0x00, 0x11
	};
	u8 test_cmd[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x55, 0x55, 0x55, 0x55
	};
	u8 project_cmd[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x00, 0x06, 0x59, 0x00, 0x80,
		0x80, 0x00, 0x40
	};
	// clang-format on
	if (!g_elan_ts) {
	...
	if (atomic_read(&g_elan_ts->tp_mode) == TP_NORMAL) {
		for (i = 0; i < READ_PORJECT_ID_WORDS; i++) {
			reg_cmd[SEND_CMD_VALID_INDEX] =
				0x80 +
				i; // 0x80 project id addr index low byte ,
			ret = elan_i2c_write(reg_cmd, sizeof(reg_cmd));
			if (ret) {
				TS_LOG_ERR(
					"[elan]:set project id reg cmd fail!ret=%d\n",
					ret);
				return -EINVAL;
			}
			ret = wait_int_low();
			...
			ret = elan_i2c_read(NULL, 0, rsp_buf,
					    ELAN_RECV_DATA_LEN);
			if (ret) {
			...
			TS_LOG_INFO("[elan]:project id:%x,%x,%x\n", rsp_buf[0],
				    rsp_buf[7],
				    rsp_buf[8]); // 0 data length 7 8 valid data
			if (i < (READ_PORJECT_ID_WORDS - 1)) {
				memcpy(g_elan_ts->project_id + i * 2,
				       rsp_buf + PROJECT_ID_INDEX,
				       2); //  get two bytes once time
			} else {
				memcpy(g_elan_ts->color_id,
				       rsp_buf + PROJECT_ID_INDEX,
				       sizeof(g_elan_ts->color_id));
			}
		}
	} else {
		...
		if (ret) {
			TS_LOG_ERR(
				"[elan]:elan_i2c_write project_cmd fail!ret=%d\n",
				ret);
			ret = elan_ktf_hw_reset();
			if (ret != NO_ERR) {
				TS_LOG_ERR("%s hw reset fail\n", __func__);
		...
		memcpy(g_elan_ts->project_id, rsp_buf + PROJECT_ID_INDEX,
		       sizeof(g_elan_ts->project_id) - 1); // reserved 1 byte
		memcpy(g_elan_ts->color_id, rsp_buf + COLOR_ID_INDEX,
		       sizeof(g_elan_ts->color_id));
		ret = elan_ktf_hw_reset();
		if (ret != NO_ERR) {
			TS_LOG_ERR("%s hw reset fail\n", __func__);
	...
	g_elan_ts->project_id[sizeof(g_elan_ts->project_id) - 1] =
		'\0'; // 9 bytes valid,reserved 1 byte
	memcpy(g_elan_ts->elan_chip_data->module_name, g_elan_ts->project_id,
	       sizeof(g_elan_ts->project_id));
	TS_LOG_INFO("[elan]:module_name=%s,color_id=%x\n",
		    g_elan_ts->elan_chip_data->module_name,
		    g_elan_ts->color_id[0]);
...
static int elan_read_fw_info(void)
{
	...
	ret = elan_i2c_read(cmd_ver, sizeof(cmd_ver), resp_buf,
			    sizeof(resp_buf));
	if (ret) {
		...
		TS_LOG_DEBUG(
			"[elan]:buf[0]=%x,buf[1]=%x,buf[2]=%x,buf[3]=%x,buf[4]=%x\n",
			resp_buf[0], resp_buf[1], resp_buf[2], resp_buf[3],
			resp_buf[4]);
	}
	highbyte = ((resp_buf[FW_INFO_INDEX] & 0x0f) << 4) |
		   ((resp_buf[FW_INFO_INDEX + 1] & 0xf0) >> 4);
	lowbyte = ((resp_buf[FW_INFO_INDEX + 1] & 0x0f) << 4) |
		  ((resp_buf[FW_INFO_INDEX + 2] & 0xf0) >> 4);
	g_elan_ts->fw_ver = highbyte << 8 | lowbyte;
	ver_byte[0] = (char)highbyte;
	ver_byte[1] = (char)lowbyte;
	snprintf(g_elan_ts->elan_chip_data->version_name, MAX_STR_LEN,
		 "0x%02x%02x", ver_byte[0], ver_byte[1]);
...
static int elan_read_tx_rx(void)
{
	int ret = 0;
	// clang-format off
	u8 info_buff[ELAN_SEND_DATA_LEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x00, 0x04, 0x5b, 0x00, 0x00,
		0x00, 0x00, 0x00
	};
	// clang-format on
	u8 resp_buf[ELAN_RECV_DATA_LEN] = {0};
	...
	ret = elan_i2c_read(info_buff, sizeof(info_buff), resp_buf,
			    sizeof(resp_buf));
	if (ret) {
...
static int elan_ktf_init_chip(void)
{
...
#endif
	strncpy(g_elan_ts->elan_chip_data->chip_name, ELAN_KTF_NAME,
		strlen(ELAN_KTF_NAME) + 1);
	...
	g_elan_ts->elan_chip_client->panel_id =
		(g_elan_ts->project_id[ELAN_PANEL_ID_START_BIT] - '0') * 10 +
	...
	if (atomic_read(&g_elan_ts->tp_mode) == TP_NORMAL) {
		...
		if (ret == NO_ERR) {
			...
			g_elan_ts->finger_x_resolution =
				(g_elan_ts->tx_num - 1) * FINGER_OSR;
			g_elan_ts->pen_y_resolution =
				(g_elan_ts->rx_num - 1) * PEN_OSR;
			g_elan_ts->pen_x_resolution =
				(g_elan_ts->tx_num - 1) * PEN_OSR;
		} else {
...
static int elan_ktf_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, FINGER_MAJOR, 0,
			     0);
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MINOR, 0, FINGER_MINOR, 0,
			     0);
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, MAX_FINGER_SIZE,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
			     g_elan_ts->elan_chip_data->y_max - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
			     g_elan_ts->elan_chip_data->x_max - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, FINGER_MAJOR, 0,
			     0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MINOR, 0, FINGER_MINOR, 0,
			     0);
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, MAX_FINGER_PRESSURE,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_DISTANCE, 0, MAX_FINGER_SIZE, 0,
			     0);
	return NO_ERR;
}
...
static void elan_report_debug_info(u8 debug_info)
{
	...
	ts_dmd_report(DSM_TP_CHARGER_NOISE_HOP,
		      ...
		      "freq hop:0x%02x\n"
		      "OBL mode:0x%02x\n",
		      DSM_TP_CHARGER_NOISE_HOP, trace_mode, water_mode,
		      palm_mode, freq_mode, obl_mode);
#endif
}
				    ...
				    struct ts_cmd_node *out_cmd)
{
	...
	case PEN_REPORT_ID:
		...
		if (pens->tool.pen_inrange_status !=
		    atomic_read(&g_last_pen_inrange_status)) {
			atomic_set(&g_last_pen_inrange_status,
...
{
	int ret = 0;
	u8 cmd[ELAN_SEND_DATA_LEN] = {0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
...
static int __init elan_ktf_ts_init(void)
{
	...
	if (!g_elan_ts) {
		TS_LOG_ERR(
			"[elan]:Failed to alloc mem for struct elan_ktf_ts_data!\n");
		error = -ENOMEM;
	...
	if (!g_elan_ts->elan_chip_data) {
		TS_LOG_ERR(
			"[elan]:Failed to alloc mem for struct elan_chip_data\n");
		error = -ENOMEM;
		goto free_elan_ts;
	}
	...
	if (!root) {
		TS_LOG_ERR(
			"[elan]:honor_ts, find_compatible_node honor,ts_kit error\n");
		error = -EINVAL;
		goto exit;
	}
