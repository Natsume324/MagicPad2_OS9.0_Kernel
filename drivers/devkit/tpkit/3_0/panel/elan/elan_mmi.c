...
int alloc_data_buf(void)
{
	int rx = 0;
	int tx = 0;
	if ((!g_elan_ts) || (g_elan_ts->rx_num <= 0) ||
	    (g_elan_ts->tx_num <= 0)) {
		TS_LOG_ERR(
			"[elan]:%s,g_elan_ts is NULL or tx rx num is invalid!\n",
			__func__);
		return -EINVAL;
	}
	rx = g_elan_ts->rx_num;
	tx = g_elan_ts->tx_num;
	g_rawdatabuf =
		(uint8_t *)kzalloc(sizeof(uint8_t) * (tx * rx * 2),
				   GFP_KERNEL); // rawdata be saved by two bytes
...
static int elan_ts_get_data_by_cmd(uint8_t *cmd, uint8_t *buf, size_t w_size,
				   size_t r_size)
{
	int ret = 0;
...
int disable_finger_report(void)
{
	int ret = 0;
	// clang-format off
	uint8_t disable_finger_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x54,
		0xCA, 0x00, 0x01
	};
	...
	if (ret) {
		TS_LOG_ERR(
			"[elan]:i2c send Disable Algorithm1 cmd fail ret=%d!\n",
			ret);
	}
	return ret;
}
...
static void parse_valid_data(char *buf_start, loff_t buf_size, char *ptr,
			     int32_t *data, int rows)
{
	...
	char *tok_ptr = NULL;
	char row_data[512] = {
		0}; // ensure have enough space to save one line data
	...
	for (i = 0; i < rows; i++) {
		...
		while ((token = strsep(&tok_ptr, ", \t\n\r\0"))) {
			...
			data[j] =
				(int32_t)simple_strtol(token, NULL, STRTOL_LEN);
			j++;
		}
...
/* lint -save -e* */
int elan_parse_csvfile(char *file_path, char *target_name, int32_t *data,
...
exit_free:
	...
	if (!IS_ERR_OR_NULL(
		    fp)) { // fp open fail not means fp is NULL, so free fp may cause Uncertainty
...
int elan_get_threshold_from_csvfile(int columns, int rows, char *target_name,
				    int32_t *data)
{
	...
	if ((!data) || (!target_name) || (!g_elan_ts) ||
		...
		TS_LOG_ERR(
			"parse csvfile failed, data or target_name or g_elan_ts or elan_chip_data is NULL\n");
		return TEST_FAIL;
	}
	...
	snprintf(file_path, sizeof(file_path), "/odm/etc/firmware/ts/%s",
		 file_name);
	TS_LOG_INFO(
		"[elan]:threshold file name:%s, rows_size=%d, columns_size=%d\n",
		file_path, rows, columns);

	result =
		elan_parse_csvfile(file_path, target_name, data, rows, columns);
	if (result == 0) {
		ret = TEST_PASS;
...
int elan_get_set_opcmd(void)
{
	...
	// clang-format off
	uint8_t get_option_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x53,
		0xB1, 0x00, 0x01
	};
	uint8_t get_option_cmdt[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x53,
		0xC1, 0x00, 0x01
	};
	...
	len = elan_ts_get_data_by_cmd(get_option_cmd, buf_recv, CMD_DATALEN,
				      RECV_DATALEN);
	if (len) {
		...
		TS_LOG_DEBUG(
			"[elan]%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
			buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3],
			buf_recv[4], buf_recv[5], buf_recv[6], buf_recv[7],
			buf_recv[8], buf_recv[9]); // operation databuf
	}

	get_option_cmd[SEND_CMD_BYTE] = WRITE_CMD;
	get_option_cmd[SEND_OPTION_HBYTE] =
		(((buf_recv[OPTION_HIGHBYTE] << 8) | buf_recv[OPTION_LOWBYTE]) &
		 ALG2_SET_OPTION_BIT) >>
		8;
	get_option_cmd[SEND_OPTION_LBYTE] =
		(((buf_recv[OPTION_HIGHBYTE] << 8) | buf_recv[OPTION_LOWBYTE]) &
		 ALG2_SET_OPTION_BIT) &
		0xff;
	...
	if (len) {
		TS_LOG_ERR(
			"[elan]:i2c send Disable Algorithm2 cmd fail,len=%d!\n",
			len);
		return -EINVAL;
	} else {
	...
	len = elan_ts_get_data_by_cmd(get_option_cmdt, buf_recv, CMD_DATALEN,
				      RECV_DATALEN);
	if (len) {
		TS_LOG_ERR("[elan]:i2c send or send op cmd fail!\n");
		return -EINVAL;
		...
		TS_LOG_DEBUG(
			...
			buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3],
			buf_recv[4], buf_recv[5], buf_recv[6], buf_recv[7],
			buf_recv[8], buf_recv[9]);
	}

	get_option_cmdt[SEND_CMD_BYTE] = WRITE_CMD;
	get_option_cmdt[SEND_OPTION_HBYTE] =
		(((buf_recv[OPTION_HIGHBYTE] << 8) | buf_recv[OPTION_LOWBYTE]) &
		 ALG3_SET_OPTION_BIT) >>
		8;
	get_option_cmdt[SEND_OPTION_LBYTE] =
		(((buf_recv[OPTION_HIGHBYTE] << 8) | buf_recv[OPTION_LOWBYTE]) &
		 ALG3_SET_OPTION_BIT) &
		0xff;
	...
	if (len) {
		TS_LOG_ERR(
			"[elan]:i2c send Disable Algorithm3 cmd fail,len=%d!\n",
			len);
		return -EINVAL;
	} else {
		TS_LOG_DEBUG(
			"[elan]:i2c send Disable Algorithm3 cmd suceed!\n");
	}
...
int elan_set_read_ph3(void)
{
	int len = 0;
	// clang-format off
	uint8_t set_tp_parameter_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x54,
		0x2d, 0x00, 0x01
	};
	uint8_t ph3_cmd_buf[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x54, 0xC7, 0x00, PH3
	};
	// clang-format on
	...
	len = elan_i2c_write(set_tp_parameter_cmd,
			     sizeof(set_tp_parameter_cmd));
	if (len) {
	...
	len = elan_i2c_read(ph3_cmd_buf, sizeof(ph3_cmd_buf), res_buf,
			    sizeof(res_buf));
...
int elan_read_ph(void)
{
	...
	uint8_t buf_recv[RECV_DATALEN] = {0};
	// clang-format off
	uint8_t ph1_cmd_buf[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x53, 0xC5, 0x00, 0x01
	};
	uint8_t ph2_cmd_buf[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x53, 0xC6, 0x00, 0x01
	};
	uint8_t ph3_cmd_buf[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
		0x04, 0x53, 0xC7, 0x00, 0x01
	};
	// clang-format on

	len = elan_ts_get_data_by_cmd(ph1_cmd_buf, buf_recv, CMD_DATALEN,
				      RECV_DATALEN);
	if (len) {
		...
		TS_LOG_INFO(
			"[elan]:ph1:%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
			buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3],
			buf_recv[4], buf_recv[5], buf_recv[6],
			buf_recv[7]); // 0-3th hid head data 4-7 ph1 data
	}

	len = elan_ts_get_data_by_cmd(ph2_cmd_buf, buf_recv, CMD_DATALEN,
				      RECV_DATALEN);
	if (len) {
		...
		TS_LOG_INFO(
			"[elan]:ph2:%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
			buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3],
			buf_recv[4], buf_recv[5], buf_recv[6],
			buf_recv[7]); // 0-3th hid head data 4-7 ph2 data
	}

	len = elan_ts_get_data_by_cmd(ph3_cmd_buf, buf_recv, CMD_DATALEN,
				      RECV_DATALEN);
	if (len) {
		...
		TS_LOG_INFO(
			"[elan]:ph3:%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x\n",
			buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3],
			buf_recv[4], buf_recv[5], buf_recv[6],
			buf_recv[7]); // 0-3th hid head data 4-7 ph3 data
	}
	return NO_ERR;
}
...
int elan_calibration_base(void)
{
	...
	// clang-format off
	uint8_t rsp_buf[4] = {
		CMD_HEADER_REK, CMD_HEADER_REK, CMD_HEADER_REK, CMD_HEADER_REK
	};
	uint8_t cmd_buf[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00, 0x04, 0x54, 0xC0, 0xE1, 0x5A
	}; // flash_key
	uint8_t rek_cmd_buf[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03, 0x00,
	...
	ret = elan_ts_get_data_by_cmd(rek_cmd_buf, buff, CMD_DATALEN,
				      RECV_DATALEN);
	if (ret) {
	...
	ret = memcmp(
		buff + 4, rsp_buf,
		sizeof(rsp_buf)); // rsp_buf 4th-7th is rek data(right 0x66)
	if (ret) {
...
static int enter_test_mode(void)
{
	int ret = 0;
	// clang-format off
	uint8_t enter_test_mode_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x55,
		0x55, 0x55, 0x55
	};
...
static int exit_test_mode(void)
{
	int ret = 0;
	// clang-format off
	uint8_t exit_test_mode_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0xa5,
		0xa5, 0xa5, 0xa5
	};
...
int elan_noise_test(int rx, int tx, struct noise_limit limit)
{
	...
	for (i = 0; i < tx; i++) {
		for (j = 0; j < rx; j++) {
			if (dv[i][j] > limit.noise_limit_hb[j + rx * i] ||
			    dv[i][j] < limit.noise_limit_lb[j + rx * i]) {
	...
	if (over_hb > limit.noise_test_hb_fail_point ||
	    over_lb > limit.noise_test_lb_fail_point) {
		TS_LOG_ERR(
			"[elan]:Noise Over HB or LB Fail,Fail HBnum=%d,LBnum=%d\n",
			over_hb, over_lb);
		return 1; // test fail
	}
...
int elan_get_rawdata(int datalen, bool checkData)
{
	...
	recv_num = datalen / VALID_DATA_LEN +
		   ((datalen % VALID_DATA_LEN) != 0); // receive data times
	for (i = 0; i < recv_num; i++) {
		...
		if (checkData) {
			if ((i == (recv_num - 1)) &&
			    ((datalen % VALID_DATA_LEN) != 0)) {
				memcpy(g_rawdatabuf + VALID_DATA_LEN * i,
				       databuf + 7,
				       datalen %
					       VALID_DATA_LEN); // 0-6 head data
			} else {
				memcpy(g_rawdatabuf + VALID_DATA_LEN * i,
				       databuf + 7,
				       VALID_DATA_LEN); // 0-6 head data
...
int get_noise_test_data(struct ts_rawdata_info *info)
{
	...
	// clang-format off
	uint8_t get_dvdata_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x00, 0x06, 0x58, 0x04, 0x00,
		0x00, 0x00, 0x00
	};
	// clang-format on
	if (!info || !g_elan_ts || g_elan_ts->tx_num <= 0 ||
	    g_elan_ts->rx_num <= 0) {
	...
	ret = elan_get_threshold_from_csvfile(rx, tx, NOISE_TEST_LIMIT_HB,
					      noise.noise_limit_hb);
	if (ret == TEST_FAIL) {
		TS_LOG_ERR("get NoiseTestLimitHB err\n");
		ret = -EINVAL;
		goto free_test_space;
	}
	ret = elan_get_threshold_from_csvfile(rx, tx, NOISE_TEST_LIMIT_LB,
					      noise.noise_limit_lb);
	if (ret == TEST_FAIL) {
		TS_LOG_ERR("get NoiseTestLimitLB err\n");
		ret = -EINVAL;
		goto free_test_space;
	}
	ret = elan_get_threshold_from_csvfile(rx, tx, NOISE_TEST_HB,
					      noise.noise_test_hb);
	if (ret == TEST_FAIL) {
	...
	ret = elan_get_threshold_from_csvfile(rx, tx, NOISE_TEST_LB,
					      noise.noise_test_lb);
	if (ret == TEST_FAIL) {
	...
	ret = elan_get_threshold_from_csvfile(2, 1, NOISE_TEST_FAIL_POINT,
					      data_buf); // 2 col, 1 row
	if (ret == TEST_FAIL) {
	...
	TS_LOG_INFO(
		"[elan]:noise_test_hb_fail_point = %d,noise_test_lb_fail_point = %d\n",
	...
	for (i = 0; i < NOISE_SKIP_FRAME + NOISE_TEST_FRAME; i++) {
		...
		if (ret) {
			TS_LOG_ERR(
				"[elan]:%s,send get dv data cmd fail!ret=%d\n",
				__func__, ret);
			ret = -EINVAL;
			goto free_test_space;
		}
...
int enter_get_data_mode(int adc, int frameNum)
{
	...
	uint8_t buf_recv[RECV_DATALEN] = {0};
	// clang-format off
	uint8_t enter_getdata_mode_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23,
		0x00, 0x03, 0x00,
		0x04, 0x54, 0xcd,
		adc, frameNum
	...
	ret = elan_ts_get_data_by_cmd(enter_getdata_mode_cmd, buf_recv,
				      CMD_DATALEN, RECV_DATALEN);
	if (ret) {
		TS_LOG_ERR("[elan]:send get data mode cmd fail!\n");
		return -EINVAL;
	}
	ret = memcmp(buf_recv + 4, ready_ack,
		     sizeof(ready_ack)); // buf_recv 4th-7th is ack data
	if (ret) {
...
int get_normal_adc_data(
	bool checkData) // checkData true is test ,false is skipframe
{
	int ret = 0;
	int tx = 0;
	int rx = 0;
	int data_length = 0;
	// clang-format off
	uint8_t data_mode_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
	...
	if (checkData) {
		TS_LOG_INFO(
			"---->elan_mmi: Get Normal ADC Data_[2] Start<----\n");
		print_rawdata(rx, tx, NOISE_ADC_PRINT, NULL);
	}
#endif
	return NO_ERR;
...
int enter_short_test_mode(void)
{
	int ret = 0;
	// clang-format off
	uint8_t short_test_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00,
		0x03, 0x00, 0x04, 0x54,
		0x33, 0x10, 0x00
	};
...
int get_txrx_short_data(int data, int frame, bool checkData)
{
	...
	uint8_t enter_getdata_mode_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x00, 0x04, 0x54, 0x34, 0x80,
		(uint8_t)data | (uint8_t)frame
	};
	uint8_t get_short_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x00, 0x06, 0x58, 0x01, 0x00,
		0x00, 0x00, 0x00
	};
	uint8_t dummy_cmd[CMD_DATALEN] = {
		0x04, 0x00, 0x23, 0x00, 0x03,
		0x00, 0x06, 0x58, 0xc2, 0x00,
		0x00, 0x00, 0x00
	};
	// clang-format on
	...
	get_short_cmd[RAW_DATALEN_HIGHBYTE] =
		((unsigned int)data_length & 0xff00) >> 8;
	get_short_cmd[RAW_DATALEN_LOWBYTE] = (unsigned int)data_length & 0x00ff;

	ret = elan_i2c_write(enter_getdata_mode_cmd,
			     sizeof(enter_getdata_mode_cmd));
	if (ret) {
		...
		TS_LOG_DEBUG(
			"[elan]:send get data cmd:%02x,%02x,%02x,%02x succeed\n",
			enter_getdata_mode_cmd[7], enter_getdata_mode_cmd[8],
			enter_getdata_mode_cmd[9], enter_getdata_mode_cmd[10]);
	}
	...
	if (checkData) {
		if (data == TEST_ADC_DATA) {
			g_test_adc_databuf = (short *)kzalloc(
				sizeof(short) * (tx + rx), GFP_KERNEL);
			if (g_test_adc_databuf == NULL) {
				TS_LOG_ERR(
					"[elan]:g_test_adc_databuf is NULL\n");
				return -ENOMEM;
			}
			memcpy(g_test_adc_databuf, g_totlebuf,
			       sizeof(short) * (tx + rx));
		} else if (data == TEST_ADC_DATA_2) {
			g_test_adc_databuf2 = (short *)kzalloc(
				sizeof(short) * (tx + rx), GFP_KERNEL);
			if (g_test_adc_databuf2 == NULL) {
				TS_LOG_ERR(
					"[elan]:g_test_adc_databuf2 is NULL\n");
				return -ENOMEM;
			}
			memcpy(g_test_adc_databuf2, g_totlebuf,
			       sizeof(short) * (tx + rx));
		}
#ifdef PRINT_RAWDATA
		TS_LOG_INFO(
			"--->elan_mmi: Check Open Short Data[2] Start<---\n");
...
static int elan_tx_open_test(void)
{
	...
	short *tx_diff_buff =
		(short *)kzalloc(sizeof(short) * (tx - 1) * rx, GFP_KERNEL);
	if (!tx_diff_buff) {
	...
	res = elan_get_threshold_from_csvfile(rx, tx - 1, TX_DIFFER_HB,
					      csv_tx_diff_hb);
	if (res == TEST_FAIL) {
	...
	res = elan_get_threshold_from_csvfile(1, 1, TX_OVER_HBPT_RATIO,
					      data_buf);
	if (res == TEST_FAIL) {
	...
	res = elan_get_threshold_from_csvfile(1, 1, TX_EDGE_TRACE_COUNT,
					      data_buf);
	if (res == TEST_FAIL) {
	...
	for (i = 0; i < tx - 1; i++) {
		for (j = 0; j < rx; j++) {
			if (databuf[i][j] < 0 || databuf[i + 1][j] < 0) {
				TS_LOG_ERR(
					"[elan]:%s,databuf[%d][%d]=%d,databuf[%d][%d]=%d\n",
					__func__, i, j, databuf[i][j], i + 1, j,
					databuf[i + 1][j]);
				ret = -EINVAL;
				goto tx_open_test_exit;
			}
			tx_diff_data[i][j] = databuf[i][j] - databuf[i + 1][j];
			if (!((j >= csv_tx_edge_trace_count) &&
			      (j <= rx - 1 - csv_tx_edge_trace_count) &&
			      (i >= csv_tx_edge_trace_count) &&
			      (i <=
			       tx - 2 - csv_tx_edge_trace_count))) { // tx-2 is edge
				if (abs(tx_diff_data[i][j]) >
				    (databuf[i][j] *
				     csv_tx_diff_hb[j + rx * i] / 100)) {
					TS_LOG_INFO(
						"[over_hbelan]:Edge====i=%d,j=%d,tx_diff_data=%d\n",
				...
				if (abs(tx_diff_data[i][j]) >
				    (databuf[i][j] *
				     csv_tx_diff_hb[j + rx * i] / 100)) {
					TS_LOG_INFO(
						"[over_hbelan]:====i=%d,j=%d,tx_diff_data=%d\n",
						i, j, tx_diff_data[i][j]);
					over_hb++;
				}
			}
...
static int elan_rx_open_test(void)
{
	...
	short *rx_diff_buff =
		(short *)kzalloc(sizeof(short) * tx * (rx - 1), GFP_KERNEL);
	if (!rx_diff_buff) {
	...
	res = elan_get_threshold_from_csvfile(rx - 1, tx, RX_DIFFER_HB,
					      csv_rx_differ_hb);
	if (res == TEST_FAIL) {
	...
	res = elan_get_threshold_from_csvfile(1, 1, RX_OVER_HBPT_RATIO,
					      data_buf);
	if (res == TEST_FAIL) {
	...
	res = elan_get_threshold_from_csvfile(1, 1, RX_EDGE_TRACE_COUNT,
					      data_buf);
	if (res == TEST_FAIL) {
	...
	for (i = 0; i < tx; i++) {
		for (j = 0; j < rx - 1; j++) {
			...
			rx_diff_data[i][j] =
				abs(databuf[i][j] - databuf[i][j + 1]);
			if (!((j >= csv_rx_edge_trace_ratio) &&
			      (j <= rx - 2 - csv_rx_edge_trace_ratio) &&
			      (i >= csv_rx_edge_trace_ratio) &&
			      (i <=
			       tx - 1 - csv_rx_edge_trace_ratio))) { // rx-2 is edge
				if (rx_diff_data[i][j] >
				    (databuf[i][j] *
				     csv_rx_differ_hb[j + (rx - 1) * i] /
				     100)) {
					TS_LOG_INFO(
						"[elan over_hb]:i=%d,j=%d\n", i,
						j);
					edge_over_hb++;
				}
			} else {
...
int elan_txrx_short_test(void)
{
	...
	if (!g_test_adc_databuf || !g_test_adc_databuf2 || !g_elan_ts ||
	    g_elan_ts->rx_num <= 1 ||
	...
	rx = g_elan_ts->rx_num;
	tx = g_elan_ts->tx_num;
	ret = elan_get_threshold_from_csvfile(
		tx - 1, 1, TX_TRACE_ADJ_DIFFER_HB_COND1,
		csv_tx_trace_adjust_differ_hb_cond1);
	if (ret == TEST_FAIL) {
		TS_LOG_ERR("get TX_TRACE_ADJ_DIFFER_HB_COND1 err\n");
		return -EINVAL;
	}
	ret = elan_get_threshold_from_csvfile(
		rx - 1, 1, RX_TRACE_ADJ_DIFFER_HB_COND1,
		csv_rx_trace_adjust_differ_hb_cond1);
	if (ret == TEST_FAIL) {
	...
	for (i = 0; i < (rx + tx); i++) {
		if (g_test_adc_databuf[i] == 0 || g_test_adc_databuf2[i] == 0) {
			...
			TS_LOG_ERR(
				"[elan]:g_test_adc_databuf[%d]=%d,g_test_adc_databuf2[%d]=%d",
				i, g_test_adc_databuf[i], i,
				g_test_adc_databuf2[i]);
		}
		differ_data[i] =
			abs(g_test_adc_databuf[i] - g_test_adc_databuf2[i]);
	...
	for (i = 0; i < (rx - 1); i++) {
		...
		if ((i > 0) && (i < rx - 2)) {
			if (differ_data[i] >
			    (RX_TRACE_TARGET_VALUE *
			     csv_rx_trace_adjust_differ_hb_cond1[i] /
			     100)) { // 100 is percent
				over_hb++;
	...
	for (i = rx; i < (rx + tx - 1); i++) {
		...
		if ((i > rx) && (i < rx + tx - 2)) {
			if ((differ_data[i] >
			     TX_TRACE_TARGET_VALUE *
				     csv_tx_trace_adjust_differ_hb_cond1[i - rx] /
				     100)) {
				over_hb++;
			}
		} else {
...
static int elan_txrx_short(struct ts_rawdata_info *info)
{
	...
	ret = get_txrx_short_data(TEST_ADC_DATA_2, NORMAL_ADC_SKIP_FRAME,
				  false);
	if (ret) {
...
{
	int ret = 0;
	if (!info || !g_elan_ts || (g_elan_ts->tx_num < 1) ||
	    (g_elan_ts->rx_num < 1)) {
		TS_LOG_ERR("[elan]:%s,info is null\n", __func__);
		return -EINVAL;
	}
