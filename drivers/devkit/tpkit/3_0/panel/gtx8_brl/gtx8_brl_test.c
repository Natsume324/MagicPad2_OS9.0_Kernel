...
static void brl_put_test_result_newformat(struct ts_rawdata_info_new *info,
					  struct brl_ts_test *ts_test);
...
static int send_test_cmd(struct brl_ts_test *ts_test, struct brl_ts_cmd *cmd)
{
	int ret;
...
static int brl_init_testlimits(struct brl_ts_test *ts_test)
{
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SELF_RAW_TX_MAX, data_buf,
				   1, tx);
	if (ret) {
		TS_LOG_ERR("Failed get %s", CSV_TP_SELF_RAW_TX_MAX);
		return ret;
	}
	memcpy(ts_test->test_params.self_max_limits, data_buf,
	       tx * sizeof(data_buf[0]));

	/* obtain self_raw_tx_min */
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SELF_RAW_TX_MIN, data_buf,
				   1, tx);
	if (ret) {
		TS_LOG_ERR("Failed get %s", CSV_TP_SELF_RAW_TX_MIN);
		return ret;
	}
	memcpy(ts_test->test_params.self_min_limits, data_buf,
	       tx * sizeof(data_buf[0]));

	/* obtain self_raw_rx_max */
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SELF_RAW_RX_MAX, data_buf,
				   1, rx);
	if (ret) {
		TS_LOG_ERR("Failed get %s", CSV_TP_SELF_RAW_RX_MAX);
		return ret;
	}
	memcpy(ts_test->test_params.self_max_limits + tx, data_buf,
	       rx * sizeof(data_buf[0]));

	/* obtain self_raw_rx_min */
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SELF_RAW_RX_MIN, data_buf,
				   1, rx);
	if (ret) {
		TS_LOG_ERR("Failed get %s", CSV_TP_SELF_RAW_RX_MIN);
		return ret;
	}
	memcpy(ts_test->test_params.self_min_limits + tx, data_buf,
	       rx * sizeof(data_buf[0]));
...
static int analysis_delta_data(struct brl_ts_test *ts_test)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val > ts_test->test_params.tx_delta_max[i] ||
			...
			TS_LOG_ERR(
				"tx_data isn't in range,value:%d threshold:%d,%d\n",
				val, ts_test->test_params.tx_delta_min[i],
	...
	for (i = 0; i < size; i++) {
		...
		if (val > ts_test->test_params.rx_delta_max[i] ||
			...
			TS_LOG_ERR(
				"rx_data isn't in range,value:%d threshold:%d,%d\n",
				val, ts_test->test_params.rx_delta_min[i],
...
static int brl_cache_noisedata(struct brl_ts_test *ts_test, s16 *noise,
			       int data_size)
{
	u8 buf[1];
	...
	ret = ts->ops.i2c_read(noise_reg, (u8 *)noise,
			       data_size * sizeof(*noise));
...
static u8 analysis_raw_data(struct brl_ts_test *ts_test)
{
	...
	for (i = 0; i < size; i++) {
		...
		if ((val < ts_test->test_params.min_limits[i]) ||
			...
			TS_LOG_ERR(
				"mutual_raw data isn't in range, value:%d threshold:%d,%d\n",
	...
	for (i = 0; i < size; i++) {
		...
		if ((val < ts_test->test_params.self_min_limits[i]) ||
			...
			TS_LOG_ERR(
				"self_raw data isn't in range,value:%d threshold:%d,%d\n",
...
static int analysis_noisedata(struct brl_ts_test *ts_test)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val > ts_test->test_params.noise_threshold) {
			TS_LOG_ERR(
				"the noisedata data is not in range, value:%d threshold:%d",
...
static int analysis_self_noisedata(struct brl_ts_test *ts_test)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val > ts_test->test_params.self_noise_threshold) {
			TS_LOG_ERR(
				"the self_noisedata data is not in range, value:%d threshold:%d",
...
static void brl_capacitance_test(struct brl_ts_test *ts_test)
{
	...
	for (cnt = 0; cnt < MAX_DIFFDATA_TIMES; cnt++) {
		...
		ret = brl_cache_noisedata(ts_test, ts_test->noisedata[cnt].data,
					  data_size);
		if (ret) {
			ts_test->test_result[GTP_NOISE_TEST] =
				SYS_SOFTWARE_REASON;
...
static void brl_prepare_test_failed(struct ts_rawdata_info_new *info)
{
	int dest_len;
	int available_len;
...
{
	snprintf(info->i2cinfo, TS_RAWDATA_RESULT_CODE_LEN, "%d%c", 0, 'P');
}
 ...
 */
static void brl_data_statistics(u16 *data, size_t data_size, char *result,
				size_t res_size)
{
	u16 i;
	u16 avg;
					  ...
					  struct brl_ts_test *ts_test)
{
	...
	if (ts_test->rawdata.size) {
		pts_node->values =
			kcalloc(ts_test->rawdata.size, sizeof(int), GFP_KERNEL);
		if (!pts_node->values) {
	...
	pts_node->size =
		ts_test->tx_delta.size + ts_test->rx_delta.size + info->tx;
	if (pts_node->size) {
		pts_node->values =
			kcalloc(pts_node->size, sizeof(int), GFP_KERNEL);
		if (!pts_node->values) {
			TS_LOG_ERR("malloc failed  for values");
			kfree(pts_node);
			pts_node = NULL;
		...
		brl_data_statistics(&ts_test->tx_delta.data[0],
				    ts_test->tx_delta.size, statistics_data,
				    STATISTICS_DATA_LEN - 1);
		strncpy(pts_node->statistics_data, statistics_data,
			sizeof(pts_node->statistics_data) - 1);
	}
	...
	if (ts_test->self_rawdata.size) {
		pts_node->values = kcalloc(ts_test->self_rawdata.size,
					   sizeof(int), GFP_KERNEL);
		if (!pts_node->values) {
			TS_LOG_ERR("malloc failed  for values\n");
			kfree(pts_node);
	...
	if (ts_test->self_noisedata.size) {
		pts_node->values = kcalloc(ts_test->self_noisedata.size,
					   sizeof(int), GFP_KERNEL);
		if (!pts_node->values) {
			TS_LOG_ERR("malloc failed  for values\n");
			kfree(pts_node);
	...
	snprintf(fw_vendor, sizeof(fw_vendor), "%02X_%02X_%X",
	...
	available_len = TS_RAWDATA_DEVINFO_MAX - dest_len - 1;
	if (available_len > 0)
		strncat(&info->deviceinfo[dest_len], fw_vendor, available_len);
...
static void brl_shortcircut_test(struct brl_ts_test *ts_test)
{
	...
	ret = ts_test->ts->ops.i2c_read(SHORT_TEST_TIME_REG, (u8 *)&test_time,
					2);
	if (ret) {
		...
		if (test_time > MAX_TEST_TIME_MS) {
			TS_LOG_INFO("test time too long %d > %d", test_time,
				    MAX_TEST_TIME_MS);
	...
	do {
		...
		if (!ret && tmp_cmd.state == INSPECT_CMD_STATUS_FINISH)
			break;
		TS_LOG_INFO("short test ack 0x%x status 0x%x", tmp_cmd.ack,
			    tmp_cmd.state);
		msleep(50); /* 50 : 50ms */
	} while (--retry);
...
static int brl_load_mbist(struct brl_ts_test *ts_test)
{
	...
	if (memcmp(&mbist_fw_version.patch_pid[0], "MBIST", 5)) {
		TS_LOG_ERR("patch id error %c%c%c%c%c != %s",
			   mbist_fw_version.patch_pid[0],
			   mbist_fw_version.patch_pid[1],
			   mbist_fw_version.patch_pid[2],
			   mbist_fw_version.patch_pid[3],
...
static void brl_mbist_test(struct brl_ts_test *ts_test)
{
	...
	for (retry = 0; retry < 20; retry++) {
		ret = ts_test->ts->ops.i2c_read(REG_MBIST_TEST_CTRL, reg_val,
						2);
	...
	if (ret || reg_val[0] != MBIST_TEST_STAT0 ||
		...
		TS_LOG_ERR("failed get mbist test result 0x%x 0x%x", reg_val[0],
			   reg_val[1]);
		goto exit;
	}
