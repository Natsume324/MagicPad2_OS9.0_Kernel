 ...
 * return: 0 - OK, < 0 - error
 */
static int brl_request_firmware(struct firmware_data *fw_data, const char *name)
{
	int ret;
...
static int brl_parse_firmware(struct firmware_data *fw_data)
{
	...
	for (i = 0; i < fw_info->subsys_num; i++) {
		...
		fw_info->subsys[i].size = le32_to_cpup(
			(__le32 *)&firmware->data[info_offset + 1]);

		fw_info->subsys[i].flash_addr = le32_to_cpup(
...
static int brl_update_prepare(struct firmware_data *fw_data)
{
	...
	do {
		...
		if (!r && temp_buf[0] == 0x01)
			break;
		usleep_range(1000, 1100); /* 1000 : 1ms */
		TS_LOG_INFO("retry %d enable esd key, 0x%x", retry,
			    temp_buf[0]);
	} while (--retry);
	if (!retry) {
...
static int brl_send_flash_cmd(struct brl_flash_cmd *flash_cmd)
{
	...
	retry = 5;
	for (i = 0; i < retry; i++) {
		ret = brl_reg_read(FLASH_CMD_REG, tmp_cmd.buf,
				   sizeof(tmp_cmd.buf));
		if (!ret && tmp_cmd.ack == FLASH_CMD_ACK_CHK_PASS)
			break;
		usleep_range(5000, 5100); /* 5000 : 5ms */
		TS_LOG_ERR("flash cmd ack error retry %d, ack 0x%x, ret %d", i,
			   tmp_cmd.ack, ret);
	}
	if (tmp_cmd.ack != FLASH_CMD_ACK_CHK_PASS) {
		TS_LOG_ERR("flash cmd ack error, ack 0x%x, ret %d", tmp_cmd.ack,
			   ret);
		TS_LOG_ERR("data:%*ph", (int)sizeof(tmp_cmd.buf), tmp_cmd.buf);
		return -EINVAL;
	}
...
static int brl_flash_package(u8 subsys_type, u8 *pkg, u32 flash_addr,
			     u16 pkg_len)
{
	int ret, retry;
...
static int brl_flash_firmware(struct fw_update_ctrl *fw_ctrl)
{
	...
	for (i = 1; i < fw_num && retry;) {
		...
		if (r == 0) {
			...
			retry--;
			TS_LOG_ERR("End flash subsystem%d:Fail,errno:%d", i, r);
