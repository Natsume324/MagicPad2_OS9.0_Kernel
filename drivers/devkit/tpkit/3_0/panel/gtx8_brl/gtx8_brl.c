...
static int brl_i2c_read_transfer(u32 addr, u8 *buffer, u32 len)
{
	...
	data[3] = (u8)(addr & 0xFF);
	ret = dev_data->ts_platform_data->bops->bus_read(data, sizeof(data),
							 buffer, len);
	if (ret < 0)
...
static int brl_i2c_write_transfer(u32 addr, u8 *buffer, u32 len)
{
	...
	ret = dev_data->ts_platform_data->bops->bus_write(
		data, len + 4); /* 4: addr offset */
	if (ret < 0)
...
static void print_ic_info(struct brl_ic_info *ic_info)
{
	...
	TS_LOG_INFO("ic_info_length:                %d", ic_info->length);
	...
	TS_LOG_INFO("config_id:                     0x%4X", version->config_id);
	...
	TS_LOG_INFO("Drv*Sen,Button,Force num:      %d x %d, %d, %d",
		    parm->drv_num, parm->sen_num, parm->button_num,
		    parm->force_num);

	TS_LOG_INFO("Cmd:                           0x%04X, %d", misc->cmd_addr,
		    misc->cmd_max_len);
	...
	TS_LOG_INFO("esd_addr:                      0x%04X", misc->esd_addr);
}
...
static int brl_get_ic_info(struct brl_ic_info *ic_info)
{
	...
	for (i = 0; i < GOODIX_GET_IC_INFO_RETRY; i++) {
		ret = brl_i2c_read(GOODIX_IC_INFO_ADDR, (u8 *)&length,
				   sizeof(length));
		if (ret < 0) {
	...
	if (!ic_info->misc.cmd_addr || !ic_info->misc.fw_buffer_addr ||
		...
		TS_LOG_ERR(
			"cmd_addr fw_buf_addr and touch_data_addr is null\n");
		return -1;
	}
	return 0;
}
...
static int brl_read_config(u8 *cfg, u32 size)
{
	...
	if (cfg_head.cfg_len > max_len || cfg_head.cfg_len > size) {
	...
	ret = brl_i2c_read(reg_cfg + sizeof(cfg_head), cfg + sizeof(cfg_head),
			   cfg_head.cfg_len);
	if (ret) {
...
static int brl_read_version(struct brl_ts_version *version)
{
	...
	TS_LOG_INFO("rom_vid:%02x %02x %02x", version->rom_vid[0],
		    version->rom_vid[1], version->rom_vid[2]);
	TS_LOG_INFO("patch_pid:%s", version->patch_pid);
	TS_LOG_INFO("patch_vid:%02x %02x %02x %02x", version->patch_vid[0],
		    version->patch_vid[1], version->patch_vid[2],
		    version->patch_vid[3]);
	TS_LOG_INFO("sensor_id:%d", version->sensor_id);

	snprintf(brl_ts->dev_data->version_name, MAX_STR_LEN - 1, "0x%04X",
		 get_u32(version->patch_pid));
...
static int brl_prepare_parse_dts(struct ts_kit_platform_data *pdata)
{
	...
	ret = of_property_read_u32(pdata->chip_data->cnode, GTX8_SLAVE_ADDR,
				   &value);
	if (ret) {
		TS_LOG_ERR(
...
static int brl_pinctrl_init(void)
{
	...
	if (IS_ERR_OR_NULL(ts->pinctrl)) {
		TS_LOG_INFO("%s : Failed to get pinctrl[ignore]:%ld", __func__,
			    PTR_ERR(ts->pinctrl));
		ts->pinctrl = NULL;
...
static void brl_pinctrl_select_normal(void)
{
	...
	if (ts->pinctrl && ts->pins_default) {
		...
		if (ret < 0)
			TS_LOG_ERR("%s:Set normal pin state error:%d", __func__,
				   ret);
	}
}
...
static int brl_power_on(void)
{
	...
	if (ret)
		return ret;
	ret = ts_kit_power_supply_ctrl(TS_KIT_IOVDD, TS_KIT_POWER_ON,
				       15); /* 15: 15ms */
	if (ret)
		return ret;
...
static int brl_parse_dts(void)
{
	...
	ret = of_property_read_u32(device, GTP_ALGO_ID, &chip_data->algo_id);
	if (ret) {
...
static int brl_parse_cfg_data(const struct firmware *cfg_bin, char *cfg_type,
			      u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (i >= bin_group_num * bin_cfg_num) {
		TS_LOG_ERR("%s:(not find config ,config_status: %d)", __func__,
			   config_status);
		goto exit;
	}
...
static int brl_get_cfg_data(const struct firmware *cfg_bin, char *config_name,
			    struct brl_ts_config *config)
{
	...
	/* parse config data */
	ret = brl_parse_cfg_data(cfg_bin, config_name, cfg_data, &cfg_len, 0);
	if (ret < 0) {
	...
	TS_LOG_INFO("%s: %s  version:%d , size:%d", __func__, config_name,
		    cfg_data[34], cfg_len); /* 34:version offset */
...
static int brl_get_cfg_parms(struct brl_ts_data *ts, const char *filename)
{
	...
	ret = brl_get_cfg_data(cfg_bin, GTX8_NORMAL_CONFIG, &ts->normal_cfg);
	if (ret < 0)
		TS_LOG_ERR("%s: Failed to parse normal_config data:%d",
			   __func__, ret);
	ret = brl_get_cfg_data(cfg_bin, GTX8_TEST_CONFIG, &ts->test_cfg);
	if (ret < 0)
		TS_LOG_ERR("%s: Failed to parse test_config data:%d", __func__,
			   ret);
...
static int brl_sub_board_connect_check(struct ts_kit_platform_data *data)
{
	int gpio_value;
	int retval;
	int i;
...
static int brl_ts_roi_init(struct brl_ts_roi *roi)
{
	...
	roi->roi_mat_src.data =
		kzalloc((ROI_COLS * ROI_COLS + 1) * sizeof(short), GFP_KERNEL);
...
static int brl_get_fw_prefix(void)
{
	...
	if ((strlen(ts_platform_data->product_name) +
	     strlen(brl_ts->project_id) + 2 * sizeof(char)) > MAX_STR_LEN) {
...
static int brl_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_READ:
		TS_LOG_INFO(
			"%s: invalid glove switch%u action: TS_ACTION_READ\n",
...
static int brl_charger_switch(struct ts_charger_info *info)
{
	...
	if (!info || !info->charger_supported || !brl_ts || !brl_ts->dev_data ||
	    !brl_ts->dev_data->ts_platform_data) {
...
static int brl_holster_switch(struct ts_holster_info *info)
{
	...
	case TS_ACTION_READ:
		TS_LOG_INFO(
			"%s: invalid holster switch%u action: TS_ACTION_READ\n",
...
static int brl_chip_init(void)
{
	...
	tee_tui_data.tui_special_feature_support =
		ts->dev_data->ts_platform_data->chip_data
...
static int brl_touch_handler(struct brl_ts_data *ts, struct ts_fingers *info,
...
static int brl_gesture_handler(struct brl_ts_data *ts, struct ts_fingers *info,
			       u8 *pre_buf, int buf_len)
{
	int ret = NO_ERR;
...
static int brl_chip_get_info(struct ts_chip_info_param *info)
{
	...
	snprintf(info->fw_vendor, sizeof(info->fw_vendor), "%02X_%02X_%X",
		 ts->hw_info.patch_vid[PATCH_VID_MINOR],
	...
	if (ts->dev_data->ts_platform_data->hide_plain_id &&
		 ...
		 */
		snprintf(info->ic_vendor, CHIP_INFO_LENGTH * 2, "TP-%s-%s",
			 info->mod_vendor, info->fw_vendor);
...
static int brl_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
	int retval = NO_ERR;
	...
	switch (info->op_action) {
	case TS_ACTION_READ:
		strncpy(info->tp_test_type, brl_ts->dev_data->tp_test_type,
				   ...
				   struct roi_matrix *dst_mat)
{
	...
	memcpy(src_mat->data, temp_buf,
	       src_mat->row * src_mat->col * sizeof(short));
...
static int brl_cache_roidata_device(struct brl_ts_roi *roi)
{
	...
	/* read ROI rawdata */
	ret = brl_i2c_read(
		(ts->brl_roi_data_add + ROI_HEAD_LEN), (char *)src_mat.data,
...
struct ts_device_ops ts_brl_ops = {
	...
	.chip_get_capacitance_test_type = brl_chip_get_capacitance_test_type,
};
...
static int __init brl_ts_module_init(void)
{
	...
	brl_ts->write_buf = kzalloc(GOODIX_TRANSFER_MAX_SIZE + 4, GFP_KERNEL);
