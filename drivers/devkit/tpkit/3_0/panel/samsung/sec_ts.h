...
#define SEC_TS_DEFAULT_PARA_NAME \
	"tsp_sec/s6smc41_para_REL_DGA0_V0106_150114_193317.bin"
...
#define SEC_TS_NVM_OFFSET_LENGTH \
	(SEC_TS_NVM_LAST_BLOCK_OFFSET + SEC_TS_NVM_LAST_BLOCK_SIZE + 1)
...
#define SEC_TS_DEFAULT_ENABLE_BIT_SETFUNC                     \
	(SEC_TS_BIT_SETFUNC_TOUCH | SEC_TS_BIT_SETFUNC_PALM | \
...
#define SEC_TS_MODE_LOWPOWER_FLAG                           \
	(SEC_TS_MODE_SPONGE_SPAY | SEC_TS_MODE_SPONGE_AOD | \
...
struct sec_ts_data {
	...
	struct sec_ts_coordinate
		coord[MAX_SUPPORT_TOUCH_COUNT + MAX_SUPPORT_HOVER_COUNT];
	struct timeval
		time_pressed[MAX_SUPPORT_TOUCH_COUNT + MAX_SUPPORT_HOVER_COUNT];
	...
	int (*sec_ts_i2c_write)(struct sec_ts_data *ts, u8 reg, u8 *data,
				int len);
	int (*sec_ts_i2c_read)(struct sec_ts_data *ts, u8 reg, u8 *data,
			       int len);
	int (*sec_ts_i2c_write_burst)(struct sec_ts_data *ts, u8 *data,
				      int len);
