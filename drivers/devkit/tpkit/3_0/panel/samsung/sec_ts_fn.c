...
int sec_ts_cover_mode_enalbes(struct sec_ts_data *ts, bool enable)
{
	...
	if (enable) {
		ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SET_COVERTYPE,
					   &ts->cover_cmd, 1);
		if (ret < 0) {
			TS_LOG_ERR("%s: Failed to send covertype command: %d",
...
int sec_ts_fix_tmode(struct sec_ts_data *ts, u8 mode_type, u8 state)
{
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_CHG_SYSMODE, tBuff,
				   sizeof(tBuff));
	if (ret < 0)
		TS_LOG_ERR("%s: i2c write SEC_TS_CMD_STATEMANAGE_ON err: %d\n",
			   __func__, ret);
...
static int sec_ts_read_frame(struct sec_ts_data *ts,
			     struct sec_ts_test_mode *mode)
{
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_MUTU_RAW_TYPE, &mode->type,
				   1);
	if (ret < 0) {
	...
	if (mode->type == TYPE_OFFSET_DATA_SDC) {
		...
		ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SET_POWER_MODE, &para,
					   1);
		if (ret < 0) {
...
static int sec_ts_read_channel(struct sec_ts_data *ts,
			       struct sec_ts_test_mode *mode)
{
	unsigned char *pRead = NULL;
	u8 mode_type = TYPE_INVALID_DATA;
	...
	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_TOUCH_SELF_RAWDATA, pRead,
				  data_length);
	if (ret < 0) {
		TS_LOG_ERR("%s: read rawdata failed!\n", __func__);
		goto err_read_data;
...
int run_self_rawcap_read_all(struct sec_ts_data *ts,
			     struct sec_ts_test_mode *mode)
{
	memset(mode, 0x00, sizeof(struct sec_ts_test_mode));
	mode->type = TYPE_OFFSET_DATA_SDC;
...
int run_self_delta_read_all(struct sec_ts_data *ts,
			    struct sec_ts_test_mode *mode)
{
	memset(mode, 0x00, sizeof(struct sec_ts_test_mode));
...
void sec_ts_set_channel_count(struct sec_ts_data *ts,
 ...
 */
int sec_ts_get_delta_debug(struct sec_ts_data *ts,
 ...
 */
int sec_ts_get_rawcap_debug(struct sec_ts_data *ts,
			    struct ts_diff_data_info *info)
{
	int i = 0;
	int ret = NO_ERR;
	...
	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_SELFTEST_RESULT, rBuff,
				  result_size);
	if (ret < 0) {
		TS_LOG_ERR("%s: Selftest execution time out!\n", __func__);
		goto err_exit;
	...
	for (i = 0; i < raw_count; i++) {
		info->buff[i + 2] =
			(rBuff[SEC_TS_SELFTEST_REPORT_SIZE + (i * 2) + 1]
			 << 8) +
			    ...
			    struct sec_ts_test_mode *mode)
{
	...
	TS_LOG_INFO("mutual delta data:[%d,%d] limit:[%d,%d]\n", mode->max,
		    mode->min, chip_data->raw_limit_buf[4],
				...
				struct sec_ts_test_mode *mode)
{
	...
	if ((ts->tx_count * (ts->rx_count - 1)) +
	    ((ts->tx_count - 1) * ts->rx_count))
		mode->avg /= ((ts->tx_count * (ts->rx_count - 1)) +
			      ((ts->tx_count - 1) * ts->rx_count));
	else
	...
	TS_LOG_INFO("mutual rawcap gap test data:[%d,%d] limit:[%d,%d]\n",
		    mode->max, mode->min, chip_data->raw_limit_buf[2],
			     ...
			     struct sec_ts_test_mode *mode)
{
	...
	buf_ptr = &info->buff[info->used_size];
	for (i = 0; i < raw_count; i++) {
		buf_ptr[i] = (rBuff[SEC_TS_SELFTEST_REPORT_SIZE + (i * 2) + 1]
			      << 8) +
			     (rBuff[SEC_TS_SELFTEST_REPORT_SIZE + (i * 2)]);
		mode->avg += buf_ptr[i];
	...
	TS_LOG_ERR("mutual rawcap data:[%d,%d] limit:[%d,%d]\n", mode->max,
		   mode->min, chip_data->raw_limit_buf[0],
...
int sec_ts_get_openshort_result(struct sec_ts_data *ts,
				struct ts_rawdata_info *info)
{
	int ret = NO_ERR;
	u8 *rBuff = NULL;
	...
	if ((rBuff[16] != 0) && (rBuff[17] != 0) && (rBuff[18] != 0) &&
	    (rBuff[19] != 0)) {
				  ...
				  struct sec_ts_test_mode *mode)
{
	...
	TS_LOG_INFO(
		"self raw: col_data:[%d,%d] limit:[%d,%d] row_data:[%d,%d] limit:[%d,%d]\n",
		mode->max, mode->min, chip_data->raw_limit_buf[6],
		chip_data->raw_limit_buf[7], mode->short_max, mode->short_min,
		chip_data->raw_limit_buf[8], chip_data->raw_limit_buf[9]);
err_exit:
	kfree(rBuff);
	return ret;
}
				 ...
				 struct sec_ts_test_mode *mode)
{
	...
	TS_LOG_INFO(
		"self delta: col_data:[%d,%d] limit:[%d,%d] row_data:[%d,%d] limit:[%d,%d]\n",
		mode->max, mode->min, chip_data->raw_limit_buf[10],
		chip_data->raw_limit_buf[11], mode->short_max, mode->short_min,
		chip_data->raw_limit_buf[12], chip_data->raw_limit_buf[13]);
...
int sec_ts_run_self_test(struct sec_ts_data *ts, struct ts_rawdata_info *info)
{
	...
	if (ts->ic_name == S6SY761X) {
		__pm_stay_awake(
			&ts->wakelock); //add wakelock ,avoid i2c suspend
	}
	raw_count = ts->tx_count * ts->rx_count;
	memset(info->result, 0, sizeof(info->result));
	...
	ret = sec_ts_get_rawcapgap_result(ts, info,
					  &mode[SEC_TS_RAWCAPGAP_TEST]);
	if (ret < 0) {
	...
	ret = sec_ts_get_self_rawcap_result(ts, info, sf_offset,
					    &mode[SEC_TS_SELF_CAP_TEST]);
	if (ret < 0) {
	...
	sf_offset = 2;
	ret = sec_ts_get_self_delta_result(ts, info, sf_offset,
					   &mode[SEC_TS_SELF_DETLA_TEST]);
	if (ret < 0) {
...
err_exit:
	...
	snprintf(result_buff, SEC_CMD_STR_LEN,
		 ...
		 mode[SEC_TS_RAWCAP_TEST].avg, mode[SEC_TS_RAWCAP_TEST].max,
		 mode[SEC_TS_RAWCAP_TEST].min, mode[SEC_TS_RAWCAPGAP_TEST].avg,
		 mode[SEC_TS_RAWCAPGAP_TEST].max,
		 mode[SEC_TS_RAWCAPGAP_TEST].min, mode[SEC_TS_DETLA_TEST].avg,
		 mode[SEC_TS_DETLA_TEST].max, mode[SEC_TS_DETLA_TEST].min,
		 mode[SEC_TS_CALDATA_TEST].avg, mode[SEC_TS_CALDATA_TEST].max,
		 mode[SEC_TS_CALDATA_TEST].min);
	...
	if (ts->ic_name == S6SY761X) {
		snprintf(result_buff + ibuff_offset,
			 SEC_CMD_STR_LEN - ibuff_offset, "%s-%s%s-%x.%x.%x.%x",
			 ts->module_name, SEC_TS_VENDOR_NAME, ts->project_id,
			 img_ver[0], img_ver[1], img_ver[2], img_ver[3]);
	} else {
		snprintf(result_buff + ibuff_offset,
			 SEC_CMD_STR_LEN - ibuff_offset, "%s-%s%s-%x.%x.%x.%x",
			 ts->module_name, SEC_TS_VENDOR_NAME,
			 SEC_TS_HW_PROJECTID, img_ver[0], img_ver[1],
			 img_ver[2], img_ver[3]);
	}
