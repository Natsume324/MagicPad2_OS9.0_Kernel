				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	do {
		...
		if (offset == 0)
			ret = ts->sec_ts_i2c_read(
				ts, lv1cmd, &read_lv1_buff[offset], length);
		else
			ret = ts->sec_ts_i2c_read_bulk(
				ts, &read_lv1_buff[offset], length);
					   ...
					   const char *buf, size_t size)
{
	...
	if (on) {
		...
		ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_CALIBRATION_AMBIENT,
					   NULL, 0);
		if (ret < 0)
...
static DEVICE_ATTR(sec_ts_enter_recovery, (S_IWUSR | S_IWGRP), NULL,
		   sec_ts_enter_recovery_store);
static DEVICE_ATTR(sec_ts_regread, S_IRUGO, sec_ts_regread_show, NULL);
static DEVICE_ATTR(sec_ts_gesture_status, S_IRUGO, sec_ts_gesture_status_show,
		   NULL);
