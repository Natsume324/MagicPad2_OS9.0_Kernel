...
static int
sec_ts_wakeup_gesture_enable_switch(struct ts_wakeup_gesture_enable_info *info);
...
static int
sec_ts_chip_get_capacitance_test_type(struct ts_test_type_info *info);
...
struct ts_device_ops ts_kit_sec_ts_ops = {
	...
	.chip_get_capacitance_test_type = sec_ts_chip_get_capacitance_test_type,
...
static int sec_ts_i2c_short_read(struct sec_ts_data *ts, u8 reg, u8 *data,
				 u16 len)
{
	...
	if (remain > 0) {
		do {
			...
			TS_LOG_DEBUG("%s: I2C exceed burstmax %d\n", __func__,
				     remain);
			for (retry = 0; retry < SEC_TS_I2C_RETRY_CNT; retry++) {
				ret = platform_data->bops->bus_read(
					&reg, 0, buf, recv_len);
				if (ret == NO_ERR)
					break;
				usleep_range(1 * 1000, 1 * 1000);
...
static int sec_ts_i2c_write_burst(struct sec_ts_data *ts, u8 *data, int len)
{
	...
	for (retry = 0; retry < SEC_TS_I2C_RETRY_CNT; retry++) {
		...
		ret = ts->chip_data->ts_platform_data->bops->bus_write(data,
								       len);
		if (ret == NO_ERR)
			break;
...
static int sec_ts_i2c_read_bulk(struct sec_ts_data *ts, u8 *data, int len)
{
	...
	do {
		...
		for (retry = 0; retry < SEC_TS_I2C_RETRY_CNT; retry++) {
			ret = ts->chip_data->ts_platform_data->bops->bus_read(
				&reg, 0, data, len);
			if (ret == NO_ERR)
				break;
			usleep_range(1 * 1000, 1 * 1000);
...
static int sec_ts_i2c_boot_check(struct sec_ts_data *ts)
{
	int rc = -ENODATA;
	int retry = 0;
	u8 tBuff[SEC_TS_STATUSEVENT_SIZE] = {
		0,
	};
	...
	TS_LOG_INFO("%s: %02X, %02X, %02X, %02X, %02X, %02X, %02X, %02X [%d]\n",
	...
	*ptr_end = 0;
	...
	case TS_SWITCH_DOZE_ENABLE:
		...
		if (param > 0) {
			...
			error = sec_ts_i2c_write(
				tsp_info, SEC_TS_CMD_DOZE_PARAM, buff, 2);
			if (error < 0) {
				TS_LOG_ERR(
	...
	case TS_SWITCH_DOZE_DISABLE:
		...
		error = sec_ts_i2c_write(tsp_info, SEC_TS_CMD_DOZE_PARAM, buff,
					 2);
		if (error < 0) {
			TS_LOG_ERR("write doze delay:%u=%u error\n",
				   tsp_info->chip_data->touch_switch_reg,
				   value);
			break;
		}
...
void sec_ts_work_after_input_kit(void)
{
	...
	if (roi_switch_on) {
		...
		roi_data_addr = tsp_info->chip_data->ts_platform_data
					->feature_info.roi_info.roi_data_addr;
		TS_LOG_DEBUG("roi_data_addr=0x%04x\n", roi_data_addr);

		retval = sec_ts_i2c_read(tsp_info, roi_data_addr, roi_tmp,
...
static void sec_ts_reinit(struct sec_ts_data *ts)
{
	...
	TS_LOG_ERR("%s: charger=0x%x, Cover=0x%x, Power mode=0x%x\n", __func__,
		   ts->charger_mode, ts->touch_functions, ts->lowpower_status);
	...
	if (ts->charger_mode != SEC_TS_BIT_CHARGER_MODE_NO) {
		...
		ret = ts->sec_ts_i2c_write(ts, SET_TS_CMD_SET_CHARGER_MODE,
					   (u8 *)&w_data[0], 1);
		if (ret < 0) {
			TS_LOG_ERR("%s: Failed to send command(0x%x)", __func__,
	...
	if (ts->touch_functions & SEC_TS_BIT_SETFUNC_COVER) {
		...
		ret = sec_ts_i2c_write(ts, SEC_TS_CMD_SET_COVERTYPE,
				       (u8 *)&w_data[0], 1);
		if (ret < 0) {
			TS_LOG_ERR("%s: Failed to send command(0x%x)", __func__,
				   SEC_TS_CMD_SET_COVERTYPE);
			goto reinit_err;
		}
	...
	if (ts->lowpower_status == TO_LOWPOWER_MODE) {
		w_data[0] = (ts->lowpower_mode & SEC_TS_MODE_LOWPOWER_FLAG) >>
			    1;
		ret = sec_ts_i2c_write(ts, SEC_TS_CMD_WAKEUP_GESTURE_MODE,
				       (u8 *)&w_data[0], 1);
		if (ret < 0) {
			TS_LOG_ERR("%s: Failed to send command(0x%x)", __func__,
				   SEC_TS_CMD_WAKEUP_GESTURE_MODE);
			goto reinit_err;
		}

		w_data[0] = TO_LOWPOWER_MODE;
		ret = sec_ts_i2c_write(ts, SEC_TS_CMD_SET_POWER_MODE,
				       (u8 *)&w_data[0], 1);
		if (ret < 0) {
			TS_LOG_ERR("%s: Failed to send command(0x%x)", __func__,
				   SEC_TS_CMD_SET_POWER_MODE);
			goto reinit_err;
		}
		sec_ts_delay(50);
	}
				 ...
				 struct ts_fingers *info, u8 *event_buff)
{
	...
	if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) &&
		...
		if (p_event_status->status_data_1 == 0)
			...
			ret = sec_ts_i2c_read(ts, SEC_TS_READ_SELFCHECK_RESULT,
					      (u8 *)&rBuff,
					      SEC_TS_SELFCHECK_BUFF_SIZE);
			if (ret < 0) {
				TS_LOG_ERR("%s: i2c read one event failed\n",
					   __func__);
				return ret;
			}
			rearrange_byte((u8 *)&rBuff[1],
				       SEC_TS_SELFCHECK_BUFF_SIZE - 1);
			p_selfcheck = (struct sec_ts_selfcheck *)rBuff;
			TS_LOG_DEBUG(
				"%s: selfcheck result, %02X, %d, %d, %d, %d, %d, %d, %d, %d\n",
				__func__, p_selfcheck->result,
				p_selfcheck->mt_diff_tx,
				p_selfcheck->mt_diff_rx,
				p_selfcheck->mt_peak_max,
				p_selfcheck->mt_peak_min * -1,
				p_selfcheck->sf_txpeak_max,
				p_selfcheck->sf_txpeak_min * -1,
				p_selfcheck->sf_rxpeak_max,
				p_selfcheck->sf_rxpeak_min * -1);
		}
	}
	return ret;
}
	...
	if (t_id < MAX_SUPPORT_TOUCH_COUNT + MAX_SUPPORT_HOVER_COUNT) {
		...
		if ((ts->coord[t_id].ttype == SEC_TS_TOUCHTYPE_NORMAL) ||
			...
			if (ts->coord[t_id].action ==
				...
				if (t_id < MAX_SUPPORT_TOUCH_COUNT)
					memset(&info->fingers[t_id], 0,
					       sizeof(struct ts_finger));
				if (ts->time_longest <
				...
				ts->max_z_value =
					max((unsigned int)ts->coord[t_id].z,
					    ts->max_z_value);
				ts->min_z_value =
					min((unsigned int)ts->coord[t_id].z,
					    ts->min_z_value);
				ts->sum_z_value +=
...
static int sec_ts_read_info_event(struct sec_ts_data *ts,
				  struct ts_fingers *info)
{
	...
	do {
		ret = sec_ts_i2c_read(ts, SEC_TS_READ_ONE_EVENT,
				      read_event_buff,
				      SEC_TS_EVENT_BUFF_LONGSIZE);
		if (ret < 0) {
			TS_LOG_ERR("%s: i2c read one event failed\n", __func__);
			return ret;
		}
		...
		case SEC_TS_GESTURE_EVENT:
			...
			if ((p_gesture_status->eid == 0x02) &&
				...
				TS_LOG_ERR(
					"%s: gesture event %x %x %x %x %x %x\n",
					__func__, event_buff[0], event_buff[1],
					event_buff[2], event_buff[3],
					event_buff[4], event_buff[5]);
				remain_event = SEC_TS_EVENT_STOP;
			}
			break;
				      ...
				      struct ts_kit_device_data *chip_data)
{
	...
	ret = of_property_read_u32(device, "is_need_calibrate_after_update_fw",
				   &read_val);
	if (!ret) {
...
static void
sec_ts_parse_is_need_set_pinctrl(struct device_node *device,
				 struct ts_kit_device_data *chip_data)
{
	int ret = NO_ERR;
	int read_val = 0;
					   ...
					   struct ts_kit_device_data *chip_data)
{
	...
	ret = of_property_read_string(device, "default_projectid",
				      &tsp_info->default_projectid);
	if (ret) {
		TS_LOG_ERR(
			"%s, can't parse default_projectid from dts, use default\n",
			__func__);
		tsp_info->default_projectid = SEC_TS_HW_PROJECTID;
	}

	strncpy(tsp_info->project_id, tsp_info->default_projectid,
		SEC_TS_PROJECTID_MAX - 1);
	TS_LOG_INFO("%s,default_projectid:%s\n", __func__,
		    tsp_info->default_projectid);
}

static void
sec_ts_parse_is_need_set_reseved_bit(struct device_node *device,
				     struct ts_kit_device_data *chip_data)
{
	int ret = NO_ERR;
	int read_val = 0;
	...
	ret = of_property_read_u32(device, "is_need_set_reseved_bit",
				   &read_val);
	if (!ret) {
			   ...
			   struct ts_kit_device_data *chip_data)
{
	...
	TS_LOG_INFO("%s:%s=%d, %s=%d, %s=%d, %s=%d, %s=%d\n", __func__,
		    "algo_id", chip_data->algo_id, "x_max", chip_data->x_max,
		    "y_max", chip_data->y_max, "x_mt", chip_data->x_max_mt,
	...
	for (index = 0; index < array_len && index < SEC_TS_CAL_LIMITDATA_MAX;
	     index++) {
		ret = of_property_read_string_index(np, "cal_data_limit", index,
						    &raw_data_dts);
		if (ret) {
			TS_LOG_ERR(
	...
	for (index = 0; index < array_len && index < SEC_TS_NOLH_LIMITDATA_MAX;
	     index++) {
...
int sec_ts_read_information(struct sec_ts_data *ts)
{
	...
	if (ts->ic_name == S6SY761X) {
		if (ts->tx_count != ts->tx_default_num ||
			...
			TS_LOG_ERR(
				"%s, tx or tx invalid, [%d, %d], use default trx num\n",
...
static int
sec_ts_parse_panel_specific_config(struct sec_ts_data *ts,
				   struct ts_kit_device_data *chip_data)
{
...
static int sec_ts_power(struct sec_ts_data *data, bool on)
{
	...
	if (on) {
		...
		gpio_direction_output(
			ts->chip_data->ts_platform_data->reset_gpio, 1);
		TS_LOG_INFO("%s: reset gpio on\n", __func__);
	} else {
		gpio_direction_output(
			ts->chip_data->ts_platform_data->reset_gpio, 0);
...
static int sec_ts_chip_detect(struct ts_kit_platform_data *data)
{
	...
	ts->event_buff =
		kzalloc((MAX_EVENT_COUNT + 1) * ts->event_length, GFP_KERNEL);
	if (!ts->event_buff)
		goto event_err;
	...
	if (retval < 0) {
		...
		strncpy(ts->chip_data->chip_name, SEC_TS_I2C_NAME,
			(MAX_STR_LEN > strlen(SEC_TS_I2C_NAME) + 1) ?
				(strlen(SEC_TS_I2C_NAME) + 1) :
				(MAX_STR_LEN - 1));
	}

	if (S6SY761X == tsp_info->ic_name) {
		strncpy(tsp_info->chip_data->module_name, tsp_info->module_name,
			MAX_STR_LEN - 1);
	}
...
static int sec_ts_init_chip(void)
{
	...
	TS_LOG_INFO("%s:%s=%d, %s=%d, %s=%d, %s=%d, %s=%d\n", __func__,
		    "algo_id", ts->chip_data->algo_id, "x_max",
		    ts->chip_data->x_max, "y_max", ts->chip_data->y_max, "x_mt",
		    ts->chip_data->x_max_mt, "y_mt", ts->chip_data->y_max_mt);
	...
	if (ts->chip_data->slave_addr == SEC_TS_DEFAULT_I2C_ADDR) {
		strncpy(tee_tui_data.device_name, IC_SEC_Y761,
			strlen(IC_SEC_Y761));
...
static int sec_ts_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_X, 0, tsp_info->plat_data->max_x, 0,
			     0);
	input_set_abs_params(input_dev, ABS_Y, 0, tsp_info->plat_data->max_y, 0,
			     0);
...
static int sec_ts_chip_get_info(struct ts_chip_info_param *info)
{
	...
	memcpy(&info->mod_vendor, tsp_info->project_id,
	       strlen(tsp_info->project_id));

	snprintf(info->fw_vendor, CHIP_INFO_LENGTH * 2 - 1,
		 "%02X.%02X.%02X.%02X",
...
static int sec_ts_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
	int retval = NO_ERR;
	...
	case TS_ACTION_READ:
		strncpy(info->tp_test_type, tsp_info->cap_test_type,
...
static int sec_ts_set_info_flag(struct ts_kit_platform_data *info)
{
	tsp_info->chip_data->ts_platform_data->get_info_flag =
		info->get_info_flag;
	return NO_ERR;
}
...
static int sec_ts_fw_update_boot(char *file_name)
{
	...
	if (tsp_info->boot_mode == TOUCH_APP) {
		...
		if (ret < 0)
			TS_LOG_ERR(
				"%s: fail to download boot firmware download: %s\n",
				__func__, file_name);
...
static int sec_ts_calibration_test(void)
{
	...
	retval = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_MIS_CAL_CHECK,
				      ts->cal_limit, 3);
	if (retval < 0) {
	...
	retval = ts->sec_ts_i2c_read(ts, SEC_TS_CMD_MIS_CAL_READ, &mis_amb_data,
				     1);
	if (retval < 0) {
	...
	retval = ts->sec_ts_i2c_read(ts, SEC_TS_CMD_MIS_CAL_READ, &mis_cal_data,
				     1);
	if (retval < 0) {
		TS_LOG_ERR("%s: i2c fail!, %d\n", __func__, retval);
		mis_cal_data = 0xF3;
...
static int sec_ts_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_READ:
		info->glove_switch =
			(tsp_info->touch_functions & SEC_TS_BIT_SETFUNC_GLOVE) ?
				1 :
				0;
	...
	case TS_ACTION_WRITE:
		...
		retval =
			sec_ts_glove_mode_enables(tsp_info, info->glove_switch);
		if (retval < 0) {
...
static int sec_ts_charger_switch(struct ts_charger_info *info)
{
	...
	switch (info->op_action) {
	case TS_ACTION_WRITE:
		retval = sec_ts_charger_mode_enables(tsp_info,
						     info->charger_switch);
		if (retval < 0) {
...
static int sec_ts_holster_switch(struct ts_holster_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		retval = sec_ts_cover_mode_enalbes(tsp_info,
						   info->holster_switch);
		if (retval < 0) {
...
static int sec_ts_set_roi_switch(u8 roi_switch)
{
	...
	roi_ctrl_addr = tsp_info->chip_data->ts_platform_data->feature_info
				.roi_info.roi_control_addr;
	roi_control_bit = tsp_info->chip_data->ts_platform_data->feature_info
	...
	retval = sec_ts_i2c_read(tsp_info, roi_ctrl_addr, &temp_roi_switch, 1);
	if (retval < 0) {
	...
	retval = sec_ts_i2c_write(tsp_info, roi_ctrl_addr, &temp_roi_switch, 1);
	if (retval < 0) {
		TS_LOG_ERR(
...
static int sec_ts_read_roi_switch(void)
{
	...
	retval = sec_ts_i2c_read(tsp_info, roi_ctrl_addr, &roi_switch,
				 sizeof(roi_switch));
	if (retval < 0) {
	...
	tsp_info->chip_data->ts_platform_data->feature_info.roi_info.roi_switch =
		roi_switch;
...
static int sec_ts_regs_operate(struct ts_regs_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		if ((1 == info->num) && (8 > bit)) {
			retval = sec_ts_i2c_read(tsp_info, info->addr, value,
						 info->num);
			if (retval < 0) {
	...
	case TS_ACTION_READ:
		retval =
			sec_ts_i2c_read(tsp_info, info->addr, value, info->num);
		if (retval < 0) {
			TS_LOG_ERR("TS_ACTION_READ error, addr(%d)\n",
				   info->addr);
			retval = -EINVAL;
...
static int run_force_calibration(void)
{
	...
	if (rc < 0) {
		...
		if (ts->plat_data->mis_cal_check) {
			buff[0] = 0;
			rc = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_STATEMANAGE_ON,
						  buff, 1);
			if (rc < 0) {
				TS_LOG_ERR(
			...
			buff[0] = 0x2;
			buff[1] = 0x2;
			rc = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_CHG_SYSMODE,
						  buff, 2);
			if (rc < 0) {
				TS_LOG_ERR(
			...
			TS_LOG_ERR("%s: try mis Cal. check\n", __func__);
			rc = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_MIS_CAL_CHECK,
						  NULL, 0);
			if (rc < 0) {
				TS_LOG_ERR(
			...
			rc = ts->sec_ts_i2c_read(ts, SEC_TS_CMD_MIS_CAL_READ,
						 &mis_cal_data, 1);
			if (rc < 0) {
				TS_LOG_ERR("%s: i2c fail!, %d\n", __func__, rc);
				mis_cal_data = 0xF3;
...
{
	int restore_status = ts->chip_data->ts_platform_data->feature_info
				     .charger_info.charger_switch;
	if (!enable) {
...
static int sec_ts_after_resume(void *feature_info)
{
	...
	if (ts->chip_data->ts_platform_data->feature_info.holster_info
		    .holster_switch) {
		ret = sec_ts_i2c_write(ts, SEC_TS_CMD_SET_COVERTYPE,
				       &ts->cover_cmd, 1);
		...
		TS_LOG_ERR("%s: cover cmd write type:%d, mode:%x, ret:%d",
			   __func__, ts->touch_functions, ts->cover_cmd, ret);
	} else {
		ts->touch_functions =
			(ts->touch_functions & (~SEC_TS_BIT_SETFUNC_COVER));
		TS_LOG_ERR("%s: cover open, not send cmd", __func__);
	}
	...
	ret = sec_ts_i2c_write(ts, SEC_TS_CMD_SET_TOUCHFUNCTION,
			       (u8 *)&ts->touch_functions, 2);
	if (ret < 0)
		TS_LOG_ERR("%s: Failed to send touch function command",
			   __func__);
	...
	ret = sec_ts_glove_mode_enables(
		ts, ts->chip_data->ts_platform_data->feature_info.glove_info
			    .glove_switch);
	if (ret)
