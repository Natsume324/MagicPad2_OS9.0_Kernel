...
static int sec_ts_sw_reset(struct sec_ts_data *ts)
{
	...
	if (tBuff[0] == 0x1)
		ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_APP_SW_RESET, NULL,
					   0);
	else
		ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_BOOT_SW_RESET, NULL,
					   0);
	if (ret < 0) {
...
static int sec_ts_enter_fw_mode(struct sec_ts_data *ts)
{
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_ENTER_FW_MODE,
				   fw_update_mode_passwd, 2);
	...
	TS_LOG_INFO("%s: write ok, enter_fw_mode - 0x%x 0x%x 0x%x\n", __func__,
		    SEC_TS_CMD_ENTER_FW_MODE, fw_update_mode_passwd[0],
...
static int sec_ts_save_version_of_ic(struct sec_ts_data *ts)
{
	...
	if (S6SY761X == ts->ic_name) {
		snprintf(ts->chip_data->version_name, MAX_STR_LEN - 1,
			 "%02X.%02X.%02X.%02x", img_ver[0], img_ver[1],
			 img_ver[2], img_ver[3]);
	}
					 ...
					 const u8 *fw_info)
{
	...
	if ((pdata->img_version_of_ic[2] >= 1) &&
	    (pdata->img_version_of_ic[3] > 0)) {
		ts->event_length = SEC_TS_EVENT_BUFF_LONGSIZE;
		if (!ts->event_buff)
			kfree(ts->event_buff);
		ts->event_buff =
			kzalloc(MAX_EVENT_COUNT * ts->event_length, GFP_KERNEL);
		if (!ts->event_buff) {
	...
	for (i = 2; i < 4; i++) {
		if (pdata->img_version_of_ic[i] !=
		    pdata->img_version_of_bin[i]) {
			TS_LOG_INFO("%s: i:%d, img ver:%d, bin ver:%d",
				    __func__, i, pdata->img_version_of_ic[i],
				    pdata->img_version_of_bin[i]);
			return DO_FWUP;
		}
	}
				 ...
				 u8 *page_data)
{
	...
	tCmd[1 + 2 + flash_page_size] =
		sec_ts_checksum(tCmd, 1, 2 + flash_page_size);

	ret = ts->sec_ts_i2c_write_burst(ts, tCmd, 1 + 2 + flash_page_size + 1);
	return ret;
}

static bool sec_ts_limited_flashpagewrite(struct sec_ts_data *ts, u32 page_idx,
					  u8 *page_data)
{
	int ret = 0;
	u8 *tCmd = NULL;
	...
	copy_data[2 + flash_page_size] =
		sec_ts_checksum(copy_data, 0, 2 + flash_page_size);
			     ...
			     u32 mem_size, int retry)
{
	...
	for (page_idx = page_num - 1;; page_idx--) {
		...
		if (ts->boot_ver[0] == SEC_TS_BOOT_OLDVER) {
			ret = sec_ts_flashpagewrite(
				ts, (page_idx + page_idx_start), page_buf);
			if (ret < 0) {
				TS_LOG_ERR(
			...
			ret = sec_ts_limited_flashpagewrite(
				ts, (page_idx + page_idx_start), page_buf);
			if (ret < 0) {
				TS_LOG_ERR(
					"%s: fw write failed, page_idx = %u\n",
...
static int sec_ts_memoryread(struct sec_ts_data *ts, u32 mem_addr, u8 *mem_data,
			     u32 mem_size)
{
	int ret = NO_ERR;
	int retry = 3;
...
int set_pid_data(struct sec_ts_data *ts, u8 *data)
{
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_NVM, buff,
				   2 + SEC_TS_PROJECTID_MAX);
	if (ret < 0)
...
int sec_ts_read_pid(struct sec_ts_data *ts)
{
	...
	if (ts->is_need_set_reseved_bit) {
		data[SEC_TS_PROJECTID_MAX - 2] =
			0x30; //set the reserved bit 9(11-2) to 0x30.
	}
	...
	if ((data[0] == 0xff) || (data[0] == 0x00)) {
		...
		snprintf(ts->project_id, sizeof(ts->project_id), "%s",
			 ts->default_projectid);

		for (retry = 0; retry < SEC_TS_I2C_RETRY_CNT; retry++) {
			set_pid_data(ts, ts->project_id);
			get_pid_data(ts, data);
			data[SEC_TS_PROJECTID_MAX - 1] = '\0';
			if (memcmp(ts->project_id, data,
				   SEC_TS_PROJECTID_MAX) == 0) {
				TS_LOG_ERR(
					"%s: pid write and read: data[0] = %s \n",
					__func__, data);
				break;
			} else
				TS_LOG_ERR(
					"%s: pid write error: data[0] = %d \n",
					__func__, data[0]);
		}
	} else {
...
static int sec_ts_firmware_update(struct sec_ts_data *ts, const u8 *data,
				  size_t size, int bl_update, int restore_cal,
				  int retry)
{
	int i = 0;
	int ret = NO_ERR;
	...
	for (i = 0; i < fw_hd->num_chunk; i++) {
		...
		TS_LOG_ERR(
			"%s: firmware chunk count %d, addr=%08X, size = %d\n",
			__func__, i, fw_ch->addr, fw_ch->size);
		fd += sizeof(fw_chunk);
	...
	if (!bl_update) {
		...
		if (ts->sec_ts_i2c_read(ts, SEC_TS_READ_BOOT_STATUS, &fw_status,
					1) < 0) {
...
int sec_ts_firmware_update_on_probe(struct sec_ts_data *ts, bool force_update)
{
	...
	for (ii = 0; ii < 3; ii++) {
		ret = sec_ts_firmware_update(ts, fw_entry->data, fw_entry->size,
					     0, restore_cal, ii);
		if (ret >= 0)
			break;
	}
...
int sec_ts_load_fw_request(struct sec_ts_data *ts, u8 sd_enable)
{
	...
	if (sec_ts_firmware_update(ts, fw_entry->data, fw_entry->size, 0, 1,
				   0) < 0)
		error = -ENODATA;
	else
		error = NO_ERR;
