...
static int gtx8_parse_firmware(struct firmware_data *fw_data)
{
	...
	memcpy(fw_info, firmware->data,
	       sizeof(struct firmware_info) > FW_SUBSYS_INFO_OFFSET ?
...
static int gtx8_hold_dsp_gt7382(void)
{
	...
	for (index = 0; index < retry_times; index++) {
		...
		mdelay(10); /* ic need */
		write_value = 0;
		ret = gtx8_ts->ops.i2c_read(HW_REG_ILM_ACCESS, &(write_value),
					    1);
	...
	if (index >= retry_times) {
		TS_LOG_ERR("%s:write 0x06 to 0x50c0 and check fail!", __func__);
		return -EINVAL;
	}
...
static int gtx8_load_isp_gt7382(struct firmware_data *fw_data)
{
	...
	if (ret < 0) {
		...
		ret = gtx8_ts->ops.i2c_read(GT738X_ISP_STATE_REG, state_reg,
					    GTX8_REG_LEN);
		if (ret)
			TS_LOG_ERR("%s:read isp reg state error\n", __func__);
		TS_LOG_INFO(
			"%s, [0x4195]:%02x,[0x4196]:%02x,[0x4197]:%02x,[0x4198]:%02x\n",
...
static int gtx8_update_prepare_gt7382(void)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		if (ret >= 0) {
			ret = gtx8_ts->ops.i2c_read(HW_REG_SPI_STATE, &spi_dis,
						    len);
...
static int gtx8_send_fw_packet_gt7382(struct fw_subsys_info *fw_x)
{
	...
	while (remain_len > 0) {
		...
		for (j = 0; j < retry_cnt; j++) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"%s,Inform ISP start to write flash fail\n",
					__func__);
				continue;
			}
...
static int gtx8_flash_firmware_gt7382(struct firmware_data *fw_data)
{
	...
	for (i = 0; i < fw_num && retry;) {
		...
		if (fw_x->type == SS51_ISP_CODE) {
			TS_LOG_INFO(
				"current subsystem is isp,no need to upgrade\n");
			i++;
			continue;
		}
		...
		if ((fw_x->type == FW_BOOT_CODE) &&
			...
			TS_LOG_INFO(
				"current subsystem is bootcode,no need to upgrade\n");
			i++;
			continue;
		}
...
static int gtx8_load_isp(struct firmware_data *fw_data)
{
	...
	/* select bank0 */
	reg_val[0] = 0x00;
	ret = gtx8_reg_write(HW_REG_BANK_SELECT, reg_val, 1);
	if (ret < 0) {
	...
	/* enable bank0 access */
	reg_val[0] = 0x01;
	ret = gtx8_reg_write(HW_REG_ACCESS_PATCH0, reg_val, 1);
	if (ret < 0) {
	...
	ret = gtx8_reg_write_confirm(HW_REG_ISP_ADDR, (u8 *)fw_isp->data,
				     fw_isp->size);
	if (ret < 0) {
	...
	/* forbid patch access */
	reg_val[0] = 0x00;
	ret = gtx8_reg_write_confirm(HW_REG_ACCESS_PATCH0, reg_val, 1);
	if (ret < 0) {
	...
	for (i = 0; i < 200; i++) {
		ret = gtx8_reg_read(HW_REG_ISP_RUN_FLAG, reg_val, 2);
 ...
 * @data: packet data
 */
static int gtx8_format_fw_packet(u8 *pkt, u32 flash_addr, u16 len,
				 const u8 *data)
{
	u16 checksum = 0;
	if (!pkt || !data)
...
static int gtx8_flash_subsystem(struct fw_subsys_info *subsys)
{
	...
	while (total_size > 0) {
		...
		ret = gtx8_format_fw_packet(fw_packet,
		...
		for (i = 0; i < 3; i++) {
			ret = gtx8_send_fw_packet(subsys->type, fw_packet,
						  data_size + 6);
			if (!ret)
				break;
		}
...
int gtx8_update_firmware(void)
{
	...
	ret = gtx8_request_firmware(&update_ctrl.fw_data, update_ctrl.fw_name);
	if (ret < 0) {
