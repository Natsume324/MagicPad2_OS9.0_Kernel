	...
	if (enable) {
		...
		if (gtx8_ts->doze_mode_set_count == 0) {
			...
			for (i = 0; i < GTX8_RETRY_NUM_3; i++) {
				if (!gtx8_i2c_write_trans(GTP_REG_DOZE_CTRL,
							  &w_data, 1)) {
					result = NO_ERR;
					goto exit;
				}
		...
		if (gtx8_ts->doze_mode_set_count == 1) {
			...
			if (gtx8_i2c_write_trans(GTP_REG_DOZE_CTRL, &w_data,
						 1)) {
				TS_LOG_ERR(
			...
			usleep_range(1000, 1100);
			for (i = 0; i < 9; i++) {
				if (gtx8_i2c_read_trans(GTP_REG_DOZE_STAT,
							&r_data, 1)) {
					TS_LOG_ERR(
						"doze mode comunition disable FAILED\n");
					goto exit;
				...
				if (GTX8_DOZE_CLOSE_OK_DATA == r_data) {
					...
					w_data = GTX8_DOZE_DISABLE_DATA;
					if (gtx8_i2c_write_trans(
						    GTP_REG_DOZE_CTRL, &w_data,
						    1)) {
						TS_LOG_ERR(
							"doze mode comunition disable FAILED\n");
						goto exit;
...
static int gtx8_check_cfg_valid(u8 *cfg, u32 length)
{
	...
	for (j = 0; j < bag_num; j++) {
		if (bag_start >= length - 1) {
			TS_LOG_ERR(
				"ERROR, overflow!!bag_start:%d, cfg_len:%d\n",
				bag_start, length);
			ret = -EINVAL;
			goto exit;
		}
		...
		if (checksum != 0) {
			TS_LOG_ERR(
				"cfg INVALID, bag:%d checksum ERROR:0x%02x\n",
				j, checksum);
			ret = -EINVAL;
			goto exit;
		}
...
static int gtx8_send_large_config(struct gtx8_ts_config *config)
{
	...
	for (try_times = 0; try_times < read_try_times; try_times++) {
		if (!gtx8_i2c_read(command_reg, &buf, 1) &&
		    buf == GTX8_CMD_COMPLETED)
			break;
		usleep_range(10000, 11000);
	}
...
static int gtx8_send_small_config(struct gtx8_ts_config *config)
{
	...
	if (gtx8_send_cmd(GTX8_CMD_SEND_SMALL_CFG, 0, GTX8_NOT_NEED_SLEEP)) {
		TS_LOG_ERR(
			"Send small cfg FAILED, send COMMAND_SEND_SMALL_CFG ERROR");
		ret = -EINVAL;
		goto exit;
	}
...
static int gtx8_send_cfg(struct gtx8_ts_config *config)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"%s: failed disabled doze mode before send config[ignore]\n",
			__func__);
	}
...
static int _gtx8_do_read_config(u32 base_addr, u8 *buf)
{
	...
	for (i = 0; i < sub_bags; i++) {
		...
		TS_LOG_INFO("sub bag num:%u,sub bag length:%u\n", buf[offset],
			    subbag_len);
		ret = gtx8_i2c_read(base_addr + offset + 2, buf + offset + 2,
				    subbag_len + 1);
		if (ret)
			goto err_out;
...
static int gtx8_read_version(struct gtx8_ts_version *version)
{
	...
	if (!gtx8_ts->reg.pid || !gtx8_ts->reg.sensor_id || !gtx8_ts->reg.vid) {
		TS_LOG_ERR(
			"reg is NULL, pid:0x%04x, vid:0x%04x, sensor_id:0x%04x\n",
			gtx8_ts->reg.pid, gtx8_ts->reg.vid,
			gtx8_ts->reg.sensor_id);
		return -EINVAL;
	}
	if (!pid_read_len || pid_read_len > PID_DATA_MAX_LEN || !vid_read_len ||
	    vid_read_len > VID_DATA_MAX_LEN) {
		TS_LOG_ERR(
	...
	if (gtx8_ts->ic_type != IC_TYPE_7382) {
		if (gtx8_ts->reg.version_base &&
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"Read version base failed, reg:0x%02x, len:%d\n",
			...
			checksum =
				checksum_u8(temp_buf, gtx8_ts->reg.version_len);
			if (checksum) {
				TS_LOG_ERR(
					"checksum error:0x%02x, base:0x%02x, len:%d\n",
					checksum, gtx8_ts->reg.version_base,
					gtx8_ts->reg.version_len);

				TS_LOG_ERR("%*ph\n",
					   (int)(gtx8_ts->reg.version_len / 2),
					   temp_buf);
				TS_LOG_ERR(
					"%*ph\n",
			...
			TS_LOG_ERR(
				"Incomplete parameter for read pid, vid info\n");
			goto exit;
		}
	}
	/*read pid*/
	...
	TS_LOG_INFO("PID:%s,SensorID:%d, VID:%*ph\n", version->pid,
		    version->sensor_id, (int)sizeof(version->vid),
		    version->vid);

	snprintf(gtx8_ts->dev_data->version_name, MAX_STR_LEN - 1, "0x%04X",
		 getU32(version->vid));
exit:
	/*enable doze mode*/
	if (gtx8_set_i2c_doze_mode(true))
...
static int gtx8_pinctrl_init(void)
{
...
#else
	ts->pinctrl_state_reset_high =
		pinctrl_lookup_state(ts->pinctrl, "state_rst_output1");
	...
	ts->pinctrl_state_reset_low =
		pinctrl_lookup_state(ts->pinctrl, "state_rst_output0");
	...
	ts->pinctrl_state_as_int =
		pinctrl_lookup_state(ts->pinctrl, "state_eint_as_int");
	...
	ts->pinctrl_state_int_high =
		pinctrl_lookup_state(ts->pinctrl, "state_eint_output1");
	...
	ts->pinctrl_state_int_low =
		pinctrl_lookup_state(ts->pinctrl, "state_eint_output0");
	...
	ts->pinctrl_state_iovdd_low =
		pinctrl_lookup_state(ts->pinctrl, "state_iovdd_output0");
	...
	ts->pinctrl_state_iovdd_high =
		pinctrl_lookup_state(ts->pinctrl, "state_iovdd_output1");
...
static void gtx8_power_on_gpio_set(void)
{
	...
	ret |= gpio_direction_output(ts->dev_data->ts_platform_data->reset_gpio,
				     1);
#else
...
#else
	ret = pinctrl_select_state(ts->pinctrl, ts->pinctrl_state_iovdd_high);
	ret |= pinctrl_select_state(ts->pinctrl, ts->pinctrl_state_reset_low);
#endif
	if (ret)
		TS_LOG_INFO("%s:error./\n", __func__);
...
static int
gtx8_wakeup_gesture_enable_switch(struct ts_wakeup_gesture_enable_info *info);
/**
...
static int gtx8_feature_resume(struct gtx8_ts_data *ts)
{
	...
	info = &ts->dev_data->ts_platform_data->feature_info;
	TS_LOG_INFO(
		"holster_supported:%d,holster_switch_staus:%d,glove_supported:%d,glove_switch_status:%d\n",
		info->holster_info.holster_supported,
		info->holster_info.holster_switch,
		info->glove_info.glove_supported,
	...
	ret = gtx8_wakeup_gesture_enable_switch(&(
		info->wakeup_gesture_enable_info)); //have delay 50ms before send cmd.
skip_send_cmd:
	if (info->charger_info.charger_switch) {
		...
		TS_LOG_INFO("resume to send  charger cmd:%d\n",
			    GTX8_CMD_CHARGER_ON);
		if (gtx8_send_cmd(GTX8_CMD_CHARGER_ON, 0,
				  GTX8_NOT_NEED_SLEEP)) {
...
int gtx8_ts_esd_init(struct gtx8_ts_data *ts)
{
	...
	if (ts->reg.esd != 0) {
		...
		if (ret < 0)
			TS_LOG_ERR(
				"dynamic ESD init ERROR, i2c write failed\n");
	}
...
static void gtx8_ts_parse_recovery_res(struct device_node *device)
{
	...
	if (ret) {
		TS_LOG_INFO(
			"%s:get device use_ic_res fail, use default, ret=%d\n",
			__func__, ret);
	...
	TS_LOG_INFO("%s:%s=%d, %s=%d, %s=%d, %s=%d\n", __func__, "use_ic_res",
		    gtx8_ts->use_ic_res, "x_max_rc", gtx8_ts->x_max_rc,
		    "y_max_rc", gtx8_ts->y_max_rc, "in_recovery_mode",
		    gtx8_ts->in_recovery_mode);
}
...
static int gtx8_parse_dts(void)
{
	...
	ret = of_property_read_string(device, GTP_CHIP_NAME,
				      (const char **)&tmp_buff);
	...
	ret = of_property_read_string(device, GTP_MODULE_VENDOR,
				      (const char **)&tmp_buff);
	if (ret || tmp_buff == NULL) {
	...
	if (ret) {
		TS_LOG_INFO(
			"%s:roi_data address read failed, Use default value.\n",
			__func__);
		value = 0;
	}
	ts->gtx8_roi_data_add = value;
	TS_LOG_INFO("%s:roi_data address used is: [%x]\n", __func__,
		    ts->gtx8_roi_data_add);
	...
	if (ret) {
		value = 0;
		TS_LOG_INFO(
			"%s: support_cfg_named_with_projectid use default value\n",
			__func__);
	}
	...
	if (ret) {
		value = 0;
		TS_LOG_INFO(
			"%s: support_checked_esd_reset_chip use default value\n",
			__func__);
	}
	...
	if (ret) {
		value = 0;
		TS_LOG_INFO(
			"%s: delete_insignificant_chip_info use default value\n",
			__func__);
	}
	...
	if (ret) {
		value = 0;
		TS_LOG_ERR(
			"%s: support_priority_read_sensorid use default value\n",
			__func__);
	}
	ts->support_priority_read_sensorid = value;
	TS_LOG_INFO("%s, support_priority_read_sensorid=%d\n", __func__,
		    ts->support_priority_read_sensorid);
	ret = of_property_read_u32(device, GTX8_DOZE_FAILED_NOT_SEND_CFG,
				   &value);
	if (ret) {
		value = 0;
		TS_LOG_ERR(
			"%s: support_doze_failed_not_send_cfg use default value\n",
			__func__);
	}
	...
	if (ret) {
		value = 0;
		TS_LOG_ERR(
			"%s: support_read_more_debug_msg use default value\n",
			__func__);
	}
	...
	if (ret) {
		value = 0;
		TS_LOG_ERR(
			"%s: support_filename_contain_lcd_module use default value\n",
			__func__);
	}
	...
	ret = of_property_read_u32(
		device, GTX8_SUPPORT_READ_GESTURE_WITHOUT_CHECKSUM, &value);
	if (ret) {
		value = 0;
		TS_LOG_ERR(
			"%s: support_read_gesture_without_checksum use default value\n",
			__func__);
	}
	...
	TS_LOG_INFO(
		"%s: irq_config=%d,algo_id=%d,project_id=%s,config_flag=%d,roi_supported=%d,"
		"holster_supported=%d,glove_supported=%d, easy_wakeup_supported=%d ,"
		"tools_support=%d,ic_type=%d,chip_name=%s\n",
		__func__, chip_data->irq_config, chip_data->algo_id,
		ts->project_id, ts->config_flag,
		chip_data->ts_platform_data->feature_info.roi_info.roi_supported,
		chip_data->ts_platform_data->feature_info.holster_info
			.holster_supported,
		chip_data->ts_platform_data->feature_info.glove_info
			.glove_supported,
		ts->easy_wakeup_supported, ts->tools_support, ts->ic_type,
		ts->chip_name);
...
static int gtx8_parse_specific_dts(void)
{
	...
	snprintf(sensor_id_str, sizeof(sensor_id_str), "gtx8-sensorid-%u",
		 ts->hw_info.sensor_id);
	TS_LOG_INFO("%s: Parse specific dts:%s\n", __func__, sensor_id_str);

	device = of_find_compatible_node(ts->pdev->dev.of_node, NULL,
					 sensor_id_str);
	if (!device) {
	...
	ret = of_property_read_string(device, GTP_PROJECT_ID,
				      (const char **)&tmp_buff);
	if (ret || tmp_buff == NULL) {
	...
	TS_LOG_INFO("%s: vendor_name=%s,project_id=%s\n", __func__,
		    ts->dev_data->module_name, ts->project_id);
...
static int gtx8_ts_roi_init(struct gtx8_ts_roi *roi)
{
	...
	if (!ts->dev_data->ts_platform_data->feature_info.roi_info
		     .roi_supported ||
	    !roi) {
			       ...
			       u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (!strncmp(cfg_type, GTX8_TEST_CONFIG, strlen(GTX8_TEST_CONFIG)))
		...
		config_status = 8;
	else if (!strncmp(cfg_type, GTX8_GAME_NOISE_CONFIG,
			  strlen(GTX8_GAME_NOISE_CONFIG)))
		config_status = 9;
	else {
	...
	for (i = 0; i < bin_group_num * bin_cfg_num; i++) {
		...
		if (sid ==
			...
			if (config_status ==
				...
				one_cfg_count = getU16(
					&cfg_bin->data[CFG_HEAD_BYTES + 2 +
						       i * CFG_INFO_BLOCK_BYTES]);
				memcpy(cfg,
				       &cfg_bin->data
						[CFG_HEAD_BYTES +
...
static int gtx8_get_cfg_data(const struct firmware *cfg_bin, char *config_name,
...
static int gtx8_get_cfg_parms(struct gtx8_ts_data *ts, const char *filename)
{
	...
	ret = gtx8_get_cfg_data(cfg_bin, GTX8_NORMAL_NOISE_CONFIG,
				&ts->normal_noise_cfg);
	if (ret < 0)
	...
	ret = gtx8_get_cfg_data(cfg_bin, GTX8_NOISE_TEST_CONFIG,
				&ts->noise_test_cfg);
	if (ret < 0) {
	...
	if (ts->dev_data->ts_platform_data->feature_info.glove_info
		    .glove_supported) {
		ret = gtx8_get_cfg_data(cfg_bin, GTX8_GLOVE_CONFIG,
					&ts->glove_cfg);
		if (ret < 0)
		...
		ret = gtx8_get_cfg_data(cfg_bin, GTX8_GLOVE_NOISE_CONFIG,
					&ts->glove_noise_cfg);
		if (ret < 0)
			TS_LOG_ERR(
				"%s: Failed to parse glove_noise config data:%d\n",
				__func__, ret);
	}

	/* parse holster config data */
	if (ts->dev_data->ts_platform_data->feature_info.holster_info
		    .holster_supported) {
		ret = gtx8_get_cfg_data(cfg_bin, GTX8_HOLSTER_CONFIG,
					&ts->holster_cfg);
		if (ret < 0)
			TS_LOG_ERR(
				"%s: Failed to parse holster_config data:%d\n",
				__func__, ret);

		ret = gtx8_get_cfg_data(cfg_bin, GTX8_HOLSTER_NOISE_CONFIG,
					&ts->holster_noise_cfg);
		if (ret < 0)
			TS_LOG_ERR(
				"%s: Failed to parse holster_noise_config data:%d\n",
				__func__, ret);
	}
	...
	ret = gtx8_get_cfg_data(cfg_bin, GTX8_GAME_NOISE_CONFIG,
				&ts->game_noise_cfg);
	if (ret < 0)
...
static int gtx8_chip_detect(struct ts_kit_platform_data *pdata)
{
	...
	strncpy(gtx8_ts->dev_data->chip_name, GOODIX_VENDER_NAME,
		MAX_STR_LEN - 1);

	if (gtx8_ts->ic_type == IC_TYPE_7382)
		gtx8_ts->dev_data->fold_fingers_supported =
			1; /* use 20 fingers */
	return ret;
err_power_on:
...
static int gtx8_chip_init(void)
{
	...
	if (strlen(GTX8_DEVICE_NAME) >= name_len) {
		...
		strncpy(tee_tui_data.device_name, GTX8_DEVICE_NAME,
			name_len - 1);
	}
#endif
	...
	ts->pen_supported = ts->dev_data->ts_platform_data->feature_info
				    .pen_info.pen_supported;
	if (ts->support_wake_lock_suspend == GT8X_WAKE_LOCK_SUSPEND_SUPPORT)
		wakeup_source_init(&gtx8_ts->wake_lock, "gtx8tp_wake_lock");
	/* init register addr*/
	gtx8_reg_init(ts);
	...
	if ((gtx8_ts->support_get_tp_color == GT8X_READ_TPCOLOR_SUPPORT) ||
	    (gtx8_ts->support_read_projectid == GT8X_READ_PROJECTID_SUPPORT)) {
...
static int gtx8_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, GTX8_ABS_MAX_VALUE,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0,
			     GTX8_ABS_MAX_VALUE, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TOUCH_MINOR, 0,
			     GTX8_ABS_MAX_VALUE, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, GTP_MAX_TOUCH, 0,
			     0);

	return 0;
}

static int gtx8_get_standard_matrix(struct roi_matrix *src_mat,
				    struct roi_matrix *dst_mat)
{
	...
	temp_buf =
		kzalloc((src_mat->row * src_mat->col + 1) * sizeof(*temp_buf),
			GFP_KERNEL);
	if (!temp_buf) {
	...
	for (i = 0; i < src_mat->row; i++) {
		for (j = 0; j < src_mat->col; j++) {
			temp_buf[(src_mat->row - i - 1) +
				 (src_mat->col - j - 1) * src_mat->row] =
	...
	src_mat->col = i;
	memcpy(src_mat->data, temp_buf,
	       src_mat->row * src_mat->col * sizeof(*temp_buf));
	kfree(temp_buf);
	temp_buf = NULL;
	...
	for (i = src_row_start; i <= src_row_end; i++) {
		memcpy(&dst_mat->data[dst_col_start +
				      dst_mat->col * (dst_row_start + i -
						      src_row_start)],
...
static int gtx8_cache_roidata_fw(struct gtx8_ts_roi *roi)
{
	...
	for (j = 0, checksum = 0; j < roi_bytes / ROI_DATA_BYTES; j++) {
		roi->rawdata[j + ROI_HEAD_LEN / ROI_DATA_BYTES] = le16_to_cpu(
...
static int gtx8_cache_roidata_device(struct gtx8_ts_roi *roi)
{
	...
	src_mat.data = kzalloc((src_mat.row * src_mat.col + 1) *
	...
	dst_mat.data = kzalloc((dst_mat.row * dst_mat.col + 1) *
				       sizeof(dst_mat.data[0]),
			       GFP_KERNEL);
	if (!dst_mat.data) {
	...
	/* read ROI rawdata */
	ret = gtx8_i2c_read_trans(
		(ts->gtx8_roi_data_add + ROI_HEAD_LEN), (char *)src_mat.data,
...
static void goodix_report_finger(struct gtx8_ts_data *ts,
				 struct ts_fingers *ts_fingers, u8 *touch_data,
				 int touch_num)
{
	...
	if (ts->dev_data->ts_platform_data->aft_param.aft_enable_flag &&
		...
		rect_data_size =
			touch_num * GTX8_SINGLE_RECT_DATA_SIZE >
				...
				GTX8_RECT_DATA_SIZE :
				touch_num *
					GTX8_SINGLE_RECT_DATA_SIZE; //data len of aft data.
		if (gtx8_i2c_read_trans(GTX8_RECT_DATA_ADDR, finger_rect_data,
					rect_data_size))
			TS_LOG_INFO("%s: Faied read RECT data\n", __func__);
	}
	memset(ts_fingers, 0x00, sizeof(struct ts_fingers));
	coor_data =
		&touch_data[2]; // touch_data size = 84 (4 + BYTES_PER_COORD * GTP_MAX_TOUCH )
	...
	for (i = 0; i < touch_num; i++) {
		...
		if (ts->dev_data->ts_platform_data->aft_param.aft_enable_flag) {
			...
			wy = finger_rect_data[i * GTX8_SINGLE_RECT_DATA_SIZE +
					      1];
			ewx = finger_rect_data[i * GTX8_SINGLE_RECT_DATA_SIZE +
					       2];
			ewy = finger_rect_data[i * GTX8_SINGLE_RECT_DATA_SIZE +
					       3];
			xer = finger_rect_data[i * GTX8_SINGLE_RECT_DATA_SIZE +
					       4];
			yer = finger_rect_data[i * GTX8_SINGLE_RECT_DATA_SIZE +
					       5];
		}
		if (id >= TS_MAX_FINGER) {
		...
		if ((ts->in_recovery_mode == GTX8_USE_IN_RECOVERY_MODE) &&
			...
			ts_fingers->fingers[id].x =
				x * ts->x_max_rc / (ts->max_x - 1);
			ts_fingers->fingers[id].y =
				y * ts->y_max_rc / (ts->max_y - 1);
		} else {
		...
		if (ts->dev_data->ts_platform_data->aft_param.aft_enable_flag) {
			...
			TS_LOG_DEBUG(
				"%s:[%d] wx %d, wy %d, ewx %d, ewy %d, xer %d, yer %d\n",
					...
					u8 *touch_data, int touch_num)
{
	...
	if (ts->dev_data->ts_platform_data->aft_param.aft_enable_flag &&
		...
		rect_data_size = touch_num * GTX8_SINGLE_RECT_DATA_SIZE >
					 ...
					 GTX8_RECT_DATA_SIZE :
					 touch_num * GTX8_SINGLE_RECT_DATA_SIZE;
		if (gtx8_i2c_read_trans(GT738X_RECT_DATA_ADDR, finger_rect_data,
					rect_data_size))
			TS_LOG_INFO("%s: Faied read RECT data\n", __func__);
	}
	memset(ts_fingers, 0x00, sizeof(struct ts_fingers));
...
static void goodix_report_pen(struct gtx8_ts_data *ts, struct ts_pens *ts_pens,
			      u8 *touch_data, int touch_num)
{
	int x = 0, y = 0, w = 0;
	u8 *coor_data = NULL;
	...
	if (!ts->dev_data->ts_platform_data->feature_info.pen_info
		     .pen_supported) {
		return;
	}
	...
	if (have_pen) {
		...
		if ((touch_data[1] & 0x10) == 0x10) {
			if ((touch_data[8 * touch_num + 2] & 0x10) == 0x10) {
				ts_pens->buttons[0].status =
					TS_PEN_BUTTON_PRESS;
				ts_pens->buttons[0].key = BTN_STYLUS;
				pre_key_of_pen |= 1 << 4;
				TS_LOG_DEBUG(
					"pen buttons down, status:0x%02x, key:0x%02x, pre_key:0x%02x\n",
					ts_pens->buttons[0].status,
					ts_pens->buttons[0].key,
					pre_key_of_pen);
			} else if ((pre_key_of_pen & 0x10) == 0x10) {
				ts_pens->buttons[0].status =
					TS_PEN_BUTTON_RELEASE;
				ts_pens->buttons[0].key = BTN_STYLUS;
				pre_key_of_pen &= ~(1 << 4);
				TS_LOG_DEBUG(
					"pen buttons up, status:0x%02x, key:0x%02x, pre_key:0x%02x\n",
					ts_pens->buttons[0].status,
					ts_pens->buttons[0].key,
					pre_key_of_pen);
			}
			TS_LOG_INFO(
				"gtx8 buttons[0].status:0x%02x, key:0x%02x\n",
				ts_pens->buttons[0].status,
				ts_pens->buttons[0].key);
		} else {
			if ((pre_key_of_pen & 0x10) == 0x10) {
				...
				ts_pens->buttons[0].key = BTN_STYLUS;
				TS_LOG_INFO(
					"gtx8 buttons up, status:0x%02x, key:0x%02x, pre_key:0x%02x\n",
					ts_pens->buttons[0].status,
					ts_pens->buttons[0].key,
					pre_key_of_pen);
			}
		...
		if ((pre_key_of_pen & 0x10) == 0x10) {
			...
			TS_LOG_INFO(
				"gtx8 buttons up, status:0x%02x, key:0x%02x, pre_key:0x%02x\n",
				ts_pens->buttons[0].status,
				ts_pens->buttons[0].key, pre_key_of_pen);
		}
...
static int stylus3_command(u8 buffer[], u32 len)
{
	...
	ret_i2c = gtx8_i2c_write(GTP_REG_CMD_GT7382, &buffer[0], len);
	if (ret_i2c) {
	...
	ret_i2c = gtx8_i2c_read(GTP_REG_CMD_GT7382, &buffer[0], len);
	if (ret_i2c) {
...
static void supend_stylus_command_sent(void)
{
	...
	if ((gtx8_ts->ic_type == IC_TYPE_7382) && (dev->send_bt_status_to_fw)) {
		stylus3_status = atomic_read(
			&(g_ts_kit_platform_data.current_stylus3_status));
...
static void gtx8_stylus_shift_freq_gt7382(const u8 *coor_data)
{
	...
	ret = gtx8_i2c_read_trans(GTP_REG_STYLUS3_SHIF_FREQ, &shift_freq_flag,
				  1);
	if (ret) {
	...
	if (shift_freq_flag == GTP_SHIFT_FREQ_IDLE) { /* idle */
		...
		TS_LOG_DEBUG("idle mode f1=%d, f2=%d\n", shift_freq_info_tmp.f1,
			     shift_freq_info_tmp.f2);
		...
		TS_LOG_INFO(
			"need shift freq f1=%d, f2=%d, next_f1=%d, next_f2=%d\n",
...
static int gtx8_write_hop_ack_gt7382(void)
{
	...
	while (retry_count) {
		...
		if (ret) {
			TS_LOG_ERR(
				"%s, write stylus3 shift freq reg err, retry=%d\n",
...
static void goodix_report_pen_gt7382(struct gtx8_ts_data *ts,
				     struct ts_pens *ts_pens, u8 *touch_data,
				     int touch_num)
{
	static int x;
	static int y;
	...
	if (!ts->dev_data->ts_platform_data->feature_info.pen_info.pen_supported)
		return;
	if ((touch_num >= 1) &&
	    (touch_data[BYTES_PER_COORD * (touch_num - 1) + HEAD_LEN] >=
	     0x80)) /* pen flag */
		have_pen = true;
	else
		have_pen = false;
...
static int gtx8_request_evt_handler(struct gtx8_ts_data *ts)
{
	...
	ret = gtx8_i2c_read_trans(ts->reg.fw_request, &rqst_data,
				  1); /*TS_REG_REQUEST*/
	if (ret < 0)
		return ret;
...
clear_requ:
	rqst_data = 0x00;
	ret = gtx8_i2c_write_trans(ts->reg.fw_request, &rqst_data,
				   1); /*TS_REG_REQUEST*/
	return ret;
}

static void gtx8_set_gesture_key(struct ts_fingers *info,
				 struct ts_cmd_node *out_cmd,
				 enum ts_gesture_num gesture)
{
	info->gesture_wakeup_value = gesture;
	out_cmd->command = TS_INPUT_ALGO;
	out_cmd->cmd_param.pub_params.algo_param.algo_order =
		gtx8_ts->dev_data->algo_id;
...
static int gtx8_remap_trace_id_gt7382(u8 *coor_buf, u32 coor_buf_len,
				      int touch_num)
{
	// clang-format off
	static u8 remap_array[20] = {
	...
	if (!coor_buf || coor_buf_len > (TOUCH_DATA_LEN_4 +
					 BYTES_PER_COORD * max_touch_num + 3)) {
					 ...
					 struct ts_cmd_node *out_cmd)
{
	...
	ret = gtx8_i2c_read_trans(ts->reg.coor, buffer,
				  BYTES_PEN_COORD + extra_data);
	if (unlikely(ret))
		goto exit;
	...
	if (unlikely(touch_num > GTP_MAX_TOUCH)) {
		...
		if (have_pen) {
			...
			ret = gtx8_i2c_read_trans(
				ts->reg.coor + extra_data + BYTES_PEN_COORD,
	...
	if (have_pen && (touch_num == 1)) { /* only pen event */
		...
		out_cmd->cmd_param.pub_params.report_touch_info.finger_pen_flag =
			TS_ONLY_PEN;
	} else if (have_pen && (touch_num > 1)) { /* has finger & pen event */
		goodix_report_finger_gt7382(ts, &temp_ts_fingers, buffer,
					    touch_num - 1);
		pre_finger_num = touch_num - 1;

		goodix_report_pen_gt7382(
			ts, &temp_ts_pens,
	...
	memcpy(&(ts_finger_pen->report_pen_info), &temp_ts_pens,
	       sizeof(ts_finger_pen->report_pen_info));
	memcpy(&(ts_finger_pen->report_finger_info), &temp_ts_fingers,
	       sizeof(ts_finger_pen->report_finger_info));
				  ...
				  struct ts_cmd_node *out_cmd)
{
	...
	ret = gtx8_i2c_read_trans(ts->reg.coor, touch_data,
				  TOUCH_DATA_LEN_4 + BYTES_PER_COORD);
	if (unlikely(ret))
		goto exit;

	if (unlikely((touch_data[0] & GTX8_REQUEST_EVENT) ==
		     GTX8_REQUEST_EVENT)) {
		/* hw request */
	...
	if (unlikely(chksum != 0)) {
		for (i = 0; i < touch_num * BYTES_PER_COORD + TOUCH_DATA_LEN_4;
		     i++)
			TS_LOG_ERR("[0x%04X]:0x%02X\n", GTP_REG_COOR + i,
				   touch_data[i]);
		TS_LOG_ERR("Checksum error:%X\n", chksum);
	...
	if (touch_num >= 1 &&
	    touch_data[BYTES_PER_COORD * (touch_num - 1) + DMNNY_BYTE_LEN_2] >=
		    0x80) {
		have_pen = true;
	...
	if (have_pen) {
		...
		if (pre_pen_num > 0) {
			...
			pre_finger_num = touch_num;
			goodix_report_finger(ts, ts_fingers, touch_data,
					     touch_num);
			out_cmd->command = TS_INPUT_ALGO;
		}
	}
...
static int gtx8_input_pen_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input, ABS_PRESSURE, 0, GTX8_MAX_PEN_PRESSURE, 0,
			     0);
	input_set_abs_params(input, ABS_TILT_X, -1 * GTX8_MAX_PEN_TILT,
			     GTX8_MAX_PEN_TILT, 0, 0);
	input_set_abs_params(input, ABS_TILT_Y, -1 * GTX8_MAX_PEN_TILT,
			     GTX8_MAX_PEN_TILT, 0, 0);
	return NO_ERR;
}
				...
				struct ts_cmd_node *out_cmd)
{
	...
	ts_fingers = &out_cmd->cmd_param.pub_params.algo_param.info;
	TS_LOG_DEBUG(
		"%s: wakeup_gesture_enable:%u,sleep_mode:%d,ts_kit_gesture_func:%d,easy_wakeup_gesture:%d\n",
		__func__, ts->easy_wakeup_supported,
		ts->dev_data->easy_wakeup_info.sleep_mode, ts_kit_gesture_func,
		ts->dev_data->easy_wakeup_info.easy_wakeup_gesture);
	...
	/*sync_evt:*/
	if (!ts->rawdiff_mode) {
		ret = gtx8_i2c_write_trans(ts->reg.coor, &sync_val,
					   sizeof(sync_val));
		if (ret < 0)
...
static int gtx8_fw_update_boot(char *file_name)
{
	...
	if (ts->support_filename_contain_lcd_module)
		snprintf(update_ctrl.fw_name, GTX8_FW_NAME_LEN,
			 "ts/%s%s_%s_%s.img", file_name, ts->project_id,
			 ts->dev_data->module_name, gtx8_ts->lcd_module_name);
	else
		snprintf(update_ctrl.fw_name, GTX8_FW_NAME_LEN,
			 "ts/%s%s_%s.img", file_name, ts->project_id,
...
static int gtx8_chip_after_resume(void *feature_info)
{
	...
	if (gtx8_ts->ic_type == IC_TYPE_7382) {
		if ((tp_ops) && (tp_ops->get_status_by_type)) {
			retval = tp_ops->get_status_by_type(
				PT_STATION_TYPE, &tskit_pt_station_flag);
			if (retval < 0) {
				TS_LOG_ERR(
					"%s: get tskit_pt_station_flag fail\n",
					__func__);
				return retval;
			}
		}
		if ((gtx8_ts->dev_data->easy_wakeup_info.sleep_mode ==
		     TS_POWER_OFF_MODE) &&
		    (!tskit_pt_station_flag))
			msleep(GT73X_AFTER_REST_DELAY);
		else
	...
	if ((gtx8_ts->ic_type == IC_TYPE_7382) &&
	    (gtx8_ts->dev_data->easy_wakeup_info.sleep_mode == TS_GESTURE_MODE))
		gtx8_touch_switch();
	return NO_ERR;
}
...
static int gtx8_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_WRITE:
		if (info->glove_switch) {
			...
			ret = gtx8_feature_switch(gtx8_ts, TS_FEATURE_GLOVE,
						  SWITCH_ON);
			if (!ret)
				glove_en = true;
		} else {
...
void gtx8_pen_respons_mode_change(int soper)
{
	...
	case TS_SWITCH_PEN_RESPON_NORMAL:
		if (gtx8_ts->ic_type == IC_TYPE_7382) {
			ret = gtx8_send_cmd(GTX8_CMD_PEN_NORMAL_MODE_7382,
...
static void gtx8_touch_switch(void)
{
	...
	if (!(gtx8_ts->dev_data->touch_switch_flag & TS_SWITCH_TYPE_DOZE) &&
	    ...
	    !(gtx8_ts->dev_data->touch_switch_flag &
	      TS_SWITCH_TYPE_SCENE)) { //pen respons
...
static int gtx8_holster_switch(struct ts_holster_info *info)
{
	...
	case TS_ACTION_WRITE:
		if (info->holster_switch)
			ret = gtx8_feature_switch(gtx8_ts, TS_FEATURE_HOLSTER,
						  SWITCH_ON);
		else
			ret = gtx8_feature_switch(gtx8_ts, TS_FEATURE_HOLSTER,
						  SWITCH_OFF);
	...
	case TS_ACTION_READ:
		TS_LOG_INFO(
			"%s: invalid holster switch(%d) action: TS_ACTION_READ\n",
...
static u8 *gtx8_roi_rawdata(void)
{
	...
	if (wait_for_completion_interruptible_timeout(&roi_data_done,
		...
		if (gtx8_ts->roi.enabled && gtx8_ts->roi.data_ready) {
			...
			TS_LOG_ERR(
				"%s : roi is not enable or roi data not ready\n",
				__func__);
		}
...
static int gtx8_check_hw_status(void)
{
	...
	if (!gtx8_ts->tool_esd_disable) {
		if (gtx8_ts->ic_type == IC_TYPE_7382) {
			/* static esd check */
			for (j = 0; j < 3; j++) {
				ret = gtx8_ts->ops.read_trans(
					GTP_REG_ESD_TICK_R_GT7382, &esd_state,
					1);
				if (ret < 0)
					break;
			...
			if (ret || (j >= 3)) {
				TS_LOG_ERR(
					"HW status:Error,static{8043h}=%02X\n",
					esd_state);
				ret = -EIO;
			}
		...
		for (i = 0; i < 3; i++) {
			ret = gtx8_ts->ops.read_trans(GTP_REG_ESD_TICK_R,
						      &esd_state, 1);
			if (ret < 0)
				break;
			if (esd_state != 0xaa)
		...
		if (likely(!ret && i < 3)) {
			...
			esd_state = 0xaa;
			ret = gtx8_ts->ops.write_trans(GTP_REG_ESD_TICK_W,
						       &esd_state, 1);
		} else {
...
skip_dynamic_esd_check:
		...
		if (!gtx8_ts->support_checked_esd_reset_chip) {
			...
			read_reg_ret = gtx8_i2c_read_trans(
				read_debug_msg_reg,
				tmp_reg_buf +
					(tri_cnt - 1) * read_debug_reg_len,
				read_debug_reg_len);
		...
		if (WD_TRI_TIMES_5 == tri_cnt) {
			for (i = 0; i < WD_TRI_TIMES_5; i++) {
				if (gtx8_ts->ic_type == IC_TYPE_7382)
					...
					TS_LOG_INFO(
						"%dS:[FF00]:0x%02X, [FF01]:0x%02X, [FF02]:0x%02X, [FF03]:0x%02X, [FF04]:0x%02X, [FF05]:0x%02X\n",
						i,
						tmp_reg_buf[read_debug_reg_len *
							    i],
						tmp_reg_buf[read_debug_reg_len *
								    i +
							    1],
					...
					/* 1,2,3,4,5,6,7,8, debug reg index */
					TS_LOG_INFO(
						"%dS:[4B64]:0x%02X, [4B65]:0x%02X, [4B66]:0x%02X, [4B67]:0x%02X, [4B68]:0x%02X, [4B69]:0x%02X\n",
						i,
						tmp_reg_buf[read_debug_reg_len *
							    i],
						tmp_reg_buf[read_debug_reg_len *
				...
				if (gtx8_ts->support_read_more_debug_msg) {
					if (gtx8_ts->ic_type == IC_TYPE_7382)
						TS_LOG_INFO(
							"%dS:[FF06]:0x%02X, [FF07]:0x%02X, [FF08]:0x%02X\n",
							i,
							tmp_reg_buf
								[read_debug_reg_len *
									 i +
						...
						TS_LOG_INFO(
							"%dS:[4B6A]:0x%02X, [4B6B]:0x%02X, [4B6C]:0x%02X\n",
							i,
							tmp_reg_buf
								[read_debug_reg_len *
									 i +
...
static int gtx8_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
	int ret = NO_ERR;
	...
	switch (info->op_action) {
	case TS_ACTION_READ:
		memcpy(info->tp_test_type, gtx8_ts->dev_data->tp_test_type,
...
*******************************************************************************/
extern u8 gtx8_get_cfg_value(u8 *config, u8 *buf, u8 len, u8 sub_bag_num,
			     u8 offset, u8 buf_size)
{
	u8 *sub_bag_ptr = NULL;
	u8 i = 0;
	if (!config || !buf) {
 ...
 * Return : u8(return data len success read from config. < 0 failed)
 */
static u8 gt8x_get_cfg_value_gt7382(u8 *config, u8 *buf, u8 len, u16 addr)
{
	u8 *ptr = NULL;
	struct gtx8_ts_data *ts = gtx8_ts;
...
int gtx8_get_channel_num_gt7382(u32 *sen_num, u32 *drv_num, u8 *cfg_data)
{
	int ret;
	...
	ret = gt8x_get_cfg_value_gt7382(
		temp_cfg_data, buf, GET_CHANNEL_BUF_SIZE, GT8X_7382_DRV_ADDR);
	if (ret > 0) {
	...
	ret = gt8x_get_cfg_value_gt7382(
		temp_cfg_data, buf, GET_CHANNEL_BUF_SIZE, GT8X_7382_SEN_ADDR);
	if (ret > 0) {
...
int gtx8_get_channel_num(u32 *sen_num, u32 *drv_num, u8 *cfg_data)
{
	...
	ret = gtx8_get_cfg_value(temp_cfg_data, buf, 2, 1, 14,
...
static int gtx8_get_debug_data_(u16 *debug_data, u32 sen_num, u32 drv_num,
				int data_type)
{
	int i = 0;
	int ret = 0;
	...
	data_len =
		(sen_num * drv_num) * sizeof(u16); /*  raw data is u16 type */
	if (data_len <= 0) {
	...
	if (wait_cnt < GTX8_RETRY_NUM_3) {
		ret = gtx8_i2c_read(data_start_addr, ptr, data_len);
		if (ret) {
			...
			struct ts_cmd_node *out_cmd)
{
	...
	if (gtx8_ts->ic_type == IC_TYPE_7382)
		ret = gtx8_get_channel_num_gt7382(&sen_num, &drv_num,
						  temp_cfg_data);
	else
	...
	case READ_DIFF_DATA:
		...
		ret = gtx8_get_debug_data_(debug_data, sen_num, drv_num,
					   READ_DIFF_DATA);
		break;
	case READ_RAW_DATA:
	...
	ts_kit_rotate_rawdata_abcd2cbad(
		sen_num, drv_num, (info->buff + 2),
		rotate_type); /* 2: offset tx_num & rx_num */
...
{
	if (gtx8_ts->dev_data->ts_platform_data->feature_info.roi_info
		    .roi_supported ||
	    !gtx8_ts->roi.rawdata) {
		...
		TS_LOG_DEBUG(
			"%s: pre[%d]  cur[%d]  (cur_index & pre_index):[%d] .\n",
...
static int gtx8_charger_mode_gt7382(u8 charger_switch)
{
	...
	u8 charge_on_buffer[GTX8_CHARGE_BUFEER_LEN] = {GTX8_CHARGE_ON_VALUE_0,
						       GTX8_CHARGE_ON_VALUE_1,
						       GTX8_CHARGE_ON_VALUE_2};
	u8 charge_off_buffer[GTX8_CHARGE_BUFEER_LEN] = {
		GTX8_CHARGE_OFF_VALUE_0, GTX8_CHARGE_OFF_VALUE_1,
		GTX8_CHARGE_OFF_VALUE_2};
	if (charger_switch) {
...
static int gtx8_charger_switch(struct ts_charger_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		if ((!feature_info->holster_info.holster_switch) &&
		    ...
		    (!gtx8_ts->game_switch)) {
			if (gtx8_ts->ic_type == IC_TYPE_7382)
				return gtx8_charger_mode_gt7382(
					info->charger_switch);
			if (info->charger_switch) {
				ret = gtx8_feature_switch(
					gtx8_ts, TS_FEATURE_CHARGER, SWITCH_ON);
			...
			if (gtx8_send_cmd(cmd, 0, GTX8_NOT_NEED_SLEEP)) {
				TS_LOG_ERR(
					"Send stylus wake up config failed.\n");
				ret = -EINVAL;
			}
...
struct ts_device_ops ts_gtx8_ops = {
	...
	.chip_get_capacitance_test_type = gtx8_chip_get_capacitance_test_type,
	...
	.chip_wakeup_gesture_enable_switch = gtx8_wakeup_gesture_enable_switch,
