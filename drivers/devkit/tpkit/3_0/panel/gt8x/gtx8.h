...
struct gtx8_ts_ops {
	int (*i2c_write)(u16 addr, u8 *buffer,
			 u32 len); //need to wakeup from doze
	int (*i2c_read)(u16 addr, u8 *buffer, u32 len);
	int (*write_trans)(u16 addr, u8 *buffer,
			   u32 len); //no need to wakeup from doze
	...
	int (*parse_cfg_data)(const struct firmware *cfg_bin, char *cfg_type,
