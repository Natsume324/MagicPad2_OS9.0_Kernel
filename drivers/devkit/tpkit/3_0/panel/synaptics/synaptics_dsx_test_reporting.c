...
#define show_prototype(propname)                                       \
	...
	static struct device_attribute dev_attr_##propname =           \
		__ATTR(propname, S_IRUGO,                              \
		       concat(synaptics_rmi4_f54, _##propname##_show), NULL);
...
#define store_prototype(propname)                                       \
	static ssize_t concat(synaptics_rmi4_f54, _##propname##_store)( \
		...
		const char *buf, size_t count);                         \
                                                                        \
	static struct device_attribute dev_attr_##propname =            \
		__ATTR(propname, (S_IWUSR | S_IWGRP), NULL,             \
...
#define show_replicated_func(rtype, rgrp, propname, fmt)                       \
	...
	{                                                                      \
		...
		retval = f54->fn_ptr->read(                                    \
			rmi4_data, f54->rtype.rgrp->address,                   \
			(unsigned char *)f54->rtype.rgrp->data, length);       \
		mutex_unlock(&f54->rtype##_mutex);                             \
		if (retval < 0) {                                              \
...
#endif
		static ssize_t
	synaptics_rmi4_f54_data_read(struct file *data_file,
				     struct kobject *kobj,
...
static void set_report_size(void)
{
	...
	case F54_ADC_RANGE:
		if (f54->query.has_signal_clarity) {
			...
			retval = f54->fn_ptr->read(
				rmi4_data, f54->control.reg_41->address,
...
static int do_preparation(void)
{
	...
	if (f54->query.touch_controller_family == 1) {
		value = 0;
		retval = f54->fn_ptr->write(rmi4_data,
					    f54->control.reg_7->address, &value,
...
static ssize_t
synaptics_rmi4_f54_report_size_show(struct device *dev,
...
static ssize_t
synaptics_rmi4_f54_no_auto_cal_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", f54->no_auto_cal);
}
...
static ssize_t
synaptics_rmi4_f54_no_auto_cal_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	int retval = NO_ERR;
...
static ssize_t
synaptics_rmi4_f54_report_type_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", f54->report_type);
}
...
static int synaptics_rmi4_f54_report_type_set(char setting)
{
	...
	if (f54->status != STATUS_BUSY) {
		...
		retval = f54->fn_ptr->write(rmi4_data, f54->data_base_addr,
					    &data, sizeof(data));
		mutex_unlock(&f54->status_mutex);
		if (retval < 0) {
...
static ssize_t
synaptics_rmi4_f54_report_type_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	int retval = NO_ERR;
						 ...
						 char *buf)
{
	...
	retval = f54->fn_ptr->read(
		rmi4_data, f54->data_base_addr + DATA_REPORT_INDEX_OFFSET, data,
		sizeof(data));
	if (retval < 0) {
...
static ssize_t
synaptics_rmi4_f54_do_preparation_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	int retval = NO_ERR;
...
#ifdef WATCHDOG_HRTIMER
	hrtimer_start(&f54->watchdog, ktime_set(WATCHDOG_TIMEOUT_S, 0),
		      HRTIMER_MODE_REL);
#endif
...
static ssize_t
synaptics_rmi4_f54_get_report_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count)
{
	int retval = NO_ERR;
...
static void mmi_rawimage_report(unsigned char *buffer)
{
	...
	len = rx * tx * DATA_TEMP_BUFF_SIZE + F54_MAX_CAP_TITLE_SIZE -
	...
	for (i = 0; i < tx; i++) {
		for (j = 0; j < rx; j++) {
			...
			DataArray[i * rx + j] =
				temp; /*float is not allowed in kernel space*/
			k = k + 2;
...
static int RxtoRx1ShortTest(unsigned char *buffer)
{
	...
	for (i = 0; i < tx; i++) {
		for (j = 0; j < rx; j++) {
			...
			if (i == j) {
				len = 6 * rx + F54_MAX_CAP_TITLE_SIZE -
				      strlen(g_mmi_RxtoRxshort_report) - 1;
				strncat(g_mmi_RxtoRxshort_report, buf, len);
...
static void mmi_RxtoRxshort1_report(unsigned char *buffer)
{
	int count = 0;
	/*enum mmi_results TestResult = TEST_PASS;*/
	memset(g_mmi_RxtoRxshort_report, 0,
	       (6 * rx + F54_MAX_CAP_TITLE_SIZE)); /*only Diagonal for V3*/
...
static void mmi_maxmincapacitance_report(unsigned char *buffer)
{
	...
	len = 2 * DATA_TEMP_BUFF_SIZE + F54_MAX_CAP_TITLE_SIZE -
	      strlen(g_mmi_maxmincapacitance_report) - 1;
...
static void mmi_highresistance_report(unsigned char *buffer)
{
	...
	len = 3 * DATA_TEMP_BUFF_SIZE + F54_MAX_CAP_TITLE_SIZE -
	      strlen(g_mmi_highresistance_report) - 1;
	for (i = 0; i < 3; i++, k += 2) {
		temp = buffer[k] | (buffer[k + 1] << 8);
...
static void mmi_delta_report(unsigned char *buffer)
{
	...
	memset(g_buf_debug_data, 0,
	       (rx * tx * DATA_TEMP_BUFF_SIZE + F54_MAX_CAP_TITLE_SIZE +
		F54_MAX_CAP_TITLE_SIZE + F54_MAX_CAP_TITLE_SIZE));
	sprintf(g_buf_debug_data, "delta_data:\n");
	len = rx * tx * DATA_TEMP_BUFF_SIZE + F54_MAX_CAP_TITLE_SIZE +
	      F54_MAX_CAP_TITLE_SIZE + F54_MAX_CAP_TITLE_SIZE -
	      strlen(g_buf_debug_data) - 1;
	DataArray = (short *)kmalloc(sizeof(short) * tx * rx, GFP_KERNEL);
	if (DataArray == NULL) {
		TS_LOG_ERR("%s:kmalloc failed\n", __func__);
		return;
...
static int mmi_runtest(int report_type)
{
	...
	case F54_FULL_RAW_CAP_RX_COUPLING_COMP:
		if (!g_mmi_buf_f54raw_data)
			g_mmi_buf_f54raw_data =
				kmalloc(rx * tx * DATA_TEMP_BUFF_SIZE +
						F54_MAX_CAP_TITLE_SIZE,
					GFP_KERNEL);
	...
	case F54_RX_TO_RX1:
		if (!g_mmi_RxtoRxshort_report)
			g_mmi_RxtoRxshort_report = kmalloc(
				6 * rx + F54_MAX_CAP_TITLE_SIZE, GFP_KERNEL);
	...
	case F54_16BIT_IMAGE:
		if (!g_buf_debug_data)
			g_buf_debug_data =
				kmalloc(rx * tx * DATA_TEMP_BUFF_SIZE +
						F54_MAX_CAP_TITLE_SIZE +
						4 * F54_MAX_CAP_TITLE_SIZE,
					GFP_KERNEL); /*buf len for test*/
...
static int synaptics_rmi4_irq_enable(bool enable)
{
	...
	retval = f54->fn_ptr->read(rmi4_data, reg_addr, &reg_data,
				   sizeof(reg_data));
	...
	retval = f54->fn_ptr->write(rmi4_data, reg_addr, &reg_data,
				    sizeof(reg_data));
						...
						char *buf)
{
	...
	atomic_set(&rmi4_data->synaptics_chip_data->ts_platform_data->state,
		   TS_MMI_CAP_TEST);
...
static int read_debug_reg_status(unsigned char *buffer)
{
	...
	retval = f54->fn_ptr->read(f54->rmi4_data, 0x0051, &command,
				   sizeof(command));
	if (retval < 0) {
	...
	retval = f54->fn_ptr->read(f54->rmi4_data, 0x0052, &command,
				   sizeof(command));
	if (retval < 0) {
	...
	retval = f54->fn_ptr->read(f54->rmi4_data, 0x0013, &command,
				   sizeof(command));
	if (retval < 0) {
	...
	retval = f54->fn_ptr->read(f54->rmi4_data, 0x0014, &command,
				   sizeof(command));
	if (retval < 0) {
...
static int read_debug_power_pin_status(unsigned char *buffer)
{
	...
	char buf[10] = {0};
	size_t len = 0;
	retval = gpio_get_value(f54->rmi4_data->synaptics_chip_data
	...
	retval = gpio_get_value(
		f54->rmi4_data->synaptics_chip_data->ts_platform_data->irq_gpio);
	if (retval < 0) {
	...
	retval = gpio_get_value(
		f54->rmi4_data->synaptics_chip_data->vci_gpio_ctrl);
	if (retval < 0) {
	...
	retval = gpio_get_value(
		f54->rmi4_data->synaptics_chip_data->vddio_gpio_ctrl);
	if (retval < 0) {
	...
	sprintf(buf, "%d ",
		atomic_read(&rmi4_data->synaptics_chip_data->ts_platform_data
				     ->state));
	len = rx * tx * DATA_TEMP_BUFF_SIZE + F54_MAX_CAP_TITLE_SIZE +
	      F54_MAX_CAP_TITLE_SIZE + F54_MAX_CAP_TITLE_SIZE -
	      strlen(g_buf_debug_data) - 1;
						  ...
						  char *buf)
{
	...
	if ((TS_SLEEP == atomic_read(&rmi4_data->synaptics_chip_data
					      ->ts_platform_data->state)) ||
	    (TS_MMI_CAP_TEST ==
	     atomic_read(&rmi4_data->synaptics_chip_data->ts_platform_data
				  ->state))) {
		TS_LOG_INFO(
			"tp state unsupport debug.ts_platform_data.state is %d\n",
...
static ssize_t synaptics_rmi4_f54_mmi_test_result_show(
...
static int add_synaptics_mmi_test_interfaces(struct device *dev)
{
	...
	error = sysfs_create_file(properties_kobj,
...
static ssize_t
synaptics_rmi4_f54_resume_touch_store(struct device *dev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	int retval = NO_ERR;
						   ...
						   *attr, char *buf)
{
	...
	retval = f54->fn_ptr->read(rmi4_data, f54->control.reg_17->address,
	...
	retval = f54->fn_ptr->read(rmi4_data, f54->control.reg_18->address,
	...
	for (ii = 0; ii < f54->control.reg_17->length; ii++) {
		retval = snprintf(
			temp, PAGE_SIZE - size, "%u ",
			(1 << 8) * f54->control.reg_17->data[ii]
						.burst_count_b8__10 +
...
static int synaptics_rmi4_f54_set_sysfs(void)
{
	...
	TS_LOG_INFO("%s: in!\n", __func__);
	f54->attr_dir =
		kobject_create_and_add("f54", &rmi4_data->input_dev->dev.kobj);
	if (!f54->attr_dir) {
...
static int synaptics_rmi4_f54_set_ctrl(void)
{
	...
	if ((f54->query.touch_controller_family == 0) ||
		...
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_4__6 =
			kzalloc(sizeof(*(control->reg_4__6)), GFP_KERNEL);
		if (!control->reg_4__6)
			goto exit_no_mem;
	...
	if ((f54->query.touch_controller_family == 0) ||
		...
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_8__9 =
			kzalloc(sizeof(*(control->reg_8__9)), GFP_KERNEL);
		if (!control->reg_8__9)
			goto exit_no_mem;
	...
	if (f54->query.has_interference_metric == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_10 =
			kzalloc(sizeof(*(control->reg_10)), GFP_KERNEL);
		if (!control->reg_10)
			goto exit_no_mem;
	...
	if (f54->query.has_ctrl11 == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_11 =
			kzalloc(sizeof(*(control->reg_11)), GFP_KERNEL);
		if (!control->reg_11)
			goto exit_no_mem;
	...
	if (f54->query.has_relaxation_control == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_12__13 =
			kzalloc(sizeof(*(control->reg_12__13)), GFP_KERNEL);
	...
	if (f54->query.has_sensor_assignment == 1) {
		...
		control->reg_14 =
			kzalloc(sizeof(*(control->reg_14)), GFP_KERNEL);
		if (!control->reg_14)
			goto exit_no_mem;
	...
	if (f54->query.has_sense_frequency_control == 1) {
		...
		control->reg_17 =
			kzalloc(sizeof(*(control->reg_17)), GFP_KERNEL);
		if (!control->reg_17)
			goto exit_no_mem;
		control->reg_17->length = length;
		control->reg_17->data = kzalloc(
			length * sizeof(*(control->reg_17->data)), GFP_KERNEL);
		...
		control->reg_18 =
			kzalloc(sizeof(*(control->reg_18)), GFP_KERNEL);
		if (!control->reg_18)
			goto exit_no_mem;
		control->reg_18->length = length;
		control->reg_18->data = kzalloc(
			length * sizeof(*(control->reg_18->data)), GFP_KERNEL);
		...
		control->reg_19 =
			kzalloc(sizeof(*(control->reg_19)), GFP_KERNEL);
		if (!control->reg_19)
			goto exit_no_mem;
		control->reg_19->length = length;
		control->reg_19->data = kzalloc(
			length * sizeof(*(control->reg_19->data)), GFP_KERNEL);
	...
	if (f54->query.has_sense_frequency_control == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_21 =
			kzalloc(sizeof(*(control->reg_21)), GFP_KERNEL);
		if (!control->reg_21)
			goto exit_no_mem;
	...
	if (f54->query.has_firmware_noise_mitigation == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_22__26 =
			kzalloc(sizeof(*(control->reg_22__26)), GFP_KERNEL);
	...
	if (f54->query.has_iir_filter == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_27 =
			kzalloc(sizeof(*(control->reg_27)), GFP_KERNEL);
		if (!control->reg_27)
			goto exit_no_mem;
	...
	if (f54->query.has_firmware_noise_mitigation == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_28 =
			kzalloc(sizeof(*(control->reg_28)), GFP_KERNEL);
		if (!control->reg_28)
			goto exit_no_mem;
	...
	if (f54->query.has_cmn_removal == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_29 =
			kzalloc(sizeof(*(control->reg_29)), GFP_KERNEL);
		if (!control->reg_29)
			goto exit_no_mem;
	...
	if (f54->query.has_cmn_maximum == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_30 =
			kzalloc(sizeof(*(control->reg_30)), GFP_KERNEL);
		if (!control->reg_30)
			goto exit_no_mem;
	...
	if (f54->query.has_touch_hysteresis == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_31 =
			kzalloc(sizeof(*(control->reg_31)), GFP_KERNEL);
		if (!control->reg_31)
			goto exit_no_mem;
	...
	if (f54->query.has_edge_compensation == 1) {
		attrs_ctrl_regs_exist[reg_num] = true;
		control->reg_32__35 =
			kzalloc(sizeof(*(control->reg_32__35)), GFP_KERNEL);
	...
	if ((f54->query.curve_compensation_mode == 1) ||
		...
		control->reg_36 =
			kzalloc(sizeof(*(control->reg_36)), GFP_KERNEL);
		if (!control->reg_36)
			goto exit_no_mem;
		control->reg_36->length = length;
		control->reg_36->data = kzalloc(
			length * sizeof(*(control->reg_36->data)), GFP_KERNEL);
	...
	if (f54->query.has_per_frequency_noise_control == 1) {
		...
		control->reg_38 =
			kzalloc(sizeof(*(control->reg_38)), GFP_KERNEL);
		if (!control->reg_38)
			goto exit_no_mem;
		control->reg_38->length = num_of_sensing_freqs;
		control->reg_38->data =
			kzalloc(control->reg_38->length *
					sizeof(*(control->reg_38->data)),
				GFP_KERNEL);
...
static int synaptics_rmi4_f54_status_work(struct work_struct *work)
{
	...
	retval = f54->fn_ptr->write(
		rmi4_data, f54->data_base_addr + DATA_REPORT_INDEX_OFFSET,
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (ii = PDT_START; ii > PDT_END; ii -= PDT_ENTRY_SIZE) {
			...
			case SYNAPTICS_RMI4_F54:
				synaptics_rmi4_f54_set_regs(rmi4_data, &rmi_fd,
							    intr_count, page);
				f54found = true;
				break;
			case SYNAPTICS_RMI4_F55:
				synaptics_rmi4_f55_set_regs(rmi4_data, &rmi_fd,
							    page);
				f55found = true;
				break;
			default:
