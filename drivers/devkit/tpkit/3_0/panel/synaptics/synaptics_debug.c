...
static char tp_test_failed_reason[TP_TEST_FAILED_REASON_LEN] = {
	"-software_reason"};
...
static int synaptics_get_threshold_from_csvfile(int columns, int rows,
						char *target_name,
						int32_t *data);
...
static void set_report_size(void)
{
	...
	case F54_TEST_100_TYPE:
		f54->report_size = 2 * f54->rmi4_data->num_of_rx *
	...
	case F54_ADC_RANGE:
		if (f54->query.has_signal_clarity) {
			rc = f54->fn_ptr->read(
				f54->rmi4_data, f54->control.reg_41->address,
	...
	case F54_HYBRID_SENSING_RAW_CAP:
		if (f54->rmi4_data->synaptics_chip_data
...
static int test_wait_for_command_completion(void)
{
	...
	timeout_count = 0;
	do {
		retval = f54->fn_ptr->read(rmi4_data, f54->command_base_addr,
					   &value, sizeof(value));
		if (retval < 0) {
...
static int test_do_command(unsigned char command)
{
	...
	retval = f54->fn_ptr->write(rmi4_data, f54->command_base_addr, &command,
				    sizeof(command));
	if (retval < 0) {
...
static int write_to_f54_register(unsigned char report_type)
{
	...
	if (F54_RX_TO_RX1 != command) {
		result = f54->fn_ptr->write(f54->rmi4_data, f54->data_base_addr,
					    &command, 1);
		if (result < 0) {
	...
	result = f54->fn_ptr->write(f54->rmi4_data, f54->command_base_addr,
				    &command, 1);

retry:
	msleep(100);
	result = f54->fn_ptr->read(f54->rmi4_data, f54->command_base_addr,
				   &ready_report, 1);
	...
	if ((ready_report & COMMAND_GET_REPORT) && retry_times < 20) {
		retry_times++;
		TS_LOG_INFO(
			"report is not ready, retry times: %d, ready_report: %d\n",
...
static int mmi_add_static_data(void)
{
	...
	snprintf((buf_f54test_result + i), F54_BUF_LEN - i, "[%4d,%4d,%4d]",
	...
	snprintf((buf_f54test_result + i), F54_BUF_LEN - i - 1, "[%4d,%4d,%4d]",
...
static int f54_deltarawimage_report(void)
{
	...
	int delt_cap_lowlimit = f54->rmi4_data->synaptics_chip_data
					->raw_limit_buf[DELT_DATA_LOW];
	...
	if (f54->rmi4_data->synaptics_chip_data->test_capacitance_via_csvfile) {
		limit_tab.unique_test = kzalloc(
			sizeof(struct ts_unique_capacitance_test), GFP_KERNEL);
		if (!limit_tab.unique_test) {
			TS_LOG_ERR(
				"ts_unique_capacitance_test kzalloc error\n");
			ret = TEST_FAIL;
			goto error_release_mem;
		}
		limit_tab.unique_test->noise_data_limit = (int32_t *)kzalloc(
			NOISE_DATA_LIMIT_SIZE * sizeof(int32_t), GFP_KERNEL);
		...
		if (TEST_FAIL ==
		    synaptics_get_threshold_from_csvfile(
			    CSVFILE_NOISE_COLUMN, CSVFILE_NOISE_ROW,
			    CSV_TP_NOISE_DATA_LIMIT,
			    limit_tab.unique_test->noise_data_limit)) {
			TS_LOG_ERR(
				"csvfile get noise_data_limit err, use dts config.\n");
		} else {
			delt_cap_uplimit =
				limit_tab.unique_test
					->noise_data_limit[LIMIT_TWO];
			delt_cap_lowlimit =
				limit_tab.unique_test
					->noise_data_limit[LIMIT_ONE];
		}
	}
	...
	for (i = 0; i < mmi_buf_size; i += 2) {
		...
		if ((Rawimage >= delt_cap_lowlimit) &&
			...
			TS_LOG_ERR(
				"[No:%d,current is %d] delt_cap_uplimit = %d, delt_cap_lowlimit = %d\n",
				i / 2, Rawimage, delt_cap_uplimit,
				delt_cap_lowlimit);
		}
	}
...
static void put_hybrid_data(int index)
{
	...
	TS_LOG_INFO(
		"*******put_hybrid_data start****mmi_hybrid_abs_delta=%d****\n",
	...
	if (f54->rmi4_data->synaptics_chip_data
		...
		if ((mmi_hybrid_abs_delta / 4) + index - 1 < HYBRID_BUF_SIZE) {
			if (0 == strcmp(f54->rmi4_data->module_name, "BOE")) {
				f54->forcekey_value[0] =
					f54->hybridbuf[(mmi_hybrid_abs_delta /
							4) +
						       index - 2];
				f54->forcekey_value[1] =
					f54->hybridbuf[(mmi_hybrid_abs_delta /
							4) +
						       index - 1];
			} else if (0 ==
				   strcmp(f54->rmi4_data->module_name, "LG")) {
				f54->forcekey_value[0] =
					f54->hybridbuf[(mmi_hybrid_abs_delta /
							4) +
						       index - 2];
...
static void check_hybrid_raw_cap(void)
{
	...
	snprintf(file_name, sizeof(file_name) - 1, "%s_%s_%s_%s_raw.csv",
		 f54->rmi4_data->synaptics_chip_data->ts_platform_data
			 ->product_name,
	...
	snprintf(file_path, sizeof(file_path) - 1,
		 "/product/etc/firmware/ts/%s", file_name);
#else
	snprintf(file_path, sizeof(file_path) - 1, "/odm/etc/firmware/ts/%s",
		 file_name);
#endif
	...
	if (0 != result) {
		TS_LOG_ERR(
			"%s, ts_kit_parse_csvfile  parse tx range fail result=%dis 6F\n",
			__func__, result);
		goto test_fail;
	}
	...
	for (i = 0; i < tx_num; i++) {
		if (false == is_in_range(f54->hybridbuf[index + i + rx_num],
					 hybird_raw_tx_range[i + tx_num],
					 hybird_raw_tx_range[i])) {
			TS_LOG_ERR(
				"%s, check_hybrid_raw_cap test is out of range, test result is 6F\n",
				__func__);
			TS_LOG_ERR(
				"current(%d) :%d |uplimit :%d |downlimit:%d\n",
				i, f54->hybridbuf[index + i + rx_num],
				hybird_raw_tx_range[i + tx_num],
				hybird_raw_tx_range[i]);
			strncpy(tp_test_failed_reason, "-panel_reason",
				TP_TEST_FAILED_REASON_LEN);
			test_failed_reason[RAW_DATA_TYPE_SELFCAP] =
				TEST_THRESHOLD_FAIL;
			goto test_fail;
		}
	}
	...
	if (0 != result) {
		TS_LOG_ERR(
			"%s, ts_kit_parse_csvfile  parse rx fail result=%dis 6F\n",
			__func__, result);
		goto test_fail;
	}
	...
	for (i = 0; i < rx_num; i++) {
		if (false == is_in_range(f54->hybridbuf[index + i],
					 hybird_raw_rx_range[i + rx_num],
					 hybird_raw_rx_range[i])) {
			TS_LOG_ERR(
				"%s, check_hybrid_raw_cap test is out of range, test result is 6F\n",
				__func__);
			TS_LOG_ERR(
	...
	TS_LOG_INFO(
		"%s, hybrid_abs_delta_report_size = %d. test result is 6P \n",
...
static void check_hybrid_abs_delta(void)
{
	...
	if (0 != result) {
		TS_LOG_ERR(
			"%s, ts_kit_parse_csvfile  parse tx fail result=%dis 9F\n",
			__func__, result);
		goto test_fail;
	}
	...
	for (i = 0; i < tx_num; i++) {
		if (false == is_in_range(f54->hybridbuf[index + i + rx_num],
					 hybird_abs_tx_range[i + tx_num],
					 hybird_abs_tx_range[i])) {
			TS_LOG_ERR(
				"%s, hybrid_abs_delta test is out of range, test result is 9F\n",
				__func__);
			TS_LOG_ERR(
				"current(%d) :%d |uplimit :%d |downlimit:%d\n",
				i, f54->hybridbuf[index + i + rx_num],
			...
			strncpy(tp_test_failed_reason, "-panel_reason",
				TP_TEST_FAILED_REASON_LEN);
			test_failed_reason[RAW_DATA_TYPE_SELFNOISETEST] =
				TEST_THRESHOLD_FAIL;
			goto test_fail;
		}
	}
	...
	if (0 != result) {
		TS_LOG_ERR(
			"%s, ts_kit_parse_csvfile  parse rx fail result=%dis 9F\n",
			__func__, result);
		goto test_fail;
	}
	...
	TS_LOG_INFO(
		"%s, hybrid_abs_delta_report_size = %d. test result is 9P \n",
		__func__, f54->report_size);
...
static void check_forcekey_cap_value(void)
{
	...
	result = ts_kit_parse_csvfile(file_path,
				      CSV_TP_FORCEKEY_CAP_VALUE_RANGE,
				      forcekey_cap_value_range,
				      CAP_TEST_FORCEKEY_VALUE_NUM,
				      CAP_TEST_FORCEKEY_VALUE_NUM);
	if (0 != result) {
		TS_LOG_ERR(
			"%s, ts_kit_parse_csvfile  parse tx fail result=%dis 10F\n",
			__func__, result);
		goto test_fail;
	}
	...
	for (i = 0; i < CAP_TEST_FORCEKEY_VALUE_NUM; i++) {
		if (false ==
		    is_in_range(f54->forcekey_value[i],
				forcekey_cap_value_range
					[i + CAP_TEST_FORCEKEY_VALUE_NUM],
				forcekey_cap_value_range[i])) {
			TS_LOG_ERR(
				"%s, hybrid_abs_delta test is out of range, test result is 10F\n",
				__func__);
			TS_LOG_ERR(
				"current(%d) :%d |uplimit :%d |downlimit:%d\n",
			...
			strncpy(tp_test_failed_reason, "-panel_reason",
				TP_TEST_FAILED_REASON_LEN);
			test_failed_reason[RAW_DATA_TYPE_FORCEKEY] =
				TEST_THRESHOLD_FAIL;
			goto test_fail;
		}
	}
...
static int td43xx_ee_short_normalize_data(signed short *image)
{
	...
	int part_two_limit = f54->rmi4_data->synaptics_chip_data
				     ->tddi_ee_short_test_parttwo_limit;
	...
	right_median =
		(unsigned short *)kzalloc(rx_num * sizeof(short), GFP_KERNEL);
	if (!right_median) {
		retval = -ENOMEM;
	...
	right_column_buf = (unsigned short *)kzalloc(
	...
	left_column_buf = (unsigned short *)kzalloc(
	...
	for (i = 0; i < rx_num; i++) {
		right_median[i] = FindMedian(right_column_buf + i * right_size,
					     right_size);
		left_median[i] =
			FindMedian(left_column_buf + i * left_size, left_size);
	...
	if (NULL != g_td43xx_rt95_part_two) {
		if (!tx_num || !rx_num) {
			...
			TS_LOG_ERR("%s(%d),tx_num=%d rx_num=%d\n", __func__,
				   __LINE__, tx_num, rx_num);
			goto exit;
		} else {
			tx_rx_delta_size =
				(TX_RX_BUF_MAX <=
				 (tx_num * rx_num * MAX_CAP_DATA_SIZE)) ?
					TX_RX_BUF_MAX :
					(tx_num * rx_num * MAX_CAP_DATA_SIZE);
		}
		snprintf((char *)g_td43xx_rt95_part_two, PART_ONE_LIMIT_LENGTH,
			 "%s\n", "part_two:");
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (i < right_size) {
				...
				temp = temp * NORMAL_NUM_ONE_HUNDRED /
				       right_median[j]; /*100 for avoid decimal number*/
				snprintf(buf, sizeof(buf), "%3d,", temp);
				lens += sizeof(buf);
				if (lens > tx_rx_delta_size) {
					retval = -ENOMEM;
					TS_LOG_ERR(
						"%s(%d), lens is %d larger than buffer\n",
						__func__, __LINE__, lens);
					goto exit;
				}
				length = MIN(
					mmi_buf_size * sizeof(int) -
						(strlen((char *)g_td43xx_rt95_part_two) +
						 1),
					sizeof(buf));
				strncat((char *)g_td43xx_rt95_part_two, buf,
					length);
				if (!((j + EESHORT_PRINT_SHIFT_LENGTH) %
				      rx_num)) {
				...
				temp = (unsigned int)image[i * rx_num + j];
				temp = temp * NORMAL_NUM_ONE_HUNDRED /
				       left_median[j]; /*100 for avoid decimal number*/
				snprintf(buf, sizeof(buf), "%3d,", temp);
				lens += sizeof(buf);
				if (lens > tx_rx_delta_size) {
					retval = -ENOMEM;
...
static int td43xx_ee_short_report(void)
{
	...
	int retval = 0;
	int part_one_limit = f54->rmi4_data->synaptics_chip_data
				     ->tddi_ee_short_test_partone_limit;
	int part_two_limit = f54->rmi4_data->synaptics_chip_data
				     ->tddi_ee_short_test_parttwo_limit;
	...
	if (f54->rmi4_data->synaptics_chip_data->test_capacitance_via_csvfile) {
		...
		limit_tab.unique_test->ee_short_data_limit = (int32_t *)kzalloc(
			EE_SHORT_DATA_LIMIT_SIZE * sizeof(int32_t), GFP_KERNEL);
		if (!limit_tab.unique_test->ee_short_data_limit) {
			TS_LOG_ERR(
				"ee_short_data_limit buffer kzalloc error\n");
			TestResult = TEST_FAIL;
			goto exit;
		}
		if (TEST_FAIL ==
		    synaptics_get_threshold_from_csvfile(
			    CSVFILE_EE_SHORT_COLUMN, CSVFILE_EE_SHORT_ROW,
			    CSV_TP_EE_SHORT_DATA_LIMIT,
			    limit_tab.unique_test->ee_short_data_limit)) {
			TS_LOG_ERR(
				"csvfile get ee_short_data_limit err, use dts config.\n");
		} else {
			part_one_limit =
				limit_tab.unique_test
					->ee_short_data_limit[LIMIT_ONE];
			part_two_limit =
				limit_tab.unique_test
					->ee_short_data_limit[LIMIT_TWO];
		}
	}

	TS_LOG_INFO("%s: report_type=%d,tx=%d,rx=%d,limit(%d, %d) #%d\n",
		    __func__, f54->report_type, tx_num, rx_num, part_one_limit,
		    part_two_limit, __LINE__);
	...
	if (NULL != g_td43xx_rt95_part_one) {
		...
		snprintf((char *)g_td43xx_rt95_part_one, PART_ONE_LIMIT_LENGTH,
			 "%s\n", "part_one:");
		lens += PART_ONE_LIMIT_LENGTH;
	} else {
		TestResult = TEST_FAIL;
	...
	for (i = 0, k = 0; i < tx_num * rx_num; i++) {
		td43xx_rt95_part_one[i] = buffer[k] | (buffer[k + 1])
							      << SHIFT_ONE_BYTE;
		...
		length = MIN(
			mmi_buf_size * sizeof(int) -
				(strlen((char *)g_td43xx_rt95_part_one) + 1),
			sizeof(buf));
		...
		td43xx_rt95_part_one[i] =
			(td43xx_rt95_part_one[i] > part_one_limit) ?
				EE_SHORT_TEST_FAIL :
				EE_SHORT_TEST_PASS;
		k += NORMAL_NUM_TWO;
	}
	...
	for (i = 0, k = td43xx_rt95_report_size; i < tx_num * rx_num; i++) {
		td43xx_rt95_part_two[i] = buffer[k] | (buffer[k + 1])
							      << SHIFT_ONE_BYTE;
		k += NORMAL_NUM_TWO;
	}
	...
	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			td43xx_ee_short_data[i * rx_num + j] =
				(unsigned char)(td43xx_rt95_part_one[i * rx_num +
								     j]) ||
				td43xx_rt95_part_two[i * rx_num + j];
			if (0 != td43xx_ee_short_data[i * rx_num + j]) {
				TestResult = TEST_FAIL;
				test_failed_reason[RAW_DATA_TYPE_OPENSHORT] =
					TEST_THRESHOLD_FAIL;
				TS_LOG_ERR(
					"td43xx_ee_short_data:[%d, %d]%d\n", i,
					j,
...
static void mmi_trex_shorts_test(void)
{
	...
	for (i = 0; i < TP_3320_SHORT_ARRAY_NUM; i++) {
		TS_LOG_INFO("%s: buf_backup[%d] is %d\n", __func__, i,
			    buf_backup[i]);
		if (buf_backup[i] !=
		    f54->rmi4_data->trx_short_circuit_array[i]) {
...
static void mmi_td43xx_ee_short_report(void)
{
	...
	if (result < 0) {
		TS_LOG_ERR("Failed to get data\n");
		strncat(buf_f54test_result, TD43XX_EE_SHORT_TEST_FAIL,
			MAX_STR_LEN);
		return;
	}
	...
	if (TEST_PASS == result) {
		...
		strncat(buf_f54test_result, TD43XX_EE_SHORT_TEST_PASS,
			MAX_STR_LEN);
	} else {
		strncat(buf_f54test_result, TD43XX_EE_SHORT_TEST_FAIL,
			MAX_STR_LEN);
						...
						int32_t *data)
{
	...
	if (!data || !target_name) {
		TS_LOG_ERR(
			"parse csvfile failed, data or target_name is NULL\n");
		return TEST_FAIL;
	}

	if (!strnlen(f54->rmi4_data->synaptics_chip_data->ts_platform_data
			     ->product_name,
		     MAX_STR_LEN - 1) ||
	    !strnlen(f54->rmi4_data->synaptics_chip_data->chip_name,
		     MAX_STR_LEN - 1) ||
	    !strnlen(f54->rmi4_data->rmi4_mod_info.project_id_string,
	...
	result = ts_kit_parse_csvfile(file_path, target_name, data, rows,
				      columns);
	if (0 == result) {
		ret = TEST_PASS;
...
static int f54_delta_rx2_report(void)
{
	...
	limit_tab.unique_test =
		kzalloc(sizeof(struct ts_unique_capacitance_test), GFP_KERNEL);
	if (!limit_tab.unique_test) {
		TS_LOG_ERR("ts_unique_capacitance_test kzalloc error\n");
		ret = TEST_FAIL;
		goto error_release_mem;
	}
	limit_tab.unique_test->read_only_support_unique =
		f54->rmi4_data->synaptics_chip_data->trx_delta_test_support;
	limit_tab.unique_test->rx_delta_abslimit = (int32_t *)kzalloc(
		rows_size * columns_size * sizeof(int32_t), GFP_KERNEL);
	if (!limit_tab.unique_test->rx_delta_abslimit) {
		TS_LOG_ERR("rx_delta_abslimit buffer kzalloc error\n");
		ret = TEST_FAIL;
		goto error_release_mem;
	}
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(
				 rows_size, columns_size - 1,
				 CSV_TP_DELTA_ABS_RX_LIMIT,
				 limit_tab.unique_test->rx_delta_abslimit)) {
		TS_LOG_ERR(
			"get csv_Vertical_screen abs_rxdelt_cap_limit err\n");
		ret = TEST_FAIL;
		memset(limit_tab.unique_test->rx_delta_abslimit, 0,
		       rows_size * columns_size * sizeof(int32_t));
	}
	ts_kit_rotate_rawdata_abcd2cbad(
		rows_size, columns_size - 1,
		limit_tab.unique_test->rx_delta_abslimit,
		RAWDATA_CSV_VERTICAL_SCREEN);
	abs_rxdelt_cap_limit = limit_tab.unique_test->rx_delta_abslimit;
	if (f54->rmi4_data->synaptics_chip_data->rawdata_newformatflag ==
	    TS_RAWDATA_NEWFORMAT) {
		for (i = 0; i < mmi_buf_size; i += 2) {
			Rawimage_rx1 = (f54->mmi_buf[i]) |
				       (f54->mmi_buf[i + 1] << SHIFT_ONE_BYTE);
			Rawimage_rx2 = (f54->mmi_buf[i + 2]) |
				       (f54->mmi_buf[i + 3] << SHIFT_ONE_BYTE);
			Rawimage_rx = abs(Rawimage_rx2 - Rawimage_rx1);
			rx_delta_buf[delta_buf_count] = Rawimage_rx;
			delta_buf_count++;
			rxdelt_cap_abslimit =
				abs_rxdelt_cap_limit[i / 2 - step];
			if (Rawimage_rx <= rxdelt_cap_abslimit) {
				Result++;
			} else {
				TS_LOG_ERR(
					"[NO:%d |current is %d] abslimit rx is  %d\n",
					i / 2 - step, Rawimage_rx,
					rxdelt_cap_abslimit);
			}
			j++;
		...
		for (i = 0; i < mmi_buf_size; i += 2) {
			...
			length = MIN(mmi_buf_size * sizeof(int) -
					     (strlen((char *)rx_delta_buf) + 1),
				     sizeof(buf));
	...
	if (Result == (mmi_buf_size / 2 - rows_size)) {
		...
		TS_LOG_ERR("rawdata rx diff is out of range, Result = %d\n",
			   Result);
		ret = TEST_FAIL;
		test_failed_reason[RAW_DATA_TYPE_TRXDELTA] =
			TEST_THRESHOLD_FAIL;
	}
error_release_mem:
...
static int f54_delta_tx2_report(void)
{
	...
	if (!g_ts_kit_platform_data.chip_data->tx_num ||
	...
	limit_tab.unique_test->read_only_support_unique =
		f54->rmi4_data->synaptics_chip_data->trx_delta_test_support;
	limit_tab.unique_test->tx_delta_abslimit = (int32_t *)kzalloc(
		rows_size * columns_size * sizeof(int32_t), GFP_KERNEL);
	...
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(
				 rows_size - 1, columns_size,
				 CSV_TP_DELTA_ABS_TX_LIMIT,
				 limit_tab.unique_test->tx_delta_abslimit)) {
		TS_LOG_ERR("get abs_txdelt_cap_limit err\n");
		ret = TEST_FAIL;
		memset(limit_tab.unique_test->tx_delta_abslimit, 0,
		       rows_size * columns_size * sizeof(int32_t));
	}
	ts_kit_rotate_rawdata_abcd2cbad(
		rows_size - 1, columns_size,
	...
	if (f54->rmi4_data->synaptics_chip_data->rawdata_newformatflag ==
		...
		for (tx_n = 0; tx_n < rows_size - 1; tx_n++) {
			for (rx_n = 0; rx_n < columns_size; rx_n++) {
				txdelt_cap_abslimit =
					abs_txdelt_cap_limit[tx_n * columns_size +
							     rx_n];
				Rawimage_delta_tx = abs(
					Rawimage_tx[(tx_n + 1) * columns_size +
						    rx_n] -
					Rawimage_tx[tx_n * columns_size + rx_n]);
				tx_delta_buf[delta_buf_count] =
					Rawimage_delta_tx;
				delta_buf_count++;
				...
				if (Rawimage_delta_tx <= txdelt_cap_abslimit) {
					...
					TS_LOG_ERR(
						"[No :%d,current is %d]abslimit_tx is %d \n",
						tx_n * columns_size + rx_n,
						Rawimage_delta_tx,
						txdelt_cap_abslimit);
				}
			}
		...
		for (tx_n = 0; tx_n < rows_size - 1; tx_n++) {
			for (rx_n = 0; rx_n < columns_size; rx_n++) {
				...
				if (Rawimage_delta_tx <= txdelt_cap_abslimit) {
					...
					TS_LOG_ERR(
						"[NO :%d, current is %d]abslimit tx is %d\n",
						tx_n * columns_size + rx_n,
						Rawimage_delta_tx,
						txdelt_cap_abslimit);
				}
			...
			strncat((char *)tx_delta_buf, "\n",
				MIN((tx_rx_delta_size -
				     strlen((const char *)tx_delta_buf)),
				    2));
		}
	}
...
static int check_enhance_raw_capacitance(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		if ((f54->rawdatabuf[i + 2] > f54->rmi4_data->upper[i]) ||
		    (f54->rawdatabuf[i + 2] < f54->rmi4_data->lower[i])) {
			TS_LOG_ERR(
				"rawdata is out of range, failed at %d: upper: %d, lower: %d, raw: %d\n",
				i, f54->rmi4_data->upper[i],
				f54->rmi4_data->lower[i],
				f54->rawdatabuf[i + 2]);
			return 0;
		}
	}
...
static int check_csvfile_raw_capacitance(void)
{
	...
	limit_tab.mutual_raw_max = (int32_t *)kzalloc(
		(rows_size) * (columns_size) * sizeof(int32_t), GFP_KERNEL);
	limit_tab.mutual_raw_min = (int32_t *)kzalloc(
		(rows_size) * (columns_size) * sizeof(int32_t), GFP_KERNEL);
	...
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(
				 columns_size, rows_size, CSV_TP_CAP_RAW_MAX,
	...
	if (TEST_FAIL == synaptics_get_threshold_from_csvfile(
				 columns_size, rows_size, CSV_TP_CAP_RAW_MIN,
				 limit_tab.mutual_raw_min)) {
		TS_LOG_ERR("get rawdata_cap_min_limit err\n");
		ret = TEST_FAIL;
		goto error_release_mem;
	}
	ts_kit_rotate_rawdata_abcd2cbad(columns_size, rows_size,
					limit_tab.mutual_raw_max,
					RAWDATA_CSV_VERTICAL_SCREEN);
	ts_kit_rotate_rawdata_abcd2cbad(columns_size, rows_size,
					limit_tab.mutual_raw_min,
	...
	for (i = 0; i < count; i++) {
		/* rawdatabuf[0] rawdatabuf[1] are num_of_rx and num_of_tx */
		if ((f54->rawdatabuf[i + 2] > limit_tab.mutual_raw_max[i]) ||
		    (f54->rawdatabuf[i + 2] < limit_tab.mutual_raw_min[i])) {
			TS_LOG_ERR(
				"rawdata is out of range, failed at %d: upper: %d, lower: %d, raw: %d\n",
				i, limit_tab.mutual_raw_max[i],
				limit_tab.mutual_raw_min[i],
				f54->rawdatabuf[i + 2]);
			ret = TEST_FAIL;
			test_failed_reason[RAW_DATA_TYPE_CAPRAWDATA] =
				TEST_THRESHOLD_FAIL;
		}
	}
...
static void mmi_csvfile_rawdata_test(void)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->trx_delta_test_support) {
		if (TEST_PASS ==
		    (f54_delta_rx2_report() & f54_delta_tx2_report())) {
			TS_LOG_INFO(
				"trx_delta test is success, test result is 2P\n");
			strncat(buf_f54test_result, TRX_DELTA_CAP_TEST_PASS,
				MAX_STR_LEN);
		} else {
			TS_LOG_ERR(
				"trx_delta test is out of range, test result is 2F\n");
			strncat(buf_f54test_result, TRX_DELTA_CAP_TEST_FAIL,
				MAX_STR_LEN);
			strncpy(tp_test_failed_reason, "-panel_reason",
				TP_TEST_FAILED_REASON_LEN);
		}
...
static void mmi_enhance_raw_capacitance_test(void)
{
	...
	if (1 == result) {
		...
		TS_LOG_ERR(
			"raw data is out of range, , test result is 1F-2P\n");
...
{
	int retval = 0, result = 0, i = 0, j = 0, k = 0, ii = 0, jj = 0,
	    rxIndex = 0;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	...
	for (i = 0; i < f54->report_size; i++) {
		...
		if (p_data_rt26[i] != 0) {
			if ((i == 0) && (p_data_rt26[i] & 0x01)) {
				...
				TS_LOG_ERR(
					"%s: RT26 test fail!!!  data byte %d - data 0x%x\n",
	...
	//set no scan bit
	retval = f54->fn_ptr->read(rmi4_data, f54->control_base_addr, &data,
				   sizeof(data));
	if (retval < NO_ERR) {
	...
	retval = f54->fn_ptr->write(rmi4_data, f54->control_base_addr, &data,
				    sizeof(data));
	if (retval < NO_ERR) {
		TS_LOG_ERR("%s: Failed to set no scan bit\n", __func__);
		goto exit_26100_type;
	...
	retval = f54->fn_ptr->write(rmi4_data, f54->control.reg_96->address,
				    &temp_data[0], f54->rmi4_data->num_of_rx);
	if (retval < NO_ERR) {
		TS_LOG_ERR(
			"%s: fail to set all F54 control_96 registers to 0\n",
			__func__);
		goto exit_26100_type;
	}
	//do force update
	...
	p_data_baseline1 =
		kzalloc((size_t)2 * numberOfRows * numberOfColums, GFP_KERNEL);
	if (!p_data_baseline1) {
	...
	for (i = 0, k = 0; i < numberOfRows; i++) {
		for (j = 0; j < numberOfColums; j++) {
			temp = f54->report_data[k] |
			       (f54->report_data[k + 1] << 8);
	...
	for (i = 0; i < 4; i++) {
		...
		if (retval < NO_ERR) {
			TS_LOG_ERR(
				"%s fail to set all F54 control_96 register after changing the cbc\n",
				__func__);
			goto exit_26100_type;
		}
		...
		if (retval < NO_ERR) {
			TS_LOG_ERR(
				"%s: fail to read f54_read_report_image 100\n",
				__func__);
			goto exit_26100_type;
		}
		msleep(10);
		...
		for (ii = 0, k = 0; ii < numberOfRows; ii++) {
			for (j = 0; j < numberOfColums; j++) {
				temp = f54->report_data[k] |
				       (f54->report_data[k + 1] << 8);
				p_data_baseline2[ii * numberOfColums + j] =
					temp;
				k = k + 2;
			}
		}
		...
		for (ii = 0; ii < f54->rmi4_data->num_of_tx; ii++) {
			for (jj = 0; jj < f54->rmi4_data->num_of_rx; jj++) {
				p_data_delta[ii * f54->rmi4_data->num_of_rx +
					     jj] =
				...
				if (maxRX[jj] <
				    p_data_delta[ii * f54->rmi4_data->num_of_rx +
						 jj])
					maxRX[jj] = p_data_delta
						[ii * f54->rmi4_data->num_of_rx +
						 jj];

				if (minRX[jj] >
				    p_data_delta[ii * f54->rmi4_data->num_of_rx +
						 jj])
					minRX[jj] = p_data_delta
						[ii * f54->rmi4_data->num_of_rx +
						 jj];
			}
		}
		...
		for (rxIndex = 0; rxIndex < f54->rmi4_data->num_of_rx;
		     rxIndex++) {
			if (rxIndex == logical_pin) {
				if (minRX[rxIndex] < limit_1) {
					TS_LOG_ERR(
						"%s test failed: minRX[%-2d] = %4d when test pin %d (RX Logical pin [%d])\n",
						__func__, rxIndex,
						minRX[rxIndex],
				...
				if (maxRX[rxIndex] >= limit_2) {
					TS_LOG_ERR(
						"%s test failed: maxRX[%-2d] = %4d when test pin %d (RX Logical pin [%d])\n",
						__func__, rxIndex,
						maxRX[rxIndex],
						ExtendRT26_pin[i], logical_pin);
					result = EXT_TRX_SHORT_FAIL;
...
static void synaptics_change_report_rate(void)
{
	...
	if (0 == f54->rmi4_data->synaptics_chip_data->report_rate_test) {
		...
		TS_LOG_INFO(
			"s3207 and s3706 change_report_rate default pass\n");
		return;
	}
	...
	if ((F54_DATA120_RATE == report_rate120) &&
		...
		strncpy(tp_test_failed_reason, "-panel_reason",
			TP_TEST_FAILED_REASON_LEN);
		test_failed_reason[RAW_DATA_TYPE_FRESHIFT] =
			TEST_THRESHOLD_FAIL;
	}
	return;
}
...
static int synaptics_alloc_test_container(void)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->trx_delta_test_support) {
		tx_delta_buf =
			(int *)kzalloc(mmi_buf_size * sizeof(int), GFP_KERNEL);
		if (!tx_delta_buf) {
	...
	if (f54->rmi4_data->synaptics_chip_data->td43xx_ee_short_test_support) {
		g_td43xx_rt95_part_one = (signed int *)kzalloc(
			mmi_buf_size * sizeof(int), GFP_KERNEL);
		if (!g_td43xx_rt95_part_one) {
			TS_LOG_ERR("malloc g_td43xx_rt95_part_one failed\n");
			goto out;
		}
		g_td43xx_rt95_part_two = (signed int *)kzalloc(
			mmi_buf_size * sizeof(int), GFP_KERNEL);
		if (!g_td43xx_rt95_part_two) {
...
static int synaptics_alloc_newnode(struct ts_rawdata_newnodeinfo **pts_node,
				   int rawbufsize)
{
	int rc = NO_ERR;
	*pts_node = (struct ts_rawdata_newnodeinfo *)kzalloc(
		sizeof(struct ts_rawdata_newnodeinfo), GFP_KERNEL);
	if (!(*pts_node)) {
...
static void synap_put_cap_test_deviceinfo(struct ts_rawdata_info_new *info)
{
	...
	size_t pjt_sizemax =
		sizeof(f54->rmi4_data->rmi4_mod_info.project_id_string);
	size_t ver_sizemax =
		sizeof(f54->rmi4_data->synaptics_chip_data->version_name);
	size_t product_sizemax =
		sizeof(f54->rmi4_data->rmi4_mod_info.product_id_string);
...
int synap_put_cap_test_dataNewformat(struct ts_rawdata_info_new *info)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->rawdata_arrange_type ==
		    TS_RAWDATA_TRANS_ABCD2CBAD ||
	    f54->rmi4_data->synaptics_chip_data->rawdata_arrange_type ==
		    TS_RAWDATA_TRANS_ABCD2ADCB) {
		ts_kit_rotate_rawdata_abcd2cbad(
			info->rx, info->tx, f54->rawdatabuf + 2,
			f54->rmi4_data->synaptics_chip_data
				->rawdata_arrange_type);
		ts_kit_rotate_rawdata_abcd2cbad(
			info->rx, info->tx, f54->rawdatabuf + rawbufsize + 2,
			f54->rmi4_data->synaptics_chip_data
				->rawdata_arrange_type);
	}
	/*deviceinfo*/
	...
	if (f54->rmi4_data->synaptics_chip_data->test_capacitance_via_csvfile ||
		...
		snprintf(statistics_tmp_data, STATISTICS_DATA_LEN - 1,
			 "[%4d,%4d,%4d]", f54->raw_statics_data.RawimageAverage,
			 f54->raw_statics_data.RawimageMaxNum,
			 f54->raw_statics_data.RawimageMinNum);

		for (i = 0; i < rawbufsize; i++)
			pts_node->values[i] = f54->rawdatabuf[i + 2];
		strncpy(pts_node->statistics_data, statistics_tmp_data,
		...
		if (0 == strlen(buf_f54test_result) ||
		    strstr(buf_f54test_result, "F")) {
			strncpy(pts_node->tptestfailedreason, "software reason",
				TS_RAWDATA_FAILED_REASON_LEN);
			for (i = 0; i < RAW_DATA_END; i++) {
				if (TEST_THRESHOLD_FAIL ==
				    test_failed_reason[i]) {
					strncpy(pts_node->tptestfailedreason,
						"panel_reason",
						TS_RAWDATA_FAILED_REASON_LEN);
				}
			}
		...
		if (f54->rmi4_data->synaptics_chip_data->trx_delta_test_support) {
			...
			for (i = 0; i < rawbufsize; i++)
				pts_node->values[i + rawbufsize] =
					tx_delta_buf[i];

			ts_kit_rotate_rawdata_abcd2cbad(
				info->rx - 1, info->tx, pts_node->values,
				RAWDATA_CSV_VERTICAL_SCREEN);
			ts_kit_rotate_rawdata_abcd2cbad(
				info->rx, info->tx - 1,
				pts_node->values + rawbufsize,
				RAWDATA_CSV_VERTICAL_SCREEN);
			pts_node->size = mmi_buf_size;
		}
		pts_node->testresult = buf_f54test_result[test_num];
		pts_node->typeindex = RAW_DATA_TYPE_TRXDELTA;
		strncpy(pts_node->test_name, "Trx delta",
			sizeof(pts_node->test_name) - 1);
		list_add_tail(&pts_node->node, &info->rawdata_head);
		test_num = test_num + test_tmp_num;
	}
	...
	snprintf((statistics_tmp_data), STATISTICS_DATA_LEN - 1,
		 "[%4d,%4d,%4d]", f54->delta_statics_data.RawimageAverage,
		 f54->delta_statics_data.RawimageMaxNum,
		 f54->delta_statics_data.RawimageMinNum);
	for (i = 0; i < rawbufsize; i++)
	...
	strncpy(pts_node->statistics_data, statistics_tmp_data,
		sizeof(pts_node->statistics_data) - 1);
	memset(statistics_tmp_data, 0, sizeof(statistics_tmp_data));
	strncpy(pts_node->test_name, "noise delta",
		sizeof(pts_node->test_name) - 1);
	list_add_tail(&pts_node->node, &info->rawdata_head);
	test_num = test_num + test_tmp_num;
	...
	if (f54->rmi4_data->synaptics_chip_data->report_rate_test) {
		...
		strncpy(pts_node->test_name, "Frequency shift",
			sizeof(pts_node->test_name) - 1);
		list_add_tail(&pts_node->node, &info->rawdata_head);
		test_num = test_num + test_tmp_num;
	}
	...
	if (f54->rmi4_data->synaptics_chip_data->td43xx_ee_short_test_support ||
		...
		strncpy(pts_node->test_name, "Open short",
			sizeof(pts_node->test_name) - 1);
		list_add_tail(&pts_node->node, &info->rawdata_head);
		test_num = test_num + test_tmp_num;
	}
	...
	if (f54->rmi4_data->synaptics_chip_data->self_cap_test) {
		...
		for (i = 0; i < rawSelfbuf; i++)
			pts_node->values[i] =
				f54->hybridbuf[i + mmi_hybrid_abs_delta / 4];
		strncpy(pts_node->test_name, "Self Cap",
			sizeof(pts_node->test_name) - 1);
		list_add_tail(&pts_node->node, &info->rawdata_head);
		test_num = test_num + test_tmp_num;
		...
		strncpy(pts_node->test_name, "Self Noise",
			sizeof(pts_node->test_name) - 1);
		list_add_tail(&pts_node->node, &info->rawdata_head);
		test_num = test_num + test_tmp_num;
		/*10p10f*/
		if (f54->rmi4_data->synaptics_chip_data
			    ->support_forcekey_cap_value_test) {
			rc = synaptics_alloc_newnode(
				&pts_node, CAP_TEST_FORCEKEY_VALUE_NUM);
			if (rc < 0) {
				TS_LOG_ERR("malloc failed 6pts_node\n");
				return rc;
			}
			...
			strncpy(pts_node->test_name, "forcekey",
				sizeof(pts_node->test_name) - 1);
			list_add_tail(&pts_node->node, &info->rawdata_head);
			test_num = test_num + test_tmp_num;
		}
...
int synap_get_cap_data(struct ts_rawdata_info *info)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->rawdata_newformatflag ==
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s synaptics_alloc_test_container  failed ! \n",
				__func__);
			goto exit;
		}
	...
	if (rc < 0) { /*i2c communication failed, mmi result is all failed */
		...
		} else if (f54->rmi4_data->synaptics_chip_data
				   ->test_capacitance_via_csvfile) {
			mmi_csvfile_rawdata_test();
		} else {
			TS_LOG_ERR(
				"[%s] devkit 3.0 not support dts raw threshold \n",
				__func__);
			rc = -EINVAL;
			goto exit;
	...
	if ((SYNAPTICS_TD4322 != f54->rmi4_data->synaptics_chip_data->ic_type &&
	     SYNAPTICS_TD4310 !=
		     f54->rmi4_data->synaptics_chip_data->ic_type) ||
	...
	if (f54->rmi4_data->synaptics_chip_data->rawdata_newformatflag ==
	    TS_RAWDATA_NEWFORMAT) {
		synap_put_cap_test_dataNewformat(
	...
	memcpy(&info->hybrid_buff[2], f54->hybridbuf,
	       (mmi_hybrid_abs_delta / 2) * sizeof(int));
	...
	if ((len_tmp + strlen(f54->rmi4_data->rmi4_mod_info.project_id_string) +
	     strlen(f54->rmi4_data->synaptics_chip_data->version_name) +
	     BAR_LEN) >= F54_BUF_LEN) {
		TS_LOG_ERR("F54_BUF_LEN limited\n");
		rc = -EINVAL;
		goto exit;
	}
	snprintf((buf_f54test_result + len_tmp),
		 sizeof(buf_f54test_result) - BAR_LEN - len_tmp, "-%s-%s",
		 f54->rmi4_data->rmi4_mod_info.project_id_string,
		 f54->rmi4_data->synaptics_chip_data->version_name);
...
static int synaptics_rmi4_f54_attention_cust(void)
{
	...
	retval = f54->fn_ptr->write(
		f54->rmi4_data, f54->data_base_addr + DATA_REPORT_INDEX_OFFSET,
	...
	for (i = 0; i < report_times_max; i++) {
		if (i == (report_times_max - 1)) {
			...
			*/
			report_size_temp = (report_size_temp != 0) ?
						   report_size_temp :
						   MAX_I2C_MSG_LENS;
		}
...
int synap_get_calib_info(struct ts_calibration_info_param *info)
{
	...
	rc = f54->fn_ptr->read(f54->rmi4_data,
			       ...
			       &calibration_state, sizeof(calibration_state));
	if (rc < 0) {
...
int synap_debug_data_test(struct ts_diff_data_info *info)
{
	...
	if (f54->rmi4_data->synaptics_chip_data->test_capacitance_via_csvfile) {
		memcpy(info->buff, f54->rawdatabuf, rawdata_size * sizeof(int));
		memcpy(info->result, buf_f54test_result,
		       strlen(buf_f54test_result));
...
static int synaptics_read_f34(void)
{
	...
	if (SYNAPTICS_S3718 != f54->rmi4_data->synaptics_chip_data->ic_type) {
		retval = f54->fn_ptr->read(
			f54->rmi4_data,
			f54->f34_fd.query_base_addr + BOOTLOADER_ID_OFFSET,
			f54->bootloader_id, sizeof(f54->bootloader_id));
		if (retval < 0) {
	...
	if (SYNAPTICS_S3207 != f54->rmi4_data->synaptics_chip_data->ic_type) {
		if (V5 == f54->bl_version) {
			...
			retval = f54->fn_ptr->read(
				f54->rmi4_data,
				f54->rmi4_data->rmi4_feature.f01_ctrl_base_addr +
					RX_NUMBER,
		...
		retval = f54->fn_ptr->read(
			f54->rmi4_data,
			f54->rmi4_data->rmi4_feature.f11_ctrl_base_addr +
				RX_NUMBER_S3207,
			&f54->rmi4_data->num_of_rx, 1);
		if (retval < 0) {
			TS_LOG_ERR(
	...
	if ((f54->rmi4_data->num_of_tx == 0) ||
	    (f54->rmi4_data->num_of_tx > RAWDATA_NUM_OF_TRX_MAX) ||
	    (f54->rmi4_data->num_of_rx == 0) ||
	    (f54->rmi4_data->num_of_rx > RAWDATA_NUM_OF_TRX_MAX)) {
...
static int Synaptics_test_set_controls(void)
{
	...
	if (f54->query.has_signal_clarity == 1) {
		control->reg_41 =
			kzalloc(sizeof(*(control->reg_41)), GFP_KERNEL);
		if (!control->reg_41)
			goto exit_no_mem;
	...
	if (f54->query.has_0d_acquisition_control == 1) {
		control->reg_57 =
			kzalloc(sizeof(*(control->reg_57)), GFP_KERNEL);
		if (!control->reg_57)
			goto exit_no_mem;
	...
	/* controls 60 61 62 */
	if ((f54->query.has_h_blank == 1) || (f54->query.has_v_blank == 1) ||
	...
	if (f54->query.has_ctrl88 == 1) {
		control->reg_88 =
			kzalloc(sizeof(*(control->reg_88)), GFP_KERNEL);
		if (!control->reg_88)
			goto exit_no_mem;
	...
	if (f54->query_21.has_ctrl96) {
		control->reg_96 =
			kzalloc(sizeof(*(control->reg_96)), GFP_KERNEL);
		if (!control->reg_96)
			goto exit_no_mem;
	...
	if (f54->query_27.has_ctrl110) {
		control->reg_110 =
			kzalloc(sizeof(*(control->reg_110)), GFP_KERNEL);
		if (!control->reg_110)
			goto exit_no_mem;
	...
	if (f54->query_38.has_ctrl149) {
		control->reg_149 =
			kzalloc(sizeof(*(control->reg_149)), GFP_KERNEL);
		if (!control->reg_149)
			goto exit_no_mem;
	...
	if (f54->query_49.has_ctrl188) {
		control->reg_188 =
			kzalloc(sizeof(*(control->reg_188)), GFP_KERNEL);
		if (!control->reg_188)
			goto exit_no_mem;
...
static int Synaptics_test_set_queries(void)
{
	...
	retval = f54->fn_ptr->read(rmi4_data, f54->query_base_addr,
	...
	/* query 16 */
	retval = f54->fn_ptr->read(rmi4_data, f54->query_base_addr + offset,
				   f54->query_16.data,
...
static int test_f55_set_queries(void)
{
	...
	retval = f54->fn_ptr->read(rmi4_data, f55->query_base_addr,
...
static int test_f55_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = f54->fn_ptr->read(
		rmi4_data, f55->control_base_addr + SENSOR_TX_MAPPING_OFFSET,
	...
	retval = f54->fn_ptr->read(
		rmi4_data, f55->control_base_addr + SENSOR_RX_MAPPING_OFFSET,
	...
	if (f55->extended_amp) {
		retval = f54->fn_ptr->read(
			rmi4_data, f55->control_base_addr + f55->afe_mux_offset,
			ctrl_43.data, sizeof(ctrl_43.data));
		if (retval < 0) {
		...
		f54->tx_assigned =
			ctrl_43.afe_l_mux_size + ctrl_43.afe_r_mux_size;
	}
	...
	if (f55->has_force) {
		...
		if (!f55->force_tx_assignment) {
			TS_LOG_ERR("%s no memory for force_tx_assignment",
				   __func__);
			retval = -ENOMEM;
			goto error_free_mem;
		}
		...
		if (!f55->force_rx_assignment) {
			TS_LOG_ERR("%s no memory for force_rx_assignment",
				   __func__);
			retval = -ENOMEM;
			goto error_free_mem;
		}
		...
		retval = f54->fn_ptr->read(
			...
			f55->force_tx_assignment, tx_electrodes);
		if (retval < 0) {
			TS_LOG_ERR(
		...
		retval = f54->fn_ptr->read(
			...
			f55->force_rx_assignment, rx_electrodes);
		if (retval < 0) {
			TS_LOG_ERR(
			...
			const char *module_name)
{
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (ii = PDT_START; ii > PDT_END; ii -= PDT_ENTRY_SIZE) {
			...
			retval = f54->fn_ptr->read(rmi4_data, ii,
			...
			if (rmi_fd.fn_number == SYNAPTICS_RMI4_F54) {
				hasF54 = true;
				f54->query_base_addr = rmi_fd.query_base_addr |
						       (page << 8);
				f54->control_base_addr = rmi_fd.ctrl_base_addr |
							 (page << 8);
				f54->data_base_addr = rmi_fd.data_base_addr |
						      (page << 8);
				f54->command_base_addr = rmi_fd.cmd_base_addr |
							 (page << 8);
				TS_LOG_DEBUG(
				...
				f55->query_base_addr = rmi_fd.query_base_addr |
						       (page << 8);
				f55->control_base_addr = rmi_fd.ctrl_base_addr |
							 (page << 8);
				f55->data_base_addr = rmi_fd.data_base_addr |
						      (page << 8);
				f55->command_base_addr = rmi_fd.cmd_base_addr |
							 (page << 8);
...
unsigned short
synap_f54_get_calibrate_addr(struct synaptics_rmi4_data *rmi4_data,
			     const char *module_name)
{
	int rc = NO_ERR;
	u8 value = 0;
