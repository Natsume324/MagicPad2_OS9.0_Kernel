...
static int synaptics_wakeup_gesture_enable_switch(
...
static int synaptics_get_calibration_data(struct ts_calibration_data_info *info,
					  struct ts_cmd_node *out_cmd);
static int
synaptics_get_calibration_info(struct ts_calibration_info_param *info,
			       struct ts_cmd_node *out_cmd);
...
static int
synaptics_chip_get_capacitance_test_type(struct ts_test_type_info *info);
...
static int synaptics_rmi4_device_status_check(
...
#define EXT_TREX_SHORT_TEST_WAIT_REPORT_DELAY_FLAG \
	"honor,ext_trex_short_test_wait_report_delay_flag"
...
static bool
synaptics_rmi4_crc_in_progress(struct synaptics_rmi4_data *rmi4_data,
			       struct synaptics_rmi4_f01_device_status *status);
static int synaptics_rmi4_set_page(struct synaptics_rmi4_data *rmi4_data,
				   unsigned int address);
static struct synaptics_rmi4_fn *
synaptics_rmi4_alloc_fh(struct synaptics_rmi4_fn_desc *rmi_fd, int page_number);
static int
synaptics_rmi4_query_device_info(struct synaptics_rmi4_data *rmi4_data);
...
static bool
synaptics_read_crc_value(struct synaptics_rmi4_f01_device_status *status,
			 unsigned short ic_status_reg)
{
	int ret = NO_ERR;

	ret = synaptics_rmi4_i2c_read(rmi4_data, ic_status_reg, status->data,
				      sizeof(status->data));
	if (ret < 0) {
...
static void synaptics_check_crc_status(unsigned short ic_status_reg)
{
	...
	if (synaptics_rmi4_device_status_check(status)) {
		...
		ts_dmd_report(DSM_TP_DEV_STATUS_ERROR_NO,
			      "[synaptics]device status for 20004 is:%d\n",
			      status.status_code);
#endif
	}
	return;
...
static int synaptics_chip_check_status(void)
{
	...
	if (rmi4_data->synaptics_chip_data->noise_state_reg) {
		retval = rmi4_data->i2c_read(
			rmi4_data,
			rmi4_data->synaptics_chip_data->noise_state_reg, &value,
			sizeof(unsigned char));
		if (retval < 0) {
			atomic_set(&rmi4_data->synaptics_chip_data
					    ->ts_platform_data->ts_esd_state,
				   TS_ESD_HAPPENDED);
		...
		case NOISE_VALUE_2:
			...
			if (rmi4_data->synaptics_chip_data->noise_record_num <
			    NOISE_RECORD_NUM_MAX)
	...
	if (rmi4_data->synaptics_chip_data->frequency_selection_reg) {
		...
		if (frequency_selection_reg != value) {
			...
			TS_LOG_INFO(
				"%s frequency_selection reg value change: %d\n",
	...
	if (rmi4_data->synaptics_chip_data->ic_status_reg) {
		/*check crc status*/
		synaptics_check_crc_status(
			rmi4_data->synaptics_chip_data->ic_status_reg);
	}
...
static void synaptics_doze_enable(unsigned char oper, unsigned char param)
{
	...
	if ((TS_SWITCH_TYPE_DOZE !=
	     (rmi4_data->synaptics_chip_data->touch_switch_flag &
	      TS_SWITCH_TYPE_DOZE))) {
	...
	case TS_SWITCH_DOZE_ENABLE:
		...
		value = 0;
		error = synaptics_rmi4_i2c_read(
			rmi4_data,
			rmi4_data->synaptics_chip_data->touch_switch_reg,
			&value, (unsigned short)sizeof(unsigned char));
		if (error < 0) {
		...
		if (value & (1 << 2)) {
			...
			if (param > 0) {
				...
				error = synaptics_rmi4_i2c_write(
					rmi4_data,
					rmi4_data->synaptics_chip_data
						->touch_switch_hold_off_reg,
					&param,
					(unsigned short)sizeof(unsigned char));
				if (error < 0) {
					TS_LOG_ERR(
						"write doze hold off time addr:%02x=%u error\n",
						rmi4_data->synaptics_chip_data
							->touch_switch_hold_off_reg,
						param);
					break;
			...
			if (error < 0) {
				TS_LOG_ERR(
					"write doze enable addr:%u=%u error\n",
					rmi4_data->synaptics_chip_data
						->touch_switch_reg,
					value);
				break;
...
static void synaptics_game_mode_enable(unsigned char oper)
{
	...
	case TS_SWITCH_GAME_ENABLE:
		...
		if (error < 0) {
			TS_LOG_ERR(
				"read form  addr(%02x) error\n",
				rmi4_data->synaptics_chip_data->touch_game_reg);
			break;
		} else {
			TS_LOG_INFO(
				"read game enable addr:%02x=%u\n",
				rmi4_data->synaptics_chip_data->touch_game_reg,
				value);
		}
		...
		if (error < 0) {
			TS_LOG_ERR(
				"write doze game addr:%u=%u error\n",
				rmi4_data->synaptics_chip_data->touch_game_reg,
				value);
			break;
		}
	...
	case TS_SWITCH_GAME_DISABLE:
		...
		if (error < 0) {
			TS_LOG_ERR(
				"write game enable addr:%u=%u error\n",
				rmi4_data->synaptics_chip_data->touch_game_reg,
				value);
			break;
		}
...
static int synaptics_check_scene_type(unsigned int stype)
{
	...
	for (i = 0; i < rmi4_data->synaptics_chip_data->scene_type_num; i++) {
		if (stype ==
		    rmi4_data->synaptics_chip_data->scene_support_array[i])
			error = NO_ERR;
	}
	return error;
}
...
static void synaptics_scene_switch(unsigned char scene, unsigned char oper)
{
	...
	if ((rmi4_data->synaptics_chip_data->touch_switch_flag &
	...
	case TS_SWITCH_SCENE_ENTER:
		...
		error = synaptics_rmi4_i2c_write(
			rmi4_data,
			rmi4_data->synaptics_chip_data->touch_scene_reg, &value,
			(unsigned short)sizeof(unsigned char));
		if (error < 0)
			TS_LOG_ERR(
				"write scene switch addr: %x = %u error\n",
...
static void synaptics_report_face_cover_state(int state)
{
	...
	struct ts_key_info *key_info = &cmd.cmd_param.pub_params.key_info;
...
static void synaptics_face_detect_enable(unsigned char oper)
{
	...
	case TS_SWITCH_FACE_DCT_ENABLE:
		TS_LOG_INFO("%s: enable face detect, reg:0x%x\n", __func__,
			    cd->face_dct_en_reg);
		atomic_set(&cd->ts_platform_data->face_dct_en, FACE_DCT_ENABLE);
		value = (unsigned char)FACE_DCT_ENABLE;
		error = synaptics_rmi4_i2c_write(rmi4_data, cd->face_dct_en_reg,
						 &value,
					   ...
					   unsigned char *param)
{
	...
	memcpy(in_data, rmi4_data->synaptics_chip_data->touch_switch_info,
	       MAX_STR_LEN - 1);
...
static void synaptic_chip_touch_itouch(unsigned char stype, unsigned char soper,
				       unsigned char param)
{
	switch (stype) {
...
{
	int retval = 0;
	unsigned short diff_data_addr =
		rmi4_data->synaptics_chip_data->diff_data_control_addr;
	unsigned short diff_data_len =
		rmi4_data->synaptics_chip_data->diff_data_len;
	unsigned short curren_len = 0;
	unsigned char diff_data_tmp[DIFF_DATA_MAX_LEN] = {0};
	unsigned char temp_finger = (current_finger_num > MAX_SUPPORT_FINGER ?
					     MAX_SUPPORT_FINGER :
					     current_finger_num);
	...
	memset(diff_data_tmp, 0xFF,
	       DIFF_DATA_MAX_LEN); //set invalid data to 0xFF  for deamon
	if (curren_len == 0) {
		memcpy(rmi4_data->synaptics_chip_data->diff_data, diff_data_tmp,
		       DIFF_DATA_MAX_LEN);
		return;
	}
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, diff_data_addr,
					 diff_data_tmp, curren_len);
	if (retval < 0) {
		TS_LOG_ERR(
			"[DIFF_DATA] F12 Failed to read diff data, retval= %d \n",
			retval);
	} else {
		memcpy(rmi4_data->synaptics_chip_data->diff_data, diff_data_tmp,
		       diff_data_len);
...
static void synaptics_face_dct_data_report(void)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, cd->face_dct_data_reg,
					 &face_cover_state,
...
void synaptics_work_after_input_kit(void)
{
	...
	if (f51_roi_switch) {
		...
		roi_data_addr = rmi4_data->synaptics_chip_data->ts_platform_data
					->feature_info.roi_info.roi_data_addr;
		...
		roi_data_staled = 0;
		complete_all(
			&roi_data_done); /* If anyone has been blocked by us, wake it up. */
	}
	return;
out:
...
static int synaptics_boot_detection(void)
{
	...
	detection_addr =
		(unsigned short)
			rmi4_data->synaptics_chip_data->boot_detection_addr;
	retval = synaptics_rmi4_i2c_read(rmi4_data, detection_addr, &value,
					 sizeof(value));
	if (retval < 0) {
	...
	if ((value + tmp_num_tx) >
		...
		TS_LOG_ERR(
			"Super threshold read addr value[%d] + tmp_num_tx[%d] > detectionflag = %d",
			value, tmp_num_tx,
			rmi4_data->synaptics_chip_data
				->boot_detection_threshold);
		ts_dmd_report(DSM_TP_BOOT_DETECTION,
			      "read boot detection addr =0x%02x\n", value);
	}
#endif
	return NO_ERR;
}
...
static void synaptics_chip_send_sensibility(int cur_value)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, sensi_addr,
					 (unsigned char *)&pre_value, 1);
	if (retval < 0) {
	...
	if (pre_value != cur_value) {
		...
		retval = synaptics_rmi4_i2c_write(
			rmi4_data, sensi_addr, (unsigned char *)&cur_value, 1);
		if (retval < 0) {
			TS_LOG_ERR("write sensitivity failed\n");
			return;
		}
#if defined(CONFIG_HONOR_DSM)
		ts_dmd_report(DSM_TP_SENSIBILITY_CHANGE,
			      "DSM_TP_SENSIBILITY_CHANGE(%d)->(%d)\n",
			      pre_value, cur_value);
#endif
	}
	return;
				 ...
				 struct ts_cmd_node *out_cmd)
{
	...
	if (rmi4_data->synaptics_chip_data->unite_cap_test_interface) {
		...
		retval = synap_rmi4_f54_init(
			rmi4_data, synaptics_sett_param_regs->module_name);
		if (retval < 0) {
					  ...
					  struct ts_cmd_node *out_cmd)
{
	...
	retval = synap_rmi4_f54_init(rmi4_data,
...
static int synaptics_reconstruct_barcode(struct ts_oem_info_param *info)
{
	...
	if ((type == 0x00 && len == 0x00) || (type == 0xFF && len == 0xFF)) {
		memcpy(&(info->buff[offset1 * 16]), tp_type_cmd,
		       tp_type_cmd[1] * 16);
	...
	if ((type == 0x00 && len == 0x00) || (type == 0xFF && len == 0xFF)) {
		memcpy(&(info->buff[offset2 * 16]), tp_type_cmd,
		       tp_type_cmd[1] * 16);
		TS_LOG_INFO("Will write the data to info_buff, offset is %d",
			    offset2);
		return retval;
...
static int
synaptics_reconstruct_brightness_whitepoint(struct ts_oem_info_param *info)
{
	int retval = NO_ERR;
...
static int synaptics_reconstruct_repair_recode(struct ts_oem_info_param *info)
{
	...
	for (; offset <= TS_NV_STRUCTURE_REPAIR_OFFSET5; ++offset) {
		...
		if ((type == 0x00 && len == 0x00) ||
		    (type == 0xFF && len == 0xFF)) {
			memcpy(&(info->buff[offset * 16]), tp_type_cmd,
			       tp_type_cmd[1] * 16);
			TS_LOG_INFO(
				"Will write the data to info_buff, offset is %d",
				offset);
			break;
...
static int synaptics_get_NVstructure_cur_index(struct ts_oem_info_param *info,
					       u8 type)
{
	int index = 0;
	int latest_index = 0;
	...
	for ((TS_NV_STRUCTURE_REPAIR == type) ?
		     (index = TS_NV_STRUCTURE_REPAIR_OFFSET1) :
		     (index = 1);
...
static int synaptics_get_NVstructure_index(struct ts_oem_info_param *info,
					   u8 type)
{
	int index = 0;
	int latest_index = 0;
...
static int synaptics_set_oem_info(struct ts_oem_info_param *info)
{
	...
	if (strlen(info->data) <= TS_CHIP_TYPE_MAX_SIZE) {
		...
		memcpy(tp_type_cmd, info->data,
		       MIN(sizeof(tp_type_cmd) - 1, len * 16));
	} else {
		error = EINVAL;
		tp_result_info[0] = TS_CHIP_WRITE_ERROR;
	...
	for (i = 0; i < flash_size / 16; ++i) {
		TS_LOG_INFO(
			...
			__func__, info->buff[0 + i * 16],
			info->buff[1 + i * 16], info->buff[2 + i * 16],
			info->buff[3 + i * 16], info->buff[4 + i * 16],
			info->buff[5 + i * 16], info->buff[6 + i * 16],
			info->buff[7 + i * 16], info->buff[8 + i * 16],
			info->buff[9 + i * 16], info->buff[10 + i * 16],
			info->buff[11 + i * 16], info->buff[12 + i * 16],
			info->buff[13 + i * 16], info->buff[14 + i * 16],
			info->buff[15 + i * 16]);
	}
	...
	if (rmi4_data->synaptics_chip_data->is_new_oem_structure) {
		...
		if (NO_ERR != synaptics_reconstruct_NVstructure(info)) {
			TS_LOG_ERR(
				"%s: synaptics_reconstruct_NVstructure fail\n",
				__func__);
			goto out;
		}
	} else {
		...
		for (index = 1; index < flash_size / 16;) {
			...
			if (store_type_count >= TS_CHIP_WRITE_MAX_TYPE_COUNT) {
				TS_LOG_ERR(
					"%s: Write TPIC Type:0x%2x are larger than %d times\n",
					__func__, type,
					TS_CHIP_WRITE_MAX_TYPE_COUNT);
			...
			if (used_size > flash_size) {
				TS_LOG_ERR(
					"%s: Used flash size is larger than TPIC Flash size\n",
					__func__);
				tp_result_info[0] = TS_CHIP_NO_FLASH_ERROR;
				goto out;
			}

			if ((tmp_type == 0x00 && tmp_len == 0x00) ||
			    (tmp_type == 0xFF && tmp_len == 0xFF)) {
				memcpy(&(info->buff[index * 16]), tp_type_cmd,
				       len * 16);
				break;
			}
			index += tmp_len;
	...
	if (rmi4_data->synaptics_chip_data->is_new_oem_structure) {
		...
		if (!latest_index) {
			TS_LOG_ERR(
				"%s: set oem data find current line fail line=%d\n",
				__func__, __LINE__);
			tp_result_info[0] = TS_CHIP_WRITE_ERROR;
			goto out;
		}
...
static int synaptics_get_oem_info(struct ts_oem_info_param *info)
{
	...
	if (0 == rmi4_data->synaptics_chip_data->support_2dbarcode_info) {
		...
		if (type == 0x0) {
			memcpy(info->data, tp_result_info,
			       TS_CHIP_TYPE_MAX_SIZE);
			TS_LOG_INFO(
				"%s:Reurn the write result=%2x to sys node.\n",
				__func__, info->data[0]);
			goto out;
		}
	...
	if (rmi4_data->synaptics_chip_data->support_gammadata_in_tp &&
	    rmi4_data->synaptics_chip_data->ts_platform_data->gammaflag) {
		rmi4_data->synaptics_chip_data->ts_platform_data->gammaflag = 0;
		if (!rmi4_data->synaptics_chip_data->ts_platform_data
			     ->gammaerrno) {
	...
	if (rmi4_data->synaptics_chip_data->support_2dbarcode_info) {
		...
		error = synap_get_oem_data(info->buff,
					   SYNAPTICS_RMI4_BARCODE_INFO_LEN);
	} else
		error = synap_get_oem_data(info->buff, flash_size);
	if (error < 0) {
	...
	if (rmi4_data->synaptics_chip_data->is_new_oem_structure) {
		...
		if (latest_index) {
			TS_LOG_INFO("%s type data find. len = %d\n", __func__,
				    info->length);
			infolength = min(TS_CHIP_TYPE_MAX_SIZE,
					 info->length * 16 + 1);
			memcpy(info->data, &(info->buff[latest_index * 16]),
			       infolength - 1);
		} else {
			info->data[0] = 0x1;
			TS_LOG_INFO(
				"%s No type data find. info->data[0] = %2x\n",
				__func__, info->data[0]);
		}
	} else if (rmi4_data->synaptics_chip_data->support_2dbarcode_info) {
		TS_LOG_INFO("[%s] ->2d bar code to info->data \n", __func__);
		memcpy(info->data, info->buff,
		       SYNAPTICS_RMI4_BARCODE_INFO_SIZE);
	} else {
		//scan each NV struct length
		for (index = 1; index < flash_size / 16;) {
			u8 tmp_type = info->buff[index * 16];
		...
		if (latest_index) {
			TS_LOG_INFO("%s type data find. len = %d\n", __func__,
				    info->buff[latest_index * 16 + 1] * 16);
			infolength =
				min(TS_CHIP_TYPE_MAX_SIZE,
				    info->buff[latest_index * 16 + 1] * 16 + 1);
			memcpy(info->data, &(info->buff[latest_index * 16]),
			       infolength - 1);
		} else {
			info->data[0] = 0x1;
...
static int synaptics_set_gamma_info(struct ts_oem_info_param *info)
{
	...
	TS_LOG_INFO(
		"%s wl indata-16 : %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x \n",
		__func__, info->data[0], info->data[1], info->data[2],
		info->data[3], info->data[4], info->data[5], info->data[6],
		info->data[7], info->data[8], info->data[9], info->data[10],
		info->data[11], info->data[12], info->data[13], info->data[14],
		info->data[15]);

	error = synaptics_get_gamma_data(info->buff,
					 SYNAPTICS_RMI4_BARCODE_GAMMA_INFO_LEN);
	if (error < 0) {
		TS_LOG_ERR("%s: get oem data failed,fail line=%d\n", __func__,
			   __LINE__);
		goto out;
	...
	TS_LOG_INFO("%s:Get buff data below :%2x\n", __func__,
		    info->buff[SYNAPTICS_RMI4_BARCODE_START_OFFSET +
			       SYNAPTICS_RMI4_BARCODE_OFFSET_4]);
	if ((0 == info->buff[SYNAPTICS_RMI4_BARCODE_START_OFFSET +
			     SYNAPTICS_RMI4_BARCODE_OFFSET_4]) &&
	    (0 == info->buff[SYNAPTICS_RMI4_BARCODE_OFFSET_4])) {
		memcpy(info->buff + SYNAPTICS_RMI4_BARCODE_OFFSET_4, info->data,
		       SYNAPTICS_RMI4_BARCODE_GAMMA_INFO_LEN);

		for (i = 0; i < 10; ++i) {
			TS_LOG_INFO(
				"%s gamadata(10*16): %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x %2x \n",
				__func__, info->buff[0 + i * 16],
				info->buff[1 + i * 16], info->buff[2 + i * 16],
				info->buff[3 + i * 16], info->buff[4 + i * 16],
				info->buff[5 + i * 16], info->buff[6 + i * 16],
	...
	error = synaptics_set_gamma_data(info->buff,
					 SYNAPTICS_RMI4_BARCODE_GAMMA_INFO_LEN);
	if (error < 0) {
...
static int synaptics_get_gamma_info(struct ts_oem_info_param *info)
{
	...
	if ((info->buff[SYNAPTICS_RMI4_BARCODE_OFFSET_4] !=
	     GAMMA_DATA_HEAD_0) &&
	    (info->buff[SYNAPTICS_RMI4_BARCODE_OFFSET_4] !=
	     GAMMA_DATA_HEAD_1)) {
	...
	memcpy(info->data, info->buff + SYNAPTICS_RMI4_BARCODE_OFFSET_4,
	       SYNAPTICS_RMI4_BARCODE_GAMMA_INFO_SIZE);
...
static int synaptics_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if (rmi4_data->synaptics_chip_data->projectid_len) {
		projectid_length =
			rmi4_data->synaptics_chip_data->projectid_len;
	} else {
	...
	TS_LOG_INFO(
		"rmi4_data->synaptics_chip_data->ts_platform_data->get_info_flag=%d\n",
		rmi4_data->synaptics_chip_data->ts_platform_data->get_info_flag);
	if (unlikely((atomic_read(&rmi4_data->synaptics_chip_data
					   ->ts_platform_data->state) ==
		      TS_SLEEP) ||
		     (atomic_read(&rmi4_data->synaptics_chip_data
					   ->ts_platform_data->state) ==
		      TS_WORK_IN_SLEEP))) {
		...
		if (!rmi4_data->synaptics_chip_data->ts_platform_data
			...
			if (SYNAPTICS_S3718 !=
			    ...
			    SYNAPTICS_S3706 !=
				    rmi4_data->synaptics_chip_data->ic_type) {
				retval = synap_fw_configid(rmi4_data, buf,
							   sizeof(buf));
				if (retval < 0)
				...
				if (retval < 0) {
					TS_LOG_ERR(
						"synap_fw_data_init error.\n");
				}
				synap_fw_data_release();
			} else {
				retval = synap_fw_s3718_configid(rmi4_data, buf,
								 sizeof(buf));
				if (retval < 0)
					TS_LOG_ERR("failed to get configid\n");
				...
				if (retval < 0) {
					TS_LOG_ERR(
						"synap_fw_data_s3718_init error.\n");
				}
...
int create_and_get_u16_array(u16 *array, struct device_node *dev_node,
			     const char *name)
{
...
static void synap_parse_chip_hybrid_specific_dts(struct device_node *device)
{
	...
	retval = of_property_read_u32(device, NOT_DELAY_ACT,
	...
	retval = of_property_read_u32(
		device, SUPPORT_SHORT_TEST,
		(u32 *)&rmi4_data->support_s3320_short_test);
	if (retval) {
	...
	TS_LOG_INFO("%s: support_s3320_short_test = %d\n", __func__,
		    rmi4_data->support_s3320_short_test);

	retval = create_and_get_u16_array(rmi4_data->trx_short_circuit_array,
					  device, TRX_SHORT_CIRCUIT);
	if (retval < 0) {
...
static void synap_parse_crc_err_reset(struct device_node *device)
{
	...
	retval = of_property_read_u32(device, CRC_ERR_DO_RESET,
				      &rmi4_data->support_crc_err_do_reset);
	if (retval < 0) {
...
void synap_parse_chip_specific_dts(struct ts_kit_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, SUPPORT_EXT_TREX_SHORT_TEST,
				      &rmi4_data->support_ext_trex_short_test);
	if (retval) {
	...
	retval = of_property_read_u32(device, "screenoff_status_reg",
				      &rmi4_data->screenoff_status_reg);
	if (retval) {
	...
	retval = of_property_read_u32(device, "screenoff_status_switch",
				      &rmi4_data->screenoff_status_switch);
	if (retval) {
	...
	retval = of_property_read_u32(device, "screenoff_status_support",
				      &rmi4_data->screenoff_status_support);
	if (retval) {
	...
	retval = of_property_read_u32(device, "sensitivity_adjust_support",
				      &rmi4_data->sensitivity_adjust_support);
	if (retval) {
	...
	retval = of_property_read_u32(device, "sensitivity_adjust_reg",
				      &rmi4_data->sensitivity_adjust_reg);
	if (retval) {
	...
	retval = of_property_read_u32(
		device, EXT_TREX_SHORT_TEST_WAIT_REPORT_DELAY_FLAG,
		&rmi4_data->ext_trex_short_wait_report_delay_flag);
	if (retval) {
		TS_LOG_ERR(
			"%s: ext_trex_short_wait_report_delay_flag has NOT been set\n",
			__func__);
		rmi4_data->ext_trex_short_wait_report_delay_flag = 0;
	}
	TS_LOG_INFO("%s: ext_trex_short_wait_report_delay_flag = %d\n",
		    __func__, rmi4_data->ext_trex_short_wait_report_delay_flag);

	retval = of_property_read_u32(
		device, TEST_ENHANCE_RAW_DATA_CAPACITANCE,
		&rmi4_data->test_enhance_raw_data_capacitance);
	if (retval) {
		TS_LOG_INFO(
			"get device test_enhance_raw_data_capacitance null, use default\n");
		...
		TS_LOG_INFO("synaptics_rawdata_count= %d\n",
			    synaptics_rawdata_count);
		rmi4_data->upper = (int *)kzalloc(
			synaptics_rawdata_count * sizeof(int), GFP_KERNEL);
		rmi4_data->lower = (int *)kzalloc(
			synaptics_rawdata_count * sizeof(int), GFP_KERNEL);
		if (!rmi4_data->upper || !rmi4_data->lower) {
			...
			retval = of_property_read_u32_array(
				device, "honor,enhance_rawdata_upperlimit",
				rmi4_data->upper, synaptics_rawdata_count);
			if (retval) {
				TS_LOG_ERR(
					"get device test_enhance_raw_data error\n");
				memset(rmi4_data->upper, 1,
				       synaptics_rawdata_count * sizeof(int));
				retval = 0;
			}
			retval = of_property_read_u32_array(
				device, "honor,enhance_rawdata_lowerlimit",
				rmi4_data->lower, synaptics_rawdata_count);
			if (retval) {
				TS_LOG_ERR(
					"get device test_enhance_raw_data error\n");
				memset(rmi4_data->lower, 0,
				       synaptics_rawdata_count * sizeof(int));
				retval = 0;
			}
		}
	...
	/* syna_wx_wy */
	retval = of_property_read_string(device, "adv_width",
	...
	TS_LOG_INFO("adv_width[0]=%c, adv_width[1]=%c, adv_width[2]=%c\n",
		    rmi4_data->adv_width[0], rmi4_data->adv_width[1],
		    rmi4_data->adv_width[2]);
...
out:
	TS_LOG_INFO(
		"need_disable_hover =%d,hover_switch_addr = 0x%d,hover_switch_bit =%d.\n",
		rmi4_data->need_disable_hover, rmi4_data->hover_switch_addr,
		rmi4_data->hover_switch_bit);
	return NO_ERR;
}
					  ...
					  struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval) {
		...
		TS_LOG_ERR(
			"Not define synaptics3718_fw_updata_flag in Dts, use fault value\n");
	} else {
	...
	retval = of_property_read_u32(device, SYNA_3718_TP_PRESSURE_FLAG,
				      &value);
	if (retval) {
		rmi4_data->synaptics3718_Tp_Pressure_flag = 0;
		TS_LOG_ERR(
			"Not define synaptics3718_Tp_Pressure_flag in Dts, use fault value\n");
	} else {
	...
	retval = of_property_read_u32(device, "is_multi_protocal",
	...
	if (retval) {
		...
		TS_LOG_INFO(
			"device use_ub_supported not exit,use default value.\n");
	}
	...
	if (retval) {
		...
		TS_LOG_INFO(
			"device delay_for_erase_fw not found,use default value.\n");
	}
	...
	if (retval) {
		...
		TS_LOG_INFO(
			"device distinguish_ic_type not found,use default value.\n");
	}
...
static int syna_tcm_i2c_read(unsigned char *data, unsigned int length)
{
	...
	struct i2c_client *i2c =
		rmi4_data->synaptics_chip_data->ts_platform_data->client;
	...
	for (attempt = 0; attempt < I2C_RW_TRIES; attempt++) {
		...
		TS_LOG_ERR("Transfer attempt %d failed\n", attempt + 1);
...
static int synaptics_chip_detect(struct ts_kit_platform_data *data)
{
	...
	if ((!data) || (!data->ts_dev)) {
		TS_LOG_ERR(
			"device, ts_kit_platform_data *data or data->ts_dev is NULL \n");
		retval = -EINVAL;
		goto out;
	}
	rmi4_data->synaptics_chip_data->ts_platform_data = data;
	rmi4_data->synaptics_dev = data->ts_dev;
	rmi4_data->synaptics_dev->dev.of_node =
		rmi4_data->synaptics_chip_data->cnode;
	...
	synaptics_private_config_parse(rmi4_data->synaptics_dev->dev.of_node,
				       rmi4_data);

	if (true == rmi4_data->synaptics_chip_data->check_bulcked) {
		if (ts_kit_get_pt_station_status(&isbulcked) ||
		    false == isbulcked) {
	...
	retval = of_property_read_u32(rmi4_data->synaptics_chip_data->cnode,
				      SYNAPTCS_SLAVE_ADDR,
				      (u32 *)&rmi4_data->synaptics_chip_data
					      ->ts_platform_data->client->addr);
	if (retval) {
		rmi4_data->synaptics_chip_data->ts_platform_data->client->addr =
			SYNAPTIC_DEFAULT_I2C_ADDR;
		TS_LOG_INFO("not set %s in dts, use default.\n",
			    SYNAPTCS_SLAVE_ADDR);
	}
	TS_LOG_INFO(
		"synaptics slave_addr= %d.\n",
		rmi4_data->synaptics_chip_data->ts_platform_data->client->addr);
	/*reset the chip */
	...
	if (retval < 0) {
		...
		strncpy(rmi4_data->synaptics_chip_data->chip_name,
...
static int synaptics_init_chip(void)
{
	...
	rmi4_data->flip_x = rmi4_data->synaptics_chip_data->flip_x;
	rmi4_data->flip_y = rmi4_data->synaptics_chip_data->flip_y;
	...
	if (NULL != rmi4_data->module_name) {
		...
		strncpy(rmi4_data->synaptics_chip_data->module_name,
			rmi4_data->module_name, MAX_STR_LEN - 1);
...
static int synaptics_get_brightness_info(void)
{
	...
	if (TS_UNINIT ==
	    atomic_read(
		    &rmi4_data->synaptics_chip_data->ts_platform_data->state)) {
...
static int synaptics_fw_update_boot(char *file_name)
{
	...
	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type &&
		...
		msleep(1000);
		retval = synap_get_fw_data_boot(file_name,
						synaptics_sett_param_regs);
		if (retval) {
			retval = 0;
		...
		if (rmi4_data->force_update || need_update) {
			...
			if (retval) {
				...
				strncpy(config_id_string,
					rmi4_data->rmi4_mod_info.image_config_id,
					strlen(rmi4_data->rmi4_mod_info
						       .image_config_id));
			}
		} else {
		...
		TS_LOG_INFO("file_name name is :%s\n", file_name);
		msleep(1000);
		retval = synap_get_fw_data_s3718_boot(
			file_name, synaptics_sett_param_regs);
		if (retval) {
			retval = 0;
...
data_release:
	...
	if (!rmi4_data->synaptics_chip_data->unite_cap_test_interface) {
		if (SYNAPTICS_S3207 ==
		    rmi4_data->synaptics_chip_data->ic_type) {
			retval = synap_rmi4_f54_s3207_init(
				rmi4_data,
				synaptics_sett_param_regs
					->module_name); /* for test report function*/
			if (retval) {
				TS_LOG_ERR("synap_rmi4_f54_init failed\n");
			}
		}
	}
	strncpy(rmi4_data->synaptics_chip_data->version_name, config_id_string,
		CHIP_INFO_LENGTH);
...
static int synaptics_fw_update_sd(void)
{
	...
	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type &&
		...
		if (retval < 0) {
			TS_LOG_ERR("failed update fw\n");
			strncpy(rmi4_data->synaptics_chip_data->version_name,
				rmi4_data->rmi4_mod_info.device_config_id,
				CHIP_INFO_LENGTH);
...
static int synaptics_regs_operate(struct ts_regs_info *info)
{
	...
	case TS_ACTION_WRITE:
		if ((1 == info->num) && (8 > bit)) {
			...
			retval = synaptics_rmi4_i2c_read(rmi4_data, info->addr,
							 value, info->num);
			if (retval < 0) {
		...
		retval = synaptics_rmi4_i2c_write(rmi4_data, info->addr, value,
						  info->num);
		if (retval < 0) {
...
static int synaptics_set_wakeup_gesture_enable_switch(u8 enable)
{
	...
	holster_enable_addr =
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info
			.holster_info.holster_switch_addr;
	holster_bit_num =
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info
			.holster_info.holster_switch_bit;
	TS_LOG_INFO(
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, holster_enable_addr,
					  &holster_temp_value, 1);
	if (retval < 0) {
...
static int synaptics_set_charger_switch(u8 charger_switch)
{
	...
	charger_bit_num =
		rmi4_data->synaptics_chip_data->ts_platform_data->feature_info
			.charger_info.charger_switch_bit;
	TS_LOG_INFO(
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, charger_enable_addr,
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, charger_enable_addr,
					  &charger_temp_value, 1);
	if (retval < 0) {
...
static int synaptics_set_holster_switch(u8 holster_switch)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, holster_enable_addr,
					 (unsigned char *)&holster_temp_value,
					 1);
	if (retval < 0) {
		TS_LOG_ERR("read holster information failed: %d\n", retval);
		goto out;
...
void synaptics_set_screenoff_status_reg(void)
{
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, fc_addr, &temp_fc_switch,
					  1);
	if (retval < 0) {
...
static int synaptics_set_roi_switch(u8 roi_switch)
{
	...
	roi_ctrl_addr = rmi4_data->synaptics_chip_data->ts_platform_data
				->feature_info.roi_info.roi_control_addr;
	roi_control_bit = rmi4_data->synaptics_chip_data->ts_platform_data
				  ->feature_info.roi_info.roi_control_bit;
	TS_LOG_INFO(
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, roi_ctrl_addr,
					 &temp_roi_switch, 1);
	if (retval < 0) {
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, roi_ctrl_addr,
					  &temp_roi_switch, 1);
	if (retval < 0) {
		TS_LOG_ERR(
...
static int synaptics_read_roi_switch(void)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, roi_ctrl_addr, &roi_switch,
					 sizeof(roi_switch));
	if (retval < 0) {
	...
	rmi4_data->synaptics_chip_data->ts_platform_data->feature_info.roi_info
		.roi_switch = roi_switch;
	...
	if (f51_roi_switch) {
		if (roi_data_staled) { /* roi_data may be refreshing now, wait it for some time(30ms). */
			if (!wait_for_completion_interruptible_timeout(
				    &roi_data_done, msecs_to_jiffies(30))) {
				roi_data_staled =
					0; /* Never wait again if data refreshing gets timeout. */
...
static int synaptics_do_calibrate(unsigned char write_value, int count)
{
	...
	if (strncmp(rmi4_data->rmi4_mod_info.product_id_string, "S332U",
		    strlen("S332U"))) {
		TS_LOG_INFO("%s, not S332U, no need to calibrate\n", __func__);
		goto out;
	}

	addr = synap_f54_get_calibrate_addr(
		rmi4_data, synaptics_sett_param_regs->module_name);
	if (!addr) {
	...
	do {
		count--;
		ret = synaptics_rmi4_i2c_read(rmi4_data, addr, &value,
					      sizeof(value));
		if (!ret) {
...
static int synaptics_get_glove_switch(u8 *glove_switch)
{
	...
	glove_enable_addr = rmi4_data->synaptics_chip_data->ts_platform_data
				    ->feature_info.glove_info.glove_switch_addr;
	glove_bit_num = rmi4_data->synaptics_chip_data->ts_platform_data
				->feature_info.glove_info.glove_switch_bit;
...
static int synaptics_set_glove_switch(u8 glove_switch)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, glove_enable_addr,
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, glove_enable_addr,
					  (unsigned char *)&glove_temp_value,
					  1);
	if (retval < 0) {
...
static int synaptics_get_palm_switch(u8 *palm_switch)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		*palm_switch = rmi4_data->synaptics_chip_data->easy_wakeup_info
				       .palm_cover_flag;
	} else {
		palm_enable_offset = rmi4_data->rmi4_feature.geswakeup_feature
					     .f12_2d_ctrl22_palm;
		palm_enable_addr = rmi4_data->rmi4_feature.f12_ctrl_base_addr +
				   palm_enable_offset;
		TS_LOG_INFO("get palm addr : 0x%02x\n", palm_enable_addr);

		retval = synaptics_rmi4_i2c_read(rmi4_data, palm_enable_addr,
						 palm_switch,
						 sizeof(*palm_switch));
		if (retval < 0) {
...
static int synaptics_set_palm_switch(u8 palm_switch)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		retval = synaptics_rmi4_i2c_write(rmi4_data, palm_enable_addr,
						  &palm_switch, 1);
		if (retval < 0) {
...
static int synaptics_palm_switch(struct ts_palm_info *info)
{
	...
	case TS_ACTION_READ:
		if (SYNAPTICS_S3207 ==
			...
			if (retval < 0) {
				TS_LOG_ERR("get palm switch(%d), failed : %d\n",
					   info->palm_switch, retval);
			}
			if ((buf & PALM_REG_BIT) ==
			    1) { /*palm close status :1 plam invald*/
				info->palm_switch = 0;
			} else if ((buf & PALM_REG_BIT) ==
				   0) { /*palm open status :0 palm int - sleep*/
				info->palm_switch = 1;
			} else {
...
static void synaptics_put_device_into_easy_wakeup(void)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, rmi4_data->rmi4_feature.f11_ctrl_base_addr,
		...
		device_ctrl = (device_ctrl &
			       ~MASK_3BIT); /*get the high five bits,i'dont*/
		...
		retval = synaptics_rmi4_i2c_read(rmi4_data, F54_ANALOG_CMD0,
						 &device_ctrl,
						 sizeof(device_ctrl));
		if (retval < 0) {
		...
		TS_LOG_DEBUG(
			" read from  f11_ctrl_base_addr92  ctrl1 = %0x retval =%0x\n",
			device_ctrl, retval);
		retval = synaptics_rmi4_i2c_write(rmi4_data, F54_ANALOG_CMD0,
						  &device_ctrl,
						  sizeof(device_ctrl));
		if (retval < 0) {
		...
		gusture_ctrl_offset = rmi4_data->rmi4_feature.geswakeup_feature
					      .f12_2d_ctrl20_lpm;
		f12_ctrl_base = rmi4_data->rmi4_feature.f12_ctrl_base_addr;
		/*reg Report Wakeup Gesture Only read first */
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, f12_ctrl_base + gusture_ctrl_offset,
			&device_ctrl_data[0], sizeof(device_ctrl_data));
		if (retval < 0) {
			TS_LOG_ERR(
		...
		device_ctrl_data[2] = device_ctrl_data[2] |
				      GESTURE_ENABLE_BIT01;
...
static void
synaptics_put_device_outof_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = NO_ERR;
	unsigned char device_ctrl = 0;
	unsigned char device_ctrl_data[4] = {0};
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		/*set fastrate*/
		device_ctrl = rmi4_data->synaptics_chip_data->easy_wakeup_info
				      .easy_wakeup_fastrate;
		retval = synaptics_rmi4_i2c_write(rmi4_data,
						  EASY_WAKEUP_FASTRATE,
						  &device_ctrl,
...
static void synaptics_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(
		rmi4_data, rmi4_data->rmi4_feature.f01_ctrl_base_addr,
		&device_ctrl, sizeof(device_ctrl));
	if (retval < 0) {
	...
	if (!strcmp(rmi4_data->synaptics_chip_data->ts_platform_data
			    ->product_name,
		    "venus") ||
	    !strcmp(rmi4_data->synaptics_chip_data->ts_platform_data
			    ->product_name,
		    "victoria") ||
...
static int synatpics_sleep_mode_in(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(
		rmi4_data, rmi4_data->rmi4_feature.f01_data_base_addr + 1,
...
static void synaptics_report_face_dct_only(unsigned char en)
{
	...
	error = synaptics_rmi4_i2c_read(rmi4_data, REG_REPORT_FACE_DCT_ONLY,
					value, (unsigned short)sizeof(value));
	if (error < 0)
	...
	error = synaptics_rmi4_i2c_write(rmi4_data, REG_REPORT_FACE_DCT_ONLY,
					 value, (unsigned short)sizeof(value));
	if (error < 0)
...
static int synaptics_suspend(void)
{
	...
	TS_LOG_INFO(
		"in last time wake mode synaptics_interrupt_num = %d interrupts tskit_pt_station_flag = %d\n",
		synaptics_interrupt_num, tskit_pt_station_flag);

	rmi4_data->ud_finger_status =
		rmi4_data->synaptics_chip_data->ts_platform_data
			->udfp_enable_flag;
	...
	case TS_POWER_OFF_MODE:
		...
		if (!rmi4_data->ud_finger_status) {
			...
			TS_LOG_INFO(
				"synaptics_suspend:udfp_enable_flag == 1.\n");
		}
		break;
...
static int synaptics_after_resume(void *feature_info)
{
	...
	if (cd->ic_type != SYNAPTICS_TD4322 && cd->ic_type != SYNAPTICS_TD4310)
		msleep(150);
	...
	struct ts_wakeup_gesture_enable_info *info)
{
	...
	if (info->op_action == TS_ACTION_WRITE) {
		retval = synaptics_set_wakeup_gesture_enable_switch(
			info->switch_value);
...
rescan:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F01:
				...
				if (synaptics_rmi4_crc_in_progress(rmi4_data,
								   &status))
					goto rescan;
				done = true;
				break;
	...
	retval = synaptics_rmi4_i2c_write(
		rmi4_data, rmi4_data->rmi4_feature.f01_cmd_base_addr, &command,
		sizeof(command));
	if (retval < 0) {
...
static int synaptics_rmi4_status_resume(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (info->charger_info.charger_supported) {
		retval = synaptics_set_charger_switch(
	...
	if (TS_GESTURE_MODE ==
	    rmi4_data->synaptics_chip_data->easy_wakeup_info.sleep_mode) {
		retval = synaptics_set_palm_switch(
			!rmi4_data->synaptics_chip_data->easy_wakeup_info
				 .palm_cover_control);
		if (retval < 0) {
			TS_LOG_ERR(
				"Failed to set plam switch(%d), err: %d\n",
				rmi4_data->synaptics_chip_data->easy_wakeup_info
					.palm_cover_control,
				retval);
		}
	...
	TS_LOG_INFO(
		"%s: glove switch: %d ,holster switch: %d,roi switch: %d, charger switch: %d\n",
		__func__, info->glove_info.glove_switch,
		info->holster_info.holster_switch, info->roi_info.roi_switch,
		info->charger_info.charger_switch);
...
#endif
static int synaptics_rmi4_i2c_read_f35(struct synaptics_rmi4_data *rmi4_data,
				       unsigned short addr, unsigned char *data,
				       unsigned short length)
{
	return 0;
...
static int synaptics_rmi4_i2c_write_f35(struct synaptics_rmi4_data *rmi4_data,
					unsigned short addr,
					unsigned char *data,
					unsigned short length)
{
				   ...
				   unsigned short length)
{
	...
	if (rmi4_data->use_ub_addr) {
		retval = synaptics_rmi4_i2c_read_f35(rmi4_data, addr, data,
						     length);
		return retval;
	}
	...
	retval = rmi4_data->synaptics_chip_data->ts_platform_data->bops
			 ->bus_read(&reg_addr, 1, data, length);
	if (retval < 0) {
				    ...
				    unsigned short length)
{
	...
	if (rmi4_data->use_ub_addr) {
		retval = synaptics_rmi4_i2c_write_f35(rmi4_data, addr, data,
						      length);
		return retval;
	}
...
static void
synaptics_check_crc_err_reset(struct synaptics_rmi4_f01_device_status status)
{
	int ret = 0;
...
rescan:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F01:
				...
				retval = synaptics_rmi4_query_device_info(
					rmi4_data);
				if (retval < 0) {
					TS_LOG_ERR(
				...
				if (status.flash_prog == 1) {
					TS_LOG_INFO(
						...
						status.status_code);
					rmi4_data->force_update =
						true; /*crc error, force update fw to workaround */
#if defined(CONFIG_HONOR_DSM)
					ts_dmd_report(
				...
				if (synaptics_rmi4_device_status_check(
					    status)) {
					TS_LOG_ERR(
			...
			case SYNAPTICS_RMI4_F11:
				...
				fhandler = synaptics_rmi4_alloc_fh(&rmi_fd,
								   page_number);
				if (NULL == fhandler) {
	...
	if (rmi4_data->num_of_intr_regs >= MAX_INTR_REGISTERS) {
		...
		memset(rmi4_data->intr_mask, 0x00,
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			...
			intr_addr = rmi4_data->rmi4_feature.f01_ctrl_base_addr +
				    1 + ii;
			retval = synaptics_rmi4_i2c_write(
				rmi4_data, intr_addr,
				&(rmi4_data->intr_mask[ii]),
			       ...
			       struct synaptics_rmi4_f01_device_status *status)
{
	...
	while (1) {
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, rmi4_data->rmi4_feature.f01_data_base_addr,
...
synaptics_rmi4_alloc_fh(struct synaptics_rmi4_fn_desc *rmi_fd, int page_number)
{
	...
	TS_LOG_DEBUG(
		...
		rmi_fd->fn_number, rmi_fd->data_base_addr,
...
synaptics_rmi4_query_device_info(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(
		rmi4_data, rmi4_data->rmi4_feature.f01_query_base_addr,
					  ...
					  unsigned short ctrl28)
{
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, ctrl_28_address,
				   ...
				   unsigned int intr_count)
{
	...
	fhandler->eratio_data = kzalloc(
		(sizeof(*eratio_data) * F12_FINGERS_TO_SUPPORT), GFP_KERNEL);
	if (fhandler->eratio_data == NULL) {
		TS_LOG_ERR(
			"Failed to alloc memory for fhandler->eratio_data\n");
		retval = -ENOMEM;
	...
	retval = synaptics_rmi4_i2c_read(
		rmi4_data,
		rmi4_data->rmi4_feature.f12_ctrl_base_addr + ctrl_8_offset,
		f12_2d_data, F12_2D_CTRL_LEN);
	if (retval < 0) {
	...
	ctrl_22_offset = ctrl_20_offset + query_5.ctrl20_is_present +
	...
	ctrl_26_offset = ctrl_23_offset + query_5.ctrl23_is_present +
			 query_5.ctrl24_is_present + query_5.ctrl25_is_present;
	rmi4_data->rmi4_feature.glove_feature.offset = ctrl_26_offset;

	ctrl_28_offset = ctrl_26_offset + query_5.ctrl26_is_present +
	...
	fhandler->num_of_data_points =
		min(ctrl_23.max_reported_objects,
	...
	if ((size_of_query8 >= SIZE_OF_QUERY8) && (query_8.data36_is_present)) {
		...
		extra_data->data36_offset =
			extra_data->data15_offset + query_8.data15_is_present +
			...
			query_8.data28_is_present + query_8.data29_is_present +
			query_8.data30_is_present + query_8.data31_is_present +
			query_8.data32_is_present + query_8.data33_is_present +
	...
	data_04_offset = query_8.data0_is_present + query_8.data1_is_present +
			 query_8.data2_is_present + query_8.data3_is_present;
	...
	rmi4_data->max_touch_width =
		max(rmi4_data->num_of_rx, rmi4_data->num_of_tx);
				   ...
				   unsigned int intr_count, unsigned char page)
{
	...
	rmi4_data->rmi4_feature.f51_ctrl_base_addr = fd->ctrl_base_addr |
						     (page << 8);
	rmi4_data->rmi4_feature.f51_data_base_addr = fd->data_base_addr |
						     (page << 8);
	rmi4_data->rmi4_feature.f51_query_base_addr = fd->query_base_addr |
						      (page << 8);
...
static int synaptics_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_X, 0, rmi4_data->sensor_max_x, 0,
			     0);
	input_set_abs_params(input_dev, ABS_Y, 0, rmi4_data->sensor_max_y, 0,
			     0);
...
static int
easy_wakeup_gesture_report_coordinate(struct synaptics_rmi4_data *rmi4_data,
				      unsigned int reprot_gesture_point_num,
				      struct ts_fingers *info)
{
	int retval = 0;
	unsigned char get_custom_data[LOCUS_DATA_NUM * F12_FINGERS_TO_SUPPORT +
				      4] = {0};
	int x = 0;
	int y = 0;
	int i = 0;
	...
	if (reprot_gesture_point_num != 0) {
		...
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, f51_data_base, get_custom_data,
		...
		for (i = 0; i < reprot_gesture_point_num; i++) {
			...
			if (SYNAPTICS_S3207 ==
			    rmi4_data->synaptics_chip_data->ic_type) {
				x = x *
				    rmi4_data->synaptics_chip_data->x_max_mt /
				    rmi4_data->sensor_max_x;
				if (rmi4_data->synaptics_chip_data
					    ->has_virtualkey) {
					y = y *
				...
				if ((y >
				     rmi4_data->synaptics_chip_data->lcd_full) ||
				    (y < 0)) {
					TS_LOG_ERR(
...
static int synaptics_rmi4_key_gesture_report(
	struct synaptics_rmi4_data *rmi4_data, struct ts_fingers *info,
	...
	unsigned char *get_gesture_wakeup_data)
{
	...
	if (0 != reprot_gesture_key_value) {
		...
		__pm_wakeup_event(&rmi4_data->synaptics_chip_data
					   ->ts_platform_data->ts_wake_lock,
				  jiffies_to_msecs(5 * HZ));
		mutex_lock(&wrong_touch_lock);
		if (true == rmi4_data->synaptics_chip_data->easy_wakeup_info
				    .off_motion_on) {
			rmi4_data->synaptics_chip_data->easy_wakeup_info
				.off_motion_on = false;
			retval = easy_wakeup_gesture_report_coordinate(
				rmi4_data, reprot_gesture_point_num, info);
			if (retval < 0) {
				mutex_unlock(&wrong_touch_lock);
				TS_LOG_INFO(
					 ...
					 struct ts_fingers *info)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		retval = synaptics_rmi4_i2c_read(
			...
			&f11_2d_data39[0], sizeof(f11_2d_data39));
		if (retval < 0) {
		...
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, f12_data_base + wake_up_value_addr_offset,
			&get_gesture_wakeup_data[0],
	...
	if (get_gesture_wakeup_data[0] != 0) {
		retval = synaptics_rmi4_key_gesture_report(
			rmi4_data, info, gesture_report_info,
...
static int
synaptics_rmi4_palm_sleep_report(struct synaptics_rmi4_data *rmi4_data,
				 struct synaptics_rmi4_fn *fhandler,
				 struct ts_fingers *info)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		f12_data_base = rmi4_data->rmi4_feature.f12_data_base_addr;
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, f12_data_base, &object_type_and_status[0],
					  ...
					  struct ts_fingers *info)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			retval = synaptics_rmi4_i2c_read(rmi4_data, data_offset,
							 data,
							 data_reg_blk_size);
			if (retval < 0) {
			...
			x = x * rmi4_data->synaptics_chip_data->x_max_mt /
			    rmi4_data->sensor_max_x;
			if (0 ==
			    rmi4_data->synaptics_chip_data->has_virtualkey) {
				y = y *
				    rmi4_data->synaptics_chip_data->y_max_mt /
				    rmi4_data->sensor_max_y;
			} else {
				y = y *
				    rmi4_data->synaptics_chip_data->lcd_full /
			...
			TS_LOG_DEBUG(
				"Finger %d:status = 0x%02x,wx = %d,wy = %d",
					  ...
					  struct ts_fingers *info)
{
	...
	if (rmi4_data->is_multi_protocal == MULTI_PROTOCAL_2) {
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, fhandler->full_addr.query_base + 7,
		...
		retval = synaptics_rmi4_i2c_read(
			rmi4_data, fhandler->full_addr.query_base + 8,
			query_8.data, size_of_query8);
		if (retval < 0) {
			TS_LOG_ERR(
				"Failed to read f12 ->full_addr.query_base = %d,here is +8\n",
		...
		if (query_8.data36_is_present) {
			...
			if (retval < 0) {
				TS_LOG_ERR(
					"Failed to read eratio data: %d, read data offset is %d \n",
					retval, extra_data->data1_offset);
				return;
			}
		}
	...
	eratio_data =
		(struct synaptics_rmi4_f12_eratio_data *)fhandler->eratio_data;
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (rmi4_data->synaptics_chip_data->support_aft &&
		    finger < MAX_FINGER_NUM && !grip_data_flag) {
			finger_grip_data =
				(struct synaptics_grip_data *)grip_data +
				finger;
		}
		...
		if (finger_status) {
...
#endif
			x = (finger_data->x_msb << 8) |
			    (finger_data->x_lsb); /* Convert 16bit number*/
			y = (finger_data->y_msb << 8) |
			    (finger_data->y_lsb); /* Convert 16bit number*/
#ifdef REPORT_2D_W
			/* syna_wx_wy */
			...
			if (rmi4_data->is_multi_protocal == MULTI_PROTOCAL_1 &&
				...
				temp_wy = ((finger_data->wx >> 4) &
					   MASK_4BIT); /*get hight 4bit number*/
				temp_sg = finger_data->wy;
				wx = temp_wx;
				wy = temp_wy;
				sg = temp_sg;
				TS_LOG_DEBUG("new protocol\n");
			} else if (rmi4_data->is_multi_protocal ==
				   MULTI_PROTOCAL_2) {
				TS_LOG_DEBUG("new protocol 2\n");
				ewx = ((finger_data->new_ew) & MASK_4BIT);
				ewy = ((finger_data->new_ew >> 4) &
				       MASK_4BIT); /*get hight 4bit number*/
				wx = ((finger_data->new_w) & MASK_4BIT);
				wy = ((finger_data->new_w >> 4) &
				      MASK_4BIT); /*get hight 4bit number*/
			...
			if (rmi4_data->synaptics_chip_data->support_3d_func) {
				...
				retval = synaptics_rmi4_i2c_read(
					rmi4_data, 0x041a, &force_level,
					1); /*Press force register addr*/
				if (retval < 0) {
					TS_LOG_ERR(
			...
			TS_LOG_DEBUG(
				"Finger %d:status = 0x%02x,wx = %d, wy = %d,sg = %d,z = %d\n",
			...
			if (rmi4_data->synaptics_chip_data->support_aft &&
				...
				TS_LOG_DEBUG(
					"grip data: ewx = %d, ewy = %d, xer = %d, yer = %d\n",
					finger_grip_data->ewx,
					finger_grip_data->ewy,
					finger_grip_data->xer,
					finger_grip_data->yer);
				...
				if (rmi4_data->is_multi_protocal ==
					...
					TS_LOG_DEBUG(
						"xer = %d|yer = %d|ewx = %d|ewy = %d|wx = %d|wy= %d\n",
						info->fingers[finger].xer,
						info->fingers[finger].yer,
						info->fingers[finger].ewx,
						info->fingers[finger].ewy,
				      ...
				      struct ts_fingers *info)
{
	...
	if (!strncmp(rmi4_data->rmi4_mod_info.product_id_string, "S3320",
		     strlen("S3320")))
		touchplus_offset =
			S3320_F51_TOUCHPLUS_OFFSET; /*S3320 touchplus addr : 0x0422 */
	else
		touchplus_offset =
			S3350_F51_TOUCHPLUS_OFFSET; /*S3350 touchplus addr : 0x040E */

	data_addr = ctrl_base_addr + touchplus_offset;
	TS_LOG_DEBUG("%s : data_addr = 0x%02x \n", __func__, data_addr);
	retval = synaptics_rmi4_i2c_read(rmi4_data, data_addr, &touchplus_data,
					...
					struct ts_fingers *info)
{
	 ...
	 */
	retval = synaptics_rmi4_i2c_read(
		rmi4_data, rmi4_data->rmi4_feature.f01_data_base_addr + 1, intr,
		rmi4_data->num_of_intr_regs);
	if (retval < 0) {
				     ...
				     struct ts_cmd_node *out_cmd)
{
	...
	if (retval < 0) {
		if (g_report_err_log_count < 30) {
			...
			TS_LOG_DEBUG(
				"synaptics_rmi4_sensor_report, error: %d\n",
				retval);
		}
		return retval;
	}
