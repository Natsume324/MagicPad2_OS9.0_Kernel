...
enum flash_area { NONE, UI_FIRMWARE, CONFIG_AREA };
...
static int fwu_read_f34_queries(void)
{
	...
	retval = fwu->fn_ptr->read(
		...
		&fwu->flash_properties, sizeof(fwu->flash_properties));
	if (retval < 0) {
	...
	retval = fwu->fn_ptr->read(
		fwu->rmi4_data, fwu->f34_fd.query_base_addr + fwu->blk_size_off,
		buf, 2);
	if (retval < 0) {
	...
	retval = fwu->fn_ptr->read(
		fwu->rmi4_data,
		fwu->f34_fd.query_base_addr + fwu->blk_count_off, buf, count);
	if (retval < 0) {
...
static int fwu_read_f34_flash_status(void)
{
	...
	retval = fwu->fn_ptr->read(
		fwu->rmi4_data, fwu->f34_fd.data_base_addr + fwu->flash_cmd_off,
...
static int fwu_write_bootloader_id(void)
{
	...
	retval = fwu->fn_ptr->write(
		fwu->rmi4_data, fwu->f34_fd.data_base_addr + fwu->blk_data_off,
		fwu->bootloader_id, sizeof(fwu->bootloader_id));
	if (retval < 0) {
...
#if defined(CONFIG_HONOR_DSM)
		rmi4_data->synaptics_chip_data->ts_platform_data->dsm_info
			.constraints_update_status = fwu_write_f34_command_fail;
#endif
		return retval;
	}
...
#if defined(CONFIG_HONOR_DSM)
		rmi4_data->synaptics_chip_data->ts_platform_data->dsm_info
			.constraints_update_status = fwu_wait_for_idle_fail;
#endif
		return retval;
	}
...
static bool in_bounds(unsigned int offset, unsigned int size,
		      unsigned int bound)
{
...
int synap_get_fw_data_sd(void)
{
	...
	retval = request_firmware(&fwu->fw_entry_sd,
				  SYNAPTICS_FW_MANUAL_UPDATE_FILE_NAME, dev);
	if (retval != 0) {
...
bool synap_check_fw_version(void)
{
	...
	if (header.config_size) {
		...
		if (!in_bounds(FW_IMAGE_OFFSET, header.config_size,
			       fwu->image_size)) {
