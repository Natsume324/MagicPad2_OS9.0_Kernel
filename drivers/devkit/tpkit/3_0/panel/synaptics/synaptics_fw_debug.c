...
static struct device_attribute attrs[] = {
	__ATTR(open, S_IWUSR, NULL, rmidev_sysfs_open_store),
	__ATTR(release, S_IWUSR, NULL, rmidev_sysfs_release_store),
	__ATTR(attn_state, (S_IRUSR | S_IRGRP), rmidev_sysfs_attn_state_show,
	       NULL),
};
...
void synap_rmidev_sysfs_irq(void *data)
{
	...
	sysfs_notify(&rmi4_data->input_dev->dev.kobj, SYSFS_FOLDER_NAME,
		     "attn_state");
				      ...
				      char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev->fn_ptr->read(rmidev->rmi4_data, address,
				       ...
				       char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev->fn_ptr->write(rmidev->rmi4_data, address,
					       (unsigned char *)buf, length);
		if (retval < 0) {
					    ...
					    char *buf)
{
	...
	attn_state = gpio_get_value(rmidev->rmi4_data->synaptics_chip_data
			   ...
			   loff_t *f_pos)
{
	...
	retval = rmidev->fn_ptr->read(rmidev->rmi4_data, *f_pos, rmidev->tmpbuf,
				      count);
	if (retval < 0) {
	...
	if (!list_empty(&rmidev->rmi4_data->rmi4_mod_info.support_fn_list)) {
		list_for_each_entry(
			fhandler,
			&rmidev->rmi4_data->rmi4_mod_info.support_fn_list,
			link) {
			if (fhandler->num_of_data_sources) {
				if (fhandler->intr_mask & intr_status) {
					rmidev->rmi4_data->report_touch(
						rmidev->rmi4_data, fhandler,
						info);
				}
			}
		}
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	if ((NULL == rmidev) || (NULL == rmidev->rmi4_data) ||
	    (NULL == rmidev->rmi4_data->synaptics_chip_data))
		return 0;
	...
	retval = rmidev->fn_ptr->write(rmidev->rmi4_data, *f_pos,
				       rmidev->tmpbuf, count);
	if (retval >= 0) {
		*f_pos += retval;
	} else {
...
static int rmidev_open(struct inode *inp, struct file *filp)
{
	...
	if (!rmi4_data->synaptics3718_Tp_Pressure_flag) {
		...
		if (0 != strncmp(comm, "ptics.redremote",
				 strlen("ptics.redremote"))) {
...
void synap_fw_debug_dev_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmidev->sysfs_dir = kobject_create_and_add(
		SYSFS_FOLDER_NAME, &rmi4_data->input_dev->dev.kobj);
	if (!rmidev->sysfs_dir) {
