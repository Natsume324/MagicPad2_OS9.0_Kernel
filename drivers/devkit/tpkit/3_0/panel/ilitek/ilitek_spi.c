				  ...
				  unsigned n_tx, void *rxbuf, unsigned n_rx)
{
	...
	xfer = kzalloc(DMA_TRANSFER_MAX_CHUNK * sizeof(struct spi_transfer),
		       GFP_KERNEL);
	if (!xfer) {
	...
	case SPI_READ:
		if (n_tx > DMA_TRANSFER_MAX_LEN) {
			TS_LOG_ERR(
				"Tx length must be lower than dma length %d.\n",
		...
		if (status == 0) {
			if (ilits->spi_rx[1] != SPI_ACK &&
				...
				TS_LOG_ERR(
					"Do spi recovery: rxbuf[1] = 0x%x, ice = %d\n",
					ilits->spi_rx[1],
					atomic_read(&ilits->ice_stat));
				break;
			}
			   ...
			   bool spi_irq, bool i2c_irq)
{
	...
	if (wlen > 0) {
		...
		if (wlen >= sizeof(wdata) - WDATA_HEAD_LENGTH) {
			TS_LOG_ERR(
				"WARNING! wlen%d > wdata%d, using wdata length to transfer\n",
	...
	case SPI_WRITE:
		if (ice) {
			...
			if (ilits->tp_suspend &&
				...
				TS_LOG_INFO(
					"write dummy to wake up spi pll clk\n");
