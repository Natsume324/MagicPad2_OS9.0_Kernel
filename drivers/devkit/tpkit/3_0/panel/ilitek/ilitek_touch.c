				    ...
				    u32 copy_size)
{
	...
	if (retry <= 0)
		TS_LOG_ERR(
			"DMA fail: Regsiter = 0x%x Flash = 0x%x, Size = %d\n",
...
int ili_proximity_far(int mode)
{
	...
	case WAKE_UP_SWITCH_GESTURE_MODE:
		...
		if (ret < 0)
			TS_LOG_ERR(
				"Switch to gesture mode failed during proximity far\n");
		break;
	default:
		TS_LOG_ERR("Unknown mode %d\n", mode);
		ret = -EINVAL;
...
int ili_move_gesture_code_iram(int mode)
{
	...
	if (i >= timeout) {
		TS_LOG_ERR(
			"Gesture is not ready (0x%x), try to run its recovery\n",
			cmd[0]);
