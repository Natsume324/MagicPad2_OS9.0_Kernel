		...
		if (ENABLE_WQ_ESD || ilits->wq_ctrl) {
			...
			if (ctrl == ENABLE) {
				...
				if (!queue_delayed_work(
					...
					TS_LOG_DEBUG(
						"esd check was already on queue\n");
			} else {
		...
		if (ENABLE_WQ_BAT || ilits->wq_ctrl) {
			...
			if (ctrl == ENABLE) {
				...
				if (!queue_delayed_work(
					...
					TS_LOG_DEBUG(
						"bat check was already on queue\n");
			} else {
...
int ili_sleep_handler(int mode)
{
	...
	case TP_SUSPEND:
		...
		if (sense_stop) {
			...
			if (ili_ic_check_busy(50, 20) < 0)
				TS_LOG_ERR(
					"Check busy timeout during suspend\n");
		}

		if (ilits->gesture) {
			ili_switch_tp_mode(P5_X_FW_GESTURE_MODE);
			enable_irq_wake(
				ilits->ts_dev_data->ts_platform_data->irq_id);
			ili_irq_enable();
		} else {
	...
	case TP_DEEP_SLEEP:
		...
		if (sense_stop) {
			...
			if (ili_ic_check_busy(50, 20) < 0)
				TS_LOG_ERR(
					"Check busy timeout during deep suspend\n");
		}
...
int ilitek_read_roi_data(void)
{
	...
	if (ts_dev_data->ts_platform_data->feature_info.roi_info.roi_switch &&
		...
		if (ilits->last_touch != ilits->finger &&
			...
			ret = ilits->wrapper(NULL, 0, ilits->roi_buf, len, OFF,
					     OFF);

			if (ret < 0) {
				TS_LOG_ERR(
					"spi read roi data failed, ret = %d\n",
					ret);
				return ret;
			}
			mutex_lock(&ilits->roi_mutex);
			/*add new roi with fw*/
			if (ilits->head_roi_supported)
				memcpy(ilits->roi_data, &ilits->roi_buf[2],
				       ROI_DATA_READ_LENGTH);
			else
				memcpy(ilits->roi_data, &ilits->roi_buf[1],
				       ROI_DATA_READ_LENGTH);
			...
			TS_LOG_DEBUG(
				"index = %d, fixed = %d, peak_row = %d, peak_colum = %d\n",
			...
			if (debug_en) {
				...
				for (i = 4; i < ROI_DATA_READ_LENGTH; i++) {
					TS_LOG_INFO(KERN_CONT "%3d ",
						    ilits->roi_data[i]);
					if ((i - 3) % 14 ==
					    0) /* 14 * 14  rawdata bytes */
						TS_LOG_INFO("\n");
				}
			}
		}
...
static int ilitek_tddi_spi_read(struct ts_fingers *p_info, int rlen)
{
	...
	if (ret < 0) {
		TS_LOG_ERR("actual_tp_mode = %d, gesture = %d, prox_near= %d\n",
			   ilits->actual_tp_mode, ilits->gesture,
			   ilits->prox_near);
...
int ilitek_tddi_read_report_data(struct ts_fingers *p_info)
{
	...
	if ((true == ts_platform_data->feature_info.wakeup_gesture_enable_info
			     .switch_value) &&
...
void ili_update_tp_module_info(void)
{
	...
	if (module == 0) {
		TS_LOG_ERR(
			"Couldn't find any tp modules, applying default settings\n");
		ilits->md_name = "DEF";
