...
static int ilitek_test_get_ini_path(void)
{
	...
	if (!strnlen(p_dev_data->ts_platform_data->product_name,
		     MAX_STR_LEN - 1) ||
	...
	if (p_dev_data->hide_fw_name)
		snprintf(file_name, ILITEK_FILE_NAME_LEN, "%s_raw.ini",
			 ilits->project_id);
	else
	...
	snprintf(ilits->ini_path, ILITEK_FILE_PATH_LEN, "%s%s",
		 ILITEK_INI_PATH_PERFIX, file_name);
...
void ilitek_result_init(struct ts_rawdata_info *info, u8 *result_str)
{
	...
	strncat(result_str, ts_test_failed_reason,
		ILITEK_TEST_FAILED_REASON_LEN - 1);
...
static int parser_get_ini_key_value(const char *section, char *key, char *value,
				    int length)
{
	int i = 0;
	int ret = -2;
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (ipio_strcmp(key, ini_info[i].key_name) == 0) {
			if (ini_info[i].key_val_len > length)
				...
				memcpy(value, ini_info[i].key_value,
...
static int parser_get_int_data(const char *section, char *keyname, char *rv,
			       int rv_len)
{
	int len = 0;
	...
	if (parser_get_ini_key_value((const char *)section, keyname, value,
				     sizeof(value)) < 0) {
...
static int parser_get_ini_phy_data(char *data, int fsize)
{
	...
	while (true) {
		...
		if (n > 2 && ((ini_buf[0] == m_cfg_ssl &&
			...
			if (ini_buf[0] == m_cfg_ssl) {
				...
				if (scount > MAX_SECTION_NUM) {
					TS_LOG_ERR(
						"seq_item is over than its define %d, abort\n",
						scount);
					ret = INI_ERR_OUT_OF_LINE;
					goto out;
				}
				...
				ini_buf[n - 1] = 0x00;
				strncpy((char *)tmp_sec_name, ini_buf + 1,
					(PARSER_MAX_CFG_BUF + 1) *
						sizeof(char));
				banchmark_flag = 0;
				nodetype_flag = 0;
				strncpy(seq_item[scount], tmp_sec_name,
					PARSER_MAX_KEY_NAME_LEN);
				scount++;
				TS_LOG_DEBUG(
		...
		strncpy(ini_info[g_ini_items].section_name, tmp_sec_name,
			((PARSER_MAX_KEY_NAME_LEN - 1) * sizeof(char)));
...
static int ilitek_tddi_mp_ini_parser(const char *path)
{
	...
	if (err_alloc_mem(f)) {
		TS_LOG_ERR(
			"Failed to open ini file at %ld, try to request_firmware\n",
			PTR_ERR(f));
		f = NULL;
				  ...
				  s32 innodetype, int full_open_rate)
{
	...
	if ((uintype == NO_COMPARE) ||
	    ((uintype & ROUND_CORNER) == ROUND_CORNER))
		return true;
...
static s32 open_c_formula_compatible(int in_cap1_dac, int in_cap1_raw,
				     int accuracy)
{
	s32 in_cap1_value = 0;
...
static s32 open_c_formula(int dac, int raw, int tvch, int gain, int accuracy)
{
	...
	if (accuracy) {
		if (type == ILI_Q)
			ret = ((dac * 275 * 39 / 2 * 10) +
			       ((raw - 8192) * 9 * (7 * 100) * 25 / 1024)) /
			      tvch / 100 / gain;
		else if (type == ILI_S)
			ret = ((dac * 377 * 42 / 2 * 10) +
			       ((raw - 8192) * 9 * (66 * 10) * 25 / 1024)) /
			      tvch / 100 / gain;
		else
			ret = 9999;
	} else {
		if (type == ILI_Q)
			ret = ((dac * 275 * 39 / 2) +
			       ((raw - 8192) * 36 * (7 * 100) * 10 * 1 /
				16384)) /
			      tvch / 100 / gain;
		else if (type == ILI_S)
			ret = ((dac * 377 * 42 / 2) +
			       ((raw - 8192) * 36 * (66 * 10) * 10 * 1 /
				16384)) /
			      tvch / 100 / gain;
		else
			ret = 9999;
...
static void allnode_open_cdc_result(int index, int *buf, int *dac,
				    const int *raw)
{
	int i;
	...
	if (ipio_strcmp(desp, "open test(integration)_sp") == 0) {
		for (i = 0; i < core_mp.frame_len; i++)
			buf[i] = open_sp_formula(dac[i], raw[i], open_spec.tvch,
						 open_spec.tvcl);
		...
		for (i = 0; i < core_mp.frame_len; i++) {
			if (ilits->production_test_formula_compatible)
				buf[i] = open_c_formula_compatible(
					dac[i], raw[i], accuracy);
			else
				buf[i] = open_c_formula(
					dac[i], raw[i],
...
static int code_to_ohm(s32 *ohm, s32 *code, u16 *v_tdf, int v_len, u16 *h_tdf,
		       int h_len)
{
	int doutdf1 = 0;
	int doutdf2 = 0;
	...
	TS_LOG_DEBUG(
		"doutvch = %d, doutvcl = %d, douvariation = %d, doutdf1 = %d, doutdf2 = %d, doucint = %d, ratio = %d,dourinternal = %d\n",
		doutvch, doutvcl, douvariation, doutdf1, doutdf2, doucint,
		ratio, dourinternal);
	TS_LOG_DEBUG(
...
static int short_test(int index, int frame_index)
{
	...
	if (ilits->production_test_formula_compatible) {
		...
		parser_get_int_data("short test", "variation", str,
				    sizeof(str));
		...
		TS_LOG_INFO(
			"TVCH = %d, TVCL = %d, Variation = %d, V_TDF1 = %d, V_TDF2 = %d, \
			...
			titems[index].h_tdf_1, titems[index].h_tdf_2,
			short_para.cint, short_para.rinternal);
	}
	...
	if (core_mp.protocol_ver >= PROTOCOL_VER_540) {
		...
		ili_dump_data(frame_buf, 10, core_mp.frame_len, core_mp.xch_len,
			      "Short Raw 1");
		if (ilits->production_test_formula_compatible)
			code_to_ohm_compatible(
				&titems[index]
					 .buf[frame_index * core_mp.frame_len],
				frame_buf, v_tdf, h_tdf);
		else
			code_to_ohm(&titems[index].buf[frame_index *
						       core_mp.frame_len],
				    frame_buf, v_tdf, sizeof(v_tdf) / v_tdf[0],
				    h_tdf, sizeof(h_tdf) / h_tdf[0]);
...
static int allnode_peak_to_peak_cdc_data(int index)
{
	...
	if (frm_buf == NULL) {
		...
		for (i = 0; i < titems[index].bch_mrk_frm_num; i++) {
			...
			if (err_alloc_mem(frm_buf)) {
				TS_LOG_ERR(
					"Failed to allocate frm_buf[%d] mem (%ld)\n",
					i, PTR_ERR(frm_buf));
				ret = -EMP_NOMEM;
				goto out;
			}
	...
	if (titems[index].catalog == TX_RX_DELTA) {
		...
		if (core_mp.tx_max_buf == NULL) {
			...
			if (err_alloc_mem(core_mp.tx_max_buf)) {
				TS_LOG_ERR(
					"Failed to allocate tx_max_buf mem\n");
		...
		if (core_mp.tx_min_buf == NULL) {
			...
			if (err_alloc_mem(core_mp.tx_min_buf)) {
				TS_LOG_ERR(
					"Failed to allocate tx_min_buf mem\n");
		...
		if (core_mp.rx_max_buf == NULL) {
			...
			if (err_alloc_mem(core_mp.rx_max_buf)) {
				TS_LOG_ERR(
					"Failed to allocate rx_max_buf mem\n");
		...
		if (core_mp.rx_min_buf == NULL) {
			...
			if (err_alloc_mem(core_mp.rx_min_buf)) {
				TS_LOG_ERR(
					"Failed to allocate rx_min_buf mem\n");
		...
		if (titems[index].result_buf == NULL) {
			...
			if (err_alloc_mem(titems[index].result_buf)) {
				TS_LOG_ERR(
					"Failed to allocate result_buf mem\n");
		...
		if (titems[index].spec_option == BENCHMARK) {
			if (titems[index].bch_mrk_max == NULL) {
				...
				if (err_alloc_mem(titems[index].bch_mrk_max)) {
					TS_LOG_ERR(
						"Failed to allocate bch_mrk_max mem\n");
				...
				for (i = 0; i < titems[index].bch_mrk_frm_num;
					...
					if (err_alloc_mem(
						    titems[index]
							    .bch_mrk_max[i])) {
						TS_LOG_ERR(
							"Failed to allocate bch_mrk_max[%d] mem\n",
							i);
						ipio_kfree(
							(void **)&titems[index]
								.bch_mrk_max[i]);
			...
			if (titems[index].bch_mrk_min == NULL) {
				...
				if (err_alloc_mem(titems[index].bch_mrk_min)) {
					TS_LOG_ERR(
						"Failed to allocate bch_mrk_min mem\n");
					ipio_kfree((void **)&titems[index]
							   .bch_mrk_min);
					return -ENOMEM;
				}
				for (i = 0; i < titems[index].bch_mrk_frm_num;
					...
					if (err_alloc_mem(
						    titems[index]
							    .bch_mrk_min[i])) {
						TS_LOG_ERR(
							"Failed to allocate bch_mrk_min[%d] mem\n",
							i);
						ipio_kfree(
							(void **)&titems[index]
								.bch_mrk_min[i]);
...
static int open_test_sp(int index)
{
	...
	int charge_aa = 0, charge_boarder = 0, charge_notch = 0,
	    full_open_rate = 0;
	char str[512] = {0};
	...
	TS_LOG_DEBUG(
		"AA = %d, Border = %d, NOTCH = %d, full_open_rate = %d, tvch = %d, tvcl = %d\n",
		charge_aa, charge_boarder, charge_notch, full_open_rate,
		open_spec.tvch, open_spec.tvcl);
...
static int open_test_cap(int index)
{
	...
	if (ilits->production_test_formula_compatible) {
		parser_get_int_data(titems[index].desp, "cbk_step", str,
				    sizeof(str));
		open_spec.cbk_step = ili_katoi(str, g_katoi_len);

		parser_get_int_data(titems[index].desp, "cint", str,
				    sizeof(str));
		...
		TS_LOG_DEBUG(
			"gain = %d, tvch = %d, tvcl = %d, cbk_step = %d, cint = %d\n",
			open_spec.gain, open_spec.tvch, open_spec.tvcl,
			open_spec.cbk_step, open_spec.cint);
	}
	...
	for (i = 0; i < titems[index].frame_count; i++) {
		...
		allnode_open_cdc_result(index, open[i].dcl_cap, open[i].cap_dac,
					(const int *)open[i].cap_raw);

		/* record fist frame for debug */
		if (i == 0) {
			memcpy(cap_dac, open[i].cap_dac,
			       core_mp.frame_len * sizeof(s32));
			memcpy(cap_raw, open[i].cap_raw,
			       core_mp.frame_len * sizeof(s32));
		}
...
static int mp_get_timing_info(void)
{
	...
	slen = parser_get_int_data("pv5_4 command", "timing_info_raw", str,
				   sizeof(str));
	if (slen < 0)
		return -1;
...
static int mp_show_result(struct ts_rawdata_info *raw_info, bool lcm_on)
{
	...
	if (parser_get_ini_key_value("pv5_4 command", "date", core_mp.ini_date,
				     sizeof(core_mp.ini_date)) < 0)
		memcpy(core_mp.ini_date, "Unknown", strlen("Unknown"));
	if (parser_get_ini_key_value("pv5_4 command", "version",
				     core_mp.ini_ver,
				     sizeof(core_mp.ini_ver)) < 0)
	...
	if (pass_item_count == 0) {
		...
		if ((core_mp.lost_parameter) &&
			...
			TS_LOG_ERR(
				"WARNING! OPEN or SHORT parameter not found in ini file!!\n");
		}
		if (lcm_on)
		...
		if (core_mp.lost_benchmark) {
			...
			TS_LOG_ERR(
				"WARNING! Golden and SPEC in ini file aren't matched!!\n");
		}
...
static void mp_test_run(int index)
{
	...
	if (titems[i].catalog == SHORT_TEST) {
		parser_get_int_data(titems[i].desp, "v_tdf_1", str,
				    sizeof(str));
		titems[i].v_tdf_1 = parser_get_tdf_value((const char *)str,
							 titems[i].catalog);
		parser_get_int_data(titems[i].desp, "v_tdf_2", str,
				    sizeof(str));
		titems[i].v_tdf_2 = parser_get_tdf_value((const char *)str,
							 titems[i].catalog);
		parser_get_int_data(titems[i].desp, "h_tdf_1", str,
				    sizeof(str));
		titems[i].h_tdf_1 = parser_get_tdf_value((const char *)str,
							 titems[i].catalog);
		parser_get_int_data(titems[i].desp, "h_tdf_2", str,
				    sizeof(str));
		titems[i].h_tdf_2 = parser_get_tdf_value((const char *)str,
							 titems[i].catalog);
	} else {
	...
	if (ipio_strcmp(titems[i].desp, "tx/rx delta") == 0) {
		...
		TS_LOG_DEBUG(
			...
			titems[i].desp, core_mp.tx_delta_max,
			core_mp.tx_delta_min, core_mp.rx_delta_max,
			core_mp.rx_delta_min);
	} else {
	...
	TS_LOG_DEBUG("v_tdf_1 = %d, v_tdf_2 = %d, h_tdf_1 = %d, h_tdf_2 = %d",
...
static void ilitek_result_generate(struct ts_rawdata_info *raw_info,
				   u8 *result_str)
{
	int i, seq, len = 0;
	...
	for (seq = 0; seq < g_run_index.count; seq++) {
		...
		if (titems[i].item_result == MP_DATA_FAIL) {
			TS_LOG_ERR("[%s] = FAIL\n", titems[i].desp);
			strncpy(ts_test_failed_reason, "panel_reason-",
				ILITEK_TEST_FAILED_REASON_LEN - 1);
			len += snprintf(result_str + len,
					TS_RAWDATA_RESULT_MAX - len, "%dF-",
					seq + 1);
		} else {
			TS_LOG_INFO("[%s] = PASS\n", titems[i].desp);
			len += snprintf(result_str + len,
					TS_RAWDATA_RESULT_MAX - len, "%dP-",
					seq + 1);
		}
	}
...
int ili_mp_test_main(struct ts_rawdata_info *raw_info, u8 *apk, bool lcm_on)
{
	...
	if (err_alloc_mem(ini_info)) {
		...
		strncpy(ts_test_failed_reason, "tp_initial_failed-",
			ILITEK_TEST_FAILED_REASON_LEN - 1);
	...
	if (ret < 0) {
		...
		strncpy(ts_test_failed_reason, "software_reason-",
			ILITEK_TEST_FAILED_REASON_LEN - 1);
		ilitek_result_init(raw_info, apk);
		ret = -EMP_INI;
		goto out;
	...
	if ((ili_str2hex(ver, ILITEK_MAX_STR_HEX_LEN)) !=
	    (core_mp.protocol_ver >> 8)) {
