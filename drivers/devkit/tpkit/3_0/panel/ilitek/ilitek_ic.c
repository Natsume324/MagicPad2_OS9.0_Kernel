...
int ili_ice_mode_ctrl(bool enable, bool mcu)
{
	...
	if (enable) {
		...
		do {
			if (ilits->wrapper(cmd_open, sizeof(cmd_open), NULL, 0,
					   OFF, OFF) < 0) {
				TS_LOG_ERR("write ice mode cmd error\n");
				atomic_set(&ilits->ice_stat, DISABLE);
				return -EIO;
			}
			if (!ili_ice_mode_read(ilits->chip->pid_addr, &pid,
					       sizeof(u32)))
...
int ili_ic_code_reset(void)
{
	...
	if (!ice)
		if (ili_ice_mode_ctrl(ENABLE, OFF) < 0)
			TS_LOG_ERR(
				"Enable ice mode failed before code reset\n");
...
int ili_ic_whole_reset(void)
{
	...
	if (!ice)
		if (ili_ice_mode_ctrl(ENABLE, OFF) < 0)
			TS_LOG_ERR(
				"Enable ice mode failed before chip reset\n");
...
void ili_ic_set_ddi_reg_onepage(u8 page, u8 reg, u8 data)
{
	...
	if (!ice)
		if (ili_ice_mode_ctrl(ENABLE, OFF) < 0)
			TS_LOG_ERR(
				"Enable ice mode failed before writing ddi reg\n");
	...
	if (!ice)
		if (ili_ice_mode_ctrl(DISABLE, OFF) < 0)
			TS_LOG_ERR(
				"Disable ice mode failed after writing ddi reg\n");
}
...
void ili_ic_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data)
{
	...
	if (!ice)
		if (ili_ice_mode_ctrl(ENABLE, OFF) < 0)
			TS_LOG_ERR(
				"Enable ice mode failed before reading ddi reg\n");
	...
	*data = ilitek_tddi_ic_rd_pack(setreg);
	...
	if (!ice)
		if (ili_ice_mode_ctrl(DISABLE, OFF) < 0)
			TS_LOG_ERR(
				"Disable ice mode failed after reading ddi reg\n");
}
...
void ili_ic_get_pc_counter(int stat)
{
	...
	TS_LOG_ERR(
		"Read counter (addr: 0x%x) = 0x%x, latch (addr: 0x%x) = 0x%x\n",
	...
	if (!ice) {
		if (ili_ice_mode_ctrl(DISABLE, ON) < 0)
			TS_LOG_ERR(
				"Disable ice mode failed while reading pc counter\n");
	}
}
...
out:
	TS_LOG_INFO("Core version = %d.%d.%d.%d\n", buf[1], buf[2], buf[3],
		    buf[4]);
	ilits->chip->core_ver = buf[1] << 24 | buf[2] << 16 | buf[3] << 8 |
				buf[4];
	return ret;
}
...
int ili_ic_get_fw_ver(void)
{
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf,
			   ilits->protocol->fw_ver_len, ON, OFF)) {
...
out:
	...
	TS_LOG_INFO("Firmware MP version = %d.%d.%d.%d\n", buf[5], buf[6],
		    buf[7], buf[8]);
	ilits->chip->fw_ver = buf[1] << 24 | buf[2] << 16 | buf[3] << 8 |
			      buf[4];
	ilits->chip->fw_mp_ver = buf[5] << 24 | buf[6] << 16 | buf[7] << 8 |
				 buf[8];
	return ret;
}
...
int ili_ic_get_protocl_ver(void)
{
	...
	if (ilits->wrapper(&cmd[1], sizeof(u8), buf,
			   ilits->protocol->pro_ver_len, ON, OFF)) {
