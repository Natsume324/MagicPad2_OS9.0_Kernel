...
static int ilitek_tddi_fw_iram_read(u8 *buf, u32 start, int len)
{
	...
	for (cnt = 0, addr = start; cnt < loop; cnt++, addr += limit) {
		...
		TS_LOG_INFO("Reading iram data .... %d%c",
				       ...
				       u32 split_len)
{
	...
	if (split_len % 4 > 0)
		TS_LOG_ERR(
			"Since split_len must be four-aligned, it must be a multiple of four");
	...
	if (split_len != 0) {
		for (addr = start, i = 0; addr < end;
			...
			if (ilits->spi_write_then_read(
				    ...
				    split_len + 5, NULL, 0)) {
				TS_LOG_ERR(
					"Failed to write data via SPI in host download (%x)\n",
					split_len + 5);
				return -EIO;
			}
			if (w_len)
...
static int ilitek_tddi_fw_iram_upgrade(u8 *p_firmware)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if (g_flash_block_info[i].mode == mode &&
		    g_flash_block_info[i].len != 0) {
			TS_LOG_DEBUG(
				"Download %s code from hex 0x%x to IRAM 0x%x, len = 0x%x\n",
				g_flash_block_info[i].name,
				g_flash_block_info[i].start,
				g_flash_block_info[i].mem_start,
				g_flash_block_info[i].len);

#if SPI_DMA_TRANSFER_SPLIT
			if (ilitek_tddi_fw_iram_program(
				    g_flash_block_info[i].mem_start,
				    (fw_ptr + g_flash_block_info[i].start),
				    g_flash_block_info[i].len,
				    SPI_UPGRADE_LEN) < 0)
			...
			if (ilitek_tddi_fw_iram_program(
				    g_flash_block_info[i].mem_start,
				    (fw_ptr + g_flash_block_info[i].start),
				    g_flash_block_info[i].len, 0) < 0)
				TS_LOG_ERR("IRAM program failed\n");
#endif
			crc = calculate_check(
				g_flash_block_info[i].start,
				g_flash_block_info[i].len - crc_len, fw_ptr);
			dma = calc_hw_dma_crc(g_flash_block_info[i].mem_start,
					      g_flash_block_info[i].len -
						      crc_len);
			...
			if (mode != GESTURE) {
				ilitek_tddi_fw_iram_read(
					crc_temp,
					(g_flash_block_info[i].mem_start +
					 g_flash_block_info[i].len - crc_len),
					sizeof(crc_temp));
			...
			if ((crc != dma) || iram_crc_err) {
				TS_LOG_ERR(
					"CRC Error! print iram data with first 16 bytes\n");
	...
	if (ilits->actual_tp_mode != P5_X_FW_GESTURE_MODE) {
		if (ili_reset_ctrl(TP_IC_CODE_RST) < 0) {
			TS_LOG_ERR(
				"TP Code reset failed during iram programming\n");
			ret = -EFW_REST;
			return ret;
		}
	}
...
static int ilitek_fw_calc_file_crc(u8 *p_firmware)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_flash_block_info); i++) {
		...
		data_crc = calculate_check(g_flash_block_info[i].start,
					   g_flash_block_info[i].len - 4,
					   p_firmware);
		file_crc = p_firmware[ex_addr - 3] << 24 |
			   p_firmware[ex_addr - 2] << 16 |
			   p_firmware[ex_addr - 1] << 8 | p_firmware[ex_addr];
...
static int ilitek_tddi_fw_update_block_info(u8 *p_firmware)
{
	...
	g_flash_block_info[AP].mem_start =
		(g_flash_block_info[AP].fix_mem_start != INT_MAX) ?
			g_flash_block_info[AP].fix_mem_start :
			0;
	g_flash_block_info[DATA].mem_start =
		(g_flash_block_info[DATA].fix_mem_start != INT_MAX) ?
			g_flash_block_info[DATA].fix_mem_start :
			DLM_START_ADDRESS;
	g_flash_block_info[TUNING].mem_start =
		(g_flash_block_info[TUNING].fix_mem_start != INT_MAX) ?
			g_flash_block_info[TUNING].fix_mem_start :
			g_flash_block_info[DATA].mem_start +
				g_flash_block_info[DATA].len;
	g_flash_block_info[MP].mem_start =
		(g_flash_block_info[MP].fix_mem_start != INT_MAX) ?
			g_flash_block_info[MP].fix_mem_start :
			0;
	g_flash_block_info[GESTURE].mem_start =
		(g_flash_block_info[GESTURE].fix_mem_start != INT_MAX) ?
			g_flash_block_info[GESTURE].fix_mem_start :
			0;
	g_flash_block_info[TAG].mem_start =
		(g_flash_block_info[TAG].fix_mem_start != INT_MAX) ?
			g_flash_block_info[TAG].fix_mem_start :
			0;
	g_flash_block_info[PARA_BACKUP].mem_start =
		(g_flash_block_info[PARA_BACKUP].fix_mem_start != INT_MAX) ?
			g_flash_block_info[PARA_BACKUP].fix_mem_start :
			0;
	g_flash_block_info[DDI].mem_start =
		(g_flash_block_info[DDI].fix_mem_start != INT_MAX) ?
			g_flash_block_info[DDI].fix_mem_start :
			0;

	/* Parsing gesture info form AP code */
	ges_info_addr = (g_flash_block_info[AP].end + 1 - 60);
	ges_area_section = (p_firmware[ges_info_addr + 3] << 24) +
			   (p_firmware[ges_info_addr + 2] << 16) +
			   (p_firmware[ges_info_addr + 1] << 8) +
			   p_firmware[ges_info_addr];
	g_flash_block_info[GESTURE].mem_start =
		(p_firmware[ges_info_addr + 7] << 24) +
		(p_firmware[ges_info_addr + 6] << 16) +
		(p_firmware[ges_info_addr + 5] << 8) +
		p_firmware[ges_info_addr + 4];
	ap_end = (p_firmware[ges_info_addr + 11] << 24) +
		 (p_firmware[ges_info_addr + 10] << 16) +
		 (p_firmware[ges_info_addr + 9] << 8) +
		 p_firmware[ges_info_addr + 8];

	if (ap_end != g_flash_block_info[GESTURE].mem_start)
		ap_len = ap_end - g_flash_block_info[GESTURE].mem_start + 1;

	ges_fw_start = (p_firmware[ges_info_addr + 15] << 24) +
		       (p_firmware[ges_info_addr + 14] << 16) +
		       (p_firmware[ges_info_addr + 13] << 8) +
		       p_firmware[ges_info_addr + 12];
	ges_fw_end = (p_firmware[ges_info_addr + 19] << 24) +
		     (p_firmware[ges_info_addr + 18] << 16) +
		     (p_firmware[ges_info_addr + 17] << 8) +
		     p_firmware[ges_info_addr + 16];
	...
	/* upgrade mode define */
	g_flash_block_info[DATA].mode = g_flash_block_info[AP].mode =
		g_flash_block_info[TUNING].mode = AP;
	...
	g_touch_fw_data.new_fw_cb =
		(ilits->fw_info[48] << 24) | (ilits->fw_info[49] << 16) |
	...
	TS_LOG_INFO("star_addr = 0x%06X, end_addr = 0x%06X, Block Num = %d\n",
		    g_touch_fw_data.start_addr, g_touch_fw_data.end_addr,
		    g_touch_fw_data.block_number);
...
static int ilitek_tddi_fw_hex_convert(u8 *phex, int size, u8 *p_firmware)
{
	...
	for (; i < size;) {
		...
		if (type == 0x04) {
			...
			if (g_touch_fw_data.hex_tag == BLOCK_TAG_AF)
				num = hex_to_dec(
					(const char *)&phex[i + 9 + 6 + 6], 2);
			else
				num = 0xFF;

			if (num > (FW_BLOCK_INFO_NUM - 1)) {
				TS_LOG_ERR(
					"ERROR! block num is larger than its define %d, %d  \n",
			...
			g_flash_block_info[num].len =
				g_flash_block_info[num].end -
				g_flash_block_info[num].start + 1;
			TS_LOG_DEBUG("Block[%d]: start_addr = %x, end = %x",
				     num, g_flash_block_info[num].start,
				     g_flash_block_info[num].end);
			...
			block++;
			   ...
			   g_touch_fw_data.hex_tag == BLOCK_TAG_AF) {
		...
		if (type == 0x00) {
			...
			for (j = 0, k = 0; j < (len * 2); j += 2, k++)
				p_firmware[addr + k] = hex_to_dec(
					(const char *)&phex[i + 9 + j], 2);
		}
	...
	case REQUEST_FIRMWARE:
		if (request_firmware(&fw, ilits->fw_name, ilits->dev) < 0) {
			...
			if (request_firmware(&fw, ilits->md_fw_rq_path,
					     ilits->dev) < 0) {
				TS_LOG_ERR(
					"Request firmware failed after retry\n");
				ret = -1;
				goto out;
			}
		}
		...
		if (err_alloc_mem(ilits->tp_fw.data)) {
			TS_LOG_ERR(
				"Failed to allocate tp_fw by vmalloc, try again\n");
			ilits->tp_fw.data = vmalloc(fsize);
			if (err_alloc_mem(ilits->tp_fw.data)) {
				TS_LOG_ERR(
					"Failed to allocate tp_fw after retry\n");
				release_firmware(fw);
				ret = -ENOMEM;
				goto out;
			}
		...
		if (fsize * sizeof(*fw->data) > fsize)
			...
			memcpy((u8 *)ilits->tp_fw.data, fw->data,
			       fsize * sizeof(*fw->data));
...
int ili_fw_upgrade(int op)
{
	...
	if (!ilits->boot || ilits->force_fw_update ||
	    err_alloc_mem(p_firmware)) {
		...
		if (err_alloc_mem(p_firmware)) {
			...
			if (err_alloc_mem(p_firmware)) {
				TS_LOG_ERR(
					"Failed to allocate p_firmware memory, %ld\n",
					PTR_ERR(p_firmware));
				ipio_vfree((void **)&p_firmware);
				ret = -ENOMEM;
				goto out;
