...
static void ilitek_tddi_of_property_read_u32_default(struct device_node *np,
						     char *prop_name,
						     u32 *out_value,
						     u32 default_value)
{
...
int ilitek_tddi_prase_ic_config_dts(struct device_node *np)
{
	...
	ilitek_tddi_of_property_read_u32_default(
		np, ILITEK_DTS_SLAVE_ADDR, &ts_dev_data->slave_addr, 0x41);
...
static int ilitek_parse_report_config_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, ILITEK_DTS_FINGER_NUMS,
				   &ilits->finger_nums);
	if (ret) {
	...
	TS_LOG_INFO("x = [%d, %d], y = [%d, %d], finger_nums = %d\n",
		    ilits->min_x, ilits->max_x, ilits->min_y, ilits->max_y,
		    ilits->finger_nums);
...
int ilitek_parse_ic_special_dts(struct device_node *np)
{
	...
	strncpy(ts_dev_data->module_name, str_value, MAX_STR_LEN - 1);
...
int ilitek_parse_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(dts_np, ILITEK_DTS_IC_TYEP,
				   &ts_dev_data->ic_type);
	if (ret < 0) {
	...
	ret = of_property_read_u32(dts_np, ILITEK_DTS_ALGO_ID,
				   &ts_dev_data->algo_id);
	if (ret < 0) {
	...
	ret = of_property_read_u32(dts_np, ILITEK_DTS_IRQ_CFG,
	...
	ret = of_property_read_u32(dts_np, ILITEK_DTS_USE_IC_RES,
				   &ilits->use_ic_res);
	if (ret) {
	...
	ret = of_property_read_u32(dts_np, ILITEK_DTS_HEAD_ROI,
	...
	if (ret) {
		TS_LOG_INFO(
			"get vcc_always_high_timing failed, use default: %d\n",
	...
	if (ret) {
		TS_LOG_INFO(
			"get production_test_formula_compatible failed, use default: %d\n",
	...
	ret = of_property_read_u32(dts_np, ILITEK_DTS_GESTURE,
	...
	ts_dev_data->ts_platform_data->feature_info.wakeup_gesture_enable_info
		.switch_value = ilits->support_gesture;
	TS_LOG_INFO("support_gesture = %d\n", ilits->support_gesture);

	ret = of_property_read_u32(dts_np, ILITEK_DTS_PRESSURE,
	...
	if (ret) {
		TS_LOG_INFO(
			"get support_get_tp_color failed, use default: %d\n",
	...
	if (ret) {
		TS_LOG_INFO(
			"get project_id_length_control failed, use default: %d\n",
	...
	ret = of_property_read_u32(
		dts_np, ILITEK_DTS_RAW_DATA_PRINT,
		(u32 *)&ts_dev_data->is_ic_rawdata_proc_printf);
	if (ret) {
		TS_LOG_INFO(
			"get is_ic_rawdata_proc_printf failed, use default: %d\n",
	...
	if (ret) {
		TS_LOG_INFO(
			"get only_open_once_captest_threshold failed, use default: %d\n",
