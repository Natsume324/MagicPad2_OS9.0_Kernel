...
#else
	error = pinctrl_select_state(ilits->pctrl,
				     ilits->pinctrl_state_reset_high);
	mdelay(1);
	error = pinctrl_select_state(ilits->pctrl,
				     ilits->pinctrl_state_reset_low);
	mdelay(5);
	error = pinctrl_select_state(ilits->pctrl,
				     ilits->pinctrl_state_reset_high);
	mdelay(ilits->rst_edge_delay);
...
int ilitek_tddi_upgrade_get_fw_name(void)
{
	...
	if (p_dev_data->hide_fw_name) {
		ret = snprintf(ilits->fw_name, ILITEK_FILE_NAME_LEN,
			       "ts/%s.img", ilits->project_id);
		...
		snprintf(ilits->fw_name, ILITEK_FILE_NAME_LEN,
			 "ts/%s_%s_%s_%s.img",
...
static int ilitek_tddi_pinctrl_init(void)
{
	...
	ilits->pins_idle =
		pinctrl_lookup_state(ilits->pctrl, ILITEK_DTS_PIN_IDLE);
...
#else
	ilits->pinctrl_state_reset_high =
		pinctrl_lookup_state(ilits->pctrl, PINCTRL_STATE_RESET_HIGH);
	...
	ilits->pinctrl_state_reset_low =
		pinctrl_lookup_state(ilits->pctrl, PINCTRL_STATE_RESET_LOW);
	...
	ilits->pinctrl_state_as_int =
		pinctrl_lookup_state(ilits->pctrl, PINCTRL_STATE_AS_INT);
	...
	ilits->pinctrl_state_int_high =
		pinctrl_lookup_state(ilits->pctrl, PINCTRL_STATE_INT_HIGH);
	...
	ilits->pinctrl_state_int_low =
		pinctrl_lookup_state(ilits->pctrl, PINCTRL_STATE_INT_LOW);
...
static int ilitek_pinctrl_select_idle(void)
{
	...
	if (IS_ERR_OR_NULL(ilits->pctrl) || IS_ERR_OR_NULL(ilits->pins_idle)) {
...
static int ilitek_tddi_chip_detect(struct ts_kit_platform_data *p_data)
{
	...
	if (err_alloc_mem(p_data) || err_alloc_mem(p_data->ts_dev)) {
	...
	if (ret) {
		...
		strncpy(ilits->ts_dev_data->chip_name, TDDI_DEV_ID,
			MAX_STR_LEN - 1);
	}
...
static int ilitek_tddi_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_X, 0, ilits->max_x - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, ilits->max_y - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_PRESSURE, 0, 255, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, 10, 0, 0);

	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0, ilits->max_x - 1,
			     0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0, ilits->max_y - 1,
			     0, 0);
...
int ilitek_tddi_irq_bottom_half(struct ts_cmd_node *in_cmd,
				struct ts_cmd_node *out_cmd)
{
	...
	TS_LOG_DEBUG(
		"report: %d, rst: %d, fw: %d, switch: %d, mp: %d, sleep: %d, esd: %d\n",
...
int ilitek_tddi_fw_update_sd(void)
{
	...
	snprintf(ilits->fw_name, ILITEK_FILE_NAME_LEN, "%s",
		 ILITEK_UPGRADE_FW_SD_NAME);
...
static int
ilitek_tddi_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
...
static int ilitek_tddi_resume(void)
{
	...
	case TS_POWER_OFF_MODE:
		...
		break;
	case TS_GESTURE_MODE:
		if (true == ts_platform_data->feature_info
...
static int ilitek_tddi_get_raw_data(struct ts_rawdata_info *info,
				    struct ts_cmd_node *out_cmd)
{
...
static int ilitek_set_roi_switch(bool roi_enable)
{
	...
	ret = ili_ic_func_ctrl("roi_set",
			       roi_enable ? CMD_ENABLE : CMD_DISABLE);
	if (ret) {
...
ilitek_get_roi_info(struct ts_kit_device_data *dev_data)
{
...
static unsigned char *chip_roi_rawdata(void)
{
	...
	if (!ilits->ts_dev_data->ts_platform_data->feature_info.roi_info
		     .roi_supported ||
	    !(roi_info->roi_switch)) {
	...
	if (wait_for_completion_interruptible_timeout(&ilits->roi_completion,
						      msecs_to_jiffies(30))) {
		 ...
		 */
		memset(ilits->roi_data, 0, sizeof(ilits->roi_data));
...
static int ilitek_tddi_charger_switch(struct ts_charger_info *info)
{
	...
	TS_LOG_INFO(
		"tddi_charger_switch called! info->charger_switch=%d, charger_switch_addr=0x%x\n",
...
static void chip_tddi_chip_touch_switch(void)
{
	...
	case TS_SWITCH_SCENE_5:
		if (TS_SWITCH_TYPE_SCENE !=
		    (ilits->ts_dev_data->touch_switch_flag &
...
struct ts_device_ops ilitek_tddi_ops = {
	...
	.chip_get_capacitance_test_type =
		ilitek_tddi_chip_get_capacitance_test_type,
	...
	.chip_wakeup_gesture_enable_switch =
		ilitek_tddi_wakeup_gesture_enable_switch,
