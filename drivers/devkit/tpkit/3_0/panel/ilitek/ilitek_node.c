...
static int ilitek_debug_node_buff_control(bool open)
{
	...
	if (open) {
		...
		ilits->dbl = (struct debug_buf_list *)kzalloc(
			TR_BUF_LIST_SIZE * sizeof(*ilits->dbl), GFP_KERNEL);
					      ...
					      loff_t *pos)
{
	...
	TS_LOG_DEBUG("f_count= %d, index = %d, mark = %d\n", ilits->dbf,
		     ilits->odi, ilits->dbl[ilits->odi].mark);
	...
	if (ilits->dbl[ilits->odi].mark) {
		...
		for (i = 0; i < need_read_data_len; i++) {
			send_data_len += snprintf(
				tmpbuf + send_data_len, sizeof(tmpbufback),
					    ...
					    loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"[Drive version] = %s\n", DRIVER_VERSION);
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"[Patch] 20200702-0001,esd recovery issue\n");
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"[Patch] esd debug\n");
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len,
			"========================================\n");
...
int ili_get_tp_recore_data(void)
{
	...
	TS_LOG_INFO(
		"addr = 0x%x, len = %d, lndex = 0x%d, fram num = %d, record_case = 0x%x\n",
...
int ili_tp_data_mode_ctrl(u8 *cmd, int length)
{
	...
	case DEBUG_LITE_ROI:
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_ROI, false,
					NULL) < 0) {
			TS_LOG_ERR(
				"Failed to set debug lite roi from gesture mode\n");
			ret = -ENOTTY;
		}
		break;
	case DEBUG_LITE_WINDOW:
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_WINDOW, false,
					NULL) < 0) {
			TS_LOG_ERR(
				"Failed to set debug lite window from gesture mode\n");
			ret = -ENOTTY;
		}
		break;
	case DEBUG_LITE_AREA:
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_AREA, false,
					&cmd[1]) < 0) {
			TS_LOG_ERR(
				"Failed to set debug lite area from gesture mode\n");
			ret = -ENOTTY;
		}
		break;
	default:
				       ...
				       size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ilits->fw_update_stat = (ret < 0) ? FW_UPDATE_FAIL :
						    FW_UPDATE_PASS;
		...
		if (rlen != 0) {
			...
			if (err_alloc_mem(rbuf)) {
				TS_LOG_ERR(
					"Failed to allocate dma_rxbuf, %ld\n",
					PTR_ERR(rbuf));
				kfree(rbuf);
				goto out;
			}
			      ...
			      unsigned long arg)
{
	...
	case ILITEK_IOCTL_I2C_WRITE_DATA:
		...
		if (copy_from_user(szbuf, (u8 *)(uintptr_t)arg,
				   i2c_rw_length)) {
	...
	case ILITEK_IOCTL_TP_CHIP_ID:
		...
		if (copy_to_user((u32 *)(uintptr_t)arg, id_to_user,
				 sizeof(u32) * 3)) {
			TS_LOG_ERR("Failed to copy driver ver to user space\n");
			ret = -ENOTTY;
		}
	...
	case ILITEK_IOCTL_TP_NETLINK_STATUS:
		...
		if (copy_to_user((int *)(uintptr_t)arg, &ilits->netlink,
				 sizeof(int))) {
			TS_LOG_ERR("Failed to copy driver ver to user space\n");
			ret = -ENOTTY;
		}
	...
	case ILITEK_IOCTL_TP_MODE_STATUS:
		TS_LOG_DEBUG("ioctl: current firmware mode = %d",
			     ilits->actual_tp_mode);
		if (copy_to_user((int *)(uintptr_t)arg, &ilits->actual_tp_mode,
				 sizeof(int))) {
			TS_LOG_ERR("Failed to copy driver ver to user space\n");
			ret = -ENOTTY;
		}
	...
	case ILITEK_IOCTL_TP_INTERFACE_TYPE:
		...
		if (copy_to_user((u8 *)(uintptr_t)arg, &if_to_user,
				 sizeof(if_to_user))) {
			TS_LOG_ERR("Failed to copy driver ver to user space\n");
			ret = -ENOTTY;
		}
