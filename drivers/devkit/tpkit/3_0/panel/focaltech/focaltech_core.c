...
#define FTS_REPORT_DATA_ALL_LEN                                    \
	(FTS_TOUCH_DATA_LEN + 2 + FTS_FP_EDGE_TOUCH_DATA_LEN + 2 + \
...
#define focal_print_bulk(buf, len, width) \
	_focal_print_bulk(__func__, buf, len, width)
...
static int
focal_wakeup_gesture_enable_switch(struct ts_wakeup_gesture_enable_info *info);
...
static int focal_get_raw_data_all(struct ts_rawdata_info *info,
				  struct ts_cmd_node *out_cmd);
static int rawdata_proc_focal_printf(struct seq_file *m,
				     ...
				     int range_size, int row_size)
{
	...
	for (index = 0; row_size * index + 2 <
			(2 + row_size * range_size * FTS_8201_MAX_CAP_TEST_NUM);
	     index++) {
		...
		for (index1 = 0; index1 < row_size; index1++) {
			...
			seq_printf(m, "%d,",
				   info->buff[indexsum]); /*print oneline */
		}
		/*index1 = 0;*/
		seq_printf(m, "\n ");
...
int focal_read(u8 *addrs, u16 addr_size, u8 *values, u16 values_size)
{
	...
	if (TS_BUS_I2C == bops->btype) {
		ret = bops->bus_read(addrs, addr_size, values,
				     values_size); /*i2c*/
	} else {
		if (g_focal_dev_data->ic_type == FOCAL_FT8756)
			ret = fts_read_8756(addrs, addr_size, values,
					    values_size); /* spi */
		else
...
static void focal_irq_output(int level)
{
	...
	if (level)
		error = pinctrl_select_state(
			g_focal_pdata->pctrl,
			g_focal_pdata->pinctrl_state_int_high);
	else
		error = pinctrl_select_state(
			g_focal_pdata->pctrl,
			g_focal_pdata->pinctrl_state_int_low);
...
static int focal_reset_output(int level)
{
	...
	if (level)
		error = pinctrl_select_state(
			g_focal_pdata->pctrl,
			g_focal_pdata->pinctrl_state_reset_high);
	else
		error = pinctrl_select_state(
			g_focal_pdata->pctrl,
			g_focal_pdata->pinctrl_state_reset_low);
#else
...
static int focal_gpio_reset(void)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"gpio_reset:gpio direction output to 1 fail, ret=%d\n",
			ret);
		return ret;
	}
	msleep(1);
	...
	if (ret) {
		TS_LOG_ERR(
			"gpio_reset:gpio direction output to 0 fail, ret=%d\n",
			ret);
		return ret;
	}
	msleep(1);
...
int focal_hardware_reset_to_rom_update_model(void)
{
	...
	for (i = 0; i < FTS_RETRY_TIMES; i++) {
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"hardware_reset_to_rom_update_model:%s, ret=%d\n",
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"hardware_reset_to_rom_update_model:write command 0x55 fail, ret=%d\n",
				ret);
			continue;
		}
		...
		if (ret || (g_focal_pdata->chip_id == 0)) {
			TS_LOG_INFO(
				"hardware_reset_to_rom_update_model:chip id read fail, retry=%d\n",
				i);
...
static int _focal_print_bulk(const char *head, u8 *buf, u32 len, u8 width)
{
	...
	if ((0 == len) || (len > PRINT_BULK_MAX_LEN) || (NULL == head) ||
	    (NULL == buf)) {
		return -EIO;
	}
	...
	for (i = 0; (i < len) && (n < PRINT_BULK_BUF_LEN); i++) {
		...
		n += snprintf(&buf_line[n], PRINT_BULK_BUF_LEN - n, "%02x ",
			      buf[i]);
	}
	if (n > 0) {
		TS_LOG_INFO("%s:%s\n", head, buf_line);
...
static int focal_print_diff(u8 *buf, u32 len)
{
	...
	for (i = 0; i < FTS_DIFF_DATA_HEIGHT; i++) {
		for (j = 0;
		     (j < FTS_DIFF_DATA_WIDTH) && (n < PRINT_DIFF_BUF_LEN);
		     j++) {
			n += snprintf(&buf_line[n], PRINT_DIFF_BUF_LEN - n,
				      "%d,",
				      (int)diff[i * FTS_DIFF_DATA_WIDTH + j]);
		}
		TS_LOG_INFO("%s:%s\n", FOCAL_TAG, buf_line);
		n = 0;
	}
...
static int focal_print_dbg(u8 *buf)
{
	...
	focal_print_bulk(&buf[ADDR_DBG_DATA], FTS_DBG_DATA_SIZE,
			 FTS_DIFF_DATA_WIDTH);
	TS_LOG_INFO("%s:--- print dbg frame diff org data ---\n", FOCAL_TAG);
	focal_print_bulk(&buf[ADDR_DIFF_DATA], FTS_DIFF_DATA_SIZE,
			 FTS_DIFF_DATA_WIDTH);
...
static int focal_read_touch_data(struct ts_event *event_data)
{
	...
	u8 buf[FTS_REPORT_DATA_ALL_LEN + 1 + FTS_DBG_DATA_SIZE +
	       FTS_DIFF_DATA_SIZE] = {0};
	...
	if (TS_BUS_SPI == g_focal_dev_data->ts_platform_data->bops->btype) {
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"read_touch_data:read touchdata failed, ret=%d.\n",
				ret);
			return ret;
		}
		if ((buf[2] == FTS_FW_STATE_ERROR) &&
		    (buf[3] == FTS_FW_STATE_ERROR) &&
		    (buf[4] == FTS_FW_STATE_ERROR)) {
			if (true == g_focal_pdata->fw_is_running) {
				ret = focal_fw_update_boot_recovery();
				if (!ret) {
					TS_LOG_INFO(
						"read_touch_data:boot recovery pass\n");
				}
			}
			return -EIO;
		}
		...
		if (g_focal_pdata->enable_edge_touch) {
			memcpy(buf_edge, &buf[FTS_EDGE_DATA_OFF + 1],
			       FTS_FP_EDGE_TOUCH_DATA_LEN);
		}
		if (g_focal_dev_data->ts_platform_data->feature_info.roi_info
			    .roi_switch &&
		    g_focal_dev_data->ts_platform_data->feature_info.roi_info
			    .roi_supported) {
			if (ROI_DATA_READY == buf[FTS_ROI_DATA_OFF - 1]) {
				if (g_focal_dev_data->ic_type == FOCAL_FT8756)
					...
					memcpy(focal_roi_data,
					       &buf[FTS_ROI_DATA_OFF + 1],
					       ROI_DATA_READ_LENGTH);
			} else if (ROI_DATA_NOT_NEET_READ ==
				   buf[FTS_ROI_DATA_OFF - 1]) {
				TS_LOG_DEBUG(
					"read_touch_data: roi data not neet read . \n");
			} else {
				TS_LOG_ERR(
					"read_touch_data: roi data not ready. \n");
			}
		}
	} else {
		...
		if (g_focal_pdata->aft_wxy_enable) {
			...
			fts_edge_each_point_data_len =
				FTS_FP_EDGE_POINT_DATA_SIZE;
		} else {
		...
		if (g_focal_pdata->enable_edge_touch) {
			...
			ret = focal_read(buf_edge, 1, buf_edge,
					 fts_edge_data_len);
			if (ret < 0) {
	...
	for (i = 0; i < FTS_MAX_TOUCH_POINTS; i++) {
		...
		event_data->finger_id[i] = (buf[ADDR_FINGER_ID_POS + offset]) >>
					   4;
		...
		if (g_focal_pdata->enable_edge_touch) {
			event_data->ewx[i] =
				buf_edge[event_data->finger_id[i] *
						 fts_edge_each_point_data_len +
					 ADDR_EDGE_EWX];
			event_data->ewy[i] =
				buf_edge[event_data->finger_id[i] *
			...
			event_data->xer[i] =
				buf_edge[event_data->finger_id[i] *
					 ...
					 ADDR_EDGE_XER];
			if (g_focal_pdata->aft_wxy_enable) {
				event_data->wx[i] = buf_edge
					[event_data->finger_id[i] *
						 fts_edge_each_point_data_len +
					 ADDR_EDGE_WX];
				event_data->wy[i] = buf_edge
					[event_data->finger_id[i] *
						 fts_edge_each_point_data_len +
					 ADDR_EDGE_WY];
			}
			TS_LOG_DEBUG(
				"read_touch_data: edge touch data: ewx=%d, exy=%d, xer=%d, yer=%d, wx = %d, wy = %d\n",
				event_data->ewx[i], event_data->ewy[i],
				event_data->xer[i], event_data->yer[i],
				event_data->wx[i], event_data->wy[i]);
		}
		...
		if (event_data->touch_point_num == 0) {
			if (event_data->touch_event[i] == 0 ||
				...
				TS_LOG_ERR(
					"read_touch_data:abnormal touch data from fw.\n");
				return -EINVAL;
			}
		}
	}
...
static int spi_communicate_check(struct ts_kit_platform_data *focal_pdata)
{
	...
	for (i = 0; i < FTS_DETECT_SPI_RETRY_TIMES; i++) {
		...
		ret = focal_read(&cmd, FTS_COMMON_COMMAND_LENGTH, &reg_val,
				 FTS_COMMON_COMMAND_VALUE);
		if ((reg_val != 0x00) && (reg_val != 0xFF) &&
		    (reg_val !=
		     0xEF)) { /*0x00 0xFF:spi bus error.0xEF:ic error*/
			TS_LOG_INFO("%s:chip id read success, chip id:0x%X\n",
				    __func__, reg_val);
			return NO_ERR;
		} else {
			TS_LOG_ERR(
				"%s:chip id read fail, ret=%d, i=%d, reg_val=%d\n",
...
static void focal_gesture_recovery(void)
{
	...
	retval = focal_write_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS_D1,
				 FTS_REG_GESTURE_OPEN_FF);
	if (retval < 0) {
		TS_LOG_ERR("gesture_recovery, %d: have error\n", __LINE__);
	}

	retval = focal_write_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS_D2,
				 FTS_REG_GESTURE_OPEN_FF);
	if (retval < 0) {
		TS_LOG_ERR("gesture_recovery, %d: have error\n", __LINE__);
	}

	retval = focal_write_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS_D5,
				 FTS_REG_GESTURE_OPEN_FF);
	if (retval < 0) {
		TS_LOG_ERR("gesture_recovery, %d: have error\n", __LINE__);
	}

	retval = focal_write_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS_D6,
				 FTS_REG_GESTURE_OPEN_FF);
	if (retval < 0) {
		TS_LOG_ERR("gesture_recovery, %d: have error\n", __LINE__);
	}

	retval = focal_write_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS_D7,
				 FTS_REG_GESTURE_OPEN_FF);
	if (retval < 0) {
		TS_LOG_ERR("gesture_recovery, %d: have error\n", __LINE__);
	}

	retval = focal_write_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS_D8,
				 FTS_REG_GESTURE_OPEN_FF);
	if (retval < 0) {
		TS_LOG_ERR("gesture_recovery, %d: have error\n", __LINE__);
	}
...
static int focal_esdcheck_chip_id(void)
{
	...
	for (i = 0; i < FTS_ESD_MAX_TIMES; i++) {
		...
		if (err < 0) {
			TS_LOG_ERR(
				"esdcheck_chip_id:[ESD] Read Reg 0xA3 failed ret = %d \n",
				err);
			continue;
		}

		if ((reg_value == chipid_high) ||
		    (reg_value ==
		     FTS_REG_SPECIAL_VALUEl)) { /* Upgrade sometimes can't detect */
			TS_LOG_DEBUG(
				"esdcheck_chip_id:chip id read success, chip id:0x%X, i=%d\n",
				reg_value, i);
			break;
		}
	}
	...
	if (FTS_ESD_MAX_TIMES == i) {
		TS_LOG_ERR(
			"esdcheck_chip_id:chip id read fail, reg_value=%d, i=%d, chipid_high=%d, \n",
...
static int focal_esdcheck_tp_reset(void)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"esdcheck_tp_reset:[ESD] focal hardware reset fail ret = %d \n",
			ret);
#if defined(CONFIG_HONOR_DSM)
		ts_dmd_report(
			DSM_TP_ABNORMAL_DONT_AFFECT_USE_NO,
			"try to client record DSM_TP_ABNORMAL_DONT_AFFECT_USE_NO(%d): focal ESD hardware reset.\n",
			DSM_TP_ABNORMAL_DONT_AFFECT_USE_NO);
#endif
		return ret;
	}
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"esdcheck_tp_reset:[ESD] check chip_id error = %d \n",
			ret);
#if defined(CONFIG_HONOR_DSM)
		ts_dmd_report(
			DSM_TP_ABNORMAL_DONT_AFFECT_USE_NO,
			"try to client record DSM_TP_ABNORMAL_DONT_AFFECT_USE_NO(%d): focal esdcheck chip id.\n",
			DSM_TP_ABNORMAL_DONT_AFFECT_USE_NO);
#endif
		return -EINVAL;
	}
	...
	if ((g_focal_dev_data->easy_wakeup_info.sleep_mode ==
	     TS_GESTURE_MODE) &&
...
static int focal_esdcheck_flow_count(void)
{
	...
	if (err < 0) {
		TS_LOG_ERR(
			"esdcheck_flow_count:[ESD]: Read Reg 0x91 failed ret = %d \n",
			err);
		err = focal_esdcheck_chip_id();
		if (err < 0) {
			TS_LOG_ERR(
				"esdcheck_flow_count:[ESD]: read chip id failed ret = %d \n",
				err);
			return -EINVAL;
		}
	} else {
	...
	if (fts_esdcheck_data.flow_work_hold_cnt >= FTS_CHECK_FLOW_COUNT) {
		TS_LOG_INFO(
			"esdcheck_flow_count:[ESD]: Flow Work Cnt(reg0x91) keep a value for 5 times, need execute TP reset \n");
		return -EINVAL;
	}

	TS_LOG_DEBUG(
		"esdcheck_flow_count:[ESD]: check flow count no need reset ret = %d \n",
		err);
	return 0;
}
...
static int focal_esdcheck_algorithm(void)
{
	...
	if (true == fts_esdcheck_data.intr) {
		TS_LOG_INFO(
			"esdcheck_algorithm:[ESD]: In interrupt state, not check esd, return immediately! \n");
		return 0;
	}
	...
	if (true == fts_esdcheck_data.suspend) {
		TS_LOG_INFO(
			"esdcheck_algorithm:[ESD]: In suspend, not check esd, return immediately!\n");
	...
	if (true == fts_esdcheck_data.boot_upgrade) {
		TS_LOG_INFO(
			"esdcheck_algorithm:[ESD]: In boot upgrade state, not check esd, return immediately! \n");
		return 0;
	}
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"esdcheck_algorithm : focal read FTS_REG_WORKMODE error\n");
	} else if ((reg_value & FTS_REG_WORKMODE_FACTORY_OFFSET) ==
		   FTS_REG_WORKMODE_FACTORY_VALUE) {
		TS_LOG_INFO(
			"esdcheck_algorithm: [ESD]: In factory mode, not check esd, return immediately!!\n");
		return 0;
	}
	...
	if (0 == hardware_reset) {
		TS_LOG_DEBUG(
			"esdcheck_algorithm : check flow count begin hardware_reset = %d\n",
			hardware_reset);
	...
	TS_LOG_DEBUG(
		"esdcheck_algorithm : esd reset begin hardware_reset = %d \n",
		hardware_reset);
	if (hardware_reset < 0) {
				   ...
				   struct ts_event *touch_data)
{
	...
	if (touch_data->pressure[0] == 0) {
		...
		for (i = 0; i < last_data->touch_point; i++) {
			...
			if (last_data->finger_id[i] ==
			    touch_data->finger_id[0]) {
				index = i;
				break;
			}
		}
		...
		if (!check_result) {
			TS_LOG_ERR(
				"set_finger_number:not support BTN_TOUCH report, %s\n",
...
static int focal_easy_wakeup_gesture_report_coordinate(
	...
	unsigned char *buf)
{
	...
	if (reprot_gesture_point_num != 0) {
		for (i = 0; i < reprot_gesture_point_num; i++) {
			...
			y_pos_0 = 4 + (4 * i);
			y_pos_1 = 5 + (4 * i);
			x = (((s16)buf[x_pos_0]) & 0x0F) << 8 |
			    (((s16)buf[x_pos_1]) & 0xFF);
			y = (((s16)buf[y_pos_0]) & 0x0F) << 8 |
			    (((s16)buf[y_pos_1]) & 0xFF);
			TS_LOG_INFO(
				"easy_wakeup_gesture_report_coordinate: Gesture Repot Point %d \n",
				i);
			g_focal_dev_data->easy_wakeup_info.easywake_position[i] =
				(x << 16) | y;
			TS_LOG_INFO("easywake_position[%d] = 0x%04x\n", i,
				    g_focal_dev_data->easy_wakeup_info
					    .easywake_position[i]);
		}
	}
	return retval;
...
static int
focal_check_key_gesture_report(struct ts_fingers *info,
			       ...
			       unsigned char *get_gesture_wakeup_data)
{
	...
	if (0 != reprot_gesture_key_value) {
		...
		__pm_wakeup_event(
			&g_focal_dev_data->ts_platform_data->ts_wake_lock,
			jiffies_to_msecs(5 * HZ));
		mutex_lock(&wrong_touch_lock);
		if (true == g_focal_dev_data->easy_wakeup_info.off_motion_on) {
			g_focal_dev_data->easy_wakeup_info.off_motion_on =
				false;
			retval = focal_easy_wakeup_gesture_report_coordinate(
				reprot_gesture_point_num, info,
				get_gesture_wakeup_data);
			if (retval < 0) {
				mutex_unlock(&wrong_touch_lock);
				TS_LOG_INFO(
					"check_key_gesture_report: report line_coordinate error!retval = %d\n",
					retval);
				return retval;
			}
			info->gesture_wakeup_value = reprot_gesture_key_value;
			TS_LOG_INFO(
				"check_key_gesture_report: info->gesture_wakeup_value = %d\n",
...
static int
focal_read_gestrue_data(struct ts_fingers *info,
			struct ts_easy_wakeup_info *gesture_report_info)
{
	...
	if (gesture_id != DOUBLE_CLICK_WAKEUP) {
		...
		if (len < FTS_REG_GESTURE_READ_OFFSET) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"read_gestrue_data, %d: have error\n",
					__LINE__);
				return RESULT_ERR;
			}
		} else {
			ret = focal_read(buf, 1, buf,
					 FTS_REG_GESTURE_READ_OFFSET);
			if (ret < 0) {
				TS_LOG_ERR(
					"read_gestrue_data, %d: read touchdata failed have error\n",
					__LINE__);
				return RESULT_ERR;
			}
			len = (len - FTS_REG_GESTURE_READ_OFFSET) >
					      (sizeof(buf) -
					       FTS_REG_GESTURE_READ_OFFSET) ?
				      (sizeof(buf) -
				       FTS_REG_GESTURE_READ_OFFSET) :
				      (len - FTS_REG_GESTURE_READ_OFFSET);
			ret = focal_read(
				buf, 0, buf + FTS_REG_GESTURE_READ_OFFSET, len);
			if (ret < 0) {
				TS_LOG_ERR(
					"read_gestrue_data, %d: read touchdata failed have error\n",
					__LINE__);
...
static int focal_check_gesture(struct ts_fingers *info)
{
	...
	struct ts_easy_wakeup_info *gesture_report_info =
		&g_focal_dev_data->easy_wakeup_info;
	if (g_focal_dev_data->ts_platform_data->feature_info
			    .wakeup_gesture_enable_info.switch_value == 0 ||
	...
	for (i = 0; i < GESTURE_EVENT_RETRY_TIME; i++) {
		...
		if (retval < 0) {
			TS_LOG_ERR(
				"check_gesture:read touchdata failed, ret=%d.\n",
				retval);
...
static int focal_palm_esd_report(unsigned int *key, struct ts_event *event_data)
{
	...
	if (TS_BUS_I2C == g_focal_dev_data->ts_platform_data->bops->btype) {
		if (g_focal_dev_data->ts_platform_data->feature_info.roi_info
			...
			if (g_focal_pdata->roi_pkg_num_addr) {
				focal_read_reg(
					(u8)(g_focal_pdata->roi_pkg_num_addr),
					&roi_package_num);
			} else {
				focal_read_reg(FTS_ROI_PACKAGE_NUM,
					       &roi_package_num);
			}
...
static int focal_fw_update_boot_recovery(void)
{
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"fw_update_boot_recovery:read boot state failed, ret=%d.\n",
			ret);
		return ret;
	}
	...
	if (boot_state != fw_download_mode) {
		TS_LOG_INFO(
			"fw_update_boot_recovery:not in download mode,exit\n");
		return -EIO;
	}

	TS_LOG_INFO(
		"fw_update_boot_recovery:abnormal situation,need download fw");

	ret = focal_fw_update_boot_resume();
	if (ret < 0) {
		TS_LOG_ERR(
			"fw_update_boot_recovery:focal_fw_update_boot_resume fail");
		return ret;
	}
...
static int focal_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if (!focal_pdata->focal_device_data->ts_platform_data->hide_plain_id) {
		...
		focal_strncat(info->ic_vendor, focal_pdata->project_id,
			      ic_vendor_size);
	} else {
		if (focal_pdata->allow_print_fw_version) {
			...
			strncpy(info->ic_vendor, focal_pdata->project_id,
				(ic_vendor_size -
				 1) < sizeof(focal_pdata->project_id) ?
					(ic_vendor_size - 1) :
...
static int focal_suspend(void)
{
	...
	if (g_focal_pdata->fts_8201_gesture_value) {
		feature_info =
			&g_focal_dev_data->ts_platform_data->feature_info;
		feature_info->wakeup_gesture_enable_info.switch_value = true;
	}
	if (true == g_focal_dev_data->ts_platform_data->feature_info
		...
		case TS_POWER_OFF_MODE:
			if (FTS_POWER_DOWN == g_focal_pdata->self_ctrl_power &&
				...
				if (error) {
					TS_LOG_ERR(
						"suspend:gpio direction output fail, ret=%d\n",
						error);
				}
				udelay(FT5X46_RESET_KEEP_LOW_TIME);
				focal_power_off();
			} else if ((g_focal_dev_data->ic_type ==
				    FOCAL_FT8201) &&
				   g_focal_pdata->fts_8201_gesture_value) {
				error = focal_reset_output(0);
				if (error)
		...
		case TS_GESTURE_MODE:
			enable_irq_wake(
				g_focal_dev_data->ts_platform_data->irq_id);
		...
		if (FTS_POWER_DOWN == g_focal_pdata->self_ctrl_power &&
			...
			udelay(FT5X46_RESET_KEEP_LOW_TIME);
			focal_power_off();
		} else if (FTS_POWER_DOWN == g_focal_pdata->self_ctrl_reset &&
			   !tskit_pt_station_flag) {
			error = focal_reset_output(0);
			if (error) {
				TS_LOG_ERR(
			...
			if (TS_BUS_SPI ==
			    g_focal_dev_data->ts_platform_data->bops->btype) {
				focal_cs_output(0);
			}
			...
			if (FOCAL_FT8201 != g_focal_dev_data->ic_type) {
				...
				if (error) {
					TS_LOG_ERR(
						"suspend: failed to set seelp model!ret=%d\n",
						error);
				}
			} else {
	...
	if ((FOCAL_FT8719 == g_focal_dev_data->ic_type) &&
	    (!tskit_pt_station_flag)) {
		TS_LOG_INFO(
			"chip_get_capacitance_test_type: set fw_is_running to false \n");
...
static void focal_put_device_outof_easy_wakeup(void)
{
	...
	TS_LOG_DEBUG(
		"put_device_outof_easy_wakeup  info->easy_wakeup_flag =%d\n",
		info->easy_wakeup_flag);
	...
	if ((g_focal_dev_data->ic_type == FOCAL_FT8201) &&
	    g_focal_pdata->fts_8201_gesture_value) {
		TS_LOG_INFO(
			"easy wake up resume not need write Wakeup Gesture Only reg\n");
	} else {
...
static int focal_resume(void)
{
	...
	if (true == g_focal_dev_data->ts_platform_data->feature_info
			    .wakeup_gesture_enable_info.switch_value) {
		switch (g_focal_pdata->focal_device_data->easy_wakeup_info
				.sleep_mode) {
		case TS_POWER_OFF_MODE:
			if (FTS_POWER_DOWN == g_focal_pdata->self_ctrl_power &&
			    !tskit_pt_station_flag) {
				...
				if (ret) {
					TS_LOG_ERR(
						"resume:gpio direction output fail, ret=%d\n",
						ret);
				}
			} else {
				/*exit sleep mode*/
				if ((g_focal_dev_data->ic_type ==
				     FOCAL_FT8201) &&
				    g_focal_pdata->fts_8201_gesture_value) {
					ret = focal_reset_output(1);
					if (ret)
		...
		case TS_GESTURE_MODE:
			...
			if ((g_focal_dev_data->ic_type == FOCAL_FT8201) &&
			    g_focal_pdata->fts_8201_gesture_value) {
				TS_LOG_INFO(
					"gesture resume: here not need reset\n");
			} else {
				ret = focal_gpio_reset();
				if (ret)
...
static int focal_status_resume(void)
{
	...
	if (info->holster_info.holster_supported) {
		retval = focal_set_holster_switch(
...
static int focal_after_resume(void *feature_info)
{
	...
	if (TS_BUS_SPI == g_focal_dev_data->ts_platform_data->bops->btype) {
		...
		if (fw_update_duration_check) {
			if ((time_duration.tv_sec * 1000000 +
			     time_duration.tv_usec) >
			    fw_update_duration_check * 1000) {
#if defined(CONFIG_HONOR_DSM)
				ts_dmd_report(
					DSM_TP_FWUPDATE_OVERTIME_ERROR_NO,
					"error.%s:for spi:fw update time too long. duration(%d.%d) = %ld ums\n",
					__func__, time_duration.tv_sec,
					time_duration.tv_usec,
					time_duration.tv_sec * 1000000 +
						time_duration.tv_usec);
#endif
			}
		}
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"after_resume:fw update from resume fail,ret=%d",
				ret);
			return ret;
		}
		TS_LOG_INFO(
			"after_resume:fw update from resume for sip, check_threshold(%ld ums).duration(%d.%d) = %ld ums\n",
			fw_update_duration_check * 1000, time_duration.tv_sec,
			time_duration.tv_usec,
	...
	for (i = 0; i < FTS_RESUME_MAX_TIMES; i++) {
		...
		if ((reg_val == chipid_high) || (reg_val == FTS_FT86XX_HIGH)) {
			TS_LOG_INFO(
				"after_resume:chip id read success, chip id:0x%X, i=%d\n",
				reg_val, i);
			break;
		} else {
			TS_LOG_ERR(
				"after_resume:chip id read fail, reg_val=%d, chipid_high=%d, i=%d\n",
...
static int focal_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_X, 0, (g_focal_dev_data->x_max - 1),
			     0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, (g_focal_dev_data->y_max - 1),
			     0, 0);
...
static struct ts_glove_info *
focal_get_glove_info(struct ts_kit_device_data *dev_data)
{
...
static int focal_set_glove_switch(u8 glove_switch)
{
	...
	if (!glove_enable_addr) {
		TS_LOG_ERR(
			"set_glove_switch:glove_enable_addr is 0, not support glove.\n");
...
static int focal_charger_switch(struct ts_charger_info *info)
{
	...
	if ((g_focal_dev_data->ic_type == FOCAL_FT8201) ||
		...
		if (info->charger_switch == FTS_IS_CHARGER_OUT) {
			...
			if (retval) {
				TS_LOG_ERR(
					"charger_switch, write charger reg error\n");
				return retval;
			}
...
static struct ts_holster_info *
focal_get_holster_info(struct ts_kit_device_data *dev_data)
{
...
static int focal_get_holster_switch(u8 *holster_switch)
{
	...
	if (!holster_switch_addr) {
		TS_LOG_ERR(
			"%s:holster_switch_addr is 0, not support holster.\n",
			FOCAL_TAG);
...
focal_get_roi_info(struct ts_kit_device_data *dev_data)
{
...
static int focal_set_roi_switch(u8 roi_switch)
{
	...
	if (!roi_switch_addr) {
		TS_LOG_ERR(
			"set_roi_switch:roi_switch_addr is 0, not support roi.\n");
...
static int focal_get_roi_switch(u8 *roi_switch)
{
	...
	if (!roi_switch_addr) {
		TS_LOG_ERR(
			"get_roi_switch:roi_switch_addr is 0, not support roi.\n");
	...
	if (roi_info->roi_switch) {
		...
		if (focal_roi_data_staled) {
			if (!wait_for_completion_interruptible_timeout(
				...
				TS_LOG_INFO(
					"roi_rawdata:wait roi data timeout\n");
				/*
...
static void focal_set_doze_mode(unsigned int soper, unsigned char param)
{
	...
	if (TS_SWITCH_TYPE_DOZE !=
	    (focal_dev_data->touch_switch_flag & TS_SWITCH_TYPE_DOZE)) {
...
static void focal_set_game_mode(unsigned int soper)
{
	...
	if (TS_SWITCH_TYPE_GAME !=
	    (focal_dev_data->touch_switch_flag & TS_SWITCH_TYPE_GAME)) {
...
static int focal_regs_operate(struct ts_regs_info *info)
{
	...
	u8 *reg_value =
		g_focal_dev_data->ts_platform_data->chip_data->reg_values;
...
static int focal_param_init(struct focal_platform_data *focal_pdata)
{
	...
	ret = focal_read_project_id(focal_pdata, focal_pdata->project_id,
				    FTS_PROJECT_ID_LEN - 1);
	if (ret) {
		TS_LOG_ERR(
			"param_init:read project id fail, ret=%d,hope update fw to recovery!\n",
			ret);
	...
	strncpy(g_focal_dev_data->module_name, focal_pdata->vendor_name,
		(FTS_VENDOR_NAME_LEN < (MAX_STR_LEN - 1)) ?
			FTS_VENDOR_NAME_LEN :
			(MAX_STR_LEN - 1));
	...
	if (ret) {
		...
		snprintf(g_focal_dev_data->version_name, MAX_STR_LEN - 1, "%d",
...
static int focal_pinctrl_init(void)
{
	...
	g_focal_pdata->pctrl = devm_pinctrl_get(
		&g_focal_dev_data->ts_platform_data->ts_dev->dev);
	...
	g_focal_pdata->pins_idle =
		pinctrl_lookup_state(g_focal_pdata->pctrl, "idle");
...
#else
	g_focal_pdata->pinctrl_state_reset_high = pinctrl_lookup_state(
		g_focal_pdata->pctrl, PINCTRL_STATE_RESET_HIGH);
	...
	g_focal_pdata->pinctrl_state_reset_low = pinctrl_lookup_state(
		g_focal_pdata->pctrl, PINCTRL_STATE_RESET_LOW);
	...
	g_focal_pdata->pinctrl_state_as_int = pinctrl_lookup_state(
		g_focal_pdata->pctrl, PINCTRL_STATE_AS_INT);
	...
	g_focal_pdata->pinctrl_state_int_high = pinctrl_lookup_state(
		g_focal_pdata->pctrl, PINCTRL_STATE_INT_HIGH);
	...
	g_focal_pdata->pinctrl_state_int_low = pinctrl_lookup_state(
		g_focal_pdata->pctrl, PINCTRL_STATE_INT_LOW);
	...
	error = pinctrl_select_state(g_focal_pdata->pctrl,
				     g_focal_pdata->pinctrl_state_as_int);
	if (error < 0) {
...
static void focal_pinctrl_select_normal(void)
{
	...
	if (pdata->pctrl && pdata->pins_default) {
		...
		if (ret < 0)
			TS_LOG_ERR(
				"pinctrl_select_normal:Set normal pin state error:%d\n",
				ret);
	}
#endif
	return;
...
static int focal_get_fwname(struct focal_platform_data *focal_pdata)
{
	...
	if ((strlen(ts_platform_data->product_name) +
	     strlen(ts_platform_data->chip_data->chip_name) +
...
static int focal_init_chip(void)
{
	...
	if (TS_BUS_I2C == ts_platform_data->bops->btype) {
		...
		strncpy(tee_tui_data.device_name, "focal_spi",
			strlen("focal_spi"));
	...
	if (TS_BUS_SPI ==
	    g_focal_pdata->focal_device_data->ts_platform_data->bops->btype) {
...
static int focal_power_control(void)
{
	...
	if (FTS_SELF_CTRL_POWER == g_focal_pdata->self_ctrl_power) {
		...
		TS_LOG_INFO(
			"power_control, power control by LCD, nothing to do\n");
	}
...
static int focal_chip_detect(struct ts_kit_platform_data *pdata)
{
	...
	if (!pdata) {
		TS_LOG_ERR(
			"chip_detect device, ts_kit_platform_data *data is NULL \n");
		ret = -ENOMEM;
		goto exit;
	} else if (!pdata->ts_dev) {
		TS_LOG_ERR(
			"chip_detect device, ts_kit_platform_data data->ts_dev is NULL \n");
		ret = -ENOMEM;
		goto exit;
	}
	...
	g_focal_dev_data->ts_platform_data->ts_dev->dev.of_node =
		g_focal_dev_data->cnode;
	g_focal_pdata->open_threshold_status = true;

	ret = focal_prase_ic_config_dts(g_focal_dev_data->cnode,
					g_focal_dev_data);
	if (ret < 0) {
	...
	if (FOCAL_FT5X46 == g_focal_dev_data->ic_type) {
		...
		if (ret) {
			TS_LOG_ERR(
				"chip_detect:gpio direction output to fail, ret=%d\n",
				ret);
		}
	...
	if (FOCAL_FT5X46 == g_focal_dev_data->ic_type) {
		...
		if (FOCAL_FT8201 != g_focal_dev_data->ic_type) {
			...
			if (ret) {
				TS_LOG_ERR(
					"chip_detect:hardware reset fail, ret=%d\n",
					ret);
				goto pinctrl_get_err;
			}
		}
...
static int __init ts_kit_module_init(void)
{
	...
	if (!found) {
		TS_LOG_ERR(
			"core_module_init:device tree node not found, name=%s\n",
			FTS_CHIP_NAME);
		ret = -EINVAL;
		goto error_exit;
	}
