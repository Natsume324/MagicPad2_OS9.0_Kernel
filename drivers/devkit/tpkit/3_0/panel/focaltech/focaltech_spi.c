...
static int fts_wait_idle(void)
{
	...
	for (i = 0; i < BUSY_QUERY_TIMEOUT; i++) {
		...
		if (ret >= 0) {
			...
			if ((g_focal_pdata->fw_is_running &&
			     (0x01 == status)) ||
			    (!g_focal_pdata->fw_is_running &&
			     (0x00 == status))) {
				break;
			} else {
				TS_LOG_DEBUG("fw:%d,spi_st:%x",
					     g_focal_pdata->fw_is_running,
					     (int)status);
			}
		}
...
static int fts_boot_write(u8 *cmd, u8 cmdlen, u8 *data, u32 datalen)
{
	...
	if (data && datalen) {
		...
		if (datalen > SPI_BUF_LENGTH - SPI_HEADER_LENGTH) {
			txbuf = kzalloc(datalen + SPI_HEADER_LENGTH,
					GFP_KERNEL);
			if (NULL == txbuf) {
...
int fts_write(u8 *writebuf, u32 writelen)
{
	...
	if (0 == ret) {
		...
		if (cmdlen == writelen)
			...
			ret = fts_boot_write(writebuf, cmdlen,
					     writebuf + cmdlen,
					     writelen - cmdlen);
	}
...
int fts_read_8756(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
	...
	if (ret < 0)
		TS_LOG_ERR("data read addr:%x status:%s,retry:%d,ret:%d\n",
			   cmd[0], i >= SPI_RETRY_NUMBER ? "err" : "fail", i,
			   ret);
err_read:
	if (txlen_need > SPI_BUF_LENGTH_8756) {
		kfree(txbuf);
