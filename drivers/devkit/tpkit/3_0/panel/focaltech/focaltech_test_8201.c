...
static int focal_8201_get_adc_data(struct focal_test_8201 *ft8201, int *data,
...
static int focal_get_noise_format(unsigned int chl_x, unsigned int chl_y,
				  int *data, size_t size);
static int focal_8201_get_short_circuit_data(struct focal_test_8201 *ft8201,
					     int *data, size_t size,
					     unsigned int chl_x,
					     unsigned int chl_y);
static int focal_8201_get_open_test_data(struct focal_test_8201 *ft8201,
					 int *data, unsigned int size,
					 unsigned int chl_x,
					 unsigned int chl_y);
static int focal_8201_get_cb_data_format(struct focal_test_8201 *ft8201,
					 int *data, size_t size,
					 unsigned int chl_x,
					 unsigned int chl_y);
static int focal_8201_get_raw_data_format(struct focal_test_8201 *ft8201,
					  unsigned int chl_x,
					  unsigned int chl_y, int *data,
					  size_t size);
static int focal_8201_get_noise_format(struct focal_test_8201 *ft8201,
...
static int focal_8201_write_reg(struct focal_test_8201 *ft8201, u8 addr,
				u8 value);
static int focal_8201_set_slave_addr(struct focal_test_8201 *ft8201,
				     u8 slave_addr);
...
static int focal_get_cb_data_format(int *data, size_t size, unsigned int chl_x,
				    unsigned int chl_y);
static int focal_8201_raw_data_test(struct focal_test_8201 *ft8201,
				    struct focal_test_params *params,
				    struct focal_test_result **result,
				    char test_num);
static int focal_8201_cb_test(struct focal_test_8201 *ft8201,
			      struct focal_test_params *params,
			      struct focal_test_result **result, char test_num);
static int focal_8201_open_test(struct focal_test_8201 *ft8201,
				struct focal_test_params *params,
				struct focal_test_result **result,
				char test_num);
static int focal_8201_short_circuit_test(struct focal_test_8201 *ft8201,
					 struct focal_test_params *params,
					 struct focal_test_result **result,
					 char test_num);
static int focal_8201_noise_test(struct focal_test_8201 *ft8201,
				 struct focal_test_params *params,
				 struct focal_test_result **result,
				 char test_num);
...
static int focal_cb_uniformity_test(struct focal_test_params *params,
				    ...
				    struct focal_test_result **result,
				    char test_num);
static int focal_8201_cb_increase_test(struct focal_test_8201 *ft8201,
				       struct focal_test_params *params,
				       struct focal_test_result **result,
				       char test_num);
...
static void init_info_packet(struct cs_info_packet *cspt)
{
	...
	cspt->cs_direction =
		CS_UP_DOWN; /*ft8201 is up down cascade connection*/
	cspt->cs_s0_role = CS_S0_AS_MASTER;
	cspt->cs_master_addr =
		I2C_DEFAULT_MASTER_ADDR; /*master ic(up ic) addr,same to device i2c addr 0x70*/
	cspt->cs_slave_addr =
		I2C_DEFAULT_SLAVE_ADDR; /*i2c slave ic(down ic) addr 0x72 */
	cspt->cs_master_tx = 0;
	cspt->cs_master_rx = 0;
	cspt->cs_slave_tx = 0;
	cspt->cs_slave_rx = 0;
...
static int update_cs_info(struct focal_test_8201 *ft8201)
{
	...
	TS_LOG_DEBUG(
		"cs_type = %d, cs_direction = %d, cs_s0_role = %d, cs_master_addr = 0x%x, cs_slave_addr =  0x%x,\
				 cs_master_tx = %d, cs_master_rx = %d, cs_slave_tx = %d, cs_slave_rx = %d. \n",
		ft8201->cs_info->cs_type, ft8201->cs_info->cs_direction,
		ft8201->cs_info->cs_s0_role, ft8201->cs_info->cs_master_addr,
		ft8201->cs_info->cs_slave_addr, ft8201->cs_info->cs_master_tx,
		ft8201->cs_info->cs_master_rx, ft8201->cs_info->cs_slave_tx,
...
static void cs_chip_addr_mgr_init(struct cs_chip_addr_mgr *mgr,
				  struct focal_test_8201 *parent)
{
	if ((!mgr) || (!parent)) {
				     ...
				     u8 slave_addr)
{
	...
	TS_LOG_INFO("Original i2c addr 0x%x ",
		    g_focal_dev_data->ts_platform_data->client->addr);
	TS_LOG_INFO("CurrentAddr 0x%x ",
		    (slave_addr >> 1)); //0x70 to 0x38,or 0x72 to 0x39
	if (g_focal_dev_data->ts_platform_data->client->addr !=
	    (slave_addr >> 1)) {
		g_focal_dev_data->ts_platform_data->client->addr =
			(slave_addr >> 1);
		TS_LOG_INFO("Change i2c addr 0x%x to 0x%x", tmp,
			    g_focal_dev_data->ts_platform_data->client->addr);
	}
	...
	if (ret != ERROR_CODE_OK) {
		...
		TS_LOG_INFO(
			"[focal] focal_read_reg successed, Addr: 0x20, value: 0x%02x ",
			value);
	}
	/*debug end*/
...
static void cat_single_to_one_screen(struct focal_test_8201 *ft8201,
				     int *buffer_master, int *buffer_slave,
				     int *buffer_cated)
{
	...
	if (CS_SINGLE_CHIP == get_cs_type(ft8201->cs_info))
		return;
	...
	if (CS_LEFT_RIGHT == get_cs_direction(ft8201->cs_info)) {
		if (CS_S0_AS_MASTER == get_s0_role(ft8201->cs_info)) {
			...
			left_channel_num = get_master_tx(ft8201->cs_info) *
					   get_master_rx(ft8201->cs_info);
			right_channel_num = get_slave_tx(ft8201->cs_info) *
					    get_slave_rx(ft8201->cs_info);
		} else {
			split_rx = get_slave_rx(ft8201->cs_info);
			buffer_left = buffer_slave;
			buffer_right = buffer_master;
			left_channel_num = get_slave_tx(ft8201->cs_info) *
					   get_slave_rx(ft8201->cs_info);
			right_channel_num = get_master_tx(ft8201->cs_info) *
					    get_master_rx(ft8201->cs_info);
		}
		for (row = 0; row < get_master_tx(ft8201->cs_info); ++row) {
			for (col = 0;
			     col < get_master_rx(ft8201->cs_info) +
					   get_slave_rx(ft8201->cs_info);
			     ++col) {
				relative_rx = col - split_rx;
				total_rx = get_master_rx(ft8201->cs_info) +
					   get_slave_rx(ft8201->cs_info);
				cated_index = row * total_rx + col;
				right_index = row * (total_rx - split_rx) +
					      relative_rx;
				left_index = row * split_rx + col;
				if (col >= split_rx) {
					/*right data*/
					buffer_cated[cated_index] =
						buffer_right[right_index];
				} else {
					/*left data*/
					buffer_cated[cated_index] =
						buffer_left[left_index];
				}
			}
		}
		...
		if (CS_S0_AS_MASTER == get_s0_role(ft8201->cs_info)) {
			...
			up_channel_num = get_master_tx(ft8201->cs_info) *
					 get_master_rx(ft8201->cs_info);
			down_channel_num = get_slave_tx(ft8201->cs_info) *
					   get_slave_rx(ft8201->cs_info);
		} else {
			split_tx = get_slave_tx(ft8201->cs_info);
			buffer_up = buffer_slave;
			buffer_down = buffer_master;
			up_channel_num = get_slave_tx(ft8201->cs_info) *
					 get_slave_rx(ft8201->cs_info);
			down_channel_num = get_master_tx(ft8201->cs_info) *
					   get_master_rx(ft8201->cs_info);
		}
		for (row = 0; row < get_master_tx(ft8201->cs_info) +
					    get_slave_tx(ft8201->cs_info);
		     ++row) {
			for (col = 0; col < get_master_rx(ft8201->cs_info);
			     ++col) {
				relative_tx = row - split_tx;
				total_tx = get_master_tx(ft8201->cs_info) +
					   get_slave_tx(ft8201->cs_info);
				cated_index =
					row * get_master_rx(ft8201->cs_info) +
					col;
				down_index =
					relative_tx *
						get_master_rx(ft8201->cs_info) +
				  ...
				  int size)
{
	...
	for (i = 0; i < size; i++) {
		if (I_IS_OPEN !=
		    i) { /*open cb normal all are 63,so proc printf remain max min average*/
			if (!test_results[i]) {
				TS_LOG_INFO(
					"%s:test result is null, index=%d\n",
					FOCAL_TAG, i);
				continue;
			}
...
static int focal_8201_start_test_tp_new_order(struct focal_test_params *params,
					      struct ts_rawdata_info *info)
{
	...
	/* short test */
	ret = focal_8201_short_circuit_test(
		&ft8201, params, &test_results[cap_test_num], cap_test_num + 1);
	if (ret) {
	...
	/* open test */
	ret = focal_8201_open_test(&ft8201, params, &test_results[cap_test_num],
				   cap_test_num + 1);
	if (ret) {
		tp_cap_test_status = SOFTWARE_REASON;
		strncpy(tp_test_failed_reason, "-software reason",
	...
	/* cb data test */
	ret = focal_8201_cb_test(&ft8201, params, &test_results[cap_test_num],
				 cap_test_num + 1);
	if (ret) {
		tp_cap_test_status = SOFTWARE_REASON;
		strncpy(tp_test_failed_reason, "-software reason",
	...
	/* cb increase test */
	ret = focal_8201_cb_increase_test(
		&ft8201, params, &test_results[cap_test_num], cap_test_num + 1);
	if (ret) {
		tp_cap_test_status = SOFTWARE_REASON;
		strncpy(tp_test_failed_reason, "-software reason",
...
static int focal_8201_start_test_tp(struct focal_test_params *params,
				    struct ts_rawdata_info *info)
{
	unsigned char cap_test_num = 0;
	int i = 0;
					  ...
					  size_t size)
{
	...
	if (chip_has_two_heart(ft8201)) {
		...
		if (ret) {
			TS_LOG_ERR(
				"%s:focal_get_raw_data_format fail, ret=%d\n",
				FT8201_TAG, ret);
		}
		/*scan over,back to devault addr slave_addr*/
		cs_chip_addr_mgr_exit(&mgr);
				    ...
				    char test_num)
{
	...
	if (!ft8201 || !params || !result || !(params->channel_x_num) ||
	...
	for (i = 0; i < 3; i++) {
		...
		ret = focal_8201_get_raw_data_format(ft8201, chl_x, chl_y,
						     test_result->values,
						     test_result->size);
		if (ret) {
					 ...
					 unsigned int chl_x, unsigned int chl_y)
{
	...
	if (chip_has_two_heart(ft8201)) {
		...
		ret = focal_get_cb_data_format(buffer_master,
					       (unsigned int)master_buf_num,
					       (unsigned int)master_tx,
					       (unsigned int)master_rx);
		if (ret) {
		...
		ret = focal_get_cb_data_format(buffer_slave,
					       (unsigned int)slave_buf_num,
		...
		cat_single_to_one_screen(ft8201, buffer_master, buffer_slave,
					 data);
		/*scan over,back to devault addr slave_addr*/
		cs_chip_addr_mgr_exit(&mgr);
	} else {
			      ...
			      struct focal_test_result **result, char test_num)
{
	...
	ret = focal_8201_get_cb_data_format(ft8201, test_result->values,
				    ...
				    char test_num)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"%s:alloc cb uniformity data container fail, ret=%d\n",
			FOCAL_TAG, ret);
		*result = NULL;
		return ret;
	}
	/*default value true*/
	test_result->result = true;
	strncpy(test_result->test_name, "cb_uniformity_test",
		FTS_TEST_NAME_LEN - 1);
	...
	for (i = 0; i < chl_x * chl_y; i++) {
		if (i % chl_x == 0) {
			...
			deviation = focal_abs(srcresult->values[i] -
					      srcresult->values[i - 1]);
			max = focal_get_max(srcresult->values[i],
					    srcresult->values[i - 1]);
			if (0 == max) {
					 ...
					 unsigned int chl_x, unsigned int chl_y)
{
	...
	ret = focal_8201_write_reg(ft8201, REG_GIP,
				   ((reg_value & 0xCF) + 0x20));
	if (ret) {
	...
	/*cb main function*/
	ret = focal_8201_get_cb_data_format(ft8201, open_data,
					    (size_t)read_size, chl_x, chl_y);
	if (ret) {
		TS_LOG_ERR("%s:get cb data fail, ret=%d\n", FT8201_TAG, ret);
		goto free_open_data;
	}
				...
				char test_num)
{
	...
	ret = focal_8201_get_open_test_data(ft8201, test_result->values,
					    read_size, chl_x, chl_y);
	if (ret) {
	...
	for (i = 0; i < (int)test_result->size; i++) {
		...
		if ((panel_data[i] > panel_data_max) ||
			...
			focal_print_test_data("open test failed",
					      i / (int)chl_x, i % (int)chl_x,
					      panel_data_max, panel_data_min,
					      panel_data[i]);
		}
	}
				 ...
				 char test_num)
{
	...
	ret = focal_8201_write_reg(ft8201, REG_CA_FILTER,
				   CA_FILTER_UPPER_LIMIT_VALUE);
	if (ret) {
	...
	ret = focal_8201_write_reg(ft8201, REG_8201_LCD_NOISE_FRAME,
				   framenum); //0x12
	if (ret) {
		TS_LOG_ERR("%s:write reg dats read fail, ret=%d\n", FT8201_TAG,
			   ret);
	...
	/*clean counter*/
	ret = focal_8201_write_reg(ft8201, REG_RAWDATA_LINE_ADDR,
				   RAWDATA_LINE_ADDR_CLEAR);
	if (ret) {
		TS_LOG_ERR("%s:write reg dats read fail, ret=%d\n", FT8201_TAG,
			   ret);
		test_result->result = false;
	...
	ret = focal_8201_write_reg(ft8201, REG_8201_LCD_NOISE_START,
				   START_LCD_NOISE_VALUE);
	if (ret) {
	...
	for (i = 0; i <= TEST_TIMEOUT; i++) {
		...
		ret = focal_read_reg(REG_8201_LCD_NOISE_START,
				     &dataready); //0x00
		if (ret) {
	...
	/*get noise data*/
	ret = focal_8201_get_noise_format(
		ft8201, chl_x, chl_y, test_result->values, test_result->size);
	if (ret) {
		TS_LOG_ERR("%s:get raw data fail, ret=%d\n", FT8201_TAG, ret);
		test_result->result = false;
		goto test_finish;
				       ...
				       char test_num)
{
	...
	/*default value true*/
	test_result->result = true;
	strncpy(test_result->test_name, "cb_increase_test",
		FTS_TEST_NAME_LEN - 1);
	/*every test case must repeat confirm enter factory*/
	ret = focal_8201_enter_factory(ft8201);
	if (ret) {
	...
	ret = focal_write_reg(REG_8201_CB_INCREASE_START,
			      START_CB_INCREASE_VALUE);
	if (ret) {
	...
	ret = focal_8201_get_raw_data_format(
		ft8201, chl_x, chl_y, test_result->values, test_result->size);
	if (ret) {
		TS_LOG_ERR("%s:get raw data fail, ret=%d\n", FT8201_TAG, ret);
					 ...
					 char test_num)
{
	...
	/*default value true*/
	test_result->result = true;
	strncpy(test_result->test_name, "short_circuit_test",
		FTS_TEST_NAME_LEN - 1);
	/*every test case must repeat confirm enter factory*/
	ret = focal_8201_enter_factory(ft8201);
	if (ret) {
	...
	ret = focal_8201_get_short_circuit_data(ft8201, test_result->values,
						adc_data_size, chl_x, chl_y);
	if (ret) {
	...
	for (i = 0; i < adc_data_size; i++) {
		...
		if (short_data_min > test_result->values[i]) {
			...
			strncpy(tp_test_failed_reason, "-panel_reason",
				TP_TEST_FAILED_REASON_LEN);
			focal_print_test_data(
				"short circuit test fail:", i / chl_x,
				i % chl_x, 0, short_data_min,
				test_result->values[i]);
		}
	}
					...
					struct focal_test_params *params)
{
	...
	snprintf(file_name, FTS_THRESHOLD_NAME_LEN, "%s_%s_%s_%s_limits.csv",
		 ...
		 FTS_CHIP_NAME, fts_pdata->project_id, fts_pdata->vendor_name);
	/*fimd csv file path*/
	snprintf(csv_path, sizeof(csv_path), "/odm/etc/firmware/ts/%s",
		 file_name);
	...
	if (fts_pdata->support_point_to_point_test) {
		...
		/* because cb data only have max/min data, so columns is 2/rows is 1 */
		ret = ts_kit_parse_csvfile(csv_path, FTS_CB_TEST_CSV, data_buf,
					   1, 2);
		if (ret) {
	...
	ret = ts_kit_parse_csvfile(csv_path, DTS_SHORT_CIRCUIT_RES_MIN,
				   data_buf, 1, 1);
	if (ret) {
		TS_LOG_ERR("%s: Failed get %s\n", FT8201_TAG,
	...
	if (fts_pdata->support_point_to_point_test) {
		...
		ret = ts_kit_parse_csvfile(csv_file_path,
	...
	ret = ts_kit_parse_csvfile(csv_path, FTS_CB_INCREASE_CSV, data_buf, 1,
				   2);
	if (ret) {
		TS_LOG_ERR("%s: Failed get %s\n", FT8201_TAG,
			   DTS_OPEN_TEST_CB_MIN);
...
int focal_8201_get_raw_data(struct ts_rawdata_info *info,
