...
* Return: data len
***********************************************************************/
static ssize_t focal_proc_write(struct file *filp, const char __user *buff,
				size_t count, loff_t *ppos)
{
	...
	case PROC_READ_DATA:
		if (TS_BUS_SPI ==
		    g_focal_dev_data->ts_platform_data->bops->btype) {
			writelen = count - 1;
			if ((writelen >= MAX_COMMAND_LENGTH) ||
			    (0 == writelen)) {
...
***********************************************************************/
static ssize_t focal_proc_read(struct file *filp, char __user *buff,
			       size_t count, loff_t *ppos)
{
	int ret = 0;
	...
	case PROC_READ_REGISTER:
		...
		if (TS_BUS_SPI ==
		    g_focal_dev_data->ts_platform_data->bops->btype) {
			ret = focal_read(&proc_wbuf[1], proc_wbuf[0], buf,
					 readlen);
		} else {
