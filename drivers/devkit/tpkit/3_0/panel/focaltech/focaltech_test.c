...
static int focal_get_scap_raw_data_format_5x46(int raw_data_type, int *data,
					       size_t size);
static int focal_get_adc_data(int *data, size_t size, unsigned int chl_x,
			      unsigned int chl_y);
static int focal_get_raw_data_format(unsigned int chl_x, unsigned int chl_y,
				     int *data, size_t size);

static void focal_print_test_data(char *msg, int row, int col, int max, int min,
				  int value);
...
static void focal_put_test_result_fornewformat(
	struct focal_test_params *params, struct ts_rawdata_info_new *info,
	struct focal_test_result *test_results[], int size);
...
static int focal_ft8716_lcd_noise_test(struct focal_test_params *params,
				       struct focal_test_result **result,
				       char test_num);
static int focal_row_column_delta_test(struct focal_test_params *params,
				       struct focal_test_result *srcresult,
				       struct focal_test_result **result,
				       char test_num);
static int focal_ft5x46_lcd_noise_test(struct focal_test_params *params,
				       struct focal_test_result **result,
				       char test_num);
			...
			struct ts_rawdata_info *info)
{
	...
	if (g_focal_dev_data->ic_type == FOCAL_FT8756) {
		...
		if (ret) {
			...
			if (test_results[cap_test_num]) {
				TS_LOG_ERR(
					"%s: test_result is NULL cap_test_num is %d\n",
					FOCAL_TAG, cap_test_num);
				strncpy(test_results[cap_test_num]
						->tptestfailedreason,
					"-software reason",
	...
	/*  raw data test */
	ret = focal_raw_data_test(params, &test_results[cap_test_num],
				  cap_test_num + 1);
	if (ret) {
		tp_cap_test_status = SOFTWARE_REASON;
		if (test_results[cap_test_num]) {
	...
	if ((OPEN_ROW_COLUMN_TEST == params->row_column_delta) ||
	    (ROW_COLUMN_DELTA_TEST_POINT_BY_POINT ==
	     params->row_column_delta_test_point_by_point)) {
		ret = focal_row_column_delta_test(
			params, test_results[cap_test_num - 1],
	...
	if (OPEN_LCD_NOISE_DATA_TEST == params->lcd_noise_data) {
		ret = focal_ft5x46_lcd_noise_test(
			params, &test_results[cap_test_num], cap_test_num + 1);
		if (ret) {
			tp_cap_test_status = SOFTWARE_REASON;
			if (test_results[cap_test_num]) {
				TS_LOG_ERR(
	...
	if ((g_focal_dev_data->ic_type == FOCAL_FT5X46) ||
		...
		ret = focal_scap_raw_data_test_5x46(
			params, &test_results[cap_test_num], cap_test_num + 1);
		cap_test_num++;
	}
	...
	if ((FOCAL_FT8716 == g_focal_dev_data->ic_type &&
	    ...
	    FOCAL_FT8756 == g_focal_dev_data->ic_type) {
		ret = focal_ft8716_lcd_noise_test(
			params, &test_results[cap_test_num], cap_test_num + 1);
		if (ret) {
			tp_cap_test_status = SOFTWARE_REASON;
			if (test_results[cap_test_num]) {
	...
	if (FOCAL_FT8716 == g_focal_dev_data->ic_type ||
	    ...
	    FOCAL_FT8756 == g_focal_dev_data->ic_type) {
		/* open test */
		ret = focal_open_test(params, &test_results[cap_test_num],
				      cap_test_num + 1);
		if (ret) {
			tp_cap_test_status = SOFTWARE_REASON;
			if (test_results[cap_test_num]) {
	...
	/*  short test */
	ret = focal_short_circuit_test(params, &test_results[cap_test_num],
				       cap_test_num + 1);
	if (ret) {
		tp_cap_test_status = SOFTWARE_REASON;
		if (test_results[cap_test_num]) {
	...
	if (FOCAL_FT8607 == g_focal_dev_data->ic_type) {
		/* lcd_noise test */
		ret = focal_lcd_noise_test(params, &test_results[cap_test_num],
					   cap_test_num + 1);
		if (ret) {
			tp_cap_test_status = SOFTWARE_REASON;
			if (test_results[cap_test_num]) {
	...
	if (g_focal_dev_data->ts_platform_data->chip_data
		    ->rawdata_newformatflag == TS_RAWDATA_OLDFORMAT) {
		focal_put_test_result(params, info, test_results, cap_test_num);
	} else {
		focal_put_test_result_fornewformat(
			params, (struct ts_rawdata_info_new *)info,
			test_results, cap_test_num);
	}
...
static int focal_read_raw_data(u8 data_type, u8 *raw_data, size_t size)
{
	...
	if (FOCAL_FT8719 == g_focal_dev_data->ic_type ||
		...
		if (true != focal_write_flag) {
			...
			if (ret) {
				TS_LOG_ERR(
					"%s:write data type to ret, ret=%d\n",
					FOCAL_TAG, ret);
				return ret;
			}
		}
	...
	for (i = 0; i < pkg_count; i++) {
		...
		if (TS_BUS_I2C ==
			...
			if (i == 0)
				...
				ret = focal_read_default(
					raw_data + readed_count, pkg_size);
		} else {
			ret = focal_read(&cmd, 1, raw_data + readed_count,
					 pkg_size);
		}
		if (ret) {
...
static int focal_get_Scap_cb_data_5x46(int offset, u8 *data, size_t data_size)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"get_Scap_cb_data:write work mode addr fail, ret=%d\n",
			ret);
		return ret;
	}
	...
	if (data_size < BYTES_PER_TIME) {
		...
		if (ret) {
			TS_LOG_ERR(
				"get_Scap_cb_data:read cb data fail, ret=%d\n",
				ret);
			return ret;
		}
	} else {
...
static int focal_get_channel_form_ic(struct focal_test_params *params)
{
	...
	if (ret) {
		...
		TS_LOG_INFO(
			"%s:channel_x=%u, channel_y=%u,params->key_num=%u\n",
...
static int focal_ft3528_get_adc_data(int *data, size_t size)
{
	...
	for (i = 1; i < packet_num; i++) {
		if ((i == (packet_num - 1)) &&
		    packet_remainder) /* last packet */
			ret = focal_read_default(read_data + BYTES_PER_TIME * i,
						 packet_remainder);
		else
			ret = focal_read_default(read_data + BYTES_PER_TIME * i,
						 BYTES_PER_TIME);
		if (ret) {
			TS_LOG_ERR(
				"get_adc_data:read adc data error,ret = %d\n",
				ret);
			goto exit;
		}
	}
					       ...
					       size_t size)
{
	...
	ret = focal_read_raw_data(raw_data_type, scap_raw_data, raw_data_size);
	if (ret) {
		TS_LOG_ERR("%s:read raw data fail, ret=%d\n", FOCAL_TAG, ret);
		goto exit;
	}
					 ...
					 char test_num)
{
	...
	if (!params || !result || !(params->channel_x_num) ||
	    !(params->channel_y_num)) {
	...
	if (g_focal_dev_data->ts_platform_data->chip_data
		    ->rawdata_newformatflag == TS_RAWDATA_NEWFORMAT) {
	...
	for (i = 0; i < retry; i++) {
		...
		ret = focal_get_scap_raw_data_format_5x46(
			0xAC, test_result->values, raw_data_size);
		if (ret) {
		...
		ret = focal_get_scap_raw_data_format_5x46(
			0xAB, test_result->values + raw_data_size,
			raw_data_size);
		if (ret) {
			TS_LOG_ERR("%s:get raw data fail, ret=%d\n", FOCAL_TAG,
				   ret);
	...
	for (i = 0; i < raw_data_size * 2 - chl_x; i++) {
		if ((test_result->values[i] < raw_data_min) ||
			...
			strncpy(tp_test_failed_reason, "-panel_reason",
				TP_TEST_FAILED_REASON_LEN);
			strncpy(test_result->tptestfailedreason,
				"-panel_reason", TP_TEST_FAILED_REASON_LEN);
			focal_print_test_data("raw data test fail", i / chl_y,
					      i % chl_y, raw_data_max,
					      raw_data_min,
			       ...
			       struct focal_test_result **result, char test_num)
{
	...
	if ((g_focal_dev_data->ic_type == FOCAL_FT5X46) ||
		...
		if (g_focal_dev_data->ic_type == FOCAL_FT5446P03) {
			...
			if (ret) {
				TS_LOG_ERR(
					"%s:write_charnge_fre fail, ret=%d\n",
					FOCAL_TAG, ret);
				test_result->result = false;
				goto test_finish;
			}
	...
	if (RAW_POINT_BY_POINT_JUDGE == params->point_by_point_judge) {
		...
		for (i = 0; i < raw_data_size; i++) {
			if ((test_result->values[i] <
				...
				strncpy(tp_test_failed_reason, "-panel_reason",
					TP_TEST_FAILED_REASON_LEN);
				focal_print_test_data(
					"raw data test fail", i / chl_x,
					i % chl_x,
					params->threshold.raw_data_max_array[i],
					params->threshold.raw_data_min_array[i],
					test_result->values[i]);
				       ...
				       char test_num)
{
	...
	if (!params || !result || (params->channel_x_num) <= 0 ||
	    (params->channel_y_num) <= 0) {
	...
	ret = focal_write_reg(REG_DATA_TYPE,
			      READ_5X46_READ_DIFF); //Switch read diff mode
	if (ret) {
	...
	ret = focal_read_reg(REG_5X46_FIR_EN,
			     &chfir); //confirm hardware filter status
	if (ret) {
	...
	ret = focal_write_reg(REG_5X46_FIR_EN,
			      OPEN_5X46_FIR_SW); //open hardware filter
	if (ret) {
		TS_LOG_ERR("lcd_noise_test:write reg failed, ret=%d\n", ret);
		test_result->result = false;
		goto restore_rawdata_mode;
	...
	ret = focal_write_reg(REG_5X46_CHANG_FRE,
			      FTS_5X46_NOISE_TEST_HIGH_RATE);
	if (ret) {
		TS_LOG_ERR("lcd_noise_test:write reg failed, ret=%d\n", ret);
		test_result->result = false;
	...
	for (index = 0; index < FTS_RETRY_READ_RAWDATA; index++) {
		ret = focal_get_raw_data_format(
			chl_x, chl_y, test_result->values, test_result->size);
		...
		if (index >= 3) {
			TS_LOG_ERR(
				"lcd_noise_test:Get Raw Data failed, ret = %d\n",
				ret);
			test_result->result = false;
	...
	if (release_id >= FTS_5X46_RELEASE_ID) {
		inoisevalue =
			chnoisevalue * 4; //read chNoiseValue is division 4
	} else {
	...
	if (ret) {
		TS_LOG_ERR(
			"lcd_noise_test:enter factory mode failed, ret = %d\n",
			ret);
		test_result->result = false;
		goto close_noise_test;
	}
	...
	lcd_noise_data_max =
		(params->threshold.lcd_noise_max) * inoisevalue / 100;
...
restore_frequency_register:
	ret = focal_write_reg(REG_5X46_CHANG_FRE,
			      0x00); //setting default frequency
	if (ret) {
		TS_LOG_ERR("lcd_noise_test:write reg failed, ret=%d\n", ret);
	}
...
static int focal_get_cb_data_format(int *data, size_t size, unsigned int chl_x,
				    unsigned int chl_y)
{
	unsigned int i = 0;
	int ret = 0;
	...
	if (FOCAL_FT5X46 == g_focal_dev_data->ic_type) {
		...
		for (i = 0; i < size; i++) {
			CurTx = i %
				chl_y; //size = chl_x*chl_y,because  params->key_num =0
			CurRx = i / chl_y;
			 ...
			 struct focal_test_result **result, char test_num)
{
	...
	ret = focal_get_cb_data_format(test_result->values, cb_data_size, chl_x,
				       chl_y);
	if (ret) {
...
static int focal_get_open_test_data(struct focal_test_params *params, int *data,
				    size_t size, unsigned int chl_x,
				    unsigned int chl_y)
{
	unsigned int i = 0;
	int ret = 0;
	...
	if (FOCAL_FT8756 == g_focal_dev_data->ic_type) {
		...
		for (i = 0; i < 50; i++) {
			...
			ret = focal_read_reg(OPEN_TEST_REG_STOP_8756,
					     &StateValue);
			if ((ret >= 0) &&
			    (OPEN_TEST_DATA_READY == StateValue)) {
				break;
			} else {
				TS_LOG_ERR("reg%x=%x,retry:%d\n",
					   OPEN_TEST_REG_STOP_8756, StateValue,
					   i);
			}
		}
		if (i >= 50) {
			   ...
			   struct focal_test_result **result, char test_num)
{
	...
	ret = focal_get_open_test_data(params, test_result->values, read_size,
				       chl_x, chl_y);
	if (ret) {
		...
		for (i = 0; i < test_result->size; i++) {
			if ((test_result->values[i] <
				...
				strncpy(tp_test_failed_reason, "-panel_reason",
					TP_TEST_FAILED_REASON_LEN);
				focal_print_test_data(
					"open test fail", i / chl_x, i % chl_x,
					params->threshold
						.open_test_cb_min_array[i],
					params->threshold
						.open_test_cb_max_array[i],
					test_result->values[i]);
			}
		}
					...
					unsigned int chl_x, unsigned int chl_y)
{
	...
	if (FOCAL_FT8719 == g_focal_dev_data->ic_type ||
		...
		for (i = 0; i < size; i++) {
			adc_data_tmp =
				ADC_EX_RESIS_1_FT8719 /
					(data[i] - ADC_EX_RESIS_2_FT8719) -
				ADC_EX_RESIS_3_FT8719;
			data[i] = adc_data_tmp;
		}
	} else if (FOCAL_FT8756 == g_focal_dev_data->ic_type) {
		for (i = 0; i < size; i++) {
			adc_data_tmp =
				(ADC_EX_RESIS_1_FT8756 +
				 ADC_EX_RESIS_2_FT8756 * ADC_EX_RESIS_3_FT8756 *
					 data[i]) /
...
static int focal_get_short_circuit_data_5x46(int *data, size_t size,
					     unsigned int chl_x,
					     unsigned int chl_y)
{
	int i = 0;
	...
	if (ret) {
		TS_LOG_ERR(
			"get_short_circuit_data:get IC_VERSION fail, ret=%d\n",
			ret);
		goto free_mem;
	}
	ret = focal_enter_factory();
	...
	data_cal = clbdata_mutual > (data_offset + 116) ? clbdata_mutual :
							  (data_offset + 116);
	...
	for (i = 0; i < clbChannelNum; i++) {
		...
		if ((2047 + data_offset) - cg_data <= 0)
			...
			if (IcValue <= 0x05 || IcValue == 0xff) {
				adc_data_tmp =
					(cg_data - data_offset + 410) * 25 /
						(2047 + data_offset - cg_data) -
					3;
			} else {
		...
		if (IcValue <= 0x05 || IcValue == 0xff) {
			...
			if (clbdata_mutual - cc_data <= 0) {
				...
				adc_data_tmp = (cc_data - data_offset - 123) *
						       RSense /
						       (clbdata_mutual -
							cc_data /*temp*/) -
					       2;
					     ...
					     unsigned int chl_y)
{
	...
	if (!read_data) {
		TS_LOG_ERR(
			"get_short_circuit_data:adc_data buffer malloc fail\n");
		return -ENOMEM;
	}
	...
	if (ret) {
		TS_LOG_ERR(
			"get_short_circuit_data:failed to get weak short data,ret = %d",
			ret);
		goto free_mem;
	}
	...
	for (i = 0; i < size; i++) {
		...
		read_data[i] =
			((((read_data[i] - offset + SHORT_TEST_OFFSET_RES) *
				...
				char test_num)
{
	...
	lcd_noise_max =
		(params->threshold.lcd_noise_max) * lcd_value * 32 / 100;
	/* data compare */
	for (i = 0; i < lcd_data_size; i++) {
		if ((test_result->values[i] > lcd_noise_max) ||
		    (test_result->values[i] < lcd_noise_min)) {
			tp_cap_test_status = PANEL_REASON;
			strncpy(tp_test_failed_reason, "-panel_reason",
				TP_TEST_FAILED_REASON_LEN);
			strncpy(test_result->tptestfailedreason,
				       ...
				       char test_num)
{
	...
	if (FOCAL_FT8719 == g_focal_dev_data->ic_type ||
		...
		if (ret) {
			TS_LOG_ERR(
				"lcd_noise_test:write reg dats read fail, ret=%d\n",
				ret);
			goto test_finish;
		}
	}
	...
	if (FOCAL_FT8719 == g_focal_dev_data->ic_type) {
		...
		for (i = 0; i <= TEST_TIMEOUT; i++) {
			msleep(FTS_LCD_NOISE_TEST_DELAY_TIME);
			ret = focal_read_reg(REG_8716_LCD_NOISE_START,
					     &dataready);
			if (ret) {
				TS_LOG_ERR(
					"lcd_noise_test:read reg dats fail, ret=%d\n",
					ret);
				goto test_finish;
			}
		...
		for (i = 0; i <= TEST_TIMEOUT; i++) {
			msleep(FTS_LCD_NOISE_TEST_DELAY_TIME);
			ret = focal_read_reg(REG_LCD_NOISE_DATA_READY,
					     &dataready);
			if (ret) {
				TS_LOG_ERR(
					"lcd_noise_test:read reg dats fail, ret=%d\n",
			...
			if (0x00 == (dataready >> 7)) {
				...
				if (ret) {
					TS_LOG_ERR(
						"lcd_noise_test:read reg fail, ret=%d\n",
						ret);
					goto test_finish;
				}
				if (dataready == 0x00) {
	...
	if (FOCAL_FT8719 == g_focal_dev_data->ic_type ||
		...
		if (ret) {
			TS_LOG_ERR(
				"lcd_noise_test:enter work mode fail, ret=%d\n",
				ret);
			test_result->result = false;
			goto test_finish;
		}
		msleep(100);
		ret = focal_read_reg(REG_8716_LCD_NOISE_VALUE,
				     &lcd_value); //0x80
		if (ret) {
			TS_LOG_ERR(
				"lcd_noise_test:reed reg dats fail, ret=%d\n",
				ret);
			goto test_finish;
		}
		TS_LOG_INFO("get lcd  reg value: %d\n", lcd_value);
		...
		if (ret) {
			TS_LOG_ERR(
				"lcd_noise_test:enter factory mode fail, ret=%d\n",
				ret);
			test_result->result = false;
			goto test_finish;
		}
	}
	lcd_noise_max = (params->threshold.lcd_noise_max) * lcd_value *
			LEFT_SHIFT_FOUR / PERCENTAGE_DATA;
	/* data compare */
	for (i = 0; i < lcd_data_size; i++) {
		if ((test_result->values[i] > lcd_noise_max) ||
		    (test_result->values[i] < lcd_noise_min)) {
			...
			test_result->result = false;
			TS_LOG_ERR(
				"lcd_noise_test:LCD Noise test failured,data[%d] = %d",
				    ...
				    char test_num)
{
	...
	while (judge_count > 0) {
		...
		test_result->typeindex = RAW_DATA_TYPE_OPENSHORT;
		test_result->result = true;
		strncpy(test_result->test_name, "short_test",
			FTS_TEST_NAME_LEN - 1);
		...
		if (g_focal_dev_data->ic_type == FOCAL_FT5X46)
			ret = focal_get_short_circuit_data_5x46(
				test_result->values, adc_data_size, chl_x,
				chl_y);
		else if (g_focal_dev_data->ic_type == FOCAL_FT5446P03)
			ret = focal_get_short_circuit_data_3528(
				test_result->values, adc_data_size, chl_x,
				chl_y);
		else
		...
		if (SHORT_TEST_POINT_BY_POINT ==
		    params->short_test_point_by_point) {
			...
			for (i = 0; i < adc_data_size; i++) {
				if ((test_result->values[i] <
					...
					if (FOCAL_FT8006U !=
						...
						focal_print_test_data(
							"short circuit test fail",
							i / chl_x, i % chl_x, 0,
							params->threshold
								.short_test_min_array
									[i],
							test_result->values[i]);
					}
				}
				       ...
				       char test_num)
{
	...
	if (ROW_COLUMN_DELTA_TEST_POINT_BY_POINT ==
	    params->row_column_delta_test_point_by_point) {
		for (i = 0; i < adc_data_size * 2; i++) {
			if (test_result->values[i] >
				...
				focal_print_test_data(
					"row column delta test point by point fail",
					i / chl_x, i % chl_x,
					params->threshold
						.row_column_delta_max_array[i],
					0, test_result->values[i]);
			}
		}
	} else {
...
static int focal_adc_scan(void)
{
	...
	for (i = 0; i < scan_query_times; i++) {
		if (FOCAL_FT5X46 == g_focal_dev_data->ic_type) {
			ret = focal_read_reg(REG_5X46_ADC_SCAN_STATUS,
					     &reg_val);
		} else {
	...
	for (i = 0; i < pkg_count; i++) {
		...
		if (TS_BUS_I2C ==
			...
			if (i == 0) {
				...
				ret = focal_read_default(
					adc_data + readed_count, pkg_size);
			}
		} else {
			ret = focal_read(cmd, 1, adc_data + readed_count,
					 pkg_size);
		}
...
static int focal_get_statistics_data(int *data, size_t data_size, char *result,
				     size_t res_size)
{
	int avg = 0;
	int min = 0;
	...
	if (data_size <= 0 || res_size <= 0) {
		TS_LOG_ERR("%s:%s, data_size=%lu, res_size=%lu\n", FOCAL_TAG,
			   "input parameter is illega", data_size, res_size);
		return -EINVAL;
	}
	...
	struct focal_test_result *test_results[], int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		pts_node->values = kzalloc(test_results[i]->size * sizeof(int),
					   GFP_KERNEL);
		if (!pts_node->values) {
		...
		memcpy(pts_node->values, test_results[i]->values,
		       test_results[i]->size * sizeof(int));
		...
		strncpy(pts_node->test_name, test_results[i]->test_name,
			FTS_TEST_NAME_LEN - 1);
		strncpy(pts_node->statistics_data,
			test_results[i]->statistics_data,
			FTS_STATISTICS_DATA_LEN - 1);
		strncpy(pts_node->tptestfailedreason,
			test_results[i]->tptestfailedreason,
				  ...
				  int size)
{
	...
	{
		focal_strncat(info->result, tp_test_failed_reason,
			      TP_TEST_FAILED_REASON_LEN);
	}
 ...
 *
 */
int focal_get_debug_data_(int data_type, int *data, size_t size, int chl_x,
			  int chl_y)
{
	int ret = 0;
				   ...
				   struct focal_test_params *params)
{
	...
	if (fts_pdata->focal_device_data->hide_fw_name)
		snprintf(file_name, FTS_THRESHOLD_NAME_LEN, "%s_limits.csv",
			 fts_pdata->project_id);
	else
	...
	if (CB_TEST_POINT_BY_POINT == params->cb_test_point_by_point) {
		ret = ts_kit_parse_csvfile(file_path, DTS_CB_TEST_MIN_ARRAY,
					   threshold->cb_test_min_array,
					   params->channel_y_num,
					   params->channel_x_num);
		if (ret) {
			TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
		...
		ret = ts_kit_parse_csvfile(file_path, DTS_CB_TEST_MAX_ARRAY,
		...
		ret = ts_kit_parse_csvfile(file_path, FTS_CB_TEST_CSV, data_buf,
					   1, 2);
		if (ret) {
			TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
				    FTS_CB_TEST_CSV);
	...
	ret = ts_kit_parse_csvfile(file_path, FTS_SCAP_RAW_FATA_CSV, data_buf,
				   1, 2);
	if (ret) {
		TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
	...
	if (SHORT_TEST_POINT_BY_POINT == params->short_test_point_by_point) {
		ret = ts_kit_parse_csvfile(file_path,
					   DTS_SHORT_CIRCUIT_RES_MIN_ARRAY,
		...
		ret = ts_kit_parse_csvfile(file_path, DTS_SHORT_CIRCUIT_RES_MIN,
					   data_buf, 1, 1);
		if (ret) {
			TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
	...
	if (OPEN_TEST_POINT_BY_POINT == params->open_test_cb_point_by_point) {
		ret = ts_kit_parse_csvfile(file_path,
					   ...
					   threshold->open_test_cb_min_array,
					   params->channel_y_num,
					   params->channel_x_num);
		if (ret) {
			TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
		...
		ret = ts_kit_parse_csvfile(file_path,
					   ...
					   threshold->open_test_cb_max_array,
					   params->channel_y_num,
					   params->channel_x_num);
		if (ret) {
			TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
	...
	if (ROW_COLUMN_DELTA_TEST_POINT_BY_POINT ==
	    params->row_column_delta_test_point_by_point) {
		memset(threshold->row_column_delta_max_array, 0,
		       sizeof(threshold->row_column_delta_max_array));
		ret = ts_kit_parse_csvfile(
			file_path, DTS_ROW_COLUMN_DELTA_MAX_ARRAY,
		...
		ret = ts_kit_parse_csvfile(file_path, DTS_ROW_COLUMN_DELTA_TEST,
					   data_buf, 1, 1);
		if (ret) {
			TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
	...
	ret = ts_kit_parse_csvfile(file_path, DTS_LCD_NOISE_MAX, data_buf, 1,
				   1);
	if (ret) {
		TS_LOG_INFO("%s: Failed get %s\n", FOCAL_TAG,
			    DTS_LCD_NOISE_MAX);
	...
	ret = ts_kit_parse_csvfile(file_path, DTS_RAW_DATA_MIN_ARRAY,
	...
	ret = ts_kit_parse_csvfile(file_path, DTS_RAW_DATA_MAX_ARRAY,
	...
	TS_LOG_INFO(
		...
		FOCAL_TAG, "cb test thresholds", "raw_data_min",
		threshold->raw_data_min, "raw_data_max",
		threshold->raw_data_max, "cb_test_min", threshold->cb_test_min,
		"cb_test_max", threshold->cb_test_max, "lcd_noise_max",
		threshold->lcd_noise_max, "short_circuit_min",
		threshold->short_circuit_min, "row_column_delta_max",
		threshold->row_column_delta_max, "scap_raw_data_min",
		threshold->scap_raw_data_min, "scap_raw_data_max",
		threshold->scap_raw_data_max);
}
			       ...
			       struct ts_rawdata_info *info)
{
	...
	if (ret) {
		...
		focal_strncat(info->result, "software reason ",
			      TS_RAWDATA_RESULT_MAX);
		return ret;
	} else {
...
static int focal_init_test_prm_fornewformat(struct focal_platform_data *pdata,
					    struct focal_test_params *params,
					    struct ts_rawdata_info_new *info)
{
	...
	if (ret) {
		...
		focal_strncat(info->i2cerrinfo, "software reason ",
...
int focal_get_raw_dataNewformat(struct ts_rawdata_info *info,
				struct ts_cmd_node *out_cmd)
{
	...
	if (FOCAL_FT8719 == g_focal_dev_data->ic_type) {
		for (i = 0; i < MAX_RETRY_TIMES; i++) {
			ret = focal_read_reg(FTS_REG_VALUE_0XA0,
					     &judge_ic_type);
	...
	if (FOCAL_FT8719 == g_focal_dev_data->ic_type ||
		...
		for (i = 0; i < MAX_RETRY_TIMES; i++) {
			ret = focal_read_reg(
				REG_8716_LCD_NOISE_VALUE,
				&g_focal_pdata->lcd_noise_threshold);
			if (ERROR_CODE_OK != ret) {
				TS_LOG_ERR(
					"read Reg REG_8716_LCD_NOISE_VALUE fail\n");
				goto free_mem;
			}
			TS_LOG_INFO("touch thr is %d\n",
				    g_focal_pdata->lcd_noise_threshold);
			if ((0x01 != g_focal_pdata->lcd_noise_threshold &&
			     0xFF != g_focal_pdata->lcd_noise_threshold)) {
				break;
			}

			if (i >= MAX_RETRY_TIMES) {
				TS_LOG_ERR(
					"%s get wrong lcd_noise_threshold = %u\n",
					FOCAL_TAG,
					g_focal_pdata->lcd_noise_threshold);
			}
		}
			 ...
			 struct ts_cmd_node *out_cmd)
{
	...
	case READ_DIFF_DATA:
		...
		ret = focal_get_debug_data_(1, debug_data, data_size, chl_x,
					    chl_y);
		break;
	case READ_RAW_DATA:
		TS_LOG_INFO("%s:read raw data\n", FOCAL_TAG);
		ret = focal_get_debug_data_(0, debug_data, data_size, chl_x,
					    chl_y);
		break;
	case READ_CB_DATA:
		TS_LOG_INFO("%s:read CB data\n", FOCAL_TAG);
		ret = focal_get_cb_debug_data_(debug_data, data_size, chl_x,
					       chl_y);
		break;
	default:
		ret = -EINVAL;
...
int focal_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
	...
	case TS_ACTION_READ:
		strncpy(info->tp_test_type, dev_data->tp_test_type,
