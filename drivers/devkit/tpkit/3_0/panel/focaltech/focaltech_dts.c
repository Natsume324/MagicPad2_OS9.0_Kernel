...
int focal_get_vendor_compatible_name(const char *project_id, char *comp_name,
				     size_t size)
{
	int ret = 0;
...
int focal_get_vendor_name_from_dts(char *project_id, char *vendor_name,
				   size_t size)
{
	int ret = 0;
	...
	ret = focal_get_vendor_compatible_name(project_id, comp_name,
					       FTS_VENDOR_COMP_NAME_LEN);
	if (ret) {
		TS_LOG_ERR(
			"get_vendor_name_from_dts:get vendor compatible name fail\n");
		return ret;
	}
	...
	if (!ret) {
		...
		TS_LOG_ERR(
			"get_vendor_name_from_dts:find producer in dts fail, ret=%d\n",
			ret);
		return ret;
	}
	...
	if (!ret) {
		snprintf(project_id, FTS_PROJECT_ID_LEN - 1, "%s",
			 new_project_id);
	}
...
static void focal_of_property_read_u32_default(struct device_node *np,
					       char *prop_name, u32 *out_value,
					       u32 default_value)
{
	int ret = 0;
	...
	if (ret) {
		TS_LOG_INFO(
			"of_property_read_u32_default:%s not set in dts, use default\n",
			prop_name);
		*out_value = default_value;
	}
}

static void focal_of_property_read_u16_default(struct device_node *np,
					       char *prop_name, u16 *out_value,
					       u16 default_value)
{
	int ret = 0;
	u32 value = 0;
...
static void focal_of_property_read_u8_default(struct device_node *np,
					      char *prop_name, u8 *out_value,
					      u8 default_value)
{
	int ret = 0;
	u32 value = 0;
			      ...
			      struct ts_kit_device_data *dev_data)
{
	...
	TS_LOG_INFO("prase_ic_config_dts: %s= %d\n", FTS_POWER_SELF_CTRL,
		    g_focal_pdata->self_ctrl_power);

	return NO_ERR;
}
static void focal_prase_delay_config_dts(struct device_node *np,
					 struct focal_platform_data *pdata)
{
	...
	TS_LOG_INFO(
		"prase_delay_config_dts:%s=%d, %s=%d, %s=%d, %s=%d, %s=%d\n",
		"hard_reset_delay", delay_time->hard_reset_delay,
		"reboot_delay", delay_time->reboot_delay, "erase_min_delay",
		delay_time->erase_min_delay, "erase_query_delay",
		delay_time->erase_query_delay, "calc_crc_delay",
		delay_time->calc_crc_delay);

	TS_LOG_INFO("prase_delay_config_dts:%s=%d, %s=%d, %s=%d, %s=%d",
		    "erase_flash_query_times",
		    delay_time->erase_flash_query_times, "read_ecc_query_times",
		    delay_time->read_ecc_query_times, "write_flash_query_times",
		    delay_time->write_flash_query_times, "upgrade_loop_times",
		    delay_time->upgrade_loop_times);
}
...
static int focal_get_lcd_panel_info(void)
{
	...
	strncpy(focal_pdata->lcd_panel_info, lcd_type,
		    ...
		    struct focal_platform_data *focal_pdata)
{
	...
	ret = of_property_read_u32(np, FTS_PRAM_PROJECTID_ADDR,
				   &focal_pdata->pram_projectid_addr);
	if (ret) {
		focal_pdata->pram_projectid_addr = FTS_BOOT_PROJ_CODE_ADDR2;
		TS_LOG_INFO(
			"parse_dts:get pram_projectid_addr from dts failed ,use default pram addr\n");
	}
	/* get tp color flag */
	focal_pdata->support_get_tp_color =
		of_property_read_bool(np, "support_get_tp_color");
	...
	if (ret) {
		TS_LOG_INFO(
			"parse_dts:get tp color in reg failed, use def val: 0\n");
	...
	ret = of_property_read_u32(np, FTS_FW_NEED_DISTINGUISH_LCD,
				   &focal_pdata->need_distinguish_lcd);
	if (ret) {
		focal_pdata->need_distinguish_lcd = 0;
		TS_LOG_INFO(
			"parse_dts: get need_distinguish_lcd from dts failed, use default(0)\n");
	}
	...
	ret = of_property_read_u32(np, FTS_FW_ONLY_DEPEND_ON_LCD,
				   &focal_pdata->fw_only_depend_on_lcd);
	if (ret) {
		focal_pdata->fw_only_depend_on_lcd = 0;
		TS_LOG_INFO(
			"parse_dts: get fw_only_depend_on_lcd from dts failed, use default(0)\n");
	}

	ret = of_property_read_u32(
		np, FTS_OPEN_ONCE_THRESHOLD,
		&focal_pdata->only_open_once_captest_threshold);
	if (ret) {
		focal_pdata->only_open_once_captest_threshold = 0;
		TS_LOG_INFO(
			"parse_dts: get only_open_once_captest_threshold from dts failed, use default(0)\n");
	}
	...
	if (focal_pdata->need_distinguish_lcd) {
		...
		ret = of_property_read_u32(np, FTS_HIDE_PLAIN_LCD_LOG,
					   &focal_pdata->hide_plain_lcd_log);
		if (ret) {
			focal_pdata->hide_plain_lcd_log = 0;
			TS_LOG_INFO(
				"parse_dts: get hide_plain_lcd_log from dts failed, use default(0)\n");
		}
	...
	if (focal_pdata->focal_device_data->touch_switch_flag) {
		if (TS_SWITCH_TYPE_GAME ==
		    (focal_pdata->focal_device_data->touch_switch_flag &
		     TS_SWITCH_TYPE_GAME)) {
			ret = of_property_read_u32(
				np, FTS_TOUCH_SWITCH_GAME_REG, &value);
			if (ret) {
				TS_LOG_INFO(
					"parse_dts get touch_switch_game_reg from dts failed, use default(0).\n");
				focal_pdata->touch_switch_game_reg = 0;
			} else {
				focal_pdata->touch_switch_game_reg =
					(u8)(value & 0xFF);
				TS_LOG_INFO(
					"parse_dts get touch_switch_game_reg from dts succ, use value(0x%x).\n",
					focal_pdata->touch_switch_game_reg);
			}
		}
		...
		if (TS_SWITCH_TYPE_SCENE ==
			...
			value = 0;
			ret = of_property_read_u32(
				np, FTS_TOUCH_SWITCH_SCENE_REG, &value);
			if (ret) {
				TS_LOG_INFO(
					"parse_dts get touch_switch_scene_reg from dts failed, use default(0).\n");
				focal_pdata->touch_switch_scene_reg = 0;
			} else {
				focal_pdata->touch_switch_scene_reg =
					(u8)(value & 0xFF);
				TS_LOG_INFO(
					"parse_dts get touch_switch_scene_reg from dts succ, use value(0x%x).\n",
					focal_pdata->touch_switch_scene_reg);
			}
		}
	}

	ret = of_property_read_u32(np, FTS_USE_PINCTRL,
				   &focal_pdata->fts_use_pinctrl);
	if (ret) {
		TS_LOG_INFO(
			"parse_dts get use_pinctrl from dts failed, use default(0).\n");
		focal_pdata->fts_use_pinctrl = 0;
	}

	ret = of_property_read_u32(np, FTS_FW_UPDATE_DURATION_CHECK,
				   &focal_pdata->fw_update_duration_check);
	if (ret) {
		TS_LOG_INFO(
			"parse_dts get fw_update_duration_check from dts unsucceed, use default(0).\n");
		focal_pdata->fw_update_duration_check = 0;
	} else {
		TS_LOG_INFO(
			"parse_dts get fw_update_duration_check from dts succeed, use cfg %d ms.\n",
			focal_pdata->fw_update_duration_check);
	}
	...
	if (ret) {
		TS_LOG_INFO(
			"parse_dts get read_debug_reg_and_differ from dts unsucceed.\n");
	...
	ret = of_property_read_u32(np, "aft_wxy_enable",
				   &focal_pdata->aft_wxy_enable);
	if (ret) {
		TS_LOG_INFO(
			"parse_dts get aft_wxy_enable from dts failed, use default(0).\n");
	...
	focal_of_property_read_u32_default(np, FTS_CHARGER_SUPPORTED, &tmpval,
					   0);
	...
	if (charger_info->charger_supported) {
		tmpval = 0;
		focal_of_property_read_u32_default(np, FTS_CHARGER_SWITCH_ADDR,
						   &tmpval, 0);
	...
	if (ret) {
		TS_LOG_INFO(
			"parse_dts capacitance do not support point to point test.\n");
	...
	focal_of_property_read_u32_default(
		np, FTS_PROJECTID_LEN_CTRL_FLAG,
	...
	focal_of_property_read_u32_default(
		np, FTS_GESTURE_SUPPORTED,
		(u32 *)&dev_data->ts_platform_data->feature_info
			.wakeup_gesture_enable_info.switch_value,
		0);
	TS_LOG_INFO("parse_dts:gesture_supported = %u\n",
		    dev_data->ts_platform_data->feature_info
			    .wakeup_gesture_enable_info.switch_value);
				  ...
				  struct focal_test_params *params)
{
	...
	focal_of_property_read_u32_default(
		np, FTS_ROW_COLUMN_DELTA_TEST_POINT_BY_POINT,
	...
	focal_of_property_read_u32_default(np, FTS_OPEN_TEST_POINT_BY_POINT,
					   &params->open_test_cb_point_by_point,
					   0);
	...
	TS_LOG_INFO("%s:%s=%d, %s=%d, %s=%d, %s=%d, %s=%d, %s=%d,\n", FOCAL_TAG,
		    ...
		    "open_test_cb_point_by_point",
		    params->open_test_cb_point_by_point,
		    "short_test_point_by_point",
		    params->short_test_point_by_point);
}

static void focal_prase_test_threshold(struct device_node *np,
				       struct focal_test_threshold *threshold)
{
	...
	TS_LOG_INFO("%s:%s:%s=%d, %s=%d, %s=%d, %s=%d, %s=%d, %s=%d, %s=%d\n",
		    ...
		    threshold->raw_data_max, "cb_test_min",
		    threshold->cb_test_min, "cb_test_max",
		    threshold->cb_test_max, "open_test_cb_min",
		    threshold->open_test_cb_min, "lcd_noise_max",
		    threshold->lcd_noise_max, "short_circuit_min",
		    threshold->short_circuit_min);
}
				...
				struct focal_test_params *params)
{
	...
	if (FTS_THRESHOLD_IN_CSV_FILE == params->in_csv_file) {
		...
		if (FOCAL_FT8201 == g_focal_dev_data->ic_type) {
			focal_8201_prase_threshold_for_csv(
				fts_pdata->project_id, &params->threshold,
				params);
		} else {
			focal_prase_threshold_for_csv(fts_pdata->project_id,
						      &params->threshold,
						      params);
		}
	} else {
