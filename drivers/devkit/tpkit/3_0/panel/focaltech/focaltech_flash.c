...
static int
focal_enter_work_model_from_pram(struct focal_platform_data *focal_pdata);
...
static int focal_get_data_check_sum(const u8 *data, size_t data_size,
				    u8 *check_sum);
static int
focal_enter_work_model_from_rom_update(struct focal_platform_data *focal_pdata);
...
#define RIGHT_OFFSET_16BIT(m) \
	(RIGHT_OFFSET_BIT(m, 16)) /*offset 16bits to right*/
 ...
 */
int focal_enter_rom_update_model_by_software(
	struct focal_platform_data *focal_pdata)
{
	int ret = 0;
 ...
 */
static int
focal_wait_firmware_write_finish(struct focal_platform_data *focal_pdata,
				 u8 command, u32 start_addr,
				 u32 start_write_addr, u32 data_size)
{
	int i = 0;
	int ret = 0;
	u32 ic_status = 0;
	...
	TS_LOG_ERR("%s:%s, ic status=0x%X, driver status=0x%X\n", FOCAL_TAG,
		   "time out for check ic status", ic_status, driver_status);
 ...
 * return : return 0 if read ic status success, otherwize return error code
 */
static int focal_write_firmware_data(struct focal_platform_data *focal_pdata,
				     const u8 *fw_data, u32 fw_length,
				     u8 command, u32 start_addr)
{
	int i = 0;
	int ret = 0;
	...
	for (i = 0; i < package_count; i++) {
		...
		if (TS_BUS_I2C == ts_platform_data->bops->btype) {
			if (true ==
				...
				if (FOCAL_FT8201 == g_focal_dev_data->ic_type ||
					...
					ret = focal_wait_firmware_write_finish(
						focal_pdata, command,
						FTS_FW_WRITE_STATUS_ADDR_START,
						start_write_addr, data_size);
				} else {
					ret = focal_wait_firmware_write_finish(
						focal_pdata, command,
						start_addr, start_write_addr,
						data_size);
				}
			} else {
				ret = focal_wait_firmware_write_finish(
			...
			if (ret) {
				TS_LOG_ERR("%s:%s, ret=%d\n", FOCAL_TAG,
					   "wait firmware write finish fail",
					   ret);
				goto write_pram_err;
			}
		} else {
		...
		TS_LOG_DEBUG("%s:%s:index=%d, data_size=%u, writed_size=%u\n",
			     FOCAL_TAG, "fw write", i, data_size,
			     start_write_addr);
	}
...
static int focal_dram_write_ecc(const u8 *buf, u32 len)
{
	...
	pram_app_size =
		((u32)(((u16)buf[0x100 + 0] << 8) + buf[0x100 + 1])) * 2;
	dram_buf = buf + pram_app_size;
	TS_LOG_INFO("dram buf length in fact:%d,offset:%d\n", dram_size,
		    pram_app_size);
	/* write pram */
	ret = focal_write_firmware_data_8756(dram_buf, dram_size,
					     dram_start_addr, false);
	if (ret < 0) {
 ...
 * return : return 0 if read ic status success, otherwize return error code
 */
static int focal_write_pram_data(struct focal_platform_data *focal_pdata,
				 const u8 *fw_data, u32 fw_length)
{
	if (g_focal_dev_data->ic_type == FOCAL_FT8756) {
		fw_length = (((u16)fw_data[FTS_FT8756_PRAM_OFFSET + 0] << 8) +
			     fw_data[FTS_FT8756_PRAM_OFFSET + 1]) *
			    2;
		return focal_write_firmware_data_8756(fw_data, fw_length, 0,
						      true);
	} else {
		return focal_write_firmware_data(
			focal_pdata, fw_data, fw_length, FTS_CMD_WRITE_PRAM, 0);
	}
}
/*
 ...
 */
static void focal_8201_pram_data_reverse_init(const u8 *fw_data, u8 *tmp_data,
					      u32 fw_length)
{
	int i = 0;
	int j = 0;
 ...
 * return : return 0 if read ic status success, otherwize return error code
 */
static int focal_write_app_data(struct focal_platform_data *focal_pdata,
				const u8 *fw_data, u32 fw_length)
{
	if (FOCAL_FT8006U == g_focal_dev_data->ic_type) {
		return focal_write_firmware_data(focal_pdata, fw_data,
						 fw_length, FTS_CMD_WRITE_FLASH,
						 FT8006U_FW_IC_ADDR_START);
	} else if (FOCAL_FT8201 == g_focal_dev_data->ic_type) {
		return focal_write_firmware_data(focal_pdata, fw_data,
						 fw_length, FTS_CMD_WRITE_FLASH,
						 FTS_8201_FW_IC_ADDR_START);
	} else {
		return focal_write_firmware_data(focal_pdata, fw_data,
						 fw_length, FTS_CMD_WRITE_FLASH,
						 FTS_FW_IC_ADDR_START);
	}
}
 ...
 * return : return 0 if read ic status success, otherwize return error code
 */
static int focal_read_pram_from_ic(u8 *buf, u32 size, u32 pram_size)
{
	int i = 0;
	int ret = 0;
...
static int focal_auto_config_clock(void)
{
	...
	if (ret) {
		TS_LOG_ERR("%s:%s, flash type=0x%02x, ret=%d\n", FOCAL_TAG,
			   "write flash type fail", reg_val, ret);
		return ret;
	}
	...
	case FOCAL_FT5446P03:
		if (crc_length > LONG_LEN_FLASH_ECC_MAX) {
			TS_LOG_ERR("%s:%s, crc_length=%u, max=%d\n", FOCAL_TAG,
				   "crc length out of range", crc_length,
				   LONG_LEN_FLASH_ECC_MAX);
			ret = -EINVAL;
		}
		break;
	default:
 ...
 */
static int
focal_verify_check_sum_status(struct focal_platform_data *focal_pdata,
			      u32 start_addr, u32 crc_length, u8 *cmd,
			      size_t size)
{
	int i;
	u32 ic_status;
	...
	if (!focal_pdata || !cmd || (size == 0) ||
	    (size > CHECK_SUM_I2C_WRITE_BUFF_LEN)) {
 ...
 * return : if success, return 0, otherwize return error code
 */
static int focal_read_check_sum(struct focal_platform_data *focal_pdata,
				u32 start_addr, u32 crc_length, u8 *check_sum)
{
	int i = 0;
	int j = 0;
	int ret = 0;
	...
	if (FOCAL_FT8201 == g_focal_dev_data->ic_type ||
		...
		TS_LOG_INFO("ecc calc num:%d, remainder:%d", packet_num,
			    remainder);
		for (i = 0; i < packet_num; i++) {
			...
			cmd[5] = (u8)(packet_len);
			ret = focal_write(cmd,
					  CHECK_SUM_I2C_WRITE_BUFF_LEN - 1);
			if (ret) {
				TS_LOG_ERR(
					"%s:write verify parameter fail, ret=%d\n",
					FOCAL_TAG, ret);
				return ret;
			}
			msleep(packet_len /
			       256); /*delay times base on crc_length*/
			for (j = 0;
			     j < focal_pdata->delay_time->read_ecc_query_times;
			     j++) {
				ret = focal_get_status(&ic_status);
				if (ret) {
					TS_LOG_ERR(
						"%s:get ic status fail, ret=%d\n",
						FOCAL_TAG, ret);
				} else {
				...
				if (j ==
				    focal_pdata->delay_time
						    ->read_ecc_query_times -
					    1) {
					TS_LOG_ERR(
						"%s:%s, out of max retry times\n",
		...
		ret = focal_verify_check_sum_status(
			focal_pdata, start_addr, crc_length, cmd,
			CHECK_SUM_I2C_WRITE_BUFF_LEN);
		if (ret) {
		...
		for (i = 0; i < focal_pdata->delay_time->read_ecc_query_times;
		     i++) {
			ret = focal_get_status(&ic_status);
			if (ret) {
				TS_LOG_ERR("%s:get ic status fail, ret=%d\n",
			...
			if (i ==
			    focal_pdata->delay_time->read_ecc_query_times - 1) {
				TS_LOG_ERR("%s:%s, out of max retry times\n",
					   FOCAL_TAG, "status check fail");
			}
 ...
 */
static int
focal_firmware_update_outcell(struct focal_platform_data *focal_pdata,
			      const u8 *fw_data, u32 fw_len)
{
	int ret = 0;
	...
	if (ret) {
		TS_LOG_ERR("%s:enter bootloader model fail, ret=%d\n",
			   FOCAL_TAG, ret);
#if defined(CONFIG_HONOR_DSM)
		g_focal_dev_data->ts_platform_data->dsm_info
			.constraints_update_status =
			focal_enter_pram_model_fail;
#endif
...
#if defined(CONFIG_HONOR_DSM)
		g_focal_dev_data->ts_platform_data->dsm_info
			.constraints_update_status = focal_write_app_data_fail;
#endif
		goto enter_work_model_from_update_model;
	} else {
	...
	/*5.read out checksum*/
	ret = focal_read_check_sum(focal_pdata, 0, fw_len, &check_sum_in_ic);
	if (ret) {
		TS_LOG_ERR("%s:read check sum in ic fail, ret=%d\n", FOCAL_TAG,
			   ret);
#if defined(CONFIG_HONOR_DSM)
		g_focal_dev_data->ts_platform_data->dsm_info
			.constraints_update_status = focal_read_check_sum_fail;
#endif
		goto enter_work_model_from_update_model;
	}

	TS_LOG_INFO("%s:crc_in_ic=0x%02X, crc_of_fw=0x%02X\n", FOCAL_TAG,
		    check_sum_in_ic, check_sum_of_fw);
				 ...
				 const u8 *fw_data, u32 fw_len)
{
	...
	if (FOCAL_FT8006U == g_focal_dev_data->ic_type) {
		ret = focal_read_check_sum(focal_pdata,
					   FT8006U_FW_IC_ADDR_START, fw_len,
					   &check_sum_in_ic);
	} else if (FOCAL_FT8201 == g_focal_dev_data->ic_type) {
		ret = focal_read_check_sum(focal_pdata,
					   FTS_8201_FW_IC_ADDR_START, fw_len,
					   &check_sum_in_ic);
	} else {
 ...
 * return : if success, return 0, otherwize return error code
 */
static int focal_firmware_update_spi(struct focal_platform_data *focal_pdata,
				     const u8 *fw_data, u32 fw_len)
{
	int ret = 0;
	int retry_time = 0;
	...
	for (retry_time = 0; retry_time < 3; retry_time++) {
		...
		if (g_focal_dev_data->ic_type == FOCAL_FT8756) {
			ret = focal_enter_rom_update_model_by_hardware(
				focal_pdata);
		...
		if (ret) {
			TS_LOG_INFO(
				"firmware_update_spi:flash pram fail, ret=%d\n",
				ret);
			continue;
		}
		...
		if (ret) {
			TS_LOG_INFO(
				"firmware_update_spi:start pram fail, ret=%d\n",
				ret);
			continue;
		}
		break;
 ...
 * return : if success, return 0, otherwize return error code
 */
static int focal_flash_read_fw_file(char *fw_file_path, u8 *fw_buf)
{
	loff_t pos = 0;
	off_t fsize = 0;
	mm_segment_t old_fs;
 ...
 * return : if success, return file size, other wize return error code
 */
int focal_get_pram_file_name(struct focal_platform_data *focal_pdata,
			     char *pram_name, size_t size)
{
	int ret = 0;
	...
	return snprintf(
		pram_name, size, "ts/%s_focal_pram_%x.img",
...
static int focal_get_ecc_from_tp(u32 start_addr, u32 crc_length, u16 *ecc)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < 100; i++) {
		ret = focal_read(cmd, FTS_COMMON_COMMAND_LENGTH, value,
...
static int focal_get_data_ecc(const u8 *data, size_t data_size, u16 *ecc_r)
{
	u16 ecc = 0;
	u16 i = 0;
	u16 j = 0;
...
static int focal_ecc_check_8756(const u8 *buf, u32 len, u32 ecc_saddr)
{
	...
	for (i = 0; i < packet_number; i++) {
		...
		ret = focal_get_data_ecc(buf + offset, packet_length,
					 &ecc_in_host);
		if (ret < 0) {
		...
		ret = focal_get_ecc_from_tp(ecc_saddr + offset, packet_length,
					    &ecc_in_tp);
		if (ret < 0) {
			    ...
			    const u8 *pram_data, size_t pram_size)
{
	...
	if (g_focal_dev_data->ic_type == FOCAL_FT8756) {
		pram_size = (((u16)pram_data[FTS_FT8756_PRAM_OFFSET + 0] << 8) +
			     pram_data[FTS_FT8756_PRAM_OFFSET + 1]) *
			    2;
	} else {
	...
	if (FOCAL_FT8201 == g_focal_dev_data->ic_type) {
		...
		if (NULL == tmp_buffer) {
			TS_LOG_ERR(
				"flash_pram: malloc mem for tmp_buffer fail\n");
			return -ENOMEM;
		}
		focal_8201_pram_data_reverse_init(pram_data, tmp_buffer,
						  pram_size);
	...
	if (TS_BUS_I2C ==
	    focal_pdata->focal_device_data->ts_platform_data->bops->btype) {
		msleep(100);

		pcheck_buffer = kmalloc(pram_size, GFP_ATOMIC);
		if (NULL == pcheck_buffer) {
			TS_LOG_ERR(
				"flash_pram: malloc mem for pcheck_buffer fail\n");
			ret = -ENOMEM;
			goto release_tmp_buffer;
		}

		TS_LOG_DEBUG("flash_pram:read pram data from ic\n");
		ret = focal_read_pram_from_ic(pcheck_buffer, pram_size,
					      pram_size);
		if (ret) {
			TS_LOG_ERR(
				"flash_pram: read pram data from ic fail, ret=%d\n",
				ret);
		...
		for (i = 0; i < pram_size; i++) {
			if (pcheck_buffer[i] != pram_data[i]) {
				TS_LOG_ERR(
					"flash_pram:%s,index=%d, src=0x%02x, cut=0x%02x\n",
					"pram write check fail,", i,
					pram_data[i], pcheck_buffer[i]);
				ret = -EINVAL;
				goto release_pcheck_buffer;
			}
		}
		...
		if (ret) {
			TS_LOG_ERR(
				"flash_pram:read check sum in host fail, ret=%d\n",
				ret);
			goto release_pcheck_buffer;
		}
		...
		if (ret) {
			TS_LOG_ERR(
				"flash_pram:read check sum in ic fail, ret=%d\n",
				ret);
			goto release_pcheck_buffer;
		}
 ...
 * return : if success, return 0, otherwize return error code
 */
int focal_get_img_file_version(struct focal_platform_data *focal_pdata,
			       const struct firmware *fw, u8 *fw_ver)
{
	int fw_len = 0;
	...
	if (fw_len <= FTS_FW_IMG_ADDR_VER) {
		TS_LOG_ERR(
			"get_img_file_version:fw length is to long, len=%d.\n",
			fw_len);
		return -ENOMEM;
	}
...
int focal_get_ic_firmware_version(u8 *version)
{
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"get_ic_firmware_version:read firmware version in ic fail, ret=%d\n",
			ret);
		return ret;
	}
...
static int focal_get_lcd_hide_module_name(char *module_name)
{
	...
	ret = snprintf(comp_name,
		       LCD_PANEL_INFO_MAX_LEN + FTS_CHIP_NAME_LEN + 1, "%s-%s",
		       FTS_CHIP_NAME, module_name);
	if (ret >= LCD_PANEL_INFO_MAX_LEN + FTS_CHIP_NAME_LEN + 1) {
		TS_LOG_INFO("get_lcd_hide_module_name:%s, ret=%d, size=%lu\n",
			    ...
			    LCD_PANEL_INFO_MAX_LEN + FTS_CHIP_NAME_LEN + 1);
		return -EINVAL;
	}
	...
	if (!dev_node) {
		TS_LOG_ERR(
			"get_lcd_hide_module_name: NOT found device node[%s]!\n",
			comp_name);
		return -EINVAL;
	}

	lcd_hide_module_name =
		(char *)of_get_property(dev_node, "lcd_hide_module_name", NULL);
	if (!lcd_hide_module_name) {
		TS_LOG_ERR(
			"get_lcd_hide_module_name: Get lcd_hide_module_name faile!\n");
		return -EINVAL;
	}

	strncpy(g_focal_pdata->lcd_hide_module_name, lcd_hide_module_name,
		MAX_STR_LEN - 1);
...
static int focal_get_lcd_module_name(void)
{
	...
	if (focal_pdata->hide_plain_lcd_log) {
		return focal_get_lcd_hide_module_name(
 ...
 * return : if need to update firmware, return 1, else return 0
 */
static int focal_get_firmware_name(struct focal_platform_data *focal_pdata,
				   const char *product_name, char *fw_name,
				   size_t size)
{
	int ret = 0;
	...
	if (focal_pdata->focal_device_data->hide_fw_name) {
		ret = snprintf(fw_name, size, "ts/%s.img",
			       focal_pdata->project_id);
		if (ret >= size) {
		...
		if (focal_pdata
			...
			if (ret >= size) {
				TS_LOG_ERR(
					"get_firmware_name:fw name buffer out of range, ret=%d\n",
					ret);
				return -ENOMEM;
			}
		} else {
			ret = snprintf(fw_name, size, "ts/%s%s_", product_name,
				       project_id);
			if (ret >= size) {
				TS_LOG_ERR(
					"get_firmware_name:fw name buffer out of range, ret=%d\n",
			    ...
			    FTS_FW_NAME_LEN >
				    strlen(fw_name) +
					    strlen(focal_pdata
							   ->lcd_hide_module_name)) {
				strncat(fw_name,
					focal_pdata->lcd_hide_module_name,
					strlen(focal_pdata
						       ->lcd_hide_module_name));
			}
		} else {
			if (!ret &&
			    FTS_FW_NAME_LEN >
				    strlen(fw_name) +
					    strlen(focal_pdata
							   ->lcd_module_name)) {
				strncat(fw_name, focal_pdata->lcd_module_name,
					strlen(focal_pdata->lcd_module_name));
			}
		}
		...
		ret = snprintf(fw_name, size, "ts/%s%s_%s.img", product_name,
 ...
 * return : if need to update firmware, return 1, else return 0
 */
static int focal_check_firmware_version(struct focal_platform_data *focal_pdata,
					const struct firmware *fw)
{
	...
	if (ret || fw_ver_in_ic == 0xEF) {
		TS_LOG_INFO(
			"check_firmware_version:%s, ret=%d, fw_version_in_ic=%d\n",
			"firmware in ic is damaged, update firmware", ret,
			fw_ver_in_ic);
		return 1;
	}
	...
	if (ret) {
		TS_LOG_INFO(
			"check_firmware_version:%s, don't update firmware, ret=%d\n",
			       ...
			       const char *product_name)
{
	...
	/* 1. get firmware name */
	ret = focal_get_firmware_name(focal_pdata, product_name, fw_name,
				      FTS_FW_NAME_LEN);
	if (ret) {
		TS_LOG_ERR(
			"firmware_auto_update:get firmware name fail, ret=%d\n",
			ret);
	...
	if (ret != 0) {
		TS_LOG_ERR(
			"firmware_auto_update:firmware request fail, ret=%d, fw_name=%s\n",
			ret, fw_name);
		ret = 0;
		goto fw_release_flag;
	}
	...
	if (!ret) {
		TS_LOG_INFO(
			"firmware_auto_update:no need to update firmware\n");
		goto release_fw;
	} else {
	...
	if (TS_BUS_I2C ==
		...
		if (true == focal_pdata->focal_device_data->is_in_cell) {
			ret = focal_firmware_update(focal_pdata, fw->data,
						    fw->size);
		} else {
			ret = focal_firmware_update_outcell(focal_pdata,
							    fw->data, fw->size);
		}
	} else {
		ret = focal_firmware_update_spi(focal_pdata, fw->data,
						fw->size);
	}
	if (ret) {
		TS_LOG_ERR(
			"firmware_auto_update:firmware update fail, ret=%d\n",
			ret);
	...
	if (ret) {
		TS_LOG_ERR(
			"firmware_auto_update:get firmware version fail, ret=%d\n",
			ret);
		goto release_fw;
	} else {
		focal_pdata->fw_ver = fw_ver;
				 ...
				 const char *fw_name)
{
	...
	if (ret) {
		...
		focal_pdata->fw_ver = fw_ver;
		snprintf(g_focal_dev_data->version_name,
			 sizeof(g_focal_pdata->fw_ver), "%d",
			 g_focal_pdata->fw_ver);
	}
release_fw:
	/* 3. release firmware */
...
int focal_read_chip_id(struct focal_platform_data *focal_pdata, u32 *chip_id)
{
	int ret = 0;
 ...
 * return: if read project id success, return 0, else return error code
 */
int focal_enter_work_model_form_pram_update(
	struct focal_platform_data *focal_pdata)
{
	int ret = 0;
	u8 cmd = 0x00;
...
int fts_enter_test_environment(bool test_flag)
{
	...
	ret = focal_get_firmware_name(g_focal_pdata, g_focal_pdata->fw_name,
				      fw_name, FTS_FW_NAME_LEN);
	if (ret) {
 ...
 */
int focal_read_tp_color(struct focal_platform_data *focal_pdata, char *tp_color,
			size_t size)
{
	int ret = 0;
	...
	cmd[3] =
		(focal_pdata->pram_projectid_addr & 0x0FF) + FTS_PROJECT_ID_LEN;
...
int focal_read_project_id_from_rom(struct focal_platform_data *focal_pdata,
				   char *project_id, size_t size)
{
	int ret = 0;
	u8 cmd[6] = {0};
	...
	for (i = 0; i < FTS_READ_PROJECTID_RETRY_TIMES; i++) {
		...
		if ((!strncmp(project_id_lower,
			      focal_pdata->focal_device_data->ts_platform_data
				      ->product_name,
			      FTS_PROJECTID_LETTERS_LEN))) {
...
int focal_read_project_id(struct focal_platform_data *focal_pdata,
			  char *project_id, size_t size)
{
	int ret = NO_ERR;
	...
	if (TS_BUS_I2C ==
	    focal_pdata->focal_device_data->ts_platform_data->bops->btype) {
		ret = focal_read_project_id_from_pram(focal_pdata, project_id,
						      size);
	} else {
		if (g_focal_dev_data->ic_type == FOCAL_FT8756)
