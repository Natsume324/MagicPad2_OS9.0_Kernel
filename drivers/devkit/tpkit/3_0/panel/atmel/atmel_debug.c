...
void atmel_get_average_max_min_data(struct mxt_data *data, char *buf)
{
	...
	snprintf((buf), PAGE_SIZE, "[%4d,%4d,%4d]",
	...
	snprintf((buf + i), PAGE_SIZE, "[%4d,%4d,%4d]",
...
int atmel_get_refs_rx2rx_delta_test(struct mxt_data *data)
{
	...
	for (tx_n = 0; tx_n < tx_num; tx_n++) {
		for (rx_n = 0; rx_n < rx_num - 1; rx_n++) {
			...
			rx_delta_value =
				data->T37_buf[index + 1] - data->T37_buf[index];
	...
	for (tx_n = 0; tx_n < tx_num; tx_n++) {
		for (rx_n = 0; rx_n < rx_num - 1; rx_n++) {
			...
			if ((rx_delta_value <=
				...
				TS_LOG_ERR(
					"%s, Rx:%d,Tx:%d, rx_delta:%d out of range[%d, %d]\n",
					__func__, rx_n, tx_n, rx_delta_value,
					data->rawdata_rx2rx_low_limit_buf
						[index_limit],
	...
	if (result == (data->T37_buf_size / sizeof(int) - tx_num)) {
...
int atmel_get_refs_tx2tx_delta_test(struct mxt_data *data)
{
	...
	for (tx_n = 0; tx_n < tx_num - 1; tx_n++) {
		for (rx_n = 0; rx_n < rx_num; rx_n++) {
			tx_delta_value =
				data->T37_buf[(tx_n + 1) * rx_num + rx_n] -
				data->T37_buf[tx_n * rx_num + rx_n];
	...
	for (tx_n = 0; tx_n < tx_num - 1; tx_n++) {
		for (rx_n = 0; rx_n < rx_num; rx_n++) {
			...
			if ((tx_delta_value <=
				...
				TS_LOG_ERR(
					"%s, Rx:%d,Tx:%d, tx_delta:%d,out of range[%d, %d]\n",
					__func__, rx_n, tx_n, tx_delta_value,
					data->rawdata_tx2tx_low_limit_buf[index],
					data->rawdata_tx2tx_up_limit_buf[index]);
			}
	...
	if (result == (data->T37_buf_size / sizeof(int) - rx_num)) {
...
int atmel_get_refs_max_minus_min_test(struct mxt_data *data, int data_limit)
{
	...
	if (max_num - min_num < data_limit) {
		...
		TS_LOG_INFO(
			"%s, max min delta failed, delta value:%d, data_limit:%d\n",
...
int atmel_get_rawdata_test(struct mxt_data *data)
{
	...
	if (!data || !data->T37_buf || !data->rawdata_up_limit_buf ||
...
int atmel_get_refs_or_deltas_data_test(struct mxt_data *data)
{
	...
	if (!data || !data->T37_buf || !data->noise_up_limit_buf ||
...
int atmel_T37_fetch(struct mxt_data *data, u8 mode)
{
	...
	matrix_add_size = (size_t)((data->info->matrix_xsize) +
				   (data->info->matrix_ysize));
	matrix_mul_size =
		(size_t)((data->info->matrix_xsize) * data->info->matrix_ysize);
	T6 = atmel_get_object(data, MXT_GEN_COMMANDPROCESSOR_T6);
	T37 = atmel_get_object(data, MXT_DEBUG_DIAGNOSTIC_T37);
	if (!T6 || mxt_obj_size(T6) < MIN_T6_SIZE || !T37 ||
	    mxt_obj_size(T37) < MIN_T37_SIZE) {
		TS_LOG_ERR("Invalid T6 or T37 object\n");
		return -ENODEV;
	}
	if (MXT_T6_DEBUG_DELTA_SC == mode) {
		tem_buf_size =
			matrix_add_size * sizeof(int) *
			2; /*total rawdata num: x+y+y, malloc enough size*/
		...
		T37_tem_buf_size =
			matrix_add_size * sizeof(u8) * 2 *
			2; /* total rawdata num :x+y+y, rawdata is short type, malloc enough size*/
		...
		for (i = 0; i < num_pages; i++) {
			...
			if (1 == i) { // cp mode, page do not include rawdata
				...
				if (ret) {
					TS_LOG_ERR(
						"read MXT_DEBUG_DIAGNOSTIC_T37 failed\n");
					goto out;
				}
				...
				TS_LOG_INFO("Current Page is obuf[1]=%d",
					    T37_buf[1]);
				switch (i) {
				case 0: //page 0
					chunk_len =
						(size_t)matrix_add_size *
						2; /* rawdata is short type */
					break;
				case 2: //page 2
					chunk_len =
						(size_t)(data->info
				...
				if (chunk_len + pos > T37_tem_buf_size) {
					TS_LOG_ERR(
						"sc mode out of memery range, buf_size = %lu, data_size = %lu\n",
						T37_tem_buf_size,
						chunk_len + pos);
					ret = -EIO;
					goto out;
				}
				memcpy(&T37_tem_buf[pos], &T37_buf[2],
				       chunk_len);
				pos += chunk_len;
			}
		}
		/*total rawdata is :x+y+y, rawdat is short ,total byte:2x+4y */
		for (i = 0, j = 0; i < (size_t)(data->info->matrix_xsize * 2 +
						data->info->matrix_ysize * 4);
		     i += 2, j++) {
			tem_buf[j] = (short)((T37_tem_buf[i + 1] << 8) |
					     (T37_tem_buf[i]));
		}
		i = 0;
		j = 0;
		while (i < (size_t)(data->info->matrix_xsize +
				    data->info->matrix_ysize * 2)) {
		...
		T37_tem_buf_size =
			matrix_mul_size * sizeof(u8) * 2; /* rawdata is short */
		T37_tem_buf = kzalloc(T37_tem_buf_size, GFP_KERNEL);
		if (!T37_tem_buf) {
			TS_LOG_ERR("alloc mem for T37_tem_buf failed\n");
			ret = -ENOMEM;
		...
		num_pages =
			DIV_ROUND_UP(T37_tem_buf_size, mxt_obj_size(T37) - 2);
		 ...
		 */
			if (chunk_len + pos > T37_tem_buf_size) {
				TS_LOG_ERR(
					"other mode out of memery range, buf_size = %lu, data_size = %lu\n",
					T37_tem_buf_size, chunk_len + pos);
				ret = -EIO;
				goto out;
			}
			chunk_len = min((size_t)mxt_obj_size(T37) - 2,
					(size_t)T37_tem_buf_size - pos);
		...
		for (i = 0, j = 0; i < T37_tem_buf_size; i += 2, j++) {
			if (mode == MXT_T6_DEBUG_REF) {
				tem_buf[j] = (u16)((T37_tem_buf[i + 1] << 8) |
						   (T37_tem_buf[i]));
			} else if (mode == MXT_T6_DEBUG_DELTA) {
				tem_buf[j] = (s16)((T37_tem_buf[i + 1] << 8) |
						   (T37_tem_buf[i]));
			}
		}
		...
		while ((i < matrix_mul_size) &&
		       (j < (size_t)(data->x_size - data->x_origin) *
				    (data->y_size - data->y_origin))) {
			data->T37_buf[j] = tem_buf[i];
			i++;
			j++;
			if (0 == j % (size_t)(data->y_size - data->y_origin)) {
				i += (size_t)(data->info->matrix_ysize -
					      data->y_size + data->y_origin);
			}
		}
		ret = 0;
...
void atmel_debug_msg_remove(struct mxt_data *data)
{
	...
	if (data->debug_msg_attr.attr.name) {
		sysfs_remove_bin_file(&data->dev->kobj, &data->debug_msg_attr);
	}
}
...
static void mxt_debug_msg_enable(struct mxt_data *data)
{
	...
	data->debug_msg_data =
		kcalloc(DEBUG_MSG_MAX, data->T5_msg_size, GFP_KERNEL);
					 ...
					 const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &i) == MXT_GET_ONE_PARA &&
	    i < MXT_VALID_DEVICE_CTRL) {
		if (i == 1) {
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(
		buf, PAGE_SIZE, "%02x (GPIO 0x%02x)\n", data->t19_msg[0],
		(data->t19_msg[0] >> 2) & 0xf); /* bit2~bit5 is valid */
}
...
static int mxt_t19_command(struct mxt_data *data, bool enable, bool wait)
{
	...
	do {
		...
		if (ret)
			return ret;
	} while ((val[0] & 0x4) && (timeout_counter++ <=
				    100)); //4,check bit2 state, 100 retry times
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	offset += scnprintf(
		buf + offset, PAGE_SIZE, "%x %x %x %x %x %x\n",
		data->t25_msg[0], data->t25_msg[1], data->t25_msg[2],
		data->t25_msg[3], data->t25_msg[4],
		data->t25_msg[5]); /*msg[0]:test status ,msg[1]~msg[5]:test result info */
	return offset;
}
...
static int mxt_t25_command(struct mxt_data *data, u8 cmd, bool wait)
{
	...
	val[0] =
		0x3; //0x3 means enable the T25 & also enable message report of T25
	val[1] = cmd;
	...
	do {
		...
		if (ret) {
		 ...
		 (timeout_counter++ <= MAX_TIMEOUT_TIMES)); //50:retry times
...
int atmel_t25_selftest_polling(struct mxt_data *data, int cmd)
{
	...
	for (i = 0; i < MXT_T25_PROCESS_RETRY; i++) {
		...
		if (data->t25_msg[0] ==
		    0xFE) { //0xFE means open short test success
			break;
		}
...
static DEVICE_ATTR(t19, S_IWUSR | S_IRUSR /*| S_IWGRP | S_IWOTH */,
		   mxt_t19_gpio_show, mxt_t19_gpio_store);
static DEVICE_ATTR(t25, S_IWUSR | S_IRUSR /*| S_IWGRP | S_IWOTH */,
		   atmel_t25_selftest_show, atmel_t25_selftest_store);
...
int atmel_mem_access_init(struct mxt_data *data)
{
	...
	if (sysfs_create_bin_file(&data->dev->kobj, &data->mem_access_attr) <
	    0) {
...
int atmel_debug_msg_init(struct mxt_data *data)
{
	...
	if (sysfs_create_bin_file(&data->dev->kobj, &data->debug_msg_attr) <
	    0) {
		TS_LOG_ERR("Failed to create %s\n",
