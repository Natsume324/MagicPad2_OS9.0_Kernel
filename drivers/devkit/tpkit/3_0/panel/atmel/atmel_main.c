...
#define DEFAULT_ACTIVE2IDLE_TIME \
	50 //the default time of tp active status change to idle status
...
bool atmel_update_sd_mode =
	false; //true: is sd update mode , false : auto update mode
...
static int atmel_parse_threshold_file_method(const char *buf,
					     uint32_t file_size);
...
retry_write:
		...
		ret = i2c_transfer(chip_data->ts_platform_data->client->adapter,
				   &xfer, 1);
			    ...
			    unsigned long flag)
{
	...
	transferred = 0;
	while (transferred < len) {
		if (!test_flag(I2C_ACCESS_NO_REG | I2C_ACCESS_R_REG_FIXED,
			       &flag)) {
			w_cache[0] = (reg + transferred) & 0xff;
			w_cache[1] = ((reg + transferred) >> 8) & 0xff;
		}
...
static int __atmel_read_reg_ext(struct mxt_data *data, u16 addr, u16 reg,
				u16 len, void *val, unsigned long flag)
{
	u8 *r_cache = NULL, *r_cache_pa = NULL, *w_cache = NULL,
	   *w_cache_pa = NULL;
	int ret = 0;
	...
	ret = __mxt_cache_read(data->atmel_chip_data, addr, reg, len, val,
			       r_cache, r_cache_pa, w_cache, w_cache_pa, flag);
				 ...
				 u16 len, const void *val, unsigned long flag)
{
	...
	mutex_lock(&data->bus_access_mutex);
	ret = __mxt_cache_write(data->atmel_chip_data, addr, reg, len, val,
...
static void atmel_power_off(struct mxt_data *data)
{
	...
	gpio_direction_output(
		data->atmel_chip_data->ts_platform_data->reset_gpio, 0);
...
static int atmel_power_on(struct mxt_data *data)
{
	...
	gpio_direction_output(
		data->atmel_chip_data->ts_platform_data->reset_gpio, 1);
	error = gpio_direction_input(
		data->atmel_chip_data->ts_platform_data->irq_gpio);
	if (error) {
...
int atmel_probe_info_block(struct mxt_data *data)
{
	...
	return __atmel_read_reg(data, 0, 1,
				&val); /* 0,reg_addr; 1,read one byte */
}
...
static int atmel_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	if (!platform_data || !(platform_data->ts_dev) ||
	    !(platform_data->client) || !data || !(data->atmel_chip_data)) {
	...
	error = atmel_parse_dts(data->atmel_dev->dev.of_node,
				data->atmel_chip_data);
	if (error) {
...
int mxt_obj_instances(const struct mxt_object *obj)
{
	...
	return obj->instances_minus_one +
	       1; /* instance read from IC is the actual instance minus 1*/
}
...
#endif
static void mxt_parse_each_object_table(struct mxt_object *object, u8 min_id,
					u8 max_id)
{
	struct mxt_data *data = mxt_core_data;
				  ...
				  struct mxt_object *object_table)
{
	...
	data->msg_buf =
		kcalloc(data->max_reportid, data->T5_msg_size, GFP_KERNEL);
	if (!data->msg_buf) {
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	calculated_crc =
		atmel_calculate_crc(buf, 0, size - MXT_INFO_CHECKSUM_SIZE);
	...
	if ((data->info_crc == 0) || (data->info_crc != calculated_crc)) {
		...
		print_hex_dump(KERN_ERR, "[mxt] INFO:", DUMP_PREFIX_NONE, 16,
			       1, /* 16,rawsize; 1,groupsize */
			       buf, size, false);
	...
	TS_LOG_INFO(
		...
		data->info->version >> 4,
		data->info->version &
			0xf, /*print High 4 bit, Low 4 bit separately*/
		data->info->build, data->info->object_num,
		data->info->matrix_xsize, data->info->matrix_ysize);

	/* Parse object table information */
	error = mxt_parse_object_table(
		data, (struct mxt_object *)(buf + MXT_OBJECT_START));
	if (error) {
...
int atmel_t6_command(struct mxt_data *data, u16 cmd_offset, u8 value, bool wait)
{
	...
	} while ((command_register != 0) &&
		 (timeout_counter++ <= MAX_TIMEOUT_TIMES)); //timeout times
...
static int mxt_set_t7_active2ide_timeout_cfg(
	struct mxt_data *data,
	u8 timeout) // timeout in step of 200ms , eg, 50 means 10 seconds
{
	int error = NO_ERR;
	if (!data) {
	...
	error = __atmel_write_reg(data, data->T7_address + T7_ACT2IDLETO_OFFSET,
				  1, &timeout);
	if (error) {
...
static int mxt_set_t7_power_cfg(struct mxt_data *data, u8 sleep)
{
	...
	error = __atmel_write_reg(data, data->T7_address, sizeof(data->t7_cfg),
				  new_config);
	if (error) {
...
recheck:
	error = __atmel_read_reg(data, data->T7_address, sizeof(data->t7_cfg),
				 &data->t7_cfg);
	if (error) {
	...
	if (data->t7_cfg.active == 0 || data->t7_cfg.idle == 0) {
		if (!retry) {
			...
			data->t7_cfg.active =
				MXT_SCAN_ONCE_TIME; //how long to scan once
			data->t7_cfg.idle =
				MXT_NORMAL_MODE_WAIT_SCAN_ONCE_TIME; //how long to wait for a scan
			data->t7_cfg_doze.active = data->t7_cfg.active;
			data->t7_cfg_doze.idle =
				MXT_DOZE_MODE_WAIT_SCAN_ONCE_TIME; //max 255
...
static int mxt_read_t100_config(struct mxt_data *data)
{
	...
	error = __atmel_read_reg(data, object->start_address + MXT_T100_XRANGE,
	...
	error = __atmel_read_reg(data, object->start_address + MXT_T100_YRANGE,
	...
	error = __atmel_read_reg(data, object->start_address,
	...
	error = __atmel_read_reg(data, object->start_address + MXT_T100_XSIZE,
	...
	error = __atmel_read_reg(data, object->start_address + MXT_T100_YSIZE,
	...
	error = __atmel_read_reg(data, object->start_address + MXT_T100_XORIGIN,
	...
	error = __atmel_read_reg(data, object->start_address + MXT_T100_YORIGIN,
...
static void mxt_parse_module_dts(struct ts_kit_device_data *chip_data)
{
	...
	ts_parse_panel_specific_config(device,
				       chip_data); /* parse module config */

	retval = of_property_read_u32(device, "t72_noise_level_threshold",
				      &read_val);
	if (!retval) {
		TS_LOG_INFO(
...
void atmel_parse_fw_need_depend_on_lcd(struct device_node *core_node,
				       struct ts_kit_device_data *chip_data)
{
	int value = 0;
	int error = 0;
	...
	error = of_property_read_u32(core_node, "fw_need_depend_on_lcd",
				     &value);
	if (error) {
	...
	TS_LOG_INFO("%s, fw_need_depend_on_lcd = %d \n", __func__,
		    mxt_core_data->fw_need_depend_on_lcd);
}
			   ...
			   struct ts_kit_device_data *chip_data)
{
	...
	error = of_property_read_u32(device, "noise_debug_enable",
	...
	error = of_property_read_u32(device, "support_get_tp_color",
	...
	error = of_property_read_u32(device, "slave_address",
				     (u32 *)&data->addr);
	if (error) {
	...
	if (sizeof(chip_data->chip_name) > strlen(ATMEL_VENDER_NAME) + 1) {
		strncpy(chip_data->chip_name, ATMEL_VENDER_NAME,
			strlen(ATMEL_VENDER_NAME) + 1);
	} else {
		strncpy(chip_data->chip_name, ATMEL_VENDER_NAME,
			sizeof(chip_data->chip_name) - 1);
...
static int atmel_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_X, 0,
			     data->atmel_chip_data->x_max - 1, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0,
			     data->atmel_chip_data->y_max - 1, 0, 0);
	TS_LOG_DEBUG(
		"data->atmel_chip_data->x_max-1=%d, data->atmel_chip_data->y_max-1=%d\n",
		data->atmel_chip_data->x_max - 1,
		data->atmel_chip_data->y_max - 1);

	input_set_abs_params(input_dev, ABS_PRESSURE, 0, 255, 0,
			     0); /* abs_pressure range*/
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, 15, 0,
			     0); /* abs_mt_tacking_id range */
	input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, 255, 0,
			     0); /* abs_mt_pressure range */
	//#if ANTI_FALSE_TOUCH_USE_PARAM_MAJOR_MINOR
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MAJOR, 0, 100, 0,
			     0); /* abs_mt_width_major range */
	input_set_abs_params(input_dev, ABS_MT_WIDTH_MINOR, 0, 100, 0,
			     0); /* abs_mt_width_minor range */
//#else
...
//#endif
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
			     data->atmel_chip_data->x_max_mt - 1, 0, 0);

	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
			     data->atmel_chip_data->y_max_mt - 1, 0, 0);
	TS_LOG_DEBUG(
		"MT: data->atmel_chip_data->x_max-1=%d, data->atmel_chip_data->y_max-1=%d\n",
		data->atmel_chip_data->x_max_mt - 1,
		data->atmel_chip_data->y_max_mt - 1);
	return NO_ERR;
}
...
static void mxt_proc_t6_messages(struct mxt_data *data, u8 *msg)
{
	...
	status = msg[1];
	crc = msg[2] | (msg[3] << 8) |
	      (msg[4] << 16); /*crc=msg[2]+msg[3]*255+msg[3]*65535*/
	...
	if (status != data->t6_status) {
		...
		if (status & MXT_T6_STATUS_CAL) {
			mxt_reset_slots(
				data); /*release all points in calibration for safe*/
				  ...
				  struct ext_info *in)
{
	...
	************************************************/
	...
	if (aux < MAX_FINGER_DATA_SIZE) {
		if (test_flag_8bit(MXT_T100_TCHAUX_AREA,
				   &tchcfg[MXT_T100_TCHAUX]))
			in->area = message[aux++];
	}

	if (aux < MAX_FINGER_DATA_SIZE - 1) {
		if (test_flag_8bit(MXT_T100_TCHAUX_HW,
				   &tchcfg[MXT_T100_TCHAUX])) {
			in->height = message[aux++];
			in->width = message[aux++];
			if (test_flag_8bit(MXT_T100_CFG_SWITCHXY,
					   &tchcfg[MXT_T100_CFG1]))
				swap(in->height, in->width);
		}
	}

	if (aux < MAX_FINGER_DATA_SIZE) {
		if (test_flag_8bit(MXT_T100_TCHAUX_PEAK,
				   &tchcfg[MXT_T100_TCHAUX]))
			in->peak = message[aux++];
	}

	if (aux < MAX_FINGER_DATA_SIZE - 1) {
		if (test_flag_8bit(MXT_T100_TCHAUX_AREAHW,
				   &tchcfg[MXT_T100_TCHAUX])) {
			/*********************************
	...
	if (aux < MAX_FINGER_DATA_SIZE - 1) {
		if (test_flag_8bit(MXT_T100_TCHAUX_EDGE,
				   &tchcfg[MXT_T100_TCHAUX])) {
...
out:
	...
	if ((MXT_T6_CMD58_ON == atmel_gMxtT6Cmd58Flag) &&
		...
		ret = __atmel_read_reg(data, reg, sizeof(command_register),
				       &command_register[0]);
		if (ret) {
...
static void mxt_proc_t25_messages(struct mxt_data *data, u8 *msg)
{
	...
	TS_LOG_DEBUG("T25 Status 0x%x Info: %x %x %x %x %x\n", status, msg[2],
		     msg[3], msg[4], msg[5],
		     msg[6]); /*the 2nd¡¢3rd¡¢4th¡¢5th¡¢6th save t25 info*/
...
int mxt_process_messages_t25(struct mxt_data *data)
{
	...
	/* Read T5 message */
	ret = __atmel_read_reg(data, data->T5_address, data->T5_msg_size,
			       data->msg_buf); //T5_msg_size
	if (ret) {
...
static void mxt_proc_t70_messages(struct mxt_data *data, u8 *msg)
{
	...
	if (data->noise_debug_enable) {
		if ((data->T70_reportid_min + 1) ==
		    msg[0]) { //T70 report ID 1 is for moisture mode reporting
...
static void mxt_status_count(u8 *msg)
{
	...
	switch (msg[2] &
		MXT_T72_NOISE_SUPPRESSION_STATUS2STATE_MASK) { //T72_NOISE_SUPPRESSION_STATUS2STATE byte
...
static void mxt_proc_t72_messages(struct mxt_data *data, u8 *msg)
{
	...
	TS_LOG_DEBUG("T72 noise state1 = 0x%x state2 = 0x%x\n", msg[1],
		     msg[2]); /*msg[1], noise state1; msg[2], noise state2 */

	if (data->noise_debug_enable) {
		if (msg[1] &
		    MXT_T72_NOISE_SUPPRESSION_STATECHG) { /* check noise state1*/
			if ((msg[2] &
			     MXT_T72_NOISE_SUPPRESSION_STATUS2STATE_MASK) == /* check noise state2*/
	...
	if (msg[1] &
		...
		if ((mxt_core_data->noise_state ==
			...
			TS_LOG_DEBUG(
				...
				msg[2] &
					MXT_T72_NOISE_SUPPRESSION_STATUS2STATE_MASK); /*noise state2 bit0~bit2 is valid*/
			if (mxt_core_data->mxt_cur_finger_number == 0) {
				mxt_enable_feature(data, MAX_WORKAROUND4_CONF);
				mxt_core_data->t72_state_change =
					T72_STATE_NOT_CHANGE;
			} else {
				mxt_core_data->t72_state_change =
					T72_STATE_CHANGED;
			}
	...
	if (MXT_FEATURE_ENABLE ==
		...
		if (mxt_core_data->t72_noise_level.noise_level_max <
		    noise_level) {
			mxt_core_data->t72_noise_level.noise_level_max =
				noise_level;
		}
		if (mxt_core_data->t72_noise_level.noise_level_min >
		    noise_level) {
...
static void mxt_proc_T92_messages(struct mxt_data *data, u8 *msg)
{
	...
	TS_LOG_DEBUG("T92 %s 0x%x Info: %x %x %x %x %x %x\n",
		     (status & 0x80) ? "stroke" : "symbol", status, msg[2],
...
static int atmel_diagnostic_command_roi(struct mxt_data *data, u8 cmd, u8 page,
					u8 index, u8 num, char *buf,
					int interval, int interval_c)
{
	...
	if (buf) {
		...
		ret = __atmel_read_reg(data, data->T37_address,
		...
		/*read data*/
		ret = __atmel_read_reg(data,
				       data->T37_address +
					       sizeof(command_register) + index,
				       num, buf);
		if (ret) {
			TS_LOG_ERR(
		...
		TS_LOG_DEBUG(
			...
			command_register[0], command_register[1], buf[0],
			buf[1], buf[2], buf[3]);
	}
...
static void atmel_read_roi_data(struct mxt_data *data)
{
	...
	do {
		TS_LOG_DEBUG("%s \n", __func__);
		ret = atmel_diagnostic_command_roi(data,
						   MXT_T6_DEBUG_PEAKREGIONDATA,
						   0, 0, MISC_PEAKDATA_LEN_U8,
						   (char *)buf, 10, 5);
		if (ret == 0) {
			break;
		}
...
static void mxt_proc_t100_message_number(struct mxt_data *data, u8 *message)
{
	...
	atmel_parse_t100_scr_message(message, data->tchcfg[MXT_T100_SCRAUX],
				     &info);
	TS_LOG_DEBUG("[scr] status:%02X  [num]:%d [area]:%d %d %d\n", status,
		     info.num_tch, info.area_tch, info.area_atch,
		     info.area_inttch);
...
static int mxt_atmel_process_messages_t44(struct mxt_data *data)
{
	...
	ret = __atmel_read_reg(data, data->T44_address, data->T5_msg_size + 1,
			       data->msg_buf);
	if (ret) {
	...
	if (num_left) {
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"mxt_read_and_atmel_process_messages failed\n");
			goto end;
				 ...
				 struct ts_cmd_node *out_cmd)
{
	...
	if (!data || !data->atmel_chip_data || !out_cmd) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		return -EINVAL;
	}
...
static int atmel_print_chip_info(void)
{
	...
	ret = __atmel_read_reg(data, t38->start_address, sizeof(version_byte),
			       version_byte);
	if (ret) {
	...
	TS_LOG_INFO("%s, version_name = %s\n", __func__,
		    data->atmel_chip_data->version_name);
	return ret;
}
...
updata_firmware_again:
	...
	if (featurefile_read_success ==
	    FEATUREFILE_NOT_READ) { //get glove, holster e.g.  init status
...
static int atmel_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if (!data || !info || !data->atmel_chip_data ||
	...
	if (unlikely((atomic_read(&data->atmel_chip_data->ts_platform_data
					   ->state) == TS_SLEEP) ||
		     (atomic_read(&data->atmel_chip_data->ts_platform_data
					   ->state) == TS_WORK_IN_SLEEP))) {
		for (i = 1; i < VERSION_NAME_LEN;
		     i++) { // the first byte not used
			version_byte[i] = data->ver_bytes[i];
		}
	} else {
		ret = __atmel_read_reg(data, t38->start_address,
				       sizeof(version_byte), version_byte);
		if (ret) {
			TS_LOG_ERR("%s failed to read t38\n", __func__);
			return ret;
		}
...
#if defined(CONFIG_HONOR_DSM)
		ts_dmd_report(
			DSM_TP_IN_MOISTURE_ERROR_NO,
			"TP IC works in moisture mode,moisture count = %d.\n",
...
#if defined(CONFIG_HONOR_DSM)
		ts_dmd_report(
			DSM_TP_IN_VNOISE_ERROR_NO,
			"TP IC works in very noise mode,vnoise count = %d.\n",
...
static int atmel_after_resume(void *feature_info)
{
	...
	TS_LOG_INFO(
		...
		mxt_core_data->sta_cnt.noise_state,
		mxt_core_data->sta_cnt.vnoise_state);
	return retval;
}
...
static int mxt_dualX_resume(struct mxt_data *data, u8 *dualX_status)
{
	...
	retval += mxt_write_object(data, MXT_PROCG_NOISESUPPRESSION_T72,
	...
	TS_LOG_DEBUG(
		...
		dualX_status[STABLE_STATUS], dualX_status[NOISE_STATUS],
		dualX_status[VNOISE_STATUS]);
	return retval;
}
...
static int mxt_dualX_disable(struct mxt_data *data, u8 *dualX_status)
{
	...
	retval += mxt_read_object(data, MXT_PROCG_NOISESUPPRESSION_T72,
				  MXT_T72_NOISE_SUPPRESSION_STABCTRL,
				  dualX_status);
	retval += mxt_read_object(data, MXT_PROCG_NOISESUPPRESSION_T72,
				  MXT_T72_NOISE_SUPPRESSION_NOISCTRL,
	...
	tmp_dualX_status[STABLE_STATUS] =
		dualX_status[STABLE_STATUS] &
		(~MXT_T72_NOISE_SUPPRESSION_DUALXMODE);
	tmp_dualX_status[NOISE_STATUS] = dualX_status[NOISE_STATUS] &
					 (~MXT_T72_NOISE_SUPPRESSION_DUALXMODE);
	...
	TS_LOG_DEBUG(
		...
		tmp_dualX_status[STABLE_STATUS], tmp_dualX_status[NOISE_STATUS],
		tmp_dualX_status[VNOISE_STATUS]);
	return retval;
}

static void atmel_get_threshold_file_name(struct mxt_data *data,
					  char *file_name)
{
	int offset = 0;
	if (!data || !file_name || !data->atmel_chip_data ||
	    !data->atmel_chip_data->ts_platform_data) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		return;
	}
	...
	offset += snprintf(file_name + offset, MAX_FILE_NAME_LENGTH - offset,
			   "_");
	offset += snprintf(file_name + offset, MAX_FILE_NAME_LENGTH - offset,
	...
	if (data->fw_need_depend_on_lcd) {
		offset += snprintf(file_name + offset,
				   MAX_FILE_NAME_LENGTH - offset, "_");
		offset += snprintf(file_name + offset,
				   MAX_FILE_NAME_LENGTH - offset,
...
static int atmel_parse_threshold_file(void)
{
	...
	retval = request_firmware(&thresholdfile, file_name,
				  &data->atmel_dev->dev);
	if (retval < 0) {
	...
	retval = atmel_parse_threshold_file_method(&thresholdfile->data[0],
						   thresholdfile->size);
	if (retval < 0) {
	...
	for (case_num = 0; case_num < MAX_CASE_CNT; case_num++) {
			...
			for (index_count = 0; index_count < tx2tx_size;) {
				...
				if (retval >= 0) {
					data->rawdata_tx2tx_low_limit_buf
						[index_count1++] = 0 - retval;
					data->rawdata_tx2tx_up_limit_buf
						[index_count2++] = retval;
					index_count++;
				} else if (buf_offset >= m) {
					break;
			...
			for (index_count = 0; index_count < rx2rx_size;) {
				...
				if (retval >= 0) {
					data->rawdata_rx2rx_low_limit_buf
						[index_count1++] = 0 - retval;
					data->rawdata_rx2rx_up_limit_buf
						[index_count2++] = retval;
					index_count++;
				} else if (buf_offset >= m) {
					break;
		...
		case NOISE_LIMIT:
			...
			for (index_count = 0; index_count < noise_size;) {
				retval = atmel_get_one_value(buf, &buf_offset);
				if (retval >= 0) {
					data->noise_low_limit_buf[index_count1++] =
						0 - retval;
					data->noise_up_limit_buf[index_count2++] =
						retval;
					index_count++;
				} else if (buf_offset >= m) {
					break;
		...
		case RAWDATA_MIN_MAX_DELTA_LIMIT:
			for (index_count = 0;
			     index_count <
			     MAX_RAWDATA_MIN_MAX_DELTA_LIMIT_COUNT;) {
				retval = atmel_get_one_value(buf, &buf_offset);
				if (retval >= 0) {
...
static int atmel_get_one_value(const char *buf, uint32_t *offset)
{
	...
	/* Bypass extra commas */
	m = tmp_offset + 1;
	while (buf[tmp_offset] == ASCII_COMMA && buf[m] == ASCII_COMMA) {
		tmp_offset++;
		m = tmp_offset + 1;
	}
	...
	m = tmp_offset + 1;
	n = tmp_offset + 2;
	if (buf[tmp_offset] == ASCII_COMMA && buf[m] == ASCII_CR &&
	    buf[n] == ASCII_LF)
		tmp_offset += 2;
	else if (buf[tmp_offset] == ASCII_COMMA && buf[m] == ASCII_LF)
		tmp_offset += 1;
...
static int atmel_malloc_limit_buf(struct mxt_data *data)
{
	...
	TS_LOG_INFO("%s, tx2tx_size = %d, rx2rx_size = %d\n", __func__,
		    tx2tx_size, rx2rx_size);

	data->rawdata_low_limit_buf =
		kzalloc(rawdata_size * sizeof(int), GFP_KERNEL);
	data->rawdata_up_limit_buf =
		kzalloc(rawdata_size * sizeof(int), GFP_KERNEL);

	data->rawdata_rx2rx_low_limit_buf =
		kzalloc(rx2rx_size * sizeof(int), GFP_KERNEL);
	data->rawdata_rx2rx_up_limit_buf =
		kzalloc(rx2rx_size * sizeof(int), GFP_KERNEL);

	data->rawdata_tx2tx_up_limit_buf =
		kzalloc(tx2tx_size * sizeof(int), GFP_KERNEL);
	data->rawdata_tx2tx_low_limit_buf =
		kzalloc(tx2tx_size * sizeof(int), GFP_KERNEL);

	data->noise_low_limit_buf =
		kzalloc(noise_size * sizeof(int), GFP_KERNEL);
	...
	if (!data->rawdata_low_limit_buf || !data->rawdata_up_limit_buf ||
	    ...
	    !data->rawdata_rx2rx_up_limit_buf ||
	    !data->rawdata_tx2tx_up_limit_buf ||
	    !data->rawdata_tx2tx_low_limit_buf || !data->noise_low_limit_buf ||
	    !data->noise_up_limit_buf) {
			     ...
			     struct ts_cmd_node *out_cmd)
{
	...
	T37_buf_size = (size_t)(data->x_size - data->x_origin) *
		       (data->y_size - data->y_origin) * sizeof(int);

	TS_LOG_DEBUG(
		"%s: x = %d , y = %d\n Read ref*********************************\n",
		__func__, (data->info->matrix_xsize),
	...
	ret1 = atmel_get_refs_rx2rx_delta_test(data);
	ret2 = atmel_get_refs_tx2tx_delta_test(data);
	...
	retval = atmel_get_refs_or_deltas_data_test(data);
	if (retval) {
...
static void mxt_get_feature_file_name(char *feature_file_name, bool from_sd)
{
	...
	if (!data || !feature_file_name || !data->atmel_chip_data ||
	    !data->atmel_chip_data->ts_platform_data) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		return;
	}
	...
	if (!from_sd) {
		...
		snprintf(feature_file_name + offset,
			 ATMEL_FILE_NAME_LEN - offset, "_config_feature.raw");
	} else {
		offset = snprintf(feature_file_name, ATMEL_FILE_NAME_LEN,
...
static int mxt_parse_feature_file(const struct firmware *cfg)
{
	...
	int readobjectId = 0, readinstance = 0, readoffset = 0,
	    readnewvalue = 0;
	int i = 0, ret = NO_ERR;
	...
	for (featurecode = 0; featurecode < feature_list_count; featurecode++) {
		...
		regnum = mxt_search_string(tmpbuf + read_pos,
					   end_pos - read_pos, ';');
		TS_LOG_DEBUG(
		...
		if (NULL == feature->reg_value) {
			feature->reg_value = kzalloc(
				regnum * sizeof(struct feature_reg_value),
				GFP_KERNEL);
			if (NULL == feature->reg_value) {
				ret = -EINVAL;
				goto out;
		...
		for (i = 0; i < regnum; i++) {
			if (sscanf(tmpbuf + read_pos,
				   "OBJECT=T%d,INSTANCE=%d,OFFSIZE=%d,VALUE=%d;\n",
				   &readobjectId, &readinstance, &readoffset,
				   &readnewvalue) != 4) { /* get 4 para */
				ret = -EINVAL;
				goto out;
			}
...
static void mxt_glove_switch(u8 glove_switch)
{
	...
	case MXT_FEATURE_DISABLE:
		mxt_disable_all_features();
		atmel_t6_command(data, MXT_COMMAND_RESET, MXT_RESET_VALUE,
				 false);
...
static int atmel_regs_operate(struct ts_regs_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		if ((1 == info->num) && (MXT_WRITE_BIT_LEN > info->bit)) {
			retval = __atmel_read_reg(data, info->addr, info->num,
						  value);
			if (retval < 0) {
...
static int atmel_special_hardware_test_result(char *buf)
{
	...
	if (t_count >= 5) {
		if (s_count * 2 >=
		    t_count) { /*success rate is greater than 50%*/
	...
	ret = snprintf(buf, MAX_HARDWARE_TEST_RESULT_LEN,
...
static void atmel_chip_touch_switch(void)
{
	...
	if (TS_SWITCH_TYPE_DOZE !=
	    (data->atmel_chip_data->touch_switch_flag & TS_SWITCH_TYPE_DOZE)) {
	...
	memcpy(in_data, data->atmel_chip_data->touch_switch_info,
	       MAX_STR_LEN - 1);
	...
	case TS_SWITCH_DOZE_DISABLE:
		sdatatowrite = DEFAULT_ACTIVE2IDLE_TIME;
		/*Disable operation*/ /*50 means if no touch, then tp will change status from active to idle after 50/2*200ms(5s)  */
	...
	if (error) {
		TS_LOG_ERR("%s, %s, set doze mode to value %d,[fail]\n",
			   __func__,
			   (soper == TS_SWITCH_DOZE_ENABLE) ? "enable doze" :
							      "disable doze",
			   sdatatowrite);
	} else {
