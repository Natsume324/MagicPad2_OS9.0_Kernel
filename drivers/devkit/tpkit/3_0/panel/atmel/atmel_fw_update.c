...
static int atmel_get_lcd_panel_info(void)
{
	...
	strncpy(mxt_core_data->lcd_panel_info, lcd_type,
...
static int atmel_get_lcd_module_name(void)
{
	...
	strncpy(temp, mxt_core_data->lcd_panel_info,
		LCD_PANEL_INFO_MAX_LEN - 1);
 ...
 *
 */
static int atmel_diagnostic_command(struct mxt_data *data, u8 cmd, u8 page,
...
#define T37_COMMAND_RESEND_INDEX 2
	if (command_register[0] != cmd &&
		...
		if (abs((s8)current_page - (s8)page) >
		    (s8)page) { /* far away with dis page*/
			ret = atmel_t6_command(data, MXT_COMMAND_DIAGNOSTIC,
					       cmd, 0);
			if (ret) {
				TS_LOG_ERR(
					"T6 Command DIAGNOSTIC cmd 0x%x failed %d!\n",
	...
	while (current_page != page) {
		...
		ret = atmel_t6_command(data, MXT_COMMAND_DIAGNOSTIC, page_cmd,
				       0);
		if (ret) {
			TS_LOG_ERR(
		...
		if ((u8)command_register[0] != cmd) {
			TS_LOG_ERR(
				...
				cmd, page, current_page,
				(u8)command_register[0], command_register[1]);
			return -EBUSY;
		}
		current_page = command_register[1];
...
int atmel_pds_get_data(struct mxt_data *data, struct pds_code *code)
{
...
#endif
	do {
		ret = atmel_diagnostic_command(
			data, MXT_T6_DEBUG_PID, 0, 0, MISC_PDS_HEAD_LEN,
			(char *)&code->head[0], 10,
			5); //10: how long to read once, 5: retry times
		if (ret == 0) {
			...
			if (code->head[0] == PID_MAGIC_WORD0 &&
				...
				ret = atmel_diagnostic_command(
					data, MXT_T6_DEBUG_PID, 0,
					MISC_PDS_HEAD_LEN, MISC_PDS_DATA_LEN,
					(char *)&code->id[0], 10, 5);
				if (ret == 0)
					break;
			}
	...
	if (ret == 0) { //project id   AAAABBCCC   :AAAA product  , BB  IC num,  CCC   module num
		TS_LOG_INFO("Found PDS Data %c%c%c%c%c%c%c%c%c\n", code->id[0],
			    code->id[1], code->id[2], code->id[3], code->id[4],
			    code->id[5], code->id[6], code->id[7], code->id[8]);
		code->tp_color = code->id[MISC_PDS_DATA_LEN -
					  1]; // pds, byte 11 is tp color
	...
	for (i = 0; i < MISC_PDS_DATA_LEN; i++) { //project id 9bytes
		if (code->id[i] < MXT_MIN_CODEID ||
		    code->id[i] > MXT_MAX_CODEID) { // ASCII
			code->id[i] = 0;
			break;
		}
	}
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (atmel_update_sd_mode) { //sd update need not compara CRC
		TS_LOG_INFO(
			"%s, update config by sd mode do not judge version\n",
			__func__);
	} else {
		if (info_crc == data->info_crc) {
			if (config_crc == 0 || data->config_crc == 0) {
				...
				TS_LOG_INFO(
					"Config CRC 0x%06X, IC Config CRC 0x%06X: is same\n",
					info_crc, data->config_crc);
#if defined(CONFIG_MXT_CAL_TRIGGER_CAL_WHEN_CFG_MATCH)
				atmel_t6_command(data, MXT_COMMAND_CALIBRATE, 1,
						 false);
#endif
				ret = 0;
	...
	while (data_pos < cfg->size - OBJECT_CFG_HEAD_SIZE) {
		...
		ret = sscanf(cfg->data + data_pos, "%x %x %x%n", &type,
		...
		for (i = 0; i < size; i++) {
			ret = sscanf(cfg->data + data_pos, "%hhx%n", &val,
				     &offset);
			if (ret != MXT_GET_ONE_PARA) {
	...
	calculated_crc = atmel_calculate_crc(
		config_mem, data->T7_address - cfg_start_ofs, config_mem_size);
	...
	while (byte_offset < config_mem_size) {
		...
		ret = __atmel_write_reg(data, cfg_start_ofs + byte_offset, size,
					config_mem + byte_offset);
		if (ret != 0) {
...
int atmel_check_firmware_version(struct mxt_data *data)
{
	...
	/* Load information block and check */
	for (i = 0; i < sizeof(struct mxt_info); i++) {
		error = sscanf(cfg->data + data_pos, "%hhx%n",
			       (unsigned char *)&cfg_info + i, &offset);
		if (error != 1) {
			TS_LOG_ERR("Bad format\n");
			error = -EINVAL;
	...
	if (atmel_update_sd_mode) { //sd update need not to compare version
		error = 0;
		TS_LOG_INFO(
			"%s, update fimware by sd mode do not judge verison\n",
			__func__);
		goto release;
	}
...
int atmel_update_fw_file_name(struct mxt_data *data, char *file_name)
{
	...
	if (!atmel_update_sd_mode) { //auto update firmware
		...
		offset += snprintf(
			file_name + offset, MAX_FILE_NAME_LENGTH - 1 - offset,
			data->atmel_chip_data->ts_platform_data->product_name);
		offset += snprintf(file_name + offset,
				   MAX_FILE_NAME_LENGTH - 1 - offset, "_");
		offset += snprintf(file_name + offset,
				   MAX_FILE_NAME_LENGTH - 1 - offset,
		 ...
		 */
		offset = snprintf(file_name, PAGE_SIZE, "ts/atmel_firmware.fw");
	}
...
void atmel_update_config_file_name(struct mxt_data *data, char *file_name)
{
	...
	if (!data || !data->atmel_chip_data ||
	    !data->atmel_chip_data->ts_platform_data || !file_name) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		return;
	}
	...
	if (!atmel_update_sd_mode) { //auto update
		...
		if (data->fw_need_depend_on_lcd &&
		    !data->is_firmware_broken) { //if firmware is broken, use default config name
			offset += snprintf(file_name + offset,
					   MAX_FILE_NAME_LENGTH - 1 - offset,
					   "_");
			offset += snprintf(file_name + offset,
		...
		offset = snprintf(file_name, PAGE_SIZE, "ts/atmel_config.raw");
	}
...
static u8 mxt_get_bootloader_version(struct mxt_data *data, u8 val)
{
	...
	if (val & MXT_BOOT_EXTENDED_ID) {
		if (atmel_bootloader_read(data, &buf[0], 3) !=
		    0) { /* read 3 byte */
...
{
	int irq_gpio = 0;
	if (!data || (timeout < 0) || !data->atmel_chip_data ||
	    !data->atmel_chip_data->ts_platform_data) {
		TS_LOG_ERR("%s, param invalid\n", __func__);
		return -EINVAL;
	}
...
int atmel_load_fw(struct mxt_data *data)
{
	...
	if (!data->in_bootloader) {
		...
		ret = atmel_t6_command(data, MXT_COMMAND_RESET, MXT_BOOT_VALUE,
				       false);
		if (ret) {
		 ...
		 * family ID */
		do {
			ret = mxt_check_bootloader(
				data, MXT_WAITING_BOOTLOAD_CMD, false);
			if (ret == 0)
				break;
