...
static int disable_fw_monitor_mode(struct cts_device *cts_dev)
{
	...
	if (value & BIT(0)) {
		return cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_FLAG_BITS,
					 value & (~BIT(0)));
	}
...
{
	return cts_fw_reg_writeb(
		cts_dev, CTS_DEVICE_FW_REG_AUTO_CALIB_COMP_CAP_ENABLE, 0);
}
...
static int set_fw_work_mode(struct cts_device *cts_dev, u8 mode)
{
	...
	do {
		...
		if (ret) {
			TS_LOG_ERR(
				"Read firmware system busy register failed %d",
				ret);
			//return ret;
			continue;
		}
		...
		ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_GET_WORK_MODE,
				       &curr_mode);
		if (ret) {
			TS_LOG_ERR("Read firmware current work mode failed %d",
				   ret);
			//return ret;
			continue;
		}
...
static int set_display_state(struct cts_device *cts_dev, bool active)
{
	...
	if (ret) {
		TS_LOG_ERR("Write display access flag %02x failed %d",
			   access_flag, ret);
		return ret;
	}
	...
	if (ret) {
		TS_LOG_ERR("Restore display access flag %02x failed %d",
			   access_flag, ret);
		return ret;
	}
...
static int wait_test_complete(struct cts_device *cts_dev, int skip_frames)
{
	...
	for (i = 0; i < (skip_frames + 1); i++) {
		...
		for (j = 0; j < 1000; j++) {
			...
			if (ret) {
				TS_LOG_ERR("Get data ready flag failed %d",
					   ret);
				return ret;
			}
		...
		if (i < skip_frames) {
			...
			if (ret) {
				TS_LOG_ERR("Clr data ready flag failed %d",
					   ret);
				return ret;
			}
		}
...
static int set_fw_test_type(struct cts_device *cts_dev, u8 type)
{
	...
	if (type != type_readback) {
		TS_LOG_ERR("Set test type %u != readback %u", type,
			   type_readback);
		return -EFAULT;
	}
...
static bool set_short_test_type(struct cts_device *cts_dev, u8 type)
{
	...
	for (i = 0; i < 5; i++) {
		...
		ret = cts_fw_reg_writesb(cts_dev, 0x5000, &param,
					 sizeof(param));
		if (ret) {
			TS_LOG_ERR("Set short test type to %u failed %d", type,
				   ret);
			continue;
		}
...
#endif
		if (ret < 0 && ret != -EEXIST) {
			TS_LOG_INFO("Create dir '%s' failed %d(%s)", dirname,
...
#define END_BANNER "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n"
	...
	if (cts_test_data_filp) {
		cts_write_file(cts_test_data_filp, END_BANNER,
			       strlen(END_BANNER));
...
static void cts_dump_tsdata(struct cts_device *cts_dev, const char *desc,
			    const u16 *data, bool to_console)
{
...
#define DATA_FORMAT_STR "%-5u "
	...
	count = 0;
	count += scnprintf(
		line_buf + count, sizeof(line_buf) - count,
...
static bool is_invalid_node(u32 *invalid_nodes, u32 num_invalid_nodes, u16 row,
			    u16 col)
{
	int i;
...
static int validate_tsdata(struct cts_device *cts_dev, const char *desc,
			   u16 *data, u32 *invalid_nodes, u32 num_invalid_nodes,
...
#define SPLIT_LINE_STR "------------------------------"
	...
	TS_LOG_INFO(
		...
		num_invalid_nodes, min ? min[0] : INT_MIN,
		max ? max[0] : INT_MAX);
...
static int validate_comp_cap(struct cts_device *cts_dev, const char *desc,
			     u8 *cap, u32 *invalid_nodes, u32 num_invalid_nodes,
			     bool per_node, int *min, int *max)
{
#define SPLIT_LINE_STR "------------------------------"
...
static int wait_fw_to_normal_work(struct cts_device *cts_dev)
{
	...
	do {
		...
		ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_GET_WORK_MODE,
				       &work_mode);
		if (ret) {
			TS_LOG_ERR("Get fw curr work mode failed %d",
				   work_mode);
			continue;
		} else {
...
static int prepare_test(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"Set firmware work mode to WORK_MODE_CONFIG failed %d",
			ret);
		return ret;
	}
...
static void post_test(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"Set firmware work mode to WORK_MODE_NORMAL failed %d",
			ret);
	}
...
int cts_test_short(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	if (feature_ver > 0) {
		...
		if (ret) {
			TS_LOG_ERR("Set short test type to UNDEFINED failed %d",
				   ret);
			goto post_test;
		}
		...
		if (feature_ver <= 3) {
			...
			if ((val & (BIT(2) | BIT(5))) != 0) {
				ret = cts_hw_reg_writeb(cts_dev, 0x350E2,
							val & 0xDB);
				if (ret) {
					TS_LOG_ERR("Write 0x350E2 failed %d",
						   ret);
					goto post_test;
				}
			}
		...
		if (ret) {
			TS_LOG_ERR(
				"Set short test type to SHORT_TO_GND failed %d",
				ret);
			goto post_test;
		}
		...
		if (ret) {
			TS_LOG_ERR("Send command RECOVERY_TX_VOL failed %d",
				   ret);
			goto post_test;
		}
	...
	if (dump_test_data_to_file) {
		int r = cts_start_dump_test_data_to_file(
			param->test_data_filepath,
			!!(param->flags &
	...
	if (driver_validate_data) {
		ret = validate_tsdata(cts_dev, "GND-short", test_result,
				      param->invalid_nodes,
	...
	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_TEST_WITH_DISPLAY_ON,
			       &need_display_on);
	if (ret) {
	...
	if (need_display_on == 0) {
		...
		if (ret) {
			TS_LOG_ERR(
				"Set short test type to BETWEEN_COLS failed %d",
				ret);
			goto recovery_display_state;
		}
	}
	...
	if (driver_validate_data) {
		ret = validate_tsdata(cts_dev, "Col-short", test_result,
				      param->invalid_nodes,
				      param->num_invalid_node,
				      validate_data_per_node, param->min,
				      param->max);
	...
	if (ret) {
		TS_LOG_ERR("Set short test type to BETWEEN_ROWS failed %d",
			   ret);
		goto recovery_display_state;
	}
	...
	while (loopcnt > 1) {
		...
		if (driver_validate_data) {
			ret = validate_tsdata(cts_dev, "Row-short", test_result,
					      param->invalid_nodes,
					      param->num_invalid_node,
					      validate_data_per_node,
					      param->min, param->max);
			if (ret) {
				TS_LOG_ERR(
					"Short between columns test failed %d",
					ret);
				if (stop_if_failed) {
					goto recovery_display_state;
				}
...
show_test_result:
	...
	if (ret > 0) {
		TS_LOG_INFO(
			"Short test has %d nodes FAIL, ELAPSED TIME: %lldms",
			ret, ktime_to_ms(delta_time));
	} else if (ret < 0) {
		TS_LOG_INFO("Short test FAIL %d(%s), ELAPSED TIME: %lldms", ret,
...
int cts_test_open(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	if (driver_validate_data) {
		ret = validate_tsdata(cts_dev, "Open-circuit", test_result,
				      param->invalid_nodes,
				      param->num_invalid_node,
				      validate_data_per_node, param->min,
				      param->max);
...
show_test_result:
	...
	if (ret > 0) {
		...
		TS_LOG_INFO("Open test FAIL %d(%s), ELAPSED TIME: %lldms", ret,
			    cts_strerror(ret), ktime_to_ms(delta_time));
	} else {
...
int cts_test_reset_pin(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	TS_LOG_INFO("Reset-Pin test, flags: 0x%08x, "
		    "drive log file: '%s' buf size: %d",
		    param->flags, param->driver_log_filepath,
		    param->driver_log_buf_size);
			    ...
			    struct cts_test_param *param)
{
	...
	if (driver_validate_data) {
		ret = validate_comp_cap(cts_dev, "Compensate-Cap", cap,
					param->invalid_nodes,
					param->num_invalid_node,
					validate_data_per_node, param->min,
					param->max);
...
int cts_test_rawdata(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	for (frame = 0; frame < priv_param->frames; frame++) {
		...
		if (r) {
			TS_LOG_ERR("Send CMD_QUIT_GESTURE_MONITOR failed %d",
				   r);
		}
		//}
		...
		if (driver_validate_data) {
			ret = validate_tsdata(cts_dev, "Rawdata", rawdata,
					      param->invalid_nodes,
					      param->num_invalid_node,
					      validate_data_per_node,
					      param->min, param->max);
...
show_test_result:
	...
	if (ret > 0) {
		TS_LOG_INFO(
			"Noise test has %d nodes FAIL, ELAPSED TIME: %lldms",
			ret, ktime_to_ms(delta_time));
	} else if (ret < 0) {
		TS_LOG_INFO("Noise test FAIL %d(%s), ELAPSED TIME: %lldms", ret,
			    cts_strerror(ret), ktime_to_ms(delta_time));
	} else {
