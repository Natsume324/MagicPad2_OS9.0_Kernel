		       ...
		       const void *src, size_t len, int retry, int delay)
{
	...
	if (cts_data->cts_dev.rtdata.program_mode) {
		...
		do {
			ret = cts_spi_send_recv(pdata, len + 7,
						cts_data->spi_tx_buf,
						cts_data->spi_rx_buf);
			udelay(10 * data_len);
			if (ret) {
		      ...
		      int retry, int delay)
{
	...
	if (wlen > CFG_CTS_MAX_SPI_XFER_SIZE ||
		...
		TS_LOG_ERR("write/read too much data:wlen=%zd, rlen=%zd", wlen,
			   rlen);
		return -EIO;
	}
	...
	if (cts_data->cts_dev.rtdata.program_mode) {
		...
		do {
			if (wlen != 0) {
				...
				put_unaligned_le16(rlen,
						   &cts_data->spi_tx_buf[3]);
				crc = (u16)cts_data_check(cts_data->spi_tx_buf,
							  5);
				put_unaligned_le16(crc,
						   &cts_data->spi_tx_buf[5]);
			...
			if (get_unaligned_le16(&cts_data->spi_rx_buf[rlen]) !=
			    crc) {
				TS_LOG_ERR(
					"SPI RX CRC error: rx_crc %04x != %04x",
					get_unaligned_le16(
						&cts_data->spi_rx_buf[rlen]),
					crc);
				continue;
			}
			return 0;
...
int cts_plat_is_normal_mode(struct cts_platform_data *pdata)
{
	...
	ret = cts_plat_spi_read(pdata, CTS_DEV_NORMAL_MODE_SPIADDR, tx_buf, 2,
				&fwid, 2, 3, 10);
