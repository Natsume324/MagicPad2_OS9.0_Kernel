...
#define SFCTRLv2_CMD_RDID_FLAGS \
	(SFCTRLv2_OPFLAG_READ | SFCTRLv2_OPFLAG_SRAM_DATA_XCHG)

#define SFCTRLv2_CMD_RDSR_FLAGS \
	(SFCTRLv2_OPFLAG_READ | SFCTRLv2_OPFLAG_SRAM_DATA_XCHG)

#define SFCTRLv2_CMD_SE_FLAGS \
	(SFCTRLv2_OPFLAG_SET_FLASH_ADDR | SFCTRLv2_OPFLAG_WAIT_WIP_CLR)

#define SFCTRLv2_CMD_BE_FLAGS \
	(SFCTRLv2_OPFLAG_SET_FLASH_ADDR | SFCTRLv2_OPFLAG_WAIT_WIP_CLR)

#define SFCTRLv2_CMD_PP_FLAGS                                              \
	(SFCTRLv2_OPFLAG_SET_FLASH_ADDR | SFCTRLv2_OPFLAG_SRAM_DATA_XCHG | \
	 SFCTRLv2_OPFLAG_SET_DATA_LENGTH | SFCTRLv2_OPFLAG_WAIT_WIP_CLR)

#define SFCTRLv2_CMD_FAST_READ_FLAGS                             \
	(SFCTRLv2_OPFLAG_READ | SFCTRLv2_OPFLAG_SET_FLASH_ADDR | \
	 SFCTRLv2_OPFLAG_SRAM_DATA_XCHG | SFCTRLv2_OPFLAG_SET_DATA_LENGTH)
...
#define DEFINE_SFCTRL_REG_ACCESS_FUNC(access_type, data_type)                \
	static inline int sfctrl_reg_ ## access_type(                        \
		const struct cts_device *cts_dev, u32 reg, data_type data)   \
	{                                                                    \
		return cts_hw_reg_ ## access_type(                           \
			cts_dev, sfctrl_reg_addr(cts_dev, reg), data);       \
	}
// clang-format on
...
#define RDSR_XCHG_SRAM_ADDR (cts_dev->hwdata->sram_size - 1)
	...
	sfctrl_write_reg_check_ret(writeb, rSFCTRLv2_CMD_SEL,
				   SFCTRLv2_CMD_RDSR);
	sfctrl_write_reg_check_ret(writel, rSFCTRLv2_SRAM_ADDR,
				   RDSR_XCHG_SRAM_ADDR);
...
static int sfctrlv2_transfer(const struct cts_device *cts_dev, u8 cmd,
			     void *data, u32 flash_addr, u32 sram_addr,
			     size_t size, u32 flags)
{
	...
	if (flags & SFCTRLv2_OPFLAG_SET_FLASH_ADDR) {
		sfctrl_write_reg_check_ret(writel, rSFCTRLv2_FLASH_ADDR,
					   flash_addr);
	}
	if (flags & SFCTRLv2_OPFLAG_SET_DATA_LENGTH) {
		sfctrl_write_reg_check_ret(writel, rSFCTRLv2_DATA_LENGTH,
					   (u32)size);
	}
...
static int sfctrlv2_rdid(const struct cts_device *cts_dev, u32 *id)
{
	...
	ret = sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_RDID, buf, 0,
				cts_dev->hwdata->sfctrl->xchg_sram_base, 3,
...
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_SE, NULL, sector_addr, 0,
				 0, SFCTRLv2_CMD_SE_FLAGS);
}

static int sfctrlv2_be(const struct cts_device *cts_dev, u32 block_addr)
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_BE, NULL, block_addr, 0,
				 0, SFCTRLv2_CMD_BE_FLAGS);
}

static int sfctrlv2_pp(const struct cts_device *cts_dev, u32 flash_addr,
...
static int sfctrlv2_program_flash_from_sram(const struct cts_device *cts_dev,
					    ...
					    size_t len)
{
	return sfctrlv2_transfer(cts_dev, SFCTRLv2_CMD_PP, NULL, flash_addr,
				 sram_addr, len, SFCTRLv2_CMD_PP_FLAGS);
}

static int sfctrlv2_read(const struct cts_device *cts_dev, u32 flash_addr,
...
int sfctrlv2_calc_sram_crc(const struct cts_device *cts_dev, u32 sram_addr,
			   size_t size, u32 *crc)
{
	int ret;
...
int sfctrlv2_calc_flash_crc(const struct cts_device *cts_dev, u32 flash_addr,
			    size_t size, u32 *crc)
{
	int ret;
