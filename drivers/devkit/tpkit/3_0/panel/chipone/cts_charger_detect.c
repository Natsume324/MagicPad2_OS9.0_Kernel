...
static enum power_supply_property power_supply_prop_from_name(const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(power_supply_prop_token); i++) {
		if (strcasecmp(power_supply_prop_token[i].name + offset,
			       name) == 0 ||
				   ...
				   struct device_node *np)
{
	...
	cd_data->enable = of_property_read_bool(
		np, "chipone,touch-charger-detect-enable");
#endif /* CFG_CTS_DEF_CHGR_DET_ENABLE */

	cd_data->type = CFG_CTS_DEF_CHGR_DET_TYPE;
	ret = of_property_read_string(np, "chipone,touch-charger-detect-type",
				      &type_str);
	if (ret) {
	...
	ret = of_property_read_string(
		np, "chipone,touch-charger-detect-psy-name", &psy_name);
	if (ret) {
	...
	ret = of_property_read_u32(
		np, "chipone,touch-charger-detect-psp-poll-interval",
...
static int start_charger_detect(struct cts_charger_detect_data *cd_data)
{
	...
	TS_LOG_INFO("Start detect type: %d(%s)", cd_data->type,
...
static int stop_charger_detect(struct cts_charger_detect_data *cd_data)
{
	...
	TS_LOG_INFO("Stop detect type: %d(%s)", cd_data->type,
		    charger_detect_type_str(cd_data->type));
...
static int get_charger_state(struct cts_charger_detect_data *cd_data)
{
	...
	TS_LOG_DEBUG("Get state from psy: '%s' prop: %d(%s)", cd_data->psy_name,
	...
	if (cd_data->psy == NULL) {
		if (cd_data->psy_name == NULL || cd_data->psy_name[0] == '\0') {
			...
			cd_data->psy =
				power_supply_get_by_name(cd_data->psy_name);
			if (cd_data->psy == NULL) {
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"Get state from psy: '%s' prop: %d(%s) failed %d(%s)",
			cd_data->psy_name, cd_data->psp,
			power_supply_prop_str(cd_data->psp), ret,
			cts_strerror(ret));
		return -EINVAL;
	}
...
#ifdef CFG_CTS_CHARGER_DETECT_PSY_NOTIFY
static int psy_notify_callback(struct notifier_block *nb, unsigned long action,
			       void *data)
{
	struct cts_charger_detect_data *cd_data;
	struct power_supply *psy;
...
{
	TS_LOG_INFO("Enable detect type: %d(%s)", cd_data->type,
		    charger_detect_type_str(cd_data->type));
...
static int disable_charger_detect(struct cts_charger_detect_data *cd_data)
{
	...
	TS_LOG_INFO("Disable detect type: %d(%s)", cd_data->type,
		    charger_detect_type_str(cd_data->type));
					   ...
					   const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write sysfs '" CHARGER_DET_SYSFS_GROUP_NAME
		    "/%s' size %zu",
				      ...
				      enum cts_charger_detect_type type)
{
	...
	TS_LOG_INFO("Switch detect type to %d(%s)", type,
		    charger_detect_type_str(type));
...
static DEVICE_ATTR(type, S_IWUSR | S_IRUSR | S_IRGRP, charger_detect_type_show,
		   charger_detect_type_store);
...
static struct attribute *charger_detect_attrs[] = {
	&dev_attr_enable.attr, &dev_attr_running.attr, &dev_attr_type.attr,
	&dev_attr_state.attr,  &dev_attr_param.attr,   NULL};
...
#ifdef CONFIG_CTS_SYSFS
	TS_LOG_INFO("Create sysfs attr group '%s'",
