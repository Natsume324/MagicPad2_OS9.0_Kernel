...
#define CFG_CTS_DEF_EARJACK_STATE_FILEPATH "/sys/kernel/headset/state"
#else
				   ...
				   struct device_node *np)
{
	...
	ed_data->enable = of_property_read_bool(
		np, "chipone,touch-earjack-detect-enable");
#endif /* CFG_CTS_DEF_EARJACK_DET_ENABLE */

	ret = of_property_read_string(
		np, "chipone,touch-earjack-state-filepath", &filepath);
	if (ret) {
	...
	ret = of_property_read_u32(np, "chipone,touch-earjack-poll-interval",
				   &ed_data->poll_interval);
	if (ret) {
					  ...
					  char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "Earjack detect: %s\n",
					   ...
					   const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write sysfs '" EARJACK_DET_SYSFS_GROUP_NAME
		    "/%s' size %zu",
					   ...
					   char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "Earjack detect: %sRunning\n",
			 ed_data->running ? "" : "Not-");
}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "Earjack state: %s\n",
...
static void poll_earjack_state_work(struct work_struct *work)
{
	...
	if (ret) {
		...
		if (ed_data->state != prev_state) {
			...
			if (ret) {
				TS_LOG_ERR(
					"Set dev earjack attached to %s failed %d",
...
int cts_init_earjack_detect(struct chipone_ts_data *cts_data)
{
	...
	ed_data->poll_interval = CFG_CTS_DEF_EARJACK_POLL_INTERVAL;
	ed_data->earjack_state_filepath =
		kstrdup(CFG_CTS_DEF_EARJACK_STATE_FILEPATH, GFP_KERNEL);
	if (ed_data->earjack_state_filepath == NULL) {
		TS_LOG_ERR("Dup earjack state filepath failed");
		ret = -ENOMEM;
	}
