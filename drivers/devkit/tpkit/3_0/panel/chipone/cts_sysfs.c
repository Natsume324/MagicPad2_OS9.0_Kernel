...
#define PRINT_ROW_SIZE (16)
	...
	if (ret) {
		count = scnprintf(buf, PAGE_SIZE, "Read hw register failed %d",
				  ret);
		goto err_free_data;
	}
...
static DEVICE_ATTR(read_hw_reg, S_IWUSR | S_IRUSR | S_IRGRP, read_hw_reg_show,
		   read_hw_reg_store);
...
static DEVICE_ATTR(curr_version, S_IRUSR | S_IRGRP, curr_firmware_version_show,
		   NULL);
...
static DEVICE_ATTR(curr_ddi_version, S_IRUSR | S_IRGRP, curr_ddi_version_show,
		   NULL);
...
static ssize_t cols_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t res_x_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static ssize_t res_y_show(struct device *dev, struct device_attribute *attr,
			  char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(esd_protection, S_IRUSR | S_IRGRP, esd_protection_show,
		   NULL);
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	cts_lock_device(cts_dev);
	ret = cts_fw_reg_readb(&cts_data->cts_dev, CTS_DEVICE_FW_REG_FLAG_BITS,
			       &value);
	cts_unlock_device(cts_dev);
	if (ret) {
		return scnprintf(
				  ...
				  const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write firmware monitor mode to '%c', %s", buf[0],
	...
	cts_unlock_device(cts_dev);
	if (ret) {
		TS_LOG_ERR("Write firmware monitor enable register failed %d",
			   ret);
		return -EIO;
	}
...
static DEVICE_ATTR(monitor_mode, S_IRUSR | S_IRGRP, monitor_mode_show,
		   monitor_mode_store);
...
static DEVICE_ATTR(auto_compensate, S_IRUSR | S_IRGRP, auto_compensate_show,
		   NULL);
					     ...
					     const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Update driver builtin firmware '%s' to %s", argv[1],
		    to_flash ? "flash" : "sram");
...
static ssize_t updating_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Read flash from 0x%06x size %u%s%s", flash_addr, size,
...
#define PRINT_ROW_SIZE (16)
		remaining = size;
		for (i = 0; i < size && count <= PAGE_SIZE;
		     i += PRINT_ROW_SIZE) {
			size_t linelen =
...
static ssize_t open_test_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	...
	if (ret > 0) {
		...
		return scnprintf(
			...
			cts_strerror(ret), ktime_to_ms(delta_time));
	} else {
		return scnprintf(buf, PAGE_SIZE,
...
static DEVICE_ATTR(open_test, S_IWUSR | S_IRUSR | S_IRGRP, open_test_show,
		   open_test_store);
...
static DEVICE_ATTR(short_test, S_IWUSR | S_IRUSR | S_IRGRP, short_test_show,
		   short_test_store);
...
static DEVICE_ATTR(reset_pin_test, S_IRUSR | S_IRGRP, reset_pin_test_show,
		   NULL);
					...
					char *buf)
{
	...
	TS_LOG_INFO("Compensate cap test, min: %u, max: %u", min, max);
...
static DEVICE_ATTR(rawdata_test, S_IWUSR | S_IRUSR | S_IRGRP, rawdata_test_show,
		   rawdata_test_store);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Noise test, frames: %u threshold: %d", priv_param.frames,
		    max);
...
static DEVICE_ATTR(noise_test, S_IWUSR | S_IRUSR | S_IRGRP, noise_test_show,
		   noise_test_store);
...
#define COMP_CAP_TEST_DATA_FILENAME "comp-cap-test-data.txt"
	...
	snprintf(
		touch_data_filepath, sizeof(touch_data_filepath),
		TOUCH_DATA_DIRECTORY_PREFIX
		"touch_data_%04d%02d%02d_%02d%02d%02d/" RAWDATA_TEST_DATA_FILENAME,
	...
	snprintf(
		touch_data_filepath, sizeof(touch_data_filepath),
		TOUCH_DATA_DIRECTORY_PREFIX
		"touch_data_%04d%02d%02d_%02d%02d%02d/" NOISE_TEST_DATA_FILENAME,
		rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		rtc_tm.tm_hour, rtc_tm.tm_min, rtc_tm.tm_sec);
	...
	snprintf(
		touch_data_filepath, sizeof(touch_data_filepath),
		TOUCH_DATA_DIRECTORY_PREFIX
		"touch_data_%04d%02d%02d_%02d%02d%02d/" OPEN_TEST_DATA_FILENAME,
		rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		rtc_tm.tm_hour, rtc_tm.tm_min, rtc_tm.tm_sec);
	...
	snprintf(
		touch_data_filepath, sizeof(touch_data_filepath),
		TOUCH_DATA_DIRECTORY_PREFIX
		"touch_data_%04d%02d%02d_%02d%02d%02d/" SHORT_TEST_DATA_FILENAME,
		rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		rtc_tm.tm_hour, rtc_tm.tm_min, rtc_tm.tm_sec);
	...
	snprintf(
		touch_data_filepath, sizeof(touch_data_filepath),
		TOUCH_DATA_DIRECTORY_PREFIX
		"touch_data_%04d%02d%02d_%02d%02d%02d/" COMP_CAP_TEST_DATA_FILENAME,
		rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
		rtc_tm.tm_hour, rtc_tm.tm_min, rtc_tm.tm_sec);
	...
	TS_LOG_INFO("Factory test: rawdata: [%u, %u] noise: %u, "
		    ...
		    "touch data dir: " TOUCH_DATA_DIRECTORY_PREFIX
		    "%04d%02d%02d_%02d%02d%02d/",
		    rawdata_min, rawdata_max, noise_max, open_min, short_min,
		    comp_cap_min, comp_cap_max, rtc_tm.tm_year + 1900,
	...
	if (reset_pin_test_result) {
		...
		count += scnprintf(buf + count, PAGE_SIZE, "%-10s test: PASS\n",
				   "Reset-Pin");
	}
...
static DEVICE_ATTR(program_mode, S_IWUSR | S_IRUSR | S_IRGRP, program_mode_show,
		   program_mode_store);

static ssize_t rawdata_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
#define RAWDATA_BUFFER_SIZE(cts_dev) \
	...
	if (data_valid) {
		...
		for (r = 0; r < cts_dev->fwdata.rows && count < PAGE_SIZE;
		     r++) {
			count += scnprintf(buf + count, PAGE_SIZE - count,
...
static ssize_t diffdata_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
...
static ssize_t baseline_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
#define BASELINE_BUFFER_SIZE(cts_dev) \
	...
	cts_lock_device(cts_dev);
	ret = cts_get_baseline(
		cts_dev, baseline, CTS_WORK_MODE_NORMAL_ACTIVE,
	...
	if (ret) {
		kfree(baseline);
		return scnprintf(buf, PAGE_SIZE, "Get baseline failed %d(%s)\n",
	...
	for (r = 0; r < cts_dev->fwdata.rows && count < PAGE_SIZE; r++) {
		count += scnprintf(buf + count, PAGE_SIZE - count,
				   ROW_NUM_FORMAT_STR, r);
		for (c = 0; c < cts_dev->fwdata.cols && count < PAGE_SIZE;
		     c++) {
			count += scnprintf(
				buf + count, PAGE_SIZE - count, DATA_FORMAT_STR,
				baseline[r * cts_dev->fwdata.cols + c]);
		}
		buf[count++] = '\n';
	}
		...
		if (data_valid) {
			...
			for (r = 0; r < cts_dev->fwdata.rows; r++) {
				for (c = 0; c < cts_dev->fwdata.cols; c++) {
					...
					val = rawdata[r * cts_dev->fwdata.cols +
						      c];
					sum += val;
					if (val > max) {
						max = val;
...
static DEVICE_ATTR(manualdiff, S_IWUSR | S_IRUSR | S_IRGRP, manualdiffdata_show,
...
static DEVICE_ATTR(jitter, S_IWUSR | S_IRUSR | S_IRGRP, jitter_show,
		   jitter_store);
...
static DEVICE_ATTR(compensate_cap, S_IRUSR | S_IRGRP, compensate_cap_show,
		   NULL);

#ifdef CFG_CTS_HAS_RESET_PIN
static ssize_t reset_pin_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	...
	return scnprintf(buf, PAGE_SIZE, "Reset pin: %d, status: %d\n",
...
static ssize_t irq_pin_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	...
	return scnprintf(buf, PAGE_SIZE, "IRQ pin: %d, status: %d\n",
			     ...
			     char *buf)
{
	...
	return scnprintf(
		...
		cts_data->pdata->irq, desc->depth, desc->irq_count,
		desc->irqs_unhandled, desc->last_unhandled);
}
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "Fw log redirect is %s\n",
			 cts_is_fw_log_redirect(cts_dev) ? "enable" :
							   "disable");
}
...
static ssize_t debug_spi_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	return scnprintf(
		buf, PAGE_SIZE, "spi_speed=%d\n",
...
static DEVICE_ATTR(gesture_en, S_IWUSR | S_IRUSR | S_IRGRP, gesture_en_show,
		   gesture_en_store);
#endif
...
static const struct attribute_group *cts_dev_attr_groups[] = {
	&cts_dev_firmware_attr_group, &cts_dev_flash_attr_group,
	&cts_dev_test_attr_group, &cts_dev_misc_attr_group, NULL};
