...
static int cts_i2c_writeb(const struct cts_device *cts_dev, u32 addr, u8 b,
			  int retry, int delay)
{
	u8 buff[8];
...
static int cts_i2c_writew(const struct cts_device *cts_dev, u32 addr, u16 w,
			  int retry, int delay)
{
	u8 buff[8];
...
static int cts_i2c_writel(const struct cts_device *cts_dev, u32 addr, u32 l,
			  int retry, int delay)
{
	u8 buff[8];
			   ...
			   const u8 *src, size_t len, int retry, int delay)
{
	...
	while (len) {
		payload_len = min(
			(size_t)(max_xfer_size - cts_dev->rtdata.addr_width),
			len);
...
static int cts_i2c_readb(const struct cts_device *cts_dev, u32 addr, u8 *b,
			 int retry, int delay)
{
	u8 addr_buf[4];
...
static int cts_i2c_readw(const struct cts_device *cts_dev, u32 addr, u16 *w,
			 int retry, int delay)
{
	int ret;
	u8 addr_buf[4];
...
static int cts_i2c_readl(const struct cts_device *cts_dev, u32 addr, u32 *l,
			 int retry, int delay)
{
	int ret;
	u8 addr_buf[4];
...
static int cts_i2c_readsb(const struct cts_device *cts_dev, u32 addr, void *dst,
...
#else
static int cts_spi_writeb(const struct cts_device *cts_dev, u32 addr, u8 b,
			  int retry, int delay)
{
	u8 buff[8];
...
static int cts_spi_writew(const struct cts_device *cts_dev, u32 addr, u16 w,
			  int retry, int delay)
{
	u8 buff[8];
...
static int cts_spi_writel(const struct cts_device *cts_dev, u32 addr, u32 l,
			  int retry, int delay)
{
	u8 buff[8];
...
static int cts_spi_readb(const struct cts_device *cts_dev, u32 addr, u8 *b,
			 int retry, int delay)
{
	u8 addr_buf[4];
...
static int cts_spi_readw(const struct cts_device *cts_dev, u32 addr, u16 *w,
			 int retry, int delay)
{
	int ret;
	u8 addr_buf[4];
...
static int cts_spi_readl(const struct cts_device *cts_dev, u32 addr, u32 *l,
			 int retry, int delay)
{
	int ret;
	u8 addr_buf[4];
...
static int cts_spi_readsb(const struct cts_device *cts_dev, u32 addr, void *dst,
			  size_t len, int retry, int delay)
{
	int ret;
	u8 addr_buf[4];
...
static int cts_spi_readsb_delay_idle(const struct cts_device *cts_dev, u32 addr,
				     void *dst, size_t len, int retry,
				     int delay, int idle)
{
	int ret;
	u8 addr_buf[4];
	...
	while (len) {
		...
		ret = cts_plat_spi_read_delay_idle(
			cts_dev->pdata, cts_dev->rtdata.slave_addr, addr_buf,
			cts_dev->rtdata.addr_width, dst, xfer_len, retry, delay,
			idle);
		if (ret) {
			TS_LOG_ERR("Platform i2c read failed %d", ret);
			return ret;
...
static inline int cts_dev_writeb(const struct cts_device *cts_dev, u32 addr,
				 u8 b, int retry, int delay)
{
#ifdef CONFIG_CTS_I2C_HOST
...
static inline int cts_dev_writew(const struct cts_device *cts_dev, u32 addr,
...
static inline int cts_dev_writel(const struct cts_device *cts_dev, u32 addr,
				 u32 l, int retry, int delay)
{
#ifdef CONFIG_CTS_I2C_HOST
...
static inline int cts_dev_readb(const struct cts_device *cts_dev, u32 addr,
				u8 *b, int retry, int delay)
{
#ifdef CONFIG_CTS_I2C_HOST
...
static inline int cts_dev_readw(const struct cts_device *cts_dev, u32 addr,
				u16 *w, int retry, int delay)
{
#ifdef CONFIG_CTS_I2C_HOST
...
static inline int cts_dev_readl(const struct cts_device *cts_dev, u32 addr,
				u32 *l, int retry, int delay)
{
#ifdef CONFIG_CTS_I2C_HOST
...
static inline int cts_dev_readsb(const struct cts_device *cts_dev, u32 addr,
...
int cts_sram_writeb_retry(const struct cts_device *cts_dev, u32 addr, u8 b,
			  int retry, int delay)
{
...
int cts_sram_writew_retry(const struct cts_device *cts_dev, u32 addr, u16 w,
			  int retry, int delay)
{
	u8 buff[2];
...
int cts_sram_writel_retry(const struct cts_device *cts_dev, u32 addr, u32 l,
			  int retry, int delay)
{
	u8 buff[4];
...
int cts_sram_writesb_retry(const struct cts_device *cts_dev, u32 addr,
...
static int cts_calc_sram_crc(const struct cts_device *cts_dev, u32 sram_addr,
			     size_t size, u32 *crc)
{
	TS_LOG_INFO("Calc crc from sram 0x%06x size %zu", sram_addr, size);

	return cts_dev->hwdata->sfctrl->ops->calc_sram_crc(cts_dev, sram_addr,
							   size, crc);
}

int cts_sram_writesb_check_crc_retry(const struct cts_device *cts_dev, u32 addr,
				     const void *src, size_t len, u32 crc,
				     int retry)
{
	int ret, retries;
	...
	do {
		...
		if ((ret = cts_calc_sram_crc(cts_dev, 0, len, &crc_sram)) !=
		    0) {
			TS_LOG_ERR(
...
static int cts_read_sram_normal_mode(const struct cts_device *cts_dev, u32 addr,
				     void *dst, size_t len, int retry,
				     int delay)
{
	int i, ret;
...
int cts_sram_readb_retry(const struct cts_device *cts_dev, u32 addr, u8 *b,
			 int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_sram_readw_retry(const struct cts_device *cts_dev, u32 addr, u16 *w,
			 int retry, int delay)
{
	int ret;
	u8 buff[2];
	...
	if (cts_dev->rtdata.program_mode) {
		...
		ret = cts_read_sram_normal_mode(cts_dev, addr, buff, 2, retry,
						delay);
		if (ret) {
...
int cts_sram_readl_retry(const struct cts_device *cts_dev, u32 addr, u32 *l,
			 int retry, int delay)
{
	int ret;
	u8 buff[4];
...
int cts_sram_readsb_retry(const struct cts_device *cts_dev, u32 addr, void *dst,
			  size_t len, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_fw_reg_writeb_retry(const struct cts_device *cts_dev, u32 reg_addr,
			    u8 b, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_fw_reg_writew_retry(const struct cts_device *cts_dev, u32 reg_addr,
			    u16 w, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_fw_reg_writel_retry(const struct cts_device *cts_dev, u32 reg_addr,
			    u32 l, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_fw_reg_writesb_retry(const struct cts_device *cts_dev, u32 reg_addr,
			     const void *src, size_t len, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_fw_reg_readb_retry(const struct cts_device *cts_dev, u32 reg_addr,
			   u8 *b, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_fw_reg_readw_retry(const struct cts_device *cts_dev, u32 reg_addr,
			   u16 *w, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_fw_reg_readl_retry(const struct cts_device *cts_dev, u32 reg_addr,
			   u32 *l, int retry, int delay)
{
	if (cts_dev->rtdata.program_mode) {
...
int cts_hw_reg_writeb_retry(const struct cts_device *cts_dev, u32 reg_addr,
			    u8 b, int retry, int delay)
{
	return cts_sram_writeb_retry(cts_dev, reg_addr, b, retry, delay);
}

int cts_hw_reg_writew_retry(const struct cts_device *cts_dev, u32 reg_addr,
			    u16 w, int retry, int delay)
{
	return cts_sram_writew_retry(cts_dev, reg_addr, w, retry, delay);
}

int cts_hw_reg_writel_retry(const struct cts_device *cts_dev, u32 reg_addr,
			    u32 l, int retry, int delay)
{
	return cts_sram_writel_retry(cts_dev, reg_addr, l, retry, delay);
}

int cts_hw_reg_writesb_retry(const struct cts_device *cts_dev, u32 reg_addr,
			     const void *src, size_t len, int retry, int delay)
{
...
int cts_hw_reg_readb_retry(const struct cts_device *cts_dev, u32 reg_addr,
			   u8 *b, int retry, int delay)
{
...
int cts_hw_reg_readl_retry(const struct cts_device *cts_dev, u32 reg_addr,
			   u32 *l, int retry, int delay)
{
	return cts_sram_readl_retry(cts_dev, reg_addr, l, retry, delay);
}

int cts_hw_reg_readsb_retry(const struct cts_device *cts_dev, u32 reg_addr,
			    void *dst, size_t len, int retry, int delay)
{
...
static int icnl9911_set_access_ddi_reg(struct cts_device *cts_dev, bool enable)
{
	...
	TS_LOG_INFO("ICNL9911 %s access ddi reg",
		    enable ? "enable" : "disable");

	ret = cts_hw_reg_readb(cts_dev, CTS_DEV_HW_REG_DDI_REG_CTRL,
			       &access_flag);
	if (ret) {
	...
	ret = cts_hw_reg_writeb(cts_dev, CTS_DEV_HW_REG_DDI_REG_CTRL,
				access_flag);
	if (ret) {
		TS_LOG_ERR("Write HW_REG_DDI_REG_CTRL %02x failed %d",
			   access_flag, ret);
		return ret;
	}
...
disable_access_ddi_reg : {
	...
	access_flag = enable ? (access_flag & (~0x01)) : (access_flag | 0x01);
	r = cts_hw_reg_writeb(cts_dev, CTS_DEV_HW_REG_DDI_REG_CTRL,
			      access_flag);
	if (r) {
		TS_LOG_ERR("");
	}
...
static int icnl9911s_set_access_ddi_reg(struct cts_device *cts_dev, bool enable)
{
	...
	TS_LOG_INFO("ICNL9911S %s access ddi reg",
		    enable ? "enable" : "disable");
...
static int cts_init_device_hwdata(struct cts_device *cts_dev, u32 hwid,
				  u16 fwid)
{
	int i;
...
{
	int ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_DDI_VERSION,
				   version);
...
int cts_get_panel_param(const struct cts_device *cts_dev, void *param,
			size_t size)
{
	...
	return cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_PANEL_PARAM, param,
				 size);
}

int cts_set_panel_param(const struct cts_device *cts_dev, const void *param,
			size_t size)
{
	...
	return cts_fw_reg_writesb(cts_dev, CTS_DEVICE_FW_REG_PANEL_PARAM, param,
				  size);
}
...
int cts_set_dev_esd_protection(struct cts_device *cts_dev, bool enable)
{
	...
	return cts_fw_reg_writeb(cts_dev, CTS_DEVICE_FW_REG_ESD_PROTECTION,
				 enable ? CTS_DEV_FW_ESD_PROTECTION_ON :
...
int cts_enable_get_rawdata(const struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Send CMD_ENABLE_READ_RAWDATA failed %d(%s)", ret,
			   cts_strerror(ret));
		return ret;
	}
	...
	for (i = 0; i < 100; i++) {
		...
		if (ret) {
			TS_LOG_ERR(
				"Get enable get touch data flag failed %d(%s)",
		...
		if (enabled) {
			...
			TS_LOG_DEBUG(
				"Enable get touch data flag NOT set, try again");
		}
...
int cts_disable_get_rawdata(const struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Send CMD_DISABLE_READ_RAWDATA failed %d(%s)", ret,
			   cts_strerror(ret));
		return ret;
	}
...
int cts_get_rawdata(const struct cts_device *cts_dev, void *buf)
{
	...
	do {
		ret = cts_fw_reg_readsb_delay_idle(
			cts_dev, CTS_DEVICE_FW_REG_RAW_DATA, buf,
			cts_dev->fwdata.rows * cts_dev->fwdata.cols * 2, 500);
		if (ret) {
...
int cts_get_diffdata(const struct cts_device *cts_dev, void *buf)
{
	...
	do {
		ret = cts_fw_reg_readsb_delay_idle(
			cts_dev, CTS_DEVICE_FW_REG_DIFF_DATA, cache_buf,
	...
	for (i = 0; i < cts_dev->fwdata.rows; i++) {
		for (j = 0; j < cts_dev->fwdata.cols; j++) {
			((u8 *)buf)[2 * (i * cts_dev->fwdata.cols + j)] =
				cache_buf[2 * ((i + 1) * (cts_dev->fwdata.cols +
							  2) +
					       j + 1)];
...
int cts_get_baseline(const struct cts_device *cts_dev, void *baseline,
		     enum cts_work_mode work_mode, u32 flags, u16 addr,
		     u8 data_width)
{
	u8 rows, cols;
	...
	case CTS_WORK_MODE_SUSPEND:
		...
		return -ENOTSUPP;
	default:
		TS_LOG_ERR("Get baseline of work mode %d invalid", work_mode);
		return -EINVAL;
	}
	...
	TS_LOG_INFO("Get baseline of work-mode: %d(%s), flags 0x%x, "
		    ...
		    work_mode, cts_work_mode2str(work_mode), flags, addr,
		    data_width, rows, cols);

	if (flags & CTS_GET_TOUCH_DATA_FLAG_ENABLE_GET_TOUCH_DATA_BEFORE) {
		ret = cts_enable_get_rawdata(cts_dev);
		if (ret) {
			TS_LOG_ERR("Enable get touch data failed %d(%s)", ret,
				   cts_strerror(ret));
			return ret;
		}
	}
...
read_baseline:
	...
	ret = cts_fw_reg_readsb_delay_idle(
		cts_dev, addr, use_another_buf ? buf : baseline, buf_size, 500);
	if (ret) {
		TS_LOG_ERR("Read baseline from 0x%04x size %u failed %d", addr,
			   buf_size, ret);
		goto free_mem;
	}
...
clear_data_ready_flag:
	if (flags & CTS_GET_TOUCH_DATA_FLAG_CLEAR_DATA_READY) {
		...
		if (r) {
			TS_LOG_ERR("Clear data ready flag failed %d(%s)", r,
				   cts_strerror(r));
		}
	}
...
disable_get_touch_data:
	if (flags & CTS_GET_TOUCH_DATA_FLAG_DISABLE_GET_TOUCH_DATA_AFTER) {
		...
		if (r) {
			TS_LOG_ERR("Enable get touch data failed %d(%s)", r,
				   cts_strerror(r));
			// Ignore this error
		}
	}
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
	...
	TS_LOG_INFO("  %-24s: v%x.%x", "Fimrware lib verion",
		    (u8)(fwdata->lib_version >> 8), (u8)(fwdata->lib_version));
	...
	TS_LOG_INFO("  %-24s: %02x", "DDI init code verion",
		    fwdata->ddi_version);
	...
	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_INT_MODE,
			       &fwdata->int_mode);
	if (ret) {
	...
	ret = cts_fw_reg_readw(cts_dev, CTS_DEVICE_FW_REG_INT_KEEP_TIME,
	...
	ret = cts_fw_reg_readw(cts_dev, CTS_DEVICE_FW_REG_RAWDATA_TARGET,
	...
	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_ESD_PROTECTION,
			       &fwdata->esd_method);
	if (ret) {
	...
	ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_EARJACK_DETECT_SUPP,
			       &val);
	if (ret) {
		TS_LOG_ERR("Read firmware earjack detect support failed %d",
			   ret);
		return ret;
	}
...
void cts_show_fw_log(struct cts_device *cts_dev)
{
	...
	ret = cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_TOUCH_INFO + 1, &len,
				1);
	if (ret) {
	...
	ret = cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_TOUCH_INFO + 2, data,
				len);
	if (ret) {
...
int cts_suspend_device(struct cts_device *cts_dev)
{
	...
	ret = cts_send_command(cts_dev, cts_dev->rtdata.gesture_wakeup_enabled ?
...
int cts_resume_device(struct cts_device *cts_dev)
{
	...
	ret = cts_update_firmware(&cts_data->cts_dev, cts_data->cached_firmware,
				  false);
	if (ret) {
		TS_LOG_ERR("Update firmware on boot failed %d(%s)", ret,
			   cts_strerror(ret));
		goto err_set_program_mode;
	}
...
config_firmware:
	...
	ret = cts_send_command(cts_dev, (!!cts_data->roi_switch) ?
						CTS_CMD_ENABLE_NOI :
						CTS_CMD_DISABLE_NOI);
	if (ret) {
		TS_LOG_ERR("Send %s failed %d(%s)\n",
			   (!!cts_data->roi_switch) ? "CMD_ENABLE_NOI" :
						      "CMD_DISABLE_NOI",
			   ret, cts_strerror(ret));
	}
...
#ifdef CONFIG_CTS_I2C_HOST
	ret = cts_plat_i2c_write(cts_dev->pdata, CTS_DEV_PROGRAM_MODE_I2CADDR,
				 magic_num, 4, 5, 10);
	if (ret) {
...
#endif
	{
		TS_LOG_ERR("BOOT_MODE readback %u != I2C/SPI PROMGRAM mode",
			   boot_mode);
		return -EFAULT;
	}
...
int cts_enter_normal_mode(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Set BOOT_MODE to SRAM_BOOT failed %d(%s)", ret,
			   cts_strerror(ret));
		// Fall through
	}
	...
	for (retries = 0; retries < 3; retries++) {
		...
		if (ret) {
			TS_LOG_ERR(
				"Get CURR_BOOT_MODE failed %d, try to do reset",
				ret, cts_strerror(ret));
			goto reset_device;
		}
		...
		if (boot_mode != CTS_DEV_BOOT_MODE_SRAM &&
			...
			TS_LOG_ERR(
				"Curr boot mode %u(%s) != SRAM_BOOT/FLASH_BOOT",
...
init_fwdata:
	...
	if (ret) {
		TS_LOG_ERR("Init firmware data failed %d(%s)", ret,
			   cts_strerror(ret));
		return ret;
	}
...
int cts_get_fwid(struct cts_device *cts_dev, u16 *fwid)
{
	...
	ret = cts_fw_reg_readw_retry(cts_dev, CTS_DEVICE_FW_REG_CHIP_TYPE, fwid,
				     5, 1);
	if (ret) {
		goto err_out;
	}
...
int cts_get_hwid(struct cts_device *cts_dev, u32 *hwid)
{
	...
	ret = cts_hw_reg_readl_retry(cts_dev, CTS_DEV_HW_REG_HARDWARE_ID, hwid,
				     5, 0);
	if (ret) {
		goto err_out;
	}
...
read_fwid:
...
#endif
	{
		...
		if (ret) {
			TS_LOG_ERR("Get firmware id failed %d, retries %d", ret,
				   retries);
		} else {
			ret = cts_fw_reg_readw_retry(cts_dev,
						     ...
						     &device_fw_ver, 5, 0);
			if (ret) {
				TS_LOG_ERR("Read firmware version failed %d",
					   ret);
				device_fw_ver = 0;
				// TODO: Handle this error
			} else {
				device_fw_ver = be16_to_cpu(device_fw_ver);
				TS_LOG_INFO("Device firmware version: %04x",
					    device_fw_ver);
			}
			goto init_hwdata;
		}
	...
	if (ret || hwid == CTS_DEV_HWID_INVALID) {
		...
		TS_LOG_ERR("Get hardware id failed %d retries %d", ret,
			   retries);
			 ...
			 bool trace_point)
{
	...
	if (trace_point) {
		ret = cts_fw_reg_readsb(
			cts_dev, CTS_DEVICE_FW_REG_GESTURE_INFO + 2,
			gesture_info + 2,
			(((u8 *)gesture_info))[1] *
...
static void cts_esd_protection_work(struct work_struct *work)
{
...
#ifdef CONFIG_CTS_I2C_HOST
	if (!cts_plat_is_i2c_online(cts_data->pdata,
				    CTS_DEV_NORMAL_MODE_I2CADDR))
#else
...
#endif
	{
		...
		if ((cts_data->esd_check_fail_cnt % 2) == 0) {
			...
			if (ret) {
				TS_LOG_ERR(
					"ESD protection reset chip failed %d",
					ret);
			}
		}
	} else {
	...
	if (cts_data->esd_check_fail_cnt >= CFG_CTS_ESD_FAILED_CONFIRM_CNT) {
		...
		if (cts_data->cached_firmware == NULL) {
			...
			cts_data->cached_firmware =
				cts_request_firmware_from_fs(
					&cts_data->cts_dev,
					cts_data->boot_firmware_filename);
			if (cts_data->cached_firmware == NULL) {
				TS_LOG_ERR(
					"Request firmware from file '%s' failed",
					cts_data->boot_firmware_filename);
				goto unlock_device;
			}
		}
		...
		if (ret) {
			TS_LOG_ERR("ESD recovery update firmware failed %d",
				   ret);
			goto unlock_device;
		}
...
int cts_set_dev_charger_attached(struct cts_device *cts_dev, bool attached)
{
	...
	ret = cts_send_command(cts_dev, attached ? CTS_CMD_CHARGER_ATTACHED :
...
int cts_set_dev_earjack_attached(struct cts_device *cts_dev, bool attached)
{
	...
	ret = cts_send_command(cts_dev, attached ? CTS_CMD_EARJACK_ATTACHED :
...
int cts_enable_fw_log_redirect(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Send CTS_CMD_ENABLE_FW_LOG_REDIRECT failed %d",
			   ret);
	} else {
...
int cts_disable_fw_log_redirect(struct cts_device *cts_dev)
{
	...
	if (ret) {
		TS_LOG_ERR("Send CTS_CMD_DISABLE_FW_LOG_REDIRECT failed %d",
			   ret);
	} else {
		cts_dev->rtdata.fw_log_redirect_enabled = false;
	}
...
read_compensate_cap:
	...
	ret = cts_fw_reg_readsb_delay_idle(
		cts_dev, CTS_DEVICE_FW_REG_COMPENSATE_CAP, cap,
	...
	if (cts_dev->fwdata.flip_x) {
		...
		flip_comp_cap_on_row(cap, cts_dev->fwdata.rows,
				     cts_dev->fwdata.cols);
	}
	if (cts_dev->fwdata.flip_y) {
		TS_LOG_INFO(" - Flip comp cap on col");
		flip_comp_cap_on_col(cap, cts_dev->fwdata.rows,
				     cts_dev->fwdata.cols);
	}
...
int cts_start_driver_log_redirect(const char *filepath, bool append_to_file,
				  char *log_buffer, int log_buf_size,
				  int log_level)
{
#define START_BANNER ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n"
	...
	if (log_buffer && log_buf_size) {
		TS_LOG_INFO(
			" - Start driver log to buffer: %p size: %d level: %d",
	...
	if (filepath && filepath[0]) {
		...
		cts_log_filp =
			filp_open(filepath,
				  O_WRONLY | O_CREAT |
					  (append_to_file ? O_APPEND : O_TRUNC),
				  S_IRUGO | S_IWUGO);
		if (IS_ERR(cts_log_filp)) {
			...
			cts_write_file(cts_log_filp, START_BANNER,
				       strlen(START_BANNER));
		}
	}
...
void cts_log(int level, const char *fmt, ...)
{
	...
	if (cts_log_redirect) {
		if (cts_log_buffer && cts_log_buf_wr_size < cts_log_buf_size &&
		    level <= cts_log_to_file_level) {
			cts_log_buf_wr_size += vscnprintf(
				cts_log_buffer + cts_log_buf_wr_size,
				cts_log_buf_size - cts_log_buf_wr_size, fmt,
				args);
		}
