...
static void parse_dts_project_basic(const struct device_node *proj_of_node)
{
	...
	ret = of_property_read_u8_array(
		proj_of_node, "boot-hw-reset-timing",
	...
	ret = of_property_read_u8_array(
		proj_of_node, "resume-hw-reset-timing",
		cts_data->resume_hw_reset_timing,
		ARRAY_SIZE(cts_data->resume_hw_reset_timing));
	if (ret) {
		TS_LOG_INFO(
			"Get 'resume-hw-reset-timing' from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
	...
	ret = of_property_read_u32(proj_of_node, "is_incell",
				   &cts_data->tskit_data->is_in_cell);
	if (ret) {
	...
	ret = of_property_read_u32(proj_of_node, "spi-mode",
				   &cts_data->spi_mode);
	if (ret) {
	...
	ret = of_property_read_u32(proj_of_node, "algo_id",
	...
	ret = of_property_read_u32(proj_of_node, "irq_config",
...
static void
parse_dts_project_facotry_test(const struct device_node *proj_of_node)
{
	const char *str = NULL;
	int ret = 0;
	...
	ret = of_property_read_s32(proj_of_node, "rawdata-test-min",
				   (s32 *)&cts_data->rawdata_test_min);
	if (ret) {
	...
	ret = of_property_read_s32(proj_of_node, "rawdata-test-max",
				   (s32 *)&cts_data->rawdata_test_max);
	if (ret) {
	...
	ret = of_property_read_u32(proj_of_node, "noise-test-frames",
	...
	ret = of_property_read_s32(proj_of_node, "noise-test-max",
				   (s32 *)&cts_data->noise_test_max);
	if (ret) {
	...
	ret = of_property_read_s32(proj_of_node, "open-test-min",
				   (s32 *)&cts_data->open_test_min);
	if (ret) {
		TS_LOG_ERR("Get open-test-min from dts failed %d(%s)\n", ret,
			   cts_strerror(ret));
		cts_data->open_test_min = 200;
	}
	TS_LOG_INFO(" - %-32s: %u\n", "Open test min", cts_data->open_test_min);

	cts_data->test_short =
		of_property_read_bool(proj_of_node, "test-short");
	TS_LOG_INFO(" - %-32s: %c\n", "Test short",
		    cts_data->test_short ? 'Y' : 'N');
	ret = of_property_read_s32(proj_of_node, "short-test-min",
				   (s32 *)&cts_data->short_test_min);
	if (ret) {
	...
	ret = of_property_read_s32(proj_of_node, "compensate-cap-test-min",
				   (s32 *)&cts_data->compensate_cap_test_min);
	if (ret) {
		TS_LOG_ERR(
			"Get compensate-cap-test-min from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
	...
	if (ret) {
		TS_LOG_ERR(
			"Get compensate-cap-test-max from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
	...
	cts_data->dump_factory_test_data_to_console = of_property_read_bool(
		proj_of_node, "dump-factory-test-data-to-console");
	TS_LOG_INFO(" - %-32s: %c\n", "Dump test data to console",
		    cts_data->dump_factory_test_data_to_console ? 'Y' : 'N');

	cts_data->dump_factory_test_data_to_buffer = of_property_read_bool(
		proj_of_node, "dump-factory-test-data-to-buffer");
	TS_LOG_INFO(" - %-32s: %c\n", "Dump test data to buffer",
		    cts_data->dump_factory_test_data_to_buffer ? 'Y' : 'N');

	cts_data->dump_factory_test_data_to_file = of_property_read_bool(
		proj_of_node, "dump-factory-test-data-to-file");
	TS_LOG_INFO(" - %-32s: %c\n", "Dump test data to file",
		    cts_data->dump_factory_test_data_to_file ? 'Y' : 'N');

	cts_data->print_factory_test_data_only_fail = of_property_read_bool(
		proj_of_node, "print-factory-test-data-only-fail");
...
#define PINCTRL_STATE_CS_LOW_NAME "state_cs_output0"
	...
	cts_data->pinctrl_state_reset_high = pinctrl_lookup_state(
		cts_data->pinctrl, PINCTRL_STATE_RESET_HIGH_NAME);
	if (IS_ERR_OR_NULL(cts_data->pinctrl_state_reset_high)) {
		ret = -PTR_ERR(cts_data->pinctrl_state_reset_high);
		TS_LOG_ERR(
			"Lookup pinctrl state '" PINCTRL_STATE_RESET_HIGH_NAME
			"' failed %d(%s)\n",
			ret, cts_strerror(ret));
		goto put_pinctrl;
	}
	cts_data->pinctrl_state_reset_low = pinctrl_lookup_state(
		cts_data->pinctrl, PINCTRL_STATE_RESET_LOW_NAME);
	if (IS_ERR_OR_NULL(cts_data->pinctrl_state_reset_low)) {
		ret = -PTR_ERR(cts_data->pinctrl_state_reset_low);
		TS_LOG_ERR("Lookup pinctrl state '" PINCTRL_STATE_RESET_LOW_NAME
			   "' failed %d(%s)\n",
			   ret, cts_strerror(ret));
		goto put_pinctrl;
	}
	cts_data->pinctrl_state_as_int = pinctrl_lookup_state(
		cts_data->pinctrl, PINCTRL_STATE_AS_INT_NAME);
	if (IS_ERR_OR_NULL(cts_data->pinctrl_state_as_int)) {
		ret = -PTR_ERR(cts_data->pinctrl_state_as_int);
		TS_LOG_ERR("Lookup pinctrl state '" PINCTRL_STATE_AS_INT_NAME
			   "' failed %d(%s)\n",
			   ret, cts_strerror(ret));
		goto put_pinctrl;
	}
	cts_data->pinctrl_state_int_high = pinctrl_lookup_state(
		cts_data->pinctrl, PINCTRL_STATE_INT_HIGH_NAME);
	if (IS_ERR_OR_NULL(cts_data->pinctrl_state_int_high)) {
		ret = -PTR_ERR(cts_data->pinctrl_state_int_high);
		TS_LOG_ERR("Lookup pinctrl state '" PINCTRL_STATE_INT_HIGH_NAME
			   "' failed %d(%s)\n",
			   ret, cts_strerror(ret));
		goto put_pinctrl;
	}
	cts_data->pinctrl_state_int_low = pinctrl_lookup_state(
		cts_data->pinctrl, PINCTRL_STATE_INT_LOW_NAME);
	if (IS_ERR_OR_NULL(cts_data->pinctrl_state_int_low)) {
		ret = -PTR_ERR(cts_data->pinctrl_state_int_low);
		TS_LOG_ERR("Lookup pinctrl state '" PINCTRL_STATE_INT_LOW_NAME
			   "' failed %d(%s)\n",
			   ret, cts_strerror(ret));
		goto put_pinctrl;
	}
				...
				struct ts_kit_device_data *data)
{
	...
	if (ret) {
		TS_LOG_ERR(
			"Get parade,is_parade_solution from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
	...
	if (ret) {
		TS_LOG_ERR(
			"Get parade,direct_proc_cmd from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
	...
	ret = of_property_read_u8(of_node, "roi_supported",
				  (u8 *)&data->ts_platform_data->feature_info
	...
	TS_LOG_INFO(
		" - %-32s: %c\n", "ROI supported",
		data->ts_platform_data->feature_info.roi_info.roi_supported ?
			'Y' :
			'N');
	...
	if (ret) {
		TS_LOG_ERR(
			"Get get_brightness_info_flag from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
	...
	if (cts_data->project_id[0] == '\0') {
		...
		if (ret) {
			TS_LOG_ERR(
				"Get default_project_id from dts failed %d(%s)\n",
				ret, cts_strerror(ret));
			strncpy(cts_data->project_id, "TS", sizeof("TS") - 1);
		} else {
			int len = strlen(default_project_id);
			if (len > (sizeof(cts_data->project_id) - 1)) {
				strncpy(cts_data->project_id,
					default_project_id,
...
static int chipone_chip_detect(struct ts_kit_platform_data *data)
{
	...
	if (ret) {
		...
		strncpy(cts_data->tskit_data->chip_name, "AC", strlen("AC"));
	}
...
static int chipone_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_X, 0,
			     (cts_data->tskit_data->x_max - 1), 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0,
			     (cts_data->tskit_data->y_max - 1), 0, 0);
	...
	input_set_abs_params(
		input_dev, ABS_MT_TRACKING_ID, 0,
		cts_data->tskit_data->ts_platform_data->max_fingers * 2, 0, 0);
...
static int chipone_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if (tskit_dev_data->ts_platform_data->hide_plain_id)
		snprintf(info->ic_vendor, sizeof(info->ic_vendor) - 1, "%s",
			 cts_data->project_id);
	else
	...
	snprintf(info->fw_vendor, sizeof(info->fw_vendor) - 1, "0x%04x",
			 ...
			 struct cts_device_gesture_info *gesture_info)
{
	...
	case GESTURE_D_TAP:
		...
		if (cts_data->tskit_data->easy_wakeup_info.off_motion_on) {
			cts_data->tskit_data->easy_wakeup_info.off_motion_on =
				false;
				   ...
				   struct ts_cmd_node *out_cmd)
{
	...
	if (unlikely(cts_dev->rtdata.suspended)) {
		...
		if (cts_dev->rtdata.gesture_wakeup_enabled) {
			...
			ret = cts_get_gesture_info(
				cts_dev, &gesture_info,
		...
		ret = process_touch_msg(info, touch_info->msgs,
					touch_info->num_msg);
		if (ret) {
		...
		if (cts_data->tskit_data->ts_platform_data->feature_info
			    .roi_info.roi_supported &&
		    (!!cts_data->roi_switch) && info->cur_finger_number > 0 &&
		    info->cur_finger_number <= 2) {
			ret = cts_fw_reg_readsb(cts_dev,
						CTS_DEVICE_FW_REG_NOI_DATA,
						cts_data->roi_data + 4,
						7 * 7 * 2);
			if (ret) {
...
static int get_boot_firmware_filename(void)
{
	...
	if (cts_data->hide_fw_name) {
		...
		if (tskit_dev_data->ts_platform_data->product_name[0] == '\0' ||
			...
			TS_LOG_ERR(
				"Get boot firmware filename with "
...
static int chipone_fw_update_sd(void)
{
	...
	ret = cts_update_firmware_from_file(
		&cts_data->cts_dev,
				    ...
				    const char *test_data_filename)
{
	...
	ret = snprintf(touch_data_filepath, sizeof(touch_data_filepath), "%s%s",
		       cts_data->factory_test_data_directory,
		       test_data_filename);
...
static int init_factory_test_param(void)
{
	...
	if (cts_data->test_rawdata) {
		...
		if (cts_data->dump_factory_test_data_to_buffer) {
			...
			ret = alloc_test_data_mem(
				test_param,
				2 * nodes *
					cts_data->rawdata_test_priv_param
						.frames);
			if (ret) {
				return ret;
			}
	...
	if (cts_data->test_compensate_cap) {
		...
		if (cts_data->dump_factory_test_data_to_buffer) {
			...
			test_param->test_data_wr_size =
				&cts_data->comp_cap_test_data_size;
		}
		if (cts_data->dump_factory_test_data_to_file) {
			test_param->flags |=
				CTS_TEST_FLAG_DUMP_TEST_DATA_TO_FILE;
			ret = alloc_test_data_filepath(
				test_param, "comp-cap-test-data.txt");
			if (ret) {
				return ret;
			}
		}
...
static void print_test_result_to_console(const char *test_desc, int test_result)
{
	if (test_result > 0) {
		...
		TS_LOG_INFO("%-10s test: FAIL %d(%s)\n", test_desc, test_result,
			    cts_strerror(test_result));
	} else {
...
static void print_factory_test_result_to_console(void)
{
	...
	if (cts_data->test_compensate_cap) {
		print_test_result_to_console("Comp-Cap",
			       ...
			       struct ts_cmd_node *out_cmd)
{
	...
	if (ret) {
		TS_LOG_ERR("Init factory test param failed %d(%s)\n", ret,
			   cts_strerror(ret));
		strncpy(failed_reason, "software_reason-",
	 ...
	 */
	if (cts_data->rawdata_test_result || cts_data->noise_test_result ||
	    cts_data->open_test_result || cts_data->short_test_result ||
	    cts_data->reset_pin_test_result || cts_data->int_pin_test_result ||
	    cts_data->comp_cap_test_result)
		strncpy(failed_reason, "panel_reason-",
			TP_TEST_FAILED_REASON_LEN);
					  ...
					  const char *suffix, char seperator)
{
	...
	for (c = 0; c < cols; c++) {
		count += scnprintf(buf + count, size - count, "%4u%c ", data[c],
				   seperator);
	}
...
static void print_touch_data_to_seq_file(struct seq_file *m, const u16 *data,
					 int frames, int rows, int cols)
{
	...
	for (i = 0; i < frames; i++) {
		for (r = 0; r < rows; r++) {
			...
			len = print_touch_data_row_to_buffer(linebuf,
							     sizeof(linebuf),
							     data, cols, NULL,
							     "\n", ',');
			seq_puts(m, linebuf);
					...
					const char *suffix, char seperator)
{
	...
	for (c = 0; c < cols; c++) {
		count += scnprintf(buf + count, size - count, "%3u%c ", cap[c],
				   seperator);
	}
...
static void print_comp_cap_to_seq_file(struct seq_file *m, const u8 *data,
				       int rows, int cols)
{
	int r;
				       ...
				       int range_size, int row_size)
{
	...
	if (cts_data == NULL) {
		TS_LOG_ERR(
			"Print touch data to seq file with cts_data = NULL\n");
		return -EFAULT;
	}
	...
	if (cts_data->test_rawdata) {
		print_test_result_to_seq_file(m, "Rawdata",
					      cts_data->rawdata_test_result);
		if (cts_data->dump_factory_test_data_to_buffer &&
		    (cts_data->rawdata_test_result > 0 ||
		     !cts_data->print_factory_test_data_only_fail)) {
			print_touch_data_to_seq_file(
				m, cts_data->rawdata_test_param.test_data_buf,
				cts_data->rawdata_test_priv_param.frames, rows,
				cols);
		}
	}

	if (cts_data->test_noise) {
		print_test_result_to_seq_file(m, "Noise",
					      cts_data->noise_test_result);
		if (cts_data->dump_factory_test_data_to_buffer &&
			...
			print_touch_data_to_seq_file(
				...
				cts_data->noise_test_param.test_data_buf +
					2 * rows * cols *
						cts_data->noise_test_priv_param
							.frames,
				3, rows, cols);
		}
	}
	...
	if (cts_data->test_open) {
		...
		if (cts_data->dump_factory_test_data_to_buffer &&
		    (cts_data->open_test_result > 0 ||
		     !cts_data->print_factory_test_data_only_fail)) {
			print_touch_data_to_seq_file(
				m, cts_data->open_test_param.test_data_buf, 1,
				rows, cols);
		}
	}
	...
	if (cts_data->test_short) {
		...
		if (cts_data->dump_factory_test_data_to_buffer &&
		    (cts_data->short_test_result > 0 ||
		     !cts_data->print_factory_test_data_only_fail)) {
			print_touch_data_to_seq_file(
				m, cts_data->short_test_param.test_data_buf, 7,
				rows, cols);
		}
	}
	if (cts_data->test_compensate_cap) {
		print_test_result_to_seq_file(m, "Comp-cap",
					      cts_data->comp_cap_test_result);
		if (cts_data->dump_factory_test_data_to_buffer &&
		    (cts_data->comp_cap_test_result > 0 ||
		     !cts_data->print_factory_test_data_only_fail)) {
			print_comp_cap_to_seq_file(
				m, cts_data->comp_cap_test_param.test_data_buf,
				rows, cols);
		}
	}
...
static int
chipone_wakeup_gesture_enable_switch(struct ts_wakeup_gesture_enable_info *info)
{
	int ret = NO_ERR;
...
static int chipone_chip_suspend(void)
{
	...
	if (cts_is_gesture_wakeup_enabled(&cts_data->cts_dev)) {
		ret = enable_irq_wake(
			cts_data->tskit_data->ts_platform_data->irq_id);
		if (ret) {
...
static int chipone_chip_resume(void)
{
	...
	if (wakeup_info->sleep_mode != TS_GESTURE_MODE) {
		...
		gpio_direction_output(
			cts_data->tskit_data->ts_platform_data->cs_gpio, 1);
#endif /* CONFIG_HONOR_DEVKIT_MTK_3_0 */
	}
	wakeup_info->easy_wakeup_flag = false;
	...
	case TS_ACTION_WRITE:
		...
		if (ret) {
			TS_LOG_ERR("Send %s failed %d\n",
				   !!info->roi_switch ? "CMD_ENABLE_NOI" :
							"CMD_DISABLE_NOI",
				   ret);
		}
		return ret;
...
static unsigned char *chipone_roi_rawdata(void)
{
	...
	roi_info =
		&cts_data->tskit_data->ts_platform_data->feature_info.roi_info;
...
struct ts_device_ops chipone_ts_device_ops = {
	...
	.chip_wakeup_gesture_enable_switch =
		chipone_wakeup_gesture_enable_switch,
