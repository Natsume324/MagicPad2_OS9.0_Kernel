...
struct cts_sfctrl_ops {
	...
	int (*read)(const struct cts_device *cts_dev, u32 flash_addr, void *dst,
		    size_t len);
	int (*read_to_sram)(const struct cts_device *cts_dev, u32 flash_addr,
			    u32 sram_addr, size_t len);
	int (*program)(const struct cts_device *cts_dev, u32 flash_addr,
	...
	int (*calc_sram_crc)(const struct cts_device *cts_dev, u32 sram_addr,
			     size_t len, u32 *crc);
	int (*calc_flash_crc)(const struct cts_device *cts_dev, u32 flash_addr,
			      size_t len, u32 *crc);
};
