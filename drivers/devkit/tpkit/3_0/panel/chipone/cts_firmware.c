...
#define FIRMWARE_SECTION(firmware) ((firmware)->data)
...
static int calc_crc_in_flash(const struct cts_device *cts_dev, u32 flash_addr,
			     size_t size, u32 *crc)
{
	return cts_dev->hwdata->sfctrl->ops->calc_flash_crc(cts_dev, flash_addr,
							    size, crc);
}
...
static bool is_multi_section_firmware_valid(const struct cts_firmware *firmware)
{
	...
	crc = cts_data_check(FIRMWARE_SECTION(firmware),
	...
	if (DDIPARAM_SECTION_ENABLE(firmware) == CTS_SECTION_ENABLE_FLAG) {
		crc = cts_data_check(DDIPARAM_SECTION(firmware),
					  ...
					  struct cts_firmware_sect_info *info)
{
	...
	info->firmware_sect_crc =
		cts_data_check(firmware->data, firmware->size);
			  ...
			  struct cts_firmware_sect_info *info)
{
	...
	if (info->ddiparam_crc_sect) {
		TS_LOG_INFO("  DDIParam section size: %zu",
...
cts_request_driver_builtin_firmware_by_index(u32 index)
{
	...
	if (index < NUM_DRIVER_BUILTIN_FIRMWARE) {
		...
		TS_LOG_ERR(
			"Request driver builtin by index %u too large >= %zu",
					      ...
					      int curr_version)
{
	...
	if (IS_ERR(file)) {
		TS_LOG_ERR("Open file '%s' failed %ld", filepath,
			   PTR_ERR(file));
		return PTR_ERR(file);
	}
	...
	if (!is_firmware_size_valid(firmware)) {
		TS_LOG_INFO("File '%s' size: %zu invalid", filepath,
			    firmware->size);
		ret = -EINVAL;
		goto err_close_file;
	}
	...
	TS_LOG_INFO("File '%s' size: %zu version: %04x", filepath,
		    firmware->size, version);
	...
	if (read_size < 0 || read_size != firmware->size) {
		TS_LOG_ERR("Request from fs read whole file failed %d",
			   read_size);
		ret = -EIO;
...
const struct cts_firmware *
cts_request_newer_firmware_from_fs(const struct cts_device *cts_dev,
				   const char *filepath, u16 curr_version)
{
	struct cts_firmware *firmware;
	int ret;

	TS_LOG_INFO("Request from file '%s' if version > %04x", filepath,
		    curr_version);
	...
	if (filepath[0] == '/') { /* Full path */
		...
		ret = cts_request_firmware_full_filepath(firmware, filepath,
							 curr_version);
	} else { /* ONLY file name is givin */
		TS_LOG_INFO(
			"Filepath is only filename, use request_firmware()");
		ret = cts_wrap_kernel_request_firmware(
			firmware, filepath,
...
const struct cts_firmware *
cts_request_firmware_from_fs(const struct cts_device *cts_dev,
			     const char *filepath)
{
...
static int validate_flash_data(const struct cts_device *cts_dev, u32 flash_addr,
			       const u8 *data, size_t size, u8 *buf,
			       bool calc_crc, u32 crc)
{
	int ret, i;
	...
	TS_LOG_INFO("Validate flash data from 0x%06x size %zu by %s",
		    flash_addr, size,
		    calc_crc ? "check-crc" : "direct-readback");
	...
	for (i = 0; i < size; i++) {
		if (buf[i] != data[i]) {
			...
			if (ret < 100) {
				TS_LOG_ERR("  0x%06x: %02x %02x", i, buf[i],
					   data[i]);
			} else if (ret == 100) {
				TS_LOG_ERR("  ...");
			}
	...
	const struct cts_firmware_sect_info *firmware_info)
{
	...
	ret = cts_program_flash_from_sram(
		cts_dev, 4, 4, firmware_info->firmware_sect_size - 4);
	if (ret) {
	...
	if (ret) {
		TS_LOG_ERR("Program firmware section fist 4bytes failed %d",
			   ret);
		return ret;
	}
...
static int
cts_program_ddiparam(const struct cts_device *cts_dev,
		     const struct cts_firmware_sect_info *firmware_info)
{
	int ret;
	u8 crc_sect_buf[DDIPARAM_CRC_SECTION_SIZE];

	TS_LOG_INFO("Program DDIParam size: %zu",
			...
			const struct cts_firmware *firmware, bool to_flash)
{
	...
	TS_LOG_INFO("Update firmware to %s ver: %04x size: %zu",
		    to_flash ? "flash" : "sram", FIRMWARE_VERSION(firmware),
		    firmware->size);
	...
	if ((ret = cts_sram_writel_retry(cts_dev, 0x00FFF8,
					 firmware_info.firmware_sect_size, 3,
					 1)) != 0) {
	...
	if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF0, 0xCC33555A, 3,
					   1)) != 0) {
	...
	do {
		...
		if (ret) {
			TS_LOG_ERR(
				"Erase firmware section failed %d retries %d",
				ret, retries);
			continue;
		}

		ret = cts_program_firmware_from_sram_to_flash(cts_dev,
							      &firmware_info);
		if (ret) {
			TS_LOG_ERR(
	...
	if (ret == 0 && firmware_info.ddiparam_sect_size != 0) {
		...
		do {
			...
			ret = cts_erase_flash(
				cts_dev, CTS_DDIPARAM_CRC_SECTION_OFFSET,
				firmware_info.ddiparam_crc_sect_size);
			if (ret) {
				TS_LOG_ERR(
