...
static const struct cts_flash cts_flashes[] = {
	...
	{"Winbond,W25Q40EW", 0xEF6013, 256, 0x1000, 0x8000, 0x80000},
	{"Winbond,W25Q20BW", 0xEF5012, 256, 0x1000, 0x8000, 0x40000},
	{"Winbond,W25Q40BW", 0xEF5013, 256, 0x1000, 0x8000, 0x80000},
	...
	{"Macronix,MX25R4035F", 0xC22813, 256, 0x1000, 0x8000, 0x80000},

	/* Puya-Semi */
	{"Puya-Semi,P25Q20LUVHIR", 0x856012, 256, 0x1000, 0x8000, 0x40000},
	{"Puya-Semi,P25Q40LUXHIR", 0x856013, 256, 0x1000, 0x8000, 0x80000},
	{"Puya-Semi,P25Q11L", 0x854011, 256, 0x1000, 0x8000, 0x20000},
	{"Puya-Semi,P25Q21L", 0x854012, 256, 0x1000, 0x8000, 0x40000},

	/* Boya */
	{"Boya,BY25D10", 0x684011, 256, 0x1000, 0x8000, 0x20000},
	...
	{"XTX,XT25Q02", 0x0B6012, 256, 0x1000, 0, 0x40000},

	/* Xinxin-Semi */
	{"Xinxin-Semi,XM25QU20B", 0x205012, 256, 0x1000, 0x8000, 0x40000},
	{"Xinxin-Semi,XM25QU40B", 0x205013, 256, 0x1000, 0x8000, 0x80000},
};
...
static int erase_sector_retry(const struct cts_device *cts_dev, u32 sector_addr,
			      int retry)
{
	int ret, retries;
...
static int erase_block_retry(const struct cts_device *cts_dev, u32 block_addr,
			     int retry)
{
	int ret, retries;
...
int cts_read_flash_retry(const struct cts_device *cts_dev, u32 flash_addr,
			 void *dst, size_t size, int retry)
{
	...
	if (flash == NULL || sfctrl == NULL || sfctrl->ops == NULL ||
	...
	if (flash_addr > flash->total_size) {
		TS_LOG_ERR("Read from 0x%06x > flash size 0x%06zx", flash_addr,
			   flash->total_size);
		return -EINVAL;
	}
				 ...
				 int retry)
{
	...
	TS_LOG_INFO("Read from 0x%06x to sram 0x%06x size %zu", flash_addr,
		    sram_addr, size);
	...
	do {
		retries++;
		ret = sfctrl->ops->read_to_sram(cts_dev, flash_addr, sram_addr,
						size);
		if (ret) {
			TS_LOG_ERR(
					   ...
					   size_t size, u32 crc, int retry)
{
	...
	do {
		...
		ret = sfctrl->ops->calc_sram_crc(cts_dev, sram_addr, size,
						 &crc_sram);
		if (ret) {
			TS_LOG_ERR(
				...
				u32 flash_addr, u32 sram_addr, size_t size)
{
	...
	TS_LOG_INFO("Program to 0x%06x from sram 0x%06x size %zu", flash_addr,
		    sram_addr, size);
	...
	while (size) {
		...
		ret = sfctrl->ops->program_from_sram(cts_dev, flash_addr,
						     sram_addr, l);
		if (ret) {
			TS_LOG_ERR(
...
int cts_erase_flash(const struct cts_device *cts_dev, u32 addr, size_t size)
{
	...
	if (addr > flash->total_size) {
		TS_LOG_ERR("Erase from 0x%06x > flash size 0x%06zx", addr,
			   flash->total_size);
		return -EINVAL;
	}
