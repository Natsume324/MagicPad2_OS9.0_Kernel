...
#define CTS_TOOL_IOCTL_GET_RESOLUTION \
	_IOR('C', 0x03, unsigned int *) /* X in LSW, Y in MSW */
#define CTS_TOOL_IOCTL_GET_ROW_COL \
	_IOR('C', 0x04, unsigned int *) /* row in LSW, col in MSW */

#define CTS_TOOL_IOCTL_TEST _IOWR('C', 0x10, struct cts_test_ioctl_data *)
#define CTS_TOOL_IOCTL_RDWR_REG \
	_IOWR('C', 0x20, struct cts_rdwr_reg_ioctl_data *)
#define CTS_TOOL_IOCTL_UPGRADE_FW \
	_IOWR('C', 0x21, struct cts_upgrade_fw_ioctl_data *)
...
static ssize_t cts_tool_read(struct file *file, char __user *buffer,
			     size_t count, loff_t *ppos)
{
	...
	case CTS_TOOL_CMD_GET_DIFF_DATA:
		...
		if (ret) {
			TS_LOG_ERR("Get %s data failed %d",
				   cmd->cmd == CTS_TOOL_CMD_GET_RAW_DATA ?
					   "raw" :
					   "diff",
				   ret);
			break;
	...
	case CTS_TOOL_CMD_READ_HOSTCOMM:
		...
		if (ret) {
			...
			TS_LOG_DEBUG(
				"Read firmware reg addr 0x%04x, val=0x%02x",
...
static ssize_t cts_tool_write(struct file *file, const char __user *buffer,
			      size_t count, loff_t *ppos)
{
	struct chipone_ts_data *cts_data;
	struct cts_device *cts_dev;
	...
	if (cmd->cmd & BIT(0)) {
		if (cmd->data_len) {
			ret = copy_from_user(
				cmd->data, buffer + CTS_TOOL_CMD_HEADER_LENGTH,
				cmd->data_len);
			if (ret) {
				TS_LOG_ERR(
	...
	case CTS_TOOL_CMD_DOWNLOAD_FIRMWARE_WITH_FILENAME:
		TS_LOG_INFO("Write firmware path: '%.*s'", cmd->data_len,
			    cmd->data);
	...
	case CTS_TOOL_CMD_DOWNLOAD_FIRMWARE:
		...
		ret = cts_update_firmware_from_file(
			cts_dev, cts_tool_firmware_filepath, true);
		if (ret) {
...
static int cts_ioctl_test(struct cts_device *cts_dev, u32 ntests,
			  struct cts_test_param *tests)
{
	u32 num_nodes = 0;
	int i, ret = 0;
	...
	for (i = 0; i < ntests; i++) {
		...
		driver_log_to_user = !!(tests[i].flags &
		...
		if (validate_data) {
			...
			if (stop_test_if_validate_fail) {
				TS_LOG_INFO(
					"  - Flag: Stop test if validate fail");
			}
		...
		if (validate_data) {
			int num_threshold = validate_data_per_node ? num_nodes :
								     1;
			int threshold_size =
			...
			if (validate_min) {
				...
				tests[i].min = memdup_user(user_min_threshold,
							   threshold_size);
			...
			if (validate_max) {
				...
				tests[i].max = memdup_user(user_max_threshold,
							   threshold_size);
			...
			if (skip_invalid_node) {
				...
				tests[i].invalid_nodes = memdup_user(
					user_invalid_nodes,
					tests[i].num_invalid_node *
		...
		if (dump_test_data_to_user) {
			...
			tests[i].test_data_buf = kmalloc(
				tests[i].test_data_buf_size, GFP_KERNEL);
			if (tests[i].test_data_buf == NULL) {
				ret = -ENOMEM;
		...
		if (driver_log_to_user) {
			...
			tests[i].driver_log_buf = kmalloc(
				tests[i].driver_log_buf_size, GFP_KERNEL);
			if (tests[i].driver_log_buf == NULL) {
				ret = -ENOMEM;
		...
		if (driver_log_to_file) {
			user_driver_log_filepath = (const char __user *)tests[i]
							   .driver_log_filepath;
			tests[i].driver_log_filepath = strndup_user(
				user_driver_log_filepath, PATH_MAX);
			if (tests[i].driver_log_filepath == NULL) {
		...
		if (driver_log_to_file || driver_log_to_user) {
			...
			if (ret) {
				TS_LOG_ERR(
					"Start driver log redirect failed %d",
					ret);
				goto store_result;
			}
		}
		...
		if (tests[i].priv_param_size && tests[i].priv_param) {
			...
			tests[i].priv_param = memdup_user(
				user_priv_param, tests[i].priv_param_size);
...
store_result:
		if (dump_test_data_to_user) {
			if (user_test_data != NULL && test_data_wr_size > 0) {
				TS_LOG_INFO("Copy test data to user, size: %d",
					    test_data_wr_size);
				if (copy_to_user(user_test_data,
		...
		if (driver_log_to_user) {
			...
			if (user_driver_log_buf != NULL &&
				...
				TS_LOG_INFO("Copy driver log to user, size: %d",
					    driver_log_wr_size);
				if (copy_to_user(user_driver_log_buf,
						 tests[i].driver_log_buf,
						 driver_log_wr_size)) {
					TS_LOG_ERR(
						"Copy driver log to user failed");
					driver_log_wr_size = 0;
					// Skip this error
				}
			}
		...
		if (dump_test_data_to_file && user_test_data_filepath != NULL &&
...
static int cts_ioctl_rdwr_reg(struct cts_device *cts_dev, u8 reg_type,
			      u32 nregs, struct cts_rdwr_reg *regs)
{
	int i, ret = 0;
	...
	if (reg_type == CTS_IOCTL_RDWR_REG_TYPE_DDI) {
		...
		if (fw_esd_protect) {
			...
			if (ret) {
				TS_LOG_ERR("Set fw esd protection failed %d",
					   ret);
				goto unlock_device;
			}
		}
	...
	for (i = 0; i < nregs; i++) {
		...
		if (reg->data == NULL || reg->len == 0) {
			TS_LOG_ERR(
				"Rdwr reg(addr: 0x%06x) with data: %p or len: %u",
		...
		if (reg->flags & CTS_IOCTL_RDWR_REG_FLAG_RD) {
			...
			if (data == NULL) {
				TS_LOG_ERR(
					"Alloc mem for read reg(addr: 0x%06x len: %u) data failed",
					reg->addr, reg->len);
				ret = -ENOMEM;
				goto disable_access_ddi_reg;
			}
			if (reg_type == CTS_IOCTL_RDWR_REG_TYPE_FW) {
				ret = cts_fw_reg_readsb(cts_dev, regs->addr,
							data, reg->len);
			} else {
				ret = cts_hw_reg_readsb(cts_dev, regs->addr,
							data, reg->len);
			}
			if (ret) {
				kfree(data);
				TS_LOG_ERR(
					"Read reg from addr: 0x%06x len: %u failed %d",
			...
			if (copy_to_user(user_data, data, reg->len)) {
				kfree(data);
				TS_LOG_ERR(
					"Copy reg(addr: 0x%06x len: %u) data to user failed",
					reg->addr, reg->len);
			}
			kfree(data);
		} else {
			...
			if (IS_ERR(data)) {
				ret = PTR_ERR(data);
				TS_LOG_ERR(
					"Memdup reg(addr: 0x%06x len: %u) data from user failed",
					reg->addr, reg->len);
				goto disable_access_ddi_reg;
			}
			if (reg_type == CTS_IOCTL_RDWR_REG_TYPE_FW) {
				ret = cts_fw_reg_writesb(cts_dev, regs->addr,
							 data, reg->len);
			} else {
				ret = cts_hw_reg_writesb(cts_dev, regs->addr,
							 data, reg->len);
			}
			kfree(data);
			if (ret) {
				...
				struct cts_upgrade_fw_ioctl_data *ioctl_data)
{
	...
	TS_LOG_INFO("ioctl UPGRADE-FW flags: 0x%x "
		    ...
		    ioctl_data->flags, ioctl_data->builtin_fw_index,
		    ioctl_data->filepath, ioctl_data->fw_data,
	...
	if (!!(ioctl_data->flags & CTS_IOCTL_UPGRADE_FW_FLAG_FILE)) {
		...
		cts_lock_device(cts_dev);
		ret = cts_update_firmware_from_file(cts_dev, filepath,
						    to_flash);
		cts_unlock_device(cts_dev);
			   ...
			   unsigned long arg)
{
	...
	case CTS_TOOL_IOCTL_GET_RESOLUTION:
		return put_user((cts_dev->fwdata.res_y << 16) +
					cts_dev->fwdata.res_x,
				(unsigned int __user *)arg);
	case CTS_TOOL_IOCTL_GET_ROW_COL:
		return put_user((cts_dev->fwdata.cols << 16) +
					cts_dev->fwdata.rows,
				(unsigned int __user *)arg);
	...
	case CTS_TOOL_IOCTL_TEST: {
		...
		if (IS_ERR(tests_pa)) {
			...
			TS_LOG_ERR("Memdump test param to kernel failed %d",
				   ret);
			return ret;
		}
	...
	case CTS_TOOL_IOCTL_RDWR_REG: {
		...
		if (IS_ERR(regs_pa)) {
			...
			TS_LOG_ERR("Memdump cts_rdwr_reg to kernel failed %d",
				   ret);
			return ret;
		}

		return cts_ioctl_rdwr_reg(cts_dev, ioctl_data.reg_type,
	...
	case CTS_TOOL_IOCTL_UPGRADE_FW: {
		...
		if (copy_from_user(
			    ...
			    sizeof(ioctl_data))) {
			TS_LOG_ERR(
				"Copy ioctl UPGRADE-FW arg to kernel failed");
			return -EFAULT;
		}
