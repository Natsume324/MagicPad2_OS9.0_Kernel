...
void mstar_apknode_open_leather_sheath(void)
{
	...
	tskit_mstar_data->finger_touch_disable =
		TRUE; // Skip finger touch ISR handling temporarily for device driver can send i2c command to firmware.
...
void mstar_apknode_get_glove_info(u8 *pGloveMode)
{
	...
	while (i < count) {
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"Get glove info mstar_iic_write_data() success\n");
		}
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"Get glove info mstar_iic_read_data() success\n");
...
static int mstar_get_film_mode(void)
{
	...
	if (rc >= 0) {
		TS_LOG_INFO(
			"Get firmware info mstar_iic_write_data() success\n");
	}
	msleep(I2C_WRITE_COMMAND_DELAY_FOR_FIRMWARE);
...
static ssize_t mstar_jni_msg_tool_read(struct file *pFile, char __user *pBuffer,
...
static void
mstar_get_leather_sheath_info(u8 *pLeatherSheathMode) // used for MSG28xx only
{
	u8 szTxData[3] = {0};
	u8 szRxData[2] = {0};
	u32 i = 0, count = 5;
	...
	while (i < count) {
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"Get leather sheath info mstar_iic_write_data() success\n");
		}
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"Get leather sheath info mstar_iic_read_data() success\n");
...
static void mstar_jni_reg_get_xbyte(MsgToolDrvCmd_t *pCmd)
{
	...
	mstar_get_reg_xbit(nAddr, apk_info->rtn_cmd_data, pCmd->nRtnCmdLen,
...
static MsgToolDrvCmd_t *mstar_jni_trans_cmd_from_user(unsigned long nArg)
{
	...
	if ((tCmdIn.nSndCmdLen > 0) &&
	    (tCmdIn.nSndCmdLen <= SND_CMD_DATA_MAX_LEN)) {
		pTransCmd->nSndCmdLen = tCmdIn.nSndCmdLen;
		nRet = copy_from_user(apk_info->snd_cmd_data,
	...
	if ((tCmdIn.nRtnCmdLen > 0) &&
	    (tCmdIn.nRtnCmdLen <= RTN_CMD_DATA_MAX_LEN)) {
		pTransCmd->nRtnCmdLen = tCmdIn.nRtnCmdLen;
		nRet = copy_from_user(apk_info->rtn_cmd_data,
...
static void mstar_jni_trans_cmd_to_user(MsgToolDrvCmd_t *pTransCmd,
					unsigned long nArg)
{
	...
	if ((tCmdOut.nRtnCmdLen <= 0) ||
	    (tCmdOut.nRtnCmdLen > RTN_CMD_DATA_MAX_LEN))
		return;
	nRet = copy_to_user(U64ToPtr(tCmdOut.nRtnCmdDataPtr),
			    apk_info->rtn_cmd_data, tCmdOut.nRtnCmdLen);
	if (nRet) {
...
{
	apk_info->msg_tool_cmd_in =
		(MsgToolDrvCmd_t *)kmalloc(sizeof(MsgToolDrvCmd_t), GFP_KERNEL);
...
#ifdef MP_TEST_FUNCTION_2
static ssize_t
mstar_apknode_mp_test_customised_write(struct file *pFile,
...
static ssize_t mstar_apknode_mp_test_customised_read(struct file *pFile,
						     char __user *pBuffer,
						     size_t nCount,
						     loff_t *pPos)
{
...
static ssize_t mstar_apknode_chip_type_read(struct file *pFile,
					    ...
					    loff_t *pPos)
{
	...
	TS_LOG_INFO(
		"tskit_mstar_data->chip_type = 0x%x, tskit_mstar_data->chip_type_ori = 0x%x\n",
...
static ssize_t mstar_apknode_chip_type_write(struct file *pFile,
					     const char __user *pBuffer,
					     size_t nCount, loff_t *pPos)
{
	return nCount;
...
static ssize_t mstar_apknode_fw_data_read(struct file *pFile,
					  char __user *pBuffer, size_t nCount,
					  loff_t *pPos)
{
...
static ssize_t mstar_apknode_fw_data_write(struct file *pFile,
					   const char __user *pBuffer,
					   size_t nCount, loff_t *pPos)
{
	...
	if (nCount > 1025) {
		TS_LOG_ERR(
			"%s:The size of count(%d) is larger than its define\n",
			__func__, nCount);
		return nCount;
	}
	...
	if (ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data.two_dimen_fw_data)) {
		...
		tskit_mstar_data->fw_update_data.two_dimen_fw_data =
			(u8 **)kmalloc(130 * sizeof(u8 *), GFP_KERNEL);
		if (ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data
					  .two_dimen_fw_data)) {
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG22XX ||
		...
		if (nNum > 0) // nCount >= 1024
		{
			for (i = 0; i < nNum; i++) {
				...
				if (copy_from_user(apk_info->debug_buf,
						   pBuffer + (i * 1024),
						   1024)) {
					TS_LOG_INFO(
				...
				tskit_mstar_data->fw_update_data.two_dimen_fw_data
					[tskit_mstar_data->fw_update_data
						 .fw_data_cont] =
					(u8 *)kmalloc(1024 * sizeof(u8),
						      GFP_KERNEL);
				if (ERR_ALLOC_MEM(
					    tskit_mstar_data->fw_update_data.two_dimen_fw_data
						    [tskit_mstar_data
							     ...
							     .fw_data_cont])) {
					TS_LOG_ERR(
						"Failed to allocate FW buffer\n");
					goto out;
				}

				memcpy(tskit_mstar_data->fw_update_data
					       .two_dimen_fw_data
						       [tskit_mstar_data
								->fw_update_data
								.fw_data_cont],
				       apk_info->debug_buf, 1024);
				if (tskit_mstar_data->fw_update_data
					    .fw_data_cont < 130) {
					tskit_mstar_data->fw_update_data
						.fw_data_cont++;
				} else {
			...
			if (nRemainder >
			    0) // Handle special firmware size like MSG22XX(48.5KB)
			{
		...
		} else // nCount < 1024
		{
			if (nCount > 0) {
				memset(apk_info->debug_buf, 0, 1024);
				if (copy_from_user(apk_info->debug_buf, pBuffer,
						   nCount)) {
					TS_LOG_INFO(
						"copy_from_user() failed\n");
...
out:
	if (!ERR_ALLOC_MEM(
		...
		for (i = 0; i < 130; i++) {
			if (!ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data
						   .two_dimen_fw_data[i])) {
				kfree(tskit_mstar_data->fw_update_data
					      .two_dimen_fw_data[i]);
				tskit_mstar_data->fw_update_data
					.two_dimen_fw_data[i] = NULL;
			}
		}
...
static ssize_t mstar_apknode_fw_update_read(struct file *pFile,
					    char __user *pBuffer, size_t nCount,
					    loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_fw_update_write(struct file *pFile,
					     const char __user *pBuffer,
					     size_t nCount, loff_t *pPos)
{
	int i = 0;
	...
	if (ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data.two_dimen_fw_data) ||
	    tskit_mstar_data->fw_update_data.fw_data_cont == 0) {
		TS_LOG_ERR("FW Buffer is NULL\n");
		goto out;
	}

	tskit_mstar_data->fw_update_data.one_dimen_fw_data =
		vmalloc(MSG28XX_FIRMWARE_WHOLE_SIZE * 1024 * sizeof(u8));
...
static ssize_t mstar_apknode_customer_fw_ver_read(struct file *pFile,
						  char __user *pBuffer,
						  size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_customer_fw_ver_write(struct file *pFile,
						   const char __user *pBuffer,
						   size_t nCount, loff_t *pPos)
{
	char fw_version[32] = {0};
...
static ssize_t mstar_apknode_platform_fw_ver_read(struct file *pFile,
						  char __user *pBuffer,
						  size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_platform_fw_ver_write(struct file *pFile,
						   const char __user *pBuffer,
						   size_t nCount, loff_t *pPos)
{
	int ret = NO_ERR;
...
static ssize_t mstar_apknode_drive_ver_read(struct file *pFile,
					    char __user *pBuffer, size_t nCount,
					    loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_drive_ver_write(struct file *pFile,
					     const char __user *pBuffer,
					     size_t nCount, loff_t *pPos)
{
	return nCount;
...
static ssize_t mstar_apknode_fw_sdcard_update_read(struct file *pFile,
						   char __user *pBuffer,
						   size_t nCount, loff_t *pPos)
{
	char fw_version[32] = {0};
...
static ssize_t mstar_apknode_fw_sdcard_update_write(struct file *pFile,
						    const char __user *pBuffer,
						    size_t nCount, loff_t *pPos)
{
	char *pValid = NULL;
...
static ssize_t mstar_apknode_selinux_fw_update_read(struct file *pFile,
						    char __user *pBuffer,
						    size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG22XX ||
		...
		nRetVal = mstar_fw_update_sdcard(FIRMWARE_FILE_PATH_ON_SD_CARD,
						 1);
	} else {
		TS_LOG_INFO(
...
static ssize_t mstar_apknode_force_fw_update_read(struct file *pFile,
						  char __user *pBuffer,
						  size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	apk_info->force_update_firmware_enable =
		1; // Enable force firmware update
...
static ssize_t mstar_apknode_fw_debug_read(struct file *pFile,
					   char __user *pBuffer, size_t nCount,
					   loff_t *pPos)
{
	u32 i, nLength = 0;
...
static ssize_t mstar_apknode_fw_debug_write(struct file *pFile,
					    const char __user *pBuffer,
					    size_t nCount, loff_t *pPos)
{
	u32 i;
	...
	while ((pCh = strsep((char **)&pStr, " ,")) &&
		...
		apk_info->debug_reg[i] =
			mstar_convert_char_to_hex_digit(pCh, strlen(pCh));
...
static ssize_t mstar_apknode_fw_set_debug_value_read(struct file *pFile,
						     char __user *pBuffer,
						     size_t nCount,
						     loff_t *pPos)
{
...
static ssize_t
mstar_apknode_fw_set_debug_value_write(struct file *pFile,
				       const char __user *pBuffer,
				       size_t nCount, loff_t *pPos)
{
	u32 i, j, k;
	...
	while ((pCh = strsep((char **)&pStr, " ,")) && (i < 2)) {
		...
		if ((i % 2) == 0) {
			apk_info->debug_reg[j] =
				mstar_convert_char_to_hex_digit(pCh,
								strlen(pCh));
			TS_LOG_INFO("apk_info->debug_reg[%d] = 0x%04X\n", j,
				    apk_info->debug_reg[j]);
			j++;
		} else {
			apk_info->debug_reg_value[k] =
				mstar_convert_char_to_hex_digit(pCh,
								strlen(pCh));
	...
	for (i = 0; i < apk_info->debug_reg_count; i++) {
		...
		TS_LOG_INFO(
			"apk_info->debug_reg[%d] = 0x%04X, g_debug_reg_value[%d] = 0x%04X\n",
			i, apk_info->debug_reg[i], i,
			apk_info->debug_reg_value[i]); // add for debug
	}
...
static ssize_t mstar_apknode_fw_smbus_debug_read(struct file *pFile,
						 char __user *pBuffer,
						 size_t nCount, loff_t *pPos)
{
	...
	while (i < count) {
		if (apk_info->debug_cmd_arg_count > 0) // Send write command
		{
			...
			rc = mstar_iic_write_data(
				SLAVE_I2C_ID_DWI2C,
				&apk_info->debug_cmd_argu[0],
				apk_info->debug_cmd_arg_count);
			if (rc >= 0) {
				TS_LOG_INFO(
					"mstar_iic_write_data(0x%X, 0x%X, %d) success\n",
					SLAVE_I2C_ID_DWI2C,
					apk_info->debug_cmd_argu[0],
					apk_info->debug_cmd_arg_count);
		...
		if (apk_info->debug_read_data_size > 0) // Send read command
		{
			...
			rc = mstar_iic_read_data(
				SLAVE_I2C_ID_DWI2C, &szSmBusRxData[0],
				apk_info->debug_read_data_size);
			if (rc >= 0) {
				TS_LOG_INFO(
					"mstar_iic_read_data(0x%X, 0x%X, %d) success\n",
					SLAVE_I2C_ID_DWI2C, szSmBusRxData[0],
					apk_info->debug_read_data_size);
				break;
			}
	...
	if (i == count) {
		TS_LOG_INFO(
			"mstar_iic_write_data() & mstar_iic_read_data() failed, rc = %d\n",
			rc);
	}
...
static ssize_t mstar_apknode_fw_smbus_debug_write(struct file *pFile,
						  const char __user *pBuffer,
						  size_t nCount, loff_t *pPos)
{
	u32 i, j;
	...
	while ((pCh = strsep((char **)&pStr, " ,")) &&
		...
		if (strcmp(pCh, "w") == 0 || strcmp(pCh, "r") == 0) {
			...
			rc = sscanf(pCh, "%d", &apk_info->debug_read_data_size);
			if (rc <= 0)
...
static ssize_t mstar_apknode_fw_set_dq_mem_value_read(struct file *pFile,
						      char __user *pBuffer,
						      size_t nCount,
						      loff_t *pPos)
{
...
static ssize_t
mstar_apknode_fw_set_dq_mem_value_write(struct file *pFile,
					const char __user *pBuffer,
					size_t nCount, loff_t *pPos)
{
	u32 i, j, k;
...
static ssize_t mstar_apknode_fw_mode_read(struct file *pFile,
					  char __user *pBuffer, size_t nCount,
					  loff_t *pPos)
{
	int ret = NO_ERR;
...
static ssize_t mstar_apknode_fw_mode_write(struct file *pFile,
					   const char __user *pBuffer,
					   size_t nCount, loff_t *pPos)
{
	u32 nMode = 0;
	...
	if (nMode == MSG28XX_FIRMWARE_MODE_DEMO_MODE) {
		tskit_mstar_data->fw_mode =
			mstar_change_fw_mode(MSG28XX_FIRMWARE_MODE_DEMO_MODE);
	} else if (nMode == MSG28XX_FIRMWARE_MODE_DEBUG_MODE) {
		tskit_mstar_data->fw_mode =
			mstar_change_fw_mode(MSG28XX_FIRMWARE_MODE_DEBUG_MODE);
		apk_info->switch_mode = 1;
		apk_info->debug_log_time_stamp =
			0; // Set g_debug_log_time_stamp for filter duplicate packet on MTPTool APK
	} else {
...
static ssize_t mstar_apknode_fw_sensor_read(struct file *pFile,
					    char __user *pBuffer, size_t nCount,
					    loff_t *pPos)
{
	u32 nLength = 0;
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
	    tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) {
		if (tskit_mstar_data->mutual_fw_info.nLogModePacketHeader ==
		    0xA7) {
			nLength = sprintf(nUserTempBuffer, "%d,%d,%d,%d",
					  tskit_mstar_data->mutual_fw_info.nMx,
					  tskit_mstar_data->mutual_fw_info.nMy,
					  tskit_mstar_data->mutual_fw_info.nSs,
					  tskit_mstar_data->mutual_fw_info.nSd);
		} else {
			TS_LOG_INFO(
...
static ssize_t mstar_apknode_fw_sensor_write(struct file *pFile,
					     const char __user *pBuffer,
					     size_t nCount, loff_t *pPos)
{
	int ret = 0;
	...
	if (pBuffer != NULL) {
		ret = copy_from_user(cmd, pBuffer,
				     APKNODE_WRITE_MESSAGE_LENGTH);
		if (ret < 0) {
...
static ssize_t mstar_apknode_fw_packet_header_read(struct file *pFile,
						   char __user *pBuffer,
						   size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_fw_packet_header_write(struct file *pFile,
						    const char __user *pBuffer,
						    size_t nCount, loff_t *pPos)
{
	return nCount;
...
static ssize_t _DrvKObjectPacketShow(struct kobject *pKObj,
	...
	if (strcmp(pAttr->attr.name, "packet") == 0) {
		if (tskit_mstar_data->fw_mode ==
			...
			if (tskit_mstar_data->demo_packet != NULL) {
				TS_LOG_INFO(
					"tskit_mstar_data->fw_mode=%x, g_demo_packet[0]=%x, g_demo_packet[1]=%x\n",
					tskit_mstar_data->fw_mode,
					tskit_mstar_data->demo_packet[0],
					tskit_mstar_data->demo_packet[1]);
				TS_LOG_INFO(
					"g_demo_packet[2]=%x, g_demo_packet[3]=%x\n",
					tskit_mstar_data->demo_packet[2],
					tskit_mstar_data->demo_packet[3]);
				TS_LOG_INFO(
					"g_demo_packet[4]=%x, g_demo_packet[5]=%x\n",
					tskit_mstar_data->demo_packet[4],
					tskit_mstar_data->demo_packet[5]);

				memcpy(pBuf, tskit_mstar_data->demo_packet,
			...
			TS_LOG_INFO(
				"tskit_mstar_data->fw_mode=%x, g_log_packet[0]=%x, g_log_packet[1]=%x\n",
				tskit_mstar_data->fw_mode,
				tskit_mstar_data->log_packet[0],
				tskit_mstar_data->log_packet[1]);
			...
			if ((tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
			    ...
			    (tskit_mstar_data->log_packet[0] == 0xA7)) {
				memcpy(pBuf, tskit_mstar_data->log_packet,
				       tskit_mstar_data->mutual_fw_info
				...
				pBuf[tskit_mstar_data->mutual_fw_info
					     .nLogModePacketLength] =
					apk_info->debug_log_time_stamp;
				TS_LOG_INFO(
					"g_debug_log_time_stamp=%d\n",
					pBuf[tskit_mstar_data->mutual_fw_info
						     .nLogModePacketLength]); // TODO : add for debug

				nLength = tskit_mstar_data->mutual_fw_info
						  .nLogModePacketLength +
					  1;
				TS_LOG_INFO("nLength = %d\n", nLength);
			} else {
...
static ssize_t mstar_apknode_kobject_packet_store(struct kobject *pKObj,
...
static struct kobj_attribute packet_attr =
	__ATTR(packet, 0664, _DrvKObjectPacketShow,
	       mstar_apknode_kobject_packet_store);
...
static ssize_t mstar_apknode_query_feature_support_status_read(
	struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	u8 nUserTempBuffer[16] = {0};
...
static ssize_t
mstar_apknode_query_feature_support_status_write(struct file *pFile,
						 const char __user *pBuffer,
						 size_t nCount, loff_t *pPos)
{
	int ret = NO_ERR;
	...
	if (nFeature == FEATURE_GESTURE_WAKEUP_MODE) {
		apk_info->feature_support_status =
			apk_info->gesture_wakeup_enable;
	} else if (nFeature == FEATURE_GESTURE_DEBUG_MODE) {
		apk_info->feature_support_status =
			apk_info->gesture_debug_mode_enable;
	} else if (nFeature == FEATURE_GESTURE_INFORMATION_MODE) {
		apk_info->feature_support_status =
			apk_info->gesture_information_mode_enable;
		...
		apk_info->feature_support_status =
			apk_info->firmware_log_enable;

#ifdef CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA
		if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		    tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) {
			if (apk_info->feature_support_status == 1) {
				ret = mstar_get_touch_packet_addr(
					&tskit_mstar_data->fw_data
						 .packet_data_addr,
					&tskit_mstar_data->fw_data
						 .packet_flag_addr);
				if (ret < 0) {
					TS_LOG_ERR(
						"%s get touch packet addr error ret = %d\n",
						__func__, ret);
				}
			}
		}
...
static ssize_t mstar_apknode_change_feature_support_status_read(
	struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	u8 nUserTempBuffer[16] = {0};
...
static ssize_t
mstar_apknode_change_feature_support_status_write(struct file *pFile,
						  const char __user *pBuffer,
						  size_t nCount, loff_t *pPos)
{
	u32 i;
	...
	while ((pCh = strsep((char **)&pStr, " ,")) && (i < 3)) {
		...
		if (i == 0) {
			nFeature = mstar_convert_char_to_hex_digit(pCh,
								   strlen(pCh));
			TS_LOG_INFO("nFeature = 0x%04X\n", nFeature);
		} else if (i == 1) {
			nNewValue = mstar_convert_char_to_hex_digit(
				pCh, strlen(pCh));
...
//------------------------------------------------------------------------------//
static ssize_t mstar_apknode_gesture_wakeup_mode_read(struct file *pFile,
						      char __user *pBuffer,
						      size_t nCount,
						      loff_t *pPos)
{
...
static ssize_t mstar_apknode_gesture_info_mode_read(struct file *pFile,
						    char __user *pBuffer,
						    size_t nCount, loff_t *pPos)
{
	...
	if (apk_info->log_gesture_info_type ==
	    FIRMWARE_GESTURE_INFORMATION_MODE_A) //FIRMWARE_GESTURE_INFORMATION_MODE_A
	{
		for (i = 0; i < 2; i++) //0 EventFlag; 1 RecordNum
		{
			szLogGestureInfo[g_log_gesture_count] =
				tskit_mstar_data->gesture_data.log_info[4 + i];
			g_log_gesture_count++;
		}

		for (i = 2; i < 8;
		     i++) //2~3 Xst Yst; 4~5 Xend Yend; 6~7 char_width char_height
		{
			szLogGestureInfo[g_log_gesture_count] =
				tskit_mstar_data->gesture_data.log_info[4 + i];
			g_log_gesture_count++;
		}
	} else if (apk_info->log_gesture_info_type ==
		   FIRMWARE_GESTURE_INFORMATION_MODE_B) //FIRMWARE_GESTURE_INFORMATION_MODE_B
	{
		...
		for (i = 0; i < tskit_mstar_data->gesture_data.log_info[5] * 2;
		     i++) //(X and Y)*RecordNum
		{
			szLogGestureInfo[g_log_gesture_count] =
				tskit_mstar_data->gesture_data.log_info[12 + i];
			g_log_gesture_count++;
		}
	} else if (apk_info->log_gesture_info_type ==
		   FIRMWARE_GESTURE_INFORMATION_MODE_C) //FIRMWARE_GESTURE_INFORMATION_MODE_C
	{
		...
		{
			szLogGestureInfo[g_log_gesture_count] =
				tskit_mstar_data->gesture_data.log_info[i];
			g_log_gesture_count++;
		}
		...
		szLogGestureInfo[g_log_gesture_count] =
			tskit_mstar_data->gesture_data.log_info[86]; //dummy
		g_log_gesture_count++;
		szLogGestureInfo[g_log_gesture_count] =
			tskit_mstar_data->gesture_data.log_info[87]; //checksum
		g_log_gesture_count++;
	} else {
...
static ssize_t mstar_apknode_gesture_info_mode_write(struct file *pFile,
						     const char __user *pBuffer,
						     size_t nCount,
						     loff_t *pPos)
{
...
static ssize_t mstar_apknode_glove_mode_read(struct file *pFile,
					     char __user *pBuffer,
					     size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_glove_mode_write(struct file *pFile,
					      const char __user *pBuffer,
					      size_t nCount, loff_t *pPos)
{
	u32 nGloveMode = 0;
...
static ssize_t mstar_apknode_glove_open_read(struct file *pFile,
					     char __user *pBuffer,
					     size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_glove_close_read(struct file *pFile,
					      char __user *pBuffer,
					      size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
static ssize_t mstar_apknode_leather_sheath_mode_read(struct file *pFile,
						      char __user *pBuffer,
						      size_t nCount,
						      loff_t *pPos)
{
...
static ssize_t
mstar_apknode_leather_sheath_mode_write(struct file *pFile,
					const char __user *pBuffer,
					size_t nCount, loff_t *pPos)
{
	...
	if (pStr != NULL) {
		...
		while ((pCh = strsep((char **)&pStr, ",")) && (i < 1)) {
			...
			nLeatherSheathMode = mstar_convert_char_to_hex_digit(
				pCh, strlen(pCh));
		...
		} else if (nLeatherSheathMode ==
			   0x00) //close leather sheath mode
		{
...
static long mstar_apknode_jni_msgtool_ioctl(struct file *pFile,
					    unsigned int nCmd,
					    unsigned long nArg)
{
	long nRet = 0;
	...
	case MSGTOOL_IOCTL_RUN_CMD: {
		...
		case MSGTOOL_FINGERTOUCH:
			if (pTransCmd->nSndCmdLen == 1) {
				...
				mstar_finger_touch_report_enable();
				tskit_mstar_data->finger_touch_disable =
					FALSE; // Resume finger touch ISR handling after MTPTool APK have sent i2c command to firmware.
			} else if (pTransCmd->nSndCmdLen == 0) {
				TS_LOG_INFO("JNI disable touch\n");
				mstar_finger_touch_report_disable();
				tskit_mstar_data->finger_touch_disable =
					TRUE; // Skip finger touch ISR handling temporarily for MTPTool APK can send i2c command to firmware.
			}
			break;
		case MSGTOOL_DEVICEPOWEROFF:
			if (mstar_enter_sleep_mode() < 0)
				TS_LOG_ERR("%s: mstar_enter_sleep_mode fail\n",
					   __func__);
			break;
...
static ssize_t mstar_apknode_jni_msgtool_write(struct file *pFile,
					       const char __user *pBuffer,
					       size_t nCount, loff_t *pPos)
{
	long nRet = 0;
	...
	if (nBusType == SLAVE_I2C_ID_DBBUS || nBusType == SLAVE_I2C_ID_DWI2C) {
		...
		if (nWriteLen > sizeof(szCmdData)) {
			TS_LOG_ERR(
				"Count(%d) is larger than buffer size (%d)\n",
		...
		nRet = copy_from_user(buffer, pBuffer,
				      APKNODE_MSGTOOL_WRITE_LENGTH);
		if (nRet < 0) {
...
static ssize_t mstar_apknode_set_film_mode_write(struct file *pFile,
						 const char __user *pBuffer,
						 size_t nCount, loff_t *pPos)
{
	u8 nFilmType = 0;
	...
	TS_LOG_INFO("nCount = %d\n", (int)nCount);
	apk_info->debug_buf[nCount] = '\0';
	nFilmType = mstar_convert_char_to_hex_digit(
		apk_info->debug_buf, strlen(apk_info->debug_buf));
...
static ssize_t mstar_apknode_set_film_mode_read(struct file *pFile,
						char __user *pBuffer,
						size_t nCount, loff_t *pPos)
{
	u8 nFilmType = 0;
...
static ssize_t ts_info_show(struct class *class, struct class_attribute *attr,
			    char *buf)
{
			     ...
			     const char *buf, size_t count)
{
	...
	TS_LOG_DEBUG("buf = %s, g_GestureState = %d, count = %zu\n", buf,
		     apk_info->ans.g_GestureState, count);
	return count;
}

static CLASS_ATTR_RW(gesture);

static ssize_t mstar_proc_debug_switch_read(struct file *pFile,
					    ...
					    loff_t *pPos)
{
	...
	if (apk_info->debug_node_open) {
		apk_info->fw_debug_buf =
			kcalloc(1024, sizeof(unsigned char *), GFP_KERNEL);
		if (!ERR_ALLOC_MEM(apk_info->fw_debug_buf)) {
			for (i = 0; i < 1024; i++) {
				apk_info->fw_debug_buf[i] =
					kcalloc(1024, sizeof(unsigned char),
						GFP_KERNEL);
				if (ERR_ALLOC_MEM(apk_info->fw_debug_buf[i])) {
					TS_LOG_ERR(
						"%s, Failed to allocate fw_debug_buf two_dimen i = %d\n",
						__func__, i);
...
static ssize_t mstar_proc_debug_message_write(struct file *filp,
...
static ssize_t mstar_proc_debug_message_read(struct file *filp,
					     ...
					     loff_t *pPos)
{
	...
	while (apk_info->debug_data_frame <= 0) {
		...
		wait_event_interruptible(apk_info->inq,
	...
	if (ERR_ALLOC_MEM(tmpbuf)) {
		...
		ret = copy_to_user(
			buff, tmpbufback,
			send_data_len); /*apk_info->fw_debug_buf[0] */
	} else {
		if (apk_info->debug_data_frame > 0) {
			if (apk_info->fw_debug_buf[0][0] == 0x5A) {
				...
				data_count = apk_info->fw_debug_buf[0][1] *
					     apk_info->fw_debug_buf[0][2];
			...
			if (p == 5 || size == 4096 || size == 2048) {
				...
				for (i = 1; i <= apk_info->debug_data_frame;
				     i++) {
					memcpy(apk_info->fw_debug_buf[i - 1],
					       apk_info->fw_debug_buf[i], 2048);
				}
			}
		} else {
...
struct procfs_table p_table[] = {
	...
	{PROC_NODE_CUSTOMER_FIRMWARE_VERSION, NULL, &g_fops_customer_fw_ver,
	 false},
	{PROC_NODE_PLATFORM_FIRMWARE_VERSION, NULL, &g_fops_platform_fw_ver,
	 false},
	{PROC_NODE_DEVICE_DRIVER_VERSION, NULL, &g_fops_drive_ver, false},
	{PROC_NODE_SD_CARD_FIRMWARE_UPDATE, NULL, &g_fops_sdcard_fw_update,
	 false},
	{PROC_NODE_FIRMWARE_DEBUG, NULL, &g_fops_fw_debug, false},
	{PROC_NODE_FIRMWARE_SET_DEBUG_VALUE, NULL, &g_fops_fw_set_debug_value,
	 false},
	{PROC_NODE_FIRMWARE_SMBUS_DEBUG, NULL, &g_fops_fw_smbus_debug, false},
	{PROC_NODE_FIRMWARE_SET_DQMEM_VALUE, NULL, &g_fops_fw_set_dq_mem_value,
	 false},
	{PROC_NODE_FIRMWARE_MODE, NULL, &g_fops_fw_mode, false},
	{PROC_NODE_FIRMWARE_SENSOR, NULL, &g_fops_fw_sensor, false},
	{PROC_NODE_FIRMWARE_PACKET_HEADER, NULL, &g_fops_fw_packet_header,
	 false},
	{PROC_NODE_QUERY_FEATURE_SUPPORT_STATUS, NULL,
	 &g_fops_query_feature_support_status, false},
	{PROC_NODE_CHANGE_FEATURE_SUPPORT_STATUS, NULL,
	 &g_fops_change_feature_support_status, false},
	{PROC_NODE_GESTURE_WAKEUP_MODE, NULL, &g_fops_gesture_wakeup_mode,
	 false},
	{PROC_NODE_GESTURE_INFORMATION_MODE, NULL, &g_fops_gesture_info_mode,
	 false},
	...
	{PROC_NODE_LEATHER_SHEATH_MODE, NULL, &g_fops_leather_sheath_mode,
	 false},
	{PROC_NODE_CONTROL_FILM_MODE, NULL, &g_fops_film_mode, false},
#ifdef CONFIG_ENABLE_JNI_INTERFACE
	{PROC_NODE_JNI_NODE, NULL, &g_fops_jni_method, false},
#endif
	{PROC_NODE_SELINUX_LIMIT_FIRMWARE_UPDATE, NULL,
	 &g_fops_selinux_fw_update, false},
...
//{PROC_NODE_TRIM_CODE, NULL, &_gProcTrimCode, false},
	...
	{PROC_NODE_DEBUG_MESSAGE_SWITCH, NULL, &proc_debug_message_switch_fops,
	 false},
...
int mstar_apknode_create_procfs(void)
{
	...
	g_proc_ts_msg20xx_entry = proc_mkdir(PROC_NODE_MS_TOUCHSCREEN_MSG20XX,
					     g_proc_class_entry);

	g_proc_device_entry =
		proc_mkdir(PROC_NODE_DEVICE, g_proc_ts_msg20xx_entry);

	/* A main loop to create apk nodes */
	for (i = 0; i < ARRAY_SIZE(p_table); i++) {
		p_table[i].node =
			proc_create(p_table[i].node_name, PROCFS_AUTHORITY,
				    g_proc_device_entry, p_table[i].fops);
	...
	apk_info->touch_kset =
		kset_create_and_add("kset_example", NULL, kernel_kobj);
...
void mstar_apknode_remove_procfs(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(p_table); i++) {
		if (p_table[i].is_created) {
			remove_proc_entry(p_table[i].node_name,
					  g_proc_device_entry);
		}
	}
...
void mstar_gesture_notify(u8 *packet)
{
	char *pEnvp[2];
	s32 nRetVal = 0;
	if (apk_info->gesture_kobj != NULL &&
	    (*packet) == PACKET_TYPE_GESTURE_DEBUG) {
		pEnvp[0] = "STATUS=GET_GESTURE_DEBUG";
		pEnvp[1] = NULL;

		nRetVal = kobject_uevent_env(apk_info->gesture_kobj,
					     KOBJ_CHANGE, pEnvp);
		TS_LOG_DEBUG(
			"kobject_uevent_env() STATUS=GET_GESTURE_DEBUG, nRetVal = %d\n",
			nRetVal);
	}
}
