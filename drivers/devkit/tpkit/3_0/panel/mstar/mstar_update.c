...
u16 mstar_get_chip_type(void)
{
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"  %s()   Change mcu clock deglitch mux sourceerror\n",
			__func__);
		return nChipType;
	}
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"  %s()   isable cpu read, initial; read (0x1608, 0x20) error\n",
			__func__);
		return nChipType;
	}
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"  %s()    set eflash mode to read mode (0x1606, 0x01)  error\n",
			__func__);
		return nChipType;
	}
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"  %s()    set eflash mode to read mode (0x1610, 0x01)  error\n",
			__func__);
		return nChipType;
	}
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"  %s()    set eflash mode to read mode (0x1611, 0x00)  error\n",
			__func__);
		return nChipType;
	}
	// set read address
	...
	if (nChipType == CHIP_TYPE_MSG2836A ||
	    nChipType == CHIP_TYPE_MSG2846A ||
	    nChipType == CHIP_TYPE_MSG5846A ||
	    nChipType == CHIP_TYPE_MSG5856A) {
		tskit_mstar_data->chip_type_ori = nChipType;
		TS_LOG_INFO(
			"----------------------MSG Chip Type=0x%x, MSG%xA-------------------------\n",
			nChipType, nChipType);
...
static int
mstar_get_customer_fw_ver_dbbus(EmemType_e eEmemType, u16 *pMajor, u16 *pMinor,
				u8 **ppVersion) // support MSG28xx only
{
	int rc = NO_ERR;
	u16 nReadAddr = 0;
	u8 szTmpData[4] = {0};
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		...
		if (eEmemType == EMEM_MAIN) // Read SW ID from main block
		{
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"%s EMEM_MAIN read eflash start error rc = %d\n",
					__func__, rc);
			}
			nReadAddr = 0x7FFD;
		} else if (eEmemType == EMEM_INFO) // Read SW ID from info block
		{
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"%s EMEM_INFO read eflash start error rc = %d\n",
					__func__, rc);
			}
			nReadAddr = 0x81FB;
		}
...
static void mstar_fw_store_data(u8 *pBuf, u32 nSize)
{
	...
	{
		for (i = 0; i < nCount; i++) {
			memcpy(tskit_mstar_data->fw_update_data.two_dimen_fw_data
...
static void mstar_convert_fw_two_dimen_to_one(u8 **szTwoDimenFwData,
					      u8 *pOneDimenFwData)
{
	u32 i, j;
...
static u32 mstar_calculate_crc(u8 *pFwData, u32 nOffset, u32 nSize)
{
	...
	for (i = 0; i < nSize; i += 4) {
		nData = (pFwData[nOffset + i]) |
			(pFwData[nOffset + i + 1] << 8) |
...
static int mstar_access_eflash_init(void)
{
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Disable cpu read flash set reg (0x1608, 0x20) error rc = %d\n",
			__func__, rc);
	}
...
static int mstar_isp_burst_write_eflash_start(u16 nStartAddr, u8 *pFirstData,
					      ...
					      EmemType_e eEmemType)
{
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Disable cpu read flash set reg (0x1606, 0x20) error rc = %d\n",
			__func__, rc);
	}
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set e-flash mode to page write mode error rc = %d\n",
			__func__, rc);
	}
	// Set data align
	...
	if (eEmemType == EMEM_INFO) {
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s mstar_set_reg_low_byte(0x1607, 0x08) error rc = %d\n",
				__func__, rc);
		}
	}
	// Set double buffer
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set e-flash mode trigger(Trigger write mode) error rc = %d\n",
			__func__, rc);
	}
	// Set init data
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set initial address(for latch SA, CA) error rc = %d\n",
			__func__, rc);
	}
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set initial address(for latch PA) error rc = %d\n",
			__func__, rc);
	}
	// Enable burst mode
...
static int mstar_isp_burst_write_eflash_do_write(u8 *pBufferData, u32 nLength)
{
	...
	return mstar_iic_write_data(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0],
				    3 + nLength);
}
...
static int mstar_write_eflash_start(u16 nStartAddr, u8 *pFirstData,
				    EmemType_e eEmemType)
{
	int rc = NO_ERR;
	u16 nWriteAddr = nStartAddr / 4;
	...
	if (eEmemType == EMEM_INFO) {
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO mstar_set_reg_low_byte(0x1607, 0x08) error rc = %d\n",
				__func__, rc);
		}
	}
	// Set double buffer
...
static int mstar_read_eflash_start(u16 nStartAddr, EmemType_e eEmemType)
{
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s disable cpu read flash set reg (0x1608, 0x20)  error ret = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1606, 0x20);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s disable cpu read flash set reg (0x1606, 0x20)  error ret = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1606, 0x20);
	if (rc < 0) {
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s disable cpu read flash set reg (0x1600, nStartAddr)  error ret = %d\n",
			__func__, rc);
	}
	if (eEmemType == EMEM_MAIN) {
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set e-flash mode to read mode for main  error ret = %d\n",
				__func__, rc);
		}
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set info double buffer  error ret = %d\n",
				__func__, rc);
		}
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set e-flash mode to read mode for info error ret = %d\n",
				__func__, rc);
		}
	}
	return rc;
...
static int mstar_get_sft_addr3_value(void)
{
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set e-flash mode to read mode set reg (0x1606, 0x01) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1610, 0x01);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set e-flash mode to read mode set reg (0x1610, 0x01) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1607, 0x20);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set e-flash mode to read mode set reg (0x1607, 0x20) error rc = %d\n",
			__func__, rc);
	}
	// Set read address
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set read address set reg (0x1600, 0x03) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1601, 0x00);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set read address set reg (0x1601, 0x00) error rc = %d\n",
			__func__, rc);
	}
	tskit_mstar_data->fw_update_data.sfr_addr3_byte0_to_1 =
		mstar_get_reg_16bit(0x160A);
	tskit_mstar_data->fw_update_data.sfr_addr3_byte2_to_3 =
		mstar_get_reg_16bit(0x160C);

	TS_LOG_INFO(
		"tskit_mstar_data->fw_update_data.sfr_addr3_byte0_to_1 = 0x%4X, tskit_mstar_data->fw_update_data.sfr_addr3_byte2_to_3 = 0x%4X\n",
		tskit_mstar_data->fw_update_data.sfr_addr3_byte0_to_1,
		tskit_mstar_data->fw_update_data.sfr_addr3_byte2_to_3);
	return rc;
}
...
static int mstar_unset_protect_bit(void)
{
	...
	nB1 = (tskit_mstar_data->fw_update_data.sfr_addr3_byte0_to_1 &
	       0xFF00) >>
	      8;

	nB2 = tskit_mstar_data->fw_update_data.sfr_addr3_byte2_to_3 & 0xFF;
	nB3 = (tskit_mstar_data->fw_update_data.sfr_addr3_byte2_to_3 &
	       0xFF00) >>
	      8;
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Disable cpu read flash set reg (0x1610, 0x80) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1607, 0x10);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Disable cpu read flash set reg (0x1607, 0x10) error rc = %d\n",
			__func__, rc);
	}
	// Trigger SFR write
	rc = mstar_set_reg_low_byte(0x1606, 0x01);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Trigger SFR write set reg (0x1606, 0x01) error rc = %d\n",
			__func__, rc);
	}
	// Set write data
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set write data set reg (0x1602, nB0) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1602, nB1);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set write data set reg (0x1602, nB1) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1602, nB2);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set write data set reg (0x1602, nB2) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1602, nB3);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set write data set reg (0x1602, nB3) error rc = %d\n",
			__func__, rc);
	}
	// Set write address
	rc = mstar_set_reg_low_byte(0x1600, 0x03);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set write address set reg (0x1600, 0x03) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1601, 0x00);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set write address set reg (0x1601, 0x00) error rc = %d\n",
			__func__, rc);
	}
	// Set TM mode = 0
	rc = mstar_set_reg_low_byte(0x1607, 0x00);
...
int mstar_set_protect_bit(void)
{
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s Set write address set reg (0x1606, 0x02) error rc = %d\n",
			__func__, rc);
	}
	return rc;
}
...
void mstar_erase_emem(EmemType_e eEmemType)
{
	...
	if (eEmemType == EMEM_MAIN) // 128KB
	{
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set e-flash mode to erase mode error rc = %d\n",
				__func__, rc);
		}
		// Set page erase main
	...
	} else if (eEmemType == EMEM_INFO) // 2KB
	{
		...
		for (nInfoAddr = 0x20; nInfoAddr <= MSG28XX_EMEM_INFO_MAX_ADDR;
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"%s e-flash mode trigger error rc = %d\n",
					__func__, rc);
			}
			nTimeOut = 0;
			while (1) // Wait erase done
			{
				...
				if ((nTimeOut++) > 10) {
					...
					// Set main block
					rc = mstar_set_reg_low_byte(0x1607,
								    0x00);
					if (rc < 0) {
						TS_LOG_ERR(
							"%s Set main block error rc = %d\n",
							__func__, rc);
					}
					goto EraseEnd;
				}
...
EraseEnd:
	...
	if (rc < 0) {
		TS_LOG_ERR(
			"%s mstar_set_reg_low_byte(0x1606, 0x00) error rc = %d\n",
			__func__, rc);
	}
	rc = mstar_set_reg_low_byte(0x1607, 0x00);
	if (rc < 0) {
		TS_LOG_ERR(
			"%s mstar_set_reg_low_byte(0x1607, 0x00) error rc = %d\n",
			__func__, rc);
	}
	// Clear PROGRAM erase password
	rc = mstar_set_reg_16bit(0x1618, 0xA55A);
...
static void mstar_program_emem(EmemType_e eEmemType)
{
	...
	if (eEmemType == EMEM_MAIN) // Program main block
	{
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set ISP mode set reg 0x1EBE error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_low_byte(0x1604, 0x01);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set ISP mode set reg 0x1604 error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_16bit(0x161A, nPageNum);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set ISP mode set reg 0x161A error rc = %d\n",
				__func__, rc);
		}
		...
		rc = mstar_set_reg_16bit_on(0x3C00,
					    BIT0); // Disable INT GPIO mode
		if (rc < 0) {
		...
		rc = mstar_set_reg_16bit(0x1E34,
					 0x0000); // Set DQMem start address
		if (rc < 0) {
		...
		for (j = 0; j < nLength; j++) {
			szWriteData[3 + j] = tskit_mstar_data->fw_update_data
						     .two_dimen_fw_data[0][j];
		}
		rc = mstar_iic_write_data(
			SLAVE_I2C_ID_DBBUS, &szWriteData[0],
			3 + nLength); // Write the first two pages(page 0 & page 1)
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Write the first two pages(page 0 & page 1) error rc = %d\n",
				__func__, rc);
		}
		...
		for (i = 2; i < nPageNum; i++) {
			if (i == 2) {
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s first write upgrade data error rc = %d\n",
						__func__, rc);
				}
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s end write upgrade data error rc = %d\n",
						__func__, rc);
				}
			} else {
				//                szWriteData[0] = 0x10;
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s middle write upgrade data error rc = %d\n",
						__func__, rc);
				}
			}
		}
		...
		for (i = 0; i < nPageNum; i++) {
			if (i == 0) {
				...
				szFirstData[0] =
					tskit_mstar_data->fw_update_data
						.two_dimen_fw_data[0][0];
				szFirstData[1] =
					tskit_mstar_data->fw_update_data
						.two_dimen_fw_data[0][1];
				szFirstData[2] =
					tskit_mstar_data->fw_update_data
						.two_dimen_fw_data[0][2];
				szFirstData[3] =
					tskit_mstar_data->fw_update_data
						.two_dimen_fw_data[0][3];

				rc = mstar_isp_burst_write_eflash_start(
					nIndex, &szFirstData[0],
					MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024,
					nPageNum, EMEM_MAIN);
				if (rc < 0) {
					TS_LOG_ERR(
						"%s mstar_isp_burst_write_eflash_start error rc = %d\n",
						__func__, rc);
				}
				nIndex += nLength;
				...
				nLength =
					8 -
					MSG28XX_EMEM_SIZE_BYTES_ONE_WORD; // 4 = 8 - 4
#elif defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_32_BYTE_EACH_TIME)
				nLength =
					32 -
					MSG28XX_EMEM_SIZE_BYTES_ONE_WORD; // 28 = 32 - 4
#else // UPDATE FIRMWARE WITH 128 BYTE EACH TIME
				nLength =
					MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE -
...
#else // UPDATE FIRMWARE WITH 128 BYTE EACH TIME
				nLength =
					MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE; // 128
#endif
			...
			rc = mstar_isp_burst_write_eflash_do_write(
				&szBufferData[0], nLength);
			if (rc < 0) {
				TS_LOG_ERR(
					"%s mstar_isp_burst_write_eflash_do_write error rc = %d\n",
					__func__, rc);
			}
			udelay(2000); // delay about 2ms
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s mstar_isp_burst_write_eflash_end error rc = %d\n",
				__func__, rc);
		}
		// Set write done
	...
	} else if (eEmemType == EMEM_INFO) // Program info block
	{
		...
		for (i = 1; i < (nPageNum - 1);
		     i++) // Skip the first 128 byte and the last 128 byte of info block
		{
			if (i == 1) {
				// Read first data 4 bytes
				nLength = MSG28XX_EMEM_SIZE_BYTES_ONE_WORD;
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s EMEM_INFO mstar_isp_burst_write_eflash_start error rc = %d\n",
						__func__, rc);
				}
				nIndex += nLength;
				     ...
				     j++) // 124/8 = 15
				{
					...
					if (rc < 0) {
						TS_LOG_ERR(
							"%s EMEM_INFO mstar_isp_burst_write_eflash_do_write error rc = %d\n",
							__func__, rc);
					}
					udelay(2000); // delay about 2ms
				}
				...
				if (i < 8) // 1 < i < 8
				{
					     ...
					     j++) // 128/8 = 16
					{
						...
						if (rc < 0) {
							TS_LOG_ERR(
								"%s EMEM_INFO  i < 8 mstar_isp_burst_write_eflash_do_write error rc = %d\n",
								__func__, rc);
						}
						udelay(2000); // delay about 2ms
					}
	...
	defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_32_BYTE_EACH_TIME)
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO mstar_isp_burst_write_eflash_end error rc = %d\n",
				__func__, rc);
		}
		// Set write done
		rc = mstar_set_reg_16bit_on(0x1606, BIT2);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Set write done error rc = %d\n",
				__func__, rc);
		}
		// Check RBB
		nRegData = mstar_get_reg_low_byte(0x160E);
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO set reg 0x1ebe error rc = %d\n",
				__func__, rc);
		}
		...
		for (i = 0; i < nWordNum; i++) {
			if (i == 0) {
				...
				rc = mstar_write_eflash_start(
					nIndex, &szFirstData[0], EMEM_INFO);
				if (rc < 0) {
					TS_LOG_ERR(
						"%s EMEM_INFO mstar_write_eflash_start error rc = %d\n",
						__func__, rc);
				}
			} else {
				for (j = 0; j < nLength; j++) {
				...
				rc = mstar_write_eflash_do_write(
					nIndex, &szFirstData[0]);
				if (rc < 0) {
					TS_LOG_ERR(
						"%s EMEM_INFO mstar_write_eflash_do_write error rc = %d\n",
						__func__, rc);
				}
			}
...
static u32 mstar_get_fw_crc_hw(EmemType_e eEmemType)
{
	...
	if (eEmemType == EMEM_MAIN) {
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Disable cpu read flash set reg 0x1608 error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_low_byte(0x1606, 0x20);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Disable cpu read flash set reg 0x1606 error rc = %d\n",
				__func__, rc);
		}
		// Set read flag
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s CRC reset set reg (0x1620, 0x0002)  error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_16bit(0x1620, 0x0000);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s CRC reset set reg (0x1620, 0x0000)error rc = %d\n",
				__func__, rc);
		}
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set CRC e-flash block start address => Main Block : 0x0000 ~ 0x7FFE error rc = %d\n",
				__func__, rc);
		}
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s Set CRC e-flash block start address => Main Block : 0x0000 ~ 0x7FFE end addr error rc = %d\n",
				__func__, rc);
		}
		// Trigger CRC check
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Disable cpu read flash set reg (0x1608, 0x20) error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_low_byte(0x1606, 0x20);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Disable cpu read flash set reg (0x1606, 0x20) error rc = %d\n",
				__func__, rc);
		}
		// Set read flag
		rc = mstar_set_reg_16bit(0x1610, 0x0001);
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Mode reset info block set reg (0x1606, 0x0800) error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_low_byte(0x1604, 0x01);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Mode reset info block set reg (0x1604, 0x01) error rc = %d\n",
				__func__, rc);
		}
		// CRC reset
		rc = mstar_set_reg_16bit(0x1620, 0x0002);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO CRC reset (0x1620, 0x0002) error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_16bit(0x1620, 0x0000);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO CRC reset (0x1620, 0x0000) error rc = %d\n",
				__func__, rc);
		}
		...
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Set CRC e-flash block start address => Info Block : 0x0020 ~ 0x01FE error rc = %d\n",
				__func__, rc);
		}
		rc = mstar_set_reg_16bit(0x1622, 0x01FE);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Set CRC e-flash block start address => Info Block : 0x0020 ~ 0x01FE end addr error rc = %d\n",
				__func__, rc);
		}
		// Trigger CRC check
		rc = mstar_set_reg_16bit(0x1620, 0x0001);
		if (rc < 0) {
			TS_LOG_ERR(
				"%s EMEM_INFO Trigger CRC check error rc = %d\n",
				__func__, rc);
		}
		nCrcDown = mstar_get_reg_16bit(0x1620);
...
static u32 mstar_get_fw_crc_eflash(EmemType_e eEmemType)
{
	...
	if (eEmemType ==
	    EMEM_MAIN) // Read main block CRC(128KB-4) from main block
	{
...
static s32 mstar_check_fw_bin_ingerity(u8 **szFwData)
{
	...
	mstar_convert_fw_two_dimen_to_one(
		szFwData, tskit_mstar_data->fw_update_data.one_dimen_fw_data);

	/* Calculate main block CRC & info block CRC by device driver itself */
	nCrcMain = mstar_calculate_crc(
		tskit_mstar_data->fw_update_data.one_dimen_fw_data, 0,
		MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024 -
			MSG28XX_EMEM_SIZE_BYTES_ONE_WORD);
	nCrcInfo = mstar_calculate_crc(
...
static u32 mstar_28xx_fw_update(u8 **szFwData, EmemType_e eEmemType)
{
	...
	tskit_mstar_data->fw_updating =
		TRUE; // Set flag to 0x01 for indicating update firmware is processing
	...
	tskit_mstar_data->fw_update_data.fw_data_cont =
		0; // Reset g_fw_data_cont to 0 after update firmware

	mstar_dev_hw_reset();
	mdelay(300);

	tskit_mstar_data->fw_updating =
		FALSE; // Set flag to 0x00 for indicating update firmware is finished
...
s32 mstar_update_fw_cash(u8 **szFwData, EmemType_e eEmemType)
{
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		...
		TS_LOG_INFO("FORCE_TO_UPDATE_FIRMWARE_ENABLED = %d\n",
			    tskit_mstar_data->apk_info
				    ->force_update_firmware_enable);
		...
		if (tskit_mstar_data->apk_info->force_update_firmware_enable) {
			...
			{
				return mstar_28xx_fw_update(szFwData,
							    EMEM_MAIN);
			}
		}
	} else {
...
s32 mstar_fw_update_sdcard(const char *pFilePath, u8 mode)
{
	...
	for (i = 0; i < 130; i++) {
		tskit_mstar_data->fw_update_data.two_dimen_fw_data[i] =
			(u8 *)kmalloc(1024 * sizeof(u8), GFP_KERNEL);
	...
	if (mode == 1) {
		...
		// read firmware
		memset(tskit_mstar_data->fw_update_data.fw_data_buf, 0,
		       MSG28XX_FIRMWARE_WHOLE_SIZE * 1024);

		old_fs = get_fs();
		set_fs(KERNEL_DS);

		pos = 0;
		vfs_read(pfile, tskit_mstar_data->fw_update_data.fw_data_buf,
			 fsize, &pos);
		...
		if ((int)fw->size < MSG28XX_FIRMWARE_WHOLE_SIZE * 1024) {
			TS_LOG_ERR(
				"request firmware size is less than 130KB\n");
			goto out;
		}
		memcpy(tskit_mstar_data->fw_update_data.fw_data_buf, fw->data,
		       MSG28XX_FIRMWARE_WHOLE_SIZE * 1024);
		fsize = MSG28XX_FIRMWARE_WHOLE_SIZE * 1024;
	}

	mstar_fw_store_data(tskit_mstar_data->fw_update_data.fw_data_buf,
			    fsize);
	...
	if ((tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
	     tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) &&
	    fsize == 133120) {
		ret = mstar_update_fw_cash(
			tskit_mstar_data->fw_update_data.two_dimen_fw_data,
			EMEM_MAIN); // update main block only, do not update info block.
	} else {
		mstar_dev_hw_reset();
		TS_LOG_INFO(
			"The file size of the update firmware bin file is invalid, fsize = %d\n",
			fsize);
	}
...
u32 mstar_read_dq_mem_value(u16 nAddr)
{
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		...
		// Stop mcu
		rc = mstar_set_reg_low_byte(0x0FE6,
					    0x01); //bank:mheg5, addr:h0073
		if (rc < 0) {
...
static void mstar_fw_update_swid(void)
{
	...
	if (tskit_mstar_data->fw_only_depend_on_lcd) {
		sprintf(fw_name, "ts/%s_%s_%s_%s.bin",
			tskit_mstar_data->mstar_chip_data->ts_platform_data
				->product_name,
	...
	if (nCrcMainA != nCrcMainB) {
		for (i = 0; i < 5; i++) {
			...
			nCrcMainB = mstar_get_fw_crc_eflash(EMEM_MAIN);
			TS_LOG_INFO(
				"  Retry[%d] : main_hw_crc=0x%x, main_eflash_crc=0x%x  \n",
	...
	if (tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG28XXA) {
		if ((fw_sw_id != EELY_HLT_SWID) &&
		    (fw_sw_id != LENS_AUO_SWID) &&
		    (fw_sw_id != LENS_EBBG_SWID) &&
		    (fw_sw_id != EELY_AUO_SWID) &&
		    (fw_sw_id != EELY_EBBG_SWID)) {
			TS_LOG_ERR(
				"%s sw_id cannot match success, firmware file swid = 0x%x\n",
				__func__, fw_sw_id);
			goto out;
		}
	}
	if (ic_sw_id != fw_sw_id) {
		TS_LOG_INFO(
			"The MAIN_MEM swid is not equal updated fw swid, go to normal boot up process\n");
	}

	if ((nCrcMainA == nCrcMainB) && (ic_sw_id == fw_sw_id)) {
		TS_LOG_INFO("sw id is match\n");
		tskit_mstar_data->fw_update_data.swid_data.pUpdateBin =
			(u8 **)kmalloc(130 * sizeof(u8 *), GFP_KERNEL);
		if (ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data.swid_data
					  .pUpdateBin)) {
			TS_LOG_ERR("Failed to allocate FW buffer\n");
			goto out;
		}
		...
		for (i = 0; i < 130; i++) {
			tskit_mstar_data->fw_update_data.swid_data
				.pUpdateBin[i] =
				(u8 *)kmalloc(1024 * sizeof(u8), GFP_KERNEL);

			if (ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data
						  .swid_data.pUpdateBin[i])) {
				TS_LOG_ERR("Failed to allocate FW buffer\n");
				goto out;
			}
		}
		...
		nUpdateBinMajor = tskit_mstar_data->fw_update_data.swid_data
						  .pUpdateBin[127][1013]
					  << 8 |
				  tskit_mstar_data->fw_update_data.swid_data
					  .pUpdateBin[127][1012];
		nUpdateBinMinor = tskit_mstar_data->fw_update_data.swid_data
						  .pUpdateBin[127][1015]
					  << 8 |
				  tskit_mstar_data->fw_update_data.swid_data
					  .pUpdateBin[127][1014];
...
#endif
		TS_LOG_INFO(
			"The updated firmware file major=0x%x, minor=0x%x\n",
			nUpdateBinMajor, nUpdateBinMinor);
		mstar_get_customer_fw_ver_dbbus(EMEM_MAIN, &nMajor, &nMinor,
						&pVersion);
		...
		if ((nUpdateBinMinor & 0xFF) > (nMinor & 0xFF)) {
			if (tskit_mstar_data->fw_data.ver_flag) {
				TS_LOG_DEBUG(
					...
					ic_sw_id, nMajor, nMinor & 0xFF,
					(nMinor & 0xFF00) >> 8, nUpdateBinMajor,
					nUpdateBinMinor & 0xFF,
					(nUpdateBinMinor & 0xFF00) >> 8);
			} else {
				TS_LOG_DEBUG(
					"SwId=0x%x, nMajor=%d, nMinor=%d, nUpdateBinMajor=%u, nUpdateBinMinor=%u\n",
					ic_sw_id, nMajor, nMinor,
					nUpdateBinMajor, nUpdateBinMinor);
			}
			...
			for (i = 0; i < 130; i++) {
				tskit_mstar_data->fw_update_data
					.two_dimen_fw_data[i] = (u8 *)kmalloc(
					1024 * sizeof(u8), GFP_KERNEL);
			...
			if ((ic_sw_id != 0x0000) && (ic_sw_id != 0xFFFF)) {
				tskit_mstar_data->fw_update_data.fw_data_cont =
					0; // Reset g_fw_data_cont to 0 after copying update firmware data to temp buffer
				tskit_mstar_data->fw_update_data
					.update_retry_cont =
				...
				TS_LOG_INFO(
					"Start to upgrade new firmware process\n");
				mstar_update_fw_swid_do_work(NULL);

			} else {
				TS_LOG_INFO(
					"The MAIN_MEM swid is invalid, go to normal boot up process\n");
			}
		} else {
			TS_LOG_INFO(
				"update version is older than or equal to the current version, go to normal boot up process\n");
		}
	} else {
		TS_LOG_INFO(
			"IC firmware is lost, force to upgrade app firmware\n");
		tskit_mstar_data->fw_update_data.swid_data.pUpdateBin =
			(u8 **)kmalloc(130 * sizeof(u8 *), GFP_KERNEL);
		if (ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data.swid_data
					  .pUpdateBin)) {
		...
		for (i = 0; i < MSG28XX_FIRMWARE_WHOLE_SIZE; i++) {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY
			mstar_fw_store_data(tskit_mstar_data->fw_update_data
						    .swid_data.pUpdateBin[i],
					    1024);
#else
			mstar_fw_store_data((tskit_mstar_data->fw_update_data
...
out:
	...
	if (!ERR_ALLOC_MEM(
		...
		for (i = 0; i < 130; i++) {
			if (!ERR_ALLOC_MEM(tskit_mstar_data->fw_update_data
						   .swid_data.pUpdateBin[i])) {
				kfree(tskit_mstar_data->fw_update_data.swid_data
					      .pUpdateBin[i]);
				tskit_mstar_data->fw_update_data.swid_data
					.pUpdateBin[i] = NULL;
			}
	...
	if (nRetVal == 0) {
		...
		if (tskit_mstar_data->fw_update_data.update_retry_cont > 0) {
			...
			if (!dsm_client_ocuppy(ts_dclient)) {
				dsm_client_record(
					ts_dclient,
					"Mstar Update firmware by sw id failed\n");
