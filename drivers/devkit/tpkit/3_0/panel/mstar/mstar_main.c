...
static void mstar_gesture_convert_coordinate(u8 *pRawData, u32 *pTranX,
					     u32 *pTranY);
static void mstar_easy_wakeup_gesture_report_coordinate(u32 *buf, u32 count);

static s32 mstar_mutual_parse_packet(u8 *pPacket, u16 nLength,
				     MutualTouchInfo_t *pInfo,
...
int mstar_get_reg_xbit(u16 nAddr, u8 *pRxData, u16 nLength,
		       u16 nMaxI2cLengthLimit)
{
	...
	while (nLeft > 0) {
		if (nLeft >= nMaxI2cLengthLimit) {
			...
			rc = mstar_iic_read_data(SLAVE_I2C_ID_DBBUS,
			...
			nReadAddr =
				nReadAddr + nReadSize; //set next read address
...
int mstar_reg_get_xbit_write_4byte_value(u16 nAddr, u8 *pRxData, u16 nLength,
					 u16 nMaxI2cLengthLimit)
{
	int rc = 0;
	u16 nReadAddr = nAddr;
	...
	while (nLeft > 0) {
		if (nLeft >= nMaxI2cLengthLimit) {
			...
			TS_LOG_DEBUG(
				" RegGetXBitValue# Length >= I2cMax   nReadAddr=%x, nReadSize=%d n",
				nReadAddr, nReadSize);
			...
			nOffset = nOffset + nReadSize;
			TS_LOG_DEBUG(
				" RegGetXBitValue# Length >= I2cMax   nLeft=%d, nOffset=%d \n",
				nLeft, nOffset);
		} else {
			nReadSize = nLeft;
...
{
	s32 rc = NO_ERR;
	u8 szTxData[5] = {0x10, (nAddr >> 8) & 0xFF, nAddr & 0xFF, nData & 0xFF,
			  nData >> 8};
...
s32 mstar_set_reg_16bit_retry(u16 nAddr, u16 nData)
{
	...
	while (nRetryCount < count) {
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"mstar_set_reg_16bit(0x%x, 0x%x) success, rc = %d\n",
				nAddr, nData, rc);
			break;
		}
		nRetryCount++;
...
int mstar_set_reg_16bit_by_addr(u16 nAddr, u16 nData,
...
int mstar_reg_mask_16bit(u16 nAddr, u16 nMask, u16 nData,
			 AddressMode_e eAddressMode)
{
	u16 nTmpData = 0;
...
{
	// i2c response ack
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
	    tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) {
		return mstar_reg_mask_16bit(0x1E4F, BIT15, BIT15,
					    ADDRESS_MODE_16BIT);
	}
	return NO_ERR;
}
...
s32 mstar_iic_write_data(u8 nSlaveId, u8 *pBuf, u16 nSize)
{
	...
	if (tskit_mstar_data->mstar_chip_data->ts_platform_data->client !=
	    NULL) {
		if ((tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		     tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) &&
		    nSlaveId == SLAVE_I2C_ID_DWI2C &&
		    tskit_mstar_data->fw_updating) {
			TS_LOG_ERR(
		...
		tskit_mstar_data->mstar_chip_data->ts_platform_data->client
			->addr = nSlaveId;
		rc = tpkit_i2c_write(pBuf, nSize);

		// no return error if command is for serialDebug mode
		if (nSize == 5) {
			if (pBuf[0] == 0x53 && pBuf[1] == 0x45 &&
			    pBuf[2] == 0x52 && pBuf[3] == 0x44 &&
			    pBuf[4] == 0x42) {
				rc = nSize;
				goto out;
			}
...
s32 mstar_iic_read_data(u8 nSlaveId, u8 *pBuf, u16 nSize)
{
	...
	if (tskit_mstar_data->mstar_chip_data->ts_platform_data->client !=
		...
		if ((tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		    ...
		    tskit_mstar_data->fw_updating) {
			TS_LOG_ERR(
				"%s: Not allow to execute SmBus command while update firmware.\n",
				__func__);
			rc = -1;
			goto out;
		} else {
			tskit_mstar_data->mstar_chip_data->ts_platform_data
				->client->addr = nSlaveId;
			rc = tpkit_i2c_read(pBuf, nSize);
			if (rc < 0) {
				TS_LOG_ERR(
					"%s: error (%d), slave = %d, size = %d\n",
					__func__, rc, nSlaveId, nSize);
				goto out;
			}
		}
...
s32 mstar_ii2c_segment_read_dbbus(u8 nRegBank, u8 nRegAddr, u8 *pBuf, u16 nSize,
				  u16 nMaxI2cLengthLimit)
{
	s32 rc = NO_ERR;
	u16 nLeft = nSize;
	...
	if (tskit_mstar_data->mstar_chip_data->ts_platform_data->client !=
		...
		u8 *pReadBuf = NULL;
		u16 nLength = 0;
		u8 nAddrBefore = tskit_mstar_data->mstar_chip_data
		...
		while (nLeft > 0) {
			...
			if (nLeft > nSegmentLength) {
				if ((nRegAddr + nSegmentLength) <
					...
					nNextRegBank =
						nRegBank +
						1; // shift to read data from next register bank
			...
			rc = mstar_iic_write_data(
				tskit_mstar_data->mstar_chip_data
					->ts_platform_data->client->addr,
				&szWriteBuf[0], 3);
			if (rc < 0) {
				TS_LOG_ERR(
					"mstar_ii2c_segment_read_dbbus() -> i2c_master_send() error %d\n",
					rc);

				return rc;
			}

			rc = mstar_iic_read_data(
				tskit_mstar_data->mstar_chip_data
					->ts_platform_data->client->addr,
				pReadBuf, nLength);
			if (rc < 0) {
				TS_LOG_ERR(
					"mstar_ii2c_segment_read_dbbus() -> i2c_master_recv() error %d\n",
					rc);
...
s32 mstar_ii2c_segment_read_smbus(u16 nAddr, u8 *pBuf, u16 nSize,
				  u16 nMaxI2cLengthLimit)
{
	s32 rc = NO_ERR;
	u16 nLeft = nSize;
	...
	if (tskit_mstar_data->mstar_chip_data->ts_platform_data->client !=
		...
		while (nLeft > 0) {
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"mstar_ii2c_segment_read_smbus() -> i2c_master_send() error %d\n",
					rc);
				return rc;
			}
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"mstar_ii2c_segment_read_smbus() -> i2c_master_recv() error %d\n",
					rc);
				return rc;
			} else {
				nReadSize = nReadSize + nLength;
...
int mstar_hw_reset(void)
{
	...
	int high = 1, low = 0;
	int reset_gpio =
		tskit_mstar_data->mstar_chip_data->ts_platform_data->reset_gpio;
...
int mstar_optimize_current_consump(void)
{
	...
	rc = mstar_iic_write_data(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0],
...
int mstar_enter_sleep_mode(void)
{
	...
	if (reset_gpio >= 0 && reset_gpio != 1) {
		ret = pinctrl_select_state(
			tskit_mstar_data->pctrl,
			tskit_mstar_data->pinctrl_state_reset_low);
		if (ret) {
			TS_LOG_ERR(
				"%s:gpio direction output to 0 fail, ret=%d\n",
				__func__, ret);
			return ret;
		}
	}
...
void mstar_finger_touch_report_enable(void)
{
	...
	if (!tskit_mstar_data->int_flag) {
		enable_irq(tskit_mstar_data->mstar_chip_data->ts_platform_data
				   ->irq_id);
...
void mstar_esd_check(void)
{
	...
	{
		TS_LOG_DEBUG(
			"%s: Not allow to do ESD check while finger touch\n",
			__func__);
		goto out;
	}

	if (tskit_mstar_data
		    ->fw_updating) // Check whether update frimware is finished
	{
		TS_LOG_DEBUG(
			"%s: Not allow to do ESD check while update firmware is proceeding\n",
			__func__);
		goto out;
	}
	...
	{
		TS_LOG_DEBUG(
			"%s: Not allow to do ESD check while mp test is proceeding.\n",
			__func__);
		goto out;
	}
	...
	if (!tskit_mstar_data->esd_check) {
		TS_LOG_DEBUG(
			"%s: Not allow to do ESD check while screen is being touched\n",
			__func__);
		goto out;
	}

	if (tskit_mstar_data->apk_info->firmware_log_enable) {
		if (tskit_mstar_data->fw_mode ==
		    MSG28XX_FIRMWARE_MODE_DEBUG_MODE) {
			TS_LOG_DEBUG(
				"%s: Not allow to do ESD check while firmware mode is DEBUG MODE.\n",
				__func__);
			goto out;
		}
	}
	...
	while (i < count) {
		...
		if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG22XX ||
		    tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		    tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) {
			rc = mstar_iic_write_data(SLAVE_I2C_ID_DWI2C,
						  &szData[0], 1);
			if (rc < 0) {
...
int mstar_get_customer_fw_ver(char *ppVersion)
{
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		...
		TS_LOG_DEBUG(
			"%s: szRxData[0] = 0x%x, szRxData[1] = 0x%x, szRxData[2] = 0x%x, szRxData[3] = 0x%x\n",
			__func__, szRxData[0], szRxData[1], szRxData[2],
			szRxData[3]);
...
u16 mstar_change_fw_mode(u16 nMode)
{
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG22XX ||
		...
		tskit_mstar_data->finger_touch_disable =
			TRUE; // Skip finger touch ISR handling temporarily for device driver can send change firmware mode i2c command to firmware.
		...
		while (i < count) {
			...
			if (rc >= 0) {
				TS_LOG_INFO(
					"%s: Change firmware mode success\n",
					__func__);
				break;
			}
		...
		mutex_unlock(&tskit_mstar_data->mutex_common);
		tskit_mstar_data->finger_touch_disable =
			FALSE; // Resume finger touch ISR handling after device driver have sent i2c command to firmware.
	}
...
int mstar_mutual_get_fw_info(MutualFirmwareInfo_t *pInfo)
{
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		...
		szTxData[0] = 0x01;
		tskit_mstar_data->finger_touch_disable =
			TRUE; // Skip finger touch ISR handling temporarily for device driver can send get firmware info i2c command to firmware.
		...
		while (i < count) {
			...
			if (rc >= 0) {
				TS_LOG_INFO(
					"%s: Get firmware info mstar_iic_write_data() success\n",
					__func__);
			}

			mdelay(I2C_WRITE_COMMAND_DELAY_FOR_FIRMWARE);
			rc = mstar_iic_read_data(SLAVE_I2C_ID_DWI2C,
						 &szRxData[0], 10);
			if (rc >= 0) {
				TS_LOG_INFO(
					"%s: Get firmware info mstar_iic_read_data() success\n",
					__func__);
				TS_LOG_INFO("%s: FW mode: 0x%x\n", __func__,
					    szRxData[1]);
				if (szRxData[1] ==
					    MSG28XX_FIRMWARE_MODE_DEMO_MODE ||
				    szRxData[1] ==
					    MSG28XX_FIRMWARE_MODE_DEBUG_MODE) {
					break;
				} else {
		...
		if ((szRxData[1] == MSG28XX_FIRMWARE_MODE_DEBUG_MODE &&
		     szRxData[2] == 0xA7 &&
		     (szRxData[5] == PACKET_TYPE_TOOTH_PATTERN ||
		      szRxData[5] == PACKET_TYPE_CSUB_PATTERN ||
		      szRxData[5] == PACKET_TYPE_FOUT_PATTERN ||
		      szRxData[5] == PACKET_TYPE_FREQ_PATTERN)) ||
		    (szRxData[1] == MSG28XX_FIRMWARE_MODE_DEMO_MODE &&
		     szRxData[2] == 0x5A)) {
			...
			pInfo->nLogModePacketLength =
				MUTUAL_DEMO_MODE_PACKET_LENGTH;
			pInfo->nType = 0;
			pInfo->nMy = 0;
			pInfo->nMx = 0;
			pInfo->nSd = 0;
...
void mstar_restore_fw_to_log_data(void)
{
	...
	TS_LOG_INFO("%s: g_switch_mode_apk = %d\n", __func__,
		    tskit_mstar_data->apk_info->switch_mode);
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		...
		if (tskit_mstar_data->apk_info->switch_mode == 1) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"%s mutual_get_fw_info error ret = %d\n",
					__func__, ret);
			}
			TS_LOG_INFO(
				"%s: tskit_mstar_data->fw_mode = 0x%x, tInfo.nFirmwareMode = 0x%x\n",
			...
			if (tskit_mstar_data->fw_mode ==
			    ...
			    MSG28XX_FIRMWARE_MODE_DEBUG_MODE !=
				    tInfo.nFirmwareMode) {
				tskit_mstar_data->fw_mode = mstar_change_fw_mode(
					MSG28XX_FIRMWARE_MODE_DEBUG_MODE);
			} else {
				TS_LOG_INFO(
					"%s: firmware mode is not restored\n",
					__func__);
			}
		}
	}
...
int mstar_get_touch_packet_addr(u16 *pDataAddress, u16 *pFlagAddress)
{
	...
	if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
	    tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA ||
	    (tskit_mstar_data->chip_type == CHIP_TYPE_MSG22XX &&
	     tskit_mstar_data->apk_info->self_freq_scan_enable)) {
		szTxData[0] = 0x05;
		mutex_lock(&tskit_mstar_data->mutex_common);
		...
		while (i < count) {
			...
			if (rc >= 0) {
				TS_LOG_INFO(
					"%s: Get touch packet address mstar_iic_write_data() success\n",
					__func__);
			}
			...
			if (rc >= 0) {
				TS_LOG_INFO(
					"%s: Get touch packet address mstar_iic_read_data() success\n",
					__func__);
				break;
			}
		...
		if (i == count) {
			TS_LOG_ERR(
				"%s: Get touch packet address failed, rc = %d\n",
				__func__, rc);
		}
...
static int mstar_finger_touch_packet_flag_bit1(void)
{
	...
	nRetVal = mstar_iic_read_data(SLAVE_I2C_ID_DWI2C,
				      &tskit_mstar_data->fw_data.packet_flag[0],
				      2);
	if (nRetVal < 0) {
		TS_LOG_ERR("%s read data error\n", __func__);
		return nRetVal;
...
static s32 mstar_mutual_read_ftd(u8 *pPacket, u16 nReportPacketLength)
{
	...
	if (tskit_mstar_data->apk_info->firmware_log_enable) {
		if (tskit_mstar_data->fw_mode ==
		...
		} else if (tskit_mstar_data->fw_mode ==
			   MSG28XX_FIRMWARE_MODE_DEBUG_MODE) {
#ifdef CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA
			TS_LOG_DEBUG(
				"%s: packet_data_addr = 0x%2X  \n", __func__,
				tskit_mstar_data->fw_data.packet_data_addr);
			TS_LOG_DEBUG(
				"%s: packet_flag_addr = 0x%2X  \n", __func__,
				tskit_mstar_data->fw_data.packet_flag_addr);

			if (tskit_mstar_data->fw_data.support_seg == FALSE) {
				TS_LOG_ERR(
					"%s: packet_data_addr & packet_flag_addr is un-initialized\n",
					__func__);
				return -1;
			}
			...
			if (tskit_mstar_data->finger_touch_disable) {
				TS_LOG_ERR(
					"%s: Skip finger touch for handling get firmware info or change firmware mode\n",
					__func__);
				return -1;
			}
			...
			if (0 != mstar_finger_touch_packet_flag_bit1()) {
				TS_LOG_ERR(
					"%s: Bit1 is not 0. FW is not ready for providing debug mode packet to Device Driver\n",
					__func__);
				return -1;
			}

			rc = mstar_ii2c_segment_read_smbus(
				tskit_mstar_data->fw_data.packet_data_addr,
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"%s: I2C read debug mode packet data failed, rc = %d\n",
					__func__, rc);
				return -1;
			}
#else
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"%s: I2C read packet data failed, rc = %d\n",
					__func__, rc);
				return -1;
			}
#endif
		...
		rc = mstar_iic_read_data(SLAVE_I2C_ID_DWI2C, &pPacket[0],
					 nReportPacketLength);
		if (rc < 0) {
...
static int i2cuart_recv_packet(u8 *pPacket, u16 nReportPacketLength)
{
	...
	if (need_read_len > actual_len) {
		tskit_mstar_data->i2cuart_data =
			kzalloc(tskit_mstar_data->i2cuart_len, GFP_ATOMIC);
		...
		memcpy(tskit_mstar_data->i2cuart_data, pPacket,
		       nReportPacketLength);
		res = mstar_iic_read_data(SLAVE_I2C_ID_DWI2C,
					  tskit_mstar_data->i2cuart_data +
						  nReportPacketLength,
					  need_read_len - actual_len);
		if (res < 0) {
...
static void mstar_read_touch_data(struct ts_fingers *info)
{
...
#endif
	struct ts_easy_wakeup_info *gestrue_info =
		&tskit_mstar_data->mstar_chip_data->easy_wakeup_info;
	if (tskit_mstar_data->finger_touch_disable) {
		TS_LOG_ERR(
			"%s: Skip finger touch for handling get firmware info or change firmware mode\n",
			__func__);
	...
	adapter = i2c_get_adapter(tskit_mstar_data->mstar_chip_data
					  ->ts_platform_data->bops->bus_id);
	if (!adapter) {
	...
	if ((true ==
	     tskit_mstar_data->mstar_chip_data->ts_platform_data->feature_info
		     .wakeup_gesture_enable_info.switch_value) &&
	    (true == tskit_mstar_data->mstar_chip_data->easy_wakeup_info
			     .easy_wakeup_flag)) {
		do {
			if (ctrl->pwr_state == I2C_MSM_PM_SYS_SUSPENDED) {
				TS_LOG_INFO(
					"%s gesture mode, waiting for i2c bus resume\n",
					__func__);
				++try;
				...
				TS_LOG_INFO(
					"%s i2c bus resuming or resumed,wait system resume after %d ms,try %d times\n",
		...
		if (ctrl->pwr_state == I2C_MSM_PM_SYS_SUSPENDED) {
			delay_time = try * I2C_WAIT_TIME;
			TS_LOG_INFO(
				"%s trigger gesture irq in system suspending,i2c bus can't resume, so ignore irq;wait %d ms \n",
				__func__, delay_time);
			return;
		}
	}
	...
	if (tskit_mstar_data->apk_info->firmware_log_enable) {
		if (tskit_mstar_data->fw_mode ==
			...
			if (tskit_mstar_data->mutual_fw_info
				    .nLogModePacketHeader != 0xA7) {
				TS_LOG_ERR(
					"%s: WRONG DEBUG MODE HEADER : 0x%x\n",
					__func__,
					tskit_mstar_data->mutual_fw_info
						.nLogModePacketHeader);
				goto TouchHandleEnd;
			}

			nReportPacketLength = tskit_mstar_data->mutual_fw_info
						      .nLogModePacketLength;
	...
	if (tskit_mstar_data->gesture_data.wakeup_flag) {
		...
		if (!tskit_mstar_data->ges_self_debug) {
			...
			TS_LOG_DEBUG(
				"%s: Set length and packet to debug in gesture mode\n",
				__func__);
	...
	if (pPacket[0] == 0x7A) {
		TS_LOG_DEBUG(
			"%s: I2CUART(0x%x): prepare to receive rest of data\n",
			__func__, pPacket[0]);
	...
	if (tskit_mstar_data->apk_info->debug_node_open) {
		memset(tskit_mstar_data->apk_info->fw_debug_buf
			       [tskit_mstar_data->apk_info->debug_data_frame],
		       0x00, (uint8_t)sizeof(uint8_t) * 2048);
		if (!ERR_ALLOC_MEM(tskit_mstar_data->i2cuart_data)) {
			memcpy(tskit_mstar_data->apk_info
				       ->fw_debug_buf[tskit_mstar_data->apk_info
							      ->debug_data_frame],
			       tskit_mstar_data->i2cuart_data,
			       tskit_mstar_data->i2cuart_len);
	...
	if (0 == mstar_mutual_parse_packet(pPacket, nReportPacketLength, &tInfo,
					   info)) {
...
TouchHandleEnd:
	...
	if (gesture_recovery) {
		TS_LOG_INFO(
			"%s:Gesture Header is fail, recovery gesture mode form demo mode \n",
			__func__);
		mdelay(20);
		gestrue_info->easy_wakeup_flag = false;
		tskit_mstar_data->reset_touch_flag = true;
		mstar_enable_gesture_wakeup(
...
s32 mstar_chip_detect_init(void)
{
	...
	if (tskit_mstar_data->chip_type != 0) {
		if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		    tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) {
			memset(&tskit_mstar_data->mutual_fw_info, 0x0,
			       sizeof(MutualFirmwareInfo_t));
		}
		if (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XX ||
		    tskit_mstar_data->chip_type == CHIP_TYPE_MSG58XXA) {
			tskit_mstar_data->fw_mode =
...
static int mstar_get_roi_data_form_fw(void)
{
	...
	ret = mstar_iic_read_data(SLAVE_I2C_ID_DWI2C,
				  tskit_mstar_data->roi_data,
...
static int mstar_get_gesture_code(u8 *pPacket, struct ts_fingers *info)
{
	...
	TS_LOG_DEBUG(
		"%s: p[0]=%x, p[1]=%x, p[2]=%x, p[3]=%x, p[4]=%x, p[5]=%x \n",
		__func__, pPacket[0], pPacket[1], pPacket[2], pPacket[3],
		pPacket[4], pPacket[5]);
	...
	if (pPacket[0] == 0xA7 && pPacket[1] == 0x00 && pPacket[2] == 0x06 &&
		...
		bIsCorrectFormat = 1;
	} else if (pPacket[0] == 0xA7 && pPacket[1] == 0x00 &&
		   pPacket[2] == 0x80 &&
		...
		{
			tskit_mstar_data->gesture_data.log_info[nTmpCount] =
				pPacket[a];
			nTmpCount++;
		}
		...
		for (a = 6; a < 126; a = a + 3) //parse packet to coordinate
		{
			...
			mstar_gesture_convert_coordinate(&pPacket[a], &nTranX,
							 &nTranY);
			tskit_mstar_data->gesture_data.log_info[nTmpCount] =
				nTranX;
		...
		nWidth = (((pPacket[12] & 0xF0) << 4) |
			  pPacket[13]); //parse width & height
		nHeight = (((pPacket[12] & 0x0F) << 8) | pPacket[14]);

		TS_LOG_DEBUG("%s: Before convert [width,height]=[%d,%d]\n",
			     __func__, nWidth, nHeight);

		if ((pPacket[12] == 0xFF) && (pPacket[13] == 0xFF) &&
		    (pPacket[14] == 0xFF)) {
			nWidth = 0;
			nHeight = 0;
		} else {
			nWidth = (nWidth *
				  tskit_mstar_data->mstar_chip_data->x_max) /
				 TPD_WIDTH;
			nHeight = (nHeight *
				   tskit_mstar_data->mstar_chip_data->y_max) /
				  TPD_HEIGHT;
			TS_LOG_DEBUG(
				"%s: After convert [width,height]=[%d,%d]\n",
				__func__, nWidth, nHeight);
		}
	...
	if (bIsCorrectFormat) {
		...
		case KEY_CODE_M:
			TS_LOG_DEBUG(
				"%s: GESTURE_WAKEUP_MODE_M_CHARACTER_FLAG.\n",
				__func__);
		...
		case KEY_CODE_W:
			TS_LOG_DEBUG(
				"%s: GESTURE_WAKEUP_MODE_W_CHARACTER_FLAG.\n",
				__func__);
		...
		case KEY_CODE_C:
			TS_LOG_DEBUG(
				"%s: GESTURE_WAKEUP_MODE_C_CHARACTER_FLAG.\n",
				__func__);
		...
		case KEY_CODE_E:
			TS_LOG_DEBUG(
				"%s: GESTURE_WAKEUP_MODE_E_CHARACTER_FLAG.\n",
				__func__);
		...
		default:
			TS_LOG_ERR(
				"%s: Un-supported gesture wakeup mode. Please check your device driver code.\n",
				__func__);
			break;
		}
	} else {
		TS_LOG_ERR(
			"%s: gesture wakeup packet format is incorrect. pPacket[0]=%x\n",
			__func__, pPacket[0]);
		ret = -1;
		goto out;
	}
	...
	if (nReportGestureType != 0) {
		__pm_wakeup_event(&tskit_mstar_data->mstar_chip_data
					   ->ts_platform_data->ts_wake_lock,
				  jiffies_to_msecs(5 * HZ));
		if (true == tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				    .off_motion_on) {
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.off_motion_on = false;
			mstar_easy_wakeup_gesture_report_coordinate(
				nTmpBuffer, nGestureCount);
				     ...
				     struct ts_fingers *info)
{
	...
	TS_LOG_DEBUG("%s: received raw data from touch panel as following:\n",
		     __func__);
	TS_LOG_DEBUG(
		"%s: pPacket[0]=%x pPacket[1]=%x pPacket[2]=%x pPacket[3]=%x pPacket[4]=%x pPacket[5]=%x pPacket[6]=%x pPacket[7]=%x pPacket[8]=%x\n",
		__func__, pPacket[0], pPacket[1], pPacket[2], pPacket[3],
		pPacket[4], pPacket[5], pPacket[6], pPacket[7], pPacket[8]);
		...
		if (pPacket[0] == 0x2C) {
			...
			if (tskit_mstar_data->apk_info->touch_kobj != NULL) {
				...
				for (i = 0; i < nLength; i++) {
					...
					strcat(szRspFwSpecificLogPacket,
					       szValue);
				}
				...
				nRetVal = kobject_uevent_env(
					tskit_mstar_data->apk_info->touch_kobj,
					KOBJ_CHANGE, pEnvp);
				TS_LOG_DEBUG(
					"%s: kobject_uevent_env() STATUS=GET_FW_LOG, nRetVal = %d\n",
					__func__, nRetVal);
			}
	...
	if ((tskit_mstar_data->ges_self_debug == false) &&
		...
		if (pPacket[nLength - 1] != nCheckSum) {
			...
			if (!dsm_client_ocuppy(ts_dclient)) {
				dsm_client_record(
					ts_dclient,
					"Mstar mutual_parse_packet WRONG CHECKSUM\n");
	...
	/* For gesture packet */
	if (tskit_mstar_data->mstar_chip_data->ts_platform_data->feature_info
			    .wakeup_gesture_enable_info.switch_value == true &&
	    !tskit_mstar_data->ges_self_debug) {
		if (tskit_mstar_data->gesture_data.wakeup_flag) {
	...
	if (tskit_mstar_data->apk_info->firmware_log_enable) {
		if (tskit_mstar_data->fw_mode ==
		    ...
		    pPacket[0] != 0x5A) {
			TS_LOG_ERR(
				"%s: WRONG DEMO MODE HEADER.firmware_log_enable. pPacket[0]=%x \n",
				__func__, pPacket[0]);
			return -1;
		} else if (tskit_mstar_data->fw_mode ==
				   MSG28XX_FIRMWARE_MODE_DEBUG_MODE &&
			   (pPacket[0] != 0xA7 &&
			    ...
			    pPacket[3] != PACKET_TYPE_CSUB_PATTERN &&
			    pPacket[3] != PACKET_TYPE_FOUT_PATTERN &&
			    pPacket[3] != PACKET_TYPE_FREQ_PATTERN)) {
		...
		if (pPacket[0] != 0x5A) {
			TS_LOG_ERR(
				"%s: WRONG DEMO MODE HEADER. pPacket[0]=%x \n",
				__func__, pPacket[0]);
			return -1;
		}
	}
	...
	if (pPacket[0] == 0x5A) {
		for (i = 0; i < MUTUAL_MAX_TOUCH_NUM; i++) {
			...
			pInfo->tPoint[pInfo->nCount].nX =
				nX * tskit_mstar_data->mstar_chip_data->x_max /
				TPD_WIDTH;
			pInfo->tPoint[pInfo->nCount].nY =
				nY * tskit_mstar_data->mstar_chip_data->y_max /
				TPD_HEIGHT;
		...
		if (tskit_mstar_data->apk_info->firmware_log_enable) {
			...
			if (tskit_mstar_data->apk_info->touch_kobj != NULL) {
				...
				nRetVal = kobject_uevent_env(
					...
					KOBJ_CHANGE, pEnvp);
				TS_LOG_DEBUG(
					"%s: kobject_uevent_env() STATUS=GET_DEMO_MODE_PACKET, nRetVal = %d\n",
					__func__, nRetVal);
			}
		}
		...
		if (tskit_mstar_data->apk_info->touch_kobj != NULL) {
			...
			memcpy(tskit_mstar_data->log_packet,
			       tskit_mstar_data->debug_packet,
			       nLength); // Copy g_debug_packet to g_log_packet for avoiding the debug mode data which is received by MTPTool APK may be modified.
			...
			nRetVal = kobject_uevent_env(
				...
				KOBJ_CHANGE, pEnvp);
			TS_LOG_DEBUG(
				"%s: kobject_uevent_env() STATUS=GET_DEBUG_MODE_PACKET, nRetVal = %d\n",
				__func__, nRetVal);
		}
	}
...
int mstar_enable_gesture_wakeup(u32 *pMode)
{
	...
	while (i < count) {
		...
		rc = mstar_iic_write_data(SLAVE_I2C_ID_DWI2C, &szTxData[0],
					  ARRAY_SIZE(szTxData));
		if (rc >= 0) {
			TS_LOG_INFO(
				"%s: Enable gesture wakeup index 0 success\n",
				__func__);
			break;
		}
	...
	while (i < count) {
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"%s: Enable gesture wakeup index 1 success\n",
				__func__);
			break;
		}
	...
	while (i < count) {
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"%s: Enable gesture wakeup index 2 success\n",
				__func__);
			break;
		}
	...
	while (i < count) {
		...
		if (rc >= 0) {
			TS_LOG_INFO(
				"%s: Enable gesture wakeup index 3 success\n",
				__func__);
			break;
		}
	...
	mutex_lock(&tskit_mstar_data->mutex_protect);
	tskit_mstar_data->gesture_data.wakeup_flag =
		TRUE; // gesture wakeup is enabled
	mutex_unlock(&tskit_mstar_data->mutex_protect);
#else
...
void mstar_disable_gesture_wakeup(void)
{
	mutex_lock(&tskit_mstar_data->mutex_protect);
	tskit_mstar_data->gesture_data.wakeup_flag =
		FALSE; // gesture wakeup is disabled
	mutex_unlock(&tskit_mstar_data->mutex_protect);
}
					     ...
					     u32 *pTranY)
{
	...
	if ((pRawData[0] == 0xFF) && (pRawData[1] == 0xFF) &&
		...
		*pTranY = 0; // final Y coordinate
		...
		/* one touch point */
		*pTranX = (nX * tskit_mstar_data->mstar_chip_data->x_max) /
			  TPD_WIDTH;
		*pTranY = (nY * tskit_mstar_data->mstar_chip_data->y_max) /
			  TPD_HEIGHT;
	}
}
...
static void mstar_easy_wakeup_gesture_report_coordinate(u32 *buf, u32 count)
{
	...
	if (count != 0) {
		if (count == 2) {
			...
			y = (s16)(buf[1]);
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.easywake_position[0] = x << 16 | y;

			/*2.end */
			x = (s16)buf[2];
			y = (s16)buf[3];
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.easywake_position[1] = x << 16 | y;

			/*3.top */
			x = (s16)buf[6];
			y = (s16)buf[7];
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.easywake_position[2] = x << 16 | y;

			/*4.leftmost */
			x = (s16)buf[8];
			y = (s16)buf[9];
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.easywake_position[3] = x << 16 | y;

			/*5.bottom */
			x = (s16)buf[10];
			y = (s16)buf[11];
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.easywake_position[4] = x << 16 | y;

			/*6.rightmost */
			x = (s16)buf[12];
			y = (s16)buf[13];
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.easywake_position[5] = x << 16 | y;
		}
	}
}
...
static struct ts_roi_info *
mstar_get_roi_info(struct ts_kit_device_data *dev_data)
{
	...
	if (tskit_mstar_data->roi_enabled) {
		...
		if (mstar_roi_data_staled) {
			if (!wait_for_completion_interruptible_timeout(
				    &mstar_roi_data_done,
				    msecs_to_jiffies(ROI_DATA_WAIT_TIMEOUT))) {
...
static int mstar_set_roi_switch(u8 roi_switch)
{
	...
	if (error) {
		TS_LOG_ERR("%s: into fingersense mode error roi_switch  = %d",
			   __func__, roi_switch);
	}
	tskit_mstar_data->roi_enabled = roi_switch;
	TS_LOG_DEBUG("%s: roi_switch value is %u\n", __func__,
		     tskit_mstar_data->roi_enabled);
	return error;
}
...
static int mstar_read_flash_RIU_28xx(u32 nAddr, int nBlockType, int nLength,
				     u8 *pFlashData)
{
	int ret = 0;
...
static int mstar_read_flash_28xx(u32 nAddr, int nBlockType, int nLength,
				 u8 *pFlashData)
{
	int ret = NO_ERR;
	...
	if ((addr_star >= EMEM_SIZE_MSG28XX) ||
	    (addr_end > EMEM_SIZE_MSG28XX)) {
	...
	for (addr_star = nAddr; addr_star < addr_end; addr_star += addr_step) {
		...
		TS_LOG_DEBUG("  %s() _28xx_addr:0x%x addr_star:0x%x \
			addr_end:%x nLength:%d pFlashData:%pK \n",
			     __func__, _28xx_addr, addr_star, addr_end, nLength,
			     pFlashData);
		mstar_read_flash_RIU_28xx(_28xx_addr, nBlockType, nLength,
					  (pFlashData + read_byte));
		TS_LOG_DEBUG(
			"  %s() pFlashData[%x]: %02x %02x %02x %02x read_byte:%d \n",
			__func__, addr_star, pFlashData[read_byte],
			pFlashData[read_byte + 1], pFlashData[read_byte + 2],
...
int mstar_mp_read_flash(u8 nChipType, u32 nAddr, int nBlockType, int nLength,
			u8 *pFlashData)
{
	int ret = 0;
	...
	case CHIP_TYPE_MSG28XXA:
		ret = mstar_read_flash_28xx(nAddr, nBlockType, nLength,
					    pFlashData);
		if (ret < 0) {
...
static void mstar_get_project_info(void)
{
	...
	ret = mstar_mp_read_flash(tskit_mstar_data->chip_type,
				  MSTAR_PROJECT_ID_ADDR, EMEM_TYPE_INFO_BLOCK,
				  12, (u8 *)tskit_mstar_data->project_id);
	if (ret < 0) {
	...
	if (tskit_mstar_data->support_get_tp_color) {
		ret = mstar_mp_read_flash(tskit_mstar_data->chip_type,
					  MSTAR_COLOR_ID_ADDR,
					  EMEM_TYPE_INFO_BLOCK, 4, temp_buf);
		if (ret < 0) {
	...
	ret = mstar_mp_read_flash(tskit_mstar_data->chip_type,
				  MSTAR_GOLDEN_VER_ADDR, EMEM_TYPE_INFO_BLOCK,
				  4, temp_buf);
	if (ret < 0) {
		TS_LOG_ERR("%s golden sample version error ret = %d\n",
			   __func__, ret);
...
static int mstar_before_suspend(void)
{
	...
	if (tskit_mstar_data->fw_updating) {
		TS_LOG_ERR(
			"%s: Not allow to power on/off touch ic while update firmware.\n",
			__func__);
		//goto out;
	}
	...
	case TS_GESTURE_MODE:
		if (tskit_mstar_data->mstar_chip_data->ts_platform_data
			    ->feature_info.wakeup_gesture_enable_info
			    .switch_value == true) {
			if (tskit_mstar_data->gesture_data.wakeup_mode[0] !=
				    0x00000000 ||
				...
				enable_irq_wake(
					tskit_mstar_data->mstar_chip_data
						->ts_platform_data->irq_id);
				mstar_enable_gesture_wakeup(
					&tskit_mstar_data->gesture_data
						 .wakeup_mode[0]);
				mdelay(20);
				TS_LOG_INFO(
					"%s: Finsih gesture command after 20ms\n",
					__func__);
				// disable incorrect read to avoid i2c error during gesture suspend
				enable_irq(tskit_mstar_data->mstar_chip_data
						   ->ts_platform_data->irq_id);
...
int mstar_suspend(void)
{
	...
	case TS_GESTURE_MODE:
		if (tskit_mstar_data->mstar_chip_data->ts_platform_data
			    ...
			    .switch_value == true) {
			TS_LOG_INFO(
				"%s: wakeup_gesture_enable_info.switch_value = true\n",
				__func__);
			if (tskit_mstar_data->gesture_data.wakeup_mode[0] !=
				    0x00000000 ||
			    tskit_mstar_data->gesture_data.wakeup_mode[1] !=
			...
			tskit_mstar_data->mstar_chip_data->easy_wakeup_info
				.off_motion_on = true;
			TS_LOG_INFO("%s: suspend end case TS_GESTURE_MODE",
				    __func__);
		} else {
			ret = mstar_enter_sleep_mode();
			if (ret) {
...
static int mstar_status_resume(void)
{
	...
	if (info->holster_info.holster_supported) {
		retval = mstar_set_holster_switch(
			info->holster_info.holster_switch);
		if (retval < 0) {
			TS_LOG_ERR(
				"%s: Failed to set holster switch(%d), err: %d\n",
	...
	if (info->glove_info.glove_supported) {
		...
		if (retval < 0) {
			TS_LOG_ERR(
				"%s: Failed to set glove switch(%d), err: %d\n",
	...
	TS_LOG_INFO(
		"%s: glove_switch (%d), holster switch(%d), roi_switch(%d) \n",
		__func__, info->glove_info.glove_switch,
...
static int
mstar_wakeup_gesture_enable_switch(struct ts_wakeup_gesture_enable_info *info)
{
	return NO_ERR;
}
...
static int mstar_get_lcd_panel_info(void)
{
	...
	strncpy(tskit_mstar_data->lcd_panel_info, lcd_type,
...
static int get_lcd_module_name(void)
{
	...
	int i = 0;
	strncpy(temp, tskit_mstar_data->lcd_panel_info,
		LCD_PANEL_INFO_MAX_LEN - 1);
	for (i = 0; i < (LCD_PANEL_INFO_MAX_LEN - 1) && (i < (MAX_STR_LEN - 1));
	     i++) {
		if (temp[i] == '_') {
			break;
		}
...
int mstar_fw_update_boot(char *file_name)
{
	...
	snprintf(tskit_mstar_data->mstar_chip_data->version_name,
		 MAX_STR_LEN - 1, "%s", fw_version);
...
int mstar_irq_bottom_half(struct ts_cmd_node *in_cmd,
...
static int mstar_regulator_get(void)
{
	if (tskit_mstar_data->self_ctrl_power == 1) {
		...
		if (1 ==
		    tskit_mstar_data->mstar_chip_data->vci_regulator_type) {
			tskit_mstar_data->vdda = regulator_get(
				&tskit_mstar_data->mstar_dev->dev, "mstar_vci");
		...
		if (1 ==
		    tskit_mstar_data->mstar_chip_data->vddio_regulator_type) {
			tskit_mstar_data->vddd =
				regulator_get(&tskit_mstar_data->mstar_dev->dev,
					      "mstar_vddio");
	...
	if (tskit_mstar_data->self_ctrl_power == 1) {
		...
		if (1 ==
			...
			rc = regulator_set_voltage(
				tskit_mstar_data->vdda,
				tskit_mstar_data->mstar_chip_data->regulator_ctr
					.vci_value,
				tskit_mstar_data->mstar_chip_data->regulator_ctr
					.vci_value);
			if (rc < 0) {
				TS_LOG_ERR(
					"%s, failed to set parade vdda to %dV, rc = %d\n",
					__func__, rc,
					tskit_mstar_data->mstar_chip_data
						->regulator_ctr.vci_value);
				return rc;
			}
		}
		...
		if (1 ==
			...
			if (rc < 0) {
				TS_LOG_ERR(
					"%s, failed to set parade vddd to %dV, rc = %d\n",
					__func__, rc,
					tskit_mstar_data->mstar_chip_data
						->regulator_ctr.vddio_value);
				return rc;
			}
		}
	} else {
...
static int mstar_gpio_request(void)
{
	...
	if (tskit_mstar_data->self_ctrl_power == 1) {
		...
		if (0 ==
		    tskit_mstar_data->mstar_chip_data->vddio_regulator_type) {
			retval = gpio_request(tskit_mstar_data->mstar_chip_data
						      ->vddio_gpio_ctrl,
					      "parade_vddio_gpio_ctrl");
			if (retval) {
				TS_LOG_ERR(
...
static void mstar_gpio_free(void)
{
	if (tskit_mstar_data->self_ctrl_power == 1) {
		...
		if (0 ==
			...
			if (0 <=
			    tskit_mstar_data->mstar_chip_data->vci_gpio_ctrl)
				gpio_free(tskit_mstar_data->mstar_chip_data
						  ->vci_gpio_ctrl);
		}
		if (0 ==
		    tskit_mstar_data->mstar_chip_data->vddio_regulator_type) {
	...
	if (tskit_mstar_data->self_ctrl_power == 1) {
		...
		if (1 ==
			...
			if (!IS_ERR(tskit_mstar_data->vdda)) {
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s, failed to enable parade vdda, rc = %d\n",
						__func__, rc);
					return -1;
				}
			}
		} else if (0 == tskit_mstar_data->mstar_chip_data
					->vci_regulator_type) {
		...
		if (1 ==
			...
			if (!IS_ERR(tskit_mstar_data->vddd)) {
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s, failed to enable parade vddd, rc = %d\n",
						__func__, rc);
					goto ts_vddd_out;
				}
			}
		...
		if (0 == tskit_mstar_data->mstar_chip_data->vddio_gpio_type) {
			...
			gpio_direction_output(tskit_mstar_data->mstar_chip_data
						      ->vddio_gpio_ctrl,
					      1);
		}
	} else {
	...
	if (tskit_mstar_data->self_ctrl_power == 1) {
		...
		if (1 ==
			...
			if (!IS_ERR(tskit_mstar_data->vdda)) {
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s, failed to disable parade vdda, rc = %d\n",
						__func__, rc);
					return -1;
				}
			}
		...
		if (1 ==
			...
			if (!IS_ERR(tskit_mstar_data->vddd)) {
				...
				if (rc < 0) {
					TS_LOG_ERR(
						"%s, failed to disable parade vddd, rc = %d\n",
						__func__, rc);
					return -1;
				}
			}
...
static int mstar_pinctrl_init(void)
{
	...
	tskit_mstar_data->pctrl =
		devm_pinctrl_get(&tskit_mstar_data->mstar_dev->dev);
...
#else
	tskit_mstar_data->pinctrl_state_reset_high = pinctrl_lookup_state(
		tskit_mstar_data->pctrl, PINCTRL_STATE_RESET_HIGH);
	...
	tskit_mstar_data->pinctrl_state_reset_low = pinctrl_lookup_state(
		tskit_mstar_data->pctrl, PINCTRL_STATE_RESET_LOW);
	...
	tskit_mstar_data->pinctrl_state_as_int = pinctrl_lookup_state(
		tskit_mstar_data->pctrl, PINCTRL_STATE_AS_INT);
	...
	tskit_mstar_data->pinctrl_state_int_high = pinctrl_lookup_state(
		tskit_mstar_data->pctrl, PINCTRL_STATE_INT_HIGH);
	...
	tskit_mstar_data->pinctrl_state_int_low = pinctrl_lookup_state(
		tskit_mstar_data->pctrl, PINCTRL_STATE_INT_LOW);
...
#ifndef CONFIG_HONOR_DEVKIT_MTK_3_0
	if (tskit_mstar_data->pctrl == NULL ||
	    tskit_mstar_data->pins_default == NULL) {
		TS_LOG_ERR("%s: pctrl or pins_default is NULL.\n", __func__);
		return -1;
	}

	retval = pinctrl_select_state(tskit_mstar_data->pctrl,
				      tskit_mstar_data->pins_default);
	if (retval < 0) {
...
static int mstar_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	tskit_mstar_data->mstar_dev->dev.of_node =
		platform_data->chip_data->cnode;
	...
	rc = mstar_parse_dts(tskit_mstar_data->mstar_dev->dev.of_node,
			     tskit_mstar_data->mstar_chip_data);
	if (rc != 0) {
	...
	if (rc < 0) {
		...
		strncpy(tskit_mstar_data->mstar_chip_data->chip_name,
			MSTAR_CHIP_NAME, strlen(MSTAR_CHIP_NAME) + 1);
	}
...
static int mstar_param_init(void)
{
	...
	ret = mstar_get_vendor_name_from_dts(tskit_mstar_data->project_id,
					     vendor_name,
					     MSTAR_VENDOR_NAME_LEN);
	if (ret) {
		TS_LOG_ERR("%s:read vendor name fail by project id\n",
			   __func__);
		if (MSTAR_VENDOR_ID_HLT == tskit_mstar_data->vendor_id) {
			strncpy(vendor_name, MSTAR_VENDOR_NAME_HLT,
				MSTAR_VENDOR_NAME_LEN);
		} else {
		...
		strncpy(tskit_mstar_data->mstar_chip_data->module_name,
			vendor_name, MAX_STR_LEN - 1);
	}
	ret = mstar_get_platform_fw_ver(
		&tskit_mstar_data->fw_data.platform_inter_ver);
	if (ret < 0) {
	...
	if (tskit_mstar_data->lcd_panel_name_from_lcdkit) {
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"%s: get lcd panel info from lcdkit fail!\n ",
				__func__);
		}
	}
...
static int mstar_init_chip(void)
{
	...
	tskit_mstar_data->mstar_chip_data->easy_wakeup_info.sleep_mode =
		TS_POWER_OFF_MODE;
	tskit_mstar_data->mstar_chip_data->easy_wakeup_info.easy_wakeup_gesture =
		false;
	tskit_mstar_data->mstar_chip_data->easy_wakeup_info.easy_wakeup_flag =
		false;
	tskit_mstar_data->mstar_chip_data->easy_wakeup_info.palm_cover_flag =
		false;
	tskit_mstar_data->mstar_chip_data->easy_wakeup_info.palm_cover_control =
		false;
	tskit_mstar_data->mstar_chip_data->easy_wakeup_info.off_motion_on =
		false;
	tskit_mstar_data->esd_enable = TRUE;
...
static int mstar_input_config(struct input_dev *input_dev)
{
	...
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, 0,
			     tskit_mstar_data->mstar_chip_data->x_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, 0,
			     tskit_mstar_data->mstar_chip_data->y_max, 0, 0);
...
int mstar_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
	...
	switch (info->op_action) {
	case TS_ACTION_READ:
		memcpy(info->tp_test_type, dev_data->tp_test_type,
