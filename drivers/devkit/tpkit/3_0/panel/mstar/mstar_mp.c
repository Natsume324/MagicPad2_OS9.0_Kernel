...
void mp_dump_data(void *pBuf, u16 nLen, int nDataType, int nCarry,
		  int nChangeLine, const char *desp, int level)
{
	int i;
	char szStrTmp[10] = {0};
	char szStrBuf[512] = {0};
	...
	for (i = 0; i < nLen; i++) {
		if (nCarry == 16) {
			if (nDataType == 8)
				snprintf(szStrTmp, sizeof(szStrTmp), "%02X ",
					 pU8Buf[i]);
			else if (nDataType == -8)
				snprintf(szStrTmp, sizeof(szStrTmp), "%02X ",
					 pS8Buf[i]);
			else if (nDataType == 16)
				snprintf(szStrTmp, sizeof(szStrTmp), "%04X ",
					 pU16Buf[i]);
			else if (nDataType == -16)
				snprintf(szStrTmp, sizeof(szStrTmp), "%04X ",
					 pS16Buf[i]);
			else if (nDataType == 32)
				snprintf(szStrTmp, sizeof(szStrTmp), "%08X ",
					 pU32Buf[i]);
			else if (nDataType == -32)
				snprintf(szStrTmp, sizeof(szStrTmp), "%08X ",
					 pS32Buf[i]);
			...
			if (nDataType == 8)
				snprintf(szStrTmp, sizeof(szStrTmp), "%6u ",
					 pU8Buf[i]);
			else if (nDataType == -8)
				snprintf(szStrTmp, sizeof(szStrTmp), "%6d ",
					 pS8Buf[i]);
			else if (nDataType == 16)
				snprintf(szStrTmp, sizeof(szStrTmp), "%6u ",
					 pU16Buf[i]);
...
int msg30xx_check_mp_switch(void)
{
	...
	do {
		nRegData =
			mstar_get_reg_16bit_by_addr(0x1401, ADDRESS_MODE_16BIT);
...
int msg30xx_dbbus_set_default(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(tmc); i++) {
		rc = mstar_iic_write_data(SLAVE_I2C_ID_DBBUS,
					  (u8 *)(&tmc[i].data), 1);
		if (rc < 0) {
...
int msg30xx_covert_Rvalue(int dump_time, s32 deltaR)
{
	...
	if (deltaR > -1000)
		result = (int)(73318 * dump_time * 1000) /
			 (55 * (abs(deltaR) - 0));
	else
		result = (int)(53248 * dump_time * 1000) /
			 (55 * (abs(deltaR) - 0));
...
int msg2846a_covert_Rvalue(int dump_time, s32 deltaR)
{
	...
	if (deltaR >= 0)
		result = (int)(58654 * dump_time * 1000) /
			 (55 * (abs(deltaR) - 0));
	else
		result = (int)(42598 * dump_time * 1000) /
			 (55 * (abs(deltaR) - 0));
...
u16 msg28xx_read_test_pins(u16 itemID, int *testPins)
{
	...
	case 5:
	case 15:
		if (ms_get_ini_data("SHORT_TEST_N3", "MUX_MEM_20_3E", str) !=
		    0) {
			ms_get_ini_data("SHORT_TEST_N3", "TEST_PIN", str);
			nSize = ms_ini_split_int_array(str, testPins);
			TS_LOG_DEBUG("SHORT_TEST_N3 nSize = %d\n", nSize);
		} else if (ms_get_ini_data("SHORT_TEST_S3", "MUX_MEM_20_3E",
					   str) != 0) {
			ms_get_ini_data("SHORT_TEST_S3", "TEST_PIN", str);
			nSize = ms_ini_split_int_array(str, testPins);
			TS_LOG_DEBUG("SHORT_TEST_S3 nSize = %d\n", nSize);
		} else if (ms_get_ini_data("SHORT_TEST_GR", "MUX_MEM_20_3E",
					   str) != 0) {
			...
			for (i = 0;
			     i < sizeof(_gGRtestPins) / sizeof(_gGRtestPins[0]);
			     i++)
...
u16 msg30xx_read_test_pins(u16 itemID, int *testPins)
{
	...
	case 14:
		if (ms_get_ini_data("SHORT_TEST_N3", "MUX_MEM_20_3E", str) !=
			...
			for (i = 0; i < sizeof(_gMsg30xxGRtestPins) /
						sizeof(_gMsg30xxGRtestPins[0]);
			     i++)
...
s32 msg28xx_short_test_judge(int *deltac_data, u8 nItemID, s8 *TestFail,
			     u16 TestFail_check[][MAX_MUTUAL_NUM])
{
...
s32 msg30xx_short_test_judge(int *deltac_data, u8 nItemID, s8 *TestFail,
			     u16 TestFail_check[][MAX_MUTUAL_NUM])
{
	int nRet = 1, i, count_test_pin = 0;
...
int msg28xx_short_judge(int *deltac_data)
{
	...
	if (ERR_ALLOC_MEM(senseR) || ERR_ALLOC_MEM(driveR) ||
	    ERR_ALLOC_MEM(GRR)) {
		...
		if (mp_test_data->PAD2Sense != NULL) {
			for (j = 0; j < p_mp_func->sense_len; j++) {
				if (i == mp_test_data->PAD2Sense[j]) {
					mp_test_result->pShortRData[j] =
						convert[i];
					mp_test_result->pShortResultData[j] =
						deltac_data[i];
					...
					if (0 ==
					    check_thrs(deltac_data[i],
						       mp_test_data->sensorInfo
							       .thrsICpin,
						       -mp_test_data->sensorInfo
								.thrsICpin)) {
						nRetVal = -1;
						mp_test_result->nShortResult =
							ITO_TEST_FAIL;
						mp_test_result
							->pShortFailChannel[j] =
							ITO_TEST_FAIL;
		...
		if (mp_test_data->PAD2Drive != NULL) {
			for (j = 0; j < p_mp_func->drive_len; j++) {
				if (i == mp_test_data->PAD2Drive[j]) {
					mp_test_result->pShortRData
						[p_mp_func->sense_len + j] =
						convert[i];
					mp_test_result->pShortResultData
						[p_mp_func->sense_len + j] =
						deltac_data[i];
					...
					if (0 ==
						...
						mp_test_result->nShortResult =
							ITO_TEST_FAIL;
						mp_test_result->pShortFailChannel
							[p_mp_func->sense_len +
							 j] = -1;
					}
				}
...
int msg30xx_short_judge(int *deltac_data)
{
	...
	int datalen1 = 0;
	int *deltaC = NULL;
	int sum = 0, cunt = 0, avg = 0,
	    convert[MAX_AFENUMs_30XX * MAX_SUBFRAMES_30XX] = {0};
	...
	if (tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A) {
		dump_time =
			mp_test_data->SHORT_Dump2 - mp_test_data->SHORT_Dump1;
		dump_time = (dump_time * 4 * 2 * 100) / 5166;
		TS_LOG_DEBUG("%s, dump_time = %d, thrsICpin = %d\n", __func__,
			     dump_time, mp_test_data->sensorInfo.thrsICpin);
		thrs = msg2846a_covert_Rvalue(
			dump_time, mp_test_data->sensorInfo.thrsICpin);
	} else {
		dump_time = (((mp_test_data->SHORT_Dump1 + 1) * 4 * 2) * 100) /
			    5166;
		TS_LOG_DEBUG("%s, dump_time = %d, short_thrs = %d\n", __func__,
			     dump_time, mp_test_data->sensorInfo.thrsShort);
		thrs = msg30xx_covert_Rvalue(
			dump_time, mp_test_data->sensorInfo.thrsShort);
	}
	...
	for (i = 0; i < 6; i++) { //max 6 subframe
		for (j = 0; j < 14; j++) { // max 14 AFE
			if ((i * 14 + j) < MAX_CHANNEL_NUM_30XX)
				mp_test_result->pShortFailChannel[i * 14 + j] =
					(u32)PIN_UN_USE;
			mp_test_result->normalTestFail_check[i][j] = PIN_UN_USE;
		}
	}
...
int msg28xx_open_judge(int *deltac_data, int deltac_size)
{
	...
	int ratioAvg_border = 0, ratioAvg_border_max = 0,
	    ratioAvg_border_min = 0, passCount1 = 0;
	...
	for (i = 0; i < sense_num * drv_num; i++) {
		...
		if ((mp_test_data->Mutual_Key == 1 ||
		     mp_test_data->Mutual_Key == 2) &&
		    (mp_test_data->sensorInfo.numKey != 0)) {
			if (mp_test_data->Pattern_type == 5) {
				// KEY_CH = 1, it mean keys in same drive. Current one key project only KEY_CH = 1 type.
				if (mp_test_data->sensorInfo.KEY_CH !=
	...
	memset(mp_test_result->normalTestFail_check_Deltac, 0xFFFF,
	       MAX_MUTUAL_NUM);
	memset(mp_test_result->normalTestFail_check_Ratio, 0xFFFF,
	       MAX_MUTUAL_NUM);
	...
	for (k = 0; k < deltac_size; k++) {
		...
		if (result_buf[k] != NULL_DATA) {
			mp_test_result->ratio[k] =
				(result_buf[k] * 1000) /
				mp_test_data->Goldensample_CH_0[k];

			if (0 ==
			    check_thrs(result_buf[k],
				       mp_test_data->Goldensample_CH_0_Max[k],
				       mp_test_data->Goldensample_CH_0_Min[k])) {
				nNormalTestResult[0] = 1;
				mp_test_result->nNormalTestResultCheck[k] =
					(u16)(((k / drv_num) + 1) * 100 +
					      ((k % drv_num) + 1));
			} else {
				mp_test_result->nNormalTestResultCheck[k] =
					PIN_NO_ERROR;
				if ((mp_test_data->Pattern_type == 3) &&
				    (mp_test_data->sensorInfo.numKey == 0) &&
				    ((k % drv_num == 0) ||
				     ((k + 1) % drv_num == 0))) {
					ratioAvg_border +=
						mp_test_result->ratio[k];
					passCount1 += 1;
				} else if ((mp_test_data->Pattern_type == 3) &&
					   (mp_test_data->sensorInfo.numKey !=
					    0) &&
					   ((k % drv_num == 0) ||
					    ((k + 2) % drv_num == 0))) {
		...
		mp_test_result->normalTestFail_check_Deltac[k] =
			mp_test_result->nNormalTestResultCheck[k];
	}

	TS_LOG_DEBUG(
		"  Msg28xx Open Test# mp_test_result->normalTestFail_check_Deltac Channel  \n");

	ratioAvg_max =
		(int)(100000 +
		      (mp_test_data->ToastInfo.persentDC_VA_Ratio * 1000) +
		      (mp_test_data->ToastInfo.persentDC_VA_Ratio_up * 1000)) /
		100;
	ratioAvg_min =
		(int)(100000 -
		      (mp_test_data->ToastInfo.persentDC_VA_Ratio * 1000)) /
	...
	ratioAvg_border_max =
		(int)(100000 +
		      (mp_test_data->ToastInfo.persentDC_Border_Ratio * 1000) +
		      (mp_test_data->ToastInfo.persentDC_VA_Ratio_up * 1000)) /
		100;
	ratioAvg_border_min =
		(int)(100000 -
	...
	if (passCount != 0) {
		if (passCount1 != 0) {
			...
			for (i = 0; i < MAX_MUTUAL_NUM; i++) {
				if ((mp_test_data->sensorInfo.numKey == 0) &&
				    ((i % drv_num == 0) ||
				     ((i + 1) % drv_num == 0))) {
					mp_test_result->ratio_move[i] =
						mp_test_result->ratio[i] -
						ratioAvg_border_move + 1000;
				} else if ((mp_test_data->sensorInfo.numKey !=
					    0) &&
					   ((i % drv_num == 0) ||
					    ((i + 2) % drv_num == 0))) {
					mp_test_result->ratio_move[i] =
		...
		memcpy(mp_test_result->ratio, mp_test_result->ratio_move,
		       MAX_MUTUAL_NUM);
	}
			...
			if (result_buf[j] != NULL_DATA) {
				if ((mp_test_data->Pattern_type == 3) &&
				    ...
				    ((j % drv_num == 0) ||
				     ((j + 1) % drv_num == 0))) {
					if (0 ==
					    check_thrs(mp_test_result
							       ->ratio_move[j],
						       ratioAvg_border_max,
						       ratioAvg_border_min)) {
						nNormalTestResult[1] = 1;
						mp_test_result
							->nNormalTestResultCheck
								[j] =
							(u16)(((j / drv_num) +
							       1) * 100 +
							      ((j % drv_num) +
				...
				mp_test_result->nNormalTestResultCheck[j] =
					PIN_NO_ERROR;
			}
		} else {
			mp_test_result->normalTestFail_check_Ratio[j] =
				mp_test_result->nNormalTestResultCheck[j];
			continue;
		}
		mp_test_result->normalTestFail_check_Ratio[j] =
	...
	for (k = 0; k < MAX_MUTUAL_NUM; k++) {
		if (0 == mp_test_data->Goldensample_CH_0[k]) {
			...
			mp_test_result->normalTestFail_check_Deltac[k] =
				PIN_NO_ERROR;
			mp_test_result->normalTestFail_check_Ratio[k] =
				PIN_NO_ERROR;
		}
	}
	...
	for (i = 0; i < MAX_MUTUAL_NUM;
	     i++) // reduce memory operation instead of memcpy
	{
		mp_test_result->pOpenFailChannel[i] =
			mp_test_result->normalTestFail_check_Deltac[i];
		mp_test_result->pOpenRatioFailChannel[i] =
			mp_test_result->normalTestFail_check_Ratio[i];
		mp_test_result->pGolden_CH_Max_Avg[i] =
			mp_test_result->ratio_move[i];
	}
...
void msg30xx_open_judge_calc_ratio_avg(int passCount, int passCount1,
				       int ratioAvg, int ratioAvg_border)
{
...
s8 msg30xx_open_judge_ratio(int deltac_size, int *by_pass_buf, int *result_buf)
{
	...
	int ratioAvg_max, ratioAvg_min, ratioAvg_border_max,
		ratioAvg_border_min;
	s8 ret = 0;
	...
	for (j = 0; j < deltac_size; j++) {
		...
		if (result_buf[j] != NULL_DATA) {
			if ((mp_test_data->Pattern_type == 3) &&
			    (mp_test_data->sensorInfo.numKey == 0) &&
			    ((j % drv_num == 0) || ((j + 1) % drv_num == 0))) {
				if (0 == check_thrs(
						 mp_test_result->ratio_move[j],
						 ratioAvg_border_max,
						 ratioAvg_border_min) &&
				    by_pass_buf[j] == NORMAL_JUDGE) {
					ret = 1;
					mp_test_result
						->nNormalTestResultCheck[j] =
						(u16)(((j / drv_num) + 1) *
							      100 +
						      ((j % drv_num) + 1));
					TS_LOG_ERR(
						"1 open ratio fail [%d][%d]: %d, ratio_Max:%d , ratio_Min:%d\n",
						j / drv_num, j % drv_num,
						mp_test_result->ratio_move[j],
						ratioAvg_border_max,
						ratioAvg_border_min);
				...
				if (0 == check_thrs(
					...
					mp_test_result
						...
						(u16)(((j / drv_num) + 1) *
						      ...
						      ((j % drv_num) + 1));
					TS_LOG_ERR(
						"2 open ratio fail [%d][%d]: %d, ratio_Max:%d , ratio_Min:%d\n",
						j / drv_num, j % drv_num,
						mp_test_result->ratio_move[j],
						ratioAvg_border_max,
						ratioAvg_border_min);
				...
				if (0 == check_thrs(
					...
					mp_test_result
						...
						(u16)(((j / drv_num) + 1) *
						      ...
						      ((j % drv_num) + 1));
					TS_LOG_ERR(
						"3 open ratio fail [%d][%d]: %d, ratio_Max:%d , ratio_Min:%d\n",
						j / drv_num, j % drv_num,
						mp_test_result->ratio_move[j],
						ratioAvg_max, ratioAvg_min);
				}
...
int msg30xx_open_judge(int *deltac_data, int deltac_size)
{
	...
	for (k = 0; k < deltac_size; k++) {
		...
		if (result_buf[k] != NULL_DATA) {
			...
			if (0 == check_thrs(
					 ...
					 mp_test_data->Goldensample_CH_0_Max[k],
					 mp_test_data
						 ->Goldensample_CH_0_Min[k]) &&
			    by_pass_buf[k] != NO_JUDGE) {
				nNormalTestResult[0] = 1;
				mp_test_result->nNormalTestResultCheck[k] =
					(u16)(((k / drv_num) + 1) * 100 +
					      ((k % drv_num) + 1));
				TS_LOG_ERR(
					"open fail channel[%d][%d]: %d, Golden_Max:%d, , Golden_Min:%d\n",
					k / drv_num, k % drv_num, result_buf[k],
					mp_test_data->Goldensample_CH_0_Max[k],
					mp_test_data->Goldensample_CH_0_Min[k]);
			} else {
				mp_test_result->nNormalTestResultCheck[k] =
	...
	msg30xx_open_judge_calc_ratio_avg(passCount, passCount1, ratioAvg,
					  ratioAvg_border);

	memset(mp_test_result->nNormalTestResultCheck, PIN_NO_ERROR,
	       MAX_MUTUAL_NUM);

	nNormalTestResult[1] =
		msg30xx_open_judge_ratio(deltac_size, by_pass_buf, result_buf);
	...
	mp_dump_data(mp_test_result->pOpenResultData, sense_num * drv_num, -32,
		     10, drv_num, "Open data", 0);
	mp_dump_data(mp_test_result->ratio, sense_num * drv_num, -32, 10,
		     drv_num, "Open ratio data", 0);
	mp_kfree(by_pass_buf);
	return nRetVal;
}

int msg2846a_uniformity_delta_left_right(int *condition_buf, int *deltac_data,
					 int index, int judge)
{
	int ret = NO_ERR, temp;
	...
	if ((index + 1) % mp_test_data->sensorInfo.numDrv == 1 ||
		...
		return PIN_NO_ERROR;
	} else if ((condition_buf[index] == condition_buf[index + 1]) &&
		   condition_buf[index] != NO_JUDGE) {
...
int msg30xx_uniformity_delta_left_right(int *condition_buf, int *deltac_data,
					int index, int judge)
{
	int ret = NO_ERR, temp;
	...
	if ((index + 1) % mp_test_data->sensorInfo.numDrv == 1 ||
	    ...
	    (index + 2) % mp_test_data->sensorInfo.numDrv == 0) {
		return PIN_NO_ERROR;
	} else if (condition_buf[index] == NO_JUDGE ||
		   condition_buf[index + 1] == NO_JUDGE) {
		return PIN_NO_ERROR;
	} else {
		temp = UNIF_ABS(deltac_data[index], deltac_data[index + 1]);
		if (temp <= judge)
...
int msg2846a_uniformity_delta_up_down(int *condition_buf, int *deltac_data,
				      int index, int va_cut, int judge)
{
	int ret = NO_ERR, temp;

	if ((index / mp_test_data->sensorInfo.numDrv == 0) ||
	    (index / mp_test_data->sensorInfo.numDrv >
	     mp_test_data->sensorInfo.numSen - 3) ||
	    (index / mp_test_data->sensorInfo.numDrv == va_cut)) {
		ret = PIN_NO_ERROR;
	} else if ((condition_buf[index] ==
		    condition_buf[index + mp_test_data->sensorInfo.numDrv]) &&
		   (condition_buf[index] != NO_JUDGE) &&
		   (condition_buf[index] != NOTCH_JUDGE)) {
		temp = UNIF_ABS(
			deltac_data[index],
			deltac_data[index + mp_test_data->sensorInfo.numDrv]);
		if (temp <= judge)
			ret = temp;
		else
			ret = 0 - temp;
...
int msg30xx_uniformity_delta_up_down(int *condition_buf, int *deltac_data,
				     int index, int va_cut, int judge)
{
	int ret = NO_ERR, temp;
...
void msg2846a_uniformity_check_border_ratio(int *border_ratio)
{
	...
	for (i = 0, index = 0; i < 5; i++) {
		for (j = 0; j < sense_num; j++, index++) {
			if (i == 0) { // top
				ratio_max = mp_test_data->uniformity_ratio_max
						    .bd_top;
				ratio_min = mp_test_data->uniformity_ratio_min
						    .bd_top;
			} else if (i == 1) { //down
				ratio_max = mp_test_data->uniformity_ratio_max
						    .bd_bottom;
				...
				if (j <= va_cut) {
					ratio_max =
						mp_test_data
							->uniformity_ratio_max
							.bd_l_top;
					ratio_min =
						mp_test_data
							->uniformity_ratio_min
							.bd_l_top;
				} else {
					ratio_max =
						mp_test_data
			...
			border_ratio[i * sense_num + j] =
				msg30xx_uniformity_bd_ratio_checck(
					border_ratio[i * sense_num + j],
					ratio_max, ratio_min);
			if (border_ratio[i * sense_num + j] < 0 &&
			    border_ratio[i * sense_num + j] != -1)
				mp_test_result->uniformity_check_fail[2] =
					ITO_TEST_FAIL;
		}
	}
		...
		for (j = 0; j < sense_num; j++, index++) {
			if (i == 0 && j >= 10)
				border_ratio[i * sense_num + j] =
					msg30xx_uniformity_bd_ratio_checck(
						border_ratio[i * sense_num + j],
						border_ratio[4 * sense_num + j],
						border_ratio[5 * sense_num + j]);
			else
				border_ratio[i * sense_num + j] =
					msg30xx_uniformity_bd_ratio_checck(
						border_ratio[i * sense_num + j],
...
void msg30xx_uniformity_calc_data(int *d_data, int d_size, int *c_buf,
				  MpUniformityPart uni_cont)
{
		...
		for (j = 0; j < drv_num; j++, index++) {
			if (tskit_mstar_data->mstar_chip_data->ic_type ==
				...
				if (c_buf[index] == NOTCH_JUDGE) {
					judge = mp_test_result->uniformity_judge
							.notch;
				}
			}
			/* set judge data */
			...
			if (tskit_mstar_data->mstar_chip_data->ic_type ==
				...
				/* left to right */
				delta_LR_buf[index] =
					msg2846a_uniformity_delta_left_right(
						c_buf, d_data, index, judge);
			} else {
				/* left to right */
				delta_LR_buf[index] =
					msg30xx_uniformity_delta_left_right(
						c_buf, d_data, index, judge);
			}
			if (delta_LR_buf[index] < 0 &&
			    delta_LR_buf[index] != PIN_NO_ERROR) {
				mp_test_result->uniformity_check_fail[0] =
					ITO_TEST_FAIL;
			}
			if (tskit_mstar_data->mstar_chip_data->ic_type ==
			    CHIP_TYPE_MSG2846A) {
				/* up-down */
				delta_UD_buf[index] =
					msg2846a_uniformity_delta_up_down(
						c_buf, d_data, index, va_cut,
						judge);
			} else {
				/* up-down */
				delta_UD_buf[index] =
					msg30xx_uniformity_delta_up_down(
						c_buf, d_data, index, va_cut,
						judge);
			}
			if (delta_UD_buf[index] < 0 &&
			    delta_UD_buf[index] != PIN_NO_ERROR) {
				TS_LOG_ERR(
					"%s: Failed : delta_UD_buf[%d] = %d  \n",
					__func__, index, delta_UD_buf[index]);
				mp_test_result->uniformity_check_fail[1] =
					ITO_TEST_FAIL;
			}
		}
	}
...
void msg2846a_uniformity_sum_data(int *d_data, int d_size, int *c_buf,
				  int *border_ratio, MpUniformityPart *uni_cont)
{
	...
	for (i = 0, index = 0; i < sense_num; i++) {
		for (j = 0; j < drv_num; j++, index++) {
			mp_test_result->puniformity_deltac[index] =
				0 - d_data[index];

			/* Sum judge data */
			if (c_buf[index] != NO_JUDGE) {
				if (c_buf[index] == NOTCH_JUDGE) {
					mp_test_result->uniformity_judge.notch +=
						golden[index];
					uni_cont->notch++;
					...
					mp_test_result->uniformity_judge
						.bd_top += golden[index];
					uni_cont->bd_top++;
					if (d_data[index + drv_num] == 0)
						d_data[index + drv_num]++;
					...
					mp_test_result->uniformity_judge
						.bd_bottom += golden[index];
					uni_cont->bd_bottom++;
					if (d_data[index - drv_num] == 0)
						d_data[index - drv_num]++;

					border_ratio[sense_num + j] =
						((100 * d_data[index]) /
						 d_data[index - drv_num]);
				} else if (j == 0 && i <= va_cut) //left top
				{
					mp_test_result->uniformity_judge
						.bd_l_top += golden[index];
					uni_cont->bd_l_top++;
					if (d_data[index + 1] == 0)
						d_data[index + 1]++;

					border_ratio[2 * sense_num + i] =
						((100 * d_data[index]) /
						 d_data[index + 1]);
				} else if (j == 0 && i > va_cut) //left down
				{
					mp_test_result->uniformity_judge
						.bd_l_bottom += golden[index];
				...
				{
					mp_test_result->uniformity_judge
						.bd_r_top += golden[index];
					uni_cont->bd_r_top++;
					if (d_data[index - 1] == 0)
						d_data[index - 1]++;

					border_ratio[3 * sense_num + i] =
						((100 * d_data[index]) /
						 d_data[index - 1]);
				} else if (j == drv_num - 1 &&
					   i > va_cut) //right down
...
void msg30xx_uniformity_sum_data(int *d_data, int d_size, int *c_buf,
				 int *border_ratio, MpUniformityPart *uni_cont)
{
	int i, j, index, va_cut;
	int sense_num, drv_num;
...
int msg30xx_uniformity_judge(int *deltac_data, int deltac_size)
{
	...
	memset(&mp_test_result->uniformity_judge.bd_top, 0,
	       sizeof(MpUniformityPart));
	...
	if (tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A) {
		msg2846a_uniformity_sum_data(deltac_data, node_num,
					     condition_buf, border_ratio_buf,
					     &uniformity_cont);
	} else {
		msg30xx_uniformity_sum_data(deltac_data, node_num,
					    condition_buf, border_ratio_buf,
					    &uniformity_cont);
	}

	msg30xx_uniformity_calc_data(deltac_data, node_num, condition_buf,
				     uniformity_cont);
	if (tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A) {
...
static int new_get_raw_data(s16 *pRawData, u16 count, u8 dump_time)
{
	...
	pShotOriData = kcalloc(data_length * 2 * 2 + touch_info_length,
	...
	if (mp_test_data->run_type == RUN_SHORT_TEST) {
		if (dump_time == mp_test_data->SHORT_Dump1) {
			mp_test_data->SHORT_Dump1 = mstar_get_reg_16bit_by_addr(
				0x1018, ADDRESS_MODE_16BIT);
			TS_LOG_DEBUG("%s: Dump1 = 0x%x\n", __func__,
				     mp_test_data->SHORT_Dump1);
		}
		if (dump_time == mp_test_data->SHORT_Dump2) {
			mp_test_data->SHORT_Dump2 = mstar_get_reg_16bit_by_addr(
				0x1018, ADDRESS_MODE_16BIT);
	...
	/* Read DQ base */
	RegData = mstar_get_reg_16bit_by_addr(p_mp_func->fout_data_addr,
					      ADDRESS_MODE_16BIT);
	...
	for (offset = 0; offset < data_length * 2 + touch_info_length;
	     offset += MAX_I2C_TRANSACTION_LENGTH_LIMIT) {
		...
		if (offset == 0 && (MAX_I2C_TRANSACTION_LENGTH_LIMIT <
				    data_length * 2 + touch_info_length)) {
			rc = mstar_reg_get_xbit_write_4byte_value(
				fout_base_addr + offset, pShotOriData + offset,
				MAX_I2C_TRANSACTION_LENGTH_LIMIT,
				MAX_I2C_TRANSACTION_LENGTH_LIMIT);
			if (rc < 0) {
				TS_LOG_ERR(
					"%s mstar_reg_get_xbit_write_4byte_value error\n",
					__func__);
				return rc;
			}
		} else if (offset + MAX_I2C_TRANSACTION_LENGTH_LIMIT <
			   data_length * 2 + touch_info_length) {
			rc = mstar_reg_get_xbit_write_4byte_value(
				fout_base_addr + offset, pShotOriData + offset,
				MAX_I2C_TRANSACTION_LENGTH_LIMIT,
			...
			rc = mstar_reg_get_xbit_write_4byte_value(
				fout_base_addr + offset, pShotOriData + offset,
				data_length * 2 + touch_info_length - offset,
				MAX_I2C_TRANSACTION_LENGTH_LIMIT);
			if (rc < 0) {
				TS_LOG_ERR(
					"%s mstar_reg_get_xbit_write_4byte_value error\n",
	...
	if (mp_test_data->run_type == RUN_SHORT_TEST) {
		for (j = 0; j < data_length; j++) {
			pShotDataAll[j] = (pShotOriData[2 * j] |
					   pShotOriData[2 * j + 1] << 8);
		}
	} else {
		for (j = 0; j < data_length; j++) {
			pShotDataAll[j] =
				(pShotOriData[2 * j + touch_info_length + 1] |
				 pShotOriData[2 * j + touch_info_length] << 8);
		}
	}
...
static int new_get_deltac(int *delt, u8 dump_time)
{
	...
	pRawData = kcalloc(MAX_CHANNEL_SEN * MAX_CHANNEL_DRV * 2, sizeof(s16),
			   GFP_KERNEL);
	...
	if (mp_test_data->run_type == RUN_SHORT_TEST) {
		...
		count = mp_test_data->sensorInfo.numSen *
...
static int new_send_test_cmd(u16 fmode, int mode)
{
	...
	u8 post_idle, self_sample_hi, self_sample_lo, short_sample_hi,
		short_sample_lo;
	u16 chargeT, dumpT;
	int postidle;
	...
	if (mstar_iic_write_data(SLAVE_I2C_ID_DWI2C, &cmd[0], ARRAY_SIZE(cmd)) <
	    0) {
...
static int new_flow_start_test(void)
{
	...
	if (tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A) {
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"*** Switch FW Mode Busy, return fail ***\n");
		}
	...
	if (mp_test_data->run_type == RUN_SHORT_TEST &&
		...
		for (i = 0; i < MAX_SUBFRAMES_30XX * MAX_AFENUMs_30XX; i++) {
			if (abs(deltac_data[i]) >=
			    mp_test_data->SHORT_Fout_max_1) {
				deltac_data[i] = deltac_data[i];
			} else if (abs(short_deltac_data[i]) >=
				   mp_test_data->SHORT_Fout_max_2) {
				deltac_data[i] = short_deltac_data[i];
			} else {
				deltac_data[i] =
					short_deltac_data[i] - deltac_data[i];
			}
		}
		mp_dump_data(deltac_data, MAX_SUBFRAMES_30XX * MAX_AFENUMs_30XX,
			     -32, 10, 14, "short data", 0);
		mp_dump_data(deltac_data, MAX_SUBFRAMES_30XX * MAX_AFENUMs_30XX,
			     -32, 10, 10, "short dump2 - dump1", 0);
	}
	...
	if (mp_test_data->run_type == RUN_SHORT_TEST) {
		...
		mp_dump_data(deltac_data, MAX_SUBFRAMES_30XX * MAX_AFENUMs_30XX,
			     -32, 10, 14, "short judge data", 0);
	} else if (mp_test_data->run_type == RUN_OPEN_TEST) {
		mp_test_result->nOpenResult = ITO_TEST_OK;
		if (tskit_mstar_data->mstar_chip_data->ic_type ==
		    CHIP_TYPE_MSG2846A) {
			ret = p_mp_func->open_judge(
				mp_test_result->pdeltac_buffer, deltac_size);
		} else {
		...
		if (tskit_mstar_data->mstar_chip_data->ic_type ==
		    CHIP_TYPE_MSG2846A) {
			ret = p_mp_func->uniformity_judge(
				mp_test_result->pdeltac_buffer, deltac_size);
		} else {
			ret = p_mp_func->uniformity_judge(deltac_data,
							  deltac_size);
		}
	}
...
int mp_new_flow_main(int item)
{
	...
	for (i = 0; i < ARRAY_SIZE(tmc); i++) {
		ret = mstar_iic_write_data(SLAVE_I2C_ID_DBBUS,
					   (u8 *)(&tmc[i].data), 1);
		if (ret < 0) {
...
static void mp_calc_golden_range_new(int *goldenTbl, int weight_up,
				     int weight_low, int *maxTbl, int *minTbl,
				     int length)
{
	int i = 0;
	for (i = 0; i < length; i++) {
...
static void mp_calc_golden_range(int *goldenTbl, u16 weight, u16 golden_up,
				 int *maxTbl, int *minTbl, int length)
{
	int i, value = 0, value_up = 0;
	int allnode_number = mp_test_data->sensorInfo.numDrv *
			     mp_test_data->sensorInfo.numSen;

	if ((tskit_mstar_data->mstar_chip_data->ic_type ==
	     CHIP_TYPE_MSG2846A) ||
	    (tskit_mstar_data->chip_type == CHIP_TYPE_MSG28XXA)) {
		int j = 0, shift_value = 0, weight_up = golden_up,
		    weight_low = 0, weight_notch_up = 0, weight_notch_low = 0;
		int golden_Cbg = 0;
		...
		mp_dump_data(goldenTbl, allnode_number, -32, 10, 16,
			     "goldenTbl", 0);
		...
		if (golden_Cbg == 0) {
			...
			shift_value =
				(int)((((mp_test_data->Open_Swt * 10000) /
						golden_Cbg -
					10000) *
		...
		if (mp_test_data->Open_Swt > golden_Cbg) {
			TS_LOG_DEBUG("%s: shift_value = %d\n", __func__,
				     shift_value);
			weight_up = shift_value +
				    mp_test_data->Open_Csig_Vari_Up * 100;
			weight_low = shift_value +
				     mp_test_data->Open_Csig_Vari_Low * 100;
			weight_notch_up =
				shift_value +
				mp_test_data->Open_Csig_Vari_Notch_Up * 100;
			weight_notch_low =
				shift_value +
				mp_test_data->Open_Csig_Vari_Notch_Low * 100;
		...
		mp_calc_golden_range_new(goldenTbl, weight_up, weight_low,
					 maxTbl, minTbl, length);
...
static void ms_init_key_data(void)
{
	...
	for (i = 0; i < PARSER_MAX_KEY_NUM; i++) {
		memset(ms_ini_file_data[i].pSectionName, 0,
		       PARSER_MAX_KEY_NAME_LEN);
		memset(ms_ini_file_data[i].pKeyName, 0,
		       PARSER_MAX_KEY_NAME_LEN);
		memset(ms_ini_file_data[i].pKeyValue, 0,
...
static int ms_ini_get_key_data(char *filedata)
{
	...
	while (1) {
		...
		n = ms_ini_file_get_line(filedata + dataoff, ini_buf,
					 PARSER_MAX_CFG_BUF);
		...
		if (ini_buf[0] == M_CFG_SSL) {
			ms_ini_file_data[mp_test_data->ini_items]
				.iSectionNameLen = n - 2;
			if (PARSER_MAX_KEY_NAME_LEN <
			    ms_ini_file_data[mp_test_data->ini_items]
				    .iSectionNameLen) {
		...
		strcpy(ms_ini_file_data[mp_test_data->ini_items].pSectionName,
		       tmpSectionName);
		ms_ini_file_data[mp_test_data->ini_items].iSectionNameLen =
		...
		/* Get key names */
		ms_ini_file_data[mp_test_data->ini_items].iKeyNameLen =
			iEqualSign;
		if (PARSER_MAX_KEY_NAME_LEN <
		    ms_ini_file_data[mp_test_data->ini_items].iKeyNameLen) {
			TS_LOG_ERR("MAX_KEY_NAME_LEN: Out Of Length\n\n");
		...
		ms_ini_file_data[mp_test_data->ini_items].iKeyValueLen =
			n - iEqualSign - 1;
		if (PARSER_MAX_KEY_VALUE_LEN <
		    ms_ini_file_data[mp_test_data->ini_items].iKeyValueLen) {
...
int ms_ini_get_key(char *section, char *key, char *value)
{
	...
	for (i = 0; i < mp_test_data->ini_items; i++) {
		...
		if (strncmp(key, ms_ini_file_data[i].pKeyName, len) == 0) {
			memcpy(value, ms_ini_file_data[i].pKeyValue,
			       ms_ini_file_data[i].iKeyValueLen);
...
int mp_parse(char *path)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		if (IS_ERR_OR_NULL(f)) {
			TS_LOG_ERR(
				"Failed to open the file at %ld. retry = %d\n",
				PTR_ERR(f), retry);
...
static int mp_load_ini(char *pFilePath)
{
	...
	if ((tskit_mstar_data->mstar_chip_data->ic_type ==
		...
		ms_get_ini_data("OPEN_TEST_N", "Csig_SUPPORT_VARI_NOTCH_UP",
				str);
		mp_test_data->Open_Csig_Vari_Notch_Up = ms_atoi(str);
		ms_get_ini_data("OPEN_TEST_N", "Csig_SUPPORT_VARI_NOTCH_LOW",
				str);
	...
	if (mp_test_data->sensorInfo.numKey > 0) {
		...
		mp_test_data->KeySen = kcalloc(mp_test_data->sensorInfo.numKey,
					       sizeof(int), GFP_KERNEL);
		if (ERR_ALLOC_MEM(mp_test_data->KeySen)) {
			TS_LOG_ERR(
				"Failed to allocate mp_test_data->KeySen mem\n");
			return -1;
		}
		...
		mp_test_data->sensorInfo.key_type =
			kmalloc(64 * sizeof(char), GFP_KERNEL);
		if (ERR_ALLOC_MEM(mp_test_data->sensorInfo.key_type)) {
			TS_LOG_ERR(
				"Failed to allocate mp_test_data->sensorInfo.key_type mem\n");
			return -1;
		}
	...
	if (ERR_ALLOC_MEM(mp_test_data->UIConfig.sSupportIC)) {
		TS_LOG_ERR(
			"Failed to allocate mp_test_data->UIConfig.sSupportIC mem\n");
		return -1;
	}

	memset(mp_test_data->UIConfig.sSupportIC, 0,
	       FILENAME_MAX * sizeof(char));
	...
	mp_test_data->project_name =
		(char *)kmalloc(FILENAME_MAX * sizeof(char), GFP_KERNEL);
	if (ERR_ALLOC_MEM(mp_test_data->UIConfig.sSupportIC)) {
		TS_LOG_ERR(
			"Failed to allocate mp_test_data->project_name mem\n");
		return -1;
	}
	...
	mp_test_data->Goldensample_CH_0 =
		(int *)kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	if (ERR_ALLOC_MEM(mp_test_data->Goldensample_CH_0)) {
		TS_LOG_ERR(
			"Failed to allocate mp_test_data->Goldensample_CH_0 mem\n");
		return -1;
	}

	nSize = ms_ini_split_golden(mp_test_data->Goldensample_CH_0,
				    mp_test_data->sensorInfo.numSen);
	TS_LOG_DEBUG("The number of Golden line = %d\n", nSize);

	mp_test_data->Goldensample_CH_0_Max =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_data->Goldensample_CH_0_Max_Avg =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_data->Goldensample_CH_0_Min =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_data->Goldensample_CH_0_Min_Avg =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	if (ERR_ALLOC_MEM(mp_test_data->Goldensample_CH_0_Max) ||
	    ERR_ALLOC_MEM(mp_test_data->Goldensample_CH_0_Max_Avg) ||
	    ERR_ALLOC_MEM(mp_test_data->Goldensample_CH_0_Min) ||
	    ERR_ALLOC_MEM(mp_test_data->Goldensample_CH_0_Min_Avg)) {
		TS_LOG_ERR("Failed to allocate Goldensample mem\n");
		return -1;
	}

	if (mp_test_data->sensorInfo.numDrv &&
	    mp_test_data->sensorInfo.numSen) {
		mp_test_data->PAD2Drive =
			kmalloc(mp_test_data->sensorInfo.numDrv * sizeof(u16),
				GFP_KERNEL);
		...
		p_mp_func->drive_len =
			ms_ini_split_u16_array(str, mp_test_data->PAD2Drive);

		mp_test_data->PAD2Sense =
			kmalloc(mp_test_data->sensorInfo.numSen * sizeof(u16),
				GFP_KERNEL);
		...
		p_mp_func->sense_len =
			ms_ini_split_u16_array(str, mp_test_data->PAD2Sense);
	}

	if (mp_test_data->sensorInfo.numGr) {
		mp_test_data->PAD2GR =
			kmalloc(mp_test_data->sensorInfo.numGr * sizeof(u16),
				GFP_KERNEL);
		...
		p_mp_func->gr_len =
			ms_ini_split_u16_array(str, mp_test_data->PAD2GR);
	}
...
static int mp_main_init_var(void)
{
	...
	mp_test_result->nRatioAvg_max =
		(int)(100 + mp_test_data->ToastInfo.persentDC_VA_Ratio +
		      mp_test_data->ToastInfo.persentDC_VA_Ratio_up) /
		100;
	mp_test_result->nRatioAvg_min =
		(int)(100 - mp_test_data->ToastInfo.persentDC_VA_Ratio) / 100;
	mp_test_result->nBorder_RatioAvg_max =
		(int)(100 + mp_test_data->ToastInfo.persentDC_Border_Ratio +
		      mp_test_data->ToastInfo.persentDC_VA_Ratio_up) /
		100;
	mp_test_result->nBorder_RatioAvg_min =
		(int)(100 - mp_test_data->ToastInfo.persentDC_Border_Ratio) /
		100;

	mp_test_result->pCheck_Fail =
		kcalloc(TEST_ITEM_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pOpenResultData =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pOpenFailChannel =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pOpenRatioFailChannel =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);

	mp_test_result->pGolden_CH =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pGolden_CH_Max =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pGolden_CH_Max_Avg =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pGolden_CH_Min =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pGolden_CH_Min_Avg =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);

	mp_test_result->pShortFailChannel =
		kcalloc(p_mp_func->max_channel_num, sizeof(int), GFP_KERNEL);
	mp_test_result->pShortResultData =
		kcalloc(p_mp_func->max_channel_num, sizeof(int), GFP_KERNEL);
	mp_test_result->pICPinChannel =
		kcalloc(p_mp_func->max_channel_num, sizeof(int), GFP_KERNEL);
	mp_test_result->pICPinShortFailChannel =
		kcalloc(p_mp_func->max_channel_num, sizeof(int), GFP_KERNEL);
	mp_test_result->pICPinShortResultData =
		kcalloc(p_mp_func->max_channel_num, sizeof(int), GFP_KERNEL);

	mp_test_result->pICPinShortRData =
		kcalloc(p_mp_func->max_channel_num, sizeof(int), GFP_KERNEL);
	mp_test_result->pShortRData =
		kcalloc(p_mp_func->max_channel_num, sizeof(int), GFP_KERNEL);

	mp_test_result->nNormalTestResultCheck =
		kcalloc(MAX_MUTUAL_NUM, sizeof(u16), GFP_KERNEL);
	mp_test_result->normalTestFail_check_Deltac =
		kcalloc(MAX_MUTUAL_NUM, sizeof(u16), GFP_KERNEL);
	mp_test_result->normalTestFail_check_Ratio =
		kcalloc(MAX_MUTUAL_NUM, sizeof(u16), GFP_KERNEL);
	mp_test_result->ratio_border =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->ratio_move =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);

	mp_test_result->ratio =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->normalTestFail_check =
		kcalloc(TEST_ITEM_NUM, sizeof(u16 *), GFP_KERNEL);

	mp_test_result->puniformity_deltac =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pdelta_LR_buf =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	mp_test_result->pdelta_UD_buf =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	if (tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A)
		mp_test_result->pborder_ratio_buf = kcalloc(
			MSG2846A_DRV_NUM * mp_test_data->sensorInfo.numSen,
			sizeof(int), GFP_KERNEL);
	else
		mp_test_result->pborder_ratio_buf =
			kcalloc(MSTAR_DRV_NUM * mp_test_data->sensorInfo.numSen,
				sizeof(int), GFP_KERNEL);
	mp_test_result->pdeltac_buffer =
		kcalloc(MAX_MUTUAL_NUM, sizeof(int), GFP_KERNEL);
	if (!ERR_ALLOC_MEM(mp_test_result->normalTestFail_check)) {
		for (i = 0; i < TEST_ITEM_NUM; i++) {
			mp_test_result->normalTestFail_check[i] = kcalloc(
				MAX_MUTUAL_NUM, sizeof(u16), GFP_KERNEL);
			if (ERR_ALLOC_MEM(
				    mp_test_result->normalTestFail_check[i])) {
				TS_LOG_ERR(
					"Failed to allocate test result two dimen normalTestFail_check mem\n");
				return -1;
			}
		}
	}
	...
	if (ERR_ALLOC_MEM(mp_test_result->nNormalTestResultCheck) ||
	    ERR_ALLOC_MEM(mp_test_result->normalTestFail_check_Deltac) ||
	    ERR_ALLOC_MEM(mp_test_result->normalTestFail_check_Ratio) ||
	    ERR_ALLOC_MEM(mp_test_result->ratio_border) ||
	    ERR_ALLOC_MEM(mp_test_result->ratio_move) ||
	    ERR_ALLOC_MEM(mp_test_result->ratio)) {
		TS_LOG_ERR("Failed to allocate test result mem\n");
		return -1;
	}

	if (ERR_ALLOC_MEM(mp_test_result->pCheck_Fail) ||
	    ERR_ALLOC_MEM(mp_test_result->pOpenResultData) ||
	    ERR_ALLOC_MEM(mp_test_result->pOpenFailChannel) ||
	    ERR_ALLOC_MEM(mp_test_result->pOpenRatioFailChannel)) {
		TS_LOG_ERR("Failed to allocate channels' mem\n");
		return -1;
	}

	if (ERR_ALLOC_MEM(mp_test_result->pGolden_CH) ||
	    ERR_ALLOC_MEM(mp_test_result->pGolden_CH_Max) ||
	    ERR_ALLOC_MEM(mp_test_result->pGolden_CH_Max_Avg) ||
	    ERR_ALLOC_MEM(mp_test_result->pGolden_CH_Min) ||
	    ERR_ALLOC_MEM(mp_test_result->pGolden_CH_Min_Avg)) {
		TS_LOG_ERR("Failed to allocate pGolden_CH' mem\n");
		return -1;
	}

	if (ERR_ALLOC_MEM(mp_test_result->pShortFailChannel) ||
	    ERR_ALLOC_MEM(mp_test_result->pShortResultData) ||
	    ERR_ALLOC_MEM(mp_test_result->pICPinChannel) ||
	    ERR_ALLOC_MEM(mp_test_result->pICPinShortFailChannel) ||
	    ERR_ALLOC_MEM(mp_test_result->pICPinShortResultData)) {
		TS_LOG_ERR("Failed to allocate pShortFailChannel' mem\n");
		return -1;
	}

	if (ERR_ALLOC_MEM(mp_test_result->pICPinShortRData) ||
	    ERR_ALLOC_MEM(mp_test_result->pShortRData)) {
...
static int mp_start_test(void)
{
	...
	if (mp_test_data->UIConfig.bOpenTest == 1) {
		...
		for (retry = 0; retry < MP_RETRY_TEST_TIME; retry++) {
			mp_test_result->nOpenResult =
				mp_new_flow_main(RUN_OPEN_TEST);
		...
		for (i = 0; i < MAX_MUTUAL_NUM; i++) {
			mp_test_result->pGolden_CH[i] =
				mp_test_data->Goldensample_CH_0[i];
			mp_test_result->pGolden_CH_Max[i] =
				mp_test_data->Goldensample_CH_0_Max[i];
			mp_test_result->pGolden_CH_Min[i] =
				mp_test_data->Goldensample_CH_0_Min[i];
		}
	} else {
	...
	if (mp_test_data->UIConfig.bShortTest == 1) {
		for (retry = 0; retry < MP_RETRY_TEST_TIME; retry++) {
			...
			mp_test_result->nShortResult =
				mp_new_flow_main(RUN_SHORT_TEST);
	...
	TS_LOG_INFO(
		"Final Result(%d): Short = %d, Open = %d, Uniformity = %d\n",
		res, mp_test_result->nShortResult, mp_test_result->nOpenResult,
		mp_test_result->nUniformityResult);
...
void mp_main_free(void)
{
	...
	if (!ERR_ALLOC_MEM(mp_test_data)) {
		...
		if (mp_test_data->sensorInfo.numKey > 0) {
			...
			TS_LOG_DEBUG(
				"MEM free mp_test_data->sensorInfo.key_type\n");
		}
...
void mstar_set_sensor_test_result(struct ts_rawdata_info *info)
{
	if (!ERR_ALLOC_MEM(mp_test_result)) {
		if (mp_test_result->i2c_status == NO_ERR) {
			mstar_strncat(info->result, MSTAR_CONNECT_TEST_PASS,
				      TS_RAWDATA_RESULT_MAX);
		} else {
			mstar_strncat(info->result, MSTAR_CONNECT_TEST_FAIL,
				      TS_RAWDATA_RESULT_MAX);
		}
		if (mp_test_result->nDeltacResult == ITO_TEST_OK) {
			mstar_strncat(info->result, MSTAR_ALLNODE_TEST_PASS,
				      TS_RAWDATA_RESULT_MAX);
		} else {
			mstar_strncat(info->result, MSTAR_ALLNODE_TEST_FAIL,
				      TS_RAWDATA_RESULT_MAX);
		}
		if (mp_test_result->nOpenResult == ITO_TEST_OK) {
			mstar_strncat(info->result, MSTAR_OPEN_PASS,
				      TS_RAWDATA_RESULT_MAX);
		} else {
			mstar_strncat(info->result, MSTAR_OPEN_FAIL,
				      TS_RAWDATA_RESULT_MAX);
		}
		if (mp_test_result->nShortResult == ITO_TEST_OK) {
			mstar_strncat(info->result, MSTAR_SHORT_TEST_PASS,
				      TS_RAWDATA_RESULT_MAX);
		} else {
			mstar_strncat(info->result, MSTAR_SHORT_TEST_FAIL,
				      TS_RAWDATA_RESULT_MAX);
		}
		if (mp_test_result->nUniformityResult == ITO_TEST_OK) {
			mstar_strncat(info->result, MSTAR_UNIFORMITY_TEST_PASS,
				      TS_RAWDATA_RESULT_MAX);
		} else {
			mstar_strncat(info->result, MSTAR_UNIFORMITY_TEST_FAIL,
				      TS_RAWDATA_RESULT_MAX);
		}
	} else {
		TS_LOG_ERR(
			"%s, Failed to allocate mem force to set test fail\n",
			__func__);
		mstar_strncat(info->result, MSTAR_CONNECT_TEST_PASS,
			      TS_RAWDATA_RESULT_MAX);
		mstar_strncat(info->result, MSTAR_ALLNODE_TEST_FAIL,
...
void mp_send_data2kit(struct ts_rawdata_info *info)
{
	...
	if ((!ERR_ALLOC_MEM(mp_test_result)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->pOpenResultData)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->pGolden_CH)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->pGolden_CH_Max_Avg)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->pShortResultData)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->puniformity_deltac)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->pdelta_LR_buf)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->pdelta_UD_buf)) &&
	    (!ERR_ALLOC_MEM(mp_test_result->pborder_ratio_buf))) {
		for (i = 0; i < drv_num * sense_num; i++) {
			info->buff[buff_index++] =
				mp_test_result->pOpenResultData[i];
		}
		if (mp_test_result->nOpenResult == -1) {
			mp_dump_data(mp_test_result->pOpenResultData,
				     drv_num * sense_num, -32, 10, drv_num,
				     "pOpenResultData", 1);
		}
		for (i = 0; i < drv_num * sense_num; i++) {
			info->buff[buff_index++] =
		...
		for (i = 0; i < drv_num + sense_num; i++) {
			info->buff[buff_index++] =
				mp_test_result->pShortResultData[i];
		}
		if (mp_test_result->nShortResult == -1) {
			mp_dump_data(mp_test_result->pShortResultData,
				     MAX_SUBFRAMES_30XX * MAX_AFENUMs_30XX, -32,
				     10, 14, "pShortResultData", 1);
		}
		/* Uniformity deltac */
		for (i = 0; i < drv_num * sense_num; i++) {
			info->buff[buff_index++] =
				mp_test_result->puniformity_deltac[i];
		}
		/* Uniformity Left-Right */
		for (i = 0; i < drv_num * sense_num; i++) {
			info->buff[buff_index++] =
				mp_test_result->pdelta_LR_buf[i];
		}
		/* Uniformity Down-Up */
		for (i = 0; i < drv_num * sense_num; i++) {
			info->buff[buff_index++] =
				mp_test_result->pdelta_UD_buf[i];
		}
		/* Uniformity border ratio */
		for (i = 0; i < 6 * sense_num; i++) {
			info->buff[buff_index++] =
				mp_test_result->pborder_ratio_buf[i];
		}
	} else {
		for (i = 0; i < drv_num * sense_num; i++) {
...
int startMPTest(int nChipType, char *pFilePath)
{
	...
	ms_ini_file_data = (ST_INI_FILE_DATA *)vmalloc(
		PARSER_MAX_KEY_NUM * sizeof(ST_INI_FILE_DATA));
	if (ERR_ALLOC_MEM(ms_ini_file_data)) {
		res = -1;
...
int mstar_get_raw_data(struct ts_rawdata_info *info,
		       struct ts_cmd_node *out_cmd)
{
	...
	if (tskit_mstar_data->fw_only_depend_on_lcd) {
		snprintf(file_name, sizeof(file_name),
			 "%s_%s_%s_%s_settings.ini",
			 tskit_mstar_data->mstar_chip_data->ts_platform_data
				 ->product_name,
	...
	snprintf(ini_path, sizeof(ini_path), "%s%s", INI_PATH_PERF_ODM,
		 file_name);
