...
extern int32_t novatek_ts_kit_read(uint16_t address, uint8_t *buf,
				   uint16_t len);
extern int32_t novatek_ts_kit_write(uint16_t address, uint8_t *buf,
				    uint16_t len);
...
int32_t Nova_Resume_PD(void)
{
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"Resume_PD: Check 0xAA (Resume Command) error, ret:%d\n",
			ret);
		return ret;
	}
	if (buf[1] != 0xAA) {
		TS_LOG_ERR(
			"Resume_PD: Check 0xAA (Resume Command) error, status=0x%02X\n",
			buf[1]);
		return -1;
	}
	msleep(10);
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_bin_size > (i * SIZE_64KB)) {
			...
			while (1) {
				...
				buf[0] = 0x00;
				buf[1] = 0x00;
				ret = novatek_ts_kit_read(I2C_HW_Address, buf,
							  2);
				if (ret < 0) {
					TS_LOG_ERR(
						"%s: Check 0xAA (Fast Read Command) error!!(%d)\n",
						__func__, ret);
					return ret;
				}
				if (buf[1] == 0xAA) {
				...
				if (unlikely(retry > 5)) {
					TS_LOG_ERR(
						"%s: Check 0xAA (Fast Read Command) failed, buf[1]=0x%02X, retry=%d\n",
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"%s: Read Checksum (write addr high byte & middle byte) error!!(%d)\n",
					__func__, ret);
				return ret;
			}
			// Read Checksum
...
int32_t Nova_Init_BootLoader(void)
{
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"Init_BootLoader: Inittial Flash Block error,ret:%d\n",
			ret);
		return ret;
	}
	msleep(5);
	...
	if (ret < 0) {
		TS_LOG_ERR(
			"Init_BootLoader: Check 0xAA (Inittial Flash Block) error,ret:%d\n",
			ret);
		return ret;
	}
	if (buf[1] != 0xAA) {
		TS_LOG_ERR(
			"Init_BootLoader: Check 0xAA (Inittial Flash Block) error, status=0x%02X\n",
			buf[1]);
		return -1;
	}
...
static int32_t Erase_Flash(void)
{
	...
	ret = novatek_ts_kit_write(I2C_HW_Address, buf,
				   NVTTDDI_TWO_BYTES_LENGTH);
	if (ret < NO_ERR) {
		TS_LOG_ERR(
			"%s: Write Enable (for Write Status Register) error!!(%d)\n",
			__func__, ret);
		return ret;
	}
	...
	if (ret < NO_ERR) {
		TS_LOG_ERR(
			"%s: Check 0xAA (Write Enable for Write Status Register) error!!(%d)\n",
			__func__, ret);
		return ret;
	}
	if (buf[1] != NVTTDDI_DOUBLE_A_CMD) {
		TS_LOG_ERR(
			"%s: Check 0xAA (Write Enable for Write Status Register) error!! status=0x%02X\n",
			__func__, buf[1]);
		return NVTTDDI_ERR;
	}
	mdelay(NVTTDDI_DELAY_10_MS);
	...
	if (ret < NO_ERR) {
		TS_LOG_ERR(
			"%s: Check 0xAA (Write Status Register) error!!(%d)\n",
			__func__, ret);
		return ret;
	}
	if (buf[1] != NVTTDDI_DOUBLE_A_CMD) {
		TS_LOG_ERR(
			"%s: Check 0xAA (Write Status Register) error!! status=0x%02X\n",
			__func__, buf[1]);
		return NVTTDDI_ERR;
	}
	mdelay(NVTTDDI_DELAY_10_MS);
	...
	while (1) {
		...
		if (ret < NO_ERR) {
			TS_LOG_ERR(
				"%s: Read Status (for Write Status Register) error!!(%d)\n",
				__func__, ret);
			return ret;
		}
		...
		if (ret < NO_ERR) {
			TS_LOG_ERR(
				"%s: Check 0xAA (Read Status for Write Status Register) error!!(%d)\n",
				__func__, ret);
			return ret;
		}
		if ((buf[1] == NVTTDDI_DOUBLE_A_CMD) &&
		...
		if (unlikely(retry > NVTTDDI_RETRY_5_TIMES)) {
			TS_LOG_ERR(
				"%s: Check 0xAA (Read Status for Write Status Register) failed, buf[1]=0x%02X, buf[2]=0x%02X, retry=%d\n",
	...
	for (i = 0; i < count; i++) {
		...
		if (ret < 0) {
			TS_LOG_ERR(
				"%s: Check 0xAA (Write Enable) error!!(%d,%d)\n",
				__func__, ret, i);
			return ret;
		}
		if (buf[1] != 0xAA) {
		...
		while (1) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"%s: Check 0xAA (Sector Erase) error!!(%d,%d)\n",
					__func__, ret, i);
				return ret;
			}
			if (buf[1] == 0xAA) {
		...
		while (1) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"%s: Check 0xAA (Read Status) error!!(%d,%d)\n",
					__func__, ret, i);
				return ret;
			}
...
static int32_t Write_Flash(void)
{
	...
	for (i = 0; i < count; i++) {
		...
		for (j = 0; j < min(fw_entry->size - i * NVT_ONE_PAGE_SIZE,
				    (size_t)NVT_ONE_PAGE_SIZE);
		     j += 32) {
		...
		buf[4] = (Flash_Address & 0xFF);
		buf[5] = 0x00;
		buf[6] = min(fw_entry->size - Flash_Address,
			     (size_t)NVT_ONE_PAGE_SIZE) -
			 1;
		buf[7] = tmpvalue;
		...
		while (1) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"%s: Check 0xAA (Read Status) error!!(%d)\n",
					__func__, ret);
				return ret;
			}
			if (((buf[1] == 0xAA) && (buf[2] == 0x00)) ||
...
static int32_t Update_Firmware(void)
{
	...
	ret = Nova_Resume_PD();
	if (ret) {
		nvt_ts->chip_data->ts_platform_data->dsm_info
			.constraints_update_status = Nova_Resume_PD_fail;
	...
	if (ret) {
		nvt_ts->chip_data->ts_platform_data->dsm_info
			.constraints_update_status = Erase_Flash_fail;
	...
	if (ret) {
		nvt_ts->chip_data->ts_platform_data->dsm_info
			.constraints_update_status = Write_Flash_fail;
	...
	if (ret) {
		nvt_ts->chip_data->ts_platform_data->dsm_info
			.constraints_update_status = Verify_Flash_fail;
...
int32_t nvt_kit_fw_update_boot(char *file_name)
{
	...
	if (!g_nava_sd_force_update) {
		...
		ret = Check_CheckSum();
		//--------------------------------------------------------------------------------
		if (((ret == 0) && (Check_FW_Ver() == 0)) ||
		    (ret <
		     0)) { // (ic fw ver check failed) && (bin fw version > ic fw version)
			TS_LOG_INFO(
				"%s: firmware version not match or checksum failed.updata fw\n",
				NVT_TAG);
			ret = Update_Firmware();
			if (ret) {
				TS_LOG_INFO(
					"%s: updata firmware failed dmd report\n",
					NVT_TAG);
			}
		} else {
			TS_LOG_INFO(
				"%s: read firmware checksum match or IC FW version bigger, no need update\n",
				NVT_TAG);
			// Bootloader Reset
			ret = 0;
			nvt_kit_bootloader_reset();
