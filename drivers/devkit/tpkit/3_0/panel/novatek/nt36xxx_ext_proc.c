...
*******************************************************/
static ssize_t nvt_flash_i2c_read(struct file *file, char __user *buff,
				  size_t count, loff_t *offp)
{
	...
	if (i2c_wr == 0) { //I2C write
		...
		nvt_ts->chip_data->ts_platform_data->client->addr =
			(str[0] & 0x7F);

		ret = nvt_ts->chip_data->ts_platform_data->bops->bus_write(
			&str[2], str[1]);
		if (ret < 0)
...
return:
	Executive outcomes. 2---succeed. -5,-14---failed.
*******************************************************/
static ssize_t nvt_flash_spi_read(struct file *file, char __user *buff,
				  size_t count, loff_t *offp)
{
	uint8_t *str = NULL;
	int32_t ret = NO_ERR;
	...
	if (spi_wr == NVTWRITE) { //SPI write
		...
		ret = novatek_ts_kit_write(I2C_FW_Address, &str[2],
					   ((str[0] & 0x7F) << 8) | str[1]);
		if (ret < 0) {
			TS_LOG_ERR("%s: error, bus_write fail, ret=%d\n",
				   NVT_TAG, ret);
		...
		ret = novatek_ts_kit_read(I2C_FW_Address, &str[2],
					  (((str[0] & 0x7F) << 8) | str[1]));
		if (ret < 0) {
			TS_LOG_ERR("%s: error, bus_read fail, ret=%d\n",
				   NVT_TAG, ret);
...
int32_t nvt_kit_flash_proc_init(void)
{
	if (nvt_ts->btype == TS_BUS_I2C) {
		NVT_proc_entry = proc_create(DEVICE_I2C_NAME, 0444, NULL,
					     &nvt_flash_i2c_fops);
	} else if (nvt_ts->btype == TS_BUS_SPI) {
		NVT_proc_entry = proc_create(DEVICE_SPI_NAME, 0444, NULL,
					     &nvt_flash_spi_fops);
	}
	...
	if (nvt_ts->btype == TS_BUS_I2C) {
		...
		NVT_TRANSFER_LENGTH =
			SPI_TRANSFER_LENGTH; //Support SPI transfer length
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		TS_LOG_ERR(
			"kit_get_fw_info:FW info is broken! fw_ver=0x%02X, ~ fw_ver=0x%02X\n",
			buf[1], buf[2]);
		x_num = 0;
		y_num = 0;
		button_num = 0;
...
void nvt_kit_read_mdata(uint32_t xdata_addr)
{
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / NVT_TRANSFER_LENGTH);
		     j++) {
			if (nvt_ts->btype == TS_BUS_SPI) {
				//---change xdata index---
				nvt_set_page(head_addr +
					     (XDATA_SECTOR_SIZE * i) +
					     (NVT_TRANSFER_LENGTH * j));
			}

			//---read data---
			buf[0] = NVT_TRANSFER_LENGTH * j;
			ret = novatek_ts_kit_read(I2C_FW_Address, buf,
						  NVT_TRANSFER_LENGTH + 1);
			if (ret) {
	...
	if (residual_len != 0) {
		...
		for (j = 0; j < (residual_len / NVT_TRANSFER_LENGTH + 1); j++) {
			if (nvt_ts->btype == TS_BUS_SPI) {
				//---change xdata index---
				nvt_set_page(xdata_addr + data_len -
					     residual_len +
					     (NVT_TRANSFER_LENGTH * j));
			}
	...
	for (i = 0; i < (data_len / 2); i++) {
		xdata[i] = (xdata_tmp[dummy_len + i * 2] +
...
static const struct seq_operations nvt_seq_ops = {.start = c_start,
						  .next = c_next,
						  .stop = c_stop,
						  .show = c_show};
...
int32_t nvt_kit_extra_proc_init(void)
{
	...
	if (NVT_proc_fw_version_entry == NULL) {
		TS_LOG_ERR(
			"kit_extra_proc_init: create proc/fw_version Failed!\n");
		return -ENOMEM;
	} else {
		TS_LOG_INFO(
			"kit_extra_proc_init: create proc/fw_version Succeeded!\n");
	}
	...
	if (NVT_proc_baseline_entry == NULL) {
		TS_LOG_ERR(
			"kit_extra_proc_init: create proc/baseline Failed!\n");
		return -ENOMEM;
	} else {
		TS_LOG_INFO(
			"kit_extra_proc_init: create proc/baseline Succeeded!\n");
	}
	...
	if (NVT_proc_raw_entry == NULL) {
		...
		TS_LOG_INFO(
			"kit_extra_proc_init: create proc/raw Succeeded!\n");
	}
	...
	if (NVT_proc_diff_entry == NULL) {
		...
		TS_LOG_INFO(
			"kit_extra_proc_init: create proc/diff Succeeded!\n");
	}
