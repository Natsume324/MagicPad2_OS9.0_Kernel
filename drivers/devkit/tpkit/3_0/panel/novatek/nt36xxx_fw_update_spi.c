...
static int32_t nvt_download_init(void)
{
	...
	if (fwbuf == NULL) {
		fwbuf = (uint8_t *)kzalloc((nvt_ts->bus_transfer_len + 1),
					   GFP_KERNEL);
		if (fwbuf == NULL) {
	...
	while (1) {
		...
		if (nvt_ts->chip_data->ic_type == NT36675) {
			...
			if ((*(fw_entry->data + fw_need_write_size -
			       FLASH_SECTOR_SIZE) +
			     *(fw_entry->data + fw_need_write_size -
			       FLASH_SECTOR_SIZE + 1)) != 0xFF) {
				TS_LOG_ERR(
					"FW_VER + FW_VER_BAR should be 0xFF\n");
				ret = -ENOEXEC;
				goto invalid;
			}
		} else {
			...
			if ((*(fw_entry->data + FW_BIN_VER_OFFSET) +
			    ...
			    0xFF) {
				TS_LOG_ERR(
					"bin file FW_VER + FW_VER_BAR should be 0xFF!\n");
				TS_LOG_ERR(
					"FW_VER=0x%02X, FW_VER_BAR=0x%02X\n",
					*(fw_entry->data + FW_BIN_VER_OFFSET),
					*(fw_entry->data +
					  FW_BIN_VER_BAR_OFFSET));
				ret = -ENOEXEC;
				goto invalid;
			}
		}
...
static int32_t nvt_dump_partition(void)
{
	...
	for (list = 0; list < partition; list++) {
		...
		for (i = 0; i < count; i++) {
			...
			if (ret < 0) {
				TS_LOG_ERR(
					"nvt_read_ram_and_save_file failed, ret = %d\n",
					ret);
				goto out;
			}
...
static int32_t nvt_write_firmware(const u8 *fwdata, size_t fwsize)
{
	...
	for (list = 0; list < partition; list++) {
		...
		for (i = 0; i < count; i++) {
			len = (size < nvt_ts->bus_transfer_len) ?
				      size :
			...
			ret = novatek_ts_kit_write(I2C_FW_Address, fwbuf,
						   len + 1);
			if (ret) {
