...
static char mmitest_result[TS_RAWDATA_RESULT_MAX] = {
...
extern int32_t novatek_ts_kit_read(uint16_t i2c_addr, uint8_t *buf,
				   uint16_t len);
extern int32_t novatek_ts_kit_dummy_read(uint16_t i2c_addr);
extern int32_t novatek_ts_kit_write(uint16_t i2c_addr, uint8_t *buf,
				    uint16_t len);
extern void nvt_kit_sw_reset_idle(void);
extern void nvt_kit_bootloader_reset(void);
extern int32_t
nvt_kit_check_fw_reset_state(RST_COMPLETE_STATE check_reset_state);
...
static int32_t nvt_load_mp_ctrlram_bin(void)
{
	...
	snprintf(file_path, sizeof(file_path) - ONE_SIZE,
		 "/product/etc/firmware/ts/%s_short_open.bin",
		 novatek_kit_project_id);
#else
	snprintf(file_path, sizeof(file_path) - ONE_SIZE,
		 "/odm/etc/firmware/ts/%s_short_open.bin",
		 novatek_kit_project_id);
#endif
	...
	if (!retval) {
		...
		if (read_ret > 0) {
			...
			CtrlRAM_test_cmd_num =
				(int32_t)((int32_t)stat.size / 4);
			if (System_Init_CTRLRAMTableStartAddress != 0) {
				ctrlram_cur_addr =
					System_Init_CTRLRAMTableStartAddress;
			} else {
				TS_LOG_ERR(
					"%s: System_Init_CTRLRAMTableStartAddress is not initialized!\n",
					NVT_TAG);
				retval = -5;
				goto exit_free;
			}
			...
			CtrlRAM_test_cmd = (struct test_cmd *)vmalloc(
				CtrlRAM_test_cmd_num * sizeof(struct test_cmd));
			if (!CtrlRAM_test_cmd) {
				TS_LOG_ERR(
					"%s: vmalloc for CtrlRAM_test_cmd failed\n",
					NVT_TAG);
				retval = -ENOMEM;
				goto exit_free;
			}
			...
			for (i = 0; i < CtrlRAM_test_cmd_num; i++) {
				...
				CtrlRAM_test_cmd[i].data[1] =
					*(ptr + (4 * i) + 1);
				CtrlRAM_test_cmd[i].data[2] =
					*(ptr + (4 * i) + 2);
				CtrlRAM_test_cmd[i].data[3] =
					*(ptr + (4 * i) + 3);
			...
			TS_LOG_ERR(
				"%s:ret=%d,read=%d,fbufp=%pK,stat.size=%lld\n",
...
static int32_t nvt_set_adc_oper(void)
{
	...
	for (i = 0; i < retry_adc_oper; i++) {
		...
		if (ret) {
			TS_LOG_INFO(
				"%s: write i2c cmds to set ADC operation FAIL\n",
				NVT_TAG);
		}
		...
		if (ret) {
			TS_LOG_INFO(
				"%s: write i2c cmds to clear ADC operation FAIL\n",
				NVT_TAG);
		}
		...
		if (ret) {
			TS_LOG_INFO(
				"%s: write i2c cmds to set ADC operation 2rd FAIL\n",
				NVT_TAG);
		}
...
static void nvt_write_test_cmd(struct test_cmd *cmds, int32_t cmd_num)
{
	...
	for (i = 0; i < cmd_num; i++) {
		...
		ret = novatek_ts_kit_write(I2C_FW_Address, buf,
					   1 + cmds[i].len);
		if (ret) {
...
static void nvt_SwitchGlobalTable(rawdata_type_e rawdata_type)
{
	if (rawdata_type == E_RawdataType_Short) {
		nvt_set_memory(System_Init_CTRLRAMTableStartAddress,
			       (uint8_t)(TableType0_GLOBAL_Addr & 0xFF));
		nvt_set_memory(System_Init_CTRLRAMTableStartAddress + 1,
			       (uint8_t)((TableType0_GLOBAL_Addr & 0xFF00) >>
					 8));
	} else if (rawdata_type == E_RawdataType_Open) {
		nvt_set_memory(System_Init_CTRLRAMTableStartAddress,
			       (uint8_t)(TableType1_GLOBAL_Addr & 0xFF));
		nvt_set_memory(System_Init_CTRLRAMTableStartAddress + 1,
			       (uint8_t)((TableType1_GLOBAL_Addr & 0xFF00) >>
					 8));
	} else {
		// do nothing
	}
...
static int32_t nvt_mp_Initial(rawdata_type_e rawdata_type)
{
	...
	nvt_set_memory(
		0x1F448,
		(uint8_t)((System_Init_CTRLRAMTableStartAddress - 0x10000) &
			  0xFF));
	nvt_set_memory(
		0x1F449,
		(uint8_t)(((System_Init_CTRLRAMTableStartAddress - 0x10000) >>
...
int32_t Nova_OffsetToReg(uint32_t addr, uint32_t *offset_data, uint32_t afe_cnt)
{
	...
	for (col = 0; col < 9; col++) {
		...
		for (i = 0; i < (OFFSET_TABLE_SIZE / 4); i++) {
			reg_data[col * OFFSET_TABLE_SIZE + i * 4 + 0] =
				(uint8_t)((offset_data[rawdata_cnt + 0] >> 0) &
					  0xFF);
			reg_data[col * OFFSET_TABLE_SIZE + i * 4 + 1] =
				(uint8_t)(((offset_data[rawdata_cnt + 0] >> 8) &
					   0x03) |
					  ((offset_data[rawdata_cnt + 1] << 2) &
					   0xFC));
			reg_data[col * OFFSET_TABLE_SIZE + i * 4 + 2] =
				(uint8_t)(((offset_data[rawdata_cnt + 1] >> 6) &
					   0x0F) |
					  ((offset_data[rawdata_cnt + 2] << 4) &
					   0xF0));
			reg_data[col * OFFSET_TABLE_SIZE + i * 4 + 3] =
				(uint8_t)(((offset_data[rawdata_cnt + 2] >> 4) &
					   0x3F));
			rawdata_cnt += 3;
		}
		...
		memcpy(RegData_test_cmd.data,
		       reg_data + col * OFFSET_TABLE_SIZE,
		       OFFSET_TABLE_SIZE / 2);
...
static int32_t nvt_read_open(void)
{
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		//---change xdata index---
		nvt_set_page(TableType1_GLOBAL0_RAW_BASE_ADDR +
			     y * IC_X_CFG_SIZE * 2);
		//---read data---
		buf[0] = (uint8_t)((TableType1_GLOBAL0_RAW_BASE_ADDR +
				    y * IC_X_CFG_SIZE * 2) &
				   0xFF);
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		for (x = 0; x < IC_X_CFG_SIZE; x++) {
			if ((AIN_Y[y] != 0xFF) && (AIN_X[x] != 0xFF)) {
				RawData_Open[AIN_Y[y] * X_Channel + AIN_X[x]] =
	...
	for (y = 0; y < Y_Channel; y++) {
		for (x = 0; x < X_Channel; x++) {
			...
			RawData_Open[y * X_Channel + x] =
				OpenRawToCS_pF(RawData_Open[y * X_Channel + x]);
		}
		printk("\n");
	}
	printk("\n");
...
static int8_t nvt_switch_FreqHopEnDis(uint8_t FreqHopEnDis)
{
	...
	if (unlikely(retry == 20)) {
		TS_LOG_ERR(
			"%s: switch FreqHopEnDis 0x%02X failed, buf[1]=0x%02X\n",
...
*/
	if (nvt_kit_get_fw_pipe() == 0)
		nvt_kit_read_mdata(nvt_ts->mmap->RAW_PIPE0_ADDR,
				   nvt_ts->mmap->RAW_BTN_PIPE0_ADDR);
	else
		nvt_kit_read_mdata(nvt_ts->mmap->RAW_PIPE1_ADDR,
				   nvt_ts->mmap->RAW_BTN_PIPE1_ADDR);
...
*/
	if (nvt_kit_get_fw_pipe() == 0)
		nvt_kit_read_mdata(nvt_ts->mmap->DIFF_PIPE1_ADDR,
				   nvt_ts->mmap->DIFF_BTN_PIPE1_ADDR);
	else
		nvt_kit_read_mdata(nvt_ts->mmap->DIFF_PIPE0_ADDR,
				   nvt_ts->mmap->DIFF_BTN_PIPE0_ADDR);
...
static int32_t nvt_polling_hand_shake_status(void)
{
	...
	if (i >= retry) {
		TS_LOG_ERR(
			"%s: polling hand shake status failed, buf[1]=0x%02X\n",
			NVT_TAG, buf[1]);
...
static int32_t nvt_tddi_read_fw_noise(int32_t *xdata)
{
	...
	for (y = NO_ERR; y < y_num; y++) {
		for (x = NO_ERR; x < x_num; x++) {
			...
			RawData_Diff_Max =
				(int8_t)(MIDDLE_EIGHT_BITS(xdata[iArrayIndex]));
			RawData_Diff_Min =
				(int8_t)(LOW_EIGHT_BITS(xdata[iArrayIndex]));
			if (NVT_TDDI_ABS(RawData_Diff_Max) >
			    NVT_TDDI_ABS(RawData_Diff_Min)) {
...
static int32_t nvt_tddi_read_fw_open(void)
{
	...
	rawdata_buf = (uint8_t *)vmalloc(IC_X_CFG_SIZE * IC_Y_CFG_SIZE *
					 NVTTDDI_MULTIPLY_2_NUM);
	if (!rawdata_buf) {
		TS_LOG_ERR("%s: vmalloc for rawdata_buf failed!\n", NVT_TAG);
		return -ENOMEM;
	}
	...
	for (y = NO_ERR; y < IC_Y_CFG_SIZE; y++) {
		//---change xdata index---
		nvt_set_page(raw_pipe_addr +
			     y * IC_X_CFG_SIZE * NVTTDDI_MULTIPLY_2_NUM);
		//---read data---
		buf[0] = (uint8_t)(LOW_EIGHT_BITS(
			raw_pipe_addr +
			y * IC_X_CFG_SIZE * NVTTDDI_MULTIPLY_2_NUM));
		ret = novatek_ts_kit_read(
			I2C_FW_Address, buf,
			IC_X_CFG_SIZE * NVTTDDI_MULTIPLY_2_NUM +
				NVTTDDI_PLUS_ONE);
		if (ret) {
			TS_LOG_INFO("%s: read data FAIL\n", NVT_TAG);
		}
		memcpy(rawdata_buf + y * IC_X_CFG_SIZE * NVTTDDI_MULTIPLY_2_NUM,
		       buf + NVTTDDI_PLUS_ONE,
		       IC_X_CFG_SIZE * NVTTDDI_MULTIPLY_2_NUM);
	}

	for (y = NO_ERR; y < IC_Y_CFG_SIZE; y++) {
		for (x = NO_ERR; x < IC_X_CFG_SIZE; x++) {
			if ((AIN_Y[y] != NVTTDDI_DOUBLE_F_CMD) &&
			    (AIN_X[x] != NVTTDDI_DOUBLE_F_CMD)) {
				RawData_Open[AIN_Y[y] * X_Channel + AIN_X[x]] = (int16_t)((
					rawdata_buf[(y * IC_X_CFG_SIZE + x) *
						    NVTTDDI_MULTIPLY_2_NUM] +
					NVTTDDI_MULTIPLY_256_NUM *
						rawdata_buf
							[(y * IC_X_CFG_SIZE +
							  x) * NVTTDDI_MULTIPLY_2_NUM +
							 NVTTDDI_PLUS_ONE]));
			}
		}
	}
...
static int nvt_get_threshold_from_csvfile(uint64_t columns, uint64_t rows,
					  char *target_name,
					  struct get_csv_data *data)
{
	...
	if (nvt_ts->chip_data->hide_fw_name)
		snprintf(file_name, sizeof(file_name), "%s_raw.csv",
			 novatek_kit_project_id);
	else
	...
	TS_LOG_INFO(
		"threshold filename:%s, rows_size=%llu, columns_size=%llu,target_name = %s\n",
...
*******************************************************/
static int32_t nvt_rawdata_up_low(int32_t rawdata[], uint8_t RecordResult[],
				  uint8_t x_len, uint8_t y_len,
				  int32_t Upper_Lmt[], int32_t Lower_Lmt[])
{
	int32_t retval = 0;
	int32_t i = 0;
	int32_t j = 0;
	...
	if (FLAG_EXIST == nvt_ts->test_capacitance_via_csvfile &&
	    PASS == nvt_parse_csvifle_ready) {
		if (rawdata_size <= 0) {
			TS_LOG_ERR("%s, tx or rx is zero\n", NVT_TAG);
			retval = -1;
			goto exit;
		}
		rawdata_up = kzalloc(rawdata_size * sizeof(int32_t) +
					     sizeof(struct get_csv_data),
				     GFP_KERNEL);
		rawdata_low = kzalloc(rawdata_size * sizeof(int32_t) +
					      sizeof(struct get_csv_data),
				      GFP_KERNEL);
		...
		if (FAIL == nvt_get_threshold_from_csvfile(x_len, y_len,
							   parse_target_name_up,
							   rawdata_up) ||
		    FAIL == nvt_get_threshold_from_csvfile(
				    x_len, y_len, parse_target_name_low,
				    rawdata_low)) {
	...
	for (j = 0; j < y_len; j++) {
		for (i = 0; i < x_len; i++) {
			...
			if (rawdata[j * x_len + i] > Upper_Lmt[j * x_len + i]) {
				...
				retval = -1;
				TS_LOG_ERR(
					"%s, [%2d, %2d] Val is [%5d] ,limit is[%5d, %5d]",
					parse_target_name_up, i, j,
					rawdata[j * x_len + i],
					Lower_Lmt[j * x_len + i],
					Upper_Lmt[j * x_len + i]);
			}
...
*******************************************************/
static int32_t nvt_rawdata_delta(int32_t rawdata[], uint8_t x_len,
				 uint8_t y_len)
{
	int32_t retval = 0;
	int32_t i = 0;
	...
	if (FLAG_EXIST == nvt_ts->test_capacitance_via_csvfile) {
		...
		if (FAIL == nvt_get_threshold_from_csvfile(
				    x_len - 1, y_len, CSV_TP_CAP_RAW_DELTA_X,
				    rawdata_up) ||
		    FAIL == nvt_get_threshold_from_csvfile(
				    x_len, y_len - 1, CSV_TP_CAP_RAW_DELTA_Y,
				    rawdata_low)) {
			TS_LOG_ERR("%s:get threshold from csvfile failed\n",
				   NVT_TAG);
			retval = -1;
	...
	for (j = 0; j < y_len; j++) {
		for (i = 0; i < (x_len - 1); i++) {
			RecordResult_FWMutual_X_Delta[j * (x_len - 1) + i] =
				(rawdata[j * x_len + i] -
				 rawdata[j * x_len + (i + 1)]);
		}
	}
	...
	for (j = 0; j < y_len; j++) {
		for (i = 0; i < (x_len - 1); i++) {
			if (RecordResult_FWMutual_X_Delta[j * (x_len - 1) + i] >
			    0) {
				delta = RecordResult_FWMutual_X_Delta
					[j * (x_len - 1) + i];
			} else {
				delta = (0 - RecordResult_FWMutual_X_Delta
						     [j * (x_len - 1) + i]);
			}

			if (delta >
			    PS_Config_Lmt_FW_Rawdata_X_Delta[j * (x_len - 1) +
							     i]) {
				TS_LOG_ERR(
					"%s, [%2d, %2d] Val is [%5d] ,limit is[0, %5d]",
					CSV_TP_CAP_RAW_DELTA_X, i, j, delta,
	...
	for (j = 0; j < x_len; j++) {
		for (i = 0; i < (y_len - 1); i++) {
			RecordResult_FWMutual_Y_Delta[i * x_len + j] =
				(rawdata[i * x_len + j] -
				 rawdata[(i + 1) * x_len + j]);
		}
	}
	...
	for (j = 0; j < (y_len - 1); j++) {
		for (i = 0; i < x_len; i++) {
			if (RecordResult_FWMutual_Y_Delta[j * x_len + i] > 0) {
				delta = RecordResult_FWMutual_Y_Delta[j * x_len +
								      i];
			} else {
				delta = (0 -
					 RecordResult_FWMutual_Y_Delta[j * x_len +
								       i]);
			}

			if (delta >
			    PS_Config_Lmt_FW_Rawdata_Y_Delta[j * x_len + i]) {
				TS_LOG_ERR(
					"%s, [%2d, %2d] Val is [%5d] ,limit is[0, %5d]",
					CSV_TP_CAP_RAW_DELTA_Y, i, j, delta,
			       ...
			       int32_t Rawdata_Limit_Negative)
{
	...
	if (FLAG_EXIST == nvt_ts->test_capacitance_via_csvfile &&
		...
		rawdata_low->size = rawdata_size;
		if (FAIL == nvt_get_threshold_from_csvfile(
				    TEST_LMT_SIZE, TEST_LMT_SIZE,
				    parse_target_name_up, rawdata_up) ||
		    FAIL == nvt_get_threshold_from_csvfile(
				    TEST_LMT_SIZE, TEST_LMT_SIZE,
				    parse_target_name_low, rawdata_low)) {
			TS_LOG_ERR("%s: get threshold from csvfile failed\n",
				   __func__);
			isPass = false;
			goto exit;
		...
		for (i = 0; i < x_ch; i++) {
			...
			if (rawdata[iArrayIndex] > iTolHighBound) {
				...
				TS_LOG_ERR(
					"%s, [%2d, %2d] Val is [%5d] ,limit is[%5d, %5d]",
					parse_target_name_up, i, j,
					rawdata[iArrayIndex], iTolLowBound,
					iTolHighBound);
			}
...
static int mmi_add_static_data(void)
{
	...
	snprintf((mmitest_result + i), TS_RAWDATA_RESULT_MAX - i,
		 "[%5d,%5d,%5d]", nvt_get_avg(RawData_FWMutual),
		 nvt_get_max(RawData_FWMutual), nvt_get_min(RawData_FWMutual));
	...
	snprintf((mmitest_result + i), TS_RAWDATA_RESULT_MAX - i,
		 "[%5d,%5d,%5d]", nvt_get_avg(RawData_Diff),
		 nvt_get_max(RawData_Diff), nvt_get_min(RawData_Diff));
...
static void nvt_print_criteria(void)
{
	...
	nvt_print_lmt_array(PS_Config_Lmt_FW_Rawdata_X_Delta, (X_Channel - 1),
			    Y_Channel);
	printk("PS_Config_Lmt_FW_Rawdata_Y_Delta:\n");
	nvt_print_lmt_array(PS_Config_Lmt_FW_Rawdata_Y_Delta, X_Channel,
			    (Y_Channel - 1));
	...
	if (nvt_ts->open_test_by_fw > NO_ERR) {
		...
		nvt_print_lmt_array(PS_Config_Lmt_Open_Rawdata_P_A, X_Channel,
				    Y_Channel);
		printk("PS_Config_Lmt_Open_Rawdata_N_A:\n");
		nvt_print_lmt_array(PS_Config_Lmt_Open_Rawdata_N_A, X_Channel,
				    Y_Channel);
	} else {
...
static void nvt_kit_rawdata_test(struct ts_rawdata_info *info)
{
	if (nvt_read_raw(RawData_FWMutual) != 0) {
		...
		if (FLAG_EXIST == nvt_ts->test_capacitance_via_csvfile) {
			nvt_parse_csvifle_ready = set_parse_target_name(
				CSV_TP_CAP_RAW_MAX, CSV_TP_CAP_RAW_MIN);
		}
		TestResult_FWMutual = nvt_rawdata_up_low(
		...
		if (TestResult_FWMutual == -1) {
			TS_LOG_INFO(
				"%s: FW RAWDATA TEST FAIL! TestResult_FWMutual=%d\n",
				NVT_TAG, TestResult_FWMutual);
		} else {
			TS_LOG_INFO(
				"%s: FW RAWDATA TEST PASS! TestResult_FWMutual=%d\n",
				NVT_TAG, TestResult_FWMutual);
		}
	}
}
static void nvt_kit_CC_test(struct ts_rawdata_info *info)
{
	if (nvt_read_CC(RawData_FW_CC) != 0) {
		...
		if (FLAG_EXIST == nvt_ts->test_capacitance_via_csvfile) {
			nvt_parse_csvifle_ready = set_parse_target_name(
				CSV_TP_CAP_CC_MAX, CSV_TP_CAP_CC_MIN);
		}
		TestResult_FW_CC = nvt_rawdata_up_low(
			RawData_FW_CC, RecordResult_FW_CC, X_Channel, Y_Channel,
			PS_Config_Lmt_FW_CC_P, PS_Config_Lmt_FW_CC_N);
		if (TestResult_FW_CC == -1) {
			TS_LOG_INFO(
				"%s: FW CC TEST FAIL! TestResult_FW_CC=%d\n",
				NVT_TAG, TestResult_FW_CC);
		} else {
			TS_LOG_INFO(
				"%s: FW CC TEST PASS! TestResult_FW_CC=%d\n",
				NVT_TAG, TestResult_FW_CC);
		}
	}
}
static void nvt_kit_delta_test(struct ts_rawdata_info *info)
{
	if (nvt_rawdata_delta(RawData_FWMutual, X_Channel, Y_Channel) != 0) {
		...
		TS_LOG_INFO(
			"%s: XY Nearby Delta Test FAIL! TestResult_FWMutual_Delta=%d\n",
		...
		TS_LOG_INFO(
			"%s: XY Nearby Delta Test PASS! TestResult_FWMutual_Delta=%d\n",
			NVT_TAG, TestResult_FWMutual_Delta);
	}
}
static void nvt_kit_noise_test(struct ts_rawdata_info *info)
{
	...
	if (noise_ret != NO_ERR) {
		...
		if (FLAG_EXIST == nvt_ts->test_capacitance_via_csvfile) {
			nvt_parse_csvifle_ready = set_parse_target_name(
				CSV_TP_CAP_DIFF_MAX, CSV_TP_CAP_DIFF_MIN);
		}
		TestResult_FW_Diff = nvt_rawdata_up_low(
			RawData_Diff, RecordResult_FW_Diff, X_Channel,
			Y_Channel, PS_Config_Lmt_FW_Diff_P,
			PS_Config_Lmt_FW_Diff_N);
		if (TestResult_FW_Diff == -1) {
			TestResult_Noise = -1;
			TS_LOG_INFO(
				"%s: NOISE TEST FAIL! TestResult_Noise=%d\n",
				NVT_TAG, TestResult_Noise);
		} else {
			TestResult_Noise = 0;
			TS_LOG_INFO(
				"%s: NOISE TEST PASS! TestResult_Noise=%d\n",
				NVT_TAG, TestResult_Noise);
		}
	}
}
static void nvt_kit_short_test(struct ts_rawdata_info *info)
{
	if (nvt_read_fw_short(RawData_Short) != 0) {
		...
		if (FLAG_EXIST == nvt_ts->test_capacitance_via_csvfile) {
			nvt_parse_csvifle_ready = set_parse_target_name(
				CSV_TP_CAP_SHORT_MAX, CSV_TP_CAP_SHORT_MIN);
		}
		//---Self Test Check --- // 0:PASS, -1:FAIL
		TestResult_Short = nvt_rawdata_up_low(
		...
		if (TestResult_Short == -1) {
			TS_LOG_INFO(
				"%s: SHORT TEST FAIL! TestResult_Short=%d\n",
				NVT_TAG, TestResult_Short);
		} else {
			TS_LOG_INFO(
				"%s: SHORT TEST PASS! TestResult_Short=%d\n",
				NVT_TAG, TestResult_Short);
		}
	}
}
static void nvt_kit_open_test(struct ts_rawdata_info *info,
			      uint8_t x_channel_size, uint8_t y_channel_size)
{
	...
	if (open_ret != NO_ERR) {
		...
		if (nvt_ts->open_test_by_fw > NO_ERR) {
			if (FLAG_EXIST ==
			    nvt_ts->test_capacitance_via_csvfile) {
				nvt_parse_csvifle_ready = set_parse_target_name(
					CSV_TP_CAP_OPEN_MAX,
					CSV_TP_CAP_OPEN_MIN);
			}
			TestResult_Open = nvt_rawdata_up_low(
				RawData_Open, RecordResult_Open, X_Channel,
				Y_Channel, PS_Config_Lmt_Open_Rawdata_P_A,
				PS_Config_Lmt_Open_Rawdata_N_A);
		} else {
			TestResult_Open = RawDataTest_Sub(
				RawData_Open, RecordResult_Open, X_Channel,
...
int32_t nvt_kit_selftest(struct ts_rawdata_info *info)
{
	...
	if (nvt_ts->criteria_threshold_flag) {
		...
		info->used_size = X_Channel * Y_Channel * 5 +
				  CHANNEL_NUM; //4:rawdata+diff+short+open

		if (info->used_size <
		    TS_RAWDATA_BUFF_MAX) { //buff is enough for rawdata, diff, short and open

			bytes_of_array = X_Channel * Y_Channel * sizeof(int);
			memcpy(&info->buff[CHANNEL_NUM], RawData_FWMutual,
			       bytes_of_array);

			start_p = X_Channel * Y_Channel + CHANNEL_NUM;
			memcpy(&info->buff[start_p], RawData_Diff,
			       bytes_of_array);

			start_p = X_Channel * Y_Channel * 2 + CHANNEL_NUM;
			memcpy(&info->buff[start_p], RawData_Short,
			       bytes_of_array);

			start_p = X_Channel * Y_Channel * 3 + CHANNEL_NUM;
			memcpy(&info->buff[start_p], RawData_Open,
			       bytes_of_array);

			start_p = X_Channel * Y_Channel * 4 + CHANNEL_NUM;
			memcpy(&info->buff[start_p], RawData_FW_CC,
			       bytes_of_array);
		} else {
			TS_LOG_ERR(
				"%s: info->buff's size is not enough for rawdata, diff, short and open\n",
				NVT_TAG);
		}
	} else {
		info->buff[0] = X_Channel;
		info->buff[1] = Y_Channel;
		info->used_size = X_Channel * Y_Channel * 5 +
				  CHANNEL_NUM; //2:rawdata + diff

		if (info->used_size <
		    TS_RAWDATA_BUFF_MAX) { //buff is enough for rawdata, diff, short and open
			bytes_of_array =
				(X_Channel * (uint64_t)Y_Channel * sizeof(int));
			memcpy(&info->buff[CHANNEL_NUM], RawData_FWMutual,
			       bytes_of_array);
...
err_nvt_spi_read:
	...
	if (0 == strlen(mmitest_result) || strstr(mmitest_result, "F")) {
		strncat(mmitest_result, selftest_failed_reason,
			strlen(selftest_failed_reason));
	}
