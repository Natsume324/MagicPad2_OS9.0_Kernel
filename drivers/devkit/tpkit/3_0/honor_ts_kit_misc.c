...
static long ts_ioctl_get_fingers_info(unsigned long arg)
{
	...
	wait_event_interruptible(pdata->fingers_waitq,
				 (atomic_read(&pdata->fingers_waitq_flag) ==
				  AFT_WAITQ_WAKEUP));
	...
	if (atomic_read(&pdata->fingers_waitq_flag) == AFT_WAITQ_WAKEUP) {
		if (pdata->chip_data->fold_fingers_supported &&
			...
			ret = copy_to_user(
				argp, &pdata->fingers_send_aft_info,
				sizeof(pdata->fingers_send_aft_info));
		} else {
			copy_fold_to_normal_fingers(
				&n_fingers, &pdata->fingers_send_aft_info);
			       ...
			       struct ts_fingers *fingers)
{
	...
	if (waitq_flag == AFT_WAITQ_WAIT) {
		...
		aft_filtered_fingers =
			atomic_read(&pdata->last_aft_filtered_fingers);
	...
	if (atomic_read(&pdata->finger_pen_waitq_flag) == AFT_WAITQ_WAIT) {
		...
		if (finger_pen_flag == TS_ONLY_FINGER ||
			...
			for (id = 0; id < TS_MAX_FINGER; id++) {
				if ((finger_pen->report_finger_info.fingers[id]
					     .status &
				     TS_FINGER_PRESS)) {
					/* update input event from chip report */
					input_flag |= (1 << id);
			...
			if (finger_pen_flag == TS_FINGER_PEN)
				finger_pen->report_pen_info.tool
					.pressure = atomic_read(
					&pdata->last_aft_filtered_pen_pressure);
		} else {
			finger_pen->report_pen_info.tool.pressure = atomic_read(
				&pdata->last_aft_filtered_pen_pressure);
		...
		TS_LOG_DEBUG(
			"[MUTI_AFT] ts_finger_pen_algo_calibrate no wait\n");
	}
...
static int ts_ioctl_set_callback_events(unsigned long arg)
{
	...
	if (copy_from_user(
		    &g_ts_kit_platform_data.stylus3_callback_event, argp,
		    sizeof(g_ts_kit_platform_data.stylus3_callback_event))) {
		TS_LOG_ERR("%s Failed to copy_from_user\n", __func__);
		return -EFAULT;
	}
	TS_LOG_INFO("%s, eventClass=%d, eventCode=%d, extraInfo=%s\n", __func__,
...
static int ts_ioctl_set_stylus3_ack_freq(unsigned long arg)
{
	...
	if (copy_from_user(&g_ts_kit_platform_data.ack_freq, argp,
			   sizeof(g_ts_kit_platform_data.ack_freq))) {
		TS_LOG_ERR("%s Failed to copy_from_user\n", __func__);
		return -EFAULT;
	}
				    ...
				    unsigned long arg)
{
	...
	case INPUT_AFT_IOCTRL_CMD_SET_STYLUS3_CONNECT_STATUS:
		if (g_ts_kit_platform_data.feature_info.pen_info
			    .pen_change_protocol ==
		    SUPPORT_CHANGE_PEN_PROTOCOL) {
...
int ts_kit_misc_init(struct ts_kit_platform_data *pdata)
{
	...
	if (pdata->multi_panel_index != SINGLE_TOUCH_PANEL) {
		set_node_name = kzalloc(sizeof(char) * MULTI_PANEL_NODE_BUF_LEN,
					GFP_KERNEL);
		if (!set_node_name) {
		...
		get_node_name = kzalloc(sizeof(char) * MULTI_PANEL_NODE_BUF_LEN,
					GFP_KERNEL);
		if (!get_node_name) {
