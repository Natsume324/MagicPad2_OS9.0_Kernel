...
static void write_tp_color_to_nv(void)
{
	...
	if (!wait_for_completion_interruptible_timeout(
		    &nv_write_done, msecs_to_jiffies(NV_WRITE_TIMEOUT)))
...
static void read_tp_color_from_system(void)
{
	...
	if (!wait_for_completion_interruptible_timeout(
		    &nv_read_done, msecs_to_jiffies(NV_READ_TIMEOUT))) {
...
#if (defined CONFIG_HONOR_THP_MTK) || (defined CONFIG_HONOR_DEVKIT_MTK_3_0)
static struct device_attribute tp_color_file = __ATTR(
	tp_color_info, 0660, attr_get_tp_color_info, attr_set_tp_color_info);
