...
int read_tp_color_adapter(char *buf, int buf_size)
{
...
#ifdef CONFIG_HONOR_OEMINFO
	user_info = kzalloc(sizeof(struct oeminfo_info_user), GFP_KERNEL);
	if (!user_info) {
	...
	memcpy(buf, user_info->oeminfo_data,
	       (buf_size < user_info->valid_size) ? buf_size :
