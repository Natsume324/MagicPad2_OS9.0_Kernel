...
int ts_pen_open_confirm(struct ts_pens *pens)
{
	...
	if (g_ts_kit_platform_data.feature_info.wakeup_gesture_enable_info
		    .switch_value == STYLUS_WAKEUP_LOW_FREQENCY) {
		...
		if (pens->tool.pen_inrange_status == TS_PEN_OUT_RANGE) {
			...
			if (press_state != PEN_PRESS_STATE) {
				...
				interval_time = (cur_time.tv_sec -
						 pre_pen_up_time.tv_sec) *
							SENCOND_TO_MILLISECOND +
						(cur_time.tv_nsec -
						 pre_pen_up_time.tv_nsec) /
							SENCOND_TO_NANOSECOND;
				if ((interval_time > 0) &&
				    (interval_time <= interval_thr)) {
					start_check_flag = 0;
				} else {
					start_check_flag = 1;
			...
			if (start_check_flag) { /* can check */
				x = pens->tool.x;
				y = pens->tool.y;
				if ((pen_pos_x == ABNORMAL_VAL) &&
				    (pen_pos_y == ABNORMAL_VAL)) {
					pen_pos_x = x;
					pen_pos_y = y;
				} else {
					/* about 3mm length */
					leng_thr = g_ts_kit_platform_data
							   .chip_data->x_max *
						   PEN_MOV_INTERVAL / FHD_X_MAX;
					leng_thr *= leng_thr;
					mov_len = (pre_x - x) * (pre_x - x) +
						  (pre_y - y) * (pre_y - y);
					if (mov_len > leng_thr) {
						start_check_flag = 0;
						TS_LOG_INFO(
							"ts_pen_open_confirm_called, move_too_fast, THR=%d, length=%d\n",
							leng_thr, mov_len);
					} else {
						/* about 7mm length */
						leng_thr =
						...
						if (mov_len >= leng_thr) {
							TS_LOG_INFO(
								"ts_pen_open_confirm_called, THR=%d, length=%d\n",
								leng_thr,
								mov_len);
							...
							if ((interval_time >
								...
								start_check_flag =
									0;
								TS_LOG_INFO(
									"ts_pen_open_confirm_called, movetime_too_short, THR=%d, interval=%ld\n",
									MOVE_TIME_MIN,
									interval_time);
							} else {
								start_check_flag =
									0;
								open_pen_flag =
									1; /* need open pen switch */
								TS_LOG_ERR(
									"ts_pen_open_confirm_called, ret_flag=%d\n",
									open_pen_flag);
							}
						}
					}
	...
	TS_LOG_INFO(
		"ts_pen_open_confirm_called, start_check=%d, ret_flag=%d, z=%d\n",
