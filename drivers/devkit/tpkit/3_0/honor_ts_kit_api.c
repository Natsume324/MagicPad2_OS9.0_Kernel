...
void dump_fingers_info_debug(struct ts_finger *fingers, unsigned int len)
{
	int i = 0;
	...
	for (; i < g_ts_kit_platform_data.max_fingers; i++) {
		...
		TS_LOG_INFO("wx=%d wy=%d ewx=%d ewy=%d xer=%d yer=%d\n",
			    fingers[i].wx, fingers[i].wy, fingers[i].ewx,
			    fingers[i].ewy, fingers[i].xer, fingers[i].yer);
	}
}
		...
		list_for_each_entry(algo, &dev->algo_head, node) {
			if (algo->algo_index == i) {
				TS_LOG_DEBUG("algo :%s called\n",
					     algo->algo_name);
				algo->chip_algo_func(dev, in_finger,
						     out_finger);
...
out:
	memcpy(out_finger_pen, in_finger_pen, sizeof(*out_finger_pen));
	if (finger_pen_flag == TS_ONLY_FINGER)
		out_cmd->command = TS_REPORT_INPUT;
...
void ts_report_pen(struct ts_cmd_node *in_cmd)
{
	...
	if ((g_ts_kit_platform_data.pen_aft_enable_flag == SUPPORT_PEN_AFT) &&
	    (g_ts_kit_platform_data.aft_param.aft_enable_flag ==
	     TS_PEN_AFT_ENABLE))
		pens = &in_cmd->cmd_param.pub_params.report_touch_info
				.report_pen_info;
	if (!pens) {
...
void ts_report_input(struct ts_cmd_node *in_cmd)
{
	...
	if ((g_ts_kit_platform_data.pen_aft_enable_flag == SUPPORT_PEN_AFT) &&
	    (g_ts_kit_platform_data.aft_param.aft_enable_flag ==
	     TS_PEN_AFT_ENABLE))
		fingers = &in_cmd->cmd_param.pub_params.report_touch_info
				   .report_finger_info;
...
int ts_power_off_control_mode(int irq_id, struct ts_cmd_node *in_cmd)
{
	...
	case TS_SUSPEND_DEVICE: /* device power off or sleep */
		...
		if (pdata->aft_param.aft_enable_flag) {
			...
			if ((g_ts_kit_platform_data.pen_aft_enable_flag ==
			     SUPPORT_PEN_AFT) &&
			    (pdata->aft_param.aft_enable_flag ==
			     TS_PEN_AFT_ENABLE))
				atomic_set(
					&pdata->last_aft_filtered_pen_pressure,
					0);
	...
	case TS_RESUME_DEVICE: /* device power on or wakeup */
		...
		atomic_set(&pdata->power_state, TS_GOTO_WORK);
...
int ts_power_off_no_config(int irq_id, struct ts_cmd_node *in_cmd,
			   struct ts_cmd_node *out_cmd)
{
	int error = NO_ERR;
	struct ts_device_ops *ops = g_ts_kit_platform_data.chip_data->ops;
...
int ts_power_off_easy_wakeup_mode(int irq_id, struct ts_cmd_node *in_cmd,
				  struct ts_cmd_node *out_cmd)
{
	int error = NO_ERR;
	enum lcd_kit_ts_pm_type pm_type = in_cmd->cmd_param.pub_params.pm_type;
	...
	case TS_SUSPEND_DEVICE:
		...
		if (!dev->send_stylus_gesture_switch &&
			...
			out_cmd->cmd_param.prv_params =
				(void *)&g_ts_kit_platform_data.feature_info
...
static void ts_easy_wake_mode_change(struct ts_kit_device_data *dev)
{
	if (dev->easy_wakeup_info.easy_wakeup_gesture != TS_POWER_OFF_MODE) {
		...
		dev->easy_wakeup_info.easy_wakeup_gesture = TS_POWER_OFF_MODE;
	}
...
int ts_kit_power_control_notify(enum lcd_kit_ts_pm_type pm_type, int timeout)
{
	...
	if (ts_is_factory() && factory_extra_cmd->always_poweron_in_screenoff) {
	...
	if (g_ts_kit_platform_data.setting_multi_gesture_mode &&
		...
		if (dev->easy_wakeup_info.aod_mode_temp != TS_TOUCH_AOD_CLOSE) {
			dev->easy_wakeup_info.aod_touch_switch_ctrl =
				dev->easy_wakeup_info.aod_mode_temp;
			dev->easy_wakeup_info.aod_mode_temp =
				TS_TOUCH_AOD_CLOSE;
...
{
	if (panel_index != MAIN_TOUCH_PANEL && panel_index != SUB_TOUCH_PANEL) {
