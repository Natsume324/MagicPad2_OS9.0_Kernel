...
#include <uapi/linux/sched/types.h>
     ...
     (IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)))
...
extern int thp_project_id_provider(char *project_id, unsigned int len);
#endif
 ...
 */
__attribute__((weak)) int tpmodule_notifier_call_chain(unsigned long val,
						       void *v)
{
...
static void ts_set_sensibility(struct ts_cmd_node *in_cmd)
{
	...
	if (dev && dev->ops && dev->ops->chip_set_sensibility_cfg)
		dev->ops->chip_set_sensibility_cfg(
			in_cmd->cmd_param.pub_params.sensibility_cfg);
}
...
static int calibration_proc_show(struct seq_file *m, void *v)
{
	...
	/* shift 2D */
	seq_print_freq(m,
		       info->data + info->tx_num * info->rx_num * data_size +
			       info->rx_num * data_size *
				       data_size + /* shift noise */
...
static void rawdata_proc_printf_itocap_data(struct seq_file *m,
					    const struct ts_rawdata_info *info,
					    int row_size, int range_size)
{
	int i;
	int j;
					   ...
					   int row_size, int range_size)
{
	...
	for (index = 0; row_size * index + index_size < info->used_size;
	     index++) {
		if (index == 0)
			seq_puts(m, "rawdata begin\n"); /* print the title */
		for (index1 = 0; index1 < row_size; index1++)
			seq_printf(m, "%d,",
				   info->buff[index_size + row_size * index +
					 ...
					 struct ts_rawdata_info *info)
{
	...
	if (info->hybrid_buff_used_size > (index1 + tx_num + rx_num)) {
		...
		for (index = 0; index < index2; index++)
			seq_printf(m, "%d, ",
				   info->hybrid_buff[index + index1 + tx_num +
						     rx_num]);
						  ...
						  struct ts_rawdata_info *info)
{
	...
	for (index = 0; index < info->used_sharp_selcap_touch_delta_size;
	     index++)
...
static int rawdata_prade_cm_data(struct seq_file *m,
				 struct ts_rawdata_info *info, int row_size,
				 int range_size)
{
					  ...
					  int row_size, int range_size)
{
	...
	seq_puts(m,
		 "cm gradient(10x real value) begin\n"); /* print the title */
	seq_puts(m, "-rx:,");
	for (index1 = 0; index1 < row_size; index1++) {
		if (rd_index < info->used_size) {
			seq_printf(m, "%d,", info->buff[rd_index++]);
					...
					struct ts_rawdata_info *info)
{
	...
	for (index = 0; row_size * index + index_size < info->used_size_3d;
	     index++) {
		if (index == 0)
			/* print the title */
		...
		for (index1 = 0; index1 < row_size; index1++)
			/* print oneline */
			seq_printf(m, "%d,",
				   info->buff_3d[index_size + row_size * index +
						 index1]);
		seq_puts(m, "\n ");
...
static void rawdata_proc_self_cap_info(struct ts_rawdata_info_new *info,
				       struct seq_file *m,
				       struct ts_rawdata_newnodeinfo *datanode)
{
	int index;
				    ...
				    struct ts_rawdata_newnodeinfo *datanode)
{
	...
	for (index = 0; row_size * index < datanode->size; index++) {
		for (index1 = 0; (index1 < row_size) &&
				 ((row_size * index + index1) < datanode->size);
		     index1++)
			/* print oneline */
			seq_printf(m, "%d,",
				   ...
				   struct ts_rawdata_info_new *info)
{
	...
	list_for_each_entry(rawdatanode, &info->rawdata_head, node) {
		...
		if (rawdatanode->typeindex == RAW_DATA_TYPE_TRXDELTA &&
		    rawdatanode->size > 0) {
			rawdata_proc_tx_rx_delta_info(info, m, rawdatanode);
		} else if (rawdatanode->typeindex == RAW_DATA_TYPE_SELFCAP &&
			   rawdatanode->size > 0) {
			rawdata_proc_self_cap_info(info, m, rawdatanode);
		} else if (rawdatanode->typeindex ==
...
void rawdata_proc_freehook(void *infotemp)
{
	...
	if (info) {
		list_for_each_safe(pos, n, &info->rawdata_head) {
			rawdatanode = list_entry(
				pos, struct ts_rawdata_newnodeinfo, node);
...
static int rawdata_proc_for_newformat(struct seq_file *m, void *v)
{
	...
	if (g_ts_kit_platform_data.chip_data->is_direct_proc_cmd) {
		...
		if (g_ts_kit_platform_data.chip_data->rawdata_get_timeout)
			error = ts_kit_put_one_cmd(
				cmd, g_ts_kit_platform_data.chip_data
					     ->rawdata_get_timeout);
		else
			error = ts_kit_put_one_cmd(cmd, SHORT_SYNC_TIMEOUT);
	}
...
static int rawdata_proc_show(struct seq_file *m, void *v)
{
	...
	int trx_delta_test_support =
		g_ts_kit_platform_data.chip_data->trx_delta_test_support;
	int td43xx_test_support =
		g_ts_kit_platform_data.chip_data->td43xx_ee_short_test_support;
	struct ts_cmd_node *cmd = NULL;
	...
	TS_LOG_INFO("tx:%d, rx:%d, tx_rx_delta_size:%d\n",
		    ...
		    g_ts_kit_platform_data.chip_data->rx_num, tx_rx_delta_size);
	...
	if (td43xx_test_support) {
		info->td43xx_rt95_part_one =
			kzalloc(tx_rx_delta_size, GFP_KERNEL);
	...
	if (g_ts_kit_platform_data.chip_data->rawdata_arrange_type ==
		...
		ts_kit_rotate_rawdata_abcd2cbad(
			row_size, range_size, info->buff + INDEX_SIZE,
			g_ts_kit_platform_data.chip_data->rawdata_arrange_type);
		ts_kit_rotate_rawdata_abcd2cbad(
			row_size, range_size,
			info->buff + INDEX_SIZE + row_size * range_size,
			g_ts_kit_platform_data.chip_data->rawdata_arrange_type);
	...
	if (g_ts_kit_platform_data.chip_data->is_parade_solution == 0) {
		if (g_ts_kit_platform_data.chip_data
			...
			if (g_ts_kit_platform_data.chip_data->ops
				    ->chip_special_rawdata_proc_printf)
				g_ts_kit_platform_data.chip_data->ops
					->chip_special_rawdata_proc_printf(
						m, info, range_size, row_size);
		} else {
			rawdata_proc_printf(m, info, range_size, row_size);
		}
	} else {
		if (rawdata_proc_parade_printf(m, info, range_size, row_size) <
		    0)
			goto out;
	}
	...
	do {
		...
		if (ret == GET_HWLOCK_FAIL) {
			...
			if (time_after(time, timeout)) {
				TS_LOG_ERR(
					"i2c get hardware_mutex for completion timeout\n");
				return -ETIME;
			}
		}
...
int ts_i2c_write(u8 *buf, u16 length)
{
	...
	do {
		...
		if (ret == length) {
			...
			trace_touch(TOUCH_TRACE_I2C, TOUCH_TRACE_FUNC_OUT,
				    "write");
			return NO_ERR;
		} else {
...
int ts_i2c_read(u8 *reg_addr, u16 reg_len, u8 *buf, u16 len)
{
	...
	if (data->chip_data->is_i2c_one_byte) {
		...
		do {
			ret = i2c_transfer(data->client->adapter, msg_addr,
					   msg_len);
			if (ret == msg_len) {
				if (data->i2c_hwlock.tp_i2c_hwlock_flag)
					tp_i2c_release_hwlock();
...
static irqreturn_t ts_irq_handler(int irq, void *dev_id)
{
	...
	if (g_ts_kit_platform_data.chip_data->ops->chip_irq_top_half)
		error = g_ts_kit_platform_data.chip_data->ops->chip_irq_top_half(
			&cmd);
	...
	if (g_ts_kit_platform_data.chip_data->report_tui_enable) {
		...
		if (g_ts_kit_platform_data.i2c_driver_type !=
			...
			if ((dev->tui_special_feature_support &
			     TP_TUI_NEW_IRQ_MASK) == TP_TUI_NEW_IRQ_SUPPORT)
...
static int ts_charger_detect_cmd(enum ts_charger_state charger_state)
{
	...
	TS_LOG_INFO(
		"%s called, charger type: %d, [0 out, 1 in], supported: %d\n",
		__func__, charger_state,
		g_ts_kit_platform_data.feature_info.charger_info
			.charger_supported);
...
static void ts_kit_charger_notifier_register(void)
{
	...
	error = honor_register_notifier(
		&g_ts_kit_platform_data.charger_detect_notify, 1);
	if (error < 0) {
...
static void ts_kit_charger_notifier_register(void)
{
	...
	pdata->chr_consumer =
		charger_manager_get_by_name(&pdata->ts_dev->dev, "touchscreen");

	if (pdata->chr_consumer != NULL) {
		pdata->charger_detect_notify.notifier_call =
			charger_detect_notifier_callback;
		error = register_charger_manager_notifier(
			pdata->chr_consumer, &pdata->charger_detect_notify);
		if (error < 0) {
...
static int parse_spi_config(void)
{
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "spi-max-frequency",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "pl022,interface",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "pl022,com-mode",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "pl022,rx-level-trig",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "pl022,tx-level-trig",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "pl022,ctrl-len",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "pl022,wait-state",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "pl022,duplex",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "cs_reset_low_delay",
	...
	retval = of_property_read_u32(
		g_ts_kit_platform_data.node, "cs_reset_high_delay",
...
static void parse_extra_feature_config(void)
{
	...
	rc = of_property_read_u32(
		g_ts_kit_platform_data.node, "tui_another_irq_gpio_num",
...
static int parse_feature_config(void)
{
	...
	rc = of_property_read_u32(
		g_ts_kit_platform_data.node, "need_i2c_hwlock",
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node, "is_fold_device",
				  &g_ts_kit_platform_data.is_fold_device);
	if (rc)
		TS_LOG_ERR("is_fold_device read error\n");
	rc = of_property_read_u32(g_ts_kit_platform_data.node, "pen_aft_enable",
				  &value);
	if (rc) {
		value = 0;
	...
	rc = of_property_read_u32(
		g_ts_kit_platform_data.node, "aft_enable",
	...
	if (g_ts_kit_platform_data.aft_param.aft_enable_flag) {
		of_property_read_u32(
			g_ts_kit_platform_data.node, "drv_stop_width",
			&g_ts_kit_platform_data.aft_param.drv_stop_width);
		of_property_read_u32(
			g_ts_kit_platform_data.node, "lcd_width",
			&g_ts_kit_platform_data.aft_param.lcd_width);
		of_property_read_u32(
			g_ts_kit_platform_data.node, "lcd_height",
	...
	rc = of_property_read_u32(
		g_ts_kit_platform_data.node, "support_connect_check_gpio",
	...
	if (!rc && g_ts_kit_platform_data.support_connect_check_gpio == 1) {
		...
		g_ts_kit_platform_data.connect_check_gpio = of_get_named_gpio(
			g_ts_kit_platform_data.node, "connect_check_gpio", 0);
	}
#endif
	rc = of_property_read_u32(
		g_ts_kit_platform_data.node, "support_ap_shb_i2c_switch",
	...
	rc = of_property_read_u32(g_ts_kit_platform_data.node, "use_ap_gesture",
				  &g_ts_kit_platform_data.use_ap_gesture);
	if (rc)
	...
	if (rc) {
		g_ts_kit_platform_data.multi_panel_index = SINGLE_TOUCH_PANEL;
...
static int ts_kit_parse_config(void)
{
	...
	for (index = 0; index < tmp_buff_len && index < (MAX_STR_LEN - 1);
	     index++)
...
static void procfs_create_for_multi_panel(void)
{
	...
	TS_LOG_INFO("%s node_name:%s\n", __func__, node_name);
	proc_create((const char *)node_name, 0644, NULL,
...
static int ts_kit_create_sysfs(void)
{
	...
	if (g_ts_kit_platform_data.multi_panel_index == SINGLE_TOUCH_PANEL) {
		error = sysfs_create_link(
			NULL, &g_ts_kit_platform_data.ts_dev->dev.kobj,
			"touchscreen");
	} else {
	...
	if (error)
		...
		if ((strlen(dev->chip_name) > 0) &&
			...
			if (dsm_update_client_vendor_info(&dsm_tp))
				TS_LOG_ERR(
					"dsm update client_vendor_info is failed\n");
		} else {
...
static int ts_kit_input_tp_device_register(struct input_dev *dev)
{
	...
	if (data->chip_data->ops->chip_input_config)
		error = data->chip_data->ops->chip_input_config(
			g_ts_kit_platform_data.input_dev);
	if (error)
		goto err_free_dev;
...
static int ts_kit_input_pen_device_register(struct input_dev *dev)
{
	...
	if (ops && ops->chip_input_pen_config) {
		error = ops->chip_input_pen_config(
			g_ts_kit_platform_data.pen_dev);
		if (error)
			goto err_free_dev;
	}
...
static void tp_init_work_fn(struct work_struct *work)
{
	...
	while (i-- > 0) {
		...
		error = ts_kit_proc_command_directly(cmd);
		if (error != NO_ERR)
			TS_LOG_INFO("%s process init cmd %d error", __func__,
				    cmd->command);
	}
...
void ts_dmd_report(int dmd_num, const char *format, ...)
{
	...
	va_end(args);
	snprintf(
		report_buf, TS_CHIP_DMD_REPORT_SIZE - 1,
...
static void tp_tui_init(struct ts_kit_device_data *dev_data)
{
	...
	if (g_ts_kit_platform_data.multi_panel_index == SUB_TOUCH_PANEL)
		tui_data = &(tp_tui_data[SUB_TOUCH_PANEL]
				     .tp_tui_normalized_data.tpkit_tui_data);

	dev_data->tui_data = &tee_tui_data;

	if (g_ts_kit_platform_data.multi_panel_index == SINGLE_TOUCH_PANEL) {
		register_tui_driver(tui_tp_switch, "tp", dev_data->tui_data);
	} else {
...
static int ts_kit_init(void *data)
{
	...
	ts_send_init_cmd();
#if defined(CONFIG_HONOR_BCI_BATTERY) || defined(CONFIG_MTK_CHARGER) || \
	IS_ENABLED(CONFIG_HONOR_DEVKIT_QCOM_3_0)
...
static int _ts_of_property_read_u32(struct device_node *np,
				    const char *propname, int *out_val,
				    int quiet)
{
	int ret;
...
static int _ts_of_property_read_u16(struct device_node *np,
				    const char *propname, u16 *out_val,
				    int quiet)
{
	int val = 0;
	int ret;
...
static int _ts_of_property_read_u8(struct device_node *np, const char *propname,
				   u8 *out_val, int quiet)
{
	int val = 0;
	int ret;
				    ...
				    struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u32(np, "irq_config_no_suspend",
				&chip_data->irq_config_no_suspend);
	return 0;
}
				   ...
				   struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u32_quiet(np, "honor,test_capacitance_via_csvfile",
	...
	ts_of_property_read_u32_quiet(
		np, "honor,threshold_tddi_ee_short_partone",
		&chip_data->tddi_ee_short_test_partone_limit);
	ts_of_property_read_u32_quiet(
		np, "honor,threshold_tddi_ee_short_parttwo",
	...
	if (chip_data->unite_cap_test_interface) {
		...
		for (index = 0; index < array_len; index++) {
			ret = of_property_read_string_index(
				np, "raw_data_limit", index, &raw_data_dts);
			if (ret)
				TS_LOG_ERR(
					"%s: fail to read index = %d,raw_data_limit = %s,ret = %d,\n",
				   ...
				   struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u32_quiet(
		np, "fp_tp_report_touch_minor_event",
	...
	ts_of_property_read_u32_quiet(
		np, "check_status_watchdog_timeout",
				 ...
				 struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u32_default(np, "x_max", &chip_data->x_max,
					default_x_max);
	ts_of_property_read_u32_default(np, "y_max", &chip_data->y_max,
					defualt_y_max);
				   ...
				   struct ts_kit_device_data *chip_data)
{
	...
	ts_of_property_read_u8_quiet(np, "roi_control_bit",
				     &chip_data->ts_platform_data->feature_info
					      .roi_info.roi_control_bit);
	ts_of_property_read_u16_quiet(np, "roi_data_addr",
				      &chip_data->ts_platform_data->feature_info
					       .roi_info.roi_data_addr);
	...
	if (!ts_of_property_read_u16_quiet(np, "aft_data_addr",
		...
		TS_LOG_INFO(
			"cannot get chip aft_data_addr, set support_aft disable\n");
	}
...
int ts_kit_power_supply_get(enum ts_kit_power_id power_id)
{
	...
	case TS_KIT_POWER_LDO:
		power->regulator = regulator_get(
			&cd->ts_dev->dev, ts_kit_power_id2name(power_id));
...
int ts_kit_power_supply_ctrl(enum ts_kit_power_id power_id, int status,
...
{
	return blocking_notifier_call_chain(&ts_event_nh, (unsigned long)event,
					    NULL);
}
...
static const struct i2c_device_id g_ts_kit_device_id[] = {{TS_KIT_DEVICE_NAME,
							   0},
							  {}};
...
static int honor_ts_kit_platform_init(void)
{
	...
	g_ts_kit_platform_data.ts_init_task = kthread_create(
		ts_kit_init, &g_ts_kit_platform_data, "ts_init_thread:%d", 0);
	...
	g_ts_kit_platform_data.ts_task = kthread_create(
		ts_thread, &g_ts_kit_platform_data, "tskit_thread:%d", 0);
...
static const struct spi_device_id g_kit_device_id[] = {{HONOR_KIT_DEVICE_NAME,
							0},
						       {}};
