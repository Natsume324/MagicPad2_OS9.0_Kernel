...
int parse_u8_digit(const char *in, char *out, int max)
{
	...
	while (len--) {
		...
		if (last_char != 'Z') {
			...
			out[j] =
				(out[j] * LCD_HEX_BASE) + hex_char_to_value(ch);
			last_char = 'Z';
		} else {
			if (j >= max) {
				LCD_ERR("number is too much\n");
...
int dbg_parse_cmd(struct dsi_panel_cmds *pcmds, char *buf, int length)
{
	int blen;
	int len;
	char *bp = NULL;
...
static int dbg_on_cmd(char *par)
{
	...
	LCD_INFO("len = %d\n", len);
	if (len > 0)
		dbg_parse_cmd(&panel_on_cmds, dbg_buf.panel_on_buf, len);
...
int dbg_parse_config(void)
{
	...
	fd = filp_open((const char __force *)LCD_PARAM_FILE_PATH, O_RDONLY,
		       FILE_LIMIT);
	if (fd < 0) {
...
static int dbg_dsi_reg_read(const char *buf)
{
	...
	ret = dsi_panel_reg_read(pinfo->panel, read_reg.cmd,
				 read_reg.read_value, read_reg.read_count);
	if (ret) {
...
/* read function */
static ssize_t dbg_read(struct file *file, char __user *buff, size_t count,
			loff_t *ppos)
{
	int i;
	int len;
	...
	len = snprintf(
		cur, buf_len,
		"eg. echo write_reg:dsi:0 lp_mode:1 cmd:0x28 > /sys/kernel/debug/lcd-dbg/lcd_kit_dbg to write reg\n");
	buf_len -= len;
	cur += len;

	len = snprintf(
		cur, buf_len,
		"eg. echo read_reg:dsi:0 cmd:0x0a read_count:1 > /sys/kernel/debug/lcd-dbg/lcd_kit_dbg to read reg\n");
	buf_len -= len;
	cur += len;
			  ...
			  struct dsi_panel_cmds *pcmds)
{
	...
	for (i = 0; i < count; i++) {
		...
		cmds->msg.tx_len =
			((pcmds->cmds[i].dlen0 << 8) | pcmds->cmds[i].dlen1);
