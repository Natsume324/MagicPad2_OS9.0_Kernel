...
#define LCD_ERR(fmt, ...) DRM_DEV_ERROR(NULL, "[LCD /E]: " fmt, ##__VA_ARGS__)
#define LCD_INFO(fmt, ...) DRM_DEV_INFO(NULL, "[LCD /I]: " fmt, ##__VA_ARGS__)
/* fb max number */
#define FB_MAX 32
...
int dsi_panel_reg_write(struct dsi_panel *panel, u8 cmd, u8 *payload, int len,
			u32 lp_mode);
int dsi_panel_reg_read(struct dsi_panel *panel, u8 cmd, u8 *payload, int len);
