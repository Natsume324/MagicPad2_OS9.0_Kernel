...
static int lcd_check_checksum(struct panel_info *pinfo)
{
	...
	check_cnt = checksum->value.cnt - offset >= LCD_KIT_CHECKSUM_SIZE ?
			    LCD_KIT_CHECKSUM_SIZE :
			    checksum->value.cnt - offset;
...
static int lcd_checksum_check(struct panel_info *pinfo)
{
	...
	if (checksum->status == LCD_KIT_CHECKSUM_END) {
		...
		if (dsi_panel_set_cmd(pinfo->panel,
				      DSI_CMD_SET_CHECKSUM_DISABLE))
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	dsi_panel_receive_data(pinfo->panel, DSI_CMD_GET_CHECK_REG, read_value,
...
static int lcd_hor_line_test(struct panel_info *pinfo)
{
	...
	while (count--) {
		...
		msleep(pinfo->fact_info->hor_line_test.hor_duration *
		       MILLISEC_TIME);
				 ...
				 uint32_t expect_val, uint32_t compare_mode)
{
	uint32_t exp_pcd_value_mask =
		pinfo->fact_info->pcd_errflag_check.exp_pcd_mask;
	;
...
static void lcd_kit_pcd_dmd_report(struct panel_info *pinfo,
				   uint8_t *pcd_read_val, uint32_t val_len)
{
	int ret = 0;
	...
	ret = snprintf(record_buf, DMD_ERR_INFO_LEN,
		       ...
		       pinfo->panel_id, pcd_read_val[0], pcd_read_val[1],
		       pcd_read_val[2]);
	if (ret < 0) {
...
static void pcd_err_record(struct panel_info *pinfo, uint8_t *result,
			   uint8_t *pcd_read_val)
{
	error_num_pcd++;
...
static int lcd_kit_check_pcd_check(struct panel_info *pinfo)
{
	...
	if (ret == LCD_OK) {
		if (lcd_kit_judge_pcd_dmd(pinfo, read_pcd[0], expect_value,
					  pinfo->fact_info->pcd_errflag_check
				     ...
				     const char *buf, size_t count)
{
	...
	if (strlen(buf) < LCD_CMD_NAME_MAX) {
		...
		memcpy(fact_info->lcd_cmd_now, "INVALID",
		       strlen("INVALID") + 1);
	...
	u8 reg_val[DETECT_LOOPS][DETECT_NUM][VAL_NUM])
{
	...
	LCD_INFO("err_info = %s\n", err_info);
#if IS_ENABLED(CONFIG_DSM)
	(void)lcd_dsm_client_record(lcd_dclient, err_info,
				    DSM_LCD_DDIC_LV_DETECT_ERROR_NO);
#endif
}
				    ...
				    uint32_t pic_index, int loop)
{
	...
	ret = dsi_panel_receive_data(pinfo->panel, DSI_CMD_LVD_READ_ENABLE,
				     read_value, MAX_REG_READ_COUNT - 1);
...
static int lcd_ddic_lv_detect_set(struct panel_info *pinfo, uint32_t pic_index)
{
	int ret = LCD_OK;
	struct lcd_fact_info *fact_info = NULL;
	struct dsi_panel *panel = pinfo->panel;
...
static DEVICE_ATTR(amoled_pcd_errflag_check, 0644, lcd_amoled_pcd_errflag_show,
...
static DEVICE_ATTR(vertical_line_test, 0644, lcd_vertical_line_test_show,
		   lcd_vertical_line_test_store);
			    ...
			    struct dsi_parser_utils *utils)
{
	...
	fact_info->checkreg.enabled = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-lcd-check-reg-enabled");
	if (!fact_info->checkreg.enabled)
		return;

	fact_info->checkreg.expect_count = of_property_count_u32_elems(
		utils->data, "qcom,mdss-dsi-panel-lcd-check-reg-expect_vals");

	fact_info->checkreg.expect_val = kcalloc(
		fact_info->checkreg.expect_count, sizeof(int), GFP_KERNEL);
	for (i = 0; i < fact_info->checkreg.expect_count; i++) {
		of_property_read_u32_index(
			utils->data,
			"qcom,mdss-dsi-panel-lcd-check-reg-expect_vals", i,
			&val);
		      ...
		      struct dsi_parser_utils *utils)
{
	...
	/* pcd errflag */
	fact_info->pcd_errflag_check.pcd_support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-lcd-check-pcd-support");
	if (fact_info->pcd_errflag_check.pcd_support) {
		of_property_read_u32_index(
			utils->data,
			"qcom,mdss-dsi-panel-lcd-check-pcd-expect_vals", 0,
			&val);
		fact_info->pcd_errflag_check.pcd_value = val;

		of_property_read_u32_index(
			utils->data,
			"qcom,mdss-dsi-panel-lcd-pcd_value_compare_mode", 0,
			&val);
		fact_info->pcd_errflag_check.pcd_value_compare_mode = val;

		of_property_read_u32_index(utils->data,
					   "qcom,mdss-dsi-panel-lcd-pcd_mask",
					   0, &val);
		fact_info->pcd_errflag_check.exp_pcd_mask = val;

		of_property_read_u32_index(
			utils->data, "qcom,mdss-dsi-panel-lcd-pcd_det_num", 0,
			&val);
			   ...
			   struct dsi_parser_utils *utils)
{
	...
	/* hor line check */
	fact_info->hor_line_test.hor_support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-lcd-hor-line-support");
	if (fact_info->hor_line_test.hor_support) {
		fact_info->hor_line_test.hor_no_reset = utils->read_bool(
			utils->data,
...
{
	/* vtc line check */
	fact_info->vtc_line_test.vtc_support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-lcd-vtc-line-support");
	if (fact_info->vtc_line_test.vtc_support) {
		fact_info->vtc_line_test.vtc_no_reset = utils->read_bool(
			utils->data,
		      ...
		      struct dsi_parser_utils *utils)
{
	...
	fact_info->ddic_lv_detect.support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-lvd-support");

	if (fact_info->ddic_lv_detect.support) {
		fact_info->ddic_lv_detect.cnt = of_property_count_u32_elems(
			utils->data, "qcom,mdss-dsi-lvd-expect-value");

		fact_info->ddic_lv_detect.lv_expect = kcalloc(
			fact_info->ddic_lv_detect.cnt, sizeof(int), GFP_KERNEL);
			   ...
			   struct dsi_parser_utils *utils)
{
	...
	checksum->support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-checksum-support");
	if (!checksum->support)
		return;

	checksum->value.cnt = of_property_count_u32_elems(
		utils->data, "qcom,mdss-dsi-panel-checksum-value");

	checksum->value.buf =
		kcalloc(checksum->value.cnt, sizeof(u32), GFP_KERNEL);
...
int factory_init(struct dsi_panel *panel, struct panel_info *pinfo)
{
	...
	pinfo->fact_info->pt_reset_enable = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-pt-reset-enabled");
