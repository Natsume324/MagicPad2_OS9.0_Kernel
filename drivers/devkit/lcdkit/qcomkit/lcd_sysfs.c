...
static int lcd_get_2d_barcode(struct panel_info *pinfo, char *oem_data,
			      int len);
static int lcd_get_project_id(struct panel_info *pinfo, char *oem_data,
			      int len);
...
static int lcd_get_2d_barcode(struct panel_info *pinfo, char *oem_data, int len)
{
	...
	dsi_panel_receive_data(pinfo->panel, DSI_CMD_READ_OEMINFO, read_value,
			       OEM_INFO_SIZE_MAX - 1);
...
static ssize_t lcd_model_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	ssize_t ret = 0;
...
static ssize_t lcd_type_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	ssize_t ret;
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(
		buf, PAGE_SIZE,
		"blmax:%u,blmin:%u,blmax_nit_actual:%d,blmax_nit_standard:%d,lcdtype:%s\n",
...
static void lcd_fps_setting(unsigned int mode, struct dsi_panel *panel)
{
	...
	snprintf(atrace_buf, sizeof(atrace_buf), "displaySetFps:%d, cur_fps:%d",
		 adj_fps, cur_fps);
	...
	if (pinfo->fps_list_len && pinfo->fps_list) {
		if (!panel->cur_mode)
			...
			ret = snprintf(str, sizeof(str),
				       ...
				       pinfo->fps_list[0]);
		strncat(str,
			";support_fps_list:", strlen(";support_fps_list:"));
		...
		ret = snprintf(
			buf, sizeof(str),
			"current_fps:%d;default_fps:%d;support_fps_list:%d",
			fps, fps, fps);
	}
						...
						const char *buf, size_t count)
{
	...
	if (!pinfo->dual_panel_display) {
		rc = ts_power_notify_secondary(panel, TS_RESUME_DEVICE,
					       NO_SYNC);
		if (rc) {
	...
	if (pinfo->dual_panel_display != SINGLE_DISPLAY_MODE &&
	    pinfo->dual_panel_display != DUAL_DISPLAY_MODE) {
				  ...
				  const char *buf, size_t count)
{
	...
	if (mode == ALPM_LOW_LIGHT || mode == ALPM_MIDDLE_LIGHT ||
	    mode == ALPM_HIGH_LIGHT) {
		switch (mode) {
		case ALPM_LOW_LIGHT:
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < oem_info_data[1]; i++) {
		for (j = 0; j < BARCODE_BLOCK_LEN; j++) {
			snprintf(str_oem + start, sizeof(str_oem) - start,
				 "%d,",
...
static ssize_t effect_bl_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	ssize_t ret = 0;
	...
	if (pinfo->fps_list_len <= 3) {
		...
		for (i = 0; i < pinfo->fps_list_len; i++) {
			...
			if (i == 0)
				...
				strncat(str, ";", strlen(";"));
			ret += snprintf(tmp, sizeof(tmp), "%d:%d",
					pinfo->fps_list[i], ORDER_DELAY);
			strncat(str, tmp, strlen(tmp));
		}
	} else {
		u32 orer_delay = 0;
...
static ssize_t lcd_recovery_x_offset_show(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct panel_info *pinfo = NULL;
	...
	LCD_INFO("recovery_x_offset:%d\n", pinfo->recovery_x_offset);
	return ret;
}

static ssize_t lcd_panel_work_state_show(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct panel_info *pinfo = NULL;
	...
	pinfo = get_panel_info(get_fb_index(dev));
	if (!pinfo || !buf) {
		LCD_ERR("pws: point is null\n");
		return -EINVAL;
	}

	if (!pinfo->work_state_info.support) {
		LCD_ERR("pws: panel work mode is not support\n");
		return -EINVAL;
	}

	LCD_INFO("pws: cur_work_state is %d\n",
...
static ssize_t lcd_panel_work_state_store(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	struct panel_info *pinfo = get_panel_info(get_fb_index(dev));
	...
	if (!buf || !pinfo || !pinfo->panel || !pinfo->panel->pdata) {
	...
	if (!ret) {
		LCD_ERR("pws: sscanf return invaild:%zd\n", ret);
		return -EINVAL;
	}

	LCD_INFO("pws: set panel work state:%d\n", state);

	if (!pinfo->power_on ||
	    (pinfo->work_state_info.cur_work_state == S_PANEL_OFF)) {
		LCD_ERR("pws: not power on\n");
		return count;
	}

	if ((panel->power_mode == SDE_MODE_DPMS_LP1) ||
	    (panel->power_mode == SDE_MODE_DPMS_LP2) ||
	    (pinfo->work_state_info.cur_work_state == S_PANEL_DOZE)) {
		LCD_ERR("pws: in alpm mode\n");
		return count;
	}

	switch (state) {
	case S_PANEL_DEFAULT:
	case S_PANEL_FULL:
		panel->pdata->ws_change_handler(panel, E_PANEL_FULL);
		break;
	case S_PANEL_HALF_MAIN:
		panel->pdata->ws_change_handler(panel, E_PANEL_MAIN);
		break;
	case S_PANEL_HALF_SUB:
		panel->pdata->ws_change_handler(panel, E_PANEL_SUB);
		break;
	default:
		LCD_ERR("pws: unknow state:%d\n", state);
		break;
	}
...
static DEVICE_ATTR(lcd_voltage_enable, 0200, NULL, lcd_voltage_enable_store);
...
static DEVICE_ATTR(frame_update, 0644, frame_update_show, frame_update_store);
static DEVICE_ATTR(lcd_fps_scence, 0644, lcd_fps_scence_show,
		   lcd_fps_scence_store);
static DEVICE_ATTR(dual_panel_display, 0644, dual_panel_display_setting_show,
		   dual_panel_display_setting_store);
static DEVICE_ATTR(alpm_setting, 0644, alpm_setting_show, alpm_setting_store);
static DEVICE_ATTR(lcd_func_switch, 0644, lcd_func_switch_show,
		   lcd_func_switch_store);
static DEVICE_ATTR(lcd_reg_read, 0600, lcd_reg_read_show, lcd_reg_read_store);
static DEVICE_ATTR(ddic_oem_info, 0644, ddic_oem_info_show,
		   ddic_oem_info_store);
static DEVICE_ATTR(effect_bl, 0644, effect_bl_show, effect_bl_store);
static DEVICE_ATTR(lcd_fps_order, 0644, lcd_fps_order_show, NULL);
...
static DEVICE_ATTR(panel_enable_od_init, 0644, panel_enable_od_init_show, NULL);
static DEVICE_ATTR(recovery_x_offset, 0644, lcd_recovery_x_offset_show, NULL);
static DEVICE_ATTR(panel_work_state, 0644, lcd_panel_work_state_show,
		   lcd_panel_work_state_store);
...
struct attribute *lcd_sysfs_attrs[] = {
	...
	&dev_attr_recovery_x_offset.attr,
	&dev_attr_panel_work_state.attr,
	NULL,
};
...
module_param_string(panel_enable_od_init, panel_enable_od_info,
		    PANEL_ENABLE_OD_LEN, 0600);
