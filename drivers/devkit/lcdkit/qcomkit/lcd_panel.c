...
#if IS_ENABLED(CONFIG_DSM)
int lcd_dsm_client_record(struct dsm_client *lcd_dclient,
			  const char *record_buf, int lcd_dsm_error_no)
{
...
static void panel_parse_oem_info(struct dsi_panel *panel,
				 struct panel_info *pinfo)
{
	struct dsi_parser_utils *utils = &panel->utils;

	/* parse oeminfo */
	pinfo->oeminfo.support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-oem-info-enabled");
	if (!pinfo->oeminfo.support) {
		LCD_INFO("not support oem info\n");
		return;
	}

	pinfo->super_wallpaper = utils->read_bool(
		utils->data, "qcom,surport-super-wallpaper-enabled");

	pinfo->oeminfo.barcode_2d.support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-oem-2d-barcode-enabled");
	if (pinfo->oeminfo.barcode_2d.support) {
		if (utils->read_u32(utils->data,
				    "qcom,mdss-dsi-panel-oem-2d-barcode-offset",
				    &pinfo->oeminfo.barcode_2d.offset))
			DSI_ERR("failed to read: qcom,mdss-dsi-panel-oem-2d-barcode-offset\n");
	}
	pinfo->oeminfo.sn_data.support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-oem-sn-enabled");
}

static void panel_parse_gamma_info(struct dsi_panel *panel,
				   struct panel_info *pinfo)
{
	struct dsi_parser_utils *utils = &panel->utils;

	/* parse gammainfo */
	pinfo->lcd_panel_gamma_info.support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-gamma-info-enabled");
...
static void panel_parse_mipi_error_info(struct dsi_panel *panel,
					struct panel_info *pinfo)
{
	int i = 0;
	int val = 0;
	...
	pinfo->mipi_error.support = utils->read_bool(
		utils->data,
	...
	pinfo->mipi_error.expect_count = of_property_count_u32_elems(
		utils->data,
			    ...
			    struct dsi_parser_utils *utils)
{
	...
	pinfo->fps_list_len =
		utils->count_u32_elems(utils->data, "qcom,dsi-fps-list");
	if (pinfo->fps_list_len < 1) {
		LCD_ERR("fps list not present\n");
		return;
	}
	pinfo->fps_mmi_colorbar_len = utils->count_u32_elems(
		utils->data, "qcom,dsi-fps-list-mmi-color-bar");
	if (pinfo->fps_mmi_colorbar_len < 1) {
		LCD_ERR("fps_mmi_colorbar_len not present\n");
		return;
	}
	pinfo->fps_mmi_other_len = utils->count_u32_elems(
		utils->data, "qcom,dsi-fps-list-mmi-other");
	if (pinfo->fps_mmi_other_len < 1) {
		LCD_ERR("fps_mmi_other_len not present\n");
		return;
	}
	pinfo->fps_rt_image_len = utils->count_u32_elems(
		utils->data, "qcom,dsi-fps-list-rt-image");
	if (pinfo->fps_rt_image_len < 1) {
		LCD_ERR("fps_rt_image_len not present\n");
		return;
	}
	pinfo->fps_rt_video_len = utils->count_u32_elems(
		utils->data, "qcom,dsi-fps-list-rt-video");
	if (pinfo->fps_rt_video_len < 1) {
		LCD_ERR("fps_rt_video_len not present\n");
		return;
	}

	pinfo->fps_list = kcalloc(pinfo->fps_list_len, sizeof(u32), GFP_KERNEL);
	pinfo->fps_list_mmi_colorbar =
		kcalloc(pinfo->fps_mmi_colorbar_len, sizeof(u32), GFP_KERNEL);
	pinfo->fps_list_mmi_other =
		kcalloc(pinfo->fps_mmi_other_len, sizeof(u32), GFP_KERNEL);
	pinfo->fps_list_rt_image =
		kcalloc(pinfo->fps_rt_image_len, sizeof(u32), GFP_KERNEL);
	pinfo->fps_list_rt_video =
		kcalloc(pinfo->fps_rt_video_len, sizeof(u32), GFP_KERNEL);
	if (!pinfo->fps_list)
		return;

	rc = utils->read_u32_array(utils->data, "qcom,dsi-fps-list",
				   pinfo->fps_list, pinfo->fps_list_len);
	if (rc) {
	...
	rc = utils->read_u32_array(utils->data, "qcom,dsi-fps-list-mmi-other",
				   pinfo->fps_list_mmi_other,
	...
	rc = utils->read_u32_array(utils->data, "qcom,dsi-fps-list-rt-image",
				   pinfo->fps_list_rt_image,
	...
	rc = utils->read_u32_array(utils->data, "qcom,dsi-fps-list-rt-video",
				   pinfo->fps_list_rt_video,
					      ...
					      struct dsi_parser_utils *utils)
{
	...
	pinfo->adjust_backlight_enabled = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-adjust-backlight-enabled");
	if (!pinfo->adjust_backlight_enabled)
		return;

	ret = of_property_read_u32(
		utils->data, "qcom,mdss-dsi-panel-low-backlight-high-bound",
		&val);
	if (ret) {
	...
	ret = of_property_read_u32(
		utils->data, "qcom,mdss-dsi-panel-low-backlight-low-bound",
		&val);
	if (ret) {
	...
	ret = of_property_read_u32(
		utils->data, "qcom,mdss-dsi-panel-high-backlight-high-bound",
		&val);
	if (ret) {
				   ...
				   struct dsi_parser_utils *utils)
{
	...
	pinfo->lcd_panel_batch_info.support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-read-batch-info-enabled");
	if (!pinfo->lcd_panel_batch_info.support) {
		LCD_ERR("not support read panel batch info!\n");
		return;
	}
	pinfo->lcd_panel_batch_info.batch_match_hbm = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-batch-match-hbm-enabled");
	if (!pinfo->lcd_panel_batch_info.support) {
		LCD_INFO("not support panel batch match hbm!\n");
		return;
	}
	pinfo->lcd_panel_batch_info.cnt = of_property_count_u32_elems(
		utils->data, "qcom,mdss-dsi-panel-batch-value");

	pinfo->lcd_panel_batch_info.expect_val = kcalloc(
		pinfo->lcd_panel_batch_info.cnt, sizeof(int), GFP_KERNEL);
	...
	for (i = 0; i < pinfo->lcd_panel_batch_info.cnt; i++) {
		of_property_read_u32_index(
			utils->data, "qcom,mdss-dsi-panel-batch-value", i, &rc);
...
static void panel_parse_force_poweroff_info(struct panel_info *pinfo,
					    struct dsi_parser_utils *utils)
{
	pinfo->pwrkey_press.support = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-force-power-off");
					    ...
					    struct dsi_parser_utils *utils)
{
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-panel-pre-reset-time",
...
static void panel_parse_work_state_info(struct dsi_panel *panel,
					struct panel_info *pinfo)
{
	int rc = 0;
	struct dsi_parser_utils *utils = &panel->utils;

	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-recovery-x-offset",
			     &pinfo->recovery_x_offset);
	if (rc) {
		pinfo->recovery_x_offset = 0;
		LCD_INFO("use default recovery_x_offset = %d\n",
			 pinfo->recovery_x_offset);
	}
	LCD_INFO("use cust recovery_x_offset = %d\n", pinfo->recovery_x_offset);

	rc = utils->read_u32(utils->data, "qcom,mdss-half-screen-lp-support",
			     &pinfo->work_state_info.support);
	if (rc) {
		pinfo->work_state_info.support = 0;
		LCD_INFO("use default mdss-dsi-fold-LP-support = %d\n",
			 pinfo->work_state_info.support);
		return;
	}
	LCD_INFO("work_state_info.support = %d\n",
		 pinfo->work_state_info.support);
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-fold-LP-A-cycle",
			     &pinfo->work_state_info.fold_lp_A_cycle);
	if (rc) {
		pinfo->work_state_info.fold_lp_A_cycle = FOLD_LP_A_CYCLE_TIME;
		LCD_INFO("use default fold_lp_A_cycle = %d\n",
			 pinfo->work_state_info.fold_lp_A_cycle);
	}
	LCD_INFO("fold_lp_A_cycle = %d\n",
		 pinfo->work_state_info.fold_lp_A_cycle);
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-fold-LP-B-cycle",
			     &pinfo->work_state_info.fold_lp_B_cycle);
	if (rc) {
		pinfo->work_state_info.fold_lp_B_cycle = FOLD_LP_B_CYCLE_TIME;
		LCD_INFO("use default fold_lp_B_cycle = %d\n",
			 pinfo->work_state_info.fold_lp_B_cycle);
	}
	LCD_INFO("fold_lp_B_cycle = %d\n",
		 pinfo->work_state_info.fold_lp_B_cycle);
}
...
static int panel_parse(struct dsi_panel *panel, struct panel_info *pinfo)
{
	...
	/* parse dt */
	pinfo->lcd_module_name = utils->get_property(
		utils->data, "qcom,mdss-dsi-lcd-module-name", NULL);
	pinfo->lcd_ic_name = utils->get_property(
		utils->data, "qcom,mdss-dsi-lcd-ic-name", NULL);
	...
	pinfo->hbm.enabled = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-hbm-enabled");
	pinfo->local_hbm_enabled = utils->read_bool(
		utils->data, "qcom,mdss-dsi-panel-local-hbm-enabled");
	pinfo->lhbm_fpr_light_enable = utils->read_bool(
		utils->data, "qcom,mdss-dsi-lhbm-fpr-light-enable");
	pinfo->lhbm_adjust_light_enable = utils->read_bool(
		utils->data, "qcom,mdss-dsi-lhbm-light-adjust-enable");
	...
	if (rc)
		DSI_ERR("failed to read: qcom,mdss-dsi-bl-max-nit, rc=%d\n",
			rc);
	pinfo->four_byte_bl = utils->read_bool(
		utils->data, "qcom,mdss-dsi-bl-four-byte-bl-enabled");
	...
	panel_parse_work_state_info(panel, pinfo);
	return 0;
}
...
int panel_mipi_error_check(struct dsi_panel *panel)
{
	...
	for (i = 0; i < pinfo->mipi_error.expect_count; i++) {
		...
		if (i % 2) {
			LCD_INFO("l-bit read_val[%u]:0x%x, %s[%u]:0x%x.\n", i,
				 read_value[i], val_str, i, exp_val[i] & 0xFF);
		} else {
	...
	if (!judge_result) {
		tot = scnprintf(record_buf,
				DMD_RECORD_BUF_LEN * MAX_REG_READ_COUNT,
				"panel:%d mipi error check failed:\n",
				pinfo->panel_id);
		for (i = 0; i < pinfo->mipi_error.expect_count; i++) {
			tot += scnprintf(
				record_buf + tot,
				DMD_RECORD_BUF_LEN * MAX_REG_READ_COUNT - tot,
...
void lcd_register_power_key_notify(void)
{
	...
	for (i = 0; i < 2; i++) {
		...
		ret = lcd_powerkey_register_notifier(&pinfo->pwrkey_press.nb);
		if (ret < 0)
...
static void panel_handle_backlight_check(struct work_struct *work)
{
	...
	backlight_check =
		container_of(dw, struct dsi_backlight_reg_check, work);
	if (!backlight_check) {
		LCD_ERR("backlight_check is null\n");
		return;
	}
	panel = container_of(backlight_check, struct dsi_panel,
			     backlight_check);
	if (!panel) {
	...
	if (panel->pdata->pinfo->brightness) {
		ret = dsi_backlight_reg_read(panel, BACKLIGHT_REG, read_value,
					     BACKLIGHT_REG_COUNT);
		if (ret < 0)
			LCD_ERR("backlight reg read error\n");

		snprintf(record_buf, DMD_RECORD_BUF_LEN,
			 "set backlight reg =0x%x%x", read_value[0],
			 read_value[1]);
		LCD_INFO("set backlight backlight= 0x%x%x\n", read_value[0],
			 read_value[1]);
		if (read_value[0] == 0 && read_value[1] == 0) {
			(void)lcd_dsm_client_record(
				lcd_dclient, record_buf,
				DSM_LCD_BACKLIGHT_REG_ERROR_NO);
			dsi_panel_set_backlight(
				panel, panel->pdata->pinfo->brightness);
		}
	}
...
{
	INIT_DELAYED_WORK(&panel->backlight_check.work,
			  panel_handle_backlight_check);
}
...
static void dsi_panel_fold_lp_mode_work(struct work_struct *work)
{
	struct dsi_panel *panel;
	int rc = 0;
	...
	if (pinfo->work_state_info.curr_work_mode == PARTIAL_MODE_B) {
		rc = dsi_panel_set_cmd(panel, DSI_CMD_ENTER_PARTIAL_MODE_A);
		if (rc) {
			LCD_ERR("failed to send DSI_CMD_ENTER_PARTIAL_MODE_A cmds, rc=%d\n",
				rc);
		} else {
			pinfo->work_state_info.curr_work_mode = PARTIAL_MODE_A;
			schedule_delayed_work(
				&pinfo->work_state_info.fold_lp_mode_work,
				msecs_to_jiffies(FOLD_LP_A_CYCLE_TIME));
			LCD_INFO("switch to partial mode A success\n");
		}
	} else {
		rc = dsi_panel_set_cmd(panel, DSI_CMD_ENTER_PARTIAL_MODE_B);
		if (rc) {
			LCD_ERR("failed to send DSI_CMD_ENTER_PARTIAL_MODE_B cmds, rc=%d\n",
				rc);
		} else {
			pinfo->work_state_info.curr_work_mode = PARTIAL_MODE_B;
			schedule_delayed_work(
				&pinfo->work_state_info.fold_lp_mode_work,
				msecs_to_jiffies(FOLD_LP_B_CYCLE_TIME));
			LCD_INFO("switch to partial mode B success\n");
		}
	}
...
static int panel_init(struct dsi_panel *panel)
{
	...
	debugfs_init();
#endif
	if (pinfo->work_state_info.support) {
		INIT_DELAYED_WORK(&pinfo->work_state_info.fold_lp_mode_work,
				  dsi_panel_fold_lp_mode_work);
		LCD_ERR("pws: init delay work fold_lp_mode_work\n");
	}
	return 0;
}
...
static int panel_hbm_set_handle(struct dsi_panel *panel, int dimming, int level)
{
	int rc;
	...
	if (level > 0) {
		if (last_level == 0) {
			...
			if (!dimming) {
				rc = dsi_panel_set_cmd(
					panel, DSI_CMD_SET_HBM_DIMM_OFF);
				if (rc)
...
static int panel_local_hbm_identify_on_set(struct dsi_panel *panel, int level)
{
	...
	if ((level == HBM_ENTER) &&
	    (panel->cur_mode->timing.refresh_rate != FPS_120_HZ) &&
	...
	if (panel->pdata->pinfo->lhbm_fpr_light_enable) {
		...
		if (panel->pdata->pinfo->lhbm_light_changed) {
			cmds = mode->priv_info->cmd_sets[DSI_CMD_SET_LHBM_LIGHT]
				       .cmds;
			++cmds;
			if (backlight > 3515) {
				backlight = 4000;
				cmds->msg.tx_buf =
					panel->pdata->pinfo->lhbm_light_origin;
			} else {
				cmds->msg.tx_buf =
					panel->pdata->pinfo->lhbm_light_adjust;
			}
		...
		count = mode->priv_info->cmd_sets[DSI_CMD_SET_LHBM_FSR_LIGHT]
				.count;
		cmds = mode->priv_info->cmd_sets[DSI_CMD_SET_LHBM_FSR_LIGHT]
			       .cmds;
		for (i = 0; i < count - 1; i++) {
			++cmds;
		}
...
{
	LCD_INFO(
		"Panel off, local_hbm_enabled:%d, level = %d, power mode = %d\n",
		panel->pdata->pinfo->local_hbm_enabled, level,
		panel->power_mode);
	if (!panel->pdata->pinfo->local_hbm_enabled)
		return -1;
			 ...
			 struct display_engine_ddic_hbm_param *hbm_cfg)
{
	...
	case HBM_FOR_MMI:
		...
		break;
	case HBM_FOR_LIGHT:
		rc = panel_hbm_set_handle(panel, hbm_cfg->level,
					  hbm_cfg->dimming);
		break;
			   ...
			   enum lcd_kit_ts_pm_type type, int sync)
{
	...
	if (is_dual_panel()) {
		if ((panel->pdata->pinfo->panel_id == SECONDARY_PANEL) &&
		    (panel->pdata->pinfo->dual_panel_display ==
		     DUAL_DISPLAY_MODE)) {
			LCD_INFO("Don't notify TP \n");
			goto out;
		}
		if (ts_ops && ts_ops->ts_multi_power_notify)
			ret = ts_ops->ts_multi_power_notify(
				type, sync, panel->pdata->pinfo->panel_id);
			    ...
			    enum lcd_kit_usb_pm_type type, int sync)
{
	...
	if (is_dual_panel()) {
		if (usb_ops && usb_ops->usb_multi_power_notify) {
			ret = usb_ops->usb_multi_power_notify(
				type, sync, panel->pdata->pinfo->panel_id);
			LCD_INFO("dual panel, panel_id = %d\n",
				 panel->pdata->pinfo->panel_id);
		}
			      ...
			      enum lcd_kit_bat_heating_type type, int sync)
{
	...
	if (is_dual_panel()) {
		if (bat_heating_ops_handle &&
		    bat_heating_ops_handle->bat_heating_multi_notify) {
			ret = bat_heating_ops_handle->bat_heating_multi_notify(
				type, sync, panel->pdata->pinfo->panel_id);
			LCD_INFO("%s,dual panel, panel_id = %d,type = %d\n",
				 __FUNCTION__, panel->pdata->pinfo->panel_id,
				 type);
		}
	} else {
		if (bat_heating_ops_handle &&
		    bat_heating_ops_handle->bat_heating_notify) {
			ret = bat_heating_ops_handle->bat_heating_notify(type,
									 sync);
...
{
	int ret = 0;
	struct bat_leakage_kit_ops *bat_leakage_ops_handle =
		bat_leakage_kit_get_ops();

	if (is_dual_panel()) {
		if (bat_leakage_ops_handle &&
		    bat_leakage_ops_handle->bat_leakage_multi_notify) {
			ret = bat_leakage_ops_handle->bat_leakage_multi_notify(
				type, sync, panel->pdata->pinfo->panel_id);
		...
		if (bat_leakage_ops_handle &&
		    bat_leakage_ops_handle->bat_leakage_notify) {
			ret = bat_leakage_ops_handle->bat_leakage_notify(type,
									 sync);
			LCD_INFO("single panel\n");
		}
	}
...
#endif
		if (pinfo->work_state_info.support &&
		    panel->pdata->ws_change_handler)
			panel->pdata->ws_change_handler(panel, E_PANEL_ON);
	...
	case PANEL_INIT_MIPI_HS_SEND_SEQUENCE:
		...
		if (!(panel->pdata->pinfo->inited)) {
			...
			read_2d_barcode(panel,
					mutex_is_locked(&panel->panel_lock));
...
static int print_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (last_level == 0 && bl_lvl != 0) {
		...
		LOG_JANK_D(JLID_KERNEL_LCD_BACKLIGHT_ON, "LCD_BACKLIGHT_ON,%u",
			   bl_lvl);
		queue_delayed_work(
			panel->panel_workq, &panel->backlight_check.work,
			usecs_to_jiffies(8300)); /*delay one frame of 120hz*/
...
static int gamma_read(struct dsi_panel *panel, int type,
		      struct dsi_panel_cmd_set *cmd_set, uint8_t *data, int len,
		      uint8_t BF)
{
	int rc = 0;
	struct dsi_cmd_desc *cmds;
	...
	rc = dsi_panel_receive_data_nolock_cmd(panel, type, cmd_set, data, len);
	if (rc) {
...
static void gamma_code_prepare(struct dsi_panel *panel, int src_fre,
			       int des_fre, uint8_t BF)
{
	int i, j, k;
	u32 reg;
	u32 count;
	...
	for (i = SET_NOR3_CMD_BEGIN; i < count - 2; i++) {
		...
		while (cnt--) {
			tmp[k++] =
				panel->gamma_info.gamma_data_set[src_fre][j++];
		}
...
static int gamma_code_cal(struct dsi_panel *panel, int fre)
{
	...
	int gamma_nor1 = fre == SET_GAMMA_120HZ_NOR3 ? GAMMA_120HZ_NOR1 :
						       GAMMA_90HZ_NOR1;
	int gamma_nor3 = fre == SET_GAMMA_120HZ_NOR3 ? GAMMA_120HZ_NOR3 :
						       GAMMA_90HZ_NOR3;
	int gamma_nor5 = fre == SET_GAMMA_120HZ_NOR3 ? GAMMA_120HZ_NOR5 :
						       GAMMA_90HZ_NOR5;

	rc = gamma_read(panel, DSI_CMD_READ_GAMMA_8,
			&panel->gamma_info.gamma_read_8_cmd, data[0],
			LCD_GAMMA_CODE_LENGTH, gamma_nor1);
	if (rc) {
		LCD_ERR("read GAMMA1 reg failed\n");
		return rc;
	}

	rc = gamma_read(panel, DSI_CMD_READ_GAMMA_3,
			&panel->gamma_info.gamma_read_3_cmd, RGB255,
			LCD_GAMMA_RGB_LENGTH, gamma_nor3);
	if (rc) {
	...
	rc = gamma_read(panel, DSI_CMD_READ_GAMMA_8,
			&panel->gamma_info.gamma_read_8_cmd, data[1],
			LCD_GAMMA_CODE_LENGTH, gamma_nor5);
	if (rc) {
	...
	RGB255_ratio[0] = LCD_GAMMA_RATIO * (RGB255[12] << 8 | RGB255[13]) * 2 /
			  ((data[0][48] << 8 | data[0][49]) +
			   (data[1][48] << 8 | data[1][49]));
	RGB255_ratio[1] = LCD_GAMMA_RATIO * (RGB255[26] << 8 | RGB255[27]) * 2 /
			  ((data[0][98] << 8 | data[0][99]) +
			   (data[1][98] << 8 | data[1][99]));
	RGB255_ratio[2] = LCD_GAMMA_RATIO * (RGB255[40] << 8 | RGB255[41]) * 2 /
			  ((data[0][148] << 8 | data[0][149]) +
			   (data[1][148] << 8 | data[1][149]));

	LCD_INFO("gamma ratio:%d %d %d\n", RGB255_ratio[0], RGB255_ratio[1],
		 RGB255_ratio[2]);
	...
	for (i = 0; i < cnt; i++) {
		...
		data3 = (data1 + data5) * RGB255_ratio[i / 25] /
			(2 * LCD_GAMMA_RATIO);
...
static int gamma_code_copy(struct dsi_panel *panel, int src_fre, int des_fre,
			   uint8_t src_BF, uint8_t des_BF)
{
	int rc = 0;

	rc = gamma_read(panel, DSI_CMD_READ_GAMMA_8,
			&panel->gamma_info.gamma_read_8_cmd,
			panel->gamma_info.gamma_data_set[src_fre],
			LCD_GAMMA_CODE_LENGTH, src_BF);
...
int gamma_code(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_receive_data_nolock_cmd(panel, DSI_CMD_GAMMA_BATCH,
					       &panel->gamma_info.batch_cmd,
					       batch, LCD_GAMMA_BATCH_LENGTH);
	if (rc) {
	...
	gamma_code_copy(panel, SET_GAMMA_LHBM_NOR2, SET_GAMMA_LHBM_NOR2,
			GAMMA_120HZ_NOR2, GAMMA_LHBM_NOR2);
	gamma_code_prepare(panel, SET_GAMMA_120HZ_NOR3, SET_GAMMA_LHBM_NOR3,
			   GAMMA_LHBM_NOR3);
	gamma_code_copy(panel, SET_GAMMA_LHBM_NOR4, SET_GAMMA_LHBM_NOR4,
			GAMMA_120HZ_NOR4, GAMMA_LHBM_NOR4);
...
static int read_batch_match(struct dsi_panel *panel)
{
	...
	if (!pinfo->lcd_panel_batch_info.support ||
		...
		return rc;
	rc = dsi_panel_receive_data(panel, DSI_CMD_READ_BATCHINFO, &read_value,
				    MAX_REG_READ_COUNT);
	if (rc) {
		LCD_ERR("read batch reg failed\n");
		return rc;
	...
	if (pinfo->lhbm_adjust_light_enable &&
	    (read_value == 0x40 || read_value == 0x49)) {
		uint32_t rgb[3] = {0};
		int i;
		u8 *tmp;

		pinfo->lhbm_light_changed = true;
		pinfo->lhbm_light_origin =
			kzalloc(MAX_READ_LHBM_LIGHT_LENGTH + 1, GFP_KERNEL);
		tmp = pinfo->lhbm_light_origin;
		++tmp;
		pinfo->lhbm_light_origin[0] = 0xD0;
		rc = dsi_panel_receive_data(panel, DSI_CMD_READ_LHBM_LIGHT, tmp,
					    MAX_READ_LHBM_LIGHT_LENGTH);
		if (rc) {
...
static int read_2d_barcode(struct dsi_panel *panel, bool is_panel_locked)
{
	...
	if (!pinfo->oeminfo.barcode_2d.support)
		return rc;
	if (is_panel_locked) {
		rc = dsi_panel_receive_data_nolock(
			panel, DSI_CMD_READ_OEMINFO,
			pinfo->oeminfo.barcode_2d.barcode_data,
			OEM_INFO_SIZE_MAX - 1);
	} else {
		rc = dsi_panel_receive_data(
			panel, DSI_CMD_READ_OEMINFO,
			pinfo->oeminfo.barcode_2d.barcode_data,
			OEM_INFO_SIZE_MAX - 1);
	}
...
static int panel_set_boot(struct dsi_panel *panel)
{
	...
	if (!pinfo->inited) {
		...
		pinfo->inited = true;
		if (panel->pdata->pinfo->work_state_info.support) {
			mutex_init(&panel->pdata->pinfo->work_state_info
					    .work_state_lock);
			panel_set_work_state(panel, S_PANEL_FULL);
		}
	}
...
static int dsi_panel_exit_partial_mode_aod(struct dsi_panel *panel)
{
	int ret = 0;
	struct panel_info *pinfo = NULL;

	if (!panel || !panel->pdata || !panel->pdata->pinfo) {
		LCD_ERR("pws: pinfo is null\n");
		return -EINVAL;
	}
	pinfo = panel->pdata->pinfo;

	/* cancel fold work mode switch work */
	cancel_delayed_work_sync(&pinfo->work_state_info.fold_lp_mode_work);

	if (pinfo->work_state_info.curr_work_mode == PARTIAL_MODE_B) {
		ret = dsi_panel_set_cmd(panel, DSI_CMD_EXIT_PARTIAL_MODE_B);
		if (ret)
			LCD_ERR("pws: failed to SPREAD_STATE for aod, rc=%d\n",
				ret);
	} else {
		ret = dsi_panel_set_cmd(panel, DSI_CMD_EXIT_PARTIAL_MODE_A);
		if (ret)
			LCD_ERR("pws: failed to SPREAD_STATE for aod, rc=%d\n",
				ret);
	}
	...
	if (!ret) {
		LCD_INFO("pws: exit PARTIAL_MODE %d success for aod\n",
			 pinfo->work_state_info.curr_work_mode);
		pinfo->work_state_info.curr_work_mode = FULL_MODE;
	}
	return ret;
}

static int dsi_panel_exit_partial_mode_cmd(struct dsi_panel *panel)
{
	int ret = 0;
	struct panel_info *pinfo = NULL;
	...
	if (pinfo->work_state_info.curr_work_mode == PARTIAL_MODE_B) {
		ret = dsi_panel_set_cmd(panel, DSI_CMD_EXIT_PARTIAL_MODE_B);
		if (ret)
			LCD_ERR("pws: failed to send EXIT_MODE_B cmds: %d\n",
				ret);
	} else {
		ret = dsi_panel_set_cmd(panel, DSI_CMD_EXIT_PARTIAL_MODE_A);
		if (ret)
			LCD_ERR("pws: failed to send EXIT_MODE_A cmds: %d\n",
				ret);
	}

	if (!ret) {
		LCD_INFO("pws: exit PARTIAL_MODE %d success\n",
			 pinfo->work_state_info.curr_work_mode);
		pinfo->work_state_info.curr_work_mode = FULL_MODE;
	}
...
static int dsi_panel_enter_partial_mode_cmd(struct dsi_panel *panel)
{
	int ret = 0;
	struct panel_info *pinfo = NULL;
	...
	ret = dsi_panel_set_cmd(panel, DSI_CMD_ENTER_PARTIAL_MODE_A);
	if (ret) {
		LCD_ERR("pws: failed to send ENTER_MODE_A cmds: %d\n", ret);
	} else {
		pinfo->work_state_info.curr_work_mode = PARTIAL_MODE_A;
		schedule_delayed_work(&pinfo->work_state_info.fold_lp_mode_work,
				      msecs_to_jiffies(FOLD_LP_A_CYCLE_TIME));
		LCD_INFO("pws: enter PARTIAL_MODE_A success\n");
	}
	dsi_panel_release_panel_lock(panel);
	return ret;
}

static char *panel_ws_name(enum panel_work_state state)
{
	switch (state) {
	case S_PANEL_OFF:
		return "S_PANEL_OFF";
	case S_PANEL_FULL:
		return "S_PANEL_FULL";
	case S_PANEL_HALF_MAIN:
		return "S_PANEL_HALF_MAIN";
	case S_PANEL_HALF_SUB:
		return "S_PANEL_HALF_SUB";
	case S_PANEL_DOZE:
		return "S_PANEL_DOZE";
	default:
		return "S_PANEL_UNKNOWN";
	}
}

static char *panel_event_name(enum panel_ws_change_event event)
{
	switch (event) {
	case E_PANEL_ON:
		return "E_PANEL_ON";
	...
	case E_PANEL_OFF:
		return "E_PANEL_OFF";
	case E_PANEL_DOZE_ENTER:
		return "E_PANEL_DOZE_ENTER";
	case E_PANEL_DOZE_EXIT:
		return "E_PANEL_DOZE_EXIT";
	default:
		return "E_PANEL_UNKNOWN";
	}
}

void panel_set_work_state(struct dsi_panel *panel, enum panel_work_state state)
{
	struct panel_info *pinfo = NULL;

	if (!panel || !panel->pdata || !panel->pdata->pinfo) {
		LCD_ERR("pws: set_ws func param NULL\n");
		return;
	}
	pinfo = panel->pdata->pinfo;

	LCD_INFO("pws: %s-->%s\n",
		 panel_ws_name(pinfo->work_state_info.cur_work_state),
		 panel_ws_name(state));

	pinfo->work_state_info.cur_work_state = state;
}

static void panel_ws_off_handler(struct dsi_panel *panel,
				 enum panel_ws_change_event event)
{
	LCD_INFO("pws: S_PANEL_OFF: %s\n", panel_event_name(event));
	switch (event) {
	case E_PANEL_ON:
		panel_set_work_state(panel, S_PANEL_FULL);
		break;
	...
	case E_PANEL_OFF:
		break;
	case E_PANEL_DOZE_ENTER:
		panel_set_work_state(panel, S_PANEL_DOZE);
		break;
	case E_PANEL_DOZE_EXIT:
		break;
	default:
...
static void panel_ws_sub_handler(struct dsi_panel *panel,
				 enum panel_ws_change_event event)
{
	int rc;

	LCD_INFO("pws: S_PANEL_HALF_SUB: %s\n", panel_event_name(event));
	switch (event) {
	case E_PANEL_MAIN:
		rc = dsi_panel_enter_partial_mode_cmd(panel);
		if (!rc)
			panel_set_work_state(panel, S_PANEL_HALF_MAIN);
		break;
	case E_PANEL_SUB:
		break;
	case E_PANEL_FULL:
	...
	case E_PANEL_ON:
		break;
	case E_PANEL_OFF:
		panel_set_work_state(panel, S_PANEL_OFF);
		break;
	case E_PANEL_DOZE_ENTER:
		panel_set_work_state(panel, S_PANEL_DOZE);
		break;
...
static void panel_ws_full_handler(struct dsi_panel *panel,
				  enum panel_ws_change_event event)
{
	int rc;

	LCD_INFO("pws: S_PANEL_FULL: %s\n", panel_event_name(event));
	switch (event) {
	case E_PANEL_MAIN:
		rc = dsi_panel_enter_partial_mode_cmd(panel);
		if (!rc)
		...
		break;
	case E_PANEL_SUB:
		panel_set_work_state(panel, S_PANEL_HALF_SUB);
		break;
	case E_PANEL_FULL:
		break;
	case E_PANEL_ON:
...
static void panel_ws_half_main_handler(struct dsi_panel *panel,
				       enum panel_ws_change_event event)
{
	int rc;

	LCD_INFO("pws: S_PANEL_HALF_MAIN: %s\n", panel_event_name(event));
	switch (event) {
	case E_PANEL_MAIN:
		break;
	case E_PANEL_SUB:
	case E_PANEL_FULL:
		rc = dsi_panel_exit_partial_mode_cmd(panel);
		if (!rc)
			panel_set_work_state(panel, S_PANEL_FULL);
		break;
	case E_PANEL_ON:
	...
	case E_PANEL_OFF:
		cancel_delayed_work_sync(
			&panel->pdata->pinfo->work_state_info.fold_lp_mode_work);
		panel->pdata->pinfo->work_state_info.curr_work_mode = FULL_MODE;
		panel_set_work_state(panel, S_PANEL_OFF);
		break;
	case E_PANEL_DOZE_ENTER:
		rc = dsi_panel_exit_partial_mode_aod(panel);
		if (!rc)
			panel_set_work_state(panel, S_PANEL_DOZE);
		break;
	case E_PANEL_DOZE_EXIT:
...
static void panel_ws_doze_handler(struct dsi_panel *panel,
				  enum panel_ws_change_event event)
{
	LCD_INFO("pws: S_PANEL_DOZE: %s\n", panel_event_name(event));
	switch (event) {
	case E_PANEL_MAIN:
	case E_PANEL_SUB:
	case E_PANEL_FULL:
...
static void panel_work_state_change_handler(struct dsi_panel *panel,
					    enum panel_ws_change_event event)
{
	struct panel_info *pinfo = NULL;

	if (!panel || !panel->pdata || !panel->pdata->pinfo) {
		LCD_ERR("pws: func param NULL\n");
		return;
	}
	pinfo = panel->pdata->pinfo;

	if (!pinfo->work_state_info.support) {
		LCD_ERR("pws: func not support\n");
		return;
	}

	mutex_lock(&pinfo->work_state_info.work_state_lock);
	switch (pinfo->work_state_info.cur_work_state) {
	case S_PANEL_OFF:
		panel_ws_off_handler(panel, event);
		break;
	case S_PANEL_FULL:
		panel_ws_full_handler(panel, event);
		break;
	case S_PANEL_HALF_MAIN:
		panel_ws_half_main_handler(panel, event);
		break;
	case S_PANEL_HALF_SUB:
		panel_ws_sub_handler(panel, event);
		break;
	case S_PANEL_DOZE:
		panel_ws_doze_handler(panel, event);
		break;
	default:
		LCD_ERR("pws: unknown panel state:%d\n",
			pinfo->work_state_info.cur_work_state);
		break;
	}
	mutex_unlock(&pinfo->work_state_info.work_state_lock);
}
...
struct panel_data g_panel_data = {
	...
	.ws_change_handler = panel_work_state_change_handler,
};
