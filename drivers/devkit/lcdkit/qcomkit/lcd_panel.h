...
#define FOLD_LP_A_CYCLE_TIME (5 * 1000)
#define FOLD_LP_B_CYCLE_TIME (25 * 1000)

/* fold panel work state ctrl begin */
enum panel_ws_change_event {
	E_PANEL_FULL = 1,
	E_PANEL_MAIN,
	E_PANEL_SUB,
	E_PANEL_ON,
...
enum fold_lp_work_mode {
	PARTIAL_MODE_A,
	PARTIAL_MODE_B,
	FULL_MODE,
};

struct lcd_kit_panel_work_state_info {
	u32 support;
	u32 fold_lp_A_cycle;
	u32 fold_lp_B_cycle;
	u32 curr_work_mode;
	enum panel_work_state cur_work_state;
	struct delayed_work fold_lp_mode_work;
	struct mutex work_state_lock;
};
/* fold panel work state ctrl end */
...
struct panel_info {
	...
	struct lcd_kit_panel_work_state_info work_state_info;
...
struct panel_data {
	...
	void (*ws_change_handler)(struct dsi_panel *panel,
