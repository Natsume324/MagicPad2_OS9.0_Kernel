...
static int nt50356_i2c_write_u8(u8 chip_no, unsigned char addr,
				unsigned char value)
{
	i2c_set_bus_num(nt50356_bl_info.nt50356_i2c_bus_id);
	i2c_init(NT50356_I2C_SPEED, chip_no);
...
static int nt50356_parse_dts(void)
{
	...
	for (i = 0; i < NT50356_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(
			nt50356_bl_info.pfdt, nt50356_bl_info.nodeoffset,
...
static int nt50356_device_verify(void)
{
	...
	if (nt50356_bl_info.nt50356_hw_en) {
		...
		sprd_gpio_direction_output(
			NULL, nt50356_bl_info.nt50356_hw_en_gpio, GPIO_OUT_ONE);
	...
	ret = lcd_kit_get_dts_u32_default(nt50356_bl_info.pfdt,
					  nt50356_bl_info.nodeoffset,
					  NT50356_EXTEND_REV_SUPPORT,
					  &is_support, 0);
	if (ret < 0) {
		...
		if (is_support == NT50356_EXTEND_REV_VAL) {
			...
			ret = nt50356_i2c_read_u8(NT50356_SLAV_ADDR, &reg_val,
						  REG_BL_CONFIG_1);
			if (ret < 0) {
				LCD_KIT_ERR(
					"read nt50356 bl config 1 failed\n");
				goto error_exit;
			}
			LCD_KIT_INFO("nt50356 reg bl config1 = 0x%x\n",
				     reg_val);
			if ((reg_val & DEV_MASK_EXTEND_NT50356) !=
			    VENDOR_ID_EXTEND_NT50356) {
				LCD_KIT_INFO(
					"nt50356 check vendor id exend failed\n");
				ret = NT50356_FAIL;
				goto error_exit;
			}
		}
...
error_exit:
	if (nt50356_bl_info.nt50356_hw_en)
		sprd_gpio_direction_output(NULL,
					   nt50356_bl_info.nt50356_hw_en_gpio,
					   GPIO_OUT_ZERO);
...
int nt50356_init(struct sprd_panel_info *pinfo)
{
	...
	memset(&nt50356_bl_info, 0,
	       sizeof(struct nt50356_backlight_information));
	...
	if (nt50356_bl_info.nt50356_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			nt50356_bl_info.pfdt, nt50356_bl_info.nodeoffset,
			NT50356_HW_EN_GPIO, &nt50356_bl_info.nt50356_hw_en_gpio,
			0);
		if (ret < 0) {
		...
		ret = lcd_kit_get_dts_u32_default(
			nt50356_bl_info.pfdt, nt50356_bl_info.nodeoffset,
			NT50356_HW_EN_DELAY, &nt50356_bl_info.bl_on_lk_mdelay,
			0);
		if (ret < 0)
	...
	ret = lcd_kit_get_dts_u32_default(nt50356_bl_info.pfdt,
					  nt50356_bl_info.nodeoffset,
					  NT50356_BL_LEVEL,
					  &nt50356_bl_info.bl_level,
					  NT50356_BL_DEFAULT_LEVEL);
	if (ret < 0)
