...
extern int gpio_pin_set(unsigned int gpio_id, unsigned int ie_oe,
...
static int
lcd_kit_cmds_to_sprd_dsi_cmds(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
...
static int
lcd_kit_cmds_to_sprd_dsi_read_cmds(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
				   struct dsi_cmd_common_desc *cmd)
{
	if (lcd_kit_cmds == NULL) {
		LCD_KIT_ERR("lcd_kit_cmds is null point!\n");
...
static int sprd_mipi_dsi_cmds_tx(void *dsi, struct lcd_kit_dsi_cmd_desc *cmds,
				 int cnt)
{
				 ...
				 unsigned char len)
{
	...
	if (cmd == NULL || pdsi == NULL || out == NULL || len == 0) {
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (!lcd_kit_cmd_is_write(&cmds->cmds[i])) {
			...
			lcd_kit_cmds_to_sprd_dsi_read_cmds(&cmds->cmds[i],
							   &dsi_cmd);
			ret = sprd_mipi_dsi_cmds_rx(&buffer[0], hld, &dsi_cmd,
						    dsi_cmd.dlen);
			if (ret == 0) {
