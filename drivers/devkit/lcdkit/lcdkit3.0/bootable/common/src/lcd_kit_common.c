...
/* power sequence */
static struct lcd_kit_power_seq_desc
	g_lcd_kit_power_sequence[LCD_ACTIVE_PANEL_BUTT];
/* power handle */
...
static int lcd_kit_parse_power(const char *np)
{
	/* vci */
	if (power_hdl->lcd_vci.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,lcd-vci", &power_hdl->lcd_vci);
	/* iovcc */
	if (power_hdl->lcd_iovcc.buf == NULL)
	...
	/* vsp */
	if (power_hdl->lcd_vsp.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,lcd-vsp", &power_hdl->lcd_vsp);
	/* vsn */
	if (power_hdl->lcd_vsn.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,lcd-vsn", &power_hdl->lcd_vsn);
	/* lcd reset */
	if (power_hdl->lcd_rst.buf == NULL)
	...
	if (power_hdl->lcd_te0.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,lcd-te0", &power_hdl->lcd_te0);
	/* tp reset */
	if (power_hdl->tp_rst.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,tp-reset", &power_hdl->tp_rst);
	/* lcd vdd */
	if (power_hdl->lcd_vdd.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,lcd-vdd", &power_hdl->lcd_vdd);
	if (power_hdl->lcd_aod.buf == NULL)
		lcd_kit_parse_array(np, "lcd-kit,lcd-aod", &power_hdl->lcd_aod);
...
static int lcd_kit_parse_common_info(const char *np)
{
	...
	/* check reg on */
	lcd_kit_parse_get_u32_default(np, "lcd-kit,check-reg-on-support",
	...
	if (common_info->check_reg_on.support) {
		...
		lcd_kit_parse_get_u32_default(
			np, "lcd-kit,check-reg-on-support-dsm-report",
			&common_info->check_reg_on.support_dsm_report, 0);
	}
	/* check reg off */
	lcd_kit_parse_get_u32_default(np, "lcd-kit,check-reg-off-support",
