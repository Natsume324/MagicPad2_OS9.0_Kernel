...
static struct bias_ic_voltage tps65132_vol_table[] = {
	{LCD_BAIC_VOL_40, TPS65132_VOL_40}, {LCD_BAIC_VOL_41, TPS65132_VOL_41},
	{LCD_BAIC_VOL_42, TPS65132_VOL_42}, {LCD_BAIC_VOL_43, TPS65132_VOL_43},
	{LCD_BAIC_VOL_44, TPS65132_VOL_44}, {LCD_BAIC_VOL_45, TPS65132_VOL_45},
	{LCD_BAIC_VOL_46, TPS65132_VOL_46}, {LCD_BAIC_VOL_47, TPS65132_VOL_47},
	{LCD_BAIC_VOL_48, TPS65132_VOL_48}, {LCD_BAIC_VOL_49, TPS65132_VOL_49},
	{LCD_BAIC_VOL_50, TPS65132_VOL_50}, {LCD_BAIC_VOL_51, TPS65132_VOL_51},
	{LCD_BAIC_VOL_52, TPS65132_VOL_52}, {LCD_BAIC_VOL_53, TPS65132_VOL_53},
	{LCD_BAIC_VOL_54, TPS65132_VOL_54}, {LCD_BAIC_VOL_55, TPS65132_VOL_55},
	{LCD_BAIC_VOL_56, TPS65132_VOL_56}, {LCD_BAIC_VOL_57, TPS65132_VOL_57},
	{LCD_BAIC_VOL_58, TPS65132_VOL_58}, {LCD_BAIC_VOL_59, TPS65132_VOL_59},
	{LCD_BAIC_VOL_60, TPS65132_VOL_60}};

/* bias ic: rt4801h */
static struct bias_ic_voltage rt4801h_vol_table[] = {
	{LCD_BAIC_VOL_40, RT4801H_VOL_40}, {LCD_BAIC_VOL_41, RT4801H_VOL_41},
	{LCD_BAIC_VOL_42, RT4801H_VOL_42}, {LCD_BAIC_VOL_43, RT4801H_VOL_43},
	{LCD_BAIC_VOL_44, RT4801H_VOL_44}, {LCD_BAIC_VOL_45, RT4801H_VOL_45},
	{LCD_BAIC_VOL_46, RT4801H_VOL_46}, {LCD_BAIC_VOL_47, RT4801H_VOL_47},
	{LCD_BAIC_VOL_48, RT4801H_VOL_48}, {LCD_BAIC_VOL_49, RT4801H_VOL_49},
	{LCD_BAIC_VOL_50, RT4801H_VOL_50}, {LCD_BAIC_VOL_51, RT4801H_VOL_51},
	{LCD_BAIC_VOL_52, RT4801H_VOL_52}, {LCD_BAIC_VOL_53, RT4801H_VOL_53},
	{LCD_BAIC_VOL_54, RT4801H_VOL_54}, {LCD_BAIC_VOL_55, RT4801H_VOL_55},
	{LCD_BAIC_VOL_56, RT4801H_VOL_56}, {LCD_BAIC_VOL_57, RT4801H_VOL_57},
	{LCD_BAIC_VOL_58, RT4801H_VOL_58}, {LCD_BAIC_VOL_59, RT4801H_VOL_59},
	{LCD_BAIC_VOL_60, RT4801H_VOL_60}};

/* bias ic: lv52134a */
static struct bias_ic_voltage lv52134a_vol_table[] = {
	{LCD_BAIC_VOL_41, LV52134A_VOL_41}, {LCD_BAIC_VOL_42, LV52134A_VOL_42},
	{LCD_BAIC_VOL_43, LV52134A_VOL_43}, {LCD_BAIC_VOL_44, LV52134A_VOL_44},
	{LCD_BAIC_VOL_45, LV52134A_VOL_45}, {LCD_BAIC_VOL_46, LV52134A_VOL_46},
	{LCD_BAIC_VOL_47, LV52134A_VOL_47}, {LCD_BAIC_VOL_48, LV52134A_VOL_48},
	{LCD_BAIC_VOL_49, LV52134A_VOL_49}, {LCD_BAIC_VOL_50, LV52134A_VOL_50},
	{LCD_BAIC_VOL_51, LV52134A_VOL_51}, {LCD_BAIC_VOL_52, LV52134A_VOL_52},
	{LCD_BAIC_VOL_53, LV52134A_VOL_53}, {LCD_BAIC_VOL_54, LV52134A_VOL_54},
	{LCD_BAIC_VOL_55, LV52134A_VOL_55}, {LCD_BAIC_VOL_56, LV52134A_VOL_56},
...
static int bias_ic_check(const char *ic_name)
{
	...
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_TYPE,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.ic_type = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_CHECK_REG,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.check_reg = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_CHECK_VAL,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.check_val = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(
		ic_name, DTS_BIAS_IC_CHECK_MASK, &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.check_mask = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_I2C_ID,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.i2c_num = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_I2C_ADDR,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
...
static int bias_parse_config(const char *ic_name, int str_len)
{
	...
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_POS_REG,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.vpos_reg = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_POS_MASK,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.vpos_mask = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_NEG_REG,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.vneg_reg = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_NEG_MASK,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.vneg_mask = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_STATE_REG,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.state_reg = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_STATE_VAL,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.state_val = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(
		ic_name, DTS_BIAS_IC_STATE_MASK, &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.state_mask = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_WRITE_REG,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.write_reg = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(ic_name, DTS_BIAS_IC_WRITE_VAL,
						 &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.write_val = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(
		ic_name, DTS_BIAS_IC_WRITE_MASK, &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
	g_current_info.write_mask = (unsigned char)cfg_data;
	ret = bias_ops->dts_get_data_by_property(
		ic_name, DTS_BIAS_IC_WRITE_DELAY, &cfg_data, &cfg_len);
	if (ret < 0)
		return LCD_RET_FAIL;
...
static int bias_ic_verify(void)
{
	...
	for (i = 0; i < bias_ic_num; i++) {
		ret = bias_ops->dts_get_ic_name_index(DTS_BIAS_IC_LIST, i, name,
...
int bias_ic_init(void)
{
	...
	if (!bias_ops->dts_get_ic_num || !bias_ops->dts_set_ic_name ||
	...
	bias_ops->dts_set_ic_name(DTS_BIAS_IC_NAME, "default");
	...
	for (i = 0; i < bias_ic_num; i++) {
		...
		if (ret < 0) {
			...
			ret = bias_ops->dts_set_status_by_compatible(
				name, "disabled");
			if (ret < 0)
