			   ...
			   struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	if (adpat_ops->get_data_by_property)
		adpat_ops->get_data_by_property(np, propertyname, &data, &blen);
	/* scan dcs commands */
...
int lcd_kit_parse_get_u64_default(const char *np, const char *propertyname,
				  uint64_t *value, uint64_t def_val)
{
	u32 blen = 0;
	int *data = NULL;
	uint64_t *tmp = NULL;
...
int lcd_kit_parse_get_u32_default(const char *np, const char *propertyname,
				  u32 *value, u32 def_val)
{
	struct lcd_kit_adapt_ops *adpat_ops = NULL;
	int *data = NULL;
	u32 blen = 0;
...
int lcd_kit_parse_get_u8_default(const char *np, const char *propertyname,
				 u8 *value, u32 def_val)
{
	struct lcd_kit_adapt_ops *adpat_ops = NULL;
	int *data = NULL;
	u32 blen = 0;
			...
			struct lcd_kit_array_data *out)
{
	...
	if (adpat_ops->get_data_by_property) {
		ret = adpat_ops->get_data_by_property(np, propertyname, &data,
						      &blen);
		if (ret < 0)
			return LCD_KIT_FAIL;
	}
...
int lcd_kit_parse_u32_array(const char *np, const char *propertyname, u32 out[],
			    unsigned int len)
{
	int ret;
	u32 blen = 0;
			 ...
			 struct lcd_kit_arrays_data *out, u32 num)
{
	...
	out->arry_data = (struct lcd_kit_array_data *)mem_alloc(
		cnt * sizeof(struct lcd_kit_array_data));
	if (!out->arry_data) {
...
int lcd_kit_get_dts_u32_default(const void *fdt, int nodeoffset,
				const char *propertyname, u32 *value,
				u32 def_val)
{
	struct lcd_kit_adapt_ops *adpat_ops = NULL;
	int *data = NULL;
	...
	if ((fdt == NULL) || (propertyname == NULL) || (value == NULL)) {
			    ...
			    struct lcd_kit_array_data *out)
{
	...
	if ((fdt == NULL) || (propertyname == NULL) || (out == NULL)) {
		LCD_KIT_ERR("input parameter is NULL!\n");
		return LCD_KIT_FAIL;
	}
	if (out == NULL) {
	...
	if (adpat_ops->get_dts_data_by_property) {
		ret = adpat_ops->get_dts_data_by_property(
			fdt, nodeoffset, propertyname, &data, &blen);
		if (ret < 0)
			return LCD_KIT_FAIL;
	}
	if ((blen <= 0) || (blen > MAX_BLEN_LEN)) {
			       ...
			       struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	if ((fdt == NULL) || (propertyname == NULL) || (link_key == NULL) ||
	    (pcmds == NULL)) {
		LCD_KIT_ERR("input parameter is NULL!\n");
		return LCD_KIT_FAIL;
	}
	...
	if (link_key) {
		if (adpat_ops->get_dts_string_by_property)
			adpat_ops->get_dts_string_by_property(
				fdt, nodeoffset, link_key, link_key_str,
				sizeof(link_key_str));
		if (!strncmp(link_key_str, "dsi_lp_mode",
			     strlen("dsi_lp_mode")))
			pcmds->link_state = LCD_KIT_DSI_LP_MODE;
...
int lcd_kit_set_dts_data(const char *np, const char *property_name, u32 value)
{
	int ret;
	struct lcd_kit_adapt_ops *adpat_ops = NULL;
