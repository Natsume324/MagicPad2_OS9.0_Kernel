...
static int tps65132_i2c_read_u8(unsigned char addr, unsigned char *dataBuffer)
{
	...
	*dataBuffer = addr;
	...
	len = 1;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(tps65132_i2c.id, tps65132_i2c.addr, dataBuffer,
				 len, len, NULL);
#else
...
static int tps65132_i2c_write_u8(unsigned char addr, unsigned char value)
{
	...
	len = 2;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(tps65132_i2c.id, tps65132_i2c.addr, write_data, len,
			    NULL);
#else
	tps65132_i2c.mode = ST_MODE;
	tps65132_i2c.speed = 100;
...
#endif
{
	...
	offset =
		fdt_node_offset_by_compatible(kernel_fdt, 0, DTS_COMP_TPS65132);
	if (offset < 0) {
		LCD_KIT_ERR(
			"Could not find tps65132 node, change dts failed\n");
		return;
	}
...
int tps65132_init(void)
{
	...
	ret = lcd_kit_parse_get_u32_default(
		DTS_COMP_TPS65132, "ocp2138_support", &is_ocp2138_support, 0);
	...
	ret = lcd_kit_parse_get_u32_default(
		DTS_COMP_TPS65132, TPS65132_I2C_BUS_ID, &i2c_bus_id, 0);
	if (ret < 0) {
		i2c_bus_id = 0;
		return LCD_KIT_OK;
	}
