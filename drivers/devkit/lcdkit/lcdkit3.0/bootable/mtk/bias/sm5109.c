...
static int sm5109_i2c_read_u8(unsigned char addr, unsigned char *databuffer)
{
	...
	*databuffer = addr;
	...
	ret = i2c_write_read(&sm5109_i2c, databuffer, SM5109_CMD_LEN_ONE,
			     SM5109_CMD_LEN_ONE);
#endif
	if (ret != 0)
...
static int sm5109_i2c_write_u8(unsigned char addr, unsigned char value)
{
	...
	sm5109_i2c.addr = SM5109_SLAV_ADDR;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(sm5109_i2c.id, sm5109_i2c.addr, write_data,
			    SM5109_CMD_LEN_TWO, NULL);
#else
	sm5109_i2c.mode = ST_MODE;
	sm5109_i2c.speed = SM5109_I2C_SPEED;
...
static int sm5109_reg_init(unsigned char vpos_cmd, unsigned char vneg_cmd)
{
	...
	if (ret != 0)
		LCD_KIT_ERR(
			"read vol_save_value failed,vol_save_value = 0x%x\n",
			vol_save_value);
	...
	if (ret != 0)
		LCD_KIT_ERR(
			"save vol value failed, only apply to ocp2131, if ic is sm5109, err ingnore\n");
...
int sm5109_init(void)
{
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_SM5109, SM5109_I2C_BUS_ID,
					    &i2c_bus_id, 0);
	if (ret < 0) {
		i2c_bus_id = 0;
		return LCD_KIT_OK;
