...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(i2c_info.id, i2c_info.addr, val, len, len,
				 NULL);
#else
				    ...
				    unsigned int *len)
{
	...
	if (compatible == NULL || prop_name == NULL || data == NULL ||
	    len == NULL) {
...
static void dts_set_ic_name(const char *prop_name, const char *name)
{
	...
	ret = fdt_setprop_string(g_fdt, offset, prop_name, (const void *)name);
	if (ret) {
...
static int dts_get_ic_name_index(const char *prop_name, int idx, char *name_buf,
				 int buf_len)
{
	int offset;
	int length = 0;
