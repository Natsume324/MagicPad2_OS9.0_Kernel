...
static int lcd_kit_get_data_by_property(const char *compatible,
					const char *propertyname, int **data,
					int *len)
{
	int offset;
...
static int lcd_kit_get_dts_data_by_property(const void *pfdt, int offset,
					    ...
					    int **data, unsigned int *len)
{
	const struct fdt_property *property = NULL;

	if ((pfdt == NULL) || (propertyname == NULL) || (data == NULL) ||
	    (len == NULL)) {
					      ...
					      unsigned int length)
{
	...
	if ((pfdt == NULL) || (propertyname == NULL) || (out_str == NULL)) {
		LCD_KIT_ERR("input parameter is NULL!\n");
		return LCD_KIT_FAIL;
	}
...
static int parse_iovcc_dts(uint32_t *iovcc_ctrl_mode, uint32_t *ldo_gpio)
{
	...
	/* get dts index 0 */
	ret = get_dts_u32_index(DTS_COMP_LCD_PANEL_TYPE, "iovcc_lcd", 0,
				iovcc_ctrl_mode);
	if (ret < 0) {
	...
	if (*iovcc_ctrl_mode == IOVCC_CTRL_GPIO) {
		/* get dts index 1 */
		ret = get_dts_u32_index(DTS_COMP_LCD_PANEL_TYPE, "iovcc_lcd", 1,
					ldo_gpio);
		if (ret < 0)
...
static void iovcc_power_ctrl(bool power_on, uint32_t iovcc_ctrl_mode,
			     uint32_t ldo_gpio)
{
	if (power_on) {
		if (iovcc_ctrl_mode == IOVCC_CTRL_REGULATOR) {
			LCD_KIT_INFO(
				"Power on iovcc ctrl by regulator, mode=%d!\n",
				iovcc_ctrl_mode);
		...
		if (iovcc_ctrl_mode == IOVCC_CTRL_REGULATOR) {
			LCD_KIT_INFO(
				"Power off iovcc ctrl by regulator, mode=%d!\n",
				iovcc_ctrl_mode);
...
uint32_t lcdkit_get_lcd_id(void)
{
	...
	(void)get_dts_u32_index(DTS_COMP_LCD_PANEL_TYPE,
				"iovcc_keep_on_after_id_detect", 0,
				&iovcc_keep_on);

	iovcc_power_ctrl(true, iovcc_ctrl_mode, ldo_gpio);

	for (i = 0; i < lcdid_count; i++) {
		ret = get_dts_u32_index(DTS_COMP_LCD_PANEL_TYPE, "gpio_id", i,
					&gpio_id);
...
#endif
{
	...
	ret = fdt_setprop_cell(kernel_fdt, offset,
			       (const char *)"panel_ddic_max_brightness",
			       bl_val);
	if (ret)
