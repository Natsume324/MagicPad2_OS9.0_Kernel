...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(lm36274_i2c.id, lm36274_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int lm36274_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				  unsigned char value)
{
	int ret;
	...
	lm36274_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(lm36274_i2c.id, lm36274_i2c.addr, write_data, len,
			    NULL);
#else
	lm36274_i2c.mode = ST_MODE;
	lm36274_i2c.speed = LM36274_I2C_SPEED;
...
static int lm36274_i2c_write_u8(u8 chip_no, unsigned char addr,
				unsigned char value)
{
	int ret;
	unsigned char write_data[LM36274_WRITE_LEN] = {0};
	...
	if (lm36274_bl_info.dual_ic) {
		if (addr == REG_DISPLAY_BIAS_CONFIG_1)
			lm36274_2_i2c_write_u8(
				chip_no, addr,
				lm36274_bl_info.lm36274_2_display_bias_cfg_1);
		else
...
static int lm36274_parse_dts()
{
	...
	for (i = 0; i < LM36274_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(
			lm36274_bl_info.pfdt, lm36274_bl_info.nodeoffset,
...
static int lm36274_config_register()
{
	...
	int i;
	for (i = 0; i < LM36274_RW_REG_MAX; i++) {
		ret = lm36274_i2c_write_u8(LM36274_SLAV_ADDR,
					   lm36274_reg_addr[i],
					   lm36274_bl_info.lm36274_reg[i]);
		if (ret < 0) {
			LCD_KIT_ERR(
...
static int lm36274_device_verify(void)
{
	...
	ret = lcd_kit_get_dts_u32_default(lm36274_bl_info.pfdt,
					  lm36274_bl_info.nodeoffset,
					  LM36274_EXTEND_REV_SUPPORT,
					  &is_support, 0);
	if (ret < 0) {
		...
		if (is_support == LM36274_EXTEND_REV_VAL) {
			...
			ret = lm36274_i2c_read_u8(LM36274_SLAV_ADDR, &reg_val,
						  REG_BL_CONFIG_1);
			if (ret < 0) {
				LCD_KIT_ERR(
					"read lm36274 bl config 1 failed\n");
				goto error_exit;
			}
			LCD_KIT_INFO("lm32674 reg bl config1 = 0x%x\n",
				     reg_val);
			if ((reg_val & DEV_MASK_EXTEND_LM32674) ==
			    VENDOR_ID_EXTEND_LM32674) {
				LCD_KIT_INFO(
					"lm36274 check vendor id exend failed\n");
				ret = LCD_KIT_FAIL;
				goto error_exit;
			}
		}
...
int lm36274_init(struct mtk_panel_info *pinfo)
{
	...
	if (lm36274_bl_info.dual_ic) {
		...
		ret = lcd_kit_get_dts_u32_default(
			lm36274_bl_info.pfdt, lm36274_bl_info.nodeoffset,
			LM36274_2_DISPLAY_BIAS_CFG_1,
			&lm36274_bl_info.lm36274_2_display_bias_cfg_1, 0);
		if (ret < 0) {
			LCD_KIT_ERR(
				"parse dts lm36274_2_display_bias_cfg_1 fail!\n");
			return LCD_KIT_FAIL;
		}
	}
	...
	if (lm36274_bl_info.lm36274_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			lm36274_bl_info.pfdt, lm36274_bl_info.nodeoffset,
			LM36274_HW_EN_GPIO, &lm36274_bl_info.lm36274_hw_en_gpio,
			0);
		if (ret < 0) {
		...
		ret = lcd_kit_get_dts_u32_default(
			lm36274_bl_info.pfdt, lm36274_bl_info.nodeoffset,
			LM36274_HW_EN_DELAY, &lm36274_bl_info.bl_on_lk_mdelay,
			0);
		if (ret < 0)
	...
	ret = lcd_kit_get_dts_u32_default(lm36274_bl_info.pfdt,
					  lm36274_bl_info.nodeoffset,
					  LM36274_BL_LEVEL,
					  &lm36274_bl_info.bl_level,
					  LM36274_BL_DEFAULT_LEVEL);
	if (ret < 0)
...
int lm36274_set_backlight(uint32_t bl_level)
{
	...
	ret = lm36274_i2c_update_bits(LM36274_SLAV_ADDR, REG_BL_BRIGHTNESS_LSB,
				      MASK_BL_LSB, bl_lsb);
	if (ret < 0)
		goto i2c_error;

	ret = lm36274_i2c_write_u8(LM36274_SLAV_ADDR, REG_BL_BRIGHTNESS_MSB,
				   bl_msb);
	if (ret < 0)
		goto i2c_error;
	if (lm36274_bl_info.dual_ic) {
		ret = lm36274_2_i2c_update_bits(LM36274_SLAV_ADDR,
						REG_BL_BRIGHTNESS_LSB,
						MASK_BL_LSB, bl_lsb);
		if (ret < 0)
			goto i2c_error;

		ret = lm36274_2_i2c_write_u8(LM36274_SLAV_ADDR,
					     REG_BL_BRIGHTNESS_MSB, bl_msb);
		if (ret < 0)
			goto i2c_error;
	}
