...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(mp3314_i2c.id, mp3314_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int mp3314_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	*data_buffer = addr;
	...
	if (mp3314_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"mp3314_2_i2c_read_u8  failed! reg is 0x%xret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int mp3314_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				 unsigned char value)
{
	int ret;
	...
	mp3314_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(mp3314_i2c.id, mp3314_i2c.addr, write_data, len,
			    NULL);
#else
	mp3314_i2c.mode = ST_MODE;
	mp3314_i2c.speed = MP3314_I2C_SPEED;
...
static int mp3314_i2c_write_u8(u8 chip_no, unsigned char addr,
			       unsigned char value)
{
	int ret;
	unsigned char write_data[MP3314_WRITE_LEN] = {0};
	...
	if (mp3314_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"mp3314_2_i2c_write_u8  failed! reg is  0x%x ret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int mp3314_parse_dts()
{
	...
	for (i = 0; i < MP3314_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(
			mp3314_bl_info.pfdt, mp3314_bl_info.nodeoffset,
			mp3314_dts_string[i], &mp3314_bl_info.mp3314_reg[i], 0);
		if (ret < 0) {
...
static int mp3314_config_register(void)
{
	...
	if (tmp1 != 0xff) {
		LCD_KIT_INFO(
			"mp3314 backlight reg is not 0xff, mp3314 not need to config, return");
		return ret;
	}
	/*Disable bl before writing the register*/
	ret = mp3314_i2c_write_u8(MP3314_SLAV_ADDR, MP3314_BL_EN_CTL,
				  BL_DISABLE_CTL);
	if (ret < 0) {
	...
	for (i = 0; i < MP3314_RW_REG_MAX; i++) {
		...
		if (mp3314_bl_info.mp3314_reg[i] != 0xffff) {
			ret = mp3314_i2c_write_u8(
				MP3314_SLAV_ADDR, (u8)mp3314_reg_addr[i],
				(u8)mp3314_bl_info.mp3314_reg[i]);
			if (ret < 0) {
				LCD_KIT_ERR(
...
static int mp3314_backlight_ic_check(void)
{
	...
	ret = mp3314_i2c_read_u8(MP3314_SLAV_ADDR,
				 (u8 *)&mp3314_bl_info.mp3314_reg[0],
				 mp3314_reg_addr[0]);
	if (ret < 0) {
...
int mp3314_init(struct mtk_panel_info *pinfo)
{
	...
	if (mp3314_bl_info.mp3314_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			mp3314_bl_info.pfdt, mp3314_bl_info.nodeoffset,
			MP3314_HW_EN_GPIO, &mp3314_bl_info.mp3314_hw_en_gpio,
			0);
		if (ret < 0) {
		...
		if (mp3314_bl_info.dual_ic) {
			...
			if (ret < 0) {
				LCD_KIT_ERR(
					"parse dts mp3314_2_hw_en_gpio fail!\n");
				mp3314_bl_info.mp3314_2_hw_en_gpio = 0;
				goto exit;
			}
		...
		ret = lcd_kit_get_dts_u32_default(
			mp3314_bl_info.pfdt, mp3314_bl_info.nodeoffset,
			MP3314_HW_EN_DELAY, &mp3314_bl_info.bl_on_lk_mdelay, 0);
		if (ret < 0)
			LCD_KIT_INFO("parse dts bl_on_lk_mdelay fail!\n");
	}

	ret = lcd_kit_get_dts_u32_default(
		mp3314_bl_info.pfdt, mp3314_bl_info.nodeoffset, MP3314_BL_LEVEL,
