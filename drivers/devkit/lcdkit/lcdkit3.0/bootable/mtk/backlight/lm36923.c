...
static char *lm36923_dts_string[LM36923_RW_REG_MAX] = {
	"lm36923_reg_brightness_conctrol", "lm36923_reg_lsb", "lm36923_reg_msb",
	"lm36923_reg_leds_config"};

static unsigned int lm36923_reg_addr[LM36923_RW_REG_MAX] = {
	LM36923_REG_BRIGHTNESS_CONTROL, LM36923_REG_RATIO_LSB,
	LM36923_REG_RATIO_MSB, LM36923_REG_LEDS_CONFIG};
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(lm36923_i2c.id, lm36923_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int lm36923_i2c_write_u8(unsigned char addr, unsigned char value)
{
	...
	lm36923_i2c.addr = LM36923_SLAV_ADDR;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(lm36923_i2c.id, lm36923_i2c.addr, write_data, len,
			    NULL);
#else
	lm36923_i2c.mode = ST_MODE;
	lm36923_i2c.speed = LM36923_I2C_SPEED;
...
static int lm36923_parse_dts(void)
{
	...
	for (i = 0; i < LM36923_RW_REG_MAX; i++) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_LM36923, lm36923_dts_string[i],
...
static int lm36923_config_register(void)
{
	...
	for (i = 0; i < LM36923_RW_REG_MAX; i++) {
		if (lm36923_bl_info.lm36923_reg[i] != LM36923_INVALID_VAL) {
			ret = lm36923_i2c_write_u8(
				lm36923_reg_addr[i],
				(u8)lm36923_bl_info.lm36923_reg[i]);
			if (ret < 0) {
				LCD_KIT_ERR(
					"write lm36923 backlight config 0x%x failed\n",
					lm36923_reg_addr[i]);
				return ret;
			}
		}
...
static int lm36923_set_backlight(unsigned int bl_level)
{
	...
	bl_level =
		bl_level * lm36923_bl_info.bl_level / LM36923_BL_DEFAULT_LEVEL;
...
int lm36923_init(void)
{
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_LM36923, LM36923_SUPPORT,
					    &lm36923_bl_info.lm36923_support,
					    0);
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_LM36923,
					    LM36923_I2C_BUS_ID,
					    &lm36923_bl_info.lm36923_i2c_bus_id,
					    0);
	if (ret < 0) {
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_LM36923, LM36923_HW_ENABLE,
	...
	if (lm36923_bl_info.lm36923_hw_en) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_LM36923, LM36923_HW_EN_GPIO,
		...
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_LM36923, LM36923_HW_EN_DELAY,
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_LM36923, LM36923_BL_LEVEL,
