...
static char *sgm37603a_dts_string[SGM37603A_RW_REG_MAX] = {
	"sgm37603a_reg_brightness_conctrol", "sgm37603a_reg_lsb",
	"sgm37603a_reg_msb", "sgm37603a_reg_leds_config"};

static unsigned int sgm37603a_reg_addr[SGM37603A_RW_REG_MAX] = {
	SGM37603A_REG_BRIGHTNESS_CONTROL, SGM37603A_REG_RATIO_LSB,
	SGM37603A_REG_RATIO_MSB, SGM37603A_REG_LEDS_CONFIG};
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(sgm37603a_i2c.id, sgm37603a_i2c.addr,
...
static int sgm37603a_i2c_write_u8(unsigned char addr, unsigned char value)
{
	...
	sgm37603a_i2c.addr = SGM37603A_SLAV_ADDR;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(sgm37603a_i2c.id, sgm37603a_i2c.addr, write_data,
			    len, NULL);
#else
	sgm37603a_i2c.mode = ST_MODE;
	sgm37603a_i2c.speed = SGM37603A_I2C_SPEED;
...
static int sgm37603a_parse_dts(void)
{
	...
	for (i = 0; i < SGM37603A_RW_REG_MAX; i++) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_SGM37603A, sgm37603a_dts_string[i],
...
static int sgm37603a_config_register(void)
{
	...
	for (i = 0; i < SGM37603A_RW_REG_MAX; i++) {
		if (sgm37603a_bl_info.sgm37603a_reg[i] !=
		    SGM37603A_INVALID_VAL) {
			ret = sgm37603a_i2c_write_u8(
				sgm37603a_reg_addr[i],
				(u8)sgm37603a_bl_info.sgm37603a_reg[i]);
			if (ret < 0) {
				LCD_KIT_ERR(
					"write sgm37603a backlight config 0x%x failed\n",
					sgm37603a_reg_addr[i]);
				return ret;
			}
		}
...
static int sgm37603a_set_backlight(unsigned int bl_level)
{
	...
	bl_level = bl_level * sgm37603a_bl_info.bl_level /
...
#endif
{
	...
	offset = fdt_node_offset_by_compatible(kernel_fdt, 0,
					       DTS_COMP_SGM37603A);
	if (offset < 0) {
		LCD_KIT_ERR(
			"Could not find sgm37603a node, change dts failed\n");
		return;
	}
...
int sgm37603a_init(void)
{
	...
	ret = lcd_kit_parse_get_u32_default(
		DTS_COMP_SGM37603A, SGM37603A_SUPPORT,
		&sgm37603a_bl_info.sgm37603a_support, 0);
	...
	ret = lcd_kit_parse_get_u32_default(
		DTS_COMP_SGM37603A, SGM37603A_I2C_BUS_ID,
	...
	if (sgm37603a_bl_info.sgm37603a_hw_en) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_SGM37603A, SGM37603A_HW_EN_GPIO,
		...
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_SGM37603A, SGM37603A_HW_EN_DELAY,
