...
static char *rt4539_dts_string[RT4539_RW_REG_MAX] = {
	"rt4539_eprom_cfg00", "rt4539_eprom_cfg01", "rt4539_eprom_cfg02",
	"rt4539_eprom_cfg03", "rt4539_eprom_cfg06", "rt4539_eprom_cfg07",
	"rt4539_eprom_cfg08", "rt4539_eprom_cfg09", "rt4539_eprom_cfg0A",
	"rt4539_eprom_cfg05", "rt4539_eprom_cfg04", "rt4539_bl_control",
};
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(rt4539_i2c.id, rt4539_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int rt4539_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	*data_buffer = addr;
	...
	if (rt4539_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"rt4539_2_i2c_read_u8  failed! reg is 0x%xret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int rt4539_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				 unsigned char value)
{
	int ret;
	...
	rt4539_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(rt4539_i2c.id, rt4539_i2c.addr, write_data, len,
			    NULL);
#else
	rt4539_i2c.mode = ST_MODE;
	rt4539_i2c.speed = RT4539_I2C_SPEED;
...
static int rt4539_i2c_write_u8(u8 chip_no, unsigned char addr,
			       unsigned char value)
{
	int ret;
	unsigned char write_data[RT4539_WRITE_LEN] = {0};
	...
	if (rt4539_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"rt4539_2_i2c_write_u8  failed! reg is  0x%x ret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int rt4539_parse_dts()
{
	...
	for (i = 0; i < RT4539_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(
			rt4539_bl_info.pfdt, rt4539_bl_info.nodeoffset,
			rt4539_dts_string[i], &rt4539_bl_info.rt4539_reg[i], 0);
		if (ret < 0) {
...
static int rt4539_config_register(void)
{
	...
	ret = rt4539_i2c_write_u8(RT4539_SLAV_ADDR, RT4539_BL_CONTROL,
				  BL_DISABLE_CTL);
	if (ret < 0) {
	...
	for (i = 0; i < RT4539_RW_REG_MAX; i++) {
		...
		if (rt4539_bl_info.rt4539_reg[i] != 0xffff) {
			ret = rt4539_i2c_write_u8(
				RT4539_SLAV_ADDR, (u8)rt4539_reg_addr[i],
				(u8)rt4539_bl_info.rt4539_reg[i]);
			if (ret < 0) {
				LCD_KIT_ERR(
					"write rt4539 backlight config register 0x%x failed\n",
					rt4539_reg_addr[i]);
				return ret;
			}
		}
...
static int rt4539_backlight_ic_check(void)
{
	...
	ret = rt4539_i2c_read_u8(RT4539_SLAV_ADDR,
				 (u8 *)&rt4539_bl_info.rt4539_reg[0],
				 rt4539_reg_addr[0]);
	if (ret < 0) {
...
int rt4539_init(struct mtk_panel_info *pinfo)
{
	...
	if (rt4539_bl_info.rt4539_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			rt4539_bl_info.pfdt, rt4539_bl_info.nodeoffset,
			RT4539_HW_EN_GPIO, &rt4539_bl_info.rt4539_hw_en_gpio,
			0);
		if (ret < 0) {
		...
		if (rt4539_bl_info.dual_ic) {
			...
			if (ret < 0) {
				LCD_KIT_ERR(
					"parse dts rt4539_2_hw_en_gpio fail!\n");
				rt4539_bl_info.rt4539_2_hw_en_gpio = 0;
				goto exit;
			}
		...
		ret = lcd_kit_get_dts_u32_default(
			rt4539_bl_info.pfdt, rt4539_bl_info.nodeoffset,
			RT4539_HW_EN_DELAY, &rt4539_bl_info.bl_on_lk_mdelay, 0);
		if (ret < 0)
			LCD_KIT_INFO("parse dts bl_on_lk_mdelay fail!\n");
	}

	ret = lcd_kit_get_dts_u32_default(
		rt4539_bl_info.pfdt, rt4539_bl_info.nodeoffset, RT4539_BL_LEVEL,
