...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(sm5350_i2c.id, sm5350_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int sm5350_i2c_write_u8(unsigned char addr, unsigned char value)
{
	...
	sm5350_i2c.addr = SM5350_SLAV_ADDR;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(sm5350_i2c.id, sm5350_i2c.addr, write_data, len,
			    NULL);
#else
	sm5350_i2c.mode = ST_MODE;
	sm5350_i2c.speed = SM5350_I2C_SPEED;
...
static void sm5350_parse_dts(void)
{
	...
	for (i = 0; i < SM5350_RW_REG_MAX; i++) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_SM5350, sm5350_dts_string[i],
...
int sm5350_init(struct mtk_panel_info *pinfo)
{
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_SM5350, SM5350_SUPPORT,
					    &sm5350_bl_info.sm5350_support, 0);
	if (ret < 0 || !sm5350_bl_info.sm5350_support) {
		LCD_KIT_ERR("not support sm5350!\n");
		return LCD_KIT_FAIL;
	}

	ret = lcd_kit_parse_get_u32_default(DTS_COMP_SM5350, SM5350_I2C_BUS_ID,
					    &sm5350_bl_info.sm5350_i2c_bus_id,
					    0);
	if (ret < 0) {
		LCD_KIT_ERR("parse dts sm5350_i2c_bus_id fail!\n");
		return LCD_KIT_FAIL;
	}
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_SM5350, SM5350_HW_ENABLE,
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_SM5350, SM5350_BL_LEVEL,
					    &sm5350_bl_info.bl_level,
	...
	if (sm5350_bl_info.sm5350_hw_en) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_SM5350, SM5350_HW_EN_GPIO,
		...
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_SM5350, SM5350_HW_EN_DELAY,
