...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(rt8555_i2c.id, rt8555_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int rt8555_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	*data_buffer = addr;
	...
	if (g_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"rt8555_2_i2c_read_u8  failed! reg is 0x%xret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int rt8555_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				 unsigned char value)
{
	int ret;
	...
	rt8555_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(rt8555_i2c.id, rt8555_i2c.addr, write_data, len,
			    NULL);
#else
	rt8555_i2c.mode = ST_MODE;
	rt8555_i2c.speed = RT8555_I2C_SPEED;
...
static int rt8555_i2c_write_u8(u8 chip_no, unsigned char addr,
			       unsigned char value)
{
	int ret;
	unsigned char write_data[RT8555_WRITE_LEN] = {0};
	...
	if (g_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"rt8555_2_i2c_write_u8  failed! reg is  0x%x ret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int rt8555_set_backlight(uint32_t bl_level)
{
	...
	ret = rt8555_i2c_write_u8(RT8555_SLAV_ADDR, g_bl_info.rt8555_level_lsb,
				  bl_lsb);
	if (ret < 0)
		LCD_KIT_ERR("write bl level lsb:0x%x failed\n", bl_lsb);
	ret = rt8555_i2c_write_u8(RT8555_SLAV_ADDR, g_bl_info.rt8555_level_msb,
				  bl_msb);
	if (ret < 0)
...
static int rt8555_backlight_ic_check(void)
{
	...
	ret = rt8555_i2c_read_u8(RT8555_SLAV_ADDR, (u8 *)&g_bl_info.reg[0],
				 g_reg_addr[0]);
	if (ret < 0) {
...
int rt8555_init(struct mtk_panel_info *pinfo)
{
	...
	ret = lcd_kit_get_dts_u32_default(g_bl_info.pfdt, g_bl_info.nodeoffset,
					  RT8555_SUPPORT,
	...
	ret = lcd_kit_get_dts_u32_default(g_bl_info.pfdt, g_bl_info.nodeoffset,
					  RT8555_HW_DUAL_IC, &g_bl_info.dual_ic,
					  0);
	if (ret < 0)
	...
	if (g_bl_info.dual_ic) {
		ret = lcd_kit_get_dts_u32_default(
			g_bl_info.pfdt, g_bl_info.nodeoffset,
			RT8555_2_I2C_BUS_ID, &g_bl_info.rt8555_2_i2c_bus_id, 0);
		if (ret < 0) {
	...
	if (g_bl_info.rt8555_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			g_bl_info.pfdt, g_bl_info.nodeoffset, RT8555_HW_EN_GPIO,
		...
		if (g_bl_info.dual_ic) {
			...
			if (ret < 0) {
				LCD_KIT_ERR(
					"parse dts rt8555_2_hw_en_gpio fail!\n");
				g_bl_info.rt8555_2_hw_en_gpio = 0;
				goto exit;
			}
		}
		ret = lcd_kit_get_dts_u32_default(
			g_bl_info.pfdt, g_bl_info.nodeoffset,
			RT8555_HW_EN_DELAY, &g_bl_info.bl_on_lk_mdelay, 0);
		if (ret < 0)
			LCD_KIT_INFO("parse dts bl_on_lk_mdelay fail!\n");
	}

	ret = lcd_kit_get_dts_u32_default(g_bl_info.pfdt, g_bl_info.nodeoffset,
					  RT8555_BL_LEVEL, &g_bl_info.bl_level,
					  RT8555_BL_DEFAULT_LEVEL);
	if (ret < 0)
