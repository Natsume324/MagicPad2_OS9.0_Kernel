...
static char *ktd3136_dts_string[KTD3136_RW_REG_MAX] = {
	"ktd3136_reg_ctrl_a_bl_cfg", "ktd3136_reg_ctrl_a_lsb",
	"ktd3136_reg_ctrl_a_msb",    "ktd3136_reg_pwm_cfg",
	"ktd3136_reg_boost_ctrl",    "ktd3136_reg_ctrl_a_current_set"};

static unsigned int ktd3136_reg_addr[KTD3136_RW_REG_MAX] = {
	KTD3136_REG_CTRL_BL_CONFIG,	 KTD3136_REG_CTRL_BRIGHTNESS_LSB,
	KTD3136_REG_CTRL_BRIGHTNESS_MSB, KTD3136_REG_CTRL_PWM_CONFIG,
	KTD3136_REG_CTRL_BOOT_CTRL,	 KTD3136_REG_CTRL_CURRENT_SET};
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(ktd3136_i2c.id, ktd3136_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int ktd3136_i2c_write_u8(unsigned char addr, unsigned char value)
{
	...
	ktd3136_i2c.addr = KTD3136_SLAV_ADDR;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(ktd3136_i2c.id, ktd3136_i2c.addr, write_data, len,
			    NULL);
#else
	ktd3136_i2c.mode = ST_MODE;
	ktd3136_i2c.speed = KTD3136_I2C_SPEED;
...
static void ktd3136_parse_dts(void)
{
	...
	for (i = 0; i < KTD3136_RW_REG_MAX; i++) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_KTD3136, ktd3136_dts_string[i],
...
static int ktd3136_set_backlight(unsigned int bl_level)
{
	...
	bl_level =
		bl_level * ktd3136_bl_info.bl_level / KTD3136_BL_DEFAULT_LEVEL;
...
int ktd3136_init(struct mtk_panel_info *pinfo)
{
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_KTD3136, KTD3136_SUPPORT,
					    &ktd3136_bl_info.ktd3136_support,
					    0);
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_KTD3136,
					    KTD3136_I2C_BUS_ID,
					    &ktd3136_bl_info.ktd3136_i2c_bus_id,
					    0);
	if (ret < 0) {
		LCD_KIT_ERR("parse dts ktd3136_i2c_bus_id fail!\n");
		return LCD_KIT_FAIL;
	}
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_KTD3136, KTD3136_HW_ENABLE,
	...
	ret = lcd_kit_parse_get_u32_default(DTS_COMP_KTD3136, KTD3136_BL_LEVEL,
					    &ktd3136_bl_info.bl_level,
	...
	if (ktd3136_bl_info.ktd3136_hw_en) {
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_KTD3136, KTD3136_HW_EN_GPIO,
		...
		ret = lcd_kit_parse_get_u32_default(
			DTS_COMP_KTD3136, KTD3136_HW_EN_DELAY,
