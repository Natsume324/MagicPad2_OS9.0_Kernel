...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(sy7758_i2c.id, sy7758_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int sy7758_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	*data_buffer = addr;
	...
	if (sy7758_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"sy7758_2_i2c_read_u8  failed! reg is 0x%xret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int sy7758_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				 unsigned char value)
{
	int ret;
	...
	sy7758_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(sy7758_i2c.id, sy7758_i2c.addr, write_data, len,
			    NULL);
#else
	sy7758_i2c.mode = ST_MODE;
	sy7758_i2c.speed = SY7758_I2C_SPEED;
...
static int sy7758_i2c_write_u8(u8 chip_no, unsigned char addr,
			       unsigned char value)
{
	int ret;
	unsigned char write_data[SY7758_WRITE_LEN] = {0};
	...
	if (sy7758_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"sy7758_2_i2c_write_u8  failed! reg is  0x%x ret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int sy7758_parse_dts()
{
	...
	for (i = 0; i < SY7758_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(
			sy7758_bl_info.pfdt, sy7758_bl_info.nodeoffset,
			sy7758_dts_string[i], &sy7758_bl_info.sy7758_reg[i], 0);
		if (ret < 0) {
...
static int sy7758_config_register(void)
{
	...
	for (i = 0; i < SY7758_INIT_CMD_NUM; i++) {
		...
		if (sy7758_bl_info.sy7758_reg[i] != 0xffff) {
			ret = sy7758_i2c_write_u8(
				SY7758_SLAV_ADDR, (u8)sy7758_reg_addr[i],
				(u8)sy7758_bl_info.sy7758_reg[i]);
			if (ret < 0) {
				LCD_KIT_ERR(
...
static void lcd_kit_backlight_ctrl_enable(void)
{
	...
	ret = sy7758_i2c_write_u8(
		SY7758_SLAV_ADDR, sy7758_reg_addr[SY7758_RW_REG_MAX - 1],
		sy7758_bl_info.sy7758_reg[SY7758_RW_REG_MAX - 1]);
	if (ret < 0) {
...
static int sy7758_backlight_ic_check(void)
{
	...
	sy7758_parse_dts();
	sy7758_enable();
	ret = sy7758_i2c_read_u8(SY7758_SLAV_ADDR, (u8 *)&temp,
				 sy7758_reg_addr[0]);
	if (ret < 0) {
...
int sy7758_init(struct mtk_panel_info *pinfo)
{
	...
	if (sy7758_bl_info.sy7758_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			sy7758_bl_info.pfdt, sy7758_bl_info.nodeoffset,
			SY7758_HW_EN_GPIO, &sy7758_bl_info.sy7758_hw_en_gpio,
			0);
		if (ret < 0) {
		...
		if (sy7758_bl_info.dual_ic) {
			...
			if (ret < 0) {
				LCD_KIT_ERR(
					"parse dts sy7758_2_hw_en_gpio fail!\n");
				sy7758_bl_info.sy7758_2_hw_en_gpio = 0;
				goto exit;
			}
			if (sy7758_bl_info.sy7758_hw_en_gpio ==
		...
		ret = lcd_kit_get_dts_u32_default(
			sy7758_bl_info.pfdt, sy7758_bl_info.nodeoffset,
			SY7758_HW_EN_DELAY, &sy7758_bl_info.bl_on_lk_mdelay, 0);
		if (ret < 0)
			LCD_KIT_INFO("parse dts bl_on_lk_mdelay fail!\n");
	}

	ret = lcd_kit_get_dts_u32_default(
		sy7758_bl_info.pfdt, sy7758_bl_info.nodeoffset, SY7758_BL_LEVEL,
