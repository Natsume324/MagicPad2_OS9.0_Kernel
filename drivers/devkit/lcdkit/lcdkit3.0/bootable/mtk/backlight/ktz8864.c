...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(ktz8864_i2c.id, ktz8864_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int ktz8864_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				  unsigned char value)
{
	int ret;
...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(ktz8864_i2c.id, ktz8864_i2c.addr, write_data, len,
			    NULL);
#else
	ktz8864_i2c.mode = ST_MODE;
	ktz8864_i2c.speed = KTZ8864_I2C_SPEED;
...
static int ktz8864_i2c_write_u8(u8 chip_no, unsigned char addr,
				unsigned char value)
{
	int ret;
	unsigned char write_data[KTZ8864_WRITE_LEN] = {0};
...
static void ktz8864_get_target_voltage(int *vpos, int *vneg)
{
	int i;
	int tal_size =
		sizeof(ktz8864_vol_table) / sizeof(struct ktz8864_voltage);
...
static int ktz8864_config_register(void)
{
	...
	for (i = 0; i < KTZ8864_RW_REG_MAX; i++) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"write ktz backlight config register 0x%x failed",
				ktz8864_reg_addr[i]);
			return ret;
		}
	}
...
static int ktz8864_set_backlight(uint32_t bl_level)
{
	...
	if (ret < 0)
		goto i2c_error;
	if (bl_info.dual_ic) {
		ret = ktz8864_2_i2c_update_bits(KTZ8864_SLAV_ADDR,
						REG_BL_BRIGHTNESS_LSB,
						MASK_BL_LSB, bl_lsb);
		if (ret < 0)
			goto i2c_error;

		ret = ktz8864_2_i2c_write_u8(KTZ8864_SLAV_ADDR,
...
int ktz8864_init(struct mtk_panel_info *pinfo)
{
	...
	bl_info.nodeoffset = fdt_node_offset_by_compatible(
		bl_info.pfdt, OFFSET_DEF_VAL, DTS_COMP_KTZ8864);
	...
	ret = lcd_kit_get_dts_u32_default(bl_info.pfdt, bl_info.nodeoffset,
					  KTZ8864_HW_DUAL_IC, &bl_info.dual_ic,
					  0);
	if (ret < 0)
	...
	if (bl_info.dual_ic) {
		ret = lcd_kit_get_dts_u32_default(
			bl_info.pfdt, bl_info.nodeoffset, KTZ8864_2_I2C_BUS_ID,
		...
		ret = lcd_kit_get_dts_u32_default(
			bl_info.pfdt, bl_info.nodeoffset,
			KTZ8864_2_DISPLAY_BIAS_CFG_1,
			&bl_info.ktz8864_2_display_bias_cfg_1, 0);
		if (ret < 0) {
			LCD_KIT_ERR(
				"parse dts ktz8864_2_display_bias_cfg_1 fail!\n");
			return SUCC;
		}
	}
	...
	if (bl_info.ktz8864_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			bl_info.pfdt, bl_info.nodeoffset, KTZ8864_HW_EN_GPIO,
	...
	ret = lcd_kit_get_dts_u32_default(bl_info.pfdt, bl_info.nodeoffset,
					  KTZ8864_BL_LEVEL, &bl_info.bl_level,
					  KTZ8864_BL_DEFAULT_LEVEL);
	if (ret < 0)
