...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(lp8556_i2c.id, lp8556_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int lp8556_i2c_read_u8(u8 chip_no, u8 *data_buffer, int addr)
{
	...
	*data_buffer = addr;
	...
	if (lp8556_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"lp8556_2_i2c_read_u8  failed! reg is 0x%xret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int lp8556_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				 unsigned char value)
{
	int ret;
	...
	lp8556_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(lp8556_i2c.id, lp8556_i2c.addr, write_data, len,
			    NULL);
#else
	lp8556_i2c.mode = ST_MODE;
	lp8556_i2c.speed = LP8556_I2C_SPEED;
...
static int lp8556_i2c_write_u8(u8 chip_no, unsigned char addr,
			       unsigned char value)
{
	int ret;
	unsigned char write_data[LP8556_WRITE_LEN] = {0};
	...
	if (lp8556_bl_info.dual_ic) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"lp8556_2_i2c_write_u8  failed! reg is  0x%x ret: %d\n",
				addr, ret);
			return ret;
		}
	}
...
static int lp8556_parse_reg_config()
{
	...
	/* read ic sub version */
	ret = lp8556_i2c_read_u8(LP8556_SLAV_ADDR, &ic_sub_version,
				 LP8556_EPROM_CFGF);
	if (ret < 0) {
	...
	case LP8556_E03_E07_CFGF_VAL:
		strncpy_s(reg_node_name, sizeof(reg_node_name),
			  DTS_COMP_LP8556_E11, strlen(DTS_COMP_LP8556_E11));
		break;
	default:
	...
	for (i = 0; i < LP8556_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(
			lp8556_bl_info.pfdt, lp8556_bl_info.nodeoffset,
			lp8556_dts_string[i], &lp8556_bl_info.lp8556_reg[i], 0);
		if (ret < 0) {
...
static int lp8556_config_register(void)
{
	...
	for (i = 0; i < LP8556_RW_REG_MAX; i++) {
		...
		if (lp8556_bl_info.lp8556_reg[i] != 0xffff) {
			ret = lp8556_i2c_write_u8(
				LP8556_SLAV_ADDR, (u8)lp8556_reg_addr[i],
				(u8)lp8556_bl_info.lp8556_reg[i]);
			if (ret < 0) {
				LCD_KIT_ERR(
					"write lp8556 backlight config register 0x%x failed\n",
					lp8556_reg_addr[i]);
				return ret;
			}
		}
...
static int lp8556_backlight_ic_check(void)
{
	...
	ret = lp8556_i2c_read_u8(LP8556_SLAV_ADDR,
				 (u8 *)&lp8556_bl_info.lp8556_reg[0],
				 lp8556_reg_addr[0]);
	if (ret < 0) {
...
int lp8556_init(struct mtk_panel_info *pinfo)
{
	...
	if (lp8556_bl_info.lp8556_hw_en) {
		...
		if (lp8556_bl_info.dual_ic) {
			...
			if (ret < 0) {
				LCD_KIT_ERR(
					"parse dts lp8556_2_hw_en_gpio fail!\n");
				lp8556_bl_info.lp8556_2_hw_en_gpio = 0;
				goto exit;
			}
			if (lp8556_bl_info.lp8556_hw_en_gpio ==
		...
		ret = lcd_kit_get_dts_u32_default(
			lp8556_bl_info.pfdt, lp8556_bl_info.nodeoffset,
			LP8556_HW_EN_DELAY, &lp8556_bl_info.bl_on_lk_mdelay, 0);
		if (ret < 0)
			LCD_KIT_INFO("parse dts bl_on_lk_mdelay fail!\n");
	}

	ret = lcd_kit_get_dts_u32_default(
		lp8556_bl_info.pfdt, lp8556_bl_info.nodeoffset, LP8556_BL_LEVEL,
	...
	ret = lcd_kit_get_dts_u32_default(
		lp8556_bl_info.pfdt, lp8556_bl_info.nodeoffset,
		"need_compatible", &lp8556_bl_info.is_need_compatible, 0);
	if (ret < 0)
