...
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write_read(rt4831_i2c.id, rt4831_i2c.addr, data_buffer,
				 len, len, NULL);
#else
...
static int rt4831_i2c_read_u8(unsigned char chip_no, unsigned char *data_buffer,
			      int addr)
{
	int ret = -1;
	/* read date default length */
...
static int rt4831_2_i2c_write_u8(u8 chip_no, unsigned char addr,
				 unsigned char value)
{
	int ret;
	...
	rt4831_i2c.addr = chip_no;
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	ret = mtk_i2c_write(rt4831_i2c.id, rt4831_i2c.addr, write_data, len,
			    NULL);
#else
	rt4831_i2c.mode = ST_MODE;
	rt4831_i2c.speed = RT4831_I2C_SPEED;
...
static int rt4831_i2c_write_u8(unsigned char chip_no, unsigned char addr,
			       unsigned char value)
{
	int ret;
	unsigned char write_data[RT4831_WRITE_LEN] = {0};
	...
	if (rt4831_bl_info.dual_ic) {
		if (addr == REG_DISPLAY_BIAS_CONFIG_1)
			rt4831_2_i2c_write_u8(
				chip_no, addr,
				rt4831_bl_info.rt4831_2_display_bias_cfg_1);
		else
...
static int rt4831_i2c_update_bits(unsigned char chip_no, int reg,
...
static int rt4831_2_i2c_update_bits(unsigned char chip_no, int reg,
				    unsigned char mask, unsigned char val)
{
	int ret;
	unsigned char tmp;
...
static int rt4831_parse_dts(void)
{
	...
	for (i = 0; i < RT4831_RW_REG_MAX; i++) {
		ret = lcd_kit_get_dts_u32_default(
			rt4831_bl_info.pfdt, rt4831_bl_info.nodeoffset,
			rt4831_dts_string[i], &rt4831_bl_info.rt4831_reg[i], 0);
		if (ret < 0)
...
static int rt4831_config_register(void)
{
	...
	for (i = 0; i < RT4831_RW_REG_MAX; i++) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"write rt4831 backlight config register 0x%x failed",
				rt4831_reg_addr[i]);
			return ret;
		}
	}
...
int rt4831_init(struct mtk_panel_info *pinfo)
{
	...
	if (rt4831_bl_info.dual_ic) {
		...
		ret = lcd_kit_get_dts_u32_default(
			rt4831_bl_info.pfdt, rt4831_bl_info.nodeoffset,
			RT4831_2_DISPLAY_BIAS_CFG_1,
			&rt4831_bl_info.rt4831_2_display_bias_cfg_1, 0);
		if (ret < 0) {
			LCD_KIT_ERR(
				"parse dts rt4831_2_display_bias_cfg_1 fail!\n");
			return LCD_KIT_FAIL;
		}
	}
	...
	if (rt4831_bl_info.rt4831_hw_en) {
		ret = lcd_kit_get_dts_u32_default(
			rt4831_bl_info.pfdt, rt4831_bl_info.nodeoffset,
			RT4831_HW_EN_GPIO, &rt4831_bl_info.rt4831_hw_en_gpio,
			0);
		if (ret < 0) {
		...
		ret = lcd_kit_get_dts_u32_default(
			rt4831_bl_info.pfdt, rt4831_bl_info.nodeoffset,
			RT4831_HW_EN_DELAY, &rt4831_bl_info.bl_on_lk_mdelay, 0);
		if (ret < 0)
			LCD_KIT_INFO("parse dts bl_on_lk_mdelay fail!\n");
	}

	ret = lcd_kit_get_dts_u32_default(
		rt4831_bl_info.pfdt, rt4831_bl_info.nodeoffset, RT4831_BL_LEVEL,
...
int rt4831_set_backlight(uint32_t bl_level)
{
	...
	ret = rt4831_i2c_update_bits(RT4831_SLAV_ADDR, REG_BL_BRIGHTNESS_LSB,
				     MASK_BL_LSB, bl_lsb);
	if (ret < 0)
		goto i2c_error;

	ret = rt4831_i2c_write_u8(RT4831_SLAV_ADDR, REG_BL_BRIGHTNESS_MSB,
				  bl_msb);
	if (ret < 0)
		goto i2c_error;
	if (rt4831_bl_info.dual_ic) {
		ret = rt4831_2_i2c_update_bits(RT4831_SLAV_ADDR,
					       REG_BL_BRIGHTNESS_LSB,
					       MASK_BL_LSB, bl_lsb);
		if (ret < 0)
			goto i2c_error;

		ret = rt4831_2_i2c_write_u8(RT4831_SLAV_ADDR,
