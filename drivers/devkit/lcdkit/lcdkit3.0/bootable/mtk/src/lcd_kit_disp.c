				      ...
				      struct mtk_panel_info *pinfo)
{
	...
	if (pinfo->round_corner.round_corner_en) {
		params->round_corner_params.round_corner_en =
			pinfo->round_corner.round_corner_en;
		params->round_corner_params.full_content =
			pinfo->round_corner.full_content;
		params->round_corner_params.h = pinfo->round_corner.h;
		params->round_corner_params.h_bot = pinfo->round_corner.h_bot;
		params->round_corner_params.tp_size =
			pinfo->round_corner.tp_size;
		params->round_corner_params.lt_addr =
...
static void lcm_get_dsc_params(LCM_PARAMS *params, struct mtk_panel_info *pinfo)
{
	...
	params->dsi.dsc_params.bit_per_channel =
		pinfo->dsc_params.bit_per_channel;
	params->dsi.dsc_params.dsc_line_buf_depth =
		pinfo->dsc_params.dsc_line_buf_depth;
	...
	params->dsi.dsc_params.increment_interval =
		pinfo->dsc_params.increment_interval;
	params->dsi.dsc_params.decrement_interval =
		pinfo->dsc_params.decrement_interval;
	params->dsi.dsc_params.line_bpg_offset =
		pinfo->dsc_params.line_bpg_offset;
	params->dsi.dsc_params.nfl_bpg_offset =
		pinfo->dsc_params.nfl_bpg_offset;
	params->dsi.dsc_params.slice_bpg_offset =
		pinfo->dsc_params.slice_bpg_offset;
	params->dsi.dsc_params.initial_offset =
		pinfo->dsc_params.initial_offset;
	params->dsi.dsc_params.final_offset = pinfo->dsc_params.final_offset;
	params->dsi.dsc_params.flatness_minqp =
		pinfo->dsc_params.flatness_minqp;
	params->dsi.dsc_params.flatness_maxqp =
		pinfo->dsc_params.flatness_maxqp;
	params->dsi.dsc_params.rc_model_size = pinfo->dsc_params.rc_model_size;
	params->dsi.dsc_params.rc_edge_factor =
		pinfo->dsc_params.rc_edge_factor;
	params->dsi.dsc_params.rc_quant_incr_limit0 =
		pinfo->dsc_params.rc_quant_incr_limit0;
	params->dsi.dsc_params.rc_quant_incr_limit1 =
		pinfo->dsc_params.rc_quant_incr_limit1;
	params->dsi.dsc_params.rc_tgt_offset_hi =
		pinfo->dsc_params.rc_tgt_offset_hi;
	params->dsi.dsc_params.rc_tgt_offset_lo =
		pinfo->dsc_params.rc_tgt_offset_lo;
}
...
static void lcm_init(void)
{
	...
	if (lcd_kit_pinfo.blmaxnit.get_blmaxnit_type ==
		...
		LCD_KIT_INFO(
			"lcd_kit_brightness_ddic_info = %d\n",
			lcd_kit_pinfo.blmaxnit.lcd_kit_brightness_ddic_info);
	}
...
void lcd_kit_change_display_dts(void *fdt)
{
	...
	if (lcd_kit_pinfo.blmaxnit.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC)
		lcdkit_set_lcd_ddic_max_brightness(
			fdt, lcd_kit_get_blmaxnit(&lcd_kit_pinfo));
}
#else
...
#endif
{
	...
	offset = fdt_node_offset_by_compatible(pfdt, OFFSET_DEF_VAL,
	...
	/* bias ic init */
	bias_bl_ops_init();
	if (((struct mtk_panel_info *)(lcdkit_mtk_common_panel.panel_info))
		    ->bias_ic_ctrl_mode == GPIO_THEN_I2C_MODE) {
		tps65132_init();
		rt4801h_init();
		sm5109_init();
		aw3750x_init();
	...
	if (((struct mtk_panel_info *)(lcdkit_mtk_common_panel.panel_info))
		    ->bias_ic_ctrl_mode == LCD_BIAS_COMMON_MODE) {
		ret = bias_ic_init();
		if (ret < 0)
