	...
	if ((pinfo->blmaxnit.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC) &&
		...
		bl_max_nit =
			(lcd_kit_brightness_ddic_info < BL_REG_NOUSE_VALUE) ?
				(lcd_kit_brightness_ddic_info +
				 pinfo->bl_max_nit_min_value) :
				(lcd_kit_brightness_ddic_info +
				 pinfo->bl_max_nit_min_value - 1);
	} else {
...
static int lcd_kit_parse_disp_info(const void *fdt, int nodeoffset)
{
	...
	if (disp_info->quickly_sleep_out.support)
		lcd_kit_get_dts_u32_default(
			fdt, nodeoffset, "lcd-kit,quickly-sleep-out-interval",
	...
	if (disp_info->elvdd_detect.support) {
		...
		lcd_kit_get_dts_u32_default(fdt, nodeoffset,
					    ...
					    0);
		lcd_kit_get_dts_u32_default(
			fdt, nodeoffset, "lcd-kit,elvdd-value-mask",
			      ...
			      struct mtk_panel_info *pinfo)
{
	...
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,mipi-phy-mode",
				    &pinfo->mipi.phy_mode, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,mipi-lp11-flag",
				    &pinfo->mipi.lp11_flag, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset,
				    "lcd-kit,mipi-non-continue-enable",
				    &pinfo->mipi.non_continue_en, 0);

	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-lcm-type",
	...
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-ldi-ps",
				    &pinfo->panel_ps, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-vfp-lp",
				    &pinfo->ldi.v_front_porch_forlp, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-fbk-div",
				    &pinfo->pxl_fbk_div, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,ssc-disable",
				    &pinfo->ssc_disable, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,ssc-range",
				    &pinfo->ssc_range, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset,
	...
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-bl-max-nit",
				    &pinfo->bl_max_nit, 0);
	lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,panel-bl-boot",
	...
	if (pinfo->blmaxnit.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC)
		lcd_kit_parse_dts_dcs_cmds(
			fdt, nodeoffset, "lcd-kit,panel-bl-maxnit-command",
...
static void lcd_kit_parse_power(const void *fdt, int nodeoffset)
{
	...
	if (power_hdl->lcd_backlight.buf == NULL)
		lcd_kit_parse_dts_array(fdt, nodeoffset,
				     ...
				     struct mtk_panel_info *pinfo)
{
	...
	ret = lcd_kit_get_dts_u32_default(
		fdt, nodeoffset, "lcd-kit,panel-round-corner-full-content",
	...
	array_data.buf = NULL;
	array_data.cnt = 0;
	ret = lcd_kit_parse_dts_array(
		fdt, nodeoffset, "lcd-kit,panel-round-corner-top-rc-pattern",
		&array_data);
	if (ret == LCD_KIT_FAIL) {
...
int lcd_kit_utils_init(struct mtk_panel_info *pinfo, const void *fdt,
		       int nodeoffset)
{
	int ret = LCD_KIT_OK;
