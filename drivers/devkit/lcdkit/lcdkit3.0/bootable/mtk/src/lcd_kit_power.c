...
#endif
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	{68, 3000000, MT6368_PMIC_RG_LDO_VFP_EN_ADDR, 0x1, 0x1,
	 MT6368_PMIC_RG_LDO_VFP_EN_SHIFT},
	{7331, 3100000, MT6373_PMIC_RG_LDO_VFP_EN_ADDR, 0x1, 0x1,
	 MT6373_PMIC_RG_LDO_VFP_EN_SHIFT},
	{7318, 1850000, MT6373_PMIC_RG_LDO_VAUD18_EN_ADDR, 0x1, 0x1,
	 MT6373_PMIC_RG_LDO_VAUD18_EN_SHIFT},
#endif
};
...
#endif
#ifdef MTK_LCD_KIT_LK2_SUPPORT
	{68, 3000000, MT6368_PMIC_RG_VFP_VOSEL_ADDR, 0xc,
	 MT6368_PMIC_RG_VFP_VOCAL_ADDR, 0x0},
	{7331, 3100000, MT6373_PMIC_RG_VFP_VOSEL_ADDR, 0xc,
	 MT6373_PMIC_RG_VFP_VOCAL_ADDR, 0x0},
	{7318, 1850000, MT6373_PMIC_RG_VAUD18_VOSEL_ADDR, 0xc,
	 MT6373_PMIC_RG_VAUD18_VOCAL_ADDR, 0x1},
#endif
};
/*
 * power type
...
int gpio_cmds_tx(struct gpio_desc *cmds, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		if ((cm == NULL) || (cm->label == NULL)) {
			LCD_KIT_ERR(
				"cm or cm->label is null! index=%d, cnt=%d\n",
				i, cnt);
			ret = LCD_KIT_FAIL;
			goto error;
...
static int lcd_kit_get_pmu_info(uint32_t type, uint32_t *power_num,
				uint32_t *power_volt)
{
	switch (type) {
	case LCD_KIT_VCI:
...
int lcd_kit_pmu_ctrl(uint32_t type, uint32_t enable)
{
	...
	if (power_num >= LDO_BEGIN && power_num <= LDO_MAX) {
#ifdef MTK_MT6360_LDO_SUPPORT
		if (mt6360_ldo_config_interface(
			...
			LCD_KIT_ERR(
				"mt6360_ldo_config_interface ldo set volt fail\n");
			return LCD_KIT_FAIL;
		}
#endif
...
#if defined(CONFIG_MACH_MT6885) && defined(MTK_MT6360_PMIC_SUPPORT)
		if (mt6360_pmic_config_interface(
			    g_ldo_volt[array_index].ldo_addr,
			    g_ldo_volt[array_index].ldo_data,
			    g_ldo_volt[array_index].ldo_mask,
			    g_ldo_volt[array_index].ldo_shift)) {
			LCD_KIT_ERR(
				"mt6360_pmic_config_interface ldo set volt fail\n");
			return LCD_KIT_FAIL;
		}
#endif
#ifdef MTK_MT6358_LDO_SUPPORT
		if (pmic_config_interface(PMIC_RG_LDO_VLDO28_EN_0_ADDR, 0x01,
		...
		if (spmic_config_vfp_volt(g_ldo_volt[array_index].ldo_addr,
					  ...
					  g_ldo_volt[array_index].ldo_shift)) {
			LCD_KIT_ERR(
				"mt6368/mt6373 pmic_config_interface ldo set volt fail\n");
			return LCD_KIT_FAIL;
		}
#endif
		LCD_KIT_INFO("ldo%d set vol\n", power_num);
	...
	LCD_KIT_INFO(
		"[vol]ldo_addr = 0x%x,ldo_data = 0x%x,ldo_mask = 0x%x, ldo_shift = 0x%x\n",
		g_ldo_volt[array_index].ldo_addr,
		g_ldo_volt[array_index].ldo_data,
		g_ldo_volt[array_index].ldo_mask,
		g_ldo_volt[array_index].ldo_shift);
	...
	if (power_num >= LDO_BEGIN && power_num <= LDO_MAX) {
...
#endif
#ifdef MTK_MT6358_LDO_SUPPORT
		if (pmic_config_interface(PMIC_RG_VLDO28_VOSEL_ADDR, 0x03,
		...
		if (spmic_config_vfp_en(g_ldo_enable[array_index].ldo_addr,
					g_ldo_enable[array_index].ldo_data)) {
			LCD_KIT_ERR(
				"mt6368/mt6373 pmic_config_interface ldo enable fail\n");
			return LCD_KIT_FAIL;
		}
#endif
		LCD_KIT_INFO("ldo%d config\n", power_num);
	...
	LCD_KIT_INFO(
		"[enable]ldo_addr = 0x%x,ldo_data = 0x%x,ldo_mask = 0x%x, ldo_shift = 0x%x\n",
		g_ldo_enable[array_index].ldo_addr,
		g_ldo_enable[array_index].ldo_data,
		g_ldo_enable[array_index].ldo_mask,
		g_ldo_enable[array_index].ldo_shift);
