						 ...
						 const char *buf, size_t count)
{
	...
	if (sysfs_ops->ddic_lv_detect_test_store) {
		ret = sysfs_ops->ddic_lv_detect_test_store(dev, attr, buf,
							   count);
		if (ret < 0)
...
static DEVICE_ATTR(lcd_hbm_mode, 0644, lcd_kit_hbm_mode_show,
		   lcd_kit_hbm_mode_store);

static DEVICE_ATTR(ddic_alpha, 0644, NULL, lcd_kit_ddic_alpha_store);
static DEVICE_ATTR(lcd_fps_order, 0644, lcd_kit_fps_order_show, NULL);
static DEVICE_ATTR(panel_sncode, 0644, lcd_kit_panel_sncode_show,
		   lcd_kit_panel_sncode_store);
static DEVICE_ATTR(display_idle_mode, 0644, NULL, lcd_kit_idle_mode_store);
/*lint -e1564*/
static DEVICE_ATTR(pre_camera_position, 0644, lcd_kit_pre_camera_position_show,
		   NULL);
...
int lcd_kit_create_sysfs(struct kobject *obj)
{
	...
	for (i = 0; i < (int)(ARRAY_SIZE(lcd_kit_conf)); i++) {
		...
		if (lcd_kit_check_support(i) && count < LCD_KIT_SYSFS_MAX)
			lcd_kit_sysfs_attrs[count++] =
				lcd_kit_conf[i]; //lint !e661
	}
