...
static int lcd_kit_dbg_effect_on_cmd(char *par)
{
	...
	lcd_kit_dbg.dbg_effect_on_cmds =
		kzalloc(LCD_KIT_CONFIG_TABLE_MAX_NUM, 0);
...
static int lcd_kit_dbg_rgbw_backlight_cmd(char *par)
{
	...
	len = lcd_kit_parse_u8_digit(par, lcd_kit_dbg.dbg_rgbw_backlight_cmds,
				     LCD_KIT_CMD_MAX_NUM);
...
static int lcd_kit_dbg_rgbw_pixel_gainlimit_cmd(char *par)
{
	...
	lcd_kit_dbg.dbg_rgbw_pixel_gainlimit_cmds =
		kzalloc(LCD_KIT_CMD_MAX_NUM, 0);
	...
	if (dbg_ops->rgbw_pixel_gainlimit_cmd && len > 0)
		dbg_ops->rgbw_pixel_gainlimit_cmd(
			lcd_kit_dbg.dbg_rgbw_pixel_gainlimit_cmds, len);
	return LCD_KIT_OK;
}
...
static int lcd_kit_dbg_esd_reg_cmd(char *par)
{
	...
	len = lcd_kit_parse_u8_digit(par, lcd_kit_dbg.dbg_esd_cmds,
				     LCD_KIT_CMD_MAX_NUM);
	if (len > 0)
...
static int lcd_kit_dbg_brightness_color_cmd(char *par)
{
	...
	if (dbg_ops->brightness_color_cmd && len > 0)
		dbg_ops->brightness_color_cmd(
			lcd_kit_dbg.dbg_brightness_color_cmds, len);
	return LCD_KIT_OK;
}
				...
				size_t count, loff_t *ppos)
{
	...
	len = snprintf(
		cur, buf_len,
		"\teg. echo set_debug_level:4 > /sys/kernel/debug/lcd-dbg/lcd_kit_dbg to open set debug level\n");
	buf_len -= len;
	cur += len;

	len = snprintf(
		cur, buf_len,
		"\teg. echo set_param_config:1 > /sys/kernel/debug/lcd-dbg/lcd_kit_dbg to set parameter\n");
	buf_len -= len;
	cur += len;
...
static int parse_char(const char *item, const char *ch, char *out, int len,
		      int *find)
{
	int item_len;
	int tmp_len;
...
static int parse_u32(const char *path, const char *item, unsigned int *out,
		     unsigned int def)
{
	int ret;
	char *data = NULL;
...
static int parse_u8(const char *path, const char *item, unsigned char *out,
		    unsigned char def)
{
	int ret;
	char *data = NULL;
...
static int parse_dcs(const char *path, const char *item, const char *item_state,
...
static void parse_dirty_region(const char *path, const char *item, int *out)
{
	int value = 0;
	int ret;
