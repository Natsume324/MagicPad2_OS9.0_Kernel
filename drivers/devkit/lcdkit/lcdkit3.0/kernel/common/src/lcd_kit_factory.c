						...
						const char *buf, size_t count)
{
	...
	if (fact_ops->vtc_line_test_store)
		(void)fact_ops->vtc_line_test_store(dev, attr, buf, count);
	return count;
}
...
static DEVICE_ATTR(vertical_line_test, 0644, lcd_kit_vertical_line_test_show,
		   lcd_kit_vertical_line_test_store);
...
{
	/* forword-revert scan */
	lcd_kit_parse_u32(np, "lcd-kit,scan-support", &FACT_INFO->scan.support,
			  0);
	if (FACT_INFO->scan.support) {
...
{
	/* pt test */
	lcd_kit_parse_u32(np, "lcd-kit,pt-support", &FACT_INFO->pt.support, 0);
	if (FACT_INFO->pt.support) {
		lcd_kit_parse_u32(np, "lcd-kit,pt-panel-ulps-support",
