				...
				struct lcd_kit_string_arrays *string_array)
{
	...
	rc = of_property_read_string_array(np, propertyname, string_array->buf,
					   cnt);
	if (rc < 0) {
		string_array->cnt = 0;
			      ...
			      struct lcd_kit_arrays_data *out, int num)
{
	...
	out->cnt = cnt;
	out->arry_data =
		kzalloc(sizeof(struct lcd_kit_array_data) * cnt, GFP_KERNEL);
	if (!out->arry_data) {
			    ...
			    struct lcd_kit_dsi_cmd_desc_header *dchdr)
{
	...
	pcmds->cmds =
		kzalloc(cnt * sizeof(struct lcd_kit_dsi_cmd_desc), GFP_KERNEL);
	if (!pcmds->cmds) {
		LCD_KIT_ERR("kzalloc fail\n");
		return LCD_KIT_FAIL;
	}
...
int lcd_kit_parse_dirty_info(const struct device_node *np,
			     const char *prop_name, int *out)
{
	unsigned int value =
		0xffff; // init to 0xffff, represent - 1 with 0xffff
...
int lcd_kit_parse_string_array(const struct device_node *np,
			       const char *prop_name, const char **out_strs,
			       size_t sz)
{
