...
struct lcd_kit_power_desc *lcd_kit_get_panel_power_handle(uint32_t panel_id)
{
...
struct lcd_kit_power_seq *lcd_kit_get_panel_power_seq(uint32_t panel_id)
{
	if (panel_id >= LCD_ACTIVE_PANEL_BUTT) {
		LCD_KIT_ERR("panel_id %u!\n", panel_id);
		return NULL;
...
static int lcd_kit_set_bias_ctrl(int enable)
{
	...
	if (enable) {
		...
		if (power_hdl->lcd_power_down_vsp.buf == NULL) {
			LCD_KIT_INFO(
				"PowerDownVsp is not configured in xml!\n");
			return LCD_KIT_FAIL;
		}
...
static int lcd_kit_iovcc_power_ctrl(int enable)
{
	...
	case REGULATOR_MODE:
		if (enable) {
			if (adapt_ops->regulator_enable)
				ret = adapt_ops->regulator_enable(
					LCD_KIT_IOVCC);
		} else {
			if (adapt_ops->regulator_disable)
				ret = adapt_ops->regulator_disable(
					LCD_KIT_IOVCC);
		}
		break;
	case NONE_MODE:
	...
	case GPIO_MODE:
		if (enable) {
			if (adapt_ops->gpio_enable_nolock)
				adapt_ops->gpio_enable_nolock(
					LCD_KIT_MIPI_SWITCH);
		} else {
			if (adapt_ops->gpio_disable_nolock)
				adapt_ops->gpio_disable_nolock(
					LCD_KIT_MIPI_SWITCH);
		}
		break;
	case NONE_MODE:
...
static int
lcd_kit_check_reg_report_dsm(void *hld,
			     struct lcd_kit_check_reg_dsm *check_reg_dsm)
{
	...
	if (adapt_ops->mipi_rx)
		ret = adapt_ops->mipi_rx(hld, read_value,
	...
	if (ret == LCD_KIT_OK) {
		for (i = 0; i < check_reg_dsm->cmds.cmd_cnt; i++) {
			...
			if ((char)read_value[i] != expect_ptr[i]) {
				ret = LCD_KIT_FAIL;
				LCD_KIT_ERR(
					"read_value[%d] = 0x%x, but expect_ptr[%d] = 0x%x!\n",
					i, read_value[i], i, expect_ptr[i]);
#if IS_ENABLED(CONFIG_HONOR_DSM)
				dsm_client_record(
					lcd_dclient,
					"lcd register read_value[%d] = 0x%x, but expect_ptr[%d] = 0x%x!\n",
					i, read_value[i], i, expect_ptr[i]);
#endif
				break;
			}
			LCD_KIT_INFO(
				"read_value[%d] = 0x%x same with expect value!\n",
				i, read_value[i]);
		}
	} else {
...
static void lcd_kit_proxmity_proc(int enable)
{
	...
	reset_tv = &(common_info->thp_proximity.lcd_reset_record_tv);
	/* change s to us */
	delta_time = (tv.tv_sec - reset_tv->tv_sec) * 1000000 + tv.tv_usec -
		     reset_tv->tv_usec;
	/* change us to ms */
	delta_time /= 1000;
	if (delta_time >= common_info->thp_proximity.after_reset_delay_min)
		return;
	delay_margin =
		common_info->thp_proximity.after_reset_delay_min - delta_time;
...
static int lcd_kit_mipi_power_ctrl(void *hld, int enable)
{
	...
	if (enable) {
		...
		ret_check_reg = lcd_kit_check_reg_report_dsm(
			hld, &common_info->check_reg_on);
		if (ret_check_reg != LCD_KIT_OK)
			LCD_KIT_ERR("power on check reg error!\n");
	} else {
		ret = lcd_kit_off_cmds(hld);
		ret_check_reg = lcd_kit_check_reg_report_dsm(
			hld, &common_info->check_reg_off);
		if (ret_check_reg != LCD_KIT_OK)
...
static int lcd_kit_ext_ts_resume(int sync)
{
	...
	if (sync)
		ret = ts_ops->ts_multi_power_notify(
			TS_RESUME_DEVICE, SHORT_SYNC, LCD_ACTIVE_PANEL);
	else
		ret = ts_ops->ts_multi_power_notify(TS_RESUME_DEVICE, NO_SYNC,
						    LCD_ACTIVE_PANEL);
...
static int lcd_kit_ext_ts_after_resume(int sync)
{
	...
	if (sync)
		ret = ts_ops->ts_multi_power_notify(TS_AFTER_RESUME, SHORT_SYNC,
						    LCD_ACTIVE_PANEL);
	else
		ret = ts_ops->ts_multi_power_notify(TS_AFTER_RESUME, NO_SYNC,
						    LCD_ACTIVE_PANEL);
	LCD_KIT_INFO("sync is %d\n", sync);
	return ret;
}
...
static int lcd_kit_ext_ts_suspend(int sync)
{
	...
	if (sync) {
		ts_ops->ts_multi_power_notify(TS_BEFORE_SUSPEND, SHORT_SYNC,
					      LCD_ACTIVE_PANEL);
		ts_ops->ts_multi_power_notify(TS_SUSPEND_DEVICE, SHORT_SYNC,
					      LCD_ACTIVE_PANEL);
	} else {
		ts_ops->ts_multi_power_notify(TS_BEFORE_SUSPEND, NO_SYNC,
					      LCD_ACTIVE_PANEL);
		ts_ops->ts_multi_power_notify(TS_SUSPEND_DEVICE, NO_SYNC,
					      LCD_ACTIVE_PANEL);
	}
	LCD_KIT_INFO("sync is %d\n", sync);
	return LCD_KIT_OK;
...
static int lcd_kit_ext_ts_early_suspend(int sync)
{
	...
	if (sync)
		ret = ts_ops->ts_multi_power_notify(
			TS_EARLY_SUSPEND, SHORT_SYNC, LCD_ACTIVE_PANEL);
	else
		ret = ts_ops->ts_multi_power_notify(TS_EARLY_SUSPEND, NO_SYNC,
						    LCD_ACTIVE_PANEL);
	LCD_KIT_INFO("sync is %d\n", sync);
	return ret;
}
...
static int lcd_kit_gesture_mode(void)
{
	...
	if (ts_ops->get_tp_status_by_type) {
		ret = ts_ops->get_tp_status_by_type(TS_GESTURE_FUNCTION,
						    &status);
		if (ret) {
...
static bool lcd_kit_event_skip_delay(void *hld, uint32_t event, uint32_t data)
{
	struct lcd_kit_ops *lcd_ops = NULL;
...
static bool lcd_kit_event_skip_send(void *hld, uint32_t event, uint32_t enable)
{
#if defined(CONFIG_LCD_KIT_HONOR)
	...
	if (pinfo->product_type != LCD_FOLDER_TYPE)
		return false;
	if ((pinfo->skip_power_on_off == FOLD_POWER_ON_OFF) && (enable == 0)) {
	...
	if (event == EVENT_MIPI) {
		if (enable)
			(void)lcd_kit_check_reg_report_dsm(
				hld, &common_info->check_reg_on);
		else
			(void)lcd_kit_check_reg_report_dsm(
				hld, &common_info->check_reg_off);
...
static int lcd_kit_gesture_event_handler(uint32_t event, uint32_t data,
					 uint32_t delay)
{
	int ret = LCD_KIT_OK;
	int type = 0;
...
static int lcd_kit_ft_gesture_mode_power_on(void)
{
	...
	if (common_info->tp_gesture_sequence_flag && lcd_kit_gesture_mode()) {
		...
		for (i = 0; i < power_seq->gesture_power_on_seq.cnt; i++) {
			...
			ret = lcd_kit_gesture_event_handler(
				gesture_pevent->buf[EVENT_NUM],
				gesture_pevent->buf[EVENT_DATA],
				gesture_pevent->buf[EVENT_DELAY]);
			if (ret) {
				LCD_KIT_ERR(
					"send gesture_pevent 0x%x not exist!\n",
					gesture_pevent->buf[EVENT_NUM]);
				break;
			}
			gesture_pevent++;
...
static int lcd_kit_panel_on_hs(void *hld)
{
	...
	if (common_info->check_thread.enable)
		hrtimer_start(&common_info->check_thread.hrtimer,
			      ktime_set(CHECK_THREAD_TIME_PERIOD /
						1000, // change ms to s
					(CHECK_THREAD_TIME_PERIOD % 1000) *
						1000000), // change ms to ns
			      HRTIMER_MODE_REL);
	return ret;
}
...
static int lcd_kit_hbm_enable(void *hld, int fps_status)
{
	...
	if (common_info->dfr_info.fps_lock_command_support &&
		...
		ret = adapt_ops->mipi_tx(
			hld, &common_info->dfr_info.cmds[FPS_90_HBM_DIM]);
...
static int lcd_kit_enter_fp_hbm_extern(void *hld)
{
	...
	if (common_info->hbm.fp_enter_extern_cmds.cmds != NULL)
		ret = adapt_ops->mipi_tx(
			hld, &common_info->hbm.fp_enter_extern_cmds);
	return ret;
}
...
static int lcd_kit_hbm_dim_disable(void *hld, int fps_status)
{
	...
	if ((fps_status == LCD_KIT_FPS_HIGH) &&
		...
		ret = adapt_ops->mipi_tx(
			hld, &common_info->dfr_info.cmds[FPS_90_NORMAL_NO_DIM]);
...
static int lcd_kit_hbm_dim_enable(void *hld, int fps_status)
{
	...
	if ((fps_status == LCD_KIT_FPS_HIGH) &&
	    common_info->dfr_info.fps_lock_command_support) {
		ret = adapt_ops->mipi_tx(
			hld, &common_info->dfr_info.cmds[FPS_90_NORMAL_DIM]);
...
static int lcd_kit_hbm_enable_no_dimming(void *hld, int fps_status)
{
	...
	if ((fps_status == LCD_KIT_FPS_HIGH) &&
	    common_info->dfr_info.fps_lock_command_support) {
		ret = adapt_ops->mipi_tx(
			hld, &common_info->dfr_info.cmds[FPS_90_HBM_NO_DIM]);
...
int lcd_kit_pwm_gpio_esd_check(void)
{
	...
	gpio_val_pwm = (u32)lcd_kit_get_gpio_value(
		common_info->esd.gpio_detect_num +
			common_info->esd.panel_gpio_offset,
		"esd_detect_gpio");
...
int lcd_kit_dual_gpio_esd_check(void)
{
	...
	gpio_val_te_m = (u32)lcd_kit_get_gpio_value(
		common_info->esd.gpio_detect_num_first +
			common_info->esd.panel_gpio_offset,
		"esd_detect_gpio_first");
	gpio_val_te_s = (u32)lcd_kit_get_gpio_value(
		common_info->esd.gpio_detect_num_second +
			common_info->esd.panel_gpio_offset,
		"esd_detect_gpio_second");
	...
	LCD_KIT_INFO(
		"gpio_val_te_m = %d, gpio_val_te_s = %d, esd check result:%d\n",
...
static int lcd_kit_esd_handle(void *hld)
{
	...
	for (i = 0; i < common_info->esd.value.cnt; i++) {
		...
		if (lcd_kit_judge_esd(judge_type, read_value[i],
				      expect_value)) {
			...
			lcd_kit_record_esd_error_info(i, (int)read_value[i],
						      expect_value);
			LCD_KIT_ERR(
				"read_value[%d] = 0x%x, but expect_value = 0x%x!\n",
				i, read_value[i], expect_value);
			ret = 1;
			break;
		}
		LCD_KIT_INFO(
			"judge_type = %d, esd_value[%d] = 0x%x, read_value[%d] = 0x%x, expect_value = 0x%x\n",
			judge_type, i, esd_value[i], i, read_value[i],
			expect_value);
	}
...
static int lcd_kit_dsi_handle(void *hld)
{
	...
	for (i = 0; i < common_info->dsi.value.cnt; i++) {
		...
		if (read_value[i] != expect_value) {
			LCD_KIT_ERR(
				"read_value[%d] = 0x%x, expect_value = 0x%x\n",
				i, read_value[i], expect_value);
			ret = snprintf(record_buf_dsi, RECORD_BUFLEN_DSI,
				       "dsi read_value = 0x%x, 0x%x",
			...
			(void)lcd_dsm_client_record(lcd_dclient, record_buf_dsi,
						    DSM_DSI_DETECT_ERROR_NO,
						    REC_DMD_NO_LIMIT_DSI,
						    &cur_rec_time_dsi);
#endif
			ret = 1;
			break;
...
static void lcd_kit_panel_parse_effect(struct device_node *np)
{
	...
	if (common_info->ddic_alpha.alpha_support) {
		lcd_kit_parse_dcs_cmds(
			np, "lcd-kit,local-ddic-alpha-enter-cmds",
			"lcd-kit,local-fp-local-ddic-alpha-cmds-state",
			&common_info->ddic_alpha.enter_alpha_cmds);
		lcd_kit_parse_dcs_cmds(
			np, "lcd-kit,local-ddic-alpha-exit-cmds",
			"lcd-kit,local-fp-local-ddic-alpha-cmds-state",
	...
	/* hbm */
	lcd_kit_parse_u32(np, "lcd-kit,hbm-support", &common_info->hbm.support,
			  0);
	if (common_info->hbm.support) {
		...
		if (common_info->hbm.hbm_fp_support) {
			...
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,hbm-fp-enter-cmds",
				"lcd-kit,hbm-fp-enter-cmds-state",
				&common_info->hbm.fp_enter_cmds);
			lcd_kit_parse_u32(
				np, "lcd-kit,hbm-elvss-dim-support",
		...
		if (common_info->hbm.local_hbm_support) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,local-hbm-fp-alpha-enter-cmds",
				"lcd-kit,local-hbm-fp-local-hbm-cmds-state",
				&common_info->hbm.enter_alpha_cmds);
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,local-hbm-fp-alpha-exit-cmds",
				"lcd-kit,local-hbm-fp-local-hbm-cmds-state",
				&common_info->hbm.exit_alpha_cmds);
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,local-hbm-fp-circle-enter-cmds",
				"lcd-kit,local-hbm-fp-local-hbm-cmds-state",
				&common_info->hbm.enter_circle_cmds);
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,local-hbm-fp-circle-exit-cmds",
				"lcd-kit,local-hbm-fp-local-hbm-cmds-state",
			...
			lcd_kit_parse_dcs_cmds(
				...
				"lcd-kit,local-hbm-fp-local-hbm-cmds-state",
				&common_info->hbm
					 .circle_size_small_cmds); // local-hbm-fp-circle-radius-cmds
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,local-hbm-fp-circle-size-mid-cmds",
				"lcd-kit,local-hbm-fp-local-hbm-cmds-state",
			...
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,local-hbm-fp-circle-color-cmds",
				"lcd-kit,local-hbm-fp-local-hbm-cmds-state",
	...
	/* acl */
	lcd_kit_parse_u32(np, "lcd-kit,acl-support", &common_info->acl.support,
			  0);
	if (common_info->acl.support) {
	...
	/* vr */
	lcd_kit_parse_u32(np, "lcd-kit,vr-support", &common_info->vr.support,
			  0);
	if (common_info->vr.support) {
	...
	/* ce */
	lcd_kit_parse_u32(np, "lcd-kit,ce-support", &common_info->ce.support,
			  0);
	if (common_info->ce.support) {
...
static void lcd_kit_proximity_parse(struct device_node *np)
{
	...
	if (common_info->thp_proximity.support) {
		...
		lcd_kit_parse_u32(
			np, "lcd-kit,proximity-reset-delay-min",
			&common_info->thp_proximity.after_reset_delay_min, 0);
		memset(&common_info->thp_proximity.lcd_reset_record_tv, 0,
...
static void lcd_kit_panel_parse_util(struct device_node *np)
{
	...
	/* panel name */
	common_info->panel_name =
		(char *)of_get_property(np, "lcd-kit,panel-name", NULL);
	/* panel model */
	common_info->panel_model =
		(char *)of_get_property(np, "lcd-kit,panel-model", NULL);
	/* panel information */
	common_info->module_info =
		(char *)of_get_property(np, "lcd-kit,module-info", NULL);
	/* lhbm  support*/
	...
	lcd_kit_parse_dcs_cmds(np, "lcd-kit,panel-read-batch-info",
			       "lcd-kit,panel-read-batch-info-state",
			       &common_info->lhbm.panel_read_batch_info);
	...
	if (common_info->lhbm.support) {
		...
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,local-hbm-disable-cmds",
				       "lcd-kit,local-hbm-disable-cmds-state",
				       &common_info->lhbm.disable_cmds);
		/* lhbm off enable */
		lcd_kit_parse_dcs_cmds(np, "lcd-kit,local-hbm-on-enable-cmds",
				       "lcd-kit,local-hbm-on-enable-cmds-state",
				       &common_info->lhbm.on_enable_cmds);
		/* lhbm on enable*/
		lcd_kit_parse_dcs_cmds(
			np, "lcd-kit,local-hbm-off-enable-cmds",
			"lcd-kit,local-hbm-off-enable-cmds-state",
			&common_info->lhbm.off_enable_cmds);
	}
	/* panel type */
	...
	if (common_info->panel_ext_info.support) {
		...
		common_info->panel_ext_name = (char *)of_get_property(
			np, "lcd-kit,panel-ext-name", NULL);
	}

	lcd_kit_parse_u32(np, "lcd-kit,panel-type", &common_info->panel_type,
			  0);
	/* panel on command */
	...
	if (common_info->panel_on_cmd_backup.support) {
		lcd_kit_parse_u32(np, "lcd-kit,pnl-on-cmd-bak-det-gpio",
				  ...
				  0);
		lcd_kit_parse_u32(
			np, "lcd-kit,pnl-on-cmd-bak-gpio-exp-val",
	...
	if (common_info->esd.gpio_detect_dual_support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,esd-gpio-normal-value-second",
				  &common_info->esd.gpio_normal_value_second,
				  0);
		lcd_kit_parse_u32(np, "lcd-kit,panel-gpio-offset",
				  &common_info->esd.panel_gpio_offset, 0);
	}
	lcd_kit_parse_u32(np, "lcd-kit,esd-support", &common_info->esd.support,
			  0);
	...
	/* dsi detect */
	lcd_kit_parse_u32(np, "lcd-kit,dsi-support", &common_info->dsi.support,
			  0);
	if (common_info->dsi.support) {
	...
	if (common_info->check_thread.enable)
		lcd_kit_parse_u32(np, "lcd-kit,check-bl-support",
				  &common_info->check_thread.check_bl_support,
				  0);
	/* vss */
	...
	if (common_info->check_reg_on.support) {
		...
		lcd_kit_parse_u32(np, "lcd-kit,check-reg-on-support-dsm-report",
				  &common_info->check_reg_on.support_dsm_report,
				  0);
	}
	/* check reg off */
	...
	if (common_info->check_reg_off.support) {
		...
		lcd_kit_parse_u32(
			np, "lcd-kit,check-reg-off-support-dsm-report",
	...
	if (common_info->mipi_check.support) {
		lcd_kit_parse_u32(
			np, "lcd-kit,panel-mipi-error-report-threshold",
			&common_info->mipi_check.mipi_error_report_threshold,
			1);
...
int lcd_dsm_client_record(struct dsm_client *lcd_dclient, char *record_buf,
			  int lcd_dsm_error_no, int rec_num_limit,
			  int *cur_rec_time)
{
...
#define RECORD_BUFLEN 200
	...
	for (i = 0; i < common_info->mipi_check.value.cnt; i++) {
		...
		if (read_value[i] != expect_ptr[i]) {
			...
			LCD_KIT_ERR(
				"mipi check error[%d]: current error times:%d! total error times:%d, check-error-times/check-times:%d/%d\n",
			...
			ret = snprintf(
				record_buf, RECORD_BUFLEN,
				"%s:display_on_keep_time=%lds, reg_val[%d]=0x%x!\n",
				"lcd", diskeeptime,
				common_info->mipi_check.cmds.cmds[i].payload[0],
				read_value[i]);
			if (ret < 0) {
				LCD_KIT_ERR("snprintf happened error!\n");
				continue;
			...
			(void)lcd_dsm_client_record(
				...
				DSM_LCD_MIPI_TRANSMIT_ERROR_NO, REC_LIMIT_TIMES,
				&recordtime);
#endif
			continue;
		}
		LCD_KIT_INFO(
			"mipi check nomal[%d]: total error times:%d, check-error-times/check-times:%d/%d\n",
			i, mipi_errors[i].total_errors,
			mipi_errors[i].mipi_error_times,
			mipi_errors[i].mipi_check_times);
	}
}
...
static void lcd_kit_btb_init(void)
{
	...
	if (common_info->btb_check_type == LCD_KIT_BTB_CHECK_IRQ) {
		...
		btb_gpio = common_info->lcd_btb_gpio.buf[0] +
			   common_info->gpio_offset;
		...
		btb_gpio = common_info->lcd_btb_gpio.buf[1] +
			   common_info->gpio_offset;
