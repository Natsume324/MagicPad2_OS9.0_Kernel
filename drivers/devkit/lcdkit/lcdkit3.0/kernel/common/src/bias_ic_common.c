...
static void bias_ic_get_value(struct bias_ic_device *pbias_ic,
			      const char *chip_name, int pos_votage,
			      int neg_votage)
{
	int i;
	int j;
	...
	for (i = 0; i < ARRAY_SIZE(bias_config); i++) {
		...
		for (j = 0; j < bias_config[i].len; j++) {
			if (bias_config[i].vol_table[j].voltage == pos_votage) {
		...
		if (j == bias_config[i].len) {
			printk(KERN_INFO
			       "not found vsp voltage, use default\n");
			pbias_ic->bias_config.vpos_val = DEFAULT_VOL_VAL;
		}

		for (j = 0; j < bias_config[i].len; j++) {
			if (bias_config[i].vol_table[j].voltage == neg_votage) {
	...
	if (i == ARRAY_SIZE(bias_config)) {
		printk(KERN_INFO
		       "not found right voltage, use default voltage\n");
		pbias_ic->bias_config.vpos_val = DEFAULT_VOL_VAL;
		pbias_ic->bias_config.vneg_val = DEFAULT_VOL_VAL;
	}
}
...
static ssize_t bias_vsp_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
...
static ssize_t bias_vsp_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t bias_vsn_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct bias_ic_device *pchip = NULL;
	int ret;
...
static ssize_t bias_vsn_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t size)
{
	struct bias_ic_device *pchip = NULL;
	unsigned int input_val;
	...
	if (lcd_dclient) {
		if (!dsm_client_ocuppy(lcd_dclient)) {
			dsm_client_record(
				...
				pbias_ic->bias_config.vpos_reg,
				pbias_ic->bias_config.vpos_val, chip_name);
	...
	if (lcd_dclient) {
		if (!dsm_client_ocuppy(lcd_dclient)) {
			dsm_client_record(
				...
				pbias_ic->bias_config.vneg_reg,
				pbias_ic->bias_config.vneg_val, chip_name);
			dsm_client_notify(lcd_dclient,
					  DSM_LCD_BIAS_I2C_ERROR_NO);
			return BIAS_RET_OK;
		}
...
static int bias_ic_set_vneg(void)
{
	...
	ret = bias_ic_write_byte(pbias_ic, pbias_ic->bias_config.vneg_reg,
				 pbias_ic->bias_config.vneg_val);
	if (ret < 0) {
...
static int bias_ic_set_state(void)
{
	...
	if (pbias_ic->bias_config.state_mask != 0) {
		...
		value |= (pbias_ic->bias_config.state_val &
			  pbias_ic->bias_config.state_mask);
		ret = bias_ic_write_byte(
			pbias_ic, pbias_ic->bias_config.state_reg, value);
		if (ret < 0) {
			 ...
			 const struct i2c_device_id *id)
{
	...
	pbias_ic = devm_kzalloc(&client->dev, sizeof(struct bias_ic_device),
				GFP_KERNEL);
	...
	if (bias_class != NULL) {
		pbias_ic->dev =
			device_create(bias_class, NULL, 0, "%s", "lcd_bias");
		if (IS_ERR(pbias_ic->dev)) {
			printk(KERN_ERR
			       "%s: Unable to create device; errno = %ld\n",
			...
			if (ret) {
				printk(KERN_ERR
				       "%s: Create bias sysfs group node failed!\n",
				       __func__);
...
static int __init bias_ic_module_init(void)
{
	...
	memcpy(bias_ic_common_id[0].name, chip_name,
	       LCD_BIAS_I2C_NAME_SIZE - 1);
	...
	if (IS_ERR(bias_class)) {
		printk(KERN_ERR
		       "%s: Unable to create bias class; errno = %ld\n",
