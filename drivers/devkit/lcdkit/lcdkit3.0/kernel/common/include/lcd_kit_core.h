...
struct lcd_kit_ops {
	...
	bool (*panel_event_skip_delay)(void *hld, uint32_t event,
				       uint32_t data);
	int (*get_thp_notify_lcd_by_type)(int type);
};
...
struct ts_kit_ops {
	...
	int (*ts_multi_power_notify)(enum lcd_kit_ts_pm_type type, int sync,
				     int panel_index);
