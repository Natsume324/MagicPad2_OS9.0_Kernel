...
#define OF_PROPERTY_READ_U32_DEFAULT(np, propname, ptr_out_value, default)              \
	do {                                                                            \
		if (of_property_read_u32(np, propname, ptr_out_value)) {                \
			LCD_KIT_INFO(                                                   \
				"of_property_read_u32: %s not find, use default: %d\n", \
				propname, default);                                     \
...
#define OF_PROPERTY_READ_U8_DEFAULT(np, propname, ptr_out_value, default)           \
	do {                                                                        \
		...
		if (of_property_read_u32(np, propname, &temp)) {                    \
			LCD_KIT_INFO(                                               \
				"of_property_read: %s not find, use default: %d\n", \
				propname, default);                                 \
			temp = default;                                             \
		}                                                                   \
		*ptr_out_value = (char)temp;                                        \
...
struct lcd_kit_common_ops {
	...
	int (*hbm_set_handle)(void *hld, int last_hbm_level, int hbm_dimming,
			      int hbm_level, int fps_status);
	...
	void (*mipi_check)(void *pdata, char *panel_name,
