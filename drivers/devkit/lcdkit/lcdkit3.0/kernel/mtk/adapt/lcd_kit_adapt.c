...
void mipi_dsi_cmds_tx(struct mipi_dsi_device *dsi, struct dsi_cmd_desc *cmds)
{
	...
	case DTYPE_DCS_LWRITE:
	case DTYPE_DSC_LWRITE:
		ret = mipi_dsi_dcs_write_buffer(dsi, cmds->tx_buf[0],
						cmds->tx_len[0]);
		break;
	case DTYPE_GEN_WRITE:
	case DTYPE_GEN_WRITE1:
	case DTYPE_GEN_WRITE2:
	case DTYPE_GEN_LWRITE:
		ret = mipi_dsi_generic_write(dsi, cmds->tx_buf[0],
					     cmds->tx_len[0]);
		break;
	default:
		ret = mipi_dsi_dcs_write_buffer(dsi, cmds->tx_buf[0],
...
static int
lcd_kit_cmds_to_mtk_dsi_cmds(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
...
static int
lcd_kit_cmds_to_mtk_dsi_read_cmds(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
...
int mtk_mipi_dsi_cmds_tx(void *dsi, struct lcd_kit_dsi_cmd_desc *cmds, int cnt,
			 int link_state)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (lcd_kit_cmd_is_write(&cmds->cmds[i])) {
			lcd_kit_cmds_to_mtk_dsi_cmds(&cmds->cmds[i], &dsi_cmd,
						     cmds->link_state);
			ret = mtk_mipi_dsi_cmds_tx(dsi, &cmds->cmds[i], 1,
						   cmds->link_state);
			if (ret < 0)
...
static int
lcd_kit_cmds_to_mtk_cmds_extern_tx(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
...
static int
lcd_kit_cmds_to_mtk_cmds_extern_rx(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
				   struct mtk_ddic_dsi_msg *cmd_msg,
				   int link_state, unsigned char *buf)
{
	if (lcd_kit_cmds == NULL) {
		LCD_KIT_ERR("lcd_kit_cmds is NULL\n");
		return LCD_KIT_FAIL;
...
int mtk_mipi_dsi_cmds_extern_tx(struct lcd_kit_dsi_cmd_desc *cmds, int cnt,
				int link_state, bool lock_state)
{
	struct lcd_kit_dsi_cmd_desc *cm = NULL;
	...
	cm = cmds;
	for (i = 0; i < cnt; i++) {
		ret = lcd_kit_cmds_to_mtk_cmds_extern_tx(cm, &cmd_msg,
							 link_state);
		if (ret != LCD_KIT_OK) {
			       ...
			       unsigned int len)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (lcd_kit_cmd_is_write(&cmds->cmds[i])) {
			ret = mtk_mipi_dsi_cmds_extern_tx(
				&cmds->cmds[i], 1, cmds->link_state, true);
			if (ret < 0) {
				LCD_KIT_ERR("the %d mipi cmd send fail\n", i);
				return LCD_KIT_FAIL;
			}
		} else {
			ret = lcd_kit_cmds_to_mtk_cmds_extern_rx(
				&cmds->cmds[i], &cmd_msg, cmds->link_state,
				&buffer[0]);
			if (ret != LCD_KIT_OK) {
				LCD_KIT_ERR("exchange dsi mipi fail\n");
				return LCD_KIT_FAIL;
	  ...
	  struct lcd_kit_dsi_panel_cmds *pcmds)
{
	...
	for (i = 0; i < pcmds->cmd_cnt; i++) {
		extern_cb(dsi_drv, handle, pcmds->cmds[i].payload,
			  pcmds->cmds[i].dlen);
		lcd_kit_delay(pcmds->cmds[i].wait, pcmds->cmds[i].waittype,
			      true);
...
static int get_setting_table(const struct lcd_kit_dsi_panel_cmds *cmds,
			     struct LCM_setting_table_V3 *setting_table,
			     int len)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		...
		if (table.count >= sizeof(table.para_list)) {
			LCD_KIT_ERR(
				"table.count is longer than para_list size\n");
			return LCD_KIT_FAIL;
		}
		memcpy(&table.para_list, cmds->cmds[i].payload + 1,
		       table.count);
		...
		if (cmds->cmds[i].waittype == WAIT_TYPE_MS) {
			if (len <= index) {
				LCD_KIT_ERR(
					"setting_table size is not enough\n");
				return LCD_KIT_FAIL;
			}
...
int lcd_kit_parse_read_data(uint8_t *out, uint32_t *in, int start_index,
			    int dlen)
{
	return 0;
}
				...
				bool lock_state)
{
	...
	if (ret != 0) {
		LCD_KIT_ERR("fail cmds count:%d, dtype:0x%x, len:%d\n",
			    panel_cmds->cmd_cnt, cmd_msg.type[0],
			    cmd_msg.tx_len[0]);
		return ret;
	} else {
