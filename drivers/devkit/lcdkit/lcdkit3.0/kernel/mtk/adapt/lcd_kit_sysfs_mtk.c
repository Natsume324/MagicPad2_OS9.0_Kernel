...
struct lcd_kit_dsi_panel_cmds ddic_debug = {NULL, 0, ddic_test_cmds, 0, 0, 0};
	...
	if (disp_info->fps.support) {
		...
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt;
			...
			ret += snprintf(tmp, sizeof(tmp), "%d",
					timing->buf[FPS_VRE_INDEX]);
			strncat(str, tmp, strlen(tmp));
		}
		if (disp_info->fps.order_mode == CUST_MODE) {
			strncat(str,
				";current_index:", strlen(";current_index:"));
			memset(tmp, 0, sizeof(tmp));
			snprintf(tmp, sizeof(tmp), "%d",
				 lcd_fps_get_index(disp_info->fps.current_fps));
			strncat(str, tmp, strlen(tmp));
		}
...
static void lcdkit_parse_fps_test(char *buf,
				  struct lcd_kit_string_arrays *string_arrays)
{
	int i;
	int cnt;
	cnt = string_arrays->cnt;
	for (i = 0; i < cnt; i++) {
		strncat(buf, string_arrays->buf[i],
			strlen(string_arrays->buf[i]));
		LCD_KIT_INFO("%s\n", string_arrays->buf[i]);
		if (i != (cnt - 1))
			strncat(buf, "#", strlen("#"));
	}
...
static void lcd_fps_cust_test(char *buf, char test_type)
{
	...
	switch (test_type) {
	case MMI_TEST:
		lcdkit_parse_fps_test(
			buf, &disp_info->lcd_test_config.mmi_test_list);
		break;
	case RT_TEST:
		lcdkit_parse_fps_test(buf,
				      &disp_info->lcd_test_config.rt_test_list);
		break;
	case TPLCD_TEST:
		lcdkit_parse_fps_test(
			buf, &disp_info->lcd_test_config.tplcd_test_list);
		break;
	default:
		lcdkit_parse_fps_test(
			buf, &disp_info->lcd_test_config.tplcd_test_list);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (!disp_info->fps.support) {
		...
		if (disp_info->fps.order_mode == INDEX_MODE)
			ret = snprintf(
				str, sizeof(str), "1,%d,%d", 0,
				disp_info->fps.panel_support_fps_list.cnt);
		else
			ret = snprintf(
				str, sizeof(str), "1,%d,%d",
				disp_info->fps.default_fps,
				disp_info->fps.panel_support_fps_list.cnt);
		...
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt;
			...
			if (disp_info->fps.order_mode == INDEX_MODE) {
				...
				index = disp_info->fps.panel_support_fps_list
						.buf[i];
				timing = &disp_info->fps.fps_dsi_timming[index];
	...
	if (FACT_INFO->check_reg.support) {
		...
		for (i = 0; i < FACT_INFO->check_reg.value.cnt; i++) {
			if (i < FACT_INFO->check_reg.mask.cnt) {
				...
				LCD_KIT_INFO(
					"mask_ptr[i]=0x%x, after map read_value[i]=0x%x!\n",
					mask_ptr[i], read_value[i]);
			}
			if ((char)read_value[i] != expect_ptr[i]) {
				ret = -1;
				LCD_KIT_ERR(
					"read_value[%u] = 0x%x, but expect_ptr[%u] = 0x%x!\n",
					i, read_value[i], i, expect_ptr[i]);
				break;
			}
			LCD_KIT_INFO(
				"read_value[%u] = 0x%x same with expect value!\n",
				i, read_value[i]);
		}
		if (ret == 0)
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	memset(&ddic_test_cmds[0], 0,
	       DDIC_READ_MAX_REG * sizeof(struct lcd_kit_dsi_cmd_desc));
				  ...
				  const char *buf, size_t count)
{
	...
	ddic_test_cmds[ddic_debug.cmd_cnt].payload =
		kzalloc(ddic_test_cmds[ddic_debug.cmd_cnt].dlen, GFP_KERNEL);
	if (!ddic_test_cmds[ddic_debug.cmd_cnt].payload) {
		LCD_KIT_ERR("kzalloc fail\n");
		return LCD_KIT_FAIL;
	}
	for (i = 0; i < ddic_test_cmds[ddic_debug.cmd_cnt].dlen; i++) {
		ddic_test_cmds[ddic_debug.cmd_cnt].payload[i] =
			reg_value[2 + i];
		LCD_KIT_INFO("payload[%d]:0x%x ", i,
			     ddic_test_cmds[ddic_debug.cmd_cnt].payload[i]);
	}
	LCD_KIT_INFO(
		"\n ddic_test_cmds[%d]:dtype:0x%x, dlen:0x%x, payload[0]:0x%x \n",
		ddic_debug.cmd_cnt, ddic_test_cmds[ddic_debug.cmd_cnt].dtype,
				  ...
				  const char *buff, size_t count)
{
	...
	if (strlen(buff) < OEM_INFO_SIZE_MAX) {
		...
		while (token) {
			oem_info[i++] =
				(unsigned char)simple_strtol(token, NULL, 0);
			token = strsep(&cur, ",");
		}
	} else {
...
static int lcd_judge_ddic_lv_detect(unsigned int pic_index, int loop)
{
	...
	ret = lcd_kit_dsi_cmds_extern_rx(
		read_value, &FACT_INFO->ddic_lv_detect.rd_cmds[pic_index],
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	if (FACT_INFO->oneside_mode.support || FACT_INFO->color_aod_det.support)
		ret = snprintf(buf, PAGE_SIZE, "%d\n",
