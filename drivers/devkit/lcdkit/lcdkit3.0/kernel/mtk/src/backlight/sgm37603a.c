...
static int sgm37603a_config_write(struct sgm37603a_chip_data *pchip,
				  ...
				  unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != SGM37603A_INVALID_VAL) {
			...
			LCD_KIT_INFO("sgm37603a write 0x%x = 0x%x", reg[i],
				     val[i]);
			if (ret < 0) {
...
static int sgm37603a_config_read(struct sgm37603a_chip_data *pchip,
				 unsigned int reg[], unsigned int val[],
				 unsigned int size)
{
	int ret = 0;
...
static int sgm37603a_chip_init(struct sgm37603a_chip_data *pchip)
{
	...
	ret = sgm37603a_config_write(pchip, sgm37603a_reg_addr,
				     sgm37603a_bl_info.sgm37603a_reg,
				     SGM37603A_RW_REG_MAX);
	if (ret < 0) {
...
static ssize_t sgm37603a_reg_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "0x10 = 0x%x 0x11 = 0x%x\n \
		\r0x1A = 0x%x 0x19 = 0x%x\n ",
		       sgm37603a_reg_info[0], sgm37603a_reg_info[1],
		       sgm37603a_reg_info[2], sgm37603a_reg_info[3]);
	return ret;
}

static ssize_t sgm37603a_reg_store(struct device *dev,
...
{
	int ret;
	ret = regmap_write(sgm37603a_g_chip->regmap, SGM37603A_REG_LEDS_CONFIG,
			   SGM37603A_REG_LEDS_CONFIG_DISABLE);
	if (ret < 0)
		LCD_KIT_ERR("write sgm37603a 0x%x fail\n",
			    SGM37603A_REG_LEDS_CONFIG);
			   ...
			   const struct i2c_device_id *id)
{
	...
	pchip->dev =
		device_create(sgm37603a_class, NULL, 0, "%s", client->name);
	if (IS_ERR(pchip->dev)) {
		/* Not fatal */
	...
	if (sgm37603a_bl_info.sgm37603a_hw_en) {
		ret = gpio_request(sgm37603a_bl_info.sgm37603a_hw_en_gpio,
				   NULL);
		if (ret) {
