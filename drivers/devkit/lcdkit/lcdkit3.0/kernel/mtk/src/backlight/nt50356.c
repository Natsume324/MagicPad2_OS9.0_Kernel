...
static void nt50356_get_target_voltage(int *vpos, int *vneg)
{
	int i;
	int tal_size =
		sizeof(nt50356_vol_table) / sizeof(struct nt50356_voltage);
...
static int nt50356_chip_init(struct nt50356_chip_data *pchip)
{
	...
	ret = of_property_read_u32(np, NT50356_ONLY_BACKLIGHT,
				   &g_bl_info.bl_only);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, NT50356_HW_ENABLE,
				   &g_bl_info.nt50356_hw_en);
	if (ret < 0) {
		nt50356_err("get nt50356_hw_en dts config failed\n");
		g_bl_info.nt50356_hw_en = 0;
	}
	if (g_bl_info.nt50356_hw_en != 0) {
		ret = of_property_read_u32(np, NT50356_HW_EN_GPIO,
					   &g_bl_info.nt50356_hw_en_gpio);
		if (ret < 0) {
		...
		if (ret < 0) {
			nt50356_err(
				"get nt50356_hw_en_pull_low dts config fail\n");
			g_bl_info.nt50356_hw_en_pull_low = 0;
		}
	}

	ret = of_property_read_u32(np, NT50356_HW_EN_DELAY,
...
static void nt50356_get_bias_config(int vsp, int vsn, int *outvsp, int *outvsn)
{
	int i;
...
static int nt50356_set_bias_power_down(int vpos, int vneg)
{
	...
	ret = regmap_write(nt50356_g_chip->regmap, REG_DISPLAY_BIAS_CONFIG_1,
			   g_bl_info.nt50356_enable_vsp_vsn);
	if (ret < 0) {
	...
	ret = regmap_write(nt50356_g_chip->regmap, REG_LCM_BOOST_BIAS,
			   g_bl_info.nt50356_pull_down_boost);
	if (ret < 0) {
		nt50356_err("write enable_vsp_vsn failed!\n");
		return ret;
...
static int nt50356_set_bias(int vpos, int vneg)
{
	...
	if (g_bl_info.nt50356_hw_en != 0 &&
		...
		ret = gpio_direction_output(g_bl_info.nt50356_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
...
static void nt50356_check_fault(struct nt50356_chip_data *pchip, int last_level,
				int level)
{
	unsigned int val = 0;
	int ret;
...
static void backlight_ic_check_fault(struct nt50356_chip_data *pchip, int level)
{
...
static int nt50356_chip_enable(struct nt50356_chip_data *pchip)
{
	...
	ret = nt50356_config_write(pchip, nt50356_reg_addr, nt50356_reg,
				   NT50356_RW_REG_MAX);
	if (ret < 0) {
...
static DEVICE_ATTR(reg_bl, 0644, nt50356_reg_bl_show, nt50356_reg_bl_store);
...
static DEVICE_ATTR(reg, 0644, nt50356_reg_show, nt50356_reg_store);
...
static const struct i2c_device_id nt50356_id[] = {{NT50356_NAME, 0}, {}};
