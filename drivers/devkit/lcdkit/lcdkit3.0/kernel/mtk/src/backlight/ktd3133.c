				...
				unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			if (ret < 0) {
				LCD_KIT_ERR(
					"write ktd3133 bl cfg reg 0x%x failed\n",
					reg[i]);
				goto exit;
			}
		}
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = ktd3133_config_read(pchip, ktd3133_reg_addr,
				  ktd3133_bl_info.ktd3133_reg,
				  KTD3133_RW_REG_MAX);
	if (ret < 0) {
...
static void ktd3133_enable(void)
{
	...
	if (ktd3133_bl_info.ktd3133_hw_en) {
		...
		ret = gpio_direction_output(ktd3133_bl_info.ktd3133_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
			LCD_KIT_ERR("ktd3133 set gpio output not success\n");
		gpio_set_value(ktd3133_bl_info.ktd3133_hw_en_gpio,
			       GPIO_OUT_ONE);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (ktd3133_bl_info.ktd3133_hw_en) {
		...
		if (ret) {
			LCD_KIT_ERR(
				"ktd3133 Could not request ktd3133 hw_en_gpio\n");
			goto err_sysfs;
		}
	}
