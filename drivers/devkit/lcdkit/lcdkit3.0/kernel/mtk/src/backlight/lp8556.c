...
static int lp8556_parse_reg_config(struct lp8556_chip_data *pchip,
				   struct device_node *np)
{
	...
	case LP8556_E03_E07_CFGF_VAL:
		strncpy(reg_node_name, DTS_COMP_LP8556_E11,
			sizeof(reg_node_name));
		break;
	default:
...
static int lp8556_parse_dts(struct device_node *np)
{
	...
	if (ret < 0) {
		...
		if (ret < 0)
			lp8556_info(
				"can not get lp8556_2_i2c_bus_id dts node\n");
	}
	...
	if (lp8556_bl_info.lp8556_hw_en != 0) {
		...
		if (lp8556_bl_info.dual_ic) {
			ret = of_property_read_u32(
				np, "lp8556_2_hw_en_gpio",
				&lp8556_bl_info.lp8556_2_hw_en_gpio);
			if (ret < 0) {
				lp8556_err(
					"get lp8556_2_hw_en_gpio dts config failed\n");
				return ret;
			}
		}
	}
			       ...
			       unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			lp8556_info(
				"write lp8556 backlight config register 0x%x val 0x%x\n",
				reg[i], val[i]);
		}
	}
			      ...
			      unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			lp8556_err(
				"read lp8556 backlight config register 0x%x failed",
				reg[i]);
			goto exit;
		} else {
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(
		...
		lp8556_bl_info.lp8556_reg[14], lp8556_bl_info.lp8556_reg[15],
		lp8556_bl_info.lp8556_reg[16], lp8556_bl_info.lp8556_reg[17],
		lp8556_bl_info.lp8556_reg[18], lp8556_bl_info.lp8556_reg[19],
		lp8556_bl_info.lp8556_reg[20], lp8556_bl_info.lp8556_reg[21]);
	return ret;
...
static void lp8556_enable(void)
{
	...
	if (lp8556_bl_info.lp8556_hw_en) {
		...
		ret = gpio_direction_output(lp8556_bl_info.lp8556_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
			lp8556_err("lp8556 set gpio output not success\n");
		gpio_set_value(lp8556_bl_info.lp8556_hw_en_gpio, GPIO_OUT_ONE);
		if (lp8556_bl_info.dual_ic) {
			ret = gpio_request(lp8556_bl_info.lp8556_2_hw_en_gpio,
					   NULL);
			if (ret)
				lp8556_err(
					"lp8556 Could not request  hw_en2_gpio\n");
			ret = gpio_direction_output(
				lp8556_bl_info.lp8556_2_hw_en_gpio,
				GPIO_DIR_OUT);
			if (ret)
				lp8556_err(
					"lp8556 set gpio output not success\n");
			gpio_set_value(lp8556_bl_info.lp8556_2_hw_en_gpio,
				       GPIO_OUT_ONE);
		}
			...
			const struct i2c_device_id *id)
{
	...
	pchip = devm_kzalloc(&client->dev, sizeof(struct lp8556_chip_data),
			     GFP_KERNEL);
	if (!pchip)
		return -ENOMEM;
