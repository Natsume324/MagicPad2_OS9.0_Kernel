...
static int sm5350_config_write(struct sm5350_chip_data *pchip,
			       ...
			       unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != SM5350_INVALID_VAL) {
			...
			LCD_KIT_INFO("sm5350 write 0x%x = 0x%x", reg[i],
				     val[i]);
			if (ret < 0) {
...
static int sm5350_config_read(struct sm5350_chip_data *pchip,
			      unsigned int reg[], unsigned int val[],
			      unsigned int size)
{
	int ret = 0;
...
static ssize_t sm5350_reg_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	...
	ret = sm5350_config_read(pchip, sm5350_reg_addr, sm5350_reg_info,
				 SM5350_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(buf, PAGE_SIZE, "0x10 = 0x%x,0x16 = 0x%x,0x17 = 0x%x\n \
		...
		\r0x20 = 0x%x,0x21 = 0x%x,0x24 = 0x%x\n",
		       sm5350_reg_info[0], sm5350_reg_info[1],
		       sm5350_reg_info[2], sm5350_reg_info[3],
		       sm5350_reg_info[4], sm5350_reg_info[5],
		       sm5350_reg_info[6], sm5350_reg_info[7],
		       sm5350_reg_info[8]);
	return ret;
}

static ssize_t sm5350_reg_store(struct device *dev,
...
static DEVICE_ATTR(reg, (S_IRUGO | S_IWUSR), sm5350_reg_show, sm5350_reg_store);
...
static void sm5350_enable(void)
{
	...
	if (sm5350_bl_info.sm5350_hw_en != 0 &&
		...
		gpio_set_value(sm5350_bl_info.sm5350_hw_en_gpio, GPIO_OUT_ONE);
...
static void sm5350_disable(void)
{
	if (sm5350_bl_info.sm5350_hw_en != 0 &&
	    sm5350_bl_info.sm5350_hw_en_pull_low != 0) {
		gpio_set_value(sm5350_bl_info.sm5350_hw_en_gpio, GPIO_OUT_ZERO);
