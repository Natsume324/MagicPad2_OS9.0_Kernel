...
static int mp3314_parse_dts(struct device_node *np)
{
	...
	if (ret < 0) {
		...
		if (ret < 0)
			mp3314_info(
				"can not get mp3314_2_i2c_bus_id dts node\n");
	}
	...
	if (mp3314_bl_info.mp3314_hw_en != 0) {
		...
		if (mp3314_bl_info.dual_ic) {
			ret = of_property_read_u32(
				np, "mp3314_2_hw_en_gpio",
				&mp3314_bl_info.mp3314_2_hw_en_gpio);
			if (ret < 0) {
				mp3314_err(
					"get mp3314_2_hw_en_gpio dts config failed\n");
				return ret;
			}
			if (mp3314_bl_info.mp3314_hw_en_gpio ==
			    mp3314_bl_info.mp3314_2_hw_en_gpio) {
			       ...
			       unsigned int size)
{
	...
	if (ret < 0) {
		mp3314_err(
			"write mp3314 backlight config register bl_disable = 0x%x failed\n",
			BL_DISABLE_CTL);
		goto exit;
	}
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			if (ret < 0) {
				mp3314_err(
					"write mp3314 backlight config register 0x%x failed\n",
					reg[i]);
				goto exit;
			}
		}
			      ...
			      unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			mp3314_err(
				"read mp3314 backlight config register 0x%x failed",
				reg[i]);
			goto exit;
		} else {
...
static int mp3314_chip_init(struct mp3314_chip_data *pchip)
{
	...
	if (mp3314_bl_info.dual_ic) {
		...
		ret = mp3314_2_config_write(pchip, &mp3314_bl_en_ctl,
					    &mp3314_bl_disable_ctl, 1);
		if (ret < 0) {
			mp3314_err("mp3314 diable slave bl register failed\n");
			goto out;
		}
		ret = mp3314_2_config_write(pchip, mp3314_reg_addr,
					    mp3314_bl_info.mp3314_reg,
					    MP3314_RW_REG_MAX);
		if (ret < 0) {
	...
	ret = mp3314_config_write(pchip, mp3314_reg_addr,
				  mp3314_bl_info.mp3314_reg, MP3314_RW_REG_MAX);
	if (ret < 0) {
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	ret = mp3314_config_read(pchip, mp3314_reg_addr,
				 mp3314_bl_info.mp3314_reg, MP3314_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(
		...
		mp3314_bl_info.mp3314_reg[0], mp3314_bl_info.mp3314_reg[1],
		mp3314_bl_info.mp3314_reg[2], mp3314_bl_info.mp3314_reg[3],
		mp3314_bl_info.mp3314_reg[4], mp3314_bl_info.mp3314_reg[5],
		mp3314_bl_info.mp3314_reg[6], mp3314_bl_info.mp3314_reg[7],
		mp3314_bl_info.mp3314_reg[8], mp3314_bl_info.mp3314_reg[9],
		mp3314_bl_info.mp3314_reg[10], mp3314_bl_info.mp3314_reg[11],
		mp3314_bl_info.mp3314_reg[12], mp3314_bl_info.mp3314_reg[13],
		mp3314_bl_info.mp3314_reg[14], mp3314_bl_info.mp3314_reg[15]);
	return ret;
...
static void mp3314_enable(void)
{
	...
	if (mp3314_bl_info.mp3314_hw_en) {
		...
		ret = gpio_direction_output(mp3314_bl_info.mp3314_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
			mp3314_err("mp3314 set gpio output not success\n");
		gpio_set_value(mp3314_bl_info.mp3314_hw_en_gpio, GPIO_OUT_ONE);
		if (mp3314_bl_info.dual_ic &&
		    is_dual_hw_en_gpio_same == false) {
			ret = gpio_request(mp3314_bl_info.mp3314_2_hw_en_gpio,
					   NULL);
			if (ret)
				mp3314_err(
					"mp3314 Could not request  hw_en2_gpio\n");
			ret = gpio_direction_output(
				mp3314_bl_info.mp3314_2_hw_en_gpio,
				GPIO_DIR_OUT);
			if (ret)
				mp3314_err(
					"mp3314 set gpio output not success\n");
			gpio_set_value(mp3314_bl_info.mp3314_2_hw_en_gpio,
				       GPIO_OUT_ONE);
		}
...
static int mp3314_set_backlight(uint32_t bl_level)
{
	...
	ret = regmap_write(mp3314_g_chip->regmap,
			   mp3314_bl_info.mp3314_level_msb, bl_msb);
	if (ret < 0)
		mp3314_debug("write mp3314 backlight level msb:0x%x failed\n",
			     bl_msb);
	ret = regmap_write(mp3314_g_chip->regmap,
			   mp3314_bl_info.mp3314_level_lsb, bl_lsb);
	if (ret < 0)
		mp3314_debug("write mp3314 backlight level lsb:0x%x failed\n",
			     bl_lsb);

	if (mp3314_bl_info.dual_ic) {
		ret = mp3314_2_config_write(mp3314_g_chip,
					    &mp3314_bl_info.mp3314_level_msb,
					    &bl_msb, 1);
		if (ret < 0)
			mp3314_debug(
				"write slave mp3314 backlight level msb:0x%x failed\n",
				bl_msb);
		ret = mp3314_2_config_write(mp3314_g_chip,
					    &mp3314_bl_info.mp3314_level_lsb,
					    &bl_lsb, 1);
		if (ret < 0)
			mp3314_debug(
				"write slave mp3314 backlight level lsb:0x%x failed\n",
				bl_lsb);
	}
			...
			const struct i2c_device_id *id)
{
	...
	pchip = devm_kzalloc(&client->dev, sizeof(struct mp3314_chip_data),
			     GFP_KERNEL);
	if (!pchip)
		return -ENOMEM;
	...
	ret = regmap_read(pchip->regmap, mp3314_reg_addr[0],
			  &mp3314_bl_info.mp3314_reg[0]);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "mp3314_level_lsb",
				   &mp3314_bl_info.mp3314_level_lsb);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "mp3314_level_msb",
				   &mp3314_bl_info.mp3314_level_msb);
	if (ret < 0) {
