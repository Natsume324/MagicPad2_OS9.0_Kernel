...
static int sy7758_parse_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "bl_ctrl_enable",
				   &sy7758_bl_info.ctrl_enable);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		if (ret < 0)
			sy7758_info(
				"can not get sy7758_2_i2c_bus_id dts node\n");
	}
	...
	if (sy7758_bl_info.sy7758_hw_en != 0) {
		...
		if (sy7758_bl_info.dual_ic) {
			ret = of_property_read_u32(
				np, "sy7758_2_hw_en_gpio",
				&sy7758_bl_info.sy7758_2_hw_en_gpio);
			if (ret < 0) {
				sy7758_err(
					"get sy7758_2_hw_en_gpio dts config failed\n");
				return ret;
			}
		}
	}
	...
	if (plcd_kit_info != NULL) {
		...
		if (sy7758_bl_info.dual_ic)
			sy7758_bl_info.sy7758_2_hw_en_gpio +=
				plcd_kit_info->gpio_offset;
	}
#else
	sy7758_bl_info.sy7758_hw_en_gpio +=
		((struct mtk_panel_info *)(lcdkit_mtk_common_panel.panel_info))
			->gpio_offset;
	if (sy7758_bl_info.dual_ic)
		sy7758_bl_info.sy7758_2_hw_en_gpio +=
			((struct mtk_panel_info *)(lcdkit_mtk_common_panel
							   .panel_info))
				->gpio_offset;
#endif
...
static int sy7758_2_read_u8(struct sy7758_chip_data *pchip, unsigned int reg,
			    unsigned int val[], unsigned int lenght)
{
			       ...
			       unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			if (ret < 0) {
				sy7758_err(
					"write sy7758 backlight config register 0x%x failed\n",
					reg[i]);
				goto exit;
			}
		}
...
static void lcd_kit_backlight_ctrl_enable(struct sy7758_chip_data *pchip)
{
	...
	ret = sy7758_config_write(
		pchip, &sy7758_reg_addr[SY7758_RW_REG_MAX - 1],
		&sy7758_bl_info.sy7758_reg[SY7758_RW_REG_MAX - 1], 1);
	if (ret < 0) {
			      ...
			      unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			sy7758_err(
				"read sy7758 backlight config register 0x%x failed",
				reg[i]);
			goto exit;
		} else {
...
static int sy7758_chip_init(struct sy7758_chip_data *pchip)
{
	...
	if (sy7758_bl_info.dual_ic) {
		ret = sy7758_2_config_write(pchip, sy7758_reg_addr,
					    sy7758_bl_info.sy7758_reg,
					    SY7758_INIT_CMD_NUM);
		if (ret < 0) {
			sy7758_err("sy7758 slave config register failed\n");
			goto out;
	...
	ret = sy7758_config_write(pchip, sy7758_reg_addr,
				  sy7758_bl_info.sy7758_reg,
				  SY7758_INIT_CMD_NUM);
	if (ret < 0) {
		sy7758_err("sy7758 config register failed");
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	ret = sy7758_config_read(pchip, sy7758_reg_addr,
				 sy7758_bl_info.sy7758_reg, SY7758_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(
		buf, PAGE_SIZE,
		"Eprom Configuration0(0x01) = 0x%x\nEprom Configuration0(0xA0) = 0x%x\n\
		...
		sy7758_bl_info.sy7758_reg[0], sy7758_bl_info.sy7758_reg[1],
		sy7758_bl_info.sy7758_reg[2], sy7758_bl_info.sy7758_reg[3],
		sy7758_bl_info.sy7758_reg[4], sy7758_bl_info.sy7758_reg[5],
		sy7758_bl_info.sy7758_reg[6], sy7758_bl_info.sy7758_reg[7],
		sy7758_bl_info.sy7758_reg[8], sy7758_bl_info.sy7758_reg[9],
		sy7758_bl_info.sy7758_reg[10], sy7758_bl_info.sy7758_reg[11],
		sy7758_bl_info.sy7758_reg[12], sy7758_bl_info.sy7758_reg[13],
		sy7758_bl_info.sy7758_reg[14], sy7758_bl_info.sy7758_reg[15],
		sy7758_bl_info.sy7758_reg[16], sy7758_bl_info.sy7758_reg[17],
		sy7758_bl_info.sy7758_reg[18], sy7758_bl_info.sy7758_reg[19],
		sy7758_bl_info.sy7758_reg[20], sy7758_bl_info.sy7758_reg[21],
		sy7758_bl_info.sy7758_reg[22]);
	return ret;
...
static void sy7758_enable(void)
{
	...
	if (sy7758_bl_info.sy7758_hw_en) {
		...
		ret = gpio_direction_output(sy7758_bl_info.sy7758_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
			sy7758_err("sy7758 set gpio output not success\n");
		gpio_set_value(sy7758_bl_info.sy7758_hw_en_gpio, GPIO_OUT_ONE);
		if (sy7758_bl_info.dual_ic) {
			ret = gpio_request(sy7758_bl_info.sy7758_2_hw_en_gpio,
					   NULL);
			if (ret)
				sy7758_err(
					"sy7758 Could not request  hw_en2_gpio\n");
			ret = gpio_direction_output(
				sy7758_bl_info.sy7758_2_hw_en_gpio,
				GPIO_DIR_OUT);
			if (ret)
				sy7758_err(
					"sy7758 set gpio output not success\n");
			gpio_set_value(sy7758_bl_info.sy7758_2_hw_en_gpio,
				       GPIO_OUT_ONE);
		}
...
static int sy7758_set_backlight(uint32_t bl_level)
{
	...
	ret = regmap_write(sy7758_g_chip->regmap,
			   sy7758_bl_info.sy7758_level_lsb, bl_lsb);
	if (ret < 0)
		sy7758_err("write sy7758 backlight level lsb:0x%x failed\n",
			   bl_lsb);
	ret = regmap_write(sy7758_g_chip->regmap,
			   sy7758_bl_info.sy7758_level_msb, bl_msb);
	if (ret < 0)
			...
			const struct i2c_device_id *id)
{
	...
	pchip = devm_kzalloc(&client->dev, sizeof(struct sy7758_chip_data),
			     GFP_KERNEL);
	if (!pchip)
		return -ENOMEM;
	...
	if (ret < 0) {
		...
		sy7758_err("sy7758 read 0x%x success, val:%d",
	...
	ret = of_property_read_u32(np, "sy7758_level_lsb",
				   &sy7758_bl_info.sy7758_level_lsb);
	if (ret < 0) {
		sy7758_err("get sy7758_level_lsb failed\n");
		goto err_sysfs;
	}

	ret = of_property_read_u32(np, "sy7758_level_msb",
				   &sy7758_bl_info.sy7758_level_msb);
	if (ret < 0) {
