...
static int rt4831_parse_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, RT4831_ONLY_BACKLIGHT,
				   &g_bl_info.bl_only);
	if (ret < 0) {
		RT4831_INFO("No need to detect fault flags \n");
		g_bl_info.bl_only = 0;
...
static int rt4831_chip_init(struct rt4831_chip_data *pchip)
{
	...
	if (ret < 0) {
		...
		if (ret < 0)
			RT4831_INFO(
				"can not get rt4831_2_i2c_bus_id dts node\n");

		ret = of_property_read_u32(
			np, "rt4831_2_display_bias_config_1",
	...
	ret = of_property_read_u32(np, RT4831_HW_ENABLE,
				   &g_bl_info.rt4831_hw_en);
	if (ret < 0) {
		RT4831_ERR("get rt4831_hw_en dts config failed\n");
		g_bl_info.rt4831_hw_en = 0;
	}
	if (g_bl_info.rt4831_hw_en != 0) {
		ret = of_property_read_u32(np, RT4831_HW_EN_GPIO,
					   &g_bl_info.rt4831_hw_en_gpio);
		if (ret < 0) {
		...
		if (ret < 0) {
			RT4831_ERR(
				"get rt4831_hw_en_pull_low dts config fail\n");
			g_bl_info.rt4831_hw_en_pull_low = 0;
		}
	}

	ret = of_property_read_u32(np, RT4831_HW_EN_DELAY,
...
static int rt4831_set_bias_power_down(int vpos, int vneg)
{
	...
	ret = regmap_write(rt4831_g_chip->regmap, REG_DISPLAY_BIAS_CONFIG_1,
			   g_bl_info.rt4831_enable_vsp_vsn);
	if (ret < 0) {
	...
	ret = regmap_write(rt4831_g_chip->regmap, REG_LCM_BOOST_BIAS,
			   g_bl_info.rt4831_pull_down_boost);
	if (ret < 0) {
		RT4831_ERR("write enable_vsp_vsn failed!\n");
		return ret;
...
static int rt4831_set_bias(int vpos, int vneg)
{
	...
	if (g_bl_info.rt4831_hw_en != 0 &&
		...
		ret = gpio_direction_output(g_bl_info.rt4831_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
	...
	ret = rt4831_config_write(rt4831_g_chip, rt4831_reg_addr, rt4831_reg,
				  RT4831_RW_REG_MAX);
	if (ret < 0) {
...
static void rt4831_check_fault(struct rt4831_chip_data *pchip, int last_level,
			       int level)
{
	unsigned int val = 0;
	int ret;
...
static void backlight_ic_check_fault(struct rt4831_chip_data *pchip, int level)
{
...
static int rt4831_chip_enable(struct rt4831_chip_data *pchip)
{
	...
	if (g_bl_info.dual_ic) {
		ret = rt4831_2_config_write(pchip, rt4831_reg_addr, rt4831_reg,
					    RT4831_RW_REG_MAX);
		if (ret < 0) {
	...
	ret = rt4831_config_write(pchip, rt4831_reg_addr, rt4831_reg,
				  RT4831_RW_REG_MAX);
	if (ret < 0) {
...
int rt4831_set_backlight(uint32_t bl_level)
{
	...
	if (g_bl_info.dual_ic) {
		...
		ret = rt4831_2_config_write(rt4831_g_chip, &bl_addr, &bl_lsb,
					    1);
		if (ret < 0) {
		...
		bl_addr = REG_BL_BRIGHTNESS_MSB;
		ret = rt4831_2_config_write(rt4831_g_chip, &bl_addr, &bl_msb,
					    1);
		if (ret < 0) {
...
static DEVICE_ATTR(reg_bl, 0644, rt4831_reg_bl_show, rt4831_reg_bl_store);
...
static DEVICE_ATTR(reg, 0644, rt4831_reg_show, rt4831_reg_store);
...
static const struct i2c_device_id rt4831_id[] = {{RT4831_NAME, 0}, {}};
