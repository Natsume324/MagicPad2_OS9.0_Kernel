...
static int rt4539_parse_dts(struct device_node *np)
{
	...
	if (ret < 0) {
		...
		if (ret < 0)
			rt4539_info(
				"can not get rt4539_2_i2c_bus_id dts node\n");
	}
	...
	if (rt4539_bl_info.rt4539_hw_en != 0) {
		...
		if (rt4539_bl_info.dual_ic) {
			ret = of_property_read_u32(
				np, "rt4539_2_hw_en_gpio",
				&rt4539_bl_info.rt4539_2_hw_en_gpio);
			if (ret < 0) {
				rt4539_err(
					"get rt4539_2_hw_en_gpio dts config failed\n");
				return ret;
			}
			if (rt4539_bl_info.rt4539_hw_en_gpio ==
			    rt4539_bl_info.rt4539_2_hw_en_gpio) {
...
static int rt4539_2_read_u8(struct rt4539_chip_data *pchip, unsigned int reg,
			    unsigned int val[], unsigned int length)
{
	struct i2c_adapter *adap = NULL;
...
static int rt4539_config_write(struct rt4539_chip_data *pchip, unsigned int reg,
			       unsigned int val)
{
	int ret = 0;
	...
	if (ret < 0) {
		rt4539_err(
			"write rt4539 backlight config register 0x%x:0x%x failed\n",
			reg, val);
		return ret;
	}
			      ...
			      unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			rt4539_err(
				"read rt4539 backlight config register 0x%x failed",
				reg[i]);
			goto exit;
		} else {
...
static int rt4539_chip_init(struct rt4539_chip_data *pchip)
{
	...
	if (rt4539_bl_info.dual_ic) {
		ret = rt4539_2_config_write(pchip, RT4539_BL_CONTROL,
					    BL_DISABLE_CTL);
		if (ret < 0) {
			goto out;
		}
	...
	for (i = 0; i < RT4539_RW_REG_MAX; i++) {
		ret = rt4539_config_write(pchip, rt4539_reg_addr[i],
					  rt4539_bl_info.rt4539_reg[i]);
		if (ret < 0) {
			goto out;
		}
		if (rt4539_bl_info.dual_ic) {
			ret = rt4539_2_config_write(
				pchip, rt4539_reg_addr[i],
				rt4539_bl_info.rt4539_reg[i]);
			if (ret < 0) {
				goto out;
			}
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	ret = rt4539_config_read(pchip, rt4539_reg_addr,
				 rt4539_bl_info.rt4539_reg, RT4539_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(
		...
		rt4539_bl_info.rt4539_reg[0], rt4539_bl_info.rt4539_reg[1],
		rt4539_bl_info.rt4539_reg[2], rt4539_bl_info.rt4539_reg[3],
		rt4539_bl_info.rt4539_reg[4], rt4539_bl_info.rt4539_reg[5],
		rt4539_bl_info.rt4539_reg[6], rt4539_bl_info.rt4539_reg[7],
		rt4539_bl_info.rt4539_reg[8], rt4539_bl_info.rt4539_reg[9],
		rt4539_bl_info.rt4539_reg[10], rt4539_bl_info.rt4539_reg[11]);
	return ret;
...
static void rt4539_enable(void)
{
	...
	if (rt4539_bl_info.rt4539_hw_en) {
		...
		ret = gpio_direction_output(rt4539_bl_info.rt4539_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
		...
		if (rt4539_bl_info.dual_ic &&
			...
			ret = gpio_request(rt4539_bl_info.rt4539_2_hw_en_gpio,
					   NULL);
			if (ret)
				rt4539_err(
					"rt4539 Could not request  hw_en2_gpio\n");
			ret = gpio_direction_output(
				rt4539_bl_info.rt4539_2_hw_en_gpio,
				GPIO_DIR_OUT);
			if (ret)
				rt4539_err(
					"rt4539 set gpio output not success\n");
			gpio_set_value(rt4539_bl_info.rt4539_2_hw_en_gpio,
				       GPIO_OUT_ONE);
		}
...
static int rt4539_set_backlight(uint32_t bl_level)
{
	...
	if (false == rt4539_init_status) {
		rt4539_info(
			"rt4539 is disabled, can not set backlight, bl_level = %d\n",
			bl_level);
	...
	ret = regmap_write(rt4539_g_chip->regmap,
			   rt4539_bl_info.rt4539_level_msb, bl_msb);
	if (ret < 0) {
	...
	ret = regmap_write(rt4539_g_chip->regmap,
			   rt4539_bl_info.rt4539_level_lsb, bl_lsb);
	if (ret < 0) {
			...
			const struct i2c_device_id *id)
{
	...
	pchip = devm_kzalloc(&client->dev, sizeof(struct rt4539_chip_data),
			     GFP_KERNEL);
	if (!pchip)
		return -ENOMEM;
	...
	ret = of_property_read_u32(np, "rt4539_level_lsb",
				   &rt4539_bl_info.rt4539_level_lsb);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "rt4539_level_msb",
				   &rt4539_bl_info.rt4539_level_msb);
	if (ret < 0) {
