				  ...
				  unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			ktz8864_info(
				"ktz8864_2_config_write reg=0x%x,val=0x%x\n",
				buf[0], buf[1]);
		}
	}
out:
				...
				unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			ktz8864_err(
				"write ktz8864 backlight config register 0x%x failed\n",
				reg[i]);
			goto exit;
		} else {
...
static int ktz8864_chip_init(struct ktz8864_chip_data *pchip)
{
	...
	ret = of_property_read_u32(np, KTZ8864_ONLY_BACKLIGHT,
				   &bl_info.bl_only);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		if (ret < 0)
			ktz8864_info(
				"can not get ktz8864_2_i2c_bus_id dts node\n");

		ret = of_property_read_u32(
			np, "ktz8864_2_display_bias_config_1",
	...
	ret = of_property_read_u32(np, KTZ8864_HW_ENABLE,
				   &bl_info.ktz8864_hw_en);
	if (ret < 0) {
		ktz8864_debug("get ktz8864_hw_en dts config failed\n");
		bl_info.ktz8864_hw_en = 0;
	}
	if (bl_info.ktz8864_hw_en != 0) {
		ret = of_property_read_u32(np, KTZ8864_HW_EN_GPIO,
					   &bl_info.ktz8864_hw_en_gpio);
		if (ret < 0) {
			ktz8864_debug(
				"get ktz8864_hw_en_gpio dts config fail\n");
		} else {
			/* gpio number offset */
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
		...
		if (ret < 0) {
			ktz8864_err(
				"get ktz8864_hw_en_pull_low dts config fail\n");
			bl_info.ktz8864_hw_en_pull_low = 0;
		}
	}

	ret = of_property_read_u32(np, KTZ8864_HW_EN_DELAY,
...
static int ktz8864_set_ic_disable(void)
{
	...
	if (bl_info.ktz8864_hw_en != 0 && bl_info.ktz8864_hw_en_pull_low != 0) {
...
static int ktz8864_set_bias_power_down(int vpos, int vneg)
{
	...
	ret = regmap_write(ktz8864_g_chip->regmap, REG_DISPLAY_BIAS_CONFIG_1,
			   bl_info.ktz8864_enable_vsp_vsn);
	if (ret < 0) {
	...
	ret = regmap_write(ktz8864_g_chip->regmap, REG_LCM_BOOST_BIAS,
			   bl_info.ktz8864_pull_down_boost);
	if (ret < 0) {
		ktz8864_err("write enable_vsp_vsn failed!\n");
		return ret;
...
static int ktz8864_set_bias(int vpos, int vneg)
{
	...
	if (bl_info.ktz8864_hw_en != 0 && bl_info.ktz8864_hw_en_pull_low != 0) {
		...
		ret = gpio_direction_output(bl_info.ktz8864_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
...
static void ktz8864_check_fault(struct ktz8864_chip_data *pchip, int last_level,
				int level)
{
	unsigned int val = 0;
	int ret;
...
static void backlight_ic_check_fault(struct ktz8864_chip_data *pchip, int level)
{
...
static int ktz8864_chip_enable(struct ktz8864_chip_data *pchip)
{
	...
	ret = ktz8864_config_write(pchip, ktz8864_reg_addr, ktz8864_reg,
				   KTZ8864_RW_REG_MAX);
	if (ret < 0) {
...
int ktz8864_set_backlight_reg(uint32_t bl_level)
{
	...
	if (bl_info.dual_ic) {
		...
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr, &bl_lsb,
					     1);
		if (ret < 0) {
		...
		bl_addr = REG_BL_BRIGHTNESS_MSB;
		ret = ktz8864_2_config_write(ktz8864_g_chip, &bl_addr, &bl_msb,
					     1);
		if (ret < 0) {
...
static const struct i2c_device_id ktz8864_id[] = {{KTZ8864_NAME, 0}, {}};
