				  ...
				  unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != 0xffff) {
			...
			lm36274_info(
				"lm36274_2_config_write reg=0x%x,val=0x%x\n",
				buf[0], buf[1]);
		}
	}
out:
				...
				unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (ret < 0) {
			lm36274_err(
				"write lm36274 backlight config register 0x%x failed\n",
				reg[i]);
			goto exit;
		} else {
...
static int lm36274_parse_dts(struct device_node *np)
{
	...
	for (i = 0; i < LM36274_RW_REG_MAX; i++) {
		ret = of_property_read_u32(np, lm36274_dts_string[i],
					   &lm36274_reg[i]);
		if (ret < 0) {
			lm36274_err("get lm36274 dts config failed\n");
		} else {
			lm36274_info("get %s from dts value = 0x%x\n",
				     lm36274_dts_string[i], lm36274_reg[i]);
		}
	}
	ret = of_property_read_u32(np, "lm36274_check_fault_support",
				   &lm36274_fault_check_support);
	if (ret < 0)
	...
	ret = of_property_read_u32(np, LM36274_ONLY_BACKLIGHT,
				   &bl_info.bl_only);
	if (ret < 0) {
	...
	if (ret < 0) {
		...
		if (ret < 0)
			lm36274_info(
				"can not get lm36274_2_i2c_bus_id dts node\n");

		ret = of_property_read_u32(
			np, "lm36274_2_display_bias_config_1",
	...
	ret = of_property_read_u32(np, LM36274_HW_ENABLE,
				   &bl_info.lm36274_hw_en);
	if (ret < 0) {
		lm36274_err("get lm36274_hw_en dts config failed\n");
		bl_info.lm36274_hw_en = 0;
	}
	if (bl_info.lm36274_hw_en != 0) {
		ret = of_property_read_u32(np, LM36274_HW_EN_GPIO,
					   &bl_info.lm36274_hw_en_gpio);
		if (ret < 0) {
		...
		if (ret < 0) {
			lm36274_err(
				"get lm36274_hw_en_pull_low dts config fail\n");
			bl_info.lm36274_hw_en_pull_low = 0;
		}
	}

	ret = of_property_read_u32(np, LM36274_HW_EN_DELAY,
				   &bl_info.bl_on_kernel_mdelay);
	if (ret < 0)
...
static void lm36274_get_target_voltage(int *vpos, int *vneg)
{
	int i;
	int tal_size =
		sizeof(lm36274_vol_table) / sizeof(struct lm36274_voltage);
...
static void lm36274_check_fault(struct lm36274_chip_data *pchip, int last_level,
				int level)
{
	unsigned int val = 0;
	int ret;
...
static void backlight_ic_check_fault(struct lm36274_chip_data *pchip, int level)
{
	...
	if (lm36274_fault_check_support &&
	    ((last_level <= 0 && level != 0) || (last_level > 0 && level == 0)))
...
static int lm36274_chip_enable(struct lm36274_chip_data *pchip)
{
	...
	if (bl_info.dual_ic) {
		ret = lm36274_2_config_write(pchip, lm36274_reg_addr,
	...
	ret = lm36274_config_write(pchip, lm36274_reg_addr, lm36274_reg,
				   LM36274_RW_REG_MAX);
	if (ret < 0) {
...
static void lm36274_enable(void)
{
	...
	if (bl_info.lm36274_hw_en != 0 && bl_info.lm36274_hw_en_pull_low != 0) {
		ret = gpio_request(bl_info.lm36274_hw_en_gpio, NULL);
		if (ret)
			lm36274_err("lm36274 Could not request  hw_en_gpio\n");
		ret = gpio_direction_output(bl_info.lm36274_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
...
int lm36274_set_backlight(unsigned int bl_level)
{
	...
	if (bl_info.dual_ic) {
		...
		ret = lm36274_2_config_write(lm36274_g_chip, &bl_addr, &bl_lsb,
					     1);
		if (ret < 0) {
		...
		ret = lm36274_2_config_write(lm36274_g_chip, &bl_addr, &bl_msb,
					     1);
		if (ret < 0) {
...
static DEVICE_ATTR(reg_bl, (S_IRUGO | S_IWUSR), lm36274_reg_bl_show,
		   lm36274_reg_bl_store);
...
static DEVICE_ATTR(reg, (S_IRUGO | S_IWUSR), lm36274_reg_show,
		   lm36274_reg_store);
			 ...
			 const struct i2c_device_id *id)
{
	...
	pchip = devm_kzalloc(&client->dev, sizeof(struct lm36274_chip_data),
			     GFP_KERNEL);
	if (!pchip)
		return -ENOMEM;
...
static const struct i2c_device_id lm36274_id[] = {{LM36274_NAME, 0}, {}};
