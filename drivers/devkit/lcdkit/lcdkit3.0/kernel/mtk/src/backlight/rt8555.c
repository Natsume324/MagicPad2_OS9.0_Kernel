...
static unsigned int g_reg_addr[RT8555_RW_REG_MAX] = {
	RT8555_CONTROL_MODE_ADDR,	RT8555_CURRENT_PROTECTION_ADDR,
	RT8555_CURRENT_SETTING_ADDR,	RT8555_VOLTAGE_SETTING_ADDR,
	RT8555_BRIGHTNESS_SETTING_ADDR, RT8555_TIME_CONTROL_ADDR,
	RT8555_MODE_DEVISION_ADDR,	RT8555_COMPENSATION_DUTY_ADDR,
	RT8555_CLK_PFM_ENABLE_ADDR,	RT8555_LED_PROTECTION_ADDR,
...
static int rt8555_parse_dts(struct device_node *np)
{
	...
	for (i = 0; i < RT8555_RW_REG_MAX; i++) {
		ret = of_property_read_u32(np, g_dts_str[i], &g_bl_info.reg[i]);
		if (ret < 0) {
	...
	if (ret < 0) {
		...
		if (ret < 0)
			rt8555_info(
				"can not get rt8555_2_i2c_bus_id dts node\n");
	}
	ret = of_property_read_u32(np, "rt8555_hw_enable",
				   &g_bl_info.rt8555_hw_en);
	if (ret < 0) {
	...
	ret = of_property_read_u32(np, "bl_led_num", &g_bl_info.bl_led_num);
	if (ret < 0) {
...
static int rt8555_chip_init(struct rt8555_chip_data *pchip)
{
	...
	rt8555_info("in\n");
	if (g_bl_info.dual_ic) {
		ret = rt8555_2_config_write(pchip, g_reg_addr, g_bl_info.reg,
					    RT8555_RW_REG_MAX);
		if (ret < 0) {
	...
	ret = rt8555_config_write(pchip, g_reg_addr, g_bl_info.reg,
				  RT8555_RW_REG_MAX);
	if (ret < 0) {
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	ret = rt8555_config_read(pchip, g_reg_addr, g_bl_info.reg,
				 RT8555_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(buf, PAGE_SIZE,
		       ...
		       "\rConfig_reg_50 = 0x%x\n",
		       g_bl_info.reg[0], g_bl_info.reg[1], g_bl_info.reg[2],
		       g_bl_info.reg[3], g_bl_info.reg[4], g_bl_info.reg[5],
		       g_bl_info.reg[6], g_bl_info.reg[7], g_bl_info.reg[8],
		       g_bl_info.reg[9], g_bl_info.reg[10]);
	return ret;
i2c_error:
...
static void rt8555_enable(void)
{
	...
	if (g_bl_info.rt8555_hw_en) {
		...
		ret = gpio_direction_output(g_bl_info.rt8555_hw_en_gpio,
					    GPIO_DIR_OUT);
		if (ret)
		...
		if (g_bl_info.dual_ic) {
			...
			if (ret)
				rt8555_err(
					"rt8555 Could not request  rt8555_2_hw_en_gpio\n");
			ret = gpio_direction_output(
				g_bl_info.rt8555_2_hw_en_gpio, GPIO_DIR_OUT);
			if (ret)
				rt8555_err(
					"rt8555 set gpio output not success\n");
			gpio_set_value(g_bl_info.rt8555_2_hw_en_gpio,
				       GPIO_OUT_ONE);
		}
	}
...
static int rt8555_en_backlight(uint32_t bl_level)
{
	...
	if ((g_bl_info.bl_set_long_slope) && (bl_set_slope_status == true)) {
		...
		if (g_bl_info.dual_ic) {
			...
			ret = rt8555_2_config_write(
				g_rtdev, &slope_addr,
				&g_bl_info.bl_set_long_slope, 1);
			if (ret < 0)
			...
			const struct i2c_device_id *id)
{
	...
	pchip = devm_kzalloc(&client->dev, sizeof(struct rt8555_chip_data),
			     GFP_KERNEL);
	if (!pchip)
		return -ENOMEM;
	...
	ret = regmap_read(pchip->regmap, g_reg_addr[0], &g_bl_info.reg[0]);
	if (ret < 0) {
