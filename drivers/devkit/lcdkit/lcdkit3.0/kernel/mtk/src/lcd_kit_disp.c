...
static void lcm_get_esd_config(struct LCM_PARAMS *params)
{
	...
	params->dsi.esd_recovery_bl_enable =
		common_info->esd.recovery_bl_support;
	params->dsi.esd_gpio_detect_dual_enable =
		common_info->esd.gpio_detect_dual_support;
	if (common_info->esd.cmds.cmds) {
		esd_cmds = common_info->esd.cmds.cmds;
		for (i = 0; i < common_info->esd.cmds.cmd_cnt; i++) {
			params->dsi.lcm_esd_check_table[i].cmd =
				esd_cmds->payload[0];
			params->dsi.lcm_esd_check_table[i].count =
				esd_cmds->dlen;
			...
			for (j = 0; j < esd_cmds->dlen; j++) {
				...
				LCD_KIT_INFO(
					"dsi.lcm_esd_check_table[%d].para_list[%d] = %d\n",
					i, j,
					params->dsi.lcm_esd_check_table[i]
						.para_list[j]);
				k++;
			}
			esd_cmds++;
...
static void lcm_aod(int enter)
{
	...
	if (enter) {
		...
		if (disp_info->alpm_state == ALPM_START)
			lcd_kit_aod_cmd_tx(&disp_info->alpm.high_light_cmds);
...
{
	bool old = hbm_en;
	struct hbm_type_cfg hbm_source = {HBM_FOR_FP, NULL, NULL, qhandle};
