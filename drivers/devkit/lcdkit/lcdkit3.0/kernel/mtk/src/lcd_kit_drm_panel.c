			    ...
			    struct drm_file *file_priv)
{
	...
	param = (struct display_engine_ddic_rgbw_param *)data;
	param->ddic_panel_id =
		g_lcd_kit_disp_info.ddic_rgbw_param.ddic_panel_id;
	param->ddic_rgbw_mode =
		g_lcd_kit_disp_info.ddic_rgbw_param.ddic_rgbw_mode;
	param->ddic_rgbw_backlight =
		g_lcd_kit_disp_info.ddic_rgbw_param.ddic_rgbw_backlight;
	param->pixel_gain_limit =
		g_lcd_kit_disp_info.ddic_rgbw_param.pixel_gain_limit;
				      ...
				      struct drm_file *file_priv)
{
	...
	if (common_info && common_info->sn_code.support) {
		...
		memcpy(param->sn_code, lcd_kit_info.sn_code,
		       lcd_kit_info.sn_code_length);
	}
...
static int32_t poweric_cmd_detect(uint8_t *out, int out_len)
{
	...
	ret = lcd_kit_dsi_cmds_extern_rx(
		&read_value, &disp_info->elvdd_detect.cmds, out_len);
	if (ret) {
	...
	*out = read_value;
	...
	LCD_KIT_INFO("read_value = 0x%x, exp_value = 0x%x, mask = 0x%x\n",
		     read_value, disp_info->elvdd_detect.exp_value,
...
static void report_poweric_err(uint8_t cmd_val, int32_t gpio_val)
{
	...
	(void)lcd_dsm_client_record(lcd_dclient, record_buf,
				    DSM_LCD_STATUS_ERROR_NO, REC_DMD_NO_LIMIT,
				    &recordtime);
#endif
}
...
static bool lcd_kit_first_screenon(uint32_t last_bl_level, uint32_t bl_level)
{
	...
	if (last_bl_level == 0 && bl_level != 0) {
		...
		LCD_KIT_INFO("first_screenoff, last_bl_level = %d",
			     last_bl_level);
		ret = false;
	} else {
		ret = false;
			     ...
			     struct lcd_kit_array_data *timing)
{
	...
	mode_list[index].vtotal = pinfo->yres + timing->buf[FPS_VFP_INDEX] +
				  timing->buf[FPS_VS_INDEX] +
				  timing->buf[FPS_VBP_INDEX];

	mode_list[index].vrefresh = timing->buf[FPS_VRE_INDEX];
	mode_list[index].clock = mode_list[index].htotal *
				 mode_list[index].vtotal *
...
static void lcm_init_drm_mode(void)
{
	...
	if (disp_info->fps.support) {
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt;
		     i++) {
		...
		default_mode.hsync_end = pinfo->xres +
		...
		default_mode.vsync_start =
			pinfo->yres + pinfo->ldi.v_front_porch;
		default_mode.vsync_end = pinfo->yres +
			 ...
			 struct drm_connector *connector)
{
	...
	if (disp_info->fps.support) {
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt;
		     i++) {
			index = disp_info->fps.panel_support_fps_list.buf[i];
			support_mode = drm_mode_duplicate(panel->drm,
							  &mode_list[index]);
			if (!support_mode) {
				dev_err(panel->drm->dev,
					"failed to add mode %ux%ux:%u\n",
					mode_list[index].hdisplay,
					mode_list[index].vdisplay,
				 ...
				 unsigned int level)
{
	...
	if (common_info->backlight.need_prepare) {
		for (i = 0; i < common_info->backlight.prepare.cmd_cnt; i++) {
			cb(dsi, handle,
			   common_info->backlight.prepare.cmds[i].payload,
	...
	if (ret < 0)
		pr_err("%s: fail\n", __func__);
	else if ((strcmp("als_s001_002", sensor_name.name) == 0) ||
		 (strcmp("tcs3701_l", sensor_name.name) == 0))
		save_light_to_sensorhub(level, level);
#endif
	return LCD_KIT_OK;
}
				   ...
				   int lcd_fps_scence)
{
	...
	if (stage == AFTER_DSI_POWERON) {
		...
		if (ret != LCD_KIT_OK)
			LCD_KIT_ERR(
				"change to lcd_fps_scence : %d mipi tx fail\n",
				lcd_fps_scence);
		else
			LCD_KIT_INFO(
				"change to lcd_fps_scence : %d successfully\n",
				lcd_fps_scence);
	}
}
...
static void lcd_kit_fphbm_entry_delay(void)
{
	...
	delay_threshold =
		real_te_interval * disp_info->hbm_entry_delay / TE_60_TIME;
	...
	te_time = real_te_interval * disp_info->te_interval_us / TE_60_TIME;
		       ...
		       struct lcd_kit_dsi_panel_cmds *pcmds)
{
	if (hbm_source.source == HBM_FOR_FP)
		(void)cb_tx(hbm_source.dsi, hbm_source.cb, hbm_source.handle,
			    pcmds);
	else
...
static void lcd_kit_fill_hbm_cmd(unsigned int level)
{
	if (common_info->hbm.hbm_special_bit_ctrl ==
		...
		common_info->hbm.hbm_cmds.cmds[0].payload[1] = (level >> 8) &
							       0xf;
...
static void lcd_kit_restore_hbm_level(struct hbm_type_cfg hbm_source)
{
	...
	lcd_kit_hbm_set_func_by_level(common_info->hbm.hbm_level_current,
...
static void lcd_kit_hbm_enter(int level, struct hbm_type_cfg hbm_source)
{
	...
	if (common_info->hbm.hbm_fp_support) {
		...
		lcd_kit_hbm_set_func_by_level(hbm_level, LCD_KIT_FP_HBM_ENTER,
					      hbm_source);
		mutex_unlock(&COMMON_LOCK->hbm_lock);
	} else {
...
static int lcd_kit_local_hbm_identify_off_set(int level)
{
	...
	if (level == HBM_EXIT) {
		ret = lcd_kit_dsi_cmds_tx_compact(
			&common_info->lhbm.disable_cmds, true);
		if (ret)
			ret = LCD_SUSPEND;
		else {
			...
			case LCD_KIT_FPS_24:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(
					&disp_info->fps
						 .fps_to_cmds[LCD_FPS_SCENCE_24]);
				break;
			case LCD_KIT_FPS_25:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(
					&disp_info->fps
						 .fps_to_cmds[LCD_FPS_SCENCE_25]);
				break;
			case LCD_KIT_FPS_30:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(
					&disp_info->fps
						 .fps_to_cmds[LCD_FPS_SCENCE_30]);
				break;
			case LCD_KIT_FPS_60:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(
					&disp_info->fps.fps_to_cmds
						 [LCD_FPS_SCENCE_H60]);
				break;
			case LCD_KIT_FPS_90:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(
					&disp_info->fps
						 .fps_to_cmds[LCD_FPS_SCENCE_90]);
				break;
			case LCD_KIT_FPS_120:
				ret = lcd_kit_dsi_cmds_extern_tx_nolock(
					&disp_info->fps.fps_to_cmds
	...
	ret = lcd_kit_dsi_cmds_tx_compact(&common_info->lhbm.off_enable_cmds,
					  true);
...
static int lcd_kit_local_hbm_identify_on_set(int level)
{
	...
	ret = lcd_kit_dsi_cmds_tx_compact(&common_info->lhbm.on_enable_cmds,
					  true);
	disp_info->lhbm_enter = true;
	ud_fp_on_hbm_completed();
	return ret;
				 ...
				 struct drm_file *file_priv)
{
	...
	share_mem_phy = get_als_reserve_mem_phys();
#endif
	((struct display_engine_share_memory *)data)->addr_virt =
		(uint64_t)share_mem_virt;
	((struct display_engine_share_memory *)data)->addr_phy =
		(uint64_t)share_mem_phy;
...
void lcdkit_cwb_get_buffer(void **buffer)
{
	...
	if (g_lcdkit_cwb_buffer == 0) {
		g_lcdkit_cwb_buffer =
			kzalloc(sizeof(u8) * SHARE_MEMORY_SIZE, GFP_KERNEL);
				 ...
				 struct drm_file *file_priv)
{
	...
	LCD_KIT_INFO(
		"[shmmap] x %d, y %d, width %d, high %d, interval %d, en %d\n",
		share_memory_param->x, share_memory_param->y,
		share_memory_param->width, share_memory_param->height,
		share_memory_param->interval, share_memory_param->enable);
	pinfo = &lcd_kit_info;
	if (((share_memory_param->x + share_memory_param->width) >
	     pinfo->xres) ||
	    ((share_memory_param->y + share_memory_param->height) >
	     pinfo->yres)) {
	...
	if ((share_memory_param->width > SENSOR_MAX_SIDE_LENGTH) ||
		...
		LCD_KIT_ERR(
			"[shmmap] width or height over sensor size, width %d, height %d\n",
			share_memory_param->width, share_memory_param->height);
		return LCD_KIT_FAIL;
	}
	...
	if (setCaptureRect(share_memory_param->x, share_memory_param->y,
			   share_memory_param->width,
			   share_memory_param->height) == false) {
					...
					struct drm_file *file_priv)
{
	...
	wait_ret = wait_event_interruptible_timeout(
		share_mem_wq_head, share_mem_is_ready, timeout);
	if (wait_ret == 0) {
		LCD_KIT_DEBUG("[shmmap] timeout to get share_memory\n");
		return 1;
	}
	((struct display_engine_share_memory_status *)data)->frame_index =
		share_mem_frame_index;
	((struct display_engine_share_memory_status *)data)->buffer_index =
		share_mem_buffer_index;
	share_mem_is_ready = false;
	LCD_KIT_DEBUG(
		"[shmmap] success frame_index %d, share_mem_buffer_index %d!\n",
...
static void lcd_kit_hbm_set_level(int level)
{
	...
	if (common_info->hbm.hbm_cmds.cmds != NULL) {
		if (common_info->hbm.hbm_special_bit_ctrl ==
			...
			common_info->hbm.hbm_cmds.cmds[0].payload[1] =
				(level >> 8) & 0xf;
			common_info->hbm.hbm_cmds.cmds[0].payload[2] = level &
								       0xff;
		}
		(void)lcd_kit_dsi_cmds_extern_tx_nolock(
...
static int lcd_kit_hbm_set_handle(int last_hbm_level, int hbm_dimming,
				  int hbm_level)
{
	int ret = LCD_KIT_OK;
			      ...
			      dcs_write_gce cb, void *handle, bool en)
{
	int ret;
	struct lcm *ctx = panel_to_lcm(panel);
	struct hbm_type_cfg hbm_source = {HBM_FOR_FP, dsi, (void *)cb, handle};
...
static int panel_doze_enable_start(struct drm_panel *panel, void *dsi_drv,
...
static int panel_doze_enable(struct drm_panel *panel, void *dsi_drv,
			     dcs_write_gce cb, void *handle)
{
	LCD_KIT_INFO(" in\n");
	return LCD_KIT_OK;
...
static int panel_doze_post_disp_on(struct drm_panel *panel, void *dsi_drv,
				   dcs_write_gce cb, void *handle)
{
	LCD_KIT_INFO(" in\n");
	if (disp_info->alpm_state == ALPM_START)
...
static int panel_doze_area(struct drm_panel *panel, void *dsi_drv,
			   dcs_write_gce cb, void *handle)
{
	LCD_KIT_INFO(" in\n");
	return LCD_KIT_OK;
				    ...
				    struct lcd_kit_dsi_panel_cmds *cmds)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		fps_cmd = &cmds->cmds[i];
		if (fps_cmd->dlen == 0 || fps_cmd->payload == NULL ||
	...
	for (i = 0; i < common_info->esd.cmds.cmd_cnt; i++) {
		if (esd_cmds->dtype == 0x06) {
			...
			panel_params->lcm_esd_check_table[checknum].count =
				esd_cmds->dlen;
			panel_params->lcm_esd_check_table[checknum]
				.mask_list[0] =
				(common_info->esd.value.buf[read_reg] >> 8) &
				0xFF;
			panel_params->lcm_esd_check_table[checknum]
				.para_list[0] =
				common_info->esd.value.buf[read_reg] & 0xFF;
			LCD_KIT_INFO(
				"dsi.lcm_esd_check_table[%d].cmd = 0x%x, count = %d, para_list[0] = 0x%x, mask_list[0] = 0x%x\n",
				checknum,
				panel_params->lcm_esd_check_table[checknum].cmd,
				panel_params->lcm_esd_check_table[checknum]
					.count,
			...
			panel_params->lcm_esd_check_table[checknum].count =
				esd_cmds->dlen;
			LCD_KIT_INFO(
				"dsi.lcm_esd_check_table[%d].cmd = 0x%x, count = %d\n",
				checknum,
				panel_params->lcm_esd_check_table[checknum].cmd,
				panel_params->lcm_esd_check_table[checknum]
					.count);
			memcpy(panel_params->lcm_esd_check_table[checknum]
				       .para_list,
...
{
	if (pext_params == NULL || pmipi_hopping == NULL) {
...
static void lcm_set_dsc_params(struct mtk_panel_params *panel_params)
{
	...
	panel_params->dsc_params.bit_per_channel =
		disp_info->dsc_params.bit_per_channel;
	panel_params->dsc_params.dsc_line_buf_depth =
		disp_info->dsc_params.dsc_line_buf_depth;
	panel_params->dsc_params.bp_enable = disp_info->dsc_params.bp_enable;
	panel_params->dsc_params.bit_per_pixel =
		disp_info->dsc_params.bit_per_pixel;
	...
	panel_params->dsc_params.slice_height =
		disp_info->dsc_params.slice_height;
	panel_params->dsc_params.slice_width =
		disp_info->dsc_params.slice_width;
	...
	panel_params->dsc_params.scale_value =
		disp_info->dsc_params.scale_value;
	panel_params->dsc_params.increment_interval =
		disp_info->dsc_params.increment_interval;
	panel_params->dsc_params.decrement_interval =
		disp_info->dsc_params.decrement_interval;
	panel_params->dsc_params.line_bpg_offset =
		disp_info->dsc_params.line_bpg_offset;
	panel_params->dsc_params.nfl_bpg_offset =
		disp_info->dsc_params.nfl_bpg_offset;
	panel_params->dsc_params.slice_bpg_offset =
		disp_info->dsc_params.slice_bpg_offset;
	panel_params->dsc_params.initial_offset =
		disp_info->dsc_params.initial_offset;
	panel_params->dsc_params.final_offset =
		disp_info->dsc_params.final_offset;
	panel_params->dsc_params.flatness_minqp =
		disp_info->dsc_params.flatness_minqp;
	panel_params->dsc_params.flatness_maxqp =
		disp_info->dsc_params.flatness_maxqp;
	panel_params->dsc_params.rc_model_size =
		disp_info->dsc_params.rc_model_size;
	panel_params->dsc_params.rc_edge_factor =
		disp_info->dsc_params.rc_edge_factor;
	panel_params->dsc_params.rc_quant_incr_limit0 =
		disp_info->dsc_params.rc_quant_incr_limit0;
	panel_params->dsc_params.rc_quant_incr_limit1 =
		disp_info->dsc_params.rc_quant_incr_limit1;
	panel_params->dsc_params.rc_tgt_offset_hi =
		disp_info->dsc_params.rc_tgt_offset_hi;
	panel_params->dsc_params.rc_tgt_offset_lo =
		disp_info->dsc_params.rc_tgt_offset_lo;
}
					...
					struct mtk_panel_info *pinfo)
{
	...
	if (pinfo->round_corner.round_corner_en) {
		panel_params->round_corner_en =
			pinfo->round_corner.round_corner_en;
		panel_params->corner_pattern_height = pinfo->round_corner.h;
		panel_params->corner_pattern_height_bot =
			pinfo->round_corner.h_bot;
		panel_params->corner_pattern_tp_size =
			pinfo->round_corner.tp_size;
		panel_params->corner_pattern_lt_addr =
			pinfo->round_corner.lt_addr;
		...
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt;
			...
			if (timing == NULL || timing->buf == NULL) {
				...
				if (pinfo->data_rate_khz)
					ext_params_list[index].data_rate_khz =
						timing->buf[FPS_RATE_INDEX];
				else
					ext_params_list[index].data_rate =
						timing->buf[FPS_RATE_INDEX];

				ext_params_list[index].pll_clk =
					timing->buf[FPS_RATE_INDEX] / 2;
				ext_params_list[index].vfp_low_power =
					timing->buf[FPS_LOWER_INDEX];
				/* MTK_PANEL_OUTPUT_MODE */
				ext_params_list[index].output_mode =
					pinfo->output_mode;
				/* lcm_cmd_if is used to distinguish for Panel Cmd single or dual Port */
				ext_params_list[index].lcm_cmd_if =
					pinfo->lcm_cmd_if;
				/* 0: dphy 1: cphy */
				ext_params_list[index].is_cphy =
					pinfo->mipi.phy_mode;
				/* change mm to um */
				ext_params_list[index].physical_width_um =
					pinfo->width * 1000;
				/* change mm to um */
				ext_params_list[index].physical_height_um =
					pinfo->height * 1000;
				...
				/* set hopping */
				ext_params_list[index].dyn.switch_en =
					disp_info->fps.hop_support;
				if (disp_info->fps.hop_support) {
					hop_info =
						&disp_info->fps.hop_info[index];
					/* 0-7 is HFP-HBP-HS-VFP-VBP-VS-RATE-LOWER */
					ext_params_list[index].dyn.hfp =
						hop_info->buf[0];
					ext_params_list[index].dyn.hbp =
						hop_info->buf[1];
					ext_params_list[index].dyn.hsa =
						hop_info->buf[2];
					ext_params_list[index].dyn.vfp =
						hop_info->buf[3];
					ext_params_list[index].dyn.vbp =
						hop_info->buf[4];
					...
					if (pinfo->data_rate_khz)
						...
						ext_params_list[index]
							.dyn.data_rate =
							hop_info->buf[6];
					ext_params_list[index].dyn.vfp_lp_dyn =
						hop_info->buf[7];
					LCD_KIT_INFO(
						"%d %d %d %d %d %d",
						ext_params_list[index].dyn.hfp,
				...
				lcm_set_round_corner_params(
					&ext_params_list[index], pinfo);
#endif
				ext_params_list[index].lcm_color_mode =
					disp_info->lcm_color;
				ext_params_list[index].te_high_cycle =
					disp_info->mtk_high_te_inerval_time_us;
				ext_params_list[index].vsync_high_time =
					disp_info->mtk_high_left_time_th_us;
				ext_params_list[index].hbm_high_deadline =
					disp_info->mtk_high_right_time_th_us;
				ext_params_list[index].te_low_cycle =
					disp_info->mtk_low_te_inerval_time_us;
				ext_params_list[index].vsync_low_time =
					disp_info->mtk_low_left_time_th_us;
				ext_params_list[index].hbm_low_deadline =
					disp_info->mtk_low_right_time_th_us;
				/* phy_timcon */
				if (timing->cnt > FPS_DA_HS_EXIT)
					ext_params_list[index]
						.phy_timcon.da_hs_exit =
						timing->buf[FPS_DA_HS_EXIT];
				lcm_phy_time_ext_config(
					&ext_params_list[index]);
				lcd_dyn_fps_cmds_config(
					&ext_params_list[index],
...
static void lcd_kit_keep_ldo_on(struct device *dev)
{
	...
	if (of_property_read_bool(dev->of_node, "lcd_vci-supply")) {
		...
		if (!IS_ERR_OR_NULL(ldo_reg)) {
			...
			if (ret < 0)
				LCD_KIT_ERR(
					"regulator_enable lcd_vci-supply failed\n");
		}
	}
	if (of_property_read_bool(dev->of_node, "lcd_iovcc-supply")) {
		...
		if (!IS_ERR_OR_NULL(ldo_reg)) {
			...
			if (ret < 0)
				LCD_KIT_ERR(
					"regulator_enable lcd_iovcc-supply failed\n");
		}
	}
	if (of_property_read_bool(dev->of_node, "lcd_vdd-supply")) {
		...
		if (!IS_ERR_OR_NULL(ldo_reg)) {
			...
			if (ret < 0)
				LCD_KIT_ERR(
					"regulator_enable lcd_vdd-supply failed\n");
		}
	}
}
...
int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (disp_info->fps.support) {
		...
		ret = mtk_panel_ext_create(dev, &ext_params, &g_ext_funcs,
					   &ctx->panel);
		if (ret < 0)
			return ret;
	}
