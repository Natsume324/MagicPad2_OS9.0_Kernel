...
static void lcd_kit_delay_mod_aod_light(uint32_t mode)
{
	...
	diff_ms = (ktime_to_us(cur_timestamp) -
		   ktime_to_us(disp_info->alpm_start_time)) /
		  1000;
...
int lcd_kit_alpm_setting(uint32_t mode)
{
	...
	case ALPM_ON_HIGH_LIGHT:
		...
		if (disp_info->alpm_state == ALPM_IN) {
			...
			if (!atomic_read(&panel_info->aod_update_light)) {
				LCD_KIT_INFO(
					"cannot update light for not atomic update!\n");
				break;
			}
#endif
		}
	...
	case ALPM_SINGLE_CLOCK:
		...
		for (i = 0; i < num; i++)
			copy_dsi_cmd_desc(
				&(disp_info->alpm.off_cmds.cmds[i]),
	...
	case ALPM_DOUBLE_CLOCK:
		...
		if (panel_ops && panel_ops->aod_set_area)
			panel_ops->aod_set_area((mode & AOD_V_START_MASK) >>
						16);
...
void lcd_kit_pinfo_init(struct device_node *np, struct mtk_panel_info *pinfo)
{
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-xres", &pinfo->xres,
				     1440);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-yres", &pinfo->yres,
				     2560);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-width", &pinfo->width,
				     73);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-height", &pinfo->height,
				     130);
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-bl-type",
				     &pinfo->bl_set_type, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-bl-min", &pinfo->bl_min,
				     0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-bl-max", &pinfo->bl_max,
				     0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-bl-def",
				     &pinfo->bl_default, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,panel-cmd-type", &pinfo->type,
				     0);
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,ssc-disable",
				     &pinfo->ssc_disable, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,ssc-range", &pinfo->ssc_range,
				     0);
	/* frame rate */
	...
	if (disp_info->project_id.support) {
		...
		disp_info->project_id.default_project_id =
			(char *)of_get_property(
				np, "lcd-kit,default-project-id", NULL);
	}
	...
	if (common_info->esd.support) {
		pinfo->esd_enable = 1;
		OF_PROPERTY_READ_U8_DEFAULT(
			np, "lcd-kit,skip-poweron-esd-check",
			&disp_info->lcd_kit_skip_poweron_esd_check, 0);
		OF_PROPERTY_READ_U32_DEFAULT(
			np, "lcd-kit,again-esd-check-delay",
			&disp_info->lcd_kit_again_esd_check_delay, 0);
		OF_PROPERTY_READ_U32_DEFAULT(
			np, "lcd-kit,esd-check-delay",
			&disp_info->lcd_kit_esd_check_delay, 0);
		OF_PROPERTY_READ_U8_DEFAULT(
			np, "lcd-kit,skip-esd-check-cnt",
...
{
	/* rgbw */
	lcd_kit_parse_u32(np, "lcd-kit,rgbw-support", &disp_info->rgbw.support,
			  0);
	if (disp_info->rgbw.support) {
		...
		lcd_kit_parse_dcs_cmds(
			np, "lcd-kit,rgbw-pixel-gain-limit-cmds",
...
void lcd_kit_parse_fps(struct device_node *np)
{
	...
	/* lcd_fps_order */
	lcd_kit_parse_u32(np, "lcd-kit,fps-order-mode",
			  &disp_info->fps.order_mode, 0);
	if (disp_info->fps.order_mode == CUST_MODE) {
		lcd_kit_parse_string_arrays(
			np, "lcd-kit,dsi-fps-mmi-test-list",
			&disp_info->lcd_test_config.mmi_test_list);
		lcd_kit_parse_string_arrays(
			np, "lcd-kit,dsi-fps-rt-test-list",
			&disp_info->lcd_test_config.rt_test_list);
		lcd_kit_parse_string_arrays(
			np, "lcd-kit,dsi-fps-tplcd-test-list",
	...
	for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
		...
		if (index == LCD_FPS_SCENCE_60) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,fps-to-normal-60-cmds",
				"lcd-kit,fps-to-normal-60-cmds-state",
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-normal-60-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
			/* hopping */
			if (disp_info->fps.hop_support)
				lcd_kit_parse_array_data(
					np,
					"lcd-kit,fps-normal-60-mipi-hopping",
					&disp_info->fps.hop_info[index]);
		} else if (index == LCD_FPS_SCENCE_H60) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,fps-to-high-60-cmds",
				"lcd-kit,fps-to-high-60-cmds-state",
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-high-60-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
			/* hopping */
			if (disp_info->fps.hop_support)
				lcd_kit_parse_array_data(
					np, "lcd-kit,fps-high-60-mipi-hopping",
					&disp_info->fps.hop_info[index]);
			...
			lcd_kit_parse_dcs_cmds(
				...
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-90-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
			/* hopping */
			if (disp_info->fps.hop_support)
				lcd_kit_parse_array_data(
					np, "lcd-kit,fps-90-mipi-hopping",
					&disp_info->fps.hop_info[index]);
		} else if (index == LCD_FPS_SCENCE_120) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,fps-to-120-cmds",
				"lcd-kit,fps-to-120-cmds-state",
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-120-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
			/* hopping */
			if (disp_info->fps.hop_support)
				lcd_kit_parse_array_data(
					np, "lcd-kit,fps-120-mipi-hopping",
					&disp_info->fps.hop_info[index]);
		} else if (index == LCD_FPS_SCENCE_144) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,fps-to-144-cmds",
				"lcd-kit,fps-to-144-cmds-state",
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-144-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
		} else if (index == LCD_FPS_SCENCE_30) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,fps-to-30-cmds",
				"lcd-kit,fps-to-30-cmds-state",
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-30-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
		} else if (index == LCD_FPS_SCENCE_25) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,fps-to-25-cmds",
				"lcd-kit,fps-to-25-cmds-state",
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-25-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
		} else if (index == LCD_FPS_SCENCE_24) {
			lcd_kit_parse_dcs_cmds(
				np, "lcd-kit,fps-to-24-cmds",
				"lcd-kit,fps-to-24-cmds-state",
				&disp_info->fps.fps_to_cmds[index]);
			lcd_kit_parse_array_data(
				np, "lcd-kit,fps-24-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
		}
	}
}
...
void lcd_kit_parse_dsc(struct device_node *np)
{
	...
	OF_PROPERTY_READ_U32_DEFAULT(
		np, "lcd-kit,dsc-rc-quant-incr-limit0",
		&disp_info->dsc_params.rc_quant_incr_limit0, 0);
	OF_PROPERTY_READ_U32_DEFAULT(
		np, "lcd-kit,dsc-rc-quant-incr-limit1",
				     ...
				     struct mtk_panel_info *pinfo)
{
	...
	array_data.buf = NULL;
	array_data.cnt = 0;
	ret = lcd_kit_parse_array_data(
		np, "lcd-kit,panel-round-corner-top-rc-pattern", &array_data);
	if (ret == LCD_KIT_FAIL) {
...
void lcd_kit_parse_util(struct device_node *np)
{
	...
	/* mipi tx vol */
	lcd_kit_parse_u32(np, "lcd-kit,mipi-tx-vol", &disp_info->mipi_tx_vol,
			  0);

	/* fps */
	lcd_kit_parse_u32(np, "lcd-kit,fps-support", &disp_info->fps.support,
			  0);
	if (disp_info->fps.support)
		lcd_kit_parse_fps(np);

	/* dsc */
	lcd_kit_parse_u32(np, "lcd-kit,dsc-enable", &disp_info->dsc_enable, 0);
	...
	lcd_kit_parse_u32(np, "lcd-kit,hbm-en-times", &disp_info->hbm_en_times,
			  2);
	lcd_kit_parse_u32(np, "lcd-kit,hbm-dis-times",
			  &disp_info->hbm_dis_times, 1);
	/* lcd_color_mode set*/
	lcd_kit_parse_u32(np, "lcd-kit,lcm_color", &disp_info->lcm_color, 0);
	/* HBM delay set*/
	...
	/* regulator name */
	disp_info->vci_regulator_name =
		(char *)of_get_property(np, "lcd-kit,vci-name", NULL);
	disp_info->iovcc_regulator_name =
		(char *)of_get_property(np, "lcd-kit,iovcc-name", NULL);
	disp_info->vdd_regulator_name =
		(char *)of_get_property(np, "lcd-kit,vdd-name", NULL);
	/* elvdd detect */
	...
	if (disp_info->elvdd_detect.support) {
		...
		if (disp_info->elvdd_detect.detect_type ==
			...
			lcd_kit_parse_u32(np, "lcd-kit,elvdd-detect-gpio",
					  ...
					  0);
		lcd_kit_parse_u32(np, "lcd-kit,elvdd-detect-value",
				  &disp_info->elvdd_detect.exp_value, 0);
		lcd_kit_parse_u32(np, "lcd-kit,elvdd-value-mask",
				  &disp_info->elvdd_detect.exp_value_mask, 0);
		lcd_kit_parse_u32(np, "lcd-kit,elvdd-delay",
...
static void parse_dt_pcd(struct device_node *np)
{
	...
	if (disp_info->pcd_errflag.pcd_support) {
		lcd_kit_parse_dcs_cmds(
			np, "lcd-kit,start-pcd-check-cmds",
		...
		lcd_kit_parse_u32(
			np, "lcd-kit,pcd-check-reg-value-compare-mode",
...
static int lcd_kit_rgbw_pix_gain(void)
{
	...
	if ((pix_gain != pix_gain_old) && (rgbw_mode == RGBW_SET4_MODE)) {
		disp_info->rgbw.pixel_gain_limit_cmds.cmds[0].payload[1] =
			pix_gain;
		ret = lcd_kit_dsi_cmds_extern_tx(
			&disp_info->rgbw.pixel_gain_limit_cmds);
...
int lcd_kit_gpio_pcd_errflag_check(void)
{
	...
	if (lcd_kit_gpio_pcd_errflag_read(errflag_gpio, &errflag_gpio_value))
		return PCD_ERRFLAG_SUCCESS;

	LCD_KIT_INFO("pcd_gpio[%d]=%d, errflag_gpio[%d]=%d\n", pcd_gpio,
		     pcd_gpio_value, errflag_gpio, errflag_gpio_value);
...
int lcd_kit_start_pcd_check(void *hld)
{
	...
	if (disp_info->pcd_errflag.pcd_support) {
		for (i = 0;
		     i < disp_info->pcd_errflag.start_pcd_check_cmds.cmd_cnt;
		     i++)
...
static int lcd_kit_judge_pcd_dmd(uint8_t *read_val, uint32_t *expect_val,
				 int cnt, uint32_t compare_mode)
{
	int i;
...
int lcd_kit_pcd_compare_result(uint8_t *read_val, int ret)
{
	...
	if (ret == LCD_KIT_OK) {
		if (lcd_kit_judge_pcd_dmd(
			    read_val, expect_value, expect_value_cnt,
...
int lcd_kit_check_pcd_errflag_check(void *hld)
{
	...
	if (disp_info->pcd_errflag.pcd_support) {
		...
		if (adapt_ops->mipi_tx) {
			ret = adapt_ops->mipi_tx(
				hld, &disp_info->pcd_errflag.switch_page_cmds);
			if (ret)
				LCD_KIT_ERR("mipi_tx fail\n");
		}
		/* init code */
		if (adapt_ops->mipi_rx) {
			ret = adapt_ops->mipi_rx(
				hld, read_pcd, LCD_KIT_PCD_SIZE - 1,
				&disp_info->pcd_errflag.read_pcd_cmds);
			if (ret) {
	...
	if (disp_info->pcd_errflag.errflag_support) {
		...
		if (adapt_ops->mipi_rx)
			ret = adapt_ops->mipi_rx(
				hld, read_errflag, LCD_KIT_ERRFLAG_SIZE - 1,
				&disp_info->pcd_errflag.read_errflag_cmds);
		if (ret) {
			LCD_KIT_INFO("mipi_rx fail\n");
			return ret;
...
int lcd_kit_check_pcd_errflag_check_fac(void)
{
	...
	if (disp_info->pcd_errflag.pcd_support) {
		(void)lcd_kit_dsi_cmds_extern_tx(
			&disp_info->pcd_errflag.switch_page_cmds);
		ret = lcd_kit_dsi_cmds_extern_rx(
			read_pcd, &disp_info->pcd_errflag.read_pcd_cmds,
			LCD_KIT_PCD_SIZE - 1);
		expect_value = disp_info->pcd_errflag.pcd_value.buf;
		expect_value_cnt = disp_info->pcd_errflag.pcd_value.cnt;
		if (ret == LCD_KIT_OK) {
			if (lcd_kit_judge_pcd_dmd(
				    read_pcd, expect_value, expect_value_cnt,
				    disp_info->pcd_errflag
					    .pcd_value_compare_mode) ==
			    LCD_KIT_OK) {
	...
	/* Reserve interface, redevelop when needed */
	if (disp_info->pcd_errflag.errflag_support) {
		lcd_kit_dsi_cmds_extern_rx(
			read_errflag, &disp_info->pcd_errflag.read_errflag_cmds,
...
static int set_mipi_bl_level(struct hbm_type_cfg hbm_source, uint32_t level)
{
	...
	if (hbm_source.source == HBM_FOR_FP) {
		...
		(void)lcd_kit_fp_highlight_cmds_tx(
			hbm_source.handle, &common_info->backlight.bl_cmd);
		return LCD_KIT_OK;
#else
...
{
	LCD_KIT_INFO(
		"[Proximity_feature] lcd_kit_proximity_power_off enter!\n");
	if (!common_info->thp_proximity.support) {
		LCD_KIT_INFO(
			"[Proximity_feature] thp_proximity not support exit!\n");
		return LCD_KIT_FAIL;
	}
	...
	if (common_info->thp_proximity.panel_power_state == POWER_TS_SUSPEND) {
		LCD_KIT_INFO(
			"[Proximity_feature] power off suspend state exit!\n");
		up(&disp_info->thp_second_poweroff_sem);
		return LCD_KIT_OK;
	}
	if (common_info->thp_proximity.work_status == TP_PROXMITY_DISABLE) {
		LCD_KIT_INFO(
			"[Proximity_feature] thp_proximity has been disabled exit!\n");
		up(&disp_info->thp_second_poweroff_sem);
		return LCD_KIT_FAIL;
	}
...
static int lcd_kit_get_sn_code(void)
{
	...
	if (common_info && common_info->sn_code.support) {
		...
		if (panel_ops && panel_ops->lcd_get_2d_barcode) {
			...
			memcpy(pinfo->sn_code, read_value + 2,
			       LCD_KIT_SN_CODE_LENGTH);
		...
		if (disp_info && disp_info->oeminfo.barcode_2d.support) {
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
			ret = lcd_kit_dsi_cmds_extern_rx(
				read_value, &disp_info->oeminfo.barcode_2d.cmds,
				LCD_KIT_SN_CODE_LENGTH);
#else
			ret = lcd_kit_dsi_cmds_extern_rx_v1(
				read_value, &disp_info->oeminfo.barcode_2d.cmds,
				LCD_KIT_SN_CODE_LENGTH);
#endif
			if (ret != 0) {
			...
			memcpy(pinfo->sn_code, read_value,
			       LCD_KIT_SN_CODE_LENGTH);
			pinfo->sn_code_length = LCD_KIT_SN_CODE_LENGTH;
			return LCD_KIT_OK;
		}
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
		ret = lcd_kit_dsi_cmds_extern_rx(
			&read_value, &common_info->panel_ext_info.get_ver_cmds,
			1);
#else
		ret = lcd_kit_dsi_cmds_extern_rx_v1(
			&read_value, &common_info->panel_ext_info.get_ver_cmds,
