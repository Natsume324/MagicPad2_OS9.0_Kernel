...
static int aw3750x_i2c_write(struct aw3750x_power *aw3750x, u8 reg, u8 val)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(aw3750x->client, reg, val);
		if (ret < 0) {
...
static int aw3750x_i2c_bit_write(struct aw3750x_power *aw3750x, u8 reg_addr,
				 u16 mask, u8 reg_data)
{
	uint8_t reg_val = 0;
...
static int aw3750x_i2c_bit_clear(struct aw3750x_power *aw3750x, u8 reg_addr,
				 u16 mask)
{
	uint8_t reg_val = 0;
	int ret = 0;
...
static int aw3750x_soft_reset(struct aw3750x_power *aw3750x)
{
	...
	ret = aw3750x_i2c_bit_write(aw3750x, AW3750X_REG_APPS,
				    AW_SOFT_RESET_MASK, AW3750X_SOFT_RESET);
	if (ret < 0) {
...
static int aw3750x_get_voltage_by_reg(struct aw3750x_power *aw3750x,
				      int reg_val)
{
	int i = 0;
	...
	if ((aw3750x->aw3750x_version_id == AW37501_VERSION_ID) |
		...
		for (i = 0; i < ARRAY_SIZE(aw37501_vout_map); i++) {
			...
			if (reg_val > AW37501_MAX_VOLTAGE) {
				aw3750x->select =
					ARRAY_SIZE(aw37501_vout_map) - 1;
				return aw37501_vout_map
					[ARRAY_SIZE(aw37501_vout_map) - 1]
						.voutp;
			}
		}
		...
		for (i = 0; i < ARRAY_SIZE(aw37502_vout_map); i++) {
			...
			if (reg_val > AW37502_MAX_VOLTAGE) {
				aw3750x->select =
					ARRAY_SIZE(aw37502_vout_map) - 1;
				return aw37502_vout_map
					[ARRAY_SIZE(aw37502_vout_map) - 1]
						.voutp;
			}
		}
...
static int aw3750x_get_reg_by_voltage(struct aw3750x_power *aw3750x, int vol)
{
	...
	if ((aw3750x->aw3750x_version_id == AW37501_VERSION_ID) |
		...
		for (i = 0; i < ARRAY_SIZE(aw37501_vout_map); i++) {
			...
			if (vol >
			    aw37501_vout_map[ARRAY_SIZE(aw37501_vout_map) - 1]
				    .voutp) {
				aw3750x->select =
					ARRAY_SIZE(aw37501_vout_map) - 1;
				return aw37501_vout_map
		...
		for (i = 0; i < ARRAY_SIZE(aw37502_vout_map); i++) {
			...
			if (vol >
			    aw37502_vout_map[ARRAY_SIZE(aw37502_vout_map) - 1]
				    .voutp) {
				aw3750x->select =
					ARRAY_SIZE(aw37502_vout_map) - 1;
				return aw37502_vout_map
...
static int aw3750x_disn_enable(struct aw3750x_power *aw3750x)
{
	...
	ret = aw3750x_i2c_bit_write(aw3750x, AW3750X_REG_APPS,
				    AW3750X_DISN_MASK, AW3750X_DISN_ENABLE);
	if (ret < 0) {
		pr_err("%s aw3750x_i2c_bit_write failed\n", __func__);
		return ret;
	}
...
static int aw3750x_disn_disable(struct aw3750x_power *aw3750x)
{
	...
	ret = aw3750x_i2c_bit_clear(aw3750x, AW3750X_REG_APPS,
				    AW3750X_DISN_MASK);
	if (ret < 0) {
...
static int aw3750x_disp_enable(struct aw3750x_power *aw3750x)
{
	...
	ret = aw3750x_i2c_bit_write(aw3750x, AW3750X_REG_APPS,
				    AW3750X_DISP_MASK, AW3750X_DISP_ENABLE);
	if (ret < 0) {
		pr_err("%s aw3750x_i2c_bit_write failed\n", __func__);
		return ret;
	}
...
static void aw3750x_set_current(void)
{
	...
	if (ret < 0) {
		LCD_KIT_ERR(
			"aw3750x_i2c_read_u8 AW3750X_REG_APPS failed,ret = %d\n",
			ret);
		return;
	}
	...
	if (ret < 0) {
		LCD_KIT_ERR(
			"aw3750x_i2c_write_u8 AW3750X_REG_APPS failed,ret = %d\n",
			ret);
		return;
	}
	...
	if (ret < 0) {
		LCD_KIT_ERR(
			"aw3750x_i2c_read_u8 AW3750X_REG_CTRL failed,ret = %d\n",
			ret);
		return;
	}
	...
	if (ret < 0) {
		LCD_KIT_ERR(
			"aw3750x_i2c_write_u8 AW3750X_REG_CTRL failed,ret = %d\n",
			ret);
		return;
	}
	return;
...
static int aw3750x_ldo_current_set(struct aw3750x_power *aw3750x)
{
	...
	if (aw3750x->limit == MAX_LOO_CURRENT) {
		ret = aw3750x_i2c_bit_clear(aw3750x, AW3750X_REG_CTRL,
					    LOO_CURRENT_MASK);
		if (ret < 0) {
			pr_err("%s aw3750x_i2c_bit_clear failed\n", __func__);
			return ret;
		...
		ret = aw3750x_i2c_bit_write(aw3750x, AW3750X_REG_CTRL,
					    LOO_CURRENT_MASK, LOD_CURRENT_VAL);
		if (ret < 0) {
			pr_err("%s aw3750x_i2c_bit_write failed\n", __func__);
			return ret;
		}
...
static int aw3750x_set_voltage(struct regulator_dev *rdev, int vol_min,
			       int vol_max, unsigned *selector)
{
	int ret = 0;
	...
	if (strcmp(rdev->desc->name, "outp") == 0) {
		...
		aw3750x->outp_set =
			aw3750x_get_reg_by_voltage(aw3750x, aw3750x->outp);
		...
		aw3750x->outn_set =
			aw3750x_get_reg_by_voltage(aw3750x, aw3750x->outn);
...
static int aw37501_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	if (strcmp(rdev->desc->name, "outp") == 0) {
		...
		aw3750x->outp =
			aw3750x_get_voltage_by_reg(aw3750x, aw3750x->read_outp);
		return aw3750x->outp;
	} else if (strcmp(rdev->desc->name, "outn") == 0) {
		...
		aw3750x->outn =
			aw3750x_get_voltage_by_reg(aw3750x, aw3750x->read_outn);
		return aw3750x->outn;
	} else {
...
static int aw37501_set_current(struct aw3750x_power *aw3750x, int min_uA,
			       int max_uA)
{
	int ret = 0;
	u8 reg_val = 0;
...
static int aw37501b_set_current(struct aw3750x_power *aw3750x, int min_uA,
				int max_uA)
{
	int ret = 0;
	u8 reg_val = 0;
...
static int aw37502_set_current(struct aw3750x_power *aw3750x, int min_uA,
			       int max_uA)
{
	int ret = 0;
	u8 reg_val = 0;
...
static int aw37503_set_current(struct aw3750x_power *aw3750x, int min_uA,
			       int max_uA)
{
	int ret = 0;
	u8 reg_val = 0;
...
static int aw37504_set_current(struct aw3750x_power *aw3750x, int min_uA,
			       int max_uA)
{
	int ret = 0;
	u8 reg_ctrl = 0;
	...
	ret = aw3750x_i2c_bit_write(aw3750x, AW3750X_REG_CTRL,
				    AW37504_CURRENT_CTRL_MASK, reg_ctrl);
	if (ret < 0) {
		pr_err("%s,aw37504 set current ctrl failed\n", __func__);
		return ret;
	}
	ret = aw3750x_i2c_bit_write(aw3750x, AW3750X_REG_APPS,
				    AW37504_CURRENT_APPS_MASK, reg_apps);
	if (ret < 0) {
...
static int aw37505_set_current(struct aw3750x_power *aw3750x, int min_uA,
			       int max_uA)
{
	int ret = 0;
	u8 reg_val = 0;
...
static int aw3750x_set_current_limit(struct regulator_dev *rdev, int min_uA,
				     int max_uA)
{
	struct aw3750x_power *aw3750x = rdev_get_drvdata(rdev);
	int ret = 0;
...
int aw3750x_map_voltage(struct regulator_dev *rdev, int min_uV, int max_uV)
{
	int i, ret;
...
static int aw3750x_pinctrl_init(struct aw3750x_power *aw3750x)
{
	...
	/* for enn */
	pinctrl->aw_enn_default =
		pinctrl_lookup_state(pinctrl->pinctrl, "aw_enn_default");
	...
	pinctrl->aw_enn_high =
		pinctrl_lookup_state(pinctrl->pinctrl, "aw_enn_output_high");
	...
	pinctrl->aw_enn_low =
		pinctrl_lookup_state(pinctrl->pinctrl, "aw_enn_output_low");
	if (IS_ERR_OR_NULL(pinctrl->aw_enn_low)) {
		pr_info("%s: Failed get pinctrl state:enn_low\n", __func__);
		goto exit_pinctrl_init;
	}
	/* for enp */
	pinctrl->aw_enp_default =
		pinctrl_lookup_state(pinctrl->pinctrl, "aw_enp_default");
	...
	pinctrl->aw_enp_high =
		pinctrl_lookup_state(pinctrl->pinctrl, "aw_enp_output_high");
	...
	pinctrl->aw_enp_low =
		pinctrl_lookup_state(pinctrl->pinctrl, "aw_enp_output_low");
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	ret = aw3750x->regulators[0]->desc->ops->get_voltage(
		aw3750x->regulators[0]);
	if (ret < 0) {
					...
					const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%x", &databuf[0]) == 1) {
		if (databuf[0] >= MIN_VOLTAGE_VAL &&
		    databuf[0] <= MAX_VOLTAGE_VAL) {
			aw3750x->outp =
				aw3750x_get_voltage_by_reg(aw3750x, databuf[0]);
			ret = aw3750x->regulators[0]->desc->ops->set_voltage(
				aw3750x->regulators[0], aw3750x->outp,
				aw3750x->outp, NULL);
			if (ret < 0) {
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	ret = aw3750x->regulators[1]->desc->ops->get_voltage(
		aw3750x->regulators[1]);
					...
					const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%x", &databuf[0]) == 1) {
		if (databuf[0] >= MIN_VOLTAGE_VAL &&
			...
			aw3750x->outn =
				aw3750x_get_voltage_by_reg(aw3750x, databuf[0]);
			ret = aw3750x->regulators[1]->desc->ops->set_voltage(
				aw3750x->regulators[1], aw3750x->outn,
				aw3750x->outn, NULL);
			if (ret < 0) {
				dev_err(dev, "%s set_voltage_sel error\n",
					__func__);
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = aw3750x->regulators[1]->desc->ops->is_enabled(
		aw3750x->regulators[1]);
	if (ret < 0) {
				 ...
				 size_t len)
{
	...
	if (val == 1) {
		ret = aw3750x->regulators[1]->desc->ops->enable(
			aw3750x->regulators[1]);
		if (ret < 0) {
		...
		ret = aw3750x->regulators[1]->desc->ops->disable(
			aw3750x->regulators[1]);
		if (ret < 0) {
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = aw3750x->regulators[0]->desc->ops->is_enabled(
		aw3750x->regulators[0]);
	if (ret < 0) {
		dev_err(dev, "%s is_enabled error\n", __func__);
		return ret;
				 ...
				 size_t len)
{
	...
	if (val == 1) {
		ret = aw3750x->regulators[0]->desc->ops->enable(
			aw3750x->regulators[0]);
		if (ret < 0) {
			dev_err(dev, "%s enable error\n", __func__);
			return ret;
		...
		ret = aw3750x->regulators[0]->desc->ops->disable(
			aw3750x->regulators[0]);
		if (ret < 0) {
			dev_err(dev, "%s disable error\n", __func__);
			return ret;
					 ...
					 char *buf)
{
	...
	ret = aw3750x->regulators[0]->desc->ops->get_current_limit(
		aw3750x->regulators[0]);
	if (ret < 0) {
					  ...
					  const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%d %d", &databuf[0], &databuf[1]) == 2) {
		ret = aw3750x->regulators[0]->desc->ops->set_current_limit(
			aw3750x->regulators[0], databuf[0], databuf[1]);
		if (ret < 0) {
				      ...
				      const char *buf, size_t len)
{
	...
	ret = aw3750x->regulators[0]->desc->ops->list_voltage(
		aw3750x->regulators[0], val);
	if (ret < 0) {
				      ...
				      const char *buf, size_t len)
{
	...
	ret = aw3750x->regulators[1]->desc->ops->list_voltage(
		aw3750x->regulators[1], val);
	if (ret < 0) {
		dev_err(dev, "%s list_voltage error\n", __func__);
		return ret;
	}
...
static struct attribute *aw3750x_power_attributes[] = {
	&dev_attr_reg.attr,	  &dev_attr_outp.attr,
	&dev_attr_outn.attr,	  &dev_attr_enn.attr,
	&dev_attr_enp.attr,	  &dev_attr_current_mode.attr,
	&dev_attr_disn.attr,	  &dev_attr_disp.attr,
	&dev_attr_list_outp.attr, &dev_attr_list_outn.attr,
...
static int aw3750x_power_init(struct aw3750x_power *aw3750x)
{
	...
	if (ret < 0) {
		dev_err(aw3750x->dev,
			"%s aw3750x_release_load_capability error\n", __func__);
		return ret;
	}
...
static int aw3750x_gpio_init(struct aw3750x_power *aw3750x,
...
static int aw3750x_parse_dt(struct i2c_client *i2c,
			    struct aw3750x_power *aw3750x,
...
static int aw3750x_i2c_read_chipid(struct aw3750x_power *aw3750x)
{
	...
	if (val == AW37501_VERSION_ID) {
		...
		if ((reg_val & AW37504_VERSION_ID) == AW37504_VERSION_ID) {
			aw3750x->aw3750x_version_id = reg_val &
						      AW37504_VERSION_ID;
			       ...
			       const struct i2c_device_id *id)
{
	...
	aw3750x = devm_kzalloc(&i2c->dev, (sizeof(struct aw3750x_power)),
			       GFP_KERNEL);
	if (!aw3750x) {
	...
	for (i = 0; i < ARRAY_SIZE(aw3750x_desc); i++) {
		...
		if (&aw3750x->aw_node[i] == NULL) {
			dev_err(&i2c->dev,
				" %s aw3750x->aw_node[%d] is NULL.\n", __func__,
				i);
			break;
		}
		init = of_get_regulator_init_data(
			&i2c->dev, aw3750x->aw_node[i], &desc[i]);
		...
		aw3750x->regulators[i] =
			devm_regulator_register(&i2c->dev, desc, &config);
