...
static int
lcd_kit_cmds_to_qcom_cmds_tx(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
...
static int
lcd_kit_cmds_to_qcom_cmds_rx(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
			     struct dsi_cmd_desc *cmd, int link_state,
			     unsigned char *buf)
{
	if (lcd_kit_cmds == NULL) {
...
int qcom_mipi_dsi_cmd_rx(struct dsi_ctrl *dsi_ctrl, unsigned char *buffer,
			 unsigned int *len, struct lcd_kit_dsi_cmd_desc *cmds,
			 int link_state)
{
	struct dsi_cmd_desc cmd;
	u32 flags = (DSI_CTRL_CMD_FETCH_MEMORY | DSI_CTRL_CMD_CUSTOM_DMA_SCHED |
	...
	if ((dsi_ctrl == NULL) || (cmds == NULL) || (buffer == NULL) ||
	    (len == NULL)) {
				...
				struct lcd_kit_dsi_panel_cmds *cmds)
{
	...
	if ((dsi_ctrl == NULL) || (cmds == NULL) || (out == NULL) ||
	    (len == 0)) {
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (lcd_kit_cmd_is_write(&cmds->cmds[i])) {
			...
			if (ret != LCD_KIT_OK) {
				LCD_KIT_ERR(
					"the %d mipi cmd tx fail, ret = %d\n",
					i, ret);
				return ret;
			}
		} else {
			ret = qcom_mipi_dsi_cmd_rx(dsi_ctrl, &buffer[0],
						   &rx_len, &cmds->cmds[i],
						   cmds->link_state);
			if (ret != LCD_KIT_OK) {
				LCD_KIT_ERR(
					"the %d mipi cmd rx fail, ret = %d\n",
					i, ret);
				return ret;
			}
			for (j = 0; j < rx_len; j++) {
...
int lcd_kit_dsi_cmds_rx(void *hld, unsigned char *out, unsigned int len,
			struct lcd_kit_dsi_panel_cmds *cmds)
{
	int ret = LCD_KIT_OK;
	...
	struct dsi_display_ctrl *m_ctrl =
		&display->ctrl[display->cmd_master_idx];

	if ((display == NULL) || (cmds == NULL) || (out == NULL) ||
	    (len == 0)) {
		LCD_KIT_ERR("out or cmds is NULL\n");
		return LCD_KIT_FAIL;
	}
	...
	ret = dsi_display_clk_ctrl(display->dsi_clk_handle, DSI_ALL_CLKS,
				   DSI_CLK_ON);
	if (ret) {
	...
	if (enable) {
		...
		if (num_of_v > 0) {
			ret = regulator_set_voltage(
				pvreg->vreg, pvreg_info->buf[POWER_MIN_VOL],
...
error_disable_voltage:
	if (num_of_v > 0)
		(void)regulator_set_voltage(pvreg->vreg, 0,
					    pvreg_info->buf[POWER_MAX_VOL]);
error:
	return ret;
}
...
static int lcd_kit_regulator_enable(u32 type)
{
	...
	switch (type) {
	case LCD_KIT_IOVCC:
		ret = lcd_kit_regulate_ctrl(&power_hdl->lcd_iovcc,
					    &common_info->iovcc_vreg, true);
		break;
	case LCD_KIT_VSP:
		ret = lcd_kit_regulate_ctrl(&power_hdl->lcd_vsp,
					    &common_info->vsp_vreg, true);
		break;
	case LCD_KIT_VSN:
		ret = lcd_kit_regulate_ctrl(&power_hdl->lcd_vsn,
					    &common_info->vsn_vreg, true);
		break;
	default:
		ret = LCD_KIT_FAIL;
...
static int lcd_kit_regulator_disable(u32 type)
{
	...
	case LCD_KIT_IOVCC:
		ret = lcd_kit_regulate_ctrl(&power_hdl->lcd_iovcc,
					    &common_info->iovcc_vreg, false);
		break;
	case LCD_KIT_VSP:
		ret = lcd_kit_regulate_ctrl(&power_hdl->lcd_vsp,
					    &common_info->vsp_vreg, false);
		break;
	case LCD_KIT_VSN:
		ret = lcd_kit_regulate_ctrl(&power_hdl->lcd_vsn,
					    &common_info->vsn_vreg, false);
		break;
	default:
		ret = LCD_KIT_FAIL;
		LCD_KIT_ERR("regulator type:%d not support\n", type);
