				       ...
				       unsigned int level)
{
	...
	if (common_info->backlight.bl_max <= 0xFF) {
		...
		ret = mipi_dsi_dcs_write(
			dsi, common_info->backlight.bl_cmd.cmds[0].payload[0],
...
int lcd_kit_panel_prepare(struct dsi_panel *panel)
{
	...
	if (rc) {
		LCD_KIT_ERR(
			"[%s] failed to send DSI_CMD_SET_PRE_ON cmds, rc=%d\n",
			panel->name, rc);
		goto error;
	}
...
int lcd_kit_panel_disable(struct dsi_panel *panel)
{
	...
	if (!atomic_read(&panel->esd_recovery_pending)) {
		...
		if (rc) {
			 ...
			 */
			pr_warn_ratelimited(
				"[%s] failed to send DSI_CMD_SET_OFF cmds, rc=%d\n",
				panel->name, rc);
			rc = 0;
		}
	}
...
int lcd_kit_panel_unprepare(struct dsi_panel *panel)
{
	...
	if (rc)
		LCD_KIT_ERR(
			"[%s] failed to send DSI_CMD_SET_POST_OFF cmds, rc=%d\n",
			panel->name, rc);
	mutex_unlock(&panel->panel_lock);
	LCD_KIT_INFO("exit\n");
	return rc;
