...
static ssize_t backlight_reg_addr_store(struct device *dev,
...
static ssize_t backlight_reg_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t size)
{
	struct lcd_kit_backlight_device *pchip = NULL;
...
static DEVICE_ATTR(reg, S_IRUGO | S_IWUSR, backlight_reg_show,
		   backlight_reg_store);

static ssize_t backlight_bl_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct lcd_kit_backlight_device *pchip = NULL;
	int ret;
...
static ssize_t backlight_bl_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t size)
{
	struct lcd_kit_backlight_device *pchip = NULL;
	...
	if (pchip->bl_config.bl_lsb_reg_cmd.val_bits != 0) {
		ret = lcd_kit_backlight_write_byte(
			pchip, pchip->bl_config.bl_lsb_reg_cmd.cmd_reg, bl_lsb);
		if (ret < 0) {
	...
	ret = lcd_kit_backlight_write_byte(
		pchip, pchip->bl_config.bl_msb_reg_cmd.cmd_reg, bl_msb);
	if (ret < 0) {
		LCD_KIT_ERR("set backlight failed\n");
		return -EINVAL;
	}
...
static DEVICE_ATTR(backlight, S_IRUGO | S_IWUSR, backlight_bl_show,
		   backlight_bl_store);
...
static void lcd_kit_backlight_propname_cat(char *pdest, const char *psrc,
					   int dest_len)
{
	if ((pdest == NULL) || (psrc == NULL) || (dest_len == 0)) {
		LCD_KIT_ERR("input invalid\n");
		return;
	}
				       ...
				       struct backlight_ic_cmd *pcmd)
{
	...
	ret = of_property_read_u32_array(np, tmp_buf, buf,
					 BACKLIGHT_IC_CMD_LEN);
	if (ret) {
	...
	pcmd->delay = buf[4];
	LCD_KIT_INFO(
		"%s: type is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
		propertyname, pcmd->ops_type, pcmd->cmd_reg, pcmd->cmd_val,
		pcmd->cmd_mask);
	kfree(buf);
	buf = NULL;
...
static int
lcd_kit_backlight_parse_base_config(struct device_node *np,
...
static void
lcd_kit_backlight_parse_bias_config(struct device_node *np,
				    struct lcd_kit_backlight_info *pbl_config)
{
	int ret;
...
static void
lcd_kit_backlight_parse_gpio_config(struct device_node *np,
				    struct lcd_kit_backlight_info *pbl_config)
{
	if ((pbl_config == NULL) || (np == NULL)) {
		LCD_KIT_ERR("input parameter is NULL\n");
	...
	lcd_kit_parse_backlight_param(np, "hw_enable", &pbl_config->hw_en);
	if (pbl_config->hw_en) {
...
static int lcd_kit_backlight_parse_bl_cmd(struct device_node *np,
					  const char *propertyname,
					  struct backlight_reg_info *pcmd)
{
	char tmp_buf[LCD_BACKLIGHT_IC_COMP_LEN] = {0};
	struct property *prop = NULL;
	unsigned int *buf = NULL;
	...
	pcmd->cmd_mask = buf[3];
	LCD_KIT_INFO(
		"%s: val_bits is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
		tmp_buf, pcmd->val_bits, pcmd->cmd_reg, pcmd->cmd_val,
		pcmd->cmd_mask);
	kfree(buf);
	buf = NULL;
...
static void lcd_kit_backlight_parse_backlight_config(
	struct device_node *np, struct lcd_kit_backlight_info *pbl_config)
{
	int ret;
...
static void lcd_kit_backlight_parse_fault_check_config(
	struct device_node *np, struct lcd_kit_backlight_info *pbl_config)
{
	int ret;
...
static void lcd_kit_backlight_parse_init_cmds_config(
	struct device_node *np, struct lcd_kit_backlight_info *pbl_config)
{
	int ret;
	unsigned int tmp_val;
	...
	lcd_kit_backlight_propname_cat(tmp_buf, "backlight_ic_kernel_init_cmd",
				       sizeof(tmp_buf));
	ret = of_property_read_u32_array(np, tmp_buf, buf,
					 prop->length / sizeof(unsigned int));
	if (ret) {
	...
	for (i = 0; i < pbl_config->num_of_init_cmds; i++) {
		...
		LCD_KIT_ERR(
			"init i is %d, tpye is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
...
static void lcd_kit_backlight_get_bias_value(int vsp, int vsn,
					     unsigned char *pos_votage,
					     unsigned char *neg_votage)
{
	int i;
	int j;
	...
	for (i = 0; i < ARRAY_SIZE(backlight_bias_config); i++) {
		...
		for (j = 0; j < backlight_bias_config[i].len; j++) {
			if (backlight_bias_config[i].vol_table[j].voltage ==
			    vsp) {
				*pos_votage = backlight_bias_config[i]
						      .vol_table[j]
						      .value;
				break;
			}
		...
		for (j = 0; j < backlight_bias_config[i].len; j++) {
			if (backlight_bias_config[i].vol_table[j].voltage ==
			    vsn) {
				*neg_votage = backlight_bias_config[i]
						      .vol_table[j]
						      .value;
				break;
			}
		...
		if (j == backlight_bias_config[i].len) {
			LCD_KIT_INFO(
				"not found neg voltage, use default voltage\n");
...
static int lcd_kit_backlight_ic_config(void)
{
	...
	for (i = 0; i < pbacklight_dev->bl_config.num_of_init_cmds; i++) {
		...
		case REG_READ_MODE:
			ret = lcd_kit_backlight_read_byte(
				pbacklight_dev,
				pbacklight_dev->bl_config.init_cmds[i].cmd_reg,
				&(pbacklight_dev->bl_config.init_cmds[i]
					  .cmd_val));
			break;
		case REG_WRITE_MODE:
			ret = lcd_kit_backlight_write_byte(
		...
		case REG_UPDATE_MODE:
			ret = lcd_kit_backlight_update_bit(
				pbacklight_dev,
				pbacklight_dev->bl_config.init_cmds[i].cmd_reg,
...
static int lcd_kit_backlight_set_bias(int vpos, int vneg)
{
	...
	if (pbacklight_dev->bl_config.hw_en != 0 &&
		...
		ret = gpio_direction_output(
			pbacklight_dev->bl_config.hw_en_gpio,
	...
	if (g_vol_mapped == BACKLIGHT_FALSE) {
		...
		lcd_kit_backlight_get_bias_value(vpos, vneg, &vsp_vol,
						 &vsn_vol);
		pbacklight_dev->bl_config.init_cmds[vsp_index].cmd_val =
			vsp_vol;
		pbacklight_dev->bl_config.init_cmds[vsn_index].cmd_val =
			vsn_vol;
...
#if defined CONFIG_HONOR_DSM
static void
lcd_kit_backlight_ic_ovp_check(struct lcd_kit_backlight_device *pchip,
			       int last_level, int level)
{
	...
	if (!pchip->bl_config.bl_ovp_flag_cmd.cmd_mask)
		return;
	ret = lcd_kit_backlight_read_byte(
		pchip, pchip->bl_config.bl_ovp_flag_cmd.cmd_reg, &val);
	if (ret < 0) {
...
static void
lcd_kit_backlight_ic_ocp_check(struct lcd_kit_backlight_device *pchip,
			       int last_level, int level)
{
	unsigned char val = 0;
	int ret;
	...
	if (!pchip->bl_config.bl_ocp_flag_cmd.cmd_mask)
		return;
	ret = lcd_kit_backlight_read_byte(
		pchip, pchip->bl_config.bl_ocp_flag_cmd.cmd_reg, &val);
	if (ret < 0) {
...
static void
lcd_kit_backlight_ic_tsd_check(struct lcd_kit_backlight_device *pchip,
			       int last_level, int level)
{
	unsigned char val = 0;
	int ret;
	...
	if (!pchip->bl_config.bl_tsd_flag_cmd.cmd_mask)
		return;
	ret = lcd_kit_backlight_read_byte(
		pchip, pchip->bl_config.bl_tsd_flag_cmd.cmd_reg, &val);
	if (ret < 0) {
...
static void
lcd_kit_backlight_ic_fault_check(struct lcd_kit_backlight_device *pchip,
				 int level)
{
...
int lcd_kit_set_backlight(unsigned int bl_level)
{
	...
	if (pbacklight_dev->bl_config.bl_lsb_reg_cmd.val_bits != 0) {
		...
		if (ret < 0) {
			LCD_KIT_ERR(
				"set backlight ic brightness lsb failed!\n");
			goto i2c_error;
		}
	}
	ret = lcd_kit_backlight_write_byte(
				   ...
				   const struct i2c_device_id *id)
{
	...
	pbacklight_dev = devm_kzalloc(&client->dev,
				      sizeof(struct lcd_kit_backlight_device),
				      GFP_KERNEL);
	if (pbacklight_dev == NULL) {
	...
	lcd_kit_backlight_parse_dts(client->dev.of_node,
				    &pbacklight_dev->bl_config);
	sema_init(&(pbacklight_dev->sem), 1);

	if (backlight_class != NULL) {
		pbacklight_dev->dev = device_create(backlight_class, NULL, 0,
						    "%s", backlight_ic_name);
		if (IS_ERR(pbacklight_dev->dev)) {
			...
			ret = sysfs_create_group(&pbacklight_dev->dev->kobj,
						 &backlight_group);
			if (ret) {
				LCD_KIT_ERR(
					"Create backlight sysfs group node failed!\n");
	...
	.driver =
		{
			...
			.owner = THIS_MODULE,
			.of_match_table =
				of_match_ptr(lcd_kit_backlight_match_table),
		},
...
static int __init lcd_kit_backlight_init(void)
{
	...
	memcpy(backlight_ic_id[0].name, backlight_ic_name,
	       LCD_BACKLIGHT_IC_NAME_LEN);
	lcd_kit_backlight_driver.driver.name = backlight_ic_name;
	snprintf(compatible_name, sizeof(compatible_name), "honor,%s",
		 backlight_ic_name);
	len = strlen(compatible_name);
	if (len >= LCD_BACKLIGHT_IC_COMP_LEN)
		len = LCD_BACKLIGHT_IC_COMP_LEN - 1;
	memcpy(lcd_kit_backlight_match_table[0].compatible, compatible_name,
	       len);
	lcd_kit_backlight_match_table[0]
		.compatible[LCD_BACKLIGHT_IC_COMP_LEN - 1] = 0;
