...
static int
lcd_kit_cmds_to_dsi_read_cmds(struct lcd_kit_dsi_cmd_desc *lcd_kit_cmds,
...
int sprd_mipi_dsi_cmds_tx(void *dsi, struct lcd_kit_dsi_cmd_desc *cmds, int cnt,
			  int link_state)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (lcd_kit_cmd_is_write(&cmds->cmds[i])) {
			lcd_kit_cmds_to_dsi_cmds(&cmds->cmds[i], &dsi_cmd,
						 cmds->link_state);
			ret = sprd_mipi_dsi_cmds_tx(dsi, &cmds->cmds[i], 1,
						    cmds->link_state);
			if (ret < 0)
				LCD_KIT_ERR("mipi write error\n");
		} else {
			lcd_kit_cmds_to_dsi_read_cmds(&cmds->cmds[i], &out[0],
						      out_len, &dsi_cmd,
						      cmds->link_state);
			       ...
			       unsigned int len)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (lcd_kit_cmd_is_write(&cmds->cmds[i])) {
			ret = sprd_mipi_dsi_cmds_tx(panel_info->slave,
						    &cmds->cmds[i], 1,
						    cmds->link_state);
			if (ret < 0)
				LCD_KIT_ERR("mipi write error\n");
		} else {
			ret = lcd_kit_cmds_to_dsi_read_cmds(&cmds->cmds[i],
							    &buffer[0], len,
							    &dsi_cmd,
							    cmds->link_state);
			if (ret != LCD_KIT_OK) {
...
int lcd_kit_dsi_cmds_extern_tx(struct lcd_kit_dsi_panel_cmds *cmds)
{
	...
	mutex_lock(panel_lock);
	for (i = 0; i < cmds->cmd_cnt; i++)
		sprd_mipi_dsi_cmds_tx(panel_info->slave, &cmds->cmds[i], 1,
				      cmds->link_state);
	mutex_unlock(panel_lock);
	return ret;
}
