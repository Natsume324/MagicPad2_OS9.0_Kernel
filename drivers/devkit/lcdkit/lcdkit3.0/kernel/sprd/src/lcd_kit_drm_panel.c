					      ...
					      unsigned int level)
{
	...
	if (common_info->backlight.bl_max <= 0xFF) {
		...
		if (panel->info.use_dcs)
			mipi_dsi_dcs_write_buffer(dsi, bl_tb_short,
						  sizeof(bl_tb_short));
		else
			mipi_dsi_generic_write(dsi, bl_tb_short,
					       sizeof(bl_tb_short));
	} else {
		bl_tb_long[0] =
			common_info->backlight.bl_cmd.cmds[0].payload[0];
		...
		bl_tb_long[2] =
			common_info->backlight.bl_cmd.cmds[0].payload[2];
		if (panel->info.use_dcs)
			mipi_dsi_dcs_write_buffer(dsi, bl_tb_long,
						  sizeof(bl_tb_long));
		else
			mipi_dsi_generic_write(dsi, bl_tb_long,
					       sizeof(bl_tb_long));
	}
	if (ret < 0)
...
static int lcd_kit_backlight_update(struct backlight_device *bd)
{
	...
	if (bl_lvl > last_level) {
		if (bl_lvl - last_level > BL_MAX_STEP)
			LCD_KIT_INFO(
				"last brightness is %d cur brightness is %d\n",
	...
	if (disp_info->quickly_sleep_out.support) {
		if (disp_info->quickly_sleep_out.panel_on_tag && bl_lvl > 0)
...
static int lcd_kit_backlight_setup(struct sprd_panel *panel, struct device *dev)
{
	...
	panel->backlight = devm_backlight_device_register(
		dev, "sprd_backlight", dev, panel, &backlight_ops, &props);
				  ...
				  struct device_node *lcd_node)
{
	...
	info->buildin_modes = kzalloc(
		sizeof(struct drm_display_mode) * num_timings, GFP_KERNEL);
	...
	for (i = 0; i < num_timings; i++) {
		ret = of_get_drm_display_mode(lcd_node, &info->buildin_modes[i],
					      NULL, i);
		if (ret) {
 ...
 */
static int sprd_panel_bind(struct device *dev, struct device *master,
			   void *data)
{
	/* do nothing */
	LCD_KIT_INFO("%s()\n", __func__);
...
static void sprd_panel_unbind(struct device *dev, struct device *master,
			      void *data)
{
	/* do nothing */
	LCD_KIT_INFO("%s()\n", __func__);
...
static int panel_device_create(struct device *parent, struct sprd_panel *panel)
{
	...
	for (i = 0; i < info->esd.reg_items; i++) {
		...
		for (j = 0; j < info->esd.read_len[i]; j++) {
			if (info->esd.val_seq[offset] != read_buf[j]) {
				LCD_KIT_ERR(
					"val_seq is 0x%x, read_buf is 0x%x\n",
...
static int lcd_kit_panel_te_check(struct sprd_panel *panel)
{
	...
	ret = wait_event_interruptible_timeout(dpu->ctx.te_wq, dpu->ctx.evt_te,
	...
	if (!ret) {
		...
		if (dpu->core && dpu->core->check_raw_int) {
			...
			if (dpu->ctx.is_inited)
				irq_occur = dpu->core->check_raw_int(
					&dpu->ctx, DISPC_INT_TE_MASK);
			...
			if (!irq_occur) {
				...
				LCD_KIT_ERR(
					"TE occur, but isr schedule delay\n");
		} else {
			LCD_KIT_ERR("TE esd timeout.\n");
			ret = -1;
		}
...
static void lcd_kit_panel_esd_work_func(struct work_struct *work)
{
	struct sprd_panel *panel =
		container_of(work, struct sprd_panel, esd_work.work);
	...
	if (ret && panel->base.connector && panel->base.connector->encoder) {
		...
		if (!encoder->crtc ||
		    (encoder->crtc->state && !encoder->crtc->state->active)) {
		...
		if (esd_fail_count >= LCD_KIT_ESD_MAX) {
			LCD_KIT_ERR(
				"esd continue fail %d, so disable esd function\n",
				esd_fail_count);
