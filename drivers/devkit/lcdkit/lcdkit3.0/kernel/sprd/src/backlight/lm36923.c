...
static int lm36923_config_write(struct lm36923_chip_data *pchip,
				...
				unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != LM36923_INVALID_VAL) {
			...
			LCD_KIT_INFO("lm36923 write 0x%x = 0x%x", reg[i],
				     val[i]);
			if (ret < 0) {
...
static int lm36923_config_read(struct lm36923_chip_data *pchip,
			       unsigned int reg[], unsigned int val[],
			       unsigned int size)
{
	int ret = 0;
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = lm36923_config_read(pchip, lm36923_reg_addr, lm36923_reg_info,
				  LM36923_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(buf, PAGE_SIZE, "0x11 = 0x%x 0x18 = 0x%x\n \
		\r0x19 = 0x%x 0x10 = 0x%x\n ",
		       lm36923_reg_info[0], lm36923_reg_info[1],
		       lm36923_reg_info[2], lm36923_reg_info[3]);
	return ret;
}
...
{
	int ret;
	ret = regmap_write(lm36923_g_chip->regmap, LM36923_REG_LEDS_CONFIG,
			   LM36923_REG_LEDS_CONFIG_DISABLE);
	if (ret < 0)
		LCD_KIT_ERR("write lm36923 0x%x fail\n",
			    LM36923_REG_LEDS_CONFIG);
