...
static int ktd3136_config_write(struct ktd3136_chip_data *pchip,
				...
				unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != KTD3136_INVALID_VAL) {
			...
			LCD_KIT_INFO("ktd3136 write 0x%x = 0x%x", reg[i],
				     val[i]);
			if (ret < 0) {
...
static int ktd3136_config_read(struct ktd3136_chip_data *pchip,
			       unsigned int reg[], unsigned int val[],
			       unsigned int size)
{
	int ret = 0;
...
static ssize_t ktd3136_reg_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	ret = ktd3136_config_read(pchip, ktd3136_reg_addr, ktd3136_reg_info,
				  KTD3136_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(buf, PAGE_SIZE, "0x03 = 0x%x,0x04 = 0x%x,0x05 = 0x%x\n \
		\r0x06 = 0x%x,Ox07 = 0x%x,0x08 = 0x%x,0x02 = 0x%x\n",
		       ktd3136_reg_info[0], ktd3136_reg_info[1],
		       ktd3136_reg_info[2], ktd3136_reg_info[3],
		       ktd3136_reg_info[4], ktd3136_reg_info[5],
		       ktd3136_reg_info[6]);
	return ret;
}

static ssize_t ktd3136_reg_store(struct device *dev,
...
static void ktd3136_check_fault(struct ktd3136_chip_data *pchip, int last_level,
				int level)
{
	unsigned int val = 0;
	int ret;
	...
	ret = regmap_read(pchip->regmap, KTD3136_FLAGS_SETTING, &val);
	if (ret) {
...
int ktd3136_set_backlight(unsigned int bl_level)
{
	...
	if (level > 0) {
		...
		if (ret < 0) {
			...
			LCD_KIT_ERR(
				"write ktd3136 backlight level lsb:0x%x fail\n",
				bl_lsb);
		}
		...
		if (ret < 0) {
			lcd_backlight_i2c_dmd();
			LCD_KIT_ERR(
				"write ktd3136 backlight level msb:0x%x fail\n",
				bl_msb);
		}
	}
