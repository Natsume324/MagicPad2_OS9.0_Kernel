...
static int aw99703_config_write(struct aw99703_chip_data *pchip,
				...
				unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val[i] != AW99703_INVALID_VAL) {
			...
			LCD_KIT_INFO("aw99703 write 0x%x = 0x%x", reg[i],
				     val[i]);
			if (ret < 0) {
...
static int aw99703_config_read(struct aw99703_chip_data *pchip,
			       unsigned int reg[], unsigned int val[],
			       unsigned int size)
{
	int ret = 0;
...
static ssize_t aw99703_reg_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	ret = aw99703_config_read(pchip, aw99703_reg_addr, aw99703_reg_info,
				  AW99703_RW_REG_MAX);
	if (ret < 0) {
	...
	ret = snprintf(buf, PAGE_SIZE, "0x67 = 0x%x,0x23 = 0x%x,0x25 = 0x%x\n \
		...
		\r0x05 = 0x%x,0x02 = 0x%x\n",
		       aw99703_reg_info[0], aw99703_reg_info[1],
		       aw99703_reg_info[2], aw99703_reg_info[3],
		       aw99703_reg_info[4], aw99703_reg_info[5],
		       aw99703_reg_info[6], aw99703_reg_info[7],
		       aw99703_reg_info[8], aw99703_reg_info[9],
		       aw99703_reg_info[10], aw99703_reg_info[11],
		       aw99703_reg_info[12], aw99703_reg_info[13],
		       aw99703_reg_info[14]);
	return ret;
}

static ssize_t aw99703_reg_store(struct device *dev,
...
static void aw99703_disable(void)
{
	...
	ret = regmap_write(aw99703_g_chip->regmap, AW99703_REG_RESET,
			   AW99703_REG_RESET);
	if (ret < 0)
...
static void aw99703_check_fault(struct aw99703_chip_data *pchip, int last_level,
				int level)
{
	unsigned int val1 = 0;
	unsigned int val2 = 0;
	...
	ret = regmap_read(pchip->regmap, AW99703_FLAGS1_SETTING, &val1);
	if (ret) {
	...
	ret = regmap_read(pchip->regmap, AW99703_FLAGS2_SETTING, &val2);
	if (ret) {
