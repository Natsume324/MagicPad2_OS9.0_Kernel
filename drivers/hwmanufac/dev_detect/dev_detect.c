				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	return snprintf_s((char *)buf, PRINT_MAX_LEN, PRINT_MAX_LEN - 1, "%s\n",
			  dev_detect_result);
}
...
static struct detect_flag_device dev_list[] = {
	{.name = "dev_detect_result_flag",
	 .index = 0,
	 .attr = __ATTR(dev_detect_result_flag, 0444, dev_detect_result_show,
			NULL)},
	{.name = "dev_detect_dbc_flag",
	 .index = 0,
	 .attr = __ATTR(dev_detect_dbc_flag, 0444, dbc_detect_flag_show, NULL)},
	{.name = "dev_detect_rt_flag",
	 .index = 0,
	 .attr = __ATTR(dev_detect_rt_flag, 0444, rt_detect_flag_show, NULL)},
	{.name = "dev_detect_mmi1_flag",
	 .index = 0,
	 .attr = __ATTR(dev_detect_mmi1_flag, 0444, mmi1_detect_flag_show,
			NULL)},
	{.name = "dev_detect_mmi2_flag",
	 .index = 0,
	 .attr = __ATTR(dev_detect_mmi2_flag, 0444, mmi2_detect_flag_show,
			NULL)},
};
...
static int create_dev_detect_sysfs(struct class *myclass)
{
	...
	for (dev_index = 0; dev_index < ARRAY_SIZE(dev_list); dev_index++) {
		dev_list[dev_index].dev = device_create(
			myclass, NULL, MKDEV(0, dev_list[dev_index].index),
...
static void get_dev_detect_dts_config(const struct device_node *np)
{
	...
	for (i = 0; i < ARRAY_SIZE(dev_detect_table); i++) {
		result = of_property_read_u32(
			np, dev_detect_table[i].device_name, &type);
		if (!result) {
			dbc_detect_flag[i] =
	...
	.driver =
		{
			...
			.of_match_table =
				of_match_ptr(device_detect_match_table),
		},
