				...
				struct hyperhold_entry *io_entry)
{
	...
	if (!hyperhold_space_should_merge_ext(segment, io_entry))
		return false;
#endif
...
static int hyperhold_submit_bio(struct hyperhold_segment *segment)
{
	...
	struct bio *bio = NULL;
	struct block_device *bdev = segment->req->io_para.bdev;
...
#ifdef CONFIG_HYPERHOLD_FILE_ARCHIVAL
	bdev = hyperhold_space_get_bio_bdev(segment);
	if (!bdev) {
		bio_put(bio);
		hh_print(HHLOG_ERR, "get_bio_bdev error\n");
		return -EIO;
	}
#endif
...
#ifdef CONFIG_HYPERHOLD_FILE_ARCHIVAL
		if (hyperhold_space_need_encrypt(segment))
#endif
