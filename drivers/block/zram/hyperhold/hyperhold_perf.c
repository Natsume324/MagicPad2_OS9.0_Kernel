...
static unsigned long warning_threshold[HYPERHOLD_SCENARIO_BUTT] = {0, 2000,
								   5000, 0};

const char *key_point_name[HYPERHOLD_KYE_POINT_BUTT] = {
	"START",     "INIT",	      "IOENTRY_ALLOC", "FIND_EXTENT",
	"IO_EXTENT", "SEGMENT_ALLOC", "BIO_ALLOC",     "SUBMIT_BIO",
	"END_IO",    "SCHED_WORK",    "END_WORK",      "CALL_BACK",
	"WAKE_UP",   "ZRAM_LOCK",     "DONE"};

static void hyperhold_dump_point_lat(struct hyperhold_key_point_record *record,
				     ktime_t start)
{
	int i;
...
static void
hyperhold_dump_no_record_point(struct hyperhold_key_point_record *record,
...
static void hyperhold_dump_lat(struct hyperhold_key_point_record *record,
			       ktime_t curr_time, bool perf_end_flag)
{
	...
	if (perf_end_flag)
		...
		count += snprintf(log + count, (size_t)(DUMP_BUF_LEN - count),
				  " state %c",
				  task_state_to_char(record->task));
...
hyperhold_perf_warning_threshold(enum hyperhold_scenario scenario)
{
...
static void
hyperhold_perf_init_monitor(struct hyperhold_key_point_record *record,
...
static void
hyperhold_perf_stop_monitor(struct hyperhold_key_point_record *record)
{
	if (!record->warning_threshold)
		return;
...
void hyperhold_perf_start_proc(struct hyperhold_key_point_record *record,
			       enum hyperhold_key_point type, ktime_t curr_time,
			       unsigned long long current_ravg_sum)
{
	struct hyperhold_key_point_info *key_point = &record->key_point[type];
...
void hyperhold_perf_end_proc(struct hyperhold_key_point_record *record,
			     enum hyperhold_key_point type, ktime_t curr_time,
			     unsigned long long current_ravg_sum)
{
	struct hyperhold_key_point_info *key_point = &record->key_point[type];
			       ...
			       unsigned long long start_ravg_sum)
{
	...
	hyperhold_perf_end_proc(record, type, ktime_get(), current_ravg_sum);
...
void hyperhold_perf_lat_stat(struct hyperhold_key_point_record *record)
{
	struct hyperhold_stat *stat = hyperhold_get_stat_obj();
	s64 curr_lat;
	/* reclaim_in: 2s, fault_out: 100ms, batch_out: 500ms, pre_out: 2s */
	s64 timeout_value[HYPERHOLD_SCENARIO_BUTT] = {2000000, 100000, 500000,
						      2000000};
	...
	curr_lat =
		ktime_us_delta(record->key_point[HYPERHOLD_DONE].first_time,
...
void hyperhold_perf_end(struct hyperhold_key_point_record *record)
{
	...
	hyperhold_dump_lat(record, record->key_point[HYPERHOLD_DONE].first_time,
			   true);
}
