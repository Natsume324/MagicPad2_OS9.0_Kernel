...
static char scenario_name[HYPERHOLD_SCENARIO_BUTT][SCENARIO_NAME_LEN] = {
	"reclaim_in", "fault_out", "batch_out", "pre_out"};

#ifdef CONFIG_HYPERHOLD_GKI
static ssize_t hyperhold_stats_show(struct hyperhold_stat *stat, char *buf,
				    ssize_t buf_size, ssize_t off)
{
	...
	off += scnprintf(buf + off, buf_size - off, "%d:%lld\n",
			 ...
			 atomic64_read(&stat->reclaimin_infight));
	...
	off += scnprintf(buf + off, buf_size - off, "%d:%lld\n",
			 ...
			 atomic64_read(&stat->batchout_inflight));

	return off;
}

static ssize_t hyperhold_area_info_show(struct hyperhold_stat *stat, char *buf,
					ssize_t buf_size, ssize_t off)
{
	off += scnprintf(buf + off, buf_size - off, "%d:%lld\n", ENUM_FAULT_CNT,
	...
	off += scnprintf(buf + off, buf_size - off, "%d:%lld MB\n",
			 ...
			 atomic64_read(&stat->zram_stored_size));
	off += scnprintf(buf + off, buf_size - off, "%d:%lld MB\n",
			 ENUM_STORED_PAGES,
...
static ssize_t hyperhold_fail_show(struct hyperhold_stat *stat, char *buf,
				   ssize_t buf_size, ssize_t off)
{
	int i;
	...
	for (i = 0; i < HYPERHOLD_SCENARIO_BUTT; ++i) {
		off += scnprintf(buf + off, buf_size - off, "%d_%s:%lld\n",
				 ENUM_IO_FAIL_CNT, scenario_name[i],
				 atomic64_read(&stat->io_fail_cnt[i]));
		off += scnprintf(buf + off, buf_size - off, "%d_%s:%lld\n",
				 ENUM_ALLOC_FAIL_CNT, scenario_name[i],
				 atomic64_read(&stat->alloc_fail_cnt[i]));
	}
	return off;
}

static ssize_t hyperhold_extent_day_show(struct hyperhold_stat *stat, char *buf,
					 ssize_t buf_size, ssize_t off)
{
	static int count = 0;
	...
	off += scnprintf(buf + off, buf_size - off, "%d:%lld\n",
			 ENUM_DAILY_EXT_MIN,
			 atomic64_read(&stat->daily_ext_min));
	off += scnprintf(buf + off, buf_size - off, "%d:%lld\n", ENUM_MISS_FREE,
			 atomic64_read(&stat->miss_free));
	off += scnprintf(buf + off, buf_size - off, "%d:%lld\n",
...
static ssize_t hyperhold_lat_show(struct hyperhold_stat *stat, char *buf,
				  ssize_t buf_size, ssize_t off)
{
	int i;
...
static ssize_t hyperhold_in_size_day_show(struct hyperhold_stat *stat,
...
static ssize_t hyperhold_out_size_day_show(struct hyperhold_stat *stat,
					   char *buf, ssize_t buf_size,
					   ssize_t off)
{
...
#else
static void hyperhold_lat_show(struct seq_file *m, struct hyperhold_stat *stat)
{
	int i;
				 ...
				 struct hyperhold_stat *stat)
{
	seq_printf(m, "hyperhold_out_times: %lld\n",
		   atomic64_read(&stat->reclaimin_cnt));
	seq_printf(m, "hyperhold_out_time: %lld us\n",
	...
	seq_printf(m, "hyperhold_in_time: %lld us\n",
...
static void hyperhold_fail_show(struct seq_file *m, struct hyperhold_stat *stat)
{
	int i;
				      ...
				      struct hyperhold_stat *stat)
{
	...
	if (false) {
		...
		if (BETA_USER_READ == count) {
			...
			seq_printf(m, "hyperhold_extent_cnt_day: %ld\n", 0L);
		}
	} else {
		seq_printf(m, "hyperhold_extent_cnt_day: %lld\n",
			   (atomic64_read(&stat->daily_ext_max) -
				       ...
				       struct hyperhold_stat *stat)
{
	...
	if (false) {
		...
		if (BETA_USER_READ == in_count) {
			...
			seq_printf(m, "hyperhold_in_size_day: %ld\n", 0L);
		}
	} else {
		seq_printf(m, "hyperhold_in_size_day: %lld\n",
			   (atomic64_read(&stat->batchout_bytes) -
					...
					struct hyperhold_stat *stat)
{
	...
	if (false) {
		...
		if (BETA_USER_READ == out_count) {
			...
			seq_printf(m, "hyperhold_out_size_day: %ld\n", 0L);
		}
	} else {
		seq_printf(m, "hyperhold_out_size_day: %lld\n",
			   (atomic64_read(&stat->reclaimin_bytes) -
...
#ifdef CONFIG_HYPERHOLD_GKI
unsigned long long
hyperhold_read_mcg_stats_internal(struct mem_cgroup_ext *mcg_ext,
				  enum hyperhold_mcg_member mcg_member)
{
	...
	case MCG_SWAPOUT_CNT:
		...
		break;
	case MCG_SWAPOUT_SZ:
		val = atomic64_read(&mcg_ext->hyperhold_outextcnt)
		      << EXTENT_SHIFT;
		break;
	case MCG_SWAPIN_CNT:
		...
		break;
	case MCG_SWAPIN_SZ:
		val = atomic64_read(&mcg_ext->hyperhold_inextcnt)
		      << EXTENT_SHIFT;
		break;
	case MCG_DISK_SPACE:
		...
		if (extcnt < 0)
			extcnt = 0;
		val = ((unsigned long long)extcnt)
		      << EXTENT_SHIFT; /*lint !e571*/
		break;
...
static void
hyperhold_fail_record_get(struct hyperhold_fail_record_info *record_info)
{
	struct hyperhold_stat *stat = NULL;
	unsigned long flags;
...
ssize_t hyperhold_report_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
