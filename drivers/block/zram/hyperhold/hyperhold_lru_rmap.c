...
{
	unsigned int mcg_id = mcg ? memcg_ext_id_from_memcg_ext(mcg) : 0;
#else
void zram_set_memcg(struct zram *zram, u32 index, struct mem_cgroup *mcg)
{
	unsigned short mcg_id = mcg ? mcg->id.id : 0;
#endif
	hh_list_set_mcgid(obj_idx(zram->area, index), zram->area->obj_table,
			  mcg_id);
}
...
struct mem_cgroup_ext *zram_get_memcg(struct zram *zram, u32 index)
{
	...
	memcg_ext_id = hh_list_get_mcgid(obj_idx(zram->area, index),
					 zram->area->obj_table);
	return memcg_ext_from_memcg_ext_id(memcg_ext_id);
}
#else
...
int zram_get_memcg_coldest_index(struct hyperhold_area *area,
				 struct mem_cgroup_ext *mcg, int *index,
				 int max_cnt)
{
	int cnt = 0;
	unsigned int memcg_ext_id = 0;
	...
	memcg_ext_id = memcg_ext_id_from_memcg_ext(mcg);
	hh_lock_list(mcg_idx(area, memcg_ext_id), area->obj_table);
	hh_list_for_each_entry_reverse_safe(i, tmp, mcg_idx(area, memcg_ext_id),
					    area->obj_table)
	{ /*lint !e666*/
		if (i >= (u32)area->nr_objs) {
			hh_print(HHLOG_ERR,
				 "index = %d invalid, nr_objs = %d\n", i,
				 area->nr_objs);
	...
	hh_unlock_list(mcg_idx(area, memcg_ext_id_from_memcg_ext(mcg)),
		       area->obj_table);
...
#else
int zram_get_memcg_coldest_index(struct hyperhold_area *area,
				 struct mem_cgroup *mcg, int *index,
				 int max_cnt)
{
	int cnt = 0;
	u32 i, tmp;
	...
	hh_list_for_each_entry_reverse_safe(i, tmp, mcg_idx(area, mcg->id.id),
					    area->obj_table)
	{ /*lint !e666*/
		if (i >= (u32)area->nr_objs) {
			hh_print(
				HHLOG_ERR,
				"index = %d invalid, mcg_id = %d, nr_objs = %d\n",
				i, mcg->id.id, area->nr_objs);
#ifdef CONFIG_HONOR_DEBUG
			panic("mem error");
#endif
			continue;
...
int zram_rmap_get_extent_index(struct hyperhold_area *area, int ext_id,
			       int *index)
{
	int cnt = 0;
	u32 i;
	...
	hh_list_for_each_entry(i, ext_idx(area, ext_id), area->obj_table)
	{ /*lint !e666*/
		/*lint -e548*/
		if (cnt >= (int)EXTENT_MAX_OBJ_CNT) {
			hh_print(HHLOG_ERR, "ext_id = %d\n", ext_id);
			WARN_ON_ONCE(1);
#ifdef CONFIG_HONOR_DEBUG
			panic("mem error");
#endif
...
#ifdef CONFIG_HONOR_DEBUG
	if (zram_get_memcg(zram, index)) {
		hh_print(HHLOG_ERR, "index = %u\n", index);
		panic("mem error");
	}
#endif
...
#ifdef CONFIG_HYPERHOLD_GKI
	memcg_id = memcg_ext_id_from_memcg_ext(mcg);
#else
	memcg_id = mcg->id.id;
#endif
	hh_list_add(obj_idx(zram->area, index), mcg_idx(zram->area, memcg_id),
		    zram->area->obj_table);
...
#endif
{
	...
	hh_list_add_tail(obj_idx(zram->area, index),
			 mcg_idx(zram->area, memcg_id), zram->area->obj_table);
...
void zram_lru_del(struct zram *zram, u32 index)
{
	...
	memcg_id = mcg->id.id;
#endif
	hh_list_del(obj_idx(zram->area, index), mcg_idx(zram->area, memcg_id),
		    zram->area->obj_table);
...
void zram_rmap_insert(struct zram *zram, u32 index)
{
	...
	hh_list_add_tail(obj_idx(zram->area, index),
			 ext_idx(zram->area, ext_id), zram->area->obj_table);
}
...
#endif
	ext_id = esentry_extid(eswpentry);
	hh_list_del(obj_idx(zram->area, index), ext_idx(zram->area, ext_id),
		    zram->area->obj_table);
}
