 ...
 *
 * Author:	Wang Cheng Ke <wangchengke2.>
 *		Wang Xiao Yuan <wangxiaoyuan6.>
 *
 * Create: 2021-4-8
 *
 */
...
int hyperhold_cache_read_obj(struct hyperhold_cache *cache, unsigned long addr,
			     u8 *dst, int size)
{
...
repeat:
	...
	if (pg_off + size <= PAGE_SIZE) {
		...
		memcpy(dst + PAGE_SIZE - pg_off, src,
		       size + pg_off - PAGE_SIZE);
		put_page(page2);
	}
	return 0;
err_out:
