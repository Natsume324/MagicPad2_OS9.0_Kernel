...
static bool argo_is_timeout_retrans(const struct tcp_sock *tp)
{
	...
	if (rtt_ms < (ARGO_RTO_MIN >> 1)) {
		if (jiffies_to_msecs(TCP_TIME_STAMP -
				     tp->argo->rcv_nxt_jiffies) > ARGO_RTO_MIN)
			return true;
	} else {
		if (tp->argo->retrans_tsval[ARRAY_SIZE(tp->argo->retrans_tsval) -
					    1])
			return true;
	}
			      ...
			      const struct sk_buff *skb)
{
	...
	ARGO_LOGD("Argo time to init dest addr: %x, dest port: %u, "
		  ...
		  ((const struct sock *)tp)->sk_dport, sysctl_tcp_argo,
		  skb->dev->name, tp->argo, tp->rx_opt.tstamp_ok,
	...
	if (!tp->argo->retrans_tsval[0]) {
		...
		for (i = 1; i < nums; i++)
			if (!tp->argo->retrans_tsval[i] &&
				...
				tp->argo->retrans_tsval[i] =
					TCP_TIME_STAMP + tp->tsoffset;
	}
}
...
static void argo_check_seq(struct sock *sk, struct tcp_skb_cb *tcb)
{
	...
	if (!tp->argo->high_sacked ||
		...
		tp->argo->high_sacked = tcb->end_seq;
		   ...
		   !before(tp->rx_opt.rcv_tsecr, tp->argo->retrans_tsval[0])) {
...
void argo_calc_high_seq(struct sock *sk, struct sk_buff *skb)
{
	...
	ARGO_LOGD("Argo Cacl high seq. dest addr: %x, dest port: %u, "
		  ...
		  sk->sk_daddr, sk->sk_dport, tcb->seq, tcb->end_seq,
		  tp->argo->delay_ack_nums, tp->argo->disable_argo,
		  tp->argo->snd_high_seq, tp->argo->high_sacked, tp->rcv_nxt,
		  tp->rx_opt.rcv_tsval, tp->argo->snd_high_tsval,
		  tp->rx_opt.rcv_tsecr, tp->argo->snd_high_tsecr,
	...
	if (tp->rcv_nxt == tcb->seq) {
		...
		(void)memset_s(tp->argo->retrans_tsval,
			       sizeof(tp->argo->retrans_tsval), 0x00,
...
void argo_calc_delay_ack_nums(struct sock *sk, u32 seq, u32 end_seq)
{
...
#else
	is_skb_queue_empty =
		skb_queue_empty((struct sk_buff_head *)&tp->out_of_order_queue);
#endif
	...
	ARGO_LOGD("Argo Calc delay ack. dest addr: %x, dest port: %u, "
		  ...
		  "disable_argo: %u",
		  sk->sk_daddr, sk->sk_dport, seq, end_seq,
		  tp->argo->snd_high_seq, tp->rcv_nxt, is_skb_queue_empty,
	...
	} else if (tp->argo->disable_argo && is_skb_queue_empty) {
...
int argo_delay_acks_in_fastpath(struct sock *sk, struct tcp_sock *tp)
{
	...
	if (tp->argo && sysctl_tcp_argo && !tp->rx_opt.num_sacks &&
		...
		ARGO_LOGD("Argo fast path delay ack, "
			  ...
			  "argo high_snd_nxt: %u",
			  sk->sk_daddr, sk->sk_dport, tp->argo, sysctl_tcp_argo,
			  tp->rx_opt.num_sacks, tp->argo->delay_ack_nums,
			  tp->snd_nxt, tp->argo->high_snd_nxt);

		if (tp->snd_nxt == tp->argo->high_snd_nxt) {
			ARGO_LOGI("Argo reduce timestamp, "
				  "dest addr: %x, dest port: %u",
				  sk->sk_daddr, sk->sk_dport);
			reduce = tp->rx_opt.ts_recent -
				 tp->argo->snd_retrans_stamp + 1;
