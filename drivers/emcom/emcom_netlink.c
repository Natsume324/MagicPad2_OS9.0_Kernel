...
void emcom_send_msg2daemon(int cmd, const void *data, int len)
{
	...
	if (data && (len > 0)) {
		if (memcpy_s((void *)nlmsg_data(nlh), len, (const void *)data,
			     len) != EOK) {
			kfree_skb(pskb_out);
			return;
		}
...
static void emcom_common_evt_proc(const struct nlmsghdr *nlh,
				  const uint8_t *data, uint16_t len)
{
	...
	case NETLINK_EMCOM_DK_REG:
		...
		EMCOM_LOGD(
			"emcom netlink receive reg packet: g_user_space_pid = %d",
			nlh->nlmsg_pid);
#ifdef CONFIG_HONOR_NWEVAL
...
static void kernel_emcom_receive(struct sk_buff *__skb)
{
	...
	if (skb->len >= NLMSG_HDRLEN) {
		...
		if ((nlh->nlmsg_len >= sizeof(struct nlmsghdr)) &&
			...
			submod = (nlh->nlmsg_type & EMCOM_SUB_MOD_MASK) >>
				 EMCOM_SUB_MOD_MASK_LEN;
			switch (submod) {
			...
			case EMCOM_SUB_MOD_XENIGE:
				emcom_xengine_evt_proc(nlh->nlmsg_type, packet,
						       data_len);
				break;
#endif
#ifdef CONFIG_HN_NETWORK_MEASUREMENT
			case EMCOM_SUB_MOD_SMARTCARE:
				smartcare_event_process(nlh->nlmsg_type, packet,
							data_len);
				break;
#endif
#ifdef CONFIG_HONOR_NWEVAL
			case EMCOM_SUB_MOD_NWEVAL:
				nweval_event_process(nlh->nlmsg_type, packet,
						     data_len);
				break;
#endif
#ifdef CONFIG_HN_EMCOM_NSTACK
			case EMCOM_SUB_MOD_NSTACK:
				nstack_event_process(nlh->nlmsg_type, packet,
						     data_len);
				break;
#endif
			default:
				EMCOM_LOGI(
					"emcom netlink unsupport subMod, the subMod is %d",
					submod);
				break;
			}
		}
...
static int emcom_netlink_thread(void *data)
{
	...
	while (1) {
		...
		do {
			...
			if (skb) {
				if (g_user_space_pid)
					netlink_unicast(g_emcom_nlfd, skb,
							g_user_space_pid,
							MSG_DONTWAIT);
				else
					kfree_skb(skb);
...
static void emcom_netlink_init(void)
{
	...
	g_emcom_nlfd =
		netlink_kernel_create(&init_net, NETLINK_EMCOM, &emcom_nl_cfg);
	if (!g_emcom_nlfd)
	...
	g_emcom_netlink_task =
		kthread_run(emcom_netlink_thread, NULL, "emcom_netlink_thread");
