...
static void cal_rtt_indices(int32_t signal_strength, int8_t *traffic_index,
			    int8_t *stability_index);
static void nweval_report_indices(sub_indices_report *pReport);

void nweval_init(void)
{
	g_network_type = NETWORK_TYPE_UNKNOWN;
	(void)memset_s(g_rtt_array, sizeof(g_rtt_array), 0,
		       sizeof(g_rtt_array));
	g_rtt_index = 0;
...
{
	EMCOM_LOGD(" : nweval_start_rtt_report with network type %d",
		   network_type);
	if (!VALIDATE_NETWORK_TYPE(network_type)) {
		EMCOM_LOGE(
			" : illegal network type received in nweval_start_rtt_report");
		return;
	}
...
in nweval_trigger_rtt_report",
			   signal_strength);
		return;
	}

	/* calculate the traffic sub-index and stability sub-index */
	EMCOM_LOGD(" : rtt report triggered with signal strength %d",
		   signal_strength);
...
static void nweval_report_indices(sub_indices_report *report)
{
	...
	if (report == NULL) {
		EMCOM_LOGE(
			" : null report pointer in nweval_report_indices! abort report");
		return;
	}
			    ...
			    int8_t *stability_index)
{
	...
	if ((traffic_index == NULL) || (stability_index == NULL)) {
		EMCOM_LOGE(" : NULL input %s pointer in cal_rtt_indices",
			   (traffic_index == NULL) ? "traffic index" :
						     "stability index");
		return;
	}
	/* make use of a copy so that the array won't be changed by other thread */
	if (memcpy_s(temp_rtt_array, sizeof(temp_rtt_array), g_rtt_array,
		     sizeof(g_rtt_array)) != EOK) {
	...
	for (index = 0; index < RTT_COUNT; index++) {
		if ((temp_rtt_array[index] >= MIU_SECOND_TO_MILLI_SECOND) &&
			...
			rtt_array[rtt_count] = temp_rtt_array[index] /
					       MIU_SECOND_TO_MILLI_SECOND;
			rtt_count++;
		}
	}
	...
	if (rtt_count < ADEQUATE_RTT) {
		EMCOM_LOGD(
			" : failed to obtain RTTs, default sub-index will be reported");
	...
	EMCOM_LOGD(
		" : ready to cal_rtt_indices, rtt array length %d, first 3 RTTs %d, %d, %d,",
		rtt_count, rtt_array[0], rtt_array[1], rtt_array[2]);
	cal_statistics(rtt_array, rtt_count, &rtt_avg, &rtt_sqrdev);
	EMCOM_LOGD(" : statistics calculated, rtt avg %d, rtt sqrdev %d",
		   rtt_avg, rtt_sqrdev);
	*traffic_index = cal_traffic_index(g_network_type, signal_strength,
					   rtt_array, rtt_count);
	*stability_index =
		cal_rtt_stability_index(g_network_type, rtt_avg, rtt_sqrdev);
	EMCOM_LOGD(
		" : sub indices calculated, traffic index %d, rtt stability index %d",
...
void nweval_event_process(int32_t event, uint8_t *pdata, uint16_t len)
{
	...
	case NETLINK_EMCOM_DK_NWEVAL_START_RTT_REPORT:
		...
		if ((pdata == NULL) || (len != sizeof(int32_t))) {
			EMCOM_LOGE(
				" : null pointer or illegal data length %d in \
