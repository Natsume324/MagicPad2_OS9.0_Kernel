...
/* constant arrays */
static const int32_t TRAFFIC_SS_SEGMENT_ENDS[TRAFFIC_SS_SEGMENT_ENDPOINTS] = {
	-100, -60, -40, 0};
...
static const point_set
	WIFI_REFERENCE_PROBABILITY_PERCENT_SETS[TRAFFIC_SS_SEGMENTS] = {
		{{100, 0, 0},
		 {50, 40, 10},
		 {0, 20, 80}}, // good, normal, bad respectively
...
static const int32_t
	STABILITY_RTT_SEGMENT_ENDS[STABILITY_RTT_SEGMENT_ENDPOINTS] = {
		0, 80, 150, 200,
		10000}; // all network types share this rtt segment setting
static const int32_t
	STABILITY_RTT_FINE_SQRDEV_SCALS[STABILITY_RTT_FINE_STDEV_ENDPOINTS] = {
		160, 300, 500, 2500,
		10000}; // formally used stdev = {1, 10, 20, 50, 100}
			 ...
			 int32_t *rtt_array, int8_t rtt_array_length)
{
	...
	point current_point;
	int index;
	if (!VALIDATE_NETWORK_TYPE(network_type) || (signal_strength >= 0) ||
	    (rtt_array == NULL) || (rtt_array_length <= 0)) {
	...
	case NETWORK_TYPE_WIFI:
		reference_probability_set =
			(point_set *)&WIFI_REFERENCE_PROBABILITY_PERCENT_SETS
				[signal_strength_interval];
		break;
	case NETWORK_TYPE_4G:
		reference_probability_set =
			(point_set *)&LTE_REFERENCE_PROBABILITY_PERCENT_SETS
				[signal_strength_interval];
		break;
	case NETWORK_TYPE_3G:
		reference_probability_set =
			(point_set *)&OTHERS_REFERENCE_PROBABILITY_PERCENT_SETS
				[signal_strength_interval];
		break;
	case NETWORK_TYPE_2G:
		...
		return INVALID_VALUE;
	default:
		EMCOM_LOGE(" : illegal network type in cal_traffic_index, %d",
			   network_type);
		return INVALID_VALUE;
	}
	/* convert probability percent to count */
	reference_point_set = convert_probability_to_count(
		reference_probability_set, rtt_array_length);
	...
	current_point = get_space_point(rtt_array, rtt_array_length,
					TRAFFIC_RTT_SEGMENT_ENDS,
					RTT_POINT_ENDPOINTS);

	/* calculate Euclidean distance between current point and reference points */
	distance_squares[LEVEL_GOOD] = cal_euclidiean_distance_square(
		current_point, reference_point_set.good_point);
	distance_squares[LEVEL_NORMAL] = cal_euclidiean_distance_square(
		current_point, reference_point_set.normal_point);
	distance_squares[LEVEL_BAD] = cal_euclidiean_distance_square(
		current_point, reference_point_set.bad_point);
...
int8_t cal_rtt_stability_index(int8_t network_type, int32_t avg, int32_t sqrdev)
{
	...
	if (!VALIDATE_NETWORK_TYPE(network_type)) {
		EMCOM_LOGE(
			" : illegal parameter in cal_rtt_stability_index, network type %d",
			network_type);
		return INVALID_VALUE;
	}
	...
	} else if ((sqrdev > value_rtt_sqrdev_normal) &&
		   (sqrdev <= value_rtt_sqrdev_bad)) {
		      ...
		      const int32_t *segment_ends, int8_t endpoints)
{
	...
	int index1;
	int index2;
	if ((array == NULL) || (segment_ends == NULL) || (length <= 0) ||
	    (segments <= 0)) {
		EMCOM_LOGE(
			" : illegal parameter in get_probability_space_point");
		return generated_point;
	}

	count_in_segments =
		(int8_t *)kmalloc(segments * sizeof(int8_t), GFP_KERNEL);
	...
	if (POINT_DIMENSIONS == segments) {
		...
		for (index1 = INDEX_Z; index1 < segments; index1++)
			count_in_dimensions[INDEX_Z] +=
				count_in_segments[index1];
...
probably incorrect point dimension given");
	...
	EMCOM_LOGD(
		" : obtained the current point %d, %d, %d in %d dimensional space",
		generated_point.x, generated_point.y, generated_point.z,
		POINT_DIMENSIONS);
	return generated_point;
}
...
int32_t cal_euclidiean_distance_square(point point1, point point2)
{
	...
	if ((point1.x < 0) || (point1.y < 0) || (point1.z < 0) ||
		...
		EMCOM_LOGE(
			" : illegal point in cal_euclidiean_distance, point1.x = %d, \
point1.y = %d, point1.z = %d, point2.x = %d, point2.y = %d, point2.z = %d",
			point1.x, point1.y, point1.z, point2.x, point2.y,
			point2.z);
		return INVALID_VALUE;
	}
	...
	EMCOM_LOGD(
		" : calculated distance square between current point (%d, %d, %d) \
...
int8_t find_nearest_reference_point(int32_t *distance_squares, int8_t number)
{
	...
	for (index = 0; index < number; index++) {
		if (distance_squares[index] < 0) {
			EMCOM_LOGE(
				" : illegal (negative) distance in find_nearest_reference_point");
			return INVALID_VALUE;
		}
	...
	for (index = number - 1; index >= 0; index--) {
		if (distance_squares[index] == min) {
			...
			EMCOM_LOGD(
				" : found min distance square %d, nearest reference point %d",
				min, index);
...
*/
void cal_statistics(const int32_t *array, const int8_t length, int32_t *avg,
		    int32_t *sqrdev)
{
	...
	if ((array == NULL) || (length <= 0)) {
		EMCOM_LOGE(
			" : illegal parameter in cal_statistics, length : %d",
			length);
		return;
	}
	...
	for (index = 0; index < length; index++) {
		if ((array[index] <= SIGNAL_ABNORMAL_THRESHOLD) ||
			...
			EMCOM_LOGD(
				" : illegal data (index %d) %d in average value calculation",
				index, array[index]);
			actual_length--;
			continue;
		}
	...
	for (index = 0; index < length; index++) {
		if ((array[index] <= SIGNAL_ABNORMAL_THRESHOLD) ||
		    (array[index] >= RTT_ABNORMAL_THRESHOLD)) {
			EMCOM_LOGE(
				" : illegal data (index %d) %d in deviation calculation",
				index, array[index]);
				       ...
				       int8_t data_length)
{
	...
	POINT_CONVERT_PERCENT_TO_COUNT(
		count_set.good_point, probability_set->good_point, data_length);
	...
	POINT_CONVERT_PERCENT_TO_COUNT(count_set.bad_point,
				       probability_set->bad_point, data_length);

	EMCOM_LOGD(
		" : through conversion, the reference points are : good (%d, %d, %d), \
normal (%d, %d, %d), bad (%d, %d, %d)",
		count_set.good_point.x, count_set.good_point.y,
		count_set.good_point.z, count_set.normal_point.x,
		count_set.normal_point.y, count_set.normal_point.z,
		count_set.bad_point.x, count_set.bad_point.y,
		count_set.bad_point.z);
	return count_set;
}
