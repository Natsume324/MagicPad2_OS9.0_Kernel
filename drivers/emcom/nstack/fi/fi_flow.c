...
uint32_t fi_ipv4_flow_hash(__be32 addr1, __u16 port1, __be32 addr2,
			   __be16 port2, char *flow, int str_len, bool exchange)
{
	int ret;
	...
	if (exchange) {
		if (flow != NULL) {
			ret = sprintf_s(flow, str_len, "%u%u%u%u", addr2, port2,
					addr1, port1);
			if (ret == -1) {
		...
		return __inet_ehashfn(addr2, port2, addr1, port1,
				      fi_ipv4_flow_secret) &
		       FI_FLOW_TABLE_MASK;
	}
	if (flow != NULL) {
		ret = sprintf_s(flow, str_len, "%u%u%u%u", addr1, port1, addr2,
				port2);
		if (ret == -1) {
			fi_loge("sprintf_s flow err, ret %d", ret);
			flow = NULL;
	...
	return __inet_ehashfn(addr1, port1, addr2, port2, fi_ipv4_flow_secret) &
	       FI_FLOW_TABLE_MASK;
}
...
bool fi_ipv4_flow_same(struct fi_flow_node *node, __be32 saddr, __u16 sport,
		       __be32 daddr, __be16 dport)
{
	return (((node->ipv4_saddr == saddr) && (node->ipv4_daddr == daddr) &&
		 (node->sport == sport) && (node->dport == dport)) ||
		((node->ipv4_saddr == daddr) && (node->ipv4_daddr == saddr) &&
		 (node->sport == dport) && (node->dport == sport)));
}

struct fi_flow_node *fi_ipv4_flow_find(const struct fi_flow_head *head,
				       __be32 saddr, __u16 sport, __be32 daddr,
				       __be16 dport)
{
	...
	if (head != NULL) {
		list_for_each_entry(node, &(head->list), list) {
			if (fi_ipv4_flow_same(node, saddr, sport, daddr,
					      dport)) {
...
{
	int i;
	int ret = memset_s(&newnode->flow_ctx, sizeof(struct fi_flow_ctx), 0,
			   sizeof(struct fi_flow_ctx));
	if (ret) {
	...
	if (flow_cb != NULL && flow_cb->flow_identify_cfg != NULL) {
		for (i = 0; i < FI_DIR_BOTH; i++) {
			if (flow_cb->flow_identify_cfg->periodic[i]
				    .report_type == FI_RPT_TIME_CTRL) {
				int len =
					flow_cb->flow_identify_cfg->periodic[i]
						.roi_stop -
					flow_cb->flow_identify_cfg->periodic[i]
						.roi_start;
				if (len <= 0) {
					fi_logi("time interval config err]len=%d",
						len);
					flow_cb->flow_identify_cfg->periodic[i]
						.report_type = FI_RPT_OFF;
					return -1;
				}
				newnode->flow_ctx.periodic_ctrl[i].ts_start =
					jiffies +
					msecs_to_jiffies(
						flow_cb->flow_identify_cfg
							->periodic[i]
							.roi_start);
				newnode->flow_ctx.periodic_ctrl[i].ts_end =
					newnode->flow_ctx.periodic_ctrl[i]
						.ts_start +
					msecs_to_jiffies(len);
			}
			if (flow_cb->flow_identify_cfg->pkt[i].report_type ==
			    FI_RPT_TIME_CTRL) {
				int len = flow_cb->flow_identify_cfg->pkt[i]
						  .roi_stop -
					  flow_cb->flow_identify_cfg->pkt[i]
						  .roi_start;
				if (len <= 0) {
					fi_logi("time interval config err]len=%d",
						len);
				...
				newnode->flow_ctx.pkt_ctrl[i].ts_start =
					jiffies +
					msecs_to_jiffies(
						flow_cb->flow_identify_cfg
							->pkt[i]
							.roi_start);
				newnode->flow_ctx.pkt_ctrl[i].ts_end =
					newnode->flow_ctx.pkt_ctrl[i].ts_start +
					msecs_to_jiffies(len);
			}
		}
...
struct fi_flow_node *fi_ipv4_flow_add(struct fi_flow_head *head, int32_t uid,
				      __be32 saddr, __u16 sport, __be32 daddr,
				      __be16 dport, fi_flow_cb flow_cb,
				      struct sock *sk)
{
	struct fi_flow_node *newnode = NULL;
	int ret;
	if (head != NULL) {
		if (atomic_read(&g_fi_ipv4_flow.node_num) >
		    FI_FLOW_NODE_LIMIT) {
			fi_loge("IPV4 flow node out of limited %d",
				FI_FLOW_NODE_LIMIT);
			return NULL;
		}
		newnode = (struct fi_flow_node *)kzalloc(
			sizeof(struct fi_flow_node), GFP_ATOMIC);
		if (newnode == NULL) {
			fi_loge("failed to malloc new IPV4 node for uid:%d",
				uid);
			return NULL;
		}
		ret = memset_s(newnode, sizeof(struct fi_flow_node), 0,
			       sizeof(struct fi_flow_node));
		if (ret) {
			kfree(newnode);
			fi_loge("memset_s failed ret=%d", ret);
			return NULL;
		...
		if (sk != NULL && (sk->sk_protocol == IPPROTO_TCP ||
	...
	if (head != NULL && newflow != NULL) {
		...
		if (addflow) {
			if (sk && sk->sk_state >= TCP_FIN_WAIT1 &&
				...
				fi_logd("Ignore flow add:sk_state=%d",
					sk->sk_state);
				*newflow = false;
				return NULL;
			}
			*newflow = true;
			return fi_ipv4_flow_add(head, uid, saddr, sport, daddr,
						dport, flow_cb, sk);
		}
		*newflow = false;
	}
...
void fi_ipv4_flow_age(void)
{
	...
	for (i = 0; i < FI_FLOW_TABLE_SIZE; i++) {
		...
		list_for_each_entry_safe(node, tmp, &(head->list), list) {
			if (node->sk &&
			    ((node->sk->sk_protocol == IPPROTO_TCP &&
			      node->sk->sk_state >= TCP_FIN_WAIT1) ||
			     node->flow_ctx.flow_msg.dev_if !=
				     node->sk->sk_bound_dev_if ||
			     node->sk->sk_err == ECONNABORTED ||
			     sock_flag(node->sk, SOCK_DEAD))) {
				fi_logi("age free flow, node->uid=%d [%hu--%hu] uid=%d [%hu--%hu] protocol=%d sk_state=%d flag=%d",
					node->uid, node->sport, node->dport,
					node->flow_ctx.flow_msg.uid,
					node->flow_ctx.flow_msg.sport,
					node->flow_ctx.flow_msg.dport,
					node->sk->sk_protocol,
					node->sk->sk_state,
					sock_flag(node->sk, SOCK_DEAD));
				fi_ipv4_flow_clear_node(node);
			} else if (curtime - node->updatetime >
				   FI_FLOW_AGING_TIME) {
				fi_ipv4_flow_clear_node(node);
			}
		}
...
void fi_ipv4_flow_init(void)
{
	...
	ret = memset_s(&g_fi_ipv4_flow, sizeof(g_fi_ipv4_flow), 0,
		       sizeof(g_fi_ipv4_flow));
	if (ret) {
		fi_loge("memset_s failed ret=%d", ret);
		return;
	}
...
uint32_t fi_ipv6_flow_hash(const struct in6_addr *addr1, __u16 port1,
			   const struct in6_addr *addr2, __be16 port2,
			   char *flow, int str_len, bool exchange)
{
	int ret;
	...
	net_get_random_once(&fi_ipv6_flow_d_secret,
			    sizeof(fi_ipv6_flow_d_secret));
	if (exchange) {
		hash2 = (__force u32)addr2->s6_addr32[3];
		hash1 = __ipv6_addr_jhash(addr1, fi_ipv6_flow_d_secret);
		if (flow != NULL) {
			ret = sprintf_s(flow, str_len, "%u%u%u%u",
					(__force u32)addr2->s6_addr32[3], port2,
					(__force u32)addr1->s6_addr32[3],
					port1);
			if (ret == -1) {
				fi_loge("sprintf_s flow err, ret %d", ret);
				flow = NULL;
		...
		return __inet6_ehashfn(hash2, port2, hash1, port1,
				       fi_ipv6_flow_secret) &
		       FI_FLOW_TABLE_MASK;
	}
	...
	if (flow != NULL) {
		ret = sprintf_s(flow, str_len, "%u%u%u%u",
				(__force u32)addr1->s6_addr32[3], port1,
				(__force u32)addr2->s6_addr32[3], port2);
		if (ret == -1) {
			fi_loge("sprintf_s flow err, ret %d", ret);
			flow = NULL;
	...
	return __inet6_ehashfn(hash1, port1, hash2, port2,
			       fi_ipv6_flow_secret) &
	       FI_FLOW_TABLE_MASK;
}
...
bool fi_ipv6_flow_same(struct fi_flow_node *node, const struct in6_addr *saddr,
		       __u16 sport, const struct in6_addr *daddr, __be16 dport)
{
	return ((ipv6_addr_equal((const struct in6_addr *)(&(node->ipv6_saddr)),
				 saddr) &&
		 ipv6_addr_equal((const struct in6_addr *)(&(node->ipv6_daddr)),
				 daddr) &&
		 node->sport == sport && node->dport == dport) ||
		(ipv6_addr_equal((const struct in6_addr *)(&(node->ipv6_saddr)),
				 daddr) &&
		 ipv6_addr_equal((const struct in6_addr *)(&(node->ipv6_daddr)),
				 saddr) &&
		 node->sport == dport && node->dport == sport));
...
struct fi_flow_node *
fi_ipv6_flow_find(const struct fi_flow_head *head, const struct in6_addr *saddr,
		  __u16 sport, const struct in6_addr *daddr, __be16 dport)
{
	struct fi_flow_node *node = NULL;
	...
	if (saddr != NULL && daddr != NULL && head != NULL) {
		list_for_each_entry(node, &(head->list), list) {
			if (fi_ipv6_flow_same(node, saddr, sport, daddr,
					      dport)) {
				node->updatetime = jiffies_to_msecs(jiffies);
				return node;
			}
				      ...
				      struct sock *sk)
{
	...
	if (saddr != NULL && daddr != NULL && head != NULL) {
		if (atomic_read(&g_fi_ipv6_flow.node_num) >
		    FI_FLOW_NODE_LIMIT) {
			fi_loge("IPV6 flow node out of limited %d",
				FI_FLOW_NODE_LIMIT);
			return NULL;
		}
		newnode = (struct fi_flow_node *)kzalloc(
			sizeof(struct fi_flow_node), GFP_ATOMIC);
		if (newnode == NULL) {
			fi_loge("failed to malloc new IPV6 node for uid %d",
				uid);
			return NULL;
		}
		ret = memset_s(newnode, sizeof(struct fi_flow_node), 0,
				      ...
				      struct sock *sk)
{
	...
	if (saddr != NULL && daddr != NULL && head != NULL && newflow != NULL) {
		...
		if (addflow) {
			...
			*newflow = true;
			return fi_ipv6_flow_add(head, uid, saddr, sport, daddr,
						dport, flow_cb, sk);
		}
		*newflow = false;
	}
...
void fi_ipv6_flow_init(void)
{
	...
	ret = memset_s(&g_fi_ipv6_flow, sizeof(g_fi_ipv6_flow), 0,
		       sizeof(g_fi_ipv6_flow));
	if (ret) {
		fi_loge("memset_s failed ret=%d", ret);
		return;
	}
...
bool fi_payload_str_filter_check(struct fi_pkt_parse *pktinfo,
				 fi_rpt_cfg *rpt_cfg)
{
	int i;
	...
	for (i = 0; i < FI_FLOW_FILTER_STR_MAX; i++) {
		...
		if (filter_str_offset + rpt_cfg->filter_str_len[i] > payloadlen)
			continue;
		if (memcmp(rpt_cfg->filter_str[i], payload + filter_str_offset,
...
static void fi_flow_get_sk_info(struct fi_flow_node *flow)
{
	...
	if (flow->sk != NULL &&
	    ((flow->flow_ctx.flow_msg.dev_if != flow->sk->sk_bound_dev_if) ||
	     flow->sk->sk_err == ECONNABORTED ||
	     sock_flag(flow->sk, SOCK_DEAD))) {
		fi_logi("sock reset, port[%hu--%hu] l4proto[%d] sk_state[%d] dev_if[%d--%d] sk_err[%d] sk[%pK] flag[%d]",
			flow->flow_ctx.flow_msg.sport,
			flow->flow_ctx.flow_msg.dport,
			flow->flow_ctx.flow_msg.l4proto, flow->sk->sk_state,
			flow->flow_ctx.flow_msg.dev_if,
			flow->sk->sk_bound_dev_if, flow->sk->sk_err, flow->sk,
			sock_flag(flow->sk, SOCK_DEAD));
		flow->flow_ctx.flow_stat.sock_state = FLOW_RESET;
	} else {
		flow->flow_ctx.flow_stat.sock_state =
			flow->sk ? flow->sk->sk_state : 0;
	}
	...
	if (flow->sk != NULL && flow->sk->sk_protocol == IPPROTO_TCP) {
		...
		if (tp->snd_cwnd > in_flight)
			flow->flow_ctx.flow_stat.cwnd_left =
				tp->snd_cwnd - in_flight;
		else
			flow->flow_ctx.flow_stat.cwnd_left = 0;
		flow->flow_ctx.flow_stat.total_retrans =
			tp->total_retrans - flow->flow_ctx.last_total_retrans;
		flow->flow_ctx.last_total_retrans = tp->total_retrans;
		if (tp->bytes_received <
		    flow->flow_ctx.last_tcp_down_pkt_size) {
			tcp_down_pkt_byte = 0;
		} else {
			tcp_down_pkt_byte =
				tp->bytes_received -
		...
		flow->flow_ctx.flow_stat.tcp_down_pkt_byte =
			(u32)(tcp_down_pkt_byte);
		flow->flow_ctx.last_tcp_down_pkt_size = tp->bytes_received;
	}
}

static void fi_flow_raw_stat_update(struct fi_flow_node *flow,
				    ...
				    enum fi_dir dir)
{
	...
	if (dir == FI_DIR_UP) {
		flow_stat->raw_up_pkt_num +=
			pktinfo->is_gso ? pktinfo->raw_frag_num : 1;
...
static void fi_flow_stat_start(struct fi_flow_node *flow, enum fi_dir dir)
{
	flow->flow_ctx.stat_stop |= (dir == FI_DIR_UP) ?
					    FI_FLOW_PERIODIC_REPORT_NEED_UP :
					    FI_FLOW_PERIODIC_REPORT_NEED_DOWN;
}

static void fi_flow_stat_stop(struct fi_flow_node *flow, enum fi_dir dir)
{
	flow->flow_ctx.stat_stop |= (dir == FI_DIR_UP) ?
					    FI_FLOW_PERIODIC_REPORT_STOP_UP :
...
static void fi_flow_stat_update(struct fi_flow_node *flow,
				struct fi_pkt_parse *pktinfo, enum fi_dir dir)
{
	struct fi_flow_stat *flow_stat = &flow->flow_ctx.flow_stat;
	...
	if (dir == FI_DIR_UP) {
		flow_stat->up_pkt_num +=
			pktinfo->is_gso ? pktinfo->filter_frag_num : 1;
...
static void fi_flow_stat_echo_delay(struct fi_flow_node *flow,
				    struct fi_pkt_parse *pktinfo,
				    enum fi_dir dir)
{
	struct fi_flow_stat *flow_stat = &flow->flow_ctx.flow_stat;
	fi_flow_identify_cfg *flow_cfg = pktinfo->flow_cb.flow_identify_cfg;
	if (!flow_cfg->periodic[dir].opt_enable[FI_ECHO_DELAY] &&
	    !flow_cfg->pkt[dir].opt_enable[FI_ECHO_DELAY])
		return;
	...
	if (dir == FI_DIR_UP) {
		...
		if (flow->flow_ctx.last_up_timestamp) {
			flow_stat->up_down_diff_time = jiffies_to_msecs(
				jiffies - flow->flow_ctx.last_up_timestamp);
			pktinfo->msg.up_down_diff_time =
				flow_stat->up_down_diff_time;
...
static void fi_flow_stat_payload(struct fi_flow_node *flow,
				 struct fi_pkt_parse *pktinfo, enum fi_dir dir)
{
	int ret;
	uint32_t offset;
	...
	if (flow_cfg->periodic[dir].seg_begin >= payloadlen)
		return;
	offset = flow_cfg->periodic[dir].seg_end -
		 flow_cfg->periodic[dir].seg_begin + 1;
	if (flow_cfg->periodic[dir].seg_end >= payloadlen)
		offset = payloadlen - flow_cfg->periodic[dir].seg_begin;
	ret = memcpy_s(flow_stat->payload_seg[dir],
		       sizeof(flow_stat->payload_seg[dir]),
		       payload + flow_cfg->periodic[dir].seg_begin, offset);
	if (ret)
		fi_loge("memcpy_s failed ret=%d, len=%u", ret, offset);
	fi_logd("get period payload_seg len=%u, payloadlen=%d begin=%u end=%u",
		offset, payloadlen, flow_cfg->periodic[dir].seg_begin,
		flow_cfg->periodic[dir].seg_end);
}

void fi_flow_statistics_time_ctrl(struct fi_flow_node *flow,
				  struct fi_pkt_parse *pktinfo, enum fi_dir dir)
{
...
void fi_flow_statistics_num_ctrl(struct fi_flow_node *flow,
				 struct fi_pkt_parse *pktinfo, enum fi_dir dir)
{
	int num;
	fi_rpt_cfg *periodic_cfg =
		&pktinfo->flow_cb.flow_identify_cfg->periodic[dir];
			...
			enum fi_dir dir)
{
	...
	if (flow->flow_ctx.stat_stop &
	    ((dir == FI_DIR_UP) ? FI_FLOW_PERIODIC_REPORT_STOP_UP :
				  FI_FLOW_PERIODIC_REPORT_STOP_DOWN))
		return;

	if (pktinfo->msg.flow_msg.new_flow) {
		ret = memcpy_s(&flow->flow_ctx.flow_msg,
			       sizeof(struct fi_flow_msg),
			       &pktinfo->msg.flow_msg,
			       sizeof(struct fi_flow_msg));
		if (ret) {
...
bool fi_pkt_report_is_enable(struct fi_flow_node *flow,
			     struct fi_pkt_parse *pktinfo, enum fi_dir dir)
{
	bool enable = false;
	bool filter_pass = false;
	...
	if (dir == FI_DIR_UP &&
	    pkt_cfg->report_type == FI_RPT_DOWN_NUM_CTRL_BY_UP_PACKET &&
	    pktinfo->payloadlen > 0) {
		...
		flow->flow_ctx.expect_downlink_ack =
			pktinfo->msg.tcp_seq + pktinfo->msg.payloadlen;
		flow->flow_ctx.expect_downlink_seq = pktinfo->msg.tcp_ack;
	}

	if (flow->flow_ctx.stat_stop &
	    ((dir == FI_DIR_UP) ? FI_FLOW_PKT_REPORT_STOP_UP :
	...
	if (fi_payload_len_filter_check(pktinfo->msg.payloadlen,
					pkt_cfg->filter_pkt_size_start,
					pkt_cfg->filter_pkt_size_stop) &&
	...
	case FI_RPT_DOWN_NUM_CTRL_BY_UP_PACKET:
		if (filter_pass) {
			...
			if ((flow->flow_ctx.pkt_ctrl[dir].pkt_num >=
			     pkt_cfg->roi_start) &&
			    (flow->flow_ctx.pkt_ctrl[dir].pkt_num <=
			     pkt_cfg->roi_stop))
				enable = true;
			else if (flow->flow_ctx.pkt_ctrl[dir].pkt_num >
				 pkt_cfg->roi_stop)
				flow->flow_ctx.stat_stop |=
					((dir == FI_DIR_UP) ?
						 FI_FLOW_PKT_REPORT_STOP_UP :
				...
				void (*flow_unlock)(uint32_t))
{
	...
	/*lint -e574*/
	local_var.total_left = atomic_read(&flow->node_num) *
			       sizeof(struct fi_periodic_rpt_msg);
	if (!local_var.total_left) {
		fi_logd("no data need to do periodic report");
		return;
	}

	local_var.total_left =
		(local_var.total_left >
		 FI_NETLINK_REPORT_MAX_NUM * FI_MAX_ALIGN_SIZE) ?
			(FI_NETLINK_REPORT_MAX_NUM * FI_MAX_ALIGN_SIZE) :
			local_var.total_left;
	local_var.size = (local_var.total_left > FI_MAX_ALIGN_SIZE) ?
				 FI_MAX_ALIGN_SIZE :
				 local_var.total_left;
	...
	for (i = 0; i < FI_FLOW_TABLE_SIZE; i++) {
		...
		(*flow_lock)(i);
		list_for_each_entry_safe(node, tmp, &(flow->flow_table[i].list),
					 list) {
			if (node->flow_cb.flow_identify_cfg != NULL &&
			    ((node->flow_ctx.stat_stop &
			      FI_FLOW_PERIODIC_REPORT_NEED) ||
			     node->flow_cb.flow_identify_cfg->basic_rpt_en)) {
				if (node->flow_cb.flow_identify_cfg
					    ->basic_rpt_en)
				...
				if (local_var.offset >= local_var.size) {
					...
					if (local_var.total_left <= 0) {
						fi_logi("exceed total size]size=%u",
							local_var.size);
						(*flow_unlock)(i);
						return;
					}
					local_var.size = FI_MAX_ALIGN_SIZE;
					pskb = fi_get_netlink_skb(
						FI_PERIODIC_MSG_RPT,
						local_var.size, &data);
					if (pskb == NULL) {
				...
				local_var.ret = memcpy_s(
					data + local_var.offset,
					local_var.size - local_var.offset,
					&node->flow_ctx.flow_stat,
					sizeof(struct fi_periodic_rpt_msg));
				if (local_var.ret) {
					fi_loge("memcpy_s failed]ret=%d,size=%u,offset=%u",
						local_var.ret, local_var.size,
						local_var.offset);
					kfree_skb(pskb);
					(*flow_unlock)(i);
					return;
				...
				local_var.ret =
					memset_s(&node->flow_ctx.flow_stat,
						 sizeof(struct fi_flow_stat), 0,
						 sizeof(struct fi_flow_stat));
				if (local_var.ret)
					fi_loge("memset_s failed ret=%d",
						local_var.ret);

				node->flow_ctx.flow_stat.up_down_diff_time = -1;
				node->flow_ctx.flow_stat.down_seq_max =
					FI_INVALID_SEQ_NUM;
				node->flow_ctx.flow_stat.down_seq_min =
					FI_INVALID_SEQ_NUM;
	...
	if (pskb != NULL) {
		if (local_var.offset == 0) {
			...
			struct nlmsghdr *nlh =
				(struct nlmsghdr *)(data - NLMSG_HDRLEN);
			nlh->nlmsg_len = local_var.offset + NLMSG_HDRLEN;
			fi_logd("need to adjust netlink len]size=%u,offset=%u",
				local_var.size, local_var.offset);
		}
		fi_enqueue_netlink_skb(pskb);
	}
...
{
	fi_ip_flow_periodic_report(&g_fi_ipv4_flow, fi_ipv4_flow_lock,
				   fi_ipv4_flow_unlock);
#if IS_ENABLED(CONFIG_IPV6)
	fi_ip_flow_periodic_report(&g_fi_ipv6_flow, fi_ipv6_flow_lock,
				   fi_ipv6_flow_unlock);
#endif
}

int fi_flow_qos_msg(const struct fi_flow_head *head, struct sk_buff **pskb,
		...
		if (fi_flow_qos_rpt_enable(node)) {
			...
			if (*offset >= *size) {
				...
				if (buff_num >= FI_NETLINK_REPORT_MAX_NUM) {
					fi_logi("exceed max report num %u",
						FI_NETLINK_REPORT_MAX_NUM);
					return -1;
				}
				*pskb = fi_get_netlink_skb(FI_QOS_MSG_RPT,
							   *size, data);
				if (*pskb == NULL) {
					fi_loge("failed get netlink skb");
					return -1;
			...
			node->qos.bw_est = 0;
			node->flow_ctx.up_pkt_num = 0;
			ret = memcpy_s(*data + *offset, *size - *offset,
				       &flow_qos_msg,
				       sizeof(struct fi_qos_rpt_msg));
			if (ret) {
				fi_loge("memcpy_s failed]ret=%d,size=%u,offset=%u",
					ret, *size, *offset);
				kfree_skb(*pskb);
			   ...
			   void (*flow_unlock)(uint32_t))
{
	...
	uint32_t offset = 0;
	uint32_t size =
		(FI_NETLINK_BUF_MAX_SIZE / sizeof(struct fi_qos_rpt_msg)) *
		sizeof(struct fi_qos_rpt_msg);
	char *data = NULL;
	struct sk_buff *pskb = NULL;
...
{
	fi_ip_flow_qos_report(&g_fi_ipv4_flow, fi_ipv4_flow_lock,
			      fi_ipv4_flow_unlock);
#if IS_ENABLED(CONFIG_IPV6)
	fi_ip_flow_qos_report(&g_fi_ipv6_flow, fi_ipv6_flow_lock,
			      fi_ipv6_flow_unlock);
#endif
}
