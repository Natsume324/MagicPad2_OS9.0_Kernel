...
{
	spin_lock_bh(
		&(g_fi_ipv4_flow_netinfo
			  .flow_lock[index & FI_FLOW_NET_INFO_TABLE_MASK]));
}
...
uint32_t fi_ipv4_flow_netinfo_hash(__be32 addr1, __u16 port1, __be32 addr2,
				   __be16 port2, char *flow, int str_len,
				   bool exchange)
{
	int ret;
...
{
	return &(g_fi_ipv4_flow_netinfo
			 .flow_table[index & FI_FLOW_NET_INFO_TABLE_MASK]);
}

bool fi_ipv4_flow_netinfo_same(struct fi_netinfo_flow_node *node, __be32 saddr,
			       __u16 sport, __be32 daddr, __be16 dport)
{
...
fi_ipv4_flow_netinfo_find(const struct fi_netinfo_flow_head *head, __be32 saddr,
			  __u16 sport, __be32 daddr, __be16 dport)
{
	...
	if (head != NULL) {
		list_for_each_entry(node, &(head->list), list) {
			if (fi_ipv4_flow_netinfo_same(node, saddr, sport, daddr,
						      dport)) {
...
static int fi_flow_ctx_init(struct fi_netinfo_flow_node *newnode,
			    fi_flow_cb *flow_cb)
{
	int ret = memset_s(&newnode->flow_ctx,
			   sizeof(struct fi_netinfo_flow_ctx), 0,
			   sizeof(struct fi_netinfo_flow_ctx));
	if (ret) {
...
struct fi_netinfo_flow_node *
fi_ipv4_flow_netinfo_add(struct fi_netinfo_flow_head *head, int32_t uid,
			 __be32 saddr, __u16 sport, __be32 daddr, __be16 dport,
			 fi_flow_cb flow_cb, struct sock *sk)
{
	struct fi_netinfo_flow_node *newnode = NULL;
	int ret;
	if (head != NULL) {
		if (atomic_read(&g_fi_ipv4_flow_netinfo.node_num) >
		    FI_FLOW_NET_INFO_NODE_LIMIT) {
			fi_loge("IPV4 flow node out of limited %d",
				FI_FLOW_NET_INFO_NODE_LIMIT);
			return NULL;
		}
		newnode = (struct fi_netinfo_flow_node *)kzalloc(
			sizeof(struct fi_netinfo_flow_node), GFP_ATOMIC);
		if (newnode == NULL) {
			fi_loge("failed to malloc new IPV4 node for uid:%d",
				uid);
			return NULL;
		}
		ret = memset_s(newnode, sizeof(struct fi_netinfo_flow_node), 0,
			       sizeof(struct fi_netinfo_flow_node));
		if (ret) {
			kfree(newnode);
			fi_loge("memset_s failed ret=%d", ret);
			return NULL;
...
struct fi_netinfo_flow_node *
fi_ipv4_flow_netinfo_get(struct fi_netinfo_flow_head *head, int32_t uid,
			 __be32 saddr, __u16 sport, __be32 daddr, __be16 dport,
			 fi_flow_cb flow_cb, bool addflow, bool *newflow,
			 struct sock *sk)
{
	struct fi_netinfo_flow_node *node = NULL;

	if (head != NULL && newflow != NULL) {
		node = fi_ipv4_flow_netinfo_find(head, saddr, sport, daddr,
						 dport);
		if (node != NULL) {
			if (addflow && sk &&
		...
		if (addflow) {
			...
			*newflow = true;
			return fi_ipv4_flow_netinfo_add(head, uid, saddr, sport,
							daddr, dport, flow_cb,
							sk);
		}
		*newflow = false;
...
void fi_ipv4_flow_netinfo_init(void)
{
	...
	ret = memset_s(&g_fi_ipv4_flow_netinfo, sizeof(g_fi_ipv4_flow_netinfo),
		       0, sizeof(g_fi_ipv4_flow_netinfo));
	if (ret) {
		fi_loge("memset_s failed ret=%d", ret);
		return;
	}
...
struct fi_netinfo_flow_node *
fi_ipv6_flow_netinfo_find(const struct fi_netinfo_flow_head *head,
			  const struct in6_addr *saddr, __u16 sport,
			  const struct in6_addr *daddr, __be16 dport)
{
	struct fi_netinfo_flow_node *node = NULL;
	...
	if (saddr != NULL && daddr != NULL && head != NULL) {
		list_for_each_entry(node, &(head->list), list) {
			if (fi_ipv6_flow_netinfo_same(node, saddr, sport, daddr,
						      dport)) {
				node->updatetime = jiffies_to_msecs(jiffies);
				return node;
			}
...
struct fi_netinfo_flow_node *
fi_ipv6_flow_netinfo_add(struct fi_netinfo_flow_head *head, int32_t uid,
			 const struct in6_addr *saddr, __u16 sport,
			 const struct in6_addr *daddr, __be16 dport,
			 fi_flow_cb flow_cb, struct sock *sk)
{
	...
	if (saddr != NULL && daddr != NULL && head != NULL) {
		if (atomic_read(&g_fi_ipv6_flow_netinfo.node_num) >
		    FI_FLOW_NET_INFO_NODE_LIMIT) {
...
struct fi_netinfo_flow_node *
fi_ipv6_flow_netinfo_get(struct fi_netinfo_flow_head *head, int32_t uid,
			 const struct in6_addr *saddr, __u16 sport,
			 const struct in6_addr *daddr, __be16 dport,
			 fi_flow_cb flow_cb, bool addflow, bool *newflow,
			 struct sock *sk)
{
	...
	if (saddr != NULL && daddr != NULL && head != NULL && newflow != NULL) {
		node = fi_ipv6_flow_netinfo_find(head, saddr, sport, daddr,
						 dport);
		if (node != NULL) {
			if (addflow && sk &&
			    ((node->flow_ctx.flow_msg.dev_if !=
		...
		if (addflow) {
			...
			*newflow = true;
			return fi_ipv6_flow_netinfo_add(head, uid, saddr, sport,
							daddr, dport, flow_cb,
							sk);
		}
		*newflow = false;
...
void fi_ipv6_flow_netinfo_init(void)
{
	...
	ret = memset_s(&g_fi_ipv6_flow_netinfo, sizeof(g_fi_ipv6_flow_netinfo),
		       0, sizeof(g_fi_ipv6_flow_netinfo));
	if (ret) {
		fi_loge("memset_s failed ret=%d", ret);
		return;
	}
	...
	if (flow->sk->sk_state == TCP_SYN_SENT && tp->total_retrans > 0) {
		if (!strncmp(FI_CELL0_IFNAME, flow->flow_ctx.flow_msg.dev,
			...
			flow->flow_ctx.flow_app_info.rtt =
				tp->total_retrans * USEC_PER_SEC;
		else
			flow->flow_ctx.flow_app_info.rtt = 0;
		fi_logi("syn time out, set rtt = %u",
			flow->flow_ctx.flow_app_info.rtt);
		return;
	}
	...
	for (i = 0; i < FI_FLOW_TABLE_SIZE; i++) {
		...
		list_for_each_entry_safe(node, tmp, &(flow->flow_table[i].list),
			...
			memset_s(&node->flow_ctx.flow_app_info,
				 sizeof(struct fi_flow_app_info_stat), 0,
				 sizeof(struct fi_flow_app_info_stat));
		}
		(*flow_unlock)(i);
	}
	return;
...
int fi_app_netinfo_msg(char *data, uint32_t size, int index)
{
	...
	//int rtt_ipv4 = 0;
	int rtt_cnt = 0;
	struct fi_app_stat_report *app_report =
		(struct fi_app_stat_report *)data;
	...
	ret = memcpy_s(app_report, size, &app_net_info->app_stat_report,
		       sizeof(struct fi_app_stat_report));
	if (ret) {
	...
	for (i = 0; i < FI_LAUNCH_APP_MAX; i++) {
		memset_s(&app_net_info->app_stat_ctx,
			 sizeof(struct fi_app_stat_ctx), 0,
			 sizeof(struct fi_app_stat_ctx));
		memset_s(&app_net_info->app_stat_report.app_stat_msg,
			 sizeof(struct fi_app_stat_msg), 0,
			 sizeof(struct fi_app_stat_msg));
	}
	for (i = 0; i < FI_LAUNCH_APP_MAX; i++) {
		app_net_info->app_stat_ctx.dns_ctx.timer_counter =
			FI_APP_INFO_DNS_TIMER_COUNTER_INVALID;
	}
	spin_unlock_bh(&g_fi_ctx.app_stat_lock);

	app_msg->uplink_pkt_num =
		app_msg->uplink_tcp_num + app_msg->uplink_udp_num;
	app_msg->dlink_pkt_num =
		app_msg->dlink_tcp_num + app_msg->dlink_udp_num;
	...
	if (app_msg->dlink_tcp_num == 0)
		...
		app_msg->dlink_retrans_rate = app_msg->dlink_retrans_num *
					      FI_PERCENTAGE_100 /
					      app_msg->dlink_tcp_num;
	...
	if (app_msg->uplink_tcp_num == 0)
		...
		app_msg->uplink_retrans_rate = app_msg->uplink_retrans_num *
					       FI_PERCENTAGE_100 /
					       app_msg->uplink_tcp_num;
	...
	fi_logi("HiHonorQoECollectData, dnsSuccCount:%u, dnsFailCount:%u, dnsAvgDelay:%u, httpGetCount:%u,"
		...
		app_msg->dns_succ_count, app_msg->dns_fail_count,
		app_msg->dns_avg_delay, app_msg->http_get_count,
		app_msg->uplink_pkt_num, app_msg->dlink_pkt_num,
		app_msg->avr_rtt, app_msg->dlink_tcp_num,
		app_msg->dlink_tcp_len, app_msg->uplink_tcp_num,
		app_msg->uplink_tcp_len, app_msg->dlink_udp_num,
		app_msg->dlink_udp_len, app_msg->uplink_udp_num,
		app_msg->uplink_udp_len, app_msg->dlink_retrans_num,
		app_msg->uplink_retrans_num, app_msg->socket_num,
		app_msg->syn_num);
