...
void fi_register_nfhook(void)
{
	...
	for (i = 0; i < FI_DEV_NUM; i++) {
		...
		fi_logi("init ifname: %s, total_byes: %llu", iface,
			stats.rx_bytes);
	}
	spin_lock_bh(&(g_fi_ctx.app_stat_lock));
	for (i = 0; i < FI_LAUNCH_APP_MAX; i++) {
		memset_s(&g_fi_ctx.app_net_info[i].app_stat_ctx,
			 sizeof(struct fi_app_stat_ctx), 0,
			 sizeof(struct fi_app_stat_ctx));
		memset_s(&g_fi_ctx.app_net_info[i].app_stat_report.app_stat_msg,
			 sizeof(struct fi_app_stat_msg), 0,
			 sizeof(struct fi_app_stat_msg));
	}
	for (i = 0; i < FI_LAUNCH_APP_MAX; i++) {
		g_fi_ctx.app_net_info[i].app_stat_ctx.dns_ctx.timer_counter =
			FI_APP_INFO_DNS_TIMER_COUNTER_INVALID;
	}
...
void fi_dns_statistic(void)
{
	...
	spin_lock_bh(&g_fi_ctx.app_stat_lock);
	app_stat = &g_fi_ctx.app_net_info[g_fi_ctx.index]
			    .app_stat_report.app_stat_msg;
	...
	if (app_ctx->dns_ctx.timer_counter >= 0) {
		...
		while (i < app_ctx->dns_ctx.num) {
			if (timestamp - app_ctx->dns_ctx.req_timestamp[i] >
			    FI_APP_INFO_DNS_TIME_OUT) {
				fi_loge("fi_dns_statitic dns fail tid:%u",
					app_ctx->dns_ctx.tid[i]);
				app_ctx->dns_ctx.req_timestamp[i] =
					app_ctx->dns_ctx.req_timestamp
						[app_ctx->dns_ctx.num - 1];
				app_ctx->dns_ctx.tid[i] =
					app_ctx->dns_ctx
						.tid[app_ctx->dns_ctx.num - 1];
				app_ctx->dns_ctx.num--;
		...
		if (app_ctx->dns_ctx.timer_counter >
			    FI_APP_INFO_DNS_TIMER_EXPIRE ||
		    app_ctx->dns_ctx.num == 0) {
			app_ctx->dns_ctx.timer_counter =
				FI_APP_INFO_DNS_TIMER_COUNTER_INVALID;
			app_ctx->dns_ctx.num = 0;
	...
	if (app_stat->dns_succ_count)
		app_stat->dns_avg_delay =
			(uint32_t)(app_ctx->dns_ctx.total_delay /
...
static inline bool fi_payloadlen_parse(struct sk_buff *skb,
				       struct fi_pkt_parse *pktinfo,
				       struct fi_flow_node *flow);
static inline bool fi_tcpseq_parse(struct sk_buff *skb,
				   struct fi_pkt_parse *pktinfo,
				   struct fi_flow_node *flow);
static inline bool fi_tcpack_parse(struct sk_buff *skb,
				   struct fi_pkt_parse *pktinfo,
				   struct fi_flow_node *flow);
static inline bool fi_tcpflags_parse(struct sk_buff *skb,
				     struct fi_pkt_parse *pktinfo,
				     struct fi_flow_node *flow);
static inline bool fi_tcpwindowsize_parse(struct sk_buff *skb,
					  struct fi_pkt_parse *pktinfo,
					  struct fi_flow_node *flow);
static inline bool fi_tcpoptions_parse(struct sk_buff *skb,
				       struct fi_pkt_parse *pktinfo,
				       struct fi_flow_node *flow);
static inline bool fi_payload_segment_parse(struct sk_buff *skb,
					    struct fi_pkt_parse *pktinfo,
					    struct fi_flow_node *flow);
static inline bool fi_payload_seq_parse(struct fi_pkt_parse *pktinfo);
static inline bool fi_tcp_hole_parse(struct sk_buff *skb,
				     struct fi_pkt_parse *pktinfo,
				     struct fi_flow_node *flow);
static void fi_update_frags_and_loss(struct fi_flow_node *flow,
				     struct fi_pkt_parse *pktinfo);
...
static void fi_payload_parse(struct fi_flow_node *flow, struct sk_buff *skb,
			     struct fi_pkt_parse *pktinfo)
{
	...
	if (skb_shinfo(skb)->frag_list != NULL) {
		...
		pktinfo->is_gso = true;
		while (frag_list &&
		       pktinfo->raw_frag_num < FI_SKB_MAX_FRAG_LIST_NUM) {
	...
	if (skb_shinfo(skb)->nr_frags) {
		...
		for (i = 0; i < skb_shinfo(skb)->nr_frags && i < MAX_SKB_FRAGS;
		     i++) {
			...
			if (!skb_is_gso(skb)) {
				...
				fi_update_frags_and_loss(flow, pktinfo);
			} else if (skb_frag_size(frag) >=
				   skb_shinfo(skb)->gso_size) {
				pktinfo->is_gso = true;
				tmpNum = skb_frag_size(frag) /
					 skb_shinfo(skb)->gso_size;
				for (j = 0; j < tmpNum; j++) {
					pktinfo->payloadlen =
						skb_shinfo(skb)->gso_size;
	...
	if (pktinfo->is_gso && pktinfo->filter_payload != NULL &&
...
static bool fi_payloadlen_parse(struct sk_buff *skb,
				struct fi_pkt_parse *pktinfo,
				struct fi_flow_node *flow)
{
	if (pktinfo->msg.flow_msg.l4proto == IPPROTO_UDP) {
...
static bool fi_tcpwindowsize_parse(struct sk_buff *skb,
				   struct fi_pkt_parse *pktinfo,
				   struct fi_flow_node *flow)
{
	struct tcphdr *tcph = NULL;
...
static bool fi_tcpoptions_parse(struct sk_buff *skb,
				struct fi_pkt_parse *pktinfo,
				struct fi_flow_node *flow)
{
	...
	if (optionslen > FI_TCP_OPTIONS_LEN)
		return false;
	ret = memcpy_s(
		pktinfo->msg.tcp_options, FI_TCP_OPTIONS_LEN,
		(char *)(skb_transport_header(skb) + sizeof(struct tcphdr)),
		optionslen);
	if (ret) {
...
static bool fi_tcp_hole_parse(struct sk_buff *skb, struct fi_pkt_parse *pktinfo,
			      struct fi_flow_node *flow)
{
	...
	if (after(pktinfo->msg.tcp_seq, flow->flow_ctx.expect_tcp_seq)) {
		flow->flow_ctx.flow_stat.total_tcp_hole += fi_tcp_seq_minus(
			pktinfo->msg.tcp_seq, flow->flow_ctx.expect_tcp_seq);
	}

	if (!flow->flow_ctx.flow_tcp_period_init) {
		flow->flow_ctx.flow_stat.min_tcp_seq =
			flow->flow_ctx.expect_tcp_seq;
		flow->flow_ctx.flow_stat.max_tcp_seq =
			flow->flow_ctx.expect_tcp_seq;
	...
	cur_last_byte_seq =
		pktinfo->msg.tcp_seq +
		pktinfo->msg.payloadlen; // tcp seq wrap around compatible
...
static bool fi_payload_segment_parse(struct sk_buff *skb,
				     struct fi_pkt_parse *pktinfo,
				     struct fi_flow_node *flow)
{
	int ret;
	...
	if (flow_cfg->pkt[pktinfo->dir].seg_begin >= payloadlen)
		return true;
	offset = flow_cfg->pkt[pktinfo->dir].seg_end -
		 flow_cfg->pkt[pktinfo->dir].seg_begin + 1;
	if (flow_cfg->pkt[pktinfo->dir].seg_end >= payloadlen)
		offset = payloadlen - flow_cfg->pkt[pktinfo->dir].seg_begin;
	ret = memcpy_s(pktinfo->msg.payload_seg,
		       sizeof(pktinfo->msg.payload_seg),
	...
	fi_logd("get payload_seg len=%u, payloadlen=%d begin=%u end=%u", offset,
		payloadlen, flow_cfg->pkt[pktinfo->dir].seg_begin,
		flow_cfg->pkt[pktinfo->dir].seg_end);
	return true;
}
...
static uint16_t fi_dns_transaction_id_parse(struct sk_buff *skb)
{
	...
	if (skb_headlen(skb) > skb_transport_offset(skb) + transport_len) {
		/* step1: Point to the linear region */
		payloadlen = skb_headlen(skb) - skb_transport_offset(skb) -
			     transport_len;
...
static inline bool fi_payload_seq_parse(struct fi_pkt_parse *pktinfo)
{
	...
	pktinfo->msg.seq_curr = MAKE_SEQ_NUM(
		payload[flow_cfg->periodic[pktinfo->dir].spec_seq_high],
				     ...
				     struct fi_pkt_parse *pktinfo)
{
	...
	periodic_cfg =
		&pktinfo->flow_cb.flow_identify_cfg->periodic[pktinfo->dir];
	if (fi_payload_len_filter_check(pktinfo->msg.payloadlen,
					periodic_cfg->filter_pkt_size_start,
					periodic_cfg->filter_pkt_size_stop) &&
		...
		if (flow_stat->down_seq_max == FI_INVALID_SEQ_NUM)
			...
			flow_stat->down_seq_max = fi_get_max_seq(
				flow_stat->down_seq_max, pktinfo->msg.seq_curr);

		if (flow_stat->down_seq_min == FI_INVALID_SEQ_NUM)
			flow_stat->down_seq_min = pktinfo->msg.seq_curr;
		else
			flow_stat->down_seq_min = fi_get_min_seq(
				flow_stat->down_seq_min, pktinfo->msg.seq_curr);

		if (flow_ctx->down_seq_expect == FI_INVALID_SEQ_NUM) {
			flow_ctx->down_seq_expect =
				(pktinfo->msg.seq_curr + 1) %
				(FI_MAX_SEQ_NUM + 1);
			return;
		}
		if (fi_seq_is_increase(flow_ctx->down_seq_expect,
				       pktinfo->msg.seq_curr)) {
			loss_cnt =
				(pktinfo->msg.seq_curr + (FI_MAX_SEQ_NUM + 1) -
				 flow_ctx->down_seq_expect) %
				(FI_MAX_SEQ_NUM + 1);
			flow_ctx->down_seq_expect =
				(pktinfo->msg.seq_curr + 1) %
...
static void fi_reset(void)
{
	...
	fi_logi("fi need reset when netlink pid change]free_cnt=%d",
		atomic_read(&(g_fi_app_info.free_cnt)));
...
static void fi_set_nl_pid(const struct fi_cfg_head *cfg)
{
	if (cfg->len < sizeof(struct fi_cfg_head) + sizeof(uint32_t)) {
		fi_loge(" received a message without enough FI app info, len %d, min_len %zu\n",
			cfg->len,
			sizeof(struct fi_cfg_head) + sizeof(uint32_t));
		return;
	}
	/* fi need reset when netlink pid change, in case of daemon process restart */
	fi_reset();
	g_fi_ctx.nl_pid =
		*(uint32_t *)((char *)cfg + sizeof(struct fi_cfg_head));
...
static bool
fi_app_info_update_status_check_invalid(const enum fi_app_status status)
{
	if ((status <= FI_INVALID_APP_STATUS) ||
	    (status >= FI_MAX_APP_STATUS)) {
...
static bool
fi_app_info_update_same_app_check(struct fi_app_update_msg *update_msg,
...
static void fi_app_info_update(const struct fi_cfg_head *cfg)
{
	...
	if (cfg->len !=
		...
		fi_loge("received message without enough FI app info, len %d, min_len %zu",
			...
			sizeof(struct fi_cfg_head) +
				sizeof(struct fi_app_update_msg));
		return;
	}
	...
	for (i = 0; i < FI_LAUNCH_APP_MAX; i++) {
		spin_lock_bh(&(g_fi_app_info.app[i].lock));
		if (fi_app_info_update_same_app_check(update_msg,
						      &g_fi_app_info.app[i]) &&
	...
	for (i = 0; i < FI_FLOW_IDENTIFY_MAX; i++)
		app->msg.flow_identify_cfg[i].proto =
	...
	for (i = 0; i < FI_DIR_BOTH; i++) {
		...
		fi_logi("Config received.app %s[%d], appNetInfoReport[%d], status=%d,proto=%d,dport=%d,"
			...
			app->msg.app_name, app->msg.app_netinfo_rpt,
			app->msg.uid, app->msg.status,
			app->msg.flow_identify_cfg[0].proto,
			app->msg.flow_identify_cfg[0].dport[0], i,
			periodic->report_type, periodic->roi_start,
			periodic->roi_stop, i, pkt->report_type, pkt->roi_start,
			pkt->roi_stop, FI_VER);
	}
	fi_register_nfhook();
...
static void fi_app_status_update(const struct fi_cfg_head *cfg)
{
	...
	if (app == NULL) {
		fi_loge("app %s, uid %d not exist", update_msg->app_name,
			update_msg->uid);
		return;
	}
	if (update_msg->status == FI_COLLECT_STOP) {
		...
		fi_logd("app %s, uid %d stop", update_msg->app_name,
			update_msg->uid);
	} else {
	...
	fi_logi("app %s status update, uid:%d, status:%d, rand_num:%lu, free_cnt:%d",
		app->msg.app_name, app->msg.uid, app->msg.status, app->rand_num,
		atomic_read(&(g_fi_app_info.free_cnt)));
	return;
}
...
static void fi_pkt_msg_report(const struct fi_pkt_parse *pktinfo, int type)
{
	...
	if (pktinfo->rand_num == pktinfo->flow_cb.app_info->rand_num) {
		...
		ret = memcpy_s(data, size, &(pktinfo->msg),
			       sizeof(struct fi_pkt_msg));
		if (ret) {
			fi_loge("memcpy_s failed ret=%d", ret);
			kfree_skb(pskb);
			return;
		...
		fi_logd("finish to send a message to native, flow:%s",
...
static bool fi_app_netinfo_cfg_get(struct fi_pkt_parse *pktinfo, int32_t uid,
...
static bool fi_app_identify_cfg_get(struct fi_pkt_parse *pktinfo, int32_t uid,
				    enum fi_app_status status)
{
	struct fi_app_info_node *app = NULL;
	int i, j;
	...
	if (pktinfo != NULL) {
		...
		for (i = 0; i < FI_FLOW_IDENTIFY_MAX; i++) {
			...
			if (proto == FI_PROTO_ANY ||
			    proto == pktinfo->msg.flow_msg.l4proto) {
				for (j = 0; j < FI_DPORT_MAX; j++) {
					if (app->msg.flow_identify_cfg[i]
							    .dport[j] ==
						    FI_PORT_ANY ||
					    pktinfo->msg.flow_msg.dport ==
						    app->msg.flow_identify_cfg[i]
							    .dport[j]) {
						match = true;
						break;
					}
				}
				if (pktinfo->msg.flow_msg.dport >=
					    app->msg.flow_identify_cfg[i]
						    .dport_range[FI_DPORT_LOW] &&
				    pktinfo->msg.flow_msg.dport <=
					    app->msg.flow_identify_cfg[i]
						    .dport_range[FI_DPORT_HIGH]) {
					match = true;
				}
			}
			if (match)
		...
		if (!match)
			return false;
		pktinfo->flow_cb.flow_identify_cfg =
			&app->msg.flow_identify_cfg[i];
		return true;
	}
	return false;
...
static void fi_app_netinfo_update(struct sk_buff *skb,
				  struct fi_netinfo_flow_node *flow,
				  const struct fi_pkt_parse *pktinfo,
				  enum fi_dir dir)
{
	struct tcphdr *tcph = NULL;
	...
	if (pktinfo->msg.flow_msg.l4proto == IPPROTO_UDP) {
		if (skb->len >
		    skb_transport_offset(skb) + sizeof(struct udphdr))
			payloadlen = skb->len - skb_transport_offset(skb) -
				     sizeof(struct udphdr);
	} else {
		if (skb->len > skb_transport_offset(skb) + tcp_hdrlen(skb))
			payloadlen = skb->len - skb_transport_offset(skb) -
	...
	if (pktinfo->msg.flow_msg.l4proto == IPPROTO_TCP) {
		...
		if (dir == FI_DIR_DOWN) {
			...
			if (before(ntohl(tcph->seq) + tcp_len - 1,
				   tp->rcv_nxt) &&
			    tcp_len > 0)
		...
		if (dir == FI_DIR_UP) {
			...
			if (tp->snd_nxt != 0 &&
			    before(ntohl(tcph->seq), tp->snd_nxt) &&
			    (flow->sk->sk_state == TCP_ESTABLISHED ||
			     flow->sk->sk_state == TCP_SYN_SENT))
...
static void fi_ip_save_5tuple_info(struct fi_pkt_parse *pktinfo,
				   uint16_t protocol, struct sk_buff *skb)
{
...
static void fi_flow_get_dev(struct fi_flow_msg *flow_msg,
			    const struct net_device *dev)
{
	int ret = memcpy_s(flow_msg->dev, IFNAMSIZ, dev->name,
			   strlen(dev->name) + 1);
	if (ret)
...
static bool fi_dns_proc(struct fi_pkt_parse *pktinfo, enum fi_dir dir,
			struct sk_buff *skb)
{
	int i;
	uint64_t timestamp;
	...
	if (pktinfo->msg.flow_msg.l4proto != IPPROTO_UDP)
		return false;
	if (dir == FI_DIR_UP &&
	    pktinfo->msg.flow_msg.dport != FI_APP_INFO_DNS_PORT)
		return false;
	if (dir == FI_DIR_DOWN &&
	    pktinfo->msg.flow_msg.sport != FI_APP_INFO_DNS_PORT)
		return false;
	...
	if (dir == FI_DIR_UP) {
		...
		app_ctx->dns_ctx.req_timestamp[app_ctx->dns_ctx.num] =
			timestamp;
		app_ctx->dns_ctx.tid[app_ctx->dns_ctx.num] = tid;
		app_ctx->dns_ctx.num++;
		app_ctx->dns_ctx.timer_counter = 0; // start timer count
		fi_loge("fi_dns_proc up tid:%u, app_ctx->dns_ctx.num:%d", tid,
			app_ctx->dns_ctx.num);
	} else {
		i = 0;
		while (i < app_ctx->dns_ctx.num) {
			if (app_ctx->dns_ctx.tid[i] == tid) {
				app_ctx->dns_ctx.total_delay +=
					timestamp -
					app_ctx->dns_ctx.req_timestamp[i];
				app_stat->dns_succ_count++;
				app_ctx->dns_ctx.req_timestamp[i] =
			      ...
			      struct fi_flow_node *flow)
{
	...
	for (i = 0; i < FI_END_PARSE; i++) {
		if ((pktinfo->flow_cb.flow_identify_cfg->pkt[pktinfo->dir]
			     .opt_enable[i] ||
		     pktinfo->flow_cb.flow_identify_cfg->periodic[pktinfo->dir]
			     .opt_enable[i]) &&
...
static void fi_ipv4_5tuple_parse(struct fi_pkt_parse *pktinfo,
				 struct sk_buff *skb, enum fi_dir dir)
{
				      ...
				      struct sk_buff *skb, enum fi_dir dir)
{
	...
	if (dir == FI_DIR_UP) {
		...
		if (pktinfo->msg.flow_msg.l4proto == IPPROTO_TCP &&
			...
			fi_logd("Ignore SYN_RECV sk, sk_state=%d l4proto=%d port[%hu--%hu]",
				sk->sk_state, pktinfo->msg.flow_msg.l4proto,
	...
	hash = fi_ipv4_flow_netinfo_hash(
		pktinfo->msg.flow_msg.ipv4_sip, pktinfo->msg.flow_msg.sport,
		pktinfo->msg.flow_msg.ipv4_dip, pktinfo->msg.flow_msg.dport,
		pktinfo->msg.flow_msg.flow, FI_FLOW_LEN + 1,
		((dir == FI_DIR_DOWN) ? true : false));
	head = fi_ipv4_flow_netinfo_header(hash);
	fi_ipv4_flow_netinfo_lock(hash);

	flow = fi_ipv4_flow_netinfo_get(
		head, kuid.val, pktinfo->msg.flow_msg.ipv4_sip,
		pktinfo->msg.flow_msg.sport, pktinfo->msg.flow_msg.ipv4_dip,
		pktinfo->msg.flow_msg.dport, pktinfo->flow_cb,
		((dir == FI_DIR_UP) ? true : false),
	...
	if (pktinfo->msg.flow_msg.l4proto == IPPROTO_TCP && flow->sk &&
	    flow->sk->sk_state >= TCP_FIN_WAIT1) {
		fi_logd("Ignore flow:l4proto=%d,sk=%pK, sk_state=%d port[%hu--%hu]",
			pktinfo->msg.flow_msg.l4proto, flow->sk,
			flow->sk->sk_state, pktinfo->msg.flow_msg.sport,
			pktinfo->msg.flow_msg.dport);
		fi_ipv4_flow_netinfo_unlock(hash);
		return;
	}
				    ...
				    struct sk_buff *skb, enum fi_dir dir)
{
	...
	flow = fi_ipv4_flow_get(head, kuid.val, pktinfo->msg.flow_msg.ipv4_sip,
				pktinfo->msg.flow_msg.sport,
				pktinfo->msg.flow_msg.ipv4_dip,
				pktinfo->msg.flow_msg.dport, pktinfo->flow_cb,
				((dir == FI_DIR_UP) ? true : false),
	...
	if (fi_flow_qos_rpt_enable(flow) &&
	    pktinfo->msg.flow_msg.l4proto == IPPROTO_TCP) {
		if (dir == FI_DIR_UP) {
	...
	if ((flow->flow_ctx.stat_stop & FI_FLOW_ALL_REPORT_STOP) ==
		    FI_FLOW_ALL_REPORT_STOP &&
	...
	if (enable) {
		int ret = strncpy_s(pktinfo->msg.flow_msg.dev, IFNAMSIZ,
				    flow->flow_ctx.flow_msg.dev, IFNAMSIZ);
		if (ret)
			fi_loge("strcpy_s failed ret=%d, dev=%s", ret,
				flow->flow_ctx.flow_msg.dev);
	}
	fi_ipv4_flow_unlock(hash);
...
static void fi_ipv4_hook(struct sk_buff *skb, const struct nf_hook_state *state,
			 enum fi_dir dir)
{
	struct fi_pkt_parse pktinfo;
	...
	if (memset_s(&pktinfo, sizeof(pktinfo), 0,
		     sizeof(struct fi_pkt_parse))) {
...
static void fi_ipv6_5tuple_parse(struct fi_pkt_parse *pktinfo,
				 struct sk_buff *skb, enum fi_dir dir)
{
	...
	pktinfo->msg.dir = dir;
	iph = ipv6_hdr(skb);
	ret = memcpy_s(&(pktinfo->msg.flow_msg.ipv6_sip),
		       sizeof(pktinfo->msg.flow_msg.ipv6_sip), &(iph->saddr),
	...
	ret = memcpy_s(&(pktinfo->msg.flow_msg.ipv6_dip),
		       sizeof(pktinfo->msg.flow_msg.ipv6_dip), &(iph->daddr),
				      ...
				      struct sk_buff *skb, enum fi_dir dir)
{
	...
	hash = fi_ipv6_flow_netinfo_hash(
		...
		pktinfo->msg.flow_msg.dport, pktinfo->msg.flow_msg.flow,
		FI_FLOW_LEN + 1, ((dir == FI_DIR_DOWN) ? true : false));
	head = fi_ipv6_flow_netinfo_header(hash);
	fi_ipv6_flow_netinfo_lock(hash);

	flow = fi_ipv6_flow_netinfo_get(
		head, kuid.val,
		(const struct in6_addr *)&(pktinfo->msg.flow_msg.ipv6_sip),
		pktinfo->msg.flow_msg.sport,
		(const struct in6_addr *)&(pktinfo->msg.flow_msg.ipv6_dip),
...
static void fi_ipv6_hook(struct sk_buff *skb, const struct nf_hook_state *state,
			 enum fi_dir dir)
{
	struct fi_pkt_parse pktinfo;
...
void fi_init(struct sock *nlfd)
{
	...
	ret = memset_s(&g_fi_app_info, sizeof(g_fi_app_info), 0,
		       sizeof(g_fi_app_info));
	if (ret) {
		fi_loge("memset_s failed ret=%d", ret);
		return;
	}
	...
	g_fi_ctx.netlink_task =
		kthread_run(fi_netlink_thread, NULL, "fi_netlink_thread");
...
void fi_bw_calc(struct fi_flow_node *flow)
{
	...
	if ((flow->rcv_bw.first_rcv_time.stamp_jiffies == 0) &&
	    (flow->rcv_bw.first_rcv_time.stamp_us == 0))
		return;
	delta_us = fi_mstamp_us_delta(&(flow->rcv_bw.last_rcv_time),
				      &(flow->rcv_bw.first_rcv_time));
	if (flow->rcv_bw.total_bytes == 0) {
		if (flow->rcv_bw.burst_bytes < FI_MIN_BURST_SIZE ||
		    delta_us * FI_TIMER_INTERVAL < USEC_PER_SEC)
			return;
	}
	if (flow->rcv_bw.burst_bytes >= FI_MIN_BURST_SIZE) {
	...
	if (flow->rcv_bw.total_time < FI_MIN_BURST_DURATION)
		flow->qos.bw_est = (uint64_t)flow->rcv_bw.total_bytes *
				   USEC_PER_SEC / FI_MIN_BURST_DURATION;
	else
		flow->qos.bw_est = (uint64_t)flow->rcv_bw.total_bytes *
				   USEC_PER_SEC / flow->rcv_bw.total_time;
...
void fi_bw_update(struct fi_flow_node *flow)
{
	if (flow->rcv_bw.burst_bytes > FI_MIN_BURST_SIZE) {
		uint32_t delta_us =
			fi_mstamp_us_delta(&(flow->rcv_bw.last_rcv_time),
					   &(flow->rcv_bw.first_rcv_time));
		fi_logd("burst: uid %u, sport: %u, burst_bytes: %u, delta_us: %u",
			flow->uid, flow->sport, flow->rcv_bw.burst_bytes,
			delta_us);
		delta_us = (delta_us > FI_MIN_BURST_DURATION) ?
				   delta_us :
				   FI_MIN_BURST_DURATION;
		flow->rcv_bw.total_bytes += flow->rcv_bw.burst_bytes;
		flow->rcv_bw.total_time += delta_us;
	}
}
...
void fi_rtt_update(struct fi_flow_node *flow)
{
	...
	fi_logd("sport %u now %lu srtt_time %lu srtt %u now_mstamp %llu rcv_rtt_time %llu rcv_rtt %u",
		flow->sk->sk_num, now_jiffies, srtt_time, srtt, now_mstamp,
		rcv_rtt_time, tp->rcv_rtt_est.rtt_us);
	fi_logd("updatetime %u tcp_mstamp %llu mdev_max_us %u rttvar_us %u total_retrans %u retrans_stamp %u",
		flow->updatetime, tp->tcp_mstamp, tp->mdev_us, tp->rttvar_us,
		tp->total_retrans, tp->retrans_stamp);
	/* SYN TIMEOUT */
	if (flow->sk->sk_state == TCP_SYN_SENT && tp->total_retrans > 0) {
		if (!strncmp(FI_CELL0_IFNAME, flow->flow_ctx.flow_msg.dev,
			     strlen(flow->flow_ctx.flow_msg.dev)) ||
		    !strncmp(FI_CELL1_IFNAME, flow->flow_ctx.flow_msg.dev,
			     strlen(flow->flow_ctx.flow_msg.dev)))
	...
	if (time_before(now_jiffies,
			srtt_time + msecs_to_jiffies(MSEC_PER_SEC)) ||
	    time_before(now_jiffies, srtt_time + usecs_to_jiffies(delta_us)) ||
	    srtt < tp->rcv_rtt_est.rtt_us)
		flow->qos.rtt = srtt;
	else
...
void fi_qos_update(const struct sk_buff *skb, struct fi_flow_node *flow)
{
	...
	delta_time_last =
		fi_mstamp_us_delta(&now, &(flow->rcv_bw.last_rcv_time));
	delta_time_first =
		fi_mstamp_us_delta(&now, &(flow->rcv_bw.first_rcv_time));
	fi_logd(" dev %s, sport %u, jiffies %u us %u seq %u len %u",
		flow->flow_ctx.flow_msg.dev, flow->sk->sk_num,
		now.stamp_jiffies, now.stamp_us, TCP_SKB_CB(skb)->seq,
		skb->len);
	if (delta_time_last <= max_delta ||
	    delta_time_last <= FI_MIN_DELTA_US ||
	    delta_time_first <= flow->qos.rtt) {
...
void fi_filesize_update(const struct sk_buff *skb, struct fi_flow_node *flow)
{
	...
	if (skb->len <= skb_transport_offset(skb) + tcp_hdrlen(skb) ||
		...
		download_bytes =
			tp->bytes_received - flow->file_size.last_rcv_bytes;
		flow->file_size.last_rcv_bytes = tp->bytes_received;
		if (download_bytes > flow->file_size.size) {
			flow->file_size.size = download_bytes;
			fi_logd("sport %u filesize %u", flow->sk->sk_num,
				flow->file_size.size);
		}
	}
}
...
int fi_iface_msg_by_index(char *data, int index, uint32_t size)
{
	...
	if (dev) {
		...
		if (stats.rx_bytes > g_fi_iface_bytes[index])
			iface_msg.rcv_bytes =
				stats.rx_bytes - g_fi_iface_bytes[index];
		else
	...
	ret = memcpy_s(data + index * sizeof(struct fi_iface_msg),
		       size - index * sizeof(struct fi_iface_msg), &iface_msg,
		       sizeof(struct fi_iface_msg));
	if (ret != EOK) {
