...
#define FI_FLOW_ALL_REPORT_STOP                                                \
	(FI_FLOW_PERIODIC_REPORT_STOP_UP | FI_FLOW_PERIODIC_REPORT_STOP_DOWN | \
...
#define FI_FLOW_PERIODIC_REPORT_NEED \
	(FI_FLOW_PERIODIC_REPORT_NEED_UP | FI_FLOW_PERIODIC_REPORT_NEED_DOWN)

#define FI_FLOW_QOS_REPORT_NEED 1
#define FI_MAX_ALIGN_SIZE                                                 \
	((FI_NETLINK_BUF_MAX_SIZE / sizeof(struct fi_periodic_rpt_msg)) * \
...
extern uint32_t fi_ipv4_flow_hash(__be32 addr1, __u16 port1, __be32 addr2,
...
extern struct fi_flow_node *fi_ipv4_flow_add(struct fi_flow_head *head,
					     int32_t uid, __be32 saddr,
					     __u16 sport, __be32 daddr,
					     __be16 dport, fi_flow_cb flow_cb,
					     struct sock *sk);

extern struct fi_flow_node *
fi_ipv4_flow_get(struct fi_flow_head *head, int32_t uid, __be32 saddr,
		 __u16 sport, __be32 daddr, __be16 dport, fi_flow_cb flow_cb,
		 bool addflow, bool *newflow, struct sock *sk);
...
extern struct fi_flow_node *
fi_ipv6_flow_add(struct fi_flow_head *head, int32_t uid,
		 const struct in6_addr *saddr, __u16 sport,
		 const struct in6_addr *daddr, __be16 dport, fi_flow_cb flow_cb,
		 struct sock *sk);

extern struct fi_flow_node *
fi_ipv6_flow_get(struct fi_flow_head *head, int32_t uid,
		 const struct in6_addr *saddr, __u16 sport,
		 const struct in6_addr *daddr, __be16 dport, fi_flow_cb flow_cb,
		 bool addflow, bool *newflow, struct sock *sk);
...
extern bool fi_payload_str_filter_check(struct fi_pkt_parse *pktinfo,
					fi_rpt_cfg *rpt_cfg);
