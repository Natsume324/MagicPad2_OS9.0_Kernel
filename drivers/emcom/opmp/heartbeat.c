...
static struct sockaddr_in
	g_link_addrs[LINK_NUM]; // LINK_NUM definied in mptcp_heartbeat.h
			       ...
			       int32_t period, int8_t retry_count)
{
	...
	g_teid = teid;
	(void)memcpy_s(g_link_addrs, sizeof(g_link_addrs), link_addrs,
		       sizeof(*link_addrs));
	/* init mpheartbeat */
	...
	LOGI("mpheartbeat enabled, teid %d, period %d, retry_count %d", teid,
	     period, retry_count);
}
...
static void disable_mpheartbeat(void)
{
	...
	g_teid = 0;
	(void)memset_s(g_link_addrs, sizeof(g_link_addrs), 0,
		       sizeof(g_link_addrs));
...
static int get_total_traffic(u64 *total_rx_bytes, u64 *total_tx_bytes)
{
	...
	while (true) {
		...
		pos = pos - bytes_read +
		      line_len; /* remember to move the pos to the start of next line */
	}
...
static bool check_heartbeat_needed(void)
{
	...
	if (total_tx_bytes > g_previous_tx_bytes &&
	    total_rx_bytes == g_previous_rx_bytes) {
...
g_previous_rx_bytes is %llu, total_tx_bytes is %llu, total_rx_bytes is %llu",
		     g_previous_tx_bytes, g_previous_rx_bytes, total_tx_bytes,
		     total_rx_bytes);
		ret = false;
	}
...
static void trigger_mpheartbeat(void)
{
	...
	unsigned int detection_type =
		WIFI_DETECTION + LTE_DETECTION; // both paths need detection
...
static void handle_init_info(struct heartbeat_init_info *data)
{
	...
	LOGD("wifi %d, lte %d, p %d, teid %d, interval %d, count %d",
	     data->wifi_addr, data->lte_addr, data->port, teid, data->interval,
	     data->count);
}
