...
{
	if (node->sip == pktinfo->sip && node->dip == pktinfo->dip &&
	    node->sport == pktinfo->sport && node->dport == (pktinfo)->dport)
		return true;

	if (node->sip == pktinfo->dip && node->dip == pktinfo->sip &&
	    node->sport == pktinfo->dport && node->dport == pktinfo->sport)
		return true;
	return false;
}
				 ...
				 struct fi_flow_head *head)
{
	...
	fi_logd(" : FI new flow node, port %u:%u", pktinfo->sport,
		pktinfo->dport);
