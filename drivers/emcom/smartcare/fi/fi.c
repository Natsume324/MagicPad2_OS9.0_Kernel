...
static int fi_parse_mptcp(struct fi_pkt *pktinfo, struct tcphdr *tcph,
			  uint32_t hdrlen)
{
	uint32_t leftlen = 0;
	...
	fi_logd(" : FI mptcp seq=%u ack=%u flow: %u:%u", pktinfo->seq,
		pktinfo->ack, pktinfo->sport, pktinfo->dport);

	return FI_SUCCESS;
}

static int fi_pkt_parse(struct fi_pkt *pktinfo, struct sk_buff *skb, int dir)
{
	...
	if (iph->protocol == FI_IPPROTO_UDP) {
		...
		pktinfo->bufdatalen =
			skb->len - skb->data_len - iphlen - l4hlen;
...
static void fi_appid_shrink(struct smfi_ctx *fictx)
{
	...
	for (i = fictx->appidmin; (i <= fictx->appidmax) && (i < FI_APPID_MAX);
	     i++) {
...
static void fi_proc_applaunch(uint8_t *data, uint32_t len,
			      struct smfi_ctx *fictx)
{
	...
	if (!fi_appid_valid(appid)) {
		fi_logerr(" : FI unknown app name %s, len %u", msg->appname,
			  len);
		return;
	}
...
static void fi_proc_appstatus(uint8_t *data, uint32_t len,
			      struct smfi_ctx *fictx)
{
	...
	fi_loginfo(" : FI recv app status appid=%u appstatus=%u", appid,
		   msg->appstatus);
...
void fi_reflect_status(int32_t event, uint8_t *data, uint16_t len)
{
	...
	fi_logd(" : FI reflect status to daemon, uid=%u status=%u", report.uid,
		status);
}
