...
void fi_rtt_send(uint32_t appid)
{
	...
	if ((appinfo->switches & FI_SWITCH_RTT) != 0) {
		...
		fi_logd(" : FI rtt=%u uid=%u (btr=%d bts=%d hbr=%d hbst=%d)",
			...
			gamectx->battlertt, gamectx->hbrtt, gamectx->hbsrtt);
	}
}
...
void fi_rtt_status(uint32_t appid, uint32_t status)
{
	...
	if ((appinfo->switches & FI_SWITCH_STATUS) != 0) {
		...
		fi_logd(" : FI appid=%u uid=%u status=%u", appid, report.uid,
			status);
	}
}
...
void fi_rtt_timer(void)
{
	...
	for (i = 0; i < FI_APPID_MAX; i++) {
		...
		fi_logd(" : FI appid=%d up=%u/%u down=%u/%u", i,
			gamectx->preuplinkpktnum, gamectx->nouplinkpkt,
...
void fi_rtt_update_integral(struct fi_gamectx *gamectx, int32_t newrtt,
			    int64_t curms)
{
	...
	if (cursec == fi_ms_to_sec(gamectx->updatetime)) {
		gamectx->rtt_integral +=
			gamectx->rtt * (curms - gamectx->updatetime);
	} else {
		gamectx->rtt_integral +=
			gamectx->rtt * (cursec * FI_MS - gamectx->updatetime);
		...
		if ((gamectx->preuplinkpktnum >= FI_BATTLE_ONGOING) ||
			...
			gamectx->srtt = fi_get_srtt(
				gamectx->srtt, fi_min(avgrtt, FI_MAX_ORG_RTT),
				FI_SRTT_VAR);
...
void fi_rtt_judge_reconn(struct fi_pkt *pktinfo, struct fi_gamectx *gamectx)
{
	...
	fi_logd(" : FI noup=%u nodown=%u appstatus=%u", gamectx->nouplinkpkt,
		gamectx->nodownlinkpkt, gamectx->appstatus);
		       ...
		       struct fi_gamectx *gamectx)
{
	...
	fi_logd(" : FI up: %u/s down: %u/s flow: %u:%u times: %u", up_pps,
		flowctx->downpktnum, pktinfo->sport, pktinfo->dport,
		    ...
		    struct fi_gamectx *gamectx)
{
	...
	fi_logd(" : FI hbup=%u hbdown=%u time=%u, flow %u:%u", flowctx->hbupnum,
		flowctx->hbdownnum, timediff, pktinfo->sport, pktinfo->dport);
			      ...
			      struct fi_gamectx *gamectx)
{
	...
	fi_logd(" : FI up=%u/s down=%u/s len=%u times=%u flow %u:%u",
		uppktpersec, downpktpersec, avgpktlen, flowctx->btflow_times,
		pktinfo->sport, pktinfo->dport);
		      ...
		      struct fi_gamectx *gamectx)
{
	...
	if (pktinfo->dir == SMFI_DIR_UP) {
		...
		gamectx->rawrtt =
			fi_get_srtt(gamectx->rawrtt, rawrtt, FI_SRTT_VAR);
		     ...
		     struct fi_gamectx *gamectx)
{
	...
	if (pktinfo->dir == SMFI_DIR_UP) {
		...
		if ((mpctx->uppkttime > 0) &&
			...
			(void)memset_s(mpctx, sizeof(*mpctx), 0,
				       sizeof(*mpctx));
		}
	...
	fi_logd(" : FI get ack rawrtt=%d seq=%u ack=%u flow %u:%u", rawrtt,
		mpctx->seq, pktinfo->ack, pktinfo->sport, pktinfo->dport);
		    ...
		    struct fi_gamectx *gamectx)
{
	...
	fi_rtt_update_integral(gamectx, newrtt, pktinfo->msec);
	fi_logd(" : FI update hyxd rtt, appid=%u rtt=%d", gamectx->appid,
		gamectx->battlertt);
		   ...
		   struct fi_gamectx *gamectx)
{
	...
	for (i = 0; i < FI_UU_CACHE_NUM; i++) {
		if (uu_seq[curcache] &&
		    (pktinfo->ack - uu_seq[curcache] < (uint32_t)FI_ACK_MAX)) {
	...
	fi_logd(" : FI update uu rtt newrtt=%d battlertt=%d", newrtt,
		gamectx->battlertt);
}
...
void fi_rtt_cal_bh3_uplink(struct fi_pkt *pktinfo, struct fi_flowctx *flowctx)
{
	...
	/*lint -e676*/
	for (i++; (i < FI_BH3_SEQ_CACHE_NUM) && (flowctx->flow_bh3[i].seq > 0);
	     i++)
			     ...
			     struct fi_gamectx *gamectx)
{
	...
	fi_logd(" : FI bh3 rtt=%d srtt=%d btrtt=%d", gamectx->rtt,
		gamectx->srtt, gamectx->battlertt);
}
...
void fi_rtt_amend(struct fi_gamectx *gamectx, struct fi_gamectx *uuctx)
{
	...
	case FI_APPID_QJCJ:
		if (gamectx->hbsrtt) {
			gamectx->final_rtt =
				fi_range(gamectx->battlertt + gamectx->hbsrtt -
						 FI_RTT_BASE,
					 FI_MIN_RTT, FI_MAX_RTT);
		} else {
		      ...
		      uint32_t appid)
{
	...
	if (pktinfo->msec < prepkttime)
		fi_logd(" : FI pkt timestamp error, %lld < %lld", pktinfo->msec,
			prepkttime);
