...
static bool emcom_xengine_mpflow_ai_is_excluded_port(
	const struct emcom_xengine_mpflow_ai_info *app, uint16_t dport,
	uint8_t proto);
static bool emcom_xengine_mpflow_ai_get_port(struct sockaddr *addr,
					     uint16_t *port);
...
emcom_xengine_mpflow_ai_hash_get_policy(__be32 *addr, uint8_t addr_len,
					const struct hlist_head *hashtable)
{
	...
	hlist_for_each_entry_safe(policy, tmp, &hashtable[hash], node) {
		if (memcmp((const void *)(policy->addr), (const void *)addr,
			   addr_len * sizeof(__be32)) == 0) {
			result = policy;
			break;
		}
	}
...
static bool emcom_xengine_mpflow_ai_get_addr_port(struct sockaddr *addr,
						  __be32 *s_addr,
						  uint16_t *port)
{
	uint8_t i;
	...
	if (addr->sa_family == AF_INET) {
		...
		*(s_addr + EMCOM_MPFLOW_AI_CLAT_IPV6 - 1) =
...
static struct emcom_xengine_mpflow_ip_bind_policy *
emcom_xengine_mpflow_ai_hash(__be32 *addr, uint8_t addr_len,
			     struct hlist_head *hashtable)
{
	...
	policy = kzalloc(sizeof(struct emcom_xengine_mpflow_ip_bind_policy),
			 GFP_ATOMIC);
	if (!policy)
		return NULL;
...
static void
emcom_xengine_mpflow_ai_hash_delete(struct emcom_xengine_mpflow_ai_priv *priv,
				    __be32 *addr, uint8_t addr_len)
{
	...
	hlist_for_each_entry_safe(ip, tmp, &priv->hashtable[hash], node) {
		if (memcmp((const void *)(ip->addr), (const void *)addr,
			   addr_len * sizeof(__be32)) == 0) {
			hlist_del(&ip->node);
			kfree(ip);
		}
...
static bool emcom_xengine_mpflow_ai_get_addr(uint16_t sa_family,
					     struct in_addr *addr,
					     __be32 *s_addr, uint8_t s_addr_len)
{
	uint8_t i;
...
{
	EMCOM_LOGD("reset sk %pK sport is %u, state[%u]", sk, sk->sk_num,
		   sk->sk_state);
	local_bh_disable();
	bh_lock_sock(sk);
...
static void
emcom_xengine_mpflow_ai_enable_selected_path(struct sock *sk,
					     const char *selected_path_iface)
{
	struct net_device *net_dev =
		dev_get_by_name(sock_net(sk), selected_path_iface);
...
#if IS_ENABLED(CONFIG_IPV6)
			if (inet->pinet6 &&
			    !ipv6_addr_any(&inet->pinet6->saddr))
...
static bool emcom_xengine_mpflow_ai_rehash_sk(struct sock *sk)
{
	if (sk->sk_protocol == IPPROTO_UDP &&
		...
		if (sk->sk_prot && sk->sk_prot->rehash) {
			/*lint -e666*/
			EMCOM_LOGD(
				"UDP sock is already binded by user, rehash sk sk[%pK] sk_userlocks[%#x] "
				"dev_if[%d] inet_num[%d] hash[%d] inet_rcv_saddr: " IPV4_FMT
				" inet_saddr: " IPV4_FMT,
				sk, sk->sk_userlocks, sk->sk_bound_dev_if,
				inet_sk(sk)->inet_num,
				udp_sk(sk)->udp_portaddr_hash,
				IPV4_INFO(inet_sk(sk)->inet_rcv_saddr),
				IPV4_INFO(inet_sk(sk)->inet_saddr));
			inet_sk(sk)->inet_rcv_saddr = 0;
			inet_sk(sk)->inet_saddr = 0;
			/*lint +e666*/

#if IS_ENABLED(CONFIG_IPV6)
			EMCOM_LOGD("[MPFlow_KERNEL] sk_v6_rcv_saddr: " IPV6_FMT,
				   IPV6_INFO(sk->sk_v6_rcv_saddr));
			if (inet_sk(sk)->pinet6 &&
			    !ipv6_addr_any(&inet_sk(sk)->pinet6->saddr)) {
				EMCOM_LOGD(
					"[MPFlow_KERNEL] inet6 saddr: " IPV6_FMT,
					IPV6_INFO(inet_sk(sk)->pinet6->saddr));
						  ...
						  uint16_t bind_mode)
{
	...
	if (bind_mode == EMCOM_MPFLOW_BIND_NONE) {
		EMCOM_LOGI(
			"path_handover to default route, sk:%pK sport[%u] dev_if[%d] bind[%d] family[%u]",
			sk, sk->sk_num, sk->sk_bound_dev_if,
			sk->is_mp_flow_bind, sk->sk_family);
	...
	EMCOM_LOGI(
		"path_handover sk:%pK sport[%u] dev_if[%d] bind[%d] family[%u] dev[%s]",
		sk, sk->sk_num, sk->sk_bound_dev_if, sk->is_mp_flow_bind,
		sk->sk_family, selected_path_iface);
	emcom_xengine_mpflow_ai_enable_selected_path(sk, selected_path_iface);
}

extern struct inet_hashinfo tcp_hashinfo;

static void emcom_xengine_mpflow_ai_tcp_intf_reset(
	const struct emcom_xengine_mpflow_ai_info *app,
	struct reset_flow_policy_info *reset, bool *need_reset_device,
	struct sock *sk)
{
	if (reset->flow.l3proto == ETH_P_IP) {
		sk = inet_lookup_established(
			&init_net, &tcp_hashinfo, reset->flow.ipv4_dip,
			htons(reset->flow.dst_port), reset->flow.ipv4_sip,
			htons(reset->flow.src_port), reset->flow.sk_dev_itf);
	} else {
		sk = __inet6_lookup_established(
			&init_net, &tcp_hashinfo, &reset->flow.ipv6_dip,
			htons(reset->flow.dst_port), &reset->flow.ipv6_sip,
			reset->flow.src_port, reset->flow.sk_dev_itf, 0);
	}
	if (sk == NULL) {
	...
	if ((!sk->is_mp_flow_bind && sk->sk_bound_dev_if) ||
	    emcom_xengine_mpflow_ai_is_excluded_port(app, ntohs(sk->sk_dport),
						     IPPROTO_TCP))
		return;
	EMCOM_LOGI(
		"reset sk:%pK sport[%u], state[%u] bind[%d] act[%u] family[%u] protocol[%u]",
		sk, sk->sk_num, sk->sk_state, sk->is_mp_flow_bind,
		reset->policy.act, sk->sk_family, sk->sk_protocol);
	if (reset->policy.act == SK_ERROR) {
		emcom_xengine_mpflow_reset(tcp_sk(sk));
		*need_reset_device = true;
		EMCOM_LOGI("tcp reset completed");
	} else {
		EMCOM_LOGE("tcp reset action not support, action: %u",
			   reset->policy.act);
	}
	sock_put(sk);
}
...
static void
emcom_xengine_mpflow_ai_udp_intf_reset(struct reset_flow_policy_info *reset,
				       bool *need_reset_device, struct sock *sk)
{
	if (reset->flow.l3proto == ETH_P_IP) {
		sk = udp4_lib_lookup(&init_net, reset->flow.ipv4_dip,
				     htons(reset->flow.dst_port),
				     reset->flow.ipv4_sip,
				     htons(reset->flow.src_port),
				     reset->flow.sk_dev_itf);
	} else {
		sk = udp6_lib_lookup(&init_net, &reset->flow.ipv6_dip,
				     htons(reset->flow.dst_port),
				     &reset->flow.ipv6_sip,
				     htons(reset->flow.src_port),
				     reset->flow.sk_dev_itf);
		...
		if (sk->sk_protocol == IPPROTO_UDP) {
			if (reset->policy.act == SK_ERROR) {
				...
				*need_reset_device = true;
				...
				emcom_xengine_mpflow_ai_path_handover(
					sk, reset->policy.rst_bind_mode);
			} else {
				EMCOM_LOGE(
					"udp reset action not support, action: %u",
					reset->policy.act);
			}
...
void emcom_xengine_mpflow_ai_reset_loc_intf(const uint8_t *data, uint16_t len)
{
	...
	if (reset->flow.l3proto == ETH_P_IP) {
		EMCOM_LOGI("receive reset. SrcIP[" IPV4_FMT "] SrcPort[%u] "
			   "DstIP[" IPV4_FMT
			   "] DstPort[%u] l4proto[%u] l3proto[%u] intf[%u]",
			   IPV4_INFO(reset->flow.ipv4_sip),
			   reset->flow.src_port,
			   IPV4_INFO(reset->flow.ipv4_dip),
			   reset->flow.dst_port, reset->flow.l4proto,
			   reset->flow.l3proto, reset->flow.sk_dev_itf);
	} else {
		EMCOM_LOGI("receive reset. SrcIP[" IPV6_FMT "] SrcPort[%u] "
			   "DstIP[" IPV6_FMT
			   "] DstPort[%u] l4proto[%u] l3proto[%u] intf[%u]",
			   IPV6_INFO(reset->flow.ipv6_sip),
			   reset->flow.src_port,
			   IPV6_INFO(reset->flow.ipv6_dip),
			   reset->flow.dst_port, reset->flow.l4proto,
			   reset->flow.l3proto, reset->flow.sk_dev_itf);
	}
	EMCOM_LOGI("reset mode[%u] blinktime[%u]", reset->policy.rst_bind_mode,
		   reset->policy.const_perid);
	...
	case IPPROTO_TCP:
		emcom_xengine_mpflow_ai_tcp_intf_reset(app, reset,
						       &need_reset_device, sk);
		break;

	case IPPROTO_UDP:
		emcom_xengine_mpflow_ai_udp_intf_reset(reset,
						       &need_reset_device, sk);
		break;
	default:
	...
	if (need_reset_device) {
		...
		if (reset->flow.l3proto == ETH_P_IP) {
			EMCOM_LOGI(
				"[MPFlow_KERNEL]Reset Completed. SrcIP[" IPV4_FMT
				"] SrcPort[%u] "
				"DstIP[" IPV4_FMT
				"] DstPort[%u] l4proto[%u] intf[%u]",
				IPV4_INFO(reset->flow.ipv4_sip),
				reset->flow.src_port,
				IPV4_INFO(reset->flow.ipv4_dip),
				reset->flow.dst_port, reset->flow.l4proto,
			...
			EMCOM_LOGI(
				"[MPFlow_KERNEL]Reset Completed. SrcIP[" IPV6_FMT
				"] SrcPort[%u] "
				"DstIP[" IPV6_FMT
				"] DstPort[%u] l4proto[%u] intf[%u]",
				IPV6_INFO(reset->flow.ipv6_sip),
...
{
	if (strncmp(ifname, EMCOM_WLAN0_IFNAME, strlen(EMCOM_WLAN0_IFNAME)) ==
		    0 ||
	    strncmp(ifname, EMCOM_DEFAULT_WLAN0_IFNAME,
		    strlen(EMCOM_DEFAULT_WLAN0_IFNAME)) == 0) {
		return EMCOM_MPFLOW_BIND_WIFI0;
	}
	if (strncmp(ifname, EMCOM_CELL0_IFNAME, strlen(EMCOM_CELL0_IFNAME)) ==
	    0) {
		return EMCOM_MPFLOW_BIND_LTE0;
	}
	if (strncmp(ifname, EMCOM_WLAN1_IFNAME, strlen(EMCOM_WLAN1_IFNAME)) ==
		    0 ||
	    strncmp(ifname, EMCOM_DEFAULT_WLAN1_IFNAME,
		    strlen(EMCOM_DEFAULT_WLAN1_IFNAME)) == 0) {
		return EMCOM_MPFLOW_BIND_WIFI1;
	}
	if (strncmp(ifname, EMCOM_CELL1_IFNAME, strlen(EMCOM_CELL1_IFNAME)) ==
	    0) {
...
static void emcom_xengine_net_type_transfer_ifname(int net_type, char *ifname,
						   int len)
{
	int ret;
	switch (net_type) {
	...
	if (ret)
		EMCOM_LOGE("strcpy_s failed ret=%d, net_type=%d", ret,
			   net_type);
	...
	if (emcom_xengine_is_v6_sock(sk)) {
		if (ipv6_addr_equal(&sk->sk_v6_daddr, &sk->sk_v6_rcv_saddr)) {
			if (ipv6_addr_any(&sk->sk_v6_daddr))
				return false;
			EMCOM_LOGI("ipv6 loop sk_v6_daddr:" IPV6_FMT,
				   IPV6_INFO(sk->sk_v6_daddr));
			return true;
		}
	} else {
		if (sk->sk_daddr == sk->sk_rcv_saddr) {
			if (sk->sk_daddr == 0)
				return false;
			EMCOM_LOGI("ipv4 loop sk_daddr:" IPV4_FMT,
				   IPV4_INFO(sk->sk_daddr));
			return true;
		}
	}
 ...
 */
static void emcom_xengine_mpflow_ai_close_tcp_flow(
	const struct emcom_xengine_mpflow_ai_info *app, const uint16_t bindmode,
	const uint16_t ori_dev)
{
		...
		sk_nulls_for_each(sk, node, &head->chain) {
			if (sk->sk_state == TCP_ESTABLISHED ||
				...
				if ((!sk->is_mp_flow_bind &&
				     sk->sk_bound_dev_if) ||
				    emcom_xengine_mpflow_ai_is_excluded_port(
					    app, ntohs(sk->sk_dport),
					    IPPROTO_TCP))
					continue;

				if (netdev_get_name(sock_net(sk), ifname,
						    sk->sk_bound_dev_if) != 0) {
					if (sk->sk_bound_dev_if ==
					    EMCOM_MPFLOW_BIND_NONE) {
						int net_type =
							emcom_xengine_get_net_type(
								sk->sk_mark &
								0xFFFF);
						emcom_xengine_net_type_transfer_ifname(
							net_type, ifname,
							sizeof(ifname));
					} else {
						continue;
				...
				bind_type =
					emcom_xengine_transfer_ifname(ifname);
				if (bindmode == EMCOM_MPFLOW_BIND_NONE &&
				    ((sk->sk_bound_dev_if ==
				      EMCOM_MPFLOW_BIND_NONE) ||
				     bind_type ==
					     emcom_xengine_get_net_type(0) +
						     EMCOM_MPFLOW_BIND_WIFI0 -
						     EMCOM_XENGINE_NET_WLAN0)) {
					EMCOM_LOGI(
						"ignore flow already in defult dev, port: %u, state: %u devif: %d bind_type: %u",
						sk->sk_num, sk->sk_state,
						sk->sk_bound_dev_if, bind_type);
					continue;
				}
				if (bind_type == bindmode ||
				    bind_type == EMCOM_MPFLOW_BIND_NONE ||
				    (ori_dev && bind_type != ori_dev) ||
				    unlikely(!refcount_inc_not_zero(
					    &sk->sk_refcnt))) {
					continue;
				}
				sk_arr[accum] = sk;
	...
	EMCOM_LOGI(
		"reset tcp flow, uid: %u bindmode: %hu ori: %hu flow_num: %d",
		uid, bindmode, ori_dev, accum);
...
{
	struct inet_sock *inet = NULL;
	EMCOM_LOGI("clear sk %pK sport is %u, state[%u]", sk, sk->sk_num,
		   sk->sk_state);
	local_bh_disable();
	bh_lock_sock(sk);
	sk->is_mp_flow_bind = 0;
...
static void emcom_xengine_mpflow_ai_close_udp_flow(const uint32_t uid,
						   ...
						   const reset_act act)
{
		...
		sk_for_each(sk, &hslot->head) {
			if (sk->sk_state != TCP_SYN_RECV &&
				...
				if (netdev_get_name(sock_net(sk), ifname,
					...
					if (sk->sk_bound_dev_if ==
						...
						emcom_xengine_net_type_transfer_ifname(
							...
							sizeof(ifname));
						EMCOM_LOGI(
							"sk:%pK port: %u, state: %u devif: %d ifname: %s mark: %#x",
							sk, sk->sk_num,
							sk->sk_state,
							sk->sk_bound_dev_if,
							ifname, sk->sk_mark);
				...
				if (bindmode == EMCOM_MPFLOW_BIND_NONE &&
					...
					if (sk->sk_protocol == IPPROTO_UDP &&
					    sk->is_mp_flow_bind)
						emcom_xengine_mpflow_ai_udp_bind_clear(
							sk);
					EMCOM_LOGI(
						"ignore flow already in defult dev, port: %u, state: %u devif: %d bind_type: %u",
						sk->sk_num, sk->sk_state,
	...
	EMCOM_LOGI(
		"reset udp flow, uid: %u action: %u bindmode: %hu ori: %hu flow_num: %d",
	...
	for (; idx < accum; idx++) {
		if (act == SK_ERROR) {
			...
			emcom_xengine_mpflow_ai_path_handover(sk_arr[idx],
							      bindmode);
		} else {
			EMCOM_LOGE("close udp flow not support, action: %u",
				   act);
		}
		sock_gen_put(sk_arr[idx]);
	}
...
void emcom_xengine_mpflow_ai_close_all_flow(const uint8_t *data, uint16_t len)
{
	...
	low_bit = transfer->policy.rst_bind_mode &
		  EMCOM_MPFLOW_BINDMODE_ORI_DEV_MASK;
	ori_dev = transfer->policy.rst_bind_mode >>
		  EMCOM_MPFLOW_BINDMODE_ORI_DEV_OFFSET;
	uid = transfer->uid;
	bindmode = emcom_xengine_transfer_bindmode(low_bit);
	ori_dev = emcom_xengine_transfer_bindmode(ori_dev);
	act = transfer->policy.act;
	EMCOM_LOGI(
		"reset all flow:uid[%u] low_bit[%#x] bindmode[%hu] act[%d] time_ms[%u]",
...
void emcom_xengine_mpflow_ai_change_burst_ratio(const uint8_t *data,
						uint16_t len)
{
	...
	if ((data == NULL) || (len < sizeof(struct change_burst_ratio))) {
		EMCOM_LOGE("change burst ratio pointer null or length %u error",
			   len);
		return;
	}
	...
	app = &g_mpflow_ai_uids[app_index];
	app->burst_info.burst_ratio[WLAN0_RATIO] =
		new_burst_ratio->ratio[WLAN0_RATIO];
	app->burst_info.burst_ratio[CELL0_RATIO] =
		new_burst_ratio->ratio[CELL0_RATIO];
	app->burst_info.burst_ratio[WLAN1_RATIO] =
		new_burst_ratio->ratio[WLAN1_RATIO];
	app->burst_info.burst_ratio[CELL1_RATIO] =
		new_burst_ratio->ratio[CELL1_RATIO];
	spin_unlock_bh(&g_mpflow_ai_lock);
}
...
void emcom_xengine_mpflow_ai_app_clear(int8_t index, uid_t uid)
{
	...
	g_mpflow_ai_uids[index].rst_bind_mode =
		EMCOM_XENGINE_MPFLOW_AI_BINDMODE_NONE;
...
static void
emcom_xengine_mpflow_ai_hash_clear(struct emcom_xengine_mpflow_ai_priv *priv)
{
	struct emcom_xengine_mpflow_ip_bind_policy *ip = NULL;
	struct hlist_node *tmp = NULL;
	int i;
...
void emcom_xengine_mpflow_ai_ip_config(const char *data, uint16_t len)
{
	...
	app = &g_mpflow_ai_uids[app_index];
	if (!emcom_xengine_mpflow_ai_get_addr(config->sa_family,
					      &config->v4addr, daddr,
					      sizeof(daddr) / sizeof(__be32))) {
		EMCOM_LOGE("get address fail, uid: %d", config->uid);
		spin_unlock_bh(&g_mpflow_ai_lock);
		return;
	}

	ip = emcom_xengine_mpflow_ai_hash(daddr, sizeof(daddr) / sizeof(__be32),
					  app->priv->hashtable);
	if (!ip) {
	...
	if (config->sa_family == AF_INET) {
		EMCOM_LOGI("ip policy config. dest ip:" IPV4_FMT
			   ", bindmode: %#x, uid: %u",
			   IPV4_INFO(config->v4addr), config->bind_mode,
			   config->uid);
	} else if (config->sa_family == AF_INET6) {
		EMCOM_LOGI("ip policy config. dest ip:" IPV6_FMT
			   ", bindmode: %#x, uid: %u",
			   IPV6_INFO(config->v6addr), config->bind_mode,
			   config->uid);
	}
	if (config->bind_mode != EMCOM_MPFLOW_BIND_NONE) {
		...
		emcom_xengine_mpflow_ai_hash_delete(
			app->priv, daddr, sizeof(daddr) / sizeof(__be32));
	}
	...
	struct emcom_xengine_mpflow_ai_info *app)
{
	...
	for (index = 0; index < config->policy_num; index++) {
		...
		if ((policy->l4_protocol != IPPROTO_TCP) &&
		    (policy->l4_protocol != IPPROTO_UDP)) {
		...
		for (i = 0; (i < policy->port_num) &&
			    (i < EMCOM_MPFLOW_BIND_PORT_CFG_SIZE);
		     i++) {
			EMCOM_LOGI("port[%d, %d]",
				   policy->port_range[i].start_port,
				   policy->port_range[i].end_port);

			if (policy->port_range[i].start_port >
			    policy->port_range[i].end_port) {
				EMCOM_LOGE("error port");
				continue;
			}
			app->ports[app->port_num].protocol =
				policy->l4_protocol;
			app->ports[app->port_num].range.start_port =
				policy->port_range[i].start_port;
			app->ports[app->port_num].range.end_port =
				policy->port_range[i].end_port;

			/* reserved for future logic */
			app->ports[app->port_num].pattern.select_mode =
				policy->mode;
			app->ports[app->port_num].pattern.ratio[WLAN0_RATIO] =
				policy->ratio[WLAN0_RATIO];
			app->ports[app->port_num].pattern.ratio[CELL0_RATIO] =
				policy->ratio[CELL0_RATIO];
			app->ports[app->port_num].pattern.ratio[WLAN1_RATIO] =
				policy->ratio[WLAN1_RATIO];
			app->ports[app->port_num].pattern.ratio[CELL1_RATIO] =
				policy->ratio[CELL1_RATIO];
			/* reserved for future logic end */

			if (policy->l4_protocol == IPPROTO_TCP) {
				app->ports[app->port_num].bind_mode =
...
void emcom_xengine_mpflow_ai_init_bind_config(const char *data, uint16_t len)
{
	...
	if (!data ||
	    (len != sizeof(struct emcom_xengine_mpflow_ai_init_bind_cfg))) {
		EMCOM_LOGE("input length error expect: %zu, real: %u",
			   sizeof(struct emcom_xengine_mpflow_ai_init_bind_cfg),
			   len);
		return;
	}
	...
	EMCOM_LOGI("[MPFlow_KERNEL] Config received. policy num: %u",
		   config->policy_num);
	...
	EMCOM_LOGI("burst: proto[%u] mode[%u] portnum[%u]",
		   burst_policy->l4_protocol, burst_policy->mode,
		   burst_policy->port_num);

	if (burst_policy->l4_protocol != 0) {
		if ((burst_policy->l4_protocol != IPPROTO_TCP) &&
		    (burst_policy->l4_protocol != IPPROTO_UDP)) {
			EMCOM_LOGE("burst: invalid protocol: %d",
				   burst_policy->l4_protocol);
			return;
		}
	}
	...
	app->burst_info.burst_ratio[WLAN0_RATIO] =
		burst_policy->ratio[WLAN0_RATIO];
	app->burst_info.burst_ratio[CELL0_RATIO] =
		burst_policy->ratio[CELL0_RATIO];
	app->burst_info.burst_ratio[WLAN1_RATIO] =
		burst_policy->ratio[WLAN1_RATIO];
	app->burst_info.burst_ratio[CELL1_RATIO] =
		burst_policy->ratio[CELL1_RATIO];

	j = 0;
	for (i = 0; i < burst_policy->port_num; i++) {
		EMCOM_LOGI("burst: port range[%d, %d]",
			   burst_policy->port_range[i].start_port,
			   burst_policy->port_range[i].end_port);

		if (burst_policy->port_range[i].start_port >
		    burst_policy->port_range[i].end_port) {
			EMCOM_LOGE("burst: invalid port range");
			continue;
		}
		app->burst_info.burst_ports[j].start_port =
			burst_policy->port_range[i].start_port;
		app->burst_info.burst_ports[j].end_port =
			burst_policy->port_range[i].end_port;
		j++;
	}
...
int8_t emcom_xengine_mpflow_ai_get_port_index(
	int8_t index, struct emcom_xengine_mpflow_dport_range *range,
	uint8_t proto)
{
	...
	for (i = 0; i < app->port_num; i++) {
		...
		if ((range->start_port == exist->start_port) &&
		    (range->end_port == exist->end_port) &&
...
static void emcom_xengine_mpflow_ai_clear_blocked(int uid, uint16_t net_bit)
{
	...
	for (index = 0; index < EMCOM_MPFLOW_MAX_LIST_NUM; index++) {
		...
		if (node->uid == uid &&
			...
			uint8_t bind_device =
				emcom_xengine_transfer_ifname(node->name);
			...
			if (time_before_eq(
				    jiffies,
				    node->report_jiffies +
					    EMCOM_MPFLOW_FALLBACK_WAIT_TIME *
						    HZ)) {
				continue;
			}
			node->mpflow_fallback = EMCOM_MPFLOW_FALLBACK_CLR;
...
void emcom_xengine_mpflow_ai_iface_cfg(const char *data, uint16_t len)
{
	...
	if (!data ||
	    (len != sizeof(struct emcom_xengine_mpflow_ai_iface_config))) {
		EMCOM_LOGE("invalid data length %u expect: %zu", len,
	...
	if (config->port_range.start_port > config->port_range.end_port) {
		EMCOM_LOGE("invalid port range[%d, %d]",
			   config->port_range.start_port,
			   config->port_range.end_port);
		return;
	}
	...
	port_index = emcom_xengine_mpflow_ai_get_port_index(
		app_index, &config->port_range, config->l4protocol);
	...
	config_high_bit = (config->bind_mode >> EMCOM_MPFLOW_BINDMODE_SHIFT) &
			  EMCOM_MPFLOW_BINDMODE_MASK;
	app_high_bit = (app->ports[port_index].bind_mode >>
			EMCOM_MPFLOW_BINDMODE_SHIFT) &
		       EMCOM_MPFLOW_BINDMODE_MASK;
	if (config->bind_mode != app->ports[port_index].bind_mode) {
		if ((config->bind_mode & EMCOM_MPFLOW_BINDMODE_MASK) == 0 &&
		    app->priv)
			emcom_xengine_mpflow_ai_hash_clear(app->priv);
		if (config_high_bit != app_high_bit)
			emcom_xengine_mpflow_ai_clear_blocked(
				config->uid, (config_high_bit ^ app_high_bit) &
						     config_high_bit);
		app->ports[port_index].pattern.tot_cnt = 0;
		EMCOM_LOGI("iface policy port[%d, %d] bind mode: %#x",
			   config->port_range.start_port,
			   config->port_range.end_port, config->bind_mode);
	}
...
static bool emcom_xengine_mpflow_ai_is_block(const int bind_device,
					     const bool is_wifi0_block,
					     const bool is_lte0_block,
					     const bool is_wifi1_block,
					     const bool is_lte1_block)
{
	bool res = true;
...
int emcom_xengine_mpflow_ai_bind_random(
	struct emcom_xengine_mpflow_ai_bind_pattern *calc, uid_t uid,
	uint16_t high_bit)
{
	uint16_t bind_device = calc->last_device;
	int cnt = 0;
	bool is_wifi0_block = emcom_xengine_mpflow_blocked(
		uid, EMCOM_WLAN0_IFNAME, EMCOM_MPFLOW_VER_V2);
	bool is_lte0_block = emcom_xengine_mpflow_blocked(
		uid, EMCOM_CELL0_IFNAME, EMCOM_MPFLOW_VER_V2);
	bool is_wifi1_block = emcom_xengine_mpflow_blocked(
		uid, EMCOM_WLAN1_IFNAME, EMCOM_MPFLOW_VER_V2);
	bool is_lte1_block = emcom_xengine_mpflow_blocked(
		uid, EMCOM_CELL1_IFNAME, EMCOM_MPFLOW_VER_V2);
	...
	do {
		...
		if (bind_device == EMCOM_MPFLOW_BIND_LTE1) {
		 ...
		 emcom_xengine_mpflow_ai_is_block(bind_device, is_wifi0_block,
						  is_lte0_block, is_wifi1_block,
						  is_lte1_block));
...
bool emcom_xengine_mpflow_ai_start(uid_t uid)
{
	...
	if (index == INDEX_INVALID) {
		...
		if (newindex == INDEX_INVALID) {
			EMCOM_LOGE(
				"mpflow ai start get free index exceed. uid: %d",
				uid);
			spin_unlock_bh(&g_mpflow_ai_lock);
			return false;
		}
	...
	if (is_new_uid) {
		...
		priv = kzalloc(sizeof(struct emcom_xengine_mpflow_ai_priv),
			       GFP_ATOMIC);
		if (!priv) {
			EMCOM_LOGE("mpflow ai start alloc priv failed, uid: %d",
				   uid);
			spin_unlock_bh(&g_mpflow_ai_lock);
			return false;
		}
...
void emcom_xengine_mpflow_ai_stop(const char *pdata, uint16_t len)
{
	...
	EMCOM_LOGD("mpflow ai stop uid: %u, stop reason: %u", stop->uid,
		   stop_reason);
...
bool emcom_xengine_mpflow_ai_in_busrt_range(int8_t index, uint16_t dport,
					    uint8_t proto)
{
	struct emcom_xengine_mpflow_ai_info *app = &g_mpflow_ai_uids[index];
	...
	if (app->burst_info.burst_protocol == 0 ||
	    app->burst_info.burst_port_num == 0) {
		return true;
	}
	...
	for (i = 0; i < app->burst_info.burst_port_num; i++) {
		...
		if ((entry->start_port <= dport) &&
		    (dport <= entry->end_port)) {
			return true;
		}
	}
	return false;
...
static bool emcom_xengine_mpflow_ai_bind_block(uid_t uid, uint16_t qos_device)
{
	...
	if ((is_wifi0_block && (qos_device == EMCOM_MPFLOW_BIND_WIFI0)) ||
	    ...
	    ((is_wifi0_block == true) && (is_lte0_block == true) &&
	     (is_wifi1_block == true) && (is_lte1_block == true))) {
		EMCOM_LOGI(
			"mpflow bind blocked uid: %u, bindmode: %#x, blocked:%d, %d, %d, %d",
			uid, qos_device, is_wifi0_block, is_lte0_block,
			is_wifi1_block, is_lte1_block);
		return true;
	}
...
static void emcom_xengine_mpflow_ai_get_ratio(
...
int emcom_xengine_mpflow_ai_bind_burst(
	int8_t index, uint16_t dport, uint8_t proto, uint32_t bind_mode,
	struct emcom_xengine_mpflow_ai_bind_pattern *pattern_calc, uid_t uid)
{
	uint16_t low_bit = bind_mode & EMCOM_MPFLOW_BINDMODE_MASK;
	uint16_t high_bit = (bind_mode >> EMCOM_MPFLOW_BINDMODE_SHIFT) &
			    EMCOM_MPFLOW_BINDMODE_MASK;
	...
	if (emcom_xengine_mpflow_ai_in_busrt_range(index, dport, proto)) {
		...
		struct emcom_xengine_mpflow_ai_burst_port *burst_info =
			&g_mpflow_ai_uids[index].burst_info;
		emcom_xengine_mpflow_ai_get_ratio(burst_info, cur_burst_ratio,
						  high_bit);
		EMCOM_LOGI(
			"burst current qos device: %u last burst qos device: %u, burst cnt: %lu, mode[%u]",
			qos_device, burst_info->launch_device,
			burst_info->burst_cnt, burst_info->burst_mode);

		if (burst_info->burst_mode != EMCOM_MPFLOW_IP_BURST_FIX)
			return qos_device;

		if (!cur_burst_ratio[0] && !cur_burst_ratio[1] &&
		    !cur_burst_ratio[2] && !cur_burst_ratio[3])
			return qos_device;
		...
		/*lint -e666*/
		if (time_after(jiffies, (burst_info->jiffies +
					 EMCOM_MPFLOW_FLOW_BIND_BURST_TIME)) ||
		    (burst_info->jiffies == 0)) {
			burst_device = qos_device;
			burst_info->burst_cnt = 0;
			burst_info->jiffies = jiffies;
			burst_info->launch_device =
				burst_device; // update launch device every burst period begining
		} else {
			// base device
			launch_device = burst_info->launch_device;
			launch_cnt = burst_info->burst_cnt %
				     (cur_burst_ratio[0] + cur_burst_ratio[1] +
				      cur_burst_ratio[2] + cur_burst_ratio[3]);
			...
			while (launch_cnt >= cur_burst_ratio[cur_pos]) {
				launch_cnt =
					launch_cnt - cur_burst_ratio[cur_pos];
...
int emcom_xengine_mpflow_ai_get_reset_device(uint16_t mode)
{
	...
	EMCOM_LOGI("mpflow ai bind using reset mode: %u, device: %u", mode,
		   bind_device);
	return bind_device;
}

int8_t emcom_xengine_mpflow_ai_get_port_index_in_range(int8_t index,
						       uint16_t dport,
						       uint8_t proto)
{
	struct emcom_xengine_mpflow_ai_info *app = &g_mpflow_ai_uids[index];
	...
	for (i = 0; i < app->port_num; i++) {
		exist = &app->ports[i].range;
		if ((exist->start_port <= dport) &&
		    (dport <= exist->end_port) &&
...
int emcom_xengine_mpflow_ai_getmode(int8_t index, uid_t uid,
				    struct sockaddr *uaddr, uint8_t proto)
{
	...
	if (app->rst_all_flow)
		return emcom_xengine_mpflow_ai_get_reset_device(
			app->all_flow_mode);
	...
	port_index = emcom_xengine_mpflow_ai_get_port_index_in_range(
		index, dport, proto);
	...
	ip_policy = emcom_xengine_mpflow_ai_hash_get_policy(
		daddr, sizeof(daddr) / sizeof(__be32), app->priv->hashtable);
	if (ip_policy) {
	...
	high_bit = (bind_mode >> EMCOM_MPFLOW_BINDMODE_SHIFT) &
		   EMCOM_MPFLOW_BINDMODE_MASK;

	if (low_bit == EMCOM_MPFLOW_BINDMODE_MASK) {
		bind_mode = emcom_xengine_mpflow_ai_bind_random(pattern_calc,
								uid, high_bit);
	} else if (low_bit == 0) {
		bind_mode = EMCOM_MPFLOW_BIND_NONE;
		EMCOM_LOGD("mpflow ai get spec mod error, mode: %u", bind_mode);
	} else {
		bind_mode = emcom_xengine_mpflow_ai_bind_burst(
			index, dport, proto, bind_mode, pattern_calc, uid);
	}
...
bool emcom_xengine_mpflow_ai_check_port(int8_t index, uint16_t dport)
{
	...
	for (i = 0; i < app->port_num; i++) {
		...
		if (mpflow_ai_in_range(dport, range->start_port,
				       range->end_port)) {
			is_in_range = true;
			break;
		}
...
bool emcom_xengine_mpflow_ai_is_excluded_port(
	const struct emcom_xengine_mpflow_ai_info *app, uint16_t dport,
	uint8_t proto)
{
	unsigned int i;
...
bool emcom_xengine_mpflow_ai_checkvalid(struct sock *sk, struct sockaddr *uaddr,
					int8_t index, uint16_t dport)
{
	...
	if (emcom_xengine_mpflow_ai_is_excluded_port(app, dport,
						     sk->sk_protocol))
		return false;
	...
	if (app->enableflag & EMCOM_MPFLOW_AI_ENABLEFLAG_DPORT) {
		...
		if (time_after(jiffies, app->all_flow_jiffies +
						app->all_flow_duration)) {
		...
		EMCOM_LOGD(
			"mpflow ai check uid: %u sk: %pK, famliy: %d, sk_proto: %d, "
		...
		if (sk->sk_protocol != app->ports[port_index].protocol) {
			EMCOM_LOGD("protocol not match uid: %u, sk: %pK",
				   app->uid, sk);
			return false;
		}
	} else {
		EMCOM_LOGD(
			"mpflow ai valid uid: %u enableflag: %u sk: %pK, famliy: %d, sk_proto: %d, "
			"policy_proto: %d, port: %d bindmode: %#x",
			app->uid, app->enableflag, sk, sk->sk_family,
			sk->sk_protocol, app->ports[port_index].protocol, dport,
			app->ports[port_index].bind_mode);
	}
...
static int emcom_xengine_mpflow_rehash(int bind_device, struct sock *sk,
				       int8_t index, struct inet_sock *inet,
				       uid_t uid, uint16_t dport,
				       struct sockaddr *uaddr)
{
	...
	if (bind_device == EMCOM_MPFLOW_BIND_WIFI0) {
		...
		err = memcpy_s(ifname, sizeof(char) * IFNAMSIZ,
			       EMCOM_WLAN1_IFNAME,
			       (strlen(EMCOM_WLAN1_IFNAME) + 1));
	} else if (bind_device == EMCOM_MPFLOW_BIND_LTE1) {
		err = memcpy_s(ifname, sizeof(char) * IFNAMSIZ,
			       EMCOM_CELL1_IFNAME,
			       (strlen(EMCOM_CELL1_IFNAME) + 1));
		...
		EMCOM_LOGD(
			"EMCOM_MPFLOW_BIND_NONE uid: %u, sk: %pK, protocol: %u, DstPort: %u, dev_if: %u",
	...
	if (!dev || (emcom_xengine_mpflow_getinetaddr(dev) == false)) {
		rcu_read_unlock();
		EMCOM_LOGD(
			"device not ready uid: %u, sk: %pK, dev: %pK, name: %s",
	...
	if (sk->sk_protocol == IPPROTO_TCP &&
	    emcom_mpdns_is_allowed(uid, NULL)) {
...
void emcom_xengine_mpflow_ai_bind2device(struct sock *sk,
					 struct sockaddr *uaddr)
{
	uint16_t dport = 0;
	int bind_device;
	...
	if (sk->sk_protocol == IPPROTO_TCP &&
	    (sk->sk_bound_dev_if || sk->is_mp_flow_bind))
		return;
	...
	emcom_xengine_mpflow_ai_get_port(uaddr,
					 &dport); // not check return value
	if (emcom_xengine_mpflow_ai_checkvalid(sk, uaddr, index, dport) ==
	    false) {
	...
	bind_device = emcom_xengine_mpflow_ai_getmode(index, uid, uaddr,
						      sk->sk_protocol);
	spin_unlock_bh(&g_mpflow_ai_lock);

	sk->is_mp_flow_bind = 1;

	if (emcom_xengine_mpflow_rehash(bind_device, sk, index, inet, uid,
					dport, uaddr) != MPFLOW_OK)
		return;

	usin = (struct sockaddr_in *)uaddr;
	if (usin->sin_family == AF_INET) {
		EMCOM_LOGI(
			"[MPFlow_KERNEL] Bind Completed. SrcIP: *.*, SrcPort: %u, DstIP:" IPV4_FMT
			", DstPort: %u ",
		...
		EMCOM_LOGI(
			"[MPFlow_KERNEL] Bind Completed. SrcIP: *.*, SrcPort: %u, DstIP:" IPV6_FMT
			", DstPort: %u ",
