...
void emcom_xengine_supply_mpdns_cache(const struct sk_buff *skb,
				      const char *host, int ip_type);
...
static inline struct emcom_xengine_netem_skb_cb *
emcom_xengine_netem_skb_cb(const struct sk_buff *skb)
{
	/* we assume we can use skb next/prev/tstamp as storage for rb_node */
	qdisc_cb_private_validate(skb,
				  sizeof(struct emcom_xengine_netem_skb_cb));
...
void emcom_xengine_init(void)
{
	...
	(void)memset_s(g_network_infos, sizeof(g_network_infos), 0,
		       sizeof(g_network_infos));
	emcom_mpdns_init();
}
...
int emcom_xengine_clear(void)
{
	...
	err = memset_s(g_ifacename, sizeof(char) * IFNAMSIZ, 0,
		       sizeof(char) * IFNAMSIZ);
	if (err != EOK)
...
uint8_t emcom_xengine_found_avaiable_accindex(uid_t uid)
{
	...
	if (old_age > EMCOM_UID_ACC_AGE_MAX) {
		EMCOM_LOGD(
			"Emcom_Xengine_StartAccUid: uid:%d added too long, remove it",
	...
	EMCOM_LOGD(
		"Emcom_Xengine_StartAccUid: idle_index=%d,old_index=%d,old_age=%d",
	...
	if (old_index < EMCOM_MAX_ACC_APP) {
		EMCOM_LOGD(
			"Emcom_Xengine_StartAccUid: uid:%d replace the oldest uid:%d",
...
int emcom_xengine_set_speedctrl_info(const uint8_t *data, uint16_t len)
{
	...
	if (len != sizeof(struct emcom_xengine_speed_ctrl_data)) {
		EMCOM_LOGI("Emcom_Xengine_SetSpeedCtrlInfo: len:%d is illegal",
			   len);
		return -EINVAL;
	}
	...
	if (!uid && !size) {
		EMCOM_LOGD(
			"Emcom_Xengine_SetSpeedCtrlInfo: clear speed ctrl state");
		EMCOM_XENGINE_SET_SPEEDCTRL(g_speedctrl_info, UID_INVALID_APP,
					    0);
		return 0;
	}
	...
	/* check uid */
	if (invalid_uid(uid)) {
		EMCOM_LOGI("Emcom_Xengine_SetSpeedCtrlInfo: uid:%d is illegal",
			   uid);
		return -EINVAL;
	}
	...
	if (invalid_speedctrl_size(size)) {
		EMCOM_LOGI("Emcom_Xengine_SetSpeedCtrlInfo: size:%d is illegal",
			   size);
		return -EINVAL;
	}
...
void emcom_xengine_speedctrl_winsize(struct sock *pstsock, uint32_t *pstsize)
{
	...
	if (pstsize == NULL) {
		EMCOM_LOGD(
			" Emcom_Xengine_SpeedCtrl_WinSize window size invalid\n");
		return;
	}
	...
	if (size)
		*pstsize = (g_speedctrl_info.size < *pstsize) ?
				   g_speedctrl_info.size :
				   *pstsize;
}
...
int emcom_xengine_config_mpip(const uint8_t *data, uint16_t len)
{
	...
	for (index = 0; index < length; index++) {
		...
		EMCOM_LOGD("The Mpip config [%d] is: lUid %d and type %d",
			   index, g_mpip_uids[index].uid,
			   g_mpip_uids[index].type);
...
void emcom_xengine_mpip_bind2device(struct sock *pstsock)
{
	...
	if (found != -1) {
		...
		if (!dev || !test_bit(__LINK_STATE_START, &dev->state)) {
			...
			g_mpip_start = false;
			mutex_unlock(&g_mpip_mutex);
			emcom_send_msg2daemon(NETLINK_EMCOM_KD_XENIGE_DEV_FAIL,
					      NULL, 0);
			EMCOM_LOGE(" get dev fail or dev is not up");
			return;
		}

		if (g_mpip_uids[found].type ==
			...
			g_socket_index++;
			g_socket_index =
				g_socket_index % EMCOM_MAX_MPIP_DEV_NUM;
		} else {
			lock_sock(pstsock);
			pstsock->sk_bound_dev_if = index;
			sk_dst_reset(pstsock);
...
void emcom_xengine_udpenqueue(const struct sk_buff *skb)
{
	...
	if (sock_uid == g_udp_retran_uid) {
		...
		if (sk->sk_socket->type == SOCK_DGRAM) {
			...
			if (unlikely(!skb2)) {
				EMCOM_LOGE(
					"Emcom_Xengine_UdpEnqueue skb2 null");
				return;
			}
			dev_queue_xmit(skb2);
			return;
...
int emcom_xengine_start_udpretran(const uint8_t *data, uint16_t len)
{
	...
	/* check len is invalid */
	if (len != sizeof(uid_t)) {
		EMCOM_LOGI("Emcom_Xengine_StartUdpReTran: len: %d is illegal",
			   len);
		return -EINVAL;
	}
	...
	/* check uid */
	if (invalid_uid(uid)) {
		EMCOM_LOGE("Emcom_Xengine_StartUdpReTran: uid is invalid %d",
			   uid);
		return -EINVAL;
	}
...
int emcom_xengine_start_fastsyn(const uint8_t *data, uint16_t len)
{
	...
	/* check len is invalid */
	if (len != sizeof(uid_t)) {
		EMCOM_LOGI(" emcom_xengine_start_fastsyn: len: %d is illegal",
			   len);
		return -EINVAL;
	}
	...
	/* check uid */
	if (invalid_uid(uid)) {
		EMCOM_LOGE(" emcom_xengine_start_fastsyn: uid is invalid %d",
			   uid);
		return -EINVAL;
	}
...
void emcom_xengine_active_ccalg(const uint8_t *data, uint16_t len)
{
	...
	if (index != INDEX_INVALID) {
		if (g_ccalg_uids[index].alg == alg) {
			...
			EMCOM_LOGD("alg: %u is activating again, uid: %u", alg,
				   uid);
		} else {
			/* already activated, but change another algorithm */
			EMCOM_LOGD(
				"CCAlg function is ready to change alg, uid: %u, alg from %u to %u",
		...
		if (index_new != INDEX_INVALID) {
			...
			g_ccalg_uid_cnt++;
			EMCOM_LOGD(
				"CCAlg function is ready to start, uid: %u, alg: %u",
				uid, alg);
		} else if (g_ccalg_uid_cnt >= EMCOM_MAX_CCALG_APP) {
			EMCOM_LOGE(
				"CCAlg has already activated %d apps, cannot activate more apps",
				g_ccalg_uid_cnt);
			return;
		} else {
			EMCOM_LOGE(
				"not supposed to happend: CCAlg has already activated %d apps",
				g_ccalg_uid_cnt);
		}
	}
}
...
void emcom_xengine_deactive_ccalg(const uint8_t *data, uint16_t len)
{
	...
	index = emcom_xengine_find_ccalg(uid);
	if (index != INDEX_INVALID) {
		EMCOM_LOGD("CCAlg function is ready to stop, uid: %u, alg: %u",
			   uid, g_ccalg_uids[index].alg);
		g_ccalg_uids[index].uid = UID_INVALID_APP;
		g_ccalg_uids[index].alg = EMCOM_XENGINE_CONG_ALG_INVALID;
		g_ccalg_uids[index].has_log = false;
		g_ccalg_uid_cnt--;
		...
		EMCOM_LOGE(
			"CCAlg function is not activated yet, cannot be deactivated, uid: %u",
			uid);
		return;
	}
	...
	if (g_ccalg_uid_cnt <= 0) {
		EMCOM_LOGD(
			" no ccalg is activated now: CCAlg function is ready to stop, cnt: %u",
			g_ccalg_uid_cnt);
		g_ccalg_start = false;
		return;
	}
 ...
 */
void emcom_xengine_change_default_ca(struct sock *sk,
				     struct list_head tcp_cong_list)
{
	struct inet_connection_sock *icsk = NULL;
		...
		if (likely(try_module_get(ca->owner))) {
			if (strcmp(ca->name, app_alg) == 0) {
				...
				if (g_ccalg_uids[index].has_log == false) {
					EMCOM_LOGD(
						"app: %d change default congcontrol alg to :%s",
						sock_i_uid(sk).val, app_alg);
	...
	if (g_ccalg_uids[index].has_log == false) {
		EMCOM_LOGE(
			"Emcom_Xengine_change_default_ca failed to find algorithm %s",
			EMCOM_CONGESTION_CONTROL_ALG_BBR);
		g_ccalg_uids[index].has_log = true;
	}
}
...
bool emcom_xengine_check_ip_addrss(struct sockaddr *addr)
{
	...
	if (usin->sin_family == AF_INET) {
		return !ipv4_is_loopback(usin->sin_addr.s_addr) &&
...
bool emcom_xengine_is_private_addr(__be32 addr)
{
	return (ipv4_is_linklocal_169(addr) || ipv4_is_private_10(addr) ||
		ipv4_is_private_172(addr) || ipv4_is_private_192(addr));
}
...
bool emcom_xengine_check_ip_is_private(struct sockaddr *addr)
{
	...
	if (usin->sin_family == AF_INET) {
		...
		if (addr_type & IPV6_ADDR_MAPPED) {
			__be32 s_addr =
				usin6->sin6_addr.s6_addr32
					[EMCOM_MPFLOW_FI_CLAT_IPV4_INDEX];
...
void emcom_xengine_evt_proc(int32_t event, const uint8_t *data, uint16_t len)
{
	...
	case NETLINK_EMCOM_DK_ACTIVE_CCALG:
		EMCOM_LOGD(
			" emcom netlink active congestion control algorithm");
		emcom_xengine_active_ccalg(data, len);
		break;
	case NETLINK_EMCOM_DK_DEACTIVE_CCALG:
		EMCOM_LOGD(
			" emcom netlink deactive congestion control algorithm");
...
int emcom_xengine_setproxyuid(struct sock *sk, const char __user *optval,
			      int optlen)
{
	uid_t uid = 0;
	int ret;
...
int emcom_xengine_setsockflag(struct sock *sk, const char __user *optval,
			      int optlen)
{
	int ret;
	int hicom_flag = 0;
...
static int
emcom_xengine_bind2device_uid(struct emcom_xengine_bind2device_node *cfg)
{
	...
	list_for_each_entry_rcu(node, &bind2device_list, list) {
		if (node->uid == cfg->uid) {
			if (cfg->add) {
				err = strncpy_s(node->iface_name, IFNAMSIZ,
						cfg->iface_name, IFNAMSIZ - 1);
				if (err != EOK)
...
static struct file *emcom_xengine_fget_by_pid(pid_t pid, unsigned int fd,
					      fmode_t mask)
{
...
static int
emcom_xengine_bind2device_fd(const struct emcom_xengine_bind2device_node *cfg)
{
	struct file *file = NULL;
...
int emcom_xengine_setbind2device(const struct sock *sk,
				 const char __user *optval, int optlen)
{
	int ret;
	bool need_free = true;
	struct emcom_xengine_bind2device_node *node = NULL;
	...
	node = kmalloc(sizeof(struct emcom_xengine_bind2device_node),
		       GFP_ATOMIC);
	if (!node)
		return -ENOMEM;
	...
	if (node->iface_name[0] == '\0' ||
	    strnlen(node->iface_name, IFNAMSIZ) >= IFNAMSIZ) {
		kfree(node);
		return -EINVAL;
	}
...
void emcom_xengine_bind(struct sock *sk)
{
	...
	list_for_each_entry_rcu(node, &bind2device_list, list) {
		if (node->uid == uid) {
			dev = dev_get_by_name_rcu(sock_net(sk),
						  node->iface_name);
			if (dev) {
				unsigned int flags = dev_get_flags(dev);
				if (flags & IFF_RUNNING) {
...
void emcom_xengine_notify_sock_error(struct sock *sk)
{
	if (sk->hicom_flag == HICOM_SOCK_FLAG_FINTORST) {
		EMCOM_LOGD(" hicom change fin to rst, uid: %u, flag: %x",
			   sk->sk_uid.val, sk->hicom_flag);
...
void emcom_xengine_process_http_req(const struct sk_buff *skb, int ip_type)
{
	...
	if (!emcom_parse_httpget_host(payload, host, EMCOM_MAX_HOST_LEN) ||
	    !emcom_is_alp_host(host))
		return;
				      ...
				      const char *host, int ip_type)
{
	...
	if (ip_type == EMCOM_XENGINE_IP_TYPE_V4) { // no use sk->sk_family
		...
		need_add_proc = emcom_mpdns_supply_v4_cache(host, act_addr,
							    act_net_type);
		...
		need_add_proc = emcom_mpdns_supply_v6_cache(host, act_addr,
							    act_net_type);
	} else {
		// do nothing
	}
...
void emcom_xengine_update_network_info(const char *data, uint16_t len)
{
	...
	if (data == NULL ||
	    (len != sizeof(struct emcom_xengine_network_info))) {
	...
	EMCOM_LOGD(
		"net type:%d, net id:%d, validated:%d, default:%d, iface: %s",
		net_info->net_type, net_info->net_id, net_info->is_validated,
		net_info->is_default, net_info->iface_name);
	if (!emcom_xengine_is_net_type_valid(net_info->net_type))
		return;

	if (g_network_infos[net_info->net_type].is_validated !=
		    net_info->is_validated &&
	    net_info->is_validated != EMCOM_TRUE_VALUE)
		emcom_mpdns_flush_cache(
			net_info->net_type); // flush cache for unavailable net
	spin_lock_bh(&g_network_infos_cache_lock);
	err = memcpy_s(&(g_network_infos[net_info->net_type]),
		       sizeof(struct emcom_xengine_network_info), net_info,
		       len);
...
int emcom_xengine_get_net_type(int net_id)
{
	...
	if (net_id <= 0) {
		spin_lock_bh(&g_network_infos_cache_lock);
		for (net_type = EMCOM_XENGINE_NET_WLAN0;
		     net_type < EMCOM_XENGINE_NET_MAX_NUM; net_type++) {
...
int emcom_xengine_get_net_type_by_name(const char *iface_name)
{
	...
	for (net_type = EMCOM_XENGINE_NET_WLAN0;
	     net_type < EMCOM_XENGINE_NET_MAX_NUM; net_type++) {
		char *cur_iface_name =
			emcom_xengine_get_network_iface_name(net_type);
