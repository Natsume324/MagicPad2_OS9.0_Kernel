...
void emcom_mpdns_init(void)
{
	...
	(void)memset_s(&g_mpdns_cache, sizeof(struct emcom_mpdns_cache), 0,
		       sizeof(struct emcom_mpdns_cache));
...
struct emcom_mpdns_entry *emcom_mpdns_lookup_cache_entry(const char *host,
							 int *entry_id)
{
	unsigned int id;
...
bool emcom_get_first_v4_addr(struct in_addr *out_addr, struct in_addr *v4_addrs,
			     const unsigned num)
{
...
bool emcom_get_first_v6_addr(struct in6_addr *out_addr,
			     struct in6_addr *v6_addrs, const unsigned num)
{
	errno_t err;
	unsigned addr_id;
	for (addr_id = 0; addr_id < num; addr_id++) {
		if (emcom_is_v6_addr_valid(&(v6_addrs[addr_id]))) {
			err = memcpy_s(out_addr, sizeof(struct in6_addr),
				       &(v6_addrs[addr_id]),
...
bool emcom_v4_addr_match(struct in_addr *cur_addr, struct in_addr *v4_addrs,
			 const unsigned num)
{
	unsigned addr_id;
	for (addr_id = 0; addr_id < num; addr_id++) {
...
bool emcom_v6_addr_match(struct in6_addr *cur_addr, struct in6_addr *v6_addrs,
			 const unsigned num)
{
	unsigned addr_id;
	for (addr_id = 0; addr_id < num; addr_id++) {
		if (memcmp(cur_addr, &(v6_addrs[addr_id]),
...
bool emcom_lookup_target_v4_addr(struct in_addr *cur_addr, int cur_net,
				 int tar_net, struct in_addr *tar_addr)
{
	unsigned int entry_id;
	for (entry_id = 0; entry_id < EMCOM_MPDNS_MAX_ENTRY_NUM; entry_id++) {
		struct emcom_mpdns_entry *cur_entry =
			&(g_mpdns_cache.entries[entry_id]);
		struct in_addr *cur_forward_addrs =
			cur_entry->net_addrs[cur_net].ipv4_addrs;
		struct in_addr *cur_reverse_addrs =
			cur_entry->net_addrs[cur_net].ipv4_reverse_addrs;
		struct in_addr *tar_forward_addrs =
			cur_entry->net_addrs[tar_net].ipv4_addrs;
		struct in_addr *tar_reverse_addrs =
			cur_entry->net_addrs[tar_net].ipv4_reverse_addrs;
		if (emcom_v4_addr_match(cur_addr, cur_forward_addrs,
					EMCOM_MPDNS_MAX_IPV4_ADDR_NUM) ||
		    emcom_v4_addr_match(cur_addr, cur_reverse_addrs,
					EMCOM_MPDNS_MAX_IPV4_ADDR_NUM)) {
			if (emcom_get_first_v4_addr(
				    tar_addr, tar_forward_addrs,
				    EMCOM_MPDNS_MAX_IPV4_ADDR_NUM) ||
			    emcom_get_first_v4_addr(
				    tar_addr, tar_reverse_addrs,
				    EMCOM_MPDNS_MAX_IPV4_ADDR_NUM))
				return true;
		}
	}
...
bool emcom_lookup_target_v6_addr(struct in6_addr *cur_addr, int cur_net,
				 int tar_net, struct in6_addr *tar_addr)
{
	unsigned int entry_id;
	for (entry_id = 0; entry_id < EMCOM_MPDNS_MAX_ENTRY_NUM; entry_id++) {
		struct emcom_mpdns_entry *cur_entry =
			&(g_mpdns_cache.entries[entry_id]);
		struct in6_addr *cur_forward_addrs =
			cur_entry->net_addrs[cur_net].ipv6_addrs;
		struct in6_addr *cur_reverse_addrs =
			cur_entry->net_addrs[cur_net].ipv6_reverse_addrs;
		struct in6_addr *tar_forward_addrs =
			cur_entry->net_addrs[tar_net].ipv6_addrs;
		struct in6_addr *tar_reverse_addrs =
			cur_entry->net_addrs[tar_net].ipv6_reverse_addrs;
		if (emcom_v6_addr_match(cur_addr, cur_forward_addrs,
					EMCOM_MPDNS_MAX_IPV6_ADDR_NUM) ||
		    emcom_v6_addr_match(cur_addr, cur_reverse_addrs,
					EMCOM_MPDNS_MAX_IPV6_ADDR_NUM)) {
			if (emcom_get_first_v6_addr(
				    tar_addr, tar_forward_addrs,
				    EMCOM_MPDNS_MAX_IPV6_ADDR_NUM) ||
			    emcom_get_first_v6_addr(
				    tar_addr, tar_reverse_addrs,
				    EMCOM_MPDNS_MAX_IPV6_ADDR_NUM))
				return true;
		}
...
bool emcom_clear_v4_addr(struct in_addr *cur_addr, struct in_addr *v4_addrs,
			 const unsigned num)
{
	unsigned int addr_id;
	unsigned int i = 0;
	...
	struct in_addr *tmp_v4_addrs =
		kzalloc(sizeof(struct in_addr) * num, GFP_ATOMIC);
	...
	err = memcpy_s(v4_addrs, sizeof(struct in_addr) * num, tmp_v4_addrs,
		       sizeof(struct in_addr) * num);
	kfree(tmp_v4_addrs);
	return (err == EOK);
}

void emcom_clear_v4_reverse_addr(struct in_addr *cur_addr,
				 struct in_addr *v4_addrs, const unsigned num)
{
	unsigned int addr_id;
	for (addr_id = 0; addr_id < num; addr_id++) {
		if (v4_addrs[addr_id].s_addr == cur_addr->s_addr)
...
bool emcom_clear_v6_addr(struct in6_addr *cur_addr, struct in6_addr *v6_addrs,
			 const unsigned num)
{
	unsigned addr_id;
	unsigned int i = 0;
	...
	struct in6_addr *tmp_v6_addrs =
		kzalloc(sizeof(struct in6_addr) * num, GFP_ATOMIC);
	...
	err = memcpy_s(v6_addrs, sizeof(struct in6_addr) * num, tmp_v6_addrs,
		       sizeof(struct in6_addr) * num);
	kfree(tmp_v6_addrs);
	return (err == EOK);
}

void emcom_clear_v6_reverse_addr(struct in6_addr *cur_addr,
				 struct in6_addr *v6_addrs, const unsigned num)
{
	unsigned addr_id;
	for (addr_id = 0; addr_id < num; addr_id++) {
		if (memcmp(cur_addr, &(v6_addrs[addr_id]),
			   sizeof(struct in6_addr)) == 0)
			(void)memset_s(&(v6_addrs[addr_id]),
				       sizeof(struct in6_addr), 0,
				       sizeof(struct in6_addr));
	}
}

bool emcom_mpdns_fill_v4_cache(const char *host, struct in_addr *v4_addr,
			       int cur_net)
{
	errno_t err;
	int existed_id;
	...
	struct emcom_mpdns_entry *existed_entry =
		emcom_mpdns_lookup_cache_entry(host, &existed_id);
	if (existed_entry == NULL) {
		struct emcom_mpdns_entry *entry =
			&(g_mpdns_cache.entries[g_mpdns_cache.next_update_id]);
		(void)memset_s(entry, sizeof(struct emcom_mpdns_entry), 0,
			       sizeof(struct emcom_mpdns_entry));

		err = memcpy_s(entry->host, EMCOM_MAX_HOST_LEN, host,
			       strlen(host));
		if (err != EOK)
			return true;
		...
		v4_reverse_addrs[entry->net_addrs[cur_net].v4_raddr_update_id]
			.s_addr = v4_addr->s_addr;
		...
		struct emcom_mpdns_entry *entry =
			&(g_mpdns_cache.entries[existed_id]);
		bool v4_forward_addrs_empty =
			(entry->net_addrs[cur_net].ipv4_addrs[0].s_addr == 0);
		if (v4_addr == NULL)
			return v4_forward_addrs_empty;
		v4_reverse_addrs = entry->net_addrs[cur_net].ipv4_reverse_addrs;
		if (emcom_v4_addr_match(v4_addr, v4_reverse_addrs,
					EMCOM_MPDNS_MAX_IPV4_ADDR_NUM))
			return v4_forward_addrs_empty;
...
bool emcom_mpdns_fill_v6_cache(const char *host, struct in6_addr *v6_addr,
			       int cur_net)
{
	errno_t err;
	int existed_id;
	...
	if (existed_entry == NULL) {
		...
		err = memcpy_s(&(v6_reverse_addrs[entry->net_addrs[cur_net]
							  .v6_raddr_update_id]),
			       sizeof(struct in6_addr), v6_addr,
			       sizeof(struct in6_addr));
		if (err != EOK)
			return true;
		...
		struct emcom_mpdns_entry *entry =
			&(g_mpdns_cache.entries[existed_id]);
		bool v6_forward_addrs_empty = !emcom_is_v6_addr_valid(
			&(entry->net_addrs[cur_net].ipv6_addrs[0]));
		if (v6_addr == NULL)
			return v6_forward_addrs_empty;

		v6_reverse_addrs = entry->net_addrs[cur_net].ipv6_reverse_addrs;
		if (emcom_v6_addr_match(v6_addr, v6_reverse_addrs,
					EMCOM_MPDNS_MAX_IPV6_ADDR_NUM))
...
void emcom_mpdns_add_cache(struct emcom_mpdns_result *mpdns_result)
{
	...
	err = memcpy_s(entry->host, EMCOM_MAX_HOST_LEN, mpdns_result->host,
		       strlen(mpdns_result->host));
	if (err != EOK)
		return;

	v4_addrs = entry->net_addrs[cur_net_type].ipv4_addrs;
	err = memcpy_s(v4_addrs,
		       sizeof(struct in_addr) * EMCOM_MPDNS_MAX_IPV4_ADDR_NUM,
		       mpdns_result->v4_addrs,
		       sizeof(struct in_addr) * EMCOM_MPDNS_MAX_IPV4_ADDR_NUM);
	if (err == EOK)
		rst = true;

	v6_addrs = entry->net_addrs[cur_net_type].ipv6_addrs;
	err = memcpy_s(v6_addrs,
		       sizeof(struct in6_addr) * EMCOM_MPDNS_MAX_IPV6_ADDR_NUM,
		       mpdns_result->v6_addrs,
		       sizeof(struct in6_addr) * EMCOM_MPDNS_MAX_IPV6_ADDR_NUM);
	if (err == EOK)
		rst = true;
...
void emcom_mpdns_mod_cache(struct emcom_mpdns_result *mpdns_result,
			   struct emcom_mpdns_entry *entry)
{
	errno_t err;
	int cur_net_type = mpdns_result->net_type;
...
struct emcom_mpdns_app_conf_entry *emcom_mpdns_lookup_app_config(int uid,
								 int *conf_id)
{
	unsigned int i;
	for (i = 0; i < EMCOM_MPDNS_MAX_APP_NUM; i++) {
...
emcom_mpdns_lookup_free_app_config(int *conf_id)
{
	unsigned int i;
	for (i = 0; i < EMCOM_MPDNS_MAX_APP_NUM; i++) {
...
void emcom_mpdns_delete_app_config(int uid)
{
	...
	struct emcom_mpdns_app_conf_entry *found_entry =
		emcom_mpdns_lookup_app_config(uid, &conf_id);
	if (found_entry == NULL)
		return;
...
int emcom_mpdns_update_app_config_hosts(
	struct emcom_mpdns_app_conf_entry *entry, const char *hosts)
{
	errno_t err;
	unsigned int len;
	if (entry->hosts != NULL) {
...
void emcom_mpdns_add_app_config(int uid, const char *hosts)
{
	...
	add_entry = &(
		g_mpdns_app_configs.entries[g_mpdns_app_configs.next_update_id]);
	add_entry->uid = uid;
...
{
	if (strstr(conf_host, COMMON_HOST_PREFIX) ==
	    conf_host) { // at the begin pos
		char *match_pos =
			strstr(host, conf_host + strlen(COMMON_HOST_PREFIX));
		if (match_pos != NULL &&
		    strlen(match_pos) ==
			    strlen(conf_host + strlen(COMMON_HOST_PREFIX)))
			return true;
		return false;
	}
...
void emcom_mpdns_flush_cache(int net_type)
{
	...
	for (entry_id = 0; entry_id < EMCOM_MPDNS_MAX_ENTRY_NUM; entry_id++) {
		struct emcom_mpdns_entry *entry =
			&(g_mpdns_cache.entries[entry_id]);
		(void)memset_s(&(entry->net_addrs[net_type]),
			       sizeof(struct emcom_mpdns_net_addrs), 0,
			       sizeof(struct emcom_mpdns_net_addrs));
	}
	spin_unlock_bh(&g_mpdns_cache_lock);
...
void emcom_mpdns_set_app_config(struct emcom_mpdns_config *mpdns_config)
{
	...
	case EMCOM_MPDNS_APP_CONF_ADD:
		emcom_mpdns_add_app_config(mpdns_config->uid,
					   mpdns_config->hosts);
		break;
	case EMCOM_MPDNS_APP_CONF_UPDATE:
		emcom_mpdns_update_app_config(mpdns_config->uid,
					      mpdns_config->hosts);
		break;
	default:
		break;
...
bool emcom_mpdns_is_allowed(uid_t uid, const char *host)
{
	...
	while (left_idx <= right_idx && right_idx <= hosts_len) {
		if (found_entry->hosts[right_idx] == ',' ||
		    found_entry->hosts[right_idx] == '\0') {
			err = memcpy_s(conf_host, EMCOM_MAX_HOST_LEN,
				       found_entry->hosts + left_idx,
				       right_idx - left_idx);
			if (err == EOK) {
				if (emcom_mpdns_host_config_match(conf_host,
								  host)) {
					spin_unlock_bh(&g_mpdns_app_conf_lock);
					return true;
				}
			}
			(void)memset_s(conf_host, EMCOM_MAX_HOST_LEN, 0,
				       EMCOM_MAX_HOST_LEN);
 ...
 */
void emcom_mpdns_update_dst_addr(struct sock *sk, struct sockaddr *uaddr,
				 int cur_net, int tar_net)
{
	errno_t err;
	bool rst = false;
	if (!emcom_xengine_is_net_type_valid(cur_net) ||
	    !emcom_xengine_is_net_type_valid(tar_net) || cur_net == tar_net)
		return;
	...
	if (uaddr->sa_family == AF_INET) {
		...
		spin_lock_bh(&g_mpdns_cache_lock);
		rst = emcom_lookup_target_v4_addr(cur_v4_addr, cur_net, tar_net,
						  &tar_v4_addr);
		spin_unlock_bh(&g_mpdns_cache_lock);
		if (!rst)
			return;
		...
		spin_lock_bh(&g_mpdns_cache_lock);
		rst = emcom_lookup_target_v6_addr(cur_v6_addr, cur_net, tar_net,
						  &tar_v6_addr);
		spin_unlock_bh(&g_mpdns_cache_lock);
		if (!rst)
			return;
		EMCOM_LOGI("got new v6 addr");
		err = memcpy_s(&(usin->sin6_addr), sizeof(struct in6_addr),
			       &tar_v6_addr, sizeof(struct in6_addr));
		if (err != EOK)
			return;
	} else {
		return;
...
void emcom_mpdns_update_cache(struct emcom_mpdns_result *mpdns_result)
{
	...
	if (!emcom_xengine_is_net_type_valid(net_type) || host_len == 0 ||
	    host_len >= EMCOM_MAX_HOST_LEN)
		return;

	spin_lock_bh(&g_mpdns_cache_lock);
	existed_entry =
		emcom_mpdns_lookup_cache_entry(mpdns_result->host, &existed_id);
	if (existed_entry == NULL) {
 ...
 */
bool emcom_mpdns_supply_v4_cache(const char *host, struct in_addr *v4_addr,
				 int cur_net)
{
	bool rst = false;
 ...
 */
bool emcom_mpdns_supply_v6_cache(const char *host, struct in6_addr *v6_addr,
				 int cur_net)
{
	bool rst = false;
	if (host == NULL || strlen(host) >= EMCOM_MAX_HOST_LEN)
...
void emcom_mpdns_clear_v4_addr(struct in_addr *addr, int net_type)
{
	...
	for (entry_id = 0; entry_id < EMCOM_MPDNS_MAX_ENTRY_NUM; entry_id++) {
		struct emcom_mpdns_entry *cur_entry =
			&(g_mpdns_cache.entries[entry_id]);
		struct emcom_mpdns_net_addrs *cur_addrs =
			&(cur_entry->net_addrs[net_type]);
		struct in_addr *cur_forward_addrs = cur_addrs->ipv4_addrs;
		struct in_addr *cur_reverse_addrs =
			cur_addrs->ipv4_reverse_addrs;
		emcom_clear_v4_addr(addr, cur_forward_addrs,
				    EMCOM_MPDNS_MAX_IPV4_ADDR_NUM);
		emcom_clear_v4_reverse_addr(addr, cur_reverse_addrs,
					    EMCOM_MPDNS_MAX_IPV4_ADDR_NUM);
	}
	spin_unlock_bh(&g_mpdns_cache_lock);
}
...
void emcom_mpdns_clear_v6_addr(struct in6_addr *addr, int net_type)
{
	...
	for (entry_id = 0; entry_id < EMCOM_MPDNS_MAX_ENTRY_NUM; entry_id++) {
		...
		struct in6_addr *cur_reverse_addrs =
			cur_addrs->ipv6_reverse_addrs;
		emcom_clear_v6_addr(addr, cur_forward_addrs,
				    EMCOM_MPDNS_MAX_IPV6_ADDR_NUM);
		emcom_clear_v6_reverse_addr(addr, cur_reverse_addrs,
					    EMCOM_MPDNS_MAX_IPV6_ADDR_NUM);
	}
	spin_unlock_bh(&g_mpdns_cache_lock);
}
...
bool emcom_is_alp_host(const char *host)
{
	...
	return !((num_num + sep_num) == len &&
		 sep_num == IPV4_ADDR_STR_SEG_NUM);
}

/**
 * parse host infomation from hooked data
 */
bool emcom_parse_httpget_host(const unsigned char *req, char *host_buf,
			      unsigned int host_len)
{
	...
	if ((req == NULL) || (host_buf == NULL) || (host_len == 0) ||
	    (host_len > EMCOM_MAX_HOST_LEN))
		return false;
	...
	if (http_req_enter_pos == NULL)
		return false;
	len = http_req_enter_pos - http_req_host -
	      strlen(HTTP_REQUEST_GET_HOST_STR);
	if (len >= host_len)
		return false;
 ...
 */
void emcom_mpdns_host_send(uid_t uid, int net_type, int ip_type,
			   const char *host)
{
	errno_t err;
	...
	err = memset_s(host_info.host, EMCOM_MAX_HOST_LEN, 0,
		       sizeof(host_info.host));
	if (err != EOK)
		return;
	...
	emcom_send_msg2daemon(NETLINK_EMCOM_KD_HOST_IND, &host_info,
			      sizeof(struct emcom_host_info));
}
