...
extern struct emcom_xengine_mpflow_ai_info
	g_mpflow_ai_uids[EMCOM_MPFLOW_AI_MAX_APP];
extern struct emcom_xengine_mpflow_stat
	g_mpflow_ai_list[EMCOM_MPFLOW_AI_MAX_LIST_NUM];

static unsigned int
emcom_xengine_mpflow_hook_out_ipv4(void *priv, struct sk_buff *skb,
				   const struct nf_hook_state *state);
static unsigned int
emcom_xengine_mpflow_hook_out_ipv6(void *priv, struct sk_buff *skb,
				   const struct nf_hook_state *state);
...
static uint8_t *emcom_xengine_mpflow_make_skip(const uint8_t *ptrn,
					       uint8_t plen)
{
	int i;
	uint8_t *skip = NULL;

	skip = (uint8_t *)kmalloc(
		EMCOM_MPFLOW_FI_ASCII_CODE_SIZE * sizeof(uint8_t), GFP_ATOMIC);
	if (!skip)
		return NULL;

	for (i = 0; i < EMCOM_MPFLOW_FI_ASCII_CODE_SIZE; i++)
		*(skip + i) = (plen >= EMCOM_MPFLOW_FI_ASCII_CODE_MARK) ?
				      EMCOM_MPFLOW_FI_ASCII_CODE_MARK :
				      (plen + 1);
	while (plen != 0)
...
static uint8_t *emcom_xengine_mpflow_make_shift(const uint8_t *ptrn,
						uint8_t plen)
{
	uint8_t *sptr = NULL;
...
{
	if ((ptn != NULL) && (ptnlen > 0) && (skip != NULL) &&
	    (shift != NULL)) {
...
static int emcom_xengine_mpflow_split(char *src, const char *separator,
				      char **dest, int *num)
{
	char *p = NULL;
	int count = 0;

	if ((src == NULL) || (strlen(src) == 0) || (separator == NULL) ||
	    (strlen(separator) == 0))
		return *num;
...
static void emcom_xengine_mpflow_hash_clear(
	const struct emcom_xengine_mpflow_app_priv *priv)
{
	int i;
...
void emcom_xengine_mpflow_register_nf_hook(uint8_t ver)
{
	...
	if (g_mpflow_nf_hook == 0) {
		ret = nf_register_net_hooks(
			&init_net, emcom_xengine_mpflow_nfhooks,
			ARRAY_SIZE(emcom_xengine_mpflow_nfhooks));
		if (ret == 0) {
...
void emcom_xengine_mpflow_unregister_nf_hook(uint8_t ver)
{
	...
	nf_unregister_net_hooks(&init_net, emcom_xengine_mpflow_nfhooks,
				ARRAY_SIZE(emcom_xengine_mpflow_nfhooks));
...
static void
emcom_xengine_mpflow_apppriv_deinit(struct emcom_xengine_mpflow_info *uid)
{
	if (!uid->app_priv)
		return;
...
static void
emcom_xengine_mpflow_fi_init(struct emcom_xengine_mpflow_info *mpflow_uid)
{
	errno_t err;

	err = memset_s(&mpflow_uid->wifi,
		       sizeof(struct emcom_xengine_mpflow_iface), 0,
		       sizeof(struct emcom_xengine_mpflow_iface));
	if (err != EOK)
		EMCOM_LOGE("emcom_xengine_mpflow_fi_init memset failed");
	mpflow_uid->wifi.is_wifi = 1;
	err = memset_s(&mpflow_uid->lte,
		       sizeof(struct emcom_xengine_mpflow_iface), 0,
		       sizeof(struct emcom_xengine_mpflow_iface));
	if (err != EOK)
		EMCOM_LOGE("emcom_xengine_mpflow_fi_init memset failed");
...
static void
emcom_xengine_mpflow_ptn_deinit(struct emcom_xengine_mpflow_ptn ptn[],
				uint8_t num)
{
	uint8_t i;
	...
	for (i = 0; i < num; i++) {
		if (ptn[i].is_init) {
			emcom_xengine_mpflow_bm_free(&(ptn[i].skip),
						     &(ptn[i].shift));
			ptn[i].is_init = false;
		}
	}
...
static void
emcom_xengine_mpflow_download_flow_del(struct emcom_xengine_mpflow_iface *iface,
				       struct emcom_xengine_mpflow_node *flow)
{
	struct sock *sk = (struct sock *)flow->tp;

	EMCOM_LOGD(
		"remove sk: %pK sport: %u srtt_us: %u bytes_received: %llu duration: %u",
		flow->tp, sk->sk_num, flow->tp->srtt_us >> 3,
		flow->tp->bytes_received,
...
static bool emcom_xengine_mpflow_ptn_init(struct emcom_xengine_mpflow_ptn ptn[],
					  uint8_t *num, const char *hex)
{
	bool ret = false;
	...
	EMCOM_LOGD("hex %s\n", hex);
	if (emcom_xengine_mpflow_split((char *)hex,
				       EMCOM_MPFLOW_FI_PTN_SEPERATE_CHAR,
				       revbuf, &n) == 0) {
	...
	for (i = 0; i < n; i++) {
		ptn[i].skip = NULL;
		ptn[i].shift = NULL;
		err = memset_s(ptn[i].ptn, EMCOM_MPFLOW_FI_PTN_MAX_SIZE, 0,
			       EMCOM_MPFLOW_FI_PTN_MAX_SIZE);
		if (err != EOK) {
			EMCOM_LOGE(
				"emcom_xengine_mpflow_ptn_init memset failed!");
			ptn[i].is_init = false;
			return false;
		}
		ptn[i].ptnlen =
			strnlen(revbuf[i], EMCOM_MPFLOW_FI_PTN_MAX_SIZE) >> 1;
		if (hex2bin(ptn[i].ptn, revbuf[i], ptn[i].ptnlen)) {
			ptn[i].is_init = false;
			EMCOM_LOGE("hex2bin fail\n");
			return false;
		}
		ret = emcom_xengine_mpflow_bm_build((const uint8_t *)ptn[i].ptn,
						    ptn[i].ptnlen,
						    &(ptn[i].skip),
						    &(ptn[i].shift));
		if (!ret) {
...
static bool emcom_xengine_mpflow_fi_start(
	bool is_new_uid_enable, uint8_t index, bool *ret,
	struct emcom_xengine_mpflow_parse_start_info *mpflowstartinfo)
{
	if (g_mpflow_uids[index].algorithm_type ==
		    EMCOM_MPFLOW_ENABLETYPE_WEIBO ||
	    g_mpflow_uids[index].algorithm_type ==
		    EMCOM_MPFLOW_ENABLETYPE_WIFI_PRI) {
	...
	if (is_new_uid_enable) {
		...
		emcom_xengine_mpflow_fi_init(&g_mpflow_uids[index]);
		app_priv = kzalloc(sizeof(struct emcom_xengine_mpflow_app_priv),
				   GFP_ATOMIC);
		if (!app_priv)
			return false;

		if (g_mpflow_uids[index].algorithm_type ==
		    EMCOM_MPFLOW_ENABLETYPE_NET_DISK) {
			if (g_mpflow_uids[index].reserve_field &
			    EMCOM_MPFLOW_ENABLEFLAG_LTE_FIRST) {
				app_priv->lte_thresh =
					EMCOM_MPFLOW_LTE_FIRST_LTE_THREH;
				app_priv->lte_thresh_max =
					EMCOM_MPFLOW_LTE_FIRST_LTE_THREH_MAX;
				app_priv->lte_thresh_min =
					EMCOM_MPFLOW_LTE_FIRST_LTE_THREH_MIN;
				app_priv->lte_first = 1;
			} else {
				app_priv->lte_thresh =
					EMCOM_MPFLOW_WIFI_FIRST_LTE_THREH;
				app_priv->lte_thresh_max =
					EMCOM_MPFLOW_WIFI_FIRST_LTE_THREH_MAX;
				app_priv->lte_thresh_min =
					EMCOM_MPFLOW_WIFI_FIRST_LTE_THREH_MIN;
		...
		if (emcom_xengine_mpflow_ptn_init(
			    g_mpflow_uids[index].ptn_80,
			    &(g_mpflow_uids[index].ptn_80_num),
			    mpflowstartinfo->ptn_80))
			*ret = true;
		if (emcom_xengine_mpflow_ptn_init(
			    g_mpflow_uids[index].ptn_443,
			    &(g_mpflow_uids[index].ptn_443_num),
			    mpflowstartinfo->ptn_443))
			*ret = true;
	} else {
		emcom_xengine_mpflow_ptn_deinit(
			g_mpflow_uids[index].ptn_80,
			g_mpflow_uids[index].ptn_80_num);
		emcom_xengine_mpflow_ptn_deinit(
			g_mpflow_uids[index].ptn_443,
...
static struct emcom_xengine_mpflow_ip *
emcom_xengine_mpflow_hash(__be32 addr, struct hlist_head *hashtable,
			  uint32_t algorithm_type)
{
	...
	hlist_for_each_entry_safe(ip, tmp, &hashtable[hash], node) {
		...
		/* free too old entrys */
		if ((jiffies - ip->jiffies[(ip->tot_cnt - 1) %
					   EMCOM_MPFLOW_FLOW_JIFFIES_REC]) >
		    aging) {
			hlist_del(&ip->node);
			kfree(ip);
		}
...
static bool
emcom_xengine_mpflow_ip_chk_bind_lte(struct emcom_xengine_mpflow_app_priv *priv,
				     ...
				     uint32_t type)
{
	if (type == EMCOM_MPFLOW_ENABLETYPE_NET_DISK) {
		...
		/*lint -e666*/
		if (time_before(now, (ip->jiffies[index] +
				      EMCOM_MPFLOW_NETDISK_DOWNLOAD_THREH))) {
			/* if (lte_cnt/tot_cnt < lte_thresh/10), then we need bind on lte */
			if ((ip->lte_cnt * EMCOM_MPFLOW_FI_NETDISK_FLOW_NUM) <
			    (priv->lte_thresh * ip->tot_cnt))
				return true;
			else
				return false;
		}
...
static bool emcom_xengine_mpflow_get_addr_port(struct sockaddr *addr,
					       __be32 *s_addr, uint16_t *port)
{
	if (addr->sa_family == AF_INET) {
		...
		*port = ntohs(usin->sin_port);
		...
		if (!ipv6_addr_v4mapped(&usin6->sin6_addr))
			return false;
		*s_addr = usin6->sin6_addr
				  .s6_addr32[EMCOM_MPFLOW_FI_CLAT_IPV4_INDEX];
...
static int
emcom_xengine_mpflow_ip_bind(struct sockaddr *addr,
...
static void emcom_xengine_mpflow_netdisk_lte_thresh(
	struct emcom_xengine_mpflow_app_priv *priv, int add)
{
...
static void emcom_xengine_mpflow_download_finish(
	const struct emcom_xengine_mpflow_info *uid)
{
...
#define EMCOM_MPFLOW_FI_RATE_RATIO_DENOMINATOR 5
	...
	if (uid->algorithm_type == EMCOM_MPFLOW_ENABLETYPE_NET_DISK) {
		...
		if ((uid->lte.bytes_received + uid->wifi.bytes_received) <
		    EMCOM_MPFLOW_LTE_THREH_ADJUST_BYTES)
			return;

		if (uid->lte.max_rate_received_flow &&
		    uid->wifi.max_rate_received_flow &&
		    (uid->lte.max_rate_received_flow <
		     uid->wifi.max_rate_received_flow)) {
			...
			add--;
		} else if ((uid->lte.max_rate_received_flow >
			    EMCOM_MPFLOW_NETDISK_RATE_GOOD) &&
			   ((EMCOM_MPFLOW_FI_RATE_RATIO_DENOMINATOR *
			     uid->wifi.max_rate_received_flow) <
			    (EMCOM_MPFLOW_FI_RATE_RATIO_NUMERATOR *
			     uid->lte.max_rate_received_flow))) {
			/* avg_speed_per_flow_LTE>1M && avg_speed_per_flow_WIFI < 0.8* avg_speed_per_flow_LTE, lte_thresh++ */
			EMCOM_LOGD("case 2: lte_thresh++");
			add++;
		} else if (!priv->lte_thresh &&
			   (uid->wifi.max_rate_received_flow <
			    EMCOM_MPFLOW_NETDISK_RATE_BAD)) {
...
static bool
emcom_xengine_get_mpflowlist(uint8_t ver,
			     struct emcom_xengine_mpflow_stat **mpflow_list)
{
	switch (ver) {
	...
	case EMCOM_MPFLOW_VER_V2:
		*mpflow_list = g_mpflow_ai_list;
		break;
	default:
		EMCOM_LOGE("get mpflowlist failed.Version %d not supported",
			   ver);
		return false;
		;
	}
...
void emcom_xengine_mpflow_reset(struct tcp_sock *tp)
{
	...
	EMCOM_LOGD("reset sk %pK sport is %u, state[%u]", tp, sk->sk_num,
		   sk->sk_state);
...
static void emcom_xengine_mpflow_update_wifi_pri(
	struct emcom_xengine_mpflow_iface *flows,
	const struct emcom_xengine_mpflow_info *mpflow_uid, u16 flow_cnt)
{
	...
	flows->is_sure_no_slow =
		0; // but not sure the interface is actually slow

	if (flow_cnt <= 1) {
		flows->is_slow =
			0; // no active flow, reconsider the interface as fast but not sure
	} else if (flows->mean_srtt_ms <
		   EMCOM_GOOD_RTT_THR_MS) { // low rtt, iface surely fast
		flows->is_slow = 0;
		flows->is_sure_no_slow = 1;
	}

	if (flows->rate_received[0] == 0 && flows->rate_received[1] == 0 &&
	    flows->rate_received[2] == 0)
		flows->is_slow =
			0; // no bytes recieved for 3s, reconsider the iface as fast but not sure

	for (i = EMCOM_MPFLOW_FI_STAT_SECONDS - 1; i >= 0; i--) {
		if (flows->rate_received[i] >
		    EMCOM_GOOD_RECV_RATE_THR_BYTE_PER_SEC) {
			flows->is_slow =
				0; // recieve rate is more than xx KBps, interface is surely fast
			flows->is_sure_no_slow = 1;
			break;
		}
	}
...
static void
emcom_xengine_mpflow_update(struct emcom_xengine_mpflow_iface *flows,
...
#define EMCOM_MPFLOW_MICROSECOND_SMOOTH_RATE 8
	...
	list_for_each_entry_safe(node, tmp, &flows->flows, list) {
		...
		node->rate_received[0] = (u32)(node->tp->bytes_received -
					       node->last_bytes_received);
		/* srtt_us is smoothed round trip time << 3 in usecs */
		srtt_ms_sum += (node->tp->srtt_us /
				EMCOM_MPFLOW_MICROSECOND_SMOOTH_RATE) /
			       USEC_PER_MSEC;
		flow_cnt++;
		for (i = EMCOM_MPFLOW_FI_STAT_SECONDS - 1; i >= 0; i--) {
			if (node->rate_received[i] >
			    EMCOM_MPFLOW_FLOW_SLOW_THREH) {
				flows->is_slow = 0;
				break;
			}
		}
		...
		flows->bytes_received += (u32)(node->tp->bytes_received -
					       node->last_bytes_received);
		flows->rate_received[0] += (u32)(node->tp->bytes_received -
						 node->last_bytes_received);
	...
	if (flows->flow_valid_cnt) {
		if (flows->max_rate_received_flow <
		    max_rate_received / flows->flow_valid_cnt)
			flows->max_rate_received_flow =
				max_rate_received / flows->flow_valid_cnt;
	}
...
static void
emcom_xengine_mpflow_set_bind(struct emcom_xengine_mpflow_info *mpflow_uid,
			      int bind_mode)
{
	switch (bind_mode) {
	...
	case EMCOM_XENGINE_MPFLOW_BINDMODE_RST2FAST:
		mpflow_uid->rst_bind_mode =
			(mpflow_uid->wifi.max_rate_received >=
			 mpflow_uid->lte.max_rate_received) ?
...
static bool emcom_xengine_mpflow_unbalance_netdisk(
	const struct emcom_xengine_mpflow_iface *iface1,
	const struct emcom_xengine_mpflow_iface *iface2, uint8_t index)
{
	if (iface1->max_rate_received &&
	    ...
	    ((iface1->is_wifi && (g_mpflow_uids[index].bindmode ==
				  EMCOM_XENGINE_MPFLOW_BINDMODE_LTE)) ||
	     (!iface1->is_wifi && (g_mpflow_uids[index].bindmode ==
				   EMCOM_XENGINE_MPFLOW_BINDMODE_WIFI)) ||
...
static bool emcom_xengine_mpflow_unbalance_market(
	const struct emcom_xengine_mpflow_iface *iface1,
	const struct emcom_xengine_mpflow_iface *iface2, uint8_t index)
{
	if (iface1->max_rate_received &&
	    ((iface1->is_wifi && iface2->is_slow &&
	      (iface1->max_rate_received < (iface2->max_rate_received >> 1)) &&
	      ((g_mpflow_uids[index].bindmode ==
		EMCOM_XENGINE_MPFLOW_BINDMODE_LTE) ||
	       (g_mpflow_uids[index].bindmode ==
		EMCOM_XENGINE_MPFLOW_BINDMODE_RANDOM))) ||
	     (!iface1->is_wifi &&
	      (iface1->max_rate_received < iface2->max_rate_received_flow) &&
	      ((g_mpflow_uids[index].bindmode ==
		EMCOM_XENGINE_MPFLOW_BINDMODE_WIFI) ||
	       (g_mpflow_uids[index].bindmode ==
		EMCOM_XENGINE_MPFLOW_BINDMODE_RANDOM)))))
...
static bool emcom_xengine_mpflow_single_path(
	const struct emcom_xengine_mpflow_iface *iface1,
	const struct emcom_xengine_mpflow_iface *iface2, uint8_t index)
{
	if (!g_mpflow_uids[index].rst_to_another &&
...
static int emcom_xengine_mpflow_chk_rst_market(
	const struct emcom_xengine_mpflow_iface *iface1,
	const struct emcom_xengine_mpflow_iface *iface2, uint8_t index)
{
	struct emcom_xengine_mpflow_node *node = NULL;
	...
	list_for_each_entry_safe(node, tmp, &iface1->flows, list) {
		/*lint -e666*/
		if (time_before(jiffies, (node->start_jiffies +
					  EMCOM_MPFLOW_FLOW_TIME_THREH)))
			continue;
		/*lint +e666*/

		/* all downloading flows are on same iface */
		if ((node->last_bytes_received >
		     EMCOM_MPFLOW_RST_RCV_BYTES_THREH) &&
		    emcom_xengine_mpflow_single_path(iface1, iface2, index)) {
			emcom_xengine_mpflow_reset(node->tp);
			need_set_bind =
				iface1->is_wifi ?
					EMCOM_XENGINE_MPFLOW_BINDMODE_RST2LTE :
					EMCOM_XENGINE_MPFLOW_BINDMODE_RST2WIFI;
			g_mpflow_uids[index].rst_to_another = 1;
			break;
		}
		if ((iface1->bytes_received + iface2->bytes_received) <=
		    EMCOM_MPFLOW_RST_IFACE_GOOD)
			break;
		/* iface1 is slower than half of iface2 */
		if (iface1->max_rate_received &&
		    (iface1->max_rate_received <
		     (iface2->max_rate_received >> 1)))
			need_set_bind = EMCOM_XENGINE_MPFLOW_BINDMODE_RST2FAST;
		/* both wifi and lte download over */
		if (iface1->is_slow &&
		    (iface2->is_slow || !iface2->bytes_received)) {
			emcom_xengine_mpflow_reset(node->tp);
			need_set_bind = EMCOM_XENGINE_MPFLOW_BINDMODE_RST2FAST;
		}
		...
		if ((node->last_bytes_received >
		     EMCOM_MPFLOW_RST_RCV_BYTES_THREH) &&
		    emcom_xengine_mpflow_unbalance_market(iface1, iface2,
							  index)) {
			emcom_xengine_mpflow_reset(node->tp);
			need_set_bind = EMCOM_XENGINE_MPFLOW_BINDMODE_RST2FAST;
		}
...
static int emcom_xengine_mpflow_chk_rst_netdisk(
	const struct emcom_xengine_mpflow_iface *iface1,
	const struct emcom_xengine_mpflow_iface *iface2, uint8_t index)
{
	struct emcom_xengine_mpflow_node *node = NULL;
	...
	list_for_each_entry_safe(node, tmp, &iface1->flows, list) {
		...
		if (time_before(jiffies, (node->start_jiffies +
					  EMCOM_MPFLOW_FLOW_TIME_THREH)) ||
		    ((iface1->bytes_received + iface2->bytes_received) <=
		     EMCOM_MPFLOW_RST_IFACE_GOOD))
			continue;
		/*lint +e666*/
		...
		if ((node->last_bytes_received >
		     EMCOM_MPFLOW_RST_RCV_BYTES_THREH) &&
		    emcom_xengine_mpflow_unbalance_netdisk(iface1, iface2,
							   index))
			emcom_xengine_mpflow_reset(node->tp);
	}
	return need_set_bind;
...
static int emcom_xengine_mpflow_chk_rst_weibo(
	const struct emcom_xengine_mpflow_iface *iface1,
	const struct emcom_xengine_mpflow_iface *iface2)
{
	...
	/*lint -e666*/
	if (!iface1->max_rate_received_flow ||
	    !iface2->max_rate_received_flow ||
	    time_before(jiffies, (iface1->start_jiffies +
				  EMCOM_MPFLOW_FLOW_TIME_THREH)) ||
	    time_before(jiffies,
			(iface2->start_jiffies + EMCOM_MPFLOW_FLOW_TIME_THREH)))
		return need_set_bind;
	/*lint +e666*/
	wifi_download_time = MSEC_PER_SEC * EMCOM_MPFLOW_WEIBO_SIZE /
			     iface1->max_rate_received;
	lte_download_time = MSEC_PER_SEC * EMCOM_MPFLOW_WEIBO_SIZE /
			    iface2->max_rate_received;
	if ((iface1->mean_srtt_ms + wifi_download_time) <
		...
		need_set_bind = EMCOM_XENGINE_MPFLOW_BINDMODE_RST2WIFI;
		 ...
		 (iface1->mean_srtt_ms +
		  (wifi_download_time / EMCOM_MPFLOW_DEV_NUM)))
		need_set_bind = EMCOM_XENGINE_MPFLOW_BINDMODE_RST2LTE;
	return need_set_bind;
}

static int emcom_xengine_mpflow_chk_rst_wifi_pri(
	const struct emcom_xengine_mpflow_iface *iface1,
	const struct emcom_xengine_mpflow_iface *iface2,
	struct emcom_xengine_mpflow_info *mpflow_uid)
{
	...
	if (!iface1->max_rate_received_flow ||
	    !iface2->max_rate_received_flow) {
		EMCOM_LOGD(
			"emcom_xengine_mpflow_chk_rst_wifi_pri, no enough dl return NORST\n");
		return need_set_bind;
	}

	/*lint -e666*/
	if (time_before(jiffies, (iface1->start_jiffies +
				  EMCOM_MPFLOW_FLOW_TIME_THREH)) ||
	    time_before(jiffies, (iface2->start_jiffies +
				  EMCOM_MPFLOW_FLOW_TIME_THREH))) {
		EMCOM_LOGD(
			"emcom_xengine_mpflow_chk_rst_wifi_pri, not timeout, return NORST\n");
		return need_set_bind;
	}
	/*lint +e666*/
...
#endif
{
	...
	for (index = 0; index < EMCOM_MPFLOW_MAX_APP; index++) {
		...
		if ((mpflow_uid->uid == UID_INVALID_APP) ||
		    !mpflow_uid->is_downloading)
			continue;
		emcom_xengine_mpflow_update(&mpflow_uid->wifi, mpflow_uid);
		emcom_xengine_mpflow_update(&mpflow_uid->lte, mpflow_uid);
		if (g_mpflow_uids[index].algorithm_type ==
		    EMCOM_MPFLOW_ENABLETYPE_NET_DISK) {
			bind_mode_wifi = emcom_xengine_mpflow_chk_rst_netdisk(
				&mpflow_uid->wifi, &mpflow_uid->lte, index);
			bind_mode_lte = emcom_xengine_mpflow_chk_rst_netdisk(
				&mpflow_uid->lte, &mpflow_uid->wifi, index);
		} else if (g_mpflow_uids[index].algorithm_type ==
			   EMCOM_MPFLOW_ENABLETYPE_MARKET) {
			bind_mode_wifi = emcom_xengine_mpflow_chk_rst_market(
				&mpflow_uid->wifi, &mpflow_uid->lte, index);
			bind_mode_lte = emcom_xengine_mpflow_chk_rst_market(
				&mpflow_uid->lte, &mpflow_uid->wifi, index);
		} else if (g_mpflow_uids[index].algorithm_type ==
			   EMCOM_MPFLOW_ENABLETYPE_WEIBO) {
			bind_mode_wifi = emcom_xengine_mpflow_chk_rst_weibo(
				&mpflow_uid->wifi, &mpflow_uid->lte);
		} else if (g_mpflow_uids[index].algorithm_type ==
			   EMCOM_MPFLOW_ENABLETYPE_WIFI_PRI) {
			bind_mode_wifi = emcom_xengine_mpflow_chk_rst_wifi_pri(
				&mpflow_uid->wifi, &mpflow_uid->lte,
				mpflow_uid);
		}
		bind_mode = (bind_mode_wifi ==
			     EMCOM_XENGINE_MPFLOW_BINDMODE_NORST) ?
				    bind_mode_lte :
				    bind_mode_wifi;
...
static void emcom_xengine_mpflow_download_flow_add(struct sock *sk)
{
	...
	dst = sk_dst_get(sk);
	if (dst) {
		if (strncmp(EMCOM_WLAN0_IFNAME, dst->dev->name, IFNAMSIZ) ==
		    0) {
			iface = &g_mpflow_uids[index].wifi;
		} else if (strncmp(EMCOM_CELL0_IFNAME, dst->dev->name,
				   IFNAMSIZ) == 0) {
			iface = &g_mpflow_uids[index].lte;
		} else {
			EMCOM_LOGD("sk port %u iface is %s", port,
				   dst->dev->name);
			dst_release(dst);
			spin_unlock_bh(&g_mpflow_lock);
			return;
	...
	if (node) {
		...
		if (!g_mpflow_uids[index].is_downloading) {
			EMCOM_LOGD("uid %u is_downloading",
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
			timer_setup(&g_mpflow_tm, emcom_xengine_mpflow_timer,
				    0);
#else
...
static bool
emcom_xengine_mpflow_bm_search(const uint8_t *buf, uint32_t buflen,
			       const struct emcom_xengine_mpflow_ptn *sptn,
			       uint32_t *offset)
{
	uint32_t pindex;
	uint8_t skip_stride;
	...
	while (bindex <= buflen) {
		...
		bindex += ((skip_stride > shift_stride) ? skip_stride :
							  shift_stride);
	}
	return false;
}
...
static bool emcom_xengine_mpflow_chk_download_flow(struct sk_buff *skb)
{
	...
	if (g_mpflow_uids[index].algorithm_type ==
		...
		spin_unlock_bh(&g_mpflow_lock);
		return true;
	} else if ((port == EMCOM_MPFLOW_FI_PORT_80) &&
		   (g_mpflow_uids[index].ptn_80_num != 0)) {
		for (i = 0; i < g_mpflow_uids[index].ptn_80_num; i++) {
			if (emcom_xengine_mpflow_bm_search(
				    (const uint8_t *)skb->data, buflen,
				    (const struct emcom_xengine_mpflow_ptn *)&(
					    g_mpflow_uids[index].ptn_80[i]),
				    &offset)) {
				EMCOM_LOGD(
					"received a port 80 packet match ptn: %s\n",
					g_mpflow_uids[index].ptn_80[i].ptn);
				spin_unlock_bh(&g_mpflow_lock);
				return true;
			}
		}
	} else if ((port == EMCOM_MPFLOW_FI_PORT_443) &&
		   (g_mpflow_uids[index].ptn_443_num != 0)) {
		for (i = 0; i < g_mpflow_uids[index].ptn_443_num; i++) {
			if (emcom_xengine_mpflow_bm_search(
				    (const uint8_t *)skb->data, buflen,
				    (const struct emcom_xengine_mpflow_ptn *)&(
					    g_mpflow_uids[index].ptn_443[i]),
				    &offset)) {
				EMCOM_LOGD(
					"received a port 443 packet match ptn: %s\n",
...
void emcom_xengine_mpflow_init(void)
{
	...
	err = memset_s(g_mpflow_ai_uids, sizeof(g_mpflow_ai_uids), 0,
		       sizeof(g_mpflow_ai_uids));
	if (err != EOK)
		EMCOM_LOGD("emcom_xengine_mpflow_init ai failed");
	err = memset_s(g_mpflow_ai_list, sizeof(g_mpflow_ai_list), 0,
		       sizeof(g_mpflow_ai_list));
	if (err != EOK)
	...
	for (index = 0; index < EMCOM_MPFLOW_MAX_APP; index++) {
		if ((g_mpflow_uids[index].uid != UID_INVALID_APP) &&
			...
			list_for_each_entry_safe(
				node, tmp, &g_mpflow_uids[index].wifi.flows,
				list)
				emcom_xengine_mpflow_download_flow_del(
					&g_mpflow_uids[index].wifi, node);

			list_for_each_entry_safe(
				node, tmp, &g_mpflow_uids[index].lte.flows,
				list)
				emcom_xengine_mpflow_download_flow_del(
					&g_mpflow_uids[index].lte, node);
			...
			emcom_xengine_mpflow_ptn_deinit(
				g_mpflow_uids[index].ptn_443,
				g_mpflow_uids[index].ptn_443_num);
			emcom_xengine_mpflow_apppriv_deinit(
				&g_mpflow_uids[index]);
		}
		...
		err = memset_s(&g_mpflow_uids[index].dport_range, /*lint !e545*/
			       sizeof(g_mpflow_uids[index].dport_range), 0,
...
void emcom_xengine_mpflow_start(const char *pdata, uint16_t len)
{
	...
	/* input param check */
	if (!pdata ||
	    (len != sizeof(struct emcom_xengine_mpflow_parse_start_info))) {
	...
	EMCOM_LOGD("mpflow start uid: %u, enableflag: %d, "
		   ...
		   mpflowstartinfo->algorithm_type);
	...
	if (tmp < 0) {
		...
		EMCOM_LOGD("mpflow add new mpinfo uid: %d",
			   mpflowstartinfo->uid);
		newindex = emcom_xengine_mpflow_getfreeindex();
		if (newindex == INDEX_INVALID) {
			EMCOM_LOGE(
				"mpflow start get free index exceed. uid: %d",
				mpflowstartinfo->uid);
			spin_unlock_bh(&g_mpflow_lock);
			return;
		}
	...
	if ((g_mpflow_uids[index].algorithm_type ==
	    ...
	    (g_mpflow_uids[index].algorithm_type ==
	     EMCOM_MPFLOW_ENABLETYPE_WIFI_PRI)) {
		bool fi_start_ret = emcom_xengine_mpflow_fi_start(
			is_new_uid_enable, index, &ret, mpflowstartinfo);
		if (!fi_start_ret) {
			spin_unlock_bh(&g_mpflow_lock);
			return;
		}
...
void emcom_xengine_mpflow_stop(const char *pdata, uint16_t len)
{
	...
	/* input param check */
	if (!pdata ||
	    (len != sizeof(struct emcom_xengine_mpflow_parse_stop_info))) {
	...
	EMCOM_LOGD("mpflow stop uid: %u, stop reason: %u", mpflowstopinfo->uid,
		   stop_reason);
	spin_lock_bh(&g_mpflow_lock);
	...
	if (index != INDEX_INVALID) {
		if ((stop_reason == EMCOM_MPFLOW_STOP_REASON_NETWORK_ROAMING) ||
			...
			EMCOM_LOGD("mpflow stop clear info uid: %u, index: %d ",
				   mpflowstopinfo->uid, index);
			...
			if ((g_mpflow_uids[index].algorithm_type ==
				...
				list_for_each_entry_safe(
					node, tmp,
					&g_mpflow_uids[index].wifi.flows, list)
					emcom_xengine_mpflow_download_flow_del(
						&g_mpflow_uids[index].wifi,
						node);

				list_for_each_entry_safe(
					node, tmp,
					&g_mpflow_uids[index].lte.flows, list)
					emcom_xengine_mpflow_download_flow_del(
						&g_mpflow_uids[index].lte,
						node);
			...
			err = memset_s(
				g_mpflow_uids[index].dport_range,
				sizeof(g_mpflow_uids[index].dport_range), 0,
				sizeof(g_mpflow_uids[index].dport_range));
			if (err != EOK)
				EMCOM_LOGE(
					"emcom_xengine_mpflow_stop memset failed");
		} else {
			g_mpflow_uids[index].bindmode =
				EMCOM_XENGINE_MPFLOW_BINDMODE_NONE;
		}
...
bool emcom_xengine_mpflow_checkvalid(struct sock *sk, struct sockaddr *uaddr,
				     int8_t index, uint16_t *dport)
{
	...
	isvalidaddr = emcom_xengine_check_ip_addrss(uaddr) &&
		      (!emcom_xengine_check_ip_is_private(uaddr));
	...
	if (!(((sk->sk_protocol == IPPROTO_TCP) &&
		...
		EMCOM_LOGD(
			"mpflow check valid protocol not correct uid: %u, sk: %pK",
	...
	if (g_mpflow_uids[index].enableflag & EMCOM_MPFLOW_ENABLEFLAG_DPORT) {
		...
		if (usin->sin_family == AF_INET) {
			...
			*dport = (uint16_t)ntohs(usin6->sin6_port);
			...
			EMCOM_LOGD(
				"mpflow check valid not support family uid: %u,"
				" sin_family: %d",
				g_mpflow_uids[index].uid, usin->sin_family);
			return false;
		}
		bfinddport = emcom_xengine_mpflow_finddport(
			&g_mpflow_uids[index], *dport);
		if (bfinddport == false) {
			EMCOM_LOGD(
				"mpflow check valid can not find uid: %u, dport: %d",
...
bool emcom_xengine_mpflow_getinetaddr(struct net_device *dev)
{
	...
	if (!in_dev) {
		EMCOM_LOGD("mpflow get inet addr in_dev is null dev: %s",
			   dev->name);
		return false;
	}
...
int emcom_xengine_mpflow_getmode_rand(int8_t index, uid_t uid,
				      struct sockaddr *uaddr)
{
	...
	if (g_mpflow_uids[index].rst_bind_mode ==
	    EMCOM_XENGINE_MPFLOW_BINDMODE_WIFI) {
		bind_device = EMCOM_MPFLOW_BIND_WIFI;
	} else if (g_mpflow_uids[index].rst_bind_mode ==
		   EMCOM_XENGINE_MPFLOW_BINDMODE_LTE) {
		bind_device = EMCOM_MPFLOW_BIND_LTE;
	} else if (g_mpflow_uids[index].app_priv) {
		bind_device = emcom_xengine_mpflow_ip_bind(
			uaddr, &g_mpflow_uids[index]);
	} else {
		g_mpflow_index++;
	...
	is_wifi_block = emcom_xengine_mpflow_blocked(uid, EMCOM_WLAN0_IFNAME,
						     EMCOM_MPFLOW_VER_V1);
	is_lte_block = emcom_xengine_mpflow_blocked(uid, EMCOM_CELL0_IFNAME,
						    EMCOM_MPFLOW_VER_V1);
	if ((is_wifi_block == true) && (is_lte_block == true))
		bind_device = EMCOM_MPFLOW_BIND_NONE;
	else if ((is_wifi_block == true) &&
		 (bind_device == EMCOM_MPFLOW_BIND_WIFI))
		bind_device = EMCOM_MPFLOW_BIND_LTE;
	else if ((is_lte_block == true) &&
		 (bind_device == EMCOM_MPFLOW_BIND_LTE))
		bind_device = EMCOM_MPFLOW_BIND_WIFI;
...
int emcom_xengine_mpflow_getmode_spec(int8_t index, uid_t uid)
{
	...
	if ((is_wifi_block && (g_mpflow_uids[index].bindmode ==
			       EMCOM_XENGINE_MPFLOW_BINDMODE_WIFI)) ||
	    (is_lte_block && (g_mpflow_uids[index].bindmode ==
			      EMCOM_XENGINE_MPFLOW_BINDMODE_LTE)) ||
	    ((is_wifi_block == true) && (is_lte_block == true))) {
		EMCOM_LOGD(
			"mpflow bind blocked uid: %u, bindmode: %d, blocked:%d, %d, connnum:%d",
			uid, g_mpflow_uids[index].bindmode, is_wifi_block,
			is_lte_block,
...
int emcom_xengine_mpflow_getmode(int8_t index, uid_t uid,
				 struct sockaddr *uaddr)
{
	int bind_device;
	...
	if (g_mpflow_uids[index].rst_bind_mode !=
	    EMCOM_XENGINE_MPFLOW_BINDMODE_NONE) {
		if (time_after(jiffies, (g_mpflow_uids[index].rst_jiffies +
					 EMCOM_MPFLOW_RST_TIME_THREH)))
			g_mpflow_uids[index].rst_bind_mode =
				EMCOM_XENGINE_MPFLOW_BINDMODE_NONE;
	}
	/*lint +e666*/

	if (g_mpflow_uids[index].bindmode ==
	    EMCOM_XENGINE_MPFLOW_BINDMODE_RANDOM)
		bind_device =
			emcom_xengine_mpflow_getmode_rand(index, uid, uaddr);
	else
...
void emcom_xengine_mpflow_bind2device(struct sock *sk, struct sockaddr *uaddr)
{
	...
	if (emcom_xengine_mpflow_checkvalid(sk, uaddr, index, &dport) ==
	    false) {
	...
	if (bind_device == EMCOM_MPFLOW_BIND_WIFI) {
		err = memcpy_s(ifname, sizeof(char) * IFNAMSIZ,
			       EMCOM_WLAN0_IFNAME,
			       (strlen(EMCOM_WLAN0_IFNAME) + 1));
	} else if (bind_device == EMCOM_MPFLOW_BIND_LTE) {
		err = memcpy_s(ifname, sizeof(char) * IFNAMSIZ,
			       EMCOM_CELL0_IFNAME,
			       (strlen(EMCOM_CELL0_IFNAME) + 1));
	...
	if (!dev || (emcom_xengine_mpflow_getinetaddr(dev) == false)) {
		rcu_read_unlock();
		EMCOM_LOGD(
			"mpflow bind2device dev not ready uid: %u, sk: %pK, dev: %pK, name: %s",
...
bool emcom_xengine_mpflow_finddport(
	struct emcom_xengine_mpflow_info *mpflowinfo, uint16_t dport)
{
	int i;
...
emcom_xengine_mpflow_get(int uid, const char *name, int ifindex, uint8_t ver)
{
	...
	for (index = 0; index < EMCOM_MPFLOW_MAX_LIST_NUM; index++) {
		if ((node == NULL) &&
		    (mpflow_list[index].uid == UID_INVALID_APP))
			node = &mpflow_list[index];
		if ((mpflow_list[index].uid == uid) &&
		    (!strncmp(mpflow_list[index].name, name, strlen(name))))
	...
	if (err != EOK) {
		EMCOM_LOGE(
			"emcom_xengine_mpflow_get strncpy_s failed, errcode: %d",
			err);
	...
	err = memset_s(node->retrans_count, sizeof(node->retrans_count), 0,
		       sizeof(node->retrans_count));
	if (err != EOK) {
		EMCOM_LOGE(
			"emcom_xengine_mpflow_get memset failed, errcode: %d",
			err);
		node->uid = UID_INVALID_APP;
		return NULL;
	}
...
void emcom_xengine_mpflow_clear_blocked(int uid, uint8_t ver)
{
	...
	for (index = 0; index < EMCOM_MPFLOW_MAX_LIST_NUM; index++) {
		...
		if (node->uid == uid) {
			...
			node->mpflow_fail_syn_timeout = 0;
			node->start_jiffies = 0;
			err = memset_s(node->retrans_count,
				       sizeof(node->retrans_count), 0,
				       sizeof(node->retrans_count));
			if (err != EOK)
				EMCOM_LOGD(
					"emcom_xengine_mpflow_clear_blocked memset failed");
		}
	}
}
...
bool emcom_xengine_mpflow_blocked(int uid, const char *name, uint8_t ver)
{
	...
	for (index = 0; index < EMCOM_MPFLOW_MAX_LIST_NUM; index++) {
		node = &mpflow_list[index];
		if ((node->uid == uid) &&
		    (!strncmp(node->name, name, strlen(name))))
			return (node->mpflow_fallback ==
				EMCOM_MPFLOW_FALLBACK_SET);
	}
...
void emcom_xengine_mpflow_show(void)
{
	...
	for (index = 0; index < EMCOM_MPFLOW_MAX_LIST_NUM; index++) {
		...
		if (node->uid == UID_INVALID_APP)
			continue;
		EMCOM_LOGE(
			"MpFlow showinfo uid:%d inf:%d(%s) fail:%d estlink:%d "
...
bool emcom_xengine_mpflow_errlink(int reason,
				  struct emcom_xengine_mpflow_stat *node)
{
	...
	if ((node->mpflow_fail_nopayload >=
	     EMCOM_MPFLOW_FALLBACK_NOPAYLOAD_THRH) ||
	    (node->mpflow_fail_syn_rst >= EMCOM_MPFLOW_FALLBACK_SYN_RST_THRH) ||
	    (node->mpflow_fail_syn_timeout >=
	     EMCOM_MPFLOW_FALLBACK_SYN_TOUT_THRH))
		return true;

	return false;
}

bool emcom_xengine_mpflow_retrans(int reason,
				  struct emcom_xengine_mpflow_stat *node)
{
	int i;
	errno_t err;
	...
	for (i = 1; (i <= node->retrans_count[0]) &&
		    (i <= EMCOM_MPFLOW_FALLBACK_RETRANS_TIME);
	     i++) {
	...
	if (i <= node->retrans_count[0]) {
		/* expand time range */
		if (node->retrans_count[i] ==
		    EMCOM_MPFLOW_FALLBACK_RETRANS_THRH) {
			node->retrans_count[0]++;
			EMCOM_LOGD(
				"MpFlow fallback uid:%d inf:%d(%s) count:%d, jiffies:%lu\n",
				node->uid, node->ifindex, node->name,
				node->retrans_count[0], node->start_jiffies);
		}
		/* retransmission fallback */
		if (node->retrans_count[0] >
		    EMCOM_MPFLOW_FALLBACK_RETRANS_TIME) {
			err = memset_s(node->retrans_count,
				       sizeof(node->retrans_count), 0,
				       sizeof(node->retrans_count));
			if (err != EOK)
				EMCOM_LOGE(
					"emcom_xengine_mpflow_retrans memset failed");
			return true;
		}
	} else {
...
int8_t emcom_xengine_mpflow_checkstatus(const struct sock *sk, int *reason,
					int state,
					struct emcom_xengine_mpflow_stat *node)
{
	struct tcp_sock *tp = tcp_sk(sk);
	...
	if (*reason == EMCOM_MPFLOW_FALLBACK_NOPAYLOAD) {
		...
		if ((oldstate == TCP_ESTABLISHED) &&
			...
			if (state != TCP_FIN_WAIT1) {
				result = ((tp->bytes_received <= 1) &&
					  ((tp->bytes_acked > 1) ||
					   (tp->snd_nxt - tp->snd_una >
					    EMCOM_MPFLOW_SND_BYTE_THRESHOLD))) ?
						 EMCOM_MPFLOW_FALLBACK_SET :
						 EMCOM_MPFLOW_FALLBACK_CLR;
			} else {
		...
		result = ((1 << oldstate) & (TCPF_SYN_SENT | TCPF_SYN_RECV)) ?
				 EMCOM_MPFLOW_FALLBACK_SET :
	...
	if (result != EMCOM_MPFLOW_FALLBACK_NONE)
		EMCOM_LOGD(
			"MpFlow checkinfo uid:%d sk:%pK src_addr:" IPV4_FMT
			":%d dst_addr:" IPV4_FMT ":%d inf:%d[%s]"
			"R:%d ost:%d nst:%d P[%d->%d] ret:%d,snt,fly,ack,rcv:%u,%u,%llu,%llu\n",
			node->uid, sk, IPV4_INFO(sk->sk_rcv_saddr), sk->sk_num,
			IPV4_INFO(sk->sk_daddr), ntohs(sk->sk_dport),
			node->ifindex, node->name, *reason, oldstate, state,
			ntohs(inet->inet_sport), ntohs(inet->inet_dport),
			result, tp->segs_out, tp->snd_nxt - tp->snd_una,
			tp->bytes_acked, tp->bytes_received);

	return result;
}

static void
emcom_xengine_mpflow_fallback_report(struct sock *sk, int reason, int state,
				     const char *name,
				     struct emcom_xengine_mpflow_stat *node)
{
	int8_t result =
		emcom_xengine_mpflow_checkstatus(sk, &reason, state, node);
	if (result == EMCOM_MPFLOW_FALLBACK_SET) {
		if (emcom_xengine_mpflow_errlink(reason, node) ||
		    emcom_xengine_mpflow_retrans(reason, node)) {
			struct emcom_xengine_mpflow_fallback fallback;
			EMCOM_LOGE(
				"MpFlow fallback uid:%d inf:%d(%s) estlink:%d nodata,rst,tout:%d,%d,%d\n",
				node->uid, node->ifindex, node->name,
				node->mpflow_estlink,
				node->mpflow_fail_nopayload,
				node->mpflow_fail_syn_rst,
			...
			if (!strncmp(EMCOM_WLAN0_IFNAME, name, strlen(name))) {
				fallback.reason =
					EMCOM_MPFLOW_FALLBACK_WLAN_OFFSET +
					reason;
			} else if (!strncmp(EMCOM_CELL0_IFNAME, name,
					    strlen(name)) &&
				   (reason != EMCOM_MPFLOW_FALLBACK_RETRANS)) {
				fallback.reason =
					EMCOM_MPFLOW_FALLBACK_LTE_OFFSET +
					reason;
			} else if (!strncmp(EMCOM_WLAN1_IFNAME, name,
					    strlen(name))) {
				fallback.reason =
					EMCOM_MPFLOW_FALLBACK_WLAN1_OFFSET +
					reason;
			} else if (!strncmp(EMCOM_CELL1_IFNAME, name,
					    strlen(name)) &&
				   (reason != EMCOM_MPFLOW_FALLBACK_RETRANS)) {
				fallback.reason =
					EMCOM_MPFLOW_FALLBACK_LTE1_OFFSET +
					reason;
			} else {
				return;
			}
...
void emcom_xengine_mpflow_fallback(struct sock *sk, int reason, int state)
{
	...
	if (!dev || (dev->name[0] == '\0')) {
		rcu_read_unlock();
		EMCOM_LOGE("get dev name failed.dev[%pK] sk[%pK] dev_if[%d]",
			   dev, sk, sk->sk_bound_dev_if);
		return;
	}
	...
	node = emcom_xengine_mpflow_get(uid_info.uid, name, sk->sk_bound_dev_if,
					uid_info.ver);
	if (!node)
		return;
...
void emcom_mpflow_check_mpdns_addr(struct sock *sk, int reason, int result)
{
	...
	if (bound_net_type == EMCOM_XENGINE_NET_INVALID ||
