...
static int hitrace_test_spantest_002(void)
{
	...
	struct hitrace_id_struct id =
		hitrace_begin("test", HITRACE_FLAG_DONOT_CREATE_SPAN);
...
static int hitrace_test_tracepointtest_001(void)
{
	...
	if (!hitrace_is_flag_enabled(&id, HITRACE_FLAG_TP_INFO))
		return HITRACE_TEST_FAIL;
	hitrace_trace_point(HITRACE_TP_CS, &id, "client send msg content %d",
			    msg_conent);
	hitrace_trace_point(HITRACE_TP_CS, NULL, "client send msg content %d",
			    msg_conent);
	hitrace_end(&id);
	return HITRACE_TEST_OK;
}
...
static int hitrace_test_tracepointtest_002(void)
{
	...
	hitrace_clear_id();
	id = hitrace_begin("test no tp flag", HITRACE_FLAG_INCLUDE_ASYNC |
						      HITRACE_FLAG_NO_BE_INFO);
	if (hitrace_is_flag_enabled(&id, HITRACE_FLAG_TP_INFO))
		return HITRACE_TEST_FAIL;
...
static int hitrace_test_syncasynctest_001(void)
{
	...
	if (hitrace_is_flag_enabled(&sync_id, HITRACE_FLAG_INCLUDE_ASYNC))
		return HITRACE_TEST_FAIL;
	hitrace_trace_point(HITRACE_TP_CS, &sync_id, "client send msg: %s",
			    "sync");
	hitrace_end(&sync_id);
	...
	if (!hitrace_is_flag_enabled(&async_id, HITRACE_FLAG_INCLUDE_ASYNC))
		return HITRACE_TEST_FAIL;
	hitrace_trace_point(HITRACE_TP_CS, &async_id, "client send msg: %s",
			    "async");
...
static int hitrace_test_invalid_param_test_001(void)
{
	...
	invalid_flag_id =
		hitrace_begin("test invalid flag", invalid_large_flag);
	if (hitrace_is_valid(&invalid_flag_id))
		return HITRACE_TEST_FAIL;
	invalid_flag_id =
		hitrace_begin("test invalid flag", invalid_little_flag);
	if (hitrace_is_valid(&invalid_flag_id))
		return HITRACE_TEST_FAIL;
...
static int __init hitrace_debug_init(void)
{
	...
	hitrace_file = debugfs_create_file("test", 0644, hitrace_root, NULL,
					   &hitrace_test_fops);
	return 0;
}
