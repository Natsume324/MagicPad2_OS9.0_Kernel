...
static const char *const hitrace_type_to_name[] = {
	HITRACE_TO_STRING(HITRACE_TP_CS),      HITRACE_TO_STRING(HITRACE_TP_CR),
	HITRACE_TO_STRING(HITRACE_TP_SS),      HITRACE_TO_STRING(HITRACE_TP_SR),
...
static uint64_t hitrace_create_chain_id(void)
{
	...
	struct timeval tv;
#endif
	struct hitrace_chain_id_struct chain_id = {.padding = 0, .chain_id = 0};
...
struct hitrace_id_struct hitrace_begin(const char *name, int flags)
{
	...
	hitrace_init_id(&id);
	if (hitrace_is_valid(&(thread_id.id)) || (flags < HITRACE_FLAG_MIN) ||
	...
	if (!hitrace_is_flag_enabled(&(thread_id.id), HITRACE_FLAG_NO_BE_INFO))
...
void hitrace_end(const struct hitrace_id_struct *id_pointer)
{
	...
	if (hitrace_get_chain_id(id_pointer) !=
		...
		/* use hilog interface instead. */
		pr_err("HiTraceEnd error: end id(%lld) != thread id(%lld)\n",
...
{
	if (!hitrace_is_valid(id_pointer) || (len < (int)HITRACE_ID_LEN) ||
	    (!pid_array))
		return 0;
				   ...
				   const char *fmt, va_list args)
{
	...
	if (!hitrace_is_valid(id_pointer) || type < 0 || type >= HITRACE_TP_MAX)
		return;
