...
#if (defined(CONFIG_HONOR_UFS_DSM) && \
			       ...
			       struct scsi_cmnd *cmd)
{
	...
	i = iomt_host_io_latency(iomt_host_info, &(cmd->iomt_start_time), dir,
				 chunk_size);
...
static void iomt_ufs_io_timeout_dsm_action(struct Scsi_Host *host)
{
	...
	if (host != NULL && host->iomt_host_info != NULL &&
...
#define FLAG_DSM_UFS_IO_TIMEOUT 0x00200000
		pr_err("%s:%s(%d) ufs io timeout, judge slot = %u, "
		       ...
		       __func__, ufs_host_name(host),
		       hba->dev_info.wmanufacturerid,
		       iomt_host_info->io_timeout_dsm.judge_slot, opcode,
		       op_arg, block_ticks >> IOMT_OP_BLOCK_SHIFT,
					   ...
					   char *buf)
{
	...
	if (host->iomt_host_info != NULL)
		return io_latency_scatter_show(
			(struct iomt_host_info *)host->iomt_host_info, buf);
	return 0;
}
...
static DEVICE_ATTR(io_latency_scatter, 0644, io_latency_ufs_show_checker,
...
static void ufs_host_create_latency_attr(void *iomt_host_info)
{
	...
	if (device_create_file(&host->shost_dev, &dev_attr_io_latency_scatter))
		dev_err(&host->shost_dev,
			"Failed to create io_latency_scatter sysfs entry\n");
...
static DEVICE_ATTR(rw_size_scatter, 0444, rw_size_ufs_show_checker, NULL);
...
static void ufs_host_create_rw_size_attr(void *iomt_host_info)
{
	...
	if (device_create_file(&host->shost_dev, &dev_attr_rw_size_scatter))
		dev_err(&host->shost_dev,
			"Failed to create rw_size_scatter sysfs entry\n");
}
...
EXPORT_SYMBOL(dsm_iomt_ufs_host_init);
