...
#define iomt_cls_dev_to_mmc_host(d) container_of(d, struct mmc_host, class_dev)

#ifndef MMC_CARD_CMDQ_BLK_SIZE
#define MMC_CARD_CMDQ_BLK_SIZE 512
#endif

static void iomt_mmc_io_timeout_dsm_action(struct mmc_host *host)
{
#if (defined(CONFIG_HONOR_EMMC_DSM) || defined(CONFIG_HONOR_EMMC_DSM))
	unsigned int op_arg;
	...
	if (host != NULL && host->card != NULL &&
	    host->iomt_host_info != NULL && !mmc_card_is_removable(host)) {
...
static DEVICE_ATTR(io_latency_scatter, 0644, io_latency_mmc_show_checker,
...
static DEVICE_ATTR(rw_size_scatter, 0444, rw_size_mmc_show_checker, NULL);
