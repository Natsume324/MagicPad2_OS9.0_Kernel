...
static inline u32 io_measure_blk_optype_get(unsigned int op, unsigned int flag)
{
	...
	case REQ_OP_WRITE_SAME:
		...
		if (flag &
		    (REQ_SYNC | REQ_FUA | REQ_PREFLUSH | REQ_META | REQ_FG))
#else
...
{
#ifdef CONFIG_LONG_IO_LATENCY
	strncpy(rq->curr, current->comm, MAX_COMM_LEN - 1);
#endif
...
#endif
				grp_idx = blkcg->type < grp_idx ? blkcg->type :
								  grp_idx;
		}
		rcu_read_unlock();
	}
					      ...
					      struct scsi_cmnd *cmd)
{
	...
	if (iolatency_usertype == IOM_USER_NOT_COMMERCIAL) {
		if (delta >= SCSI_LATENCY_THRESHOLD &&
		    (rq->cmd_flags & REQ_HW_VIP)) {
			ioscsi_num++;
			if (ioscsi_num % LONG_IO_INTERVAL == 1) {
				LOG_JANK_D(
					JLID_LONG_IO_LATENCY,
					"#ARG1:<%s>#ARG2:<%llu>#TY:<%s>#NU:<%lu>",
					rq->curr, IOM_NS_TO_MS(delta),
					SCSI_LATENCY, ioscsi_num);
			}
			iom_pr_info(
				"scsi appeared long IO, delta = %llums, comm = %s, num = %lu\n",
				IOM_NS_TO_MS(delta), rq->curr, ioscsi_num);
		}
	}
#endif
