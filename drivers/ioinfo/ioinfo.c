...
static bool g_get_statpath =
	true; /*true:path not obtained, false:obtained path*/
...
static void io_top_save_io_info(struct io_output_info *pio_inf,
				struct io_node *ion, u64 read_bytes,
				u64 write_bytes)
{
...
lock_task_sighand_ko(struct task_struct *task, unsigned long *flags)
{
...
static int io_top_io_accounting(int pid_nr, struct io_node *ion)
{
	...
	if (task) {
		...
		if (ret != 0)
			pr_debug(
				"io_stat:ERR: iotop do io accounting failed\n");
...
static void io_get_statpath(void)
{
	...
	while (g_get_statpath) {
		...
		link_path =
			d_path(&filp_userdata->f_path, devpath, IOSTAT_PATHLEN);
...
static int io_dev_get_devstats(struct io_diskstats *pio_d)
{
	...
	if (ret > 0) {
		sscanf(buf, "%lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
		       &pio_d->read_tps, &pio_d->read_merged,
		       &pio_d->read_sectors, &pio_d->read_time_ms,
		       &pio_d->write_tps, &pio_d->write_merged,
		       &pio_d->write_sectors, &pio_d->write_time_ms,
		       &pio_d->io_current, &pio_d->time_ms_io,
		       &pio_d->time_weight);
	}
...
static void io_dev_get_cachestats(struct io_output_info *pio_inf)
{
	...
	pio_inf->pgstat.refault =
		PAGETOKB(pgcache_new_d.refault - pgcache_old_d.refault);
...
static void io_dev_stat(struct io_output_info *pio_inf)
{
	...
	pio_inf->read_kb =
		SECTOKB(io_new_d.read_sectors - io_old_d.read_sectors);
	pio_inf->write_kb =
		SECTOKB(io_new_d.write_sectors - io_old_d.write_sectors);
...
static void io_wait_stat(struct io_output_info *pio_inf)
{
	...
	pio_inf->iowait =
		(u32)(inc.iowait * 100 /
		      (inc.user + inc.nice + inc.system + inc.softirq +
		       inc.irq + inc.idle + inc.iowait));
...
static void io_info_output_msg(struct io_output_info *pio_inf)
{
...
#ifdef CONFIG_HONOR_IO_INFO_DEBUG
	trace_io_info_debug(pio_inf->read_kb, pio_inf->write_kb,
			    &pio_inf->pgstat);
#endif
	...
	len += snprintf(buf + len, IOSTAT_BUFLEN - len,
			" file:%lu free:%lu slab:%lu refault:%lu pgpress:%d",
			pio_inf->pgstat.pgfile, pio_inf->pgstat.free,
			pio_inf->pgstat.slab, pio_inf->pgstat.refault,
			get_pgpress());
#endif
#endif
	...
	for (i = 0; i < pio_inf->iotop_cnt; i++) {
		...
		len += snprintf(buf + len, IOSTAT_BUFLEN - len,
				"pid:%d %s rw:%u/%u; ", io_out->ion->pid,
				io_out->ion->command, io_out->read_kb,
				io_out->write_kb);
...
{
	g_io_pid_hash = (struct hlist_head *)kmalloc(
		sizeof(struct hlist_head) * IOSTAT_PID_HASH_LEN, GFP_KERNEL);
	if (g_io_pid_hash == NULL)
		return -ENOMEM;

	memset(g_io_pid_hash, 0,
	       sizeof(struct hlist_head) * IOSTAT_PID_HASH_LEN);
...
static void io_data_stat_func(struct work_struct *work)
{
	...
	if ((KBTOMB(g_io_inf.read_kb + g_io_inf.write_kb) >
	     IOSTAT_TOP_DISK_THRESHOLD) ||
	    (KBTOMB(rw_delayed_kb) > IOSTAT_TOP_DISK_THRESHOLD) ||
	    (g_io_inf.iowait > IOSTAT_IOWAIT_THRESHOLD)) {
