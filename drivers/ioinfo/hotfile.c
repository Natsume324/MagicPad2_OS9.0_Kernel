...
{
	struct hf_inode *node =
		kmem_cache_alloc(hf_stat.inode_cachep, GFP_ATOMIC);
...
static inline void hf_add_node(struct hlist_bl_head *head,
			       struct hf_inode *node)
{
	hlist_bl_add_head(&node->hlist, head);
}

static inline struct hf_inode *hf_get_node(struct hlist_bl_head *head, u32 ino,
					   u32 dev, u32 *list_len)
{
...
#else
static void hf_page_access_record(void *ignore, struct folio *folio)
#endif
{
	...
	if (!node) {
		...
		if (HF_GET_FLAG(hf->enable_flag, HF_SET_AUTO)) {
			if (HF_HOT_WEIGHT(node->count, node->size) >
			    hf->hot[type].cfg_weight)
...
static void hf_add_hot_files(struct hf_inode *node)
{
	...
	if (weight <= hot->min_weight) {
		...
		list_for_each(pos, &hot->hot_head) {
			...
			cur_weight =
				HF_HOT_WEIGHT(cur_node->count, cur_node->size);
...
static void hf_stat_start(void)
{
	...
	if (hf->inode_hash)
		return;
	hf->inode_hash = kvmalloc(
		sizeof(struct hlist_bl_head) * HF_HASH_BUCKET_NUM, GFP_KERNEL);
	if (!hf->inode_hash) {
#ifdef CONFIG_HONOR_HOTFILE_DETECT_DEBUG
	...
	//register tracepoint
	ret = register_trace_mm_filemap_add_to_page_cache(hf_page_access_record,
							  NULL);
	if (ret)
		hf_pr_info(
			"ERR: register add_to_page_cache tracepoint failed.");
...
static void hf_stat_end(void)
{
	...
	unregister_trace_mm_filemap_add_to_page_cache(hf_page_access_record,
						      NULL);
...
#else
	bdev = blkdev_get_no_open((dev_t)dev);
#endif
	if (!bdev)
		goto err;
				...
				struct kobj_attribute *attr, char *buf)
{
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"inode_protect_set_cnt:%d\n",
			atomic_read(&hf_debug.inode_protect_set_cnt));
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"inode_protect_clr_cnt:%d\n",
	...
	for (i = 0; i < HF_CNT; i++) {
		...
		if (!list_empty(&hot->hot_head)) {
			hot_node = list_first_entry(&hot->hot_head,
						    struct hf_inode, list);
			ret += snprintf(
				buf + ret, PAGE_SIZE - ret, "  max_weight:%d\n",
...
HF_ATTR(hotfile_enable_stat, HF_MODE_RW, hf_enable_stat_show,
	hf_enable_stat_store);
HF_ATTR(hotfile_enable_stat_sys, HF_MODE_RW, hf_enable_stat_sys_show,
	hf_enable_stat_sys_store);
HF_ATTR(hotfile_enable_stat_app, HF_MODE_RW, hf_enable_stat_app_show,
	hf_enable_stat_app_store);
HF_ATTR(hotfile_limit_sys_mbytes, HF_MODE_RW, hf_hot_limit_sys_show,
	hf_hot_limit_sys_store);
HF_ATTR(hotfile_limit_app_mbytes, HF_MODE_RW, hf_hot_limit_app_show,
	hf_hot_limit_app_store);
HF_ATTR(hotfile_hot_threshold, HF_MODE_RW, hf_hot_threshold_show,
	hf_hot_threshold_store);
HF_ATTR(hotfile_min_fsize_kbytes, HF_MODE_RW, hf_min_fsize_show,
	hf_min_fsize_store);
HF_ATTR(hotfile_enable_set_auto, HF_MODE_RW, hf_enable_set_auto_show,
	hf_enable_set_auto_store);
 ...
 */
static void hf_file_name_list_show(struct seq_file *m, struct hotfile_stat *hf,
				   u32 type)
{
	struct hf_hot *hot = NULL;
	struct hf_inode *hot_node = NULL;
	...
	list_for_each(pos, &hot->hot_head) {
		...
		if (inode) {
			pbuf = android_fstrace_get_pathname(fname, PATH_MAX,
							    inode);
			iput(inode);
		} else {
			pbuf = NULL;
 ...
 */
static void hf_file_list_show(struct seq_file *m, struct hotfile_stat *hf,
			      u32 type)
{
	struct hf_hot *hot = NULL;
	struct hf_inode *hot_node = NULL;
...
static struct kobject *hf_interface_create(void)
{
	...
	if (proc_entry) {
		proc_entry = proc_create_data(HF_HOT_PROC_SYS_FLIST, HF_MODE_RW,
					      NULL, &hf_sys_flist_proc_fops,
					      &hf_stat);
		if (!proc_entry)
			hf_pr_info("Create proc/%s fail.\n",
				   HF_HOT_PROC_SYS_FLIST);
		proc_entry = proc_create_data(HF_HOT_PROC_APP_FLIST, HF_MODE_RW,
					      NULL, &hf_app_flist_proc_fops,
					      &hf_stat);
		if (!proc_entry)
			hf_pr_info("Create proc/%s fail.\n",
				   HF_HOT_PROC_APP_FLIST);
#ifdef CONFIG_HONOR_HOTFILE_DETECT_DEBUG
		proc_entry = proc_create_data(HF_HOT_PROC_SYS_FN_LIST,
					      HF_MODE_RO, NULL,
					      &hf_sys_fn_list_proc_fops,
					      &hf_stat);
		if (!proc_entry)
			hf_pr_info("Create proc/%s fail.\n",
				   HF_HOT_PROC_SYS_FN_LIST);
		proc_entry = proc_create_data(HF_HOT_PROC_APP_FN_LIST,
					      HF_MODE_RO, NULL,
					      &hf_app_fn_list_proc_fops,
					      &hf_stat);
		if (!proc_entry)
			hf_pr_info("Create proc/%s fail.\n",
