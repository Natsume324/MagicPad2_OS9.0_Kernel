...
bool bst_fg_is_socket_set(const struct sock *sock,
			  struct bst_fg_custom_info *custom_info)
{
	uint8_t idx;
	...
	for (idx = 0; idx < custom_info->socket_num; idx++) {
		if (sock == custom_info->socket_info[idx].sock_ptr) {
			...
			bastet_loge(
				"socket compare fail, pointer and ino is not corresponding");
		}
	}
			   ...
			   bool ipv4_flag, struct sk_buff *skb)
{
	...
	uc_app_num = bst_min(g_st_bastet_fg_custom.uc_app_num,
			     BST_FG_MAX_CUSTOM_APP);
	for (idx = 0; idx < uc_app_num; idx++) {
		if (sock_uid ==
		    g_st_bastet_fg_custom.ast_custom_info[idx].uid) {
			past_custom_info =
					      ...
					      uint16_t *infos_msg_len)
{
	...
	*infos_msg_len -= BST_FG_CUSTOM_INFO_FIX_LEN;
	...
	for (socket_idx = 0; socket_idx < save_info_ptr->socket_num;
	     socket_idx++) {
		...
		*infos_msg_len -= BST_FG_SOCKET_FD_LEN;
		...
		if (sock_ptr != NULL) {
			save_info_ptr->socket_info[socket_idx].ino =
				sock_i_ino(sock_ptr);
			save_info_ptr->socket_info[socket_idx].src_port =
				sock_ptr->sk_num;
			sock_put(sock_ptr);
		} else {
...
static void bastet_fastgrab_save_info(uint16_t app_num, void *infos_ptr,
				      uint16_t infos_msg_len)
{
	uint8_t idx;
	...
	for (idx = 0; idx < g_st_bastet_fg_custom.uc_app_num; idx++) {
		if (!bastet_fastgrab_save_one_app_info(idx, &infos_ptr,
						       &infos_msg_len)) {
...
static void bastet_fastgrab_print_info(void)
{
	uint8_t info_idx;
	uint8_t socket_idx;
	uint16_t len = 0;
	...
	for (info_idx = 0; info_idx < g_st_bastet_fg_custom.uc_app_num;
		...
		for (socket_idx = 0; socket_idx < custom_info->socket_num;
		     socket_idx++) {
			len = snprintf_s(
				print_buffer_ptr, available_buffer_size,
				available_buffer_size - 1, "%d,%u;",
				custom_info->socket_info[socket_idx].socket_fd,
				custom_info->socket_info[socket_idx].src_port);
			if (len < 0) {
				bastet_loge(
					"socket print buffer is full, len is %u, available_buffer_size is %u",
...
static void bastet_fastgrab_do_command(struct req_msg_head *msg)
{
	...
	if (!msg || (msg->len < sizeof(struct req_msg_head) + sizeof(uint8_t) +
					sizeof(uint16_t))) {
	...
	custom_infos_ptr =
		(struct bst_fg_custom_info *)(app_num_ptr + sizeof(uint16_t));
	custom_infos_msg_len = msg->len - sizeof(struct req_msg_head) -
			       sizeof(uint8_t) - sizeof(uint16_t);

	bastet_fastgrab_save_info(app_num, custom_infos_ptr,
				  custom_infos_msg_len);
...
msg_process *bastet_fastgrab_init(notify_event *fn)
{
	...
	memset_s(&g_st_bastet_fg_custom, sizeof(g_st_bastet_fg_custom), 0,
		 sizeof(g_st_bastet_fg_custom));
