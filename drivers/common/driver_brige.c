...
struct tp_spi_interface_ops *g_tp_spi_interface_ops;
...
void tp_spi_interface_ops_register(struct tp_spi_interface_ops *ops)
{
	if (g_tp_spi_interface_ops == NULL) {
		g_tp_spi_interface_ops = ops;
		pr_info("%s g_tp_spi_interface_ops register success\n",
			__func__);
	} else {
		pr_info("%s:g_tp_spi_interface_ops has been registered\n",
			__func__);
	}
}

void tp_spi_interface_ops_unregister(struct tp_spi_interface_ops *ops)
{
	if (g_tp_spi_interface_ops == ops) {
		g_tp_spi_interface_ops = NULL;
		pr_info("%s g_tp_spi_interface_ops unregister success\n",
			__func__);
	} else {
		pr_info("%s:g_tp_spi_interface_ops unregister fail\n",
			__func__);
	}
}

struct tp_spi_interface_ops *get_tp_spi_interface_ops(void)
{
	return g_tp_spi_interface_ops;
}
EXPORT_SYMBOL(tp_spi_interface_ops_register);
EXPORT_SYMBOL(tp_spi_interface_ops_unregister);
EXPORT_SYMBOL(get_tp_spi_interface_ops);
