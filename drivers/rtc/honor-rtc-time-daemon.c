...
static int honor_rtc_time_daemon_read_time(struct device *dev,
					   struct rtc_time *tm)
{
	int ret = -1;
	...
	if (honor_rtc_secs > MAX_TIME_VALUE) {
		dev_err(dev,
			"honor_rtc_secs:%llu is bigger than MAX_TIME_VALUE\n",
			honor_rtc_secs);
...
int honor_rtc_time_daemon_init(struct device *dev)
{
	...
	if ((offset_info.offset + qpnp_rtc_secs) >= MAX_TIME_VALUE) {
		...
		check_rtc_timer.expires =
			jiffies + msecs_to_jiffies(CHECK_RTC_DELAY_TIME);
