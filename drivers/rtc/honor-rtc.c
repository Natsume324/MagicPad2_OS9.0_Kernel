...
static int write_emmc_raw_part(const char *dev_path, uint64_t offset, char *buf,
			       uint64_t buf_size)
{
	struct file *fp = NULL;
	int ret = -1;
...
static void write_offset_to_raw_part(struct work_struct *work)
{
	int ret = -1;
...
static int honor_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	int ret = -1;
...
static int honor_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	int ret = 0;
	...
	dev_info(
		dev,
		"func %s: the setting time is h:m:s == %d:%d:%d, d/m/y = %d/%d/%d\n",
		__func__, tm->tm_hour, tm->tm_min, tm->tm_sec, tm->tm_mday,
	...
	if (offset_info.offset == (secs - qpnp_rtc_secs)) {
		dev_info(
			dev,
			"func %s: the offset is not changed, need not to update offset !\n",
			__func__);
		return ret;
	}
	...
	if (delayed_work_pending(&write_offset_delay_work)) {
		dev_info(
			dev,
			"func %s: There is a work pending in rtc_off_work_queue \n",
			__func__);
		if (!cancel_delayed_work(&write_offset_delay_work)) {
			dev_err(dev,
				"func %s: cancel_delayed_work write_offset_delay_work failed ! \n",
				__func__);
		}
...
static int honor_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	int ret;
	...
	if (ret) {
		dev_err(dev,
			"func %s: Invalid time used to set qpnp rtc alarm\n",
			__func__);
		return ret;
	}
...
static int honor_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	int ret;
	uint64_t secs, secs_rtc;
	struct rtc_time *rtc_tm;
...
get_honor_rtc_rw_ops(const struct rtc_class_ops *qpnp_rtc_rw_ops_ptr)
{
	...
	memcpy(honor_rtc_rw_ops_ptr, qpnp_rtc_rw_ops_ptr,
...
int honor_rtc_init(struct device *dev)
{
	...
	if (qpnp_rtc_secs < RTC_MIN_VALUE_TO_RESET_OFFSET) {
		...
		queue_delayed_work(rtc_off_work_queue, &write_offset_delay_work,
				   msecs_to_jiffies(CLEAR_OFFSET_DELAY_TIME));
		return -1;
	}
