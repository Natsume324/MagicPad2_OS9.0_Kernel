...
static struct bg_ctrl_policy_t aware_net_bg_ctrl_policy[] = {
	{20, 1},    {50, 10},	 {100, 25},   {200, 10},   {500, 25},
	{1000, 50}, {1000, 100}, {1000, 200}, {1000, 300}, {0x1fffffff, 100}};
...
static void updata_network_data(const unsigned long *cur_time,
				unsigned long *network_sum,
				unsigned long *total_len_sum,
				unsigned long *oldest_time,
...
static void limit_bg(void)
{
	...
	if (cur_mode == AWARE_NET_CTRL_MODE_FIRSE_LEVEL) {
		...
		for (i = 0; i < MAX_FG_NET_STAT; i++) {
			...
			if (tmp_for_cnt->time + MAX_FG_NET_STAT * PERIOD_TIME <
			    cur_time)
				continue;
...
void reinit_ctrl_policy(int limit_ratio, int package_ratio)
{
	...
	for (i = 0; i < count; i++) {
		...
		else if (aware_net_bg_ctrl_policy[i].sleep_long > MAX_BG_SLEEP)
...
void dump_aware_net_stats(struct seq_file *m)
{
	...
	seq_printf(
		m,
		"bg_stats rate:%lu  avg_long:%lu size:%lu count:(bg:%lu, fg:%lu) time_long:%lu\n",
...
static int aware_ctrl_show(struct seq_file *m, void *v)
{
	seq_printf(
		m,
		"aware_ctrl enable:%d mode:%d limit_rate:%dk limit_ratio:%d package_ratio:%d\n",
		aware_net_ctrl.enable, aware_net_ctrl.mode,
		aware_net_ctrl.limit_rate, aware_net_ctrl.limit_ratio,
