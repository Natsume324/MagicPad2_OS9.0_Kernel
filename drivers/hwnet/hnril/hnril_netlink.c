...
static const struct nla_policy hnril_policy[HNRIL_ATTR_MAX + 1] = {
	[HNRIL_ATTR_VSIM_IFNAME] = {.type = NLA_NUL_STRING,
				    .len = IFNAMSIZ - 1},
...
int hnril_nl_vsim_flow_notify(u64 tx_bytes, u64 rx_bytes)
{
	...
	hdr = genlmsg_put(skb, 0, 0, &hnril_genl_family, 0,
	...
	if (nla_put_u64_64bit(skb, HNRIL_ATTR_VSIM_TX_BYTES, tx_bytes,
			      HNRIL_ATTR_PAD)) {
		pr_info("%s: msg size error", __func__);
	...
	if (nla_put_u64_64bit(skb, HNRIL_ATTR_VSIM_RX_BYTES, rx_bytes,
			      HNRIL_ATTR_PAD)) {
		pr_info("%s: msg size error", __func__);
		genlmsg_cancel(skb, hdr);
		nlmsg_free(skb);
...
module_init(hnril_netlink_init) module_exit(hnril_netlink_exit)
