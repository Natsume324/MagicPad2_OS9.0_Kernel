...
static unsigned int vsim_hook(void *priv, struct sk_buff *skb,
...
#endif
{
	...
	pr_info("vsim: flow notify tx=%lu rx=%lu err=%d", tx_bytes, rx_bytes,
		err);
...
void vsim_notify_tether_stats(const char *ifname, uint64_t tether_rx_bytes,
			      uint64_t tether_tx_bytes)
{
	if (unlikely(!spin_trylock_bh(&vsim_lock))) {
		return;
	}
	...
	pr_info("%s: ifname:%s, rx_bytes:%llu, tx_bytes:%llu", __func__, ifname,
		tether_rx_bytes, tether_tx_bytes);
...
void regist_vsim_tcc_timeout(void)
{
	...
	const unsigned char uuid[] = {0x47, 0x91, 0xe8, 0xab, 0x61, 0xcd,
				      0x3f, 0xf4, 0x71, 0xc4, 0x1a, 0x31,
				      0x7e, 0x40, 0x53, 0x12};
	pr_info("%s\n", __func__);

	if (TC_NS_RegisterServiceCallbackFunc(
		    (const char *)uuid, vsim_tee_timeout_callback, NULL) != 0) {
