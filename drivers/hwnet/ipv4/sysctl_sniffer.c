...
proc_file_config_struct g_configs = {
	0,  TRIM_DATA_PAYLOAD_LEN, PCAP_FILE_NUM, PCAP_FILE_LEN, {0}, {NULL},
	{0}};
...
static int proc_write_pcap_hdr_for_empty_file(int flag_rx_tx)
{
	...
	if (flag_rx_tx != DIRECTION_MONITOR_MODE) {
		printk(KERN_ERR
		       "{%s:%d} ERROR : without radiotap flag_rx_tx = %d\n",
...
static int solve_packet_filter(const st_packet_filter *packet_filter, int loop,
			       const char *p_llc_hdr)
{
	int port_loop;
	...
	for (port_loop = 0; port_loop < packet_filter[loop].port_count;
		...
		if (ipv4_src_port == (ipv4_src_port &
		    ...
		    ipv4_dst_port ==
			    (ipv4_dst_port & get_filter_dport(loop, port_loop)))
...
static unsigned int
proc_get_trimed_packet_len_monitor(const char *frame_buff,
				   unsigned int frame_len,
				   unsigned int packet_len_trim)
{
	...
	if (IEEE80211_FRAME_TYPE_DATA == p_frame_type->bit_type) {
		if (packet_len_trim + sizeof(mac_ieee80211_qos_stru) <
		    frame_len)
			real_len = packet_len_trim +
				   sizeof(mac_ieee80211_qos_stru);
	}
	return real_len;
}

static int proc_trim_packet_len_hybrid(const char *header_buff,
				       int *p_header_len,
				       const char *frame_buff, int *p_frame_len,
				       int packet_len_trim)
{
	int trim_flag;
	char *p_llc_hdr = NULL;
	...
	if (p_frame_type->bit_type == IEEE80211_FRAME_TYPE_DATA) {
		...
		if ((*p_frame_len + *p_header_len) >
		    (packet_len_trim + IEEE80211_MAC_HDR_LEN))
			*p_frame_len = packet_len_trim + IEEE80211_MAC_HDR_LEN -
				       *p_header_len;
	}
	return p_frame_type->bit_type;
}
					       ...
					       char *dst_buff)
{
	...
	if ((DIRECTION_TX == flag_rx_tx) &&
		...
		((mac_header_control_stru *)dst_buff)->bit_sub_type =
			IEEE80211_FRAME_SUBTYPE_QOS;
...
static void proc_write_packet_to_file(char *dst_buff, const char *header_buff,
				      int header_len, const char *frame_buff,
				      int frame_len, int flag_rx_tx,
				      int orig_len)
{
...
static void record_cur_file_time(const oam_pcap_pkthdr_stru *packet_header)
{
	...
	g_configs.microtime[g_configs.cur_file] =
		packet_header->time.micro_time;
}

int proc_sniffer_write_file(const char *header_buff, unsigned int header_len,
			    const char *frame_buff, unsigned int frame_len,
			    int flag_rx_tx)
{
	char *dst_buff = NULL;
	unsigned long flag;
	...
	orig_len = frame_len;
	if (flag_rx_tx != DIRECTION_MONITOR_MODE) {
		frame_type = proc_trim_packet_len_hybrid(
			header_buff, &header_len, frame_buff, &frame_len,
			SNIFFER_MAX_PACKET_LEN);
	} else {
		// DEBUG: temp not trim length temperially for full mode
		frame_len = proc_get_trimed_packet_len_monitor(
			frame_buff, frame_len, SNIFFER_MAX_PACKET_LEN);
	}

	proc_init_file_to_save_packet(sizeof(oam_pcap_pkthdr_stru) + frame_len +
				      header_len);
	proc_write_pcap_hdr_for_empty_file(flag_rx_tx);

	dst_buff = SNIFFER_CUR_FILE_BUFF_POS;
	proc_increase_cur_file_pos(sizeof(oam_pcap_pkthdr_stru) + frame_len +
				   header_len);
	proc_write_packet_to_file(dst_buff, header_buff, header_len, frame_buff,
				  frame_len, flag_rx_tx, orig_len);
	...
	if (flag_rx_tx != DIRECTION_MONITOR_MODE)
		proc_set_rx_packet_to_no_proection(
			flag_rx_tx, frame_type,
			dst_buff + sizeof(oam_pcap_pkthdr_stru));
		      ...
		      size_t *lenp, loff_t *ppos)
{
	...
	if (file_index < 0 || file_index >= PCAP_FILE_NUM || buffer == NULL ||
	    lenp == NULL || ppos == NULL) {
		if (lenp != NULL)
			*lenp = 0;
		return 0;
	...
	if (*ppos >= file_pos) {
		*lenp = 0;
		printk(KERN_ERR
		       "{%s:%d} ERROR:Invalid ppos parameter:0x%x, current file position:0x%x\n",
...
static int get_channel_type(int channel, char channel_type)
{
	...
	int channel_category[CHANNEL_CATEGORY_ARRAY_DIMENSION_ONE_NUM]
			    [CHANNEL_CATEGORY_ARRAY_DIMENSION_TWO_NUM] = {
				    {36, 52, 100, 116, 132, 149, 184},
				    {40, 56, 104, 120, 136, 153, 188},
				    {44, 60, 108, 124, 140, 157, 192},
				    {48, 64, 112, 128, 144, 161, 196}};
	for (i_loop = 0; i_loop < CHANNEL_CATEGORY_ARRAY_DIMENSION_ONE_NUM;
	     i_loop++) {
		for (j_loop = 0;
		     j_loop < CHANNEL_CATEGORY_ARRAY_DIMENSION_TWO_NUM;
		     j_loop++) {
			if (channel_category[i_loop][j_loop] != channel)
				continue;
			if (channel_type == 'W' || channel_type == 'w')
				return i_loop +
				       (j_loop % 2) * 4; // 4:20/40/80/160MHz
			else
				return i_loop;
		}
	}
...
{
	char new_cmds[50] = {0};
	char *channel_str_5[] = {"11ac80plusplus", "11ac80plusminus",
				 "11ac80minusplus", "11ac80minusminus"};
	char *channel_str160_5[] = {
		"11ac160plusplusplus",	 "11ac160plusplusminus",
		"11ac160plusminusplus",	 "11ac160plusminusminus",
		"11ac160minusplusplus",	 "11ac160minusplusminus",
		"11ac160minusminusplus", "11ac160minusminusminus"};
	char *channel_str_6[] = {"11ax5g80plusplus", "11ax5g80plusminus",
				 "11ax5g80minusplus", "11ax5g80minusminus"};
	char *channel_str160_6[] = {
		"11ax5g160plusplusplus",   "11ax5g160plusplusminus",
		"11ax5g160plusminusplus",  "11ax5g160plusminusminus",
		"11ax5g160minusplusplus",  "11ax5g160minusplusminus",
	...
	if (channel < 36) {
		...
		snprintf(new_cmds, sizeof(new_cmds), cmd_str,
			 get_mode_string(expand_type, g_wifi_chip_type));
	} else {
		...
		if (channel == 165) { // ch#165 only support VHT20
			...
			if ((channel_type == 'W' || channel_type == 'w') &&
			    (channel <= 128)) // VHT160 support
				snprintf(new_cmds, sizeof(new_cmds),
					 (const char *)cmd_str,
					 channel_str160[get_channel_type(
								channel,
								channel_type) %
							8]);
			else
				snprintf(new_cmds, sizeof(new_cmds),
					 (const char *)cmd_str,
					 channel_str[get_channel_type(channel,
								      ' ') %
						     4]);
		}
	}
...
{
	char new_cmds[50] = {0};
	snprintf(new_cmds, sizeof(new_cmds), cmd_str, mac_filter[0],
		 mac_filter[1], mac_filter[2], mac_filter[3], mac_filter[4],
		 mac_filter[5]);
	strncpy(cmd_str, new_cmds, sizeof(new_cmds));
	return 0;
}
...
static int execute_shell(const char *cmd_str)
{
	...
	char *argv2[] = {"/system/bin/sh", "/system/lib/hipriv.sh",
			 "'Honor create wlan0 sta'", NULL};
	char *argv3[] = {"/system/bin/ifconfig", "wlan0", "hw", "ether",
			 "00:E0:52:22:22:14",	 NULL};
...
**/
static int set_monitor_mode_param(const int sniffer_mode, const int channel,
				  const char channel_type,
				  const char *mac_filter)
{
	...
	proc_cmd_execution_struct cmds_off[] = {
		...
		{"wlan0 pm_switch 1", 50,
		 CMD_EXECUTE_TYPE_DRIVER}, // re-enable PS
		{"wlan0 monitor 0", 50, CMD_EXECUTE_TYPE_DRIVER},
		{"wlan0 sniffer 0 ff:ff:ff:ff:ff:ff", 50,
		 CMD_EXECUTE_TYPE_DRIVER}};

	proc_cmd_execution_struct cmds_on[] = {
		{"wlan0 mode %s", 100, CMD_EXECUTE_TYPE_DRIVER},
		{"wlan0 freq %d", 100, CMD_EXECUTE_TYPE_DRIVER},
		{"wlan0 sniffer 1 %02x:%02x:%02x:%02x:%02x:%02x", 50,
		 CMD_EXECUTE_TYPE_DRIVER},
	...
	convert_mode_cmd_str(cmds[CMD_MODE_INDEX].cmds_str, channel,
			     channel_type);
	...
	for (i_loop = 0; i_loop < count; i_loop++) {
		if (g_hipriv_func) {
			if (cmds[i_loop].execute_type ==
			    CMD_EXECUTE_TYPE_SHELL) {
...
static int parse_channel_info(int *p_channel, char *p_ch_type,
			      const char *p_param)
{
	int loop = 0;
...
static void proc_handle_command_sniffer_enable(ctl_cmd_stru *p_cmd2parse)
{
	...
	if ((sniffer_mode < 0) || (sniffer_mode > CMD_ENABLE_MONITOR_MODE)) {
		printk(KERN_ERR
		       "{%s:%d} Invalid parameter:  sniffer_mode = %d \n",
	...
	if (sniffer_mode) {
		if ((!SNIFFER_CUR_FILE_BUFFER) ||
			...
			printk(KERN_INFO
			       "ERROR : Init proc_sniffer_init_buff \n");
			proc_sniffer_init_buff();
		}
		if (sniffer_mode == CMD_ENABLE_MONITOR_MODE) {
			/* sniffer_mode(1)+channel(1)+mac(6) */
			if (p_cmd2parse->ctl_cmd_hdr.i_para_num !=
			    CMD_ENABLE_MONITOR_MODE_PARANUM) {
				printk(KERN_ERR
				       "{%s:%d} Invalid parameter:  i_para_num = %d \n",
				       __FUNCTION__, __LINE__,
				       p_cmd2parse->ctl_cmd_hdr.i_para_num);
				return;
			}
			parse_channel_info(&channel, &channel_type,
					   (char *)&(p_cmd2parse->i_paras[1]));
			for (mac_loop = 0; mac_loop < MAC_ADDR_LEN;
			     mac_loop++) {
				...
				filter[mac_loop] =
					p_cmd2parse->i_paras[2 + mac_loop];
			}

			// send commands to wifi driver to enable it
			set_monitor_mode_param(sniffer_mode, channel,
					       channel_type, filter);
		}
		// create a new file
		proc_init_file_content(g_configs.cur_file,
...
static void proc_handle_command_cur_file_len(ctl_cmd_stru *pCmd2Parse,
					     char __user *buffer,
					     int file_index)
{
	...
	if (pCmd2Parse->ctl_cmd_hdr.i_para_num >= 4) {
		copy_to_user(
			(char __user *)&(((ctl_cmd_stru *)buffer)->i_paras[0]),
...
static int proc_handle_command_file_spec(ctl_cmd_stru *pCmd2Parse,
					 char __user *buffer)
{
	int old_file_num;
	int old_file_len;
	...
	if (pCmd2Parse->i_paras[0] == -1 || pCmd2Parse->i_paras[0] == 0xff ||
	    pCmd2Parse->i_paras[1] == -1 ||
	    pCmd2Parse->i_paras[1] == 0xff) { // it is a read command
	...
	if ((pCmd2Parse->i_paras[0] < PCAP_FILE_NUM_MIN) ||
		...
		printk(KERN_ERR
		       "{%s:%d} Invalid Parameter max_file_num (%d) \n",
	...
	if ((pCmd2Parse->i_paras[1] < PCAP_FILE_LEN_MIN) ||
		...
		printk(KERN_ERR
		       "{%s:%d} Invalid Parameter g_max_file_len (%d) \n",
...
static int proc_handle_command_data_payload(ctl_cmd_stru *pCmd2Parse,
					    char __user *buffer)
{
...
static void proc_handle_command_mumimo_aid(ctl_cmd_stru *pCmd2Parse,
					   char __user *buffer)
{
...
static void proc_handle_command_wifi6_enable(ctl_cmd_stru *pCmd2Parse,
					     char __user *buffer)
{
	if (-1 == pCmd2Parse->i_paras[0]) {
...
int proc_handle_commands_check_param(int file_index, ctl_cmd_stru *cmd2_parse)
{
	...
	if ((cmd2_parse->ctl_cmd_hdr.i_cmd_id <= CMD_BUFF_CMDID_TOP) ||
		...
		printk(KERN_ERR
		       "{%s:%d} ERROR : Invalid Command ID (0x%08x) \n",
		       __FUNCTION__, __LINE__,
		       cmd2_parse->ctl_cmd_hdr.i_cmd_id);
		return -1;
	}
	...
	if ((cmd2_parse->ctl_cmd_hdr.i_para_num < 0) ||
	    (cmd2_parse->ctl_cmd_hdr.i_para_num > CMD_BUF_MAX_PARAM)) {
		printk(KERN_ERR "{%s:%d}: Invalid Para Num (0x%08x) \n",
		       __FUNCTION__, __LINE__,
		       cmd2_parse->ctl_cmd_hdr.i_para_num);
		return -1;
	}
...
int proc_handle_commands(int file_index, char __user *buffer)
{
	...
	case CMD_BUFF_CMDID_CURFILELEN:
		proc_handle_command_cur_file_len(&cmd2_parse, buffer,
						 file_index);
		break;
