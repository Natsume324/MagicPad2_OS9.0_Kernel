...
#define SNIFFER_CUR_FILE_BUFF_POS               \
	(g_configs.buffer[g_configs.cur_file] + \
...
typedef struct __attribute__((__packed__)) {
	...
	unsigned short bit_frag_num : 4, /* sequence control */
		bit_seq_num : 12;
	unsigned char bit_qc_tid : 4, bit_qc_eosp : 1, bit_qc_ack_polocy : 2,
		bit_qc_amsdu : 1;
	union {
...
typedef struct {
	...
	unsigned int gmttime
		[PCAP_FILE_NUM]; /* ms, between now and 1970-01-01 00:00:00 */
	unsigned int microtime
		[PCAP_FILE_NUM]; /* us, between now and 1970-01-01 00:00:00 */
