...
static char *s_arr_title[MAX_ARR_TITLE_COUNT] = {"WEBSENDSEGS", "WEBRESENDSEGS",
						 "WEBRECVSEGS",
						 "WEBRTTDURATION",
						 "WEBRTTSEGS"};
...
struct wifipro_nl_packet_msg {
	...
	unsigned int wlan_v6_tx_pkts;
	unsigned int wlan_v6_rx_pkts;
	unsigned int wlan_v6_retrans;
};
...
static unsigned int wlan_v6_rx_pkt = 0;
static unsigned int wlan_v6_tx_pkt = 0;
static unsigned int wlan_v6_retrans = 0;
...
static void updata_wlan_hidata(struct sock *sk, const struct sk_buff *skb)
{
	if ((!strncmp(sk->wifipro_dev_name, DEV_NAME_WLAN, WLAN_LENGTH)) ||
		...
		update_hidata_tcp_stat_by_uid(get_socket_uid(sk),
					      INDEX_WEBSENDSEGS, 1);
	} else if ((!strncmp(sk->wifipro_dev_name, DEV_NAME_RMNET,
			     RMNET_LENGTH)) ||
...
static bool wifipro_is_v6_sock(struct sock *sk)
{
	if (sk == NULL) {
		return false;
	}
...
{
	if (skb->dev != NULL) {
		if (!strncmp(skb->dev->name, DEV_NAME_WLAN, WLAN_LENGTH)) {
			WIFIPRO_TCP_INC_STATS_BH(&init_net,
						 WIFIPRO_TCP_MIB_WLAN_OUTSEGS);
			return 0;
		} else if (!strncmp(skb->dev->name, DEV_NAME_RMNET,
				    RMNET_LENGTH)) {
			WIFIPRO_TCP_INC_STATS_BH(
				&init_net, WIFIPRO_TCP_MIB_MOBILE_OUTSEGS);
			return 0;
		} else {
			unknown_dev_count++;
						  ...
						  u_int8_t af)
{
	...
	if (is_full_sk &&
	    !strncmp(sk->wifipro_dev_name, DEV_NAME_WLAN, WLAN_LENGTH)) {
		if (af == AF_INET6) {
			wlan_v6_tx_pkt++;
		} else {
	...
	} else if (is_full_sk && !strncmp(sk->wifipro_dev_name, DEV_NAME_RMNET,
					  RMNET_LENGTH)) {
		WIFIPRO_TCP_INC_STATS(&init_net,
...
static int solve_wifipro_tcp_mib_outsegs(struct sock *sk,
					 const struct sk_buff *skb, u_int8_t af)
{
	bool is_full_sk = sk_fullsock(sk);
	if (is_full_sk &&
	    !strncmp(sk->wifipro_dev_name, DEV_NAME_WLAN, WLAN_LENGTH)) {
...
void wifipro_reset_tcp_statistics(void)
{
	wlan_v6_rx_pkt = 0;
	wlan_v6_tx_pkt = 0;
	wlan_v6_retrans = 0;
...
EXPORT_SYMBOL(wifipro_reset_tcp_statistics);

void wifipro_update_tcp_statistics_outsegs(int mib_type,
					   const struct sk_buff *skb,
					   struct sock *from_sk, u_int8_t af)
{
	...
	if (af == AF_INET6) {
		ip6h = ipv6_hdr(skb);
		proto = ip6h->nexthdr;
		if (ip6h == NULL || proto != IPPROTO_TCP) {
			return;
		}
	} else if (af == AF_INET) {
		iph = ip_hdr(skb);
	...
	if (mib_type == WIFIPRO_TCP_MIB_OUTSEGS) {
		if (solve_wifipro_tcp_mib_outsegs_no_check(sk, skb, af))
			return;
	} else {
		unknown_segs_count++;
				   ...
				   struct sock *from_sk, u_int8_t af)
{
	...
	if (mib_type == WIFIPRO_TCP_MIB_OUTSEGS) {
		if (solve_wifipro_tcp_mib_outsegs(sk, skb, af))
			return;
	} else if (mib_type == WIFIPRO_TCP_MIB_INSEGS) {
		if (solve_wifipro_tcp_mib_insegs(sk, skb, af))
			return;
	} else {
		unknown_segs_count++;
		WIFIPRO_DEBUG("mib_type is %d", mib_type);
	...
	if (task != NULL) {
		...
		if (ret > 0) {
			...
			if (strstr(proc_name, "oogle")) {
				...
				WIFIPRO_DEBUG(
					"find a process name %s match system_server. ",
					proc_name);
				return true;
			} else {
				WIFIPRO_DEBUG(
...
static void wifipro_set_cong_stat(unsigned int dest_addr,
				  unsigned int dest_port,
				  wifipro_cong_sock_t *src, unsigned int offset)
{
...
int wifipro_handle_retrans(struct sock *sk, struct inet_connection_sock *icsk)
{
	...
	if (!strncmp(sk->wifipro_dev_name, DEV_NAME_WLAN, WLAN_LENGTH)) {
		if (af == AF_INET6) {
			...
			WIFIPRO_TCP_INC_STATS_BH(
				sock_net(sk), WIFIPRO_TCP_MIB_WLAN_RETRANSSEGS);
		}
		update_wlan_tcp_stat_by_uid(get_socket_uid(sk),
	...
	WIFIPRO_DEBUG("%d retrans=%d  sk_state=%d  rto=%d", dest_port,
				   ...
				   unsigned int quality)
{
	...
	packet->wlan_v6_rx_pkts = wlan_v6_rx_pkt;
	packet->wlan_v6_tx_pkts = wlan_v6_tx_pkt;
	packet->wlan_v6_retrans = wlan_v6_retrans;
				    ...
				    unsigned int dest_addr)
{
	...
	for (i = 1; i <= WIFIPRO_RTT_RECORD_SECONDS; i++) {
		if (time_after(curr->expire +
				       (WIFIPRO_RTT_RECORD_SECONDS - 1) * HZ,
			       jiffies)) {
			/* it's valid now */
			if (!is_rtt_added &&
				      ...
				      unsigned int dest_addr)
{
	...
	WIFIPRO_DEBUG("mobile: rto=%d  srtt_us=%d  pkts=%d, rtt_avg=%d",
		      icsk->icsk_rto, tp->srtt_us, mobile_rtt_calc_pkg,
		      mobile_rtt_average);
}
...
static int __init wifipro_tcp_monitor_module_init(void)
{
	...
	wifipro_congestion_stat = (wifipro_cong_sock_t *)kzalloc(
		WIFIPRO_CONG_ARRAY * sizeof(*wifipro_congestion_stat),
		GFP_KERNEL);
	if (wifipro_congestion_stat == NULL) {
		WIFIPRO_ERROR("kzalloc failed");
		return -1;
...
static ssize_t wifipro_log_level_write(struct file *file,
				       ...
				       loff_t *data)
#else
static int wifipro_log_level_write(struct file *file, char *buffer,
				   size_t count)
#endif
{
...
static void solve_mobile_rtt_second_stat(struct seq_file *seq)
{
	...
	seq_printf(seq,
		   "average rtt is %dms\npacket trans:%d\nrecorded %lus ago\n",
		   mobile_rtt_average / 8,
		   mobile_rtt_calc_pkg, /* 8 rtt average */
...
static int wifipro_snmp_seq_show(struct seq_file *seq, void *v)
{
	...
	seq_printf(seq, "wlan IPv6 rx packet is %u\n", wlan_v6_rx_pkt);
	seq_printf(seq, "wlan IPv6 tx packet is %u\n", wlan_v6_tx_pkt);
	seq_printf(seq, "wlan IPv6 retrans packet is %u\n", wlan_v6_retrans);
	seq_puts(seq, "\n");
	return 0;
}
...
int wifipro_init_proc(struct net *net)
{
	...
	if (!proc_create("wifipro_log_level",
			 (S_IRUSR | S_IRGRP) | (S_IRUSR | S_IRGRP),
	...
	if (!proc_create_net_single("wifipro_tcp_stat", 0444, net->proc_net,
		...
		goto out_wifipro_tcp_stat;
	if (!proc_create_net_single_write("wifipro_log_level", 0644,
					  net->proc_net, wifipro_log_level_show,
					  wifipro_log_level_write, NULL))
		goto out_wifipro_log_level;
#endif
