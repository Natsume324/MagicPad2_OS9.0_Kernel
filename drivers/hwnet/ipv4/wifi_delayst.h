...
#define GET_AVG_UPLOAD                                  \
	(rcv_delay_s.t_totaldelay[TP_SKB_HMAC_UPLOAD] / \
	 rcv_delay_s.t_totalpkts[TP_SKB_HMAC_UPLOAD])
#define GET_RCV_AVG_IP                         \
	(rcv_delay_s.t_totaldelay[TP_SKB_IP] / \
	 rcv_delay_s.t_totalpkts[TP_SKB_IP])
#define GET_AVG_RECV                             \
	(rcv_delay_s.t_totaldelay[TP_SKB_RECV] / \
	 rcv_delay_s.t_totalpkts[TP_SKB_RECV])

#define GET_SND_AVG_IP \
	(delay_s.t_totaldelay[TP_SKB_IP] / delay_s.t_totalpkts[TP_SKB_IP])
#define GET_AVG_XMIT                              \
	(delay_s.t_totaldelay[TP_SKB_HMAC_XMIT] / \
	 delay_s.t_totalpkts[TP_SKB_HMAC_XMIT])
#define GET_AVG_TX                              \
	(delay_s.t_totaldelay[TP_SKB_HMAC_TX] / \
	 delay_s.t_totalpkts[TP_SKB_HMAC_TX])
#define GET_AVG_DMAC \
	(delay_s.t_totaldelay[TP_SKB_DMAC] / delay_s.t_totalpkts[TP_SKB_DMAC])

#define GET_UPLOAD_ALL                                           \
	get_rcv_upload(0), get_rcv_upload(1), get_rcv_upload(2), \
		get_rcv_upload(3), get_rcv_upload(4)
#define GET_RCV_IP_ALL                                              \
	get_rcv_ip(0), get_rcv_ip(1), get_rcv_ip(2), get_rcv_ip(3), \
		get_rcv_ip(4)
#define GET_RECV_ALL                                                        \
	get_rcv_recv(0), get_rcv_recv(1), get_rcv_recv(2), get_rcv_recv(3), \
		get_rcv_recv(4)
#define GET_SND_IP_ALL                                              \
	get_snd_ip(0), get_snd_ip(1), get_snd_ip(2), get_snd_ip(3), \
		get_snd_ip(4)
#define GET_XMIT_ALL                                                        \
	get_snd_xmit(0), get_snd_xmit(1), get_snd_xmit(2), get_snd_xmit(3), \
		get_snd_xmit(4)
#define GET_TX_ALL                                                  \
	get_snd_tx(0), get_snd_tx(1), get_snd_tx(2), get_snd_tx(3), \
		get_snd_tx(4)
#define GET_DMAC_ALL                                                        \
	get_snd_dmac(0), get_snd_dmac(1), get_snd_dmac(2), get_snd_dmac(3), \
		get_snd_dmac(4)
...
#define tp_skb_store_addr_v4(__entry, skb, sport, dport)           \
	do {                                                       \
		...
		v4->sin_port = sport;                              \
		if (iph)                                           \
			v4->sin_addr.s_addr = iph->saddr;          \
		v4 = (void *)(__entry)->daddr;                     \
		v4->sin_family = AF_INET;                          \
		v4->sin_port = dport;                              \
		if (iph)                                           \
			v4->sin_addr.s_addr = iph->daddr;          \
	} while (0)
...
#define tp_skb_store_addr(__entry, skb, sport, dport)                     \
	do {                                                              \
		if (skb->protocol == htons(ETH_P_IPV6)) {                 \
			...
			v6->sin6_port = sport;                            \
			if (hdr)                                          \
				v6->sin6_addr = hdr->saddr;               \
			v6 = (void *)__entry->daddr;                      \
			v6->sin6_family = AF_INET6;                       \
			v6->sin6_port = dport;                            \
			if (hdr)                                          \
				v6->sin6_addr = hdr->daddr;               \
