...
static uid_t find_skb_uid(struct sk_buff *skb)
{
	...
	if (filp) {
		if (filp->f_cred)
			sock_uid =
				from_kuid(&init_user_ns, filp->f_cred->fsuid);
	} else
		return 0xffffffff;
			      ...
			      const char *data, int dlen, char *ip)
{
	...
	if (!in_irq() && !in_interrupt()) {
		...
		pr_info("hnad:in_irq=%d in_interrupt=%d", (int)in_irq(),
			(int)in_interrupt());
	}
	if (opt != DLST_NOT) {
		if (opt == DLST_ALLOW)
...
*/
	...
	if (match_type == MATCH_TYPE_AD_UID) {
		if (match_ad_url(&g_deltalist_info, uid, data, dlen)) {
			...
			if (get_cur_time() - g_deltalist_info.lastrepot >
			    1000) {
				g_deltalist_info.lastrepot = get_cur_time();
				tempurl = get_url_form_data(data, dlen);

				if (tempurl) {
					tempurl = add_refer_for_data(data, dlen,
								     tempurl);
					buf = get_report_msg(
						NETLINK_REPORT_DLID_TO_USER,
						uid, tempurl, ip);

					if (buf) {
						ret = notify_event(REPORTCMD,
								   g_uspa_pid,
								   buf);
						if (ret < 0)
...
static unsigned int net_hn_hook_localout(void *priv, struct sk_buff *skb,
					 const struct nf_hook_state *state)
{
	...
	if ((unsigned char *)data > skb->data) {
		...
		if (skb_copy_bits(skb, (unsigned char *)data - skb->data,
				  (void *)http_resp, dlen) == 0) {
			data = http_resp;
		}
	}
#endif
...
static int __init nf_init(void)
{
	...
	init_appdl();
	init_ad();
	ret = nf_register_net_hooks(&init_net, hn_net_hooks,
				    ARRAY_SIZE(hn_net_hooks));
...
static void __exit nf_exit(void)
{
	uninit_ad();
	uninit_appdl();
	nf_unregister_net_hooks(&init_net, hn_net_hooks,
				ARRAY_SIZE(hn_net_hooks));
}
