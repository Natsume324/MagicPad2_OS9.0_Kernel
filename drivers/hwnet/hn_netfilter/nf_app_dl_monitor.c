...
char *get_report_msg(unsigned int dlid, unsigned int uid, const char *url,
		     char *ip)
{
	char *urlhex = NULL;
	char *buf = NULL;
	...
	if (snprintf_s(buf, blen + 1, blen, "%u %u %s", dlid, uid, urlhex) <
	    0) {
		pr_info("hnad: error EOK %s %d", __func__, __LINE__);
		kfree(temp);
		kfree(urlhex);
...
void nofity_loop(unsigned int uid, unsigned int dlid, char *url, int action)
{
	...
	list_for_each_entry(node, &g_dl_wait, list) {
		if (uid == node->uid && len == node->len &&
		    0 == memcmp(node->url, url, len) && dlid != node->dlid) {
			node->action = action;
