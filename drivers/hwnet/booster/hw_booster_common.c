...
#define IFACE_NUM 12
static const char mobile_ifaces[IFACE_NUM][IFNAMSIZ]
...
static bool ifname_equal(const char *if1, size_t if1_len, const char *if2,
			 size_t if2_len)
{
...
bool is_ds_rnic(const struct net_device *dev)
{
	...
	for (i = 0; i < IFACE_NUM; ++i) {
		if (ifname_equal(dev->name, sizeof(dev->name), mobile_ifaces[i],
				 sizeof(mobile_ifaces[i])))
			return true;
	}
	return false;
}
...
bool is_wifi_rnic(const struct net_device *dev)
{
	...
	for (i = 0; i < WIFI_IFACE_NUM; ++i) {
		if (ifname_equal(dev->name, sizeof(dev->name), wifi_ifaces[i],
				 sizeof(wifi_ifaces[i])))
			return true;
	}
	return false;
}
