...
static int close_socket(struct sock *sk, u8 protocol)
{
	...
	if (protocol == IPPROTO_TCP) {
		...
		sock_gen_put(sk);
	} else if (protocol == IPPROTO_UDP || protocol == IPPROTO_UDPLITE) {
...
static bool is_addrs_match(struct remote_address *addr,
			   struct remote_address *addr_in_rule)
{
	...
	if (addr->ai_family == AF_INET) {
		...
		if (addr_in_rule->addr_u.ip6_addr.prefix_len == 0) {
			return ipv6_addr_equal(
				&addr->addr_u.ip6_addr.addr,
				&addr_in_rule->addr_u.ip6_addr.addr);
		} else {
			return ipv6_prefix_equal(
				&addr->addr_u.ip6_addr.addr,
				&addr_in_rule->addr_u.ip6_addr.addr,
...
static bool is_addr_in_rule(struct slice_route_rule *rule,
...
static bool is_addr_match_rule(struct slice_route_rule *rule,
			       struct remote_address *addr)
{
	if (rule == NULL || addr == NULL)
		return false;
...
static int add_address_to_rule(struct slice_route_rule *rule,
			       struct remote_address *addr)
{
	int err = -EINVAL;
	if (rule == NULL || addr == NULL)
...
static int add_uid_to_rule(struct slice_route_rule *rule,
			   struct uid_list *curr_uid)
{
	int err = -EINVAL;
...
static int set_uid_to_route_rule(struct slice_route_rule *route_rule,
				 char **pos, int len)
{
	char *position = NULL;
	u32 uid_num;
...
static int set_ipv4_to_route_rule(struct slice_route_rule *route_rule,
				  char **pos, int len)
{
	u32 i;
	u8 ipv4_num;
	...
	for (i = 0; i < ipv4_num; i++) {
		...
		memcpy(&ipv4_addr->addr_u.ip4_addr.addr, position,
		       IPV4_ADDR_LEN);
		skip_byte(position, IPV4_ADDR_LEN);
		memcpy(&ipv4_addr->addr_u.ip4_addr.mask, position,
		       IPV4_ADDR_LEN);
		skip_byte(position, IPV4_ADDR_LEN);
...
static int set_ipv6_to_route_rule(struct slice_route_rule *route_rule,
				  char **pos, int len)
{
	u32 i;
	u8 ipv6_num;
	...
	for (i = 0; i < ipv6_num; i++) {
		...
		memcpy(&ipv6_addr->addr_u.ip6_addr.addr, position,
		       IPV6_ADDR_LEN);
...
static int set_protocol_to_route_rule(struct slice_route_rule *route_rule,
				      char **pos, int len)
{
	char *position = NULL;
	u32 i;
...
static int set_port_to_route_rule(struct slice_route_rule *route_rule,
				  char **pos, int len)
{
	char *position = NULL;
	u8 port_range_num;
...
static void get_new_rule_and_add_to_list(struct slice_route_rule *route_rule,
					 char **pos, int len)
{
	int msg_len = len;
