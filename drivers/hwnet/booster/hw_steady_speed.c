...
{
	return is_client_syn_request(th) && atomic_read(&node->stats_in) > 0 &&
...
static void delete_stats_node(u32 index, struct stats_node *node,
			      bool del_after)
{
				...
				const struct nf_hook_state *state)
{
	...
	if (state->hook == NF_INET_LOCAL_IN) {
		if (!is_ds_rnic(state->in))
			return NF_ACCEPT;
		tcp_rcv_stats(skb, ntohs(iph->tot_len) -
					   iph->ihl * TCP_HDR_IHL_QUAD);
	}
	if (state->hook == NF_INET_LOCAL_OUT) {
		if (!is_ds_rnic(state->out))
			return NF_ACCEPT;
		tcp_send_stats(skb, ntohs(iph->tot_len) -
					    iph->ihl * TCP_HDR_IHL_QUAD);
	}
	return NF_ACCEPT;
}
	...
	for (i = 0; i < MAX_SOCK_HASH_SIZE; ++i) {
		...
		for (node = table[i].chain; node; node = node->next)
			if (time_after(jiffies,
				       node->stamp + SOCK_LISTEN_DURATION))
				break;
		if (node != NULL)
