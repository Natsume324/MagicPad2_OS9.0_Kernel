...
static inline void __set_stack_to_buf(struct slub_ring_buf *srb,
				      struct stack_track *slub_stack)
{
	...
	if (unlikely(idx >= srb->size))
		idx = 0;
	err = memmove_s(srb->stack_buf + idx, sizeof(struct stack_track),
			slub_stack, sizeof(struct stack_track));
	if (err == EOK)
		srb->in++;
}

static inline int __get_stack_from_buf(struct slub_ring_buf *srb,
				       struct stack_track *slub_stack)
{
	errno_t err = EOK;
	...
	if (unlikely(idx >= srb->size))
		idx = 0;
	err = memmove_s(slub_stack, sizeof(struct stack_track),
			srb->stack_buf + idx, sizeof(struct stack_track));
	if (err == EOK)
		srb->out++;
...
inline void get_slub_stacks(struct stack_track *slub_stack,
			    unsigned long caller)
{
	...
	slub_stack->stack_len = stack_trace_save(
		slub_stack->addrs, MM_TRACK_STACK_DEPTH, SLUB_STACK_SAVE_SKIP);
	slub_stack->stack_hash = jhash2(
		(u32 *)slub_stack->addrs,
		slub_stack->stack_len * sizeof(unsigned long) / sizeof(u32),
		0xface);
#else
...
static void slub_add_node(struct stack_track *slub_stack, int type)
{
	...
	while (*p) {
		...
		if (slub_stack->stack_hash < entry->slub_track.stack_hash) {
			p = &(*p)->rb_left;
		} else if (slub_stack->stack_hash >
			   entry->slub_track.stack_hash) {
			p = &(*p)->rb_right;
		} else {
	...
	if (rbnode) {
		err = memmove_s(&rbnode->slub_track, sizeof(struct stack_track),
				slub_stack, sizeof(struct stack_track));
		if (err == EOK) {
			atomic_set(&rbnode->ref, 1);
			rb_link_node(&rbnode->node, parent, p);
			rb_insert_color(&rbnode->node,
					&slub_tracks[type].slubrb);
		}
	}
}
...
static size_t slub_read_node(struct mm_stack_info *buf, const size_t len,
			     int type)
{
	...
	while (n && (length--)) {
		...
		err = memmove_s(&(buf + cnt)->stack_entry,
				sizeof(struct stack_track), &vnode->slub_track,
				sizeof(struct stack_track));
		if (err == EOK)
			atomic_set(&((buf + cnt)->ref),
...
static int kslubtrackd(void *p)
{
	...
	while (!kthread_should_stop()) {
		prepare_to_wait(&slub_tracks[type].slub_wait, &wait,
...
static long slub_create_track(int type)
{
	...
	slub_tracks[type].slub_ktrackd =
		kthread_run(kslubtrackd, &slub_tracks[type], "slub-%s",
...
static inline int enable_slab_alloc_node_hook(void)
{
	...
	ret = register_trace_android_vh_slab_alloc_node(slab_alloc_node_hook,
							NULL);
	if (ret)
...
static inline int disable_slab_alloc_node_hook(void)
{
	...
	ret = unregister_trace_android_vh_slab_alloc_node(slab_alloc_node_hook,
							  NULL);
	if (ret)
...
int slub_stack_track_on(const char *name)
{
	...
	list_for_each_entry(cachep, &slab_caches, list) {
		...
		if (strncmp(cachep->name, name, strlen(cachep->name) + 1) ==
		    0) {
...
static int __init slub_track_init(void)
{
	slub_track_cache = kmem_cache_create("slub_track_cache",
					     sizeof(struct slub_rb_node), 0,
					     SLAB_MM_NOTRACK, NULL);
	if (!slub_track_cache) {
