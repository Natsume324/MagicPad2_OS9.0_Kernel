...
int mm_track_on(int type, const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(mm_track_table); i++) {
		if (type == mm_track_table[i].type &&
		    mm_track_table[i].stack_track) {
			return mm_track_table[i].stack_track->mm_stack_track_on(
				name);
		}
	}
	return -EINVAL;
...
int mm_track_off(int type, const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(mm_track_table); i++) {
		if (type == mm_track_table[i].type &&
		    mm_track_table[i].stack_track) {
			return mm_track_table[i].stack_track->mm_stack_track_off(
				name);
		}
	}
...
size_t mm_track_read(int type, struct mm_stack_info *info, size_t len,
		     int subtype)
{
	unsigned int i;
	...
	for (i = 0; i < ARRAY_SIZE(mm_track_table); i++) {
		if (type == mm_track_table[i].type &&
		    mm_track_table[i].stack_track) {
			return mm_track_table[i].stack_track->mm_stack_read(
				info, len, subtype);
		}
	}
