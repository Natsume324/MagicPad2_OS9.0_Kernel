...
static void test_slub_hook(struct kunit *test)
{
	...
	list_for_each_entry(cachep, &slab_caches, list) {
		if (strncmp(cachep->name, slub_test_cache_name,
...
static inline void kuint_expect_check_stack_list(struct kunit *test,
						 struct mm_stack_info *list,
						 int stack_counts,
						 unsigned long return_addr)
{
	...
	for (i = 0; i < stack_counts; i++) {
		KUNIT_EXPECT_EQ(test, SLUB_TEST_CACHE_OBJ_UNIQUE_CALL,
				list[i].ref.counter);
...
static void test_slub_alloc_track(struct kunit *test)
{
	...
	for (i = 0; i < SLUB_TEST_CACHE_OBJ_COUNT; i++)
		slub_cache_test_array[i] =
			kmem_cache_alloc(slub_test_cache, GFP_ATOMIC);

	mm_track_off(SLUB_TRACK, slub_test_cache_name);

	list = vzalloc(STACK_ENTRY_MAX_SIZE * sizeof(*list));
	stack_entry_size = mm_track_read(SLUB_TRACK, list, STACK_ENTRY_MAX_SIZE,
					 SLUB_ALLOC);

	KUNIT_EXPECT_EQ(test, SLUB_TEST_CACHE_OBJ_COUNT, stack_entry_size);
	kuint_expect_check_stack_list(test, list, stack_entry_size,
				      return_addr);
...
{
	slub_test_cache = kmem_cache_create(slub_test_cache_name,
