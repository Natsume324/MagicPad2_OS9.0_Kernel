...
{
	ras_retn_if((fault_temp->type == FAULT_NA && fault_temp->cmd == -1),
		    -EINVAL);
	ras_retn_if((fault_temp->type != FAULT_NA && fault_temp->cmd != -1),
		    -EINVAL);
	if (fault_temp->cmd != -1) {
...
static int fill_fault(struct fault *fault_temp)
{
	...
	if (fault_pos != NULL) {
		ret = memcpy_s(fault_pos, sizeof(struct fault), fault_temp,
			       sizeof(struct fault));
		if (ret != EOK) {
			printk(KERN_ERR "memcpy fault_pos failed\n");
			return -1;
		}
		return 0;
	}
...
static int rasprobe_handler(mmc_sd_get_cid)(struct rasprobe_instance *ri,
					    struct pt_regs *regs)
{
...
#ifdef CONFIG_MMC_DW
static int rasprobe_handler(dw_mci_get_ro)(struct rasprobe_instance *ri,
					   struct pt_regs *regs)
#else
static int rasprobe_handler(sdhci_check_ro)(struct rasprobe_instance *ri,
					    struct pt_regs *regs)
#endif
{
	...
	for (i = 0; i < ARRAY_SIZE(fault_list.faults); i++) {
		...
		if (fault_temp->cmd == cmd->opcode) {
			...
			buf_bit_inject(cmd->resp, 8 * 4 * sizeof(u32),
				       fault_temp->offset_bit, fault_temp->bit);
		}
	}
	cmd_record(cmd);
	return 0;
...
static int rasprobe_handler(mmc_app_sd_status)(struct rasprobe_instance *ri,
					       struct pt_regs *regs)
{
	...
	logd("sd_scr: version:%d,width:%d,spec:%d,scr_cmd:%d\n", scr->sda_vsn,
	     scr->bus_widths, scr->sda_spec3, scr->cmds);
...
static ssize_t proc_ops_write(rSd)(struct file *filp, const char __user *bff,
