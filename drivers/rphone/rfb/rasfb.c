...
static int rasio_args_set(struct fault_impl *fault_args, char *prm)
{
	...
	rasbase_set(fault_args, step, prm) return 0;
}
	...
	if (fault_args->type != FAULT_FRAME_FAILED) {
		...
		if (fault_args->input_type != FAULT_FRAME_DELAY) {
			fault_args->delay =
				...
				((fault_args->lost * RFB_FRAME_RATE_USEC) /
				 1000);
			fault_args->udelay =
...
static struct fault_impl *args2fault(int args, char *argv[])
{
	...
	errno_t ret;
	ret = memset_s(&fault_args, sizeof(fault_args), 0, sizeof(fault_args));
	if (ret != EOK) {
		return NULL;
	}
	...
	if (fault_cur) {
		ret = memcpy_s(fault_cur, sizeof(struct fault_impl),
		       ...
		       unsigned long arg)
{
	...
	if (fault) {
		if (fault->input_type == FAULT_FRAME_LOST_ASC) {
			delay = fault->frame * RFB_FRAME_RATE_MSEC +
			...
			delay = (fault->lost + 1 - fault->frame) *
					RFB_FRAME_RATE_MSEC +
				((fault->lost + 1 - fault->frame) *
				 RFB_FRAME_RATE_USEC) /
					1000;
			udelay = ((fault->lost + 1 - fault->frame) *
				  RFB_FRAME_RATE_USEC) %
				 1000;
		} else {
			delay = fault->delay;
	...
	for (i = 0; i < ARRAY_SIZE(fault_injected.impl); i++) {
		...
		if (impl->input_type == FAULT_FRAME_DELAY) {
			ret = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
					 "%d %s  %d  match(%d)\n", i,
					 type2name(impl->input_type),
					 impl->delay, impl->match);
			if (ret < 0) {
...
static ssize_t proc_ops_write(rfb)(struct file *filp, const char __user *bff,
...
static int tool_init(void)
{
	...
	ras_retn_iferr(
		proc_init(MODULE_NAME, &proc_ops_name(rfb), &fault_injected));
	return 0;
}
