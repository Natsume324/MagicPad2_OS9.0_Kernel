...
static const struct fault_ops fault_ops_list[] = {
	...
	{
		.name = "all_invers",
		.type = FAULT_ALL_INVERS,
	},
	{
		.name = "gap_invers",
		.type = FAULT_GAP_INVERS,
	},
};
...
static int check_conflict_record_offset(struct fault_impl *fault)
{
	...
	if (!area_node)
		return -EINVAL;
	ret = memcpy_s(&(area_node->fault), sizeof(area_node->fault), fault,
...
static int args2fault(int args, char *argv[])
{
	...
	errno_t ret;
	ret = memset_s(&fault_args, sizeof(struct fault_impl), 0,
		       sizeof(struct fault_impl));
	if (ret != EOK) {
		return -EINVAL;
	}
			       ...
			       unsigned int length)
{
	...
	if (fault->type == FAULT_ALL_0) {
		ret = memset_s(buf + fault->offset, fault->size, 0x0,
			       fault->size);
		if (ret != EOK) {
			return -EINVAL;
		}
	} else if (fault->type == FAULT_ALL_1) {
		ret = memset_s(buf + fault->offset, fault->size, 0xff,
			       fault->size);
		if (ret != EOK) {
			return -EINVAL;
		}
				 ...
				 enum fault_area area_type)
{
	...
	errno_t err;
	if (bdev) {
		err = snprintf_s(disk_name, sizeof(disk_name),
				 sizeof(disk_name) - 1, "%pg", bdev);
		if (err < 0) {
			ras_fail("snprintf_s disk_name failed\n");
		}
	}
...
static int rasf2fs_get_area_index(struct f2fs_sb_info *sbi, block_t disk_index,
				  enum fault_area *area_type,
				  block_t *relative_index, unsigned int *size)
{
	...
	__le32 cp_blocks = super->segment_count_ckpt
			   << super->log_blocks_per_seg;
	__le32 nat_blocks = super->segment_count_nat
			    << super->log_blocks_per_seg;
	__le32 sit_blocks = super->segment_count_sit
			    << super->log_blocks_per_seg;
	__le32 ssa_blocks = super->segment_count_ssa
			    << super->log_blocks_per_seg;
	__le32 offset = 0;
	...
	if (disk_index >= cp_blkaddr &&
		...
		*size = sizeof(struct f2fs_nat_block);
		...
		offset = (disk_index < sit_blkaddr + (sit_blocks >> 1)) ?
				 0 :
				 (sit_blocks) >> 1;
						...
						struct pt_regs *regs)
{
	...
	if (rasf2fs_get_area_index(sbi, disk_index, &area_type, &relative_index,
				   &size))
		return 0;
...
static int proc_ops_show(rF2fs)(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < FAULT_AREA_MAX; i++) {
		...
		if (list_empty(area_list_head))
			continue;
		list_for_each_safe(pos, next, area_list_head) {
			area_node = list_entry(pos, struct fault_area_node,
					       st_list);
			if (i > FAULT_AREA_SB1)
				seq_printf(
					m,
...
static ssize_t proc_ops_write(rF2fs)(struct file *filp, const char __user *bff,
				     size_t count, loff_t *data)
{
	...
	if (unlikely(count >= sizeof(buf_cmd)))
		return -ENOMEM;
	ret = memset_s(buf_cmd, sizeof(buf_cmd), 0, sizeof(buf_cmd));
	if (ret != EOK) {
		return -ENOMEM;
	}
...
static int tool_init(void)
{
	...
	if (ret != EOK) {
		printk(KERN_ERR "memset fault_injected failed\n");
	}
	...
	ras_retn_iferr(
		proc_init(MODULE_NAME, &proc_ops_name(rF2fs), &fault_injected));
	return 0;
}
...
static void tool_exit(void)
{
	...
	for (i = 0; i < FAULT_AREA_MAX; i++) {
		...
		if (list_empty(area_list_head))
			continue;
		list_for_each_safe(pos, next, area_list_head) {
			st_offset = list_entry(pos, struct fault_area_node,
					       st_list);
			list_del_init(pos);
			kfree(st_offset);
		}
