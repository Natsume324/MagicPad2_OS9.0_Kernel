...
static int fail(const char *type, int temp)
{
	...
	if (j == -1)
		return -1;
	ras_retn_iferr(
		ras_malloc((void **)&flt.ft[j].thermal_type, strlen(type) + 1));
	ret = strncpy_s(flt.ft[j].thermal_type, strlen(type) + 1, type,
			strlen(type));
	if (ret != EOK) {
		return -1;
	}
...
static void restore(struct Fault *ft)
{
	...
	ret = memset_s(ft, sizeof(struct Fault), 0, sizeof(struct Fault));
	if (ret != EOK) {
		printk(KERN_ERR "memset_s ft failed\n");
	}
}
...
static int
rasprobe_handler(honor_peripheral_ntc_2_temp)(struct rasprobe_instance *ri,
					      struct pt_regs *regs)
{
	int temp;
...
static int
rasprobe_handler(honor_battery_temperature)(struct rasprobe_instance *ri,
					    struct pt_regs *regs)
{
...
static int
rasprobe_handler(power_supply_read_temp)(struct rasprobe_instance *ri,
					 struct pt_regs *regs)
{
	int temp;
	struct RasRegs *rd = (struct RasRegs *)ri->data;
...
static int tool_init(void)
{
	...
	ret = memset_s(&flt, sizeof(flt), 0, sizeof(flt));
	if (ret != EOK) {
		printk(KERN_ERR "memset_s flt failed\n");
	}
	rwlock_init(&flt.rwk);
