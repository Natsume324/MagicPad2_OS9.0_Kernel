...
static int fault_restore(struct fault_impl *fault)
{
	...
	err = memset_s(fault, sizeof(struct fault_impl), 0,
		       sizeof(struct fault_impl));
	if (err != EOK) {
		return -1;
	}
	return 0;
...
static struct fault_impl *args2fault(int args, char *argv[])
{
	...
	errno_t err;
	err = memset_s(&fault_args, sizeof(fault_args), 0, sizeof(fault_args));
	if (err != EOK) {
		return NULL;
	}
	...
	if (fault_cur) {
		err = memcpy_s(fault_cur, sizeof(struct fault_impl),
			       &fault_args, sizeof(struct fault_impl));
		if (err != EOK) {
			return NULL;
		}
	}
...
static struct fault_impl *
rasio_check_fault_match(struct bio *bio, enum fault_type mode, bool isRw)
{
	int random_val = 0;
...
static int is_lost(struct bio *bio, struct pt_regs *regs)
{
	...
	if (!fault) {
		fault = rasio_check_fault_match(bio, FAULT_W_LOST,
						(bio_rw == WRITE));
	}
	if (fault) {
...
void rasio_fail_work(struct work_struct *work)
{
	struct rasio_work *t_work =
		container_of(work, struct rasio_work, io_work);
...
static int is_fail(struct bio *bio, struct pt_regs *regs)
{
	...
	if (!fault) {
		fault = rasio_check_fault_match(bio, FAULT_W_FAILED,
						(bio_rw == WRITE));
	}
	if (!fault)
		return 0;
...
static int
rasprobe_handler(generic_make_request_checks)(struct rasprobe_instance *ri,
					      struct pt_regs *regs)
{
	/*io lost*/
...
#endif
{
	...
	fault = rasio_check_fault_match(bio, FAULT_R_DELAY,
					(bio_data_dir(bio) == READ));
	if (!fault) {
		fault = rasio_check_fault_match(bio, FAULT_W_DELAY,
...
#endif
			if (fault_injected.arr_queue_disk[i].gendisk == bdisk) {
				request_fn = fault_injected.arr_queue_disk[i]
						     .make_request_fn;
				find = 1;
				break;
			}
...
static int proc_ops_show(rio)(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < ARRAY_SIZE(fault_injected.impl); i++) {
		...
		if (impl->type < FAULT_R_DELAY || impl->type > FAULT_W_DELAY) {
			err = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
					 "%d %s %s %s %d,    match(%ld)\n", i,
					 type2name(impl->type), impl->dir,
					 impl->disk, impl->pid, impl->match);
			if (err < 0) {
				printk(KERN_ERR "snprintf buf failed\n");
				return -1;
			}
		} else {
			err = snprintf_s(
...
static ssize_t proc_ops_write(rio)(struct file *filp, const char __user *bff,
				   size_t count, loff_t *data)
{
	...
	if (unlikely(count >= sizeof(buf_cmd)))
		return -ENOMEM;
	err = memset_s(buf_cmd, sizeof(buf_cmd), 0, sizeof(buf_cmd));
	if (err != EOK) {
		return -ENOMEM;
	}
...
static int tool_init(void)
{
	...
	rasio_work_cache =
		kmem_cache_create("rasio_fit_wk", sizeof(struct rasio_work), 0,
	...
	ret = memset_s(&fault_injected, sizeof(struct fault_list), 0,
		       sizeof(struct fault_list));
	if (ret != EOK) {
		printk(KERN_ERR "memset_s fault_injected failed\n");
	}
	atomic_set(&(fault_injected.use_res), 0);
	rwlock_init(&fault_injected.rwk);
	ras_retn_iferr(
		proc_init(MODULE_NAME, &proc_ops_name(rio), &fault_injected));
	return 0;
}
