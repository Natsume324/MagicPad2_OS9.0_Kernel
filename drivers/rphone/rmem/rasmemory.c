...
#endif
#endif
static inline void rasmemele_addmm(struct mm_struct *mm, struct RasMemEle *ele,
				   long block)
{
...
static inline void rasmemele_decmm(struct mm_struct *mm, struct RasMemEle *ele,
				   long block)
{
...
static int rasmemele_set(struct RasMemEle *ele, struct RasMemArgs *args)
{
	...
	errno_t ret;
	if (0 == ele->args.total) {
		ret = memcpy_s(&ele->args, sizeof(struct RasMemArgs), args,
			       sizeof(struct RasMemArgs));
		if (ret != EOK) {
			return -EEXIST;
		}
		if (idx == 0)
...
static int rasmemele_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ele->len > len &&
		    0 == rasmemele_touch(ele, ele->len - len, rasmemele_addmm))
			ele->tch += (ele->len - len);
...
static int rasmemele_start(struct RasMemEle *ele)
{
	...
	errno_t err;
	ele->dtor = 1;
	err = snprintf_s(name, sizeof(name), sizeof(name) - 1, "rasmem%lu",
			 ele->idx);
	if (err < 0) {
		printk(KERN_ERR "snprintf name failed\n");
		return -1;
	}
...
static int rasmem_main(void *mem, int argc, char *args[])
{
	...
	int i;
	errno_t err;
	err = memset_s(&rma, sizeof(rma), 0, sizeof(rma));
	if (err != EOK) {
		printk(KERN_ERR "memset rma failed\n");
	}
	for (i = 0; i < argc; i++)
...
static int rasmem_ctor(struct RasMem *mem)
{
	...
	int i, len = ARRAY_SIZE(mem->eles);
	errno_t err;
	err = memset_s(mem, sizeof(struct RasMem), 0, sizeof(struct RasMem));
	if (err != EOK) {
		printk(KERN_ERR "memset mem failed\n");
		return -1;
	}
	...
	for (i = 0; i < len; i++) {
		...
		ele->next = ele;
		ele->tail = &ele->head;
		err = memset_s(&ele->head, sizeof(struct RasMemory), 0,
			       sizeof(struct RasMemory));
		if (err != EOK) {
			printk(KERN_ERR "memset head failed\n");
		}
	}
	...
	for (i = 0; i < len; i++) {
		...
		if (0 != ele->args.total) {
			err = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
					 rasfmt, ele->idx, ele->len,
					 ele->args.total, ele->args.block,
...
static ssize_t proc_ops_write(rmem)(struct file *filp, const char __user *bff,
				    size_t count, loff_t *data)
{
	...
	ras_retn_iferr(ras_args(buf_cmd, count, rasmem_main,
