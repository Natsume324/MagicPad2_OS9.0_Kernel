...
static int rasmap_eachrme(struct RasMap *rm,
			  int (*each)(void *data, void *key, void *val),
			  void *data)
{
...
static inline int rasvector_grow(struct RasVector *vec, struct RasBff *trb,
				 int length)
{
	while (trb->length + length >= sizeof(trb->bff)) {
		...
		if (trb->nxt == vec->bff) {
			...
			ras_retn_iferr(ras_malloc((void **)&nrb,
						  sizeof(struct RasBff)));
			vec->page++;
			nrb->nxt = trb->nxt;
			nrb->nxt->prv = nrb;
			nrb->prv = trb;
...
static inline int rasvector_copy(struct RasVector *vec, struct RasBff *rb,
				 const char *str)
{
...
int rasproc_create(struct RasProc **self, const char *name, read_proc_t *pread,
...
static inline int ras_invalidparam(const char *a, unsigned long pos)
{
	char bff[256];
	errno_t ret;
	ret = snprintf_s(bff, sizeof(bff), sizeof(bff) - 1,
			 "%s[%lu] is a invalid parameter", a, pos);
	if (ret < 0) {
		return -EINVAL;
	}
	ras_fail(bff);
