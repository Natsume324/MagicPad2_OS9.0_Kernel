...
#define proc_ops_init(name)                                                  \
	{                                                                    \
		.proc_open = proc_ops_open(name), .proc_read = seq_read,     \
		.proc_write = proc_ops_write(name), .proc_lseek = seq_lseek, \
...
#define proc_ops_init(name)                                         \
	{                                                           \
		.open = proc_ops_open(name), .read = seq_read,      \
		.write = proc_ops_write(name), .llseek = seq_lseek, \
