...
int rasproc_create(struct RasProc **, const char *name, read_proc_t *pread,
		   write_proc_t *pwrite, void *data);
#endif
int rasproc_read(struct RasProc *, void (*update)(struct RasProc *), char *buf,
		 char **bl, off_t offset, int len, int *eof);
...
#define ras_retn_if(con, ret) \
	if (con)              \
	ras_retn(ret)

#define ras_retn_iferr(ret)                \
	do {                               \
		int ____ret = (ret);       \
		if (____ret) {             \
			ras_retn(____ret); \
...
#define ras_retn_iferr_then(ret, op)       \
	do {                               \
		int ____ret = (ret);       \
		if (____ret) {             \
			(op);              \
...
#define ras_setllu_retn(rase, valu, flag, fld, arg)                      \
	do {                                                             \
		int len = strlen(#fld "=");                              \
		if (0 == strncmp(#fld "=", arg, len)) {                  \
			...
			ras_retn_iferr(                                  \
				ras_atoll(&val, cval, strlen(cval), 0)); \
			rase->flag.fld = 1;                              \
			rase->valu.fld = val;                            \
			return 0;                                        \
		}                                                        \
...
#define ras_setstr_retn(rase, valu, flag, fld, arg)                           \
	do {                                                                  \
		int len = strlen(#fld "=");                                   \
		if (0 == strncmp(#fld "=", arg, len)) {                       \
			char *cval = arg + len;                               \
			ras_retn_if(0 == *cval, -EINVAL);                     \
			rase->flag.fld[0] = 1;                                \
			if (strncpy_s(rase->valu.fld, sizeof(rase->valu.fld), \
				      cval, sizeof(rase->valu.fld)) != EOK) { \
				printk(KERN_ERR "strncpy_s fld failed)\n")    \
			}                                                     \
			return 0;                                             \
		}                                                             \
	} while (0)
...
#define rasbase_set(ins, fld, arg)                                       \
	do {                                                             \
...
#define rasbase_set_func(ins, fld, arg, func)             \
	do {                                              \
		int len = strlen(#fld "=");               \
		if (0 == strncmp(#fld "=", arg, len)) {   \
			char *cval = arg + len;           \
...
#define rasbase_cset(ins, fld, arg)                                          \
	do {                                                                 \
		int len = strlen(#fld "=");                                  \
		if (0 == strncmp(#fld "=", arg, len)) {                      \
			char *cval = arg + len;                              \
			ras_retn_if(0 == *cval, -EINVAL);                    \
			if (snprintf_s(ins->fld, sizeof(ins->fld),           \
				       sizeof(ins->fld) - 1, cval) < 0) {    \
				printk(KERN_ERR "snprintf_s fld failed)\n"); \
			}                                                    \
			return 0;                                            \
		}                                                            \
	} while (0);
