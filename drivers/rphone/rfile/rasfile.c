...
#include "fs/mount.h"
#define MNT_PARENT(mnt) \
	(real_mount(mnt)->mnt_parent ? (&real_mount(mnt)->mnt_parent->mnt) : 0)
			      ...
			      struct dentry *den, int ct)
{
	...
	case CHFILETYPE_UNMOUNT:
		ras_retn_if(ras_malloc((void **)&ci->path, strlen(name) + 1),
			    -ENOMEM);
		err = strncpy_s(ci->path, strlen(name) + 1, name, strlen(name));
		if (err != EOK) {
			return -ENOMEM;
		}
		break;
...
static int chfil_get(char *name, struct file **ppflp, struct dentry **ppden)
{
	...
	if (S_ISBLK(den->d_inode->i_mode)) {
		...
		bdev = (struct block_device *)kmalloc(
			sizeof(struct block_device), GFP_ATOMIC);
...
static struct {
	...
	{.len = 8,
	 ...
	 .ret = EDEADLK,
	 .val = 0,
	 .ct = CHFILETYPE_DEADLOCK},
	{.len = 6,
	 .op = "unread",
	 .ret = EACCES,
	 .val = 0,
	 .ct = CHFILETYPE_UNREAD},
	{.len = 7,
	 .op = "unwrite",
	 .ret = EACCES,
	 .val = 0,
	 .ct = CHFILETYPE_UNWRITE},
	...
	{.len = 4,
	 ...
	 .ret = EACCES,
	 .val = 0,
	 .ct = CHFILETYPE_UNRENAME},
	{.len = 4, .op = "humt", .ret = 0, .val = 0, .ct = CHFILETYPE_HUNMOUNT},
	{.len = 7,
	 .op = "humtall",
	 .ret = 0,
	 .val = 0,
	 .ct = CHFILETYPE_HUNMOUNTALL},
	{.len = 7,
	 .op = "umtbusy",
	 .ret = 0,
	 .val = 0,
	 .ct = CHFILETYPE_UNMOUNTBUSY},
	{.len = 9,
	 .op = "deplenish",
	 .ret = EMFILE,
	 .val = 0,
	 .ct = CHFILETYPE_DEPLENISH},
	{.len = 8,
	 .op = "readonly",
	 .ret = EROFS,
	 .val = 0,
	 .ct = CHFILETYPE_READONLY},
	{.len = 6,
	 .op = "unopen",
	 .ret = EACCES,
	 .val = 0,
	 .ct = CHFILETYPE_UNOPEN},
	{.len = 6,
	 .op = "unexec",
	 .ret = EPERM,
	 .val = 0,
	 .ct = CHFILETYPE_UNEXEC},
	{.len = 7,
	 .op = "unmount",
	 .ret = ENOTBLK,
	 .val = 0,
	 .ct = CHFILETYPE_UNMOUNT},
	{.len = 8,
	 .op = "unumount",
	 .ret = EPERM,
	 .val = 0,
	 .ct = CHFILETYPE_UNUMOUNT},
	{.len = 6,
	 .op = "missed",
	 .ret = ENOENT,
	 .val = 0,
	 .ct = CHFILETYPE_MISSED},
};
...
void file_ops_tracer(struct task_struct *who, struct dentry *den,
		     struct vfsmount *mnt, const char *what, enum ChfilType why)
{
	struct path f_path;
	char path_buff[1024];
	char *path;
	...
	logger_unblocked(who->pid, who->comm, path, what, type2name(why));
#else /*just printk*/
	pr_err("file_ops_tracer:%s, %s, %s\n", path, what, type2name(why));
#endif
}
...
static unsigned long long get_block_free_size(struct super_block *sb)
{
	...
	if (sb == 0)
		return 0;
	err = memset_s(&fs_state, sizeof(struct kstatfs), 0,
...
static int rasprobe_handler(security_inode_unlink)(struct rasprobe_instance *ri,
						   struct pt_regs *regs)
{
...
static int rasprobe_handler(security_inode_rmdir)(struct rasprobe_instance *ri,
						  struct pt_regs *regs)
{
...
static int rasprobe_handler(security_inode_create)(struct rasprobe_instance *ri,
						   struct pt_regs *regs)
{
	if (0 == regs_return_value(regs)) {
		int ret;
...
static int rasprobe_handler(security_inode_mkdir)(struct rasprobe_instance *ri,
						  struct pt_regs *regs)
{
...
{
	if (impl->ct == CHFILETYPE_UNRW || impl->ct == CHFILETYPE_UNWRITE ||
	    impl->ct == CHFILETYPE_DEADLOCK || impl->ct == CHFILETYPE_FULL ||
	    impl->ct == CHFILETYPE_READONLY) {
...
{
	if (impl->ct == CHFILETYPE_UNRW || impl->ct == CHFILETYPE_UNREAD ||
...
static long long chfil_rebuild(void *data, const struct ChfilImpl *impl)
{
	...
	if (impl->ct == CHFILETYPE_ZERO || impl->ct == CHFILETYPE_MISSED ||
		...
		err = memset_s((void *)impl, sizeof(struct ChfilImpl), 0x00,
			       sizeof(struct ChfilImpl));
		if (err != EOK) {
			return -1;
		}
		return 1;
					    ...
					    struct pt_regs *regs)
{
	...
	if (WRITE == (long)rd->args[0]) {
		...
		unsigned long long size = (ULLMAX - size_add < size_sub) ?
						  ULLMAX :
						  size_add + size_sub;
...
static int rasprobe_handler(security_inode_rename)(struct rasprobe_instance *ri,
						   struct pt_regs *regs)
{
	if (0 == regs_return_value(regs)) {
		int ret;
...
{
	return (CHFILETYPE_UNMOUNT == impl->ct && NULL != impl->path &&
		strcmp((char *)data, impl->path) == 0) ?
		       chfilimpl_geturn(impl) :
		       0;
}
...
static int fail_deplenish_ck(void *data, const struct ChfilImpl *impl)
{
...
#endif
{
	...
	if (IS_ERR(fp))
		return 0;
	if (should_fail((checker)fail_deplenish_ck, fail_deplenish,
			NULL)) { /*file handler deplenished */
...
static ssize_t proc_ops_write(rfile)(struct file *filp, const char __user *bff,
				     size_t count, loff_t *data)
{
	...
	ras_retn_iferr(
		ras_args(buf_cmd, count, cmd_main, pde_data(FILE_NODE(filp))));
	return count;
}
...
static int rfile_init(void)
{
	...
	ret = memset_s(&chfil, sizeof(chfil), 0, sizeof(chfil));
	if (ret != EOK) {
		printk(KERN_ERR "memset_s chfil failed\n");
	}
