...
#define rasbase_setstr(ins, fld, arg)                                       \
	do {                                                                \
		int len = strlen(#fld "=");                                 \
		errno_t ret;                                                \
		if (0 == strncmp(#fld "=", arg, len)) {                     \
			...
			ret = strncpy_s(ins->fld, 256, cval, strlen(cval)); \
			if (ret != EOK) {                                   \
				return -1;                                  \
			}                                                   \
			return 0;                                           \
...
static int should_fail_check(struct fault_args *pfa, struct scsi_cmnd *scmd)
{
	...
	if (pfa->gdisk != scsi_cmd_to_rq(scmd)->q->disk &&
	    scsi_cmd_to_rq(scmd)->q->disk) {
		tool_err("Gendisk different:%p,%p\n", pfa->gdisk,
			 scsi_cmd_to_rq(scmd)->q->disk);
		return 0;
	}
	if (!scsi_cmd_to_rq(scmd)->q->disk)
...
int add_entry_scsi(struct scsi_cmnd *scmd)
{
	...
	if (NULL == find_entry_scsi(scmd)) {
		ras_retn_iferr(
			ras_malloc((void **)&entry, sizeof(struct entry_scmd)));
		entry->done_src = scsi_done;
		entry->scmd = scmd;
...
static int should_fail_attr(struct fault_attribute *pattr)
{
	...
	if (pattr->percent)
		return (get_random_u32() % 100 >= pattr->percent) ? 0 : 1;
	if (pattr->times > 0) {
		pattr->times--;
		return 1;
	}
...
int fill_fault(struct fault_args *pa)
{
	...
	if (j == -1) /*max the faults */
		return -1;
	fa = &ft->fts[j];
	ret = memcpy_s(fa, sizeof(struct fault_args), pa,
		       sizeof(struct fault_args));
	if (ret != EOK) {
		return -1;
	}
...
int cmd_main(void *data, int argc, char *args[])
{
	...
	if (argc < 1)
		return -1;
	ret = memset_s(&ft_args, sizeof(ft_args), 0, sizeof(ft_args));
	if (ret != EOK) {
		return -1;
	}
...
static ssize_t proc_ops_write(tool)(struct file *filp, const char __user *bff,
...
static int scsi_init(void)
{
	...
	delay_cache = kmem_cache_create("scsi_delay_wq",
					sizeof(struct delay_work), 0,
