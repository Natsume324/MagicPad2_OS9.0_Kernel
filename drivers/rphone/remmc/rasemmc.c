...
static const struct fault_ops fault_ops_list[] = {
	{
		.name = "r_failed",
		.type = FAULT_READ_FAILED,
	},
	{
		.name = "w_failed",
		.type = FAULT_WRITE_FAILED,
	},
	{
		.name = "r_delay",
		.type = FAULT_READ_DELAY,
	},
	{
		.name = "w_delay",
		.type = FAULT_WRITE_DELAY,
	},
	{
		.name = "r_delay_noblk",
		.type = FAULT_READ_DELAY_NONBLOCK,
	},
	{
		.name = "w_delay_noblk",
...
void replace_done(struct mmc_request *mrq)
{
	...
	unsigned long delay = data->flags & MMC_DATA_READ ? read_delay_noblk :
							    write_delay_noblk;
...
int fail_req_err(struct mmc_request *mrq)
{
	...
	if (fault) {
		if ((read && fault->type == FAULT_READ_FAILED) ||
			...
			data->error = data_errors[get_random_u32() %
						  ARRAY_SIZE(data_errors)];
			data->bytes_xfered =
				(get_random_u32() % (data->bytes_xfered >> 9))
				<< 9;
			return 1;
		}
	}
...
static int
rasprobe_handler_entry(mmc_request_done)(struct rasprobe_instance *ri,
					 struct pt_regs *regs)
{
...
static int
rasprobe_handler_entry(cmdq_finish_data)(struct rasprobe_instance *ri,
					 struct pt_regs *regs)
{
	struct RasRegs *rr = NULL;
...
static ssize_t proc_ops_write(rEmmc)(struct file *filp, const char __user *bff,
				     size_t count, loff_t *data)
{
	...
	if (err != EOK) {
		printk(KERN_ERR "memset buf_cmd failed\n");
	}
...
static int tool_init(void)
{
	...
	err = memset_s(&fault_injected, sizeof(struct fault_list), 0,
...
#if (defined CONFIG_MMC_CQ_HCI) && (defined CONFIG_ARCH_HN)
	cmdq_req_done_fun =
		(replace_fun)kallsyms_lookup_name("mmc_blk_cmdq_req_done");
	cmdq_dcmd_done_fun =
		(replace_fun)kallsyms_lookup_name("mmc_blk_cmdq_dcmd_done");
	...
	sd_work_cache = kmem_cache_create("sd_fit_work", sizeof(struct sd_work),
	...
	ras_retn_iferr(
		proc_init(MODULE_NAME, &proc_ops_name(rEmmc), &fault_injected));
	return 0;
}
