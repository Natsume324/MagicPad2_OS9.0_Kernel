...
struct ion_replace_item {
	...
	int (*orig_alloc)(struct ion_heap *heap, struct ion_buffer *buffer,
...
static const struct fault_ops fault_ops_list[] = {
	{
		.name = "soft_reset",
		.fault = FAULT_SOFT_RESET,
	},
	{
		.name = "hard_reset",
		.fault = FAULT_HARD_RESET,
	},
...
static struct fault_impl *args2fault(int args, char *argv[])
{
	...
	err = memset_s(&fault_tmp, sizeof(struct fault_impl), 0,
		       sizeof(struct fault_impl));
	if (err != EOK) {
		printk(KERN_ERR "memset fault_tmp failed\n");
	}
	...
	if (NULL == fault)
		return NULL;
	err = memcpy_s(fault, sizeof(struct fault_impl), &fault_tmp,
		       sizeof(struct fault_impl));
	if (err != EOK) {
		return NULL;
	}
...
static int
rasprobe_handler(kbase_mmu_hw_do_operation)(struct rasprobe_instance *ri,
					    struct pt_regs *regs)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
static int
rasprobe_handler_entry(fence_check_cb_func)(struct rasprobe_instance *ri,
					    struct pt_regs *regs)
{
	...
	if (fault->delay >= 0)
		fence_wk->expire =
			msecs_to_jiffies(100 + fault->delay) + jiffies;
...
void check_del_work(int ischeck)
{
	...
	list_for_each_entry_safe(wk_item, next, &(wq_list.stlist), stlist) {
		if (ischeck && (wk_item->timeout < 0 ||
				time_after(wk_item->expire, jiffies)))
			continue;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0)
static int
rasprobe_handler(kbase_pm_wait_for_reset)(struct rasprobe_instance *ri,
					  struct pt_regs *regs)
{
...
static int
rasprobe_handler_entry(kbase_gpu_complete_hw)(struct rasprobe_instance *ri,
					      struct pt_regs *regs)
{
	struct RasRegs *rr = NULL;
...
#if defined(CONFIG_MTK_PLATFORM) || defined(CONFIG_RPHONE_MTK)
static int rasprobe_handler_entry(key_int_handler)(struct rasprobe_instance *ri,
						   struct pt_regs *regs)
{
	struct RasRegs *rr = NULL;
...
static int proc_ops_show(rGPU)(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < ARRAY_SIZE(fault_injected.impl); i++) {
		...
		seq_printf(m, "%2d\t%s\t time=%d\t space=%u\n", i,
			   type2name(impl->fault), impl->times, impl->space);
	}
	return 0;
}
...
static ssize_t proc_ops_write(rGPU)(struct file *filp, const char __user *bff,
...
static int tool_init(void)
{
	...
	fence_timeout_tsk =
		kthread_run(fit_fence_timeout_thread, NULL, "fit_fencetimeout");
	if (!fence_timeout_tsk) {
		kmem_cache_destroy(ion_heap_cache);
		goto out_proc;
	}
	fence_wk_cache =
		kmem_cache_create("wq_work_item", sizeof(struct wq_work_item),
