...
static int
rasprobe_handler(honor_battery_capacity)(struct rasprobe_instance *ri,
					 struct pt_regs *regs)
{
	if (capacity == -1)
		return 0;
...
static int rasprobe_handler(honor_battery_current)(struct rasprobe_instance *ri,
						   struct pt_regs *regs)
{
	if (capacity == -1)
		return 0;
...
static int rasprobe_handler(honor_battery_voltage)(struct rasprobe_instance *ri,
						   struct pt_regs *regs)
{
...
static int tool_init(void)
{
	...
	hook_capacity_sum =
		(unsigned int *)kallsyms_lookup_name("capacity_sum");
	hook_capacity_filter =
		(unsigned int *)kallsyms_lookup_name("capacity_filter");
