	...
	list_for_each_entry_safe(node, next, &(head.list), list) {
		if (jiffies - node->expires >= 2 * HZ) {
			...
			default:
				...
				list_del(&(node->list));
				total_delay_size[node->probe_type] -=
					4096 << (*((int *)(node->para1)));
				total_num--;
...
static int tool_init(void)
{
	...
	total_num = 0;
	ret = memset_s(single_filler_page, PAGE_SIZE, FILLER, PAGE_SIZE);
	if (ret != EOK) {
		printk(KERN_ERR "memset_s single_filler_page failed\n");
	}
	ret = memset_s(filler_pages_with_order_4, PAGE_SIZE * (1 << 4), FILLER,
		       PAGE_SIZE * (1 << 4));
	if (ret != EOK) {
		printk(KERN_ERR "memset_s filler_pages_with_order_4 failed\n");
	}
	ret = memset_s(filler_pages_with_order_8, PAGE_SIZE * (1 << 8), FILLER,
		       PAGE_SIZE * (1 << 8));
	if (ret != EOK) {
		printk(KERN_ERR "memset_s filler_pages_with_order_8 failed\n");
	}
	...
	main_timer = (struct timer_list *)kmalloc(sizeof(struct timer_list),
						  GFP_KERNEL);
	...
	list_for_each_entry_safe(node, next, &(head.list), list) {
		...
		case MEM_REMAP_PAGES:
			printk("timer func: free_pages  probe_type:%d    para0:%lx    para1 addr:%lx, para1 value:%d\n",
			       node->probe_type, (unsigned long)node->para0,
			       (unsigned long)node->para1,
			       *((int *)(node->para1)));
			list_del(&(node->list));
			node->function(node);
