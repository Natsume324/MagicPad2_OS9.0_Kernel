...
#include "securec.h"
int rasprobe_handler_entry(ion_page_pool_free_immediate)(
	struct rasprobe_instance *ri, struct pt_regs *regs)
{
	...
	if (total_delay_size[MEM_ION_SYSTEM_HEAP] >
	    MAX_DELAY_SIZE_FOR_ION_SYSTEM_HEAP) {
		return 0;
	}
	...
	node = (struct transaction_node *)kmalloc(
		sizeof(struct transaction_node), GFP_KERNEL);
	...
	*((unsigned int *)(node->para1)) = order;
	...
	//memory poison
	ret = memset_s((char *)addr, size, FILLER, size);
	if (ret != EOK) {
		printk(KERN_ERR "memset addr failed\n");
		return -1;
	}
	rasprobe_entry(ri, regs);
...
int rasprobe_handler_entry(remap_pfn_range)(struct rasprobe_instance *ri,
					    struct pt_regs *regs)
{
	struct RasRegs *rb;
	unsigned long pfn = 0;
	...
	if (total_delay_size[MEM_REMAP_PAGES] >
	    MAX_DELAY_SIZE_FOR_REMAP_PAGES) {
		return 0;
	}
...
int rasprobe_handler_entry(__free_pages)(struct rasprobe_instance *ri,
					 struct pt_regs *regs)
{
	int size = 0;
	struct RasRegs *rb;
