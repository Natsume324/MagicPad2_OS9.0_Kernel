...
static int rxcollie_thread_handle(void *arg)
{
	...
	while (1) {
		...
		/* Set overtime Value */
		id = hwxcollie_start(
			__func__, (int)(ovt_time * HW_TIMERRING_CHECK_INTVAL),
			rxcollie_callback, (void *)hahaha,
...
static int rxcollie_init(void)
{
	...
	hwxcollie_end = (replace_fun_end)kallsyms_lookup_name("hw_xcollie_end");

	rxcollie_task =
		kthread_run(rxcollie_thread_handle, NULL, "rxcollietask");
