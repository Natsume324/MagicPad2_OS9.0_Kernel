...
#define SET_TASK_STATE(tsk, task_value) \
	smp_store_mb((tsk)->__state, (task_value))
...
static struct {
	...
	{.op = "D-state", PROCESS_D_STATE}, {.op = "Z-state", PROCESS_Z_STATE},
	{.op = "T-state", PROCESS_T_STATE}, {.op = "exit", PROCESS_EXIT},
...
static int resume_fault(struct FaultImpl *ft)
{
	...
	err = memset_s(ft, sizeof(struct FaultImpl), 0,
		       sizeof(struct FaultImpl));
	if (err != EOK) {
		printk(KERN_ERR "memset ft failed\n");
	}
	return 0;
}
...
static int tool_init(void)
{
	...
	err = memset_s(&fault_list, sizeof(fault_list), 0, sizeof(fault_list));
	if (err != EOK) {
		printk(KERN_ERR "memset fault_list failed\n");
	}
	rwlock_init(&fault_list.rwk);
	ras_retn_iferr(
		proc_init(TOOL_NAME, &proc_ops_name(rProcess), &fault_list));
	return 0;
}
