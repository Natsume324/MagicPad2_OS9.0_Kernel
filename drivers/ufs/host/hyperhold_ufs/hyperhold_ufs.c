...
int ufshcd_get_health_info(struct scsi_device *sdev, u8 *pre_eol_info,
			   u8 *life_time_est_a, u8 *life_time_est_b)
{
	...
	if ((pre_eol_info_val != 0) && (life_time_est_a_val != 0) &&
	    (life_time_est_b_val != 0)) {
	...
	ret = ufshcd_read_desc_param(hba, QUERY_DESC_IDN_HEALTH, 0, 0, buff,
	...
	pre_eol_info_val = *pre_eol_info =
		buff[HEALTH_DEVICE_DESC_PARAM_PREEOL];
	life_time_est_a_val = *life_time_est_a =
		buff[HEALTH_DEVICE_DESC_PARAM_LIFETIMEA];
	life_time_est_b_val = *life_time_est_b =
		buff[HEALTH_DEVICE_DESC_PARAM_LIFETIMEB];
			     ...
			     u8 *life_time_est_a, u8 *life_time_est_b)
{
	...
	return sdev->host->hostt->get_health_info(
		sdev, pre_eol_info, life_time_est_a, life_time_est_b);
}
			 ...
			 u8 *life_time_est_a, u8 *life_time_est_b)
{
	...
	ret = scsi_health_query(q, pre_eol_info, life_time_est_a,
				life_time_est_b);
