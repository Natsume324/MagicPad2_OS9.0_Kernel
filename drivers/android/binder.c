...
// TODO -- add define -- more
#define BINDER_SCHED_ENABLE (1)
#define QOS_SCHED_LOCK_ENABLE (0)

#ifdef CONFIG_HN_QOS_SCHED_BUILDIN
static int get_qos_trans_by_type_gki(struct task_struct *task, int type)
{
	int qos = VALUE_QOS_INVALID;
	trace_android_vh_get_trans_qos_by_type(task, type, &qos);
	return qos;
}

static int get_task_qos_gki(struct task_struct *task)
{
	int qos = VALUE_QOS_INVALID;
	trace_android_vh_get_task_qos(task, &qos);
	return qos;
}
#endif // CONFIG_HN_QOS_SCHED_BUILDIN
...
#endif // CONFIG_HN_BINDER_SCHED --> no QOS depend
...
bool check_binder_worklist_ilocked(struct list_head *todo_list,
	...
	hlist_for_each_entry(proc, &binder_procs, proc_node) {
		if (proc->pid == called_pid) {
			...
			if (!binder_worklist_empty_ilocked(&cache_called_proc->todo)) {
				if(check_binder_worklist_ilocked(&cache_called_proc->todo, called_pid, async_exempted, "todo")) {
			...
			if (!binder_worklist_empty_ilocked(&cache_called_proc->fg_todo)) {
				if(check_binder_worklist_ilocked(&cache_called_proc->fg_todo, called_pid, async_exempted, "fg_todo")) {
					binder_inner_proc_unlock(cache_called_proc);
					mutex_unlock(&binder_procs_lock);
					return binder_info_prt(called_pid);
				}
			...
			if (!binder_worklist_empty_ilocked(&cache_called_proc->bg_todo)) {
				if(check_binder_worklist_ilocked(&cache_called_proc->bg_todo, called_pid, async_exempted, "bg_todo")) {
					binder_inner_proc_unlock(cache_called_proc);
					mutex_unlock(&binder_procs_lock);
					return binder_info_prt(called_pid);
				}
			...
			for (n = rb_first(&cache_called_proc->threads); n != NULL; n = rb_next(n)) {
				...
				if (!binder_worklist_empty_ilocked(&called_binder_thread->todo)) {
					if(check_binder_worklist_ilocked(&called_binder_thread->todo, called_pid, async_exempted, "th_todo")) {
						binder_inner_proc_unlock(cache_called_proc);
						mutex_unlock(&binder_procs_lock);
						return binder_info_prt(called_pid);
					}
				      ...
				      struct binder_proc *proc)
{
	...
	if (atomic_read(&binder_sched_switch) && current->cred &&
		...
		if (cur_uid > MIN_APPLICATION_UID)
			...
			// For QOS-GKI
#ifdef CONFIG_HN_QOS_SCHED_BUILDIN
			task_qos = get_task_qos_gki(thread->task);
#endif
#endif
	}
#ifdef CONFIG_HN_QOS_THREAD
	// check and set thread qos trans by binder,
	is_binder_trans_vertical =
		binder_thread_check_and_set_dynamic_qos(thread, from, oneway);
#else
	//TODO hook function for above --> Done
	// check qos was trans by binder only because trans happened before.
#ifdef CONFIG_HN_QOS_SCHED_BUILDIN
	is_binder_trans_vertical = get_qos_trans_by_type_gki(thread->task, DYNAMIC_QOS_BINDER) == VALUE_QOS_CRITICAL;
#endif
#endif
						 ...
						 struct binder_transaction *t)
{
	...
	// TODO : hook function for above --> Done
#ifdef CONFIG_HN_QOS_SCHED_BUILDIN
	qos = t->from ? get_task_qos_gki(t->from->task) : BINDER_SCHED_DEFAULT_QOS;
#endif
#endif
				    ...
				    struct binder_thread *thread)
{
	...
	if (thread) {
		...
		is_vip = false;
#endif
#ifdef CONFIG_HN_QOS_ENABLE
#ifdef CONFIG_HN_QOS_THREAD
		// Check if add binder-work to fg-todo
		do_enqueue = binder_enable_fg_switch && QOS_SCHED_LOCK_ENABLE &&
			     (!oneway) &&
			     (get_task_qos(current) == VALUE_QOS_CRITICAL);
#else
		// Use hook For QOS-GKI
#ifdef CONFIG_HN_QOS_SCHED_BUILDIN
		do_enqueue = binder_enable_fg_switch && QOS_SCHED_LOCK_ENABLE &&
			     (!oneway) &&
			     (get_task_qos_gki(current) == VALUE_QOS_CRITICAL);
#endif
#endif // CONFIG_HN_QOS_THREAD
#endif // CONFIG_HN_QOS_ENABLE
#ifdef CONFIG_HN_VIP_THREAD_BUILDIN
