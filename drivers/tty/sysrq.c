...
static void sysrq_shutdown_crash(int key)
{
	pr_info("kernel shutdown crash");
	kernel_power_off();
}
static const struct sysrq_key_op sysrq_shutdown_crash_op = {
	.handler	= sysrq_shutdown_crash,
	.help_msg	= "shutdown_crash(A)",
	.action_msg	= "Trigger a kernel shutdown crash",
	.enable_mask	= SYSRQ_ENABLE_DUMP,
};
