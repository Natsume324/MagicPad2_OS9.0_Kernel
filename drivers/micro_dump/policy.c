...
static void dabt_policy_local_stack_dump(void)
{
	if (is_stack_ptr(g_exp_ctrl.regs.sp,
			 (unsigned long)g_exp_ctrl.task->stack)) {
		microdump_stack_local_var(g_exp_ctrl.regs.regs[FP_REGISTER],
					  g_exp_ctrl.task->stack,
					  MAX_STACK_FRAME);
	} else {
		microdump_stack_local_irq(g_exp_ctrl.regs.regs[FP_REGISTER]);
		microdump_stack_local_var(
			g_exp_ctrl.task->thread.cpu_context.fp,
			g_exp_ctrl.task->stack, MAX_STACK_FRAME);
	}
}

static void dump_core_irqstack(void)
{
	int i;
...
int dabt_policy(unsigned long type)
{
	...
	case SUB_TYPE_UNHANDLE:
		...
		microdump_stack_by_sp(g_exp_ctrl.regs.sp, g_exp_ctrl.task,
				      g_exp_ctrl.regs.regs[SCS_REGISTER]);
		...
		if (g_exp_ctrl.regs.sp != 0) {
			...
			if (microdump_check_addr_valid(g_exp_ctrl.regs.sp)) {
				microdump_stack_by_sp(
					g_exp_ctrl.regs.sp, g_exp_ctrl.task,
					g_exp_ctrl.regs.regs[SCS_REGISTER]);
			} else {
