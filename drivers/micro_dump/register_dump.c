...
static void dump_valid_memory_for_register(struct pt_regs *regs)
{
	...
	for (i = 0; i < MICRO_REGS_NUM; i++) {
		if (microdump_check_addr_valid(regs->regs[i])) {
			...
			MD_PRINT(
				"X%d register:0x%016llx PHY_ADDR:%016lx PAGE_TYPE:%s \n",
				i, regs->regs[i], info.phy_addr,
				TYPE_NAME[info.type]);

			if (info.type == TYPE_NONE ||
			    is_stack_ptr(regs->sp, regs->regs[i]))
				continue;
			...
			case TYPE_SLAB:
			case TYPE_PAGE:
				microdump_page_dump_checked(regs->regs[i],
							    ONE_PAGE, &info);
				break;
			case TYPE_BSS:
			case TYPE_DATA:
			case TYPE_RODATA:
			case TYPE_VMEM:
				microdump_byte_dump_checked(regs->regs[i], 0,
							    PAGE_SIZE >> 5,
							    &info);
				break;
			case TYPE_MODULE:
	...
	if (microdump_check_addr_valid(regs->pc)) {
		...
		MD_PRINT(
			"PC register:0x%016llx PHY_ADDR:%016lx PAGE_TYPE:%s \n",
			regs->pc, info.phy_addr, TYPE_NAME[info.type]);
		// 4: print 1/16 pagesize, 5: print 1/32 pagesize
		microdump_byte_dump_checked(regs->pc, PAGE_SIZE >> 5,
					    PAGE_SIZE >> 4, &info);
	} else {
