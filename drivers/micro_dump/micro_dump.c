...
static void touch_watchdog(void)
{
	touch_nmi_watchdog();
}
...
static void microdump_print_pstate(struct pt_regs *regs)
{
	...
	if (compat_user_mode(regs)) {
		MD_PRINT("pstate: %08lx (%c%c%c%c %c %s %s %c%c%c)\n", pstate,
		...
		MD_PRINT("pstate: %08lx (%c%c%c%c %c%c%c%c %cPAN %cUAO)\n",
			 pstate, pstate_flag(pstate & PSR_N_BIT, 'N', 'n'),
...
{
	int i;
	for (i = 0; i < (sizeof(g_fault_reason) / sizeof(g_fault_reason[0]));
	     i++) {
...
void micro_dump_entry(void)
{
	...
	MD_PRINT(
		"<<<<<<< micro dump to log_buf start has_die=%d, has_panic=%d \n",
...
static void micro_dump_reason_set(const char *reason, unsigned long die_type,
				  void *arg)
{
	if (g_exp_ctrl.error_type != DIE_TYPE_UNKNOWN &&
		...
		MD_PRINT(
			"error reason has set: %s, new type=%lx, old type=%lx\n",
			reason, die_type, g_exp_ctrl.error_type);
		return;
	}
...
static int die_callback(struct notifier_block *self, unsigned long cmd,
			void *ptr)
{
	unsigned long type;
	const char *reason;
	...
	if (atomic_inc_and_test(&g_has_die)) {
		...
		if (dargs && dargs->regs) {
			...
			MD_PRINT(
				"die_callback, task.comm:%s, task.pid:%d, stack:0x%lx \n",
				g_exp_ctrl.task->comm, g_exp_ctrl.task->pid,
				(unsigned long)g_exp_ctrl.task->stack);
		...
		MD_PRINT(
			"die_callback has called by cpu:%d, it must be double die in die flow.\n",
			g_exp_ctrl.die_cpu);
	}
...
static int panic_callback(struct notifier_block *self, unsigned long cmd,
			  void *ptr)
{
	...
	if (atomic_inc_and_test(&g_has_panic)) {
		...
		if (!has_die()) { // no die flow, direct panic flow, so need save regs by asm manully
			...
			g_exp_ctrl.task = current;
			MD_PRINT(
				"panic_callback, task.comm:%s, task.pid:%d, stack:0x%lx \n",
				g_exp_ctrl.task->comm, g_exp_ctrl.task->pid,
				(unsigned long)g_exp_ctrl.task->stack);
			microdump_show_regs(&g_exp_ctrl.regs);
			dump_stack();
		...
		MD_PRINT(
			"panic_callback has called by cpu:%d, it must be double panic in panic flow.\n",
...
{
	const char *src = g_microdump_enable ? MICRODUMP_ENABLE :
					       MICRODUMP_DISABLE;
	errno_t err;
	err = strncpy_s(buf, strlen(buf), src, strlen(src));
	if (err != EOK) {
		MD_PRINT("strncpy_s buf failed\n");
	}
	return strlen(buf);
}
...
static struct attribute *sysfs_attrs[] = {&microdump_enable_attr.attr, NULL};
...
static int micro_mrdump_parse_chosen(struct mrdump_params *mparams)
{
	...
	memset_s(mparams, sizeof(struct mrdump_params), 0,
		 sizeof(struct mrdump_params));
	node = of_find_node_by_path("/chosen");
	if (node) {
		if (of_property_read_u32_array(node, "mrdump,cblock", reg,
					       ARRAY_SIZE(reg)) == 0) {
...
static int __init debug_register_init(void)
{
#ifdef CONFIG_HN_MICRODUMP_GKI
	int ret;
#endif
...
#ifdef CONFIG_HN_MICRODUMP_GKI
	ret = register_trace_android_rvh_do_undefinstr(microdump_instr_vh,
						       NULL);
	if (ret)
