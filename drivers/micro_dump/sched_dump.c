...
{
	MD_PRINT(
		"%c %15s %5d %9lld %3d %9lld.%06ld %9lld.%06ld %9lld.%06ld"
		" %9lld.%06ld %9lld.%06ld %9lld.%06ld %9lld.%06ld %9lld.%06ld\n",
		task_state_to_char(p), p->comm, task_pid_nr(p),
		(long long)(p->nvcsw + p->nivcsw), p->prio,
		SPLIT_NS(p->sched_info.last_arrival),
		SPLIT_NS(p->sched_info.last_queued),
		SPLIT_NS(p->sched_info.run_delay),
		SPLIT_NS(p->last_enqueued_ts), SPLIT_NS(p->last_sleep_ts),
		SPLIT_NS((p->last_sleep_ts - p->last_enqueued_ts)),
		SPLIT_NS(p->se.sum_exec_runtime),
		SPLIT_NS(p->se.statistics.wait_sum));
}

void print_rq(const struct rq *rq, const struct task_struct *p)
{
#define P(x)                                                                  \
	do {                                                                  \
		if (sizeof(rq->x) == 4)                                       \
...
#define PN(x) MD_PRINT("  .%-30s: %lu\n", #x, SPLIT_NS(rq->x))
	...
	MD_PRINT("  .%-30s: %ld %ld %ld %ld %ld\n", "cpu_load",
		 (long)(rq->cpu_load[0]), (long)(rq->cpu_load[1]),
		 (long)(rq->cpu_load[2]), (long)(rq->cpu_load[3]),
		 (long)(rq->cpu_load[4]));
...
static void microdump_print_cfs_group_stats(const int cpu,
...
#define P(x) MD_PRINT("  .%-30s: %lld\n", #x, (long long)(rt_rq->x))
#define PU(x) MD_PRINT("  .%-30s: %lu\n", #x, (unsigned long)(rt_rq->x))
#define PN(x) MD_PRINT("  .%-30s: %lu\n", #x, SPLIT_NS(rt_rq->x))
...
void microdump_sched_dump(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		MD_PRINT(
			"state        name   pid  switches prio     exec_start       last_quened"
		...
		for_each_process_thread(g, p) {
			...
			timeout = msecs_to_jiffies(
				msec_high(p->sched_info.last_arrival) * 1000);
			timeout += msecs_to_jiffies(PROCESS_RUN_TIME * 1000) +
				   INITIAL_JIFFIES;
