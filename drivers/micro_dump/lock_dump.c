...
static pid_t get_hung_pid(void)
{
	...
	dumper.active = true;
	while (kmsg_dump_get_line_nolock(&dumper, true, line, sizeof(line),
					 &len)) {
		line[len] = '\0';
	...
	if (!pid) {
		MD_PRINT(
			"Unable to convert out correct pid, maybe no pid info\n");
		return -1;
	}
...
static unsigned long get_lock_addr(struct task_struct *task)
{
	...
	do {
		sprintf_s(buf, sizeof(buf) - 1, "%ps", (void *)frame.pc);
		...
		if (i != ARRAY_SIZE(g_lock_list)) {
			...
			if (microdump_check_addr_valid(
				    (unsigned long)lock_addr)) {
				MD_PRINT("task %s is doing %s lock:0x%016lx",
					 task->comm, g_lock_list[i].lock_symbol,
					 lock_addr);
				return lock_addr;
			}

			MD_PRINT(
				"lock addr:0x%016lx is invalid, try another way\n",
				lock_addr);
...
static unsigned long identify_lock_holder(unsigned long lock_addr,
					  enum lock_type type)
{
	...
	switch (type) {
	case RW_SEM:
		rw_sem = (struct rw_semaphore *)lock_addr;
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
...
static struct task_struct *get_lock_holder(struct task_struct *task)
{
	...
	holder = (struct task_struct *)identify_lock_holder(lock_addr, type);
	if (!holder) {
		MD_PRINT(
			"lock struct don't have owner or lock holder is NULL\n");
		return NULL;
	}
...
static void traverse_lock_holders(struct task_struct *entry_task)
{
	...
	for (i = 0; i < MAX_CHECK_DEEP; i++) {
		...
		MD_PRINT(
			"lock:%d holder info task_addr:0x%016lx, name:%s, pid:%d, tgid:%d, state:%d\n",
			i, task, task->comm, task->pid, task->tgid,
			task->state);
		switch (task->state) {
...
void microdump_hungtask_lock_dump(void)
{
	...
	MD_PRINT("hung task info name:%s, pid:%d, tgid:%d, state:%d\n",
		 hung_task->comm, hung_task->pid, hung_task->tgid,
		 hung_task->state);
