...
static void get_slab_page_info(unsigned long addr,
			       struct microdump_page_info *info)
{
	...
	if (PageSlab(page)) {
		/* No need to convert page to page-head
		 * Cause page = virt_to_head_page((void *)addr)
		 */
		slab = page_slab(page);
		info->type = TYPE_SLAB;
	} else {
	...
	/* save slab info cause slab is split from page struct */
	if (!slab) {
		info->slab = slab;
	}
}

void microdump_addr_page_info(unsigned long addr,
			      struct microdump_page_info *info)
{
	if (!info)
		return;
...
#define IN_REGION_PHYS(a, b, c)                          \
	(__phys_to_virt((a)) >= (__phys_to_virt((b))) && \
	 __phys_to_virt((a)) <= (__phys_to_virt((c))))
#endif

	if (IN_REGION(addr, __phys_to_virt(memblock.memory.regions[0].base),
		      (unsigned long)high_memory)) {
		get_slab_page_info(addr, info);
	} else if (IN_REGION(addr, module_alloc_base,
		...
		if (IN_REGION_PHYS(addr, _text, _etext)) {
			info->type = TYPE_TEXT;
		} else if (IN_REGION_PHYS(addr, __bss_start, __bss_stop)) {
			info->type = TYPE_BSS;
		} else if (IN_REGION_PHYS(addr, _sdata, _edata)) {
			info->type = TYPE_DATA;
		} else if (IN_REGION_PHYS(addr, __init_begin, __init_end)) {
			info->type = TYPE_INIT;
		} else if (IN_REGION_PHYS(addr, __start_rodata, __init_begin)) {
			info->type = TYPE_RODATA;
		}
	} else if (IN_REGION(addr, VMEMMAP_START,
...
static bool page_has_dump(unsigned long addr)
{
	...
	for (i = 0; i < MAX_DUMP_ADDR_NUM; i++) {
		for (j = 0; j <= g_exp_ctrl.dump_size[i]; j += PAGE_SIZE) {
			if (page_start ==
			    ((g_exp_ctrl.dump_addr[i] + PAGE_SIZE * j) &
			     PAGE_ALIGN_MASK)) {
				return true;
			}
		}
...
static bool bytes_has_dump(unsigned long addr, int nr_byte)
{
	...
	for (i = 0; i < MAX_DUMP_ADDR_NUM; i++) {
		if (g_exp_ctrl.dump_addr[i] <= addr) {
			if (g_exp_ctrl.dump_addr[i] + g_exp_ctrl.dump_size[i] >=
			    addr + nr_byte) {
				return true;
			}
		}
...
static void *microdump_nearest_obj(struct kmem_cache *cache, struct slab *slab,
				   void *x)
{
	void *last_object;
	void *result;
	...
	object = x - (x - slab_address(slab)) % cache->size;
	if (object != x) {
		MD_PRINT(
			"SLUB: slub 0x%lx not align object=0x%lx ,slab_address=0x%lx size=0x%x\n",
			(uintptr_t)x, (uintptr_t)object,
			(unsigned long)slab_address(slab), cache->size);
	}
	last_object = slab_address(slab) + (slab->objects - 1) * cache->size;
...
static inline void print_slub_info(struct slab *slab, void *slub_info,
				   bool free)
{
	struct kmem_cache *cache = slab->slab_cache;
	MD_PRINT(
		"SLUB: 0x%lx,pa(0x%llx),0x%lx objects=%u used=%u freelist=0x%lx flags=0x%04lx, %s, cache name:%s, %d\n",
		(uintptr_t)slab, __pa(slub_info), (uintptr_t)slub_info,
		slab->objects, slab->inuse, (unsigned long)slab->freelist,
		slab->__page_flags, free ? "free" : "not free", cache->name,
		cache->size);
}
...
static phys_addr_t
find_prev_slab_and_print_slub(phys_addr_t pa_addr, void *ptr,
			      struct microdump_page_info *info)
{
	void *va = ptr;
	...
	slab_kmem_cache = slab->slab_cache;
	if (!slab_kmem_cache)
		return 0;

	slub_info = microdump_nearest_obj(slab_kmem_cache, slab, va);
	if (!slub_info) {
		return 0;
	}

	for (s = slab->freelist; s != NULL;
	     s = get_free_ptr(slab_kmem_cache, s)) {
		if (s == slub_info)
			break;

		free_count++;
		if (free_count > (slab->objects - slab->inuse) ||
		    free_count > MAX_PAGE_OBJECTS)
			break;
	}

	if (s == slub_info)
		print_slub_info(slab, slub_info, true);
	else
		print_slub_info(slab, slub_info, false);

	if (slab_kmem_cache->flags & SLAB_STORE_USER)
		print_tracking(slab_kmem_cache, slub_info);

	// prev slab info
	if (slub_info - slab_kmem_cache->size <= slab_address(slab)) {
...
static void check_and_print_slub_info(unsigned long ptr,
				      struct microdump_page_info *info)
{
	...
	if (microdump_check_addr_valid(ptr)) {
		if (info && info->type == TYPE_SLAB) {
			MD_PRINT("SLUB:0x%lx - pa:%px\n", (unsigned long)ptr,
				 (void *)__pa(ptr));
			prev_slub = find_prev_slab_and_print_slub(
				(phys_addr_t)info->phy_addr, (void *)ptr, info);
			if (prev_slub)
				prev_slub = find_prev_slab_and_print_slub(
					prev_slub, __va(prev_slub), info);
		}
	}
}
#endif
...
static int microdump_unique_info_ext_by_page(unsigned long ptr, int *start,
					     int *size,
					     struct microdump_page_info *info)
{
	int by_page = 0;
	switch (info->type) {
	case TYPE_VMALLOC:
		dump_page_summary_info(ptr, VMALLOC_START, VMALLOC_END,
				       "vmalloc");
		by_page = 1;
		*start = 0;
		*size = PAGE_SIZE;
		break;
	case TYPE_PAGE:
		dump_page_summary_info(
			ptr, __phys_to_virt(memblock.memory.regions[0].base),
			(unsigned long)high_memory, "liner");
		by_page = 1;
		*start = 0;
	...
	case TYPE_STACK:
		...
		dump_page_summary_info(ptr, info->start_vir_addr,
				       info->start_vir_addr + 4 * PAGE_SIZE,
				       "stack");
		by_page = 1;
		*start = 0;
		*size = PAGE_SIZE;
				      ...
				      struct microdump_page_info *info)
{
	switch (info->type) {
	case TYPE_BSS:
		dump_page_summary_info(ptr, __phys_to_virt(__bss_start),
				       __phys_to_virt(__bss_stop), "bss");
		*start = (int)(sizeof(unsigned long)) * 8;
		*size = PAGE_SIZE >> 2; // 2: print 1/4 pagesize
		break;
	case TYPE_DATA:
		dump_page_summary_info(ptr, __phys_to_virt(_sdata),
				       __phys_to_virt(_edata), "data");
		*start = (int)(sizeof(unsigned long)) * 8;
...
#endif
	case TYPE_VMEM:
		dump_page_summary_info(ptr, VMEMMAP_START,
				       VMEMMAP_START + VMEMMAP_SIZE, "vmemmap");
		*start = (int)(sizeof(unsigned long)) * 8;
		*size = PAGE_SIZE >> 4; // 4: print 1/16 pagesize
		break;
	case TYPE_MODULE:
		dump_page_summary_info(ptr, module_alloc_base,
				       module_alloc_base + MODULES_VSIZE,
				       "module");
		*start = (int)(sizeof(unsigned long)) * 8;
		*size = PAGE_SIZE >> 4; // 4: print 1/16 pagesize
...
static void dump_page_data(unsigned long ptr, int nr_page,
			   struct microdump_page_info *info)
{
	...
	if (!page_has_dump(addr)) {
		...
		nr_page = (nr_page > TASK_MULTIPLE_PAGE_SIZE) ?
				  TASK_MULTIPLE_PAGE_SIZE :
				  nr_page;
		pages = nr_page;
		do {
		...
		g_exp_ctrl.print_func(ptr & PAGE_ALIGN_MASK,
				      (pages - nr_page) * PAGE_SIZE,
				      pre_info_priv);
	} else {
...
static void dump_byte_data(unsigned long ptr, int before_byte, int nr_byte,
			   struct microdump_page_info *info)
{
	unsigned long addr = ptr;
	int align_bytes;
	...
	if (microdump_check_addr_valid(addr - before_byte)) {
		...
		MD_PRINT(
			"previous page is invalid, print it from the start of this page");
	...
	if (!microdump_check_addr_valid(addr + nr_byte)) {
		MD_PRINT(
			"next page is invalid, print it to the end of the last valid page");
...
void microdump_page_dump_checked(unsigned long ptr, int nr_page,
				 struct microdump_page_info *info)
{
	dump_page_data(ptr, nr_page, info);
}

void microdump_page_dump(unsigned long ptr, int nr_page,
			 struct microdump_page_info *info)
{
	if (!microdump_check_addr_valid(ptr)) {
...
void microdump_byte_dump_checked(unsigned long ptr, int before_byte,
				 int nr_byte, struct microdump_page_info *info)
{
	unsigned long addr = ptr;
	int by_page;
	int start = 0;
