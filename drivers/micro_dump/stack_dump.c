...
void microdump_stack_adjust_by_task(struct task_struct *task)
{
	...
	for (i = 0; i < TASK_MULTIPLE_PAGE_SIZE; i++) {
		if (microdump_check_addr_valid((unsigned long)task->stack +
					       i * PAGE_SIZE)) {
			info.type = TYPE_STACK;
			info.start_vir_addr = (unsigned long)task->stack;
			MD_PRINT(
				"%s task=0x%lx , stack=0x%lx, sp=0x%lx ,fp=0x%lx ,pc=0x%lx\n",
				__func__, (uintptr_t)task,
				(unsigned long)task->stack,
				thread_saved_sp(task), thread_saved_fp(task),
				thread_saved_pc(task));
			microdump_page_dump_checked(
				(unsigned long)task->stack + i * PAGE_SIZE,
...
void microdump_stack_adjust_by_sp(unsigned long sp, struct task_struct *task,
				  unsigned long fp)
{
	struct microdump_page_info info = {0};
	...
	if (sp > (unsigned long)task->stack &&
	    sp < (((unsigned long)task->stack) +
	...
	if (!same_stack) {
		...
		if (microdump_check_addr_valid(sp)) {
			...
			MD_PRINT(
				"%s not same stack end, sp:0x%lx, fp:0x%lx task->stack:0x%lx\n",
				__func__, sp, fp, (unsigned long)task->stack);
		}
	} else {
		print_len = (unsigned long)task->stack +
			    TASK_MULTIPLE_PAGE_SIZE * PAGE_SIZE - sp;
		if (print_len >= (TASK_MULTIPLE_PAGE_SIZE * PAGE_SIZE)) {
			MD_PRINT(
				"%s: invalid sp:0x%lx or task->stack:0x%lx, print_len:%ld \n",
				__func__, sp, (unsigned long)task->stack,
				print_len);
			return;
		}
		MD_PRINT(
			"%s: sp:0x%lx, fp:0x%lx, task->stack:0x%lx, print_len:%ld\n",
			__func__, sp, fp, (unsigned long)task->stack,
			print_len);
		if (microdump_check_addr_valid(sp)) {
			microdump_addr_page_info(sp, &info);
				       ...
				       unsigned long *size, u32 nr)
{
	...
	stack_bottom = ((unsigned long)(uintptr_t)stack) +
		       (PAGE_SIZE * TASK_MULTIPLE_PAGE_SIZE);
	MD_PRINT(
		"%s in, sp:0x%lx, stack:0x%lx, nr:0x%x bottom:0x%lx sp aglin:0x%x\n",
		__func__, sp, (uintptr_t)stack, nr, stack_bottom,
	...
	MD_PRINT("%s in, sp:0x%lx, stack:0x%lx, nr:0x%x\n", __func__, sp,
		 (uintptr_t)stack, nr);

	fp = sp;
	p = (uintptr_t)fp;

	while (nr-- > 0 && (unsigned long)p >= sp &&
	       (unsigned long)p < stack_bottom) {
...
void microdump_stack_local_var(unsigned long sp, unsigned long *stack,
			       u32 nr_frame)
{
	unsigned long addrs;
	...
	if (sp < (unsigned long)(uintptr_t)stack ||
	    sp > ((unsigned long)(uintptr_t)stack +
		  TASK_MULTIPLE_PAGE_SIZE * PAGE_SIZE))
		return;

	if (nr_frame > 0) {
		def_nr_frame = (nr_frame > MAX_STACK_FRAME) ? MAX_STACK_FRAME :
							      nr_frame;
	}
	...
	for (i = 0; i < size / sizeof(unsigned long); i++) {
		...
		if (microdump_check_addr_valid(*p)) {
			if (is_stack_ptr(*p, (unsigned long)(uintptr_t)stack) ||
			    *p < PAGE_OFFSET)
				continue;
...
void microdump_stack_local_irq(unsigned long irq_sp)
{
	...
	unsigned long page_bottom =
		(irq_sp & PAGE_ALIGN_MASK) + PAGE_SIZE - sizeof(unsigned long);
	struct microdump_page_info info = {0};
...
void microdump_stack_by_sp(unsigned long sp, struct task_struct *task,
			   unsigned long fp)
{
