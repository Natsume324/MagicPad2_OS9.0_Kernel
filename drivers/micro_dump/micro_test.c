...
static noinline int test_bitflip_dabt(void)
{
	...
	unsigned long test_addr_value = ((uintptr_t)(&test_data)) &
					0xfdffffffffffffff;
...
static noinline void micro_handle_sysrq(char key, bool check_mask)
{
	...
	case 't':
		...
		for (tmp = 0; tmp < 30; tmp++) {
			MD_PRINT(
				"micro_handle_sysrq asm_get_regs x-%2d = 0x%lx \n",
				tmp, regs.regs[tmp]);
		}
		break;
	case 'n':
...
{
	if (!proc_create("microdump",
			 S_IWOTH | S_IWGRP | S_IWUSR | S_IROTH | S_IRGRP |
				 S_IRUSR,
