				 ...
				 unsigned long slub_ram_size)
{
	...
	strncpy_s(top_slub_detail->name, MEMCHECK_MEM_NAME_MAX,
		  slab_cache->name, MEMCHECK_MEM_NAME_MAX - 1);
}
...
void get_top_slub_info(struct mm_slub_detail_info *top_slab_info)
{
	...
	memset_s(top_slab_info, sizeof(*top_slab_info), 0,
		 sizeof(*top_slab_info));
	...
	list_for_each_entry(s, &slab_caches, list) {
		...
		if (slub_ram_size > top_slab_info->size)
			update_top_slub_info(top_slab_info, &sinfo, s,
					     slub_ram_size);
	}
...
static int memcheck_slub_detail_read(size_t num, char *buf, size_t total)
{
	...
	for (i = 0; i < ret_num; i++) {
		tmp = snprintf(buf + used, total - used, "%s %lu %lu %u %lu\n",
			       list[i].name, list[i].active_objs,
			       list[i].num_objs, list[i].objsize, list[i].size);
		if (tmp < 0)
			goto err_list;
				    ...
				    char *buf, size_t total, size_t used)
{
	...
	formate_ret = check_buf_write_result(
		snprintf_s(buf + used, total - used, total - used - 1,
			   "leak(top) slab name: %s\n",
			   memcheck_get_top_slub()),
		total, &used);
	...
	for (i = 0; i < num; i++) {
		...
		for (j = 0; j < list[i].stack_entry.stack_len; j++) {
			formate_ret = check_buf_write_result(
				snprintf_s(buf + used, total - used,
					   total - used - 1, "[%pS]",
					   list[i].stack_entry.addrs[j]),
				total, &used);
			if (formate_ret != BUF_FORMATE_OK)
				goto buf_done;
		}
				      ...
				      char *buf, size_t total)
{
	...
	for (i = 0; i < ARRAY_SIZE(slub_type); i++) {
		ret_num = mm_track_read(SLUB_TRACK, list, num, slub_type[i]);
		if (ret_num == 0) {
					 ...
					 char *buf, size_t total)
{
	...
	for (i = 0; i < ARRAY_SIZE(vm_type); i++) {
		ret_num = mm_track_read(VMALLOC_TRACK, list, num, vm_type[i]);
		if (ret_num == 0) {
...
int memcheck_stack_read(void *buf, struct stack_info *info)
{
	...
	for (idx = 0; idx < STACK_NUM; idx++) {
		...
		if (copy_to_user(buf + sizeof(*info), stack_buf[idx], len)) {
			memcheck_err("copy_to_user failed\n");
			goto unlock;
		}
		memcheck_info("read idx=%d,len=%zu\n", idx, len);
		total_len += len;
...
int memcheck_stack_write(const void *buf, const struct stack_info *info)
{
	...
	idx = ((info->type & MTYPE_USER_PSS_JAVA)   ? IDX_JAVA :
	       (info->type & MTYPE_USER_PSS_NATIVE) ? IDX_NATIVE :
						      IDX_FDTRACK);
	mutex_lock(&stack_mutex);
	if (stack_buf[idx])
		vfree(stack_buf[idx]);
				      ...
				      bool is_fdtrack)
{
	if (!left) {
		...
		if (is_fdtrack)
			memcheck_err("wait for fd stack timeout\n");
		return -ETIMEDOUT;
	} else if (left < 0) {
		if (is_java)
		...
		if (is_fdtrack)
			memcheck_err("wait for fd stack return %d\n", left);
		return -EFAULT;
	}
...
int memcheck_wait_stack_ready(u16 type)
{
	...
	bool is_fdtrack = (type & MTYPE_USER_PSS_FDTRACK) ? true : false;
	int index;
	int ret = 0;

	index = (is_java ? IDX_JAVA : (is_native ? IDX_NATIVE : IDX_FDTRACK));

	mutex_lock(&stack_mutex);
	is_waiting[index] = true;
	init_waitqueue_head(&stack_ready);
	mutex_unlock(&stack_mutex);
	left = wait_event_interruptible_timeout(stack_ready, stack_buf[index],
						STACK_WAIT_TIME_SEC * HZ);
	mutex_lock(&stack_mutex);
	if (stack_buf[index])
		memcheck_info("get %s stack successfully\n",
			      is_java ? "java" :
					(is_native ? "native" : "fdtrack"));
	else
...
int memcheck_lmk_oom_write(const struct lmk_oom_write *write)
{
	...
	if (!rec_list) {
		rec_list = kcalloc(MEMCHECK_OOM_LMK_MAXNUM, sizeof(write->data),
				   GFP_ATOMIC);
		if (!rec_list) {
			spin_unlock_irqrestore(&list_lock, flags);
			return -ENOMEM;
	...
	if (rec_num == MEMCHECK_OOM_LMK_MAXNUM) {
		...
		memcpy(&rec_list[MEMCHECK_OOM_LMK_MAXNUM - 1], &write->data,
		       len);
	} else {
		len = sizeof(write->data);
