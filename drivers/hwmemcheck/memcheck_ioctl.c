...
static unsigned int process_get_fd_stat(void *arg)
{
	struct fd_num_stat fd_info;

	if (copy_from_user(&fd_info, arg, sizeof(fd_info))) {
		memcheck_err("copy_from_user failed\n");
		return -EFAULT;
	}

	if (fd_info.magic != MEMCHECK_MAGIC) {
		memcheck_err("bad maginc number\n");
		return -EINVAL;
	}
	...
	switch (fd_info.type) {
	case FD_NEXT_TO_USE:
		fd_info.fd_num = get_next_fd_by_pid(fd_info.id);
		break;
	case FD_IN_USE_ALL:
		fd_info.fd_num = get_inuse_fd_by_pid(fd_info.id);
		break;
	case FD_IN_USE_PIPE:
		fd_info.fd_num = get_pipe_fd_by_pid(fd_info.id);
		break;
	default:
		break;
	}
	if (copy_to_user((void *)arg, &fd_info, sizeof(fd_info))) {
		memcheck_err("copy_to_user failed\n");
		return -EFAULT;
	}
...
static int process_do_command(const void *arg)
{
	...
	memcheck_info(
		"IOCTL COMMAND id=%u,type=%u,timestamp=%llu,cmd=%d,full_threshold=%d,rate=%d,mem_name=%s\n",
		cmd.id, cmd.type, cmd.timestamp, cmd.cmd,
		cmd.sample_info.param.full_threshold,
		cmd.sample_info.param.rate, cmd.sample_info.mem_name);
	...
	if (cmd.type &
	    (MTYPE_USER_VSS | MTYPE_KERN_ION | MTYPE_KERN_CMA |
	     MTYPE_KERN_ZSMALLOC | MTYPE_KERN_SKB | MTYPE_KERN_VMALLOC)) {
...
static int process_detail_read(void *arg)
{
	...
	if ((!info.size) || (info.size > MEMCHECK_DETAILINFO_MAXSIZE)) {
		memcheck_err("wrong size=%llu\n", info.size);
		return -EINVAL;
	}
	if (info.type != MTYPE_KERN_SLUB && info.type != MTYPE_KERN_ION &&
	    info.type != MTYPE_KERN_VMALLOC) {
...
static int process_stack_read(void *arg)
{
	...
	if (!(info.type &
	      (MTYPE_USER_PSS | MTYPE_KERN_SLUB | MTYPE_KERN_LSLUB |
	       MTYPE_KERN_VMALLOC | MTYPE_KERN_BUDDY | MTYPE_USER_ION |
	       MTYPE_USER_GPU | MTYPE_USER_PSS_FDTRACK))) {
		memcheck_err("invalid memtype %d\n", info.type);
		return -EINVAL;
	}
...
long memcheck_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case LOGGER_MEMCHECK_GET_FD_STAT:
		ret = process_get_fd_stat((void *)arg);
		break;
