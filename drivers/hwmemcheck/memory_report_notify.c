...
static ssize_t memleak_report_show(struct kobject *kobj,
				   struct kobj_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%u,%d,%s,%u,%u,%lu\n",
			 global_memory_report.mem_type,
			 global_memory_report.pid, global_memory_report.pname,
			 global_memory_report.mem_size,
			 global_memory_report.mem_ratio,
...
void send_memory_report(const struct memory_report_info *leak_proc)
{
	...
	memcpy_s(&global_memory_report, sizeof(global_memory_report), leak_proc,
		 sizeof(global_memory_report));
...
static int memory_report_notify_setup(void)
{
	...
	memory_report_kobj =
		kobject_create_and_add("memory_report", kernel_kobj);
	if (unlikely(!memory_report_kobj)) {
		goto error;
	}
	err = sysfs_create_group(memory_report_kobj,
				 &memory_report_sysfs_attr_group);
	if (unlikely(err)) {
		goto error;
	}
	return err;
