...
static const char *const java_tag[] = {
	"dalvik-alloc space",	     "dalvik-main space",
	"dalvik-large object space", "dalvik-free list large object space",
	"dalvik-non moving space",   "dalvik-zygote space",
};
...
static u64 g_addr_array[][ADDR_NUM] = {
	...
	{0, 0, 0},
	/* MEMCMD_ENABLE */
	{ADDR_JAVA_ENABLE, ADDR_NATIVE_ENABLE, ADDR_MMAP_ENABLE,
	 ADDR_FD_TRACK_ENABLE, ADDR_ION_ENABLE, ADDR_GPU_ENABLE},
	/* MEMCMD_DISABLE */
	{ADDR_JAVA_DISABLE, ADDR_NATIVE_DISABLE, ADDR_MMAP_DISABLE,
	 ADDR_FD_TRACK_DISABLE, ADDR_ION_DISABLE, ADDR_GPU_DISABLE},
	/* MEMCMD_SAVE_LOG */
	{ADDR_JAVA_SAVE, ADDR_NATIVE_SAVE, ADDR_MMAP_SAVE, ADDR_FD_TRACK_SAVE,
	 ADDR_ION_SAVE, ADDR_GPU_SAVE},
	/* MEMCMD_CLEAR_LOG */
	{ADDR_JAVA_CLEAR, ADDR_NATIVE_CLEAR, ADDR_MMAP_CLEAR,
	 ADDR_FD_TRACK_CLEAR, ADDR_ION_CLEAR, ADDR_GPU_CLEAR},
};
					    ...
					    long count)
{
	...
	for (i = 0; i < num_pin; i++) {
		...
		if (i == 0) {
			...
			p = kaddr;
			sz = min((long)PAGE_SIZE,
				 count - page_left - (i - 1) * (long)PAGE_SIZE);
		}
				     ...
				     const char __user *src, long count)
{
	...
	size_t num_pages =
		DIV_ROUND_UP(src_page_offset + count, (long)PAGE_SIZE);

	page = alloc_page_pointers(num_pages);
	if (IS_ERR_OR_NULL(page))
		return PTR_ERR(page);
#if (KERNEL_VERSION(5, 10, 0) <= LINUX_VERSION_CODE)
	num_pin = get_user_pages_remote(remote_mm, src_page_start, num_pages, 0,
					page, NULL, NULL);
#elif (KERNEL_VERSION(4, 14, 0) <= LINUX_VERSION_CODE)
	num_pin = get_user_pages_remote(current, remote_mm, src_page_start,
					num_pages, 0, page, NULL, NULL);
#else
	num_pin = get_user_pages_remote(current, remote_mm, src_page_start,
					num_pages, 0, page, NULL);
#endif
	if (num_pin < 1) {
		kfree(page);
		return 0;
...
enum heap_type memcheck_anon_vma_name(struct vm_area_struct *vma)
{
	...
	unsigned long max_len =
		min((unsigned long)NAME_MAX + 1, (unsigned long)PAGE_SIZE);
	long retcpy;
	...
	retcpy = strncpy_from_remote_user(out_name, vma->vm_mm, name_user,
					  max_len);
	if (retcpy <= 0)
		goto free_name;
#endif
...
static int memcheck_pte_range(pmd_t *pmd, unsigned long addr, unsigned long end,
			      struct mm_walk *walk)
{
	struct vm_area_struct *vma = walk->vma;
	pte_t *pte = NULL;
	...
	if (ptl) {
		memcheck_debug("pmd_trans_huge_lock pmd=%p.\n", pmd);
		spin_unlock(ptl);
		cond_resched();
		return 0;
	}
	...
	if (pmd_trans_unstable(pmd)) {
		memcheck_debug("pmd_trans_unstable pmd=%p.\n", pmd);
		cond_resched();
		return 0;
	}
#endif
...
static int __memcheck_get_mss(struct mm_struct *mm,
...
#if (KERNEL_VERSION(5, 4, 0) <= LINUX_VERSION_CODE)
			walk_page_range(memstat_walk.mm, vma->vm_start,
					vma->vm_end, memstat_walk.ops,
					memstat_walk.private);
#else
...
static int memcheck_get_mss(pid_t pid, struct memsize_stats *mss_total)
{
	...
	__memcheck_get_mss(mm, mss_total);
	put_task_struct(tsk);
	return 0;
}
...
static void memcheck_set_tags(const void *buf, u16 num)
{
	...
	if ((!buf) || num > MAX_TAG_TYPE_NUM) {
		memcheck_err("memtag param illegal or buf is null, num is %d!",
			     num);
		return;
	}
	...
	for (i = 0; i < num; i++) {
		if (mem_stat_ptr->tag_list[i].type == IDX_MMAP) {
			err = memcpy_s(
				g_mmap_tag, sizeof(g_mmap_tag),
				mem_stat_ptr->tag_list[i].tag_body,
...
int memcheck_do_command(const struct track_cmd *cmd)
{
	...
	if ((cmd->type & MTYPE_USER_PSS_NATIVE) &&
	    (cmd->type & MTYPE_USER_PSS_MMAP)) {
	...
	if (p && (task_tgid_vnr(p) == cmd->id)) {
		...
		if (cmd->type & MTYPE_USER_PSS_JAVA) {
			addr = g_addr_array[cmd->cmd][IDX_JAVA];
			if (cmd->cmd == MEMCMD_ENABLE)
				info.si_int =
					(cmd->sample_info.param.full_threshold
					 << SIGNO_VALUE_SHIFT) |
					cmd->sample_info.param.rate;
		}
		if (cmd->type & MTYPE_USER_PSS_NATIVE) {
		...
		if (cmd->type & MTYPE_USER_PSS_MMAP)
			addr |= g_addr_array[cmd->cmd][IDX_MMAP];

		if (cmd->type & MTYPE_USER_PSS_FDTRACK) {
		...
		if (cmd->type & MTYPE_USER_ION)
			addr |= g_addr_array[cmd->cmd][IDX_ION];

		if (cmd->type & MTYPE_USER_GPU)
			addr |= g_addr_array[cmd->cmd][IDX_GPU];

		info.si_addr = (void *)addr;
		if ((cmd->type & MTYPE_USER_PSS_JAVA) ||
		    (cmd->type & MTYPE_USER_PSS_NATIVE) ||
		    (cmd->type & MTYPE_USER_PSS_MMAP) ||
		    (cmd->type & MTYPE_USER_PSS_FDTRACK) ||
		    (cmd->type & MTYPE_USER_ION) ||
		    (cmd->type & MTYPE_USER_GPU))
