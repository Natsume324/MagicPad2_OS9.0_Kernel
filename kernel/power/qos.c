...
#ifdef CONFIG_HONOR_FREQ_STACK_ENABLE
...
#define MIN_INTERVAL 5000000000
#define MIN_FREQ_THRESHOLD 1600000
#define MAX_FREQ_THRESHOLD 1000000
#define MAX_NUM 10
...
#ifdef CONFIG_HONOR_FREQ_STACK_ENABLE
static int freq_stack_enable __read_mostly;
static int set_min_thershold __read_mostly;
static int set_max_thershold __read_mostly;
static int min_period __read_mostly;
#endif
...
#ifdef CONFIG_HONOR_FREQ_STACK_ENABLE
	if (freq_stack_enable > 0) {
		dump_stack();
		pr_err("uniperf freq_stack pm_qos_update_target %d %d %d\n", action, prev_value, curr_value);
	}
#endif
...
int max_count = 0;
int min_count = 0;
char node_list[BUFFER_LEN];
char node_info[BUFFER_LEN];
s32 max_freq[MAX_NUM] = {0};
s32 min_freq[MAX_NUM] = {0};
#endif
			  ...
			  enum pm_qos_req_action action, s32 value)
{
	int ret;
#ifdef CONFIG_HONOR_FREQ_STACK_ENABLE
	struct freq_qos_request *iter;
	int count;
	int i = 0;
	s32 decision_value = -1;
	struct pm_qos_constraints *c;
	bool node_list_empty = true;
	...
	case FREQ_QOS_MIN:
		...
		c = &req->qos->min_freq;
		req->tid = current->pid;
		strlcpy(req->comm, current->comm, sizeof(req->comm));

		if (req->update_freq != value) {
			req->update_freq = value;
		}
		if (jiffies64_to_nsecs(get_jiffies_64()) - last_time_min >= (min_period > 0 ? min_period : MIN_INTERVAL)) {
			bool print_plist = true;
			last_time_min = jiffies64_to_nsecs(get_jiffies_64());
			plist_for_each_entry(iter, &c->list, pnode) {
				if (iter->update_freq > decision_value || decision_value < 0) {
					decision_value = iter->update_freq;
				}
			}
			min_freq[min_count] = decision_value;
			min_count = (min_count + 1) % MAX_NUM;
			for (int j = 0; j < MAX_NUM; j++) {
				if (min_freq[j] < (set_min_thershold > 0 ? set_min_thershold : MIN_FREQ_THRESHOLD) && min_freq[j] > 0) {
					print_plist = false;
				}
			}
			if (print_plist) {
				plist_for_each_entry(iter, &c->list, pnode) {
					i++;
					count = snprintf_s(node_info, sizeof(node_info), sizeof(node_info) - 1,
						"[tid:%d - comm:%s - min_freq:%d], ", iter->tid, iter->comm, iter->update_freq);
					if (count < 0) {
						pr_err("freq_qos_apply node_info error\n");
					}
					strlcat(node_list, node_info, BUFFER_LEN);
					node_list_empty = false;
					if (i % 5 == 0) {
						pr_err("changing, qos min_freq node_list = %s\n", node_list);
						memset(node_list , 0, sizeof(node_list));
						node_list_empty = true;
					}
				}
				if (!node_list_empty) {
					pr_err("changing, qos min_freq node_list = %s\n", node_list);
		...
		if (freq_stack_enable == 1) {
			pr_err("uniperf freq_stack FREQ_QOS_MIN %d value %d\n", FREQ_QOS_MIN, value);
		}
#endif
	...
	case FREQ_QOS_MAX:
		...
		if (jiffies64_to_nsecs(get_jiffies_64()) - last_time_max >= (min_period > 0 ? min_period : MIN_INTERVAL)) {
			bool print_plist = true;
			last_time_max = jiffies64_to_nsecs(get_jiffies_64());
			plist_for_each_entry(iter, &c->list, pnode) {
				if (iter->update_freq < decision_value || decision_value < 0) {
					decision_value = iter->update_freq;
				}
			}
			max_freq[max_count] = decision_value;
			max_count = (max_count + 1) % MAX_NUM;
			for (int j = 0; j < MAX_NUM; j++) {
				if (max_freq[j] >= (set_max_thershold > 0 ? set_max_thershold : MAX_FREQ_THRESHOLD)) {
					print_plist = false;
				}
			}
			if (print_plist) {
				plist_for_each_entry(iter, &c->list, pnode) {
					i++;
					count = snprintf_s(node_info, sizeof(node_info), sizeof(node_info) - 1,
						"[tid:%d - comm:%s - max_freq:%d], ", iter->tid, iter->comm, iter->update_freq);
					if (count < 0) {
						pr_err("freq_qos_apply node_info error\n");
					}
					strlcat(node_list, node_info, BUFFER_LEN);
					node_list_empty = false;
					if (i % 5 == 0) {
						pr_err("changing, qos max_freq node_list = %s\n", node_list);
						memset(node_list , 0, sizeof(node_list));
						node_list_empty = true;
					}
				}
		...
		if (freq_stack_enable == 2) {
			pr_err("uniperf freq_stack FREQ_QOS_MAX %d value %d\n", FREQ_QOS_MAX, value);
		}
#endif
...
#ifdef CONFIG_HONOR_FREQ_STACK_ENABLE
module_param(freq_stack_enable, int, 0600);
module_param(set_min_thershold, int, 0600);
module_param(set_max_thershold, int, 0600);
module_param(min_period, int, 0600);
#endif
