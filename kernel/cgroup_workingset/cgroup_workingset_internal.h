...
#define FILE_OFFSET_BITS                                                  \
	(8 * sizeof(uint32_t) - FILE_SEQNUM_BITS - PAGE_RANGE_HEAD_BITS - \
	 PAGE_MAJOR_BITS - PAGE_STAGE_NUM_BITS)
#define PAGE_STAGE_NUM_SHIFT                                          \
	(FILE_SEQNUM_BITS + FILE_OFFSET_BITS + PAGE_RANGE_HEAD_BITS + \
	 PAGE_MAJOR_BITS)
#define PAGE_STAGE_NUM_MASK ((1U << PAGE_STAGE_NUM_BITS) - 1)
...
#define PAGE_RANGE_HEAD_BIT_MASK (PAGE_RANGE_HEAD_MASK << PAGE_RANGE_HEAD_SHIFT)
#define FILE_IDX_AND_OFFSET_MASK ((1U << PAGE_RANGE_HEAD_SHIFT) - 1)
...
#define COLLECTOR_REMAIN_CACHE_LOW_WATER (COLLECTOR_BATCH_COUNT << 4)
...
enum ws_cgroup_states {
	...
	E_CGROUP_STATE_MONITOR_INWORKING =
		(E_CGROUP_STATE_ONLINE | E_CGROUP_STATE_MONITORING |
		 E_MONITOR_STATE_INWORKING),
