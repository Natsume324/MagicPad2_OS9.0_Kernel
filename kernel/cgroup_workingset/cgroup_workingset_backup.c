...
static void wscg_record_debug_show(struct seq_file *m,
				   struct s_ws_record *record)
{
	...
	for (i = 0; i < record->data.pageseq_cnt; i++) {
		file_idx = (cacheseq[i] >> FILE_OFFSET_BITS) &
			   MAX_TOUCHED_FILES_COUNT;
		if ((file_idx >= record->data.file_cnt) ||
		    !path_nodes[file_idx].path)
			continue;
		stage = (cacheseq[i] >> PAGE_STAGE_NUM_SHIFT) &
			PAGE_STAGE_NUM_MASK;
...
static void __init workingset_init_debugfs(void)
{
	...
	debugfs_create_file("records", 0444, g_record_list_dentry, NULL,
			    &wscg_records_debug_fops);
#endif
}
...
void __init workingset_record_list_init(void)
{
...
void __exit workingset_record_list_deinit(void)
{
	struct s_ws_record *record;
...
unsigned int workingset_crc32c(unsigned int crc, const void *address,
			       unsigned int length)
{
 ...
 */
static int
workingset_get_record_header(struct file *filp, size_t offset,
			     struct s_ws_backup_record_header *header)
{
	...
	if ((header->magic != WORKINGSET_RECORD_MAGIC) ||
		...
		pr_err("%s line %d: magic=%u, headercrc=%u\n", __func__,
		       __LINE__, header->magic, header->header_crc);
		ret = -EIO;
	}
...
static int workingset_record_writeback_data(struct file *filp, const void *data,
					    unsigned int size,
	...
	unsigned int *pplayload_length, unsigned int *pchecksum)
{
	...
	*pchecksum = 0;
	...
	for (idx = 0; idx < record->data.file_cnt; idx++) {
		...
		ret = workingset_record_writeback_data(
			filp, record->data.file_array[idx].path, length,
			pchecksum, &pos);
		if (ret)
			return ret;
	}
...
static bool workingset_record_writeback_rcrdlocked(struct s_ws_record *record,
						   struct file *filp)
{
	...
	if (record->state & E_RECORD_STATE_UPDATE_HEADER_ONLY) {
		...
		if (workingset_record_writeback_playload_rcrdlocked(
			    filp, record, &header.playload_length,
	...
	crc_val = workingset_crc32c(0, &header.record_version,
				    sizeof(header) - offset);
	if (!crc_val) {
...
static bool workingset_backup_record_rcrdlocked(struct s_ws_record *record)
{
	...
	ws_dbg("%s: writeback %s record data to %s\n", __func__,
	       record->owner.name, record->owner.record_path);
	filp = filp_open(record->owner.record_path, O_LARGEFILE | O_RDWR,
			 S_IRWUSR);
 ...
 */
static int
workingset_record_realloc_ownerbuffer_if_need(struct s_ws_owner *scanned,
...
void workingset_writeback_last_record_if_need(void)
{
	...
	if (g_record_cnt >= g_max_records_count) {
		spin_lock(&g_record_list_lock);
		record = list_empty(&g_record_list) ?
				 NULL :
...
static void *workingset_get_playload_addr_rcrdlocked(
	...
	bool is_exist, struct page **page_array, unsigned int playload_pages)
{
	void *playload;
	...
	if (memset_s(playload, playload_pages * PAGE_SIZE, 0,
		     playload_pages * PAGE_SIZE)) {
		vunmap(playload);
		return NULL;
	}
	return playload;
...
struct s_ws_record *workingset_get_available_record(void)
{
	...
	list_for_each_prev(pos, &g_record_list) {
		record = container_of(pos, struct s_ws_record, list);
		if (!(record->state &
		      (E_RECORD_STATE_DIRTY | E_RECORD_STATE_PREREADING)))
			break;
	}
...
static int
workingset_get_record_header_wrapper(struct file *filp,
				     struct s_ws_backup_record_header *header)
{
	int ret;
	size_t offset;
 ...
 */
static struct page **
workingset_prepare_record_pages(struct s_ws_data *data,
				unsigned int playload_pages, unsigned int *pidx)
{
	unsigned int idx = 0;
...
int workingset_prepare_record_space_wsrcrdlocked(const struct s_ws_owner *owner,
						 struct s_ws_record *record,
						 bool is_exist,
						 ...
						 void **pplayload)
{
	...
	if (data->array_page_cnt < playload_pages) {
		...
		playload = workingset_get_playload_addr_rcrdlocked(
			owner, record, is_exist, page_array, playload_pages);
		if (!playload) {
			while (idx-- > data->array_page_cnt)
				__free_page(page_array[idx]);
			kfree(page_array);
		...
		playload = workingset_get_playload_addr_rcrdlocked(
			owner, record, is_exist, data->page_array,
			playload_pages);
		if (!playload)
			return -ENOSPC;
...
static int
workingset_get_record_playload(const struct s_ws_owner *owner,
			       ...
			       void *playload)
{
	...
	if (header->playload_checksum !=
	    workingset_crc32c(0, playload, header->playload_length)) {
	...
	ret = memcpy_s(record->owner.record_path, len, owner->record_path, len);
	if (ret) {
		pr_err("%s Line%d,ret=%d\n", __func__, __LINE__, ret);
		return ret;
	}
...
static void
init_record_with_data(const struct s_ws_owner *owner,
		      const struct s_ws_backup_record_header *header,
		      struct s_ws_record *record, void *playload)
{
	...
	for (idx = 0, len = 0; idx < header->file_cnt; idx++) {
		if (!data->file_array[idx].path)
			continue;
		data->file_array[idx].path = playload + pathnode_size + len;
...
workingset_get_record_from_backup(const struct s_ws_owner *owner)
{
	int ret;
	struct file *filp = NULL;
	struct s_ws_record *record = NULL;
	...
	ret = workingset_prepare_record_space_wsrcrdlocked(
		owner, record, false, header.playload_length, &playload);
	if (ret)
		goto alloc_space_fail;

	ret = workingset_get_record_playload(owner, &header, filp, record,
					     playload);
	if (ret)
		goto alloc_space_fail;
 ...
 */
struct s_ws_record *
workingset_get_existed_record_wslocked(const struct s_ws_owner *owner,
				       bool onlycache)
{
	struct s_ws_record *record = NULL;
	struct list_head *pos = NULL;
