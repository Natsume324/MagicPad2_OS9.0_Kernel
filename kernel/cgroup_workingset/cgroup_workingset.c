...
static const char *moniter_states[E_MONITOR_STATE_MAX] = {
	"OUTOFWORK", "INWORKING", "PAUSED", "STOP",
	"ABORT",     "PREREAD",	  "BACKUP", "CLEARYOUNG"};
					      ...
					      struct shrink_control *sc)
{
	...
	struct s_workingset *ws = shrinker->private_data;
#else
...
{
#ifdef CONFIG_SHRINKER_LOCKLESS_OPT
	ws->shrinker = shrinker_alloc(0, "workingset_shrinker");
	if (!ws->shrinker) {
		ws_dbg("%s: Allocate metadata cache shrinker failed\n",
		       __func__);
		return -ENOMEM;
	}
	...
	ws->shrinker->private_data = ws;
	shrinker_register(ws->shrinker);
...
static inline struct s_workingset *
css_workingset(struct cgroup_subsys_state *css)
{
...
static int clear_pte_young_range(pmd_t *pmd, unsigned long addr,
...
#else
		walk_page_range(vma->vm_start, vma->vm_end, &clear_young_walk);
#endif
	}
	/*
...
static ssize_t workingset_state_write(struct kernfs_open_file *of, char *buf,
...
static void init_owner(struct s_workingset *ws, unsigned int uid, int pid,
		       char *owner_name, char *record_path)
{
	mutex_lock(&ws->mutex);
	ws->owner.uid = uid;
	ws->owner.pid = pid;
...
static ssize_t workingset_data_write(struct kernfs_open_file *of, char *buf,
				     size_t nbytes, loff_t off)
{
	int ret;
	struct cgroup_subsys_state *css = NULL;
...
static int workingset_data_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "RecordState:%s\n",
		   !record ? "none" :
			     (record->need_update ? "older" : "uptodate"));
	mutex_unlock(&ws->mutex);

	return 0;
}

static ssize_t workingset_clear_record_write(struct kernfs_open_file *of,
...
static int workingset_cachepage_info_init(struct file *file, pgoff_t offset,
					  unsigned int count, bool is_major,
					  struct s_cachepage_info *info)
{
	if (unlikely(!file) || unlikely(offset > MAX_TOUCHED_FILE_OFFSET))
		return -EINVAL;
...
void workingset_pagecache_record(struct file *file, pgoff_t offset,
				 unsigned int count, bool is_pagefault)
{
...
#endif
	if (workingset_cachepage_info_init(file, offset, count, is_major,
					   &info))
		return;
	...
	mmap_only = !is_major && (g_collector->monitor->state ==
...
static int __init cgroup_workingset_init(void)
{
	...
	if (COLLECTOR_CACHE_SIZE % sizeof(struct s_cachepage_info)) {
		pr_err("%s, size of cache is not aligned with %lu\n", __func__,
		       sizeof(struct s_cachepage_info));
		ret = -EINVAL;
		goto out;
	}
	...
	g_collector->collector_thread =
		kthread_run(workingset_collect_kworkthread, g_collector,
			    "workingset:collector");
