...
static const char *const g_selfread_dir[] = {"/data/user/", "/data/data/",
					     "/data/user_de/", NULL};

void workingset_preread_qos_wsrcrdlocked(struct s_workingset *ws,
...
static int workingset_page_cache_read(struct s_readpages_control *rpc)
{
	...
	do {
		...
		if (ret == 0)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
			ret = mapping->a_ops->read_folio(rpc->filp,
							 page_folio(page));
#else
...
static bool workingset_adjust_page_lru(struct page *page)
{
	bool adjusted = false;
	struct folio *folio;
#if (defined(CONFIG_HONOR_MEMLOCK_STATS) &&           \
	...
	if (!PageUnevictable(page) &&
       ...
       KERNEL_VERSION(5, 15, 0) > LINUX_VERSION_CODE)
			...
			if (PageLRU(page) && !PageProtect(page) &&
			    !PageSwapBacked(page) && !PageUnevictable(page)) {
			...
			if (PageLRU(page) && !PageSwapBacked(page) &&
				...
				list_move(
					&page->lru,
					&lruvec->lists[folio_lru_list(folio)]);
#else
				list_move(&page->lru,
...
static int workingset_read_pages(struct address_space *mapping,
				 struct file *filp, struct list_head *pages,
...
static int workingset_page_cache_range_read(struct s_readpages_control *rpc)
{
	...
	pgoff_t end_index =
		min(rpc->offset + rpc->nr_to_read - 1, page_count - 1);
	pgoff_t begin_index;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
	...
	if (unlikely(!rpc->mapping->a_ops->read_folio &&
		     !rpc->mapping->a_ops->readahead)) {
		pr_err("%s,a_ops no readahead\n", __func__);
		return 0;
	}
	...
	while (index <= end_index) {
		...
		nr_pages = filemap_get_folios(rpc->mapping, &index, end_index,
					      &fbatch);
#else
		pagevec_init(&pvec);
		nr_pages = pagevec_lookup_range(&pvec, rpc->mapping, &index,
						end_index);
#endif
		if (!nr_pages) {
			workingset_readahead(rpc->mapping, rpc->filp,
					     begin_index, index - begin_index);
...
static void
workingset_prereader_open_all_files_rcrdlocked(struct s_ws_record *record,
					       struct file **filpp)
{
	int flags = O_RDONLY;
	unsigned int idx;
	...
	while (next_loop < data->file_cnt) {
		...
		for (idx = next_loop, next_loop = data->file_cnt;
			...
			filpp[idx] =
				filp_open(data->file_array[idx].path, flags, 0);

			if (!IS_ERR_OR_NULL(filpp[idx])) {
				if ((data->file_array[idx].ino !=
				     filpp[idx]->f_inode->i_ino) &&
				    (data->file_array[idx].file_size >=
				     NEED_UPDATE_FILE_SIZE_THRESOLD)) {
...
static struct file **
workingset_prereader_alloc_filepp_rcrdlocked(struct s_ws_record *record)
{
	unsigned int idx;
...
static bool workingset_filepage_is_need_skip_read(struct s_read_control *rc,
						  unsigned int idx,
						  unsigned int stage,
						  bool read_major,
						  unsigned int *pfile_idx)
{
	bool ret = false;
	bool is_major;
...
static int workingset_read_filepage_looper(struct s_read_control *rc,
					   unsigned int stage_begin,
...
static void check_cachemiss_threshold_rcrdlocked(struct s_ws_record *record,
						 ...
						 struct file **filpp)
{
	...
	struct s_read_control rc = {.data = data,
				    .filpp = filpp,
	 ...
	 */
	while (1) {
		if (!workingset_read_filepage_looper(&rc, stage_begin, stage,
						     read_major)) {
			if (read_major) {
				read_major = false;
				continue;
				...
				ws_dbg("%s %s, stage %u prsnt %u,mv %u,rd %u\n",
				       ...
				       rc.present_pages_cnt, rc.move_lru_cnt,
				       rc.read_pages_cnt);
				read_major = true;
		 ...
		 */
		if (g_selfread_state != E_SELFREAD_DOING)
			check_cachemiss_threshold_rcrdlocked(
				record, rc.present_pages_cnt,
				rc.read_pages_cnt);
		break;
	}

	ws_dbg("%s %s,%u fls,%u sqs,%u pgs,prsnt %u,mv %u,rd %u\n", __func__,
	       record->owner.name, data->file_cnt, data->pageseq_cnt,
	       data->page_sum, rc.present_pages_cnt, rc.move_lru_cnt,
	       rc.read_pages_cnt);
}
...
void workingset_do_preread_work_rcrdlocked(struct s_ws_record *record)
{
	...
	if (!data->file_cnt || !data->pageseq_cnt || !data->file_array ||
	    !data->cacheseq)
		return;
