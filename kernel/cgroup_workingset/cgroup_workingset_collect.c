...
static const char *const g_excluded_dir[] = {"/dev/", "/bin/", "/etc/selinux/",
					     NULL};

static inline unsigned int create_handle(unsigned int file_num,
...
static void
workingset_range_rb_change_to_front(struct s_pagecache_info **old_entry,
...
static void workingset_merge_ranges(struct rb_root *root,
				    struct s_pagecache_info **pp_entry,
				    struct s_pagecache_info **pp_merged_entry,
				    bool probe, int *delta)
{
 ...
 */
static int workingset_range_stick_flags(struct s_range *range,
					...
					int major, int stage)
{
	...
	delta -= (range->start & PAGE_RANGE_HEAD_BIT_MASK) ?
			 MULTI_PAGES_SEQ_WORDS :
			 SINGLE_PAGE_SEQ_WORDS;
	range->start = start | ((major & PAGE_MAJOR_MASK) << PAGE_MAJOR_SHIFT) |
		       ((stage & PAGE_STAGE_NUM_MASK) << PAGE_STAGE_NUM_SHIFT);
	range->end = end | ((major & PAGE_MAJOR_MASK) << PAGE_MAJOR_SHIFT) |
		     ((stage & PAGE_STAGE_NUM_MASK) << PAGE_STAGE_NUM_SHIFT);
	if (end - start > 1) {
...
static void workingset_rbtree_probe_left(struct s_probe_context *pc,
					 unsigned int cur_start)
{
	if (!pc->merged_entry || pc->probe_left) {
		...
		if (pc->start < cur_start) {
			 ...
			 */
			pc->parent =
				rb_prev_middleorder(&pc->cur_entry->rbnode);
			workingset_merge_ranges(pc->root, &pc->cur_entry,
						&pc->merged_entry,
...
static void workingset_rbtree_probe_right(struct s_probe_context *pc,
					  unsigned int cur_end)
{
	if (!pc->merged_entry || (pc->probe_right && !pc->probe_left)) {
		if (pc->look_side && pc->merged_entry) {
			...
			pc->look_side = false;
			pc->parent =
				rb_next_middleorder(&pc->merged_entry->rbnode);
			if (!pc->parent)
...
static unsigned int workingset_get_repeate_pages_count(unsigned int start,
						       unsigned int end,
						       unsigned int cur_start,
						       unsigned int cur_end)
{
...
static void workingset_rb_insert_looper(struct s_probe_context *pc)
{
	...
	while (*pc->link) {
		pc->parent = *pc->link;
		pc->cur_entry =
			rb_entry(pc->parent, struct s_pagecache_info, rbnode);
 ...
 */
static bool workingset_range_rb_insert(struct rb_root *root,
				       struct s_pagecache_info *entry,
				       struct s_cachepage_info *info,
				       unsigned int *repeat_cnt, int *seq_delta)
{
	...
	if (pc.merged_entry) {
		pc.delta += workingset_range_stick_flags(
			&pc.merged_entry->offset_range, pc.start, pc.end,
			pc.major, pc.stage);
	...
	if (pc.stage) {
		entry->offset_range.start |= (pc.stage & PAGE_STAGE_NUM_MASK)
					     << PAGE_STAGE_NUM_SHIFT;
		entry->offset_range.end |= (pc.stage & PAGE_STAGE_NUM_MASK)
					   << PAGE_STAGE_NUM_SHIFT;
	}
 ...
 */
static struct s_pagecache_info *
workingset_pagecache_info_cache_alloc_wslocked(struct s_workingset *ws)
{
...
static unsigned int get_file_path_and_hashcode(struct file *file, char *buf,
...
static struct s_file_info *workingset_alloc_fileinfo(unsigned int path_len,
						     gfp_t gfp_mask)
{
	...
	const unsigned int hashcode, const unsigned int path_len)
{
	...
	ret = strncpy_s(info->path_node.path, path_len + 1, filepath, path_len);
	if (ret) {
...
static int get_file_with_same_inode(const struct s_workingset *ws,
				    struct file *file,
...
static int get_file_with_same_path(struct s_workingset *ws,
				   const struct s_find_file_by_path *ffbp,
				   struct s_file_info **file_info,
				   bool *is_existed, int *existed_files)
{
	int seq_num = 0;
	struct list_head *pos = NULL;
	...
	list_for_each(pos, &ws->file_list) {
		...
		if ((info->path_node.hashcode == ffbp->hashcode) &&
			...
			struct s_filp_list *filp_list =
				kzalloc(sizeof(struct s_filp_list), GFP_NOFS);
	...
	struct s_file_info **file_info, bool *is_existed)
{
	...
	ret = get_file_with_same_path(ws, &ffbp, file_info, is_existed,
				      &seq_num);
	if (ret >= 0)
		return ret;
	...
	*file_info = workingset_record_new_file_tree(ws, file, filepath,
						     hashcode, path_len);
	if (*file_info == NULL)
		return -EPERM;
...
static void workingset_rbtree_insert_node(struct s_workingset *ws,
					  struct s_file_info *file_info,
					  struct s_pagecache_info *page_cache,
					  struct s_cachepage_info *info)
{
	unsigned int repeat_count = 0;
	int page_count_delta = 0;

	if (workingset_range_rb_insert(&file_info->rbroot, page_cache, info,
				       &repeat_count, &page_count_delta)) {
						  ...
						  struct s_workingset *ws)
{
	...
	file_idx = workingset_record_fileinfo_if_need_wslocked(
		ws, info->filp, &file_info, &is_existed_file);
	if (file_idx < 0) {
		ret = file_idx;
		goto done;
	}
	...
	page_cache->offset_range.start = create_handle(file_idx, info->offset);
	page_cache->offset_range.end =
...
static unsigned int
workingset_collector_dequeue_buffer_locked(struct s_ws_collector *collector,
...
/*lint -e454*/
static void
workingset_collector_do_collect_locked(struct s_ws_collector *collector)
{
		...
		for (idx = 0; idx < buffer_pos;
			...
			if (ws) {
				...
				workingset_dealwith_pagecache_wslocked(
					(struct s_cachepage_info *)(buffer +
								    idx),
					ws);
				mutex_unlock(&ws->mutex);
...
static int workingset_collector_fill_record_cacheseq_wsrcrdlocked(
	...
	struct s_pagecache_info **pagecache_array, unsigned int page_idx,
...
static int
workingset_collector_fill_record_filenode_wsrcrdlocked(struct s_ws_data *data,
	...
	struct s_workingset *ws, struct s_ws_record *record, bool is_exist)
{
	...
	playload_size =
		pathnode_size + sizeof(unsigned int) * ws->pageseq_count;
	ret = workingset_prepare_record_space_wsrcrdlocked(
		&ws->owner, record, is_exist, playload_size, &playload);
	if (!ret)
...
static int
workingset_collector_fill_record_owner_wsrcrdlocked(struct s_workingset *ws,
	...
	struct s_workingset *ws, struct s_ws_record *record, bool is_exist)
{
	...
	ret = workingset_collector_fill_record_filenode_wsrcrdlocked(
		data, &ws->file_list);
	if (ret)
		goto fill_data_fail;
...
static struct s_ws_record *
workingset_get_record_wslocked(struct s_workingset *ws, bool *is_exist)
{
	struct s_ws_record *record;
...
static void workingset_collector_do_record_wslocked(struct s_workingset *ws,
						    unsigned long discard_count)
{
	bool is_exist = false;
	struct s_ws_record *record =
		workingset_get_record_wslocked(ws, &is_exist);
	...
	if (!workingset_collector_read_data_wsrcrdlocked(ws, record,
							 is_exist)) {
		if (ws->clear_young)
