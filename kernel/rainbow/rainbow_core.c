...
static void rb_header_init(void)
{
	...
	rb_header_dts_addr =
		(unsigned long)of_translate_address(rb_dev_node, rb_dts_basep);
	...
	RB_LOG_ERR("phy addr %lu, size is %lu\n", rb_header_dts_addr,
		   rb_header_dts_size);
#ifdef CONFIG_ARM
	...
	if (g_rb_header == NULL)
		...
		RB_LOG_ERR("addr %lu ioremap to %p, size is %lu\n",
			   rb_header_dts_addr, (void *)g_rb_header,
			   rb_header_dts_size);
	err = memset_s(g_rb_header, sizeof(struct rb_header), 0,
		       sizeof(struct rb_header));
	if (err != EOK) {
		RB_LOG_ERR("memset_s g_rb_header fail\n");
	}
}
...
void *rb_bl_log_get(unsigned int *size)
{
	...
	if (g_rb_header == NULL || size == NULL) {
		RB_LOG_ERR(
			"rb_bl_log_get: addr or size or g_rb_header is null\n");
		return NULL;
	}
	RB_LOG_DEBUG("g_rb_header:%p\n", (void *)g_rb_header);
...
static void rb_sreason_str_set(char *sreason_info)
{
	...
	if (sreason_info_size >= RB_SREASON_STR_MAX) {
		memcpy_toio(reason_node_local->sreason_str, sreason_info,
			    RB_SREASON_STR_MAX);
...
#else
	strlcpy(reason_node_local->sreason_str, sreason_info,
		RB_SREASON_STR_MAX);
#endif
...
static void rb_attach_info_str_set(char *attach_info)
{
	...
	if (attach_info_size >= RB_SREASON_STR_MAX) {
		memcpy_toio(reason_node_local->attach_info, attach_info,
			    RB_SREASON_STR_MAX);
...
#else
	strlcpy(reason_node_local->attach_info, attach_info,
		RB_SREASON_STR_MAX);
#endif
...
void rb_attach_info_set(const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	err = vsnprintf_s(attach_info_buffer, RB_SREASON_STR_MAX,
			  RB_SREASON_STR_MAX - 1, fmt, ap);
	if (err < 0) {
		RB_LOG_ERR("vsnprintf_s attach_info_buffer fail\n");
	}
	va_end(ap);
	if (err < 0) {
		RB_LOG_ERR("fail\n");
...
void rb_kallsyms_set(const char *fmt, unsigned long addr)
{
	...
	err = snprintf_s(attach_info_buffer, RB_SREASON_STR_MAX,
			 RB_SREASON_STR_MAX - 1, fmt, kallsyms_buffer);
	if (err < 0) {
		RB_LOG_ERR("fail\n");
		return;
	}
...
static int rb_apanic_handler(struct notifier_block *this, unsigned long event,
			     void *ptr)
{
	struct rb_reason_header *reason_node_local = NULL;
