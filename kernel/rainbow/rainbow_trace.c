...
unsigned int g_rb_t_event_len[TR_MAX] = { RB_TRACE_IRQ_INFO_SIZE,
...
static void rb_trace_percup_buffer_write(struct rb_trace_pinfo *q, u8 *element)
{
	int rear;
	...
	rear = q->rear++;
	memcpy((void *)&q->data[(long)rear * q->field_len], (void *)element,
	       q->field_len * sizeof(u8));
}
...
void rb_trace_irq_write(unsigned int dir, unsigned int new_vec)
{
	...
	cpu_pinfo =
		(struct rb_trace_pinfo *)g_rb_trace_buf[TR_IRQ].percpu_addr[cpu];
...
void rb_trace_task_write(void *next_task)
{
	...
	cpu_pinfo = (struct rb_trace_pinfo *)g_rb_trace_buf[TR_TASK]
			    .percpu_addr[cpu];
	rb_trace_percup_buffer_write(cpu_pinfo, (u8 *)&info);
}
...
void rb_trace_init(void *paddr_start, void *vaddr_start)
{
	...
	g_rb_trace_mem[TR_IRQ].paddr =
		(unsigned long)paddr_start + RB_TRACE_IRQ_OFFSET;
	g_rb_trace_mem[TR_IRQ].size = RB_TRACE_SECTION_SIZE;
	g_rb_trace_mem[TR_IRQ].vaddr =
		(unsigned long)vaddr_start + RB_TRACE_IRQ_OFFSET;
	RB_LOG_ERR("%x,%x\n", g_rb_trace_mem[TR_IRQ].paddr,
		   g_rb_trace_mem[TR_IRQ].vaddr);
	...
	g_rb_trace_mem[TR_TASK].paddr =
		(unsigned long)paddr_start + RB_TRACE_TASK_OFFSET;
	g_rb_trace_mem[TR_TASK].size = RB_TRACE_SECTION_SIZE;
	g_rb_trace_mem[TR_TASK].vaddr =
		(unsigned long)vaddr_start + RB_TRACE_TASK_OFFSET;
	RB_LOG_ERR("%x,%x\n", g_rb_trace_mem[TR_TASK].paddr,
		   g_rb_trace_mem[TR_TASK].vaddr);
