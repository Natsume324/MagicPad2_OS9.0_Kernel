...
int set_rtg_sched(struct task_struct *task, bool is_rtg, int grp_id, int prio)
{
	...
	if (is_rt_task && is_rtg && ((prio < 0) || (prio > MAX_RT_PRIO - 1)))
	...
	if (is_rtg) {
		...
		if (strncmp("ndroid.launcher", task->comm, TASK_COMM_LEN) ==
			...
			if (strncmp("RenderThread", task->comm,
				    TASK_COMM_LEN) == 0) {
				is_launcher_render = 1;
				trace_rtg_frame_sched("launcher_renderThread",
						      task->pid);
			} else {
				if (strncmp("hwuiTask0", task->comm,
					    TASK_COMM_LEN) == 0 ||
				    strncmp("hwuiTask1", task->comm,
					    TASK_COMM_LEN) == 0) {
					is_launcher = 1;
					trace_rtg_frame_sched(
						"launcher_hwuiTask", task->pid);
				}
			}
		}
...
void set_frame_rtg_thread(struct task_struct *task, bool is_rtg)
{
	...
	if (set_rtg_sched(task, is_rtg, DEFAULT_RT_FRAME_ID, DEFAULT_RT_PRIO) <
	    0) {
		task->rtg_depth = depth;
		return;
	}
