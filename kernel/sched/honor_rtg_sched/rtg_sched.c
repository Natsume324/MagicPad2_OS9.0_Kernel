...
#include "include/rtg_pseudo.h"
	...
	defined(CONFIG_HONOR_SCHED_CLUSTER)
...
#if defined(CONFIG_HONOR_RTG_NORMALIZED_UTIL) || \
	defined(CONFIG_HONOR_RTG_UCLAMP_UTIL)
...
_group_cpu_time(struct walt_related_thread_group *grp, int cpu)
{
...
group_update_cpu_time(struct rq *rq, struct walt_related_thread_group *grp)
{
...
bool group_migrate_task(struct task_struct *p, struct rq *src_rq,
			struct rq *dest_rq)
{
	struct group_cpu_time *cpu_time = NULL;
...
void sched_update_group_load(struct rq *rq)
{
	...
	for_each_related_thread_group(grp) {
		/* Protected by rq_lock */
		struct group_cpu_time *cpu_time = _group_cpu_time(grp, cpu);
		group_sync_window_start(rq, cpu_time);
...
static inline bool valid_normalized_util(struct walt_related_thread_group *grp)
{
	...
	trace_sched_rtg_valid_normalized_util(grp->id, grp->nr_running,
					      &rtg_cpus, valid);
	return valid;
}
...
static struct walt_sched_cluster *
best_cluster(struct walt_related_thread_group *grp)
{
	struct walt_sched_cluster *cluster = NULL;
...
int find_rtg_cpu(struct task_struct *p, struct cpumask *preferred_cpus)
{
	...
	for_each_cpu(i, &search_cpus) {
		...
		if (idle_cpu(i) ||
			...
			if (idle_backup_cpu == -1 ||
			    capacity_orig_of(i) >
				    capacity_orig_of(idle_backup_cpu))
				idle_backup_cpu = i;
	...
	if (idle_backup_cpu != -1) {
		trace_find_rtg_cpu(p, preferred_cpus, "idle_backup",
				   idle_backup_cpu);
					 ...
					 unsigned int new_util)
{
	...
	need_update_freq = group_should_update_freq(
		grp, cpu, FRAME_NORMAL_UPDATE, ktime_get_ns());
...
void sched_set_group_min_util(struct walt_related_thread_group *grp,
			      int min_util)
{
	struct task_struct *p = NULL;
	unsigned long flags;
...
unsigned int honor_freq_to_util(unsigned int cpu, unsigned int freq)
{
	...
	util = arch_scale_cpu_capacity(cpu) * (unsigned long)freq /
	       cpu_rq(cpu)->wrq.cluster->max_possible_freq;
...
static inline bool cluster_should_invalid_freq(u64 now)
{
	return (now - global->last_freq_update_time >=
		RTG_FREQ_INVALID_INTERVAL);
}

unsigned int sched_restrict_cluster_freq(const struct cpumask *query_cpus,
					 unsigned int freq, int cpu,
					 ...
					 unsigned int policy_max)
{
	...
	if (global->preferred_cluster != NULL &&
		...
		trace_sugov_freq_update(cpu, freq, global->us_set_min_freq,
					global->us_set_max_freq, next_f,
...
int set_task_rtg_cluster_freq(struct task_struct *p, int cluster_id,
			      unsigned int min_freq, unsigned int max_freq)
{
	...
	trace_rtg_global_update(
		global->preferred_cluster ? global->preferred_cluster->id : -1,
		global->last_freq_update_time, global->us_set_min_freq,
		global->us_set_max_freq);
