...
static long ctrl_set_config(int abi, unsigned long arg)
{
	...
	case IOCTL_ABI_ARM32:
		ret = copy_from_user(
			&temp,
			(void __user *)compat_ptr((compat_uptr_t)rs.data),
			rs.len);
		break;
...
static long do_proc_rtg_ioctl(int abi, struct file *file, unsigned int cmd,
			      unsigned long arg)
{
	long ret = SUCC;
...
{
	/*lint -e712*/
	return do_proc_rtg_ioctl(
		IOCTL_ABI_ARM32, file, cmd,
