...
TRACE_EVENT(
	...
	TP_fast_assign(__entry->pid = p->pid;
		       ...
		       __assign_bitmask(cpus, cpumask_bits(perferred_cpumask),
					num_possible_cpus());
...
TRACE_EVENT(
	...
	TP_STRUCT__entry(
		__field(unsigned int, id) __field(unsigned int, nr_running)
			__array(char, comm, TASK_COMM_LEN) __field(pid_t, pid)
				__field(int,
					prio) __bitmask(allowed,
							num_possible_cpus())
					__field(int, cpu) __field(int, state)
						__field(bool, on_rq)
							__field(int, on_cpu)),

	TP_fast_assign(
		__entry->id = id; __entry->nr_running = nr_running;
		memcpy(__entry->comm, task->comm, TASK_COMM_LEN);
		__entry->pid = task->pid; __entry->prio = task->prio;
#if KERNEL_VERSION(5, 4, 0) > LINUX_VERSION_CODE
		__assign_bitmask(allowed, cpumask_bits(&task->cpus_allowed),
				 num_possible_cpus());
#else
		__assign_bitmask(allowed, cpumask_bits(&task->cpus_mask),
				 num_possible_cpus());
#endif
		__entry->cpu = task_cpu(task); __entry->state = task->__state;
		__entry->on_rq = task->on_rq; __entry->on_cpu = task->on_cpu;),

	TP_printk(
		"comm=%s pid=%d prio=%d allowed=%s cpu=%d state=%s%s on_rq=%d on_cpu=%d",
		__entry->comm, __entry->pid, __entry->prio,
		__get_bitmask(allowed), __entry->cpu,
		__entry->state &(TASK_REPORT_MAX) ?
			__print_flags(__entry->state &(TASK_REPORT_MAX), "|",
				      {TASK_INTERRUPTIBLE, "S"},
				      {TASK_UNINTERRUPTIBLE, "D"},
				      {__TASK_STOPPED, "T"},
				      {__TASK_TRACED, "t"}, {EXIT_DEAD, "X"},
				      {EXIT_ZOMBIE, "Z"}, {TASK_DEAD, "x"},
				      {TASK_WAKEKILL, "K"},
				      {TASK_WAKING, "W"}) :
			"R",
		__entry->state &TASK_STATE_MAX ? "+" : "", __entry->on_rq,
		__entry->on_cpu));
...
TRACE_EVENT(sched_rtg_valid_normalized_util,
	    ...
	    TP_STRUCT__entry(__field(unsigned int, id) __field(unsigned int,
							       nr_running)
				     __bitmask(cpus, num_possible_cpus())
					     __field(unsigned int, valid)),

	    TP_fast_assign(__entry->id = id; __entry->nr_running = nr_running;
			   __assign_bitmask(cpus, cpumask_bits(rtg_cpus),
					    num_possible_cpus());
			   __entry->valid = valid;),

	    TP_printk("id=%d nr_running=%d cpus=%s valid=%d", __entry->id,
		      __entry->nr_running, __get_bitmask(cpus),
		      __entry->valid));
...
DECLARE_EVENT_CLASS(
	...
	TP_PROTO(struct task_struct *p, struct task_struct *from, int type,
		 int op),

	TP_ARGS(p, from, type, op),

	TP_STRUCT__entry(
		__array(char, comm, TASK_COMM_LEN) __field(pid_t, pid) __field(
			int, target_cpu) __field(int, prio)
#ifdef CONFIG_HONOR_VIP_COMMON
			__field(int, vip_prio)
#endif
				__field(int, min_util) __field(
					pid_t, trans_from) __field(int,
								   from_prio)
#ifdef CONFIG_HONOR_VIP_COMMON
	...
	TP_fast_assign(memcpy(__entry->comm, p->comm, TASK_COMM_LEN);
		       __entry->pid = p->pid; __entry->target_cpu = task_cpu(p);
		       __entry->prio = p->prio;
#ifdef CONFIG_HONOR_VIP_COMMON
...
#ifdef CONFIG_HONOR_VIP_COMMON
		       __entry->from_vip_prio = (from == NULL) ? 0 :
								 from->vip_prio;
#endif
#ifdef CONFIG_UCLAMP_TASK
		       __entry->from_min_util =
			       (from == NULL) ? 0 :
						p->uclamp_req[UCLAMP_MIN].value;
#else
#ifdef CONFIG_SCHED_TASK_UTIL_CLAMP
		       __entry->from_min_util = (from == NULL) ?
							0 :
							from->util_req.min_util;
#else
		       __entry->from_min_util = -1;
#endif
#endif
		       __entry->trans_type = type; __entry->op = op;),
	...
	TP_printk("comm=%s pid=%d target_cpu=%03d prio=%d vip_prio=%d"
		  ...
		  __entry->prio, __entry->vip_prio, __entry->min_util,
		  __entry->trans_from, __entry->from_prio,
		  __entry->from_vip_prio, __entry->from_min_util,
