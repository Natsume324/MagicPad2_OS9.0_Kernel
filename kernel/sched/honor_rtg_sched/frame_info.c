...
rtg_frame_info_inner(const struct walt_related_thread_group *grp)
{
...
void set_frame_sched_state(bool enable)
{
	...
	if (enable) {
		...
		(void)sched_set_group_normalized_util(DEFAULT_RT_FRAME_ID, 0,
						      FRAME_NORMAL_UPDATE);
...
static inline bool check_frame_util_invalid(const struct frame_info *frame_info,
					    u64 timeline)
{
	return ((frame_info_rtg(frame_info)->util_invalid_interval <=
		 timeline) &&
		(frame_info_rtg_load(frame_info)->curr_window_exec *
			 FRAME_UTIL_INVALID_FACTOR <=
		 timeline));
}
...
static void set_frame_start(struct frame_info *frame_info)
{
	...
	if (!frame_info->margin_imme) {
		frame_info->frame_vload = 0;
		frame_info->frame_util = clamp_t(
			unsigned long, frame_info->prev_frame_load_util,
