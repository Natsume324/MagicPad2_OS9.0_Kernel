...
static void update_rtg_frame_thread(const struct rtg_data_head *proc_info)
{
	...
	pr_debug(
		"[AWARE_RTG] %s uid=%d, pid=%d, tid=%d\n, hwuitask0_tid=%d, hwuitask1_tid=%d",
		__func__, atomic_read(&g_rtg_uid), atomic_read(&g_rtg_pid),
		atomic_read(&g_rtg_tid), atomic_read(&g_rtg_hwuitask0_tid),
		atomic_read(&g_rtg_hwuitask1_tid));
}
...
int init_proc_state(const int *config, int len)
{
	...
	if ((config[RTG_FREQ_CYCLE] >= MIN_FREQ_CYCLE) &&
		...
		sched_set_group_freq_update_interval(
			DEFAULT_RT_FRAME_ID,
	...
	if (config[RTG_INVALID_INTERVAL] != INVALID_VALUE)
		sched_set_group_util_invalid_interval(
			DEFAULT_RT_FRAME_ID, config[RTG_INVALID_INTERVAL]);
	else
...
static struct st_aux_comms s_aux_comms = {.count = 0, .comms = {{0}}};
...
int parse_aux_comm_config(const struct rtg_str_data *rs_data)
{
	...
	while (p != NULL) {
		...
		tmp = strsep(&p, ",");
		if (tmp && *tmp != '\0') {
			strncpy(s_aux_comms.comms[s_aux_comms.count], tmp,
				TASK_COMM_LEN - 1);
			s_aux_comms.count++;
		}
	}
...
int update_frame_state(const struct rtg_data_head *rtg_head, int frame_type,
		       bool in_frame)
{
#ifdef CONFIG_USE_RTG_FRAME_SCHED
	int margin;
