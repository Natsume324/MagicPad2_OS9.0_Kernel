...
void set_task_qos_by_tid(struct task_struct *task, int qos)
{
	...
	if (qos == VALUE_QOS_CRITICAL)
		...
		if (!strncmp("ndroid.launcher", task->comm, TASK_COMM_LEN)) {
			...
			if (p && !strncmp("ndroid.launcher", p->comm,
					  TASK_COMM_LEN)) {
#ifdef CONFIG_HONOR_RTG_FRAME
				trace_rtg_frame_sched("LAUNCHER_TID",
						      task->pid);
#endif
	...
	if (qos == VALUE_QOS_CRITICAL)
		...
		if (!strncmp("ndroid.launcher", task->comm, TASK_COMM_LEN)) {
			...
			if (p && !strncmp("ndroid.launcher", p->comm,
					  TASK_COMM_LEN)) {
#ifdef CONFIG_HONOR_RTG_FRAME
				trace_rtg_frame_sched("LAUNCHER_PID",
						      task->pid);
#endif
				return;
			}
...
static void dynamic_vip_prio_dequeue(struct task_struct *task,
				     unsigned int type)
{
	if (!VIP_PRIO_TRANS_ENABLE || type != DYNAMIC_QOS_BINDER)
		return;
...
static void dynamic_min_util_dequeue(struct task_struct *task,
				     unsigned int type)
{
	if (!MIN_UTIL_TRANS_ENABLE || type != DYNAMIC_QOS_BINDER)
		return;
...
bool dynamic_qos_enqueue(struct task_struct *task, struct task_struct *from,
			 unsigned int type)
{
	bool ret = false;
