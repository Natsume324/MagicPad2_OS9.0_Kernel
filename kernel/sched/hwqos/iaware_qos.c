...
static long qos_ctrl_get_qos_whole(unsigned long arg)
{
	...
	if (qs_whole.pid) {
		...
		if (!tsk) {
			...
			qs_whole.trans_flags = atomic_read(&tsk->trans_flags);
		}
	}
	rcu_read_unlock();
...
static long qos_ctrl_ioctl(struct file *file, unsigned int cmd,
			   unsigned long arg)
{
	long ret = 0;
...
#ifdef CONFIG_COMPAT
static long qos_ctrl_compat_ioctl(struct file *file, unsigned int cmd,
				  unsigned long arg)
{
	/*lint -e712*/
