...
static int find_cas_cpu(struct sched_domain *sd, struct task_struct *task,
			struct cpumask *lowest_mask)
{
	...
	defined(CONFIG_HONOR_GLOBAL_BOOST)
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0)
	...
	do {
		...
		if (favor_larger_capacity) {
			...
			if (cpumask_test_cpu(rd->max_cap_orig_cpu,
					     sched_group_span(sg))) {
#else
			if (cpumask_test_cpu(rd->wrd.max_cap_orig_cpu,
					     sched_group_span(sg))) {
#endif
				sg_target = sg;
				break;
...
retry:
...
#else
					      cpu_isolated(cpu),
					      honor_walt_cpu_high_irqload(cpu),
#endif
					      idle_cpu(cpu), boosted_tutil,
					      cpu_util(cpu),
	...
	trace_sched_find_cas_cpu(task, lowest_mask, boosted_tutil, prefer_idle,
				 prev_cpu, target_cpu);
	return target_cpu;
}
#else
