...
static int rt_active_load_balance_cpu_stop(void *data)
{
	...
	if (capacity_orig_of(cpu_of(lowest_rq)) <=
	    capacity_orig_of(task_cpu(next_task)))
		goto unlock;
...
#else
	trace_android_rvh_sched_honor_task_fits_capacity(
		p, capacity_orig_of(cpu), cpu, &misfit_task);
...
#else
	trace_sched_rt_misfit_check(p, honor_uclamp_task_util(p), cpu,
				    misfit_task);
#endif
	...
	if (misfit_task) {
		...
		if (need_actvie_lb)
			stop_one_cpu_nowait(task_cpu(p),
					    rt_active_load_balance_cpu_stop, rq,
					    &rq->rt_active_balance_work);
	}
}
#endif
