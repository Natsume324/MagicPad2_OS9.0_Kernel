...
TRACE_EVENT(
	...
	TP_STRUCT__entry(
		__array(char, comm, TASK_COMM_LEN) __field(pid_t, pid) __field(
			int, prio) __field(unsigned long, task_util)
			__field(int, cpu) __field(unsigned long, cap_curr)
	...
	TP_fast_assign(memcpy(__entry->comm, task->comm, TASK_COMM_LEN);
		       ...
		       __entry->task_util = task_util; __entry->cpu = cpu;
