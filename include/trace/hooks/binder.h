...
DECLARE_RESTRICTED_HOOK(android_vh_get_trans_qos_by_type,
	TP_PROTO(struct task_struct *task, int type, int *ret),
	TP_ARGS(task, type, ret), 1);
DECLARE_RESTRICTED_HOOK(android_vh_get_task_qos,
	TP_PROTO(struct task_struct *task, int *ret),
	TP_ARGS(task, ret), 1);
DECLARE_RESTRICTED_HOOK(android_rvh_should_binder_do_set_priority,
	TP_PROTO(struct task_struct *task, int desired_prio, bool verify, bool *ret),
	TP_ARGS(task, desired_prio, verify, ret), 1);
#endif
