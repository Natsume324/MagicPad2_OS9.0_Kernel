...
#define HIMNTN_WARNING(msg, ...)                                \
	do {                                                    \
		if (HIMNTN_LOG_LEVEL >= HIMNTN_LOG_WARNING)     \
			pr_warn("[HIMNTN/W]%s: " msg, __func__, \
				##__VA_ARGS__);                 \
	} while (0)
...
#define HIMNTN_INFO(msg, ...)                                   \
	do {                                                    \
		if (HIMNTN_LOG_LEVEL >= HIMNTN_LOG_INFO)        \
			pr_info("[HIMNTN/I]%s: " msg, __func__, \
				##__VA_ARGS__);                 \
	} while (0)
...
#define HIMNTN_DEBUG(msg, ...)                                   \
	do {                                                     \
		if (HIMNTN_LOG_LEVEL >= HIMNTN_LOG_DEBUG)        \
			pr_debug("[HIMNTN/D]%s: " msg, __func__, \
				 ##__VA_ARGS__);                 \
	} while (0)
...
enum himntn_id_enum {
	...
	HIMNTN_ID_RESERVED,
	HIMNTN_ID_APLOG_TO_LOGPART, // 3
	HIMNTN_ID_SETMAXCPUS_SWITCH = 8, // 8
	HIMNTN_ID_ONFAILUREPANIC_DISABLE,
	HIMNTN_ID_BATTERYCHECK_DISABLE,
