...
{
	return mrq->data ?
		       ((mrq->data->flags & MMC_DATA_WRITE) ? IOMT_DIR_WRITE :
							      IOMT_DIR_READ) :
		       IOMT_DIR_OTHER;
}
						   ...
						   struct mmc_request *mrq)
{
	...
	if (areq != NULL && areq->mrq == mrq) {
		mrq_que = areq->mrq_que;
		if (mrq_que != NULL)
			mrq->iomt_start_time = mrq_que->iomt_start_time;
	}
#endif
}
					     ...
					     struct mmc_request *mrq)
{
	...
	i = iomt_host_io_latency(iomt_host_info, &(mrq->iomt_start_time), dir,
				 chunk_size);
...
#define dsm_iomt_mmc_host_pre_init(host)                          \
	do {                                                      \
		if (!strncmp(mmc_hostname((host)->mmc), "mmc0",   \
			     strlen("mmc0")))                     \
