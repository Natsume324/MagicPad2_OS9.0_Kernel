...
#if IS_ENABLED(CONFIG_HIHONOR_USB_DMD)
	int (*update_eye_value)(struct usb_phy *x, bool enable,
			u32 *param, u8 param_cnt);
#endif
	/*
...
#if IS_ENABLED(CONFIG_HIHONOR_USB_DMD)
static inline int
usb_phy_update_eye_value(struct usb_phy *x, bool enable,
	u32 *param, u8 param_cnt)
{
	if (x && x->update_eye_value)
		return x->update_eye_value(x, enable, param, param_cnt);
	else
		return 0;
