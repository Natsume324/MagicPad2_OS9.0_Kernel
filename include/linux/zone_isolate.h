...
extern void record_zi_index(unsigned int zi_index, unsigned int order,
			    int migratetype);
#endif

extern void add_page_to_list(struct page *page, struct zone *zone,
			     unsigned int order, int mt);
extern int page_to_zi_index(struct page *page);
extern void setup_zone_isolate(struct zone *zone);
extern unsigned int get_zi_index(unsigned int current_zi_index,
