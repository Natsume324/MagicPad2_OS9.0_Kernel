...
#define MAIN_IC_IBUS_CURRENT_LIMIT_TH 4000
#define AUX_IC_IBUS_CURRENT_LIMIT_TH 3000
#define IBUS_CURRENT_LIMIT_DELTA_TH 100
#define GET_IBUS_COUNT 10
#define GET_IBUS_FIFO_LEN 3

struct multi_ic_unbalanced_current_sc_para {
	bool first;
	bool limit_current_flag;
	int count;
	int ibus_main_sum;
	int ibus_aux_sum;
	int ibus_main_buff[GET_IBUS_FIFO_LEN];
	int ibus_aux_buff[GET_IBUS_FIFO_LEN];
};
...
#define MULTI_IC_COMB_NAME_PARA_LEVEL 8
#define MULTI_IC_COMB_NAME_LEN 25

enum multi_ic_comb_name_info {
	MULTI_IC_COMB_MAIN_IC_NAME = 0,
	MULTI_IC_COMB_AUX_IC_NAME,
	MULTI_IC_COMB_CURRENT_RATION_NAME,
	MULTI_IC_COMB_NAME_MAX,
};

struct multi_ic_comb_name_para {
	char main_ic_name[MULTI_IC_COMB_NAME_LEN];
	char aux_ic_name[MULTI_IC_COMB_NAME_LEN];
	char current_ratio_name[MULTI_IC_COMB_NAME_LEN];
};
...
struct multi_ic_check_para {
	...
	int ibat_comb;
	int ratio_result;
	int main_ibus_current_limit_max;
	int aux_ibus_current_limit_max;
	int ibus_limit_current_delta;
	u32 dc_use_fg_get_battery_current;
	u32 support_unbalanced_current_sc;
	int single_main_ic_ibat_th;
	int single_aux_ic_ibat_th;
	int multi_ic_info_ibat_th;
	...
	struct multi_ic_curr_ratio_para
		curr_ratio[MULTI_IC_CURR_RATIO_PARA_LEVEL];
	struct multi_ic_vbat_error_para
		vbat_error[MULTI_IC_VBAT_ERROR_PARA_LEVEL];
	struct multi_ic_tbat_error_para
		tbat_error[MULTI_IC_TBAT_ERROR_PARA_LEVEL];
	struct multi_ic_comb_name_para comb_name[MULTI_IC_COMB_NAME_PARA_LEVEL];
};
...
int multi_ic_check_select_init_mode(struct multi_ic_check_mode_para *para,
				    int *mode);
void multi_ic_check_set_ic_error_flag(int flag,
