...
int power_dsm_dmd_report_s(enum power_dsm_type type, int err_no,
...
int power_dsm_bigdata_report(enum power_dsm_type type, int err_no,
			     const void *msg);
#else
static inline int power_dsm_bigdata_report(enum power_dsm_type type, int err_no,
					   const void *msg)
{
	return 0;
}
	...
	do {                                                                   \
		if (power_dsm_get_dclient(type)) {                             \
			if (!dsm_client_ocuppy(power_dsm_get_dclient(type))) { \
				dsm_client_record(power_dsm_get_dclient(type), \
						  fmt, ##args);                \
				dsm_client_notify(power_dsm_get_dclient(type), \
						  err_no);                     \
...
static inline int power_dsm_dmd_report(enum power_dsm_type type, int err_no,
				       const char *buf)
{
	return 0;
}

static inline int power_dsm_dmd_report_s(enum power_dsm_type type, int err_no,
					 const char *buf, int len)
{
	return 0;
}
