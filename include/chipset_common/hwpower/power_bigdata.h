...
int power_bigdata_send_integer(struct imonitor_eventobj *obj, const char *para,
			       long value);
int power_bigdata_send_string(struct imonitor_eventobj *obj, const char *para,
			      const char *value);
#else
static inline int power_bigdata_report(enum power_bigdata_type type,
				       power_bigdata_send_cb send_cb,
				       void *data)
{
	return -1;
}
