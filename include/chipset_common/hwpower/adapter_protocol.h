...
int adapter_auth_encrypt_start(int prot, int key, unsigned char *hash,
			       int size);
...
static inline int
adapter_protocol_ops_register(struct adapter_protocol_ops *ops)
{
	return -1;
}
...
static inline int
adapter_protocol_chg_det_register(struct adapter_chg_det_ops *ops)
{
	return -1;
}
