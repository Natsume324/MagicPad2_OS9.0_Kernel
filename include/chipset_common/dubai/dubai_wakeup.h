...
struct dubai_sysmon_time_in_state {
	...
	int64_t lpi_time;
	struct dubai_sysmon_freq_time
		freq_time[SUBSYSTEM_POWER_STATS_MAX_CLK_LEVELS];
} __packed;
...
#ifdef CONFIG_HONOR_DUBAI_SUBSYSTEM_STATS
extern int honor_subsystem_size_get(void);
extern int honor_subsystem_sleep_stats_get(struct honor_sleep_stats *stats);
extern int honor_soc_sleep_stats_get(struct honor_sleep_stats *stats);
#endif
...
typedef int (*dubai_wakeup_stats_fn_t)(struct dubai_wakeup_info *data,
				       size_t num);
