...
#define IOMT_LATENCY_GAP_MID_IDX (IOMT_LATENCY_GAP_ARRAY_SIZE / 2)
#define IOMT_LATENCY_STAT_ARRAY_SIZE (IOMT_LATENCY_GAP_ARRAY_SIZE + 1)

#define IOMT_LATENCY_INVALID_INDEX IOMT_LATENCY_STAT_ARRAY_SIZE

#define IOMT_LATENCY_STAT_RING_BUFFER_SIZE 12
#define IOMT_STAT_RING_SIZE IOMT_LATENCY_STAT_RING_BUFFER_SIZE
#define IOMT_LATENCY_SCATTER_TIMER_INTERVAL (5 * HZ)
#define IOMT_IO_TIMEOUT_DSM_JUDGE_SLOT (13 % IOMT_LATENCY_STAT_ARRAY_SIZE)
...
static const long iomt_chunksize_array[IOMT_LATENCY_CHUNKSIZE_ARRAY_SIZE] = {
	0, 4, 8, 16, 64, 256, 512, 1024, 2048};
...
typedef void (*iomt_reinit_gap_array_func)(unsigned long *array,
...
#define IOMT_RW_GAP_MID_IDX (IOMT_RW_SIZE_GAP_ARRAY_SIZE / 2)
#define IOMT_RW_SIZE_STAT_ARRAY_SIZE (IOMT_RW_SIZE_GAP_ARRAY_SIZE + 1)
...
static inline int iomt_host_io_latency(struct iomt_host_info *iomt_host_info,
				       ...
				       unsigned int chunk_size)
{
	...
	if (begin_time->type == IOMT_LATNECY_LOW_ACC) {
		...
		latency_index = iomt_get_array_index(
			gap_array, IOMT_LATENCY_GAP_ARRAY_SIZE,
			(unsigned long)diff, 0, 1);

	} else {
		diff = (unsigned long)ktime_to_us(
			ktime_sub(ktime_get(), begin_time->ktime));
		gap_array = latency_scatter->high_acc_gap_array;
		latency_index = iomt_get_array_index(
			gap_array, IOMT_LATENCY_GAP_ARRAY_SIZE,
			(unsigned long)diff, 0, IOMT_MS_TO_US);
	}

	chunk_size_index = iomt_get_array_index(
		iomt_chunksize_array, IOMT_LATENCY_CHUNKSIZE_ARRAY_SIZE,
		chunk_size, 1, 1);

	latency_scatter_node =
		&(latency_scatter->io_latency_scatter_buffer[chunk_size_index]);
...
iomt_host_stat_latency(struct iomt_host_info *iomt_host_info,
		       struct iomt_timestamp *begin_time, unsigned char dir)
{
	struct iomt_host_latency_stat *latency_scatter = NULL;
	unsigned int i;
	unsigned long diff;
	...
	if (likely(diff < gap_array[IOMT_LATENCY_GAP_MID_IDX])) {
...
static inline void iomt_host_stat_rw_size(struct iomt_host_info *iomt_host_info,
					  ...
					  unsigned char dir)
{
	...
	if (block_cnt < rw_size_scatter->gap_array[IOMT_RW_GAP_MID_IDX]) {
		...
		for (i = IOMT_RW_GAP_MID_IDX; i < IOMT_RW_SIZE_GAP_ARRAY_SIZE;
		     i++) {
			if (block_cnt <= rw_size_scatter->gap_array[i])
				break;
		}
