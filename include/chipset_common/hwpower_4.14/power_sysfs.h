...
#define power_sysfs_attr_rw(_func, _mode, _type, _name)          \
	{                                                        \
		.attr = __ATTR(_name, _mode, _func##_sysfs_show, \
			       _func##_sysfs_store),             \
		.name = _type,                                   \
	}
...
struct power_sysfs_attr_info *
power_sysfs_lookup_attr(const char *name,
			struct power_sysfs_attr_info *attr_info, int size);
struct device *
power_sysfs_create_link_group(const char *cls_name, const char *dev_name,
			      const char *link_name, struct device *target_dev,
			      const struct attribute_group *link_group);
void power_sysfs_remove_link_group(const char *cls_name, const char *dev_name,
				   const char *link_name,
				   struct device *target_dev,
				   const struct attribute_group *group);
struct device *power_sysfs_create_group(const char *cls_name,
					const char *dev_name,
					const struct attribute_group *group);
