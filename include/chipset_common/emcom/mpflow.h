...
#define EMCOM_MPFLOW_AI_MAX_LIST_NUM \
	(EMCOM_MPFLOW_DEV_NUM * EMCOM_MPFLOW_AI_MAX_APP)
...
#define EMCOM_MPFLOW_BINDMODE_ORI_DEV_MASK \
	((1 << EMCOM_MPFLOW_BINDMODE_ORI_DEV_OFFSET) - 1)
...
#define mpflow_ai_in_range(value, svalue, evalue) \
	((value) >= (svalue) && (value) <= (evalue))
...
typedef enum {
	...
	EMCOM_XENGINE_MPFLOW_AI_BINDMODE_RANDOM =
		(1 << BIND_MODE_HIGH_BIT_START) - 1
...
struct emcom_xengine_mpflow_parse_start_info {
	...
	struct emcom_xengine_mpflow_dport_range
		dport_range[EMCOM_MPFLOW_PORT_RANGE_NUM_MAX]; /* prot range */
...
struct emcom_xengine_mpflow_iface {
	...
	uint16_t flow_cnt;
	uint16_t is_wifi : 1, is_slow : 1, is_sure_no_slow : 1;
...
struct emcom_xengine_mpflow_info {
	...
	uint32_t reserve_field;
	struct emcom_xengine_mpflow_dport_range
		dport_range[EMCOM_MPFLOW_PORT_RANGE_NUM_MAX]; /* port range */
	struct emcom_xengine_mpflow_iface wifi;
	struct emcom_xengine_mpflow_iface lte;
	uint16_t is_downloading : 1, rst_to_another : 1;
...
typedef enum { SK_ERROR = 1, INTF_CHANGE } reset_act;
...
struct emcom_xengine_mpflow_ai_port_cfg {
	...
	struct emcom_xengine_mpflow_dport_range
		port_range[EMCOM_MPFLOW_BIND_PORT_CFG_SIZE];
};
...
struct emcom_xengine_mpflow_ai_init_bind_cfg {
	...
	struct emcom_xengine_mpflow_ai_init_bind_policy
		scatter_bind[EMCOM_MPFLOW_BIND_PORT_CFG_SIZE];
};
...
struct emcom_xengine_mpflow_ai_burst_port {
	...
	struct emcom_xengine_mpflow_dport_range
		burst_ports[EMCOM_MPFLOW_BIND_PORT_CFG_SIZE];
};
