...
#define is_do_nothing_fault(method)                                            \
	(((method) == FRAMEWORK_DO_NOTHING) ||                                 \
	 ((method) == NATIVE_DO_NOTHING) || ((method) == KERNEL_DO_NOTHING) || \
	 ((method) == BL2_DO_NOTHING) || ((method) == BL1_DO_NOTHING))
...
#define is_bopd_run_mode(mode)                                              \
	(((mode) == BOPD_BEFORE_DL) || ((mode) == BOPD_STG_RO_BEFORE_DL) || \
	 ((mode) == BOPD_HW_DEG_BEFORE_DL) ||                               \
	 ((mode) == BOPD_STG_RO_HW_DEG_BEFORE_DL) ||                        \
	 ((mode) == BOPD_AFTER_DL) || ((mode) == BOPD_HW_DEG_AFTER_DL) ||   \
