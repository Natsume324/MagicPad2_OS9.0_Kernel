...
#define kshield_func_ret_int(no, name, ...)                               \
	...
	static __always_inline int kshield_chk_##name(                    \
		__MAP(no, __SC_DECL, __VA_ARGS__))                        \
	{                                                                 \
		if (likely(!current->inspected))                          \
			return 0;                                         \
...
#define kshield_func_ret_void(no, name, ...)                           \
	...
	static __always_inline void kshield_chk_##name(                \
		__MAP(no, __SC_DECL, __VA_ARGS__))                     \
	{                                                              \
		if (likely(!current->inspected))                       \
			return;                                        \
...
#define kshield_func_ret_void0(name)                         \
	extern void sub_chk_##name(void);                    \
...
static inline long ks_dev_ioctl(unsigned int cmd, char *cur)
{
	return 0;
}
...
/* EXP */
kshield_func_ret_int4(change_mem, unsigned long, start, unsigned long, size,
