...
static inline bool workingset_pagecache_skip_readfile(loff_t *pos,
						      pgoff_t index,
						      unsigned long offset)
{
	return likely(!task_wscg_monitor(current)) || !pos ||
	       (*pos >= ((loff_t)index << PAGE_SHIFT) + offset);
}

static inline void workingset_pagecache_on_readfile(struct file *file,
						    loff_t *pos, pgoff_t index,
						    unsigned long offset)
{
