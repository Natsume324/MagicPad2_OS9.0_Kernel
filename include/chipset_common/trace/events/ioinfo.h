...
TRACE_EVENT(io_info_debug,
	    ...
	    TP_STRUCT__entry(__field(unsigned long, read_kb) __field(
		    unsigned long, write_kb) __field(unsigned long, pgfile)
				     __field(unsigned long,
					     free) __field(unsigned long, slab)
					     __field(unsigned long, refault)),

	    TP_fast_assign(__entry->read_kb = r_kb; __entry->write_kb = w_kb;
