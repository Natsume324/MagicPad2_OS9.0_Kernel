...
TRACE_EVENT(mutex_lock_contention,
	    ...
	    TP_STRUCT__entry(__field(int, pid) __field(unsigned long, time)),

	    TP_fast_assign(__entry->pid = pid; __entry->time = time),
...
TRACE_EVENT(mutex_lock_contention_length,
	    ...
	    TP_STRUCT__entry(__field(const char *, name) __field(int, pid)
				     __field(unsigned long, time)),

	    TP_fast_assign(__entry->name = name; __entry->pid = pid;
			   __entry->time = time),
...
TRACE_EVENT(mutex_wakeup,
	    ...
	    TP_STRUCT__entry(__field(int, target_pid) __field(int, sender_pid)),

	    TP_fast_assign(__entry->target_pid = target_pid;
			   __entry->sender_pid = sender_pid),

	    TP_printk("tpid=%d spid=%ld", __entry->target_pid,
		      __entry->sender_pid));
