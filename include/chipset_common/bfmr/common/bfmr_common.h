...
#define BFMR_PRINT_INVALID_PARAMS(format, ...)                            \
	pr_err("func: %s line: %d invalid parameters: " format, __func__, \
	       __LINE__, ##__VA_ARGS__)
#define BFMR_PRINT_ERR(format, ...) \
	pr_err("func: %s line: %d, " format, __func__, __LINE__, ##__VA_ARGS__)
#define BFMR_PRINT_INFO(format, ...) \
	pr_info("func: %s line: %d, " format, __func__, __LINE__, ##__VA_ARGS__)
#define BFMR_PRINT_KEY_INFO(format, ...) \
	pr_err("func: %s line: %d, " format, __func__, __LINE__, ##__VA_ARGS__)
#define BFMR_PRINT_SIMPLE_INFO(args...) pr_err(args)
#define BFMR_PRINT_DBG(format, ...)                                \
	pr_devel("func: %s line: %d, " format, __func__, __LINE__, \
		 ##__VA_ARGS__)
...
int bfmr_sha256(unsigned char *pout, unsigned int out_len, const void *pin,
...
int bfmr_read_emmc_raw_part(const char *dev_path, unsigned long long offset,
			    char *buf, unsigned long long buf_size);
int bfmr_write_emmc_raw_part(const char *dev_path, unsigned long long offset,
			     char *buf, unsigned long long buf_size);
