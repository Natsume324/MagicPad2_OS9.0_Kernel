...
#define BFR_RRECORD_2ND_PART_OFS \
	(BFR_RRECORD_1ST_PART_OFS + BFR_EACH_RECORD_PART_SIZE)
#define BFR_RRECORD_3RD_PART_OFS \
	(BFR_RRECORD_2ND_PART_OFS + BFR_EACH_RECORD_PART_SIZE)
 ...
 */
enum bfr_method
try_to_recovery(unsigned long long boot_fail_detected_time,
		enum bfm_errno_code boot_fail_no,
...
#else
static inline int
bfr_get_hardware_fault_times(struct bfmr_hw_fault_info_param *pfault_info_param)
{
	return -1;
}

static inline int
bfr_get_real_recovery_info(struct bfr_real_rcv_info *preal_recovery_info)
{
	return 0;
}
...
static inline enum bfr_method
try_to_recovery(unsigned long long boot_fail_detected_time,
		enum bfm_errno_code boot_fail_no,
		enum bfmr_detail_stage boot_fail_stage,
		enum bfr_suggested_method suggested_recovery_method, char *args)
{
