...
TRACE_EVENT_CONDITION(
	...
	TP_STRUCT__entry(__field(u16, sport) __field(int, tx) __field(int, rx)),

	TP_fast_assign(__entry->sport = inet_sk(sk)->inet_num;
		       netqos_trace_assign_set(sk, &__entry->tx, &__entry->rx,
					       cur_time);),

	TP_printk("port=%u tx=%d rx=%d", __entry->sport, __entry->tx,
		  __entry->rx));
