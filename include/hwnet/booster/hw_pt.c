...
static void prepare_report_data(struct hw_kernel_delay_info *info_rpt)
{
	...
	if (g_hw_pt_dl_in_abnormal_data_count == 0)
		...
		info_rpt->downlink_time_cost_ave =
			g_hw_pt_dl_data_time_cost /
			g_hw_pt_dl_in_abnormal_data_count;
...
static void set_chr_report_timer(int status, int len)
{
	...
	if (status == CHR_TIMER_NOT_START || len <= 0)
		return;
	if (g_chr_data_process_timer_state == CHR_TIMER_NOT_START &&
	    !timer_pending(&g_kernel_data_process_timer)) {
...
{
	return (g_app_top_uid > MIN_APP_UID) &&
	       (g_last_app_top_uid != g_app_top_uid);
}

static void notify_packet_delay_event(int uid,
				      struct hw_kernel_delay_info *info_rpt)
{
	...
	if (is_top_app_changed()) {
		if (g_chr_data_process_timer_state == CHR_TIMER_STATISTICS) {
			...
			if (is_chr_report_needed()) {
				...
				notify_packet_delay_event(g_last_app_top_uid,
							  &info_rpt);
				set_chr_report_timer(
					CHR_TIMER_REPORT_WAIT,
					TIMER_REPORT_BACKOFF_PEROID);
			}
		}
...
static void hw_pt_exception_happen(int dir, int time_cost)
{
	...
	if (time_cost >= MAX_UL_TRANS_TIME) {
		...
		case CHR_TIMER_NOT_START:
			set_chr_report_timer(CHR_TIMER_STATISTICS,
					     TIMER_STATISTICS_PEROID);
...
static bool hw_pt_log_exception_need_continue(struct sk_buff *skb)
{
	...
	if ((shinfo->proc_bitmask & PROC_BITMASK_SET_MASK) !=
		    PROC_BITMASK_SETTED ||
	    shinfo->born_stamp == 0) {
...
static bool hw_pt_is_uplink_need_continue(struct sk_buff *skb,
...
void hw_pt_dev_uplink(struct sk_buff *skb, struct net_device *dev)
{
	...
	case IPPROTO_TCP:
		...
		if (th != NULL) {
			...
			if (skb->protocol == ntohs(ETH_P_IPV6)) {
				ip6h = (struct ipv6hdr *)skb_network_header(
					skb);
				len = ntohs(ip6h->payload_len);
			} else if (skb->protocol == ntohs(ETH_P_IP)) {
				iph = (struct iphdr *)(skb_network_header(skb));
				len = ntohs(iph->tot_len) -
				      iph->ihl * TCP_HDR_IHL_QUAD;
			} else {
				return;
			}
			...
			if (len > package_len) {
				shinfo->proc_bitmask |=
					PROC_BITMASK_RNIC_UL_OUT;
				return;
			}
		}
		hw_pt_log_exception(NULL, skb, IPPROTO_TCP, RNIC_UL_DELAY_LIMIT,
				    __func__, UPLINK);
		break;
	case IPPROTO_UDP:
		hw_pt_log_exception(NULL, skb, IPPROTO_UDP, RNIC_UL_DELAY_LIMIT,
				    __func__, UPLINK);
		break;
	default:
		return;
	...
	shinfo->proc_bitmask |=
		PROC_BITMASK_RNIC_UL_OUT; // record this skb is sent
}
...
void hw_pt_l4_downlink_in(struct sock *skt, struct sk_buff *skb, int eaten)
{
	...
	case IPPROTO_TCP:
		hw_pt_log_exception(sk, skb, IPPROTO_TCP, RNIC_DL_DELAY_LIMIT,
				    __func__, DOWNLINK_IN);
		if (!eaten) {
			shinfo = skb_shinfo(skb);
			if (shinfo == NULL)
				break;
		...
		break;
	case IPPROTO_UDP:
		hw_pt_log_exception(sk, skb, IPPROTO_UDP, RNIC_DL_DELAY_LIMIT,
				    __func__, DOWNLINK_IN);
		shinfo = skb_shinfo(skb);
		if (shinfo == NULL)
			break;
...
void hw_pt_l4_downlink_out(struct sock *skt, struct sk_buff *skb)
{
	...
	switch (sk->sk_protocol) {
	case IPPROTO_TCP:
		hw_pt_log_exception(sk, skb, IPPROTO_TCP, L4_DL_DELAY_LIMIT,
				    __func__, DOWNLINK_OUT);
		break;
	case IPPROTO_UDP:
		hw_pt_log_exception(sk, skb, IPPROTO_UDP, L4_DL_DELAY_LIMIT,
				    __func__, DOWNLINK_OUT);
		break;
	default:
		break;
...
static void chr_kernel_data_process_timer(unsigned long data)
{
	...
	if (is_chr_report_needed()) {
		...
		set_chr_report_timer(CHR_TIMER_REPORT_WAIT,
				     TIMER_REPORT_BACKOFF_PEROID);
		chr_kernel_clear_data_record();
	}
}
