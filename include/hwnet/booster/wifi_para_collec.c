...
static u32 wifi_calculate_bitrate_extended_sc_dmg(struct rate_info *rate)
{
	// clang-format off
	static const u32 __mcs2bitrate[] = {
...
static u32 wifi_calculate_bitrate_eht(struct rate_info *rate)
{
#define SCALE 6144
...
static u32 wifi_calculate_bitrate(struct rate_info *rate)
{
	...
	if (rate->flags & RATE_INFO_FLAGS_EXTENDED_SC_DMG)
		return wifi_calculate_bitrate_extended_sc_dmg(rate);
	...
	if (rate->flags & RATE_INFO_FLAGS_EHT_MCS)
		return wifi_calculate_bitrate_eht(rate);
...
static int get_wifi_para_for_booster(struct wifi_info *w_info, int w_info_len)
{
	...
	for (i = 0; i < w_info_len; i++) {
		...
		if (g_wifi_dev[i].wifi_state_flag == 1 && tmp_rdev != NULL &&
		    tmp_dev != NULL && tmp_mac_addr != NULL) {
			struct station_info s_info;

			memset(&s_info, 0, sizeof(s_info));
			if (tmp_rdev->ops->get_station) {
				rtnl_lock();
				ret = tmp_rdev->ops->get_station(
					&tmp_rdev->wiphy, tmp_dev, tmp_mac_addr,
					&s_info);
				rtnl_unlock();
			}
			if (ret)
			...
			if (s_info.filled & BIT(NL80211_STA_INFO_CH_WIDTH))
				w_info[i].channel_width = s_info.channel_width;
			if (s_info.filled & BIT(NL80211_STA_INFO_NSS))
				w_info[i].nss = s_info.nss;
		}
	}
...
extern struct neigh_table *neigh_find_table(int family);
...
static void get_wifi_para(struct wifi_stat *cur, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		cur[i].info.channel_width = w_info[i].channel_width;
		cur[i].info.nss = w_info[i].nss;

		cur[i].info.arp_state = get_wifi_arp_state(
			w_info[i].name, IFNAMSIZ, g_gateway[i]);
	}
}
...
static void update_wifi_res(struct wifi_stat *past, struct wifi_stat *cur,
			    struct wifi_stat *res, int cnt)
{
	int i;
	...
	for (i = 0; i < cnt; i++) {
		...
		res[i].info.rx_packets = turn_over(cur[i].info.rx_packets,
						   past[i].info.rx_packets);
		res[i].info.tx_packets = turn_over(cur[i].info.tx_packets,
						   past[i].info.tx_packets);
		res[i].info.tx_failed = turn_over(cur[i].info.tx_failed,
		...
		res[i].info.channel_width = cur[i].info.channel_width;
		res[i].info.nss = cur[i].info.nss;
		res[i].info.arp_state = cur[i].info.arp_state;
	}
}

static void update_wifi_para(struct wifi_stat *past, struct wifi_stat *cur,
			     int cnt)
{
...
msg_process *wifi_para_collec_init(notify_event *fn)
{
	...
	g_wifi_ctx->fn = fn;
	g_wifi_ctx->past =
		kmalloc(sizeof(struct wifi_stat) * MAX_WIFI_NUM, GFP_KERNEL);
