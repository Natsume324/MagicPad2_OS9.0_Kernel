...
static void icmp_ping_detected(void)
{
	...
	if (g_icmp_info.ap_cmd == START_DETECT) {
		if (timer_pending(&g_icmp_info.timer)) {
			mod_timer(&g_icmp_info.timer,
				  jiffies + STOP_TIMER_INTERVAL);
		} else {
...
static unsigned int icmp_detect_hook4(void *priv, struct sk_buff *skb,
...
static unsigned int icmp_detect_hook6(void *priv, struct sk_buff *skb,
				      const struct nf_hook_state *state)
{
