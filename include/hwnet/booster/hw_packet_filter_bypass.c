...
bool hw_bypass_skb(int af, int hook, const struct sock *sk, struct sk_buff *skb,
		   struct net_device *in, struct net_device *out, int pass)
{
	uid_t uid;
	int i;
	int ret = 0;
	...
	if (i < listen_uids.nums) {
		...
		case HW_PFB_INET6_LOCAL_OUT:
			ret = update_filter_pkts(
				&listen_uids.infos[i],
				&listen_uids.infos[i].local_out_drop_stats,
				pass);
			break;
		...
		case HW_PFB_INET6_LOCAL_IN:
			ret = update_filter_pkts(
				&listen_uids.infos[i],
				&listen_uids.infos[i].local_in_drop_stats,
				pass);
			break;
		...
		case HW_PFB_INET6_BPF_INGRESS:
			ret = update_filter_pkts(
				&listen_uids.infos[i],
				&listen_uids.infos[i].bpf_ingress_drop_stats,
				pass);
			break;
