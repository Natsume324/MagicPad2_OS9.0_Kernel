...
static bool is_param_valid(struct sock *sk, u8 *str, u32 len,
			   struct stream_info info)
{
	if (sk == NULL || str == NULL)
		return false;
...
static bool is_video_stream(struct sock *sk, u8 *str, u32 len,
			    struct stream_info info)
{
	u32 cnt;
		...
		if (str[cnt] == ASCII_CR && str[cnt + 1] == ASCII_LF) {
			...
			if (cnt >= len - CONTENT_TYPE_LEN) {
				...
				content_length_start =
					&str[cnt + CONTENT_LENGTH_LEN];
				content_length =
					simple_strtoull(content_length_start,
							NULL, DECIMAL_BASE);
				tp->content_length =
					content_length + tp->bytes_received;
				break;
			} else {
				continue;
			}
...
bool is_target_stream(struct sock *sk, u8 *str, u32 len, u32 stream_type,
		      struct stream_info info)
{
...
static bool is_match_stream(struct stream_info skb_info,
			    struct stream_info info)
{
	if (skb_info.uid != info.uid)
		return false;
	if (skb_info.src_port != info.src_port ||
	    skb_info.dest_port != info.dest_port)
		return false;
	if (skb_info.family == AF_INET)
		return (skb_info.dest_addr.ip4_addr == info.dest_addr.ip4_addr);
	else
		return ipv6_addr_equal(&skb_info.dest_addr.ip6_addr,
				       &info.dest_addr.ip6_addr);
}
...
static struct sock *get_suit_sk(struct sk_buff *skb)
{
	...
	if (sk->sk_state != TCP_SYN_SENT &&
	    sk->sk_state != TCP_ESTABLISHED) // sk_state
		return NULL;
	return sk;
}

static u32 get_tcp_len(struct sk_buff *skb, char const *tcp_data,
		       char const *piph)
{
	u32 total_len;
	u32 ip_hdr_len;
...
static struct stream_list *process_new_stream(struct sock *sk,
					      struct stream_stat *stream,
					      struct stream_info info,
					      char *tcp_data, u32 tcp_len)
{
	struct stream_list *pos_selec = NULL;

	if (!is_target_stream(sk, tcp_data, tcp_len, g_stream_ctx->stream_type,
			      info))
		return NULL;
...
static void update_tcp_in_para(struct stream_res *stat, struct sk_buff *skb,
			       struct sock *sk, u32 tcp_len)
{
	...
	if (before(ntohl(th->seq) + tcp_len - 1, tp->rcv_nxt) && tcp_len > 0)
		stat->in_rts_pkt++;
}
...
void stream_process_hook_in(struct sk_buff *skb, u16 family)
{
	...
	if (pos_selec == NULL) {
		pos_selec =
			process_new_stream(sk, stream, info, tcp_data, tcp_len);
		if (pos_selec == NULL)
			goto unlock;
...
static void update_tcp_out_para(struct stream_res *stat, struct sk_buff *skb,
				struct sock *sk)
{
...
#endif
{
	...
	res_msg.len =
		8 +
		stream_res->cnt *
			sizeof(struct stream_res); // 8 is size of type,len,cnt.
	msg = (char *)&res_msg;
	if (g_stream_ctx->fn)
...
static void stream_para_report_start(struct req_msg_head *msg)
{
	...
	pr_info("[STREAM_DETECT]%s,uid=%u,stream_type=%u,report_expires=%u,"
		"list_expires=%u,res=%u",
		__func__, req->uid, req->stream_type, req->report_expires,
		req->list_expires, req->res);
