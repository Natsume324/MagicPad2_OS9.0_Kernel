...
#define PTP_TIME_STANDARD_CAL_LEN \
	((PTP_TIME_QUEUE_LEN) - (PTP_TIME_STANDARD_CAL_START) * (DOUBLE_SIZE))
...
static void time_sync_status_report(const struct ptp_dev *dev, int status,
				    int reason);
static void ptp_timesync_set_status(struct ptp_dev *dev, int status);
	...
	for (channel = 0; channel < CHANNEL_MAX; channel++) {
		if ((ptp_infor->dev[channel].status == PTP_START) &&
			...
			if ((dev->role == PTP_MASTER) &&
				...
				wifi_audio_log_info(
					"write time reg timer cancel");
				hrtimer_cancel(
					&(ptp_infor->write_time_reg_timer));
...
static int get_delay_event_id(const struct ptp_dev *dev, unsigned char data)
{
	int id;
	if (dev == NULL) {
...
static int get_ptp_event_data_id(const struct ptp_dev *dev, unsigned char data)
{
	int id;
	if (dev == NULL) {
		wifi_audio_log_err("dev is NULL Pointer");
...
static int pcm_skb_match(__be32 ip_dest, __be32 ip_source, __be16 port_dest,
			 const struct wifi_audio_ptp *ptp)
{
	int i;
	if (ptp == NULL) {
				  ...
				  struct ptp_event_data *event)
{
	...
	if ((event->id < PTP_SEND_SYNC_REQUEST) ||
	    (event->id >= PTP_EVENT_ID_MAX))
		event->id = get_ptp_event_data_id(
			dev, event->msg[MSG_SUB_TYPE_INDEX]);
...
static void event_handle_work_handle(unsigned long data)
{
	...
	while (list_queue_is_empty(&(ptp->waudio_skb_handle)) == 0) {
		...
		if (sec_ret != 0) {
			...
			dev_kfree_skb_any(
				((struct waudio_skb *)(node->data))->skb);
			 ...
			 unsigned int len)
{
	...
	sec_ret +=
		memcpy_s(ethdr->h_source, ETH_ALEN, dev->mac_source, ETH_ALEN);
	if (sec_ret != 0) {
...
static void sync_offset_report(const struct ptp_dev *dev, long long offset_us)
{
...
static void data_sync_status_report(const struct ptp_dev *dev, int status)
{
...
static void ptp_time_calculation_earlier_stage(struct ptp_dev *dev)
{
	...
	if ((list_queue_len(&(dev->ptp_time_queue)) <= FIRST_SET_ARRAY_LEN) &&
		...
		if (dev->cur_ptp_time.counter <= PTP_TIME_FIRST_SET_THRES) {
			...
			wifi_audio_log_info(
				"First set system time offset %lld offset_pre %lld",
			...
			if (dev->cur_ptp_time.counter ==
				...
				sync_offset_report(
					dev, dev->cur_ptp_time.offset_pre);
			return;
			...
			list_for_each(p, &(dev->ptp_time_queue.queue)) {
				...
				first_set_array[i] =
					((struct time_infor *)(node->data))
						->offset;
				i++;
			}
			sort(first_set_array, FIRST_SET_ARRAY_LEN,
			     sizeof(long long), ptp_time_cmp, NULL);
		...
		if ((abs(dev->cur_ptp_time.offset) >= STOP_DATA_SYNC_THRES)) {
			...
			dev->data_sync_stop_counter++;
			if ((dev->data_sync_stop_counter >
			     STOP_DATA_SYNC_TRIGGER) &&
			...
			dev->data_sync_start_counter++;
			if ((dev->data_sync_start_counter >
			     START_DATA_SYNC_TRIGGER) &&
			    (dev->data_sync_status == DATA_SYNC_STOP)) {
...
static void ptp_time_calculation_later_stage(struct ptp_dev *dev)
{
	...
	if ((list_queue_len(&(dev->ptp_time_queue)) == PTP_TIME_QUEUE_LEN) &&
	    ...
	    (dev->cur_ptp_time.t4.tv_usec < PTP_TIME_SET_USEC_THRES)) {
		i = 0;
		list_for_each(p, &(dev->ptp_time_queue.queue)) {
			node = list_entry(p, struct list_node, list);
			ptp_time_array[i] =
		...
		sort(ptp_time_array, PTP_TIME_QUEUE_LEN, sizeof(long long),
		     ptp_time_cmp, NULL);
		stand = ptp_standard_deviation(
			&ptp_time_array[PTP_TIME_STANDARD_CAL_START],
...
static void ptp_time_calculation(struct ptp_dev *dev)
{
	...
	/* Calculate offset */
	dev->cur_ptp_time.offset =
		((((long long)(dev->cur_ptp_time.t2.tv_sec) * US_IN_SECOND) +
		  (long long)dev->cur_ptp_time.t2.tv_usec) -
		 (((long long)(dev->cur_ptp_time.t4.tv_sec) * US_IN_SECOND) +
		  (long long)dev->cur_ptp_time.t4.tv_usec) +
		 (((long long)(dev->cur_ptp_time.t3.tv_sec) * US_IN_SECOND) +
		  (long long)dev->cur_ptp_time.t3.tv_usec) -
		 (((long long)(dev->cur_ptp_time.t1.tv_sec) * US_IN_SECOND) +
		  (long long)dev->cur_ptp_time.t1.tv_usec)) /
		2;
	/* Calculate delay */
	dev->cur_ptp_time.delay =
		((((long long)(dev->cur_ptp_time.t2.tv_sec) * US_IN_SECOND) +
		  (long long)dev->cur_ptp_time.t2.tv_usec) +
		 (((long long)(dev->cur_ptp_time.t4.tv_sec) * US_IN_SECOND) +
		  (long long)dev->cur_ptp_time.t4.tv_usec) -
	...
	wifi_audio_log_info(
		"channel %u role %d counter %u offset %lld delay %lld t4 sec %u usec %u",
		dev->channel, dev->role, dev->cur_ptp_time.counter,
		dev->cur_ptp_time.offset, dev->cur_ptp_time.delay,
		dev->cur_ptp_time.t4.tv_sec, dev->cur_ptp_time.t4.tv_usec);
				     ...
				     const struct ptp_event_data *event)
{
	...
	ret = memcpy_s(&msg[MSG_PTP_TV_SEC_INDEX],
		       MSG_LEN - MSG_PTP_TV_SEC_INDEX,
		       &(dev->cur_ptp_time.t4.tv_sec),
		       sizeof(dev->cur_ptp_time.t4.tv_sec));
	if (ret != 0) {
		wifi_audio_log_err("memcpy fail \n");
		return ERROR;
...
static void ptp_offset_delay_report(
	const struct netlink_event_report_offset_delay *offset_delay)
{
	struct netlink_data netlink_event = {0};
	int ret;
				 ...
				 const struct ptp_event_data *event)
{
	...
	ret = memcpy_s(&(dev->cur_ptp_time.offset),
		       sizeof(dev->cur_ptp_time.offset),
		       &(event->msg[MSG_PTP_OFFSET_INDEX]),
		       sizeof(dev->cur_ptp_time.offset));
	ret += memcpy_s(&(dev->cur_ptp_time.delay),
			sizeof(dev->cur_ptp_time.delay),
			&(event->msg[MSG_PTP_DELAY_INDEX]),
			sizeof(dev->cur_ptp_time.delay));
	if (ret != 0) {
		wifi_audio_log_err("memcpy fail \n");
		return ERROR;
	...
	if ((list_queue_is_empty(&(dev->ptp_time_queue)) != 0) &&
		...
		wifi_audio_log_info("report offset %lld delay %lld",
				    offset_delay.offset_usec,
				    offset_delay.delay_usec);
	...
	if (list_queue_len(&(dev->ptp_time_queue)) == PTP_TIME_QUEUE_LEN) {
		...
		sort(offset_array, PTP_TIME_QUEUE_LEN, sizeof(long long),
		     ptp_time_cmp, NULL);
		sort(delay_array, PTP_TIME_QUEUE_LEN, sizeof(long long),
		     ptp_time_cmp, NULL);
		offset_delay.channel = dev->channel;
		offset_delay.offset_usec =
			ptp_average(&offset_array[PTP_TIME_STANDARD_CAL_START],
				    PTP_TIME_STANDARD_CAL_LEN);
		offset_delay.delay_usec =
			ptp_average(&delay_array[PTP_TIME_STANDARD_CAL_START],
				    PTP_TIME_STANDARD_CAL_LEN);
		ptp_offset_delay_report(&offset_delay);
		list_queue_clean(&(dev->ptp_time_queue));
	}
...
static int ptp_receive_restart_time_sync(struct ptp_dev *dev)
{
	...
	wifi_audio_log_info("channel %d status %d role %d counter %d",
			    dev->channel, dev->status, dev->role,
			    dev->cur_ptp_time.counter);
			       ...
			       const struct ptp_event_data *event)
{
	...
	case PTP_RECEIVE_RESTART_TIME_SYNC: {
		...
		if (ret != SUCCESS)
			wifi_audio_log_err(
				"ptp receive restart timesync, error");
		break;
	}
...
static void dev_event_handle(unsigned long data)
{
	...
	while (list_queue_is_empty(&(dev->dev_event_queue)) == 0) {
		...
		sec_ret =
			memcpy_s(&event, sizeof(event), node->data, node->len);
		if (sec_ret != 0) {
			wifi_audio_log_err("memcpy node->data fail");
			free_list_node(node);
			node = NULL;
...
static void time_sync_timeout_handle(struct work_struct *work)
{
	...
	if (dev->time_sync_timeout_counter >=
...
#endif
		ptp_send_stop_timesync(dev);
		time_sync_status_report(dev, PTP_ABNORMAL,
					TIME_SYNC_ABNORMAL_TIMEOUT);
		ptp_timesync_set_status(dev, PTP_ABNORMAL);
		spin_unlock_bh(&ptp_infor_lock);
		return;
...
static enum hrtimer_restart time_sync_timeout_timer(struct hrtimer *timer)
{
	...
	wifi_audio_log_info("time sync timeout ch %u status %d", dev->channel,
			    dev->status);
	if (dev->status == PTP_START)
...
static enum hrtimer_restart write_time_reg_handle(struct hrtimer *timer)
{
	...
	if (((ptp->dev[CHANNEL_LEFT].status == PTP_START) &&
		...
		time_us_h = (time_us >> 32) &
			    0x00000000FFFFFFFF; /* hanle high 32bit */
		__raw_writel(time_us_l, ptp->time_regs);
		__raw_writel(
			time_us_h,
			(void __iomem *)((unsigned char *)(ptp->time_regs) +
					 4));
...
static int ptp_start_timesync(const struct netlink_data *msg_rev)
{
	...
	ptp_infor->dev[channel].net_dev = dev_get_by_name(
		&init_net, ptp_infor->dev[channel].interface_name);
	...
	ptp_infor->dev[channel].cur_ptp_time.offset_pre =
		SECOND_IN_HALF_HOUR * SECOND_IN_MINUTE * US_IN_SECOND;
...
int wifi_audio_time_sync_init(void)
{
	...
	for (i = 0; i < CHANNEL_MAX; i++) {
		...
		INIT_WORK(&(ptp->dev[i].start_ptp_work), start_ptp_work_handle);

		hrtimer_init(&(ptp->dev[i].time_sync_timeout), CLOCK_MONOTONIC,
			     HRTIMER_MODE_REL);
		ptp->dev[i].time_sync_timeout.function =
			time_sync_timeout_timer;
