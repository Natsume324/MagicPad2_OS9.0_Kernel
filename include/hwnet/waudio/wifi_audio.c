...
static int netdev_notifier_event(struct notifier_block *this,
				 unsigned long event, void *ptr);
static void netlink_event_send_tasket(unsigned long arg);
DECLARE_TASKLET(netlink_event_report_tasklet, netlink_event_send_tasket, 0);
...
static void netlink_msg_receive_handle(struct work_struct *work)
{
	...
	while (list_queue_is_empty(&(ctrl->receive_queue)) == 0) {
		...
		sec_ret = memcpy_s(&msg_rev, sizeof(msg_rev), node->data,
				   node->len);
		free_list_node(node);
		node = NULL;
		if (sec_ret != 0) {
			    ...
			    const struct netlink_data *msg_rev)
{
	...
	ctrl = pctrl;
	wifi_audio_log_info("id = %d, len = %u", msg_rev->id, msg_rev->msg_len);
...
static void netlink_event_receive(struct sk_buff *netlink_skb)
{
	...
	if ((wifi_audio_netlink == NULL) || (netlink_skb == NULL)) {
	...
	sec_ret = memcpy_s(&msg_rev, sizeof(msg_rev), NLMSG_DATA(nlh),
			   sizeof(msg_rev));
	if (sec_ret != 0) {
		wifi_audio_log_err(
			"memcpy_s ERROR! bt_netlink_event_receive return");
		kfree_skb(skb);
		return;
	}
	...
	if (msg_rev.id == NETLINK_MSG_SET_USER_ID) {
		...
		wifi_audio_log_info("set netlink user ID pctrl->user_pid %u",
				    pctrl->user_pid);
		ret = wifi_audio_time_sync_init();
		if (ret != SUCCESS)
			wifi_audio_log_err("WiFi audio time sync init failed");
	} else {
		wifi_audio_log_info("set data msg_rev.id %d, nlmsg_pid %d",
				    msg_rev.id, nlh->nlmsg_pid);
...
static int netlink_init(void)
{
	...
	pctrl->dev_sock = netlink_kernel_create(
		&init_net, NETLINK_WIFI_AUDIO_EVENT, &config_cfg);
	if (pctrl->dev_sock == NULL) {
		kfree(pctrl);
		pctrl = NULL;
