...
void add_ad_uid_rule(struct list_info *listinfo, const char *rule, int len)
{
	...
	if (uid < 1000) {
		pr_info("hnad: error EOK %s %d uid %d", __func__, __LINE__,
			uid);
		return;
	}
...
void clear_ad_rule_string(struct list_info *listinfo, const char *rule)
{
	...
	while (p && *p != 0) {
		...
		if (p) {
			...
			if (uid < 1000) {
				pr_info("hnad: error EOK %s %d uid %d",
...
bool match_ab_string(const char *str, const char *a, int al, const char *b,
		     int bl)
{
	const char *ps;
...
bool match_url_string(const char *url, const char *filter, int len)
{
	...
	if (filter == ps) {
		...
		/*4. xxx*yyy*/
		return match_ab_string(url, filter, ps - filter, ps + 1,
				       filter + len - ps - 1);
	}
}
		  ...
		  const char *data, int datalen)
{
	...
	if (listinfo == NULL || data == NULL || uid == 0xffffffff ||
	    datalen <= 0)
		return false;
