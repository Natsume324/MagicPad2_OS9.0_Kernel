...
static unsigned long abn_stamp_list_syn_no_ack_update(unsigned long time_stamp);
static unsigned long abn_stamp_list_web_no_ack_update(unsigned long time_stamp);
static unsigned long abn_stamp_list_web_fail_update(unsigned long time_stamp);
static unsigned long abn_stamp_list_web_delay_update(unsigned long time_stamp);
static unsigned long
abn_stamp_list_tcp_rtt_large_update(unsigned long time_stamp);
...
static u32 identify_network_protocol(struct sk_buff *skb, struct ipv6hdr **ip6h,
...
void chr_no_ack_report(u32 hash_cnt, u8 if_type)
{
	...
	} else if (type == HTTP_SYN &&
		   time_after(jiffies, g_stream_list[hash_cnt].time_stamp +
					       DELETE_TIME)) {
...
u8 hash3_v6(struct in6_addr dst, struct in6_addr src, u32 port)
{
	...
	hash = dst.s6_addr32[IPV6_ADD_P1] + dst.s6_addr32[IPV6_ADD_P2] +
	       ...
	       src.s6_addr32[IPV6_ADD_P3] + src.s6_addr32[IPV6_ADD_P4] + port;
	hash = hash + hash / HASH_MAX + hash / HASH_MAX_16_BIT +
	       hash / HASH_MAX_24_BIT;
	hash = hash % HASH_MAX;
	return (u8)hash;
...
bool stream_list_http_para_equal(struct http_stream http_para,
				 struct http_stream stream_list,
				 bool is_http_para_in)
{
	u8 proto;
...
u8 get_hash_cnt(struct http_stream http_para)
{
	...
	if (http_para.proto == IPV6_NETWORK)
		...
		hash_cnt = hash3(http_para.dst_addr, http_para.src_addr,
				 http_para.tcp_port);
	return hash_cnt;
}
...
void wifi_kernel_delay_report(struct delay_chr_report *delay_chr)
{
	...
	g_rtn_stat_wifi[WLAN_INTERFACE].exception_cnt =
		delay_chr->exception_cnt;
...
void http_para_in_syn_succ(u8 interface_type, u8 hash_cnt)
{
	...
	if (g_http_para_in.time_stamp >= g_stream_list[hash_cnt].time_stamp)
		...
		handshake_delay =
			(MAX_JIFFIES - g_stream_list[hash_cnt].time_stamp +
			 g_http_para_in.time_stamp) *
			MULTIPLE;
...
void chr_rat_change_notify_event(void)
{
	...
	is_need_notify =
		is_notify_chr_event(g_old_data_reg_tech, g_data_reg_tech);
	if (!is_need_notify)
		return;
...
void tcp_win_monitor(struct sock *sk, struct tcphdr *th, char *p_http_str,
		     s64 dlen)
{
...
static s64 get_dlen(const char *p_http_str, bool is_ipv6_pkt,
...
u32 get_http_stream_iface(struct sk_buff *skb, bool is_http_para_in)
{
	...
	if (strncmp(skb->dev->name, WEB_DS_NET, WEB_DS_NET_LEN) == 0 ||
	    strncmp(skb->dev->name, WEB_DS_NET_SPRD, WEB_DS_NET_SPRD_LEN) ==
		    0) {
				   ...
				   const struct nf_hook_state *state)
{
	...
	if (skb->data_len > HTTP_RSP_LEN && (p_http_str - skb->data > 0)) {
		if (skb_copy_bits(skb, p_http_str - skb->data,
				  (void *)http_resp, HTTP_RSP_LEN) == 0) {
...
void set_http_para_in(bool is_ipv6_pkt, struct ipv6hdr *ip6h, struct iphdr *iph,
		      struct tcphdr *tcph)
{
	if ((is_ipv6_pkt && ip6h == NULL) || (!is_ipv6_pkt && iph == NULL) ||
	    tcph == NULL)
...
int is_http_response(char *p_http_str, s64 dlen, struct tcphdr *tcph)
{
	if (dlen > HTTP_RSP_LEN &&
	    ...
	    g_http_para_in.is_valid == IS_UNUSE)
		return 1;
	else if (g_http_para_in.is_valid == IS_UNUSE && tcph->syn == 1 &&
		 tcph->ack == 1)
		return -1;
	return 0;
}
...
int web_chr_init(void)
{
	...
	g_stream_list =
		kmalloc(sizeof(struct http_stream) * HASH_MAX, GFP_KERNEL);
	if (g_stream_list == NULL)
		return -1;
...
unsigned long abn_stamp_list_syn_no_ack_update(unsigned long time_stamp)
{
...
unsigned long abn_stamp_list_web_no_ack_update(unsigned long time_stamp)
{
...
{
	g_abn_stamp_list_web_fail[g_abn_stamp_list_web_fail_idx] = time_stamp;
...
unsigned long abn_stamp_list_web_delay_update(unsigned long time_stamp)
{
...
unsigned long abn_stamp_list_tcp_rtt_large_update(unsigned long time_stamp)
{
	int idx;
