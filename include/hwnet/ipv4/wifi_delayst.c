...
void skbprobe_record_first(struct sk_buff *skb, u32 type)
{
	...
	delayst_skb_cb(skb)->pdirect =
		((type == 0) ? (TP_SKB_DIRECT_SND) : (TP_SKB_DIRECT_RCV));
...
int delay_record_get_interval_index(uint32_t delay)
{
	const int threshold_values[THRESHOLD_INIT] = {INTERVAL_THRESHOLD_FIRST,
						      INTERVAL_THRESHOLD_SECOND,
						      INTERVAL_THRESHOLD_THIRD,
...
void delay_record_send(delay_stat_t *delay, struct sk_buff *skb)
{
	...
	for (i = 0; i < THRESHOLD_INIT; ++i) {
		...
		delay->t_gap[i + 1][delay_record_get_interval_index(un_delay)]++;
	}
}
	...
	if (is_direct(skb, TP_SKB_DIRECT_RCV)) {
		...
		if (packet_is_enough_for_print(delay_s, TP_SKB_DMAC)) {
			printk("SumPkts record in driver: %u ,Send Delay [<50us, <200us, <1ms, <10ms, >=10ms]transport:[%u, %u, %u, %u, %u ]ip:[%u, %u, %u, %u, %u ]hmac:[%u, %u, %u, %u, %u ]driver:[%u, %u, %u, %u, %u ]Average: transport %u, ip %u,hmac %u,driver %u",
			       delay_s.t_totalpkts[TP_SKB_DMAC], GET_SND_IP_ALL,
			       GET_XMIT_ALL, GET_TX_ALL, GET_DMAC_ALL,
			       GET_SND_AVG_IP, GET_AVG_XMIT, GET_AVG_TX,
			       GET_AVG_DMAC);
...
void delay_flow_ctl(struct sk_buff *skb)
{
	...
	if ((kt - t_begin) > INTERVAL_TIME) {
		...
		if ((total_pkt > DELAY_FLOW_THRESHOLD && delayst_switch) ||
		    (total_pkt < DELAY_FLOW_THRESHOLD && (!delayst_switch))) {
			/* overflow/lowflow */
			delay_cnt++;
...
static void print_exceed_tx(u8 delay_exceed_count_tx, delayskb_cb_t *tx_buff)
{
	...
	delay_excp_chr_report.driver_delay = skbprobe_get_latency_from_delayst(
		tx_buff, TP_SKB_HMAC_TX, TP_SKB_DMAC);
	delay_excp_chr_report.hmac_delay = skbprobe_get_latency_from_delayst(
		tx_buff, TP_SKB_HMAC_XMIT, TP_SKB_HMAC_TX);
	delay_excp_chr_report.ip_delay = skbprobe_get_latency_from_delayst(
		tx_buff, TP_SKB_IP, TP_SKB_HMAC_XMIT);
...
static void print_exceed_rx(u8 delay_exceed_count_rx, delayskb_cb_t *rx_buff)
{
	...
	delay_excp_chr_report.exception_cnt = delay_exceed_count_rx;
	delay_excp_chr_report.driver_delay = skbprobe_get_latency_from_delayst(
		rx_buff, TP_SKB_DMAC, TP_SKB_HMAC_UPLOAD);
	delay_excp_chr_report.ip_delay = skbprobe_get_latency_from_delayst(
		rx_buff, TP_SKB_HMAC_UPLOAD, TP_SKB_IP);
	delay_excp_chr_report.transport_delay =
...
int delay_set_wifi_command(const char *val)
{
	...
	const char flag_names[][FLAG_NAME_LENGTH] = {"flag_off", "flag_on",
						     "flag_auto"};
	...
	case 'c':
		printk("delay_command configure:delay_switch = %s,dealy_mode = %s,print_interval = %u,android_uid = %u",
		       ...
		       mode_names[settings.dp_mode], settings.print_interval,
		       settings.android_uid);
		break;
	default:
		break;
	}
