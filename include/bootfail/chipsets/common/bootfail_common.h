...
#define GET_BFI_DEV_PATH _IOR(BOOT_DETECTOR_IOCTL_BASE, 9, struct bfi_dev_path)
#define LEGACY_ENABLE_CTRL _IOW(BOOT_DETECTOR_IOCTL_BASE, 10, int)
#define ACTION_TIMER_CTL \
	_IOW(BOOT_DETECTOR_IOCTL_BASE, 11, struct action_ioctl_data)
#define IOC_CREATE_DYNAMIC_RAMDISK \
	_IOWR(BOOT_DETECTOR_IOCTL_BASE, 12, struct dbrd_ioctl_block)
#define IOC_DELETE_DYNAMIC_RAMDISK \
	_IOW(BOOT_DETECTOR_IOCTL_BASE, 13, struct dbrd_ioctl_block)
#define IOC_CHECK_BOOTDISK_WP \
	_IOWR(BOOT_DETECTOR_IOCTL_BASE, 14, struct bootdisk_wp_status_iocb)
#define IOC_ENABLE_MONITOR \
	_IOW(BOOT_DETECTOR_IOCTL_BASE, 15, struct storage_rochk_iocb)
#define IOC_DO_STORAGE_WRTRY \
	_IOWR(BOOT_DETECTOR_IOCTL_BASE, 16, struct storage_rochk_iocb)
#define IOC_GET_STORAGE_ROFA_INFO \
	_IOWR(BOOT_DETECTOR_IOCTL_BASE, 17, struct storage_rofa_info_iocb)
#define IOC_GET_BOOTDEVICE_DISK_COUNT \
	_IOWR(BOOT_DETECTOR_IOCTL_BASE, 18, struct storage_rochk_iocb)
#define IOC_GET_BOOTDEVICE_DISK_INFO \
	_IOWR(BOOT_DETECTOR_IOCTL_BASE, 19, struct bootdevice_disk_info_iocb)
#define IOC_GET_BOOTDEVICE_PROD_INFO \
	_IOWR(BOOT_DETECTOR_IOCTL_BASE, 20, struct bootdevice_prod_info_iocb)
...
#define SIMULATE_STORAGE_RDONLY _IOW(BOOT_DETECTOR_IOCTL_BASE, 1001, int)

#define DISK_NAME_MAX_SIZE 32
#define BOOTDEVICE_PROD_INFO_MAX_SIZE 64

#define print_invalid_params(format, ...)                              \
	pr_err("bootfail: func: %s invalid params: " format, __func__, \
	       ##__VA_ARGS__)
#define print_err(format, ...)                                             \
	pr_err("bootfail: func: %s line: %d, " format, __func__, __LINE__, \
	       ##__VA_ARGS__)
#define print_info(format, ...) pr_err("bootfail: " format, ##__VA_ARGS__)
#define print_enter(format, ...)                                               \
	pr_err("bootfail: enter [%s], line: %d.\n" format, __func__, __LINE__, \
	       ##__VA_ARGS__)
#define print_exit(format, ...)                                               \
	pr_err("bootfail: exit [%s], line: %d.\n" format, __func__, __LINE__, \
	       ##__VA_ARGS__)
