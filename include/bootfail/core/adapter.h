...
struct adapter {
	...
	struct {
		...
		unsigned long size;
		struct {
			int (*read)(unsigned long dst, unsigned long dst_max,
				    void *phys_mem_addr,
				    unsigned long data_len);
			int (*write)(unsigned long dst, unsigned long dst_max,
				     void *src, unsigned long src_len);
		} ops;
	} pyhs_mem_info;
	struct {
		int (*memset_s)(void *dst, unsigned long dst_max, int c,
				unsigned long count);
		...
		int (*memcmp)(const void *dst, const void *src,
			      unsigned long count);
		unsigned long (*bootfail_strlen)(const char *s);
	} sec_funcs;
	struct {
	...
	struct {
		void (*reboot)(const char *detail, unsigned int len);
		void (*shutdown)(const char *detail, unsigned int len);
	} sys_ctl;
	struct {
	...
	struct {
		...
		void (*capture_bl_log)(char *pbuf, unsigned int buf_size);
	} bl_log_ops;
	struct {
		char log_name[16]; /* max file name length */
		unsigned int log_size;
		void (*capture_kernel_log)(char *pbuf, unsigned int buf_size);
	} kernel_log_ops;
	struct {
