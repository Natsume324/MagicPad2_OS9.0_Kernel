...
#endif
#ifndef BF_SIZE_2M
#define BF_SIZE_2M (BF_SIZE_1K * BF_SIZE_1K * 2)
#endif
...
#define is_bottom_stage(stage) (is_bl1_stage(stage) || (is_bl2_stage(stage)))
#define is_upper_stage(stage)                                \
	(is_kernel_stage(stage) || is_native_stage(stage) || \
	 is_framework_stage(stage) || is_after_suss_stage(stage))
#define is_valid_stage(stage) (is_bottom_stage(stage) || is_upper_stage(stage))
 ...
 * You're strongly recommended to dynamically allocate memory for the struct.
 */
#define BOOTFAIL_DETAIL_INFO_LEN 384
struct bootfail_proc_param {
	unsigned int magic;
	struct bootfail_basic_info binfo;
	char detail_info[BOOTFAIL_DETAIL_INFO_LEN]; /* max exception info count */
