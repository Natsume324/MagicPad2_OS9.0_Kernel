...
#define EMCOM_WLAN0_IFNAME \
	emcom_xengine_get_network_iface_name(EMCOM_XENGINE_NET_WLAN0)
#define EMCOM_CELL0_IFNAME \
	emcom_xengine_get_network_iface_name(EMCOM_XENGINE_NET_CELL0)
#define EMCOM_WLAN1_IFNAME \
	emcom_xengine_get_network_iface_name(EMCOM_XENGINE_NET_WLAN1)
#define EMCOM_CELL1_IFNAME \
	emcom_xengine_get_network_iface_name(EMCOM_XENGINE_NET_CELL1)
...
#define EMCOM_XENGINE_GET_SPEEDCTRL_INFO(speedctrl_info, uid_value, \
					 size_value)                \
	do {                                                        \
		spin_lock_bh(&((speedctrl_info).stlocker));         \
...
#define IPV6_INFO(addr)                                                      \
	IP6_ADDR_BLOCK1(addr), IP6_ADDR_BLOCK2(addr), IP6_ADDR_BLOCK3(addr), \
		IP6_ADDR_BLOCK4(addr), IP6_ADDR_BLOCK5(addr),                \
		IP6_ADDR_BLOCK6(addr), IP6_ADDR_BLOCK7(addr),                \
...
#define IPV6_FMT "%x:***:%x"
#define IPV6_INFO(addr) IP6_ADDR_BLOCK1(addr), IP6_ADDR_BLOCK8(addr)
#endif

#define emcom_xengine_is_net_type_valid(net_type)  \
	((net_type) > EMCOM_XENGINE_NET_INVALID && \
	 (net_type) < EMCOM_XENGINE_NET_MAX_NUM)
