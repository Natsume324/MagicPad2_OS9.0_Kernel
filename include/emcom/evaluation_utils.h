...
#define PROB_CONVERT_COUNT(x1, y1, z1, x2, y2, z2, x3, y3, z3)           \
	{                                                                \
		{(x1 * RTT_COUNT / 100), (y1 * RTT_COUNT / 100),         \
		 (z1 * RTT_COUNT / 100)},                                \
			{(x2 * RTT_COUNT / 100), (y2 * RTT_COUNT / 100), \
			 (z2 * RTT_COUNT / 100)},                        \
		{                                                        \
			(x3 * RTT_COUNT / 100), (y3 * RTT_COUNT / 100),  \
				(z3 * RTT_COUNT / 100)                   \
		}                                                        \
	}
/* validate parameters */
#define VALIDATE_NETWORK_TYPE(parameter)                                      \
	(NETWORK_TYPE_UNKNOWN == parameter || NETWORK_TYPE_2G == parameter || \
	 NETWORK_TYPE_3G == parameter || NETWORK_TYPE_4G == parameter ||      \
...
#define VALIDATE_POINT(pt)                                                    \
	(pt.x >= 0 && pt.x <= 100 && pt.y >= 0 && pt.y <= 100 && pt.z >= 0 && \
	 pt.z <= 100)
#define INDEX_X 0
#define INDEX_Y 1
#define INDEX_Z 2
...
int8_t cal_rtt_stability_index(int8_t network_type, int32_t avg,
			       int32_t sqrdev);
