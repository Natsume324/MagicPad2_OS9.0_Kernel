...
void xreclaimer_cond_quick_reclaim(struct task_struct *tsk, int sig,
				   struct kernel_siginfo *info, bool skip)
{
	/* Signal from kernel may hold task lock, so... */
	if (!sysctl_xreclaimer_enable || skip || !tsk || sig != SIGKILL ||
	    !__is_si_fromuser(info))
		return;
...
static bool xreclaimer_can_quick_reclaimed(struct task_struct *tsk)
{
	...
	if (!tsk->signal || tsk_is_oom_victim(tsk))
		return false;
...
static bool xreclaimer_mm_is_oom_victim(struct mm_struct *mm)
{
#ifdef CONFIG_MEMCG
	...
	is_victim = task && tsk_is_oom_victim(task);
	rcu_read_unlock();
	if (is_victim)
		return true;
#endif
...
void xreclaimer_exit_mmap(struct mm_struct *mm)
{
	...
	if (xreclaimer_mm_is_oom_victim(mm))
		return;
