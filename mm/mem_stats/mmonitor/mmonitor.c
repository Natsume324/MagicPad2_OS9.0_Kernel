...
static int mmonitor_show(struct seq_file *s, void *data)
{
	...
	seq_printf(s,
		   ...
		   atomic_long_read(&slowpath_pgalloc_count[0]),
		   atomic_long_read(&slowpath_pgalloc_count[1]),
		   atomic_long_read(&slowpath_pgalloc_count[2]),
		   atomic_long_read(&slowpath_pgalloc_count[3]),
		   atomic_long_read(&slowpath_pgalloc_count[4]),
#endif
#ifdef CONFIG_VM_EVENT_COUNTERS
		   vm_buf[COMPACTSTALL], vm_buf[COMPACTSUCCESS],
		   mmonitor_buf[FILE_CACHE_MISS_COUNT] + vm_buf[PGMAJFAULT],
#endif
		   mmonitor_buf[ALLOC_FAILED_COUNT],
		   mmonitor_buf[FILE_CACHE_READ_COUNT] +
			   mmonitor_buf[FILE_CACHE_MAP_COUNT]);
...
static ssize_t mmonitor_write(struct file *file, const char *buffer,
...
static const struct proc_ops mmonitor_file_operations = {
	.proc_open = mmonitor_open,
	.proc_write = mmonitor_write,
	.proc_read = seq_read,
