...
static int g_reclaimacct_disable = 0;
...
static struct reclaim_acct *reclaimacct_alloc(void)
{
	...
	if (g_mempool_index >= 0 && g_mempool_index < NR_POOLMEMBER) {
...
{
	spin_lock(&g_mempool_lock);
	if (g_mempool_index >= -1 && g_mempool_index < NR_POOLMEMBER - 1) {
		g_mempool_index++;
		/*requester init -1*/
		elem->requester = -1;
...
void reclaimacct_directreclaim_start(void)
{
	...
	if (!g_reclaimacct_disable && !g_reclaimacct_is_off &&
		...
		if (!current->reclaim_acct)
			return;
		current->reclaim_acct->requester = RA_DIRECTRECLAIM;
	}
...
static void __reclaimacct_collect_data(int level,
				       enum reclaimacct_stubs reclaimacct_stub)
{
	...
	g_reclaimacct_show->delay[level][reclaimacct_stub] +=
		current->reclaim_acct->delay[reclaimacct_stub];
	g_reclaimacct_show->count[level][reclaimacct_stub] +=
		current->reclaim_acct->count[reclaimacct_stub];

	if (current->reclaim_acct->delay[reclaimacct_stub] >
...
static void reclaimacct_collect_data(enum reclaimacct_stubs reclaimacct_stub)
{
	int i;
	const u64 delay[NR_DELAY_LV] = {DELAY_LV0, DELAY_LV1, DELAY_LV2,
					DELAY_LV3, DELAY_LV4, DELAY_LV5};

	for (i = 0; i < NR_DELAY_LV; i++) {
		if (current->reclaim_acct->delay[reclaimacct_stub] < delay[i]) {
			__reclaimacct_collect_data(i, reclaimacct_stub);
			break;
		}
	}
}
...
void reclaimacct_directreclaim_end(void)
{
	...
	reclaimacct_end(&(current->reclaim_acct->start[RA_DIRECTRECLAIM]),
			...
			&(current->reclaim_acct->count[RA_DIRECTRECLAIM]), NULL,
			NULL);

	if (current->reclaim_acct->delay[RA_DIRECTRECLAIM] > DELAY_LV4)
		pr_warn_ratelimited(
			"Summary %s=%llu %llu %s=%llu %llu %s=%llu %llu %s=%llu %llu %s=%llu %llu\n",
			DIRECT_RECLAIM_STR,
			current->reclaim_acct->delay[RA_DIRECTRECLAIM],
	...
	reclaimacct_collect_data(RA_DIRECTRECLAIM);
	reclaimacct_clear(&(current->reclaim_acct->start[RA_DIRECTRECLAIM]),
			  &(current->reclaim_acct->delay[RA_DIRECTRECLAIM]),
			  &(current->reclaim_acct->count[RA_DIRECTRECLAIM]));

	if (current->reclaim_acct->requester == RA_DIRECTRECLAIM) {
...
void reclaimacct_drainallpages_start(void)
{
	if (!g_reclaimacct_disable && !g_reclaimacct_is_off &&
		...
		if (!current->reclaim_acct)
			return;
		current->reclaim_acct->requester = RA_DRAINALLPAGES;
	}
...
void reclaimacct_drainallpages_end(void)
{
	...
	reclaimacct_collect_data(RA_DRAINALLPAGES);
	reclaimacct_clear(&(current->reclaim_acct->start[RA_DRAINALLPAGES]),
			  &(current->reclaim_acct->delay[RA_DRAINALLPAGES]),
			  &(current->reclaim_acct->count[RA_DRAINALLPAGES]));

	if (current->reclaim_acct->requester == RA_DRAINALLPAGES) {
		reclaimacct_free(current->reclaim_acct);
		current->reclaim_acct = NULL;
	}
}
...
void reclaimacct_shrinklist_start(int file)
{
	if (file) {
		if (!g_reclaimacct_disable && !g_reclaimacct_is_off &&
			...
			if (!current->reclaim_acct)
				return;
			current->reclaim_acct->requester = RA_SHRINKFILE;
		}
		if (!current->reclaim_acct)
			return;
		...
		if (!g_reclaimacct_disable && !g_reclaimacct_is_off &&
			...
			if (!current->reclaim_acct)
				return;
			current->reclaim_acct->requester = RA_SHRINKANON;
		}
		if (!current->reclaim_acct)
			return;
...
void reclaimacct_shrinklist_end(int file)
{
	...
	if (file) {
		...
		reclaimacct_collect_data(RA_SHRINKFILE);
		reclaimacct_clear(
			&(current->reclaim_acct->start[RA_SHRINKFILE]),
			&(current->reclaim_acct->delay[RA_SHRINKFILE]),
			&(current->reclaim_acct->count[RA_SHRINKFILE]));

		if (current->reclaim_acct->requester == RA_SHRINKFILE) {
			reclaimacct_free(current->reclaim_acct);
			current->reclaim_acct = NULL;
		}
	} else {
		...
		reclaimacct_collect_data(RA_SHRINKANON);
		reclaimacct_clear(
			&(current->reclaim_acct->start[RA_SHRINKANON]),
			&(current->reclaim_acct->delay[RA_SHRINKANON]),
			&(current->reclaim_acct->count[RA_SHRINKANON]));

		if (current->reclaim_acct->requester == RA_SHRINKANON) {
			reclaimacct_free(current->reclaim_acct);
			current->reclaim_acct = NULL;
		}
	}
...
void reclaimacct_shrinkslab_start(void)
{
	if (!g_reclaimacct_disable && !g_reclaimacct_is_off &&
		...
		if (!current->reclaim_acct)
			return;
		current->reclaim_acct->requester = RA_SHRINKSLAB;
	}
...
void reclaimacct_shrinkslab_end(const void *shrinker)
{
	...
	reclaimacct_collect_data(RA_SHRINKSLAB);
	reclaimacct_clear(&(current->reclaim_acct->start[RA_SHRINKSLAB]),
			  &(current->reclaim_acct->delay[RA_SHRINKSLAB]),
			  &(current->reclaim_acct->count[RA_SHRINKSLAB]));

	if (current->reclaim_acct->requester == RA_SHRINKSLAB) {
		reclaimacct_free(current->reclaim_acct);
		current->reclaim_acct = NULL;
	}
}
...
static int reclaimacct_init_handle(void *p)
{
	...
	g_reclaimacct_show =
		kzalloc(sizeof(struct reclaimacct_show), GFP_KERNEL);
	if (!g_reclaimacct_show)
		goto alloc_show_failed;

	alloc_cnt = 0; /* For safe */
	for (i = 0; i < NR_POOLMEMBER; i++) {
		g_mempool[i] = kzalloc(sizeof(struct reclaim_acct), GFP_KERNEL);
		if (!g_mempool[i]) {
			alloc_cnt = i;
