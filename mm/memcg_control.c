...
struct mem_cgroup *get_next_memcg(struct mem_cgroup *prev)
{
	...
	memcg = list_entry(pos->next, struct mem_cgroup, score_node);
...
static int process_reclaim_all_anon_memcg(void *param)
{
	struct mem_cgroup *memcg = (struct mem_cgroup *)(param);
					    ...
					    loff_t off)
{
	...
	tsk = kthread_create(process_reclaim_all_anon_memcg, (void *)memcg,
			     "reclaim_all_anon_memcg");
	if (IS_ERR(tsk)) {
...
static int memcg_total_info_per_app_show(struct seq_file *m, void *v)
{
	...
	seq_printf(
		m,
		"name memcgid anon_size zram_compress_size eswap_compress_size\n");
	while ((memcg = get_next_memcg(memcg))) {
		...
		if (lru_gen_enabled()) {
			anon_size = lruvec_page_state_local(
				lruvec, NR_LRU_BASE + LRU_ACTIVE_ANON);
			anon_size += lruvec_page_state_local(
				lruvec, NR_LRU_BASE + LRU_INACTIVE_ANON);
		} else
		...
		zram_compress_size =
			hyperhold_read_mcg_stats(memcg, MCG_ZRAM_STORED_SZ);
		eswap_compress_size =
			hyperhold_read_mcg_stats(memcg, MCG_DISK_STORED_SZ);
		...
		if (memcg == root_mem_cgroup)
			seq_printf(m, "root %d %lu %lu %lu\n", memcg->id.id,
				   anon_size, zram_compress_size,
				   eswap_compress_size);
		else
			seq_printf(m, "%s %d %lu %lu %lu\n", memcg->name,
				   memcg->id.id, anon_size, zram_compress_size,
				   eswap_compress_size);
	}
					 ...
					 struct cftype *cft, u64 val)
{
	...
	size = atomic64_read(&memcg->memcg_reclaimed.ub_ufs2zram_ratio) * size /
	       ratio;
#ifdef CONFIG_HYPERHOLD_CORE
	hyperhold_batch_out(memcg, size, val ? true : false);
#endif
...
void memcg_app_score_update(struct mem_cgroup *target)
{
	...
	list_for_each(pos, &score_head) {
		struct mem_cgroup *memcg =
			list_entry(pos, struct mem_cgroup, score_node);
...
static ssize_t eswap_avail_buffers_write(struct kernfs_open_file *of, char *buf,
					 size_t nbytes, loff_t off)
{
					  ...
					  char *buf, size_t nbytes, loff_t off)
{
	...
	if (sscanf(buf, "%u %u", &inactive_file_ratio_value,
...
static int
anon_refault_snapshot_min_interval_write(struct cgroup_subsys_state *css,
					 struct cftype *cft, u64 val)
{
...
static int memcg_active_app_info_list_show(struct seq_file *m, void *v)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		...
		zram_size = hyperhold_read_mcg_stats(memcg, MCG_ZRAM_PG_SZ);
		...
		if (memcg == root_mem_cgroup)
			seq_printf(m, "root %llu %lu %lu %lu %llu %lld\n",
				   score, anon_size, zram_size, eswap_size,
				   memcg->memcg_reclaimed.reclaimed_pagefault,
				   atomic64_read(
					   &memcg->hyperhold_cold_writeback));
		else
			seq_printf(m, "%s %llu %lu %lu %lu %llu %lld\n",
				   memcg->name, score, anon_size, zram_size,
				   eswap_size,
				   memcg->memcg_reclaimed.reclaimed_pagefault,
				   atomic64_read(
					   &memcg->hyperhold_cold_writeback));
...
static u64
anon_refault_snapshot_min_interval_read(struct cgroup_subsys_state *css,
					struct cftype *cft)
{
	return atomic64_read(&anon_refault_snapshot_min_interval);
}
...
void memcg_eswap_info_show(struct seq_file *m)
{
	...
	do {
		...
		if (lru_gen_enabled()) {
			file = lruvec_page_state_local(
				lruvec, NR_LRU_BASE + LRU_ACTIVE_FILE);
			file += lruvec_page_state_local(
	...
	if (IS_LEAK_PROC(fault_type)) {
		...
		if (IS_LEAK_PROC(cur_fault_type)) {
			if (atomic_read(&mem_leak_proc_count) == 0) {
				pr_warn("%s: mem_leak_proc_count is 0 when atomic_dec",
					__func__);
				return nbytes;
			}
...
static ssize_t mem_cgroup_leak_count_write(struct kernfs_open_file *of,
					   char *buf, size_t nbytes, loff_t off)
{
	const unsigned int base = 10;
