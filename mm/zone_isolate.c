...
static unsigned long zi_index_alloc_record[ZONE_ISOLATE_COUNT][MAX_ORDER]
					  [MIGRATE_TYPES] = {
						  0,
};
	...
	for (zone_type = 0; zone_type < MAX_NR_ZONES; zone_type++) {
		...
		for (zi_index = 0; zi_index < ZONE_ISOLATE_COUNT; zi_index++)
			seq_printf(m, "[%d]: end_pfn = %lu, mid_pfn = %lu\n",
				   zi_index, zone->zi_info[zi_index].end_pfn,
				   zone->zi_info[zi_index].mid_pfn);
		seq_printf(
			m,
			"\n---------------------------------------------------------------------------------------------------------------\n");
		...
		for (zi_index = 0; zi_index < ZONE_ISOLATE_COUNT; zi_index++) {
			...
			for (order = 0; order < MAX_ORDER; order++) {
				...
				for (t = 0; t < MIGRATE_TYPES; t++)
					seq_printf(
						m, "%12lu ",
						zi_index_alloc_record[zi_index]
								     [order][t]);
				seq_printf(m, "\n");
			}
			seq_printf(
...
static ssize_t zone_isolate_info_write(struct file *file,
...
void add_page_to_list(struct page *page, struct zone *zone, unsigned int order,
		      int mt)
{
	...
	if (unlikely(pfn > zone->zi_info[ZONE_ISOLATE_COUNT - 1].end_pfn)) {
		list = &(zone->free_area[ZONE_ISOLATE_COUNT - 1][order]
				 .free_list[mt]);
...
void setup_zone_isolate(struct zone *zone)
{
	...
	unsigned long end_pfn_pages
		[ZONE_ISOLATE_COUNT]; // = zone_managed_pages(zone) / ZONE_ISOLATE_COUNT;
	...
	for (i = 0; i < ZONE_ISOLATE_COUNT; i++) {
		end_pfn_pages[i] =
			zone_managed_pages(zone) * count_percent[i] / 100;
	...
	for (pfn = start_pfn; pfn < end_pfn; pfn += pageblock_nr_pages) {
		...
		if (!mid_pfn_inited) {
			...
			if (count_mid_pfn * pageblock_nr_pages >
			    mid_pfn_pages[i]) {
				zone->zi_info[i].mid_pfn =
...
unsigned int get_zi_index(unsigned int current_zi_index, unsigned int order,
			  int migratetype)
{
	int index;
