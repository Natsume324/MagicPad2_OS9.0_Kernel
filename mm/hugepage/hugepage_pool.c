...
#define hugepage_debug(fmt, ...) \
	pr_debug("[" HUGEPAGE_TAG "][D] %s: " fmt "\n", __func__, ##__VA_ARGS__)
#define hugepage_info(fmt, ...) \
	pr_info("[" HUGEPAGE_TAG "][I] %s: " fmt "\n", __func__, ##__VA_ARGS__)
#define hugepage_err(fmt, ...) \
	pr_err("[" HUGEPAGE_TAG "][E] %s: " fmt "\n", __func__, ##__VA_ARGS__)
#else
...
static void try_wake_up_refill_worker(struct hugepage_pool *pool)
{
	...
	hugepage_debug("refill_worker_rs flags = %lu", refill_worker_rs.flags);

	if (!atomic_read(&pool->refill_wait_flag) &&
	    ((pool->count < pool->noshrink_hpages) ||
...
static void nozerolist_to_list(struct hugepage_pool *pool)
{
	...
	while (!list_empty(&pool->nonzero_list)) {
		struct page *page =
			list_first_entry(&pool->nonzero_list, struct page, lru);
	...
	if (pool->nonzero_count > 0)
		hugepage_info(
			"move_list move list unfinished still have count %d",
			pool->nonzero_count);
...
static unsigned int hugepage_refill_count(struct hugepage_pool *pool)
{
	...
	s64 delta;
#endif
	unsigned int avail_hps =
		(unsigned int)(zones_available_simple() >> pool->order);
	...
	hugepage_info("hugepage_refill_count refill_low_mark = %u",
		      refill_low_mark);
...
static inline void hugepage_pool_add(struct hugepage_pool *pool,
				     struct page *page)
{
	spin_lock(&pool->list_lock);
	list_add(&page->lru, &pool->page_list);
...
#ifdef CONFIG_HUGEPAGE_POOL_DEBUG
	delta = ktime_us_delta(ktime_get(), t_ktime);
	hugepage_info("pool_refill delta = %lld, refill cnt = %d", delta,
		      (cnt - refill_count));
	if (refill_count >= 0) {
		refill_skip_cnt++;
				     ...
				     enum hpage_type type)
{
	...
	if (unlikely(order != pool->order))
		return alloc_pages(GFP_KERNEL | __GFP_COMP | __GFP_NORETRY,
				   order);
...
struct folio *alloc_hgfolio(int order, enum hpage_type type)
{
	struct page *page = alloc_hugepage(order, type);
	if (page && order > 1)
		prep_transhuge_page(page);

	return (struct folio *)page;
}
EXPORT_SYMBOL_GPL(alloc_hgfolio);

static unsigned long hugepage_pool_shrink_scan(struct shrinker *shrink,
					       struct shrink_control *sc)
{
#ifdef CONFIG_SHRINKER_LOCKLESS_OPT
	struct hugepage_pool *pool = shrink->private_data;
#else
	struct hugepage_pool *pool =
		container_of(shrink, struct hugepage_pool, pool_shrink);
#endif
	unsigned long freed = 0;
	unsigned long nr_to_scan = sc->nr_to_scan;
	unsigned int shrink_free = hugepage_pool_size_nonreserved(pool) >>
				   SHRINK_STEP_MAX;
	struct page *page;
	...
	hugepage_info(
		"hugepage_pool_shrink_scan--nr_to_scan = %lu, nr_scanned = %lu",
	...
	spin_lock(&pool->nonzero_list_lock);
	while (!list_empty(&pool->nonzero_list) && freed < nr_to_scan) {
		page = list_first_entry(&pool->nonzero_list, struct page, lru);
		list_del_init(&page->lru);
		if (page_count(page)) {
			hugepage_debug(
				"hugepage_pool_shrink_scan nonzero_list page_count = %d",
				page_count(page));
	...
	spin_lock(&pool->list_lock);
	while (!list_empty(&pool->page_list) && freed < nr_to_scan) {
		page = list_first_entry(&pool->page_list, struct page, lru);
		list_del_init(&page->lru);
		if (page_count(page)) {
			hugepage_debug(
				"hugepage_pool_shrink_scan page_list page_count = %d",
				page_count(page));
			put_page_testzero(page);
		}
		___free_pages_ok(page, HUGEPAGE_ORDER, FPI_NONE, true);
	...
	hugepage_info("hugepage_pool_shrink_scan--freed = %lu", freed);
...
static void hugepage_pool_debugfs_init(struct hugepage_pool *pool)
{
	...
	pool->hugepage_pool_debugfs_dir =
		debugfs_create_dir("hugepage_pool", NULL);
	if (IS_ERR_OR_NULL(pool->hugepage_pool_debugfs_dir))
		return;

	dentry = debugfs_create_file("pool_info", 0444,
				     pool->hugepage_pool_debugfs_dir, pool,
				     &pool_info_fops);

	if (IS_ERR(dentry))
		WARN_ONCE(
			1,
			"Unable to create 'pool_info' file for hugepage_pool\n");
}
...
static ssize_t pool_info_show_(struct kobject *kobj,
			       struct kobj_attribute *attr, char *buf)
{
	struct hugepage_pool *pool =
		container_of(kobj, struct hugepage_pool, kobj);
	ssize_t ret;
...
static ssize_t pool_shrink_show(struct kobject *kobj,
				struct kobj_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	...
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
			 "dma_alloced : %u, gpu_alloced : %u, vmalloced : %u\n",
			 alloced_cnts[HPAGE_DMA_BUF], alloced_cnts[HPAGE_GPU],
			 alloced_cnts[HPAGE_VMALLOC]);
...
static void __init hugepage_pool_create(struct hugepage_pool *pool)
{
	...
	pool->max_hpages = pool->reserved_hpages;
...
void set_cpus_bind(const char *bind_cpu, struct task_struct *tsk)
{
	int retval;
	...
	retval = cpulist_parse(bind_cpu, &temp_mask);
...
static int __init hugepage_pool_init(void)
{
...
#ifdef CONFIG_SHRINKER_LOCKLESS_OPT
	pool->pool_shrink = shrinker_alloc(0, "hugepage_pool_shrinker");
	if (!pool->pool_shrink) {
		hugepage_err("Allocate metadata cache shrinker failed");
		kthread_stop(refill_worker);
		return -ENOMEM;
	}
	...
	pool->pool_shrink->private_data = pool;
	shrinker_register(pool->pool_shrink);
#else
	pool->pool_shrink.count_objects = hugepage_pool_shrink_count;
	pool->pool_shrink.scan_objects = hugepage_pool_shrink_scan;
	pool->pool_shrink.seeks = DEFAULT_SEEKS;
	if (register_shrinker(&pool->pool_shrink, "hugepage-pool")) {
	...
	set_cpus_bind(REFILL_WORKER_BIND_CPUS, refill_worker);
...
#ifdef CONFIG_SHRINKER_LOCKLESS_OPT
	shrinker_free(pool->pool_shrink);
#else
