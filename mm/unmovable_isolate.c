...
static int valid_ddr_size_for_ui(long long size)
{
	...
	if (size_offset >= MEMSIZE_INDEX_MIN &&
	    size_offset <= MEMSIZE_INDEX_MAX)
		return 1;
	else
		return 0;
}
 ...
 */
int __init parse_unmovable_isolate_size(long long *size_cmdline,
					char *ui_cmdline)
{
	...
	if (first_colon) {
		...
		do {
			...
			if (valid_ddr_size_for_ui(parsed_ddr_size)) {
				size_cmdline[parsed_ddr_size -
					     MEMSIZE_INDEX_OFFSET] = size;
			}
		} while (*cur++ == ',');
	} else {
...
int unmovable_isolate_pageblock(struct zone *zone, struct page *page)
{
	...
	if (unmovable_isolate_enabled(zone) &&
	    (is_unmovable_isolate1(migratetype) ||
...
unsigned long get_total_managed_pages(void)
{
	...
	for_each_zone(zone) {
		if (!is_highmem(zone))
			...
			total_managed_pages += (unsigned long)atomic_long_read(
				&zone->managed_pages);
#else
 ...
 * get the related unmovable-isolate size
 */
static long long get_unmovable_isolate_blocks(struct zone *zone,
					      int unmovable_isolate_type)
{
	int ddr_size;
	unsigned long total_managed_pages = 0;
	...
	if (is_unmovable_isolate1(unmovable_isolate_type)) {
		if (ddr_size != 0 &&
		    size_cmdline_ui1[ddr_size - MEMSIZE_INDEX_OFFSET] > 0)
			ui_blocks =
				(unsigned long long)
					size_cmdline_ui1[ddr_size -
							 MEMSIZE_INDEX_OFFSET] /
				PAGE_SIZE / pageblock_nr_pages;
		else
			ui_blocks = UNMOVABLE_ISOLATE1_SIZE_BLOCKS;
	} else if (is_unmovable_isolate2(unmovable_isolate_type)) {
		if (ddr_size != 0 &&
		    size_cmdline_ui2[ddr_size - MEMSIZE_INDEX_OFFSET] > 0)
			ui_blocks =
				(unsigned long long)
					size_cmdline_ui2[ddr_size -
							 MEMSIZE_INDEX_OFFSET] /
				PAGE_SIZE / pageblock_nr_pages;
		else
	...
	if (total_managed_pages != 0)
		...
		ui_counts =
			ui_blocks *
			(unsigned long)atomic_long_read(&zone->managed_pages) /
			total_managed_pages;
#else
		ui_counts =
			ui_blocks * zone->managed_pages / total_managed_pages;
#endif
 ...
 */
static int pageblock_is_reserved_for_ui(unsigned long start_pfn,
					  ...
					  int disable)
{
	...
	if (!disable)
		unmovable_isolate_count = get_unmovable_isolate_blocks(
			zone, unmovable_isolate_type);
	else
		...
		if (unmovable_isolate_count > 0) {
			...
			if (block_migratetype == MIGRATE_MOVABLE) {
				set_pageblock_migratetype(
					page, unmovable_isolate_type);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0))
				pages_moved = move_freepages_block(
					zone, page, unmovable_isolate_type);
#else
				pages_moved = move_freepages_block(
					zone, page, unmovable_isolate_type,
					NULL);
#endif
				__mod_zone_page_state(zone, ui_stat_item,
						      pages_moved);
				unmovable_isolate_count--;
				continue;
			}
