				     ...
				     struct mem_cgroup *memcg)
{
	...
	if (lru_gen_enabled()) {
		sc.nr_to_reclaim = lruvec_page_state_local(
	...
	nr_taken = isolate_lru_folios(nr_to_scan, lruvec, &folio_list,
				      &nr_scanned, &sc, LRU_INACTIVE_ANON);
...
void zswapd_status_show(struct seq_file *m)
{
	...
	seq_printf(m, "recent refault: %llu\n", global_anon_refault_ratio);
}
...
static bool get_memcg_anon_refault_status(struct mem_cgroup *memcg)
{
	...
	if (!lruvec)
		return false;
	if (lru_gen_enabled()) {
		anon_total = lruvec_page_state_local(
	...
	anon_total += hyperhold_read_mcg_stats(memcg, MCG_DISK_STORED_PG_SZ) +
...
static bool get_area_anon_refault_status(void)
{
	...
	ratio = (cur_anon_pagefault - area_last_anon_pagefault) *
		percent_constant /
		(jiffies_to_msecs(cur_time - last_anon_snapshot_time) + 1);
...
static bool anon_size_is_low(void)
{
	...
	unsigned long anon_min_pages = get_zswapd_min_anon_size() * SZ_1M /
				       PAGE_SIZE; /* MB to pages */
...
static int snapshotd(void *p)
{
	...
	while (!kthread_should_stop()) {
		...
		ret = wait_event_interruptible(
			snapshotd_wait, atomic_read(&snapshotd_wait_flag));
		/*lint +e578 */
		if (ret)
			continue;
...
void wakeup_zswapd(pg_data_t *pgdat)
{
	...
	curr_interval = jiffies_to_msecs(jiffies - last_zswapd_time);
	...
	while (nr[LRU_INACTIVE_ANON] || nr[LRU_ACTIVE_ANON]) {
		for (lru = 0; lru <= LRU_ACTIVE_ANON; lru++) {
			if (nr[lru]) {
				...
				nr[lru] -= nr_to_scan;
				nr_reclaimed += zswapd_shrink_list(
...
static bool zswapd_shrink_anon(pg_data_t *pgdat, struct scan_control *sc)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		...
		nr_active = lruvec_lru_size(lruvec, LRU_ACTIVE_ANON,
					    MAX_NR_ZONES - 1);
		nr_inactive = lruvec_lru_size(lruvec, LRU_INACTIVE_ANON,
					      MAX_NR_ZONES - 1);
		if (lru_gen_enabled()) {
			nr_active = lruvec_page_state_local(
				lruvec, NR_LRU_BASE + LRU_ACTIVE_ANON);
			nr_inactive = lruvec_page_state_local(
				lruvec, NR_LRU_BASE + LRU_INACTIVE_ANON);
		...
		nr[LRU_INACTIVE_ANON] = nr_inactive >>
...
static void zswapd_shrink_node(pg_data_t *pgdat)
{
	...
	if (sc.nr_reclaimed < get_empty_round_check_threshold_value()) {
		...
		if (last_round_is_empty)
			zswapd_skip_interval = min(
				zswapd_skip_interval * increase_rate,
