...
static inline size_t eima_get_user_pages_remote(struct calc_params *params,
						...
						int *locked)
{
	return (size_t)get_user_pages_remote(params->vm_a->vm_mm, start, 1,
					     FOLL_FORCE, pages, NULL, locked);
}
...
static inline size_t eima_get_user_pages_remote(struct calc_params *params,
						...
						int *locked)
{
	return (size_t)get_user_pages_remote(params->task, params->vm_a->vm_mm,
					     start, 1, FOLL_FORCE, pages, NULL,
					     locked);
}
...
static int __do_hash(struct calc_params *params, struct hashgen_handle *hgh)
{
	...
	for (i = 0; i < params->num_pages; ++i) {
		...
		loc_rv = eima_get_user_pages_remote(params, addr, &page,
						    &locked);
		if (loc_rv != 1) {
			...
			struct task_struct *target_proc, int digest_size)
{
	...
	mas.tree = &tmsg->mm->mm_mt;
	mmap_read_unlock(tmsg->mm);
	...
	mas_for_each(&mas, vma, ULONG_MAX) {
		if (__is_xp_segment(vma)) {
			...
			if (seg_len % PAGE_SIZE != 0) {
				++(params.num_pages);
				eima_debug(
					"Adjusted num_pages from %lu to %lu\n",
					params.num_pages - 1, params.num_pages);
			}
			       ...
			       size_t pn_len)
{
	...
	if (tmsg->mm->exe_file) {
		process_p =
			d_path(&tmsg->mm->exe_file->f_path, process_pn, pn_len);
		if (IS_ERR(process_p)) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0)
		    ...
		    char *hash, int hash_len, int *hlen)
{
	...
	if (target_msg.mm == NULL) {
		eima_info(
			"Could not get mm_struct for process with PID:%d and TGID:%d\n",
