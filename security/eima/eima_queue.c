...
static int eima_add_digest_entry(struct eima_template_entry *entry)
{
	...
	ret = memcpy_s(&qe->entry, sizeof(struct eima_template_entry), entry,
		       sizeof(struct eima_template_entry));
	if (ret != EOK) {
		kfree(qe);
...
int eima_add_template_entry(struct eima_template_entry *entry)
{
	...
	s_ret = memcpy_s(digest, EIMA_HASH_DIGEST_SIZE, entry->digest,
	...
	eima_debug("eima add template entry, name %s, type %d", entry->fn,
		   entry->type);

	tpmresult = eima_pcr_extend(digest, EIMA_HASH_DIGEST_SIZE, entry->type);
