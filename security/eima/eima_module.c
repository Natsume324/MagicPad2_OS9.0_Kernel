...
#define ROOTAGENT_LOADING_TIMEOUT_MS \
	40000 // rootagent is loaded in kernel-40s usually
...
static const kgid_t g_system_gid =
	KGIDT_INIT((gid_t)1000); /* 1000 is the system gid */
...
#endif
	g_proc_entry = proc_create("root_scan", g_file_creat_ro_mode, NULL,
				   &eima_proc_fops);
...
int stp_eima_trigger(void)
{
	...
	ret = memcpy_s(item.name, STP_ITEM_NAME_LEN, item_info[EIMA].name,
		       strlen(item_info[EIMA].name) + 1);
	if (ret != EOK) {
	...
	if ((file_path == NULL) || (file_path[0] == '\0')) {
		...
		eima_debug("received tampered file path from ta: %s, uploading",
			   file_path);
				   ...
				   struct task_struct **proc)
{
	...
	if (filename_len > EIMA_NAME_STRING_LEN) {
		eima_error("filename length is %d, exceeds max length",
			   filename_len);
		return 0;
	}
	...
	for_each_process(p) {
		ret = eima_comp_proc_filename(file_name, target_pn, PATH_MAX,
					      p);
		if (ret == 0) {
			*proc = p;
			kfree(target_pn);
				    ...
				    uint32_t hash_len, struct m_entry *entry)
{
	...
	s_ret = memcpy_s(entry->hash, EIMA_HASH_DIGEST_SIZE, temp_entry->digest,
			 hash_len);
	if (s_ret != EOK) {
	...
	s_ret = memcpy_s(entry->fn, EIMA_NAME_STRING_LEN + 1, temp_entry->fn,
			 strlen(temp_entry->fn) + 1);
	if (s_ret != EOK) {
		eima_error("memcpy_s fail");
		return s_ret;
	}
				   ...
				   enum eima_measure_type type)
{
	...
	ret = memcpy_s(entry->fn, (EIMA_NAME_STRING_LEN + 1), tmp_target->file,
		       strlen(tmp_target->file) + 1);
	if (ret != EOK) {
		eima_error("memcpy_s failed, s_ret=%d\n", ret);
		return ret;
	}
...
static int get_hash_value(char *hash, int *hlen,
			  integrity_targets_t *tmp_target)
{
	int ret;
	...
	if (ret != 0) {
		eima_error("eima get target hash value failed: %s",
			   tmp_target->file);
		return ret;
	}
				      ...
				      bool is_baseline)
{
	...
	for (i = 0; i < g_eima_pif.policy_count; i++) {
		for (j = 0; j < g_eima_pif.policy[i].target_count; j++) {
			...
			ret = eima_set_entry_contents(&entry, hash,
						      sizeof(hash), targets,
						      EIMA_STATIC);
			if (ret != 0)
				return ret;
			...
			if (ret != EOK && ret != -EEXIST)
				eima_error(
					"static eima add template entry failed. fc = %d",
					ret);

			ret = eima_encap_measure_entry(
				&entry, EIMA_HASH_DIGEST_SIZE,
				&(mlist_msg->m_list[mlist_msg->num]));
			if (ret != EOK)
				return ret;
...
static int eima_static_do_work(struct m_list_msg *mlist_msg)
{
	...
	if (mlist_msg->num > 0) {
		ret = memcpy_s(mlist_msg->usecase, EIMA_NAME_STRING_LEN + 1,
			       ...
			       strlen(_EIMA_POLICY_USECASE_NAME) + 1);
		if (ret != EOK) {
			eima_error(
				"mlist_msg memcpy_s failed, line=%d ret = %d\n",
				__LINE__, ret);
			return ret;
		}
		...
		if (ret != 0) {
			eima_error(
				"eima static measurement send failed, ret = %d\n",
				ret);
			return ret;
		}
	}
...
static int eima_dynamic_do_work(struct m_list_msg *mlist_msg)
{
	...
	if (mlist_msg->num > 0) {
		...
		if (ret != EOK) {
			eima_error(
				"mlisg_msg memcpy_s failed. line = %d, ret = %d\n",
				__LINE__, ret);
			return ret;
		}
		...
		if (ret != 0) {
			eima_error(
				"eima dynamic measurement data send failed, ret = %d\n",
				ret);
			return ret;
		}
	}
...
static int eima_match_target(const struct file *file,
			     integrity_targets_t **target)
{
	int i;
	int j;
	...
	if ((path == NULL) || IS_ERR(path) ||
		...
		eima_error(
			"the length of path is overflow ,or path is null or error");
		kfree(pathname);
		return 0;
	}
	...
	for (i = 0; i < g_eima_pif.policy_count; i++) {
		for (j = 0; j < g_eima_pif.policy[i].target_count; j++) {
			if ((g_eima_pif.policy[i].targets[j].type ==
			     EIMA_STATIC) &&
...
static void eima_async_send_do_work(struct work_struct *w)
{
	...
	if (ret != 0)
		eima_error("eima measure data async send failed, ret = %d",
			   ret);
	else
...
static int eima_measure_file(struct file *file, integrity_targets_t *target)
{
	...
	if ((ret != EOK) && (ret != -EEXIST))
		eima_error(
			"static runtime eima add template entry failed ret = %d",
			ret);
...
static int generate_whitelist(struct m_list_msg *mlist_msg)
{
	...
	for (i = 0; i < g_eima_pif.policy_count; i++) {
		for (j = 0; j < g_eima_pif.policy[i].target_count; j++) {
			...
			if (mlist_msg->num >= EIMA_MAX_MEASURE_OBJ_CNT) {
				eima_error(
					"measure data cnt exceed max num in whitelist");
				return 1;
			}
...
static __init int eima_module_init(void)
{
	...
	if (queue_work(system_wq, &eima_init) == 0)
		eima_error(
			"eima module init module work has joined the queued\n");
	return 0;
}
