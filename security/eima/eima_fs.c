...
struct file *eima_file_open(const char *path, int flag, umode_t mode)
{
	...
	if (IS_ERR(filp)) {
		err = PTR_ERR(filp);
		eima_error("open file %s failed with error code %ld", path,
			   err);
		return NULL;
	}
...
static void eima_add_default_policy(void)
{
	...
	for (i = 0; (i < ARRAY_SIZE(g_measure_targets_list)) &&
		    (i < EIMA_MAX_MEASURE_OBJ_CNT);
	     i++) {
		s_ret = memcpy_s(g_eima_pif.policy[0].targets[i].file,
				 EIMA_NAME_STRING_LEN + 1,
				 g_measure_targets_list[i].file,
				 strlen(g_measure_targets_list[i].file) + 1);
		if (s_ret != EOK) {
			eima_error("memcpy_s fail");
			return;
		}
		g_eima_pif.policy[0].targets[i].target_pid =
			g_measure_targets_list[i].target_pid;
		g_eima_pif.policy[0].targets[i].type =
			g_measure_targets_list[i].type;
...
static int eima_ascii_measurements_show(struct seq_file *file, void *iter)
{
	...
	list_for_each_entry_rcu(qe, &g_eima_measurements_list, later) {
		...
		pcr_idx = (e->type == EIMA_STATIC) ?
				  EIMA_STATIC_MEASURE_PCR_IDX :
				  EIMA_DYNAMIC_MEASURE_PCR_IDX;
...
{
	return single_open(file, eima_ascii_measurements_show,
			   inode->i_private);
}
