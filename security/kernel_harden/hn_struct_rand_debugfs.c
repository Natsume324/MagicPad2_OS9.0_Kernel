...
struct randdomize_struct_test_hn struct_randomize_sample = {.a1 = 1,
							    .b2 = 2,
							    .c3 = 3,
							    .d4 = 4,
							    .e5 = 5,
							    ...
							    .h8 = 8,
							    .i9 = 9};
struct randdomize_ops_test_hn ops_randomize_sample = {.check = NULL,
						      .get_result = NULL,
						      .get_setting = NULL,
						      .set_setting = NULL};
#define STRUCT_RAND_DEBUG_ENTRY(name)                                     \
	static int struct_rand_##name##_open(struct inode *inode,         \
					     struct file *file)           \
	{                                                                 \
		return single_open(file, struct_rand_##name##_show,       \
				   inode->i_private);                     \
	}                                                                 \
                                                                          \
...
static int struct_rand_offset_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		   ...
		   &ops_randomize_sample.check,
		   &ops_randomize_sample.get_result,
		   &ops_randomize_sample.get_setting,
		   &ops_randomize_sample.set_setting);
	return 0;
}
...
static int __init struct_rand_debugfs_init(void)
{
	...
	debugfs_file = debugfs_create_file("offset", 0444, struct_rand_root,
					   NULL, &struct_rand_offset_fops);
	if (!debugfs_file) {
