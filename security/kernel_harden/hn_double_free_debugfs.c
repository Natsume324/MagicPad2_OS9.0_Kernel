...
#define SLUB_DEBUG_ENTRY(name)                                     \
	static int slub_##name##_open(struct inode *debug_inode,   \
				      struct file *file)           \
	{                                                          \
		return single_open(file, slub_##name##_show,       \
				   debug_inode->i_private);        \
	}                                                          \
                                                                   \
...
static int slub_double_free_show(struct seq_file *m, void *v)
{
	...
	struct kmem_cache *my_cachep = kmem_cache_create(
		"my_cache", KMEM_CACHE_SIZE, 0, SLAB_HWCACHE_ALIGN, NULL);
	if (!my_cachep)
		return -ENOMEM;
...
static int __init hnslub_double_free_debugfs_init(void)
{
	...
	if (!slub_double_free_root)
		return -ENODEV;
#ifdef CONFIG_HN_SLUB_SANITIZE
	debugfs_file = debugfs_create_file("double_free", 0444,
