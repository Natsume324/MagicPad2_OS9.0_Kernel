...
#define SLUB_DEBUG_ENTRY(name)                                     \
	static int slub_##name##_open(struct inode *free_inode,    \
				      struct file *file)           \
	{                                                          \
		return single_open(file, slub_##name##_show,       \
				   free_inode->i_private);         \
	}                                                          \
                                                                   \
					 ...
					 unsigned int num)
{
	...
	struct kmem_cache *my_cachep =
		kmem_cache_create("my_cache", num, 0, SLAB_HWCACHE_ALIGN, NULL);
	if (!my_cachep)
		return -ENOMEM;
...
static int __init hn_freelist_random_debugfs_init(void)
{
	...
	if (!slub_freelist_root)
		return -ENODEV;
#ifdef CONFIG_SLAB_FREELIST_RANDOM
	debug_file = debugfs_create_file("freelist_random", 0444,
