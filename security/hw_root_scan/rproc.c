			     ...
			     char *rprocs, ssize_t rprocs_len)
{
	...
	for (i = 0; i < ntoken; i++) {
		if (!find_proc_in_init_list(exec_path[i])) {
			...
			RSLogError(TAG, "the root proc is abnormal: %s",
				   exec_path[i]);
		}
	}
...
void rprocs_strip_whitelist(char *rprocs, ssize_t rprocs_len)
{
	...
	final_tokens =
		kmalloc((ulong)MAX_PROC_NUM * sizeof(char *), GFP_KERNEL);
 ...
 */
static int get_task_exe(struct mm_struct *mm, pid_t pid, char *out, int outlen,
...
int get_root_procs(char *out, size_t outlen)
{
	...
	for_each_process(task) { /* lint -save -e550 */
		...
		if ((task->flags & PF_KTHREAD) || (task->mm == NULL) ||
		    (from_kuid_munged(&init_user_ns, task_euid(task)) != 0)) {
			continue;
		}
