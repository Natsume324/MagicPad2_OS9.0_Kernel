...
static uint rscan_trigger_by_stp(char *upload_rootproc)
{
	...
	root_masks =
		rscan_dynamic(dynamic_ops, scan_result_buf, &scan_err_code, 0);
...
static int need_to_upload(unsigned int masks, unsigned int mask, int ree_status,
			  int tee_status, int flag)
{
	if (flag == 1)
		return 1;
			  ...
			  const char *rootproc, unsigned int mask, int flag)
{
	...
	for (i = 0; i < MAX_NUM_OF_ITEM; i++) {
		item_status =
			check_status(ree_status, itembits[i].item_ree_bit);
		item_tee_status =
			check_status(tee_status, itembits[i].item_tee_bit);
		...
		if (stp_item_info == NULL) {
			RSLogError(TAG,
				   "idx is %d, get item info by index failed",
				   i);
			return;
		}

		item_credible =
			get_credible_of_item(item_status, item_tee_status);
				     ...
				     struct rscan_result_dynamic *result)
{
	...
	if (op_mask & D_RSOPID_SE_HOOKS) {
		ret = sescan_hookhash(result->sehooks, sizeof(result->sehooks));
		if (ret != 0) {
		   ...
		   uint *error_code, int for_tee)
{
	...
	*error_code = rscan_dynamic_raw_unlock(op_mask, result);
	...
	if ((op_mask & D_RSOPID_SE_STATUS) &&
	    ...
	    (result->seenforcing != g_rscan_clean_scan_result.seenforcing)) {
...
int rscan_init_data(void)
{
	...
	if ((ret != 0) ||
		...
		if (sizeof(g_rscan_clean_scan_result.rprocs) <
		    strlen(DEFAULT_PROC)) {
	...
	error_code = rscan_dynamic_raw(D_RSOPID_KCODE | D_RSOPID_SYS_CALL |
					       D_RSOPID_SE_HOOKS,
...
{
	uint root_status =
		rscan_dynamic_raw_unlock(mask, &g_rscan_clean_scan_result);
...
static int __root_scan_pause(unsigned int op_mask, const void *reserved)
{
	...
	g_rscan_skip_flag.skip_kcode =
		SKIP & ((op_mask & D_RSOPID_KCODE) >> KCODE_OFFSET);
	g_rscan_skip_flag.skip_syscall =
		SKIP & ((op_mask & D_RSOPID_SYS_CALL) >> SYSCALL_OFFSET);
	g_rscan_skip_flag.skip_se_hooks =
		SKIP & ((op_mask & D_RSOPID_SE_HOOKS) >> SEHOOKS_OFFSET);
	g_rscan_skip_flag.skip_se_status =
		SKIP & ((op_mask & D_RSOPID_SE_STATUS) >> SESTATUS_OFFSET);
...
int root_scan_pause(unsigned int op_mask, const void *reserved)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)
	do_gettimeofday(&tv);
	RSLogTrace(TAG, "pause item:%u, time:%ld:%ld", op_mask, tv.tv_sec,
		   tv.tv_usec);
#else
	ktime_get_real_ts64(&tv);
	RSLogTrace(TAG, "pause item:%u, time:%lld:%ld", op_mask, tv.tv_sec,
		   tv.tv_nsec);
#endif
	...
	dynamic_ops = D_RSOPID_KCODE | D_RSOPID_SYS_CALL | D_RSOPID_SE_HOOKS |
		      D_RSOPID_SE_STATUS;
	root_status =
		rscan_dynamic(dynamic_ops, scan_result_buf, &scan_err_code, 0);
	if (root_status == 0) {
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 0, 0)
	do_gettimeofday(&tv);
	RSLogTrace(TAG, "resume item:%u, time:%ld:%ld", op_mask, tv.tv_sec,
		   tv.tv_usec);
#else
	ktime_get_real_ts64(&tv);
	RSLogTrace(TAG, "resume item:%u, time:%lld:%ld", op_mask, tv.tv_sec,
		   tv.tv_nsec);
#endif
...
#ifdef CONFIG_HN_ROOT_SCAN_ENG_DEBUG
static ssize_t copy_private_data_to_user(struct file *file, char __user *buf,
					 size_t count, loff_t *offp)
{
	...
	if (*offp < priv->len) {
		tocopy = ((priv->len - *offp) < count) ? (priv->len - *offp) :
							 count;
...
static ssize_t kcode_read(struct file *file, char __user *buf, size_t count,
			  loff_t *offp)
{
				  ...
				  size_t count, loff_t *offp)
{
	int test = 0;
	int pause_result =
		root_scan_pause(D_RSOPID_KCODE | D_RSOPID_SYS_CALL |
				   ...
				   size_t count, loff_t *offp)
{
	int test = 0;
	int resume_result =
		root_scan_resume((D_RSOPID_KCODE | D_RSOPID_SYS_CALL |
				  D_RSOPID_SE_HOOKS | D_RSOPID_SE_STATUS),
				 &test);
	g_test_count++;
...
static ssize_t rev_kcode_read(struct file *file, char __user *buf, size_t count,
			      loff_t *offp)
{
	memcpy(g_rscan_clean_scan_result.kcode, g_rscan_orig_result.kcode,
...
static ssize_t syscall_read(struct file *file, char __user *buf, size_t count,
			    loff_t *offp)
{
...
static ssize_t sehooks_read(struct file *file, char __user *buf, size_t count,
			    loff_t *offp)
{
...
static ssize_t rev_syscall_read(struct file *file, char __user *buf,
				size_t count, loff_t *offp)
{
	memcpy(g_rscan_clean_scan_result.syscalls, g_rscan_orig_result.syscalls,
...
static ssize_t rev_sehooks_read(struct file *file, char __user *buf,
				size_t count, loff_t *offp)
{
	memcpy(g_rscan_clean_scan_result.sehooks, g_rscan_orig_result.sehooks,
