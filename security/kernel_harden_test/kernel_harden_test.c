...
static void __check_object_size(const void *ptr, unsigned long n, bool to_user)
{
}
#endif
...
static int usercopy_slab_cache_test(unsigned long arg, bool to_user)
{
	...
	test_cache = kmem_cache_create("test_cache", sizeof(unsigned int), 0,
				       SLAB_PANIC, NULL);
	if (!test_cache) {
...
static int usercopy_normal_x_test(void *user_buf, void *p, unsigned long len,
				  bool to_user, const char *prefix)
{
	int ret = 0;
...
static int __init kernel_harden_test_init(void)
{
	...
	g_kharden_tty_drv =
		tty_alloc_driver(KH_TTY_MINORS, TTY_DRIVER_REAL_RAW);
	...
	g_kharden_tty_drv->init_termios.c_cflag = B921600 | CS8 | CREAD |
						  HUPCL | CLOCAL;
