...
static int kernel_stp_data_adapter(char **uevent_envp, char *result)
{
	...
	for (index = 0; index < KERNEL_STP_UEVENT_LEN - 1; index++) {
		...
		KSTPLogDebug(TAG, "uevent_envp[%d] is %s", index,
			     uevent_envp[index]);
	}
			    ...
			    char *upload_info)
{
	...
	if (addition_info == NULL)
		...
		snprintf(upload_info, STP_INFO_MAXLEN,
			 "stpinfo=%u:%u:%u:%u:%s:%s", result.id, result.status,
			 result.credible, result.version, result.name,
			 addition_info);
	return 0;
}
...
int kernel_stp_upload(struct stp_item result, const char *addition_info)
{
	...
	do {
		...
		ret = kernel_stp_upload_parse(result, addition_info,
					      upload_info);
		if (ret != 0) {
...
int kernel_stp_uploader_init(void)
{
	...
	do {
		...
		if (ret != 0) {
			KSTPLogError(TAG,
				     "kobj_uevent add failed, result is %d",
				     ret);
			break;
		}
	} while (0);
