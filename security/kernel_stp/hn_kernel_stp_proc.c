...
static ssize_t set_hhee_switch(unsigned int param)
{
	...
	if (param & ENABLE_HHEE_TVM)
		arm_smccc_hvc(HHEE_HVC_ENABLE_TVM, 0, 0, 0, 0, 0, 0, 0, &res);
...
static ssize_t kernel_stp_proc_write(struct file *file,
				     ...
				     loff_t *pos)
{
	...
	KSTPLogTrace(TAG, "stp proc feature %u, param %u", data.s.feat,
		     data.s.para);
