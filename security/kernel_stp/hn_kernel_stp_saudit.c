...
void saudit_log_stack(struct saudit_buffer *sab)
{
	...
	sab->offset += snprint_stack_trace(sab->info + sab->offset,
					   SAUDIT_BUFSIZ - sab->offset - 1,
					   &trace, 0);
#else
	trace_nr_entries = stack_trace_save(entries, SAUDIT_STACK_DEPTH,
					    SAUDIT_SELF_DEPTH - 1);
			      ...
			      unsigned int flags)
{
	...
	sab->item.credible = (sab->flags & SAUDIT_REF) ? STP_REFERENCE :
							 STP_CREDIBLE;
...
void saudit_log(int idx, int status, unsigned int flags, const char *fmt, ...)
{
	...
	saudit_log_format(sab,
			  ...
			  (unsigned long long)tv.tv_sec, current->pid,
	...
	saudit_log_format(sab,
			  "time=%ull,pid=%d,pcomm=%.20s,gid=%d,gcomm=%.20s,",
			  (unsigned long long)ts.tv_sec, current->pid,
			  current->comm, current->tgid,
			  current->group_leader->comm);
#endif
	if (fmt) {
