...
int kernel_stp_scanner_register(stp_cb callbackfunc)
{
	...
	if (g_scanner_list_init == KSTP_SCAN_LIST_UNINIT) {
		...
		KSTPLogTrace(TAG,
			     "kernel stp scanner init success at register.");
	}

	listnode = kzalloc(sizeof(struct kernel_stp_scanner_node), GFP_KERNEL);
	if (listnode == NULL) {
	...
	list_for_each_entry(p, &g_stp_scanner_node.stp_scanner_list,
			    stp_scanner_list) {
		if (listnode->callbackfunc == p->callbackfunc) {
			repeat_count++;
			KSTPLogDebug(
				TAG,
				"callbackfunc has in scanner list,no need add again!");
			break;
		}
	}
...
static void kernel_stp_do_callback(struct work_struct *data)
{
	...
	struct kernel_stp_scan_work *work_node =
		container_of(data, struct kernel_stp_scan_work, work);
...
void kernel_stp_scanner(void)
{
	...
	list_for_each_entry(p, &g_stp_scanner_node.stp_scanner_list,
		...
		work_node = kzalloc(sizeof(struct kernel_stp_scan_work),
				    GFP_KERNEL);
		if (work_node == NULL) {
...
{
	g_kernel_stp_scan_wq =
		create_singlethread_workqueue("HN_KERNEL_STP_SCANNER");
