...
static ssize_t testcall_read(struct file *file, char __user *buf, size_t size,
			     loff_t *ppos)
{
...
static ssize_t testattack_read(struct file *file, char __user *buf, size_t size,
			       loff_t *ppos)
{
	/*
...
static int __init cfi_test_init(void)
{
	...
	cfi_call_entry =
		proc_create("cfitest-call", 0444, NULL, &testcall_fops);
	if (!cfi_call_entry) {
	...
	cfi_attack_entry =
		proc_create("cfitest-attack", 0444, NULL, &testattack_fops);
	if (!cfi_attack_entry) {
