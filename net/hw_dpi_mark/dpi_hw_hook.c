						    ...
						    const uid_t uid)
{
	...
	while (node != NULL) {
		struct uid_netid *data =
			rb_entry(node, struct uid_netid, uid_netid_node);
		if (uid < data->uid)
			node = node->rb_left;
				       ...
				       struct rb_root *root)
{
	...
	while (*new != NULL) {
		struct uid_netid *this =
			rb_entry(*new, struct uid_netid, uid_netid_node);
		parent = *new;
...
static void mplk_uid_search_need_delete_uid(unsigned int uid)
{
	...
	if (uid_netid_entry != NULL) {
		MPLINK_DEBUG("%s: uid need to delete = %d", __func__, uid);
...
{
	struct uid_netid *netid_entry =
		from_timer(netid_entry, t, delete_timer);
	mplk_uid_search_need_delete_uid(netid_entry->uid);
}
#endif
...
void mplk_add_nw_bind(uid_t uid, uint32_t netid)
{
	...
	if (uid_netid_entry->netid) {
		MPLINK_INFO("change %u from %u to %u then num=%d\n", uid,
			    uid_netid_entry->netid, netid, uid_netid_entry_num);
	} else {
		MPLINK_INFO("add a new bind for %u to %u then num=%u\n", uid,
			    netid, uid_netid_entry_num);
	}
	if (netid > 0)
...
void mplk_close_socket_by_uid(uint32_t strategy, uid_t uid)
{
	...
	MPLINK_INFO("close old udp sk for %u when %lu with S:%u\n", uid,
		    uid_netid_entry->bind_stamp, strategy);
...
void add_dpi_rule(const char *data)
{
	...
	if (g_dpi_rule_next_index < 0 ||
	    g_dpi_rule_next_index >= DPI_RULE_MAX_SIZE)
		return;
		...
		for (i = 0; i < g_dpi_rule_next_index; i++) {
			...
			if (iph != NULL && iph->protocol == mask->tmgp_tp) {
				...
				if (skb_uid == mask->tmgp_uid) {
					sk->sk_hwdpi_mark = sk->sk_hwdpi_mark |
							    MR_TMGP_2;
					skb->mark = mask->tmgp_mark;
					return;
				}
