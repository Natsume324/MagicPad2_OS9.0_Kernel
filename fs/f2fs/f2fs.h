...
#ifdef CONFIG_F2FS_HYPER_PREALLOC
	unsigned int segment_count[4];		/* # of allocated segments */
...
struct f2fs_bigdata_info {
	...
	/* LFS = 0, SSR = 1 */
#ifdef CONFIG_F2FS_HYPER_PREALLOC
	unsigned int node_alloc_cnt[4], data_alloc_cnt[4], data_ipu_cnt;
#else
...
int __curseg_set_prealloc(struct f2fs_sb_info *sbi, int type,
		     unsigned int segno, struct curseg_info *backupseg);
void __curseg_restore(struct f2fs_sb_info *sbi, int type,
				struct curseg_info *backupseg);
unsigned int f2fs_find_consecutive_secs(struct f2fs_sb_info *sbi, int secnum);
void __init_curseg_prealloc(struct f2fs_sb_info *sbi, struct curseg_info *curseg);
#define	__init_curseg_if_prealloc(sbi, curseg, type)	((curseg)->alloc_type != PREALLOC ? 0 : \
						({__init_curseg_prealloc(sbi, curseg, type);1;}))
